 ...
 * 
 * 2021-05-04 - Lilith
 * - Modified SS_GRACE add and drop amounts because Warriors
 *   were going negative. This is cosmetic, not balance-related.
 * - Daggers drop out of opponents after 30 secs using
 *   stuck_dagger_sh.c  This allows SU to reclaim daggers
 *   during battle and it prevents enemies from being encumbered
 *   by the daggers.
 * - Changed the time for lose_knife() to trigger pick up of
 *   misses from 1 to 2 seconds to 3 to 6 seconds.  Latency is
 *   preventing SU from getting a chance to pick up their knives
 *   even with triggers set.
 *
 * 2021-06-09 - Zizuph
 * - reset UNION_FLINGING property on knife when ability is cancelled
 *  (such as when spellcasting).
 *
 * 2021-08-01 - Lilith
 * - fixed query_ability_time to return min values (which is what they
 *   get by default at max skill).  This prevents novices from getting
 *   extra damage per special due to their slower fling.
 * 2021-09-27 - Lilith
 * - reset UNION_FLINGING property on knife when ability is
 *   cancelled due to movement.
 *   Remove shield restriction.  The loss of two handed benefits to
 *   the rate of special is the standard penalty for other guilds.
 *
 */
#pragma no_clone
#pragma no_shadow
...
{
    // Novices should not have a longer prep time
    // boost to their attack - return sup guru 
    // rate instead when calling query_ability_time
    // to compute damage.
    if (calling_function(0) == "query_ability_time")
        return MIN_FLING_PREP;
...
{
    ::hook_stopped_ability(ability, player);
   if (objectp(knives[QRN(player)]))
   {
       knives[QRN(player)]->remove_prop(UNION_FLINGING);
       knives[QRN(player)] = 0;
   }
}
		...
		mixed *items,  int result)
{
    ...
    clone_object(STUCK_DAGGER_SH)->initiate_shadow(knife, hdesc);
...
hook_changed_env()
{
   write("Your sudden movement interrupted your stance, causing you to "
       + "forfeit your " + short() + ".\n");
   if (objectp(knives[QRN(this_player())]))
   {
       knives[QRN(this_player())]->remove_prop(UNION_FLINGING);
       knives[QRN(this_player())] = 0;
   }
} /* hook_changed_env */
