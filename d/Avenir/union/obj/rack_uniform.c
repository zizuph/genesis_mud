 ...
 *   Cirion 033096
 *
 *  Rack specifically for Union uniform items.
 *
 */
#pragma strict_types
...
create_container(void)
{
    ...
    set_name(({"rack", "_union_uniform_rack"}));
	add_adj(({"uniform", "clothes"}));
    set_short("tall clothes rack");
    set_long("It is a tall rack spanning the west wall. There "+
      "are many pegs for hanging the distinctive uniform of the "+
      "Shadow Warriors on.@@within@@\n");
...
prevent_enter(object ob)
{
	...
	if (!ob->adjectiv_id("union"))
    {
	    TP->catch_msg("The "+ ob->short() + " is not part of your "+
           "uniform and should not be hung on this rack.\n");
	    return 1;
    }
    ...
    {
	if (this_interactive())
	    write("Do not pollute the purity of your uniforms with " +
    	dirty + " items.\n");
	return 1;
    }
    ...
    {
	if (this_interactive())
	    write("Placing food in with your uniforms is a bad idea.\n");
	return 1;
    }
...
prevent_leave(object ob)
{
	...
	say(QTNAME(TP)+" starts to reach for something in "+
	  "the uniform rack, then "+HE(TP)+" jerks back "+
...
open(void)
{
   write("The uniform rack is always open by design.\n");
   return 1;   
}
...
start_sort(string arg)
{
    ...
    if (arg != "rack")
    {
	notify_fail("Sort what? The uniform rack?\n");
	return 0;
    }

    if (alarm)
    {
	write("The uniform rack is already being sorted.\n");
	return 1;
    }
    ...
    write("You start sorting the uniform rack.\n");
    say(QCTNAME(this_player()) +" starts sorting the uniform rack.\n");
