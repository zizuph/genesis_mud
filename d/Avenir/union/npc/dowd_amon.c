// file name:     /d/Avenir/union/npc/dowd_amon.c
...
create_monster()
{
    ...
    add_prop("_i_am_npc_falcata_wielder", 1);
    ...
    add_ask(({ "plantilsi amon", "plantilsi amon dowd"}),
      "say With those words I have been Called fully from the Shadows, "+
      "and with me all the glory of the Flame stands at the Gate.", 1); 
    add_ask(({"follow", "follow me", "team", "to team"}), 
      "say Alas, my days of following are over.", 1);	  
    add_ask(({ "return", "return home", "leave", "leave me", "go" }), 
      VBFC_ME("do_leave"), 1);
...
{
    command("sway");
    set_alarm(1200.0, 0.0, leave); // Leaves in 20 mins.

	if (IS_MEMBER(TP) || FRIEND(TP))
	{
	    command("respect "+ TP->query_name());
