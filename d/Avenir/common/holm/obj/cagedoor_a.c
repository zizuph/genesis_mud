...
create_door()
{
    ...
    set_pass_command(({"go through","cage","enter", "south"}));         
    ...
    set_door_name(({"iron grill","cage door", "cell door", "grill",
        "grille", "door"}));
    set_door_desc("A flat iron grill that swings into a cage. "+
        "There is a lock on it.\n");
    set_other_room(T_CAMP + "cage1");
    set_open(0);
    set_locked(1);
    ...
    set_lock_desc("It is a small lock made of iron. It looks to be "+
        "rather simple.\n");
