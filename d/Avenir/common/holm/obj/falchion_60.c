                ...
                int form, int phit, int dam)
{
    ...
    sprintf("AID %d PHURT %d element %d form %d phit %d dam %d\n",
       aid, phurt, element, form, phit, dam));
    if (enemy)
    {
       query_wielded()->catch_msg("Enemy " + QTNAME(enemy) + ".\n");
    }
    return ::did_spell_hit(aid, phurt, enemy, element, form, phit, dam);
}
