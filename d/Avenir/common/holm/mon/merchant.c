...
// last update: Jan 18, 1995 Cirion, 051396 - Fixed some typos.
//              97 April 06, Lilith: adapted for use in the Holm
// purpose:     mobile shop for Avenir's Holm
...
create_monster() 
{
    ...
    add_name("merchant");
    set_race_name("gnome");
    set_adj(({"crafty", "smiling"}));
    add_adj(({"queer", "little"}));
    set_title("ini Biruel, Merchant Extraordinary");
    set_short("crafty smiling gnome");
    set_long("@@my_long@@");
    set_gender(G_MALE);
    ...
    set_random_move(100, 0);
    set_restrain_path(({ HOLM, PK }));
    set_monster_home( HOLM +"lf5");

    set_chat_time (6);
      add_chat("ItIsHardToMakeALivingOnTheHolm.NoRegularCustomers."+
          "TheyAllDieBeforeLong.");
      ...
      add_chat("NitikasCommandedMeHere.");
      add_chat("HaveYouEverSeenASliverOfMulticoloredStone?");
      add_chat("ICanHearTheScreamingFromInsideTheSpire.");
      add_chat("TheMasterOfTheHolmIsACruelBeingWhoServesEvenCruel"+
          "erGods.");
      add_chat("AllKindsOfFoulFolkWanderAroundHere.BeCareful.");
      add_chat("ItTookMeMonthsToTrainMyTroloby.");
      add_chat("IHaveACousinWorkingInTheUtterdark.HeIsABraverMan"+
          "ThanI.");
      add_chat("IAssumeThatIfYouAreHereYouKnowTheRule.");
      add_chat("TheGodsPlayHere.");
    ...
    add_ask(({"stone", "sliver of stone", "jasper", "orbicular jasper",
        "multicoloured stone"}), VBFC_ME("stone_ask"));
...
my_long()
{
    ...
    if (TP->query_met(query_real_name()))
    {
     ...
     str += "A queer little man who is both unusually quiet and con"+
         "genial.\nHis hair is long and held in place by a braided "+
         "leather thong.\nHis dark eyes are constantly scanning "+
         "the area, as though he is expecting something.\n";
    return str;
}
...
hunt_ask()
{
    if (IS_SYBARITE(TP))
    {
        command("say to "+ TP->query_real_name() +
            " Are you on a Hunt?");
        command("say to "+ TP->query_real_name() +
            " I'd appreciate it if you passed me by. I'm trying to "+
            "earn a living here.");
        command("grin");
        return "";
    }
...
rule_ask()
{
    ...
    command("say to "+ TP->query_real_name() +
        " If you're looking for prey, I'd appreciate it if you "+
        "passed me by. I'm trying to earn a living here, not get "+
        "myself killed.");
    return "";
}
...
stone_ask()
{
    command("say I'mLookingForPiecesOfARose-ColouredOrbicular"+
        "JasperStone.IDon'tKnowHowManyThereAre.");
    command("say MySearchIsHopelessIThink,ButWhenAGodCommands,IObey.");
    return "";
}
...
shop_hook_list_empty_store(string str)
{
    ...
    tell_room(ENV(to), "The gnome merchant checks through the packs on " 
        + QTNAME(pony) + " but comes up emptyhanded.\n");
