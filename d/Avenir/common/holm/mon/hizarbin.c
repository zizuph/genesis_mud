//-*-C++-*-
// file name:    /d/Avenir/common/holm/mon/hizarbin.c
// creator(s):   Lilith 3 May 1997
// revised:      Lilith Sep 1997:  Changes to prevent runtime errors.
// purpose:      Hizarbin incarnation looking for his daughter's shawl
//               (*/holm/obj/sea_shawl).
// note:         He will reclaim it by whatever means necessary, not
//               excluding killing.
...
create_monster()
{
    ...
    set_name("omatu Hizarbin");
    set_living_name("hizarbin");
    set_title("the Lord of the Waters");
    add_name(({"god", "lord", "hizarbin", "omatu", "spirit being",
        "avatar"}));
    set_adj("furious");
    set_race_name("god");
    set_gender(G_MALE);
    ...
    set_m_in("flows in");
    set_m_out("flows");
    set_mm_in("takes shape from a rope of shimmering water.");
    set_mm_out("becomes a pulsing ball of water that suddenly "+
        "explodes, showering you with a hundred tiny droplets.");
    ...
    add_prop(OBJ_M_NO_MAGIC_ATTACK, "Omatu Hizarbin whispers to you: "+
        "Your pathetic magicks will not work against Me, fool!\n");
...
my_long()
{
    ...
    if (TP->query_met(query_real_name()))
    {
     ...
     str += "He appears before you in a body shaped from water "+
            "in constant, flowing motion. Inner light reflects "+
            "off his watery form, dazzling your eyes.\nHe appears "+
            "to be very angry.\n";
    return str;
}
