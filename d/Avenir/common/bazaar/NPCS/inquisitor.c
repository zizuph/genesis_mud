...
// last update:   
//                Lilith, Sept 2014: added alt solutions for Maivia quests.
// bug(s):
// to-do:
...
create_monster()
{
    ...
    /* For statue defile quest */
	add_ask(({"[about] [the] [defiled] 'statue' "}),
			"say Another inquisitor is working on that case.", 1);
...
check_meet(object tp)
{
    ...
    if (!present(tp, environment(TO)))
	 ...
	 if(tp->query_prop("_is_cat_"))
	 {
		if(i<2)
		{
			command("say Somone take this flea-bag to the butchers!");
			return;
		}
		if(i<4)
		{
			command("shouts: Out, cat!");
			return;
		}
      return;
	 }
...
/* This is quest code for the Maivia quests in the Utterdark.
 * Players can solve it as he asks, which is anti-Sybarus
 * Or they can turn the items in to the authorities, a 
 * Pro-Sybarus solution.
 */
