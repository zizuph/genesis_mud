// -*-C++-*-
// file name: 	  /d/Avenir/common/bazaar/Obj/flower/buttoniere.c
// creator(s):	  Lilith July 1997
// last update:	  Zielia July 2009 converted from armour to wearable ob
// purpose:       This is wearable men's flower from the flower_stall.
// note:      
// bug(s):
// to-do:
...
long_desc()
{
    ...
    str = "This boutonniere is made with a "+ colour +" "+ type +
          " flower, along with a small spray of baby's breath. "+
          flower_desc[type][2] +"\n   This "+
          "compact flower arrangment is made to be worn pinned to "+
          "a man's shirt or coat, or inserted into a button-hole.";
...
short_desc()
{
    return colour + " "+ (flower_desc[type][0]) +" boutonniere";    
}

string
pshort_desc()
{
    return colour + " "+ (flower_desc[type][0]) +" boutonnieres";    
}
...
create_flower()
{
    ...
    if (strlen(message))
    {
        ...
        add_cmd_item(({ "card", "message", "boutonniere", type +" card", 
    ...
    add_cmd_item(({ type, "flower", "flowers", "boutonniere", 
        (flower_desc[type][1]) }),
