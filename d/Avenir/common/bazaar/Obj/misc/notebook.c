...
{
  if (num_of_pages == 0)
    return "All its pages have been torn out";
...
static int start_id(string str)
{
  string str2;
...
note = clone_object(BAZAAR + "Obj/misc/note");

  if (note == 0 || note->move(this_player())) {
    write("The notebook whispers: " +
          "Sorry, but something does not permit me to give you the note.\n");
    if (note != 0)
      note->remove_object();
  } else {
