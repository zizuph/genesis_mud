// -*-C++-*-
// file name: 	  /d/Avenir/common/bazaar/Obj/forge/bracers.c
// creator(s):	  Lilith 
// last update:	  
// purpose:       
// note:          
...
long_desc()
{
    ...
    if (plating && plating != 0)
        str = "Spiralling up each bracer, from shoulder to wrist, "+
              "are elegant "+ plating +"-plated knotworks of "+
              "dazzling complexity.\n";
    else
        str = "The steel has been "+ decor +" in "+ colour +", "+
              "a simple, yet elegant, embellishment that should "+
    ...
    return  "This "+ short() +", unsurpassed in beauty and function"+
            ...
            "bracers are lined with "+ colour +" "+ lining +" and "+
            "moulded to fit the arms of "+ LANG_ADDART(query_owner()) +
            " perfectly. "+ str;
}
...
{
    if (plating && plating != 0)
        return plating +"-plated pair of "+ astyle +" bracers";
    else    
        return "pair of "+ colour +" "+ decor +" "+ astyle +" bracers";
...
{
    if (plating && plating != 0)
        return plating +"-plated pairs of "+ astyle +" bracers";
    else    
        return "pairs of "+ colour +" "+ decor +" "+ astyle +" bracers";
}
