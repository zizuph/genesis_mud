// Museum writing pad
// For people to practice their poetry composing on.
// Lilith Nov 2021
...
#define WRITING		(BAZAAR +"museum/pad_writing")

public int num_pages = 10;
...
tablet_long(void)
{
    ...
    return "It is a tablet of soft white vellum pages, perfectly square.\n"+
	tablet_writing();
}
...
write_message(string str)
{
    ...
    if (!parse_command(str, TP, "[on] [the] 'tablet' / 'pad' / 'vellum'"))
	return 0;
...
tear_page(string str)
{
    ...
    paper->set_adj(({ "soft", "white", "vellum", "writing"}));
    paper->add_prop(OBJ_I_WEIGHT, 1);
    paper->add_prop(OBJ_I_VOLUME, 1);
