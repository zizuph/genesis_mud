/*  /d/Avenir/common/bazaar/intr/sigilist.c
...
#include "/d/Avenir/common/outpost/room/paint_gon.h"
...
read_sign()
{
    ...
    "  Available colours are the High Lords' colors: green, "+
    "  red and purple.\n");
}
...
create_room()
{
    set_short("In a busy shop at the Fort of Sybarus");
    set_long("This is a shop is where the dwarven guards go to get "+
        "the paint on their weapons and armours updated to keep them "+
        "looking pristine. While the proprietor appears to been quite "+
		"busy, she might have time to do a small job for you. "+
        " A sign has been pinned to the wall.\n");
    ...
    add_item(({"stone table", "sturdy table", "quartzite table",
        ...
        "and strong.\n");
    add_item(({"stains", "paint stains"}), "Red, geen, and purple "+
        "paint is splattered all over the surface of the work "+
        "table.\n");
...
*/
    ...
    if (!gPainter->allow_paint(this_player()->query_real_name(), paint))
    {
        gPainter->command("whisper to " + OB_NAME(this_player()) +
            " You haven't given me the right additives for " +
            paint + " paint yet. You need to " +
            "collect the right ingredients for me.");
        return 1;
    }
