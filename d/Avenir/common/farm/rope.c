...
short(object for_obj = this_object())
{
    string str = "";
 ...
 */
public varargs string
plural_short(object for_obj = this_player())
{
    string str = "";
 ...
 */
public varargs string
long(object for_obj = this_player())
{
    string str = "";
...
rope_access(object ob)
{
    if ((environment(ob) == this_player() ||
         environment(ob) == environment(this_player())) &&
                (function_exists("create_object", ob) == BOAT_ROPE))
        return 1;
    else
        return 0;
}
...
tie_it(string str)
{
    ...
    if (ob[0]->tie_object(this_object(), str))
    {
        ...
        if (item)
            ...
            set_tied_to_text(ob[0]->query_short());

    say(QCTNAME(this_player()) + " ties " + (this_object()->query_short()) +
            " to the " + tied_to_text + ".\n");
        write("Ok.\n");
...
untie_it(string str)
{
    if (!tied_to)
    {
        notify_fail("But the " + short(this_object()) + " is not tied " +
                "to anything,\n");
        return 0;
    }
