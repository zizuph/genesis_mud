...
create_creature()
{
        ...
        set_long("This is a horse-like creature that seems to bear "
                ...
                +"protruding from it's head. Its coat is shiny black, "
        ...
        set_stats(({110,120,105,70,70,114}));
        set_hp(query_max_hp());
...
int special_attack(object enemy)
{
   ...
   if(random(5))
    ...
    if(enemy->query_not_attack_me())
     return 1;

   /* miss */
   if(query_combat_object()->cb_tohit(A_HORN, 40, enemy) <= 0)
   {
   ...
   hitres = enemy->hit_me(45, W_IMPALE, TO, A_HORN);
   ...
   enemy->catch_msg(QCTNAME(TO) + " charges at you with its long black horn and "
     + str + " your " + hitloc + "!\n");
   tell_watcher(QCTNAME(TO) + " charges at " + QTNAME(enemy)
     + " with its long black horn and "
