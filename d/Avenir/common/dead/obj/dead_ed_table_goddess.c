 ...
 *   Lilith June 2021
 *
 * updates: Lilith Sept 2021: removed rose and kesoit mask so as not
 *                            to hold up the opening of the catacombs.
 */
...
reset_container(void)
{
    ...
    if (!objectp(vase))
    {
		...
		/* Probably not going to be approved by AoB I hear, so removed.
        if (!objectp(rose))
	    {	
    ...
    // kesoit death mask -- may hold up opening catacombs due to need for
	// balance review, so removed.
	
    if (!objectp(kmask))
	{	
    //    kmask = clone_object(OBJ +"kesoit_mask");
	    kmask = clone_object(OBJ +"death_mask");
		kmask->move(TO,1);
	}
...
prevent_leave(object ob)
{
    ...
    {
	    logrotate(MASQ_GET_LOG, ctime(time()) +": " +
           TP->query_real_name() +" took "+ob->short() +".\n");
	    return 0;
    }
	...
	logrotate(MASQ_GET_LOG, ctime(time()) +": " +
	    TP->query_real_name() +" tried to take "+
