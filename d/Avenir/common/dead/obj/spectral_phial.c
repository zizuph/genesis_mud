...
create_object()
{
    set_name("_Avenir_spectral_phial");
    add_name(({"glass", "phial"}));
    set_adj(({"transparent","greenish"}));
    set_short("phial of greenish something");
    set_long("It is a small phial of what might be glass. " +
             "Contained within it is a glowing greenish "+
			 "energy.\n");
...
its_cold()
{
	// Spectral energy is bitterly, burning cold.
	
    string *cmds = ({ "shiver", "sniff cold", "sneeze"});
    ...
    for (i = 0; i<sizeof(people); i++)
       if (random(400) > people[i]->query_stat(SS_CON))
           people[i]->command("$" + cmds[random(sizeof(cmds))]);
}


void
energy_effect(object who)
{
    if(!objectp(who))
        return;
    ...
    if (who->query_prop(LIVE_I_CONCENTRATE))
    {
        ...
        who->catch_tell("The spectral energy released nearby seems "+
		    "to be slowing you down.\n");

        if(who->query_stat(SS_DIS) > 70 + random(400))
        {
        ...
        /* break his spell */
        who->cmdhooks_break_spell("You feel so cold you can't concentrate.\n");
    }
}
