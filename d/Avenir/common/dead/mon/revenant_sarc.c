// file name: /d/Avenir/common/dead/mon/revenant_sarc
// creator(s): Lilith Sept 2021
// purpose:    To guard the tombs of the high lords.
// note:       Stat av 100 - 160+random35. Cloned to sarcophagus rooms.
//             Other revenants are wandering off leaving the sarcophagi
//             unguarded, so make these no-wandering
// update(s):  Lilith Feb 2022: Reduced native armour. Got feedback
//                from Cherek and Zizuph that they are too tough for 
//                the amount of exp generated at kill. Also removed magic res
//                since the combat system calcs it based off AC now.
//             Lilith May 2022: fixed indentation, cleaned up.
...
setup_random_mob()
{
    ...
    base_stat = 80 + (mob_type * 30) + random(25);
...
my_long()
{
	...
	+"There is a coin covering each of its eyes.\n"
	+"";
}
void
create_monster()
{
   ...
   // fights unarmed so lets give skills based on mob size.
    ...
    set_skill(SS_DEFENCE,      50 + (15*Strength)); // reduce melee damage 	
    set_skill(SS_ACROBAT,      50 + (15*Strength)); // reduce melee damage
    set_skill(SS_UNARM_COMBAT, 50 + (15*Strength));	 
    set_skill(SS_BLIND_COMBAT, 50 + (15*Strength)); 
    set_skill(SS_SPELLCRAFT,   50 + (15*Strength)); // reduce caster damage
    set_skill(SS_FORM_ABJURATION,50 + (15*Strength)); // reduce caster damage
 	
    // native armour and unarmed attacks are based on mob size.
    wep = 30 + (10*Strength);  // 30:40:50
...
special_attack(object victim)
{
    ...
    {
        // Handle misses.
        query_combat_object()->tell_watcher(QCTNAME(TO) + " tries to bite " + 
            QTNAME(victim) +" but misses completely.\n", victim);
    ...
    hdesc = hitresult[1];
    query_combat_object()->tell_watcher(QCTNAME(TO) + " lunges for "+ 
        QTNAME(victim) +" and sinks its teeth into "+ 
        victim->query_possessive() +" "+ hdesc +"\n", victim); 
    victim->catch_msg(QCTNAME(TO) + " lunges toward you, teeth "+
        "snapping, and bites you on the "+ hdesc +"!\n");
