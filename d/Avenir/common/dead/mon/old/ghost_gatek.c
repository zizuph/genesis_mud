...
arm_me()
{
   ...
   key = clone_object("/d/Avenir/common/dead/obj/cryptmain_key");
   key->move(TO);
...
create_creature()
{
  set_name(({"ghost","apparition","_Dead_Isle_Ghost"}));
  add_name("ghost_gatekeeper_");
  set_race_name("ghost");
  set_gender(G_NEUTER);

  set_long("This creature is shadowy and translucent.\n"
