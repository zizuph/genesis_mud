//           ~Avenir/common/dead/mon/merchant_porter.c taken from:
// file name:   ~Avenir/common/dark/mon/pony.c
// creator(s):   Boriska
// last update:  Lilith Aug 2021
// purpose:      Merchant's porter
// bug(s):
// to-do:     
...
#define PORTER       MON + "merchant_porter"
...
void create_creature()
{
    ...
    set_name("primo");
    add_name(({"porter", "troloby"}));
    set_short("huge docile troloby");
    set_adj(({"huge", "docile"}));
    ...
    set_stats (({ 180, 180, 180, 90, 50, 100 }));
    ...
    set_alarm(0.1, 0.0, &set_m_in("arrives shouldering a new pack "
	    +"and dripping water."));
    set_alarm(0.1, 0.0, &set_m_out("trots away toward the raft "
	    +"anchored offshore."));
}
...
private void empty_store()
{
    ...
    if(sizeof(inv) > 0 && !query_attack() && !query_no_show())
    {
        ...
        tell_room(ENV(TO), QCTNAME(merchant)+" whispers something to "+
           QTNAME(TO)+".\n" +QCTNAME(TO)+" trots off toward the "+
           "raft anchored just offshore.\n");
...
private void return_pony()
{
    ...
    tell_room(ENV(TO), QCTNAME(TO) +" arrives with a new pack and "
	    +"dripping water.\n");
}
...
void do_die(object killer )
{
    tell_room(ENV(TO), "In a final, desperate act, "+ QTNAME(TO)+ 
        " tosses its pack into the sea and then falls dead "+
        "at your feet!\n");
    merchant->pony_died();
...
private void run_away()
{
    ...
    tell_room(ENV(TO), QTNAME(TO) +" senses his freedom and trots "+
        "off toward the raft floating just offshore.\n");
