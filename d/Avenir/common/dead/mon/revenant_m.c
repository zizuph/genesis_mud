...
// note:       Stat av 190 +random35. Limit 7 cloned in the 150 rooms.
// update(s):  Lilith Feb 2022: Reduced native armour. Got feedback
//                from Cherek and Zizuph that they are too tough for 
//                the amount of exp generated at kill. Also removed magic
//                res since the combat system calcs it based on AC now.
//             Lilith May 2022: fixed indents, cleaned up.
...
equip_me()
{
    ...
    //It can do more damage with unarmed attacks.

    if (present("sarcophagus"), environment(TO))
    {
	command("close sarcophagus");
    }
...
my_long()
{
	return "This "+ short_desc +" was animated from one of the "
      ...
      +"It seems to be about myth in size, "
      +"but it is hard to be sure of anything where the undead "
      +"are involved.\n"
      +"It appears to be provoked by your presence here.\n"
...
public string cr_attack_desc(int id)
{
    if (id == W_LEFT)
    {
	return "rotten left fist";
    }
    if (id == W_RIGHT)
    {
	return "slimy right fist";
    }
...
special_attack(object victim)
{
    ...
    hdesc = hitresult[1];
    query_combat_object()->tell_watcher(QCTNAME(TO) + 
        " lunges for " + QTNAME(victim) 
      + " and sinks its teeth into "+victim->query_possessive() 
      +" "+ hdesc +"\n", victim); 
    victim->catch_msg(QCTNAME(TO) + " lunges toward you, teeth snapping, "
      +"and bites you on the "+ hdesc +"!\n");
...
do_die(object killer)
{
    ...
    /* Clone the crystalline skull (red herring) */
    if (!present("_av_crystal_skull"))
	clone_object(OBJ +"crystalline_skull")->move(TO, 1);
    /* Coin for each eye, PC based on npc myth size */
    ...
    STATSERV_LOG_EVENT("crypt", "myth revs killed");
    /* Logging death for various reasons */
    SCROLLING_LOG(DEATH_LOG, "RevM ("+ TO->query_average_stat() +") by "+
