...
create_crypt_room()
{
    ...
    add_exit(CRYPT+"crypt114","down", 0, 10, 1);
	add_exit(CRYPT+"crypt112","up", 0, 20, 1);
	add_exit(CRYPT+"crypt112","climb", 0, 20, 1);

    set_short("wall of the pit of despair");
    set_long("You are clinging to the slick wall "
	    +"of a very deep pit somewhere in the catacombs of Sybarus. "
		+"The stench of death wafts up from below, making you gag. "
		+"Up ahead the the darkness is not as deep as the "
        +"miasma of despair emanating below you."		
		+"\n");
