...
//              Lilith, Jan 2022: added "enter" add_cmd_item to rooms.
...
void crypt_long(string desc)
{
	...
	"niche", "niches"}), 
      ...
      +"of decay. \n");
	add_cmd_item(({"hollow", "hollows", "crypt", "crypts", "shelf",
	"shelves", "recess", "recesses", "tiers", "gallery",
	"niche", "niches"}), "enter", "There's no room for you there.\n");
    ...
    add_item(({"mist","fog"}),"A thin white mist moves along "
      ...
      +TO->query_exit_cmds()[random(sizeof(TO->query_obvious_exits()))]
      +".\n");
    ...
    add_item(({"bones", "bone"}), "There are bones everywhere, "
	...
	add_item(({"marks", "clawmarks", "claw marks", "grooves" }),
	  "Long, deep grooves have been scraped into the walls. They look "
	  +"like claw marks made by a preternaturally large beast with "
	  +"paws wider than your hand and 6 or 7 individual claws.\n");
    add_cmd_item(({"bone", "bones"}), ({"touch", "feel", "push"}),
	   "You hear a faint clicking sound and feel a puff of air from "
	   +"the "+TO->query_exit_cmds()[random(sizeof(TO->query_obvious_exits()))]
	   +".\n");
	...
	add_tell("A small muscular goblin sprints past you, screaming in "
	   +"terror.\n");
	add_tell("In the periphery of your vision you catch sight of something "
	   +"moving.\n");
	add_tell("The darkness around you seems to exhale. You feel a breath on "
	   +"your skin.\n");
	add_tell("The quietude is interrupted by a snarling growl not unlike the "
	   +"roar of a very large cat!\n");
	add_tell("You feel the crunch of bones underfoot and hear a loud rumbling "
	   +"noise.\n");
	add_tell("A blade slashes toward you from out of the darkness.\n");
	add_tell("There is a sudden scuffle and squeaking sounds.\n"); 
	add_tell("A finger of mist stabs toward your eyes.\n");
	add_tell("The strong scent of fungi permeates the gloom.\n");
	
	IN;        // underground
    IN_IN;     // indoors -- no flux no steeds
    set_terrain(TERRAIN_SHADOWS | TERRAIN_HASDEAD | TERRAIN_UNDERGROUND | TERRAIN_ROCK);
...
{
    object bones;
    object skeleton = present("_crypt_crawler", TO);
    ...
    // any mob with the name "_crypt_crawler" will block 
    if(skeleton)
    {
	if (random(5) == 0) // 1 in 5 chance of being blocked by mob
	{	
    ...
    // 1 in 10 chance of player tripping on bones and missing exit.
    if (random(10) == 0)  
    {                    
	seteuid(getuid()); 
	write("Grr! You tripped on some bones as you tried to "
	  +"move "+ exit +".\n");
	say(QCTNAME(TP) +" tripped over some bones while trying "
	  +"to move "+ exit +".\n");
	return 1; // block the movement even if there is no skeleton.
    }		

    // 1 in 5 chance of exit making 1 skeleton this reset.
    if ((random(5) == 0) && !num_skeletons)  
    {
	seteuid(getuid()); 
