...
// 
//  * Lucius May 2009: Made corrections to fix bad porting process.
//
...
answer_log(void)
{
    delay_action(1.0, ({"shrug .","say I remember the "
		+"captain kept a logbook, but it probably went down "
...
equip_me(void)
{
    clone_object(RAPIER)->move(this_object(),1);
...
where_to(void)
{
	command("say The Melancholy sails between the Rift of Sybarus "+
			"and Sparkle township.");
	return "";
}
...
sad_ask(void)
{
	command("say Aye, twas misery indeed. Captain Branston was a great "+
        "man. I pity the widow most of all, though.");
    return "";
}
public void
create_captain(void)
{
    ...
    set_title("icu Lochnir, Captain of the Melancholy");
    set_syb_house("Lochnir");
    set_name("staek");
    add_name("captain");
    ...
    set_long("He would be a handsome man if he wasn't so very slender.\n"+
    ...
    add_chat("They say there is an ancient machine of stone "+
	"somewhere on the Isle of the Dead that carries deep "+
	"unexplored secrets, but the mist there covers it from " +
    ...
    add_ask(({"'where' [are] [you] [going] / [sailing] [to]"}), where_to);
	add_ask(({"[what] [is] [your] / [the] 'destination'"}), where_to);
    add_ask(({"[what] [is] [your] / [the] 'route'"}), where_to);
	add_ask(({"[why] [are] [you] [so] 'unhappy' / 'sad' / 'miserable'"}), sad_ask);
	add_ask(({"[your] 'tragedy'/'sadness' / 'grief' / 'misery'"}), sad_ask);
    add_ask(({"[the] [ss] 'concubine' / 'wreck' / 'shipwreck'"}), answer_ship);
    add_ask(({"[the] 'captain' / 'braston'"}), answer_captain);
    add_ask(({"[the] 'wife' / 'widow' / 'maya' [of] [captain] [braston]"}), answer_widow);
    add_ask(({"[the] 'book' / 'log' / 'logbook'"}), answer_log);
	add_ask(({"[the] 'necronisi'"}),
		"say The Necronisi is where Sybarites are laid to rest. If they've pleased "+
		"the Gods, they are permitted rebirth. If not... well, you will see.",1);
	add_ask(({"'infidel' / 'infidels' [hideout]"}), "say I heard there"+
		"is a place in the Bazaar where some infidels hide.", 1);
    add_ask(({"[where] / [who] [is] 'nochipa'"}), "say She's supposedly the "+
		"leader of those infidels woh are hiding in the Bazaar somewhere.", 1);
    add_ask(({"[where] [is] [the] 'bazaar' [island]"}), "say Follow the path "+
		"down to the Fort. Knock and the guards will let you pass. Then find "+
			"the shoreline and hit the pipe to summon the boat.", 1);
	...
	"knighthood"}), knight_ask);
    add_ask(({"god", "gods", "Gods", "Will of the Gods", "will" }), gods_ask);
...
what_me(void)
{
	...
	break;
    case 1:
	command("say I don't know. I stick to what I do know:"+
...
call_demon(void)
{
    ...
    if ((query_hp() < (query_max_hp() / 2)) && !objectp(demon))
    {
	tell_room(ETO, QCTPNAME(this_object()) +
	    " body arches grotesquely and "+
...
void enter_inv(object what, object from)
{
	...
	delay_action(1.0, ({"whistle","say This was Braston's ring!",
	    ...
	    "say His widow will probably want it as a keepsake.",
	    "give ring to "+ OB_NAME(from),
	    "drop "+ RING_ID}));
	return;
    }

    if (what->id(BONES_ID))
	set_alarm(1.0, 0.0, &command("say Whose are these? They "+
		"deserve a proper burial on the Necronisi, I'd think."));

    set_alarm(1.5, 0.0, &give_back(what, from));
}
