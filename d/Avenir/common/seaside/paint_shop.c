...
inherit "/d/Avenir/inherit/room.c";
...
read_sign()
{
    cat("/d/Avenir/common/seaside/help/paint.txt");
	return "";
}
...
create_domain_room()
{
    set_short("Good As New shop");
    set_long("Very well-lighted with lots of lamps, this place of "+
	    "business is very colourful. There is a very long table running "+
        "down the center with splotches and smears of paint staining "+
        "the surface. "+ 
    ...
    add_item(({"wall", "walls", "lime plaster", "plaster"}), 
        ...
        "giving them a faintly greyish tint.\n");
...
start_paint(string str)
{
    ...
    if (!sizeof(paint_me))
    {
        notify_fail("Paint <what> item <which> colour?\n");
        return 0;
    }
    ...
    if (sizeof(arr) > 0)
    {
        ...
        {
            mon = ((paint_me[0]->query_prop(OBJ_I_VOLUME) * 22) / 7);
            if (mon < 864) mon = 864;
...
inquire(string str)
{
    ...
    while(++i < sz)
    {
        ...
        {
            mon = ((arm[i]->query_prop(OBJ_I_VOLUME) * 22) / 7);
            if (mon < 144) mon = 864;
            if (mon > 175000) mon = 17500;
