 ...
 * syb_coin.c
 *
 * This is a reward for doing quests and tasks. 
 * It can be found on occasion on various npcs.
 * It can be used to gain access to places like 
 * the holm, the bathhouse, and the village.
 *
 * Revised:
 *   Lucius, May 2005: added set_short() and masked singular_short
 *                     to work properly.
 *   Lucius, Mar 2007: updated to reflect changes in /std/coins.c
 *   Lucius, Jun 2007: Removed no-steal prop, coins are prolific now.
 *                     Also made a couple slight changes to attempt
 *                     to solve the mysterious runtime errors.
 *   Lucius, Dec 2007: Changed WEIGHT/VOLUME to be more inline with standard.
 */
#pragma save_binary
#pragma strict_types
...
{
    set_name("coin");
    add_name("_sybarun_token");
    set_pname("coins");
    set_short("sybarun coin");
    set_pshort("sybarun coins");
    set_coin_type("sybarun");
}
...
create_heap()
{
    ...
    add_cmd_item( ({ "coin", "round", "wood", "token"}), ({ "sniff" }),
        "Despite the fact that this coin has probably changed hands "+
        "many times, it still retains the wonderful spicy scent of "+
        "kesoit wood.\n");
...
long()
{
	...
	(num_heap() >= 1000))
    {
	return "It is " + short() + ". It is made of a scented wooden "+
               "round that is filigreed at the edges. One side has "+
               "the symbol of Sybarus, the trefoil, in bas relief. "+
               "The other side reveals a stern-looking elf in his "+
               "middle years, probably the Hegemon. "+
               "This token has been granted as a reward for a deed "+
               "well done, and can be used to gain access to restricted "+
               "areas of Sybarus.\n";
    }
    else
    {
	return "There are " + short() + ". They are made of scented "+
               "wooden rounds that are filigreed at the edges. "+
               "One side has the symbol of Sybarus, the trefoil, in "+
               "bas relief. The other side reveals a stern-looking "+
               "elf in his middle years, probably the Hegemon.\n"+
               "These tokens are granted as rewards for deeds done "+
               "well, and can be used to gain access to restricted "+
               "areas of Sybarus.\n";
...
set_coin_type(string str)
{
    ...
    set_adj(coin_type);
    add_adj(({"wooden", "filigreed", "scented"}));
    add_name(({coin_type + " coin", "token"}));
    add_pname(coin_type);
