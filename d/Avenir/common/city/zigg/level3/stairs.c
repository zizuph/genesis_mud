...
public void
init_porter(object ob)
{
    ob->arm_me();
    ob->set_random_move(120);
    ob->set_restrain_path(ZIG_RL3);
    ob->set_monster_home(ZIG_RL3 + "stairs");
}
...
zig_room(void)
{
    ...
    set_short("Stairwell outside the Galleria");
    set_long("You've come upon a small landing in a stairwell outside the "+
    	"Galleria. On the west side of the landing, dim, flickering light spills "+
	"through an archway opening to the south. On the east side, "+
	"stairs climb south toward the next level and descend north, "+
    ...
    add_item(({"stair","stairwell","stairs","steps"}),
        ...
        "toward the north. The stairs seem just as dark going up toward the "+
        "south. Climbing either way would be precarious at best were it "+
    ...
    add_item(({"crevice","dust", "Source-crystal dust", "source-crystal dust",
    	...
    	"crevice is is just enough to guide your steps and keep you from "+
    ...
    add_exit("room2", "south",  0);
    add_exit(ZIG_RL2 + "stairs", "up", 0);
    add_exit(ZIG_RL4 + "stairs", "down", 0);
    
    add_prop(OBJ_S_SEARCH_FUN, "do_search");

    add_zigg();

    add_npc(COM + "mon/porter", 1, init_porter);
}
