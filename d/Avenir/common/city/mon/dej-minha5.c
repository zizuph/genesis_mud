//   The dej with the unarmed fighting aids/
// file name:      /d/Avenir/common/city/mon/dej-minha5.c
// creator(s):     Lilith Oct 2021
// last update:    Lilith Nov 2021 Tweaked skills and stats a bit.
// purpose:	       Copy of dej3, she is the one who does unarmed sparring.
//                 She has the crozier and the bruising boots which are 
//                 unarmed combat aids.
// note:           
#pragma strict_types
...
#define KILL_FILE	"/d/Avenir/log/deaths/dej-unarmed"
#define DEBUG   1
...
mixed *everyone = ({
    ({ "Merciless Feather", "flexible",
       "narrowed, intense grey", "red-brown",
       "First Maiden of the Rod", ({"merciless", "feather"}), }),

    ({ "Unfettered Pearl", "muscular", 
       "assessing, coal-black", "aquamarine",
       "Second Maiden of the Rod", ({"unfettered", "pearl"}), }),

    ({ "Incomparable Corset", "willowy", 
       "derisive, dark-green", "dark brown", 
       "First Maiden of the Lash", ({"incomparable", "corset"}), }),

    ({ "Dauntless Dove", "well-muscled", 
	   "hawkish, nearly black", "white-blonde", 
	   "Second Maiden of the Lash", 
       ({"dauntless", "dove", "dragon", "flower"}), }),
   
    ({ "Kesoit Splinter", "slender",
       "eager, reddish-brown", "glossy black",
       "Mistress of the Rod", ({"kesoit", "splinter"}), }),

    ({ "Infinite Cord", "graceful",
       "unsettling, pale-blue", "light-brown",
       "Mistress of the Lash", ({"infinite", "cord"}), }),
...
create_monster()
{
    ...
    set_act_time(2);
     ...
     add_act("emote makes an acrobatic leap, flips twice, and lands with "+
	     "one foot on the ground and the other kicking high into the air.");
     add_act("emote traces the tattoo on her chest with her fingertips.");
     add_act("emote tugs at one of her braids.");
     add_act("emote runs through a series of precise unarmed combat moves.");
  ...
  set_cact_time(1);
      ...
      add_cact("spar enemy");
      add_cact("emote ducks and rolls.");
      add_cact("emote demonstrates her impressive flexibility.");
      add_cact("emote nearly succeeds in knocking your feet out "+
        "from under you.");
...
equip_me()
{
    ...
    sw1 = clone_unique((CITY +"obj/wep/crozier"), 7, 
            (CITY +"obj/wep/crozier_nm"), 1, 33);
    sw1->move(this_object());
        
    clone_unique((CITY +"obj/worn/bruising_boots"), 10, 
            (CITY +"obj/worn/buskins"), 1, 33)->move(this_object());
...
reconfig_me(object ob)
{
    ...
    switch (s)
    {
        ...
        case 145..224:  t = (s / 4); break; /* 25% larger */
        case 225..500:  t = (s / 3); break; /* 33% larger */       
        default:        t = (s / 6); 
    }   
...
spar_me()
{
    ...
    if (DEBUG)
    {
        SCROLLING_LOG(KILL_FILE, "Dej5 challenged by: " 
...
attacked_by(object ob)
{
    ...
    if (!been_attacked)
    {
        ...
        set_alarm (1.5, 0.0, "command", "hold crozier");
