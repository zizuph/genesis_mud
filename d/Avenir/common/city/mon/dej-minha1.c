//-*-C++-*-
// file name:      /d/Avenir/common/city/mon/dej-minha1.c
// creator(s):     Lilith May 2002
// last update:    
// purpose:	       A member of the Hegemon's Honour Guard. 
//                 guards the First gate, outside.
...
mixed *everyone = ({
    ({ "Lace Garrotte", "delicate", "zealous, grayish black", 
       "golden", "First Maiden of the Pearl", 
       ({"lace", "garrotte"}), }),

    ({ "Scent of Lilies", "languid", "green-flecked blue", 
       "chocolate-brown", "Second Maiden of the Thorn", 
       ({"scent", "of", "lilies", "lily"}), }),

    ({ "Flail Song", "fine-boned",
       "somber, dark brown", "black silver-streaked", 
       "Mistress of the Lash", ({"flail", "song"}), }),
...
create_monster()
{
    ...
    default_config_npc(random(50) + 110);
    ...
    add_item(({"hair", "braid", "braids", "twin braids", "hairstyle", 
     ...
     add_act("emote flicks her glorious hair back over her "+
         "shoulder.");
     add_act("coat longsword");
     add_act("fill harness");   
     add_act("empty harness");
