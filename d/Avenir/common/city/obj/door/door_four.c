...
create_door()
{
    ...
    set_pass_command(({"nw", "northwest", "enter", "pass"}));
    set_fail_pass("The door to room four is closed.\n");
    set_door_name(({"door", "door four", "fourth door"}));
    set_door_desc("This is the door to Private Room Four.\n");
    set_other_room(BATH +"priv_4.c");
    set_open(0);
    set_locked(1);
    set_open_desc("The door to room four is open.\n");
    set_closed_desc("The door to room four is closed.\n");
    set_open_command(({"open","push"}));
    set_open_mess(({"opens the door to room four.\n",
      "The door opens.\n"}));
   set_fail_open(({"The door is already open.\n",
      "The door is locked.\n"}));
   set_close_command(({"close","shut"}));
   set_close_mess(({"closes the door to room four.\n",
       "The door closes.\n"}));
...
set_combo()
{
    ...
    "/d/Avenir/common/city/bath/bath.c"->set_four_key(i);
}
