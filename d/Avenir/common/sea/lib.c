/* Avenir's sea lib file is a copy of Khalakhor's:
...
query_sailing_difficulty_desc ()
{
  ...
  default:
    return "The waters here are nearly impossible to navigate.";
  }
}
...
try_row (string str)
{
  ...
  if (sizeof(row_exits_path[e]->query_boats()) >= MAX_BOATS) 
