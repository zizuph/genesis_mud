// water2           (/d/Avenir/common/port/water2.c)
...
do_climb(string str)
{
    ...
    else if (TP->query_skill(SS_CLIMB) < random(40))
    {
	write("You try to climb the ledge, but slide back into the water.\n");
	TP->add_fatigue(-(fat/2));
    }
    else if (!TP->move_living("climbing up on the ledge", PORT + "port13", 1))
	TP->add_fatigue(-fat);
...
create_room(void)
{
    ...
    set_long("You are swimming in dark, murky water. Powerful " +
      ...
      "the ledge you dived from towers above you, looking hard to climb " +
      "back on. You are starting to wonder if diving was such a good idea.\n");
    ...
    add_item(({"cliff","rock","ledge"}),
      "The ledge you dived from is right to the north. Going back there is " +
      "probably not a bad idea.\n");

    add_exit(PORT + "water1", "west");
    add_exit(PORT + "uw1a", "down");
}
