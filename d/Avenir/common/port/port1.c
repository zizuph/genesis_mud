// file name:        /d/Avenir/common/port/port1.c
// creator(s):       Ilyian
...
create_port_room()
{
    ...
    set_long("You are on a wide ledge to the east of a stormy " +
             ...
             "in any direction. To the east a narrow, precarious path " +
             "winds steeply down the face of the cliff, and to the north " +
             "you can hear the roar of a waterfall.\n");

    IN
    LIGHT

    add_exit("port7", "north");
    add_exit("port2", "south");
    add_exit(LEDGE + "ledge1", "east");
