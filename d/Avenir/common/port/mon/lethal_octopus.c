 // Octopus9.          (/d/Avenir/common/port/mon/giant_octopus9.c)
...
create_creature()
{
    ...
    set_short("grotesque octopus");
    set_adj(({"eight armed" }));
    set_long("This grotesque creature has found a new home here in the " +
        "storage room of the old shipwreck. The environment being " +
        "ideal, providing shelter for its enormous body. Though " +
        "vulnerable, it still looks frightening with its nine " +
        "tentacles and giant head. Dont make any mistake now. " +
        "He is going to eat you soon if you dont leave.\n");

    set_stats(({ 200, 200, 200, 90, 90, 200}));
...
special_attack(object enemy)
{
    ...
    robe = enemy->query_armour(A_ROBE);
    if (robe->id("_oilskin_cloak_"))
        enemy->catch_msg("The tenacles slip on your slippery cloak.\n");
    else
    if(!random(5))
    {
        ...
        {
            enemy->add_stun();
            enemy->catch_msg("The octopus slashes you with one of " +
                "his tentacles.\nYou feel stunned!\n");
            set_alarm(8.0, 0.0, &enemy->catch_msg("You can " +
                "move easier again!"));
            set_alarm(8.0 , 0.0, &enemy->remove_stun());
        }
    }
...
{
    remove_dark(TO);
    environment(TO)->test_props();  // Removes properties in players
    ::do_die(killer);
}
...
do_search()
{
   TP->catch_msg("The grotesque pale octopus stops you from " +
       "doing that.\n");
   return 1;
}
...
attacked_by( object ob )
{
   ...
   if (ob->query_prop("_is_entangled"))
        ob->catch_msg("Being entangled like this makes your moves very " +
            "slow.\n");
}
