...
create_creature(void)
{
	...
	set_act_time(13);
	add_act("emote moves with restless energy.");
    add_act("@@growls");
    add_act("@@hiss");
    add_act("@@flap");
    ...
    set_attack_unarmed(A_BITE,  30, 60, W_IMPALE,   20, "lion's jaws");
    set_attack_unarmed(A_LCLAW, 70, 30, W_SLASH,    30, "left claw");
    set_attack_unarmed(A_RCLAW, 70, 30, W_SLASH,    30, "right claw");
    set_attack_unarmed(A_TAIL,  20, 75, W_BLUDGEON, 30, "barbed tail");
    set_attack_unarmed(A_HORN,  40, 60, W_IMPALE,   20, "goat's horns");
    set_attack_unarmed(A_SBITE, 60, 40, W_IMPALE,   20, "serpent's fangs");

    set_hitloc_unarmed(H_HEAD, ({ 85 }), 20, "head");
    set_hitloc_unarmed(H_BODY, ({ 90 }), 80, "body");
	add_prop(OBJ_I_RES_MAGIC, 55); // magic res should be close to
...
do_die(object killer)
{
	...
	treasure->set_heap_size(3);
	treasure->move(TO);
    treasure = clone_object("/d/Genesis/gems/obj/tourmaline_pink");	
	treasure->set_heap_size(3);
	treasure->move(TO);
