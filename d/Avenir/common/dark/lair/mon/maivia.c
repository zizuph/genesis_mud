...
give_wangle_quest(object player)
{
    ...
    WRITE_THIS_LOG(QUEST_LOG, "Maivia gave Wangle quest to "+ player->query_real_name());
...
give_ogre_quest(object player)
{
    ...
    set_alarm(13.0, 0.0, &command("say Beware, too! That package must never "+
	    ...
	    "be uncovered!"));
    WRITE_THIS_LOG(QUEST_LOG, "Maivia gave Ogre quest to "+ player->query_real_name());
    STATSERV_LOG_EVENT("Utterdark", "Maivia gave out a quest");
}				
...
give_nochipa_quest(object player)
{
    ...
    WRITE_THIS_LOG(QUEST_LOG, "Maivia gave Nochipa quest to "+
...
do_die(object killer)
{
    ...
    WRITE_THIS_LOG(DEATH_LOG, "Maivia ("+ TO->query_average_stat() +") killed by "+ 
