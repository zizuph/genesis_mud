...
equip_me()
{
    ...
    /* 50% chance of getting a gem */
    gem = RANDOM_GEM_BY_RARITY(GEM_RARE);
    gem->move(this_object());              
    gem->set_heap_size(random(2));  

    clone_object("/d/Avenir/common/obj/coal")->move(TO);
...
light_cry()
{
    ...
    switch (i)
    {
        case 0: command("emote covers his eyes with one arm."); 
                break;
        case 1: command("emote cries in pain from the brightness of the light.");
                break;
...
enter_env(object inv, object from)
{
    ...
    set_alarm(1.0 + itof(random(10)), 0.0, light_cry);
}
