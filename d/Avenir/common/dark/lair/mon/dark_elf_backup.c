...
special_attack(object victim)
{
    ...
    /* 
     * Let's use npc stat av as the pen. 
     * Hurt of 220 is equivalent of F_PENMOD(45, 45) so the 
     * larger npcs will be doing more damage than a magic weapon.
     */
	
    // Choose power of hit based on mob stat size.
     hurt = TO->query_average_stat();
    ...
    hdesc = hitresult[1];
    tell_watcher(QCTNAME(TO) + " slams " + QTNAME(victim) + " on the " +
	hdesc + " with the flat side of his weapon.\n", victim); 
    victim->catch_msg(QCTNAME(TO) + " slams you on the " + hdesc +
	" with the flat side of his weapon!\n");
...
equip_me(void)
{
     ...
     */
    clone_unique(OBJ + "tooth_knife", 6, 0, 0, 15, 0)->move(TO);
	...
	clone_unique(OBJ +"smash_tool", 3, 0, 0, 25, 0)->move(TO);
	break;
	...
	clone_object(OBJ +"ddagger")->move(TO);	
	clone_object(OBJ +"ssword")->move(TO);	
	clone_unique(OBJ + "f_knife", 45, 0, 0, 25, 0)->move(TO);
	break;
    }
