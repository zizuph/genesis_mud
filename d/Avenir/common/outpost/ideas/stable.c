...
#define TEMP_FILE CALATHIN_DIR+"stable_temp"
 ...
 do_stable(string arg)
 {
   ...
   if (!jonas) 
	...
	jonas->command("say Just to be clear - I can only keep "+
		       ...
		       " for a month without you settling up your bill! "+
  ...
  if (!jonas) 
	...
	notify_fail("Can't find your horse! Please bug with error #102!");
		...
		tell_room(TO,"The stablehand retrieves " +
	      ...
	      {
                jonas->command("say My condolences for your loss.");
/*
	 ...
	 *customers, *saved_info, *temp_saved_info, 
       ...
       for (i=0;i<sizeof(customers);i++)
	 {
	   ...
	   if (!stringp(saved_info))
		 ...
		 {
		   gHorse[playername] = temp_saved_info;
		 }
	       else
		 {
		   gHorse[playername] = "";
