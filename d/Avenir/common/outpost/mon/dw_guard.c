...
int alarm_id;
string eyes, hair, adj1, adj2, fhair, fhair2, mstache;
...
my_long()
{
    ...
    str += "He is a typical specimen of male sybarite dwarves, "+
		   ...
		   "He has a "+ query_fhair() +" "+ query_hair() +
		   " beard plaited in a battle braid.\n"+
...
add_gate_talk()
{
    set_act_time(5+random(7));
    add_act("emote shifts his feet slightly.");
    add_act("emote suddenly stares toward the chasm.");
    add_act("emote squints his eyes, staring at you.");
    add_act("emote wipes the sweat from his brow.");
    add_act("emote cringes.");
    add_act("emote turns his ear towards the chasm.");

    set_chat_time(8+random(2));
    add_chat("I like the heat, but it sure makes me thirsty.");
    add_chat("I can't wait to get back to the islands!");
    add_chat("I wish there was some Source in this dead cave. We've "+
        "gotta use these pools of burning oil for light instead.");
    add_chat("Hey! Did something move on the other side of the chasm?!");
    add_chat("Beware the horror dwelling deep underground.");
    add_chat("If you want past the gate, just knock.");
    add_chat("You will need some torches if you're going into those tunnels." );
