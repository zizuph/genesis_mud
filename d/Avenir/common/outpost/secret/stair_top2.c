...
create_room()
{
    ...
    set_short("chasm");
    set_long("You stand on a ledge of the chasm wall. "+
        "A low grinding sound emanates from "+
        "deep below, and hot dry winds buffet you. Your eyes "+
        "pick out cracks and gaps in the "+
        "wall that may make it possible for you to work your "+
        "way downward.\n");
     ...
     */
    link_master("/d/Avenir/common/outpost/secret/stair_link2");

    add_exit("tunnel4", "north", 0, 20);
    add_exit("stair_bottom2", "down", -3, 60, 1);
}
...
link_room(string lfile, mixed dest, int pos)
{
    ...
    ob = clone_object("/d/Avenir/common/outpost/secret/stair_link2");
