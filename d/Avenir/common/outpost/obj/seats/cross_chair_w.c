...
create_seating(void)
{
    string str = one_of_list(({"black", "white", "silvery", "golden",
	             "shimmering", "red", "brown", "orange" }));
	
    set_name("chair");
    set_adj(str);
    add_adj(({"sling", "cross", "hammock", "webcloth"}));
    set_short("webcloth sling chair");
    set_long("This is an ancient chair made with four beautifully carved " 
     ...
     +"seat of the chair have layers of "+ str +" webcloth slung between "
	 +"them. Sitting in this is like being cradled in a hammock.\n");

    set_seat_names(({"chair", "sling", "hammock"}));

    // Need a random # because two seats in a room can't have the same seat id.
    set_seat_id("_cross_chair_webcloth" +random(100));
