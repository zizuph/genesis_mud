/* 
   Alco bottle by Milan.

   A bug correction made, Maniac, 16/10/95
   Another bug corrected, Maniac, 30/5/96
   Keep added, Maniac, 8/6/96 
   Bug fix, Maniac, 9/5/98
   Changed indentation, fixed "empty" bug, Maniac, 8/8/01 
*/
...
set_soft(int i)
{
    ...
    if (!i) 
        filled_short = ({ });
    // Need to update the state because of the effect the soft has on weight
    // and volume.
    update_state();
}
...
set_max(int i)
{
    ...
    if (i > 0) 
        max_amount = i;
    // Need to update the state because of the effect the max has on weight
    // and volume.
    update_state();
}
