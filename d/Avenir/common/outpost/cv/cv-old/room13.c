 ...
 *
 * Talk to Nerull about adding a check to the west
 * exit in the event a "removed" vampire tries to 
 * raid the rack.
 */
...
create_vampires_room()
{
	...
	set_long("This is the west end of the main corridor, just outside the "
		...
		+"impale intruders. " 
        +"The main corridor of the coven slopes east from here, down "
		+"toward the cystal chamber and the lounging area there."
		+"\n");

		
    add_exit("room12", "north");
	add_exit("room10", "east");
	// add a check on the armory exit for coven members?
