...
block_check()
{
    if (!present("_vampire_ward_gargoyle", this_object()))
    {
        return 0;
    }  
    ...
    write("The large menacing gargoyle blocks you from "
...
create_vampires_room()
{
   ...
   add_item(({"crystal", "crystals", "opening"}),
    ...
    clone_object(VAMPIRES_GUILD_DIR + "npc/ward2")->move(this_object(), 1);		
...
/* Changes to these commands should be duplicated in the board.c room. */
void
init()
{
    ::init();
