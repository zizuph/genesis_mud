...
// note: This sword, named Shintor, has a high hit/pen (40/50) when
...
#define ROUNDS           5
#define PEN_NORMAL      30
#define HIT_NORMAL      35
#define PEN_SHIMMERING  50
#define HIT_SHIMMERING  40
...
string *colouradj = (({ "deep abyssmal" , "strong hellish" ,
...
void create_weapon()
{
  ...
  set_long("This sword is very thin and long, with the blade "+
           "suddenly tapering off to an infintesimal point at the "+
  ...
  add_prop(MAGIC_AM_ID_INFO,({
     ...
     "This weapon was forged in the depths of the Unterdark by the "+
        "legendary wizard Orestis.\n",80}));

  add_prop(OBJ_S_WIZINFO,"This sword, named Shintor, has a "+
        "high hit/pen (40/50), and also has a special attack "+
...
string my_long()
{
    if(shimmering)
     ...
     return "The blade is a a dull steely colour.";
}
...
int unwield(object to)
{
    ...
    say(QCTNAME(TP)+"'s aura fades away.\n");
...
/* Check for magic resistance */
      ...
      tell_watcher("A flash of "+
           colourtype[ colournum + 5] +
           " light leaps from "+QTNAME(query_wielded())+
           "'s "+ short() +" and bounces harmlessly off the "+
           hdesc+" of "+QTNAME(enemy)+".\n",enemy);
      return;
         }
