 ...
 * Flying steed.
 *
 * The base is Fysix's inheritable steed, but has
 * additional functionality to allow if to fly
 * over rooms to a destination.
 *
 * Cirion, June 1997
 * Lilith, May 2014  Updated move time to 2 seconds
 */
...
#ifdef DEBUGGING
#define DEBUG(x)      find_living("lilith")->catch_tell("#steed#: (" + \
                           calling_function() + ") " + x + "\n")
#else
#define DEBUG(x)         ;
#endif
...
static object  air_room;

/* FUNCTION PROTOTYPES */
public           object   get_air_room();
public varargs   string   air_room_short(object for_obj);
...
{
    return QCTNAME(this_object()) + " spreads its wings and "
             + "launches into the air.\n";
    STATSERV_LOG_EVENT("outpost", "Rode the aglyna");
...
as_hook_left_room()
{
    return QCTNAME(this_object()) + " flies out of sight.\n";
}
...
{
    return "High overhead, " + QNAME(this_object()) + " flies into sight.\n";
}
...
{
    return QCTNAME(this_object()) + " continues to fly.\n"
           + capitalize(air_room_short()) + ".\n";
}
...
{
    return QCTNAME(this_object()) + " swoops down and lands.\n";
}
...
as_hook_landed_room()
{
    return QCNAME(this_object()) + " swoops down and lands.\n";
}
...
//       desc = capitalize(air_room_short(for_obj)) + ".\n";

    if(!room)
       return as_hook_default_room_long();

    return "Far below, you see the following:\n" + desc;
}    
 ...
 *                This is used in air_room_short() so we can
...
air_room_short(object for_obj)
{
    ...
    return "flying over " + fix_short_desc(room->short(for_obj));
}    
...
make_air_room()
{
    ...
    ob->set_long(air_room_long);
    ob->set_short(air_room_short);
...
move_air_steed(object o)
{
    ...
    if (!o->query_prop(LIVE_O_STEED))
    {
        ...
        DEBUG("Func called w/o mounted rider");
        return;
    }
    if (!location_index)   // => beginning of the trip
    {
        ...
        {
            DEBUG("could not establish the trip type");
            return;
        }
        else
        {
            ...
            move_living("M", get_air_room(), 1, 1);
       }
    }
    ...
    else    // => in transit
    {
        ...
        if(environment() != get_air_room())
            move_living("M", get_air_room(), 1, 1);

        riders_glance();

        DEBUG("environment: " + file_name(environment()));

        set_alarm(itof(move_delay), 0.0, move_air_steed);
    }
...
check_destination(mixed to_dest)
{
    ...
    if(member_array(dest, places[trip_type]) < 0 && ((object)to_dest != air_room))
    {
       DEBUG("tried to move to illegal location: " + dest);
       return 0;
    }
    else
       return 1;
...
do_mount(object o, int silent = 0)
{
    ...
    if (ret)
    {
        set_alarm(itof(move_delay), 0.0, &move_air_steed(o));
    }
    return ret;
}
...
do_dismount(object o, int silent = 0)
{
    ...
    {
        notify_fail("You would fall to your death.\n");
        return 0;
    }
