...
add_item(({"sign","signs"}),"The signs are emblazoned with shop and "
                           +"merchant names.\n");
add_item(({"red sign"}),"This sign says Goralg's Basket Shop");
add_item(({"door"}),"The door is slightly ajar, as if someone wants "
                   +"you to come inside.\n");
add_exit(TRADE(tsquare/t9),"southwest");
add_exit(TRADE(tsquare/t7),"east");
add_exit(TRADE(tsquare/hconn2),"north");
}
