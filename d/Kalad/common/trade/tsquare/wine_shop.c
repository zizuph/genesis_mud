...
#define STORE_ROOM "/d/Kalad/common/trade/tsquare/wine_store"
object ob1;
void
create_room()
{
   ...
   hear_bell = 1;
   INSIDE;
   set_short("Barney's Wine Shop");
   set_long("This is Barney's wine shop. Barney sells the finest wine "
      +"in the city of Kabal. They are weak wines, "
      +"but very high quality. This is like a bar, but Barney "
      +"only has a limited supply of drinks.\n");
   add_exit(TRADE(tsquare/t3),"west");
   add_exit(TRADE(tsquare/wine_store),"south","@@wiz_check");
...
{
   if(ob->id("_kalad_wine_shop_"))return 1;
   return 0;
}
int
do_sell(string str)
