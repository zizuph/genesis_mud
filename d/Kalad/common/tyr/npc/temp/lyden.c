...
create_monster ()
{
    ...
    set_stats(({150,150,150,50,50,125}));
...
arm_me()
{
    if(!random(4))
    { 
      clone_object(TYR_ARM+"fire_gloves.c")->move(TO); 
    }
    else
    { 
      clone_object(TYR_WEP+"l_triden")->move(TO);
      clone_object(TYR_ARM+"l_gaunt")->move(TO);
    };


    clone_object(TYR_ARM+"l_greave")->move(TO);
    clone_object(TYR_ARM+"l_vhelm")->move(TO);
    clone_object(TYR_ARM+"body_scl")->move(TO);
    command("wield all");
    command("wear all");
}
