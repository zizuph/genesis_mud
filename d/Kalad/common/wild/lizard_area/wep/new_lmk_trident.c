 ...
 *
 * Mirandus: February 5, 2022
 * - Boosted stats a bit to make it more competitive. 
 */ 
...
create_weapon()
{
    ...
    add_prop(OBJ_S_WIZINFO, "This trident, found on the New Saurian king "+
      "(/d/Kalad/common/wild/lizard_area/npc/liz_king) will, when wielded by "+
      "someone with strength greater than 175, have its hit/pen increased "+
      "from its normal 38/38 to 55/55. Resultantly, when the weapon is "+
      "not wielded or is wielded by someone with a strength of 175 or less, "+
      "it will only have its normal hit/pen. Anyone with strength less than "+
      "176 will also be shocked for 500 hp of damage, if they attempt to "+
      "wield the trident.\n");
...
wield(object what)
{
    if(TP->query_base_stat(SS_STR) > 170 && TP->query_race()!= "human")
    {
	write("The " + short() + " seems to pulse within your grip.\n");
	say(QCTNAME(TP) + "'s " + short() + " seems to pulse in " + 
    TP->query_possessive() + " grip.\n");
    TP->add_prop(LIVE_I_QUICKNESS,TP->query_prop(LIVE_I_QUICKNESS) + 75);
	what->set_hit(55);
	what->set_pen(55);
	E(TO)->update_weapon(TO);
	return 0;
    }
    write("The " + short() + " shocks you!\n");
    say(QCTNAME(TP) + " is shocked by " + TP->query_possessive() + " " + short() + ".\n");
    TP->heal_hp(-500);
    what->set_hit(38);
    what->set_pen(38);
    return 0;
