...
{
   ::create_room();
   set_short("dwarven Citadel. A hallway continues to the north and southwest");
   set_long("You are inside an ancient citadel, long abandoned "+
      ...
      "This part of the hallway seems to have been changed into "+
      "something that resembles a guardsroom. Several chairs and "+
      "two huge tables crowds the hallway. Combined with "+
      "an acidious smell that seems to come from a corner, you "+
      "assume this is a room that never are abandoned. Not even "+
      "at night. "+
      "The hallway continues towards the north and southwest, hiding "+
   ...
   add_exit(CPASS(citadel/x19),"north","@@block",-1);
   add_exit(CPASS(citadel/level3/c6),"southwest",0,-1);
...
reset_room()
{
   ...
   if (!objectp(brigand4))
   {
      brigand4=clone_object("/d/Kalad/common/wild/pass/citadel/npc/brigand_big");
      brigand4->move_living("M",TO);
      brigand4->arm_me();
      brigand4->command("say Lousy visitors! Making me work, even.");
   }
   ogre->team_join(brigand1);
   ogre->team_join(brigand2);
   ogre->team_join(brigand3);
   ogre->team_join(brigand4);
...
block()
{
   object stun;
   if (!present(ogre,TO) && 
       !present(brigand4,TO)) return 0; /* return if no ogre is here */
   if (!IS_OUTLAW(TP) && CAN_SEE(ogre,TP)) /* react if player is not outlaw */
   {
      ogre->command("emote laughs at you: Idiot! Me have ta kill yah now.");
      tell_room(TO,QCTNAME(TP)+" tried to walk further into the citadel but "+
         "was brutally stopped by a huge ogre smashing a fist into "+HIS(TP)+
         "face.\n",TP);
   ...
   /* check on brigand */
   if (!IS_OUTLAW(TP) && CAN_SEE(brigand4,TP)) /* react if player is not outlaw */
   {
      brigand4->command("emote grins at you: You're not going in there, pal!.");
      tell_room(TO,QCTNAME(TP)+" tried to walk further into the citadel but "+
         "was brutally stopped by a brigand smashing a fist into "+HIS(TP)+
         "face.\n",TP);
      TP->catch_msg("The brigand smashes a fist into your face. You feel "+
         "dazed!\n");
      stun=clone_object("/d/Kalad/common/wild/pass/citadel/obj/stun");
      stun->move(TP);
      return 1;
