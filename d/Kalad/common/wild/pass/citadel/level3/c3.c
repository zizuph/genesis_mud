...
create_room()
{
   ...
   set_long("You are inside an ancient citadel, long abandoned "+
      ...
      "The room has just one exit to the north "+
   ...
   add_exit(CPASS(citadel/level3/c1),"north","@@arch",-1);
...
reset_room()
{
   if (!objectp(prisoner))
   {
      prisoner=clone_object("/d/Kalad/common/wild/pass/citadel/npc/lordprisoner");
