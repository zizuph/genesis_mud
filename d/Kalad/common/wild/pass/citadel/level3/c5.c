...
create_room()
{
   ...
   set_long("You are inside an ancient citadel, long abandoned "+
      ...
      "The room has two exits. One to the northwest and another south "+
   ...
   add_exit(CPASS(citadel/level3/c2),"northwest","@@arch",-1);
   add_exit(CPASS(citadel/level3/c6),"south","@@block",-1);
}
...
reset_room()
{
   ...
   if (!objectp(brigand1))
   {
      brigand1=clone_object("/d/Kalad/common/wild/pass/citadel/npc/brigand_big");
      brigand1->arm_me();
