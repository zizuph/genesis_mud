...
create_room()
{
   ...
   set_long("You are inside an ancient citadel, long abandoned "+
      ...
      "The hallway seems to go upwards to the west, while "+
      "northwestwards you know there is another room hidden "+
   ...
   add_exit(CPASS(citadel/x13),"west","@@up",-1);
   add_exit(CPASS(citadel/level2/b7),"northwest","@@arch",-1);
}
...
down()
{
   TP->catch_msg("As you walk on you notice that you slowly accend upwards.\n");
   return 0;
}
