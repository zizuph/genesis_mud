...
create_room()
{
   ...
   set_long("You are inside an ancient citadel, long abandoned "+
      ...
      "The room has an exit to the northeast and branches "+
      "southwest and southeast into what looks like two more rooms hidden "+
   ...
   add_exit(CPASS(citadel/level2/b1),"northeast","@@block",-1);
   add_exit(CPASS(citadel/level2/b4),"southwest","@@arch",-1);
   add_exit(CPASS(citadel/level2/b5),"southeast","@@block",-1);
}
...
reset_room()
{
   if (!objectp(ogre))
   {
      ogre=clone_object("/d/Kalad/common/wild/pass/citadel/npc/ogre_medium");
      ogre->arm_me();
