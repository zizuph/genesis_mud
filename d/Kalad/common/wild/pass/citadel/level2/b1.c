...
create_room()
{
   ...
   set_long("You are inside an ancient citadel, long abandoned "+
      ...
      "southwest and southeast into what looks like two rooms hidden "+
   ...
   add_exit(CPASS(citadel/x12),"east","@@down",-1);
   add_exit(CPASS(citadel/level2/b2),"southwest","@@arch",-1);
   add_exit(CPASS(citadel/level2/b3),"southeast","@@arch",-1);
}
