...
{
   ::create_room();
   set_short("dwarven Citadel. A hallway continues to the south and north");
   set_long("You are inside an ancient citadel, long abandoned "+
      ...
      "The hallway continues towards south and north, hiding "+
   ...
   add_exit(CPASS(citadel/x8),"south","@@down",-1);
   add_exit(CPASS(citadel/x10),"north","@@block",-1);
...
reset_room()
{
   if (!objectp(ogre))
   {
      ogre=clone_object("/d/Kalad/common/wild/pass/citadel/npc/ogre");
      ogre->arm_me();
   ...
   if (!objectp(brigand1))
   {
      brigand1=clone_object("/d/Kalad/common/wild/pass/citadel/npc/brigand");
      brigand1->arm_me();
      brigand1->move_living("M",TO);
   }
   if (!objectp(brigand2))
   {
      brigand2=clone_object("/d/Kalad/common/wild/pass/citadel/npc/brigand");
      brigand2->arm_me();
...
down()
{
   TP->catch_msg("As you walk on you notice that you slowly decend downwards.\n");
   return 0;
}
...
block()
{
   ...
   {
      ogre->command("emote screams at you: Bloodey goodey, drinkin with "+
         "dem Kabal guards are yah!!");
