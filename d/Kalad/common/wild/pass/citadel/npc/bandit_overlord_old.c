...
arm_me()
{
   ...
   arm -> move(TO);
   command("wear all");
   axe = unique_item("/d/Kalad/common/wild/pass/citadel/wep/mbgaxe",
                     "/d/Kalad/common/wild/pass/citadel/wep/alt_axe",4);
   axe -> move(TO);
   command("wield all");
   wep = clone_object("/d/Kalad/common/wild/pass/citadel/wep/goliath_dagger");
   wep -> move(TO);
...
greet_friend(ob)
{
      ...
      !ob->query_npc() && random(4)>1 && CAN_SEE(TO,ob) && CAN_SEE_IN_ROOM(TO))
   {
      if (!query_attack())
         {
        command("say Greetings, "+ob->query_name()+".");
         return;
       }
      else
         {
        command("say Ahh, "+ob->query_name()+
