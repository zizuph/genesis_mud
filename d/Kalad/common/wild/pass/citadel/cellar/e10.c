 ...
 * This room is only accessable when a player wields
 * a special dagger. (quest-area)
*/
...
create_room()
{
   ...
   set_long("Large boulders and stones litter this hallway, making "+
      ...
      /* exits are visible according to the wielded dagger of
       * the player
       */
   ...
   add_exit(CPASS(citadel/cellar/e11),"east","@@walk",3,1);
   add_exit(CPASS(citadel/cellar/e3),"southwest",0,3,1);
}
...
tracking()
{
   ...
   if (!dagger->query_prop("_korat_is_kabal_dagger")) return "A creepy silence seems to enclose you.";
   
   return "The dagger seems to pull you towards a crack "+
      "inbetween two large boulders to the east.";
}
