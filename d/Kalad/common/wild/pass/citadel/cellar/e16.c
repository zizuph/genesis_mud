...
create_room()
{
   ...
   add_exit(CPASS(citadel/cellar/e15),"west",0,3,1);
   add_exit(CPASS(citadel/cellar/death3),"northeast","@@walk",3,1);
}
...
tracking()
{
   ...
   switch (dagger->query_prop("_korat_is_kabal_dagger")) 
   {
      ...
      break;
      case -1:
         return "The pull in the dagger seems to waver towards a path "+
