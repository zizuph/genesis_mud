/* 
 * /d/Kalad/common/wild/pass/npc/sand_mar.c
 * Purpose    : The sand maruader
 * Located    : /d/Kalad/commmon/wild/pass/desert/sand/
...
#define H_RARM 3
#define H_LEGS 4
/* The sand marauder, infamous dweller of the sand dunes in the Kalad
desert. */
/* by Antharanos */
create_monster()
{
   ...
   set_long("A vaguely humanoid-shaped mass of wrinkled flesh. Its "+
      ...
      "sand dunes, making it very well camouflaged. Its disgusting "+
      "head is dominated by a large, circular mouth filled with needle-sharp "+
      "rows of teeth.\n");
   set_stats(({40,60,40,20,20,60}));
   set_gender(G_NEUTER);
...
cr_did_hit(int aid, string hdesc, int phurt, object enemy, int dt, int phit)
{
   if(aid == A_MOUTH)
      {
      tell_object(enemy,"The sand marauder's suckered mouth rips into you!\n");
      {
         enemy -> add_mana(-30);
