...
create_room()
{
   ...
   set_long("The vastness of the Great Kalad Waste stretches in every "+
      ...
      "A large salt flat lies to the northeast.\n"+
   ...
   add_item(({"large salt flat","large flat","salt flat","flat"}),"A "+
      "large expanse of salt crystals lying within the desert.\n");
   add_exit(CPASS(desert/d19),"northwest",0,3);
   add_exit(CPASS(desert/salt/s1),"northeast","@@blind",3);
   add_exit(CPASS(desert/d21),"southeast",0,3);
}
blind()
{
   write("The intense brightness of the salt crystals forces you to "+
      "squint your eyes in protection.\n");
   return 0;
}
