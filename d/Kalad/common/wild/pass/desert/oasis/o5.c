...
create_room()
{
   ...
   set_long("You are in an isolated spot within the oasis.\n"+
      ...
      "west, where the vegetation is much heavier.\n");
   ...
   add_item(({"ground"}),"It isn't visible, due to the abundance of "+
      "vegetation.\n");
   add_exit(CPASS(desert/oasis/o6),"west");
...
reset_room()
{
   if(!ob1)
      ...
      tell_room(TO,"A wolf appears from amidst the bushes, growling slightly.\n");
   }
   if(!ob2)
      {
      ob2 = clone_object(CPASS(npc/d_wolf));
      ob2 -> move_living("M",TO);
      tell_room(TO,"A wolf stalks out of the surrounding vegetation.\n");
   }
   if(!ob3)
      {
      ob3 = clone_object(CPASS(npc/d_wolf));
      ob3 -> move_living("M",TO);
      tell_room(TO,"A wolf emerges from a nearby bush, its fangs bared in warning.\n");
      ob1 -> team_join(ob2);
      ob1 -> team_join(ob3);
   }
}
