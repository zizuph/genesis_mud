...
create_room()
{
   ...
   set_short("Oasis");
   set_long("You are now almost at the center of the oasis.\n"+
      ...
      "south, where the vegetation is much thicker.\n");
   ...
   add_item(({"ground"}),"It isn't visible, due to the abundance of "+
      "vegetation.\n");
   add_exit(CPASS(desert/oasis/o1),"north");
   add_exit(CPASS(desert/oasis/o6),"south");
...
reset_room()
{
   if(!ob1)
      {
      ob1 = clone_object(CPASS(npc/d_hare));
      ob1 -> move_living("M",TO);
      tell_room(TO,"A small desert hare bounces into view.\n");
   }
}
