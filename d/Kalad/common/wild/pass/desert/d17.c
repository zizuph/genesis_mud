...
create_room()
{
   ...
   set_long("The vastness of the Great Kalad Waste stretches in every "+
      ...
      "The Caravan Route bends here, turning to the west and southeast.\n");
   ...
   add_exit(CPASS(desert/d16),"west",0,3);
   add_exit(CPASS(desert/d18),"southeast",0,3);
...
reset_room()
{
   if(!ob1)
      {
      ob1 = clone_object(CPASS(npc/bed_war));
      ob1 -> my_stuff();
      ob1 -> move_living("M",TO);
   }
   if(!ob2)
      {
      ob2 = clone_object(CPASS(npc/bed_camel));
      ob2 -> move_living("M",TO);
      ob1 -> team_join(ob2);
      tell_room(TO,"A desert nomad along with a camel arrives from around the bend.\n");
   }
}
