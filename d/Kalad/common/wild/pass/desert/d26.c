...
create_room()
{
   ...
   set_long("The vastness of the Great Kalad Waste stretches in every "+
      ...
      "Far to the northeast lies the Desertrise mountains, a range colored "+
      "dark brown by the sand of the waste.\n"+
   ...
   add_item(({"desertrise mountains","mountains","mountain","range"}),
      "A series of low mountains that lies within the center of the "+
      "Great Kalad Waste, rising above the surrounding landscape.\n");
   add_exit(CPASS(desert/d25),"northwest",0,3);
   add_exit(CPASS(desert/d27),"southeast",0,3);
...
reset_room()
{
   if(!ob1)
      {
      ob1 = clone_object(CPASS(npc/bed_camel));
      ob1 -> move_living("M",TO);
      tell_room(TO,"A solitary camel comes into view.\n");
   }
}
