...
create_room()
{
   ...
   set_long("Rising skyward in great sloping mounds all around you are "+
      ...
      "The closest dunes lie just to the west and east, while passages "+
      "lead among them in several directions. "+
   ...
   add_item(({"passages","passage"}),"Depressions among the sand dunes that "+
      "allow travel between them.\n");
   add_exit(CPASS(desert/sand/d13),"northwest",0,3);
   add_exit(CPASS(desert/sand/d14),"north",0,3);
   add_exit(CPASS(desert/sand/d24),"south",0,3);
   add_exit(CPASS(desert/sand/d23),"southwest",0,3);
...
reset_room()
{
   if(!ob1)
      ...
      tell_room(TO,"A giant scorpion burrows up from the sand before you.\n");
   }
}
