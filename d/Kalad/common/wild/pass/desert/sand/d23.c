...
create_room()
{
   ...
   set_long("Rising skyward in great sloping mounds all around you are "+
      ...
      "Passage among the dunes lies in several directions. "+
   ...
   add_item(({"passage"}),"Little more than depressions between the "+
      "dunes, allowing travel among them.\n");
   add_exit(CPASS(desert/sand/d19),"northwest",0,3);
   add_exit(CPASS(desert/sand/d20),"northeast",0,3);
   add_exit(CPASS(desert/sand/d24),"east",0,3);
   add_exit(CPASS(desert/sand/d28),"southwest",0,3);
...
reset_room()
{
   if(!ob1)
      ...
      tell_room(TO,"A giant lizard arrives in your vicinity.\n");
   }
}
