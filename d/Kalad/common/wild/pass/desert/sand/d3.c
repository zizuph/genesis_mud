...
create_room()
{
   ...
   set_long("All around you, in great sloping mounds are "+
      ...
      "A way through the sand dunes seems to lies to the northeast, "+
   ...
   add_item(({"way"}),"Nothing more than a depression between the sand dunes.\n");
   add_exit(CPASS(desert/sand/d2),"northeast",0,3);
   add_exit(CPASS(desert/sand/d7),"southeast",0,3);
   add_exit(CPASS(desert/sand/d6),"southwest",0,3);
...
reset_room()
{
   if(!ob1)
      ...
      tell_room(TO,"A pile of sand shifts before you.\n");
   }
}
