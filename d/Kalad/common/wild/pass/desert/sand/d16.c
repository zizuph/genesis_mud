...
create_room()
{
   ...
   set_long("Towering in great sloping mounds around you are the "+
      ...
      "Many are the ways through the sand dunes here. "+
   ...
   add_item(({"ways","way"}),"Nothing but a depression amongst the "+
      "great mounds of sand that surround you.\n");
   add_exit(CPASS(desert/sand/d15),"west",0,3);
   add_exit(CPASS(desert/sand/d11),"north",0,3);
   add_exit(CPASS(desert/sand/d12),"northeast",0,3);
   add_exit(CPASS(desert/sand/d17),"east",0,3);
   add_exit(CPASS(desert/sand/d21),"south",0,3);
}
