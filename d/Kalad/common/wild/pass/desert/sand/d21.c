...
create_room()
{
   ...
   set_long("All around you, in great sloping mounds are "+
      ...
      "The closest dunes lie to the west and east, with several passages "+
      "leading among the dunes themselves. "+
   ...
   add_item(({"passages","passage"}),"Depressions among the sand dunes "+
      "that barely allow travel through them.\n");
   add_exit(CPASS(desert/sand/d15),"northwest",0,3);
   add_exit(CPASS(desert/sand/d16),"north",0,3);
   add_exit(CPASS(desert/sand/d17),"northeast",0,3);
   add_exit(CPASS(desert/sand/d26),"south",0,3);
   add_exit(CPASS(desert/sand/d25),"southwest",0,3);
...
reset_room()
{
   if(!ob1)
      ...
      tell_room(TO,"A rattling sound alerts you to the presence of a rattler snake.\n");
   }
}
