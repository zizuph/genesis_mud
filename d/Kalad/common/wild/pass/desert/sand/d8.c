...
create_room()
{
   ...
   set_long("Rising skyward in great sloping mounds all around you are "+
      ...
      "Many different ways lead among the sand dunes. "+
   ...
   add_item(({"ways","way"}),"Depressions between the sand dunes that "+
      "are marginally more travellable than the dunes themselves.\n");
   add_exit(CPASS(desert/sand/d7),"west",0,3);
   add_exit(CPASS(desert/sand/d4),"north",0,3);
   add_exit(CPASS(desert/sand/d5),"northeast",0,3);
   add_exit(CPASS(desert/sand/d12),"southeast",0,3);
   add_exit(CPASS(desert/sand/d11),"south",0,3);
}
