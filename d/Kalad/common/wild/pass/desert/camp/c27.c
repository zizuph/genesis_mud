...
#define CML "/d/Kalad/common/wild/pass/npc/bed_camel"
object ob1;
object ob2;
object ob3;
object ob4;
...
create_room()
{
   ...
   set_long("You are standing within the Bedellin nomads' camp. The "+
      ...
      "Blocking most of your vision to the west is a large and impressive "+
      "tent. "+
   ...
   add_exit(CPASS(desert/camp/c18),"north",0,2);
   add_exit(CPASS(desert/camp/c28),"east",0,2);
...
reset_room()
{
   if(!ob1)
      {
      ob1 = clone_object(CML);
      ob1 -> move_living("M",TO);
   }
   if(!ob2)
      {
      ob2 = clone_object(CML);
      ob2 -> move_living("M",TO);
   }
   if(!ob3)
      {
      ob3 = clone_object(CML);
      ob3 -> move_living("M",TO);
   }
   if(!ob4)
      {
      ob4 = clone_object(CML);
      ob4 -> move_living("M",TO);
   }
   if(!ob5)
      {
      ob5 = clone_object(CML);
      ob5 -> move_living("M",TO);
      tell_room(TO,"A herd of camels arrive.\n");
   }
}
