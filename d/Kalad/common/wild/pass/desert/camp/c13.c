...
create_room()
{
   ...
   set_long("You are within the camp of the Bedellin nomads, the "+
      ...
      "West from here is a small tent, while in the south lies a massive "+
      "tent. "+
   ...
   add_item(({"small tent"}),"A relatively small-sized tent fit "+
      "for a small Bedellin family.\n");
   add_item(({"massive tent"}),"A large canvas tent that lies within the "+
      "center of the camp.\n");
   add_exit(CPASS(desert/camp/c12),"west",0,-1);
   add_exit(CPASS(desert/camp/c8),"north",0,2);
   add_exit(CPASS(desert/camp/c14),"east",0,2);
}
