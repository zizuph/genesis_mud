...
create_room()
{
   ...
   add_item(({"kaladian sun","sun"}),
      ...
      "animal.\n");
   add_exit(CPASS(desert/rock/p27),"northwest",0,4);
   add_exit(CPASS(desert/rock/p33),"north",0,4);
   add_exit(CPASS(desert/rock/p31),"northeast",0,4);
   add_exit(CPASS(desert/rock/p14),"east",0,4);
   add_exit(CPASS(desert/rock/p15),"southeast",0,4);
...
reset_room()
{
   if(!ob1)
      ...
      tell_room(TO,"A giant scorpion arrives, emerging from a nearby rock.\n");
   }
}
