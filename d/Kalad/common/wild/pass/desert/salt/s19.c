...
create_room()
{
   ...
   add_exit(CPASS(desert/salt/s20),"west",0,4);
   add_exit(CPASS(desert/salt/s30),"north",0,4);
   add_exit(CPASS(desert/salt/s18),"east",0,4);
   add_exit(CPASS(desert/salt/s15),"south",0,4);
...
reset_room()
{
   if(!ob1)
      {
      ob1 = clone_object(CPASS(npc/s_snake));
      ob1 -> move_living("M",TO);
      tell_room(TO,"The hissing of a snake alerts you to its presence.\n");
   }
}
