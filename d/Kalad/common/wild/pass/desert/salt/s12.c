...
create_room()
{
   ...
   add_exit(CPASS(desert/salt/s11),"west",0,4);
   add_exit(CPASS(desert/salt/s22),"north",0,4);
   add_exit(CPASS(desert/salt/s13),"east",0,4);
   add_exit(CPASS(desert/salt/s9),"south",0,4);
...
reset_room()
{
   if(!ob1)
      ...
      tell_room(TO,"The salt crystals before you shifts slightly.\n");
   }
}
