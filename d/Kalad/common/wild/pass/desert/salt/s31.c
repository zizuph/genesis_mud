...
create_room()
{
   ...
   add_exit(CPASS(desert/salt/s30),"west",0,4);
   add_exit(CPASS(desert/salt/s33),"north",0,4);
   add_exit(CPASS(desert/salt/s32),"east",0,4);
   add_exit(CPASS(desert/salt/s18),"south",0,4);
...
{
   ::init();
   clone_object("/d/Kalad/common/wild/pass/obj/mirage3")->move(TP,1);
}
leave_inv(ob, to)
{
   object mirage;
   ::leave_inv(ob);
   mirage = present("mirage3", ob);
   if (mirage) mirage -> remove_object();
}
mirage()
{
   tell_room(TO,"Dessicated arms burst forth from the ground and grab you "+
      "with an iron grip.\n");
   tell_room(TO,"You feel yourself being suffocated as the salt pours into "+
      "your mouth.\n");
   tell_room(TO,"Then you blink, and notice that nothing has changed.\n");
   return 1;
}
