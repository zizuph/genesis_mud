...
create_room()
{
   ...
   add_exit(CPASS(desert/d20),"southwest",0,3);
   add_exit(CPASS(desert/salt/s9),"north",0,4);
   add_exit(CPASS(desert/salt/s2),"east",0,4);
   set_alarm(2.0,0.0,"mirage");
}
void
init()
{
...
mirage()
{
   tell_room(TO,"A gigantic worm suddenly rears up before you, its "+
      "horrible pincered face suddenly rushing towards you!!!\n");
   tell_room(TO,"The gigantic worm attacks you!\n");
   tell_room(TO,"The gigantic worm smashes you with its great fanged "+
      "mouth, with a bone crushing sound.\n");
   tell_room(TO,"The gigantic worm massacres you with its great fanged mouth.\n");
...
recover()
{
   tell_room(TO,"You suddenly snap back to reality, having recovered from "+
      "what apparently was a mirage.\n");
   return 1;
}
