...
create_room()
{
   ...
   add_exit(CPASS(desert/salt/s1),"west",0,4);
   add_exit(CPASS(desert/salt/s8),"north",0,4);
   add_exit(CPASS(desert/salt/s3),"east",0,4);
...
reset_room()
{
   if(!ob1)
      ...
      tell_room(TO,"A snake emerges from the salt crystals.\n");
   }
}
