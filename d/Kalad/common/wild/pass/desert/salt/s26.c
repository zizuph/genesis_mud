...
create_room()
{
   ...
   add_exit(CPASS(desert/salt/s25),"west",0,4);
   add_exit(CPASS(desert/salt/s38),"north",0,4);
   add_exit(CPASS(desert/salt/s27),"east",0,4);
   add_exit(CPASS(desert/salt/s23),"south",0,4);
...
mirage()
{
   tell_room(TO,"A gigantic pillar of fire blazes before you.\n");
   tell_room(TO,"It suddenly vanishes.\n");
   return 1;
}
