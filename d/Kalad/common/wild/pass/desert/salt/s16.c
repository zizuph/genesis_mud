...
create_room()
{
   ...
   add_exit(CPASS(desert/salt/s15),"west",0,4);
   add_exit(CPASS(desert/salt/s18),"north",0,4);
   add_exit(CPASS(desert/salt/s5),"south",0,4);
...
mirage()
{
   tell_room(TO,"The sound of braying trumpets reaches your ears.\n");
   return 1;
}
