...
create_room()
{
   ...
   add_item(({"sky"}),"Colored the deepest blue, it is completely free of "+
      "clouds.\n");
   add_exit(CPASS(desert/grass/g11),"north");
   add_exit(CPASS(desert/grass/g12),"northeast");
   add_exit(CPASS(desert/grass/g19),"east");
   add_exit(CPASS(desert/grass/g25),"southeast");
}
