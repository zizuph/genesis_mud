...
create_room()
{
   ...
   add_item(({"sky"}),"Colored the deepest blue, it is completely free of "+
      "clouds.\n");
   add_exit(CPASS(desert/grass/g2),"west");
   add_exit(CPASS(desert/grass/g4),"east");
   add_exit(CPASS(desert/grass/g8),"southeast");
   add_exit(CPASS(desert/grass/g7),"south");
   add_exit(CPASS(desert/grass/g6),"southwest");
}
