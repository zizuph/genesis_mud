...
create_room()
{
   ...
   add_item(({"sky"}),"Colored the deepest blue, it is completely free of "+
      "clouds.\n");
   add_exit(CPASS(desert/grass/g2),"north");
   add_exit(CPASS(desert/grass/g3),"northeast");
   add_exit(CPASS(desert/grass/g7),"east");
   add_exit(CPASS(desert/grass/g13),"southeast");
   add_exit(CPASS(desert/grass/g12),"south");
   add_exit(CPASS(desert/grass/g11),"southwest");
}
