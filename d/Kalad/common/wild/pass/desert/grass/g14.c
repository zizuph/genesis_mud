...
create_room()
{
   ...
   add_item(({"sky"}),"Colored the deepest blue, it is completely free of "+
      "clouds.\n");
   add_exit(CPASS(desert/grass/g13),"west");
   add_exit(CPASS(desert/grass/g7),"northwest");
   add_exit(CPASS(desert/grass/g8),"north");
   add_exit(CPASS(desert/grass/g9),"northeast");
   add_exit(CPASS(desert/grass/g15),"east");
   add_exit(CPASS(desert/grass/g22),"southeast");
   add_exit(CPASS(desert/grass/g21),"south");
   add_exit(CPASS(desert/grass/g20),"southwest");
...
reset_room()
{
   if(!ob1)
      {
      ob1 = clone_object(CPASS(npc/wild_dog));
      ob1 -> move_living("M",TO);
      tell_room(TO,"A wild dog suddenly arrives in the area.\n");
   }
}
