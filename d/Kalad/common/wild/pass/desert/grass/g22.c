...
create_room()
{
   ...
   add_item(({"sky"}),"Colored the deepest blue, it is completely free of "+
      "clouds.\n");
   add_exit(CPASS(desert/grass/g21),"west");
   add_exit(CPASS(desert/grass/g14),"northwest");
   add_exit(CPASS(desert/grass/g15),"north");
   add_exit(CPASS(desert/grass/g16),"northeast");
   add_exit(CPASS(desert/grass/g23),"east");
   add_exit(CPASS(desert/grass/g29),"southeast");
   add_exit(CPASS(desert/grass/g28),"south");
   add_exit(CPASS(desert/grass/g27),"southwest");
...
reset_room()
{
   if(!ob1)
      {
      ob1 = clone_object(CPASS(npc/d_wolf));
      ob1 -> move_living("M",TO);
   }
   if(!ob2)
      {
      ob2 = clone_object(CPASS(npc/d_wolf));
      ob2 -> move_living("M",TO);
      ob1 -> team_join(ob2);
      tell_room(TO,"A pair of wolves arrives in your vicinity.\n");
   }
}
