...
create_room()
{
   ...
   add_exit(CPASS(uc/u21),"north","@@check1",2,1);
   add_exit(CPASS(uc/u38),"south","@@check2",2,1);
}
void
init()
{
...
jump(string str)
{
   ...
   say(QCTNAME(TP) + " foolishly jumps and falls from the bridge and "+
      "down into the chasm.\n");
   TP->move_living("M","/d/Kalad/common/wild/pass/uc/fall_e",1);
   return 1;
}
int
throw(string str)
{
   ...
   arr[i]->move("/d/Kalad/common/wild/pass/uc/fall_e");
