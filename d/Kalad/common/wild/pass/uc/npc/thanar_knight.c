...
create_monster()
{
   ...
   add_act("glare all");
   add_act("say Only the faithful of Thanar may pass me and go west.");
   set_cact_time(3);
...
pass()
{
   ...
   if(ob[i]->id("_sniff_my_licker"))
      {
      write("You move past the knight and head west.\n");
      TP->move_living("west","/d/Kalad/common/wild/pass/uc/u25");
      return 1;
   }
...
arm_me()
{
   object wep, arm;
   seteuid(getuid(TO));
   arm = clone_object("/d/Kalad/common/caravan/arm/lkhelm");
   arm -> move(TO);
   arm = clone_object("/d/Kalad/common/caravan/arm/lkchain");
   arm -> move(TO);
   command("wear all");
...
return_leave(string who){
   ...
   if(obj = present(who, environment())){
      ...
      obj -> move_living("west","/d/Kalad/common/wild/pass/uc/u25");
   }
}
int
...
return_symbol(string who){
   ...
   int ran;
   if(obj = present(who, environment())){
      command("beg " + (obj->query_real_name()));
