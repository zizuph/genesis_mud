...
create_room()
{
   ...
   set_long("At the edge of a chasm.\n"+
      "A near limitless amount of stone surrounds you from all angles, "+
      ...
      "impenetrable barrier between you and the sunlit realms of Kalad. "+
      ...
      "diverges here, one branch heading to the southwest, the other to the "+
      "southeast.\n");
   add_item(({"edge"}),
   ...
   add_exit(CPASS(uc/u30),"north","@@check",2,1);
   add_exit(CPASS(uc/u44),"southeast",0,2,1);
   add_exit(CPASS(uc/u43),"southwest",0,2,1);
}
check()
{
   if(TP->query_stat(SS_DEX) < random(CHECK_DEX))
      ...
      TP->move_living("falling into the chasm","/d/Kalad/common/wild/pass/uc/d5");
      TP->heal_hp(-DAMAGE);
      if(TP->query_hp() <= 0)
         TP->do_die(TO);
      return 1;
