...
create_room()
{
   ...
   set_long("A crossroads of passages.\n"+
      "Stone as black as a grieving widow's veil is visible from all angles, "+
      "its dark embrace promising the only certainty in life, death. Above "+
      "lies an unimaginable amount of rock, a massive volume of which "+
      ...
      "Three passages are open to you here, one leading to the north, "+
      "another to the east and a third one to the southwest.\n");
   ...
   add_exit(CPASS(uc/u14),"north",0,2,1);
   add_exit(CPASS(uc/g2),"east",0,2,1);
   add_exit(CPASS(uc/u23),"southwest",0,2,1);
}
