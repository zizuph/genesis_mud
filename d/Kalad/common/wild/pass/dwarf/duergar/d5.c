...
create_room()
{
   ...
   set_long("You are within an old ruin. Large blocks of granite that once "+
      ...
      "There seems to be possible to walk northwest, north, northeast and "+
      "southwest, but the "+
   ...
   add_exit(CPASS(dwarf/duergar/d3),"southwest");
   add_exit(CPASS(dwarf/duergar/d6),"north");
   add_exit(CPASS(dwarf/duergar/d7),"northeast");
   add_exit(CPASS(dwarf/duergar/d4),"northwest");
