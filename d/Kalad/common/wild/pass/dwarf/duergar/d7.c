...
create_room()
{
   ...
   set_long("You are within an old ruin. Large blocks of granite that once "+
      ...
      "There seems to be possible to walk southwest and south, but the "+
      "other ways are closed off by the ruins. "+
      "On the ground you see a stairway going downwards.\n");
   ...
   add_exit(CPASS(dwarf/duergar/d8),"south");
   add_exit(CPASS(dwarf/duergar/d5),"southwest");
   add_exit(CPASS(dwarf/guild/entrance),"down");
...
reset_room()
{
   ...
   if (!objectp(ob2))
   {
      ...
      tell_room(TO,"You notice something moving behind a stone-block.\n");
   }
   ob1->team_join(ob2);
}
