...
create_room()
{
   ...
   set_long("You are within an old ruin. Large blocks of granite that once "+
      ...
      "There seems to be possible to walk south and southeast, but the "+
   ...
   add_exit(CPASS(dwarf/duergar/d5),"southeast");
   add_exit(CPASS(dwarf/duergar/d2),"south");
...
reset_room()
{
   ...
   if (!objectp(ob2))
   {
     ...
     ob2->move_living("M",TO);
      tell_room(TO,"A couple of duergars stand up as you arrive.\n");
   }
   ob1->team_join(ob2);
}
