...
create_room()
{
   ...
   set_long("You are within an old ruin. Large blocks of granite that once "+
      ...
      "There seems to be possible to walk northeast and east, but the "+
      "other ways are closed off by the ruins. You see the street to the "+
      "southwest.\n");
   ...
   add_exit(CPASS(dwarf/duergar/d2),"northeast");
   add_exit(CPASS(dwarf/duergar/d3),"east");
   add_exit(CPASS(dwarf/s1),"southwest");
...
reset_room()
{
   if (!objectp(ob1))
   {
      ob1 = clone_object("/d/Kalad/common/wild/pass/dwarf/npc/duergar");
      ob1 -> arm_me();
   ...
   if (!objectp(ob2))
   {
      ...
      tell_room(TO,"Some duergars comes rambling in from the street.\n");
   }
   ob1->team_join(ob2);
}
