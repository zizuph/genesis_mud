...
create_room()
{
   ...
   add_item(({"exits","exit"}),"You see two of them that "+
      ...
      "pure darkness.\n");
   add_exit(CPASS(dwarf/temple/t5),"west");
   add_exit(CPASS(dwarf/temple/t10),"east","@@temple");
...
temple()
{
   ...
   if(TP->query_base_stat(SS_DIS)>45)
      {
      write("You feel extremely uneasy about walking eastwards, but continue "+
         "slowly.\n");
      say(QCTNAME(TP)+" looks extremely uneasy about going further eastwards, "+
   ...
   if(random(TP->query_base_stat(SS_DIS))<35)
      {
...
reset_room()
{
   if(!objectp(statue1))
      ...
      statue1->set_stats(({50,60,70,30,30,70}));
