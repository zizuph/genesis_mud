...
create_room()
{
   ...
   set_long("You seem to be inside a small chamber inside a temple "+
      ...
      "Back northwest you will leave this place that makes your blood "+
      "freeze, or you could try to continue to the north. "+
   ...
   add_item(({"exits","exit"}),"You see two of them that "+
      "continues back northwest and one north into "+
      "pure darkness.\n");
   add_exit(CPASS(dwarf/temple/t12),"northwest");
   add_exit(CPASS(dwarf/temple/t15),"north","@@temple");
...
temple()
{
   ...
   if(present("statue"))
      ...
      write("The statues suddenly becomes alive "+
         "and blocks you from continuing to the north!\n");
      return 1;
   }
   
   if(TP->query_base_stat(SS_DIS)>90)
      {
      write("You feel very frightened about walking northwards, but continue "+
         "slowly.\n");
      say(QCTNAME(TP)+" looks very frightened about going further northwards, "+
   ...
   write("A very chilling feeling decends upon you as you start to walk "+
      "northwards.\n");
   say(QCTNAME(TP)+" looks extremely afraid as "+TP->query_pronoun()+
      "tries to walk northwards.\n");
   if(random(TP->query_base_stat(SS_DIS))<70)
      {
...
reset_room()
{
   if(!objectp(statue1))
      ...
      statue1->set_stats(({120,120,140,50,50,140}));
