...
create_room()
{
   ...
   set_long("You seem to be at the entrance of a temple that has "+
      ...
      "but you have a creeping feeling that peril lingers inside the "+
      "darkness.\n");
   ...
   add_item(({"exits","exit"}),"You see three of them that "+
      ...
      "pure darkness.\n");
   add_exit(CPASS(dwarf/temple/t3),"west");
   add_exit(CPASS(dwarf/temple/t6),"northeast","@@temple");
   add_exit(CPASS(dwarf/temple/t8),"southeast","@@temple");
   add_exit(CPASS(dwarf/temple/t7),"east","@@temple");
...
temple()
{
   ...
   if(TP->query_base_stat(SS_DIS)>35)
      {
      write("You feel very uneasy about walking eastwards, but continue "+
         "slowly.\n");
      say(QCTNAME(TP)+" looks very uneasy about going further eastwards, "+
   ...
   write("A chilling feeling decends upon you as you start to walk "+
      "eastwards.\n");
   say(QCTNAME(TP)+" looks very uneasy as "+TP->query_pronoun()+
      "tries to walk eastwards.\n");
   if(random(TP->query_base_stat(SS_DIS))<20)
      {
...
reset_room()
{
   if(!objectp(statue1))
      ...
      statue1->set_stats(({30,40,50,30,30,60}));
   ...
   if(!objectp(statue2))
      ...
      statue2->set_stats(({30,40,50,30,30,60}));
   ...
   if(!objectp(statue3))
      ...
      statue3->set_stats(({30,40,50,30,30,60}));
      statue3->move_living("M",TO);
      statue3->heal_hp(statue1->query_max_hp());
   }
   statue3->team_join(statue1);
   statue3->team_join(statue2);
}
