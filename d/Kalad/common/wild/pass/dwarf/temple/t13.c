...
create_room()
{
   ...
   set_long("You seem to be inside a small chamber inside a temple "+
      ...
      "freeze, or you could try to continue to the south. "+
   ...
   add_item(({"exits","exit"}),"You see two of them that "+
      "continues back southwest and one south into "+
      "pure darkness.\n");
   add_exit(CPASS(dwarf/temple/t12),"southwest");
   add_exit(CPASS(dwarf/temple/t15),"south","@@temple");
...
temple()
{
   ...
   if(present("statue"))
      {
      write("The statue suddenly becomes alive "+
         "and blocks you from continuing to the south!\n");
      return 1;
   }
   
   if(TP->query_base_stat(SS_DIS)>70)
      {
      write("You feel very frightened about walking southwards, but continue "+
         "slowly.\n");
      say(QCTNAME(TP)+" looks very frightened about going further southwards, "+
