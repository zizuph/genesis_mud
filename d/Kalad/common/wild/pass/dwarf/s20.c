...
create_room()
{
   ...
   set_long("You seem to be at the eastern side of a small "+
      ...
      "a monument in the center of this old plaza. The whole area here "+
      "is lightened up by a weak red light that streams out from "+
      "the east, where you notice some hardworking dwarves sweating and "+
   ...
   add_exit(CPASS(dwarf/smith/s1),"east");
   add_exit(CPASS(dwarf/s19),"northwest");
   add_exit(CPASS(dwarf/s21),"southwest");
...
reset_room()
{
   if(!objectp(ob1))
   {
      ...
      tell_room(TO,"A dwarf comes wandering out of the eastern "+
         "building.\n");
   }
}
