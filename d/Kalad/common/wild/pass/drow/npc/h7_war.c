...
create_monster()
{
   ...
   set_extra_long("From the symbol of a sword and axe crossed over "+
      ...
      "House Claddath.\n");
   set_stats(({115,115,115,25,25,100}));
   set_alignment(-750);
   ...
   add_speak("Only those that show the insignia of House Claddath may "+
      "pass, all others will die.\n");
   add_prop(LIVE_I_SEE_DARK,10);
   add_prop(LIVE_I_NEVERKNOWN,1);
   add_prop(NPC_I_NO_RUN_AWAY,1);
   trig_new("%w 'tried' 'to' 'go' 'south' %s","react_block");
   trig_new("%w 'screams' 'a' 'rally-cry:' %s","react_rally");
}
int
react_rally(string who)
{
   if(who)
      {
      who = lower_case(who);
      set_alarm(2.0,0.0,"return_rally",who);
      return 1;
   }
}
void
return_rally(string who)
{
   object obj;
   int ran;
   if(obj = present(who, environment()))
      {
      command("say Claddath ultrin!");
      command("assist " + (TP->query_real_name()));
   }
}
void
emote_hook(string emote, object actor, string adv)
{
    if(emote == "hshow")
        set_alarm(2.0,0.0,"return_show",actor);
}
...
return_show(object obj)
{
   if(present(obj, environment()))
      ...
      obj -> move_living("south","/d/Kalad/common/wild/pass/drow/house7/main_hall");
   }
}
int
