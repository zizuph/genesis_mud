...
create_monster()
{
   ...
   set_adj("elite");
   add_adj(extra);
   set_pshort("elite " + extra + " drow");
   set_long("A warrior whose body has become the ultimate weapon, "+
      "becoming one with the blade in the heat of battle. Her finely "+
      "chiselled features seem almost out of place coupled with her battle "+
      "ready attire, yet one glance at the gleam in her eyes convinces you "+
      "of her foul nature.\n"+
      "You face a member of the foul race of drow.\n");
   set_extra_long("From the symbol of a sword crossed over a shield on "+
      "her forehead, it appears as though she belongs to House Kilsek.\n");
   set_stats(({100,120,100,25,25,100}));
   set_alignment(-900);
   ...
   set_act_time(6);
   add_act("emote gazes at you without a trace of fear.");
   ...
   add_speak("Only those bearing the insignia of House Kilsek may pass. "+
      "All others will die if they attempt to venture further.\n");
   add_prop(LIVE_I_SEE_DARK,10);
   add_prop(LIVE_I_NEVERKNOWN,1);
   add_prop(NPC_I_NO_RUN_AWAY,1);
   trig_new("%w 'proudly' 'shows' 'the' 'insignia' 'of' 'House' 'Kilsek.\n' %s","react_show");
   trig_new("%w 'tries' 'to' 'go' %s","react_block");
}
int
...
return_show(string who)
{
   ...
   if(obj = present(who, environment()))
      ...
      obj -> move_living("up","/d/Kalad/common/wild/pass/drow/house5/mm_chamber");
   }
}
int
