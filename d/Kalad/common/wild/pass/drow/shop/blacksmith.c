/*
*   2021-08-11 Meton Reformatted and added drow to array with drow smith
*       and smith.
*/
...
create_room()
{
    set_short("The blacksmith shop of Undraeth");
    set_long("The intensity of the heat in this room is nearly "+
        "intolerable. The source of the heat seems to be a forge that "+
        "lies at the far end of the room. Here, adventurers from all "+
        "walks of drow life may get their weapons fixed. The drow are "+
        "known for the fine quality of "+
        "their weapons, so you would do well to let them handle your "+
        "weapons.\n"+
        "A little sign is hanging on the wall.\n"+
        "A drow smith is standing at the forge.\n");

    add_item(({"walls","wall","floor","ground","ceiling"}),
        "It is constructed from a strangely dark rock that is common to "+
        "this area of the Dark Dominion.\n");
    add_item(({"forge"}),
        "It is so fiercely hot that you cannot get a good glimpse at "+
        "it.\n");
    add_item(({"drow smith","smith","drow"}),
        "A drow that is so covered in sweat that he appears to have "+
        "taken a shower...with his clothes on.\n");
    add_item(({"list","pricelist","sign","little sign"}), "@@sign_long");
    INSIDE;
    add_prop(ROOM_M_NO_TELEPORT,1);
    add_exit(CPASS(drow/d21),"east","@@msg",-1,-1);
...
msg()
{
    ...
    say(QCTNAME(TP) + " looks relieved as " + TP->query_pronoun() +
        " exits the building.\n");
    return 0;
}
...
do_fix(str)
{
    ...
    *   and what you would like in return.
    */
    if (sscanf(str, "%s for %s and get %s",
    ...
    notify_fail("The smith says: I cannot " + verb +
        " that any further.\n");
    if (value == -1)
        return 0;
    ...
    if (sizeof(money_arr = pay(value, tp, pay_type, 0, 0, get_type)) == 1)
    {
        ...
        {
            tell_room(this_object(), break_string(
                "The smith says: You do not have any " + pay_type +
    ...
    coin_get_text = text(exclude_array(money_arr,
        0, sizeof(MONEY_TYPES)-1));
...
if (!(obj->query_prop(OBJ_I_BROKEN)))
    return -1;
return (F_VALUE_WEAPON(obj->query_pen(),
    obj->query_hit()) * MEND_COST)/100;
}
