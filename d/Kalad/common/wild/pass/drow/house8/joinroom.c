...
create_room()
{
   ...
   set_short("The private hall of House Noquar");
   set_long("The private hall of House Noquar.\n"+
...
join(str)
{
   ...
   if(str == "house")
      ...
      if(insignia)
         {
         NF("You are already a member of House Noquar.\n");
         return 0;
      }
      write("You are now a member of House Noquar.\n");
      
      log_file("house8",TP->query_real_name()+" joined "+
         extract(ctime(time()),4,15)+".\n");
      
      if(!insignia)
         {
         insignia = clone_object(CPASS(drow/guild/h8_i));
         insignia -> move(TP);
         write("You receive the house insignia of Noquar.\n");
         say(QCTNAME(TP) + " enters the ranks of House Noquar!\n");
      }
      return 1;
   }
   return 0;
