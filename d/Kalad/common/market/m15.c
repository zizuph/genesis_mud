...
create_room()
{
   ...
   set_long("\n"+
      ...
      "\n\n"+
      "    To the south is a red tent. It has a "+
      "sign posted above the door which you "+
      "think you might be able to read. To the "+
      "northeast you can see a wagon. "+
      "\n\n");
   
   add_item(({ "red tent" }), "\n"+
      "This tent is made of bright, crimson red "+
      "material. There is brightly polished sign "+
      "hanging over the doorway. "+
      "\n\n");
   add_item(({ "sign", "polished sign" }), "\n"+
      "You think you can read what is written on it. "+
      "Besides the writing, you notice an engraving "+
      "of what appears to be an herb. "+
      "\n\n");
   add_item(({ "engraving", "herb" }), "\n"+
      "The engraving is simple in design, but there "+
      "is no mistaking that it depicts an herb of "+
      "some type. "+
      "\n\n");
   add_item(({ "wagon", "northeast" }), "\n"+
   ...
   add_exit(MRKT_PATH(sh_charm), "south");
...
reset_room()
{
   if(!ob1)
      {
      ob1 = clone_object(MRKT_PATH(npc/oldwoman));
      ob1 -> move_living("xxx", TO);
   }
   if(!ob2)
      {
      ob2 = clone_object(MRKT_PATH(obj/wagon))->move(TO);
   }
}
...
read(string str)
{
   if((str=="polished sign")||(str=="sign"))
      {
      write("The sign says:\n\n"+
         "         'Charms for all Occasions'\n\n");
      say(QCTNAME(this_player()) + " studies the "+
         "sign on the crimson tent to the south.\n");
      return 1;
   }
