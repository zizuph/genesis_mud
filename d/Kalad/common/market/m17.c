...
create_room()
{
   ...
   set_long("\n"+
      ...
      "\n\n"+
      "    There is plain looking tent to the north of "+
      "here. It has a sign attached to it. The entrance "+
      "to Serpent Street, which runs westward into the "+
      "city is to the west of here. "+
      "\n\n");
   
   add_item(({ "tent", "plain tent" }), "\n"+
      "This tent is made of an olive colored canvas "+
      "which, strangely, glistens brightly in the "+
      "intense sunlight. There is brightly polished sign "+
      "hanging over the doorway. "+
      "\n\n");
   add_item(({ "sign", "polished sigh" }), "\n"+
   ...
   add_exit(MRKT_PATH(sstreet01), "west");
   add_exit(MRKT_PATH(bowyer), "north");
   add_exit(MRKT_PATH(m12), "southeast");
   add_exit(MRKT_PATH(m13), "south");
   add_exit(MRKT_PATH(m16), "east");
   add_exit(MRKT_PATH(m20), "northeast");
}
...
read(string str)
{
   if((str=="polished sign"))
      {
      write("The sign says:\n\n"+
         "         'Bowyer'\n\n");
      say(QCTNAME(this_player()) + " studies the "+
         "brightly polished sign on the plain tent "+
         "to the north.\n");
      return 1;
   }
   write("Read what? The polished sign?\n");
   return 1;
}
