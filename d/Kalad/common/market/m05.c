...
create_room()
{
   ...
   set_long("\n"+
      ...
      "\n\n"+
      "    To the south is red tent. It has a "+
      "sign posted above the door which you "+
      "think you might be able to read. "+
      "\n\n");
   ...
   add_exit(MRKT_PATH(sh_clothes), "south");
   add_exit(MRKT_PATH(m01), "southwest");
   add_exit(MRKT_PATH(m10), "north");
   add_exit(MRKT_PATH(m11), "northwest");
}
...
read(string str)
{
   if((str=="polished sign"))
      {
      write("The sign says:\n\n"+
         "         'Daphne's Clothing Market'\n\n");
      say(QCTNAME(this_player()) + " studies the "+
         "sign on the red tent to the south.\n");
      return 1;
   }
