...
create_room()
{
   ...
   add_exit(MRKT_PATH(m20), "southwest");
   add_exit(MRKT_PATH(m22), "east");
   add_exit(MRKT_PATH(m24), "west");
   add_exit(MRKT_PATH(m27), "northwest");
}
...
reset_room()
{
   if(!ob1)
      {
      ob1 = clone_object(MRKT_PATH(npc/brigand));
