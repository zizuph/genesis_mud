...
create_room()
{
   ...
   set_long("\n"+
      ...
      "\n\n"+
      "    To the east is a brightly colored tent with "+
      "flowing letters painted across the face of it. "+
      "It has a regular flow of people entering and "+
      "leaving it. "+
      "\n\n");
   
   add_item(({ "tent", "bright tent", }), "\n"+
      "This tent is made of colorful canvas. "+
      "material. There is brightly polished sign "+
      "hanging over the doorway. "+
      "\n\n");
   add_item(({ "letters", "painted letters" }), "\n"+
      "The letters are painted in various bright "+
      "colors. They have a sweeping style to them. "+
      "You can easily make out what they say from "+
      "here. "+
      "\n\n");
   ...
   add_exit(MRKT_PATH(dine01), "east");
   add_exit(MRKT_PATH(m12), "south");
   add_exit(MRKT_PATH(m13), "southwest");
   add_exit(MRKT_PATH(m17), "west");
   add_exit(MRKT_PATH(m20), "north");
}
...
read(string str)
{
   if((str=="painted letters")||(str=="bright letters"))
      {
      write("The brightly painted letters read:\n\n"+
         "     'Pierre's Fine Kaladian Gourmet'\n\n");
      say(QCTNAME(this_player()) + " studies the "+
         "bright letters painted on the tent to the "+
         "east.\n");
      return 1;
   }
   write("Read what? The bright letters?\n");
   return 1;
}
