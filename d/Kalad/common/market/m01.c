...
create_room()
{
   ...
   add_item(({ "tents", "colorful tents" }), "\n"+
      ...
      "aren't completely covered in mud. Their many bright "+
      "colors are still obvious. It appears that most of the "+
   ...
   set_time_desc("    The crowds are tightly packed "+
      "right now. You feel somewhat claustrophobic "+
      "in these tight confines.\n\n", "    Considering "+
      "the time of day, there are still a large number of "+
   ...
   add_exit(MRKT_PATH(m02), "west");
   add_exit(MRKT_PATH(m05), "northeast");
   add_exit(MRKT_PATH(m06), "north");
   add_exit(MRKT_PATH(m07), "northwest");
}
