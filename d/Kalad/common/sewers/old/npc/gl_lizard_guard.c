...
create_monster()
{
    ...
    set_long("This is a proud tall lizardman. His eyes are thin and ever watching. " +
    ...
    set_stats(({85+random(3),100+random(5),100+random(5),50,40,80+random(10)})); 
    ...
    set_skill(SS_WEP_CLUB,60+random(5));
    set_skill(SS_DEFENCE,60 + random(4));
    set_skill(SS_PARRY,70 + random(4));
...
arm_me()
{
   ...
   MONEY_MAKE_GC(random(3)+3)->move(this_object(),1);
   MONEY_MAKE_SC(random(10)+8)->move(this_object(),1);
...
special_attack(object ob)
{
    ...
    if (query_combat_object()->cb_tohit(-1, 90 + random(20), ob) < 1)
        return 0;

    hitres = ob->hit_me(random(170)+170,W_BLUDGEON,TO,-1);
    phurt = "";
    switch(hitres[0])
    {
        case 1..20:
