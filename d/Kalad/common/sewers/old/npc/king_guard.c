/*     Created by:      Sir Toby, 97-07-24
 *     Location:        Lizard kings elite guards. 
 *     Modified:        Sir Toby, 97-09-17 (added the slap function)
 */
...
#define BODY	"/d/Kalad/common/sewers/arm/black_scalemail.c"
...
#define SEWERS_HAS_DISHONOURED "_sewers_has_dishonoured"
...
create_monster()
{
    ...
    set_short("lizard king's elite guard");
    set_pshort("lizard king's elite guards");

    set_long("This is a proud tall lizardman, guarding the Lizard King. " +
	     "His eyes are thin and ever watching. Though he looks like " +
	     "a man he has a tail. His skin is black.\n");
    ...
    set_stats(({119+random(3),125+random(5),115+random(5),50,40,90+random(10)})); 

    set_alignment(-100);
    set_knight_prestige(50);   

    set_skill(SS_AWARENESS,60 + random(4));
    set_skill(SS_BLIND_COMBAT,80 + random(4));
    set_skill(SS_WEP_SWORD,80 + random(4));
    set_skill(SS_WEP_CLUB,70);
    set_skill(SS_DEFENCE,60 + random(4));
    set_skill(SS_PARRY,60 + random(4));
    set_skill(SS_UNARM_COMBAT,70 + random(4));
...
arm_me()
{
   ...
   MONEY_MAKE_PC(random(3))->move(this_object(),1);
   MONEY_MAKE_GC(random(10)+2)->move(this_object(),1);
...
//------------ REACT TO BEING KICKED OR SLAPPED ------
...
{
	if(actor->query_prop(SEWERS_HAS_DISHONOURED) > 0)
	{
	   command("say You again! You will regret this dishonourable act!");
	   actor->remove_prop(SEWERS_HAS_DISHONOURED);
	...
	switch(emote)
        {
	case "slap":
		actor->add_prop(SEWERS_HAS_DISHONOURED,1);
		command("say Hey, you! Stop that, one more time and you will die");
		command("frown");
	break;
