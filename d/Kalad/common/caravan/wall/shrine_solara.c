...
create_cvan_room()
{
    ...
    add_item(({ "sun symbol", "symbol", "symbol of the sun" }),
      "\nIt is fashioned from pure gold and sculpted into the likeness "+
      "of the sun. The Symbol appears to gather in light and reflects it "+
...
{
    if(!ob1)
	(ob1 = clone_object(NPC + "s_priestess"))->move_living("xxx", 
	  this_object());
}
