...
create_room()
{
   ...
   add_item(({"large winch","winch"}),"A large metal rod used to open up the "+
      "Caravan Gate.\n");
   hear_bell = 2; /*normal: outdoors in the caravan dist*/
   add_exit(CVAN(caravan/s1),"south",0);
   add_exit("/d/Kalad/common/caravan/test2", "west");
   add_exit("/d/Kalad/common/caravan/gateway/s5", "east",0,0);
   add_exit("/d/Kalad/common/caravan/caravan/test2", "in",0,-1,-1);
   door = clone_object("/d/Kalad/common/caravan/door/cgdoor");
   door -> move(TO);
...
climb(string what)
{
   ...
   TP->move_living("climbing over the gate.","/d/Kalad/common/wild/pass/w1",1,0);
   return 1;
}
