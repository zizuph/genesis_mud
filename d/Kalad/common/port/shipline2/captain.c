...
create_monster()
{
    ...
    set_name("engels");
    add_name("captain");
    set_living_name("engels");
    set_race_name("human");
    set_adj("philosophical");
    add_adj("thoughtful");
    set_stats(({100,125,100,90,150,75}));
    set_alignment(150);
    ...
    add_act("say The sea is an unpredictable thing...");
    add_act("say One must constantly be alert when travelling the deep blue sea.");
    add_act("ponder");
    add_act("say The sea is plentiful in its bounty, as well as in its danger.");
    set_title("the Wizened Seafarer");
}
...
buy(string str)
{
    ...
    buy_ob=clone_object(PORT + "shipline2/ticket");
    if (!buy_ob) {
