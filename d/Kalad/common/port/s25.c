...
create_port_room()
{
    ...
    set_long("This large wooden enclosure is the interior of one of the "+
      ...
      "high up in the northern wall. All about you are a myriad number of "+
    ...
    add_item(({"large wooden crates","large crates","wooden crates","crates","crate"}),
      ...
      "it belongs to.\n");
    add_item(({"symbol","mark"}),"An image of a blue tidal wave "+
      "has been placed on each crate.\n");
    add_item(({"northern wall","wall"}),"It holds up the warehouse.\n");
    add_exit(PORT + "s9", "north");
}
