...
void create_armour()
{
	...
	"horns tapering together, forming a circle with a small space between "+
	"them, which allows the earring to be attached to an ear. The bronze is "+
	"polished to rich lustrous sheen, and the broadest part is engraved with "+
	"some pattern.\n"
	);
...
public mixed remove(object ob)
{
	...
	if (objectp(tp) && activated == 1)
	{
		...
		activated=0;
		if (noted > 0)
			tell_object(tp,"As you remove the "+short()+", the surrounding "+
				"sounds become bleaker and duller.\n");
	}
	return 0;
}
