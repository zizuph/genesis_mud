...
reset_room()
{
	...
	tell_room(TO,"Someone small bumps into you from behind.\n");
    }
}
...
create_port_room()
{
    ...
    set_long("You are standing on a long wooden pier that sticks out "+
      "into the bay. An enormous ship is docked directly to the east. The "+
    ...
    add_exit(PORT + "s82", "north");
    add_exit(PORT + "s86", "south");
    reset_room();
}
