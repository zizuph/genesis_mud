...
reset_room()
{
	...
	tell_room(TO,"You feel a slight bump from the back.\n");
    }
}
...
create_port_room()
{
    ...
    set_long("You are standing on a long wooden pier that sticks out "+
      "into the bay. An enormous ship is docked directly to your north. The "+
    ...
    add_exit(PORT + "s89", "west");
    add_exit(PORT + "s91", "east");
