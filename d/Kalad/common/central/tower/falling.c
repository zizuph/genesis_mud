...
land(object ob)
{
   ...
   tell_room("/d/Kalad/common/central/tower/landing","You hear a horrible noise from above, "+
      "suddenly you see ");
   
   if (living(ob))
      {
      tell_room("/d/Kalad/common/central/tower/landing",QTNAME(TP) + " slam into the ground with terrible force!\n");
      
      ob->move("/d/Kalad/common/central/tower/landing");
      
      tell_object(ob, "You slam into the ground with terrible force!\n");
      ob->heal_hp(-210);
      call_other(CENTRAL(high-circle-south),"patrol_text","and "+
      capitalize(ob->query_real_name())+" tried to kill "+
      ob->query_objective()+"self today. What a fool!");
      ...
      {
      tell_room("/d/Kalad/common/central/tower/landing", LANG_ASHORT(ob) + " fall down.\n");
      ob->move("/d/Kalad/common/central/tower/landing");
   }
}
