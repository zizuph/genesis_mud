...
create_room()
{
   ...
   set_long("You find yourself in a plain room with few noticeable "+
      ...
      "You see two similar rooms to the southwest and west, while the "+
      "northeastern way leads into an archway.\n");
   
   add_exit(CENTRAL(citadel/cit2),"west",0,-1,-1);
   add_exit(CENTRAL(citadel/entrance),"northeast",0,-1,-1);
   add_exit(CENTRAL(citadel/cit4),"southwest",0,-1,-1);
