...
create_room()
{
    ...
    set_long("You gasp when you watch the size of this room. A large "+
      ...
      "meets selections of his people en masse. To the north you see a podium "+
      "with a huge throne in the centre. If you walk east or to the "+
      "southeast, you will continue further along this huge dome.\n");
    ...
    add_exit(CENTRAL(citadel/cit11),"north",0,-1,-1);
    add_exit(CENTRAL(citadel/cit9),"east",0,-1,-1);
    add_exit(CENTRAL(citadel/cit7),"southeast",0,-1,-1);
...
reset_room()
{
	...
	ob2 -> arm_me();
	ob2 -> move_living("A guard arrives to do his duty.", this_object());
    }
    return;
}
