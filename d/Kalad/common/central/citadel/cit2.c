...
create_room()
{
   ...
   set_long("You find yourself in a plain room with few noticeable "+
      ...
      "guards lives when they are not on duty, earning their pay. "+
      "You see three similar rooms to the south, east and west.\n");

   add_exit(CENTRAL(citadel/cit1),"east",0,-1,-1);
   add_exit(CENTRAL(citadel/cit3),"west",0,-1,-1);
   add_exit(CENTRAL(citadel/cit4),"south",0,-1,-1);
