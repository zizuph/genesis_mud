...
create_room()
{
   ...
   set_long("You find yourself in a plain room with few noticeable "+
      ...
      "You see three similar rooms to the northeast, northwest and north.\n");

   add_exit(CENTRAL(citadel/cit2),"north",0,-1,-1);
   add_exit(CENTRAL(citadel/cit1),"northeast",0,-1,-1);
   add_exit(CENTRAL(citadel/cit3),"northwest",0,-1,-1);
