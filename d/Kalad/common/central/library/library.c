/* library.c
   ...
   Modified by Antharanos in 1995 for the Kabal library
 ...
 */
void set_librarian_room(string room)
{
  librarian_room = room;
  secure_add = 1;
}
 ...
 */
public void no_lib_init(int flag)
{
  noinit=flag;
}
 ...
 */
public void remove_book(string book)
{
   int i, j;
 ...
 */
public void set_save_path(string s)
{
  savepath = s;
 ...
 */
public void set_book_type(string s)
{
  booktype = s;
}
 ...
 */
public void set_list_cmd(string s)
{
  listcmd = s;
}
 ...
 * Arguments:   the command to use
 */
public void set_return_cmd(string s)
{
  returncmd = s;
}
 ...
 * Arguments:   the command to use
 */
public void set_borrow_cmd(string s)
{
  borrowcmd = s;
}
 ...
 * Arguments:   libname
 */
public void set_lib_name(string s)
{
  libnam = s;
}
 ...
 */
public void add_book(string name, string descr, string path, int flag)
{
  int i,tt;
  string t;
  ...
  else /* else link the stuff in at correct places in the lists */
    {
      names += ({ name });
      paths += ({ path });
      descrs += ({ descr });
 ...
 */
public void lib_exit(string path, string direction)
{
 ...
 */
public nomask int libblock()
{
  if(TP->query_prop(BORROWED_BOOK+libnam))
    {
      TP->catch_msg(
  "The librarian won't let you pass until you return the " + booktype + ".\n");
...
void writebook(string s)
{
  if(s=="**")
    {
      write_file(fnam,"\nWritten by "+capitalize(this_player()->query_real_name())+".\n");
      busy = 0;
      if(!secure_add)
	{
	  write("You finish your "+booktype+" and put it on one of the shelves.\n");
      ...
      else
	{
      ...
      say(QCTNAME(this_player())+" finishes a "+booktype+" and adds it to the library.\n");
      /* finish book */
    }
  else
    if(s=="~q")
	...
	say(QCTNAME(this_player())+" stops writing and throws the "+booktype+
	    " away.\n");
	/* abort book */
	bnr--;
	busy = 0;
...
public int list()
{
  ...
  TP->catch_msg(
       "The " + booktype + "s you can borrow in " + libnam + " are:\n");
