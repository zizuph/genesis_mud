/* 
 * /d/Kalad/common/central/arena/outer_circle16.c
...
void create_room()
{
   ...
   set_long ("You walk below the outer archway of the arena. "+
      ...
      "To the northeast, a broad stairways of stone ascend upwards."
   );
   ...
   add_exit (CENTRAL(arena/outer_circle15),"southeast",0,1);
   add_exit (CENTRAL(arena/outer_circle1),"northwest",0,1);
   add_exit (CENTRAL(arena/entrance_arena),"southwest",0,1);
   add_exit(CENTRAL(arena/upper_circle1),"up","@@stop",-1,-1);
...
stop()
{
   write("A guard stops you and says: The upper circle is "+
      "not for common people.\n");
   say(QCTNAME(TP)+" is stopped from going up.\n");
   return 1;
}
