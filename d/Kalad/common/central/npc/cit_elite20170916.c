...
help_friend(object ob)
{
   if (ob && !query_attack() && present(ob, environment()))
      ...
      command("shout we are under attack! Warn the Lord!!\n");
   }
}   
...
do_assist()
{
   ...
   {
   case 0:
      command("shout I can't leave my post. Just kill'em!");
      command("sigh");
      break;
   case 1:
      ...
      break;
   case 2:
      command("shout earn you pay! Splatter them.");
      command("emote peers nervously around.");
      break;
   default:
      command("shout We will make sure noone else will enter. You just "+
         "kill the bastards!");
      break;
   }
}
...
found_player(string who)
{
   ...
   if (ob = present(dummy[0], environment()))
     ...
     command("shout we are under attack by an assassin!");
   }
}
...
special_attack(object enemy)
{
   ...
   if (random(6)>0)
      return 0;
   if (!shouted) command("shout we are under attack!");
   shouted = random(10);
