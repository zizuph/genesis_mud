...
#define BASE ("/d/Kalad/common/central/")
#define WEP  (BASE + "wep/")
#define ARM  (BASE + "arm/")
...
create_monster()
{
   ...
   set_long("This thin human seems to be an important man to the High " +
      ...
      "covered in a @@robe_desc@@. All you notice from him is his " +
   ...
   equip(({
        clone_unique(WEP + "councellor_wand", 10, WEP + "councellor_dagger"),
        clone_unique(ARM + "councellor_robe", 10, ARM + "councellor_robe_alt")
        }));   
...
robe_desc()
{
    object robe = this_object()->query_armour(A_ROBE);
    if (robe) 
        return robe->short();
    return "robe";
}
