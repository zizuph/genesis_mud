...
create_room()
{
   ...
   set_long("You walk along the circle road, a road paved with large "+
      ...
      "while the road continues north and southwest.\n");
   ...
   add_exit(CENTRAL(circle/s12),"southwest",0,1);
   add_exit(CENTRAL(circle/s14),"north",0,1);
   add_exit(CENTRAL(plaza/sq_st_nw),"east",0,1);
