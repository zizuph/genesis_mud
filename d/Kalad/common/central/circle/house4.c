...
create_room()
{
   ...
   set_long(
      ...
      "an exit from where the smell of food wifts into your nose.\n");
   
   add_prop(ROOM_NO_TIME_DESC, 1);
   
   add_exit(CENTRAL(circle/s7),"out",0,-1,-1);
   add_exit(CENTRAL(circle/kitchen4),"south",0,-1,-1);
   ...
   if (str=="with child" || str=="with young human")
      ...
      if (ob2)
         ...
         say(QCTNAME(TP)+" sits down on the floor and plays some "+
            "with the child.\n");
         ob2->command("emote climbs up on a "+TP->query_nonmet_name()+
               "'s back.");
...
play1(object who)
{
   ob2->command("emote sits on a "+who->query_nonmet_name()+
    "'s shoulders.");
...
play3(object who)
{
   if (random(2)==0)
      {
      ob2->command("emote grips around a "+who->query_nonmet_name()+
        "'s head with his small hands.");
