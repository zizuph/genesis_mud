...
create_room()
{
    ...
    set_long("You walk along the circle road, a road paved with large "+
        ...
        "Council of the Lord's building, while on the other side you only see "+
    ...
    add_item(({"window","windows"}),"You see six of them, but you can "+
        "only reach the three lowest ones. It looks like all of them are "+ 
        " closed.\n");
    ...
    add_exit(CENTRAL(high-circle-north),"west",0,1);
    add_exit(CENTRAL(circle/s3),"southeast",0,1);
}
