...
create_room()
{
   ...
   set_long("You walk along the circle road, a road paved with large "+
      "bricks of stone. To the east you see Caravan street, "+
      "while the circle street continues northwest and southwest.\n");
   
   add_item(({"brick","bricks"}),"Most of the bricks are hole, but some "+
      "are craced up, leaving smaller pieces that you might be able to "+
      "pry loose.\n");
   
   add_exit(CENTRAL(circle/s6),"southwest",0,1);
   add_exit(CENTRAL(circle/s4),"northwest",0,1);
   add_exit(CENTRAL(caravan-circle),"east");
}
