...
#define MAXLIST 30
#define NUM 4
#define STORE_ROOM G_ROOMS + "storeroom"
...
create_room()
{
    ...
    set_short("Items of Protection, Death, and Destruction Aquisition Area");
    set_long( break_string(
"This is a very cluttered room containing various nasty looking items.  " +
"The shopkeeper is ugly, but he is willing to fatten his purse by selling " +
"you an item or two.  Additionally, if you have an item to sell to him, he " +
"is more than willing to turn around and sell it for twice as much to the " +
"next guy.  What do you expect from a shop in a den of thieves?  There is a " +
"sign here to read for more instructions.\n", 78));

    add_exit(G_ROOMS + "ghall3", "east", 0);
    add_exit(G_ROOMS + "storeroom", "west", "@@wiz_check");

    add_item(({"sign", "instructions"}), break_string(
"Well, lets see.  It is constructed of wood, painted red, with yellow " +
"letters.  The letters were carved into the wood carefully, implying a " +
"sense of great craftmanship.  This makes the sign very easy to READ!\n", 78));
...
wiz_check()
{
    if (this_player()->query_wiz_level())
      	return 0;
    write("The shopkeeper prevents you from going west.\n");
...
do_read(string str)
{
	...
	"                'value' or 'buy' too. However, you will never buy\n"+
	"                more than one item at a time.  Beware!\n" +
...
buy_it(object *ob, string str2, string str3) 
{
	...
	if (error = arr[sizeof(arr) - 1])
	{
	    ...
	    /* Couldn't take the money from player, the coins were stuck */
	       write("You are carrying funny money, scumbag!  No deal!.\n");
		continue;
	    }
