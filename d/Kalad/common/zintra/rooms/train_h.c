...
/*
     Training room for members of the Order of Hyrklan.
...
void create_room()
{
   ...
   set_long("\n"+
      "This is the training area for Assassins of the Order of Hyrklan.  "+
      ...
      "High BlackHand of Hyrklan here.  He is willing to instruct you in "+
   ...
   /*  Need to create an npc... with improve and learn in his descrip.  */

   add_prop(ROOM_I_INSIDE, 1);
   add_prop(ROOM_I_LIGHT, 1);

   add_exit(ROOMS_DIR + "<room?>", "<direction?>", 0);
...
void set_up_skills()
{
   ...
   sk_add_train(SS_KICK, train_text(SS_KICK), "smashing high kick", 100, 100, 1);
   sk_add_train(SS_AWARENESS, train_text(SS_AWARENESS), "awareness", 0, 60);
   sk_add_train(SS_BLINDFIGHTING, train_text(SS_BLINDFIGHTING), "appraise monsters", 0, 90);
   ...
   sk_add_train(SS_WEP_SWORD, train_text(SS_WEP_SWORD), "sword", 0, 75);
...
sk_improve(string sk)
{
   if (!MEMBER(TP))
   {
      notify_fail("You are NOT an assassin! You can't train here.\n");
   
      if (sk)
         return 0;
   
      write("Only the members of the Order of Hyrklan may train here.\n");
   }

   if (ZSERVER->query_order_name(TP->query_real_name()) != "hyrklan")
   {
      notify_fail("You are not of the Order of Hyrklan. You are not "+
