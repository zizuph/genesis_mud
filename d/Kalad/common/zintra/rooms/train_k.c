...
/*
     This is the training room for the Order of Kolnari
...
set_up_skills()
{
   ...
   sk_add_train(SS_WEP_CLUB, train_text(SS_WEP_CLUB), "club",0, 90);
...
string *train_text(int skill)
{
   ...
   switch (skill) 
   {
      ...
      case SS_ZKICK:
         what = "throw a smashing high kick an opponent";
         does = what;
      break;
...
sk_hook_raise_rank(int snum, int to_lev, int cost)
{
   ...
   this_player()->catch_msg("You improve your ability to " +
                             sk_tdesc[snum][0] + ".\n" +
                             "You achieve the ranked train_s.c
a
