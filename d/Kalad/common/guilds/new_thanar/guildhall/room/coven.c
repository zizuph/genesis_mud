...
create_std_thanar()
{
    ...
    set_long("This is the upper vestibule. It is just as "+
      ...
      "the same marvellous marble. There are six war banners "+
      "that hang upon the walls each denoting a given Coven "+
      "of Thanar. A large marble table is erected in the "+
      "centre of the room with a golden plaque placed upon "+
      "its surface.\n");
    add_item(({ "vestibule", "room", "area", "chamber"}), query_long);
    add_item(({"plaque", "golden plaque"}), "You can read it.\n");
    ...
    add_item(({"second banner"}),
      "This banner is faintly worn although the workmanship "+
      "has been elaborately done. There is an image depicted "+
      "of a human warrior wearing the colours of sapphire and "+
    ...
    add_item(({"fifth banner"}),
      ...
      "has been elaborately done. There is an image depicted "+
      "of a human warrior wearing the colours of sapphire and "+
      "gold. He is surrounded by a bloodied battlefield but stand victorious. "+
    ...
    add_exit("join", "west");
    set_no_exit_msg(get_all_no_exits(({"west"})),
...
/*
 * Function name: choose_cover(string)
 * Description  : Allows the player to select a coven to join
 * Variables    : String is the name of the coven
 */
public int
