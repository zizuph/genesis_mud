/*
 * /d/Kalad/common/guilds/new_thanar/spells/obj/deity_stun_obj.c
 * Stun spell used by the Deity Thanar in Kalad.
 * 
...
create_effect()
{    
    set_name("deity_spell_stun_object");
    add_name( ({"deity_spell_stun_object" }) );
...
attack_blocked(object player)
{
    ...
    if (enemy)
    {
        ...
        player->catch_msg("You attempt to attack " + QTNAME(enemy)
                         + ", but you are to terrified to do so.\n");
