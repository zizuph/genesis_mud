 ...
 * /d/Kalad/common/guilds/new_thanar/rooms/upper1.c
 */
...
create_std_thanar()
{
    ...
    add_exit(RDIR + "upper2","east");
    add_exit(RDIR + "upper_left_path1","northeast");
	add_exit(BASE + "se_basement1","down");
    
    //sets the no exit message for unlisted directions
    set_no_exit_msg(get_all_no_exits(({"w","ne","d"})),
