 ...
 * /d/Kalad/common/guilds/new_thanar/rooms/upper20.c
 */
...
create_std_thanar()
{
    ...
    add_exit(RDIR + "upper19","west");
    add_exit(RDIR + "upper_right_path3","northwest");
	add_exit(BASE + "sw_basement1","down");
    
    //sets the no exit message for unlisted directions
    set_no_exit_msg(get_all_no_exits(({"w","nw","d"})),
    "The large wooden pews block your path in that direction.\n");
    
    //adds the npc and # of them, to the room and arms them.
    add_npc(LIV +"cpriest", 1, &->arm_me());
