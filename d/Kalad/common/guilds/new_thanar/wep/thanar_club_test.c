...
create_weapon() 
{
    ...
    set_name(({"club","warhammer"}));
    set_adj("blackened");
    add_adj("magnificent");
    set_short("blackened magnificent warhammer");
...
did_hit(int aid,string hdesc,int phurt,object enemy,int dt,int phit,int dam)
{
    ...
    *ment_stats2;
    ...
    > 5))
    {
        ...
        enemy->catch_tell("You feel the " +short()+" hits harder than "+
        "expected.\n" );
        wielder->catch_tell("You feel the "+short()+" hits harder than "+
        "expected.\n");
        wielder->tell_watcher(QTNAME(wielder)+" grins with grim satisfaction "
        +"as the "+short()+" smashes into "+QTNAME(enemy)+".\n");
