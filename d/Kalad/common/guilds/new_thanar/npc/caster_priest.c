...
special_attack(object enemy)
{
  ...
  object *enemies, enemy, kill_enemy, me;
    ...
    {
       if(!("/d/Kalad/common/guilds/new_thanar/spells/tresist"->has_spell_object(TO)))
       {
    ...
    if(!random(5))
    {
        if(!random(2))
        {
             command("cast tblind " + enemy->query_real_name()); 
             return 1;           
        }
        else
        {
             command("cast tstun " + enemy->query_real_name()); 
             return 1;   
        }
    }
    
    //See if we want to cast a Drain or Damage - Always damage for undead
    if (!random(5))
    {
        if(!random(2))
           {
               command("cast tdamage " + enemy->query_real_name()); 
               return 1;
           }
       ...
       command("cast tdrain " + enemy->query_real_name()); 
       return 1;    
    }
} /* special_attack */
