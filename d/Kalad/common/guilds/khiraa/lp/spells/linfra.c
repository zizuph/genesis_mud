/*
 * /d/Kalad/common/guilds/khiraa/lp/spells/linfra.c
...
void
do_spell_linfra(object caster, object *targets)
{
    object target, *sh;
    int time, power;
    target = targets[0];

    time = 600 + caster->query_stat(SS_WIS);

    power = (caster->query_guild_order()/6);
...
int
create_spell_linfra(object caster, object *targets, string arg)
{
    ...
    set_spell_effect(do_spell_linfra);
