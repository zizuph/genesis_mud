 ...
 *
 * : Sarr 8.Jul.97
 *             Added so wizards don't have to have components.
 *             
 */
#pragma strict_types
...
hook_no_skills_fail()
{
    return "Sorry, but you are not skilled enough for this prayer!\n";
}
...
hook_no_mana_fail()
{
    return"You don't have mana enough to properly invoke this prayer.\n";
}
...
{
    return "You don't have the proper sacrifices for this prayer.\n";
}
...
{
    return "You sacrificed " + COMPOSITE_DEAD(ingrs) + " to Him.\n";
}
...
check_ingredients()
{
    ...
    /* Wizards don't have to use ingredients */
    if(gCaster->query_wiz_level())
        return 1;
    /* Use filter here. */
    if ((i = sizeof(gIngredients)))
        while (i--)
        {
        if(parse_command(gIngredients[i], deep_inventory(gCaster),
			      "%i", items))
	    {
...
cast_spell()
{
    ...
    /* Check Ingredients once again */
    if(!check_ingredients())
    {
        gFound_ingrs->remove_prop(TEMP_I_HEAP_COUNT);
        gCaster->catch_msg(hook_no_ingredients_fail());
        return 0;
    }
