/* 
 * /d/Kalad/common/guilds/khiraa/temple/rooms/herb_shop.c
 * Purpose    : Herb shop of the Guild. Contents restore every boot.
...
inherit "/lib/shop.c";
inherit  "/d/Kalad/common/guilds/khiraa/temple/rooms/hshop_list"; 

#define STORE_ROOM "/d/Kalad/common/guilds/khiraa/temple/rooms/herb_store"
...
create_room()
{
    ...
    set_short("In the study room");
    
    set_long("\n   You have entered a small chamber in the "+
    "temple of the Khiraa. The floor is made of solid black "+
    "marble and reflects no light. The walls and ceiling are "+
    "made of solid black stone.\n"+ 
    "   The room contains a large "+
    "table in the center, surrounded by many stone chairs. "+
    "There are some shelves on one side of the room. This "+
    "is probably where you can come and work on spells or "+
    "study up on ancient texts.\n\n");    

    add_item("table","The table is made of solid black marble. "+
    "In the center you see the emblem of the Khiraa, a green "+
    "grim-looking skull. Also on the table, you notice a "+
    "strange red pen.\n");
    add_item(({"pen","red pen"}),"This pen seems to be filled "+
    "with an eerie red ink that glows slightly. It is made "+
    "to write on glass. If you had a glass jar with you, "+
    "you could probably 'label jar with <name>' so you could "+
    "perhaps keep track of what jars have what in them.\n");
    add_item("chairs","The chairs are made of stone and don't "+
    ...
    clone_object(KHIRAA_DIR(temple/obj/shelves))->move(TO);

    werners = clone_object(KHIRAA_DIR(temple/obj/werners));
    werners->move_living("xxx",TO);
    clone_object(KHIRAA_DIR(temple/doors/studydoor2))->move(TO);
    call_other(STORE_ROOM,"??");
...
do_label(string str)
{
    ...
    string jar,label;
    NF("Label what?\n");
    if(sscanf(str,"%s with %s",jar,label) != 2)
        return 0;
    obs = FIND_STR_IN_OBJECT(jar,TP);
    if(sizeof(obs))
        ob = obs[0];
    if(!objectp(ob))
        return 0;
    NF("You can't label that.\n");
    if(MASTER_OB(ob) != LP_DIR(obj/jar) )
        return 0;
    write("Using the red pen, you lable the "+ob->short()+" with "+
    "the label '"+label+"'.\n");
    say(QCTNAME(TP)+" writes something on "+HIS(TP)+" "+ob->short()+
    " with the red pen on the table.\n");
    ob->set_label(label);
    return 1;
}

object *
setupstats(object *items,object me)
{
    int i;
    for(i=0;i<sizeof(items);i++)
        items[i]->set_mixer_stats(({ me->query_stat(SS_INT), 
           me->query_stat(SS_WIS), me->query_stat(SS_OCCUP)}));
    return items;
}
...
shop_hook_allow_sell(object ob)
{
    ...
    {
        werners->command("emote hisses: I will only buy herbs and "+
        "potions!");
        return 0;
    }
    return 1;
...
{
    werners->command("emote hisses: I don't have a "+str+"!");
    return 1;
}
...
{
    werners->command("emote hisses: I don't have that many "+str+"!");
    return 1;
}
...
shop_hook_bought_items(object *arr)
{
    ...
    werners->command("emote opens his chest and gets out "+
    COMPOSITE_DEAD(arr)+".");
    ...
    save_map(temp,KHIRAA_DIR(log/buy_log));

    werners->command("emote hisses: A pleasure.");
    write(werners->short()+" gives you "+COMPOSITE_DEAD(arr)+".\n");
    return 1;
}
...
shop_hook_sold_items(object *item)
{
    ...
    werners->command("emote opens his chest and puts the new herbs "+
    "in them.");
    ...
    werners->command("emote hisses: Ah, a pleasure.");
    return 1;
}
...
do_buy(string str)
{
    ...
    if(sizeof(item) && item[0]->query_potion_name())
        item = setupstats(item,TP);
    if (sizeof(item))
    {
        qty = sizeof(item);
        qty_old -= qty;
