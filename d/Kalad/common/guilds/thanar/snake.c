...
void create_creature()
{
   ...
   set_long("This small black snake stares at you with tiny red eyes. It "+
      ...
      "You can command it to send with 'send <player's name>'\n");
...
void return_home()
{
   ...
   tell_room(room,"The "+query_short()+" slithers away.\n");
   remove_object();
}
...
int deliver()
{
   ...
   if (target == find_player(gSender))
      ...
      "slithers out of nowhere, slides up your leg, and coils up your "+
      "arm. It gives you a message from "+capitalize(gSender)+".\n");
   
   tell_room(environment(target),
      "A "+query_short()+" slithers out of nowhere, slides up "+QTNAME(target)+
      "'s leg and coils around "+HIS(target)+" arm, giving "+HIM(target)+" a "+
      "message.\n",target);
