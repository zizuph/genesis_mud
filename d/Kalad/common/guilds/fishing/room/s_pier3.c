...
create_std_southpier()
{
   ...
   set_long("You are on a large pier in the southern port area. It was made "
	...
	add_exit(F_ROOM + "s_pier2.c", "north");
	add_exit(F_ROOM + "s_pier4.c", "east");
	add_exit(F_ROOM + "s_pier6.c", "west");
	add_exit(F_ROOM + "f_joinroom.c", "south");
 
    /*************************************************************************
   *  Sets the no exit message for unlisted directions
    *************************************************************************/
   set_no_exit_msg(get_all_no_exits(({"n","s","e","w"})),
