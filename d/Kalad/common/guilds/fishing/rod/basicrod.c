 /*****************************************************************************
*  /d/Kalad/common/guilds/fishing/rod/basicrod.c
*
*  Rod File: Basic Rod
*
...
create_object()
{
   ...
   set_adj("basic");
   add_adj("fishing");
   set_short("basic fishing rod");
   set_long("It isn't much to look at but this basic rod is functional at "
	+ "least. You're not sure what it's made from but it looks as if someone "
	+ "cut a branch off a tree and attached an old reel, some fishing line "
	+ "and a rusty hook to it. With some luck you should still be able to "
	+ "catch something at the very least.\n");
...
release()
{
   ...
   if(Casted)
	{
		return "Your line is cast, reel in first!\n";
	}
...
do_drop(string str)
{
   ...
   if(parse_command(str, TP, " [the] [fishing] [basic] 'rod' "))
   {
		write("testing\n");
		Casted = 0;
		Reeling = 0;
...
cast_rod(string str)
{
   ...
   FISH_PLAYER(6.0,"Your line is cast, you should <reel rod> to try "
   + "and catch something.\n");
