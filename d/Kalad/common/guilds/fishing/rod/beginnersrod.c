 /*****************************************************************************
*  /d/Kalad/common/guilds/fishing/rod/beginnersrod.c
*
*  Rod File: Beginners Rod
*
...
public int          attach_lure(string str);
...
create_object()
{
   ...
   set_adj("beginners");
   add_adj("fishing");
   set_short("beginners fishing rod");
   set_long("It isn't much to look at but this beginners rod is functional "
	+ "at least. You're not sure what it's made from but it looks as if someone "
	+ "cut a branch off a tree and attached an old reel, some tangled line and a rusty "
	+ "hook to it. With some luck you should still be able to catch something at "
	+ "the very least.\n");
...
reel_rod(string str)
{
   ...
   switch (reel_chance)
   {
      ...
      say(QCTNAME(this_player()) + " yanks up hard on their line but it "
		+ "snaps!\n");
      ...
      say(QCTNAME(this_player()) + " reels in their line all the way to the docks.\n");
      Reeling = 1;
...
attach_lure(string str)
{
   ...
   if (str != "basic lure")
   {
      notify_fail("Attach what exactly?\n");
      return 0;
   }     
    
   flure = filter(all_inventory(this_player()), &->id("_fishing_lure"));
   if(!sizeof(flure))
   {
      notify_fail("You don't seem to have any lures on you.\n");
      return 0;
   }
    
   elure = present("_basic_lure", this_player());
    
   if(!objectp(elure))
   {
      notify_fail("You don't seem to have any e on you.\n");
      return 0;
   }
    
   if(Lured)
   {
      notify_fail("You already have a lure on your hook\n");
      return 0;
   }
        
   elure->remove_object();
   write("You attach a lure to the end of your line.\n");
   say(QCTNAME(this_player()) + " attaches a lure to their fishing rod.\n");
...
cast_rod(string str)
{
   ...
   if (!Lured)
   {
      notify_fail("You must first attach a lure!\n");
      return 0;
   }
