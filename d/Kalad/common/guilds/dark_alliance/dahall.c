...
create_room()
{
    ...
    INSIDE;
    add_exit(COMMON(guilds/dark_alliance/joinroom),"up",0,-1,-1);
    add_exit(COMMON(guilds/dark_alliance/dashop),"north",0,-1,-1);
    add_exit(COMMON(guilds/dark_alliance/rstable),"east",0,-1,-1);
    add_exit(COMMON(guilds/dark_alliance/rtrain),"west",0,-1,-1);
    add_exit(COMMON(guilds/dark_alliance/start),"south",0,-1,-1);
    add_exit(COMMON(guilds?dark_alliance/dacommon),"common",0,-1,-1);
    clone_object(COMMON(guilds/dark_alliance/da_board))->move(TO,1);
}
