...
/* 
 * /d/Kalad/common/guilds/dark_alliance/rstore.c
 * Purpose    : store room for rdashop.c(used store room exapmles)
...
create_room()
{
   ...
   set_long("You are in the store room of the Dark Alliance guild.\n");
   add_exit("/d/Kalad/common/guilds/dark_alliance/dashop", "south",0,-1,-1);
   reset_room();
}
 ...
 */
mixed to_check = ({ ({ "create_container", BAG, 3 }),
({ "create_torch", TORCH, 3 }),
({ "create_container", PACK, 10 }),
({ "create_object", LAMP, 4 }),
({ "create_object", OIL, 8 }),
                 });
