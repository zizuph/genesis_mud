/* 
 * /d/Kalad/common/wild/pass/goblin/guild/dark_alliance/joinroom.c
...
create_room()
{
   ...
   "There is a large sign boldy claiming: 'MEMBERS ONLY', hanging "+
   ...
   add_exit(CPASS(goblin/guild/joinroom),"up");
    add_exit(COMMON(guilds/dark_alliance/dahall),"down");
...
check_member()
{
   ...
   for(i = 0; i < sizeof(ob); i++)
    if(ob[i]->id("_rider_") || ob[i]->query_wiz_level())
      {
      return 0;
   }
   write("You try to head downstairs, but are pushed backwards by a "+
         "crowd of rowdy, yelling warriors of the Dark Alliance.\n");
   say(QCTNAME(TP) + " tries to enter the Dark Alliance guildhall, but is pushed back "+
...
join(str)
{
   ...
   if(str == "in")
      ...
      if(!collar)
         {
         collar = clone_object(COMMON(guilds/dark_alliance/rcollar));
         collar -> move(TP);
...
update(string str)
{
   ...
   if(collar == 0)
   {
       clone_object("/d/Kalad/common/guilds/dark_alliance/rcollar")->move(TP);
   }  
