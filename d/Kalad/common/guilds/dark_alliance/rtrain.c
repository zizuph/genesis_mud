...
create_room() 
{
   ...
   set_short("Dark Alliance Training Area");
   ...
   "wall. The smell of food comes from the north. Above the sothern "+
   "exit to this room you see a sign with a picture of a bright and "+
   "shining sword on it, that must be the armoury. You can see people "+
   "training in this area, mounting wolves, hiding from eachother, "+
   "trying to control their warwolves, and swords with "+
   "stunning ability.\n");
   ...
   add_exit(COMMON(guilds/dark_alliance/dahall),"east",0,-1,1);
   add_exit(COMMON(guilds/dark_alliance/dabar),"north",0,-1,1);
   add_exit(COMMON(guilds/dark_alliance/dashop2),"south",0,-1,1);
...
train_text(skill) {
   ...
   switch(skill) {
      case SS_PARRY:
      what="deflect attacks with a weapon";
      break;
      case SS_WOLF_RIDING:
      ...
      what="handle a warwolf in battle, and correctly control the attacks";
      break;
