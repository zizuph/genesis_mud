/*
   The new stables for the Red Fang racial guild.
   Modified by Zignur October 2017

   20210731 Meton Changed stabling/fetching wolf from setting wolf
        at 0 hp and 0 fatigue, to using timer like fsummon/fdismiss.
        Changed sign to represent this instead of promising instant refresh
*/
...
wolf_sign()
{
    ...
    "|  list adjectives                              |\n"+
    "|                                               |\n"+
    "|  If you leave your wolf in the stables for a  |\n"+
    "|  while, he will be fully rested and fed.      |\n"+
...
do_send(string str)
{
    ...
    int wolf_fatigue = wolf->query_fatigue();
    TP->add_prop(WOLF_FATIGUE, wolf_fatigue);

    int dismiss_time;
    dismiss_time = time();
    TP->add_prop(WOLF_DISMISS_TIME, dismiss_time);
...
do_fetch(string str)
{
    ...
    if(me->query_prop(WOLF_DISMISS_TIME))
    {
        int dismiss_time = me->query_prop(WOLF_DISMISS_TIME);

        // after 10 minutes of dismissing the wolf is fully rested.
        if((dismiss_time + 600) < time())
            nwolf->set_fatigue(nwolf->query_max_fatigue());
        else
            nwolf->set_fatigue(me->query_prop(WOLF_FATIGUE));
    }
    else
    {
        nwolf->set_fatigue(nwolf->query_max_fatigue());
    ...
    nwolf->set_hp(nwolf->query_max_hp());
    // You can't use set_stuffed, and eat_food has limit on food size eaten
    int wolfhunger = (nwolf->eat_max() - nwolf->query_stuffed());
    int diff = (wolfhunger / 200);
    for (i = 0; i < diff; i++)
    {
        nwolf->eat_food(200);
    }
    nwolf->eat_food(nwolf->eat_max() - nwolf->query_stuffed());
...
idle_pets()
{
    ...
    if (objectp(wolf = find_object(FANG_DIR (obj/wolf))) &&
        pointerp(object_clones(wolf)))
        mounts = filter(object_clones(wolf), &operator(>)(,3600) @
                        &->idle_time());
    
    for (i=0; i < sizeof(mounts);i++)
    {
         mounts[i]->remove_object();
    }
 }
