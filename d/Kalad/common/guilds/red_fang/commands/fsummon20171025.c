                ...
                int *resist, mixed *results)
{
    ...
    tell_room(environment(mount), "The " + mount->query_short() +
        " steps out of the shadows with a deadly gleam in"+
        " " + mount->query_possessive() + " eyes.\n", 0, mount);
     
    tell_room(environment(actor),
    capitalize(LANG_ADDART(mount->query_short())) +
        " steps out of the shadows with a deadly gleam in"+
        " " + mount->query_possessive() + " eyes.\n", 0, mount);
                   ...
                   int execute)
{
    ...
    {
        player->catch_msg("Your wolf refuse to answer your call after" +
            " your recent actions!\n");
        return 0;
    }    
