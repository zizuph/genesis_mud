...
create_kalad_monster()
{
    ...
    set_stats(({125,130,110,80,70,115}));
    ...
    unique_item(NOBLE + "wep/shimmering_mace",
        NOBLE + "wep/spiked_mace", 4)->move(TO, 1);
    add_equipment(({
        NOBLE + "arm/as_helm",
    ...
    switch (hitloc_id)
    {
       case A_HEAD:
        switch (random(2))
        {
           case 0:
            ...
            tell_room(ENV(TO), QCTNAME(TO) + " backhands " + QTNAME(ob) +
           ...
           case 1:
            ...
            tell_room(ENV(TO), QCTNAME(TO) + " deflects " + QTNAME(ob) + "'s " +
       ...
       case A_BODY:
        ob->catch_msg(QCTNAME(TO) + " slams his knee into your " +
            "stomach!\n");
        tell_room(ENV(TO), QCTNAME(TO) + " slams his knee into " +
            QTNAME(ob) + "'s stomach!\n", ob);
        break;
    }
}
