// 2021-11-07 Meton Changed value to 510, to mirror single_stud
inherit "/std/object";
inherit "/lib/keep";
...
query_ring_short()
{
    string qgem = query_ring_gem();
    string qmetal = query_ring_metal();
    if (gem && metal)
...
{
    if (query_ring_gem())
        return query_ring_metal() + " rings with " + 
            LANG_PWORD(query_ring_gem()); 

    return query_ring_metal() + " rings";
}
...
init_arg(string arg)
{
    ...
    if(gem == "0")
        gem = 0;
    if (gem == "amber" || gem == "emerald" || gem == "opal" || gem == "onyx")
    {
        add_adj(gem);
        article = "an";
    }
