...
create_room()
{
    ...
    set_long("You now find yourself in northeast corner of what "+
    "appears to be a giant ball room. The floor beneath you is "+
    "made of polished wood for dancing. There are nobles here "+
    "dancing and having a good time. A hallway leads out to the north "+
    "while the rest of the ball room is to the south and west.\n");

    INSIDE;

    add_exit(NOBLE(paullus/m9),"north",0,-1,-1);
    add_exit(NOBLE(paullus/m19),"west",0,-1,-1);
    add_exit(NOBLE(paullus/m20),"southwest",0,-1,-1);
    add_exit(NOBLE(paullus/m2),"south",0,-1,-1);
    add_exit(NOBLE(paullus/m1),"southeast",0,-1,-1);
...
{
    if(!ob)
        ob = clone_npc(NOBLE(npc/yf_ball_noble));
    if(!ob2)
        ob2 = clone_npc(NOBLE(npc/om_ball_noble));
    if(!ob3)
        ob3 = clone_npc(NOBLE(npc/of_ball_noble));
