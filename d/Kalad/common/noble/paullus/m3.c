...
create_room()
{
    set_short("In the southern part of the Dining Room");
    set_long("You now stand in the south part of a very long "+
      "room. This appears to be the dining room. A very long "+
      "wooden table is placed here, stretching all the way up "+
      "to the north end of this room. Various nobles and their "+
    ...
    add_exit(NOBLE(paullus/m4),"north",0,-1,-1);
    reset_room();
}
...
{
    if(!ob)
        ob = clone_npc(NOBLE(npc/om_ball_noble));
    if(!ob2)
