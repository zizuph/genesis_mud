...
create_room()
{
    ...
    set_long("You have entered the begining of the beautiful gardens "+
      ...
      "your east and southeast, while the "+
      "grand entrance to the estate is just off to the east.\n");
    ...
    add_exit(NOBLE(paullus/g3),"east",0,-1,-1);
    add_exit(NOBLE(paullus/g1),"southeast",0,-1,-1);
    reset_room();
}
...
reset_room()
{
    if(!obj)
    {
	obj = clone_object(NOBLE(npc/gardener));
	obj->arm_me();
