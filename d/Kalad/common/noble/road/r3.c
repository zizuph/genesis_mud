inherit "/d/Kalad/room_std";
#include "/d/Kalad/defs.h"
/* Sarr */

object ob;
void reset_room();

void
create_room()
{
    ::create_room();
    set_short("On a small road");
    set_long("You are now on a small, white-stoned paved road. Large "+
      "estates with beautiful mansions lie to your north, while "+
      "you see a castle rising over the park to the east. The "+
      "road splits here to the northwest, and leads to an estate "+
      "entrance.\n");
    add_item("castle","The castle is large and forboding. It must "+
      "belong to the high-lord of Kabal.\n");
    add_item("park","The great park of the noble district.\n");
    add_item("estates","They are large and look very well kept.\n");
    add_item("mansion","Beautiful architecture, only the very elite "+
      "could afford such mansions.\n");
    add_item("road","A very nice white-stoned road that is clean "+
      "and well-kept.\n");
    add_prop(ROOM_NO_TIME_DESC,1);
    add_exit(NOBLE(road/r2),"southwest",0,0,-1);
    add_exit(NOBLE(road/r4),"northwest",0,0,-1);
    add_exit(NOBLE(road/r5),"north",0,0,-1);
    reset_room();
}

void
reset_room()
{
    if(!ob)
    {
	ob = clone_object(NOBLE(npc/b_noble_guard_patrol));
	ob->arm_me();
	ob->move_living("xxx",TO);
    }
}

