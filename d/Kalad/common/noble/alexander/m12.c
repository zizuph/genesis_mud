...
create_room()
{
    ...
    set_long("You now find yourself in south side of what "+
    ...
    add_exit(NOBLE(alexander/m10),"west",0,-1,0);
    add_exit(NOBLE(alexander/m11),"north",0,-1,0);
    reset_room();
}
...
reset_room()
{
    ...
    if(!ob2)
    {
        ob2 = clone_object(NOBLE(npc/ym_ball_noble));
        ob2->arm_me();
