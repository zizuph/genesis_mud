...
create_room()
{
    ...
    add_exit(NOBLE(aemilius/m3),"east",0,-1,-1);
    add_exit(NOBLE(aemilius/m5),"north",0,-1,-1);
    add_exit(NOBLE(aemilius/m11),"west",0,-1,-1);
    add_exit(NOBLE(aemilius/m7),"south",0,-1,-1);
    reset_room();
}
...
reset_room()
{
    if(!ob)
    {
        ob = clone_object(NOBLE(npc/om_ball_noble));
        ob->arm_me();
    ...
    if(!ob3)
    {
        ob3 = clone_object(NOBLE(npc/of_ball_noble));
        ob3->arm_me();
