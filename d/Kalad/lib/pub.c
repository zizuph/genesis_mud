 ...
 * Updated menu by Lucius 01/2001
 */
#pragma strict_types
...
void
pub_hook_menu_footer()
{
    write("\nTry 'buy <item> with <cointype>' to specify how "+
	"you would like to pay.\nYou may also 'buy <number> <item>' "+
	"to buy 'number' of 'items'.\n\n");
}

void
pub_hook_menu_list(string what, int price)
{
    write(sprintf("\t%'.'-50s%'.'4d cc\n", what, price));
}
...
read_menu(string str)
{
    ...
    if (str != "menu" && str != "the menu")
    {
	notify_fail("Read what?\n");
	return 0;
    }

    if (!sizeof(dr_id) && !sizeof(fo_id))
    {
    ...
    while(++ix < size) 
    {
	string name = drinks[ix][5];
	if (!strlen(name))
	    name = drinks[ix][0][0];
	pub_hook_menu_list(capitalize(name), drinks[ix][4]);
    }
    ...
    while(++ix < size)
    {
	string name = food[ix][4];
	if (!strlen(name))
	    name = food[ix][0][0];
	pub_hook_menu_list(capitalize(name), food[ix][3]);
    }
