...
static object shopkeeper;	/* The shopkeeper */
...
{
    write(break_string("You would get " + text + " for " +
	LANG_THESHORT(ob) + ".\n", 75));
}
 ...
 * Arguments:	  ob - The object
 * 		  price - The price of 'ob'
 * 		  amount - How many of 'ob' are there
 */
void
shop_hook_list_object(object ob, int price, int amount = 1)
{
    string str;
    int *arr = split_values(price);;
    ...
    write(sprintf("|%2d | %'.'-51s|%2d P %2d G %2d S %2d C|\n", 
	    amount, capitalize(str), arr[3], arr[2], arr[1], arr[0]));
}
...
query_buy_price(object ob)
{
    ...
    if (IS_HEAP_OBJECT(ob))
	val = ob->query_prop(HEAP_I_UNIT_VALUE);
    else
	val = ob->query_prop(OBJ_I_VALUE);

    sscanf(OB_NUM(ob), "%d", seed);
    return 2 * val * (query_money_greed_buy() +
...
/*
 * Function name: set_shopkeep
 * Description	: Set the shopkeeper for this shop.
 * Arguments	: mixed what - Object to use, or a filepath
 * 			to an object to clone.
 * 		  object move_to - Where to move the the shopkeeper
 * 		  	to. (optional)
 * 		  string how - the 'how' argument to move_living.
 * Returns	: void.
 */
varargs void
set_shopkeep(mixed what, object move_to, string how = "M")
{
    if (objectp(what))
	shopkeeper = what;
    else if (strlen(what))
	shopkeeper = clone_object(what);
    else
	throw("Invalid argument 0 to set_shopkeep.\n");

    if (objectp(move_to))
    {
	if (shopkeeper->move_living(how, move_to))
	    shopkeeper->move(move_to, 1);
    }
}

/*
 * Function name: query_shopkeep
 * Description	: Return the objectp to the set shopkeeper or
 * 			void if not set or destroyed.
 */
object
query_shopkeep() { return shopkeeper; }
 ...
 */
string
query_store_room() { return store_room; }
 ...
 */
int
weapon_filter(object ob) { return IS_WEAPON_OBJECT(ob); }
 ...
 */
int
armour_filter(object ob) { return IS_ARMOUR_OBJECT(ob); }
...
do_store(string str)
{
    ...
    {
	notify_fail("The store-room cannot be found.\n");
	return 0;
    }
...
/*
 * Function name: query_shop_sign
 * Description	: The text of the shop sign when read.
...
nomask int
sort_list_len(string a, string b)
{
    if (a > b)
	return 1;
    if (a < b)
...
{
    string one = a->short(), two = b->short();

    if (one > two)
	return 1;
    if (one < two)
	return sort_list_len(one, two);
    return 0;
}
...
do_list(string str)
{
    ...
    item_arr = unique_array(item_arr, "short");
    ...
    for (i = 0; i < max; i++)
    {
	price = query_buy_price(item_arr[i][0]);
	shop_hook_list_object(item_arr[i][0], price, sizeof(item_arr[i]));
    }

    if (max < sizeof(item_arr))
	write(sprintf("%'.'-77s|", "|** Truncated\n"));
