...
create_kurinost_room()
{
    ...
    add_exit(KROOM + "road6","northeast","@@knight_check");
    add_exit(KROOM + "road","southwest","@@no_exit");
    reset_kurinost_room();
}
...
no_exit()
{
  write("A hulking evilfaced draconian steps out in front of you.\n"+
"The hulking evilfaced draconian says: The road southwest is closed "+
"for the time being.\n"+
"The draconian leaves again into the trees.\n");
   say(QCTNAME(TP)+ " tries to move southwest, but the road is closed.\n");
  return 1;
}
...
knight_check()
{
	    ...
	    (objectp(drac5) && (TO == E(drac5)) ) )
	{
	    ...
	    say(QCTNAME(TP)+ " is blocked from entering the camp by the draconian.\n");
	    return 1;
	}
    }
    return 0;
