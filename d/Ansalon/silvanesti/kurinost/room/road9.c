...
create_kurinost_room()
{
    ...
    add_item("tent","A tent is near with its flap open.\n");
    add_item(({"tree","trees"}),"The trees here look normal and healthy.\n");
    add_exit(KROOM + "road6","southwest",0);
    add_exit(KROOM + "tent7","tent",0);
...
long_descr()
{
   return "This is the Green Dragonarmy's camp outside "+
     ...
     "The tent that stands here is noticeably larger than the ones that "+
     "surround it.  It is obvious the tent belongs to the outpost's "+
     "ranking officer.  "+
