...
{
    set_short("West side of Park Square");
    set_long("@@long_descr");
...
add_item(({"tree","trees"}),"The trees of the park haven't had leaves since "+
    ...
    add_item(({"park","square"}),"The park is square in shape with trees " +
      ...
      "people to relax and obserbe the fountain.\n");
    add_item("fountain","The fountain lies east of here.  You feel you "+
    ...
    add_exit(BROOM + "park1","north",0);
    add_exit(BROOM + "park6","south",0);
    add_exit(BROOM + "street15","west",0);
    add_exit(BROOM + "fountain","east",0);
    add_exit(BROOM + "park2","northeast",0);
    add_exit(BROOM + "park7","southeast",0);
    reset_balinost_room();
}
...
long_descr()
{
    return "This is the west side of Balinost's Park Square.  "+
