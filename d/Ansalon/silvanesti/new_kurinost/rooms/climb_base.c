...
status gNest;
status *gClimb_dir;
string gHeight, gCliff_desc, gVegetation_desc;
string *gExit_desc;
 ...
 *  Arguments: int - 0 = no vegetation, 1 = nest.
 *  Returns  : void
 * 
 *  May add a birdsnest to this location.
 *
 */
public void
add_vegetation(int nest_type)
{
    gNest = nest_type;

    add_item(({ "patch", "patch of soil", "patches",
        "patches of soil", "dirt", "soil" }), "Barely visible " +
        "here and there are patches of soil allowing small " +
        "bushes, low cactus plants and other smaller plants and " +
        "mushrooms to grow in this inhospitable area.\n");
    ...
    if (!gNest)
    {
        add_item(({ "nest", "nests" }), "Building their nests " +
            "high on the sides of the cliff, the seagulls attempt " +
            "to keep their young away from predators. Constructed " +
            "by natural materials like twigs and small branches, " +
            "the nests seem fragile but are probably quite " +
            "effective in sheltering the eggs and young gulls.\n");
        add_item(({ "seagull", "seagulls", "white seagull",
            ...
            "often a lone white seagull appears to be suspended " +
            "in midair above the dark granite cliff. Rising winds " +
            "from the ocean keep the seagull high in the air. " +
            "Looking closer, you see that the birds are nested in " +
            "small colonies, resting in their nests on the sides " +
            "of the cliffs.\n");
    }
}
 ...
 *  Returns  : string - description.
 * 
 *  Returns a random generated description of the cliffs.
 *  Also adds items to examine and produces a nest to
 *  harvest for eggs.
 *
 */
public string
generate_cliffs()
{
    ...
    if (gNest)
    {
        add_desc = " Standing on a tiny ledge, you notice you have " +
            "climbed into the middle of a small colony of seagull " +
            "nests. One of the nests is within reach.";
    }
    ...
    add_item(({ "cliff", "cliffs", "steep cliff", "steep cliffs" }),
        ...
        "cliffside. " + show_height() + " Although steep, you " +
        "have managed to climb the sides of the cliff to this " +
        "point." + add_desc + "\n");
    add_item(({ "ledge", "tiny ledge" }), "Having climbed the " +
        "steep sides of the cliffs, you have reached a tiny ledge " +
        "and are far from save at this height. Though barely wide " +
        "enough for your feet, the ledge seems solid enough for " +
        "you to stand on.\n");
    ...
    add_item(({ "shore", "coast" }), "Stretching like a dark " +
        ...
        "many places. Only here and there a small beach can be " +
        "spotted.\n");
    ...
    add_item(({ "view", "spectacular view" }), "A strong ocean " +
        ...
        "on this spot, some " + gHeight + " meters above the " +
        "ocean, you have a spectacular view of the shore as it " +
        "stretches several kilometers northwest and a few hundred " +
        "meters east, where the coast turns south, out of view.\n");
    ...
    if (!gNest)
    {
        add_desc = " Looking closer, you see that the birds are " +
            "nested in small colonies on the sides of the cliffs.";
    }
    add_item(({ "seagull", "seagulls", "white seagull",
        "white seagulls", "life", "bird", "birds" }),
        "Crying seagulls can be heard from all directions and " +
        ...
        "the ocean keep the seagull high in the air. " + add_desc +
        "\n");
    ...
    {
    case 0..2:
        gCliff_desc = "You find yourself high on the side of a cliff." +
            add_desc+ " Towering above you, the steep granite " +
            "cliffs of the coast are almost barren, dark and grey.";
        break;

    case 3..4:
        gCliff_desc = "You find yourself high on the side of a cliff." +
            add_desc+ " Tall and windswept, the towering cliffs of " +
    ...
    default:
        gCliff_desc = "You find yourself high on the side of a cliff." +
            add_desc+ " Climbing the currents of the strong ocean " +
...
generate_path(string path_desc)
{
    ...
    add_item(({ "path", "narrow path", "small path",
        "small narrow path" }), "Climbing uphill towards a " +
        "distant pass high in the cliffs, the small narrow path " +
        "twists and turns its way up the side of the cliff like a " +
        "great black snake. Peering around from your position " +
        "high up on the cliffside, you are treated to a " +
        "spectacular view. Here, some " + gHeight + " meters " +
        "above the ocean, you can see far in all directions. " +
        "Having left the path, you have climbed atleast ten meters " +
        path_desc + " to reach this spot.\n");
    add_item(({ "bend", "turn" }), "Twisting and turning like a " +
        ...
        "the opposite direction. Most of the turns appear quite " +
        "steep and dangerous.\n");
...
show_me_long_desc()
{
    return gCliff_desc + " " + show_me_day() + " " +
        generate_season() + "\n";
}
...
climb_cliff(string arg)
{
    ...
    if (arg == "up" || arg == "down")
    {
        ...
        climb_desc = "Preparing for another taxing climb, you " +
            "quickly find some handholds and begin ";
        say_desc = QCTNAME(TP) + " seems to prepare for another " +
            "climb and begins ";
        ...
        // Climbing back to the path should be easy so we do not
        // keep the player stuck here for ages. :)
        if (TP->resolve_task(TASK_HARD, ({ TS_CON, SS_CLIMB })))
        {
            say(QCTNAME(TP) + " leaves, climbing the cliffside.\n");

            TP->move_living("M",
                            RSROOMS + gExit_desc[direction], 1, 0);
        ...
        else
        {
            NF("Unable to find some good handholds, you give " +
                "up climbing the cliffside and contend yourself " +
                "with hanging on to sheer life.\n");
            say("Unable to find some good handholds, " + QCTNAME(TP) +
                " has to give up climbing the cliffside.\n");
