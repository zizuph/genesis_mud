...
create_kurinost_room()
{
    ...
    add_item(({ "path", "narrow path", "small path",
        ...
        "Heading west would take you up towards the top of the " +
        "ledge and northeast the ledge leads downwards.\n");
    ...
    add_exit(RSROOMS + "ledge1", "west", ascend_path, MOUNT_FATIGUE);
    add_exit(RSROOMS + "ledge3", "northeast", descend_path, 5);

    // Other possible exits:
    set_climb_dirs(({ 0, 0 }),
        ({ "Being much too steep, the cliff seems impossible to " +
           "climb here.\n",
           "Even though you are only ten meters above the sea, " +
