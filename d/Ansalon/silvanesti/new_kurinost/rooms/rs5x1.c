...
create_kurinost_room()
{
    ...
    set_short("narrow path running up the side of a cliff overlooking " +
        "the Bay of Balifor");
    ...
    set_climb_dirs(({ 0, 0 }),
        ({ "Almost vertical cliffs obscures most of your vision " +
           ...
           "direction.\n",
           "Climbing down would take you back to the shore so you " +
           "decide against it and stick to the path instead.\n" }));

    add_exit(RSROOMS + "rs_shore", "northeast", descend_path, 2);
    add_exit(RSROOMS + "rs4x1", "west", ascend_path, MOUNT_FATIGUE);
...
descend_path()
{
    write("Following the narrow path down, it turns northeast and " +
        "you step out onto a small sandy beach.\n");
