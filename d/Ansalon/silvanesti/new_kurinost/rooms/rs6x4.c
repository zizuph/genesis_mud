...
create_kurinost_room()
{
    ...
    generate_path("west", 
                  "Having climbed onto a shelf going west, the " +
    ...
    add_exit(RSROOMS + "rs6x2", "northeast", descend_path, 2);
    add_exit(RSROOMS + "rs5x4", "west", ascend_path, MOUNT_FATIGUE);
...
ascend_path()
{
    write("Climbing the narrow path you head west up the " +
        "cliff.\n");
...
descend_path()
{
    write("Following the narrow path, you climb down towards the " +
        "southeast. Following the path a while, it turns " +
        "southwest and approaches a narrow bend.\n");
