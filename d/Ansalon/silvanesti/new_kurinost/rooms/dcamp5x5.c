...
create_kurinost_room()
{
    ...
    generate_camp(DCAMP_TENTCAMP,
        ...
        "leading up towards the tower some distance to the west.");

    // Set up descriptions about the paved path.
    generate_path("west, back towards the southern entrance to " +
        "the encampment.");

    set_short("southeastern corner of a shady encampment");
    set_long(show_long_desc);

    add_cmd_parse("[the] / [a] [crude] [grey] 'tent'",
        ({ "enter", "open" }), enter_tent);

    add_exit(RSROOMS + "dcamp4x5", "west");
    add_exit(RSROOMS + "dtent5x5", "tent", enter_tent);
...
enter_tent()
{
    ...
    if(TP->query_prop(TENT_NUMBER) == 4)
    {
