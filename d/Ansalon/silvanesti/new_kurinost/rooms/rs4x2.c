...
create_kurinost_room()
{
    ...
    set_climb_dirs(({ 0, 0 }),
        ({ "Being much too steep, the cliff seems impossible to " +
           "climb here.\n", 
           "Climb down to what? A near vertical drop prevents you " +
           "from climbing straight down.\n" }));
...
climb_cliff(string arg)
{
    ...
    if (moved)
    {
        ...
        {
            if (interactive(items[index]) &&
                !items[index]->query_prop(LEDGEFOUND))
            {
                items[index]->add_prop(LEDGESEEN, 1);
            }
        }
