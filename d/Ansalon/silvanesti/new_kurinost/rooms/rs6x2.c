...
create_kurinost_room()
{
    ...
    generate_path("southeast",
                  ...
                  "climbs southeast a while and then southwest.");
    generate_cliffs();
    add_extra_long_desc("From here the view here is particularly " +
        "spectacular, with an unequalled view of the beaches " +
        "around you.");
...
show_me_pillars()
{
    string pillar_desc = "Guarding the entrance to a small " +
    	...
    	"also steal away much of the light down there, creating a " +
    	"small sandy beach shrouded in shadows and silence. " +
    	"Marred by millennia of tear and wear from the waves of " +
    	"the ocean, the pillars are smooth and rounded near the " +
    	"waters edge, while being a bit more ragged higher up.";
    ...
    if ((TP->query_skill(SS_TRACKING) > (random(10) + 11)) ||
        TP->query_prop(LEDGEFOUND))
    {
        TP->add_prop(LEDGEFOUND, 1);

        return pillar_desc + " Barely visible on the northwest " +
            "cliffside, between the pillars directly below you, " +
            "is a narrow ledge that might be passable.\n";
            
    	return pillar_desc + " Just as your eyes follow the " +
    	    "pillars from the beach below, you notice a barely " +
    	    "visible ledge on the otherwise barren and steep " +
    	    "cliffside. From up here, you have a perfect view of " +
    	    "the twisting narrow path that leads towards the top " +
    	    "of the coastal cliffs, as it make a sharp turn unto " +
    	    "a new shelf about 30 meters west and 10 meters below " +
    	    "you. In the middle of that exact turn, just below " +
    	    "the path you notice the tiny ledge disappear east, " +
    	    "around a cliff wall.\n";
    }
...
show_me_ledge()
{
    if (TP->query_prop(LEDGEFOUND))
    {
        return "Having spotted the narrow ledge below you, it is " +
            ...
            "sheltered cove far below, up the side of the cliff " +
            "walls until it ends just below the twisting path " +
            "further back to your southwest. " +
            "Someone brave enough might climb down to that ledge " +
            "and try to follow the track down to the beach in the " +
            "cove.\n";
    }
