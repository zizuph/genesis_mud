...
// G L O B A L   V A R S
object gMob1, gMob2, gMob3, gMob4;
object *gMlist = ({ gMob1, gMob2, gMob3, gMob4 });
...
create_kurinost_room()
{
    ...
    generate_camp(DCAMP_TENTCAMP,
        "You have come as far east as you can on this paved path. " +
        "From the middle of this small camp of tents, you have a " +
        "great view across the entire encampment, with the " +
        "central pit below and the path circling the center, " +
        "leading up towards the tower.");

    // Set up descriptions about the paved path.
    generate_path("west, back towards the northern entrance to " +
        "the encampment.");

    set_short("Northeastern corner of a shady encampment");
    set_long(show_long_desc);

    add_cmd_parse("[the] / [a] [crude] [grey] 'tent'",
        ({ "enter", "open" }), enter_tent);

    add_exit(RSROOMS + "dcamp5x2", "west");
    add_exit(RSROOMS + "dtent6x2", "tent", enter_tent);
...
enter_tent()
{
    ...
    if(TP->query_prop(TENT_NUMBER) == 2)
    {
