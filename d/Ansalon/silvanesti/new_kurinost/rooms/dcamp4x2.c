...
create_kurinost_room()
{
    ...
    generate_camp(DCAMP_TENTCAMP,
        ...
        "the shadow high above you on the western side of the camp.");

    // Set up descriptions about the paved path.
    generate_path("east, back towards the northern entrance to " +
        "the encampment.");

    set_short("northwestern corner of a shady encampment");
    set_long(show_long_desc);

    add_cmd_parse("[the] / [a] [crude] [grey] 'tent'",
        ({ "enter", "open" }), enter_tent);

    add_exit(RSROOMS + "dcamp5x2", "east");
    add_exit(RSROOMS + "dtent4x2", "tent", enter_tent);
...
enter_tent()
{
    ...
    if(TP->query_prop(TENT_NUMBER) == 1)
    {
