...
show_parchment()
{
    if (gSeal_broken)
    {
        return "Only slightly crumbled and dirty, this piece of " +
            "parchment has been carefully folded, although the " +
            "wax seal has been broken. Some kind of thick skin " +
    ...
    return "Only slightly crumbled and dirty, this piece of " +
        "parchment has been carefully folded and sealed with " +
        "wax. Some kind of thick skin has been used to make this " +
...
break_seal(string arg)
{
                      ...
                      noun))
    {
        if (gSeal_broken)
        {
            write("The seal on the dirty parchment has already " +
                "been broken. You can read the parchment.\n");

            return 1;
        }

        write("You break off the wax seal on the dirty " +
            "parchment.\n");
        say(QCTNAME(TP) + " breaks the seal on a dirty piece of " +
            "parchment.\n");
...
read_parchment(string arg)
{
    ...
    {
        say(QCTNAME(TP) + " unfolds a dirty piece of parchment " +
            "and reads it.\n");

        write("You unfold the parchment and read:\n");
        write(read_file(PARCHMENT_FILE));
