...
create_weapon()
{
    ...
    add_adj("kender");
    add_name("staff");
    set_short("lemonwood hoopak");
    set_long("@@long_desc@@");
    set_default_weapon(15,15,W_POLEARM, W_BLUDGEON | W_IMPALE, W_BOTH);
    add_prop(OBJ_I_VOLUME, 250);
    add_prop(OBJ_I_WEIGHT, F_WEIGHT_DEFAULT_WEAPON(31, W_POLEARM));
    add_prop(OBJ_I_VALUE, F_VALUE_WEAPON(35,31)); 
    set_wf(TO);
    tstaff = this_object();
}
...
long_desc()
{
	...
	"one. It is made from a single piece of flexible lemonwood that, " + 
	"from the forked top to the very end, is polished and smoothed, " + 
	"most likely from long usage. The bottom end of it is iron clad " + 
