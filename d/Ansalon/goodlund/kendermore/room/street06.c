...
{
    set_short("East of the entrance to Kendermore");
    set_long("@@long_descr");
    ...
    add_cmd_item("enter", "house", "@@enter_house");

    add_exit(KROOM + "street07", "north", 0);
    add_exit(KROOM + "street05", "south", 0);
    add_exit(KROOM + "in_gate", "west", 0);
...
long_descr()
{
    return (tod_descr() + season_descr() + "Houses stand on both sides " + 
        ...
        "winding way to the " + find_exits() + ". You can make out the " + 
        "entrance to the city to the west. Numerous signs point " + 
