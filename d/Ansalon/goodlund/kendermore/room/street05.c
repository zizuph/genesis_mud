...
{
    set_short("South of the entrance to Kendermore");
    set_long("@@long_descr");
    ...
    add_item(({"house", "houses"}), "Even the houses here have a " + 
        "carefree feeling. There doesn't seem to be any way in from here.\n");
    add_exit(KROOM + "street04", "southwest", 0);
    add_exit(KROOM + "street20", "northeast", 0);
    add_exit(KROOM + "street06", "north", 0);
...
long_descr()
{
    return (tod_descr() + season_descr() + "Houses stand on both sides " +
        "of you. " + street_descr() + " The road continues on its winding " +
        "way to the " + find_exits() + ". You can make out the " +
        "entrance to the city to the north. There are numerous street " +
        "signs pointing in all directions, but they tend to contradict " +
        "each other and point in directions where there are no exits.\n");
