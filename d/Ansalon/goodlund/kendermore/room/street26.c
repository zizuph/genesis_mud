...
create_kendermore_room()
{
    ...
    add_item(({"alley", "small alley"}), "You stand in a dark alley, which " + 
        "leads only to a dead end.\n");
    add_item("dead end", "The alley comes to an end here.\n");
    ...
    add_invis_exit(KROOM + "alley", "north", 0);
    add_exit(KROOM + "street25", "southwest", 0);
...
long_descr()
{
    return (tod_descr() + season_descr() + "Houses stand on both sides " + 
        ...
        "winding way to the southwest. A small alley lies to the north. " + 
        "Numerous signs point " + 
