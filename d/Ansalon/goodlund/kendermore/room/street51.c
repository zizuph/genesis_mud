...
{
    set_short("Outside a candle store");
    set_long("@@long_descr");
    ...
    add_item("prominent sign", "This doesn't look like the palace!\n");
    add_item("store", "The door is propped open with a brick, so you " + 
        "assume it's open.\n");

    add_cmd_item("house", "enter", "@@enter_house");

    add_exit(KROOM + "street50", "southwest", 0);
    add_exit(KROOM + "can_store1.c", "east", 0);    
...
long_descr()
{
    return (tod_descr() + season_descr() + "Houses stand on both sides " + 
        ...
        "winding way to the southwest. Numerous signs point " + 
        "in all directions, but they tend to contradict each other and " + 
        "point in directions where there are no exits. There is a small " + 
        "store to your east with a prominent sign reading 'Palace' " + 
