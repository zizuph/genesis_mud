...
{
    set_short("Outside a post office");
    set_long("@@long_descr");
    ...
    add_item(({"street", "road"}), "The road is made of cobblestones. " +
        "It winds " + 
        "beyond your view.\n");
    add_item("building", "It looks like a post office, though little used.\n");
    add_item(({"sign", "signs"}), "Best to ask directions instead.\n");

    add_cmd_item("house", "enter", "@@enter_house");

    add_exit(KROOM + "po", "east", 0);
    add_exit(KROOM + "street15", "southwest", 0);
...
long_descr()
{
    return (tod_descr() + season_descr() + "Houses stand on both sides " + 
        "of you, and an unfinished building stands to your east. " + 
        street_descr() + " The road continues on its " + 
