...
{
    set_short("North of a wide courtyard");
    set_long("@@long_descr");
    ...
    add_item("courtyard", "It appears to be a training grounds.\n");

    add_cmd_item("house", "enter", "@@enter_house");

    add_exit(KROOM + "street53", "north", 0);
    add_exit(KROOM + "aguild", "southwest", 0);
...
long_descr()
{
    return (tod_descr() + season_descr() + "Houses stand on both sides " + 
        ...
        "winding way to the north. A courtyard " + 
        "opens up to the southwest. Numerous signs point " + 
...
enter_house()
{
    ...
    tell_room(KROOM + "house8", "The door opens, and " + QTNAME(TP) +  
        " enters the house.\n");
    TP->move_living("M", KROOM + "house8", 1, 0);
    return "";
}
