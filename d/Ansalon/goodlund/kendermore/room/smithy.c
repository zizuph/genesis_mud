 ...
 * Modified for Maen's Smithy in Kendermore
 */
...
reset_kendermore_room()
{
    if (!maen_smith)
    {
	maen_smith = clone_object(KNPC + "maen");
	maen_smith->move_living("xx", TO);
	maen_smith->seq_new("do_thing");
	maen_smith->seq_addfirst("do_thing","@@arm_me");
    }
}
...
{
    set_short("Maen's smithy");
    set_long("The windows and door of this smithy have been flung wide " + 
        "open to help cool down the oppressive heat. A barrel of water " + 
    ...
    add_exit(KROOM + "street23","north",0);
    ...
    set_tell_time(200);
    add_tell("A bead of sweat runs down your forehead.\n");
    add_tell("@@pound_tell");
...
do_fix(string str)
{
    ...
    // Until we get a time limit on how long you are enemy, I am disabling this
    // check on the smith: Navarre April 9th 2009.
    //
    //    if (KMASTER->query_banished(this_player()->query_real_name()))
    //    {
    //        notify_fail("The blacksmith won't deal with the likes of you!\n");
    //        return 0;
    //    }
