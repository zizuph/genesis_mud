...
reset_kendermore_room()
{
    if (!junk)
    {
        junk=clone_object(things[random(6)]);
        junk->move(TO);
    }
    if (!kender && !random(2))
    {
...
leave_house()
{
    ...
    tell_room(KROOM + "street54", "The door opens, and " + QTNAME(TP) +  
        " steps onto the street.\n");
    TP->move_living("M", KROOM + "street54", 1, 0);
    return "";
}
...
squeeze_out()
{
    ...
    tell_room(KROOM + "street54", QCTNAME(TP) + " comes tumbling out " + 
