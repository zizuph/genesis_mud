...
reset_kendermore_room()
{
    ...
    if (!bear_cub)
        {
            bear_cub = clone_object(KNPC + "bear_cub");
            bear_cub->move(TO);
            bear_cub = clone_object(KNPC + "bear_cub");
            bear_cub->move(TO);
            bear_cub = clone_object(KNPC + "bear_cub");
...
{
    set_short("South of a wide courtyard");
    set_long("@@long_descr");
    ...
    add_exit(KROOM + "street24", "south", 0);
    add_exit(KROOM + "aguild", "northwest", 0);
...
long_descr()
{
    return (tod_descr() + season_descr() + "Houses stand on both sides " + 
        ...
        "winding way to the south. A courtyard " + 
        "opens up to your northwest. Numerous signs point " + 
