...
{
    set_short("Outside a candy shop");
    set_long("@@long_descr");
    ...
    add_item("shop", "It looks like a small candy shop.\n");

    add_cmd_item("house", "enter", "@@enter_house");

    add_exit(KROOM + "street21", "northeast", 0);
    add_exit(KROOM + "candy_store", "southeast", 0);
    add_exit(KROOM + "street05", "southwest", 0);
...
long_descr()
{
    return (tod_descr() + season_descr() + "Houses stand on both sides " + 
        "of you, and a small shop stands to the southeast. " + street_descr() + 
        " The road continues on its winding way to the southwest and " + 
        "northeast. Numerous signs point in all directions, but they " + 
