...
create_kendermore_room()
{
    ...
    add_item(({"hall", "city hall"}), "It is four stories tall. The bottom " + 
        "appears to be intact, but each floor is less finished than the one " + 
        "below it. In fact, the top floor looked to be still under " + 
        "construction!\n");
...
switch_street()
{
    if (GET_MONTH > 6)
    {
        ...
        add_exit(KROOM + "street35", "northeast", 0);
        add_exit(KROOM + "street33", "southeast", 0);
        add_exit(KROOM + "chall_entry", "east", "@@chall_enter");
    }
    else
    {
        add_invis_exit("null", "northeast", "@@blocked_street");
        add_invis_exit("null", "southeast", "@@blocked_street");
        add_exit(KROOM + "street35", "north", 0);
        add_exit(KROOM + "street33", "south", 0);
        add_exit(KROOM + "chall_entry", "east", "@@chall_enter");
...
chall_enter()
{
    write("You follow the street a short ways to the east and enter City " + 
        "Hall.\n");
    return 0;
}
