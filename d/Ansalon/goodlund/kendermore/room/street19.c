...
{
    set_short("Outside a small shop");
    set_long("@@long_descr");
    ...
    add_item("shop", "It looks like a small general store.\n");

    add_cmd_item("house", "enter", "@@enter_house");

    add_exit(KROOM + "street18", "south", 0);
    add_exit(KROOM + "gen_store", "west", 0);
...
long_descr()
{
    return (tod_descr() + season_descr() + "Houses stand on both sides " + 
        "of you, and a small shop stands to the west. " + street_descr() + 
        " The road continues on its winding way to the south. Numerous " + 
