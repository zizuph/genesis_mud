...
object kender1, kender2, kender3;
...
create_kendermore_room()
{
    ...
    add_item(({"curtain", "curtains"}), "It covers the east wall, which "+
               "leads back into the hallway of the doctor's office.\n");
    ...
    add_exit(KROOM + "waiting3", "north", 0);
    add_exit(KROOM + "waiting4", "south", 0);
    add_exit(KROOM + "waiting2", "west", 0);
    reset_kendermore_room();
}
...
move_curtain()
{
  write("You enter through the curtain into the hallway.\n");
  TP->move_living("through the curtain into the hallway", KROOM+"pc_office", 1, 0);
  return "";
}
