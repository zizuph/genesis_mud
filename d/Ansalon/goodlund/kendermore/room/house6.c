...
leave_house()
{
    ...
    tell_room(KROOM + "street43", "The door opens, and " + QTNAME(TP) +  
        " steps onto the street.\n");
    TP->move_living("M", KROOM + "street43", 1, 0);
    return "";
}
...
squeeze_out()
{
    ...
    tell_room(KROOM + "street43", QCTNAME(TP) + " comes tumbling out " + 
...
lift_rug()
{
    ...
    clone_object(KOBJ + "box_key")->move(TP, 1);
    found_key = 1;

    return ("You look around to make sure nobody is watching, then lift " + 
...
search_shelf()
{
    ...
    return("You find something interesting on the shelf!\n");
    say(QCTNAME(TP) + " takes something from the shelf.\n");
}
