...
long_descr()
{
    return (tod_descr() + season_descr() + "Houses stand on both sides " + 
        ...
        "winding way to the " + find_exits() + ". Numerous signs " + 
        "point in all directions, but they tend to contradict each other " + 
        "and point in directions where there are no exits.\n");
}
...
switch_street()
{
    if (GET_MONTH > 6)
    {
        ...
        add_exit(KROOM + "street55", "north", 0);
        add_exit(KROOM + "street25", "northeast", 0);
        add_exit(KROOM + "street23", "northwest", 0);
    }
    else
    {
        ...
        add_exit(KROOM + "street55", "north", 0);
        add_exit(KROOM + "street25", "southeast", 0);
        add_exit(KROOM + "street23", "southwest", 0);
    }
}
