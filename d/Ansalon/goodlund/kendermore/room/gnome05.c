...
#define H2 KRHERB + "chokecherry"
#define H3 KRHERB + "hawthorn"
#define H4 KRHERB + "hop"
...
{
    set_short("An ancient peaceful forest");
    set_long("@@long_descr");
    add_item(({"forest", "plants", "plant", "tree", "trees"}), "Numerous " + 
    ...
    add_exit(KROOM + "gnome04", "west", "@@go_west");
    add_invis_exit(KROOM + "nowhere", "south", "@@stop_message");
    add_exit(KROOM + "gnome06", "north", "@@go_north");
...
go_west()
{
    write("You walk through the shimmering light to the west, leaving the " + 
        "ancient oak forest behind and stepping into a green pine forest.\n");
...
long_descr()
{
    return tod_descr() + "You've entered a peaceful forest of ancient " +
        "broad-leafed trees. Thick underbrush covers the ground, and " + 
        "the sounds of nature put you at ease. A shimmering light shifts " + 
        "to the west.\n";
}
