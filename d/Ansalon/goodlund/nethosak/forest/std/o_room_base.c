...
*
* Added bait that can be found in forest trees.
* Arman, March 2021
*/
...
#define HAS_SEARCHED_GRUBS   "_has_searched_trees"
#define HOW_MANY_GRUBS       5
#define GRUB_OBJECT          "/d/Krynn/common/fishing/bardee_grub"
...
mixed
search_trees(object searcher, string what)
{
  ...
  if (CAN_SEE_IN_ROOM(this_player()))
    {
	    ...
	    return ("Your search of the trees reveals nothing special, " +
                "although you do find indications of moth larvae living " +
                "under the bark of the trees in this forest.\n");

	if (times_searched >= HOW_MANY_GRUBS)
	  {
	      return ("Your search of the trees reveals nothing special. " +
                  "You don't think you'll find any grubs in this location.\n");
	  }
	else
	  {
              say(QCTNAME(this_player())+ " pulls a bardee grub from " +
	      ...
	      return "You find a bardee grub in the bark of the trees!\n";
	  }
    }
  return 0;
}
...
create_krynn_room()
{
   ...
   add_search(({"trees", "trees for grubs", "trees for grub", 
       "bark", "bark for larvae", "bark for grubs",
       "bark for moth larvae", "trees for larvae" }), 
       10, "search_trees", 1);

   add_item(({"bark"}),
       "Underneath the bark of the trees of this forest " +
       "it seems the larvae of the local Rain Moth may " +
       "be found with a skilled search.\n");
