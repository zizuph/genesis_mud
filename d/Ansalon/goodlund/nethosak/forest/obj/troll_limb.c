 ...
 * Modified:
 * 2001-06-25 - Thorgal
 * - I reduced exp for spawns to 85% of original - those trolls are in
 *   one place and it is huge exp. Right now I consider it balanced
 *   quite dangerouse but still expy - originals are on 125% -
 *
 * Someone 
 * Fixed up the bug : players couldn't burn the limb
 * 
 * 2004-12-15 - Cotillion
 * - Improved burn code, made burnt limbs decay.
 */
inherit "/std/object";
#include <cmdparse.h>
#include <composite.h>
...
#define TROLL_LIMB_BURNT    "_neraka_limb_burnt"
#define DEBUG(x)    (find_player("torqual")->catch_msg("LIMB: " + x + "\n"))
...
decay_limb()
{
    ...
    tell_room(environment(), "The " + QSHORT(this_object()) + " decays.\n");
    remove_object();
}
...
burn_limb()
{
    ...
    set_short("burnt " + query_adj(0) + " " + query_name() + " of a troll");
