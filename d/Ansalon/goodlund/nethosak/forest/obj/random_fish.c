...
gut_fish()
{
    ...
    if (chance == 1 && gutted_fish == 0 && TP->query_prop(I_MUST_FIND_JILANS_WEAPON))
    {
        write("You find a small golden key inside the fish!\n");
        setuid();
        seteuid(getuid());
        obj = clone_object("/d/Ansalon/goodlund/nethosak/forest/obj/golden_key");
        obj->move(TP,1);
        gutted_fish = 1;
        TP->add_prop(CAUGHT_FISH, 1);
        return 1;
    }
