 ...
 * Added gems.
 * - Elmore, Jun 2001.
 * - due to recent events I again downgraded exp given by the trolls to 110%
 *   Also upgraded offensive special if player is teamed
 *       Thorgal Jun 2001
 * Added minor vulnerability to fire and acid.
 * Reduced xp by 5% to account for that.
 * - Arman, July 2016
 ******************************************************************************/
...
limb_fall_off(object attacker, string limb)
{
    ...
    attacker->catch_tell("You severed " + TO->query_the_name(attacker) + "'s " + limb +
...
{
    int attacktype, pen, att, wloc, team_size, pen_bonus;
    mixed hitres;
    string *hitloc, *attack, how, weapon;
    object stun;
    int currentHp, maxHp;

    if (!objectp(enemy) || E(enemy) != E(TO))
	return 0;
    ...
    if ( (80*enemy->query_hp()/(enemy->query_max_hp())) > (100*TO->query_hp()/(TO->query_max_hp())) 
          && !enemy->query_prop(I_AM_SLOWED_FROM_TROLLS) && !attacktype)
    {
        if (enemy->query_not_attack_me(TO, -1))
        {
            return 1; // round is ended if the attack was not allowed.
        }
        
        stun = clone_object("/d/Ansalon/goodlund/nethosak/forest/obj/stun");
        stun->move(enemy,1);
             
        enemy->add_prop(I_AM_SLOWED_FROM_TROLLS,1);
        ...
        enemy->catch_tell(TO->query_The_name(enemy) + 
          ...
          "The blow "+how+" muscles in your "+ hitloc[wloc]+ "!\n");

        enemy->tell_watcher(QCTNAME(TO) + " joins his fists and takes a swing " +
          "at " + QTNAME(enemy) + ". The blow " + how + " the muscles in "+
...
set_hunting(object ob)
{
    ...
    if (member_array(ob, gHunted) == -1)
    {
        if(E(ob) == E(TO))
        {
            ob->catch_tell(TO->query_The_name(ob) + " sniffs at you and a light " +
                "glints in his eyes.\n");
