...
reset_mithas_room()
{
    if(!objectp(remains))
    {
        remains = clone_object("/d/Ansalon/goodlund/nethosak/chislev_temple/obj/scribe");
        remains->move(this_object());
    }
    else if (remains->query_map())
    {
        remains->reset_map();
    }
}
...
create_mithas_room()
{
    ...
    add_exit(MROOM + "trail2","south",0);
    add_exit(MROOM + "forest67","west",0);
