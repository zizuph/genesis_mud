...
void create_object()
{
    ...
    set_adj("great");
    add_adj("iron");
    set_short("great iron statue");
    set_long("This is a iron statue of a minotaur. Both his hands "+
             ...
             "a very impressive piece of art. This statue looks very old "+
             "and it is covered with the corrosion.\n");

   add_item(("bracers"),"The bracers are made out of once bright iron "+
...
search_object(string what)
{
	...
	set_alarm(6.0,0.0, &write("You find nothing special except "+
                "that the statue is it's right horn.\n"));
    }
    if (!this_player()->query_prop(HAS_SEARCHED_FOURTH_STATUE))
    {
	this_player()->add_prop(HAS_SEARCHED_FOURTH_STATUE, 3);
	set_alarm(10.0,0.0, &write("You carefully examine the statue "+
                "closer, and you notice that it's missing it's right "+
                "horn.\n"));
    }
  return 0;
}
