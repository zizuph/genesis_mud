...
#define HAS_FIXED_FIRST_STATUE   "_i_fixed_first_statue_ansalon"
...
reset_mithas_room()
{
   ...
   if(!objectp(statue))
   {
      statue = clone_object(MOBJ + "statue");
...
create_mithas_room()
{
    ...
    add_exit(MROOM + "road4","north",0);
    add_exit(MROOM + "road48","south",0);
    add_exit(MROOM + "road2","west",0);
    add_exit(MROOM + "road7","east",0);

    add_search(("statue"),8,"search_statue", 1);
    add_cmd_item(({"finger"}),"attach","@@attach");
    reset_mithas_room();
}
...
{ 
   ...
   return tod_descr()+ "You are standing at an intersection "+
          ...
          "are made out of grey stones. A humid "+
          "breeze blows in from the sea, making you feel "+
          "refreshed. The road continues to north and south. "+
          "To the west you notice the piers and the road to "+
          "the east leads further into the city."+
          season_descr()+ "\n";
...
attach()
{
    ...
    if (this_player()->query_prop(HAS_FIXED_FIRST_STATUE))
    {
        write("You have already fixed this statue.\n");
        return 1;
    }
    if(!P("finger",this_player()))
    {
        write("You have nothing to attach on the statue!\n");
        return 1;
    }

    write("You carefully attach the finger on the marble statue, "+
          "amazingly the finger fits perfectly.\n");
    P("finger",this_player())->remove_object();
    this_player()->add_prop(HAS_FIXED_FIRST_STATUE, 4);
