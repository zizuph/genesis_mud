...
reset_mithas_room()
{
    if(!objectp(pirate))
    {
      pirate = clone_object(MNPC + "pirate");
...
add_item(({"water", "sea"}), "The water within the Blood Sea has "+
    ...
    add_item(("warehouse"), "The warehouse looks ancient. It has been made out "+
               "of old wooden boards, and the entire house has been build on "+
               "eight thick pillars that reach deep into the red waters of "+
               "the Bloodsea.\n");

   add_exit(MROOM + "pier3","north",0);
   add_exit(MROOM + "pier6","south",0);
   add_exit(MROOM + "warehouse","east",0);   
...
{ 
   ...
   return tod_descr()+ "You are standing on a small wooden "+
          ...
          "the Blood Sea. To the east a small warehouse "+
          "has been built on wooden pillars. The pier "+
