...
#define HAS_FIXED_FOURTH_STATUE   "_i_fixed_fourth_statue_ansalon"
...
reset_mithas_room()
{
   if(!objectp(statue))
   {
      statue = clone_object(MOBJ + "statue4");
...
create_mithas_room()
{
    ...
    add_item(({"forest"}), "The forest lies to the "+
          "south and it looks like it continues "+
          "to the southwest.\n");

    add_exit(MROOM + "road39","north",0);
    add_exit(MROOM + "road24","south",0);
    add_exit(MROOM + "road42","west",0);
    add_exit(MROOM + "road22","east",0); 
    add_search(("statue"),8,"search_statue", 1);
    add_cmd_item(({"hoof"}),"attach","@@attach");   
    reset_mithas_room();
}
...
{ 
   ...
   return tod_descr()+ "You are standing at an intersection "+
          ...
          "the southwest coner of the city while going north "+
          "would take you to the marketplace. You notice a "+
          "small forest to the south, that seems to continues "+
          "eastwards." +season_descr()+ "\n";
...
attach()
{
    if (!this_player()->query_prop(HAS_SEARCHED_FOURTH_STATUE))
    {
        write("Why would you want to attach something on a prefect statue?\n");
        return 1;
    }
    if (this_player()->query_prop(HAS_FIXED_FOURTH_STATUE))
    {
        write("You have already fixed this statue.\n");
        return 1;
    }
    if(!P("hoof",this_player()))
    {
        write("You have nothing to attach on the statue!\n");
        return 1;
    }

    write("You carefully attach the hoof on the iron statue, "+
          "amazingly the hoof fits perfectly.\n");
    P("hoof",this_player())->remove_object();
    this_player()->add_prop(HAS_FIXED_FOURTH_STATUE, 4);
