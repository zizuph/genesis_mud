...
#define ROWER_DESC ({  "sleek", "swift", "barnacled", "trim", \
                       "narrow", "broad-decked", "new", "freshly-painted", \
...
boat_long_desc()
{
    ...
    string boat_long = "A " +d2+ " sailing ship, commonly seen sailed by mariners " +
        "all over the Courrain Ocean. On its side is painted a picture of " +
        LANG_ADDART(d3) + ". ";
...
update_boat_descriptions()
{
    ...
    string boat_long = "You are sitting inside a sailing ship. " +
        "Designed by Kalaman shipwrights, this craft can be piloted by a single " +
        "skilled sailor, yet is sturdy enough to handle the high seas of the " +
        "Bloodsea of Istar. A single mast rises out of the " +
...
config_boat_inside()
{
    ...
    query_boat_in()->set_long("You are sitting inside a sailing ship. " +
        "Designed by Kalaman shipwrights, this craft can be piloted by a single " +
        "skilled sailor, yet is sturdy enough to handle the high seas of the Bloodsea of " +
    ...
    query_boat_in()->add_item(({"mast", "single mast"}), 
        ...
        "to handle the strong winds common on the Courrain Ocean and " +
        "Bloodsea of Istar. \n");
    query_boat_in()->add_item("ship", "Your standard sailing ship... a quick " +
       "vessel around the coastlines, suitable for taking a " +
