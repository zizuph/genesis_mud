...
#define MAP_ID     "_ansalon_map_piece_4"
#define FIXED_MAP3  "_ansalon_bloodsea_map3"

void
create_object()
{
    set_name(({"map", "piece", "map piece","_ansalon_tq_map"}));
    set_adj("torn");
    add_adj("three-quarters");
    add_adj("almost completed");
    set_short("almost completed map piece");
    set_long("This faded piece of parchment looks like one " +
      "three-quarters of a piece of a treasure map! Unfortunately, " +
      "you will need the final fourth quarter to be able to decifer " +
