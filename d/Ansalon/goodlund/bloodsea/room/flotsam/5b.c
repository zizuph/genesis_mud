...
reset_sea_room()
{
    set_searched(0);
     ...
     if (!objectp(water_shape)) 
     {
          water_shape = clone_object(BNPC +"water_shape");
          water_shape->move(this_object());
     }
}
...
create_sea_room()
{
    ...
    add_row_exit(FROOM + "6b", "north", 0, 1);
    add_row_exit(FROOM + "5c", "east", 0, 1);
    add_row_exit(FROOM + "4b", "south", 0, 1);
    add_row_exit(FROOM + "5a", "west", 0, 1);
    ...
    set_sailing_fail_message("The waters here are particularly treacherous. " +
        "You don't have the skill to navigate a ship here!\n");
...
long_descr()
{
    return bloodsea_desc() + query_sailing_difficulty_desc()+ "\n";
}