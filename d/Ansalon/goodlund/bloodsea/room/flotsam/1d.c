...
#define FLOTSAM     "/d/Ansalon/balifor/flotsam/room/street07"
...
create_sea_room()
{
    ...
    add_item(({"flotsam","harbour","town","port town","port","docks"}), 
        "Flotsam. A wretched hive of scum and villainy that lives " +
        "up to its name, with the wreckage of many lives ending " +
        "up in this large port town. To your south is the entry " +
        "to its harbour, with docks free for ships to make land.\n");

    add_row_exit(FROOM + "2d", "north", 0, 1);
    add_row_exit(FROOM + "1c", "west", 0, 1);
    add_row_exit(FROOM + "2e", "northeast", 0, 1);
    add_row_exit(FLOTSAM, "south", "@@sail_south", 1);
...
sail_south()
{
    write("\n\nYou navigate to a vacant dock in the harbour of " +
        "the wretched port town of Flotsam.\n\n");
    say(QCTNAME(TP)+ " navigates to a vacant dock in the harbour " +
        "of Flotsam.\n");
    return 0;
}
...
short_descr()
{
    return bloodsea_short()+ ", before Flotsam harbour";
}
...
long_descr()
{
    return bloodsea_desc()+ "You find yourself within the protective confines " +
        ...
        "To your south you can make out the harbour of a large town - Flotsam, " +
        "a rough place home to those sailors and pirates foolhardy enough to " +
        "sail the Bloodsea. "
