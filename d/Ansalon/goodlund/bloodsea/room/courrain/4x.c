...
create_courrain_room()
{
    ...
    add_item(({"peninsula","kern","mainland","kern peninsula"}),
       "Far to the southwest you can just make out a peninsula to the " +
       "mainland of Ansalon, part of the ogre empire of Kern.\n");

    add_row_exit(CROOM + "4w", "west", 0, 1);
    add_row_exit(CROOM + "5x", "north", 0, 1);
    add_row_exit(CROOM + "4y", "east", 0, 1);
    add_row_exit(CROOM + "3x", "south", 0, 1);
...
short_descr()
{
    return courrain_short()+ ", in sight of Kern peninsula";
}
 
string
long_descr()
{
    return courrain_desc()+ "To the southwest you can just make " +
       "out Kern peninsula.\n";
}