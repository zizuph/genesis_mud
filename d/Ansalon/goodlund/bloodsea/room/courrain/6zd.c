...
reset_courrain_room()
{
     ...
     if (!objectp(ship)) 
     {
          ship = clone_object(BOBJ +"corsair2");
     ...
     if (!objectp(captain)) 
     {
         captain = clone_object(BNPC + "corsair_capt2");
...
create_courrain_room()
{
    ...
    add_row_exit(CROOM + "6zc", "west", 0, 1);
    add_row_exit(CROOM + "7zd", "north", 0, 1);
    add_row_exit(CROOM + "6ze", "east", 0, 1);
    add_row_exit(CROOM + "6zd", "south", "@@no_go_saifhum");
