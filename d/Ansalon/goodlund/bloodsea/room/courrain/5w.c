...
create_courrain_room()
{
    ...
    add_item(({"peninsula","kern","mainland","kern peninsula"}),
       "Far to the south you can just make out a peninsula to the " +
       "mainland of Ansalon, part of the ogre empire of Kern.\n");

    add_row_exit(CROOM + "5v", "west", 0, 1);
    add_row_exit(CROOM + "6w", "north", 0, 1);
    add_row_exit(CROOM + "5x", "east", 0, 1);
    add_row_exit(CROOM + "4w", "south", 0, 1);
...
long_descr()
{
    return courrain_desc()+ "To the south you can just make " +
       "out Kern peninsula.\n";
}