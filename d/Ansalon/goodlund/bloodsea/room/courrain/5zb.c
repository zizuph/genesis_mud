...
create_courrain_room()
{
    ...
    add_item(({"island","saifhum"}),
       "To your east lies the island of Saifhum, the home of the sea " +
       "barbarians. You notice the waters to the east ripple considerably, " +
       "indicating an extensive underwater reef surrounding the northern " +
       "part of the island.\n");
    add_item(({"reef","underwater reef"}),
       "It seems like an underwater reef runs around the western side " +
       "of the island of Saifhum, effectively blocking ships from sailing " +
       "to that part of the island.\n");

    add_row_exit(CROOM + "5za", "west", 0, 1);
    add_row_exit(CROOM + "6zb", "north", 0, 1);
    add_row_exit(CROOM + "5zc", "east", "@@no_go_saifhum");
    add_row_exit(CROOM + "4zb", "south", "@@no_go");
...
long_descr()
{
    return courrain_desc()+ "To your east you can just make " +
