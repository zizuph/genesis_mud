...
{ 
    ...
    set_long("This nightmarish creature looks at first glance to be a huge " +
        "wolf with greasy black fur, that seems to smoulder with an unholy " +
        "fire. It is hideously misshapen, however, having glowing crimson " +
        "eyes and unnaturally large fangs.\n");
    ...
    add_leftover(DARM + "pelt2.c", "pelt", 1, 0, 0, 1); 

    /* Wep_type,      to_hit,   pen,  Dam_type,   %usage,   attack_desc */ 
    set_attack_unarmed(A_BITE,  35,  40, W_IMPALE, 70, "sharp fangs"); 
    ...
    set_stats(({ 120,145,120,70,50,150 })); 

    set_act_time(40); 
    add_act("growl"); 
    add_act("emote raises its head and howls eerily!"); 
    add_act("emote snarls aggressively.");
    add_act("emote growls at you menacingly, a wisp of smoke trailing " +
        "from its mouth.");
} 
