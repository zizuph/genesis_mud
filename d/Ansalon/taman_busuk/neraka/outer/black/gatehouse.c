...
{
    if (!objectp(g1))
        g1 = clone_npc(GET_RANDOM_NAME(NT_OUTERSOLDIER),"black");
    if (!objectp(g2))
        g2 = clone_npc(GET_RANDOM_NAME(NT_OUTERSOLDIER),"black");
    if (!objectp(g3))
        g3 = clone_npc(NNPC + "humsergeant","black");
}
...
create_neraka_room()
{
    ...
    set_long("You are standing in the gatehouse, a large room within " +
        "the city wall. A doorway to the northeast leads out, and a " +
    ...
    add_exit(NOUTER+"black/b5","northeast");
    add_exit(NOUTER+"black/abovegate","up", "@@go_up");
