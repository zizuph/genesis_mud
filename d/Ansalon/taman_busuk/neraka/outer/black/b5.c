...
create_neraka_room()
{
    set_short("Intersection outside the Black Army gatehouse");
    set_extra_long("You are at the Black Army gates that lead into " +
        "the inner city of Neraka. The road intersects here, one " +
        "branch leading west through the gates into the city, while " +
        "the other two branches lead northeast and southeast. There are " +
    ...
    add_item(({"walls","city walls","inner city walls","inner walls"}),
        ...
        "a few slitted windows along them. The gatehouse that the Black " +
    ...
    add_item("camp","The Black Army camp is spread out east of here.\n");

    add_exit(NOUTER+"black/b4","southeast");
    add_exit(NOUTER+"black/b8","northeast");
    add_exit(NOUTER+"black/gatehouse","southwest","@@enter_gatehouse", 1, 1);
    
    gate = clone_object(NOUTER+"black/out_gate.c");
    gate->move(TO);
...
{
    if(!(objectp(guard1)))
        guard1 = clone_npc(NNPC+"gatesivak.c","black");
    if(!(objectp(guard2)))
        guard2 = clone_npc(NNPC+"gatesivak.c","black");
}
...
sound_alarm()
{
    (NERAKA_ADMIN)->attack_on_gate("black");
}
