...
create_neraka_room()
{
	set_short("Outside the White Army gatehouse");
    set_extra_long("You are at the White Army gates that lead into " +
        "the inner city of Neraka. The dirt road you stand upon leads " +
        "northwest back to the camp, or southeast into the city. There are " +
        ...
        "There is a doorway to the east leading into the gatehouse " +
        "proper.");
    set_quarter(WHITE);
    ...
    add_item(({"walls","city walls","inner city walls","inner walls"}),
        ...
        "a few slitted windows along them. The gatehouse that the White " +
    ...
    add_item("camp","The White Army camp is spread out west and north of here.\n");

    add_exit(NOUTER+"white/w6","northwest");
    add_exit(NOUTER+"white/gatehouse","east","@@enter_gatehouse", 1, 1);
    
    gate = clone_object(NOUTER+"white/out_gate.c");
    gate->move(TO);
    reset_neraka_room();
}
...
{
    if(!guard1)
        guard1 = clone_npc(NNPC+"gatesivak.c","white");
    if(!guard2)
        guard2 = clone_npc(NNPC+"gatesivak.c","white");
}
...
sound_alarm()
{
    (NERAKA_ADMIN)->attack_on_gate("white");
}
