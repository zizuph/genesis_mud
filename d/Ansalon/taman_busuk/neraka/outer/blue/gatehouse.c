...
{
    if (!objectp(g1))
        g1 = clone_npc(GET_RANDOM_NAME(NT_OUTERSOLDIER),"blue");
    if (!objectp(g2))
        g2 = clone_npc(GET_RANDOM_NAME(NT_OUTERSOLDIER),"blue");
    if (!objectp(g3))
        g3 = clone_npc(NNPC + "humsergeant","blue");
}
...
create_neraka_room()
{
    ...
    add_exit(NOUTER+"blue/b5","northeast");
    add_exit(NOUTER+"blue/abovegate","up", "@@go_up");
