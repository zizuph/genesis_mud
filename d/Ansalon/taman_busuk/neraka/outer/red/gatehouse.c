...
{
    if (!objectp(g1))
    	g1 = clone_npc(GET_RANDOM_NAME(NT_OUTERSOLDIER),"red");
    if (!objectp(g2))
    	g2 = clone_npc(GET_RANDOM_NAME(NT_OUTERSOLDIER),"red");
    if (!objectp(g3))
        g3 = clone_npc(NNPC + "humsergeant","red");
}
...
create_neraka_room()
{
    ...
    set_long("You are standing in the gatehouse, a large room within " +
        "the city wall. A doorway to the southeast leads out, and a " +
    ...
    add_exit(NOUTER+"red/r3","southeast");
    add_exit(NOUTER+"red/abovegate","up", "@@go_up");
