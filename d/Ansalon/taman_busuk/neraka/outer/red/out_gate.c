...
create_door()
{
    ...
    set_door_id("neraka_red_outer");
    set_other_room(NOUTER + "red/tunnel");
    set_open(1);
...
pass_door(string arg)
{
    ...
    if(query_open() && TP->query_dragonarmy_division() == "red")
    {
	write("As a member of the Red Dragonarmy, you pass through the " +
