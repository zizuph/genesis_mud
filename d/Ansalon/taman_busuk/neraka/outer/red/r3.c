...
create_neraka_room()
{
	set_short("Outside the Red Dragonarmy gatehouse");
    set_extra_long("You are at the Red Dragonarmy gates that lead into " +
        "the inner city of Neraka. The dirt road you stand upon leads " +
        "south back to the camp, or north into the city. There are " +
        ...
        "There is a doorway to the northwest leading into the gatehouse " +
        "proper.");
	set_quarter(RED);
    ...
    add_item(({"walls","city walls","inner city walls","inner walls"}),
        ...
        "a few slitted windows along them. The gatehouse that the Red " +
    ...
    add_item("camp","The Red Dragonarmy camp lies southwards of here.\n");

    add_exit(NOUTER+"red/gatehouse","northwest","@@enter_gatehouse", 1, 1);
    add_exit(NOUTER+"red/r4","south",0,0,0);

	gate = clone_object(NOUTER+"red/out_gate.c");
    gate->move(TO);
    reset_neraka_room();
}
...
{
    if(!guard1)
        guard1 = clone_npc(NNPC+"gatesivak.c","red");
    if(!guard2)
        guard2 = clone_npc(NNPC+"gatesivak.c","red");
}
...
sound_alarm()
{
    (NERAKA_ADMIN)->attack_on_gate("red");
}
