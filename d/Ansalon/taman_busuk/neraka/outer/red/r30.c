...
create_neraka_room()
{
    ...
    set_extra_long("You are walking on a dirt road in the Red Army camp. " +
        "There is a tent to the southwest, while the camp spreads out in the other " +
        "directions.");
    set_quarter(RED);
    add_item_dirtroad();
    add_item_camp();
    ...
    add_exit(NOUTER+"red/t3","southwest","@@msg@@",0,0);
    add_exit(NOUTER+"red/r26","northeast",0,0,0);
}
