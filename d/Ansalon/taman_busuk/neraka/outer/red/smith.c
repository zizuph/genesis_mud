...
create_neraka_room()
{
    set_short("A steamy smithy");
    set_long("You have entered a hot, sooty smithy. In the middle of the room, " +
             "the smith stands by his forge. You notice a small sign pinned to one of the " +
             "walls.\n");
    set_quarter(RED);
    add_item_inside();
    add_item(({"list","pricelist","sign","sign sign"}), "@@sign_long");
    add_item(({"forge", "large forge"}), "A mandatory piece of equipment in any smithy.\n");  
    add_item(({"smith", "blacksmith", "man"}),
               "This able-looking man is the resident blacksmith, hard at work by his forge.\n");
    
    add_exit(NOUTER+"red/r11", "northeast","@@msg@@",-1);
...
msg()
{
    write("You step out of the smithy.\n");
    return 0;
}
...
discriminate()
{
    ...
    if (race == "elf" || race == "hobbit" || race == "gnome")
      return 115;
    return 105;
}
