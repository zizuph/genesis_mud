...
set_up_allowed()
{
    ...
    for (x = T_GENERAL ; x <= T_RECRUIT ; x++)
        add_allowed(x, T_TEAMLEADER, ({ "west" }), "black");
...
create_neraka_room()
{
    ...
    set_extra_long("You are in the east part of the Temple Square. " +
        "The square stretches south and north of here. " +
    ...
    setuid();
    seteuid(getuid());
    clone_object( DOOR + "bkaent2")->move(TO);

    set_up_allowed();
    reset_neraka_room();

    add_exit(NINNER + "s21", "south");
    add_exit(NINNER + "s23", "north");
