...
create_krynn_monster()
{
    ...
    set_long("A huge humanoid creature, a hill giant is not very bright " +
        ...
        "This formidable killing machine has joined the @@query_Color@@ " +
        "Dragonarmy.\n");
    ...
    set_stats(({300+random(80),130+random(20),220+random(20),110+random(20),110+random(20),165+random(30)}));
    set_hp(query_max_hp());
    ...
    add_ask("close gate","@@close_the_gate@@", 1);
    add_ask("open gate","@@open_the_gate@@", 1);
    add_ask("close portcullis","@@close_the_gate@@", 1);
    add_ask("open portcullis","@@open_the_gate@@", 1);
}
...
close_the_gate()
{
      ...
      this_player()->query_wiz_level())
  {
    ...
    {
      command("say Immediately My Lord!");
    }
    else
    {
      command("say Immediately My Lady!");
    }
    command("close portcullis");
  }
}
...
special_attack(object enemy)
{
    ...
    if (attacktype == 0)
    {
	    hitres = enemy->hit_me(300+random(300), W_BLUDGEON, TO, -1);
