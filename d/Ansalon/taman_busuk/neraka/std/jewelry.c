...
#define ANSALON_JEWELRY_RHAND  "_ansalon_jewelry_rhand"
#define ANSALON_JEWELRY_LHAND  "_ansalon_jewelry_lhand"
#define ANSALON_JEWELRY_LARM   "_ansalon_jewelry_larm"
#define ANSALON_JEWELRY_RARM   "_ansalon_jewelry_rarm"
#define ANSALON_JEWELRY_LLEG   "_ansalon_jewelry_lleg"
#define ANSALON_JEWELRY_RLEG   "_ansalon_jewelry_rleg"
#define ANSALON_JEWELRY_NECK   "_ansalon_jewelry_neck"
#define ANSALON_JEWELRY_WAIST  "_ansalon_jewelry_waist"
#define ANSALON_JEWELRY_SUBLOC "_ansalon_jewelry_subloc"
...
int jewelry_value;
string jewelry_slot_name;
string *ok_jslots;
string jewelry_gem;
string jewelry_metal;
int worn;

int wear_the_jewel(object ob, object me, string str);
...
do_remove(string str)
{
    ...
    mixed *jslot,*pslot;
    NF("Remove what?\n");
    if(!parse_command(query_verb()+" "+str,TP,"'"+query_verb()+"' %i",jewels))
        return 0;
    if(!sizeof(jewels))
        return 0;
    if(jewels[0] < 0)
        jewl = jewels[(-jewels[0])];
    else
        jewl = jewels[1];
    if(!jewl)
        return 0;    
...
do_wear(string str)
{
    ...
    object jewl;
    NF("Wear what?\n");     
    if(!parse_command(query_verb()+" "+str,TP,"'"+query_verb()+"' %i %s",jewels))
        return 0;
    if(!sizeof(jewels))
        return 0;
    if(jewels[0] < 0)
    ...
    if(jewels[0] == 0)
    {
        for(i = 1; i<sizeof(jewels);i++)
        {
            if(jewels[i]->id("jewelry"))
            {
                find_player("sarr")->catch_msg("Jewel: "+i+"\n");
                wear_the_jewel(jewels[i],TP,str);
            }
        }
        return 1;
    }
    if(wear_the_jewel(jewl,TP,str))
        return 1;
    return 0;
}
...
int
wear_the_jewel(object jewl, object me,string str)
{
    mixed *jslot,*pslot;
    string *str1,str2;
    set_this_player(me);
