 ...
 * Class:       ??
 * Ingredients: One pearl
 * Mana:        ??
 * Tax:         N/A - this is for an NPC only!
 * Skills:      SS_ELEMENT_LIFE    14
...
public void pidentify_message(object caster, object *targets);

public int
create_spell_pidentify(string arg)
{
    string whom, what;
    ...
    set_spell_message(pidentify_message);
}

public int
do_pidentify(string str)
{
//    if (check_combat())
//        return 0;
...
public void
pidentify_message(object caster, object *targets)
{
    ...
    {
        if (caster->query_npc())
			caster->identify_success(info,tar);
        else
