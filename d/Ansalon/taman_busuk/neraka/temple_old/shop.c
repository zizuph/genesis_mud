...
#define STORE_ROOM      NTEMPLE + "shop_store_room"

mixed *standard_items =
    ({
    	// ({ "mortar" }), "/d/Genesis/std/potions/mortar", 720,
    	// ({ "vial", "empty vial" }), POTION_VIAL_OBJECT, 24,
    	({ "pouch", "component pouch" }), NERAKA + "guild/obj/comp_pouch", 120,
    	({ "pack", "backpack" }), NERAKA + "guild/obj/backpack", 480,
    	({ "needle" }), NERAKA + "guild/obj/needle", 67,
    	({ "gloves" }), NERAKA + "guild/obj/leat_gloves", 240,
        ({ "clerical robe", "robe" }), NERAKA + "guild/obj/clerical_robe", 420,
        ({ "sheath" }), NERAKA + "guild/obj/sheath", 600,
    });
...
long_descr()
{
    return "This is a small room that serves as an equipment shop " +
    "for the Priests of Takhisis. Here standard equipment can be purchased " +
    "for a reasonable sum. There is a sign hanging on one wall. A doorway " +
    "leads south out of the room.\n";
}
