...
create_temple_room()
{
    ...
    set_long("You are standing in Great Hall of Audience. The vast chamber " +
        ...
        "foor high platform facing the middle of the hall. To the west " +
        "and southeast are two arched openings which are the Green and " +
    ...
    add_item(({"openings", "arched openings"}),
        "These large arched openings are heavily guarded and seem to lead to " +
        "antechambers for the Green and Red Dragonarmies.\n");
    ...
    add_exit( TEMPLE + "hall5.c", "north" );
    add_exit( TEMPLE + "hall8.c", "east" );
    add_exit( GREEN + "achamber.c", "west" );
    add_exit( TEMPLE + "altar.c", "northeast" );
    add_exit( RED + "achamber.c", "southeast" );
}
