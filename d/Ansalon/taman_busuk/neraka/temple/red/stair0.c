...
set_up_allowed()
{
    ...
    for (x = T_GENERAL ; x <= T_RECRUIT ; x++)
        add_allowed(x, T_TEAMLEADER, ({ "up" }), "red");
...
create_temple_room()
{
    ...
    set_long("You are standing beneath ground level of the temple. " +
        ...
        "both north and southeast. To the east is a small storeroom.\n");

    add_exit( GREEN + "corr6.c", "north" );
    add_exit( RED + "store1.c", "east" );
    add_exit( RED + "corr4.c", "southeast" );
    add_exit( RED + "stair1.c", "up", "@@check_guards" );
