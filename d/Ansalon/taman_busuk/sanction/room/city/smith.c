/* Torqual, smithy for sanction city */
/* Navarre July 3rd 2006, fixed typo in long description */
...
{
    set_short("Cave with lava stream");
    set_long("The cave is rather vast. Irregular walls run down into the "+
	     "darkness, illuminated only by the glow from red-hot streams " +
	     "of lava. A rather large, dark shape stands near the edge " +
	     "of melted rock. Someone posted a sign at the entrance.\n");

    add_item(({"list","pricelist","sign"}), "@@sign_long");
    add_cmd_item(({"list","pricelist","sign"}), "read", "@@sign_long");
    add_item(({"cave"}),
      "It glows a fiery-red colour. Shades of all sizes, cast by glow from "+
      "lava-stream, run in whild dance across the walls of the cave." +
      "Dark stone of walls is dry and hot.\n");
    add_item(({"shape", "dark shape"}),"A piece of granite raising at the " +
	    "edge of the lava-stream. It looks like anvil.\n");
    add_item(({"lava", "lava stream", "lava-stream", "stream of lava"}),
	    "The molten rock of one of many lava-streams that "+ 
	    "run through the city of Sanction, glows in red " +
	    "and yellow. With each breath you take, the air " +
	    "heated over the stream painfuly fills up your lungs.\n");
		    


    add_exit(SCITY + "street3","out",0);
