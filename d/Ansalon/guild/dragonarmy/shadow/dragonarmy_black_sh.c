...
{
#ifdef DRAGONARMY_BLACK_STARTLOC
    if (SW->query_default_start_location() == DRAGONARMY_BLACK_STARTLOC)
    {
	SW->set_default_start_location(SW->query_def_start());
    }
#endif

#ifdef DRAGONARMY_BLACK_SOUL
    SW->remove_cmdsoul(DRAGONARMY_BLACK_SOUL);
    SW->update_hooks();
#endif
}
...
delay_init_dragonarmy()
{
    ...
    gTitles = BLACK_TITLES;

#ifdef DRAGONARMY_BLACK_SOUL
    SW->add_cmdsoul(DRAGONARMY_BLACK_SOUL);
    SW->update_hooks();
#endif
