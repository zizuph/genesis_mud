...
{
#ifdef DRAGONARMY_GREEN_STARTLOC
    if (SW->query_default_start_location() == DRAGONARMY_GREEN_STARTLOC)
    {
	SW->set_default_start_location(SW->query_def_start());
    }
#endif

#ifdef DRAGONARMY_GREEN_SOUL
    SW->remove_cmdsoul(DRAGONARMY_GREEN_SOUL);
    SW->update_hooks();
#endif
}
...
delay_init_dragonarmy()
{
    ...
    gTitles = GREEN_TITLES;

#ifdef DRAGONARMY_GREEN_SOUL
    SW->add_cmdsoul(DRAGONARMY_GREEN_SOUL);
    SW->update_hooks();
#endif
