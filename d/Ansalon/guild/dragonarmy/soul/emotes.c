...
dsalute(string str)
{
    ...
    {
	oblist = filter(all_inventory(E(TP)) - ({ TP }), &->check_seen(TP));
	oblist = filter(oblist, &->query_dragonarmy_occ_member());
    }
...
query_dsay_understand(object who)
{
    ...
    (who->query_templar_officer()) || 
        (who->query_templar_knight()) ||
            (who->query_templar_lord()) ||
...
dsay_text(string str, string adverb, object *tar, int shout = 0)
{
	...
	write("You" + adverb + " " +
		...
		" in Draco" + qcomp + ": " + str + "\n");
        TP->gmcp_comms("dsay", 0, "You" + adverb + " " +
		(shout ? "shout" : TP->actor_race_sound()) +
		" in Draco" + qcomp + ": " + str + "\n");
    }
    else
    ...
    // display only to watchers that understand in gmcp
    for (i = 0, s = sizeof(tmp); i < s; i++)
    {
        tmp[i]->gmcp_comms_vbfc("dsay", QCTNAME(TP) + adverb + r_sound + 
            qcomp + " in Draco: " + str + "\n");
    }
    ...
    for (i = 0, s = sizeof(oblist); i < s; i++)
    {
        ...
        oblist[i]->gmcp_comms_vbfc("dsay", TP->query_The_name(oblist[i]) + 
            adverb + " " + (shout ? "shouts" : TP->race_sound(oblist[i])) +
	    qcomp + query_dsay_what_get(oblist[i], words));
    }
    ...
    tmp->gmcp_comms_vbfc("dsay", QCTNAME(TP) + adverb + r_sound + to + 
        "you in Draco: " + str + "\n");
	...
	tar[i]->catch_tell(TP->query_The_name(tar[i]) + adverb + " " +
        ...
        tar[i]->gmcp_comms_vbfc("dsay", TP->query_The_name(tar[i]) + adverb + " " +
	    (shout ? "shouts" : TP->race_sound(tar[i])) + to + "you" +
	    query_dsay_what_get(tar[i], words));
    }
}
...
{
    object *oblist;
    string *how = parse_adverb_with_space(str, "summarily", 0);

    if(!strlen(how[0]))
    {
      if(this_player()->query_dragonarmy_promotions()==0)
      {
        write("Mercenaries don't dismiss people, they get dismissed!\n");
        return 1;
      }
      write("You"+how[1]+" indicate that everyone of lower rank are now excused.\n");
      allbb(how[1]+" indicates that everyone of lower rank are now excused!");
      return 1;
    }
    ...
    if(oblist[0]->query_guild_member("Priests of Takhisis"))
    {
      write("It would probably be wiser to treat the Priests of Takhisis "+
            "with more respect!\n");
      return 1;
    }
    if((oblist[0]->query_dragonarmy_promotions()) >= (this_player()->query_dragonarmy_promotions()))
    {
      write("It is not appropriate to dismiss someone of similar or higher rank.\n");
      return 1;
    }
    actor("With a small salute, you"+how[1]+" dismiss", oblist,
	  " from your presence.");
    target(how[1]+" dismisses you with a small salute.",
	   oblist);
    all2act(how[1]+" dismisses ", oblist,
            " with a small salute.");
    return 1;
