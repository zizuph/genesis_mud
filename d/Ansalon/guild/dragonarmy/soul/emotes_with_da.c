...
emotes_cmdlist()
{
    return ([
      "da"        :   "da",
      "dbow" : "dbow", "dabow" : "dbow",
      "dburp" : "dburp", "daburp" : "dburp",
      "dcheck" : "dcheck", "dacheck" : "dcheck",
      "dcheer" : "dcheer", "dacheer" : "dcheer",
       "dcongrat" : "dcongrat", "dacongrat" : "dcongrat",
      "dconvince" : "dconvince", "daconvince" : "dconvince",
      "dcry" : "dcry", "dacry" : "dcry",
      "dfare" : "dfare", "dafare" : "dfare",
      "dglare" : "dglare", "daglare" : "dglare",
      "dgreet" : "dgreet", "dagreet" : "dgreet",
      "dhalt" : "dhalt", "dahalt" : "dhalt",
      "dlook" : "dlook", "dalook" : "dlook",
      "dmarch" : "dmarch", "damarch" : "dmarch",
      "dnod" : "dnod", "danod" : "dnod",
      "dpace" : "dpace", "dapace" : "dpace",
      "dponder" : "dponder", "daponder" : "dponder",
      "dpraise" : "dpraise", "dapraise" : "dpraise",
      "drage" : "drage", "darage" : "drage",
      "dsalute" : "dsalute", "dasalute" : "dsalute",
      "dsay" : "dsay", "dasay" : "dsay",
      "dshield" : "dshield", "dashield" : "dshield",
      "dshine" : "dshine", "dashine" : "dshine",
      "dshout" : "dshout", "dashout" : "dshout",
      "dsmile" : "dsmile", "dasmile" : "dsmile",
      "dtoast" : "dtoast", "datoast" : "dtoast",
      "dvow" : "dvow", "davow" : "dvow",
    ]);
}
...
dsay_text(string str, string adverb, object *tar, int shout = 0)
{
    ...
    {
       text = TP->query_The_name(oblist[i]) + adverb +
		" " + (shout ? "shouts" : TP->race_sound(oblist[i])) +
		qcomp + query_dsay_what_get(oblist[i], words);
       who = TP->query_The_name(oblist[i]);

        oblist[i]->catch_msg(text);
        oblist[i]->gmcp_comms("say", who, text);
    }
    else
    {
        // display only to watchers that understand
        all2act(adverb + r_sound + qcomp + " in Draco: " + str + "\n",
	    0, 0, "say");
    }
    ...
    text = who + adverb + r_sound + to + "you in Draco: " + str;
    tmp->catch_msg(text  + "\n");
    tmp->gmcp_comms("say", who, text);

    tar = FILTER_PLAYERS(tar);
    for (i = 0, s = sizeof(tar); i < s; i++)
    {
        text = TP->query_The_name(tar[i]) + adverb + " " +
		(shout ? "shouts" : TP->race_sound(tar[i])) + to + "you" +
		query_dsay_what_get(tar[i], words);
	tar[i]->catch_msg(text);
        who = TP->query_The_name(tar[i]);
        tar[i]->gmcp_comms("say", who, text);
    }
}
