                ...
                int dam, mixed * items, int result)
{
    ...
    if (phurt >= 0)
    {
    	...
    	case 0:
    	    ...
    	    watcher_msg += "just barely scratches " + QTNAME(enemy) 
    	        + " as ";
    	    break;

    	case 1..2:
            enemy_msg += "grazes your " + hdesc + " as ";
            watcher_msg += "grazes " + enemy->query_possessive() + " "
                + hdesc + " as ";
    	    break;
    	case 3..5:
    	    enemy_msg += "inflicts painful wounds upon you with quick "
    	        + "double swipes as ";
            watcher_msg += "inflicts painful wounds upon " + QTNAME(enemy)
                + " with quick double swipes as ";
    	    break;
    	case 6..10:
    	    enemy_msg += "digs them deeply into your " + hdesc + " and "
    	        + "tears out a large chunk of flesh as ";
    	    watcher_msg += "digs them deeply into " + enemy->query_possessive()
    	        + " " + hdesc + " and tears out a large chunk of "
    	        + "flesh as ";
    	    break;
    	case 11..20:
    	    enemy_msg += "slashes you on the " + hdesc + " with such "
    	        + "momentum that you are lifted off the ground slightly "
    	        + "as ";
    	    watcher_msg += "slashes " + QTNAME(enemy) + " on the " + hdesc
    	        + " with such momentum that " + enemy->query_pronoun()
    	        + " is lifted off the groundly slightly as ";
    	    break;
    	case 21..30:
    	    enemy_msg += "attacks you with great ferocity and tears into "
    	        + "your " + hdesc + " with both claws and bite as ";
    	    watcher_msg += "attacks " + QTNAME(enemy) + " with great "
    	        + "ferocity and tears into " + enemy->query_possessive()
    	        + " " + hdesc + " with both claws and bite as ";
    	    break;
    	case 31..50:
    	    enemy_msg += "knocks you backwards with tremendous force "
    	        + "while heavily wounding your " + hdesc + " as ";
    	    watcher_msg += "knocks " + QTNAME(enemy) + " backwards with "
    	        + "tremendous force while heavily wounding "
    	        + enemy->query_possessive() + " " + hdesc + " as ";
    	    break;
    	case 51..75:
    	    enemy_msg += "almost literally breaks you in half as the two "
    	        + "claws pierce deeply into your " + hdesc + " as ";
    	    watcher_msg += "almost literally breaks " + QTNAME(enemy)
    	        + " in half as the two claws pierce deeply into "
    	        + enemy->query_possessive() + " " + hdesc + " as ";
    	    break;
    	case 76..99:
    	    enemy_msg += "lacerates your " + hdesc + " causing profuse "
    	        + "bleeding as ";
    	    watcher_msg += "lacerates " + enemy->query_possessive() + " "
    	        + hdesc + " causing profuse bleeding as ";
    	    break;
    	default:
            enemy_msg += "rends your " + hdesc + " ripping you literally "
                + "to pieces as ";
            watcher_msg += "rends " + enemy->query_possessive() + " "
                + hdesc + " ripping " + enemy->query_objective()
                + " literally to pieces as ";
    	}
    
        watcher_msg += dragon->query_pronoun() + " flies by and climbs back "
            + "to the sky again.\n";        
        enemy_msg += dragon->query_pronoun() + " flies by and climbs back "
            + "to the sky again.\n";        
    }
    else
    {
        enemy_msg += "badly misses you.\n";
        watcher_msg += "badly misses " + QTNAME(enemy) + ".\n";        
    }
