...
check_valid_action(object player, mixed * targets, string arg, int execute)
{
    ...
    if (!execute)
    {
        ...
        if (objectp(enemy = player->query_attack())
            && !IN_ARRAY(enemy, targets))
        {
            int skill, dexcheck;
            skill = random(player->query_skill(SS_DEFENCE) 
                           + player->query_skill(SS_DRAGONARMY_ATTACK));
            dexcheck = random(player->query_stat(SS_DEX));
            if (skill < dexcheck)
            {    
                player->catch_tell("Being already in battle, you find yourself "
...
ability_msg(object actor, mixed * targets, string arg)
{
    ...
    if (!objectp(target))
    {
        // For whatever reason, the target doesn't resolve to an actual target
        actor->catch_msg("You quickly turn the point of your " 
            + weapon->short() + " towards someone.\n");
