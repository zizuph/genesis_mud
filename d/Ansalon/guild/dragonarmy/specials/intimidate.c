 ...
 *
 * Combat benefits disabled as part of the 2017 review of the guild.
 * Is now effectively an emote with a cooldown. - Arman 2017.
 */
...
config_ability()
{
    ...
    set_ability_offensive(0); // changed to be a non-combat ability
                ...
                mixed * targets, int * resist, mixed * results)
{
    ...
    enemy = targets[0];
    player_msg = "You aggressively shout at " + QTNAME(enemy)
        + " and invoke the wrath of the Blue Lady upon "
        + enemy->query_objective() + ". " ;
    enemy_msg = QCTNAME(actor) + " aggressively shouts curses "
        + "at you and invokes the wrath of " +HIS(actor)+ " Dragon Highlord, " +
          "the Blue Lady. ";
    watcher_msg = QCTNAME(actor) + " aggressively "
        + "shouts curses at " + QTNAME(enemy) + " and invokes the "
        + "wrath of " +HIS(actor)+ " Dragon Highlord, the Blue Lady. ";

/*  Intimidate disabled as part of the Dragonarmy reviews of 2017.
...
*/
    ...
    if (enemy->query_guild_member("Solamnian Knights"))
       if (actor->resolve_task(TASK_EXACTING,
                            ({ TS_STR, TS_DIS }),
                            enemy,
                            ({ TS_DIS, TS_STR })) > 0)
       {
       ...
       else if (actor->resolve_task(TASK_HARD,
                            ({ TS_STR, TS_DIS }),
                            enemy,
                            ({ TS_DIS, TS_STR })) > 0)
       {
    ...
    {
        player_msg += capitalize(enemy->query_pronoun()) + " smirks at you "
            + "and ignores your attempts to dishearten " +HIM(enemy)+ ".\n";
        enemy_msg += "You are not affected by such vain attempts to "
            + "dishearten you.\n";
        watcher_msg += capitalize(enemy->query_pronoun()) + " smirks at "
            + actor->query_objective() + " and does not seem affected.\n";
    }
...
{
    write("You are already trying to invoke the wrath of your "
        + "Highlord!\n");
}
