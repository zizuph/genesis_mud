/* Based on the pigeon code from Vingaard Keep  */

inherit "/std/creature";
inherit "/std/combat/unarmed";

#include <wa_types.h>
#include <stdproperties.h>
#include <macros.h>
#include "/d/Krynn/solamn/vin/local.h"

/* The temporary room in the air when the falcon is flying. */
#define AIR_ROOM    VROOM + "air_room"

/* The home of the falcon. */
#define PIGEON_HOME VROOM + "pigeon_store"

/* The message. */
#define MESSAGE_OBJ VOBJ + "message"

/* The delay between sending the message and the receiving of it. */
#define SLOWLY random(30) + 20

/* The number of times the pigeon will carry a message. */
#define TIMES 20

/* Prototypes */
int set_times_to_fly(int i);
int send_mail();
int return_to(object dest);
int fly_to_someone(object dest);
void return_home();
int test_pigeon_killer(object pl);


string gMessage, gWho, gSender;
int gUsage,pigeon_busy,times,fly_alrm;
object owner;

void
create_creature()
{
    set_name("falcon");
    set_pname("falcons");
    set_race_name("falcon");
    set_adj(({"peregrine","graceful"}));
    set_short("graceful peregrine falcon");
    set_pshort("peregrine falcons");
    set_long("@@long_func");
    gUsage = 0;
    add_prop(OBJ_I_WEIGHT,350);
    add_prop(CONT_I_WEIGHT,350);
    add_prop(OBJ_I_VOLUME,350);
    add_prop(CONT_I_VOLUME,350);
    add_prop(OBJ_I_NO_GET,0);
    add_prop(OBJ_I_NO_DROP,0);
    add_prop(LIVE_I_NON_REMEMBER,1);
    add_prop(NPC_M_NO_ACCEPT_GIVE, " does not seem to be able to carry " +
	     "anything but small messages.\n");

    set_times_to_fly(TIMES);
    set_attack_unarmed(1, 35, 35, W_IMPALE, 100, "beak");
    set_hitloc_unarmed(1, ({ 15, 37, 33 }), 100, "body");
}

void
init_living()
{
    add_action("do_send","send",0);
    add_action("do_return","return",0);
    add_action("do_resend","resend",0);
}

string
long_func()
{
    if (owner && owner == this_player()) 
      return "Renowned for its speed, grace, and beauty, as well as "+
        "its rapacity, the noble peregrine falcon has been the prized "+
        "favourite of elven falconers since the Age of Dreams. Smaller " +
        "and more streamlined than hawks, this falcon has a small head, " +
        "firm compact plumage, and long pointed wings that allow it to " +
        "fly at great speed.\nYou can send messages to other players " +
        "with this falcon by typing 'send <player>'" +
#if TIMES > 1
	" or 'send reply'" +
#endif
	". If the falcon fails to deliver the message you can tell it to " +
	"try again with the 'resend' command. " + 
	"If you don't want the falcon anymore, you can do 'return falcon'.\n";
    if (owner && owner != this_player())
      return "This is "+owner->query_cap_name()+"'s peregrine falcon. "+
        "Renowned for its speed, grace, and beauty, as well as "+
        "its rapacity, the noble peregrine falcon has been the prized "+
        "favourite of elven falconers since the Age of Dreams. It is " +
	"however a friendly bird, and you might be able to send messages to " +
	"others by 'send <player>' or 'send reply'. If the falcon fails to " +
	"deliver the message you can tell it to try again with the 'resend' " +
	"command. You may also 'return falcon'.\n";
    return "This is a peregrine falcon. Renowned for its speed, grace, and "+
        "beauty, as well as its rapacity, the noble peregrine falcon has "+
        "been the prized favourite of elven falconers since the Age of "+
        "Dreams. You can use this bird of prey to send messages to other "+
        "players with 'send <player>'.\n";
}

int
do_send(string who)
{
    string known, intro, remem;
    object target, tp;
    
    if (!who || who == "")
      return 0;
    tp = this_player();
    if (tp != E(TO))
      return 0;
    if (gUsage >= times)
      {
	  write("The falcon refuses to deliver another message.\n");
	  return 1;
      }
    if (E(tp)->query_prop(ROOM_I_INSIDE))
      {
	  write("You can't send out the falcon when you're not out in " +
		"open air.\n");
	  return 1;
      }
    if (who == "reply")
      {
	  /* Has a message been written before? */
	  if (!gUsage)
	    {
		write("You are the first sender of a message. You cannot " +
		      "reply.\n");
		return 1;
	    }
	  who = gSender;
      }
    /* If a player knows the name of another player, let him send the pigeon.*/
    gWho = who;
    gSender = lower_case(tp->query_name());
    target = find_player(gWho);
    
    /* It is useless to write a message to someone who isn't in the game. */
    if ((!target) || (target->query_wiz_level()))
      {
	  write("There is no such player logged in!\n");
	  return 1;
      }
    pigeon_busy = 1;
    write("Enter your message.\n" +
	  "On any line you can give ** to end, or ~q to abort the message.\n");
    gMessage = "";
    write("-->");
    input_to("write_more");
    return 1;
}

int
write_more(string str)
{
    if (str == "**")
      {
	  send_mail();
	  return 1;
      }
    if (str == "~q")
      {
	  write("Message aborted.\n");
	  pigeon_busy = 0;
	  return 1;
      }
    gMessage += (str + "\n");
    write("-->");
    input_to("write_more");
    return 1;
}

int
send_mail()
{
    if (gMessage == "")
      {
	  write("Message aborted.\n");
	  return 0;
      }
    write("You give the message to the peregrine falcon.\n");
    say(QCTNAME(this_player()) + " gives a message to "
	+ this_player()->query_possessive() + " peregrine falcon.\n");
    /* turn autoreturn of pigeon off*/
    if (fly_alrm)
      remove_alarm(fly_alrm);
    fly_alrm = 0;
    tell_room(E(TO), "With a shriek, the peregrine falcon launches "+
              "itself into the air and swiftly disappears.\n");
    /* Move the object to a temporary room */
    move(AIR_ROOM);
    set_alarm(itof(SLOWLY),0.0,"deliver");
    return 1;
}

int
do_resend()
{
    object target;
    
    if (!gSender || !gWho || this_player() != find_player(gSender))
      {
	  notify_fail("You can't resend if you haven't sent first!\n");
	  return 0;
      }
    
    if (target = find_player(gWho))
      {
	  write("You convince the falcon to make yet another try.\n");
	  move(AIR_ROOM);
	  set_alarm(itof(SLOWLY),0.0,"deliver");
	  return 1;
      }
    
    notify_fail("The falcon doesn't want to make another try.\n");
    return 0;
}

int
deliver()
{
    object target, message;
    /* Does our initial target still exist? */
    target = find_player(gWho);
    if (!target)
      {
	  /* Does our sender still exist? */
	  target = find_player(gSender);
	  if (!target)
	    {
		if (owner) 
		  { 
		      return_to(owner);
		      return 1;
		  }
		remove_object();
		return 1;
	    }
      }
    else if (E(target)->query_prop(ROOM_I_INSIDE))
      {
	  target = find_player(gSender);
	  if (!target)
	    {
		if (owner) 
		  { 
		      return_to(owner);
		      return 1;
		  }
		remove_object();
		return 1;
	    }
      }
    
    /* Go to the target */
    move(target);
    pigeon_busy = 0;
    if (target == find_player(gSender))
      tell_object(target, "Your graceful peregrine falcon returns, unable to find "
		  + C(gWho) + ". It lands on your shoulder and " +
		  "you retrieve your message.\n");
    else
      tell_object(target, "A graceful peregrine falcon descends from the " +
		  "sky, lands on your shoulder and gives you a message " +
		  "from " + C(gSender) + ".\n");
    /*return to owner if i'm not needed.*/
    if (owner && target != owner)
      {
	  tell_object(target, "If do not use this falcon it will return to " +
		      "it's owner again.\n");
	  fly_alrm = set_alarm(60.0,0.0,"attempt_fly_to_someone",owner);
      }
    
    tell_room(E(target),
	      "A peregrine falcon lands on " + QTNAME(target) + "'s shoulder and gives "
	      + target->query_objective() + " a message.\n",target);
    
    message = clone_object(MESSAGE_OBJ);
    message->set_message(gMessage);
    message->set_sender(gSender);
    message->move(target);
    gUsage++;
    if (gUsage >= times)
      set_alarm(10.0,0.0,"return_home");
}

int
do_return(string str)
{
    if (!str) {
	write("Return what?\n");
	return 1;
    }
    if (str == "falcon" || str == "peregrine falcon")
      {
	  write("You send the peregrine falcon up in the air again, to fly " +
		"back home.\n");
	  say(QCTNAME(this_player()) + " sends the peregrine falcon away.\n",
	      this_player());
	  /* turn autoreturn of pigeon off*/
	  if (fly_alrm)
	    remove_alarm(fly_alrm);
	  fly_alrm = 0;

	  if (this_player() != owner) 
	    fly_to_someone(owner);
	  else 
	    return_home();
	  return 1;
      }
    return 0;
}

int
query_busy()
{
    return pigeon_busy;
}

void
return_home()
{
    object room;
    if (living(room = E(TO)))
      room = E(room);
    tell_room(room,
	      "The graceful peregrine falcon launches into the air and flies away.\n");
    remove_object();
}

int
return_to(object dest)
{
    if (!dest) 
      {
	  return_home();
	  return 1;
      }
    pigeon_busy = 0;
    set_alarm(4.0,0.0,"fly_to_someone",dest);
    if (living(E(TO)))
      {
	  tell_object(E(TO),
		      "You hear a whistle from somewhere in the distance.\n");
      }
    else tell_room(E(TO),
		   "You hear a whistle from somewhere in the distance.\n");
    return 1;
}

int
attempt_fly_to_someone(object dest)
{
    if (query_busy())
      {
	  fly_alrm = set_alarm(60.0,0.0,"attempt_fly_to_someone",dest);
	  return 0;
      }
    return fly_to_someone(dest);
}

int
fly_to_someone(object dest)
{
    if (!dest) 
      tell_object(owner,"Bug - no dest.\n");
    if (living(E(TO)))
	  tell_room(E(E(TO)), "The graceful peregrine falcon launches into the air and flies " +
		    "up in the sky!\n");
    else 
      tell_room(E(TO),
		"The graceful peregrine falcon launches into the air and flies up in the sky!\n");
    if(!dest)
      {
	  remove_object();
	  return 1;
      }
    this_object()->move(dest);
    tell_object(dest,"A graceful peregrine falcon comes flying, and lands on your " +
		"shoulder.\n");
    tell_room(E(dest), "A graceful peregrine falcon comes flying, and lands on the " +
	      "shoulder of " + QTNAME(dest) + ".\n", dest);
    test_pigeon_killer(dest);
    return 1;
}

int
set_times_to_fly(int i)
{
    if (!i) i = TIMES;
    if (i > 10) i = 10;
    times = i;
    return 1;
}

int
set_owner(object player)
{
    if (!player) return 0;
    owner = player;
    return 1;
}

int
start_place()
{
    this_object()->move(PIGEON_HOME);
    tell_room(E(TO),
	      "A graceful peregrine falcon comes flying, landing on the floor.\n");
    set_alarm(15.0,0.0,"return_to",owner);
    return 1;
}

int
test_pigeon_killer(object pl)
{
    if (pl->query_prop("_player_is_pigeon_killer")) 
      {
	  tell_object(pl,"The graceful peregrine falcon shrieks at you angrily,\n"+
		      "and swiftly flies away again!\n");
	  tell_room(E(pl), "The peregrine falcon is spooked by " + QTNAME(pl) + 
		    ", and flies away again!\n",pl);
	  return_home();
      }
    return 1;
}

void
do_die(object killer)
{
    killer->add_prop("_player_is_pigeon_killer",1);
    ::do_die(killer);
}

