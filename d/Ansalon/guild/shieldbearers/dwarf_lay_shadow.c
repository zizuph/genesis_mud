...
/* Function name: acceptable_shieldbearer_member
 ...
 */
public nomask mixed
acceptable_shieldbearer_member(object who)
{
    if ( !objectp(who) )
        return 0;
 ...
 /*
    // There is some strange bug that causes Shieldbearers to get kicked
    // out. To prevent this, we no longer make it so that they get
    // kicked out if they aren't a friend and part of the clan. Instead,
    // we now added code to prevent them from using their abilities if
 ...
 */
    ...
    if(BANISHED(who))
        return "You have been banished are not considered a friend of " +
            "the Neidar Clan. You can not be a Shieldbearer!\n";
...
check_valid_members(object who)
{
    ...
    if ( stringp(why = acceptable_shieldbearer_member(who)) )
    {
        who->catch_msg(why);
