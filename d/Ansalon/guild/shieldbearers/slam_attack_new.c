 ...
 *    Lavellan, 2011/03/25
 * 
 *  - Changed stun length and applicability from driven by
 *    phurt (impossible to balance) to standard stun length and
 *    applicable upon a >= mean pen dice roll.
 *        80 tohit -> 75%
 *        >= mean pen dice roll -> 50%
 *        75% * 50% = 37.5% chance that stun will apply
 *    The stun_obj is adjusted to 37.5% hit odds so that the
 *    proper stun length can be correctly computed by standard
 *    library.
 *    
 *    Ckrik, 2022/02/01
 *
...
#define SLAM_DAMAGE_CAID 28
#define SLAM_STUN_CAID   22
#define SLAM_TIME        45
...
// Because the raw pen is not reported in special_did_hit, we
// pass them via side channel
static float reported_pen_mean;
static int reported_pen_threshold;
...
{
    ::config_ability();
    set_combat_aid(SLAM_DAMAGE_CAID); // original was 30.61% according to docs
    ...
    // Can slam once every 45 seconds on average
...
{
    ::report_resolve_ability(pen, dt, damage);

    int pen_roll_sides = pen / 4;

    reported_pen_mean = 4.0 * (itof(pen_roll_sides) / 2.0 - 0.5);

    // P(phit >= report_pen_threshold) ~= 50%
    reported_pen_threshold = ftoi(reported_pen_mean + 0.5);
}
                ...
                int dam, mixed * items, int result)
{
    object player, slam;
    string shield_short;
    string attacker_desc, defender_desc, watcher_desc;
    ...
    if (phurt >= 0) 
    {
            ...
            (player->query_race_name() == "gnome") )
        {
            attacker_desc = "With a shriek, you run into " + QTNAME(enemy) +
    	        ", slamming " + HIM(enemy) + " with your " +
    	        shield_short + ".\n";
            defender_desc = "With a shriek, " + QTNAME(player) + " runs " +
    	        "into you, slamming you with " + HIS(player) + " " + 
    	        shield_short + ".\n";
    	    watcher_desc = "With a shriek, " + QTNAME(player) + 
    	        " runs into " + QTNAME(enemy)+ ", slamming " + 
    	        HIM(enemy) + " with " + HIS(player) + " " + 
    	        shield_short + ".\n";
        }
        else
        {
    	    ...
    	    defender_desc = "With a roar, " + QTNAME(player) + " charges " +
    	        "into you, slamming you with " + HIS(player) + " " + 
    	        shield_short + ".\n";
    	    watcher_desc = "With a roar, " + QTNAME(player) + 
    	        " charges into " + QTNAME(enemy)+ ", slamming " + 
    	        HIM(enemy) + " with " + HIS(player) + " " + 
    	        shield_short + ".\n";
        }
    	...
    	{
        case 0:
            attacker_desc += QCTNAME(enemy) + " looks " +
                "unaffected, shrugging off the attack effortlessly.\n";
            defender_desc += "You are unaffected by " + HIS(player) + " attack.\n";
            watcher_desc += QCTNAME(enemy) + " looks " +
                "unaffected, shrugging off the attack effortlessly.\n";
    	    break;
        ...
        case 1..6:
            if (in_cooldown)
            {
                attacker_desc += "Your " + shield_short + " crashes into "
                    + QTNAME(enemy) + " and bruises " + HIM(enemy)
                    + " deeply.\n";
                defender_desc += QCTNAME(player) + " crashes into you with "
                    + HIS(player) + " " + shield_short + " and bruises "
                    + "you deeply.\n";
                watcher_desc += QCTNAME(player) + " crashes into "
                    + QTNAME(enemy) + " with " + HIS(player) + " " 
                    + shield_short + " and bruises " + HIM(enemy) 
                    + " deeply.\n";
            }
            else
            {
                attacker_desc += QCTNAME(enemy) + " stumbles, " +
        	        "knocked slightly off balance by the attack.\n";
           	    defender_desc += "You stumble, knocked slightly off balance " +
    	            "by the attack.\n";
        	    watcher_desc += QCTNAME(enemy) + " stumbles, " +
        	        "knocked slightly off balance by the attack.\n";
        	}
            break;
        ...
        case 7..18:
            if (in_cooldown)
            {
                attacker_desc += "Your momentum propels your " 
                    + shield_short + " into the side of " + QTNAME(enemy) 
                    + " and the impact causes " + HIM(enemy) + " to "
                    + "trickle blood.\n";                    
                ...
                watcher_desc += QCTNAME(player) + " forces " + HIS(player)
                    + " " + shield_short + " into " + QTNAME(enemy) 
                    + " and smashes " + HIS(enemy) + " sides so hard that "
                    + QTNAME(enemy) + " begins to trickle blood.\n";
            }
            else
            {
        	    attacker_desc += QCTNAME(enemy) + " struggles " +
        	        "to keep " + HIS(enemy) + " footing.\n";
        	    defender_desc += "You struggle to keep your " +
        	        "feet after the mighty attack.\n";
        	    watcher_desc += QCTNAME(enemy) + " struggles " +
        	        "to keep " + HIS(enemy) + " footing.\n";
        	}
            break;
        case 19..36:
            if (in_cooldown)
            {
                attacker_desc += "You hold your " + shield_short + " tightly "
                    + "as you crash forcefully into " + QTNAME(enemy)
                    + " crushing " + HIS(enemy) + " bones.\n"; 
                defender_desc += QCTNAME(player) + " crashes into you, "
                    + "forcing " + HIS(player) + shield_short + " into your "
                    + "body so hard it you hear your bones crack.\n";
                watcher_desc += QCTNAME(player) + " crashes into "
                    + QTNAME(enemy) + ", forcing " + HIS(player) + " "
                    + shield_short + " into " + QTNAME(enemy) + "'s body "
                    + "so hard you hear bones crack.\n";
            }
            else
            {        
                if ( enemy->query_humanoid() )
        	    {
        	        attacker_desc += QCTNAME(enemy) + " retreats " +
        	            "backwards fearfully after such a mighty hit!\n";
        	        defender_desc += "Your knees buckle and you " +
        	            "fall backwards, unable to keep your footing.\n";
        	        watcher_desc += QCTNAME(enemy)+ "'s knees " +
        	            "buckle, forcing " + HIM(enemy) + " backwards onto " +
        	            "the ground.\n";
        	    }
        	    else
        	    {
        	        attacker_desc += QCTNAME(enemy) + " retreats " +
        	            "backwards fearfully after such a mighty hit!\n";
        	        defender_desc += "You retreat backwards fearfully " +
        	            "after such a hard hit!\n";
        	        watcher_desc += QCTNAME(enemy) + " retreats " +
        	            "backwards fearfully after such a mighty hit!\n";
        	    }
        	}
            break;
        default:
            if (in_cooldown)
            {
                attacker_desc += "You pull your " + shield_short
                    + " up at just the right moment and hit " + HIM(enemy)
                    + " squarely in the head causing " + HIM(enemy)
                    + " to spew blood.\n";
                defender_desc += QCTNAME(player) + " brings up " + HIS(player) + " "
                    + shield_short + " at just the right moment. The blow "
                    + "hits you with great force and causes you to spew "
                    + "blood.\n";
                watcher_desc += QCTNAME(player) + " brings up " + HIS(player) + " "
                    + shield_short + " at just the right moment. The blow "
                    + "hits " + QTNAME(enemy) + " with great force and "
                    + "causes " + HIM(enemy) + " to spew blood.\n";
            }
            else
            {        
        	    attacker_desc += QCTNAME(enemy)+ " collapses to the " +
        	        "ground, dazed and totally unaware of " + HIS(enemy) +
        	        " surroundings.\n";
        	    defender_desc += "You feel dizzy... Is that the " +
        	        "ground coming up to meet you?\n";
        	    watcher_desc += QCTNAME(enemy)+ " collapses to the " +
        	        "ground, dazed and totally unaware of " + HIS(enemy) +
        	        " surroundings.\n";
        	}
            break;
        ...
        // If slam is >= mean dice roll, we apply stun
        if (!in_cooldown && phit >= reported_pen_threshold)
        {
            ...
            send_debug_message("slam", "Slam attack cycle for "
                             + capitalize(player->query_real_name())
                             + " is " + stun_cycle);
            
            /* If the slam was strong enough -> we stun the enemy */
            seteuid(getuid(TO));
           
            // We apply a stun to the opponent.    
            slam = clone_object(GUILD_DIR + "slam_stun_obj");
            slam->set_stun_chance((100 - calc_hitme_modifier()) / 2);
            slam->set_stun_combat_aid(SLAM_STUN_CAID);
            ...
            if (slam->start())
            {
                ...
                send_debug_message("slam", "Stunning enemy for a " + stun_cycle
                                   + " seconds stun cycle.");                
    
                if (OPTIONS_MGR->query_option(OPT_POWER_STUN, player->query_real_name()))
                {
                    ...
                    stun_cycle = ftoi(itof(stun_cycle) * size_adj);
                    if (size_adj < 1.0)
                    {
                        send_debug_message("slam", "Power slam time adjusted "
                                           + "downwards to " + stun_cycle + " because of "
                                           + "size differences.");
                    }
                
                    // If player has specified to execute a power stun, then both
                    // the attacker and the target will be stunned.
                    slam = clone_object(GUILD_DIR + "slam_stun_obj_power");
                    slam->set_stun_cycle_time(stun_cycle);
                    slam->set_effect_caster(player);
                    slam->set_effect_target(enemy);
                    slam->set_phurt_value(phurt);
                    ...
                    enemy->catch_msg(QCTNAME(player) + " swings " + HIS(player)
                                     ...
                                     + "of the impact.\n");
                    player->tell_watcher(QCTNAME(player) + " swings " + HIS(player)
                                         + " " + shield_short + " around, places " + HIS(player)
                                         + " head inside and forces " + HIS(player) + " whole "
                                         + "body into " + QTNAME(enemy) + "! Both are dazed "
...
notify_prep_time(float prep_time, object actor, mixed * targets)
{
    ...
    set_alarm(prep_time - running_time, 0.0, 
              &slam_message(actor, targets));
    send_debug_message("slam", ftoi(prep_time - running_time) + " is the time before running.");
}
...
public int query_combat_aid_modifier()
{
    ...
    player = this_player();
    // Shieldbearers will not get the maximum combat aid
    // if they cannot train to 100 skill. This is for
    // non-Neidars
        
    return player->query_skill(SHIELDSLAM_SPECIAL);
}
...
query_ability_time()
{
    ...
    player = this_player();
    if (objectp(player)
        && OPTIONS_MGR->query_option(OPT_SHORT_SLAM, 
