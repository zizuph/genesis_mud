...
set_up_skills()
{
    ...
    sk_add_train(SS_DEFENCE, "survive in combat", "defence", 0, 50);
    sk_add_train(SS_PARRY,  "parry with a weapon","parry",   0, 40);
...
sk_query_max(int snum, int silent)
{
    ...
    {
        // All Shieldbearers can train this to 100
        return 100;
    }
 ...
 */
    ...
    // Copied from dwarf_lay_shadow where we commented this check out.
    // We need the check, but here is 'safer'.
    if (!TP->query_guild_member(NEIDAR_CLAN) &&
        !TP->test_bit("Ansalon", 3, 13))
    {
      notify_fail("You are not considered a friend of the Neidar Clan. " +
    ...
    if ( stringp(why = shadow->acceptable_shieldbearer_member(TP)) )
    {
        write(why);
    }
    else
	...
	// We are making the shieldbearer a friend of the neidars.
	// This is mainly done due to a bug we encountered where
	// the shieldbearer shadow was loaded before the neidar shadow
	// and a check was called to see if he was a neidar
	// and came back as a no. So suddenly he was kicked out of 
	// shieldbearers. This should have no impact, but it would
	// safe guard that strange behavior we experienced.
        TP->test_bit("Ansalon", 3, 13);
    }
