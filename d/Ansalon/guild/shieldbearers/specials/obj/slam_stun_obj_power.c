...
public void
config_stun_effect(int caid, int control)
{
    int con_aid;
    
    con_aid = (((control * 100) / (control + 70)) * caid) / 100;
    
    if (control > 49)
        con_aid += ((control * 100) / ((100 - control) + 100)) / 5;
        
    caid = max(0, (caid * con_aid) / 50);
    
    find_player("carnak")->catch_msg("Caid: " + caid + "\n");
    //set_stun_combat_aid(caid);
}
