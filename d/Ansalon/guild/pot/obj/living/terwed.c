...
do_reward(int quest, object questobj, object quester, int reward)
{
    ...
    switch(quest)
    {
    	case Q_VERMINAARD:
    	    ...
    	    quester->add_prop(POT_GOT_NEW_SPELL, ({"pheal", PHEAL_ID}));
//    	    quester->set_skill(PS_SPELLS, quester->query_skill(PS_SPELLS) | PHEAL_ID);
...
answer_member(object asker, string member)
{
    ...
    if (member == "council")
    {
        command("psay The positions in our council consists of the " +
            "Nightlord or lady, the Chancellor, the Inquisitor, the " +
