...
inherit "/std/room";
inherit "/d/Ansalon/guild/pot/lib/rank";
...
{
    string tm = ctime(time()), str, logpath = "/d/Ansalon/log/pot/";
...
{
    return add_council_member(name, "leader");
}
...
set_planeswalk_loc(string name, string room, string loc)
{
    ...
    if (!pointerp(gGuildlist[name][M_LOCS]))
        gGuildlist[name][M_LOCS] = allocate(PLANESWALK_LOCS);

    for (x = 0, oldest = 0 ; x < PLANESWALK_LOCS ; x++)
    {
        if (sizeof(gGuildlist[name][M_LOCS]) &&
            sizeof(gGuildlist[name][M_LOCS][x]) &&
            !sizeof(gGuildlist[name][M_LOCS][(x + 1) % PLANESWALK_LOCS]))
            oldest = (x + 1) % PLANESWALK_LOCS;

        if (sizeof(gGuildlist[name][M_LOCS]) &&
            sizeof(gGuildlist[name][M_LOCS][x]) &&
            sizeof(gGuildlist[name][M_LOCS][(x + 1) % PLANESWALK_LOCS]) &&
            gGuildlist[name][M_LOCS][x % PLANESWALK_LOCS][3] <
            gGuildlist[name][M_LOCS][(x + 1) % PLANESWALK_LOCS][3])
            oldest = x;
    }
    
    gGuildlist[name][M_LOCS][oldest] = ({ room, room->query_short(), loc,
        time() });
    save_vars();
...
query_planeswalk_loc(string name, string loc)
{
    ...
    for (x = 0 ; x < PLANESWALK_LOCS ; x++)
    {
        if (sizeof(gGuildlist[name][M_LOCS]) &&
            sizeof(gGuildlist[name][M_LOCS][x]))
            if (gGuildlist[name][M_LOCS][x][2] == loc)
...
/* Make phelp council available
/* Make a choice of choosing council title or ordinary title just
 * as utitle instead
 *
...
do_list(string str)
{
         ...
         x < size ; x++)
    {
        str += sprintf("%-11s %3d days %3d days  %3d   %|6s  %2d.%1d %8s %-6s\n",
            ...
            "/d/Ansalon/guild/pot/lib/rank"->due_in(arr[x]),
