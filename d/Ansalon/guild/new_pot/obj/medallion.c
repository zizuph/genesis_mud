...
private mapping m_spells = ([]);

public object cmd_obj;
...
cmdsoul(void)
{
    ...
    cmd_obj = find_object(GUILD_CMDSOUL);
    return cmd_obj;
}
...
public void
m_remove_all_spells(void)
{
    cmdsoul()->remove_spells(environment());
    m_spells = ([]);
}

public void
m_add_all_spells(void)
{
    ...
    for (i = 0 ; i < sz ; i += 2)
    {
	string name = m_spell_list[i + 1];
	m_spells[name] = POT_SPELL_DIR + name;
    }
    cmdsoul()->add_spells(environment());
}
...
aspect_status()
{
    if(this_player()->query_prop(DARK_WARRIOR_ASPECT))
        return "a warrior clad in shining black armour. ";
    else if(this_player()->query_prop(DARK_LADY_ASPECT))
        return "a darkly beautiful temptress. ";
    else
        return "a five-headed dragon. ";
...
insert_gem(string str)
{
	...
	if(g[0]->query_held())
	{
	  ...
	  return 1;
        ...
        if(g[0]->query_priest_name() != TP->query_real_name())
	{
	  write("Your medallion rejects the " +g[0]->short() +
                ", as the soul gem was not created by you.\n");
	  return 1;
	}

        write("You insert the " + g[0]->short() + " in the indentation. " +
...
list_spells(void)
{
    ...
    while(i--)
    {
    	...
    	if (ob = get_ability_object(verbs[i]))
    	{
    	    desc = ({ ob->query_spell_desc() || "" });

            if (strlen(desc[0]) > 45)
                desc = explode(break_string(desc[0],45),"\n");
