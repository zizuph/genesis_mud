...
void
set_planeswalk_loc(object pl, object room)
{
    gLoc[pl] = room;
}
...
do_act(object pl)
{
    ...
    switch (random(3))
    {
        ...
        case 1:
            ...
            break;
        case 2:
            msg("Not even the slightest breeze breaks the dreary " +
                "sight of bleak dunes before you.", pl);
            break;
        case 3:
            msg("", pl);
    }
...
public string
query_teleport_destination(object player)
{
    if(!gLoc[player])
...
do_arrive(object pl)
{
    ...
    /* If you cant teleport to the destination, remove the room */
    if (gLoc[pl]->query_prop(ROOM_M_NO_TELEPORT) ||
        gLoc[pl]->query_prop(ROOM_M_NO_TELEPORT_TO) ||
        gLoc[pl]->query_prop(OBJ_I_NO_MAGIC))
        gLoc[pl] = 0;
    
    msg("You spot a shimmering portal in the distance. Suddenly " +
        "you stand right in front of it and chance going through it.",
        pl, "A shimmering portal speeds its way towards " + QTNAME(pl) +
        ". It stops just in front of " + HIM(pl) + ", and " + HE(pl) + 
        " quickly steps through it.");
    pl->move_living("M", query_teleport_destination(pl), 1, 0);
    tell_room(E(pl), "A shimmering portal briefly opens, displaying " +
        "a hellish scene of bleak dunes. "+QCTNAME(pl)+ " quickly " +
        "steps through it, and the portal closes behind " +HIM(pl)+ 
        ".\n", ({ pl }) );
    pl->remove_prop(POT_TELEPORT_DESTINATION);
}
...
remove_nonliving(object ob)
{
    if (!ob)
        return;
      tell_room(E(ob), "The " +ob->short()+ " begins to fade, and " +
        "eventually loses its material substance.\n", ({ ob }) );
      ob->remove_object();
      return;
}
