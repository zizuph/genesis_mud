/*
 * Command Soul for the Priests of Takhisis (Ansalon)
 *
 * This is the command soul that every Priest of Takhisis will
...
/*
 * This mapping is used to keep track of how often a specific player
 * used the emotes last. This is used to adjust standing.
 * The mapping is populated from the update_from_emote function.
 */
 mapping gEmotes = ([]);
...
{
    return ([
        "ptell" : POT_SPELLS + "ptell",
        "palign" : POT_SPELLS + "palign",
        "pbless" : POT_SPELLS + "pbless",
        "pblind" : POT_SPELLS + "pblind",
        "pcurse" : POT_SPELLS + "pcurse",
    ]);
}
...
do_dissect(object pl, object corpse, object knife, object paralyze, int round = 0)
{
    ...
    round++;
    set_alarm(3.5 + (itof(random(3))), 0.0,
              &do_dissect(pl, corpse, knife, paralyze, round));
}
...
{  
find_player("navarre")->catch_tell("Stuff\n");
    if (stringp(arg))
    {
        return 0;
    }

    // We add our special line with standing to the regular 'vitals' command
    // Then we return 0 to allow for the regular 'vitals' command to go 
    // through.
    write(this_player()->query_standing_desc(this_player()) + ".\n");
    return CMD_LIVE_STATE->vitals("all");
}

/********** EMOTES **********/

/*
 * Speak with the Tongue of the Dragon - Reuses the code from the Dragonarmy
 *
 * @param str The message the player wants to speak.
 *
 * @return The converted message into dragon tongue.
 */
int
dsay(string str)
{
...
/*
 * This function lists the spells available to the priest.
 * (Just like typing 'spells').
 *
 * @param str Passed onto the 'spells' command, which currently ignores any
 *            arguments.
 *
 * @return 1 to indicate it handled the command,
 *         0 otherwise (shouldn't happen).
 */
int
prayers(string str)
{
...
/*
 * Emote for accepting a punishment, decreases your standing.
 * 
 * @param arg Should not be provided.
 *
 * @return 0 if you provide argument, 1 otherwise.
 */
int
paccept(string arg)
{
...
/*
 * Emote for when the priest is annoyed with the components contained in their
 * pouch.
 *
 * @param arg The component pouch that the player wants to show his
 *            displeasure with.
 * 
 * @return 0 if no valid component pouch is found, 1 otherwise.
 */
int
pannoyed(string arg)
{
    ...
    // If the player provided an argument for which container to be annoyed 
    // with, then we use that one, otherwise we will look through the
    // inventory to find all that match.
    if (arg)
    {
        // Find items that match the arg. 
        oblist = parse_this(arg, "[about] [the] %i");
        
        // Filter out all items that matched the arg that aren't a component
        // container.
        oblist = filter(oblist, filter_component_holder);
        
        if (!sizeof(oblist))
        {
            notify_fail("You must specify a valid component pouch.\n");
            return 0;
        }
    }
    else
    {
        // When no argument just check inventory for component container.
...
/*
 * Emote that bestows a blessing <how> at <who>.
 *
 * @param arg String that contains how (optionally) to bestow the blessing
 *            and at who.
 *
 * @return 0 if no valid target is found, 1 otherwise.
 */
int
pbestow(string arg)
{
...
/*
 * Emote to express your defiance of the target, challenging him.
 *
 * @param arg The person targeted by the challenge.
 */
int
...
/*
 * Emote to claim something or someone's soul in the name of Takhisis.
 * The player claims someone's soul if the target is living.
 *
 * @param arg The person to claim the soul from.
 *
 * @return 0 if the target is not found, 1 otherwise.
 */
int
pclaim(string arg)
{
...
/*
 * Emote to clean your hands using clothing from a corpse.
 *
 * @param arg The corpse that you want to use for cleaning your hands.
 * 
 * @return 0 if no corpse is found, 1 otherwise.
 */
int
pclean(string arg)
{
...
/*
 * Emote to <how> consider what wickedness to engage in next.
 *
 * @param arg The adverb to add how the player wants to consider
 *            For example: angrily, carefully, etc.
 * 
 * @return 0 if a non-valid adverb is specified, 1 otherwise.
 */
int
...
/*
 * Emote to show that you are thinking about the matter at hand.
 *
 * @param arg adverb to describe how you are thinking about the matter.
 *            For example: carefully, quickly, etc.
 *
 * @return 0 if an invalid adverb is specified, 1 otherwise.
 */
int
...
/*
 * Emote to cause fear in non-dragonarmy and non-priests in the room by crying
 * out to Takhisis.
 * 
 * @param arg May not be specified.
 *
 * @return 0 if an argument is given, 1 otherwise.
 */
int
pcry(string arg)
{
...
/*
 * Emote to smile <how> (default is gleefully) at <who> (optionally) knowing
 * the players devotion to the Dark Queen.
 *
 * @param arg String containing optional adverb and optional target.
 * 
 * @return 0 if invalid adverb or invalid target is specified, 1 otherwise.
 */
int
pdevout(string arg)
{
...
/*
 * Emote that dismisses someone with a slight wave of the hand.
 *
 * @param arg The person to dismiss.
 *
 * @return 0 if the target person is not found, 1 otherwise.
 */ 
int
pdismiss(string arg)
{
...
/*
 * Emote to display to the target that you are a POT and not to be mettled
 * with.
 *
 * @param arg optional target for who to displaying the medallion to.
 *
 * @return 0 if the priest has no medallion, 0 if a non-valid target is
...
ptitle(string str)
{
    ...
    for (x = 0, titles = TITLES_COUNCIL ; x < 5 ; x++)
    {
            ...
            this_player()->query_real_name())
        {
            ...
            {
                arr += titles[index][(this_player()->query_gender() ==
                       G_FEMALE ? 1 : 0)];
            }
            else
            {
