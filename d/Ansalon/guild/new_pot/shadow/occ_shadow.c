 ...
 *
 * Converted to new magic system April 2016 by Arman
 */

/* Navarre : Added check to ensure only one NightLady/NightLord can exist */
/* Carnak  : Fixed the check that lowered players below Nightlord/lady    */
/* Arman   : Added guru quest via Krynn warfare club functionality */
...
query_priest_title()
{
    ...
    leader  = GUILD_ADMIN->query_guild_leader();

    /* Make sure that you can only be the NightLady or Nightlord if you are the
     * leader of the guild.
     *
     * Modified by Carnak: Lowered the title of the player to Dragonlord/lady
     *                     instead of Nightmaster/mistress. Should now lower
     *                     the title correctly.
     */
    if (leader != query_shadow_who()->query_real_name() &&
        rank == 2 && subrank > 3)
    {
        subrank = 3;
    }
...
adjust_standing(int change)
{
    ...
    if (file_size("/d/Ansalon/log/new_pot/standing") >= 50000)
        rename("/d/Ansalon/log/new_pot/standing", "/d/Ansalon/log/new_pot/standing.old");
    write_file("/d/Ansalon/log/new_pot/standing",
...
s_init_priest()
{
    ...
    query_shadow_who()->add_cmdsoul(SPELL_MANAGER_SOUL);
...
{
    // Already got guru quest
    if(shadow_who->test_bit("ansalon", 3, 19))
        return;

    string army = CLUB_MANAGER->query_pot_guru_army_focus(shadow_who);
    mapping list = CLUB_MANAGER->query_pot_guru_remaining_armies(shadow_who);
    string chosen_area = list[army];

    if ((newarmy == army) && (area == chosen_area))
    {
        CLUB_MANAGER->remove_pot_guru_remaining_armies(shadow_who, army);
	shadow_who->catch_tell("You feel the presence of the Dragon " +
            "Queen look upon you favourably.\n");  
        shadow_who->set_skill(PS_STANDING, 1200);      

        list = CLUB_MANAGER->query_pot_guru_remaining_armies(shadow_who);

        if(!m_sizeof(list))
        {
	    setuid();
	    seteuid(getuid());
	    shadow_who->set_bit(3, 19);
	    shadow_who->catch_tell("You feel an unholy sense of achievement! " +
                "Might makes right is the ethos of Takhisis, but only success " +
                "is rewarded! You may now seek out spellcraft guru training " +
                "in the Temple of Neraka.\n");
	    log_file("pot_guru", extract(ctime(time()),4,15) +
        ...
        else
        {
            ...
            string chosen_army = ((armies)[random(sizeof(armies))]);
            CLUB_MANAGER->set_pot_guru_army_focus(shadow_who, chosen_army);
        }
    }
}
