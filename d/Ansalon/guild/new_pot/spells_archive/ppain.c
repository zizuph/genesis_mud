...
{
    return ({"green slime","heart",ingredient_knife});
}
...
hook_use_ingredients(object *ingrs)
{
    ...
    if (ingrs[k]->query_leftover_powder())
       write("You stab " + LANG_THESHORT(ingrs[i]) + " into " +
          LANG_THESHORT(ingrs[j]) + ", its ichor dripping on to " +
          LANG_THESHORT(ingrs[k]) + " and combusting, consuming the " +
          "powder and blade.\n"); 
    else
