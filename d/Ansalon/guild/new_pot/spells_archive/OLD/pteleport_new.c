...
//        set_spell_ingredients(0);
        ...
        tell_room(environment(caster), QCTNAME(caster) + "'s eyes turns white as " 
            + HE(caster) +" looks inward for a while.\n", ({ caster }), caster);
        set_this_player(tp);
    }
...
//        set_spell_ingredients(({"elidross", "amanita"}));
        ...
        tp = this_player();
        set_this_player(caster);
        tell_room(environment(caster), QCTNAME(caster) + " traces an X-like " +
            "figure on the " + 
            (targets[0]->query_prop(ROOM_I_INSIDE) ? "floor" : "ground") +
