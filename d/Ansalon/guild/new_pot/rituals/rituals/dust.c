/*
 * Ritual to Convert Black Pearls into dust
 *
...
{
    GUILD_ADMIN->log(query_chantee(), "rituals", "pearl dust;");
    
    // We convert all the black pearls on the altar
    object * all_pearls = filter(all_inventory(query_altar()),
        &operator(==)("pearl") @ &->query_gem_type());
    all_pearls = filter(all_pearls, not @ &->query_gem_dust());
    foreach (object pearl : all_pearls)
    {
        object ritual_product = clone_object(GUILDDIR + "rituals/obj/gem_dust");
        ritual_product->dust_init(pearl->singular_short());
        find_player("novo")->catch_msg(pearl->singular_short() + "\n");
        // Each pearl will result in between 2-4 pieces of pearl dust.
        int new_quantity = 2 * pearl->num_heap() + random(3 * pearl->num_heap());
        ritual_product->set_heap_size(new_quantity);
        ritual_product->move(query_altar());
    }
    all_pearls->remove_object();
}
...
perform_ritual(int round = 0)
{
    ...
    switch (round)
    {
        case 0:
            ...
            break;
        case 1:
            chantee("You continue: Bless these gems that they might be a " +
                "vessel of your power.");
            others(0, " continues: Bless these gems that they might be a " +
                "vessel of your power.");
            others("In unison with ", " you chant: The power of Takhisis.");
            onlookers(QCTNAME(query_chantee()) + " chants: Bless these " +
                "gems that they might be a vessel of your power", " while " +
                COMPOSITE_LIVE(query_others()) + " chants in unison.");
            break;
        case 2:
