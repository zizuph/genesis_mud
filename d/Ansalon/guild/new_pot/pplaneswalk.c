...
int get_next_teleport_delay(object caster, object destination)
{
    if (!destination)
    {
        return ::get_next_teleport_delay(destination);;
    }
    object true_destination = destination->query_teleport_destination(caster);
    
    if (true_destination 
      && MASTER_OB(true_destination) == NTEMPLE + "lvl2/dorm")
    {
