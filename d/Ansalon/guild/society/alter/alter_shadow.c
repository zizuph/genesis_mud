/* Shadow for polearms altered to hoopaks.
 *
 * Added maximum OBJ_I_AIDS_ACROBATIC benefit when wielded by kender.
...
kender_wield()
{
    if (shadow_who->query_wielded())
    {
        ...
        shadow_who->add_prop(OBJ_I_AIDS_ACROBATIC, 100);
    }
}
...
{    
    ...
    return shadow_who->wield(what);
}

public int
unwield(object ob)
{
    shadow_who->remove_prop(OBJ_I_AIDS_ACROBATIC);
    return shadow_who->unwield(ob);
}
