...
query_dam_mod_valid(object player, object attacker, int attack_id, int dt)
{
    ...
    send_debug_message("kender_taunt_obj", "mod_valid: rage: " 
    + player->query_kender_rage() + "\n");
    
    team = attacker->query_team_others() + ({ attacker });
    team = filter(team, &operator(==)(, environment(player)) @ &environment());
    
    send_debug_message("kender_taunt_obj", "mod_valid: team: " 
    + sizeof(team) + "\n");
    ...
    send_debug_message("kender_taunt_obj", "mod_valid: kenders: " 
    + sizeof(kenders) + "\n");
    
    if (sizeof(kenders) && random((sizeof(team) * 10) / sizeof(kenders)) < 10)
	    return 1;
    
    send_debug_message("kender_taunt_obj", "mod_valid: failed\n");
