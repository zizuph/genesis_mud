                ...
                int dt, int phit, int dam, mixed *items, int result)
{
    ...
    /* Removing the pebble */
    if (items[0]->query_prop(HEAP_I_IS))
        items[0]->reduce_heap_size(1);
    else
    ...
    switch(phurt)
    {
        ...
        if (phurt < 81)
        {
            find_player("carnak")->catch_msg("didnt work.\n");
        }
...
check_required_items(object actor, mixed *targets, string arg, int pretest)
{
    ...
    if (!actor->find_hoopak())
    {
        write("You must wield a hoopak to be able to sling.\n");
        return 0;
    }
    
    if (!objectp(pebble = present("pebble", actor)))
    {
