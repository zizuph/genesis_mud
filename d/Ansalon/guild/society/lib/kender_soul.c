 ...
 *
 * Arman July 4th 2020
 * - Added Maddest Acolyte of lost Causes
 * 
...
#include "/d/Ansalon/guild/society/wimpy_check.c"
#include "/d/Ansalon/guild/society/emotions.c"

#define DELAY       9.0
#define FAERIE_ROOM (KGROOM + "faerie_tree")
#define TEMP_ROOM   (KGROOM + "faerie_temp")

#define CAUGHT_IN_THE_COOKIE_JAR "_caught_in_the_cookie_jar"
...
int
closed_container(string str)
{
    string what, from, whom, failure = "";
    object *target, *item;
    mapping caught = ([]);
    
    if (mappingp(this_player()->query_prop(CAUGHT_IN_THE_COOKIE_JAR)))
        caught = this_player()->query_prop(CAUGHT_IN_THE_COOKIE_JAR);
    
    if (!stringp(str) || !strlen(str))
        return 0;
    
    if (sscanf(str, "%s from %s of %s", what, from, whom) != 3)
        return 0;

    if (!CAN_SEE_IN_ROOM(this_player()))
        return 0;
    
    if (environment(this_player())->query_prop(ROOM_M_NO_STEAL))
        return 0;
    
    if (objectp(this_player()->query_attack()))
        return 0;
    
    if (!sizeof(target = parse_this(whom, "%l")))
        return 0;
    
    item = FIND_STR_IN_OBJECT(from, target[0]);
    ...
    if (!IS_CONTAINER_OBJECT(item[0]))
        return 0;
    
    if (!item[0]->query_prop(CONT_I_CLOSED))
        return 0;
    ...
    if (mappingp(caught))
    {
        if (time() - caught[target[0]->query_real_name()] < 600)
        {
            this_player()->catch_msg(QCTNAME(target[0]) + " is too " +
            "vigilant of " + target[0]->query_possessive() + " surroundings " +
            "for you to rummage through the " + item[0]->query_short() + ".\n");
            return 1;
        }
    }
    
    this_player()->catch_msg("In investigating " + QTNAME(target[0]) +
        "'s " + item[0]->query_short() + " you note that it is closed! " +
        "This was clearly designed as a challenge for you to open!\n");
    
    if (item[0]->query_prop(CONT_I_LOCK))
    {
        
        if (item[0]->query_no_pick())
        {
            write("Close examination of the lock on the " +
            item[0]->query_short() + " teaches you that it is unpickable.\n");
            return 1;
        }
        
        if (item[0]->query_pick() >= this_player()->query_skill(SS_OPEN_LOCK))
        {
            write("Close examination of the lock on the " +
            item[0]->query_short() + " teaches you that it is too difficult " +
            "to open with your current ability.\n");
            return 1;
        }
    
        if (random(105) > this_player()->query_skill(SS_PICK_POCKET))
        {
            caught[target[0]->query_real_name()] = time();
            this_player()->add_prop(CAUGHT_IN_THE_COOKIE_JAR, caught);
            
            failure = " Your attempt has been noticed.";
        }
        
        if (this_player()->resolve_task(TASK_HARD, 
            ({ SS_PICK_POCKET, SS_SNEAK, TS_DEX }), target[0], 
            ({ SS_AWARENESS, TS_WIS, TS_INT })) < 0)
        {
            if (strlen(failure))
            {
                target[0]->catch_msg(QCTNAME(this_player()) + " attempted " +
                "pick the lock on your " + item[0]->query_short() + ", but " +
                "failed.\n");
                
                tell_room(environment(this_player()), QCTNAME(target[0]) +
                " catches " + QTNAME(this_player()) + " in an attempt to " +
                "rummage through " + target[0]->query_possessive() +
                "belongings.\n", ({this_player(), target[0]}), target[0]);
            }
            
            this_player()->catch_msg("You failed to pick the lock on the " + 
            item[0]->query_short() + "." + failure + "\n");
        ...
        if (strlen(failure))
        {
            target[0]->catch_msg("You caught " + QTNAME(this_player()) +
            " picking the lock on your " + item[0]->query_short() + ".\n");
        ...
        if ((this_player()->query_skill(SS_OPEN_LOCK) * 3 / 4) +
            random(this_player()->query_skill(SS_OPEN_LOCK) / 3) >
            item[0]->query_pick())
        {
            this_player()->catch_msg("You fail to pick the lock on the " + 
            item[0]->query_short() + "." + failure + "\n");
            return 1;
        }
        
        item[1]->remove_prop(CONT_I_LOCK);
        
        this_player()->catch_msg("You successfully pick the lock on the " + 
        item[0]->query_short() + "." + failure + "\n");
    }
    ...
    if (this_player()->resolve_task(TASK_DIFFICULT, 
        ({ SS_PICK_POCKET, TS_WIS, TS_DEX }), target[0], 
        ({ SS_AWARENESS, TS_WIS, TS_INT })) < 0)
    {
        if (strlen(failure))
        {
            target[0]->catch_msg(QCTNAME(this_player()) + " attempted " +
            "to open your " + item[0]->query_short() + ", but failed.\n");
        ...
        this_player()->catch_msg("You failed to open the " + 
        item[0]->query_short() + "." + failure + "\n");
        return 1;
    }
    ...
    {
        target[0]->catch_msg("You caught " + QTNAME(this_player()) + 
        " opening your " + item[0]->query_short() + ".\n");
    ...
    this_player()->catch_msg("With some deft handwork the " +
    item[0]->query_short() + " is now open for inspection." + failure +"\n");
    
    item[0]->remove_prop(CONT_I_CLOSED);
...
string *new_titles = ({"Blacksmith of the Disappearing Hammer", 
                     ...
                     "Maddest Acolyte of Lost Causes",
                     });
