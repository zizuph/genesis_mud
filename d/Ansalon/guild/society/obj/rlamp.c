/*
 * /d/Ansalon/guild/society/obj/rlamp.c
 * 
...
create_torch()
{
    ...
    set_short("red glass oil-lamp");
    set_pshort("red glass oil-lamps");
...
long_description()
{
    string longstr;
    if (!strlen(lamp_desc))
        longstr = "It is a red glass oil-lamp made in the shape of an oblong "+
                  "bowl with a lid. It has a wick on one end and a curved "+
                  "handle on the other. The red glass of the oil-lamp " + 
                  "reflects the light, giving off red shadows when lit. ";
    else longstr = lamp_desc;
 
    if (!query_time(1))
    {
        longstr += "The red glass lamp does not contain any oil. ";
    }
    else
    {
        longstr += "The red glass lamp contains some oil. ";
    }
...
wick_description()
{
    return ("The wick of the red glass oil-lamp sticks out from a hole in "+
...
do_refill(string str)
{
    ...
    if(!parse_command(str,TP," [the] [red] [glass] [oil] 'lamp' / 'oil-lamp' "+
    ...
    {
        NFN0("The red glass lamp is already filled with oil!");
    }
    ...
    if ((query_time(1) + oil_flask->query_amount()) > query_max_time())
    {
        ...
        WRITE("You fill the red glass lamp with as much lamp oil as " + 
            "it can hold.");
        SAY(" fills the red glass lamp with as much lamp oil as it can hold.");
    ...
    WRITE("You fill the red glass lamp with all the lamp oil in the flask. " +
        "The oil-lamp is not completely filled with oil.");
    SAY(" fills extra lamp oil into the red glass lamp.");
...
notify_low_on_oil()
{
    ...
    if (living(env))
    {
        env->catch_msg("Your red glass lamp flickers briefly, giving " + 
            "off many red shadows. It seems " +
            "to be running out of oil.\n");
        LTELL_ROOM(ENV(env), env, "The red glass lamp that ", " holds, " +
            "flickers briefly, giving off many red shadows. It seems " + 
