/*
 * /d/Ansalon/guild/society/obj/mclamp.c
 *
...
create_torch()
{
    ...
    add_adj("multi-coloured");
    add_adj("glass");
    add_adj("oil");
    set_pname("lamps");
    add_pname("oil-lamps");
 
    set_short("multi-coloured glass oil-lamp");
    set_pshort("multi-coloured glass oil-lamps");
...
long_description()
{
    string longstr;
    if (!strlen(lamp_desc))
        longstr = "It is a multi-coloured glass oil-lamp made in the shape "+
                  "of an oblong bowl with a lid. It has a wick on one end "+
                  "and a curved handle on the other. The multi-coloured " + 
                  "glass of the oil-lamp reflects the light, giving off " + 
                  "multi-coloured shadows when lit. ";
    else longstr = lamp_desc;
 
    if (!query_time(1))
    {
        longstr += "The multi-coloured glass lamp does not contain any oil. ";
    }
    else
    {
        longstr += "The multi-coloured glass lamp contains some oil. ";
    }
...
wick_description()
{
    return ("The wick of the multi-coloured glass oil-lamp sticks out "+
           "from a hole in one end. " + (query_prop(OBJ_I_LIGHT) ?
...
do_refill(string str)
{
    ...
    if(!parse_command(str,TP,
        " [the] [multi-coloured] [glass] [oil] 'lamp' / 'oil-lamp' "+
    ...
    {
        NFN0("The multi-coloured glass lamp is already filled with oil!");
    }
    ...
    if ((query_time(1) + oil_flask->query_amount()) > query_max_time())
    {
        ...
        WRITE("You fill the multi-coloured glass lamp with as much " + 
            "lamp oil as it can hold.");
        SAY(" fills the multi-coloured glass lamp with as much lamp " + 
            "oil as it can hold.");
    ...
    WRITE("You fill the multi-coloured glass lamp with all the lamp " +
        "oil in the flask. The oil-lamp is not completely filled with oil.");
    SAY(" fills extra lamp oil into the multi-coloured glass lamp.");
...
notify_low_on_oil()
{
    ...
    if (living(env))
    {
        env->catch_msg("Your multi-coloured glass lamp flickers briefly, " + 
            "giving off many different coloured shadows. " +
            "It seems to be running out of oil.\n");
        LTELL_ROOM(ENV(env), env, "The multi-coloured glass lamp " + 
            "that ", " holds, " +
            "flickers briefly, giving off many different coloured shadows. " + 
            "It seems to be running out of oil.", env);
    }
    else if (env->query_prop(ROOM_I_IS))
    {
        tell_room(env, "The " + short() + " flickers briefly, giving off " + 
            "many different coloured shadows. It seems " +
