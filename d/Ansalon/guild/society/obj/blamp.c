/*
 * /d/Ansalon/guild/society/obj/blamp.c
 *
...
create_torch()
{
    ...
    set_short("blue glass oil-lamp");
    set_pshort("blue glass oil-lamps");
...
long_description()
{
    string longstr;
    if (!strlen(lamp_desc))
        longstr = "It is a blue glass oil-lamp made in the shape of an oblong "+
                  "bowl with a lid. It has a wick on one end and a curved "+
                  "handle on the other. The blue glass of the oil-lamp " + 
                  "reflects the light, giving off blue shadows when lit. ";
    else longstr = lamp_desc;
 
    if (!query_time(1))
    {
        longstr += "The blue glass lamp does not contain any oil. ";
    }
    else
    {
        longstr += "The blue glass lamp contains some oil. ";
    }
...
wick_description()
{
    return ("The wick of the blue glass oil-lamp sticks out from a hole in "+
...
do_refill(string str)
{
    ...
    if(!parse_command(str,TP," [the] [blue] [glass] [oil] 'lamp' / 'oil-lamp' "+
    ...
    {
        NFN0("The blue glass lamp is already filled with oil!");
    }
    ...
    if ((query_time(1) + oil_flask->query_amount()) > query_max_time())
    {
        ...
        WRITE("You fill the blue glass lamp with as much lamp oil as " + 
            "it can hold.");
        SAY(" fills the blue glass lamp with as much lamp oil as it can hold.");
    ...
    WRITE("You fill the blue glass lamp with all the lamp oil in the flask. " +
        "The oil-lamp is not completely filled with oil.");
    SAY(" fills extra lamp oil into the blue glass lamp.");
...
notify_low_on_oil()
{
    ...
    if (living(env))
    {
        env->catch_msg("Your blue glass lamp flickers briefly, giving " + 
            "off many blue shadows. It seems " +
            "to be running out of oil.\n");
        LTELL_ROOM(ENV(env), env, "The blue glass lamp that ", " holds, " +
            "flickers briefly, giving off many blue shadows. It seems " + 
            "to be running out of oil.", env);
    }
    else if (env->query_prop(ROOM_I_IS))
    {
        tell_room(env, "The " + short() + " flickers briefly, giving " + 
            "off many blue shadows. It seems " +
            "to be running out of oil.\n");
    }
}
