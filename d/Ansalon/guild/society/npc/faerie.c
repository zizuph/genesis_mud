...
faerie_owner(string owner)
{
	...
	gOwner = owner;
        kender = find_player(gOwner);
        if (!objectp(kender))
            race_title = "Faerie";
        else
            race_title = kender->query_guild_title_race();

        race_title = (strlen(race_title) ? race_title : "Faerie");

        set_title(race_title + ", Conjurer of Mystical Faerie Magic, " +
            "Friend of " + capitalize(gOwner));
}
...
normal_receive_message(object who)
{
	who->catch_tell(capitalize(LANG_ASHORT(this_object())) + 
		" flutters down from above and lands on your " +
                "shoulder, handing you a rolled up scroll.\n");
...
send_message(string str)
{
	  ...
	  lower_case(str) == "a reply")
	{
        ...
        write("You begin to jot down a message.\n");
	gWho = lower_case(str);
	...
	case 3:
                  ...
                  command3[0] != "present"))
		{
			...
			return 0;
                ...
                if ((command3[0] == "introduce" || command3[0] == "present") &&
                    (command3[1] != "myself" && command3[1] != "me"))
                {
			notify_fail("Your faerie can only " + command3[0] +
                            " 'me' or 'myself'.\n");
			return 0;
                }
	...
	case 2:
                  ...
                  command2[0] != "present"))
		{
			...
			return 0;
                ...
                if ((command2[0] == "introduce" || command2[0] == "present") &&
                    (command2[1] != "myself" && command2[1] != "me"))
                {
			notify_fail("Your faerie can only " + command2[0] +
                            " 'me' or 'myself'.\n");
			return 0;
                }
