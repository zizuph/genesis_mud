...
join_race(string str)
{
    ...
    {
        notify_fail("Kender can't be students of the school of High Magic. " +
           "Leave the school before trying to become a kender.\n");
        return 0;
    }
...
change_name(string name)
{
  if(!(TP->query_race_name() == "kender"))
  {
    notify_fail("You need to be a kender before you can rename your family kender name.\n");
    return 0;
  }

  if (!name)
  {
    notify_fail("Rename yourself to what name? Use 'rename <new name>'\n");
    return 0;
  }

  if (!"/d/Krynn/raceguild/masterroom"->check_name_ok(TP, "kender", name))
  {
    notify_fail("The name you provided aren't available.\n");
    return 0;
  }

  TP->remove_autoshadow("/d/Krynn/raceguild/obj/race_shadow");
  TP->add_autoshadow("/d/Krynn/raceguild/obj/race_shadow" + ":" + name + "#kender");
  TP->set_race_title(name, "kender");
  TP->catch_msg("You have changed your family name to '"+name+"'.\n");
  TP->catch_tell("You may have to relog for the changes to take effect!\n");
  return 1;
}
