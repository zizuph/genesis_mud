...
{    
  ...
  set_evade_combat_aid(SHIELD_MASTER_AID);
}                           
...
{ 
  //This effect is valid for 30 seconds after use.
  //After that, we remove it.
  if (!(player->query_prop(DEFENCE_MASTER_EXPIRE_TIME) > time()))
  {
    set_alarm(0.1, 0.0, &remove_neidar_defence_master_object());
    return 0;       
  }
  return 1;
}
...
attack_blocked(object player, object attacker, int attack_id)
{
  ...
  string* valid_watcher_desc = ({QCTNAME(attacker)+" strikes "+QTNAME(player)+
				 ...
				 QTNAME(player)+" looks at "+QTNAME(attacker)+" unwavering.\n",
  ...
  {
    string player_desc = "Sparkles fly as you block "+QTNAME(attacker)+"'s " +
                  attacker_weapon->short() + " with your "+player_shield->short()+"./n";

    valid_player_desc = valid_player_desc + ({player_desc});

    string attacker_desc = "Sparkles fly as "+QTNAME(player)+" blocks "+
      "your "+attacker_weapon->short()+" with "+HIS(player)+" "+player_shield->short()+".\n";

    valid_attacker_desc = valid_attacker_desc + ({attacker_desc});

    string watcher_desc = "Sparkles fly as "+QTNAME(player)+" blocks "+
  ...
  if(attacker_weapon)
  {
    string player_desc1 = QCTNAME(attacker)+"'s "+attacker_weapon->short() + " launches " +
                   "straight at you, but only manages to get entancled in your beard.\n";
    ...
    string watcher_desc1 = QCTNAME(attacker)+"'s "+attacker_weapon->short()+" launches "+
                           "straight at "+QTNAME(player)+", but only manages to get entancled in "+HIS(player)+
                           "beard.\n";
  ...
  tell_room(environment(player),watcher_descr, exclude_list);
}
...
find_weapon(object player)
{
  ...
  if(num_of_weapons > 0)
  {
    return wielded_weapons[num_of_weapons];
  }
  return 0;
}
