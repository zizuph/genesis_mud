/*
 * Neidar Battlerage Fiercely Damage Reduction Object
 * 
 * When Neidars go into Battlerage Fiercely, they gain a little
 * amount of damage reduction, essentially translating some of 
 * their offensive aid into a defensive posture
 *
...
create_effect()
{    
    set_name("_neidar_battlerage_fierce_object_");
    add_name( ({"neidar_battlerage_fierce_object_" }) );
    set_short("hidden neidar battlerage fierce object");    
    set_long("The Battlerage Fiercely object for Neidar Clan members.  "
        + "You shouldn't see this.\n");

    setuid();
    seteuid(getuid());

    set_dam_mod_combat_aid(BATTLERAGE_FIERCE_DAM_REDUCE_AID);
}                           

/*
 * Function:    is_neidar_battlerage_fierce_object
...
/*
 * Function     : remove_neidar_battlerage_fierce_object
 * Description  : Destructor function.
 * Arguments    : none
 * Returns      : nothing
 */
public void
remove_neidar_battlerage_fierce_object()
{
    remove_object();
}
...
start()
{
    ...
    {
        player->catch_tell("You grit your teeth fiercely, fully prepared "
            + "for the hits to come.\n");
        tell_room(environment(player), QCTNAME(player) + " grits " 
            + player->query_possessive() + " teeth fiercely, fully "
            + "prepared for the hits to come.\n", ({ player }) );
    }
