...
dappraise(string str)
{
    ...
    write("You carefully appraise your worn and wielded equipment, " +
        "trying to estimate their condition.\n");
...
private int
query_dmorse_understand(object who)
{
    if(who->query_guild_name_occ() ==
    ...
    if(who->query_skill(SS_LANGUAGE) > 75 - random(20) && random(2))
    {
	return 1;
    }
    else
...
dmorse(string str)
{
    if(!str || str == "")
    {
	notify_fail("Use your pipe to morse what?\n");
	return 0;
    }

    object pipe = present(GUILD_EMBLEM_ID, TP);

    if(!pipe)
    {
	notify_fail("You seem to have misplaced your pipe!\n");
	return 0;
    }

    object* oblist = FILTER_PLAYERS(all_inventory(E(TP)));

    object* understand = filter(oblist, query_dmorse_understand);

    oblist -= understand; // We now get those who do not understand

    say(QCTNAME(TP)+" taps "+HIS(TP)+ " " +pipe->short()+ " morsing: "+str+"\n", oblist+({TP}));
    TP->catch_msg("You tap your "+pipe->short()+" morsing: "+str+"\n");

    understand -= ({TP});    

    for(int i = 0, s = sizeof(oblist); i < s; i++)
    {
      oblist[i]->catch_msg(QCTNAME(TP)+" taps "+HIS(TP)+" "+ pipe->short()+".\n");
    }
    return 1;
}
...
verse_two_2()
{
    ...
    song = "\n\n   Axes and clubs have been chopping\n" +
    "   making all those goblins dropping\n" +
    "   The power of the dwarves are whopping\n" +
    "   cause all the blows from goblins gets blockin'\n" +
    "   The armours of the goblins have been nogging\n" +
    "   My axes and clubs have them been mocking\n" +
    "   So DRINK! My friends, CHEERS!\n\n\n";

    write("You continue to sing:" +song+
      "\nYour blood stirred by the song, you feel ready for either " +
      "a tavern or a battle!\n");
    TP->tell_watcher(QCTNAME(TP)+ " continues to sing:" +song+
      "\n" +QCTNAME(TP)+ " falls silent, and looks ready for either a " +
      "tavern or a battle... it is hard to tell which!\n");
}
...
dsong2(string str)
{
    ...
    song = "\n   After a day of bashing and swingin'\n" +
    "   You spend your time with friends drinkin'\n" +
    "   That axe has been smiting the green goblin\n" +
    "   and that shieldbash have send them wobblin'\n" +
    "   Axing, hacking, drinking and cleaving\n" +
    "   made no goblin able to be leaving\n" +
    "   After been hammering in the cavern\n" +
    "   I wanna spend time drinkin' in the tavern\n" +
    "   Spend times with your friends drinking and cheering\n" +
    "   All the battles, stories and adventures you are sharing.\n\n\n";

    write("Your deep dwarven voice rises in song:\n" +song);
    TP->tell_watcher(QCTNAME(TP)+ "'s deep dwarven voice " +
      "rises in song:\n" +song);

    set_alarm(3.0, 0.0, "verse_two_2");
...
help(string str)
{
    ...
    if(file_size(HELP_DIR + "doomseeker/doomseekers") > 0)
    {
	setuid();
	seteuid(getuid());
        ...
        if(IS_DOOMSEEKER(TP))
        {
	    cat(HELP_DIR + "doomseeker/" +subject);
	    return 1;
        }
    }
...
deyes(string str)
{
    ...
    write("Like a long-cold hearth stuttering back into life, your " +
        "eyes recover their old fire at the thought of redemption.\n");
    TP->tell_watcher("Like a long-cold hearth stuttering back into " +
        "life, " +QTPNAME(TP)+ " eyes recover their old fire at the " +
...
dglory(string str)
{
    ...
    if(!strlen(str))
    {
	notify_fail("Who do you you think may be a glorious opponent?\n");
	return 0;
    }
    ...
    if ( !sizeof(wep) )
    {
        notify_fail("You don't feel it would be a glorious battle without " +
            "a weapon.\n");
        return 0;
    }
    ...
    actor("You grip your " + weapon->short() + " eagerly, determined your " +
      "possible death against", obj, " will be glorious!");
    target(" grips " +HIS(TP)+ " " + weapon->short() + " eagerly, determined " +
      HIS(TP)+ " possible death against you will be glorious!", obj);
    all2act(" grips " +HIS(TP)+ " " + weapon->short() + " eagerly, determined " +
      HIS(TP)+ " potential death against", obj, " will be glorious!");
    return 1;
}
...
dlook(string str)
{
    ...
    if( !strlen(str) )
    {
        write("You look around you with cold eyes, a soul longing for " +
            "redemption in death.\n");
	say(QCTNAME(TP) + " looks around with cold eyes, a soul longing " +
            "for redemption in death.\n");
	return 1;
    }
    ...
    actor("You look at",ob," with cold eyes, a soul longing for " +
        "redemption in death.");
    target(" looks at you with cold eyes, a soul longing for " +
        "redemption in death.", ob);
    all2act(" looks at",ob," with cold eyes, a soul longing for " +
        "redemption in death.");
...
doath(string str)
{
    ...
    write("You declare your death-oath: 'Without my honour I am nothing! " +
        "I seek redemption in the eyes of my clan and ancestors through " +
        "battle. I am death to my enemies until one can claim my life and " +
        "my shame!'\n");
    allbb(" declares " +HIS(TP)+ " death-oath: " +
...
dpull(string str)
{
    ...
    write("You pull at your " +beard+ " in frustration, fearful the black " +
        "dog of depression will get you before you cross a foe capable of " +
        "sending you to a glorious death.\n");
    allbb(" pulls at " +HIS(TP)+ " " +beard+ 
        " in frustration, fearful the black dog of depression will get " +
        HIM(TP)+ " before a capable foe sends " +HIM(TP)+ " to a glorious " +
        "death in battle.");
    return 1;
}
...
dsigh(string str)
{
    ...
    write("You sigh, longing for mighty battles with overwhelming odds.\n");
    allbb(" sighs, longing for mighty battles with overwhelming odds.");
    return 1;
}
...
dslump(string str)
{
    ...
    write("You slump, the shame of past dishonours a weight too heavy to " +
        "bear.\n");
    allbb(" visibly slumps, the shame of past " +
        "dishonours throwing " +HIM(TP)+ " into a black depression.");
    return 1;
}
