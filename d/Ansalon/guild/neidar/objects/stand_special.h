...
stand(string str)
{
	...
	stand_firm = clone_object(GUILD_OBJS + "stand_firm");
	stand_firm->set_ac(bonus_ac);
	stand_firm->set_duration(30);
	stand_firm->move(who, 1);
	...
	stand_strong = clone_object(GUILD_OBJS + "stand_strong");
	stand_strong->set_ac(bonus_ac);
	stand_strong->move(who, 1);
        who->add_fatigue(-(who->query_stat(SS_CON) / 8));
    }
    if(str == "aggressively")
    {
	clone_object(GUILD_OBJS + "stand_aggro")->move(who, 1);
