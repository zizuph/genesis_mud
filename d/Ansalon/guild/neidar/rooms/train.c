...
set_up_skills()
{
    ...
    sk_add_train(SS_AWARENESS, "aware of your surroundings", "awareness", 0, 60);
    sk_add_train(BATTLERAGING_SKILL,"take hits while in a dwarven battlerage",
      "battleraging", 80, 100);
}
...
reset_dwarf_room()
{
    if(!objectp(trainer))
    {
	trainer = clone_object(GUILD_NPCS +"trainer");
	trainer->arm_me();
	trainer->move(TO);
    }
}
