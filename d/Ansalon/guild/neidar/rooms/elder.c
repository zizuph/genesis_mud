/*
 * Louie May 2006 - Added in removal of leader via ADMIN when retiring.
 *
 * - Fixed runtime in members log book, Petros August 2009
 */
...
#define SPONSOR_THREE "_clan_sponsor_three"
#define SAVEFILE (SAVE_PATH+"elder")
...
create_dwarf_room()
{
    ...
    add_exit(GUILD_ROOMS + "board","north",0);
...
read_new_scroll()
{
    ...
    TP->more(read_file("/d/Ansalon/log/"+ELDER_LIST,1));
    return "";
}
...
read_ancient_scroll()
{
    ...
    TP->more(read_file("/d/Ansalon/log/" + OLD_ELDER_LIST,1));
    return "";
}
...
resign(string str)
{
	...
	if(TP->test_bit("Ansalon",3,3))
	{
		...
		save_object(RETIRED_FILE);
                ...
                log_file("/d/Ansalon/log/"+ELDER_LIST, extract(ctime(time()), 4, 15)+ " "+
                   C(str) + 
                   " was retired from the position of the Clan Elder!\n", -1);
	...
	TP->clear_bit(3,3);
	TP->set_bit(3,4);
    ADMIN->remove_guild_leader(TP);
	whose_thane = "";
	save_object(SAVEFILE);
	
        log_file("/d/Ansalon/log/"+ELDER_LIST, extract(ctime(time()), 4, 15)+ " "+
            C(TP->query_real_name())+
            " steps down as Thane of the Hill Dwarves!\n", -1);
    ...
    log_file("/d/Ansalon/log/"+ELDER_LIST, extract(ctime(time()), 4, 15)+ " "+
        C(TP->query_real_name())+
        " steps down from the position as Dwarven Elder!\n", -1);
...
vote(string str)
{
    ...
    if(obj->query_prop(SPONSOR_ONE) || obj->query_prop(SPONSOR_TWO))
    {
	...
	else
	{
		...
		obj->add_prop(SPONSOR_THREE, 1);
	}
	obj->add_prop("_" +who+ "_has_voted", 1);
	return 1;
    }

    if(obj->query_prop(SPONSOR_THREE))
    {
	...
	obj->clear_bit(3,4);
	obj->set_bit(3,3);
	obj->remove_prop(SPONSOR_THREE);
	whose_thane = obj->query_real_name();
	save_object(SAVEFILE);

        log_file("/d/Ansalon/log/"+ELDER_LIST, extract(ctime(time()), 4, 15)+ " "+
            C(obj->query_real_name())+
            " was elected as Thane of the Hill Dwarves!\n", -1);
...
read_book()
{
    ...
    names = explode(read_file("/d/Ansalon/log/" + MEMBERS_LOG), "\n");
    ...
    names_tmp = sort_array(names_tmp);
