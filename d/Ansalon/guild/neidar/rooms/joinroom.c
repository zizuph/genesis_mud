 ...
 * Petros July 31 2009 - Fixed typo
 */
...
create_dwarf_room()
{
    ...
    set_long("You stand within the audience chamber of the Hall " +
      ...
      "worthy candidate by the Thane or one of his Elders. Clan " +
    ...
    add_exit(GUILD_ROOMS + "hall1","northwest","@@go_west");
...
occjoin(string str)
{
    ...
    if(!this_player()->query_prop(I_CAN_JOIN_ANSALON_CLAN) &&
       !ADMIN_MANAGER->query_is_approved(this_player()))
    {
    ...
    sh = clone_object(GUILD_SHADOWS+"occ_shadow");

    /* See if this member is acceptable */
    if (stringp(why = sh->acceptable_neidar_member(this_player())))
    {
	write(why);
    }
    else
...
make_elder(string str)
{
	...
	if(strlen(str))
	{
            if(str == "list")
            {
                ...
                mapping list = ADMIN_MANAGER->query_neidar_elder_list();

                if(!m_sizeof(list))
                {
                   write("There are no active Elders.\n");
                   return 1;
                }
                ...
                foreach(mixed name, mixed num: list)
                {
                    psize++;
                    approve_list += capitalize(name);
                    if (m_sizeof(list) - psize == 1)
                        approve_list += ", and ";
                    else if (m_sizeof(list) - psize > 1)
                        approve_list += ", ";
                }

	        write("The list of Elders of the Neidar Clan are: " +
                    approve_list+ ".\n");
	        return 1;
        ...
        {
            notify_fail("Honour which dwarf? Or check the honour <list>?\n");
            return 0;
        }
...
sit_throne(string str)
{
	...
	TP->catch_msg("You sit yourself down on the throne of the " +
	  ...
	  "       'deathmark' an exile for expulsion from the Clan\n" +
	  "       'check' a members last entrance into the realms\n" +
	  "       'approve' someone to join the Neidar in your absence\n" +
	  "        check your 'approve list'\n" +
	  "       'revoke' your approval of someone to join the Clan in your absence\n" +
          "       'exile list' to see the list of exiles\n" +
          "       'exile info <name>' to see when one was exiled\n" +
	  "        display a 'friends list' of those with Runes of Friendship\n" +
...
forgive_dwarf(string str)
{
    ...
    if(find_player(str)->test_bit("Ansalon",3,3) && 
       find_player(str)->test_bit("Ansalon",3,4))
    {
        write("You forgive " + capitalize(str) + ".\n");
	TP->tell_watcher(QCTNAME(TP)+ " forgives " +QTNAME(find_player(str))+
	  "!\n", find_player(str));
	find_player(str)->catch_msg("You have been forgiven.\n");
        find_player(str)->clear_bit(3,4);
        find_player(str)->clear_bit(3,3);
        ADMIN_MANAGER->remove_exile(str);
        ADMIN_MANAGER->remove_death_mark(str);
        log_file(ELDER_LIST, extract(ctime(time()), 4, 15)+ " "+
           C(str) + " was forgiven by the Thane of the Hill " +
            "Dwarves!\n", -1);
    ...
    TP->tell_watcher(QCTNAME(TP)+ " forgives " +QTNAME(find_player(str))+
	  "!\n", find_player(str));
    find_player(str)->catch_msg("You have been forgiven by the Thane of the " +
          "Hill Dwarves!\n");
...
int
check_member(string str)
{
    if(IS_ELDER(this_player()) || IS_THANE(this_player()))
    {
	if(!str)
	{
	    notify_fail("Check who?\n");
	    return 0;
        }

        str = lower_case(str);

        if(!ADMIN_MANAGER->query_is_member(str))
	{
	    notify_fail(capitalize(str)+ " isn't a member of the Neidar " +
                "Clan or hasn't been in the realms for quite some time.\n");
	    return 0;
        }

        if(find_player(str))
	{
	    notify_fail(capitalize(str)+ " is currently in the realms.\n");
	    return 0;
        }

        int date = ADMIN_MANAGER->last_member_login(str);
        
	TP->catch_msg(capitalize(str)+ " was last in the realms " +
	  TIME2FORMAT(date, "d mmm yyyy") + ".\n");
	return 1;
    }

    write("You are not Thane or an Elder! You cannot " +
      "check the activity of members of the Clan!\n");
    return 1;
}

int
approve_membership(string str)
{
    if(IS_THANE(this_player()))
    {
	if(!str)
	{
	    notify_fail("Approve whose entry into the Neidar Clan?\n");
	    return 0;
        }
	...
	if(str == "list")
	{
            mapping list = ADMIN_MANAGER->query_approved_to_join_list();

            if(!m_sizeof(list))
            {
               write("There is no one approved to join the Neidar Clan in " +
                   "absence of the Thane or Elder.\n");
               return 1;
            }
	    ...
	    write("The list of those who have been approved to join the " +
                "Neidar Clan are: " +approve_list+ ".\n");
	    return 1;
        }

        if(ADMIN_MANAGER->query_is_member(str))
	{
	    notify_fail(capitalize(str)+ " is already a member of the Neidar " +
                "Clan?\n");
	    return 0;
        }
        ...
        if(find_player(str))
	{
	    notify_fail(capitalize(str)+ " is currently in the realms. They should " +
                "come to the throne room to be accepted.\n");
	    return 0;
        }

        string elder = TP->query_real_name();
        
	TP->catch_msg("You approve " +capitalize(str)+ " to join the Neidar " +
	    "Clan, and has your endorsement to declare their loyalty here " +
            "in your absence.\n");
        ADMIN_MANAGER->approved_to_join_neidar(str, elder);

	return 1;
    }

    write("Only the Thane can approve absentee entry into the Neidar Clan!\n");
    return 1;
}

int
unapprove_membership(string str)
{
    if(IS_ELDER(this_player()) || IS_THANE(this_player()))
    {
	if(!str)
	{
	    notify_fail("Unapprove whose entry into the Neidar Clan?\n");
	    return 0;
        }

        str = lower_case(str);

        if(!ADMIN_MANAGER->query_is_approved(str))
	{
	    notify_fail(capitalize(str)+ " hasn't been approved to join the " +
                "Neidar Clan.\n");
	    return 0;
        }
	...
	TP->catch_msg("You revoke your approval of " +capitalize(str)+ " to join the " +
	    "Neidar Clan.\n");
        ADMIN_MANAGER->unapproved_to_join_neidar(str, elder);

	return 1;
    }

    write("Only the Thane or an Elder can revoke approval entry into the Neidar Clan!\n");
    return 1;
}

int
friends_list(string str)
{
    if(IS_THANE(this_player()))
    {
	if(!str)
	{
	    notify_fail("Check the 'friends list' of those granted a Rune of " +
                "Friendship?\n");
	    return 0;
        }
	...
	if(str == "list")
	{
            mapping list = ADMIN_MANAGER->query_neidar_friends_list();

            if(!m_sizeof(list))
            {
               write("There is no one with a Rune of Friendship.\n");
               return 1;
            }
	    ...
	    write("The list of those who are friends of the Neidar Clan and have " +
                "a Rune of Friendship are: " +approve_list+ ".\n");
	    return 1;
        }

    }

    write("Only the Thane or Elders can check the friends list!\n");
    return 1;
}

int
death_mark(string str)
{
    if(IS_THANE(this_player()))
    {
	if(!str)
	{
	    notify_fail("Check the 'deathmark list' of those exiles marked for " +
                "expulsion through death? Or deathmark an exiled member?\n");
	    return 0;
        }
	...
	if(str == "list")
	{
            mapping list = ADMIN_MANAGER->query_neidar_death_mark_list();

            if(!m_sizeof(list))
            {
               write("There is no one death marked for expulsion.\n");
               return 1;
            }
	    ...
	    write("The list of those who are marked for expulsion through " +
                "death are: " +approve_list+ ".\n");
	    return 1;
        }

        if(!ADMIN_MANAGER->query_is_exile(str))
	{
	    notify_fail(capitalize(str)+ " isn't an Exile of the Neidar Clan " +
                "so can't be death marked.\n");
	    return 0;
        }

        if(ADMIN_MANAGER->query_death_mark(str))
	{
	    notify_fail(capitalize(str)+ " is already death marked!\n");
	    return 0;
        }

        write("You mark " +capitalize(str)+ " for expulsion from the Neidar " +
            "Clan through death!\n");  
        ADMIN_MANAGER->add_death_mark(str);      
        return 1;
    }

    write("Only the Thane can death mark someone for expulsion!\n");
    return 1;
}
...
init()
{
    ...
    add_action(approve_membership,"approve");
    add_action(unapprove_membership,"revoke");
    add_action(friends_list,"friends");
    ...
    add_action(death_mark,"deathmark");
    add_action(check_member,"check");
}
