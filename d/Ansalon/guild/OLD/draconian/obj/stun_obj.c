/*
Stun object - borrowed from Serions stun, by Elmore.
...
no_command()
{
    write("You are stunned, and unable to do that.\n");
    return 1;
}
...
remove_dragon_fear(object victim)
{
    ...
    vict->catch_msg("You no longer feel stunned, and can move again.\n");

    tell_room(E(vict), QCTNAME(vict) + " no longer looks stunned.\n", vict, TO);
...
do_dragon_fear()
{
    ...
    victim->catch_msg("You feel stunned!\n");
    victim->add_stun();
    ...
    tell_room(E(victim), QCTNAME(victim) + " looks stunned!\n", victim, TO);
