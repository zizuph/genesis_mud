...
#define BLIND_SUBLOC "dk_blind_subloc"
...
create_object()
{
    ...
    add_prop(OBJ_S_WIZINFO, "Death Knights magical object blinding the affected.\n");
    set_no_show();
}
...
dispel_spell_effect(object dispeller)
{
    ...
    spell_target->catch_tell("The shroud of darkness disappears from your eyes "+
        "and you can see again.\n");
    obs = all_inventory(environment(spell_target));
    obs -= FILTER_IS_SEEN(spell_target, obs);
    obs += ({ spell_target });
    tell_room(environment(spell_target), "The shroud of darkness disappears from "+
		QTNAME(spell_target)+"'s eyes.\n",
		obs, spell_target);

    spell_target->change_prop(LIVE_I_SEE_DARK,
        spell_target->query_prop(LIVE_I_SEE_DARK) + changeAmount);
...
start()
{
    ...
    alarm = set_alarm(30.0, 0.0, dispel_spell_effect);
...
show_subloc(string subloc, object me, object for_obj)
{
    ...
    if (for_obj == me)
        return "Your are covered with a shroud of darkness.\n";

    return C(HIS(me)) + " eyes are covered with a shroud of darkness.\n";
}
