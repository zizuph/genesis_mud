 ...
 * Date             : March 2001.
 * Description      : Contrains the shadow for the knight
 *                    guild.
 *
 */
...
{
    return MIN(MAX_GUILD_LEVEL, query_shadow_who()->query_stat(SS_LAYMAN) /9);
}
...
{
    if(query_shadow_who()->query_name() == GUILD_LEADER)
        return "Noble Guard of Palanthas";
