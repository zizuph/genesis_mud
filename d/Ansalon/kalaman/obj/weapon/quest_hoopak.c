...
{
    seteuid(getuid(TO));
    set_name(({"hoopak","_ansalon_quest_hoopak_"}));
    set_adj("kender");
    add_name("staff");
    set_short("kender hoopak");
    set_pshort("kender hoopaks");
    set_long("@@long_desc@@");
    set_default_weapon(20, 9, W_POLEARM, W_BLUDGEON | W_IMPALE, W_ANYH);
...
string long_desc()
{
	...
	"feathers are attacked to the top. Judging from the shape and adornment its " +
	ken));
