/*
 * Entrance to the Dungeon, through Tunnel
 *
 * Lunatari of Krynn
 *
 * 2008-01-30: Added search and beginning of entrance code
 * 2008-01-31: Finished the entering code.
 */
#include "local.h"
#include <ss_types.h>
...
create_dungeon_room()
{
   ...
   set_long(get_long_desc() + "A pale light comes in through a small opening " +
       "in the southern wall. There seems to be a little bit room " +
       "towards the north.\n");

   add_exit("tun2", "north", 0, 5, 1);

   add_item(({ "shadows", "shadow", "light" }),
       "The space between the bars of the grate lets some of the light " +
       "through from the other room, making this room dusky instead of dark.\n");
   add_item(({ "south wall", "southern wall" }),
       "In the southern wall a man made steel grating covers any possible " +
       "exit to the south.\n");
...
get_grate_desc()
{
  ...
  {
      case 0:
        text = "A rusty steel grate blocks any passage to the " + G_DIR +
          ", the steel bars are very rusty and old.";
      break;
      case 1:
        text = "A rusty steel grate blocks the passage going " + G_DIR +
          ", the rusty steel bars seem to have been slightly bent, " +
          "creating a narrow opening."; 
      case 2:
        text = "A rusty steel grate used to blocked the passage to the " +
           G_DIR + ", now the bars seems to have been bent and pulled " +
           "apart so that one can enter the grate.";
      break;
      default:
...
open_grating(string str)
{
  ...
  if(parse_command(str, ({ }), "[the] 'bars'"))
  {
    ...
    {
        write("You muster up all your strenght once again and you manage " +
              "bend the bars apart enough to enter the grate!\n");
        tell_room(TO, QCTNAME(TP)+ " get's all worked up and " +
              "manages to bend the bars of the grating a bit more.\n", TP, TP);
...
enter_grating(string str)
{
  ...
  if (parse_command(str, ({ }), "[the] 'grate' / 'grating'"))
  {
    ...
    notify_fail("The grating is closed.\n");
