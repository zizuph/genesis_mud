 ...
 */
mapping gAdjacent = ([
  "cc1" : ( [ "n" : "cell1", "s" : "cell4", "e" : "cc2", "w" : "dc1" ]),
  "cc2" : ( [ "n" : "cell2", "s" : "cell5", "e" : "cc3", "w" : "cc1" ]),
  "cc3" : ( [ "n" : "cell3", "s" : "cell6", "e" : 0, "w" : "cc2" ]),
]);
  ...
  switch (gType) 
  {
    ...
    case ROOM_CLROW1:
      ...
      add_item(({"water", "dripping", "drops"}), 
        "Water running down the walls and dripping from the ceiling form small pools " +
        "on the ground. The water looks completely undrinkable.\n");
      add_item(({"hinge", "hinges", "sturdy hinges"}),
        "These are the hinges that hold the massive door attached to the wall. They " +
        "are made out of decent quality steel and makes sure that no one can break " +
        "out from or into the cell.\n");
      add_item(({"window", "windows","barred window"}), "There is a window in each of " +
        "the two cell doors here.\n");
    break;
    case ROOM_CLROW2:
      add_item(({"cell", "cells"}),
...
void create_dungeon_room()
{
  ...
  if (gType == ROOM_TUNNEL1 || gType == ROOM_TUNNEL1 || gType == ROOM_WELL 
      || gType == ROOM_CLROW2 || gType == ROOM_CELL2)
  {
      /* ADD DARKNESS */
      DARK;
  }
