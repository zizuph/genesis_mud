...
reset_kalaman_room()
{
    if(!objectp(merchant))
    {
	merchant = clone_object(KNPC + "boat_seller");
	merchant->move(TO);
    }
}
...
create_kalaman_room()
{
    ...
    add_item(({"dry docks","dry dock"}),
      BSN("To your southeast you can make out the dry docks of Kalaman, the place where " +
          "ships great and small are made and repaired. At the moment it is inaccessable."));
