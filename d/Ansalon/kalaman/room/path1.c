...
create_kalaman_room()
{
    ...
    add_item("unnoticeable path",
      BSN("A thin path has been roughly fashioned on the eastern side of " +
	"the breakwater, allowing people to travel single file " +
	"down to the base of the city wall. You guess " +
	"it was made by smugglers."));
    add_item(({"clouds","fluffy white clouds"}),
      BSN("Clouds sail across the sky above you."));
    add_item(({"paved road","road"}),
      BSN("To your west is the start of a paved road that travels along the harbour as well as " +
	"to the city gates."));

    add_exit(KROOM + "path3","northeast",0);
    add_exit(KROOM + "road4","west",0);
    add_invis_exit(KROOM + "lower_path","down",0);
...
long_descr()
{
    ...
    tod_descr1() +
    "The path leads down to a paved road to your west, while it " +
    "continues along the breakwater to your northeast. " +
    "An almost unnoticeable path seems to have been fashioned " +
    "on the eastern side of the breakwater, leading down.";
