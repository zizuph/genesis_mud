...
create_kalaman_room()
{
	...
	"to the southeast. A path branches off it to the north that heads along the " +
	"breakwater of the harbour."));
    add_item("path",
      BSN("A path leads off to the north towards the breakwater of the harbour."));
    add_item("breakwater",
    ...
    add_exit(KROOM + "path2","north",0);
    add_exit(KROOM + "dock6","northeast",0);
    add_exit(KROOM + "road1","southeast",0);
...
long_descr()
{
    ...
    tod_descr1() +
    "A wooden pier leads off to the northeast, while a path heads northwards along " +
