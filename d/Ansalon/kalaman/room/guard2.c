...
create_kalaman_room()
{
    ...
    add_exit(KROOM + "gate","northwest",0);
    add_exit(WALL + "g3","up","@@no_go");
    reset_kalaman_room();
}
...
no_go()
{
    ...
    {
	write("The well-armoured guard stops you from going up.\n");
	say("A well-armoured guard stops " +QTNAME(TP)+ " from going " +
	  "up.\n");
	return 1;
    }
    write("You climb the stone stairway up to the battlements.\n");
    return 0;
}
