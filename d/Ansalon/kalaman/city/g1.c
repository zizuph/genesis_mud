...
#define H1 KRHERB + "saskatoon"
#define H2 KRHERB + "fdlhead"
#define H3 "/d/Gondor/common/herbs/oregano"
#define H4 "/d/Gondor/common/herbs/redweed"
...
create_kalaman_room()
{
    ...
    add_exit(CITY + "g3","southwest",0);
    add_exit(CITY + "g2","south",0);
    add_exit(CITY + "g4","southeast",0);

    add_cmd_item(({"fence","gate"}),"climb","@@climb_fence");

    door = clone_object(KOBJ + "garden_gate1a");
    door->move(TO);
    reset_kalaman_room();
}
...
climb_fence()
{
	...
	write("You try and climb the fence, but your " +
	  ...
	  "have to get used to the grass on this side of the fence.\n");
	say(QCTNAME(TP) + " pitifully tries to climb " +
	  "the gate, but fails miserably.\n");
	return "";
    }
    write("You climb over the fence to the street on the other " +
      "side.\n");
    say(QCTNAME(TP) + "climbs over the fence.\n");
    TP->move_living("M",CITY + "r7",1,0);
    return "";
}
