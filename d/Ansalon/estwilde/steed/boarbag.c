/*
 * Boarbag for Estwilde boar mounts, modified from 
...
#define BACKPACK_SUBLOC  "_boarbag_backpack"
...
read_tag()
{
    return "On the tag you read the following:\n\n"+
        "\t+---------------------------------------------+\n"+
        "\t| When strapped on a boar, the following      |\n"+
        "\t| commands are available:                     |\n"+
        "\t|                                             |\n"+
        "\t|      strap boarbag on boar                  |\n"+
        "\t|      unstrap boarbag on boar                |\n"+
        "\t|      fill boarbag on boar                   |\n"+
        "\t|      open boarbag on boar                   | \n"+
        "\t|      close boarbag on boar                  |\n"+
        "\t|      empty boarbag on boar                  |\n"+
        "\t|      boarloot (see boarbag contents)        |\n"+
        "\t|                                             |\n"+
        "\t| When boarbag is not on a boar, it acts      | \n"+
...
long_description()
{
    string long_desc = "A large leather bag designed to be " +
        "worn by a burly boar mount. A tag has been stitched " +
        "onto it that you could read.";

    if (query_keep())
    {
        long_desc += " For now the boarbag is protected from " +
        "being sold. <unkeep boarbag> will remove this protection.";
    }
    else
    {
        long_desc += " The boarbag is not protected from being " +
        "sold. To protect it, type <keep boarbag>. This " +
...
do_keep(string str)
{
    if (!parse_command(str, TP, " [the] [leather] 'boarbag' / " +
        "'boarbag' / 'pack' "))
    {
    ...
    {
        set_keep(0);
        write("You may sell the boarbag now.\n");
    }
    else
    {
        set_keep(1);
        write("You will now keep the boarbag when you are selling " +
