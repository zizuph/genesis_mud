...
create_dwarf_room()
{
    ...
    add_item(({"hole","small hole"}),
      ...
      "rised so far are perfectly lost in it.\n");
    add_item(({"sign","instructions"}),
      "@@read_recruit_fund_sign@@");
    add_cmd_item(({"sign","instructions"}),"read",
      "@@read_recruit_fund_sign@@");

    add_exit(IRON + "store","north",0);
    add_exit("/d/Ansalon/guild/dwarf/start","west",0);
    add_exit(IRON + "common","east",0);
    add_exit(IRON + "elder","south","@@elder_enter");

    // Removing this board as of June 2009 because it is now being
    // placed in the new location at /d/Ansalon/guild/neidar/rooms/board
...
long_descr()
{
    return "You stand in the board room of the Hall of Warriors, " +
        ...
        "There is also a small hole in the wall and " +
        "a sign with instructions hanging above it." +
        "\n";
}
