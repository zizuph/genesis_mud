...
#define MARKED "_marked_as_neidar_property"
#define SM_FORGE "/d/Shire/guild/smiths/rooms/neidar_forge"
...
create_dwarf_room()
{
    ...
    add_item(({"anvil"}),
      ...
      "Iron Delving as well as <mend> their armours, but since the loss of " +
      "their smith, it has been largely unused for some time.\n");
    add_item(({"path"}),
    ...
    add_item(({"split","crack","wide split","wide crack",
	"jagged split", "jagged crack"}), "A jagged and wide "+
	"crack splits open the hollowed out boulder on its "+
	"eastern face, allowing you to enter it from here.\n");

    add_item(({"boulder","craggy boulder","hollow boulder"}),
	"It's a hollowed out boulder, likely an upward "+
	"extrusion of a larger below-ground rock shelf.\n");

    add_item(({"flashes","orange flashes","sporadic flashes"}),
	"Occasional flashes of orange highlight the jagged "+
	"split in the boulder's entrance.\n");

    add_item(({"pile", "rocks","bundled pile","smaller rocks",
	"smoke","grey smoke","black flecks"}),
	"Sitting haphazardly on top of the craggy boulder are "+
	"a pile of small rocks. Either covering or obscuring "+
	"some sort of opening through which a grey smoke emits, "+
	"shedding black flecks up into the air.\n");

    add_prop(ROOM_S_EXIT_FROM_DESC, "from the outdoor forge.");

    add_exit(IRON + "r10","northeast",0);
    add_exit(SM_FORGE ,"boulder",0);
...
long_descr()
{
    ...
    "northeast. "+
    " A short distance west a tall craggy boulder lifts up "+
    "from the ground.  A wide, jagged split faces you on its east "+
    "side, sporadic orange flashes reveal glimpses of a hollowed "+
    "out interior.  The roundish top of the boulder spits black-"+
    "flecked grey smoke from a bundled pile of smaller rocks.\n";
}
...
forge_item(string str)
{
    ...
    string material, strap_short;
    int ac, hit, pen;

    if(!str)
    {
	notify_fail("Forge what? A shield, axe or warhammer?\n");
	return 0;
    }

    if(TP->query_race() != "dwarf")
    {
	notify_fail("You have a feeling the local dwarven " +
	  "population would not be happy if you made an attempt " +
	  "to use their forge. Best to leave it alone.\n");
	return 0;
    }
    ...
    if(!fired_up)
    {
	TP->catch_msg("The forge hasn't been fired up yet!\n");
	return 1;
    }

    if(!P("ore", TP))
    {
	TP->catch_msg("You have no ore to forge anything with!\n");
	return 1;
    }
...
light_fire(string str)
{
    ...
    if(!got_coal)
    {
	TP->catch_msg("There is no coal to light up the forge! " +
	  "How about you fill it with coal first?\n");
	return 1;
    }
...
process_marked(int num, object ob, string text)
{
    ...
    string obj_short, stringtmp, longname, longname2, longname3;
	...
	longname = ob->query_long();
	longname2 = longname[..1];
	longname3 = longname[2..];

	if (longname2 != "@@")
		ob->set_long(ob->query_long() + "It has been recently marked with "
		+ "the symbol of two twin peaks.\n");

	else
	{
		string stringtmp = call_other(ob, longname3);

		if (!stringtmp)
		{
			ob->set_long(ob->long() + "It has been recently marked with the "
		+ "symbol of two twin peaks.\n");
		}

		else
		ob->set_long(stringtmp + "It has been recently marked with the "
		+ "symbol of two twin peaks.\n");
	}
...
do_mark(string str) 
{
    ...
    if (parse_command(str, all_inventory(TP), "[the] %o", ob))  
	return process_marked(1, ob, text);

    if (parse_command(str, all_inventory(TP), "[the] %d %o", num, ob))
...
private int
process_mended(int num, object ob, string text)
{
    object tp = this_player();
    string obj_short, stringtmp;
    int arm_condition, repair_status;
    ...
    if(TP->query_race() != "dwarf")
    {
	TP->catch_msg("You have a feeling the local dwarven " +
	  "population would not be happy if you made an attempt " +
	  "to use their forge. Best to leave it alone.\n");
	return 1;
    }
    ...
    if (!ob->check_armour())
    {   
	TP->catch_msg("Only armours can be mended.\n");
	return 1;
    }

    arm_condition = ob->query_condition();
    repair_status = ob->query_repair();

    if(arm_condition < 1 || (repair_status >= arm_condition))
    {
        TP->catch_msg("This armour can't be mended further!\n");
        return 1;
    }

    if(!TP->query_guild_member("The Smiths Guild") && 
         ((arm_condition - repair_status) > 1))
    {
        TP->catch_msg("While you are capable of repairing minor damage " +
          "to armours, the repairs required for the " +ob->short()+ 
          " are beyond your ability to fix. "+
          "Only a dwarven smith would be able to mend this armour!\n");
        return 1;
    }
	...
	ob->set_repair(repair_status + 1);
	TP->catch_msg("You fire up the forge until the coals glow like " +
	  "the sun, and then skillfully work on mending the " +ob->short()+ ".\n");
	say(QCTNAME(TP)+ " fires up the forge until the coals glow like " +
	  "the sun, and then works on mending the " +ob->short()+ ".\n");
	return 1;
    }
}
...
do_mend(string str) 
{
    ...
    notify_fail("Mend what item?\n");

    if (parse_command(str, all_inventory(TP), "%o", ob))  
	return process_mended(1, ob, text);

    if (parse_command(str, all_inventory(TP), "%d %o",
	num, ob))  
	return process_mended(num, ob, text);
    return 0;
}
