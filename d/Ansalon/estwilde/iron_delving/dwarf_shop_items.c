...
inherit "/d/Ansalon/estwilde/iron_delving/shop_list";
...
shop_hook_list_object(object ob, int price)
{
    ...
    {
        mess = sprintf(" %-8s | %3d %2d %2d %2d", categ, price / 1728, (price % 1728) / 144, (price
% 144) / 12, price % 12);
        write("| " +str + "|" + mess + " |\n");
    }
    else
...
do_list(string args)
{
    ...
    {
        ::do_list(args);
        write(" --- Standard weapons, armours and equipment sold here can be listed \n" +
              " --- through the 'list equipment' command.\n");
        return 1;
    }
