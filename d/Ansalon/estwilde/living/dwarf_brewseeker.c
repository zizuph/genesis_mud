...
static int gQ1TestMode = 0;
static int gNoQuests = BREWCRAFT_NO_QUESTS;

public void do_answer(string what, object who);
void remove_quest_props(object who);
void calculate_rewards(object who);
void arm_me();
...
create_krynn_monster()
{
    ...
    set_long("This is dwarven warrior of Iron Delving -- " +
      ...
      "look like the type of dwarf to shy away from a drunken fight.\n");
    ...
    add_act("emote grumbles about the quality of the latest batch of "+
               "Iron Delving Export Brew.");
    add_act("emote looks wistfully at the brew mug in his hands "+
               "and shakes his head.");
    add_act("emote burps loudly.");
    add_act("emote mumbles drunkenly about something.");
    add_act("emote looks about with bleary eyes.");
    add_act("emote checks to see that he still has a "+
             "bulging purse at his belt; what with kender about.");
    add_act("emote pulls grumpily at his greying forked beard.");
    ...
    add_ask(({"exile","exiled"}), "@@ask_exile");
    add_ask(({"iron delving"}), "@@ask_irondelving");

    // quest1 test functionality
    add_ask(({"test"}), "@@ask_test");

    // allow / disallow quests
    add_ask(({"allow"}), "@@ask_allow");
    add_ask(({"disallow"}), "@@ask_disallow");
...
do_answer(string what, object who)
{
    ...
    if(!CAN_SEE(TO, who))
    {
        who->catch_msg("\n" + QCTNAME(TO) + " peers about drunkenly... "+
           "seeing nothing before returning to his drink.\n");
        return;
    }

    if (query_attack()) 
    {
        who->catch_msg("\n" + QCTNAME(TO) + " peers at you drunkenly.\n");
        who->catch_msg(QCTNAME(TO) + " says to you: Things are a bit "+
              "tense around here... no time for talk...\n");
    ...
    if ( !is_known(who) )
    {
        ...
        who->catch_msg(QCTNAME(TO) + " says to you: And who might you "+
          ...
          "but to have strangers asking me stuff as well?\n");
    ...
    {
    case "irondelving":
        who->catch_msg("\n" + QCTNAME(TO) + " eyes you incredulously.\n");
        who->catch_msg(QCTNAME(TO) + " mutters to you: Iron Delving? " +
            "Home to the Neidar Dwarves?... but not me... now... "+
            "You don't know it? Well you need to get about more. "+
            "... travel to the plains of Estwilde and set your path "+
            "to the south east... you'll find it sure enough.\n");
        say(QCTNAME(TO) + " mutters to " + QTNAME(who)
             + " in surprise.\n", who);
        return;

    case "exile":
        who->catch_msg("\n" + QCTNAME(TO) + " eyes you mournfully.\n");
        who->catch_msg(QCTNAME(TO) + " mutters to you: I done nothing " +
            "wrong... Me and the Neidar just gone our separate ways. "+
            "I do miss the good stuff from Iron Delving... I used "+
            "to lend a hand in the brewery... help out me elder-kin... " + 
            "she knew how to make it back then...\n");
        say(QCTNAME(TO) + " mutters to " + QTNAME(who)
             + " unhappily.\n", who);
        return;
    ...
    case "help":
        ...
        if ( gNoQuests ) 
        {
            who->catch_msg(QCTNAME(TO) + " barks at you: I'm not up " +
                "for talkin' bout help and such right now!\n");
            say(QCTNAME(TO) + " barks at " + QTNAME(who) + ".\n", who);
            return;
        }
        ...
        if ( who->test_bit(QUEST_DOMAIN,QUEST_BITGROUP,QUEST_BIT1) )
        {
            if( who->test_bit(QUEST_DOMAIN,QUEST_BITGROUP,QUEST_BIT2) )
            {
                ...
                who->catch_msg(QCTNAME(TO) + " says to you: It's good to see you " +
                    "again... a proper yeasty brewer of Iron Delving no less! "+
                    "While you're welcome to buy me a drink of this stuff, "+
                    "I'll give you a shiny plat reward for a crystal vessel of " + 
                    capitalize(BREWCRAFT_SHORTG) + " if you have it to give me. I "+
                    "have a hankering for the good stuff!\n");
                ...
                else
                {
                    ...
                    if ( is_champion_brewer(who) ) 
                    {
                        ...
                        who->catch_msg(QCTNAME(TO) + " says to you: You have improved the "+
                         ...
                         "I honour your yeast in all its glory!\n");
                        who->catch_msg(QCTNAME(TO) + " bows deeply again.\n");
        ...
        { 
            ...
            if (who->query_prop(BREWCRAFT_QUEST1))
            {
                ...
                if ( gQ1TestMode || is_export_brewer(who) )
                {
                    who->catch_msg(QCTNAME(TO) + 
    ...
    case "reward":
        ...
        if( !(who->test_bit(QUEST_DOMAIN,QUEST_BITGROUP,QUEST_BIT2)) )
        {
            if ( !gQ1TestMode && !(who->query_prop(BREWCRAFT_QUEST2)) 
        ...
        calculate_rewards(who);
        break;
    case "test":
        if ( (who->query_wiz_level()) >= 0 )
            gQ1TestMode = 1;
        break;
    case "allow":
        if ( (who->query_wiz_level()) >= 0 )
...
ask_disallow()
{
    set_alarm(1.0, 0.0, &do_answer("disallow", TP));
    return "";
}
...
ask_allow()
{
    set_alarm(1.0, 0.0, &do_answer("allow", TP));
    return "";
}
...
ask_test()
{
    set_alarm(1.0, 0.0, &do_answer("test", TP));
    return "";
}
...
ask_exile()
{
    set_alarm(1.0, 0.0, &do_answer("exile", TP));
    return "";
}
...
ask_irondelving()
{
    set_alarm(1.0, 0.0, &do_answer("irondelving", TP));
    return "";
}
...
calculate_rewards(object who)
{
    ...
    if ( who->test_bit(QUEST_DOMAIN,QUEST_BITGROUP,QUEST_BIT2) ) 
    {
        ...
        if (!vessel)
        {
            ...
            who->catch_msg(QCTNAME(TO) + " says to you: I'm hankering for "+
             "you to give me a crystal vessel! Do you have one? "+
        ...
        if( wildmatch(strpat,lower_case(vessel->short_descr())) )
        {
            who->catch_msg("\n" + QCTNAME(TO) + 
            ...
            if (environment(reward) == this_object())
            {
                command("drop coins");
                who->catch_msg(QCTNAME(TO) + " says to you: "+
                "You won't take me money? Well, I'll leave it on the bar.\n\n");
            }
        }
    ...
    if ( who->query_prop(BREWCRAFT_QUEST1) )
    {
        ...
        strpat = sprintf("*%s's*",who->query_real_name());
        if( gQ1TestMode || wildmatch(strpat,lower_case(mug->query_short())) )
        {
            who->catch_msg("\n" + QCTNAME(TO) + 
        ...
        {
            who->catch_msg("\n" + QCTNAME(TO) + 
                " tastes the contents of the mug... then sniffs you... "+
                "then pours out the mug. "+
                "He casts the empty mug aside. It smashes "+
                "somewhere in the corner... attracting a scowl from Tika.\n");
            say(QCTNAME(TO) + " mumbles to " + QTNAME(who) + 
             " quietly.\n", who);
            who->catch_msg(QCTNAME(TO) + " says to you: "+
             "That was just the regular offerin'... not yours at all! "+
