...
create_krynn_monster()
{
    ...
    add_ask(({"guru","guru quest","superior guru axe","guru axe"}),
      VBFC_ME("quest_hint"));
    add_ask(({"teregul stormbolt","teregul","stormbolt","taint", 
      "taint on clan","shame","shame to the clan"}),
      VBFC_ME("quest_hint2"));
...
quest_hint()
{
    ...
    {
	command("say Ah lad...");
	command("say You aren't ready to be trained to be a guru in yer " +
          "weapon of choice.");
	command("say Come back to me when yer an experienced ogreslayer.");
	return "";
    }
    if(TP->test_bit("ansalon", 3, 5))
    {
	command("wink sly");
	command("say Ye, train away!");
	return "";
    }

    command("sigh");
    command("say I swore never to train another to that level of weapon " +
       "mastery until Teregul Stormbolt is avenged, and the taint on the " +
       "clan removed.");
    command("emote growls angrily in memory of the shame to the clan.");
    return "";
}
...
quest_hint2()
{
    ...
    if(TP->query_stat(SS_OCCUP) < GURU_QUEST_STAT)
    {
	command("emote looks at you funnily, as if " +
          "you should already know the answer to that.");
	return "";
    }
    if(TP->test_bit("ansalon", 3, 5))
    ...
    command("say It would take a lot of dwarven spirits to get any clansman " +
       "to talk of that shameful event!");
    command("emote falls stubbornly silent.");
    return "";
}
