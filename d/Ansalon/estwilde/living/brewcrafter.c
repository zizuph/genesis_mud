...
get_culture(object brewer)
{
    ...
    for (ii=0;ii<ni;ii++)
    {
       if ( IS_ARMOUR_OBJECT(inv[ii]) )
       {
           ...
           // if worstloc is still unmodified...
           // force examination irrespective of 
           // armour condition

           if ( (icond > iworstcond) || (iworstloc == -1) )
           {
               iworstcond = icond;
               ...
               // query_slots() without an argument applies
               // only to wearable items that are worn
    ...
    if ( iBeerType < 0 ) 
        whisper = "you come here cleanly sweetheart, you have no "+
                  "brew culture I can work with...";
    else
        whisper = sprintf("hmmm, you may well have the yeasty "+
                 "makings of a fine %s here...",
                 BEERTYPES[iBeerType]);

    tell_room(environment(brewer), QCTNAME(TO)
         + " sniffs her tong-rag critically, before inserting it in "
         + "a copper tube at her belt.\n");
