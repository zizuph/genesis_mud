...
create_deck()
{
    ...
    add_item("view", "You are looking out at the turbulent waters "+
             "of the Blood Sea.\n");
    ...
    add_exit("d4","aft",0);
    reset_deck();
    set_no_disembark(); /* To prevent 'Disembark what?', Lunatari */
}
...
reset_deck()
{
    ...
    if(!objectp(p1))
    {
	p1 = clone_object(FNPC + "large_corsair");
	p1->arm_me();
	p1->move(this_object(), 1);
    }
    if(!objectp(p2))
    {
	p2 = clone_object(FNPC + "sailor1");
	p2->arm_me();
	p2->move(this_object(), 1);
    }

    if(!objectp(p3))
    {
	p3 = clone_object(FNPC + "sailor1");
	p3->arm_me();
	p3->move(this_object(), 1);
    }

    if(!objectp(p4))
    {
	p4 = clone_object(FNPC + "sailor1");
	p4->arm_me();
	p4->move(this_object(), 1);
    }
}
