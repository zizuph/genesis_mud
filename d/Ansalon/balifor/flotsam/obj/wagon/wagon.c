...
mixed *gPath = FLOTSAM_TO_NERAKA + NERAKA_TO_FLOTSAM;
string *gMessage = FLOTSAM_TO_NERAKA_MESSAGES +
                      NERAKA_TO_FLOTSAM_MESSAGES;
...
create_object()
{
	...
	set_long("The wagon is covered with " +
    ...
    add_item(({"remounts","string of remounts","string"}),
        "A line of horses tied together with ropes trail after the " +
        "wagon.\n");
	...
	{
	    write("Could not load file: " + WAGONBED);
...
do_enter(string arg)
{
	...
	if (!TP->query_prop(FLOTSAM_PLAYER_I_BRIBED_DRIVER))
		return 0;

    TP->remove_prop(FLOTSAM_LIVE_O_WAGON_STEED);
    if ((steed = TP->query_prop(LIVE_O_STEED)) && (!TP->query_wiz_level()))
    {
        ...
        tell_room(E(TP), QCTNAME(TP) + " ties " + HIS(TP) + " " +
            ...
            ({ TP }));
        TP->add_prop(FLOTSAM_LIVE_O_WAGON_STEED, steed);
        steed->move_living("M",AVOID);
        
    }
    write("You climb into the covered wagon.\n");
	if (!TP->move_living("entering the wagon",gWagonbed,1))
		TP->add_prop(FLOTSAM_PLAYER_I_BRIBED_DRIVER,
		    TP->query_prop(FLOTSAM_PLAYER_I_BRIBED_DRIVER) - 1);
    else if (steed = TP->query_prop(FLOTSAM_LIVE_O_WAGON_STEED))
    {
        ...
        log_file("flotsam_wagon", extract(ctime(time()), 4, 15) +
            " " + TP->query_real_name() + " boarded wagon.\n");
...
do_bribe(string arg)
{
    ...
    {
        NF("The driver looks at you and gets something wild in his eyes " +
            "as he refuses to have anything to do with you.\n");
        return 0;
    }
	...
	say(QCTNAME(TP) + " hands some coins to the driver.\n");
        ...
        log_file("flotsam_wagon", extract(ctime(time()), 4, 15) +
            " " + TP->query_real_name() + " bought ticket.\n");
