...
/* various typos corrected by Ashlar 23 May 97 */
/* Milan 27.1.2004 Fixed bugs in inproper use of alarms */
/* Cotillion 05.10.2004 Fixed dumping of paralyzes in void */
...
create_weapon()
{
    ...
    set_adj("kender");
    add_name("staff"); 
    set_short("kender hoopak");    set_pshort("kender hoopaks");
    set_long("@@long_desc@@");
    set_default_weapon(25,14,W_POLEARM, W_BLUDGEON | W_IMPALE, W_ANYH);
...
long_desc()
{
    ...
    return("This is a staff, that is for sure, but quite an unusual one. "+
        "It is made from a single piece of supple willow wood that, from the forked top to " +
...
make_noise(string str)
{
    ...
    NF("You don't have anything to swing with.\n");
    return 0;
...
aim(object tp, object target)
{
    if (!objectp(tp) || !objectp(target) || (E(tp) != E(target)))
        return;
    tp->catch_msg("You start to aim the hoopak at " + target->query_the_name(tp) +
      " and hope they stand still long enough for you to get them in your " +
      "sight.\n");
    target->catch_msg(tp->query_The_name(target) + " aims the hoopak in your direction.\n");
...
shoot(object tp, object target)
{
    if (!objectp(tp) || !objectp(target) || (E(tp) != E(target)))
        return;
    tp->catch_msg("You let the pebble fly towards " + target->query_the_name(tp) + ".\n" +
      "The pebble hits " + target->query_objective() + " harmlessly.\n");
    target->catch_msg(tp->query_The_name(target) + " lets the pebble fly from the hoopak.\n"+
...
shoot_player(string str)
{
    ...
    */
	...
	else
	{
        ...
        if(!(living(target)))
        {
            NF("Seems like a useless idea to shoot "+a+", doesn't it??\n");
            return 0;
        }
