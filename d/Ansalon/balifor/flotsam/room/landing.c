...
reset_flotsam_room()
{
	...
	if (!E(patrol[0]))
	{
    	// If there are errors in NPC loading, it's possible that the 
    	// leader doesn't have an environment. This will cause this code 
    	// to continue to throw runtimes. We bring the leader into this 
    	// room. Petros, June 2009
	    patrol[0]->move(TO, 1);
	}
...
create_flotsam_room()
{
    ...
    /*
	add_prop(ROOM_M_NO_ATTACK, "Considering the crowded nature of this area, " +
	  "and the likelihood of a dragon landing on you, this is not an appropriate " +
