...
reset_flotsam_room()
{
   if(!objectp(ac1))
      {
      ac1 = clone_object(TEMP + "living/acolyte");
      ac1->arm_me();
      ac1->move(TO);
   }
   ...
   if(!objectp(ac2))
      {
      ac2 = clone_object(TEMP + "living/acolyte");
      ac2->arm_me();
      ac2->move(TO);
   }
}
...
create_flotsam_room()
{
   ...
   add_exit(TEMP + "ent","northwest",0);
   add_exit(TEMP + "temp3","southwest",0);
   add_exit(TEMP + "temp5","south",0);
