...
create_flotsam_room()
{
    set_short("Sandy beach under a cliff");
    set_long("@@long_descr");
    ...
    add_item(({"sand","beach","sandy beach"}),"A thin " +
      ...
      "length.\n");
    add_item(({"shells","shell"}),"Sifting through the sand " +
      "you find the odd piece of broken shell, but apart " +
      "from that nothing else of interest.\n");
    add_item(({"wreckage","vessel"}),"Scattered along the " +
      "beach are the remains " +
    ...
    add_item("cliff","The cliff rises a fair distance up into the sky. " +
      ...
      "that heads up to the top of the cliff, starting to your west. \n");

    add_exit(FROOM + "street24","east","@@peace",0);
    add_exit(FROOM + "street22","west",0);
...
long_descr()
{
    return tod_descr1() + tod_descr2() + "You stand upon a sandy " +
    "beach at the bottom of the cliff. " + season_descr() +
    "\n";
}
...
peace()
{
    write("The wind dies down, and all seems peaceful as you " +
      "enter the cove. \n");
    return 0;
}
...
wave_sounds()
{
    tell_room(FROOM + "street23","Waves lap against " +
      "the sandy shore.\n");
}
