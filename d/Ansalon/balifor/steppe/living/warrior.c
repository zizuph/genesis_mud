/* Navarre : June 17th 2007: Added null check for enemy in special attack */
/* Navarre : Nov 2nd 2007: Removed set_gender(G_NEUTER) as it should 
 *                         but wasn't, it set the player to neuter forever.
 *                         I removed the neuter descriptions and setting.
 */
...
attacked_by(object ob)
{
	...
	command("say Take your tainted honour scum!");
	ob->catch_msg(QCTNAME(TO) + " slashes at you with his "+
	  "blade with incredible determination!\nYou are badly wounded.\n");
	ob->tell_watcher(QCTNAME(TO) + " slashes at " +QTNAME(ob)+ " with "+
	  "his blade!  Before " +QTNAME(ob)+ " could react, the swordmaster " +
	  "wounds "+HIM(ob)+ ".\n", ob);
        ob->set_hp(ob->query_hp() - 900);
    }
    else
    {
	if(random(2) == 1)
...
special_attack(object enemy)
{
    ...
    *attack,
    ...
    if (!sizeof(query_weapon(-1)) || !enemy)
    {
	return 0;
    }
    attacktype = random(5);
