...
create_flotsam_room()
{
    ...
    add_exit(FOREST_PATH + "/dark_forest/path4","southeast","@@str_check");
    add_exit(FOREST_PATH + "/dark_forest/clearing","northwest","@@str_check");
...
animal_trail()
{
	...
	"tracks. The trail heads southeast and northwest.\n";
    }
    ...
    "stealthfully. The trail heads southeast and northwest.\n";
}
...
str_check()
{
    ...
    say(QCTNAME(TP) + " tries to travel deeper into the " +
      "forest, but grasping branches and tree roots hold "+
      HIM(TP)+ " "+
      "back!\n");
    return 1;
}
