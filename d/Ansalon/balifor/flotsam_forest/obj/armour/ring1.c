// Changed the armour type from A_ANY_FINGER to A_L_FINGER
// doesn't make sense to suck the darkness away twice.
// Boron, June 24, 2003

// Changed the armour type to A_ANY_FINGER from A_L_FINGER
// doesn't make sense to try and prevent it to suck the darkness
// twice, since if two people with each a ring in the room
// can do it just fine, and the same if you use a SOD with the ring etc.
// Navarre, Mar 2007.

// Changed to use new light level macros, Cotillion, 2009-04-10
// Change set_keep to 1, this is a rare item, Zizuph 2021-11-18
...
effect_on(object player)
{
    ...
    add_prop(OBJ_I_LIGHT, DARKNESS_PASSIVE_RARE);
}
