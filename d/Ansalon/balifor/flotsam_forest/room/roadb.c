...
create_flotsam_room()
{
   ...
   add_item(({"small path","path"}),"This small path passes " +
      "through the fields to the east towards some farmhouses.\n");
   add_item("farmhouses","To the east are some small farmhouses, " +
      "half hidden by the fields of wheat. \n");
   
   add_exit(FROOM + "roada","north",0);
   add_exit(FROOM + "patha","east",0);
   add_exit(FROOM + "roadc","south",0);
...
long_descr()
{
   ...
   "A small path leads off the road to the east. " +
   tod_descr1() + "\n";
}
