...
reset_flotsam_room()
{
    ...
    if(!objectp(dragon))
    {
	dragon = clone_object(FNPC + "faery_dragon");
	dragon->add_prop(OBJ_I_HIDE, 60 + random(30));
	dragon->move(TO);
    }
}
...
create_flotsam_room()
{
    ...
    add_invis_exit(FROOM + "forest1s","north",0,4);
    add_invis_exit(FROOM + "forest3s","south",0,4);
