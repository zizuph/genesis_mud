 ...
 *
 *    Ashlar 980829 - supports a bartender that must be present for commerce
 *                    supports drinks that are separate objects, not cloned
 *                      from /std/drink
 */
#pragma strict_types
...
static  object  barkeeper;
static  int     require_barkeeper;
...
fix_drink(mixed data)
{
    ...
    if (stringp(data[0]))
    {
        ob = clone_object(data[0]);
        if (!ob->query_prop(HEAP_I_UNIT_VOLUME))
            ob->add_prop(HEAP_I_UNIT_VOLUME, ob->query_prop(OBJ_I_VOLUME));
        if (!ob->query_prop(HEAP_I_UNIT_WEIGHT))
            ob->add_prop(HEAP_I_UNIT_WEIGHT, ob->query_prop(OBJ_I_WEIGHT));
        if (!ob->query_prop(HEAP_I_UNIT_VALUE))
        {
            if (ob->query_prop(OBJ_I_VALUE))
                ob->add_prop(HEAP_I_UNIT_VALUE, ob->query_prop(OBJ_I_VALUE));
            else
...
{
    if (require_barkeeper)
    {
        barkeeper->command("say You can't afford it!");
    }
    else
    {
...
/*
 * Function name: pub_hook_no_barkeep
 * Description:   A hook to redefine if you want your own message when the
 *                barkeeper isn't present.
 * Returns:       int 0/1 - 0 for a notify_fail, 1 for a write.
 */
int
pub_hook_no_barkeep()
{
    notify_fail("There is noone here to take your order.\n");
    return 0;
}
...
pub_hook_player_buys(object ob, int price)
{
    if (require_barkeeper)
    {
        write("You pay the price of " + price + " coppers to " +
            barkeeper->query_the_name(this_player()) + ", who " +
            "gives you " + ob->short() + ".\n");
    }
    else
    {
...
pub_hook_ordered_too_many(int num)
{
    if (require_barkeeper)
    {
        barkeeper->command("say I will not serve more than " +
            MAX_PURCHASED + " items at a time.");
        return 1;
    }
    else
    {
...
order(string str)
{
    ...
    if (require_barkeeper && (!objectp(barkeeper) ||
        (environment(barkeeper) != this_object() &&
        environment(barkeeper) != environment(this_object()))))
    {
        return pub_hook_no_barkeep();
    }

    /* This will bar some people from buying in the pub. */
    if (check_cheater(this_player(), barkeeper))
        return 1;
...
/*
 * Function name: add_drink_object
 * Description:   Add a drink to the menu list
 * Arguments:	  id	    - An array with names to id the drink when buying
 *                file      - The filename of the drink object
 *                price     - The price of the drink
 */
varargs void
add_drink_object(mixed id, string file, int price)
{
    if (!pointerp(id))
    	id = ({ id });
...
/*
 * Function name: add_food_object
 * Description:   Add an item of food  to the menu list
 * Arguments:     id        - An array with names to id the food when buying
 *                file      - The filename of the food object
 *                price     - The price of the food (cost to buy)
 */
varargs void
add_food_object(mixed id, string file, int price)
{
    if (!pointerp(id))
        id = ({ id });
...
/*
 * Function name: set_barkeeper
 * Description:   Set the barkeeper object, and require it to be
 *                  present in order to buy things.
 * Arguments:     obj - The barkeeper object
 */
void
set_barkeeper(object obj)
{
    barkeeper = obj;
    require_barkeeper = 1;
}
...
/*
 * Function name: clear_barkeeper
 * Description:   Remove the requirement of a barkeeper
 */
void
clear_barkeeper()
{
