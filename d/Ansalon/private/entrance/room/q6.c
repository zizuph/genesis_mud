...
create_kendermore_room()
{
    ...
    set_long("You are standing in a dimly lit stone hall. Rows of " + 
        ...
        "throwing eerie shadows. A dark stair winds up into the darkness.\n");
    add_item("stair", "It winds beyond your view into the darkness. Who " + 
    ...
    add_item(({"rows", "west wall"}), "The doors probably lead into " + 
        "individual dungeon cells.\n");
    set_tell_time(300);
    ...
    add_exit(RDIR + "stairs", "up", "@@stair_message");
    add_exit(RDIR + "q4", "south", "@@hall_message");
    door = clone_object(ODIR + "qdoor3b");
    door->move(TO);
    reset_kendermore_room();
}
