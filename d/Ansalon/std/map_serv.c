/*
 *  map_serv.c
 *  by Milan       Revised: 20.1.2000
 *
 *  see also:  map_map.c
 *
 *  Look at code and function headers below. Also have a look in
 *  /d/Ansalon/open/example/mapserver dir
 */
#pragma save_binary
#pragma strict_types
#pragma no_clone
 ...
 * Note:	  Default to all exits obvious
 */
public int
...
add_exit(string place, string cmd, mixed efunc, mixed tired, mixed non_obvious)
{
    if (member_array(cmd, Std_dir) != -1)
    {
    	if (Relative == 1)
	    return 0;
	remove_exit(cmd); // we do not want two exits to same direction
    }
    else if (member_array(cmd, Rel_dir) != -1)
	return 0;
    return ::add_exit(place, cmd, efunc, tired, non_obvious);
}
...
add_map_exits()
{
	...
	{
	    ::add_exit(find_room_file(i), Std_dir[i], &block_map_room(i),
			query_map_fatigue, exit_not_obvious(Zz, d));
	    Map_exit_mask[i] = 1;
	}
	else if (Relative == 1)
	    ::add_exit(0, Std_dir[i], 1, 0, 1);
    }
}
