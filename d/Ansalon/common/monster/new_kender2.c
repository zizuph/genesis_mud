...
string *gKenderAdjOne = ({"whistling", "excited", "happy", "unkempt", 
    "carefree", "nosy", "inquisitive", "busy", "cheerful", "talkative",
    "bouncy", "friendly", "curious", "grinning", "merry", "smiling", 
    "energetic"});

string *gKenderAdjTwo = ({"tanned", "wrinkled", "chubby", "slender",
    "young", "red-cheeked", "dimpled", "wide-eyed", "pointy-eared",
    "fair-skinned", "ivory-skinned", "round-faced"});
...
create_krynn_monster()
{
    ...
    set_name("wanderlust");
    set_living_name("wanderlust");
    set_race_name("kender");
    set_gender(random(2));                 /* Male or female */
    set_extra_long("@@kender_extra_long"); /* Hair and eyes */

    set_alignment(600);         /* Good aligned */
    set_knight_prestige(-20);   /* Bad knights, killing kender */
    set_pick_up(1);             /* Pick things up */

    /* Random size */
    set_stats(({30 + i, 40 + i, 30 + i, 35 + i, 35 + i, 45 + i}));
    ...
    set_kender_adjs();    /* Set adjectives */
    adj_array = TO->query_adj(-1);
    gAdj = adj_array[0];
    gAdj2 = adj_array[1];
...
arm_me()
{
    ...
    /* Shirts for males, blouses for females */
...
/* Function name : set_kender_adjs
 * Description   : Set the adjectives
*/

public void
set_kender_adjs()
{
    add_adj(gKenderAdjOne[random(sizeof(gKenderAdjOne))]);
    add_adj(gKenderAdjTwo[random(sizeof(gKenderAdjTwo))]);
}
