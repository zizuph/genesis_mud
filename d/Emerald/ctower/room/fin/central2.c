...
create_fin()
{
    ...
    add_exit("finalsu1", "northwest");
    add_exit("finalsu3", "southwest");
    add_exit("finalsu2", "northeast");
    add_exit("finalsu4", "southeast");
}
...
sing(string str)
{
    ...
    if (!this_player()->move_living("M", CTOWER_ROOM_ENTRY + "central1", 1))
    {
    	say(QCTNAME(this_player()) +
	    "The chandelier starts to glow. Getting brighter and brighter " +
            "until the room is nothing but a blinding radiance. The light " +
            "lasts but a moment, and when your eyes readjust to the now " +
            "dim light, you notice that " + this_player()->query_pronoun() +
            " has disappeared.\n");
    
    	tell_room(CTOWER_ROOM_ENTRY + "central1", 
	    "The chandelier starts to glow. Getting brighter "+
	    "and brighter until the room is nothing but a blinding radiance. "+
	    "The light lasts but a moment, and when your eyes readjust " +
            "to the now dim light, you notice that " + QTNAME(this_player()) +
	    " has appeared.\n", this_player());
    }
