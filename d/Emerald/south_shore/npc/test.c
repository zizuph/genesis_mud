...
create_emerald_monster()
{
    ...
    set_stats(({ 150+random(20), 150+random(20), 165+random(20),
	150+random(10), 140+random(10), 150+random(10) }));
    set_alignment(0);

    set_all_attack_unarmed(50, 50);
    set_all_hitloc_unarmed(40);
    set_hitloc_unarmed(A_BODY, 75, 50, "body");
    ...
    if (CLONE_MASTER->may_clone(FANG, 50))
    {
	add_leftover(FANG, "fang", 1, "", 0, 1);
    }
    if (CLONE_MASTER->may_clone(SCALE, 50))
    {
	add_leftover(SCALE, "scale", 1, "", 0, 1);
    }
...
{
    find_living("velith")->catch_msg("DEBUG: react_intro called.\n");
...
special_attack(object victim)
{
    ...
    if (attack_type < 5)
    {
	set_alarm(0.0, 0.0, &tailwhip(victim));
	return 1;
    }
} /* special_attack */
...
{
    object *enemies = victim->query_team_others() + ({ victim });
    ...
    {
	damage = random(20) + F_PENMOD(160, 100);
	hitresult = enemies[i]->hit_me(damage, W_BLUDGEON, TO, -1);
...
drown_victim(object victim, object searcher)
{
    ...
    find_living("velith")->catch_msg("DEBUG: drown_victim "
      + "called.\n");

    victim->catch_msg("The serpent whips her tail around with "
      + "lightning speed. Before you can react, one of her tail "
    ...
    tell_room(environment(TO), "With lightning speed, the serpent "
...
{
    find_living("velith")->catch_msg("DEBUG: remove_victim "
      + "called.\n");

    tell_room(environment(TO), "The waters of Lake Telberin "
    ...
    tell_room(environment(TO), "You gasp for air, clinging "
      ...
      + "swept away. As the waters ebb and recede back into the "
...
notify_death(object killer)
{
    tell_room(environment(this_object()),
      "\n" + QCTNAME(this_object()) + " sways, and its graceful "
