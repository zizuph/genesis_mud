 ...
 *     for an acceptable room; the more invalid files in a directory,
...
varargs int clone(mixed id, mixed where, mixed how, mixed path)
{
    ...
    if (where)
    {
        ...
        if (!pointerp(where))
        {
            tell_room(this_object(), "Invalid destination designation.\n");
            return 0;
        }
    ...
    if (!objectp(where))
    {
        tell_room(this_object(), "No destination found.\n");
        return 0;
    }   
    ...
    else if (!strlen(path) || !(cl = clone_object(path)))
    {
        return 0;
    }
...
void restore_clones()
{
     ...
     */
    set_alarm(0.0, 0.0, &add_many_clones(clone_add_list));
}
...
{
    string *rooms = ({ }), *dir_files, *valid_files = ({ }), ran_room, fn, dir;
    object room;
    int i, j;
    ...
    for (i = 0; i < sizeof(area); i++)
    {
        ...
        else if ((file_size(area[i]) > 0) || (file_size(area[i] + ".c") > 0))
	{
            rooms += ({ area[i] });
	}
        else
	{
	    ...
	    {
	        dir = area[i] = "/" +
                    implode(explode(area[i], "/") - ({ "" }), "/") + "/";
            }
            else
	    {
	        dir = "/" +
                    implode(explode(area[i], "/")[..-2] - ({ "" }), "/") + "/";
	    }
...
void enter_inv(object ob, object from)
{
    ...
    if (id = clone_map[ob])
    {
        where = clones_to_add[id][1];
    }

    ran_room = random_room(where ? where : default_area);
    if (ran_room)
    {
