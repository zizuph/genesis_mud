...
start()
{
    ...
    spell_target->add_mana_drain(3);
    spell_target->modify_thirst_rate(-300);
    mkcmds();
...
mkcmds()
{
    setuid();
    seteuid(getuid());
    cmds = clone_object("/d/Emerald/common/guild/vamp/spells/obj/obfuscate_cmd");
...
dispel_spell_effect(object dispeller)
{
    ...
    if (invis_on)
    {
        ...
        {
            spell_target->catch_tell("Your cloak of invisibility fades " +
                "slightly, but you remain imperceptable to the naked eye.\n");
        }
    ...
    {
        if (!spell_target->quey_prop(OBJ_I_INVIS))
        {
            spell_target->catch_tell("Your cloak of invisibility expires " +
                "while you are still exposed, leaving you unable to fade " +
                "back out of the visible world.\n");
        }
        else
        {
            spell_target->catch_tell("Your cloack of invisibility fades, " +
                "but you remain imperceptable to the naked eye.\n");
        }
    }
...
fade_out()
{
    ...
    // Give some appropriate message to onlookers
}
