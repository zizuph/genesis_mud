...
#define MIN_REGEN_HPS  80

#define DRAIN_LOG(x)

#define POSNAME(x) \
    ("@@pos_name:" + file_name(this_object()) + "|" + file_name(x) + "@@")
#define CPOSNAME(x) \
    ("@@cpos_name:" + file_name(this_object()) + "|" + file_name(x) + "@@")

static int drain_alarm, times, regenerating, hps_drained;
...
pos_name(string s)
{
    ...
    if (!(who = find_object(s)))
    {
        return "someone";
    }

    for_whom = previous_object(-1);
    return LANG_POSS(who->query_the_name(for_whom));
}

string
cpos_name(string s)
{
    return capitalize(pos_name(s));
}
...
desc_vamp_drink()
{
    ...
    tell_room(({
        "A thin stream of blood trickles down " + POSNAME(drainee) +
            " neck.\n",
        ...
        QCTNAME(drainee) + " clutches at " + POSNAME(drainer) + " " +
        ...
        CPOSNAME(drainee) + " eyelids flutter.\n",
...
release_vamp_drainee(int silent)
{
    ...
    {
        regenerating = hps_drained = 0;
        drainer->catch_tell("You failed to regenerate.\n");
    }
}
...
do_vamp_drain()
{
        ...
        if (!drainee || drainee->query_ghost())
	{
            ...
            tell_others(QCTNAME(drainer) + " releases " +
                QTNAME(drainee) + " from " + POS(drainer) +
                " embrace, letting " + POS(drainee) +
...
#else
        if (regenerating && (hps_drained >= MIN_REGEN_HPS))
	{
            drainer->catch_tell("Regenerated!\n");
	}
        else
	{
...
void
start_vamp_regeneration()
{
    if (!drainee || regenerating)
    {
        return;
    }
