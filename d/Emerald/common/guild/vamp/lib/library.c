 ...
 *
 *     // Add a blocked exit so no-one can leave with a book
 *     add_exit("/d/Domain/subdir/room", "north", library_exit_block);
 * }
 *
 * void
 * init()
 ...
 *        
 * Questions, comments, suggestsions:  mail Shiva
 */
 ...
 * Description:   Redefine this to alter the message given when
 *                incorrect syntax is used for the "borrow" command
 ...
 *                user attempts to borrow a nonexistant book from
 ...
 * Description:   Redefine this to alter the message given when a
 *                user attempts to borrow a nonexistant book
 * Arguments:     string title - the title of the book
 * Returns:       1/0
 */
public int
...
library_give_book(string text_filename, string book_name)
{
    ...
    book = clone_object(SCROLL_OBJECT);
         ...
         string text, int approval)
{
    ...
    write_file(get_book_name(dir), 
        sprintf("%|75s\n%|75s\n%|75s\n%s", title, summary, author, text));
...
done_writing(string title, string summary, string input)
{
    ...
    add_book(title, summary, "by " + this_player()->query_name(), 
...
library_discard(string str)
{
    ...
    if (!strlen(str))
    {
        library_discard_syntax_failure_hook(str);
    }
    ...
    if (!strlen(book_file = book_map[str]))
    {
        library_discard_unavailable_book_hook(str);
    }
...
library_help()
{
    ...
    if (strlen(appr_dir))
    {
        str += 
            ...
            help_cmd("Borrow approval <title>",
