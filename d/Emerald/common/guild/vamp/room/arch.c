...
string reveal_door()
{
    this_player()->add_prop(FOUND_GTRAIN_DOOR);
...
string do_search(object who, string str)
{
    ...
    if (str == "door")
    {
        ...
        if (!this_player()->query_prop(FOUND_GTRAIN_DOOR))
        {
            return "";
        }

        if ((this_player()->query_skill(SS_AWARENESS) > 70) &&
            (this_player()->resolve_task(TASK_DIFFICULT,
            ({ TS_INT, TS_WIS, SS_AWARENESS })) > 0))
        {
            return search_door() + reveal_buttons();
        }
    ...
    else if ((this_player()->query_skill(SS_AWARENESS) > 50) &&
        (this_player()->resolve_task(TASK_DIFFICULT,
        ({ TS_INT, TS_WIS, SS_AWARENESS })) > 0))
    {
        return reveal_door();
...
void open_door()
{
    ...
    this_player()->move_living(" through the doorway", VAMP_ROOM_DIR + "gtrain", 1);
}
