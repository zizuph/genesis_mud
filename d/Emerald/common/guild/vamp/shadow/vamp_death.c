...
vampire_death(object ob, int extra_xp_loss)
{
  ...
  if (SW->remove_cmdsoul(VAMP_CMDSOUL))
  {
    ...
    //There is no penalty for leaving the guild at this time.
    //SW->set_hp(100);
    //SW->do_die(ob);

    SW->catch_tell("You feel your soul losing its grip on your immortal form, "+
        "fading away from the darkness that granted you eternal life.\n\n\n");

    tell_room(E(SW),
        QCTNAME(SW) + "'s body suddenly crumbles to the ground, disintegrating into a " +
            "clear dust as a heavy, swirling mist rises from the ashes " +
            "and drifts away.\n", SW);

    //Reset their default start loc 
    SW->remove_default_start_location();
    SW->move_living("M", SW->query_default_start_location(), 1, 0);
