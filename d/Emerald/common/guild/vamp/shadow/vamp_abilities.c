 ...
 *
 * Lucius Dec 01 2007: Changed apply_cost() to stop mana drain after
 *                     exceeding ALARM_INTERVAL unit of time instead of
 *                     letting it keep draining for 15 minutes.
 */
...
apply_costs()
{
    if (SW->query_linkdead() &&
        ((time() - SW->query_linkdead()) > ftoi(ALARM_INTERVAL)))
    {
        set_unseen(0);
        return;
    }
