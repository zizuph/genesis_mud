...
create_room()
{
    ...
    set_long(
        "   The passage is definately warmer to the southwest. It seems to " +
        "be getting lighter, too. You could probably see in here without " +
        "the light from the torches. Perhaps there is a way out of these " +
       "caves that is pretty close.\n\n");

    add_prop(ROOM_I_INSIDE, 1);

    add_exit(THIS_DIR + "secret_passage3c", "northeast", 0, 1);
    add_exit(THIS_DIR + "secret_passage1c", "southwest", 0, 1);
