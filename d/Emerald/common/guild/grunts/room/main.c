...
create_room()
{
	...
	"   What a disgusting smell! This cave has never been cleaned "
      ...
      + "race guild: and that's exactly what this place is.\n\n"
...
join(string str)
{
    ...
    write("A collar suddenly appears around your neck. You feel a bit"
      + " smellier ... and you like it!\n");

    write_file(GRUNTS_LOG_DIR + "join",
...
my_leave(string str)
{
    ...
    if (query_verb() == "leave" && str == "grunts")
    {
        write("You may <hate the smell> here to leave the guild.\n");
        return 1;
    }
    ...
    write("The collar around your neck falls to pieces, and you finally"
      + " get rid of that fithly smell! Ahhh ... much better.\n");

    write_file(GRUNTS_LOG_DIR + "leave",
        TIME2FORMAT(time(), "mm/dd/yyyy") + " ("
      + ctime(time())[11..18] + ") "
      + capitalize(this_player()->query_real_name()) 
      + " left the Grunts.\n");
