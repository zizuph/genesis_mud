/*
 *  /d/Emerald/common/guild/aod/rooms/glow_wep_drop.c
 * 
 *  This is the droproom for glowing weapons for the Army of
 *  Darkness guild.
 *
 *  Created December 2006, by Cooper Sherry (Gorboth)
 */
#pragma strict_types
#include "../defs.h"

inherit AOD_ROOM_DIR + "drop_room";

#include <language.h>

/* definitions */
#define            FALL_LOCATION (AOD_ROOM_DIR + "norm_wep_drop")

/* global variables */
public object      Fissure;

/* prototypes */
public void        create_drop_room();
public void        enter_inv(object ob, object from);
public void        continue_fall(object ob, object from);

/*
 * Function name:        create_drop_room
 * Description  :        set up the room with area defaults
 */
public void
create_drop_room()
{
    set_short("a droproom beside a pile of glowing weapons");
    set_em_long("This appears to be, quite literally, the"
      + " drop room system for the Army of Darkness. A huge"
      + " pile of glowing weapons lies upon a ledge here where"
      + " (evidently) the things dropped from the top of the"
      + " ravine have landed. Right beside the pile is a deep fissure,"
      + " which shimmers with purple magic! Switchbacks travel down into"
      + " the depths and a stair leads up out of the area.\n");

    add_item( ({ "stair", "stairs", "stairway", "set of stairs" }),
        "The stairs lead up, away from the switchbacks.\n");
    add_item( ({ "pile", "large pile", "pile of weapons",
                 "weapon pile", "pile of glowing weapons",
                 "large weapon pile", "large pile of glowing weapons",
                 "weapon", "glowing weapon", "weapons",
                 "glowing weapons" }),
        "This ledge appears to be where glowing weapons that are"
      + " dropped off of the ravine by members of the Army pile up."
      + " Sadly, all of these ones were broken by the fall. Perhaps"
      + " there are others lying around here that fared better.\n");
    add_item( ({ "broken weapon", "broken weapons" }),
        "A highly imperfect system, this seems to be! Ah well, what"
      + " can be expected of ogres?\n");
    add_item( ({ "purple magic", "magic energies",
                 "magic energy", "purple energy" }),
        "Ooo! Maybe this fissure is magic!\n");

    add_droproom_items();

    add_spike("glow_wep_drop");

    Fissure = clone_object(AOD_OBJ_DIR + "glow_wep_fissure");
    Fissure->move(this_object());

    add_exit("norm_wep_drop", "down");
    add_exit("start", "up");
} /* create_drop_room */


/*
 * Function name:        enter_inv
 * Description  :        Called when objects enter this container or when
 *                       an object has just changed its weight/volume/light
 *                       status. In this case, we want to move anything
 *                       non-living to the proper droprooms.
 * Arguments    :        ob: The object that just entered this inventory
 *                       from: The object from which it came.
 */
public void
enter_inv(object ob, object from)
{
    set_alarm(1.0, 0.0, &continue_fall(ob, from));
    ::enter_inv(ob, from);
} /* enter_inv */


/*
 * Function name:        continue_fall
 * Description  :        determine whether or not the object is
 *                       supposed to continue falling to a
 *                       different droproom.
 * Arguments    :        object ob - the object
 *                       object from - where it came from
 */
public void
continue_fall(object ob, object from)
{
    if (!ob)
        return;
    
    if (living(ob) ||
        ob->id("_aod_trophy_spike") ||
        ob->id("coin") ||
        ob->id("_aod_fissure") ||
        environment(ob) != this_object())
    {
        return;
    }

    if (ob->check_weapon() && ob->check_recoverable())
    {
        if (objectp(Fissure))
        {
            tell_room(this_object(), capitalize(LANG_THESHORT(ob))
              + " skids along the ledge and drops into a deep"
              + " fissure that is right beside the switchbacks here.\n");
            ob->move(Fissure);
            return;
        }

        tell_room(this_object(), capitalize(LANG_THESHORT(ob)) + " skids"
          + " along the ledge and comes to rest beside the pile of"
          + " glowing weapons.\n");
        return;
    }

/*    tell_room(this_object(), capitalize(LANG_THESHORT(ob)) + " bounces"
      + " bounces off a rock and disappears into the ravine.\n");
    tell_room(FALL_LOCATION, capitalize(LANG_ADDART(ob->short()))
      + " tumbles into the area from above.\n");*/

    tell_room(this_object(), capitalize(LANG_THESHORT(ob))
      + " bounces off a rock and disappears into the ravine.\n");
    tell_room(FALL_LOCATION, capitalize(LANG_ASHORT(ob))
      + " tumbles into the area from above.\n");

    ob->move(FALL_LOCATION);
} /* continue_fall */