/*
 *  /d/Emerald/common/guild/aod/lib/shop.c
 *
 *  A modified shop module for the Army of Darkness shop.
 *
 *  This file is modified from /d/Gondor/common/lib/shop which
 ...
 *
 *  Created in December 2006 by Cooper Sherry (Gorboth)
 *
 ...
 *
 *  Modified Mirandus to make sure that filled containers couldn't be 
 *  sold.  10/05/2015
 */
#pragma save_binary
#pragma strict_types
...
setup_shop()
{
    set_store_room(AOD_ROOM_DIR + "store");
    set_shop_name("The Army of Darkness Standard Shop");

    set_no_sell_prop("_aod_no_sell");
        ...
        if (keeper)
        {
            ...
            else
            {
                notify_fail("The shopkeeper seems to be gone. You can"
                  + " <bash counter> to try and get some service.\n");
                return 0;
            }
        }
        else
...
{
    notify_fail("The shopkeeper booms: You not have " + str + "!\n");
    return 0;
}
...
shop_hook_sold_nothing()
{
    if (!this_player()->query_prop(SHOP_TMP_WANT_NO_BUSINESS))
        notify_fail("The shopkeeper booms: Not sell anything!\n");
    return 0;
}
...
{
    notify_fail("The shopkeeper booms: " + capitalize(LANG_THESHORT(ob))
      + "?? Not want! Not buy!\n");
}
...
shop_hook_sell_no_sell(object ob, string str)
{
    if (stringp(str))
        ...
        notify_fail("The shopkeeper booms: Not buy that! Not want!\n");
}
...
{
    notify_fail("The shopkeeper booms: You stuck with that!\n");
}
...
{
    notify_fail("The shopkeeper says: Not have " + str + "! Not sell.\n");
}
...
shop_hook_cant_carry(object ob, int err)
{
    notify_fail("You drop " + LANG_THESHORT(ob) + " on the floor.\n" +
       "The shopkeeper booms: You sissy! Too weak to carry! HOHOHO!\n");
}
...
shop_hook_sell_filled_container(object ob)
{
    notify_fail(capitalize(LANG_THESHORT(ob)) + " is not empty.\n" +
	   "The shopkeeper booms: You dumb! Bag still have things! HOHOHO!\n");
}

public void
shop_hook_buy_magic_money(object ob)
{
    keeper->command("say This money ... MAGIC! NOT DEAL!\n");
}
...
shop_hook_buy_pay_money(string str, string change)
{
    ...
    if (change)
        write("The shopkeeper, taking almost ten minutes to"
          + " do the math, gives you " + change + " back.\n");
}
...
{
    if (!this_player()->query_prop(SHOP_TMP_WANT_NO_BUSINESS))
        notify_fail("The shopkeeper booms: Not want that!\n");
    else
        notify_fail("");
}
...
{
    keeper->command("say Maybe pay " + text +
        " for " + ob->short() + ". MAYBE!");
}
...
{
    keeper->command("say Err ... " + LANG_THESHORT(ob) +
        " worth? Hmmm ... maybe " + text + ".");
}
...
{
    notify_fail("The shopkeeper booms: Not have '" + str + "'!\n");
}
...
{
    notify_fail("The shopkeeper booms: Store empty!\n");
}
...
{
    keeper->command("say HEY! THIS LOOK LIKE FROM DROPROOM!!!");
    keeper->command("glare " + pl->query_real_name());
    keeper->command("say NOT BUY! YOU PULP-PACK!");
}
...
{
    keeper->command("say NOT YOU! NO SELL! NO BUY!!");
    keeper->command("say OUT! OUT! PULP!! PULP!! PULP!!");
}
...
shop_hook_allow_sell(object ob)
{
    ...
    if ((msg = shop_hook_filter_sell(ob)))
    {
        if (stringp(msg))
            ...
            notify_fail("The shopkeeper booms: Not buy " + ob->short() + "!\n");
        return 0;
    }
...
sell_it(object *ob, string str, int check) 
{
    ...
    for (i = 0, s = sizeof(ob); i < s; i++)
    {
		...
		if (sizeof(all_inventory(ob[i])) > 0)
		{
			shop_hook_sell_filled_container(ob[i]);
			continue;
		}
