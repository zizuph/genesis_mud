/*
 * /d/Emerald/common/guild/aod/lib/bed_code.c
 *
 * I have modified this open Gondor code to work for a bed and
 * lying down, rather than for sitting on things.
 *
 *   - Gorboth, January 2006
 * 
 * Originally: /d/Gondor/open/room_chairs/room_chairs.c
 *
 ...
 */
public int
add_chair(string name, string parse_pat, int capacity = 1, string prep = "on",
int mentioned = 1)
{
    if (strlen(name) &&
        strlen(parse_pat))
...
sitting_on_chair(string name, object for_obj, int namesonly, int definite = 1)
{
    ...
    if (!namesonly)
        desc += " lying " + chairptr[I_PREP] + my_art + name + ".\n";
    return desc;
} /* sitting_on_chair */
...
do_sit(string arg)
{
    ...
    {
        notify_fail("You are already lying down.\n");
        return 0;
    }
    ...
    if (tp->query_prop(OBJ_I_HIDE) ||
        ...
        tp->query_invis())
    {
        write(break_string("You decide not to, since that would " +
...
do_stand(string arg)
{
    ...
    {
        notify_fail("You are not lying down.\n");
        return 0;
    }
...
show_subloc(string subloc, object pl, object for_obj)
{
        ...
        pointerp(chairptr = Room_chairs[name]))
    {
        if (for_obj != pl)
        {
            return capitalize(pl->query_pronoun()) + " is lying " +
                chairptr[I_PREP] + " the " + name + ".\n";
        }
        return "You are lying " + chairptr[I_PREP] + " the " +
            name + ".\n";
    }
    return "";
} /* show_subloc */
...
{
    write("You lie down " + prep + " the " + name + ".\n");
    say(QCTNAME(this_player()) + " lies down " + prep + " the " +
        name + ".\n");
} /* do_sit_msgs */
