...
create_nen_hithoel()
{
    ...
    add_exit("west6", "north");
    set_distance("north", 1);
    add_exit("mid6", "northeast");
    set_distance("northeast", 1);
    add_exit(DELRIMMON_DIR + "lakeside/parth1", "west");
    set_distance("west", 1);
    add_exit("far_west6", "northwest");
    set_distance("northwest", 1);

    set_no_exit_msg( ({ "east" }),
        "The sides of Ael Rannath rise from the water to the east,"
      + " their rock walls blocking your path.\n");
} /* create_nen_hithoel */
