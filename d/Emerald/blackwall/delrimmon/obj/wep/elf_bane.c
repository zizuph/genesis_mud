...
public int          check_elf_present(mixed roomstr);
int                 exa_runes(string str);
public string       long_func();
private string      rune_description();
public int          elf_near(object room);
public int          elf_present(object room);
...
public int
check_elf_present(mixed roomstr)
{
    object *liv,
            room;
     ...
     * first.
     *
     * Note that query_elf() is defined only in the 
    ...
    return sizeof(filter(liv, &operator(==)("elf") @ &->query_race()));
}

public int
elf_near(object room)
{
    int     n,
            elf_found;
    string *exit_rooms;
    ...
    n = sizeof(exit_rooms);
    while (n--)
        elf_found += check_elf_present(exit_rooms[n]);

    return elf_found;
}

public int
elf_present(object room)
{
    if (room->query_prop(ROOM_I_IS))
        ;
    else if (ENV(TO) != room)
        ...
        int dt, int phit, int dam)
{
    ...
    if (phurt == -1 || phurt == -2) 
    {
        query_wielded()->catch_msg(BSN("The " +short()+ " slashes"
    ...
    {
        query_wielded()->catch_tell("Your " +short()+ " just barely misses "+
            QTNAME(enemy)+"!\n");
    ...
    {
        enemy->heal_hp(-dam); /* The sword does double damage on the elf */

        query_wielded()->catch_msg("Your " +short()+ " slashes violently into"
          + " the elf flesh of " +QTNAME(enemy)+ "!\n");
...
long_func()
{
    string  txt = 
        ...
        + " silver, leaving its rune-carved surface a with a blackened"
        + " hue.";

    if (elf_present(ENV(TO)))
        txt += " The " +short()+ " is vibrating violently!";
    else if (elf_near(ENV(TO)))
...
wield(object obj)
{
    ...
    if (TP->query_race() == "elf" && !TP->query_wiz_level())
        return "The " + short() + 
