 ...
 * - Fixed type confusion in read runes
 */
#pragma strict_types
...
#define  TPH_MSG   ({  ("struggles desperately to control the " + short() + "!"),\
                       ("grips the " + short() + " tightly, as it darts violently about."),\
                       ("attempts to steady the " + short() + ", which is vibrating violently."),\
                       ("is nearly unable to control the " + short() + ", which darts wildly of its own accord!"),\
                    })
...
create_weapon()
{
    ...
    set_short("blackened mithril lance");
    set_long(long_func);

    add_item(({"rune","runes"}), rune_description);
    add_item(({"haft","emblem"}), BSN(
        "The haft of the blackened mithril lance bears at its"
    ...
    // functions:
    set_default_weapon(50, 52, W_POLEARM, W_IMPALE, W_BOTH, TO);

    add_prop(OBJ_I_IS_MAGIC_WEAPON,1);
    add_prop(OBJ_S_WIZINFO, BSN(
        "This lance is magically enhanced to slay undead opponents."
    ...
    add_prop(MAGIC_AM_ID_INFO, ({
        "An aura of evil magic radiates from the lance!\n",1,
        ...
        int dt, int phit, int dam)
{
    ...
    {
        query_wielded()->catch_msg("The " +short()+ " stabs"
          + " violently through open air.\n");
        enemy->catch_msg("You deftly duck as "+QTNAME(query_wielded())
          + " stabs " +query_wielded()->query_possessive() + short()
          + " at you!\n");
    }
    else if (phurt == 0) // This means hit, but weapon bounces off the armour.
    {
      ...
      + short() + " of " +QTNAME(query_wielded())+ " as it stabs violently"
      + " in front of you!\n");
    }
    else
    {
        enemy->heal_hp(-dam); /* The lance does double damage on the undead */

        query_wielded()->catch_msg("Your " +short()+ " stabs violently into"
          + " the undead flesh of " +QTNAME(enemy)+ "!\n");
        enemy->catch_msg("You shriek as the " +short()+ " stabs"
          + " violently through your flesh!\n");
        say(QCTNAME(enemy)+" shrieks in dismay as the "
          + short() + " pierces deeply!\n",({enemy,query_wielded()}));
    }
    return 1;
}
...
long_func()
{
    string  txt =
        ...
        + " blade of mithril extends from the weapon's long haft."
...
exa_runes(string str)
{
    ...
    write("You study the runes on the lance carefully.\n"
             + rune_description());
