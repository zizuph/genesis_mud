...
// Signal patterns are bit-encoded values, 1 = long 0 = short. A 1 is prepended
// to allow for leading shorts; it should not be used as part of the signal.
// For example, four shorts is binary 10000, but only 0000, four shorts,
...
public void
xfer_signals2(string *names)
{
    mixed pattern;
    int i;

    for (i = 0; i < sizeof(names) && i < 10; i++)
    {
        write(names[i]);
        pattern = query_first_pattern(names[i]);
        if (stringp(pattern))
        {
            switch (pattern)
            {
                 case SIGNAL_PATTERN_1:
                   set_first_pattern(names[i], SIGNAL_PATTERNS[0]);
                   break;
                 case SIGNAL_PATTERN_2:
                   set_first_pattern(names[i], SIGNAL_PATTERNS[1]);
                   break;
                 case SIGNAL_PATTERN_3:
                   set_first_pattern(names[i], SIGNAL_PATTERNS[2]);
                   break;
                 case SIGNAL_PATTERN_4:
                   set_first_pattern(names[i], SIGNAL_PATTERNS[3]);
                   break;
                 case SIGNAL_PATTERN_5:
                   set_first_pattern(names[i], SIGNAL_PATTERNS[4]);
                   break;
            }
        }

        pattern = query_second_pattern(names[i]);
        if (stringp(pattern))
        {
            switch (pattern)
            {
                 case SIGNAL_PATTERN_1:
                   set_second_pattern(names[i], SIGNAL_PATTERNS[0]);
                   break;
                 case SIGNAL_PATTERN_2:
                   set_second_pattern(names[i], SIGNAL_PATTERNS[1]);
                   break;
                 case SIGNAL_PATTERN_3:
                   set_second_pattern(names[i], SIGNAL_PATTERNS[2]);
                   break;
                 case SIGNAL_PATTERN_4:
                   set_second_pattern(names[i], SIGNAL_PATTERNS[3]);
                   break;
                 case SIGNAL_PATTERN_5:
                   set_second_pattern(names[i], SIGNAL_PATTERNS[4]);
                   break;
            }
        }

        pattern = query_third_pattern(names[i]);
        if (stringp(pattern))
        {
            switch (pattern)
            {
                 case SIGNAL_PATTERN_1:
                   set_third_pattern(names[i], SIGNAL_PATTERNS[0]);
                   break;
                 case SIGNAL_PATTERN_2:
                   set_third_pattern(names[i], SIGNAL_PATTERNS[1]);
                   break;
                 case SIGNAL_PATTERN_3:
                   set_third_pattern(names[i], SIGNAL_PATTERNS[2]);
                   break;
                 case SIGNAL_PATTERN_4:
                   set_third_pattern(names[i], SIGNAL_PATTERNS[3]);
                   break;
                 case SIGNAL_PATTERN_5:
                   set_third_pattern(names[i], SIGNAL_PATTERNS[4]);
                   break;
            }
        }

        pattern = query_fourth_pattern(names[i]);
        if (stringp(pattern))
        {
            switch (pattern)
            {
                 case SIGNAL_PATTERN_1:
                   set_fourth_pattern(names[i], SIGNAL_PATTERNS[0]);
                   break;
                 case SIGNAL_PATTERN_2:
                   set_fourth_pattern(names[i], SIGNAL_PATTERNS[1]);
                   break;
                 case SIGNAL_PATTERN_3:
                   set_fourth_pattern(names[i], SIGNAL_PATTERNS[2]);
                   break;
                 case SIGNAL_PATTERN_4:
                   set_fourth_pattern(names[i], SIGNAL_PATTERNS[3]);
                   break;
                 case SIGNAL_PATTERN_5:
                   set_fourth_pattern(names[i], SIGNAL_PATTERNS[4]);
                   break;
            }
        }

        pattern = query_fifth_pattern(names[i]);
        if (stringp(pattern))
        {
            switch (pattern)
            {
                 case SIGNAL_PATTERN_1:
                   set_fifth_pattern(names[i], SIGNAL_PATTERNS[0]);
                   break;
                 case SIGNAL_PATTERN_2:
                   set_fifth_pattern(names[i], SIGNAL_PATTERNS[1]);
                   break;
                 case SIGNAL_PATTERN_3:
                   set_fifth_pattern(names[i], SIGNAL_PATTERNS[2]);
                   break;
                 case SIGNAL_PATTERN_4:
                   set_fifth_pattern(names[i], SIGNAL_PATTERNS[3]);
                   break;
                 case SIGNAL_PATTERN_5:
                   set_fifth_pattern(names[i], SIGNAL_PATTERNS[4]);
                   break;
            }
        }
    ...
    if (sizeof(names))
    {
        set_alarm(0.5, 0.0, &xfer_signals2(names));
    }
}
...
xfer_signals()
{
    ...
    files = get_dir("/d/Emerald/log/del_rimmon/quest_data/*.o");
    names = map(files, &extract(, 0, -3));
    set_alarm(0.5, 0.0, &xfer_signals2(names));
}
