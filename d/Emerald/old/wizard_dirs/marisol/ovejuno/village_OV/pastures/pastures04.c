...
inherit OV_PASTURES;
#define SHEPHERD1 OV_VILLAGE_NPC + "shepherd01.c"
...
void reset_room()
{
 if(!objectp(shepherd1))
 {
  shepherd1=clone_object(SHEPHERD1);
  shepherd1->move(TO);
  shepherd1->command("emote walks in.");
 }
}
...
create_emerald_pasture()
{
    ...
    add_exit("pastures01.c", "east");
    add_exit("pastures03.c", "north");
    add_exit("pastures02.c", "northeast");
