 ...
 * 09/02/1998 Copied/Modified to make a terra cota lamp for Ovejuno/
 * Emerald by Marisol
 */
...
create_torch()
{
    ...
    set_short("terra cota oil-lamp");
    set_pshort("terra cota oil-lamps");
...
long_description()
{
    string longstr;
    if (!strlen(lamp_desc))
        longstr = "It is a terra cota oil-lamp. The oil container is "+
                  "a terra cota vase with a hole at the top. The wick "+
                   "goes through the hole and a tall glass flute "+
                   "cover the wick to protect against the wind. "+
                   "A metal handle is attached to both sides "+
                   "of the terra cota vase to easy carry.\n";

    else longstr = lamp_desc;
 
    if (!query_time(1))
    {
        longstr += "The terra cota lamp does not contain any oil. ";
    }
    else
    {
        longstr += "The terra cota lamp contains some oil. ";
    }
...
wick_description()
{
    return ("The wick of the terra cota oil-lamp sticks out from a hole "+
           "in the top of the vase.. " + (query_prop(OBJ_I_LIGHT) ?
...
do_refill(string str)
{
    ...
    {
        NFN0("The terra cota lamp is already filled with oil!");
    }
    ...
    if ((query_time(1) + oil_flask->query_amount()) > query_max_time())
    {
        ...
        WRITE("You fill the terra cota lamp with as much oil as it can hold.");
        SAY(" fills the terra cota lamp with as much oil as it can hold.");
    ...
    WRITE("You fill the terra cota lamp with all the oil in the flask. " +
        "The oil-lamp is not completely filled with oil.");
    SAY(" fills extra lamp oil into the terra cota lamp.");
...
notify_low_on_oil()
{
    ...
    if (living(env))
    {
        env->catch_msg("Your terra cota lamp flickers briefly. It " +
