...
e_rwait()
{
    allbb(" draws the symbol of the Red Fang Riders on the ground in the "+
        "dirt with "+HIS(TP)+" foot.");
    write("You draw the symbol of the Red Fang Riders on the ground in the "+
        "dirt with your foot.\n");
    return 1;
}
...
e_rtwirl(string arg)
{
    ...
    if(present(obj,TP))
        ...
        allbb(" twirls the "+obj->query_short()+" in "+HIS(TP)+
            " impressively with a malevolent gleam in "+HIS(TP)+" eye.");
...
e_rgrowl(string arg)
{
    ...
    if (!strlen(arg))
        {
        allbb(" growls with menacing obvious hatred.");
        write("You growl with menacing obvious hatred.\n");
        SOULDESC("growling with menacing obvious hatred");
        return 1;
    }
    ...
    actor("You growl at", oblist, " with menacing obvious hatred.");
    all2actbb(" growls at", oblist, " with menacing obvious hatred.");
    targetbb(" growls at you with menacing obvious hatred.", oblist);
    return 1;
}
...
{
    if (!ob->query_guild_member("Red Fang Riders Guild"))
        return 0;
...
e_rlist(string arg)
{
    ...
    write("You know that the following members of the Red Fang Riders "+
...
e_risay(string arg)
{
    if (!strlen(arg))
        {
        NF("Say what in the Rider tongue?\n");
        return 0;
    }
    ...
    if(TP->query_skill(SS_LANGUAGE) < 30)
        ...
        NF("You are not skilled enough in foreign tongues to be able to "+
            "speak the Rider tounge.\n");
        return 0;
    }
    
    if (this_player()->query_get_echo())
        write("In the Rider Tounge, you say: " + arg + "\n");
    else
        write("Ok.\n");
...
e_rhowl()
{
    write("You viciously howl in fury like a wolf.\n");
    say(QCTNAME(TP)+" viciously howls in fury like a wolf.\n");
    return 1;
}
...
e_rwar()
{
    ...
    switch(random(6))
    {
        ...
        break;
        case 2:
        warcry = "Long live the Red Fang Riders!\n";
        break;
        case 3:
...
rslash(string str)
{
    object wolf, enemy,*enemys;
    int skill,my_dis,pen;
    string mess;
    string com;
    mixed *hitres;
    wolf = TP->query_prop(WOLF_O_RIDING);
    NF("You are not on a wolf!\n");
    if(!objectp(wolf))
        return 0;
    NF("Your warwolf is too tired.\n");
    if(wolf->query_fatigue() < 1)
        return 0;
    ...
    wolf->add_fatigue(-5);
    pen = F_PENMOD(MIN(50,20+(my_dis/4)),skill);
    ...
    switch(hitres[0])
    {
        ...
        break;
        case 11..20:
           mess = "firmly, opening a deep wound";
        break;
        case 21..30:
    ...
    TP->catch_msg("Your "+wolf->short()+" slashes "+QTNAME(enemy)+
    " "+mess+".\n");
    enemy->catch_msg(QCTNAME(TP)+"'s "+wolf->short()+" slashes "+
    "you "+mess+".\n");
    tell_room(E(TP),QCTNAME(TP)+"'s "+wolf->short()+" slashes "+
    QTNAME(enemy)+" "+mess+".\n",({enemy,TP}));
    if(com)
        enemy->command(com);
    ...
    if(enemy->query_hp() <= 0)
    {
        tell_room(E(TP),"The "+wolf->short()+"'s final slash rends it's target apart, leaveing a mutilated corpse in it's place!.\n");
    wolf->command("emote snarls savagely at the kill, it's eyes blazing with battlelust.");
    enemy->do_die(TP);
    }
    return 1;
}
