...
#include "/d/Emerald/cave/pearl_defs.h"
...
create_creature()
{
    ...
    set_long("This is a large octopus (wroughly human size), and is not " +
...
give_reward(object player)
{
    ...
    {
	WRITE_LOG(PEARL_LOG, player, "earned a pearl.");
	return;
    }
    else
    {
	if (player->query_exp() > PEARL_QUEST_XP )
	{
	    WRITE_LOG(PEARL_LOG, player, "earned a pearl " +
		"and " + PEARL_QUEST_XP + " xp.");
