...
// debugging:   
   player->catch_msg("End_conversation not called!\n " +
		     "PlayerUnresponsive = " +
		     PlayerUnresponsive + "\n" +
		     player->query_name());
...
start_conversation(object player)
{
    ...
    // do nothing if the lieutenant is not present
    if (! lieutenant)
    {
	return;
    }
...
// First, lets make sure a player who has left the room is not attacked!
    ...
    /* Commented out because it seems to cause problems
    lieutenant = present("_gregarry");
...
*/
    ...
    if (! arg)
    {
	notify_fail("You must respond with a word!\n");
	return 0;
    }
...
create_banditcave_room()
{
    ...
    add_cmd_item(({"puddles", "pools"}), ({"drink", "drink from"}),
	   ...
	   "The swift flowing stream, on the other hand, looks much "+
	   "more pleasant.\n");
...
// Turning counter-clockwise: Raising the platform.
    ...
    write("As you turn the crank, the platform is gradually lifted off "+
	  "of the ground, sliding upwards along the metal rails. You "+
