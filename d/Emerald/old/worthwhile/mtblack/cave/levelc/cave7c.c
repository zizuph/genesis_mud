...
create_room()
{
    ...
    set_long("   "+
        "You wander a bit further along the cave. " +
        "This cave seems a bit odd for some reason. " +
        "It looks like it has been dug out, and the floor is too " +
        "flat and smooth for a normal cave. There seems to be " +
        "light coming down the cave from the east.\n\n");
    ...
    add_exit(THIS_DIR+"cave11c", "northwest", 0, 1);
    add_exit(THIS_DIR+"cave8c", "east", 0, 1);
    add_exit(THIS_DIR+"cave4c", "southwest", 0, 1);
