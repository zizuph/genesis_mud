...
create_room()
{
    ...
    set_long("   "+
        "The cave forks here, with two smaller caves branching off of " +
        "the main one. You can guess what you'll find in the caves, too, " +
        "judging by the smell of Ogres. Going down there without armour " +
        "or weapons would probably be inadvisable, possibly fatal.\n\n");
    ...
    add_exit(THIS_DIR+"cave9c", "northwest", 0, 1);
    add_exit(THIS_DIR+"cave2c", "southwest", 0, 1);
    add_exit(THIS_DIR+"cave6c", "east", 0, 1);
}
