...
login_vampire()
{
    ...
    {
        vamps = VAMPS(users());
        name = SW->query_real_name();
        sire = query_sire();
        coven = query_vamp_coven();
...
start_vampire()
{
    ...
    CP(SW, PRE_OBJ_MAGIC_RES + MAGIC_I_RES_DEATH, VAMP_DEATH_RES);
#endif

#ifdef VAMP_POISON_RES
    // we're resistant to poisons
    CP(SW, PRE_OBJ_MAGIC_RES + MAGIC_I_RES_POISON, VAMP_POISON_RES);
#endif
...
{
    return (IS_WIZ(SW) ? SW->drink_alco(strength, ask) : 0);
}
...
{
    return (IS_WIZ(SW) ? SW->drink_soft(amnt, ask) : 0);
}
