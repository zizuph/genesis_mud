...
/* The cave has the entrance to the under-water cave area.        */
...
create_room()
{
    ...
    set_short("A dark cave, with what seems to be a large well");    
    set_long("   "+
        "This cave has three tunnels leading from it. " +
        "Also, there is a large hole in the middle of the cave, which " +
        "seems to contain a lot of water. It seems to be very deep, " +
        "and you cannot see the bottom. At least there are not too many " +
        "Ogres here. It could be that the water scares them!\n\n");
    ...
    add_item(({"floor","ground"}),
        ...
        "of volcanic rock. There is a pool of water here, too.\n");
    add_item(({"hole","well","pool","water"}), 
        "The water in the hole is quite cold, and rather murky. " +
    ...
    add_exit(THIS_DIR+"cave13c", "south", 0, 1);
    add_exit(THIS_DIR+"cave15c", "west", 0, 1);
    add_exit(THIS_DIR+"cave22c", "northeast", 0, 1);
}

/*
 * Function name:   do_dive
 * Description:     move player into the well
...
do_dive(string str)
{
    ...
    */
    ...
    TP->move_living("into the water", WELL_DIR + "water1d", 0, 0);
