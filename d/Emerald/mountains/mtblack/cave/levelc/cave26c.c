...
/* This cave is adjacent to where Enzalcaspin lives.               */
...
create_room()
{
    set_noshow_obvious(1);  /* This hides the exits */
    add_prop(ROOM_I_INSIDE, 1);
    add_prop(ROOM_I_LIGHT, 0);
    set_short("A dark cave, with light from the north.\n" +
              "There are two obvious exits: north, dive");
    set_long("   "+
        "This cave has a passage leading on to the north. The air " +
        "in here is remarkably fresh, and it is not at all stuffy. " +
        "There seems to be light coming from the passage to the north.\n" +
        "\nThere are two obvious exits: north, dive.\n");
    ...
    add_exit(THIS_DIR + "cave27c", "north");
...
do_dive(string str)
{
    ...
    TP->move_living("into the water", WELL_DIR + "water7d", 0, 0);
