...
create_room()
{
    ...
    set_short("You are going round a corner");    
    set_long("   " +
        "You are now standing in a part of the cave where it turns " +
        "a rather tight corner. It is really quite amazing how still " +
        "the air is down here. If you dropped a feather here it would " +
    ...
    add_exit(THIS_DIR+"cave2b", "south", 0, 1);
    add_exit(THIS_DIR+"cave4b", "east", 0, 1);
...
reset_room()
{
    ...
    if (!objectp(npc2))
    {
        npc2=clone_object(CAVE_DIR + "npc/cave_goblin");
        npc2->arm_me();
