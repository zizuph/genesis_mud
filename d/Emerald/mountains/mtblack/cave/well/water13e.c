...
create_room()
{
    ...
    add_exit(THIS_DIR + "water12e", "southwest", "@@try_hor");
...
reset_room()
{
    if (objectp(octopus))
        return;
    octopus=clone_object(CAVE_DIR + "npc/octopus.c");
    octopus->arm_me();
    octopus->move_living("into an underwater cave", this_object());
...
{
#if 0
    set_alarm( 600.0, 0.0, reset_room);
#endif
}
