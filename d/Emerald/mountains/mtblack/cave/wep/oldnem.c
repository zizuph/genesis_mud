...
public void reset_pen();
public string shimmer_desc();
...
create_weapon()
{
    ...
    set_hit(40); /* This is a very rare sword indeed. Its excellent weapon */
    set_pen(44); /* class is justified by the difficulty of getting it.    */
...
extra_sword_desc()
{
    ...
    {
        return (" The sword seems to shimmer slightly.\n");
    }
}

/*
 * Function name: shimmer_desc
 * Description:   This function adds the add_item vbfc 'shimmer' to the sword.
 * Returns:       description of the shimmering, if seen.
 */
public string
shimmer_desc()
{
    if ( TO->query_pen() >= 45 )
    {
        return ("The whole sword is shimmering with a mysterious "
              + "magical power!\n");
    }
    else
    {
        return ("The sword shimmers slightly with magic.\n");
    }
}
 ...
 *                in an attack. Here we use it to increase the pen of the
 *                weapon, on average once every twenty attacks. 
...
try_hit(object ob)
{
    ...
    a=random(20);                     /* 1 in 20 probability of shimmering */
 
    if (a!=0)                                       /* Not lucky this time */
        return 1;
 
    if (this_object()->query_pen()>=45)
        return 1;                       /* Sword may already be shimmering */
 
    b=random(8);                        /* Random part of wc_pen increase  */
 
    set_pen(47+b);                            /* Raise wc_pen of weapon    */
    
    QW->update_weapon( TO );                  /* Weapon already wielded    */
 
    tell_room(ENV(QW), "The sword wielded by " + QTNAME(QW) + " suddenly " +
        "shimmers with magic!\n", QW);        /* Tell others about shimmer */
                                         
    QW->catch_msg("The sword you wield starts to " +
        "shimmer with magical power!\n");     /* Tell wielder too          */

    a=random(50) + 50;                        /* Time delay for reset      */
    
    set_alarm( itof(a), 0.0, reset_pen);      /* Set alarm for pen reset   */
...
/*
 * Function name: try_hit
 * Description:   This function called to reset the wc_pen of the weapon to 
 *                its original 44.
 */
public void
reset_pen()
{
    set_pen(44);                /* Spoilsport! - wc_pen is reset to normal */
    
    if (QW)                              /* Sword might not be wielded now */
    {
        QW->update_weapon( TO );
        
        tell_room(ENV(QW), "The sword wielded by " + QTNAME(QW) +
            " stops shimmering.\n", QW);
 
        QW->catch_msg("Your sword stops shimmering.\n");
    }
    else
    {
        tell_room( ENV(TO), "The Nemesis sword stops shimmering.\n");
    }
