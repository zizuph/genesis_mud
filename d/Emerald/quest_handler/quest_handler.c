...
static nomask int admin_check(object who)
{
    ...
    // Arches, Emerald wizards designated as admin, Emerald Steward and Liege
    return ((rank >= WIZ_MAGE) ||
            ((rank >= WIZ_NORMAL) &&
            (member_array(who->query_real_name(), admin) != -1)) &&
...
static nomask int
read_check(object who)
{
    return (admin_check(who) || 
           (SECURITY->query_wiz_dom(this_player()->query_real_name()) ==
            domain(this_object())));
...
int _quest_add(string *args)
{
    ...
    if (!write_check(this_interactive()))
    {
...
int _quest_list(string *args)
{
    ...
    if (!read_check(this_interactive()))
    {
        write("You do not have permission to do that.\n");
        return 1;
    }
...
_quest_admin(string *args)
{
    ...
    if (args[0] == "list")
    {
        if (sizeof(admin) > 5)
        {
            ...
            admin_str = implode(map(admin, capitalize), "\n");
            this_player()->more(sprintf("Designated quest administrators:\n" +
                "%-#" + scrwdth + "s", implode(admin, "\n")));
        }
        else if (sizeof(admin) > 0)
        {
            write("Designated quest administrators: " +
                COMPOSITE_WORDS(map(admin, capitalize)) + ".\n");
        }
        else
        {
            write("No designated quest administrators.\n");
        }
    ...
    else if (args[0] == "add")
    {
        ...
        if (!admin_check(this_interactive()))
        {
            write("You do not have permission to do that.\n");
            return 1;
        }
        ...
        if (!SECURITY->exist_player(name) ||
            ...
            (SECURITY->query_wiz_dom(this_player()->query_real_name()) !=
             domain(this_object())))
	{
  	    write("Invalid quest administrator. An administrator must be " +
		  "full wizard rank and " + LANG_ADDART(domain(this_object())) +
                  " domain member.\n");
            return 1;
	}
	...
	{
	    write(capitalize(name) + " is already an administrator.\n"); 
	    return 1;
	}
...
varargs mixed query_quest_names(int active = -1) {
    mixed *info, *keys, *result;
    keys = m_indexes(quest_info);
    result = ({});
    int i = sizeof(keys);
    /* 0=inactive quests, 1=active quests, -1= all quests*/
    switch (active) {
        case 0:
            while (i--) {
                info = quest_info[keys[i]];
                if (info[DEACT]) {
                    result += ({ keys[i] });
                }
            }
...
static void
log_quest_completed_error(object who, string quest_name, string error)
{
    setuid();
    seteuid(getuid());

    write_file(QH_LOG_DIR + "quest_completed_errors",
               sprintf("%-11s %s %s.\n",
...
int quest_completed(object who, string quest_name)
{
    ...
    {
        log_quest_completed_error(who, quest_name, "Quest already completed");
        return 0;
    }
    ...
    if (!pointerp(info = quest_info[quest_name]))
    {
        log_quest_completed_error(who, quest_name, "Quest not registered");
        return 0;
    }
    ...
    if (info[DEACT])
    {
        log_quest_completed_error(who, quest_name, "Quest deactivated");
        return 0;
    }
    ...
    {
        log_quest_completed_error(who, quest_name, "Invalid euid: " + euid);
        return 0;
    }
