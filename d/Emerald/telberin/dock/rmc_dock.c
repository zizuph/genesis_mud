...
create_emerald_room()
{
    ...
    set_em_long("Here, a side pier extends out from the main dock of"
      + " Telberin whose timbers rise out of the lake to the west."
    ...
    add_exit("telberin_dock", "west");
    add_pier_entry(MASTER_OB(this_object()), "Telberin", "Balifor - Poros Circle");    
    initialize_pier();
} /* create_emerald_room */
...
add_dock_stuff()
{
    ...
    add_item( ({ "sign", "signpost", "post", "nearby post" }),
        read_sign);
