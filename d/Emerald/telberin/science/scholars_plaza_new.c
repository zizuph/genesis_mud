...
#define PATH_ROOM ("/d/Emerald/telberin/science/path1")
...
create_telberin_street()
{
    ...
    add_item( ({ "obelisk", "large obelisk", "white obelisk",
      ...
      + " round stones. A map has been painted upon the sides of the"
      + " obelisk in dark gold.\n");

    add_item( ({ "map", "paint", "painting" }), read_file(
    ...
    set_no_exit_msg( ({ "southwest" }),
      ...
      + " work appears to be underway, as there is debris blocking"
      + " any sort of travel.\n");

    add_item(({ "southwest", "Scholars Way", "repair work" }),
        "To the southwest, destruction from recent raids has forced"
      + " Scholars Way to be shut off from transportation. Repair"
      + " work appears to be underway, as there is debris blocking"
      + " any sort of travel.\n");

    add_item(({ "debris","barricade" }),
        "Stone and wood debris has been placed across Scholars Way "+
        "as a makeshift barricade to block travel while repairs "+
        "are completed.  As you study the debris you notice a small "+
        "gap in the barricade among a pile of wooden beams.  "+
        "\n");

    add_item(({ "gap","small gap","gap in the barricade",
                "beams","wooden beams","pile","pile of wooden beams",
                "pile of beams"}),
        "Among a pile of wooden beams is a gap in the barricade which "+
        "appears wide enough to squeeze through.\n");

} /* create_telberin_street */

/*
 * Function name:        do_squeeze
 * Description  :        allow players to bypass the barricade
 * Arguments    :        string arg - what the player typed
 * Returns      :        int 1 - success, 0 - failure
 */
public int
do_squeeze(string arg)
{
    if (!strlen(arg))
    {
        return 0;
    ...
    if (!parse_command(arg, ({}),
        "[through] [the] 'gap' / 'barricade' [in] [the] [barricade]"))
    {
        return 0;
    }

    write("You squeeze through the gap in the barricade.\n");
    say(QCTNAME(this_player())+" squeezes through a gap in the barricade.\n");
    this_player()->move_living("M",PATH_ROOM,1,0);
    return 1;
