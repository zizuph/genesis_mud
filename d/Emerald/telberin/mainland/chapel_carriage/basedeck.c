...
create_deck()
{
    set_short("Inside the Carriage");
    set_long("You are inside the carriage.\n");
}
...
create_room()
{
    ...
    set_deck_sound("The carriage bounces on some rough road.\n");
    set_bump_sound("The driver shouts \"Woah!\" and the carriage "+
	"slams to a halt.\n");
    create_deck();
}
...
surroundings()
{
	...
	return "It seems like you are riding in the middle of a thick, "+
	"completely impenetrable fog, almost like a void.\n";
...
disembark(mixed what)
{
    ...
    if (special_disembark())
    {
        return 1;
    }

    if (!environment(ship)->query_carriage_stop())
    {
	notify_fail("You really feel like smashing into the "+
	    "road at this speed?\n");
	return 0;
    }

    write("You leave the carriage.\n");
    this_player()->move_living("the carriage", environment(ship));
    return 1;
}
