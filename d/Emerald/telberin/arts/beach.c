...
#define BEACH_DIR TELBERIN_DIR + "beach/"
#define LAKESHORE_ROOM BEACH_DIR + "lakeshore_room"
#define LAKESHORE_NPC_DIR BEACH_DIR + "npc/"
...
{
    set_short("a crowded beach");
    set_em_long("Many artists have gathered here along the lakeshore, where painters have " +
    ...
    add_exit("cantors_plaza", "southwest");

    set_no_exit_msg( ({ "south", "west" }),
        "The campus to the Minstrel Guild is walled off in that"
      + " direction. You cannot proceed there.\n");
    set_no_exit_msg( ({ "north", "northeast", "east" }),
        "You wade a bit into the waters of the lake, returning again to"
      + " shore a bit wet.\n");
    set_no_exit_msg( ({ "northwest", "southeast" }),
        "The beach turns to sloping rocks in that direction, making"
      + " travel impossible.\n");
}
