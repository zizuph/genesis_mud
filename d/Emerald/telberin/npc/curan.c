 ...
 *
 * This code is somewhat messy.  I wouldn't suggest it as an example.
 */
...
void create_emerald_monster()
{
    ...
    set_alignment(1000);
    set_stats(({ 240, 260, 220, 140, 150, 250 }));
    ...
    add_prop(PRE_OBJ_MAGIC_RES + MAGIC_I_RES_DEATH, 90);
...
#ifndef NO_WANDER
    set_monster_home(TELBERIN_DIR + "clone_handler");
...
void log(string str)
{
    write_file(TELBERIN_DIR + "log/curan.log", file_name(this_object()) + 
        " (" + time() + "): " + str);
}
...
void souls_and_shadows()
{
    add_cmdsoul(GUILD_SOUL_WARRIOR);
    update_hooks();
}
...
do_something()
{
    ...
    if (sizeof(bad))
    {
        target = OB_NAME(bad[random(sizeof(bad))]);
        return ({
                  "peer " + target,
                  "ktdisgust " + target,
               })[random(2)];
    ...
    target = OB_NAME(who[random(sizeof(who))]);
...
void init_living()
{
        ...
        CAN_SEE_IN_ROOM(this_object()))
    {
        ...
        {
            set_alarm(3.0, 0.0, &command("kill " + OB_NAME(this_player())));
        }
...
void update_specials(object *enemies)
{
  ...
  if (!sizeof(enemies))
  {
    ...
    add_special_attack(summon, 20, "summon");
  ...
  if (!sizeof(undead))
  {
    ...
    add_special_attack(heal, 45, "heal");
    add_special_attack(shout, 10, "shout");
    add_special_attack(glance, 15, "glance");
    add_special_attack(summon, 20, "summon");
...
void reduce_light(int i)
{
  ...
  // Curan tends to leave dark rooms...I'll add this just for safety.
...
special_attack(object enemy)
{
    ...
    /* First we check to see if the room is light.
     * If not, take care of it, and skip regular attacks.
     */
    if (environment()->query_prop(OBJ_I_LIGHT) < 1)
    {
        ...
        set_alarm(120.0, 0.0, &reduce_light(20));
        return 1;
    }
...
/* Special Attack: Heal
 * Effects:        1) Heals 120 + random(200) hps (one in fifteen chance of 500 hps)
...
heal(object enemy)
{
    ...
    heal_hp((random(15) ? 120 + random(200) : 500));
...
glance(object enemy)
{
    ...
    {
        hold->move(who, 1);
        who->catch_tell(query_The_name(who) + " mumbles a prayer and shoots " +
            "a quick glance at you; immediately, you feel your mind and " +
            "body freeze.\n");
        tell_room(environment(), QCTNAME(this_object()) + 
            " mumbles a prayer and shoots a quick glance at " + QTNAME(who) +
            "; immediately " + who->query_pronoun() + " freezes in place.\n",
            ({ this_object(), who }));
    }
...
summon(object ob)
{
    ...
    if (!(ran = MIN(5 - sizeof(guardians -= ({ 0 })), random(4) + 1)))
    {
        return 0;
    }
    ...
    while (ran--)
    {
        ...
        if (sizeof(enemies))
        {
            ...
            // Make the guardian attack all current enemies
            map(enemies - ({ target }), &guardian->attack_object());
            // Finally, switch back to the target
    ...
    tell_room(environment(), QCTNAME(this_object()) + " cries out, " +
        "asking for holy assistance.\n", this_object());
  
    tell_room(environment(), 
        capitalize(
        COMPOSITE_ALL_LIVE(guardians[(sizeof(guardians) - arrived)..])) +
        ((arrived != 1) ? " answers" : "answer") + " his summons.\n",
        this_object());
...
{ 
    tell_room(environment(), QCTPNAME(this_object()) + " aura fades.\n",
        this_object());
    invulnerable = 0; 
}

int
invulnerability(object enemy)
{
...
/* Hps cannot be reduced while Curan is invulnerable */

int
reduce_hit_point(int hps)
{
 ...
 *                   Current, undead enemies who are present - 1900
...
retributive_strike()
{
    ...
    enemies->catch_tell("As " + POS(this_object()) + " lifeless " +
    ...
    damaged->catch_tell("The light hits you with intense power, burning " +
...
do_die(object killer)
{
    ...
    // Remove any KoT items.  We don't want players getting guild objects.  
    deep_inventory()->remove_kot_object();
...
void
kot_notify(object attacker)
{
    if (attacker->query_wiz_level() || wildmatch("*jr", attacker->query_real_name()))
    {
        return;
    }

    /* Notify KoT members of the attack */  
...
remove_poisons()
{
    ...
    poisons = filter(all_inventory(), &operator(==)(POISON_OBJECT) @
...
enter_inv(object ob, object from)
{
    ...
    if (!poison_alarm && IS_POISON_OBJECT(ob))
    {
...
int
query_guild_member_race(string guild)
{
    return (guild == GUILD_NAME);
}

int
query_guild_member_lay(string guild)
{
    return (guild == GUILD_NAME);
}

int
query_guild_member_occ(string guild)
{
    return (guild == GUILD_NAME);
}
