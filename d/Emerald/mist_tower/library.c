...
public string get_new_book_name(string dir);

/*
 * Function name: query_books
 * Description:   Get the names of the approved books in the library
 * Returns:       An array of (string) book titles
 */
public string *
query_books()
{
    return m_indices(book_map);
}

/*
 * Function name: query_book_path
 * Description:   Get the path to the file that holds the text for the
 *                specified book
 * Arguments:     The title of the book to find.
 * Returns:       The filepath.
 */
public string
query_book_path(string book)
{
    return book_map[book];
}

/*
 * Function name: query_approval_books
 * Description:   Get the names of the books in the library awaiting approval
 * Returns:       An array of (string) book titles
 */
public string *
query_approval_books()
{
    return m_indices(appr_map);
}
     
/*
 * Function name: query_approval_book_path
 * Description:   Get the path to the file that holds the text for the
 *                specified book.  This only finds books which have not been
 *                approved yet.
 * Arguments:     The title of the book to find.
 * Returns:       The filepath.
 */
public string
query_approval_book_path(string book)
{
    return appr_map[book];
}

/*
 * Function name: query_book_id
 * Description:   Get the id that is added to all books created by this
 *                library.
 * Returns:       The id
 */
public string
 ...
 * Description:   Set the name which will be used to refer to the library.
 *                Ex. "Library of Genesis", "library", "scroll cabinet"
 * Arguments:     string name - The library name
 */
public void
...
/*
 * Function name: query_library_name
...
{
    write("There are no " + PLURAL_BOOK_TYPE + " available.\n");
    return 1;
}
...
{
    write("There are no " + PLURAL_BOOK_TYPE + " needing approval.\n");
    return 1;
}
...
library_borrow_hook(object book, string title)
{
    ...
    {
	write("That " + BOOK_TYPE + " is currently unavailable.\n");
    }
}
 ...
 *                it is deleted.
 * Arguments:     string title - the title of the book
 *                string filename - the filename of the book file
 */
public void
remove_book(string title, string filename)
{
    ...
    {
        rename(filename, get_new_book_name(removal_dir));
    }
    else
    {
        rm(filename);
 ...
 */
public int
move_book(string book_file, string new_dir)
{
    setuid();
    seteuid(getuid());

    return rename(book_file, get_new_book_name(new_dir));
}
 ...
 */
public void
configure_book(object book, string text_filename, string book_name)
{
    ...
    book->add_prop(OBJ_M_NO_DROP, "You may only \"return\" the " +
        book->short() + " to the " + library_name + ".\n");
 ...
 */
public object
give_book(string text_filename, string book_name)
{
    object book;

    book = make_book(book_name);
    configure_book(book, text_filename, book_name);
 ...
 */
public void
read_book(string text_filename, string book_name, int mread)
{
    setuid();
    seteuid(getuid());
...
library_read(string str)
{
    ...
    read_book(book_file, str, query_verb() == "mread");
...
library_return(string str)
{
    ...
    if (!sizeof(CMDPARSE_ONE_ITEM(str, "return_book", "return_access")))
    {
...
/*
 * Function name: validate_summary
 ...
 */
public int
validate_summary(string summary)
{
...
/*
 * Function name: write_get_summary
 ...
 */
public int
write_get_summary(object who, string title, string input)
{
    if (input == "~q")
    {
        return 0;
    ...
    if (!validate_summary(input))
    {
        input_to(&write_get_summary(this_player(), title));
        return 0;
    }
         ...
         string text, int approval)
{
    ...
    if (!write_file(get_new_book_name(dir),
...
validate_title(string title)
{
    ...
    if (book_map[title] || appr_map[title])
    {
 ...
 */
public int
write_get_title(object who, string input)
{
    if (input == "~q")
    {
        return 0;
    ...
    if (!validate_title(input))
    {
        input_to(&write_get_title(this_player()));
        return 0;
    }

    library_write_prompt_summary_input_hook();
    input_to(&write_get_summary(this_player(), input));
    return 1;
}
 ...
 */
public int
classify_get_shelf(object who, string title, string input)
{
    string book_file; 
    ...
    move_book(book_file, book_dir + input + "/");
...
library_classify(string str)
{
    ...
    input_to(&classify_get_shelf(this_player(), str));
    return 1;
}
...
library_help()
{
    ...
    string str =
        "Commands available for the " + library_name + ":\n" +
        help_cmd("List " + 
            (m_sizeof(book_shelves) ? "[<" + SHELF_TYPE + ">] " : "") +
            "[titles]", "List " + PLURAL_BOOK_TYPE + " in the " +
            library_name + ".");

    if (m_sizeof(book_shelves))
    {
        str += help_cmd("List " + PLURAL_SHELF_TYPE, "List " +
            PLURAL_SHELF_TYPE + " in the " + library_name + ".");
    }
    ...
    if (borrow_required)
    {
        str += 
            help_cmd("Borrow <title>",
                     "Borrow a " + BOOK_TYPE + " for reading.") +
            help_cmd("Return <book>",
                     "Return a " + BOOK_TYPE + " to the " + library_name + ".");
    }
    else
    {
        str += help_cmd("Read <title>", "Read a " + BOOK_TYPE + " in the " +
                        library_name + ".") +
               help_cmd("Mread <title>", "Mread a " + BOOK_TYPE +
                        " in the " + library_name + ".");
    }

    str += help_cmd("Write", "Write a " + BOOK_TYPE +
        " and submit it to the " + library_name + ".");

    str += "\nAdministrator commands available:\n";

    if (strlen(appr_dir))
    {
        str += help_cmd("Approve <title>", "Approve a " + BOOK_TYPE +
            " for inclusion in the " + library_name + ".");

        if (borrow_required)
        {
            str += help_cmd("Borrow approval <title>",
                "Administrators may borrow " + PLURAL_BOOK_TYPE + 
                " which have not been approved by using the \"approval\" " +
                "option.");
        }
        else
        {
            str += help_cmd("(m)read approval <title>",
                "Administrators may read " + PLURAL_BOOK_TYPE + 
                " which have not been approved by using the \"approval\" " +
                "option.");
        }

        str +=
            help_cmd("Deny <title>", "Deny a " + BOOK_TYPE + 
                 " from inclusion in the " + library_name + ".") +
            help_cmd("List approval [titles]",
                 "Administrators may list " + PLURAL_BOOK_TYPE + 
                 " that need approval with the \"approval\" option.");
    }

    str += help_cmd("Discard <title>", "Remove a " + BOOK_TYPE + 
                    " from the " + library_name + ".");

    if (m_sizeof(book_shelves))
    {
        str += help_cmd("Classify <title>",
                        "Assign a " + BOOK_TYPE + " to a " + SHELF_TYPE + ".");
    }
