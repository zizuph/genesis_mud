...
public void
power_up_vamp(object who)
{
    if (!present(who) || who->query_prop(VAMP_I_VS_POWER))
    {
        return;
    }

    who->catch_tell("\nAs if it were emanating from all directions around you, " +
        "a low-pitched voice speaks out to you, its power reverberating " +
        "through your body, \"" + who->query_name() + "....\"\n\nA rush of " +
        "energy passes through you, and the voice speaks again, \"Go..." +
        "with my protection, and fear not the Light of this land....\"\n");
    who->add_prop(VAMP_I_VS_POWER, 1);
}
...
#endif
            set_alarm(0.5, 0.0, &power_up_vamp(ob));
        }
    }
    else if (!clean_alarm)
    {
...
out(string str)
{
    ...
    this_player()->move_living("M", MIST_TOWER_DIR + "tower_start", 1);
