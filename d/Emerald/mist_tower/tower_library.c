...
create_tower_room()
{
    ...
    set_short("A study");
    set_em_long("This floor of the tower seems to be dedicated to the"
    ...
    add_exit("tower_start", "up", library_exit_block);
    add_exit("tower_entrance", "down", library_exit_block);
    add_exit("/d/Emerald/common/guild/vamp/room/plibrary", "north",
        library_exit_block);

    frustum = clone_object(MIST_TOWER_DIR + "obj/frustum");
    frustum->move(this_object(), 1);
    frustum->set_no_show_composite(1);

    cabinet = clone_object(MIST_TOWER_DIR + "obj/scroll_cabinet");
