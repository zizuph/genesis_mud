...
trav_help(string str)
{
    ...
    if (str != "traveller")
        return 0;
    filename="/d/Roke/vladimir/open/thelp";
...
emote_help(string str)
{
    ...
    if (str != "emote")
        return 0;
      filename="/d/Roke/vladimir/open/ehelp";
    if (file_size(filename)>0) {
        this_player()->more(filename);
        return 1;
    }
...
carve_me(string str)
{
 ...
 animal = ({"/d/Roke/varl/atuan/cape/guild/obj/cat",
           "/d/Roke/varl/atuan/cape/guild/obj/dog",
           "/d/Roke/varl/atuan/cape/guild/obj/horse",
           "/d/Roke/varl/atuan/cape/guild/obj/bird",
           "/d/Roke/varl/atuan/cape/guild/obj/pig", })[random(5)];

    wood = present("_trav_wood_", TP);
    if (!wood)
    {
     write("You need some wood.\n");
     return 1;
    }

    weps = TP->query_weapon(-1);
    if (!weps)  {
   write("You need something sharp to carve with.\n");
        return 1;
    }
    for (i=0; i<sizeof(weps); ++i)  {
        if (weps[i]->query_wt() & W_SLASH)
            break;
    }
    if (i == sizeof(weps))  {
        write("Your weapon" + (sizeof(weps)==1 ? " isn't" : "s aren't") +
            " capable of carving the wood!\n");
        return 1;
    }
    animal_obj = clone_object(animal);
    animal_obj->move(TP);
    wood->remove_object();
    write("You slowly but surely carve some of your wood into a "+
        animal_obj->query_name() + ".\n");
   say(QCTNAME(TP)+" begins to carve some wood. Slowly it begins "+
       "to take the shape of a "+ animal_obj->query_name() +".\n");
    return 1;
}
