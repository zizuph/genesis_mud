...
#define GUILD_SAVE ATUAN+"cape/guild/secret"
...
north()
{
    ...
    if (member_array(this_player()->query_name(), cmembers) != -1)
	return 0;
 ...
 * Functions:	Add one guild member to the council
 *			The guild member must be logged in.
 * Arguments:	player - the player object of the guild member
 * Returns:	0 - failure, 1 - success
 */
int
add_council_member(object player)
{
    restore_object(GUILD_SAVE);

    if (!objectp(player) || !interactive(player) ||
        player->query_guild_name_race() != GUILD_NAME)
        return 0;

   cmembers += ({ player->query_name() });
...
remove_council_member(string name)
{
    ...
    if ((i = member_array(name, cmembers)) == -1)
        return 0;
...
create_room()
{
 ...
 add_exit("/d/Roke/varl/atuan/cape/guild/councilrm","north","@@north");
...
my_leave(str)
{
    ...
    if (TP->remove_guild_race())
    {
       ...
       insignia = present("stick", TP);
        if (insignia)
        {
            insignia->remove_object();
    write("You are stripped of Traveller status, your walking "+
          "stick is taken from you and destroyed.\n");
