...
create_room()
{
   ...
   set_long(
      ...
      + "to it's name! There are four crudely built tables scattered\n"
   ...
   add_food( ({ "rat", "rats" }),
      "rat", "fried", 85, 80, 0, 0,
      "It smells and looks discusting.\n", 0);
...
reset_room()
{
   ...
   if(!orc3)
      {
      orc3 = clone_object("/d/Genesis/start/human/obj/orc");
      orc3->move_living("xxx", TO);
   }
   if(!borc)
      {
      borc = clone_object(ORC+"monsters/borc");
      borc->move_living("xxx", TO);
   }
   if(!borc2)
      {
      borc2 = clone_object(ORC+"monsters/borc2");
