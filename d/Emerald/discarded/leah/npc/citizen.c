...
create_emerald_monster()
{
   ...
   set_race_name("elf");
   set_long("This elf walks blissfully around the city of Leah, enjoying "+
	"the beautiful surroundings. The inhabitants are naturally happy, "+
	"and all seem to have a special emotional weakness to lovely "+
	"weddings.\n");
   ...
   set_gender(random(2));
   set_act_time(6);
   add_act("emote listens towards the chapel, wondering if a wedding will start soon.");
   add_act("smile hap");
   set_chat_time(8);
   add_chat("Didn't I see you behind the bushes with that one goblin?");
   add_chat("Oh I love the sound of the chapel bells!");
   set_restrain_path( ({ LEAH_DIR }) );
   set_random_move(5);
   set_whimpy(40);
