...
create_room()
{
   ...
   add_cmd_item("sign", "read", "Reminder to Brides and Grooms: You can " +
...
{
   if (str != "help" && str != "for help" && str != "jeweler"+
    " for help" && str != "the jeweler for help")
    {
   ...
   write("Now as this "+
      ...
      "To purchase your wedding set use <buy (type) rings>\n\n");
...
buy(string arg)
{
   notify_fail("Use <buy (type) rings> to buy a wedding set.\n");
   if (!arg)
      return 0;
...
get_master(string arg)
{
   ...
   if (bobj->query_prop(PLAYER_I_IS_GETTING_MARRIED))
   {
      write(capitalize(bride) + " is already engaged to be married!\n");
      return;
   }

   if (gobj->query_prop(PLAYER_I_IS_GETTING_MARRIED))
   {
   ...
   if (wand->move(mobj))
      wand->move(mobj, 1);
   bobj->add_prop(PLAYER_I_IS_GETTING_MARRIED, 1);
   gobj->add_prop(PLAYER_I_IS_GETTING_MARRIED, 1);
