...
{
   if (ob->for_groom() && this_player()->query_prop(IS_GROOM))
      return 1;
   if (ob->for_bride() && this_player()->query_prop(IS_BRIDE))
      return 1;
   if (ob->for_groom())
      NF("You have to be a groom to buy that.\n");
   else
      NF("You have to be a bride to buy that.\n");
   return 0;
}
