...
dis_check()
{
	...
	say(QCTNAME(this_player()) + 
            " attempts to step through the gate, but as "+
	    "soon as "+this_player()->query_pronoun()+ " gets halfway through, "+
	    this_player()->query_pronoun()+" recoils in absolute fear!!\n");
	return 1;
    }
    else
    {
...
create_room()
{
    ...
    add_item( ({"overcast", "mist", "fog", "droplets", "water droplets"}),
	     ...
	     "Peering through the fog, you notice the shadows of "+
	     "an ancient building and a few trees.\n");
    ...
    add_item("shadows",
	     ...
	     "of the trees? The building?\n");

    add_item( ({"structure", "building", "building shadows", 
             "ancient building" }),
	     "As you peer through the dark veil of fog covering the road "+
	     "you notice an immense structure just to the southeast.\n");
    ...
    add_exit("coast8", "south", dis_check, 4);
    add_exit("coast6", "north", 0, 2);
}
