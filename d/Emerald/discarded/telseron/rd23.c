...
create_room()
{
    ...
    set_long("   A boulevard in the southern part of the city. " +
        ...
        "about their business. It is wide and well used. On either " +
        "side of the road are two story stone buildings. The one " +
        "on the west has bars on the windows, as if to protect " +
        "whatever may be inside. The one on the east is looks to " +
        "be able to withstand an assault, too. You see signs on " +
        "both buildings. Looking north is a park while south of " +
        "here, a castle twinkles in the sunlight. The road running " +
        "north and south here is Amaryllis street.\n\n");
    add_item("trees",
    ...
    add_cmd_item("west sign", "read", "@@read_w_sign@@");
    add_cmd_item("east sign", "read", "@@read_e_sign@@");
     add_cmd_item("first sign", "read", "@@read_w_sign@@");
    add_cmd_item("second sign", "read", "@@read_e_sign@@");
                                          
    add_exit(TOWNSHIP_DIR + "pk07", "north");
    add_exit(TOWNSHIP_DIR + "rd29", "south");
    add_exit(TOWNSHIP_DIR + "shop", "east");
    add_exit(TOWNSHIP_DIR + "bank", "west");
}                    
...
read_w_sign()
{
    return "Hanging above the west door, you read the sign, it says:\n" +
        "'Telseron Bank'.\n";
}
...
read_e_sign()
{
    return "On the sign, hanging above the door east, you see painted " +
        "in bright letters:\n" + 
        "'Telseron Store'.\n";
}
