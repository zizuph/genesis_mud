...
create_emerald_monster()
{
   ...
   set_race_name("elf");
   set_gender(random(2));
   set_long("This elf is one of the many servants of the castle. "+
	capitalize(HE_SHE(TO)) + " is constantly running around "+
	"taking care of the daily needs of all in the castle. Their "+
	"ancestory is not of noble blood, yet their positions in the "+
	"castle leaves the common citizen of Telseron with respect "+
	"and even envy of their favoured status.\n");
   ...
   set_random_move(5);
   MONEY_MAKE_CC(10 + random(11))->move(TO, 1);
   set_whimpy(50);
...
mixed my_equip()
{
   switch (query_gender())
	...
	case 0:
	  {
	    return ({ TELSERON_DIR + "arm/serv_tunic" });
	    break;
	  }
	case 1:
	  {
	    return ({ TELSERON_DIR + "arm/serv_dress" });
	    break;
	  }
     }
}
