...
void create_room()
{
    ...
    add_exit( THIS_DIR + "bridge4", "east");
    add_exit( THIS_DIR + "bridge2", "west");
...
{
    if (from() == "east")
        return "   The island dominates your view. Many details " +
            "can be made out now. Beautiful houses can be seen " +
            "lining the avenues. Trees fill the island. Towers " +
            "rise from the island with pennants fluttering " +
            "from their tops. An arch can be seen standing " +
            "before the city. Birds fly about the island and " +
            "around the lake. Boats bounce across the lake.\n\n";
 
 
    if (from() == "west")
        return "   The lake fills your view. It seems to " +
            "stretch out forever. Boats float by as elves fish " +
            "the water for food. Birds fly past occaisonally. " +
            "Waves lap at the bridge and occasionally, a wave " +
            "crashes into the side, spraying you with water.\n\n";
