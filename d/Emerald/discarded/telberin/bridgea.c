...
void create_room()
{
    ...
    add_exit( THIS_DIR   + "bridgeb", "east");
    add_exit( LROAD + "xroada", "west");
}
...
{
   if (from() == "east")
      return "   As you travel east, a large island looms " +
        "into view. It grows in size as you approach it. Little " +
        "specks can be seen floating on the water, probably boats. " +
        "Birds bob on.\n\n";
 
   if (from() == "west")
      return "   The crossroad can be seen from here. It " +
        "stands out as the road passes by it. The trees surround " +
        "the area like a green frame. They line the shore and " +
        "prevent a clear view of the road.\n\n";
