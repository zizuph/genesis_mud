/* Giant guard in Tantallon */
/* Modified to be a smaller version of the Neraka hill giants 25 Oct 2015 */
...
string *gAdj1 = ({ "tall", "bony", "lanky", "skeletal",
            "elderly", "lean" }),
       *gAdj2 = ({ "grumpy", "drunken", "hungover", "sullen",
            "evil", "hungry" });
...
create_creature()
{
    ...
    set_long("A huge humanoid creature, a hill giant is not very bright " +
        ...
        "While not the finest specimen of its race, this " +
        "killing machine has joined the @@query_Color@@ " +
        "Dragonarmy.\n");
    ...
    set_stats(({130,50,110,40+random(20),40+random(20),80+random(30)}));
    set_hp(query_max_hp());
    ...
    set_hitloc_unarmed(1, 15, 17, "left thigh");
    set_hitloc_unarmed(2, 15, 17, "right thigh");
    set_hitloc_unarmed(3, 25, 20, "left shin");
    set_hitloc_unarmed(4, 25, 20, "right shin");
    set_hitloc_unarmed(5, 15,  6, "abdomen");
    set_hitloc_unarmed(6, 25, 15, "left foot");
    set_hitloc_unarmed(7, 25, 15, "right foot");

    set_attack_unarmed(1, 15, 30, W_BLUDGEON, 20, "left foot");
    set_attack_unarmed(2, 15, 30, W_BLUDGEON, 30, "right foot");
    set_attack_unarmed(3, 25, 30, W_BLUDGEON, 20, "left fist");
    set_attack_unarmed(4, 25, 30, W_BLUDGEON, 30, "right fist");
...
special_attack(object enemy)
{
    ...
    if (attacktype == 0)
    {
	    hitres = enemy->hit_me(250/2, W_BLUDGEON, TO, -1);
...
/*
    ...
    clone_object(UNIQUE(ARM2A,5,ARM2B))->move(this_object(),1);
    command("wear all");
*/    
}
