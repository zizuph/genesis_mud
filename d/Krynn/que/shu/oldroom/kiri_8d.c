
#include "../local.h"
inherit ATTACKROOM;
#include "/d/Krynn/common/random_meet.h"

object monster;

void
create_attack_room()
{
    set_short("On a grass path in Que Kiri");
    set_long(BS("You are standing in the middle of a grass path"
      + " within the Que Kiri village. Scattered around you are the"
      + " tents and huts of the Que Kiri people. A stray dog takes"
      + " to liking you and starts to follow in your steps. That is,"
      + " until it spots an unguarded roasting pit and slinks away"
      + " hoping to steal a few juicy morsels.", 70)); 

    OUTSIDE;

    add_exit(ROOM + "kiri_7d","north");
    add_exit(ROOM + "kiri_9c","southwest");

}

int
hook_change_room(int which)
{
    if (which == 0)
      {
	  if (monster)
	    return 0;
	  monster = RANDOM_MEET(TABLE);
	  monster->move_living("xx",TO);
	  return 1;
      }
    if (which == 1)
      if (monster)
	return 0;
    return 1;
}

