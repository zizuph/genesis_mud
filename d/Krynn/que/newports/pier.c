...
create_newport_room()
{
    ...
    set_long("You are on a pier in the village Newports. Despite the desolate " +
         ...
         "overhead. A wooden sign has been affixed to the pier, and another " +
         "pier extends to the northeast.\n");
    ...
    add_item("sign","You can read the words on it.\n");
    add_cmd_item(({"sign","words"}),"read","You read: 'Ship to Palanthas.'\n");
    ...
    (SHIPLINE_OTHER_PIER)->load_me();

    /* Warfare: This is a pier, we do not want guards here! */
    set_num_npc(0);

    add_exit(TDIR + "village","west");
    add_exit(TDIR + "pier4","northeast");
}
