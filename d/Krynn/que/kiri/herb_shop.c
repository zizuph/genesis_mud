...
#define STDHERBFILE 	"/std/herb"
#define HERBJAR         "/d/Gondor/elessar/obj/herbjar"
#define HERBPOUCH       "/d/Gondor/elessar/obj/herbpouch"
...
read_sign()
{
   return "\t _________________________________________\n" +
          ...
          "\t| From our large stock, we also sell jars |\n" +
          "\t|  and pouches, but we do not buy them!   |\n" +
...
{
    if (function_exists("create_object", ob) != STDHERBFILE)
        return 0;
...
{
    return (function_exists("create_object", ob) == STDHERBFILE);
}
...
do_list(string str)
{
    ...
    for (i = 0; i < sizeof(herb_files); i++)
    {
        ...
        price = query_buy_price(herb = find_object(herb_files[i]));
...
do_buy(string str)
{
    ...
    herb_files = filter(m_indexes(herbs), "filter_herb_name", TO, str1);

    for (i = 0; i < sizeof(herb_files); i++)
        item += ({ clone_object(herb_files[i]) });
    ...
    item -= bought_item;
    // remove herbs that were no bought
    if (sizeof(item))
        item->remove_object();
