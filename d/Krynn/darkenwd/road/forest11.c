...
create_darkenwd_room()
{
    ...
    set_long_desc("This forest is quite dense, with a solid undergrowth " +
                  ...
                  "is small here and only leads to the Solace-Haven road "+
                  "in the north and east directions.");
    AI("forest", "The forest is omnipresent, except to the east and north "+ 
        "where it leads to the Solace-Haven road.\n");
    ...
    AI(({"ground","leaf litter","litter","wood","decaying wood"}),"It may "+
        ...
        "adventurer.\n");
    AI("bird","The bird is most likely a blue jay.\n");

    add_exit(TDIR + "road8", "north",0,4,1);
    add_exit(TDIR + "road7", "east",0,4,1);

    set_tell_time(250);
    add_tell("A far off rustling sound can be heard.\n");
    add_tell("A bird swoops from one tree to another.\n");

    seteuid(getuid(TO));
    set_up_herbs( ({ ONE_OF(herbs), ONE_OF(herbs), ONE_OF(herbs),
        ONE_OF(herbs) }), ({"forest","undergrowth","herbs"}), 8);
