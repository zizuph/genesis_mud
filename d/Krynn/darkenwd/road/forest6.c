...
create_darkenwd_room()
{
    ...
    set_long_desc("This forest is quite dense, with a solid undergrowth " +
                  ...
                  "continues to the northwest, or leads back "+
    ...
    AI(({"organic","organic matter","decaying matter",
    ...
    add_exit(TDIR + "forest7", "northwest",0,4,1);
    add_exit(TDIR + "road2", "northeast",0,4,1);

    set_tell_time(120);
    add_tell("A rustling sound is made nearby.\n");
    add_tell("A vulture circles the sky above.\n");

    seteuid(getuid(TO));
    set_up_herbs(({ONE_OF(herbs),ONE_OF(herbs),ONE_OF(herbs),
        ONE_OF(herbs) }), ({"forest","undergrowth","herbs",}), 6);
