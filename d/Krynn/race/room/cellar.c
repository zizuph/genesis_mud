...
do_join(string region, string fname, int silent = 0)
{
    ...
    if (!silent)
    {
        ...
        set_alarm(12.0,0.0, &write("You feel a terribly pain as two large horns pierce the "+
                                  "skin on your head.\n"));
        set_alarm(15.0,0.0, &write("You're body is covered with rough fur.\n"));
        set_alarm(18.0,0.0, &write("You collaps on the ground.\n"));
...
deny(string str, int silent = 0)
{
	...
	say("When you open your eyes again you see that "+QCTNAME(TP) +
            " has been transformed into a normal human again!\n");
    }
    return 1;
}
