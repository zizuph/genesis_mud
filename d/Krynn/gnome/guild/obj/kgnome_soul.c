...
void
give_info(string str, object player, int ti)
{
    if (ti < 43)
...
void
end(object ob, object player, string str, int ti)
{
    object heap;
    ...
    heap->move(E(player));
    tell_room(E(player), QCTNAME(player) + " drops a heap of parts.\n", player);
...
analyze(string str)
{
    ...
    write("You sit down and start to take apart the " + ob->short()
      + " with your gizmo.\n");
    say(QCTNAME(TP) + " seats " + HIM(TP) + "self on the ground "
      + "and starts to take apart " + HIS(TP) + " " + ob->short() + ".\n");

    success = (40 * ((TP->query_skill(SS_BUILD)+10) * (60+random(50))) /100) * ((TP->query_stat(3))/5+(TP->query_stat(4)*2/5)+40) /10000;
	...
	for (i=0; i<anzahl; i++)
	{
	    ...
	    {
		ti = 42+(i*3);
		set_alarm(40.0+itof(i*3), -1.0, &give_info(info[2*i], TP, ti));
	    }
	}
    }
    else
	...
	for (i=anzahl-1; i>-1; i--)
	{
		...
		set_alarm(40.0+itof(anzahl*3)-itof(i*3), -1.0, 
					    &give_info(info[2*i], TP, ti));
	    }
	}
    }
    if (ti < 43)
	set_alarm(itof(ti) , -1.0, &fail(TP));
    else
	paralyze_player(TP, "You are too absorbed with taking apart "
	  + "your " + ob->short() + ".\n", "", ti);
    set_alarm(itof(ti), -1.0, &end(ob, TP, str, ti));
    return 1;
}
