...
create_room()
{
    ...
    add_exit(GGUILD + "planner", "north", "@@fworker_check");
    add_exit(GGUILD + "head_tin", "west", "@@head_tin_check");
    reset_room();
}
...
reset_room()
{
    ...
    seteuid(getuid());
    gender = call_other(GGUILD + "gmaster", "query_headtin_gender");
    if(!gender)
	title = GNOME_TITLES[14];
    else
	title = GNOME_TITLES_F[14];
    name = call_other(GGUILD + "gmaster", "query_headtin_name");
...
head_tin_check()
{
    ...
    /* This is bug prone, replace with real stuff in future */
    if (TP->query_wiz_level() || 
	(stringp(title) &&
	    (title[0..20] == "Assistant Head Tinker" ||
		title == "Head Tinker" ||
                title == "Head Tinkeress")))
    {
...
check_it(string str)
{
    ...
    write("You have a brief look at the folder titled " + C(str) + " and read the "
	+ "following:\n\n");
