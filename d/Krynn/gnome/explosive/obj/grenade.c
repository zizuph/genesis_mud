...
create_object()
{
    ...
    add_prop(OBJ_M_NO_SELL, "No one would risk buying such a device!\n");
    add_prop(OBJ_M_NO_DROP, "No, it's too dangerous to do that!\n");
}
...
do_throw(string str)
{
    ...
    if (!CAN_SEE_IN_ROOM(TP) || !CAN_SEE(TP,target)) {
    	NF("You can't see to throw!\n");
    	return 0;
    }
	...
	{
		NF("You are too stunned to do that!\n");
		return 0;
	}
	
	enemyname = target->query_the_name(TP);
	
	if ((tmp = environment(TP)->query_prop(ROOM_M_NO_ATTACK)) ||
		(tmp = target->query_prop(OBJ_M_NO_ATTACK)))
	{
		if (stringp(tmp))
		{
			NF(tmp);
		...
		{
			NF("You feel a divine force preventing you from that.\n");
		}
		return 0;
	}
	
	if (target->query_not_attack_me(TP, -1))
	{
		/*query_not_attack_me prints a message */
		return 0;
	}
	
	/**Do DIS checks for new attack **/
	if (member_array(target, TP->query_enemy(-1)) == -1)
	{
		/** DIS **/
		if (!F_DARE_ATTACK(TP, target))
		{
			NF("You don't dare to attack "+enemyname+"!\n");
			return 0;
		}
		...
		if ((!TP->query_npc()) &&
			(TP->query_met(target)) &&
			(TP->query_prop(LIVE_O_LAST_KILL) != target))
		{
			TP->add_prop(LIVE_O_LAST_KILL, target);
			NF("Attack "+enemyname+" ?!?  Please confirm by trying again.\n");
			return 0;
		}
	}
