...
create_room()
{
    set_short("Second floor guard room");
...
climb()
{
    if (random(50) < TP->query_skill(SS_CLIMB))
    {
        ...
        TP->move_living("climbing the pole", TDIR + "guard3");
        return 1;
    }
