...
create_room()
{
	...
	set_long(BS(
		"There is dust and smoke all over the place. To the south "
		+ "an entrance opens into a larger cave. The origin of the "
		+ "smoke seems to be there. The walls and ceiling are "
		+ "covered with cracks. This tunnel might cave in any "
		+ "minute. You see something on the ground, gleaming "
		+ "through the smoke.", 70));
	
	add_exit(TDIR + "cave1", "north", 0, 1);
	add_exit(TDIR + "explosroom", "south", 0, 1);
	add_exit(TDIR + "corridor4", "east", 0, 1);

	add_item(({"tracks", "iron tracks"}),
		"They run from the east to the south.\n");
	add_item("dust", "You are COVERED with it!\n");
	add_item("ground", BS(
		"You cannot see it well because it is covered by smoke. But "
		+ "there seem to be iron tracks on the ground.", 70));
	add_item(({"ceiling","walls","cracks"}), BS(
		"There are cracks in the walls and ceiling "
		+ "all over this place. You do not feel safe at all"
		+ " in here.", 70));
	add_item(({"smoke", "clouds"}), BS(
		"This is the sort of smoke you do not like. "
...
{
	tell_room(TO,
		"Now also the very ground you are standing on is shaking "
...
{
	tell_room(TO,
		"Suddenly there's a terrifying explosion in the room to "
...
string wayout() {return "east north south";}
