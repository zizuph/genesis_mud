...
create_room()
{
    ...
    set_long(BS("You are in the middle of a long tunnel leading from" +
	" the first floor "+
    ...
    add_item(({"mark", "marks"}), "Long black marks, radiating from further" +
	" to the west...  The air seems cool,\nso the blasts could not have" +
	" been too recent.\n");
    INSIDE;
    LIGHT;
    add_exit(TDIR + "cavern14", "west", 0);
    add_exit(TDIR + "cavern12", "east", 0);
}
