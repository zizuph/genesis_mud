...
/* This is a base file for warfare npcs of Throtyl.
 *
 * Arman Sept 2018.  
 */
    ...
    switch (level) 
    {
        default:
        {
            switch(sublevel)
            {
                ...
                case 1:
                    str = "This is " +LANG_ADDART(gAdj[level])+ 
                        " hobgoblin looter, a larger version of their goblin " +
                ...
                case 3:
                    str = "This is " +LANG_ADDART(gAdj[level])+ 
                        " ogre marauder, a massive brute that has come down from " +
                        "the Throtyl forest to raid the vulnerable border towns " +
                        "of Solamnia. Slow, but incredibly strong, this is one " +
...
arm_me()
{
    ...
    string wep1, wep2, body, helm, greaves, gauntlets, cloak, shield;
    int hand_type;
    int wielding_bow = 0;
    ...
    equip(ARM_PATH + "std_warfare_armour", 1, &init_warfare_item(, gLevel, A_TORSO));
    equip(ARM_PATH + "std_warfare_helm", 1, &init_warfare_item(, gLevel, A_HEAD));
    equip(ARM_PATH + "std_warfare_grv_gnt", 1, &init_warfare_item(, gLevel, A_ARMS));
    equip(ARM_PATH + "std_warfare_grv_gnt", 1, &init_warfare_item(, gLevel, A_LEGS));
...
update_throtyl_soldier()
{
    ...
    set_pick_up_team(({ "_throtyl_warfare_npc_" }));
