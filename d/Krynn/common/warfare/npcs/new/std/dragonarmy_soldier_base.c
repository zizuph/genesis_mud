...
/* This is a base file for warfare dragonarmy npcs.
 *
 ...
 * {
 *     set_dragonarmy_level(level, sublevel, colour);
 * }
 *
 *   Where level is a number range of 0-5, dictating the 
 *   mortal size these mobs are designed for.
 *
 *   Level 0 to 2 - rising hero to titan level npcs 
 *                 ("scarred", "experienced", "veteran")
 *   Level 3 to 5 - champion to myth level npcs
 *                 ("fearsome", "deadly", "elite")
 *
 *   Where sublevel is a number range of 0-2 dictating
 *   the mob type within set_dragonarmy_level:
 *
 *   Level 0 to 2 (sub-level 0-2):
 *      0. <scarred / experienced> dragonarmy scout [light armour melee]
 *      1. <scarred/experienced/veteran> dragonarmy mercenary 
 *         [medium armour melee]
 *      2. <experienced/veteran> dragonarmy soldier [heavy armour melee]
 *
 *   Level 3 to 5 (sub-level 0-2):
 *      0. <fearsome / deadly / elite > dragonarmy soldier 
 *         [heavy armour melee]
 *      1. <fearsome / deadly / elite > dragonarmy officer 
 *         [masterwork armour melee w/dragon special]
 *         *officer spawns with 2+ other draconian/soldier mobs
 *      2. <fearsome / deadly / elite > dragonarmy high officer 
 *         [masterwork armour melee w/dragon special]
 *         *officer spawns with 2+ other draconian/soldier mobs
 *
 * Arman May 2018.  
...
public string   set_dragonarmy_description(int level, int sublevel,
                    string colour);
 ...
 *
 *              : int sublev     0 - scout / soldier
 *                               1 - mercenary / officer
 *                               2 - soldier / high officer
 *
 */
varargs void
...
{
    string dragonarmy_desc = set_dragonarmy_description(gLevel, 
        gSublevel, gColour);
    ...
    {  
       ...
       default: 
          if(level > 2)
              name_desc = ({ "high officer", "officer" });
          else
              name_desc = ({ "soldier" });
          break;
    }
    ...
    add_name(name_desc);
    add_adj(colour);
    set_short(gAdj[gLevel] + " dragonarmy " + name_desc[0]); 
    set_long(dragonarmy_desc);
...
/* Function name: set_dragonarmy_description
 * Description  : Sets the long description of the dragonarmy, depending 
 *                on his level.
 * Arguments    : int level        - range 0-5
 *              : int sublevel     - range 0-2.
 */
public string
set_dragonarmy_description(int level, int sublevel, string colour)
{
    string str = "";
            ...
            switch(sublevel)
            {
                case 0:
                    str = "This is " +LANG_ADDART(gAdj[level])+ 
                        " scout of the " +
                        colour+ " dragonarmy. Lightly armed and " +
                        "armoured, scouts like this one are commonly " +
                        "used as the vanguard of an advancing army.\n"; 
                break;
                case 1:
                    str = "This is " +LANG_ADDART(gAdj[level])+ 
                        " mercenary of the " +
                        colour+ " dragonarmy, lured to the battlefields " +
                        "by the spoils of war.\n"; 
                break;
                default:
                    str = "This is " +LANG_ADDART(gAdj[level])+ 
                        " soldier of the " +colour+ " dragonarmy.\n"; 
                break;
            }
            break;
        }
        default:
        {
            switch(sublevel)
            {
                ...
                case 1:
                    str = "This is " +LANG_ADDART(gAdj[level])+ 
                        " officer of the " +
                        colour+ " dragonarmy, a soldier who has worked " +
                        "his way up the ranks through success on the " +
                        "battlefield.\n"; 
                break;
                default:
                    str = "This is " +LANG_ADDART(gAdj[level])+ 
                        " high officer " +
                        "of the " +colour+ " dragonarmy. An opponent " +
                        "not to be taken lightly, this warrior has " +
                        "worked his way to the top of the hierarchy " +
                        "through skill, merciless cunning, and " +
                        "brutal success in the arena of war.\n"; 
                break;
            }
            break;
        }
...
public void
update_dragonarmy_soldier()
{
    string dragonarmy_adj = gAdj[gLevel];

    set_pick_up_team(({"_" +gColour+ "_da_member_", 
        "warfare_" +gColour+ "_dragon" }));
    set_max_team_size_auto_join(8);
        
    add_name("_" +gColour+ "_da_member_");
    set_race_name("human");    
    set_adj("dragonarmy");
    add_adj(dragonarmy_adj);    
    ...
    set_act_time(25);
    add_act("dcheck weapons");
    add_act("dcheer");
    add_act("dconvince all");
    add_act("dhalt");
    add_act("dlook");
    add_act("dnod");
    add_act("dpace");
    add_act("dponder");
    add_act("dshield");
    add_act("dshine armour");

    set_cact_time(25);
    add_cact("assist");
    add_cact("assist");
    add_cact("emote snarls in fury.");
    add_cact("say For the " +capitalize(gColour)+ " Dragonarmy!");
    add_cact("dglare");
    add_cact("dpraise");
    add_cact("drage");
    add_cact("dsmile");
...
hook_friend_killed(object friend, object killer)
{
    ...
    // summon dragon if able to
    command("dfear");    
    command("kill " + OB_NAME(killer));
}

/* Function name: special_attack
 * Description  : The code of a dragonarmy special attacks.
 */
int
special_attack(object enemy)
{
    ...
    if (TO->query_dragonarmy_promotions() > 1 &&
...
{
    string npc_army = capitalize(gColour) + " Dragon Army";

    if(WAR_CLUB_ADMIN->query_warfare_dragonarmy(attacker))
    {
        if(WAR_CLUB_ADMIN->query_warfare_army(attacker) == npc_army)
