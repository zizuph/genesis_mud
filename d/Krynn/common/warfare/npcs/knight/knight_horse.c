...
//Navarre March 2006, Added random description,
//added extra parry and extra speed to rider (for npcs)
//used for riding knight with cavalry special.

//Navarre: April 19th, Fixed bug with horse trying to make the
//         knight rescue if the knight was not present.
...
long_desc()
{
    string str = ", the type favoured by the Knights of Solamnia. "+
                 "This type of war-horse weigh about twice as much "+
                 "as regular horses. They have been bred in Solamnian "+
                 "with the only purpose to become a great war-horse. What "+
                 "makes a horse a great war-horse is its additional weight, "+
                 "giving the knight's lance a devastating impact in combat. "+
                 "One would never suspect that these horses with such mild manners "+
                 "would indeed be such a potent weapon.\n";
...
create_steed()
{
    ...
    set_adj(({"tall", "white"}));
    set_gender(2);
    set_short(ONE_OF(horse_adjs)+" "+ONE_OF(horse_cols)+" warhorse");
    ...
    set_rider_object_file(WARFARE_KNIGHT_PATH+"horse_rider_object");
...
void save_me()
{
    if(knightRider && present(knightRider, environment(this_object())))
    {
      knightRider->command("maneuver");
      knightRider->command("charge");
    }
}
...
{
    set_alarm(15.0, 0.0, &delete_warfare_warhorse());
}
...
mount_hook(object rider)
{
    ...
    /* Just a precaution as to not give Knight players an advantage for
