...
/* Lavellan looked into a runtime and suggested we change the charge commands
   to use an alarm. This has hereby been done. Navarre, September 2010. */

/* Bug with set_adj 2011 06 25, line 250 missed adj2 in its set_adj, Lunatari */
...
hook_friend_attacked(object friend, object enemy)
{
    ...
    set_alarm(0.0, 0.0, &command("charge"));
}
...
attack_func(object enemy)
{
    if ((enemy->query_race_name() == "goblin" ||
        enemy->query_race_name() == "hobgoblin") &&
        !random(3))
    {
