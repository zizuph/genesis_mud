 ...
 * 
 *   Navarre, March 2006 - Fixed messages when horse dies in the middle of an attack.
 *                         Few other tweaks to improve it.
 */
...
maneuver(string str)
{
    ...
    {
        NF("Your " + st->query_short() + " is not under attack, "+
          "there is no need.\n");
        return 0;
    }
    ...
    st->add_prop(OBJ_M_NO_ATTACK, "As you move to attack the "+
      st->query_short() + ", " + "the "+me->query_nonmet_name() + " whirls the steed "+
      "around to face you.\n");
    me->catch_tell("Recalling your training of mounted combat, you "+
      "spur your " + st->query_short() + " forward and whirl " +
    ...
    tell_room(E(me), QCTNAME(me) + " spurs " + POSSESSIVE(me) + " " +
      st->query_short() + " forward and whirls around, facing " +
    ...
    while( i-- )
    {
        ...
        enemies[i]->catch_tell(me->query_The_name(enemies[i]) +
          " spurs " + POSSESSIVE(me) + " " + st->query_short() +
...
do_charge_special(object ob, object liv, object horse)
{
    ...
    if (!P(ob, E(liv)) || liv->query_prop(LIVE_O_STEED) != horse || !present(horse, environment(liv)))
    {
    ...
    if (ob->query_hp() < 1)
    {
        ...
        if (weapon->query_wt() == W_AXE || weapon->query_wt() == W_SWORD)
        {
            liv->catch_tell("The thundering crescendo of your "+
              horse->query_short() +" reaches its peak as you collide "+
            ...
            ob->catch_tell("The thundering crescendo of "+
              liv->query_the_name(ob) +"'s "+ horse->query_short()+
            ...
            liv->tell_watcher("The thundering crescendo of "+
              QTNAME(liv) +"'s "+ horse->query_short() +" reaches its "+
    ...
    case 0:
        ...
        " with your "+ rcry +" but miss. You tug on the "+
...
{
    if (!P(ob, E(me)) || me->query_prop(LIVE_O_STEED) != horse || !present(horse, environment(me)))
    {
    ...
    me->catch_tell("You lean forward in the saddle as your "+
      horse->query_short()+ " thunders toward "+
    ...
    ob->catch_tell("The ground beneath your feet rumbles as "+
      ...
      POSSESSIVE(me) +" "+ horse->query_short() +"! You feel fear "+
    ...
    me->tell_watcher("The ground rumbles beneath your feet as "+
      ...
      POSSESSIVE(me) +" "+ horse->query_short() +". Leaning forward "+
...
do_charge_prespecial1(object ob, object me, object horse)
{
    ...
    me->catch_tell("Your adrenaline begins to surge as you slap "+
      "the reins back and forth as your "+ horse->query_short()+
      " gains momentum.\n");

    ob->catch_tell(me->query_The_name(ob) +" slaps the reins "+
      "back and forth as "+ POSSESSIVE(me) +" "+ horse->query_short()+
      " gains momentum.\n");

    me->tell_watcher(QCTNAME(me)+" slaps the reins back and forth as "+
      POSSESSIVE(me)+" "+horse->query_short()+" gains momentum.\n",
      ({ ob, me }));
}
...
do_charge_engaged_prespecial1(object ob, object me, object horse)
{
    ...
    if (tmp && sizeof(enemies) == 1)
    {
        me->catch_tell("You turn back to face the battle, noticing "+
          ...
          "your "+ horse->query_short() +", sending the great steed "+
    ...
    else
    {
        me->catch_tell("You turn back to face the battle, pausing "+
          ...
          horse->query_short() +", sending the great steed charging "+
    ...
    ob->catch_tell(me->query_The_name(ob) +" turns back to face "+
      ...
      POSSESSIVE(me) +" heels into the "+ horse->query_short()+
    ...
    tell_room(E(me), QCTNAME(me) +" turns back to face the battle, "+
      ...
      " heels into the "+ horse->query_short() +", which begins "+
...
charge_special(object str)
{
    ...
    if(!horse)
    {
      NF("You will need a horse for this attack!\n");
      return 0;
    }
