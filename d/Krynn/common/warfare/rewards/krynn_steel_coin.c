 ...
 * krynn_steel_coin.c
 *
 * This is a reward for conquering areas in the Krynn warfare system. 
 *
 * It can be used to gain masterwork equipment from Theros Ironfeld
 * in his Solace forge.
 *
 * Arman, October 2018
 */
...
{
    set_name("coin");
    add_name("_krynn_warfare_token");
    set_pname("coins");
    set_short("steel coin");
    set_pshort("steel coins");
    set_coin_type("steel");

    set_commodity_name ("steel coin"); // required to link this
                                  // object with the domain server
    set_commodity_value(2304, 0);
}
...
create_heap()
{
    ...
    add_cmd_item( ({ "coin", "steel"}), ({ "sniff" }),
        "This coin smells like cold steel.\n");
...
long()
{
	...
	(num_heap() >= 1000))
    {
	return "It is " + short() + ". Since the Cataclysm, steel has " +
            "been the resource of demand across Krynn, valued more than " +
            "gold, silver and even platinum! With the war forges churning " +
            "out weapons of war, these steel coins can often be traded to " +
            "the smiths of the land for masterwork equipment of the highest " +
            "quality.\n";
    }
    else
    {
	return "There are " + short() + ". Since the Cataclysm, steel has " +
            "been the resource of demand across Krynn, valued more than " +
            "gold, silver and even platinum! With the war forges churning " +
            "out weapons of war, these steel coins can often be traded to " +
            "the smiths of the land for masterwork equipment of the highest " +
...
set_coin_type(string str)
{
    ...
    // One steel coin is worth around 16 gold coins
    add_prop(HEAP_I_UNIT_VALUE,  2304);   
