...
#define DEBUG(x) find_player("arman")->catch_msg("[see invis] " + x + "\n")
...
create_spell_object()
{
    set_name("_mw_see_invis_object_");
    set_short("masterwork see invis spell object");    
    set_long("This is the standard see invisibility spell's object"
        + " for enchanted masterwork armours.\n");

    add_prop(OBJ_S_WIZINFO, "This is a spell object " +
                            "created by the see_invis_ability.c spell.\n");
    set_spell_effect_desc("see invisible");  
}
...
hook_spell_effect_started()
{
    ...
    if (objectp(target))
    {
        target->catch_tell("You feel yourself infused with magical " +
            "energies, heightening your sensitivity to detect " +
            "the magically hidden.\n");
    }    
}
...
hook_spell_effect_ended()
{
    ...
    if (objectp(target))
    {
        target->catch_tell("Your heightened sensitivity to detect " +
            "the magically hidden suddenly ends.\n");
    }
}
...
hook_spell_effect_not_maintained()
{
    ...
    if (objectp(caster))
    {
        caster->catch_tell("Your heightened sensitivity to detect " +
            "the magically hidden ends.\n");
    }    
}
