/*
 * Truesight divination spell for warfare armour helms
...
config_propchange_spell()
{
    ...
    set_spell_name("scryward");
    set_spell_desc("protect from scrying");
    ...
    set_shadow_filename(ABILITY_LOC + "prot_scry_ability_sh");
...
check_valid_action(object caster, mixed * targets, string arg, int execute = 0)
{
    ...
    if (helm->query_crystal_id_two() != "_scry_protection_crystal")
    {
        return 0;
    }
...
hook_describe_cast_propchange(object caster, object* targets)
{
    ...
    if (IN_ARRAY(caster, targets))
    {
        ...
        {
            caster->catch_tell("You feel your " +helm_str+ " grow " +
                "uncomfortably warm and then suddenly cold as a " +
                "ward of divination protection activates.\n");
        }
        else
        {
            caster->catch_tell("You feel your " +helm_str+ " grow " +
