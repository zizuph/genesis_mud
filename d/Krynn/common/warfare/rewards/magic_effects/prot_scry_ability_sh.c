/*
 * Shadow for prot vs scry ability
 */
...
initialize_shadow(object spell_object, mapping spell_input)
{
    ...
    spell_object->set_name("_mw_scry_prot_sh_");
    spell_object->set_spell_effect_desc("scry protection");
    spell_object->set_short("masterwork see invis spell object");
    spell_object->set_dispel_time(900);
...
query_prop(string prop)
{
    ...
    string fail_message;
    object helm = shadow_who->query_armour(TS_HEAD);
    string helm_str = helm->query_short();

    int scry_prot = 75 + random(20);
    ...
    if (intp(result))
    {
      if(!fail_message)
      {
        shadow_who->catch_tell("You feel the " +helm_str+ "'s wards " +
            "against divination bypassed!\n");
        return 0;
      }
        shadow_who->catch_tell("You feel the " +helm_str+ "'s wards " +
            "against divination protect you from a scrying attempt.\n");
        return fail_message;
    }
    else 
    {
