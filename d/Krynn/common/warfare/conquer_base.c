...
public void reset_conquer_room();
...
set_area_name(string str)
{
    ...
    set_alarm(0.1,0.0, &reset_conquer_room());
    */

    //Flat 20% chance.  The biggest area has 200 rooms, so that would
    //be about 120 npcs.  Quite a few, but not atrocious.
    set_num_npc(random(5) ? 0 : 1 + random(3));
    set_npc_chance(2);
...
/* Temporary function for testing - Carnak */
public status
add_room_npc(mixed npc)
{
    if (!gNpcsThisRoom)
        return 0;
    ...
    if (!sizeof(npc = filter(npc, objectp)))
        return 0;
    
    npc->move_living("M", this_object());
    npc->command("emote arrives to defend the contested area.");
    gNpc += npc;
...
remove_room_npc(mixed npc)
{
    ...
    /* Remove local npcs which might have been left over */
    npc->command("emote retreats as the sound of a horn echoes through " +
	"the area.");
    npc->remove_object();
    gNpc -= npc;
...
check_area(string str)
{
    ...
    {
        write("You recognize this area as the " + gAreaName + ", and it "+
        "appears to be currently owned by the "+ 
        ARMYMASTER->army_in_area(gAreaName)+".\n");
    }
    return 1;
}
...
reset_conquer_room()
{
    ...
    while (sizeof(gNpc) < gNpcsThisRoom)
    {
        object ob;
        
        if (!objectp(ob = ARMYMASTER->clone_new_npc(gAreaName)))
            break;
...
conquer_update(string area, string newarmy, string oldarmy)
{
    ...
    gNpc->command("emote retreats as the sound of a horn echoes through " +
	"the area.");
    gNpc->remove_object();
    gNpc -= ({ 0 });
