 ...
 * Teth, Feb 97, added Krynn props.
 * Khail Feb 14, 1997. Fixed the messages bug.
 * Mercade, 21 may 2004; removed decay support.
 */
#pragma save_binary
#pragma strict_types
    ...
    for (i = 0; i < 10; i++)
    {
        ...
        if (calling_object(-i) == TO)
        {
            ...
            if (function_exists("set_herb_name", arr[0]) != "/std/herb")
                return "You can only put herbs in the pouch.\n";
        }
    }
...
do_keep(string str)
{
    ...
    *belt_cont;
    ...
    if (!parse_command(L(str), all_inventory(tp),
