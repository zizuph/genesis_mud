...
#define LOG_FILE        "/d/Gondor/log/enhance"
...
/* you can enhance a skill only as much levels as you trained the skill */
    if(strength > 0)
        strength = MIN(tp_bsk, strength);
...
/* enhance the skill */
    if(strength)
    {
        ...
        seteuid(getuid());
        write_file(LOG_FILE,target->query_name()+": "+effect_msg+" <"+
            skill+"> changed by "+
            strength+" to "+target->query_skill(skill)+" on "+ctime(time())+
            " for "+time+" secs.\n");
    }

    gAlarm_id = set_alarm(itof(time), 0.0, remove_effect);
}
...
int query_time()
{
    ...
    alarm = get_alarm(gAlarm_id);

    return (pointerp(alarm) ? ftoi(alarm[2]) : -1 );
}
...
void remove_effect()
{
    ...
    if(interactive(tp) && enhance_flag)
    {
        if(strength)
        {
            ...
            tp->catch_msg("You feel your ability to "+effect_msg+
                (strength<0 ? " improve" : " deteriorate")+"!\n");
            write_file(LOG_FILE,tp->query_name()+": "+effect_msg+" <"+
                skill+"> reset to "+tp->query_skill(skill)+" by "+strength+
                " on "+ctime(time())+".\n");
        }
    }
...
{
    string str;
    mixed alarm = get_alarm(gAlarm_id);
