/* A touristing object, small quest. Made by Aridor for Krynn */
...
ask(object ob)
{
    ...
    tell_object(ob,
		"All you have to do is find the lighthouse in Palanthas.\n");
    return 1;
}
...
reward(object ob)
{
    ...
    if (!sum)
    {
tell_object(ob, "It seems you haven't found the lighthouse yet. \n");
	return 0;
    }
    ...
    K_QUEST_LOG("quest", "Tour 4", ob, sum);
    return sum;
}
...
create_object()
{
    ...
    set_short("@@my_short");
    set_long("" +
	"It's the fourth touristing card you have from Krynn.\n" +
	     "You must find the lighthouse in Palanthas and\n" +
	     "then return to the Master and ask for your reward.\n" +
	"");
...
my_short()
{
    ...
    short = "touristing card marked 4 ";
    if (arr[0])
	short += "(solved)";
    else
	short += "(unsolved)";
