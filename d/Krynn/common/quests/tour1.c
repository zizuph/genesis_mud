...
ask(object ob)
{
    ...
    tell_object(ob,"Please find the public adventurer's guild in Solace.\n");
    return 1;
}
...
reward(object ob)
{
    ...
    if (!sum)
    {
	tell_object(ob, "It seems you haven't found the adventurer's guild yet. \n");
	return 0;
    }
    ...
    K_QUEST_LOG("quest", "Tour 1", ob, sum);
    return sum;
}
...
create_object()
{
    ...
    add_name("tour1");
    set_short("@@my_short");
    set_long("It's the first touristing card you have from Krynn. " +
	     "You must find the public adventurer's guild in Solace and " +
...
my_short()
{
    ...
    short = "touristing card marked 1 ";
    if (arr[0])
	short += "(solved)";
    else
	short += "(unsolved)";
