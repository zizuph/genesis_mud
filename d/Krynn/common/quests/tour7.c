...
ask(object ob)
{
    ...
    tell_object(ob,"Please find Hawkstone's Taproom in Flotsam.\n");
    return 1;
}
...
reward(object ob)
{
    ...
    if (!sum)
    {
	tell_object(ob, "It seems you haven't found the inn in Flotsam yet. \n");
	return 0;
    }
    ...
    K_QUEST_LOG("quest", "Tour 7", ob, sum);
    return sum;
}
...
create_object()
{
    ...
    add_name("tour7");
    set_short("@@my_short");
    set_long("It's the seventh touristing card you have from Krynn. " +
        "You must find Hawkstone's Taproom in Flotsam, and " +
...
my_short()
{
    ...
    short = "touristing card marked 7 ";
    if (arr[0])
	short += "(solved)";
    else
	short += "(unsolved)";
