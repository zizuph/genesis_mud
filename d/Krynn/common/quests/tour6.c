...
ask(object ob)
{
    ...
    tell_object(ob,"Prove that you know the way to the courtyard of " +
      "Vingaard Keep. Be careful if you are a goblin, though.\n");
    return 1;
}
...
reward(object ob)
{
    ...
    if (!sum)
    {
        tell_object(ob, "It seems you haven't found the keep yet. \n");
        return 0;
    }
    ...
    K_QUEST_LOG("quest", "Tour 6", ob, sum);
    return sum;
}
...
create_object()
{
    ...
    add_name("tour6");
    set_short("@@my_short");
    set_long("It's the sixth touristing card you have from Krynn. " +
      "You must find the courtyard in Vingaard Keep and " +
      "then get back to the Master and tell him you want the " +
      "reward. Be careful if you are a goblin!\n");
...
my_short()
{
    ...
    short = "touristing card marked 6 ";
    if (arr[0])
        short += "(solved)";
    else
        short += "(unsolved)";
