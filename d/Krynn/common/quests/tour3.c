/* A touristing object, small quest. Made by Percy (orig. Nicks) for Krynn */
...
ask(object ob)
{
    ...
    tell_object(ob,
	"Find and bless the grave of Drakmar Dragonbane.\n");
    return 1;
}
...
reward(object ob)
{
    ...
    if (!sum)
    {
	tell_object(ob, "It seems you haven't blessed the grave yet.\n");
	return 0;
    }
    ...
    K_QUEST_LOG("quest", "Tour 3", ob, sum);
    return sum;
}
...
create_object()
{
    ...
    set_short("@@my_short");
    set_long("" +
	"It's the third touristing card you have from Krynn.\n" +
	"If you could visit the grave of Drakmar Dragonbane \n" +
	"who was a good friend of the Master. Bless the grave\n" +
	"then get back to the Master and tell him you want the reward.\n" +
	"");
...
my_short()
{
    ...
    short = "touristing card marked 3 ";
    if (arr[0])
	short += "(solved)";
    else
	short += "(unsolved)";
