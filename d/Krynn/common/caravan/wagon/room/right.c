/*
 *  Filename:       right.c
 *  Description:    This is the room to the right of every caravan wagon.
 *
...
make_connections()
{
    ...
    if (myWagon->query_rear())
        ...
        add_exit(myWagon->query_back_wagon()->query_room(R_RIGHT, M_OBJECT),
                                            "back", 0, MOVE_BACK_FAT);

    if (myWagon->query_leader())
        add_exit(myWagon->query_room(R_FRONT, M_OBJECT), "forward", 0, MOVE_FORWARD_FAT);
    else
        add_exit(myWagon->query_front_wagon()->query_room(R_RIGHT, M_OBJECT),
...
long_desc(string longdesc)
{
    ...
    if (!myWagon->i_am_burned_down())
        desc =  "You are on the right side of a caravan, next to you you " +
                "see " + LANG_ADDART(myWagon->short()) + "." +
                "Without a doubt it would be possible to move 'forward' or fall " +
                "'back' through the wagons of the caravan.\n";
    else
        desc =  "You are on the right side of a wagon, next to you you " +
                "see " + LANG_ADDART(myWagon->short()) + "." +
...
shorter_desc(string shortdesc)
{
    ...
    if (!myWagon->i_am_burned_down())
        desc = "On the right hand side of a caravan beside " +
             LANG_ADDART(myWagon->short());
    else
        desc = "On the right hand side of the remains of a wagon";
