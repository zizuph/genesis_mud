...
create_vin_mount_room()
{
    ...
    add_exit(ROOM + "road8","northeast",0,4);
    add_exit(ROOM + "road10","southeast",0,1);

    add_item(({"track","ground","road","gravel","mountain track"}),
	     BS("The track itself is simply " +
		"laid out with small stones and gravel.",SL));
    add_item(({"mountains","mountain"}),
	     BS("These are high mountains indeed, rocks and occasional " +
		"shrubs are scattered everywhere, it is definitely " +
		"a very hostile area.",SL));
    add_item(({"shrubs","rocks"}),
	     BS("The mountainsides are covered with rocks with a few " +
		"shrubs making their living in between.",SL));
}
