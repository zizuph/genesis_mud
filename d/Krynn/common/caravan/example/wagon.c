No such file: /d/Gondor/open/wagon.c}
> /* 
...
-- More -- 200 -- (<cr> t b r n a <num> q x !<cmd> h ?) -- private void
...
tour_move()
{
    ...
    move(Path[Step]);
    tell_room(ENV(TO), "A wooden carriage pulled by horses comes rambling
in.\n");
    ...
    {
        set_leavable(1);
-- More -- 300 -- (<cr> t b r n a <num> q x !<cmd> h ?) --      if (Goal == Tour_Goal)
            back_goal = Retour_Goal;
        else
            back_goal = Tour_Goal;
        tell_room(ENV(TO), "The wagon-driver yells: "
                  + "The carriage will return to " + back_goal + " in 1
hour!\n");
        tell_room(Wagon_Inside, "The wagon-driver yells: "
...
pay_driver(string str)
{
    ...
    if (Horses < 1) 
-- More -- 400 -- (<cr> t b r n a <num> q x !<cmd> h ?) --     {
