...
create_wagon()
{
    ...
    set_tour_goal("Edoras");
    set_retour_goal("Minas Tirith");
-- More -- 100 -- (<cr> t b r n a <num> q x !<cmd> h ?) -- -- More -- 100 -- (<cr> t b r n a <num> q x !<cmd> h ?) -- Rohim tells you:
Sounds good :)
    set_max_passengers(5);
    set_max_horses(4);

    if (!set_wagon_inside(MINAS_DIR + "obj/mt_inwagon"))
        log_file("bugs", file_name(TO) + "could not find " +
                 MINAS_DIR + "obj/mt_inwagon" + ".\n");
}

public string
query_pay_prop()
{
    return "_Gondor_MT_Wagon";
}
