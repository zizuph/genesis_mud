 ...
 *
 * 04/21/2022 - Ckrik
 *     - Added a property check so that we have can have
 *         an exception for NPCs that are always injured.
 * 11/14/2022 - Zizuph
 *     - Bumped up guild XP multiplier as F_PENMOD changes.
 */
...
hook_describe_heal_result(object caster, object *targets, int * heal_amounts)
{
    ...
    else
    {
        ...
        tell_room(environment(caster), QCTNAME(caster) + " places " +
            ...
            HE(tar) + " shivers as " + HIS(tar) + " wounds are" +
            heal_desc+ " healed.\n",
            ({ caster, tar }), caster);
    }

    // experience for healing is 33% of heal value to a max of 300xp
    int heal_xp = min(300, (heal / 3));
    // int guild_xp = caster->query_skill(OOTS_GUILD_XP);
    // caster->set_skill(OOTS_GUILD_XP, guild_xp + heal_xp);

    if (!tar->query_prop(LIVE_I_OOTS_HEAL_NO_EXP))
    {
