/*
 * oBless - protection from evil
 */

#pragma strict_types

// inherit "/d/Genesis/specials/new/magic/spells/stoneskin";
...
{
    set_spell_name("obless");
    set_spell_desc("Divine blessing - protection from evil");
    ...
    // Spell effect object
    set_spell_object(OOTS_SPELL_OBJS + "obless_obj");
...
query_spell_can_be_learned(object player)
{
    ...
    // Level 6 (Disciple) spell
    if(OOTS_LEVEL(player) < 6)
    {
        return 0;
    }
...
{
    return ({ "_oots_holy_water", "sarsaparilla" });
}

void
hook_use_ingredients(object *ingrs)
{
    write("Your prayers are granted as you crumble a licorice-scented " +
          "pliable root in your hand and cleanse yourself with a " +
          "phial of holy water.\n");
...
{
    return "sarsaparilla, holy water";
}
...
{
    write("You are already blessed with a protection from evil, and " +
...
{
    write("You are missing an ingredient to invoke the protection " +
          "from evil blessing!\n");
}
