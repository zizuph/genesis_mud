...
query_lords()
{
    mixed keys = m_indexes(members),
        result_ar = ({});
    int i = sizeof(keys);

    while (i--)
    {
        if (members[keys[i]][T_RANK] == TEMPLAR_RANK_LORD 
            && !wildmatch("*jr",keys[i]))
        {
            result_ar += ({ keys[i] });
        }
...
query_officers()
{
    ...
    while (i--)
    {
        if (members[keys[i]][T_RANK] == TEMPLAR_RANK_OFFICER 
            && !wildmatch("*jr",keys[i]))
        {
            result_ar += ({ keys[i] });
        }
...
query_knights()
{
    ...
    while (i--)
    {
        if (members[keys[i]][T_RANK] == TEMPLAR_RANK_KNIGHT 
            && !wildmatch("*jr",keys[i]))
        {
            result_ar += ({ keys[i] });
        }
...
query_warriors()
{
    ...
    while (i--)
    {
        if (members[keys[i]][T_RANK] == TEMPLAR_RANK_WARRIOR 
            && !wildmatch("*jr",keys[i]))
        {
            result_ar += ({ keys[i] });
        }
...
query_betrayers()
{
    ...
    while (i--)
    {
        if (members[keys[i]][T_RANK] == TEMPLAR_RANK_BETRAYER 
            && !wildmatch("*jr",keys[i]))
        {
            result_ar += ({ keys[i] });
        }
...
adjust_templar_rank(mixed who, int ranks)
{
    ...
    if (i < TEMPLAR_RANK_BETRAYER) {
        i = TEMPLAR_RANK_BETRAYER;
    }
    
    if (i > TEMPLAR_RANK_LORD) {
        i = TEMPLAR_RANK_LORD;
    }
...
set_templar_rank(mixed who, int rank)
{
    ...
    if (rank < TEMPLAR_RANK_BETRAYER)
    {
        rank = TEMPLAR_RANK_BETRAYER;
    }
    
    if (rank > TEMPLAR_RANK_LORD)
    {
        rank = TEMPLAR_RANK_LORD;
    }
