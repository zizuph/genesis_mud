...
create_board()
{
    ...
    set_board_name(LOG + "board_conclave");
    set_num_notes(50);
    set_silent(1);
    set_show_lvl(0);
...
block_reader()
{
    set_alarm(0.0, 0.0, &(TP)->remove_prop(TEMP_STDREAD_CHECKED));
    TP->add_prop(TEMP_STDREAD_CHECKED, 1);
    
    if ((SECURITY->query_wiz_rank(TP) >= WIZ_LORD) ||
        (member_array(SECURITY->query_wiz_dom(TP), 
            ({ "Krynn", "Ansalon" })) >= 0))
        return 0;
    
    if (!ADMIN->query_conclave_member(TP))
    {
        /* Don't write any stupid messages to mbs readers */
