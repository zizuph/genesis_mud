...
create_tower_room()
{
   ...
   add_item(({"pool","central pool","captured light","light","lunitari",
       "energies","captured energies","red light"}),
        "@@pool_desc");
   ...
   add_exit(WOHS_TEST + "room/daltigoth_tower3", "northeast", 0);
   add_exit(WOHS_TEST + "room/daltigoth_tower6", "up", 0);
   ...
   add_cmd_item(({"from pool","water","from shallow pool"}), "drink",
       "@@drink_water");

   add_cmd_item(({"pool","shallow pool","water"}),
       ({"enter","dive","swim"}), "@@enter_pool");
...
pool_desc()
{
      if(TP->query_prop(HEARTROOM_ACTIVATED))
          return "Occupying the centre of the hall is a large shallow " +
              ...
              "To the north and south of the pool staircases writhe and " +
              "twist as the Tower is constricted by a powerful force.\n";

      return "Occupying the centre of the hall is a large shallow pool " +
          "that shines red with the magically captured light of " +
          "Lunitari. To the north and south of the pool staircases " +
          "rise to the upper levels of the tower.\n";
...
enter_pool()
{
      if(TP->query_prop(HEARTROOM_ACTIVATED))
          return "You step into the pool. Radiant energies swirl " +
              ...
              "the well at Wayreth, however nothing happens. It feels " +
