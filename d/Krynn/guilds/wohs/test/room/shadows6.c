...
string *bronze_str = ({"You notice Solinari is waning, while Nuitari " +
    "and Lunitari are at high sanction", "You notice a blank sky",    
    "You notice a blank sky"});

string *silver_str = ({"You notice they all look like they are at high " +
    "sanction - the Night of the Eye", "You notice a blank sky",    
    "You notice a blank sky"});

string *copper_str = ({"You notice Lunitari is waning, while Solinari " +
    "and Nuitari are at high sanction", "You notice a blank sky",    
    "You notice a blank sky"});
...
create_tower_room()
{
   ...
   add_item(({"shadows","ever-shifting shadows","dark fog","fog"}),
       "This room seems to consist entirely shadows, with the " +
       "exception of three mirrors the hang on the shadowy walls here.\n");
   add_item(({"tall mirrors","mirrors","wall","walls","shadowy walls",
       "three tall mirrors","three mirrors"}), "Three tall mirrors hang " +
       "on each of the three walls here - a bronze mirror, " +
       "a silver mirror, and a copper mirror.\n");
...
enter_bronze_mirror()
{
   ...
   // There is no setting 4, this option should never occur
   if(mirror_setting == 4)
   {
      if(objectp(guardian))
      {
...
enter_inv(object ob, object from)
{
    ...
    if(objectp(guardian) && ob->query_prop(FOURTH_TASK))
    {
        int stat = ob->query_average_stat();  
        guardian->set_stats(({stat, stat, stat, stat, stat, stat}));
        guardian->set_gender(ob->query_gender());
    }
