/* Navarre December 15th 2011, Added 'great book' to the add_item 
 * and add_cmd_item as the gardener guild's book interferes with 
 * 'book'. */
...
#define MAX_LORD_IDLE     7776000
// 90 days * 60sec * 60min * 24 hours = 7776000 secs (90 days)
...
create_vin_room()
{
	...
	add_item(({"book", "great book"}), "@@book_desc");
...
init()
{
    ...
    add_action("add_myself_member", "claim"); //
...
book_desc()
{
    ...
    " - <claim> [surname] lineage, with a certain house\n"+
...
/* Add yourself as a member to a house */
int
add_myself_member(string str)
{
    string fam, name, surname;

    NF("Claim what lineage?\n");
    if (!str)
	return 0;

   if (sscanf(str, "%s lineage", str) != 1)
   {
      notify_fail("Claim <what surname> lineage?\n");
      return 0;
   }
    ...
    surname = TP->query_family_title();

    NF("You are a member of another house already.\n");
    if (member_array(C(name), m_indexes(members)) >= 0)
	return 0;

    NF("No such family exists to claim lineage to.\n");
    if (member_array(fam, m_indexes(crest)) >= 0)
	return 0;

    NF("You have no family claim to the " +str+ " name.\n");
    if(!wildmatch(str, surname))
        return 0;
    ...
    members[C(name)] = fam;
    write("You claim lineage to house "+fam+".\n");
    say(QCTNAME(TP)+" adds a name to the book on the desk.\n");
    find_living(L(str))->catch_msg("You are now a member of "+
      "house "+fam+".\n");
    save_object(CREST);
...
appoint_new_lord(string str)
{
    ...
    // Lord can be inactive up to 90 days
    // before someone else can take the role.
    int t = time() - MAX_LORD_IDLE;
    ...
    if(strlen(lord))
    {
        ...
        {
            // check last login time of Lord
            int j = file_time(PLAYER_FILE(lord) + ".o");  

            // If Lord active i, do not replace.      
            if(j > t)
            {
                notify_fail("Only the house lord may appoint a new house lord.\n");
	        return 0;
            }
        }
    }
