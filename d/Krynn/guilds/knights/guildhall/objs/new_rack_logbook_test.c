...
set_rack(object obj)
{
    ...
    if (!stringp(rack_file = obj->query_rack_log_file()))
        return;
    
    if (!mappingp(rack_log = restore_map(rack_file)) || !m_sizeof(rack_log))
        return;
    
    foreach(string stamp, string *item: rack_log)
    {
        if (!pointerp(logfile))
            logfile = ({});
        
        if (sscanf(stamp, "timestamp_%d", value) && (time() - value) < 2592000)
        {
            logfile += ({ sprintf("%26-s: %10-s %8-s ", ctime(value),
            capitalize(item[0]), (item[3] == "enter" ? "stored" : "withdrew"))
            + " [" + (sizeof(item) > 3 ? item[1] : "?") + "] "
            + LANG_ADDART(item[2]) });
        }
    }
}
...
create_object()
{
    ...
    set_long("This is a logbook which details the usage of the Vingaard Keep "
    + "armoury. You may read it to get a recent history of rack usage. Once "
    + "finished, you can 'return' it to the attendant.\n");
...
do_read(string str)
{
    ...
    */
    ...
    this_player()->more(sort_array(logfile[(tail ? (sizeof(logfile) - tail) : 0)..]));
    return 1;
}
