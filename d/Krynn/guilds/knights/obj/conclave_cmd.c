...
do_help(string str)
{
    ...
    if (str == "conclave")
    {
    	if (ADMIN->is_conclave(name) || ADMIN->is_grandmaster(name) ||
    	    ...
    	    || IN_ARRAY("aob", SECURITY->query_team_membership(name)))
        {
      	    write(  "Available commands of the Knight Conclave:\n"+
                    ...
                    "ccheck       - Check the list of assistants\n" +
      	            ...
      	            "claylist     - Lists the current approval status for layman guilds\n" +
      	            "clayapprove  - Adds a layman guild to the approval list\n" +
      	            "clayreject   - Removes a layman guild from the approval list\n" +
	...
	case "knlist":
      ...
      "           Give info about a knight, even when s/he is not logged on.\n");
	    break;
	case "claylist":
	    write("claylist   - 'claylist'\n" +
	  "           Lists the current approval and rejection list for layman guilds.\n");
	    break;
	case "clayapprove":
	    write("clayapprove - 'clayapprove <guildname>'\n" +
	  "           Add a layman guild to the approved list for Knights.\n");
	    break;
	case "clayreject":
	    write("clayreject - 'clayreject <guildname>'\n" +
	  "           Removes a layman guild from the approved list for Knights.\n");
	    break;
	default:
...
check_owner_assistant()
{
    if (E(TO)->query_knight_sublevel() == 11 ||
      ...
      IN_ARRAY("aob", SECURITY->query_team_membership(E(TO)->query_real_name())))
	return 1;
...
cdemote(string str)
{
    NF("Demote <player> from crown|sword|rose conclave.\n");
    if (!str)
	return 0;
...
claylist(string str)
{
    mapping allowed_guilds, restricted_guilds;
    int index;
    
    allowed_guilds = LAYMAN_MANAGER->query_allowed_laymans();
    restricted_guilds = LAYMAN_MANAGER->query_restricted_laymans();
    
    index = 1;
    write("These are the layman guilds available to Knights:\n");    
    foreach (string guildname : sort_array(m_values(allowed_guilds)))
    {
        write(sprintf("%3d. %s\n", index++, guildname));
    }
    write("\n");
    ...
    index = 1;
    write("These are the layman guilds restricted from Knights:\n");    
    foreach (string guildname : sort_array(m_values(restricted_guilds)))
    {
        write(sprintf("%3d. %s\n", index++, guildname));
    }
    write("\n");
...
public int
clayapprove(string str)
{    
    mapping restricted_guilds;
    
    if (!str)
    {
        notify_fail("You need to specify a guild to approve.\n");
        return 0;
    }
    
    notify_fail("Failed to approve the layman guild '" + str + "'\n");
    str = lower_case(str); // reduce to lower case
    restricted_guilds = LAYMAN_MANAGER->query_restricted_laymans();
    setuid();
    seteuid(getuid());    
    foreach (string guild : m_indices(restricted_guilds))
    {
        if (lower_case(restricted_guilds[guild]) == str)
        {
            if (!LAYMAN_MANAGER->add_allowed_layman(guild))
            {
                return 0;
            }
            write_file(KLOG + "layman_approvals", ctime(time())[4..] + " "
              + capitalize(this_player()->query_real_name()) + " has "
              + "approved '" + restricted_guilds[guild] + "' as a layman "
              + "guild.\n");
            write("Ok.\n");
            return 1;
        }
...
public int
clayreject(string str)
{
    mapping allowed_guilds;
    
    if (!str)
    {
        notify_fail("You need to specify a guild to reject.\n");
        return 0;
    }
    
    notify_fail("Failed to reject the layman guild '" + str + "'\n");
    str = lower_case(str); // reduce to lower case
    allowed_guilds = LAYMAN_MANAGER->query_allowed_laymans();
    setuid();
    seteuid(getuid());
    ...
    {
        if (lower_case(allowed_guilds[guild]) == str)
        {
            if (!LAYMAN_MANAGER->remove_allowed_layman(guild))
            {
                return 0;
            }
            write_file(KLOG + "layman_approvals", ctime(time())[4..] + " "
              + capitalize(this_player()->query_real_name()) + " has "
              + "rejected '" + allowed_guilds[guild] + "' as a layman "
              + "guild.\n");
            write("Ok.\n");
            return 1;
        }
