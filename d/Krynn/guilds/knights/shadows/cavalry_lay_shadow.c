 ...
 *
 *   Louie, April 2006 - Added set_solamnian_steed and query_solamnian_steed.
 *
 *   Navarre August 2006 - Fixed so horse cant be attacked when far away.
 *
 *   Navarre February 2007 - Added checks for TP and medal in reset_charge_special
 *                           to avoid runtime errors, which showed in the logs.
 */
...
public object
query_solamnian_steed()
{
    return sol_steed;
}

public void
set_solamnian_steed(object ob)
{
    sol_steed = ob;
}
...
remove_maneuver(object st)
{
    ...
    if(shadow_who->query_prop("_kcharge_preparing"))
    {
      st->add_prop(OBJ_M_NO_ATTACK, "The "+st->steed_short() + " is too far away.\n");
    }
}
...
maneuver(string str)
{
    ...
    if (sizeof(enemies) >= 4)
    {
        ...
        set_alarm(me->query_speed(60), 0.0, &reset_maneuver(me));
        return 1;
    }
...
reset_charge_special(object liv)
{
    ...
    if (medal && medal->query_auto_aid())
    {
      liv->command("$kaid");
    }

    if (medal && liv->query_attack() && medal->query_auto_kattack())
    {
      liv->command("$kattack");
    }

    if (medal && liv->query_attack() && medal->query_auto_charge())
    {
      liv->command("$charge");
      return;
    }
...
check_enemies(object liv)
{
    ...
    remove_maneuver(shadow_who->query_prop(LIVE_O_STEED));
...
do_charge_special(object ob, object liv, object horse)
{
      ...
      !weapon->query_prop(OBJ_I_IS_MAGIC_WEAPON))
    {
        ...
        liv->remove_prop("_kcharge_preparing");
        liv->remove_prop(OBJ_M_NO_ATTACK);
        set_alarm(liv->query_speed(60), 0.0, &reset_charge_special(liv));
...
do_charge_prespecial2(object ob, object me, object horse)
{
    if (!P(ob, E(me)) || me->query_prop(LIVE_O_STEED) != horse)
    {
        ...
        me->remove_prop(OBJ_M_NO_ATTACK);
        me->remove_stun();
        check_enemies(me);
        return;
    }

    set_alarm(me->query_speed(3), 0.0, &do_charge_special(ob, me, horse));
...
do_charge_prespecial1(object ob, object me, object horse)
{
    ...
    set_alarm(me->query_speed(3), 0.0, &do_charge_prespecial2(ob, me, horse));
...
do_charge_engaged_prespecial1(object ob, object me, object horse)
{
    ...
    set_alarm(me->query_speed(4), 0.0, &do_charge_special(ob, me, horse));
    tmp = ob->query_attack();
...
charge_special(string str)
{
    ...
    {
        tmp = ("/cmd/std/command_driver")->parse_this(str, "%l");
        if (sizeof(tmp) > 1)
        {
            NF("Choose one enemy to charge.\n");
            return 0;
        }
        else if (!sizeof(tmp))
        {
            NF("Just whom are you trying to charge?\n");
            return 0;
        }
        ob = tmp[0];
    }
...
/*  Removed, as it doesn't take DIS to ride away from battle, and when
    the horse turns around there is no way to stop it.
    Boron, oct 26/03
    tmp = c_can_attack(ob, query_verb());
    if (stringp(tmp))
    {
        NF(tmp);
        return 0;
...
*/
    ...
    if (tmp = shadow_who->query_prop("_kcharge_resetting"))
    {
        ...
        if (time() > (tmp + 60))
        {
            set_alarm(shadow_who->query_speed(60), 0.0, &reset_charge_special(shadow_who));
        }
    ...
    horse->add_prop(OBJ_M_NO_ATTACK, "The "+ tmp + " is too far away.\n");
    shadow_who->add_prop(OBJ_M_NO_ATTACK, shadow_who->query_The_name(this_player()) +
                                          " is too far away at the moment.\n");
    ...
    {
        set_alarm(shadow_who->query_speed(4), 0.0,
    ...
    {
        set_alarm(shadow_who->query_speed(3), 0.0, &do_charge_prespecial1(ob, shadow_who, horse));
