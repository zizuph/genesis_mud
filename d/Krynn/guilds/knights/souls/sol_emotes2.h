...
* 
* Navarre April 2007, Changed tend emote to ktent to avoid
*                     conflicts with smiths in the realm.
*/
...
ks(string msg)
{
    ...
    {
	text = "In old Solamnian, you utter: " + msg;
	write(text + "\n");
	...
	if ((living(oblist[i]) && oblist[i] != TP)) {
	    ...
	    { 
                ...
                text = who + " utters in old Solamnian: " + msg;       
	    ...
	    {
                who = TP->query_The_name(oblist[i]);
                text = who + " utters something in the Solamnian language.";
		oblist[i]->catch_msg(who +
...
ksqnod(string str)
{
    ...
    if (!sizeof(oblist))
    {
	write("Nod at whom in obedience?\n");
	return 0;
    }
    if (oblist[0]->query_knight_level() <= 2) {
	write("That person is not a Knight.\n");
...
int kthrust(string str)
{
    ...
    clone_object(SHADOWS+"kthrust_weapon_shadow")->shadow_me(weapon);
    weapon->setup_things();
    weapon->move(room);
