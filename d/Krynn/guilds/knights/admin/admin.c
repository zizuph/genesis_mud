 ...
 * /d/Krynn/guilds/knights/admin/admin.c
 *
 ...
 *
 * Navarre, September 4th 2014, Added check to allow promtion with 1 vote if
 *          only one conclave exists.
 */
...
#endif
#define SPONSOR_TREE        KADM+"sponsor_tree"
#define BACKUP_SAVE_FILE    KLOG+"backup_admin"
...
crown_highest()
{
    return query_conclave("crown");
}

public string
sword_highest()
{
    return query_conclave("sword");
}

public string
rose_highest()
{
    return query_conclave("rose");
}
...
{
    // Make sure to set the orders correctly so that the variables are
    // not null.
    if (!crown)
        crown = "";
    if (!sword)
    ...
    // If no argument was specified, simply return
    if (!order)
        return;
        
    // The order can be specified by integer (3=crown, 4=sword, 5=rose)
    if (intp(order))
    {
        if (order == 3)
            order = "crown";
    ...
    // The rest of the function only accepts strings. So return if this is
    // not a string.
    if (!stringp(order))
        return;
        
    // There is a possibility that the argument passed in is the name
    // of the player that needs to step down from Conclave
    if (lower_case(order) == lower_case(crown))
    {
        order = "crown";
    }
    else if (lower_case(order) == lower_case(sword))
    {
        order = "sword";
    }
    else if (lower_case(order) == lower_case(rose))
    {
        order = "rose";
    }
...
set_grandmaster(string who)
{
    ...
    if (SECURITY->query_wiz_rank(TP->query_real_name()) <= WIZ_NORMAL &&
      MASTER_OB(previous_object()) != (VOBJ+"voting_box"))
    return;
...
log_action(string path, string message)
{
    ...
    if (path)
    write_file(KLOG + path, ctime(time())[4..15] + " " + mes + ".\n");
...
do_promote(string str)
{
    ...
    int number_of_conclaves = 0;
    
    for (int i = 0; i < sizeof(conclave); i++)
    {
        if (wildmatch("*currently no*", conclave[i]))
        {
            number_of_conclaves++;
        }
    }
    ...
    if (sscanf(str,"%s to %s conclave", who, reason) < number_of_conclaves)
    {
        NF("Attempting to promote... required votes : " +
           number_of_conclaves +
           ". Didn't have enough.\n");
        return 0;
    }
