/*
 * Arcanisvaes: Magic missile - Arman 2016
 *
...
#include "/d/Krynn/guilds/sohm/spells/magic_missile_desc.h"
...
config_at_will_spell()
{
    ...
    set_spell_name("arcanisvaes");
    set_spell_desc("Magic missile - a conjured bolt of magical energy");
...
query_spell_can_be_learned(object player)
{
    ...
    setuid();
    seteuid(getuid());
    return SOHM_MANAGER->query_memorised_spell(player,"arcanisvaes_memorised");
}
...
concentrate_msg(object caster, mixed * targets, string arg)
{
    string caster_message, watcher_message;
        ...
        caster_message = "You close your eyes and begin reciting from "
            + "memory powerful words of magic, 'Arcanis vaes!'\n";
...
{
    write("You feel ready to conjure a magic missile again.\n");
}
...
{
    write("You do not feel ready to conjure a magic missile again.\n");
}

public void
hook_already_active(object ability)
{
    write("You are currently conjuring a magic missile!\n");
}
...
desc_harm_cast(object caster, object *targets)
{
    ...
    magic_missile_desc_cast(caster, target);
...
{
        magic_missile_desc_damage(caster, target, result);
}
...
{
    int * needed_skills = ({ });
    int * check_skills = ({ SS_ELEMENT_AIR, SS_FORM_CONJURATION });
