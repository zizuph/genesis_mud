/*
 * Protection from fire spell for SOHM
 *
 * Coded by Arman 2016.
 */
...
{
    set_spell_name("trothixen");
    set_spell_desc("Minor protection, fire - a fire protection spell");
    ...
    set_spell_object(SOHM_SPELL_OBJS + "protect_fire_obj");
    add_resistance(MAGIC_I_RES_FIRE, 30);
...
query_spell_can_be_learned(object player)
{
    ...
    setuid();
    seteuid(getuid());
    return SOHM_MANAGER->query_memorised_spell(player,"trothixen_memorised");
}
...
concentrate_msg(object caster, mixed * targets, string arg)
{
    ...
    object target = targets[0];
        ...
        caster_message = "Your eyes narrow in concentration as you begin reciting " +
            "ancient words of magic, a ward of protection from fire.\n";
...
hook_describe_cast_resistance(object caster, object * targets)
{
    ...
    if (IN_ARRAY(caster, targets))
    {
        ...
        {
            caster->catch_tell("You cast a fire protection spell upon "
                + "yourself and " + COMPOSITE_LIVE(message_targets) + ".\n");
        }
        else
        {
            caster->catch_tell("You cast a fire protection spell " +
                  "upon yourself.\n");
        }
    }
    else
