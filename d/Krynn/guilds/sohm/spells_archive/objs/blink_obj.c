...
hook_spell_effect_started()
{
    ...
    if (objectp(target))
    {
        target->catch_tell("You will now phase in and out of the material plane.\n");
...
hook_spell_effect_ended()
{
    ...
    if (objectp(target))
    {
        target->catch_tell("You stop phasing in and out of the material plane.\n");
...
{
    if (for_obj == on)
        return  "You feel your body is phasing in and out of the material realm.\n";
    else
        return capitalize(on->query_pronoun()) + " seems to flicker out of existance periodically.\n";
}
...
{
    player->catch_msg("You blink out of the material realm, reappearing behind "
                      + QTNAME(friend) + ". Your enemies move "
                      + "to attack " + friend->query_objective()
                      + "!\n");
    friend->catch_msg(QCTNAME(player) + " blinks out of the material realm, reappearing " +
                      "behind you. "+ capitalize(player->query_possessive()) + " enemies "
                      + "move to attack you!\n");                           
