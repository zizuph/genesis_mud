/*
 * Geyser spell for School of High Magic - Arman 2016
 */
...
config_harm_spell()
{
    ...
    set_spell_name("skaerenpab");
    set_spell_desc("Geyser - summon a column of boiling water");
...
query_spell_can_be_learned(object player)
{
    ...
    setuid();
    seteuid(getuid());
    return SOHM_MANAGER->query_memorised_spell(player,"skaerenpab_memorised");
}
...
check_valid_action(object caster, mixed * targets, string arg, int execute = 0)
{
    ...
    if(caster->query_prop("_sohm_portents_and_omens_effect"))
    {
        caster->catch_tell("Your portents and omens spell interferes " +
            "with your ability to cast geyser.\n");
	return 0; 
    }
...
concentrate_msg(object caster, mixed * targets, string arg)
{
    ...
    caster_message = "You trace flowing runes of magic in the air with your " 
        +wep[0]->short()+ ", summoning the intense powers of the deep.\n";
    watcher_message = QCTNAME(caster) +" traces flowing runes in the air with "+
...
desc_harm_cast(object caster, object * targets)
{
    ...
    caster_message = "You point your " +wep[0]->short()+ " towards " +
        QTNAME(target) + " and call out 'Skaer Enpab!' - releasing " +
...
{
    write("You need to have quaffed a skaer potion to cast geyser.\n");
}
