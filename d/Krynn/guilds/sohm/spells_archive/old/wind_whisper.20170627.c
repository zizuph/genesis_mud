 ...
 * 
 * Changes:
 * Carnak - 2017-06-21: Tell now uses QTNAME for caster in the target msg
...
concentrate_msg(object caster, mixed * targets, string arg)
{
    string caster_message, watcher_message;
        ...
        caster_message = "You cup your hands around a copper coin and " +
        "whisper words of magic, 'Suaco visp'!\n";
        watcher_message = QCTNAME(caster) + " cups " +
        caster->query_possessive() + " hands and begins whispering words " +
        "of magic into them.\n";
...
{
        caster->catch_tell(capitalize(target) + " is not contactable with " +
        "suacovisp.\n");    
...
{
    target->catch_msg("A gust of wind whispers past you, followed by a " +
    "message from " + QTNAME(caster) + ":\n\t" + message_text + "\n");
    caster->catch_msg("The copper coin vanishes as you uncup your hands and " +
    "send a message across the winds to " +
    capitalize(target->query_real_name()) + ":\n\t" + message_text + "\n");
    
    // We add it to the target's tell history
    SOHM_MANAGER->add_tell_message(target, caster->query_real_name(),
    message_text);
}
...
hook_send_gmcp_comms(object caster, object target, string message_text)
{
    ...
    target->gmcp_comms("fartell", caster_name, "A gust of wind whispers " +
    "past you, followed by a message from " + QTNAME(caster) + ":\n\t" +
    message_text + "\n");
    caster->gmcp_comms(spell_name, 0, "The copper coin vanishes as you " +
    "uncup your hands and send a message across the winds to " + target_name +
    ":\n\t" + message_text + "\n");
}