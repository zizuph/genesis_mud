...
{
    set_spell_name("suacotrekis");
    set_spell_desc("Wind gust - conjure a gust of wind");
    ...
    set_shadow_filename(SOHM_SPELL_OBJS + "wind_gust_sh");
...
query_spell_can_be_learned(object player)
{
    ...
    setuid();
    seteuid(getuid());
    return SOHM_MANAGER->query_memorised_spell(player,"suacotrekis_memorised");
}

public int
is_valid_gloom_for_spell(object caster, object obj)
{
    if (!objectp(obj) || !obj->id("_SoHM_gloom_object"))
    {
        return 0;
    }
...
get_valid_gloom_for_spell(object caster)
{
    if (!objectp(caster) || !objectp(environment(caster)))
    {
        return ({ });
    }
    return filter(all_inventory(environment(caster)), 
        &is_valid_gloom_for_spell(caster,));    
}
...
check_valid_action(object caster, mixed * targets, string arg, int execute = 0)
{
    ...
    all_gloom = get_valid_gloom_for_spell(caster);

/*
    if (!sizeof(all_gloom))
    {
        caster->catch_tell("There are no shadow fogs here for you to " +
            "control with wind gust.\n");
        return 0;
    }
*/  
...
concentrate_msg(object caster, mixed * targets, string arg)
{
    ...
    caster_message = "You wave your hands in the air and call out a " +
        "magical word of summoning: 'Suaco Trekis!'\n";
    watcher_message = QCTNAME(caster) + " waves " +HIS(caster)+ 
        " hands in the air and calls out a word of summoning.\n";
...
{
    caster->catch_tell("You summon a gentle zephyr of wind that gusts " +
        "around you.\n");
    caster->tell_watcher(QCTNAME(caster) + " summons a gentle zephyr " +
        "of wind that gusts around " +HIM(caster)+ ".\n", ({ }) );

    if(sizeof(all_gloom))
        tell_room(environment(caster), "The fog of dark shadows shifts " +
            "with the breeze.\n", ({ }) );
}
...
{
    caster->catch_msg("You are already maintaining a wind gust spell!\n");
}