/*
 * Protection from poison spell for SOHM
 *
 * Coded by Arman 2016.
 */
...
{
    set_spell_name("trothweyog");
    set_spell_desc("Minor protection, poison - a poison protection spell");
    ...
    set_spell_object(SOHM_SPELL_OBJS + "protect_poison_obj");
    add_resistance(MAGIC_I_RES_POISON, 48);
...
query_spell_can_be_learned(object player)
{
    ...
    setuid();
    seteuid(getuid());
    return SOHM_MANAGER->query_memorised_spell(player,"trothweyog_memorised");
}
...
concentrate_msg(object caster, mixed * targets, string arg)
{
    ...
    object target = targets[0];
        ...
        caster_message = "Your eyes narrow in concentration as you begin reciting " +
            "ancient words of magic, a ward of protection from poison.\n";
...
hook_describe_cast_resistance(object caster, object * targets)
{
    ...
    if (IN_ARRAY(caster, targets))
    {
        ...
        {
            caster->catch_tell("You cast a poison protection spell upon "
                + "yourself and " + COMPOSITE_LIVE(message_targets) + ".\n");
        }
        else
        {
            caster->catch_tell("You cast a poison protection spell " +
                  "upon yourself.\n");
        }
    }
    else
