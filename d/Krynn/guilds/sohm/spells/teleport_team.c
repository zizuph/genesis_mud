...
#define FILTER_WITH_CASTER(caster, x) \
    filter((x), &operator(==)(environment(caster)) @ environment)
...
spell_target_teleport(object caster, string str)
{
   ...
   return ({ target_room, caster }) + 
       FILTER_WITH_CASTER(caster, caster->query_team_others());
}
...
hook_cannot_teleport_target_not_relaxed(object caster, object target, string arg, int execute)
{
    if (caster == target)
    {
        caster->catch_msg("You have too recently been in combat. You have to "
            + "relax first before you can teleport.\n");
    }
    else
    {
        caster->catch_msg(QCTNAME(target) + " has too recently been in combat.  They need "
            + "to relax first before they can be teleported.\n");
    }
}
...
hook_cannot_teleport_target_too_soon(object caster, object target, string arg, int execute)
{
    if (caster == target)
    {
        caster->catch_msg("You have too recently teleported. You have to "
            + "wait before you can teleport again.\n");
    }
    else
    {
        caster->catch_msg(QCTNAME(target) + " has too recently teleported.  They need "
            + "to wait before they can be teleported again.\n");
    }
}
...
hook_describe_teleport_origin_room(object caster, mixed* targets, object origin_room) 
{
    ...
    foreach (object target : targets)
    {
        if (caster == target)
            continue;
        target->catch_tell("Your feel your body begin to shimmer and everything " +
            "around you blur, before finding yourself somewhere else entirely!\n");
        tell_room(origin_room, QCTNAME(target) +  " shimmers briefly and " +
            "vanishes.\n",  targets );
    }
}
...
hook_describe_teleport_target_room(object caster, mixed* targets, object target_room)
{
    ...
    if (sizeof(moved))
    {
        caster->catch_msg("With the successful completion of your spell, " +
           COMPOSITE_LIVE(moved)+ is_or_are+ " magically teleported to you.\n");
    }
}