/*
 * Water breathing spell for SOHM
 *
 * Coded by Arman 2016.
 */
...
config_propchange_spell()
{
    ...
    set_shadow_filename(SOHM_SPELL_OBJS + "breath_water_sh");
...
query_spell_can_be_learned(object player)
{
    ...
    setuid();
    seteuid(getuid());
    return SOHM_MANAGER->query_memorised_spell(player,"pabfrahrir_memorised");
}
...
concentrate_msg(object caster, mixed * targets, string arg)
{
    ...
    caster_message = "Your eyes narrow in concentration as you begin reciting " +
        "words of magic that will provide you the ability to breath water.\n";
...
hook_describe_cast_propchange(object caster, object* targets)
{
    ...
    if (IN_ARRAY(caster, targets))
    {
        ...
        {
            caster->catch_tell("You cast a water breathing spell " +
                  "upon yourself.\n");
        }
    }
    else
    ...
    caster->tell_watcher(QCTNAME(caster) + " casts a spell of water " +
        "breathing.\n", ({ }) );
}
...
{
    caster->catch_msg(QCTNAME(target) + " already has a water breathing " +
