/*
 * Bakmadaingis: Prismatic Spray - Arman 2016
 *
...
#include "/d/Krynn/guilds/sohm/spells/prismatic_spray_desc.h"
...
{
    set_spell_name("bakmadaingis");
    set_spell_desc("Prismatic spray - a damaging beam of multicoloured light");
...
query_spell_can_be_learned(object player)
{
    ...
    setuid();
    seteuid(getuid());
    return SOHM_MANAGER->query_memorised_spell(player,"bakmadaingis_memorised");
}
...
concentrate_msg(object caster, mixed * targets, string arg)
{
    string caster_message, watcher_message;
        ...
        caster_message = "You close your eyes and begin reciting from "
            + "memory powerful words of magic, 'Bakmada Ingis!'\n";
...
{
    write("You feel ready to cast prismatic spray again.\n");
}
...
{
    write("You do not feel ready to cast prismatic spray again.\n");
}

public void
hook_already_active(object ability)
{
    write("You are currently casting a prismatic spray!\n");
}
...
desc_harm_cast(object caster, object *targets)
{
    ...
    prismatic_spray_desc_cast(caster, target);
...
{
        prismatic_spray_desc_damage(caster, target, result);
}
...
{
    int * needed_skills = ({ });
    int * check_skills = ({ SS_ELEMENT_WATER, SS_FORM_ILLUSION });
