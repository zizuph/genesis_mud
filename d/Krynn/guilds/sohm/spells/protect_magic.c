...
spell_target_one_present_living(object actor, string str)
{
   ...
   // 40 caid with only one element protection gets a 20% loading to base 48 protection
   // Modify combat aid depending on phase of the magic moons:

   if(caster->query_wohs_alignment_penalty())
       resistance = 36; // 25% penalty
   if(moon_bonus == "night of the eye")
       resistance = 67; // 40% bonus
   else if(moon_bonus == "maximum")
       resistance = 60; // 25% bonus
   else if(moon_bonus == "partial")
       resistance = 52; // 10% bonus
   else if(moon_bonus == "none")
       resistance = 48; // base protection
   else if(moon_bonus == "partial_penalty")
       resistance = 43; // 10% penalty
...
check_valid_action(object caster, mixed * targets, string arg, int execute = 0)
{
    ...
    {
        // Allow spell to be renewed if active
        if(!("/d/Krynn/guilds/sohm/spells/protect_magic"->has_spell_object(caster)))
        {
