...
#define SOHM_BLUR_EFFECT "_sohm_blur_effect"
...
create_spell_object()
{
    set_name("_blur_object_");
    set_short("blur spell object");    
    set_long("This is the standard evade/blur spell's object. It allows casters to "
        + "dodge a certain amount of hits.\n");

    add_prop(OBJ_S_WIZINFO, "This is a spell object " +
                            "created by the blur spell.\n");
    set_spell_effect_desc("blur");    
}
...
hook_spell_effect_started()
{
    ...
    if (objectp(target))
    {
        target->catch_tell("Your outline becomes blurred, shifting and wavering... " +
           "making it more difficult for an opponent to hit you.\n");
        tell_room(environment(target), QCTNAME(target) + "'s outline becomes blurred, "
            + "shifting, wavering... making it more difficult for an opponent " +
            "to hit.\n", ({ target }));
    } 

    target->add_prop(SOHM_BLUR_EFFECT, 1);   
}
...
hook_spell_effect_ended()
{
    ...
    if (objectp(target))
    {
        target->catch_tell("Your outline becomes distinct again as your blur " +
           "spell ends.\n");
        tell_room(environment(target), QCTPNAME(target) + " outline "
            + "becomes more distinct.\n", ({ target }));
    }

    target->remove_prop(SOHM_BLUR_EFFECT);
}
...
hook_spell_effect_warn_fading()
{
    ...
    if (objectp(target))
    {
        target->catch_tell("You feel your outline becoming more distinct as " +
             "your blur spell begins to wear off.\n");
    }
}
...
{
    if (for_obj == on)
        return  "Your outline is blurred, shifting and wavering.\n";
    else
        return capitalize(on->query_pronoun()) + " looks blurred, " +
           on->query_possessive()+ " outline shifting and wavering.\n";
...
hook_describe_evade_effect(object player, object attacker, int attack_id)
{
    player->catch_msg(QCTNAME(attacker) + " misses you with "
        + attacker->query_possessive() + " attack as your outline " +
        "shifts and wavers confusingly.\n");
    attacker->catch_msg(QCTPNAME(player)+" form shifts and wavers, " +
    ...
    tell_room(environment(player), QCTNAME(attacker) + " tries to "
            ...
            + QTPNAME(player) + " form shifts and wavers confusingly.\n", 
