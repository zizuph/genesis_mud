 ...
 *
 * Ckrik 05/28/2022
 *     - Added update to combat object so that the unarmed bonuses
...
private void
update_cb_object(object target)
{
    if (objectp(target))
    {
        object combat_obj = target->query_combat_object();
        combat_obj->cb_configure();
    }
}
...
hook_spell_effect_started()
{
    ...
    // Ckrik: Because the attack in the combat object is not updated when
    // the skill is added we have to update the combat object
    update_cb_object(effect_owner);
...
hook_spell_effect_ended()
{
    ...
    // Ckrik: Because the attack in the combat object is not updated when
    // the skill is removed we have to update the combat object
    update_cb_object(effect_owner);
