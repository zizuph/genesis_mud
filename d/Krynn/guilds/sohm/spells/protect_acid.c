/*
 * Protection from acid spell for SOHM
 *
 * Coded by Arman 2016.
 */
...
{
    set_spell_name("trothuvelucal");
    set_spell_desc("Minor protection, acid - an acid protection spell");
    ...
    set_spell_object(SOHM_SPELL_OBJS + "protect_acid_obj");

    // single resistance, caid of 40
    add_resistance(MAGIC_I_RES_ACID, 48);
...
query_spell_can_be_learned(object player)
{
    ...
    setuid();
    seteuid(getuid());
    return SOHM_MANAGER->query_memorised_spell(player,"trothuvelucal_memorised");
}
...
concentrate_msg(object caster, mixed * targets, string arg)
{
    ...
    object target = targets[0];
        ...
        caster_message = "Your eyes narrow in concentration as you begin reciting " +
            "ancient words of magic, a ward of protection from acid.\n";
...
hook_describe_cast_resistance(object caster, object * targets)
{
    ...
    if (IN_ARRAY(caster, targets))
    {
        ...
        {
            caster->catch_tell("You cast an acid protection spell upon "
                + "yourself and " + COMPOSITE_LIVE(message_targets) + ".\n");
        }
        else
        {
            caster->catch_tell("You cast an acid protection spell " +
                  "upon yourself.\n");
        }
    }
    else
