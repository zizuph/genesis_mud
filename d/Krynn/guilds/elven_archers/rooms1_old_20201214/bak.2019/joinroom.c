...
set_up_skills()
{
    ...
    sk_add_train(SS_TRACKING, "follow the trail left by creatures", "tracking", 0, 85);
    sk_add_train(SS_HUNTING, "hunt game", "hunting", 0, 85);
...
{
    string  name;
    int     max_skill, value, guru;

    name        = this_player()->query_real_name();
    value       = OPTIONS_MANAGER->query_option(snum, name);
    max_skill   = min(::sk_query_max(snum, silent), value);

    //if (calling_function() != "add_skill_gain")
    //    return min(max_skill, 60);
    
    if (snum != SS_WEP_MISSILE)
        return max_skill;
    
    value = OPTIONS_MANAGER->query_option("LoreMasterTraining", name);
...
can_train_this_skill(string skill, int step)
{
    ...
    if (this_player()->query_guild_name_occ() == GUILD_NAME || 
        (sk_find_skill(skill) != SS_DEFENCE &&
         sk_find_skill(skill) != SS_APPR_MON))
        return 1;
        
    write("Only occupational archers can train this skill.\n");
    return 0;
}
