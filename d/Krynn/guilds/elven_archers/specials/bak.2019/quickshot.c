                ...
                int dt, int phit, int dam, mixed *items, int result)
{
    ...
    switch(phurt)
    {
        ...
        if (enemy->query_humanoid())
        {
            ...
            others_msg = ({ 
                ...
                + "ground, desperately clutching " + enemy->query_possessive()
                + " throat",
                ...
                + "desperately clutching " + enemy->query_possessive()
                + " throat"
            });
        }
        else
    ...
    if (dam < 1 && add_skill_gain(archer, SS_WEP_MISSILE))
        archer->catch_msg("Your recent failures have taught you an important "
        + "lesson in archery.\n");
...
{
    int gain, result, modifier = 100;
    
    /* Maxed at 50 caid for layman members */
    if (this_player()->query_guild_name_lay() == GUILD_NAME)
        modifier = 77;
    
    gain = max(10, query_skill_gain(this_player(), SS_WEP_MISSILE));
    
    /* Base the strength of the special on amount of times used */
    result = (modifier * gain) / 100;

    /* Cap the special if in darkness, due to passive nightvision */
    if (environment(this_player())->query_prop(OBJ_I_LIGHT) < 1 &&
        this_player()->query_prop(LIVE_I_SEE_DARK) == 1)
        result = min(result, modifier * 3 / 5);
    
    ARCH_DEBUG("Quickshot query_skill_gain level: " + gain + ".\n");
    ARCH_DEBUG("Quickshot caid modifier: " + result + ".\n");
...
query_tohit()
{
    int value = query_skill_gain(this_player(), SS_WEP_MISSILE);
