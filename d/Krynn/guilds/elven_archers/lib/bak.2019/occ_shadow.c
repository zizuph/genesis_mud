...
{
    int hiding, retval, val, bval, did_sneak, gain = 0;
    object me;
    ...
    if (!random(M_FUNC->query_skill_gain(me, SS_SNEAK)))
        gain += M_FUNC->add_skill_gain(me, SS_SNEAK);
    
    if (!random(M_FUNC->query_skill_gain(me, SS_HIDE)))
        gain += M_FUNC->add_skill_gain(me, SS_HIDE);
    
    if (gain > 0)
        tell_object(me, "Your feel like moving stealthily has become a little "
        + "more effortless.\n");
