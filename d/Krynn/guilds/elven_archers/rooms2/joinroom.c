...
/* Modified by Vitwitch to warn guild members who have
   turned evil that they must regain the light before
   they are welcome in the guild rooms --- Dec 2020 */
    ...
    else
    {
        ...
        switch (result)
        {
            ...
            this_player()->catch_msg(QCTNAME(trainer)+" whispers to you: " +
                ...
                "All archers of a good nature are ever welcome.\n");
...
remind(object player)
{
    ...
    reminder = "You remember that you can <climb up> " +
    "from here to get home again. All archers "+
    "of a good nature are welcome.\n";

    player->catch_tell(reminder);
}
...
/**
 * The function that lets the player climb up from the ground
 */
int climb_up(string args)
{
    if(!args)
    ...
    TP->move_living("M", ROOMS + "fort_westentr", 1, 0);
    return 1;
}
