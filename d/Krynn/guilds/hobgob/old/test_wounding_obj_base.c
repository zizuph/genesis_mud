...
hook_add_wounding_effect(object target)
{
    ...
    tell_room(environment(target), QCTNAME(target)+ " bleeds from a " +
        "wound on " +HIS(target)+ " " +query_wounding_hitloc_desc() + 
        ".\n", ({ target }), target);
...
special_damage(int damage)
{
    ...
    DEBUG("Health pre-wound: " +current_health+ ".");
    ...
    DEBUG("Health post-wound: " +current_health+ ".");
...
public int add_wounding_effect(object target)
{
    ...
    if (target && living(target))
    {
        ...
        {
            send_debug_message("base_wounding", 
                               "Did not find shadow, creating shadow.");
            clone_object(WOUNDING_SH)->shadow_me(target);
        }
        else
        {
