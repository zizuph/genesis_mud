...
{ 
    ...
    if (promoted != MANAGER->query_promote(shadow_who))
    {
        if (promoted > MANAGER->query_promote(shadow_who))
            shadow_who->catch_msg("You have been demoted, your " +
            "esteem in the Inventors society has fallen.\n");
        else
            shadow_who->catch_msg("You have been promoted, your " +
            "esteem in the Inventors society has risen.\n");
        
        promoted = MANAGER->query_promote(shadow_who);
        shadow_who->set_skill(SS_PROMOTE, promoted);
    }
    
    setuid();seteuid(getuid());
    
    if(MANAGER->query_headtin_name() == shadow_who->query_real_name())
        return titles[14];
...
{
    MANAGER->set_last_awake(shadow_who);
}
