 ...
 *  when killed by the petrified stick spell.
 *  Arman, November 2020
 */
inherit "/std/shadow";
...
make_corpse()
{
    ...
    if (!no_corpse_flag)
    {
        ...
        corpse = clone_object("/d/Krynn/throtyl/obj/petrify_corpse");
        ...
        // corpse->add_prop(CONT_I_NO_REM, "Nothing can be taken " +
        //    "from the stony corpse until it crumbles to dust!\n");
...
notify_death(object killer)
{
    ...
    tell_object(killer, "You turn " + this_object()->query_the_name(killer) + 
        " into a statue!\n");

    tell_room(environment(killer),  QCTNAME(killer) + " turned " +
