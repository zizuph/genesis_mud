...
#define KEEP_EQUIP  ({ ROPE, LANTERN, OIL, BAG })
...
do_buy(string args) 
{
    ...
    if (!args || args == "")
    {
        notify_fail("The fence says: Buy what?\n");
        return 0;
    }
...
do_list(string args)
{
    ...
    {
        ::do_list(args);
        write(" --- Standard raider equipment available by 'list equipment'\n");
        return 1;
    }
    write("This is the standard raider equipment in stock at this shop:\n\n" +
    "+-------------------------------------------------------------------+\n"+
...
do_show(string str)
{
    ...
    if (!str || str == "")
    {
        notify_fail("The fence says: Show what?\n");
        return 0;
    }
    if (::do_show(str))
	return 1;
