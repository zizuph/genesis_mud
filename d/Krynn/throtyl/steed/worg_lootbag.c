/*
Inherited by Throtyl Raider worgs to allow lootbags to be
attached to them.
...
do_empty(string arg)
{
    ...
    {
        notify_fail("You can't empty the lootbag because you're not " +
            "leading or riding the worg.\n");
        if (steed)
        {
            tell_room(E(TP), QCTNAME(TP) + " tried to empty the " +
              "lootbag on the " + steed->steed_short() + ".\n", TP);
        }
        return 0;
    }
...
do_strap(string arg)
{
    ...
    notify_fail("That worg already carries a lootbag!\n");
    if (present("lootbag"))
        return 0;
    ...
    if (!present("lootbag",TP))
        return 0;

    parse_command(arg, environment(TP), "[the] 'lootbag' 'on' %o", steed);
    ...
    {
        write ("The lootbag seems to be too heavy for the "+
            steed->steed_short()+" to carry.\n");
        saddlebag=0;
        return 0;
    }

    write ("You strap the lootbag to the " + steed->steed_short() + "'s back.\n");
    say (QCTNAME(TP) + " straps a lootbag to the back of the " +
       steed->steed_short() + ".\n");
...
additional_longdesc()
{
    ...
    return capitalize(TO->query_pronoun())+" has a lootbag strapped to "+
...
do_unstrap(string arg)
{
    ...
    if (!query_valid_person())
    {
        notify_fail("You failed to unstrap the lootbag because you're not " +
            "leading or riding the steed.\n");

        if (steed)
        {
            tell_room(E(TP), QCTNAME(TP) + " tried to unstrap the lootbag " +
...
do_open(string arg)
{
    ...
    {
        write ("The lootbag is already open!\n");
        return 1;
    }

    saddlebag->add_prop(CONT_I_CLOSED, 0);

    write ("You open the lootbag on the "+
        steed->steed_short()+"'s back.\n");

    say (QCTNAME(TP)+" opens the lootbag on the "+
       steed->steed_short()+"'s back.\n");
...
raidloot(string arg)
{
    ...
    if (!query_valid_person())
    {
        notify_fail("You can't check the lootbag because you're not " +
            "leading or riding the worg.\n");
    ...
    if (!sizeof(conts))
    {
        notify_fail("Your lootbag is empty.\n");
        return 0;
    }
