...
create_door()
{
   ...
   set_door_name(({"flat iron grill","grill", "iron grill",
       "prison cell door", "cell door", "grille"}));
   set_door_desc("A flat iron grill set into a stone wall. "+
      "There is a lock on it.\n");
   set_other_room(ROOM + "cell.c");
     set_open(0);
   set_locked(1);
   //   set_open_desc();
   //  set_closed_desc();
   ...
   set_lock_mess(({"turns a key in the lock of the prison cell door.\n",
            "You hear a click from the lock of the prison cell door.\n"}));
