 ...
 *
 * This weapon turned out to be worthless due to other axes being far
 * better, having a weak special, and not working outdoors or in darkness.
 * Improved special. Increased non-stacking cold protection. Reduced dull
 * chance a tad more.
 *
 * Mortis 09.2014
 */
...
create_weapon()
{
    ...
    set_likely_dull(6);   // Decr. chance to dull due to shadowstuff infusion.
    ...
    + "cold protections when wielded.\nOffers non-cumulative 40% magic cold "
    + "damage resistance.\n");
        ...
        int hid)
{
    ...
    if (!objectp(wielder)) // Milan
    { return ret; }        // Milan

    // Check 50% special first to minimize processing.
    if (random(100) < 50)
    {
