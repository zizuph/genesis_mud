 ...
 * Mortis 12.2007
 *
 * The weapon is not desirable enough to be used with nonmagic stats, so I
 * made the user's club skill slightly more important in determining trips,
 * and I added 4 seconds onto the trip duration. Also gave it minor
 * enchantment. Fixed add_attack_delay which wasn't working.
 *
 * Mortis 09.2014
 */
...
create_weapon()
{
    ...
    add_prop(MAGIC_AM_ID_INFO, ({"You gather that the war-chain has been "
    + "enchanted with anaconda essence to hit more easily and wrap around "
    + "opponents' legs.\n", 20}));
    add_prop(OBJ_S_WIZINFO, "The war-chain hits more easily and naturally "
    + "pulls opponents off their feet after an opposed skill check.\n");
...
did_hit(int aid,string hdesc,int phurt,object enemy,int dt,int phit,int dam)
{
    ...
    if (hdesc == "legs")
    {
        int cskill = wielder->query_skill(SS_WEP_CLUB) / 2,
        ...
        if (uaskill >= 40 + random(40))
        {
            ...
            + "pulling " + HIM(enemy) + " off " + HIS(enemy)
            + " feet!\nSomehow " + HE(enemy)
        ...
        // Enemy jerked off his feet for 7-10 seconds.
        ...
        + HIM(enemy) + " off " + HIS(enemy) + " feet!\n", enemy);
        enemy->add_attack_delay(triptime, 0);
        set_alarm(9.0, 0.0, "get_back_up", enemy);
        inuse = "in use";
        return 1;
    }
