...
{
    string desc = "";
	string road = "  Old Circle runs west towards the southern city gates "
	+ "on Aesthetics row and east along the curve of the city wall "
	+ "closer to Knight's High road.  Muest street curves off to the "
	+ "north, and the city wall towers above you to the south.\n";
...
create_palan_room()
{
    SHORT("On Old Circle in the southeast of the city");
    LONG("@@get_time_desc@@");
	...
	+ "its crenallated defenses you spot cracks among the dark grey rocks "
	+ "which comprise its core.  The wall is in half-decent shape.\n");

	EXIT(MONKS + "a24", "north", 0, 0);
	EXIT(MONKS + "w13", "east", 0, 0);
	EXIT(MONKS + "w15", "west", 0, 0);
