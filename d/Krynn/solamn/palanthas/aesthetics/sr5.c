...
void create_palan_room()
{
    ...
    SHORT("On Aesthetics Row inside the southern city gates");
    LONG("@@get_time_desc@@");
	...
	+ "its core.  The wall is in fair repair.\n");


	EXIT(MONKS + "sr4", "north", 0, 0);
	EXIT(MONKS + "w15", "east", 0, 0);
	EXIT(MONKS + "w16", "west", 0, 0);

    gate = clone_object(MONKS + "doors/city_sgate1.c");
	gate->move(TO);
