/*
 * Hemp rope sold at general eq store in Aesthetics
 * quarter of Palanthas.  A grapple can attach to it.
 * Holdable item that adds climb skill.
 *
 * Mortis 05.2006
 *
 * Added a public query function to check whether the rope
 * has a grapple. Also added name "_20_meter_rope" for 
 * querying purposes.
 *
 * Arman 30.03.2017
 */
...
{
	if (have_grapple == 1)
		return "long hemp rope with grapple";
	
	return "long hemp rope";
}
...
{
	if (have_grapple == 1)
		return "This is a twenty meter length of thick, ultra-strong, brown "
		+ "hemp rope with a grapple fastened to one end.  Although heavy, it "
		+ "would be very helpful in climbing when held.\n";

	return "This is a twenty meter length of thick, ultra-strong, brown "
	+ "hemp rope.  Although heavy, it would be helpful in climbing when "
	+ "held.\n";
}
...
add_grapple()
{
	...
	+ "has been secured to the end of the long hemp rope.  You can "
	+ "<unfasten> it.\n");
...
unfasten(string str)
{
	...
	if (!parse_command(str, ({}), "[iron] [my] [the] [a] 'grapple' [from] [long] [hemp] [rope]"))
		return 0;
