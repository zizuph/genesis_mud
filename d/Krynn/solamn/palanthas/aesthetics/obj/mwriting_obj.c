/* 
 * Taken from the Cabal of Hiddukel chalk.  This is the
 * text object that is written on the ground somewhere.
 *
 * Adapted for sale in the general EQ store of the
...
create_object()
{
	...
	set_long("@@my_long");
	add_prop(OBJ_M_NO_GET, "Take the writing?  Sure, keep trying.\n");
}
...
erase(string str)
{
	...
	say(QCTNAME(TP) + " bends down and wipes across the " + where + ".\n");
...
maybe_remove_this_writing()
{
	...
	my_message = my_message[0..let] + " "
				 + my_message[(let + 2)..strlen(my_message)];
