...
get_time_desc()
{
	...
	+ "not come here.  An alley between houses leads south to Old Circle "
	+ "where the city wall towers above the homes.\n";
...
create_palan_room()
{
	...
	EXIT(MONKS + "a18", "east", 0, 0);
	EXIT(MONKS + "w17", "south", 0, 0);
	EXIT(MONKS + "a20", "west", 0, 0);

	clone_object(MONKS + "doors/a19_door_out")->move(TO);
}
