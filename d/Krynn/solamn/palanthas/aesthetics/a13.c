...
get_time_desc()
{
	...
	+ "must not come here.  An alley leads north to Palanthas Circle.\n";
...
create_palan_room()
{
	...
	ITEM(({"sw alley", "southwest alley"}), "An alley leads southwest "
	+ "between the houses towards the center of Jems court.\n");
	...
	add_alley("sw", "southwest", MONKS + "alley053");

	EXIT(MONKS + "a08", "north", 0, 0);
	EXIT(MONKS + "a14", "east", 0, 0);
	EXIT(MONKS + "a12", "west", 0, 0);

	clone_object(MONKS + "doors/a13_door_out")->move(TO);
}
