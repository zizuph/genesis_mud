/* Mortis 12.2005 
 * Housing smells?
 * Desc fix and roads outside town.
 */

#include "../local.h"
#include CLOCKH

#define HERBTRAIL_START "/d/Krynn/solamn/pal_herbtr/palherbtr6"
...
{
    string desc = "";
	string road = "  Vingaard road heads northeast into Palanthas. "
	+ "A road curves along the outside of the city wall to the northwest "
	+ "and southeast.\n";
...
create_palan_room()
{
    ...
    SHORT("On Vingaard road outside the city gates");
    LONG("@@get_time_desc@@");
	...
	+ "large block panels of polished, translucent marble.  The wall is "
	+ "in good repair.\n");

//	EXIT(MONKS + "outx", "southeast", 0, 0);
//	EXIT(MERCH + "outy", "northwest", 0, 0);

	gate = clone_object(MONKS + "doors/city_swgate2.c");
	gate->move(TO);

        add_exit(HERBTRAIL_START,"widdershins",0);
