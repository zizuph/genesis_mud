 ...
 *
 * Mortis 09.2014 - Increased stats slightly. Increased fatigue and mana
 * damage significantly to achieve originally intended results.
 *
 * Mortis 11.2014 - Fixed unwield bug behavior on death.
 *
 * Arman 06.2016 - Added task hint for SoHM spell.
 */
...
create_brix_ganger()
{
    ...
    set_stats((({155, 150, 205, 155, 195, 155})), 2);
    ...
    add_ask(({"hobgoblin","hobgoblin chief","surly hobgoblin chief",
              "grumnish", "foul grumnish"}), VBFC_ME("hobgoblin_hint"));
    add_ask(({"dougan","dougan redhammer"}), VBFC_ME("dougan_hint"));
    add_ask(({"lucky dice","lucky onjat dice"}), VBFC_ME("dice_hint"));
...
special_attack1(object enemy)    // Neck/shoulder chop -- Fatigue
{
    ...
    kfatigue = kfatigue * 2; // Doubling strength of fatigue damage.
...
special_attack2(object enemy)    // Double fisted backhand -- Mental drain
{
    ...
    kmanadrain = kmanadrain * 10; // Increasing this by 10 to a noticeable drop.
...
/* Unwield all to make sure skill enhancing weapons he may be wielding don't
 * act buggy. */
void
do_die(object killer)
{
    command("unwield all");
    command("emote drops to his knees groggily.");
...
dougan_hint()
{
    if(!this_player()->query_prop(SOHM_DOUGAN_TASK))
    {
        command("say Dougan Redhammer?");
        command("emote narrows his eyes suspiciously.");
        command("say Nope, never heard of him.");
        return "";
    }

    command("say Dougan Redhammer? That dwarven fraud?");
    command("cackle");
    command("say I haven't that old rogue in these quarters " +
       "since he lost all his coin - and his lucky dice! - " +
       "to old foul Grumnish!");
    return "";
...
hobgoblin_hint()
{
    ...
    {
        command("emote narrows his eyes suspiciously.");
        command("say Can't help you with that.");
        return "";
    }

    command("say Old foul Grumnish is the hobgoblin chieftain " +
       "that lairs in a cave by a lake in the mountains just outside of " +
       "Palanthas.");
    command("say He comes in to Jems court for a game of onjat now " +
       "and then...");
    command("say A few months back I had a cracker of a game against " +
       "him and the dwarven rogue Dougan Redhammer.");
    command("twinkle");
    command("say By the end of the night, I had all the gold, Grumnish " +
       "had Dougan's lucky onjat dice, and Dougan had a nasty hangover!");
    command("laugh");
    this_player()->add_prop(SOHM_KIRK_TASK, 1);
    return "";
...
dice_hint()
{
    ...
    command("say Dougan Redhammer lost his lucky onjat dice to " +
       "foul Grumnish, the hobgoblin chieftain.");
    command("say He was pretty bitter about that!");
    command("twinkle");
