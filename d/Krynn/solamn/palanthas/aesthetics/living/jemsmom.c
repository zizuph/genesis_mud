...
int gender = G_FEMALE;
string adj1 = one_of_list(({"thin", "hollow-cheeked", "weary", "shapely",
		"young", "smiling", "proud", "tall", "short", "old"})),
	   adj2 = one_of_list(({"bustling", "wide", "simply-dressed",
		"pony-tailed", "short-haired", "long-haired", "fretting",
		"nervous", "smooth-skinned", "pale", "rough-skinned"})),
	   myshort;
...
create_brix_citizen()
{
	...
	set_stats((({14, 15, 27, 17, 22, 18})), 40);
	...
	add_prop(WHAT_BRIX_AM_I, "woman");
	add_prop(WHAT_OCCU_AM_I, "woman");
...
{
	if (random(3) == 1)
		MONEY_MAKE_SC(random(4))->move(TO);

	MONEY_MAKE_CC(12 + random(14))->move(TO);
}
