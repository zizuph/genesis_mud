...
{
    string desc = "";
	string road = "  Jems court curves in an oval east and west from here.  "
...
create_palan_room()
{
	...
	ITEM(({"n alley", "north alley"}), "An alley leads north around the left "
	+ "of the house to your north to the center of Jems court.\n");
	...
	add_alley("n", "north", MONKS + "alley053");

	EXIT(MONKS + "a19", "east", 0, 0);
	EXIT(MONKS + "a21", "west", 0, 0);

	clone_object(MONKS + "doors/a20_door_out")->move(TO);
}
