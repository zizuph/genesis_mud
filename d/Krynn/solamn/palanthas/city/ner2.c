...
get_time_desc()
{
	...
	+ "northeast the road continues on between the noble and shipping "
	+ "districts.\n";
...
{
    SHORT("On Scholar's Walk northeast of the palatial plaza");
    LONG("@@get_time_desc@@");

	EXIT(ROOM + "ner1", "southwest", 0, 0);
	EXIT(ROOM + "ic2", "northwest", 0, 0);
	EXIT(NOBLE + "ner3", "northeast", "@@tell_watchers", 0);
	EXIT(ROOM + "ic3", "southeast", 0, 0);
...
tell_watchers()
{
	...
	tell_room(NOBLE + "alley005", "Below you on Scholar's Walk " + walker
		+ " passes by heading northeast.\n");
	return 0;
}
