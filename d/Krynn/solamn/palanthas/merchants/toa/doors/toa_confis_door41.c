/*
 * Fourth floor outside door to the confiscated item room
 * in the Tower of Arms in  Palanthas.
 * Very good lock. Key does not exist.
 * 
 * Mortis 09.2014
 */
...
create_door()
{
    set_door_desc("This bolted, iron door leads east out through the marble "
    + "wall. It has a burnished iron handle with a small keyhole.\n");
    set_door_id("toa_confis_door");
    set_pass_command(({"east","out", "door"}));
    set_door_name(({"door"}));
    set_other_room(TOA + "toa66.c");
    set_open(0);
    ...
    set_key("toa_confis_door");
    set_pick(70);
    set_lock_name("toa_confis_door");
}
