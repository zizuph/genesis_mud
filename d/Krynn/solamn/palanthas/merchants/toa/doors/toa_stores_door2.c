/*
 * Insside door to the storeroom of the Tower of Arms in Palanthas.
...
create_door()
{
    set_door_desc("This bolted, iron door leads west out into the marble "
    + "wall. It has a burnished iron handle with a small keyhole.\n");
    set_door_id("toa_store_door");
    set_pass_command(({"west","out", "door"}));
    set_door_name(({"door"}));
    set_other_room(TOA + "toaL01.c");
    set_open(0);
    ...
    set_pick(51);
    set_lock_name("toa_store_door");
}

string
relock_door()
{
    set_alarm(60.0, 0.0, "reset_door");
