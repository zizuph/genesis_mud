...
create_palan_room()
{
    ...
    LONG("A corridor with a stairway descending and ascending from here. The "
    + "chink of armour and clump of bootsteps echo from the corridors.\n");

    ITEM(({"stair", "stairs", "flight", "flights", "flight of stairs",
      "stairway"}), "A stairway descends and ascends from here. The steps "
    ...
    EXIT(TOA + "toa22", "east", 0, 0);
    EXIT(TOA + "toa28", "west", 0, 0);
    EXIT(TOA + "toa04", "down", 0, 0);
    EXIT(TOA + "toa41", "up", 0, 0);
