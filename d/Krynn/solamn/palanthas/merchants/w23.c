...
{
    string desc = "";
	string road = "  Old Circle curves north to the city gates on Merchants' "
	+ "Way and south along the curve of the city wall through a housing "
	+ "district.  A large residence is to the east, and to the west the city "
	+ "wall towers high above.\n";
...
reset_palan_room()
{
	if (!wguard)
	{
		...
		wguard->set_monster_home(MERCH + "w23");
...
create_palan_room()
{
    SHORT("On Old Circle south of Merchants' Way");
    LONG("@@get_time_desc@@");
	...
	+ "quarter, a row of good sized houses runs south near "
	...
	EXIT(MERCH + "wr5", "north", 0, 0);
	EXIT(MERCH + "w22", "south", 0, 0);
