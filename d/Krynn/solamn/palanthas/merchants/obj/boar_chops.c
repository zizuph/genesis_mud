...
create_food()
{
    ...
    set_name("slice");
    set_pname("slicess");
	add_name(({"meat", "chops", "boar", "pork"}));
	add_pname(({"meat", "chops", "boars", "porks"}));
    set_adj(({"roast"}));
	add_adj("whole");
	set_short("slice of roast boar chops");
	set_pshort("slices of roast boar chops");
    set_long("This a pale brown boar's chops from a whole, roast "
	+ "boar.  It is particularly succulent and has been sliced into "
	+ "thick chops.\n");
...
smell(string str)
{
	if (!str || !parse_command(str, ({}), "[whole] [roast] 'slice' / 'meat' / 'chops' / 'boar' / 'pork'"))
	return 0;
