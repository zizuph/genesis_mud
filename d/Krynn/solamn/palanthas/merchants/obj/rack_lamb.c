...
create_food()
{
	...
	add_pname("racks");
    set_adj(({"hot"}));
	set_short("hot rack of lamb");
	set_pshort("hot racks of lamb");
    set_long("This " + "@@warm_hot@@" + " rack of lamb is dripping with its "
	+ "own juices, its dark brown skin concealing succulent meat within.\n");

	set_alarm(360.0, 0.0, "make_temp");
}
...
smell(string str)
{
	if (!str || !parse_command(str, ({}), "[hot] [warm] 'rack' / 'lamb' [of] [lamb]"))
	return 0;

    TP->catch_msg("You smell " + short() + ".  The scent of "
	+ "freshly cooked, rotisserie style lamb meat fills your senses.\n");
...
special_effect(int n)
{
	...
	+ "filling your mouth with the taste of freshly cooked, rotisserie "
	+ "style, lamb meat.");
	tell_room(E(TP), QCTNAME(TP) + " begins consuming " + HIS(TP) + " "
	+ short() + ", its juices spilling into " + HIS(TP) + " mouth and "
	+ "filling the air with the scent of freshly cooked, rotisserie style, "
	+ "lamb meat.", ({TO, TP}));
...
make_temp()
{
	remove_adj("hot");
	set_adj("warm");
	set_short("warm rack of lamb");
	set_pshort("warm racks of lamb");
	temp = 0;
	write("Your rack of lamb has cooled down a bit.\n");
	return 1;
}
