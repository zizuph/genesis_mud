...
create_food()
{
    ...
    set_name("meat");
    set_pname("meats");
	add_name(({"chicken", "slice", "thigh"}));
	add_pname(({"chickens", "slices", "thighs"}));
    set_adj(({"dark"}));
	add_adj("rotisserie");
	set_short("slice of dark thigh meat");
	set_pshort("slices of dark thigh meat");
    set_long("This is a slice of juicy dark thigh meat from a rotisserie "
...
smell(string str)
{
	if (!str || !parse_command(str, ({}), "[dark] [rotisserie] 'meat' / 'thigh' / 'chicken' / 'slice'"))
	return 0;
