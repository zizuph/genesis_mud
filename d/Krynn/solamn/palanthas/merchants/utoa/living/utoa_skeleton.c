 ...
 *
 * Temporary added crude logging at the end of the file. // Carnak
 */
...
arm_me()
{
    ...
    command("wear unusual items");
    command("wear peculiar items");
    command("wear exotic items");
...
/*
 * The following is temporary logging to see who benefits from
 * these skeletons.
 */
...
{
    if (time() - to->query_prop(LIVE_I_LAST_STEAL) <= 1)
    {
        write_file("/d/Krynn/log/thief_palanthas", ctime(time()) +
        " * name: " + capitalize(to->query_real_name()) +
        " * avg: " + to->query_average_stat() +
        " * stole: " + ob->query_name() +
        " * worth: " + ob->query_value() +
        "\n");
    }
...
{
    if (prop == LIVE_I_VICTIM_ADDED_AWARENESS)
    {
        string *caught;
        
        caught = filter(all_inventory(environment(this_object())),
                 &operator(>=)(,time()-1) @
                 &->query_prop(LIVE_I_LAST_STEAL));
        
        if (sizeof(caught))
            write_file("/d/Krynn/log/thief_palanthas", ctime(time()) +
            " * " + capitalize(caught[0]->query_real_name()) + " " +
