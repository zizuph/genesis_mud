...
{
    string desc = "";
	string road = "  Palanthas Circle curves between the arms district "
	+ "west and a grove of oaks east.  To the north it intersects with "
	+ "Merchants' Way.  The shadow from the dark grey tower rising from "
	+ "the grove seems to spread over the marble of the road keeping it "
	+ "oddly chilled.\n";
...
create_palan_room()
{
	...
	ITEM(({"oak", "oaks", "tree", "trees", "grove", "shoikan grove"}),
	  "East is a grove of oaks, much larger than most you've seen, "
	...
	EXIT(MERCH + "wr3", "north", 0, 0);
	EXIT(MERCH + "m13", "south", 0, 0);
