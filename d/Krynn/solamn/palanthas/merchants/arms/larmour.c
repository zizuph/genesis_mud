 ...
 *
 * 09.2006 - Added my_short() to build the shorts differently by type.
 *
 * 09.2014 - Added Terellian Silver Forest cold protection to fur-lined
 ...
 * - Added proper armour recovery
 * 2021-08-08 - Cotillion
 * - Some fixes to cut memory use
 */
...
#define LINER_LOC            MERCH + "robemaker"
...
set_armour_data(string data)
{
    ...
    /* This has been deliberately put here for memory use */
    ...
    mapping name_to_desc2 = (["suit":"suit consists of a shirt with "
        ...
        "liner":"liner looks as if it needs to be attached within the folds "
            + "of a cloak or robe for added protection with <sew>.  It "]);
...
my_short()
{
    mapping name_to_desc3 = (["suit":"suit", "coat":"coat",
        "greaves":"greaves", "leggings":"leggings",
        "breastplate":"breastplate", "cuirasse":"cuirasse", "vest":"vest",
        "bracers":"bracers", "pauldrons":"pauldrons",
        "sleeves":"sleeves", "gloves":"gloves", "armet":"armet",
        "cap":"cap", "helmet":"helmet", "boots":"boots",
        "tall-boots":"tall boots", "belt":"belt", "liner":"liner"]);

    string art3 = name_to_desc3[art];

    switch (art)
    {
        case "suit":
        case "coat":
            return cusshort + colmore + " " + art3 + " of " + matmore
                   + " armour";
            break;
        case "greaves":
        case "leggings":
        ...
        case "boots":
        case "tall-boots":
            return cusshort + colmore + " pair of " + matmore + " " + art3;
            break;
        case "pauldrons":
            return cusshort + colmore + " set of " + matmore + " " + art3;
            break;
        default:
            return cusshort + colmore + " " + matmore + " " + art3;
            break;
    }
}
...
wear()
{
    if (art == "liner")
    {
        ...
        + "cloak or robe, friend. Maybe the Robemaker of Palanthas?\n");
        return -1;
    }
...
init()
{
    ...
    /* Cut down on action spam in racks */
...
sew(string str)
{
    string robename, robeshort;
    object *robe, linerloc, *daiv;

    if (!IN_ARRAY(art, ({"liner"})))
        return 0;

    NF("Sew what into what?\n");
    if (!str)
        return 0;

    NF("Sew liner into what?\n");
    if (sscanf(str, "liner into %s", robename) != 1)
        return 0;

    if (!(robe = FIND_STR_IN_OBJECT(robename, TP)) || !sizeof(robe))
        return 0;

    NF("That item is not designed to have a liner sewn into it.\n");
    if (!(robe[0]->query_prop(LINERABLE)))
        return 0;

    NF("That article already has a liner sewn into it.\n");
    if (robe[0]->query_prop(HAVE_LINER) == "on")
        return 0;

    linerloc = find_object(LINER_LOC);

    NF("You need someone to help you sew the liner in, perhaps a robe "
    + "merchant like the one in Palanthas?\n");
    if (E(TP) != linerloc)
        return 0;

    NF("You're in the right place, but there's no one here to help you at "
    + "the moment.\n");
    if (!(daiv = FIND_STR_IN_OBJECT("daiv", linerloc)) || !sizeof(daiv))
        return 0;

    robeshort = robe[0]->query_short();

    robe->add_prop(HAVE_LINER, "on");
    robe->set_the_ac();
    robe->add_liner_long(cusshort, col, matmore);
    daiv->command("say A Daronius made liner, very nice!");
    daiv->command("emote loans out a needle, thread, and a few studs "
    + "to secure the liner.");
    write("You work the liner into place inside your " + robeshort
    + " and sew it into place before returning the needle.\n");
    say(QCTNAME(TP) + " works " + HIS(TP) + " liner into place inside "
    + HIS(TP) + " " + robeshort + " and sews it into place before "
...
query_recover()
{
    return MASTER + ":" + query_arm_recover() + query_keep_recover() + "|LA|" + recover_string + "|";
}
...
init_recover(string arg)
{
    ...
    /* This attempts to support old recover strings */
    if (sscanf(arg, "%s|LA|%s|%s", foobar, data, foobar))
