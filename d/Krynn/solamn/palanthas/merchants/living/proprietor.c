 ...
 *
 * 03.2006 Edited by Mortis for content and to fit
 * in upgraded Palanthas.
 *
 * 2008-03-25 Edited by Lunatari, fixing some quest info that
 * was not updated to fit the new Palanthas.
 *
 */
#include "/ss_types.h"
#include <macros.h>
...
int has_done_all_quests(object pers);
void
reset_monster()
{
    object barkeeper;
...
{
    set_name("leiden");
	set_living_name("leiden");
    set_race_name("human");
    set_adj("corpulent");
    add_name("proprietor");
	set_gender(G_MALE);
	set_short("corpulent curled-lipped male proprietor");
    set_long("With a sniveling, curled lip, triple-chin, and gobs of "
	+ "gaudy jewelry, the proprieter appears to enjoy indulging himself "
	+ "in both material possessions and prolific consumption.\n");
    set_title("Proprietor of the Silver Dragon Inn");

    set_stats(({60, 34, 100, 80, 81, 92}));

    set_size_descs("of normal length", "very fat");
    set_appearance(2);

    set_alignment(100);
    set_knight_prestige(-50);

    set_default_answer("@@excuse_me");
    add_ask("[for] [a] 'job' / 'task' / 'work' / 'jobs'", "@@offer_job");
    add_ask("[for] 'reward'", "@@reward_job");
    add_ask("[for] 'help'", "say I'm extremely busy. I have to "
	+ "finish balancing my books.", 1);
    add_ask(({"garden", "gardens"}), "say Well, I suppose I can trust you. "
	+ "The garden is not far from here in the Gallant Field Woods, but "
        + "don't tell anyone. Anyway, I am sure an adventurer of your "
        + "calibre can find it without a map.", 1);
    add_ask(({"moor", "moors"}), "say The moors are just outside the city "
	+ "to the west. Perhaps if you go west, you can find them.", 1);
    add_ask(({"woods", "gallant field", "gallant field woods"}), "asay "
	+ "scorn Well, you use the alleys of course.", 1);
    add_ask(({"alley", "alleys"}), "asay laugh Now don't tell me you don't "
	+ "know how to use alleys! Why, I bet you haven't even bought a "
	+ "map from Edgus!", 1);
    add_ask(({"map", "maps", "edgus"}), "asay angr Go find out for yourself! "
	+ "I don't pay people to waste my time.", 1);
...
reward_job()
{
    ...
    if (number_of_job)
    {
        ...
        else if (number_of_job == 3)
        {
            ...
            {
                barkeeper = (MERCH + "inn1")->query_barkeeper();
...
reward(object pers, int exp)
{
    ...
    {
        if (exp == -1)
            write("This doesn't seem to be an authorization form with my "
			+ "name on it.\n");
        else if (exp == -2)
            write("The persons who signed this form don't seem to be "
			+ "authorized to do so.\n");
        else if (exp == -3)
            write("");
        else
        ...
        say(QCTNAME(pers) + " tried to get a reward but was not worthy of "
		+ "one.\n");
        return "";
    }
...
offer_job()
{
    if (TP->query_prop(PLAYER_I_ON_INN_QUEST))
    {
        ...
        if (has_done_quests(TP) == 0)
            command("say I need some hop and malt from my garden in the "
                + "Gallant Field Woods here in Palanthas. If you "
                + "are in trouble finding it then I would suggest "
                + "that you look in the alleys.");
        else if (has_done_quests(TP) == 1)
            command("say You are supposed to bring me some peat from the peat "
		+ "cutter on the moors just west outside of Palanthas.");
        else
