...
{
    string desc = "";
	string road = "  Palanthas Circle curves north between a shopping "
	+ "district west and a thick wood east from which rises a squat, square "
	+ "tower of marble.  Three long flights of marble steps climb northeast "
	+ "up a short hill to a marble road leading east into the wood.  To the "
	+ "south the circle intersects with Merchants' Way.\n";
...
create_palan_room()
{
	...
	EXIT(MERCH + "m03", "north", "@@tell_watchers", 0);
	EXIT(MERCH + "m05", "northeast", 0, 0);
	EXIT(MERCH + "wr3", "south", 0, 0);
...
tell_watchers()
{
	...
	tell_room(MERCH + "m05", "Below you on Palanthas Circle, " + walker
		+ " passes by on " + hiswalker + " way north.\n");
	return 0;
}
