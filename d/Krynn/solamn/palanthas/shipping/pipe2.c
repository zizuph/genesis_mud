...
create_palan_room()
{
    ...
    add_exit(SHIP + "pipewe1","south",0,1);
    add_exit(SHIP + "pipeud2","up",0,5);
  ...
  if (where == "south")
    {
      ...
      TP->move_living(where + ", blown by a gust of wind", SHIP + "pipe3",0,1);
    }
  else /*where == "north"*/
    {
      write("The wind now turns and you are blown up by a strong force of wind along the shaft.\n");
      TP->move_living("up, blown by a gust of wind", SHIP + "pipe1",0,0);
    }
}
