...
{
    string desc = "";
	string road = "  Palanthas Circle curves east through side streets "
	+ "and alleys filled with warehouses and west to its intersection "
	+ "with Trade road and Bulwark Round.  Through the buildings to the "
	+ "northwest you can make out the piers and the bay.\n";
...
create_palan_room()
{
    SHORT("On Palanthas Circle east of Trade road in a warehouse district");
    LONG("@@get_time_desc@@");

	EXIT(SHIP + "warex", "north", "@@ware_closed", 0);
	EXIT(SHIP + "s09", "east", 0, 0);
	EXIT(SHIP + "warey", "south", "@@ware_closed", 0);
	EXIT(SHIP + "tr3", "west", 0, 0);
