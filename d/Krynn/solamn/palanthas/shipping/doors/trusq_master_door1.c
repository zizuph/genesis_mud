/*
 * Door to master suite on fourth floor of
...
create_door()
{
    set_door_desc("This thick, ornate oak door leads into a private suite "
	+ "in the east.  It has a polished steel handle with a small keyhole.\n");
    set_door_id("trusq_master_door");
    set_pass_command(({"east","in", "door"}));
    set_door_name(({"door"}));
    set_other_room(SHIP + "trusq_master.c");
    set_open(0);
