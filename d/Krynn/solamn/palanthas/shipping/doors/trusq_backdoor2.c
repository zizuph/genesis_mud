...
create_door()
{
    set_door_desc("This thin door is the servants exit into the side street "
	+ "south.  It has a tarnished brass handle with a large keyhole.\n");
    set_door_id("trusq_backdoor");
    set_pass_command(({"south","in", "door"}));
    set_door_name(({"door", "backdoor"}));
    set_other_room(SHIP + "alley202.c");
    set_open(0);
