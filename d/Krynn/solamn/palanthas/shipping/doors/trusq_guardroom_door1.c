/*
 * Door to private office on guardroom third floor of
...
create_door()
{
    set_door_desc("This oak door leads into a private room in the "
	+ "southeast of the room.  It has a polished brass handle with a "
	+ "keyhole.\n");
    set_door_id("trusq_guardroom_door");
    set_pass_command(({"east","in", "door"}));
    set_door_name(({"door"}));
    set_other_room(SHIP + "trusq_priv_guardroom.c");
    set_open(0);
    ...
    set_pick(38);
    set_lock_name("trusq_guardroom_door");
}
