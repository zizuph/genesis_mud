/* Mortis 12.2005 
 *
 * Arman updated to include net and spyglass seller 10.2020
 */
...
#define SEADOG_SELLER  TDIR + "shipping/living/seadog_seller"
...
reset_palan_room()
{
    if(!objectp(seadog))
    {
        seadog = clone_object(SEADOG_SELLER);
        seadog->move(TO, 1);
    }
}
...
create_palan_room()
{
    ...
    EXIT(SHIP + "s22", "northeast", 0, 0);
    EXIT(SHIP + "s12", "west", 0, 0);
