...
reset_palan_room()
{
	if (!mgr)
	{
		mgr = clone_object(SHIP + "living/trusq_worker_mgr");
		mgr->move(TO);
	}

	if (!guard2)
	{
		guard2 = clone_object(SHIP + "living/trusq_guard_ware");
		guard2->move(TO);
	}

	if (!guard3)
	{
		guard3 = clone_object(SHIP + "living/trusq_guard_ware");
		guard3->move(TO);
	}
}
...
create_palan_room()
{
	...
	+ "covered in scattered patches of sawdust, and the warehouse stretches "
	+ "further north.  In a corner in the southeast is a small desk, chair, "
	+ "and file cabinet.\n");
	...
	ITEM("desk", "Though used daily, this simple drawerless desk is dusty.\n");
	ITEM(({"chair", "file", "files", "file cabinet", "cabinet"}), "Around a "
	+ "simple desk in the southeast corner are arranged an old, wooden chair, "
	+ "and a tin filing cabinet containing inventory files.\n");
	...
	EXIT(SHIP + "ware_trusq", "north", 0, 0);
