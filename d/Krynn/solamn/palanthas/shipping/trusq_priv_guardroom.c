...
int sackhere = 0, notused = 0;
object hguard, indumb, *inv, *ppl;
...
reset_palan_room()
{
	if (!hguard)
	{
		hguard = clone_object(SHIP + "living/trusq_guard_high");
		hguard->move(TO);
	}
...
create_palan_room()
{
	...
	+ "<view street>.\n");
	ITEM(({"dumbwaiter", "small dor"}), "Above the desk in the southwest "
	+ "corner covered by a small wooden door with a keyhole is a small "
	+ "dumbwaiter used to send meals up to the higher floors by means of a "
	+ "rope and pulley system.  " + "@@is_dumb_here@@" + "\n");
	...
	offdoor = clone_object(SHIP + "doors/trusq_guardroom_door2");
	offdoor->move(TO);
...
{
    if (hguard)
      hguard->assist_lord(who, where);
}

help_i_need_help(mixed who, object where)
{  
    set_alarm(5.0, 0.0, "help_on_the_way", who, where);
}
