...
desk_search()
{
	...
	if (pouchhere == 0 && TP->query_skill(SS_AWARENESS) > 30 + random(21))
	{
		...
		pouchh->set_general_data("plain" + "&&" + pouchcol + "&&"
								+ "suede" + "&&" + "belt-pouch");
		MONEY_MAKE_PC(1)->move(pouchh);
		MONEY_MAKE_GC(12 + random(5))->move(pouchh);
		pouchh->move(TP);
		pouchhere = 1;
		return "The great, oaken desk is ornately carved with intricate "
		+ "detail and has several drawers beneath.  In one of them, "
		+ "you find a " + pouchh->short() + " and pick it up!\n";
	}
...
pick_chest(string str)
{
	...
	{
		clone_object("/d/Genesis/gems/obj/topaz_yellow")->move(TP);
		if (random(2) == 1)
			clone_object("/d/Genesis/gems/obj/topaz_yellow")->move(TP);
		if (random(3) == 1)
			clone_object("/d/Genesis/gems/obj/topaz_yellow")->move(TP);

		clone_object("/d/Genesis/gems/obj/coral")->move(TP);
		clone_object("/d/Genesis/gems/obj/coral")->move(TP);
		clone_object("/d/Genesis/gems/obj/coral")->move(TP);
		clone_object("/d/Genesis/gems/obj/coral")->move(TP);
		clone_object("/d/Genesis/gems/obj/coral")->move(TP);
		...
		MONEY_MAKE_GC(50 + random(41))->move(TP);
