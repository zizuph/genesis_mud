...
#define VIEWNORTH			(NOBLE + "n17")
#define VIEWWEST			(NOBLE + "n16")
#define VIEWEAST			(NOBLE + "w07")
...
create_palan_room()
{
	...
	+ "gates on Horizon road, <view southeast> to see what's just outside "
	+ "the tower, or <view north> to see outside the tower's entrancef.\n");

	CMD(({"north", "n"}), "view", "@@view_north");
	CMD(({"west", "w"}), "view", "@@view_west");
	CMD(({"southeast", "se"}), "view", "@@view_east");

	trapdoor = clone_object(NOBLE + "doors/tower3_trapdoor");
	trapdoor->move(TO);
...
view_north()
{
	/* This function returns an equivalent of 'look' from n17
