 ...
 * no jeweler, the store was supposed to be closed, but something
...
#define STRM   "/d/Krynn/solamn/palanthas/nobles/jewelstore"


object bodyguard;

void rescue_employer_room(object ob);
void react_steal(string mtname);
...
reset_gem_shop()
{
	if (!bodyguard)
	{
		bodyguard = clone_object(NOBLE + "living/jguard_in");
		bodyguard->move(TO);
	}
...
create_shop()
{
	...
	create_gem_shop();
	set_store_room(STRM);
	set_keeper_file("/d/Krynn/solamn/palanthas/nobles/living/jeweler_npc");

	set_shop_name("Jeweler's Shop in Palanthas");
	SHORT("Jeweler's Shop in Palanthas");
	LONG("This is the jeweler's shop. Exquisite furnishings gleam in "
    ...
    ITEM(({"sign"}), "@@exa_sign");
    ITEM(({"case", "display case", "displays", "glass case", "display" }),
        "@@do_case");
	ITEM(({"window", "windows"}), "A great window spans the southeast wall "
	+ "beside the door leading out to the street.  Another window, much "
	+ "smaller, is situated towards the back of the shop along the southwest "
	+ "wall.  It is open and letting in a gentle, salty breeze off the bay.");
	ITEM(({"small window", "smaller window"}), "An open window towards the "
	+ "back of the shop on the southwest wall looks out on Livery street "
	+ "letting in a gentle, salty breeze off the bay.");

    CMD(({"sign"}), "read", "@@exa_sign");
	CMD(({"window", "small window", "smaller window"}), "enter",
		"@@enter_window");

    EXIT(NOBLE + "n04", "southeast", "@@block_criminals", 0);
...
void
rescue_employer_room(object ob)
{
	if (!bodyguard)
		return;

	bodyguard->rescue_employer(ob);
	return;
}
...
block_criminals()
{
	...
	{
		if (random(3) == 1)
			bodyguard->command("asay deadly Palanthian Armour:  There is "
			+ "only one way you'll pass through that door, criminal.");
		bodyguard->command("emote blocks the exit to the street!");
		return 1;
	}
}
...
enter_window()
{
	TP->move_living("climbing out the window on the southwest wall",
		NOBLE + "n03", 1, 0);
	return "";
}

void
react_steal(string mtname)
{
	if (!bodyguard)
		return;

	bodyguard->command("ddraw from scabbard");
	bodyguard->command("kill " + mtname);
	bodyguard->command("asay feroc Palanthian Armour will handle this!");
}
...
buy_it(object *ob, string str2, string str3) 
{
    ...
    for (i = 0; i < sizeof(ob); i++)
    {
	  ...
	  if (error = arr[sizeof(arr) - 1])
	  {
        ...
        if (j >= 1)
            break;
	// Well, we don't want to let a player accidently buy too much. :)
    }
