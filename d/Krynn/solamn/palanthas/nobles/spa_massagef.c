/* 
 * Massages by men.  Different for men/women.
...
reset_palan_room()
{
	if (!masseur)
	{
		...
		masseur = clone_object(NOBLE + "living/masseur");
		masseur->move(TO);
	}
}
...
create_palan_room()
{
    SHORT("Women's massage chamber of a dayspa");
	...
	+ "massage table.  Beside it on the south wall is a counter containing "
	...
	+ "Beside it against the south wall is a counter topped with jars of oils "
	...
	+ "the south wall is a marble topped counter.  Rows of jars containing "
	...
	+ "beside the counter against the south wall.  It contains used towels "
	...
	EXIT(NOBLE + "spa_massage", "north", "@@towel_thief", 0);
...
read_sign()
{
	...
	cat(NOBLE + "obj/spamassagef.txt", 1, 50);
...
mread_sign()
{
	...
	string file = NOBLE + "obj/spamassagef.txt";
	...
	if(str == "up" || !str)
	{
		...
		if (TP->query_prop(TABLED))
		{
			...
			for(i=0,j=sizeof(stinkers);i<j;i++)
			{
				...
				if (k == -1)
				{
					write("A white-coated attendant arrives from the north.\n"
					+ "The white-coated attendant smiles at you as he dusts "
					+ "off the massage table.\nThe white-coated "
					+ "attendant leaves north.\n");
					say("A white-coated attendant arrives from the north.\n"
					+ "The white-coated attendant smiles at " + QCTNAME(TP)
					+ " as he dusts off the massage table.\n"
					+ "The white-coated attendant leaves north.\n");
		...
		if(TP->query_prop(TSIT))
		{
			...
			+ "masked white-coated attendant leaves stumbling north.\n");
			return 1;
		}
...
unfold(string str)
{
	...
	write("You pick up towel from the counter against the south wall and "
	+ "unfold it.  It is ready to wear now.\n");
	say(QCTNAME(TP) + " picks up a towel from the counter against the south "
	+ "wall and unfolds it.\n");
...
towel_thief()
{
	...
	if (sizeof(towels) != 0)
	{
		if (masseur)
		{
			masseur->command("say Please return your massage towel "
