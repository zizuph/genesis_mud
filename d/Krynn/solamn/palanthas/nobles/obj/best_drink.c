 ...
 *
 * Fixed alcohol reduction amount. Was only 5. Fixed time at 30s.
 * Found 2006 notes in conjunction with Udana (silver forest coder) to
 * have coffee reduce cold by 2. Added this effect in.
 *
 * Mortis 9.2014
 */
...
#define I_AM_SO_COLD            "_player_i_cold"
...
int
sip(string str)    //Reduces intox by 25, max once per 30 seconds
{
    int i, j, k;
    int alco, newalco;
    cofrace = QRACE(TP);
    ...
    {
       write("Drinking has no effect on your undead body.\n");
       return 1;
    }   
    ...
    if (TP->query_prop("last_sip_time") < time())
    {
        if (alco > 0)
        {
            ...
            TP->add_prop("last_sip_time", time() + 29);
        }
    }
    ...
    if (TP->query_prop("last_sip_time") < time())
    {
        if (TP->query_prop(I_AM_SO_COLD) > 0 && adjfla1 != "iced" && adjfla2 != "iced")
        {
            int coldn = TP->query_prop(I_AM_SO_COLD);
            coldn = coldn -2;
            TP->add_prop(I_AM_SO_COLD, coldn);
            TP->add_prop("last_sip_time", time() + 29);
            write("You feel a little warmer.\n");
        }
    }
...
race_sip()
{
    if (TP->query_prop(LIVE_I_UNDEAD))
    {
       return "Drinking has no effect on your undead body.";
    }
