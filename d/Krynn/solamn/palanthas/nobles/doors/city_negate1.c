/*
 * The Northeast City Gate of Palanthas, Scholar's Gate.
 ...
 * gates.  Can be bribed for 15 plats to open the gates.  He does
...
create_door()
{
    set_door_desc("The NORTHEAST Gate.\n");
    set_door_id("city_northeast_gate");
    set_pass_command(({"ne","northeast"}));
    set_door_name(({"gate", "northeast gate", "northeast gates",
	"scholar gate", "scholar's gate", "scholars gate"}));
    set_other_room(NOBLE + "ner8.c");
    set_open(1);
    ...
    set_pick(80);
    set_lock_name("city_northeast_gate");
}
