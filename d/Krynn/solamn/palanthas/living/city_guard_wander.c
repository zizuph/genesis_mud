 ...
 * Mortis 02.2006
 *
 * Standard city guards wandering the street in each
 * district of Palanthas.
 *
 * If players can become Constable, these will change
 * to match their guild, but this is the default guard.
 *
 * EDITED:  Added call to remove_alrm() on do_die() to
 * eliminate cases where a guard flees at death's door,
 * the player follows, gets stunned, and kills the guard
 * within 1.5 seconds thus being permanently stuck with
 * a stun.
 * 
 * Also, eliminated guard's ability to stun people he is
...
/*	set_size_descs("tall", "of normal width");
    ...
    set_pick_up_team("wguard");
    set_max_team_size_auto_join(2);

	set_act_time(50);
	add_act("emote polishes a piece of armour.");
	add_act("emote polishes a spare weapon.");
	...
	+ "when the shift ends.");
	add_act("say Good day, citizen.");
	add_act("emote nods at you and says: Greetings.");
...
adjects()
{
	...
	if (adjstr1 == "smooth-chinned" || adjstr2 == "moustachioed")
		gender = G_MALE;
...
init_living()
{
	...
	if (TP->query_prop(LIVE_I_ATTACKED_PALANTHAS) == 1)
    {
		...
		command("shout Miscreant!  In the name of Lord Palanthas, stop!");
		
		int dmg = -55 - random(55);
		string invader = TP->query_real_name();

		command("kill " + invader);

		switch (random(5))
		{
			case 0:		
				TP->catch_msg("The guard splashes a steaming hot cup of "
				+ "Shoikan coffee in your face inflicting searing pain!\n");
				say("The guard splashes a steaming hot cup of Shoikan "
				+ "coffee in " + QTNAME(TP) + "'s face!\n", TP);
				TP->command("scream");
				TP->command("shout My face!  Oh gods, my face!");
				stunnedby = "the burning Shoikan coffee";
				break;
			case 1:
				TP->catch_msg("The guard whollops you on top of your head "
				+ "with " + HIS(TO) + " guard's sap raising a painful "
				+ "bump!\n");
				say("The guard whollops " + QTNAME(TP) + " on top of " + HIS(TP)
				+ " head with " + HIS(TO) + " guard's sap raising a painful "
				+ "bump!\n", TP);
				TP->command("groan painf");
				stunnedby = "the sap whollop";
				break;
			case 2:
				TP->catch_msg("The guard backhands your face leaving you "
				+ "dazed and woozy!\n");
				say("The guard backhands " + QTNAME(TP) + " face leaving "
				+ HIM(TP) + " dazed and woozy!\n", TP);
				command("shout You will regret your murderous ways, "
				+ "miscreant!");
				stunnedby = "the guard's backhand";
				break;
			case 3:
				TP->catch_msg("The guard lays a sickle slap across your "
				+ "neck sending you head over heels backwards to the "
				+ "ground in a bruising fall!\n");
				say("The guard lays a sickle slap across " + QTNAME(TP)
				+ "'s neck sending " + HIM(TP) + " head over heels backwards "
				+ "to the ground in a bruising fall!\n", TP);
				TP->command("say Ohhhh, why did I turn to crime?!?");
				stunnedby = "the guard's sickle slap";
				break;
			case 4:
				TP->catch_msg("The guard kicks you in the gut with his boot "
				+ "heel dropping you to your knees breathless!\n");
				say("The guard kicks " + QTNAME(TP) + " in the gut with his "
				+ "boot heel dropping " + HIM(TP) + " to " + HIS(TP)
				+ " knees breathless!\n", TP);
				TP->command("puke");
				stunnedby = "the guard's gut kick";
				break;
		}
		...
		para = clone_object(OBJ + "guard_wander_paralyze");
		para->set_fail_message("You are too stunned by " + stunnedby + " to "
...
{
	killer->add_prop(LIVE_I_ATTACKED_PALANTHAS, 1);
