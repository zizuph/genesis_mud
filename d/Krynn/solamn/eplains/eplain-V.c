...
start_ferry()
{
    ...
    if (flooded())
	ferry->start_ship(ferry->query_location());
}
...
create_eplains_room()
{
    ...
    add_item(({"vingaard keep","keep","walls","fortress"}),"The massive " +
	     "walls of Vingaard Keep can be seen across the river. " +
             "The Keep is a fortress " +
	     "located on the tip of land between the large Vingaard River " +
