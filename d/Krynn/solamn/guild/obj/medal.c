...
create_object()
{
   ...
   set_long("This is the medal awarded to followers of Paladine, and "+
      "thus it is worn by all Solamnian Knights. Its shape is that "+
      "of a dragon, one of the physical forms which Paladine has been " +
      "known to assume. Worn on a platinum chain around your neck, " +
      "the metal is both surprisingly light and warm.\n"+
...
* command from the soul. Additionally, it was modified to allow
   * Knights of the Rose to lead anyone, regardless of dis compari-
* sons. Nov 95 by Morrigan
*****************************************************************************/
int
invite(string name)
{
   ...
   /* Added Nov 95 by Morrigan to enable the new ability for Knights of the
   * Rose to be able to lead anyone regardless of dis.
   */
   if (this_player()->query_knight_level() == 5)
      member->add_prop("_rose_invite", this_player()->query_real_name());
...
init()
{
   ...
   /* added to enable the special Knight invite command by Morrigan 11/95 */
   ...
   set_alarm(9.0,0.0,"degrade_everyone", TP);
   
   set_alarm(2.0, 0.0, "news", TP);
...
news(object who)
{
   if (TP->query_real_name() == "tethjr" || TP->query_wiz_level())
      return 1;
   write("The Knights guild has been closed temporarily, and its "+
      "members removed. We expect it to be reopening soon, "+
      "hopefully within two months time. This decision was made in "+
      "order to allow the coders to make all the changes necessary at "+
      "one time, and then implement them all at once before reopening "+
      "the guild to mortals.\n"+
      "\nYou will now be removed from the Solamnian Knights guild.\n");
   (ADMIN)->leave(who, 0);
   return 1;
}

/* Function name: degrade_everyone
* Description:   All sword and rose knights that do not have the cleric
*                shadow will be degraded down to crown knight of
*                what sublevel?????
*/
void
degrade_everyone(object who)
{
   int tmp,i;
   int order = 3; /* All into crown order! */
   if (knight_level > order &&
         who->query_guild_name_lay() != GUILD_NAME_CLERIC)
   {
      who->set_knight_level(order);
      who->catch_msg("Due to Changes in the Guild, you have been " +
         "automatically degraded down to Crown Knight.\n");
      TO->set_tax(0);
   }
}
...
{
    write("The version of your medal of the Solamnian Knighthood is invalid. " +
	  "Removing... Please get the current version.\n");
...
void
give_spells(object who)
{
   object spells = P(SPELL_OB_NAME,who);
   
   if (spells)
      {
      if (!((spells->id("sword_knight_spell_obj") &&
                  knight_level == 4) ||
            (spells->id("rose_knight_spell_obj") &&
               knight_level == 5) ||
            knight_level < 4))
      spells->remove_object();
   }
   
   if (!spells)
      {
      if (knight_level == 4)
         spells = clone_object(SWORD_SPELL_OBJ);
      else if (knight_level == 5)
         spells = clone_object(ROSE_SPELL_OBJ);
      else
         spells = clone_object(SPELL_OBJ);
      spells->move(who);
   }
}
...
/* Function name: remove_cleric_guild
* Description:   removes the knight cleric layman part if necessary.
   */
void
remove_cleric_guild(object who)
{
   if (who->query_guild_name_lay() == GUILD_NAME_CLERIC)
      who->remove_guild_lay();
}

/* Function name: add_cleric_guild
* Description:   adds the knight cleric layman part if necessary.
   */
void
add_cleric_guild(object who)
{
   if (who->query_guild_name_lay())
      return;
   
   clone_object(CLERIC_SHADOW)->shadow_me(who,"layman","cleric",GUILD_NAME_CLERIC);
   if (who->query_wiz_level())
      who->catch_msg("Please 'Call me add_autoshadow " + CLERIC_SHADOW + ":' to " +
...
{ 
   ...
   if (level >= 4 && ob->query_guild_name_lay() &&
         ob->query_guild_name_lay() != GUILD_NAME_CLERIC)
   level = 3;
   
   knight_level = level;
   
   if (knight_level <= 3)
      remove_cleric_guild(ob);
...
set_tax(int i)
{
   ...
   if (MEMBER(ob))
      ...
      if (knight_level > 3)
         {
         old_tax = ob->query_learn_pref(SS_LAYMAN);
         new_tax = LAY_TAX[knight_level];
         if (new_tax != old_tax)
            ob->set_guild_pref(SS_LAYMAN, new_tax);
       }
      else
      ...
      if (new_tax != old_tax)
         ob->set_guild_pref(SS_LAYMAN, new_tax);
      if (!ob->query_guild_member_occ() && ob->query_learn_pref(SS_OCCUP))
