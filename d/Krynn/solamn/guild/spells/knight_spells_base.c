...
inherit "/std/object"; /* do it like this now */
/* ideally the spells should later be inherited into the knight medal */
...
/* temporary variable. is set when the spell is failing
 */
string fail_message;
/* temporary variable. is set when the spell will fail
 */
int spell_failed;

/* redefinition to allow check_calls (VBFC)! */
public void
list_spells()
{
    int i;
...
query_spell_mess(string verb, string str)
{
    ...
    mutter = call_other(TO, verb + "_mutter", str);

    tell_room(E(pl), QCTNAME(pl) + 
        " cradles a platinum medal in " + pl->query_possessive() + " hand " +
...
/* Redefine the spell fail function to work with different strings */
public mixed
spell_fail()
{
    mixed str = 0;
...
default_config_spell()
{
    ...
    object pl = this_player();
    int    pres = spell_availability(query_verb());
