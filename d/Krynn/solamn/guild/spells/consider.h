/* The consider spell.
 * Determines the prestige a knight gets for killing this thing.
 * Is also used to determine the current standings within the rank
 * of the knight.
 */


public object *consider_target(string arg);
public void consider_message(object caster, object target);
public mixed consider_success(object caster, object *targets);

void
create_spell_consider()
{
    set_element(SS_ELEMENT_LIFE, 0);
    set_form(SS_FORM_DIVINATION, 0);

    set_spell_time(3);

    set_mana_spell(3);
    /*set_mana_spell_fail(2);*/
    set_find_target(consider_target);

    set_spell_task(TASK_SIMPLE);
    
    set_spell_message(consider_message);
    set_spell_success(consider_success);
}
...
mixed
consider_success(object caster, object *targets)
{
    if (caster->query_stat(SS_WIS) < random(20))
...
mixed
consider_message(object caster, object *targets)
  {
		...
		if (stat >= SS_LEVEL10)
		  {
			...
			write("Unfortunately the position is filled.\n");
		      return 1;
		  }
		...
		if (MEMBER(TP))
		...
		else
		  {
		    ...
		    if (TP->query_gender() == 1)
		      {
			nr = SENTRY_F_TITLES[sublevel + 1];
			pr = SENTRY_F_TITLES[sublevel - 1];
			tr = SENTRY_F_TITLES[sublevel];
		      }
		    else
		      {
			nr = SENTRY_M_TITLES[sublevel + 1];
			pr = SENTRY_M_TITLES[sublevel - 1];
			tr = SENTRY_M_TITLES[sublevel];
		      }
		  }
