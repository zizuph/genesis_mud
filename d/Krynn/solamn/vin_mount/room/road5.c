/* created by Aridor 06/19/93 */


/* Navarre June 11th 2006, added items for snow when its blocking the road.
 * Also added line break in the long description, that were missing.
 */

#include "../local.h"
#include "/d/Krynn/common/clock/clock.h"
#include <macros.h>

inherit ROOM_BASE

#include RND_MEET

object* monster = ({});

void
init()
{
    object mon;

    ::init();
    if (random(50) > 1)
        return;
    mon = RANDOM_MEET(TABLE);
    mon->move_living("xx",TO);
    monster += ({ mon });
}

string
get_long()
{
    string ret_str = "The Vingaard Mountains seem less significant up close, though "
                   + "you realize the road a head is still significant. ";
    int season;

    season = GET_SEASON;

    switch (season)
    {
        case WINTER:
            ret_str += "However, the deeper chill of the winter is in the air, "
                    +  "and the northern track is blocked by a wall of snow, making "
                    +  "the the southern track the only possible route.\n";
            add_item(({"snow", "wall", "wall of snow"}), "A wall of snow is blocking the track.\n");
            break;
        case SPRING:
           ret_str += "The wind blowing past you is mild. ";
           remove_item("snow");
           remove_item("wall");
           remove_item("wall of snow");
           break;
        case SUMMER:
           ret_str += "The wind blow up the mountainside is warm and kind. ";
           remove_item("snow");
           remove_item("wall");
           remove_item("wall of snow");
            break;
        case AUTUMN:
            ret_str += "The wind here is rough as if it bids the darker days to come welcome. ";
            remove_item("snow");
            remove_item("wall");
            remove_item("wall of snow");
            break;
    }

    if (season != WINTER)
        ret_str += "The winding road continues southeast and west,"
        +    " winding itself up the steep mountainside. "
        +    "Rocks and cliffs make travel in other directions "
        +    "impossible.\n ";

    return ret_str;
}

int
check_northern()
{
   if (GET_SEASON != WINTER)
   {
       TP->catch_msg("You continue up the northern track...\n");
       return 0;
   }

   TP->catch_msg("The road is blocked by a wall of snow...\n");
   say(QCTNAME(TP) + " stops when " + HE(TP) + " realizes that further " +
       "travel in that direction is impossible.\n");
   return 1;
}

void
reset_vin_mount_room()
{
    monster -= ({ 0 });
    if (monster && sizeof(monster))
    {
    monster->command("emote leaves.");
    monster->remove_object();
    monster = ({});
    }
}

void
create_vin_mount_room()
{
    set_short("In the Vingaard Mountains");
    set_long(get_long());

    OUTSIDE;

    add_exit(ROOM + "road4","west",check_northern, 15);
    add_exit(ROOM + "road6","southeast",0,15);
    add_item(({"track","ground","road","gravel","mountain track"}),
         "The track itself is simply " +
        "laid out with small stones and gravel.\n");
    add_item(({"mountains","mountain","mountainside"}),
         "These are high mountains indeed, rocks and occasional " +
        "shrubs are scattered everywhere, it is definitely " +
        "a very hostile area.\n");
    add_item(({"cliffs","cliff"}),
         "Steep, almost vertical cliffs offer no chance for travel " +
        "in other directions but along the road.\n");
    add_item(({"shrubs","rocks"}),
         "The mountainsides are covered with rocks with a few " +
        "shrubs making their living in between.\n");


}


