...
create_krynn_monster()
{
    ...
    set_patrol_path(({"w", "say That mean ship captain won't let me on "+
        ...
        "smile brightly", "n", "ne", "nw", "@@shop", "n", "nw", "exa board", 2, 
        "say What's "+
        "so interesting about this board anyway? People are always coming to "+
        "look at all this scribbling.", "nw", "ne", "e", "enter", "emote sits "+
        "down at the bar and looks around the bustling inn.", "say The next "+
...
shop()
{
    ...
    if (the_time[HOUR] > 6 && the_time[HOUR] < 20)
    {
        TO->command("e");
        for(i = 0; i<5000; i++);
        TO->command("say Oh wow, that sure is pretty!");
        TO->command("emote reaches out to pick up a shiny bauble.\n"+
          "The shopkeeper yells: You rascal! I told you never to come back "+
          "here, that's it. Come here you little thief!\n");
        for(j = 0; j<3000; j++);
        TO->command("emote evades the shopkeeper's clumsy grasp, and darts out of "+
          "the store.");
        TO->command("w");
        TO->command("shout Thief?! You're the thief, you great oaf, charging "+
          "those outrageous prices!");
        TO->command("shout I didn't want to shop in your store anyway!!");
        for(k = 0; k<6000; k++);
        TO->command("emote mumbles, `I'm not a thief' in a hurt tone of voice.");
        return 1;
    }

    TO->command("time");
    TO->command("pout");
    TO->command("say I wish they would leave the shop open all night. Maybe I "+
...
test_patrol_func()
{
    set_alarm(1.0, 0.0, "command", "say Functions can be called from the "+
      "patrol directions.");
    set_alarm(2.0, 0.0, "command", "cheer");
}
...
special_attack(object enemy)
{
    ...
    if (attacktype == 1)
    {
        ...
        how = "Shindig swings his hoopak around, but misses "+
        QTNAME(enemy)+".";
        you = "Shindig swings his hoopak around, but misses you.";
        if (hitresult[0] > 0)
        {
            how = "Shindig swings his hoopak, striking "+QTNAME(enemy)+
            "'s shin painfully.";
            you = "Shindig swings his hoopak, striking your shin "+
            "painfully.";
        }
        if (hitresult[0] > 10)
        {
            how = "Shindig lifts the hoopak up, then stabs downward, "+
            "driving the pointed end of his hoopak into "+
            QTNAME(enemy)+"'s foot.";
            you = "Shindig lifts his hoopak up, then stabs downward, "+
            "driving the pointed end of his hoopak into your "+
            "foot.";
        }
        if (hitresult[0] > 20)
        {
            how = "Shindig swings his hoopak upwards, catching "+
            QTNAME(enemy)+" below the chin.";
            you = "Shindig swings his hoopak upwards, catching "+
            "you below the chin.";
        }
        if (hitresult[0] > 40)
        {
            how = "Shindig slams his hoopak into "+QTNAME(enemy)+"'s "+
            "chest with amazing force.";
            you = "Shindig slams his hoopak into your chest, with "+
            "amazing force.";
        }
        if (hitresult[0] > 70)
        {
            how = "Shindig twirls his hoopak around, reversing his "+
            "grip, and impales "+QTNAME(enemy)+" with the point "+
            "of his hoopak.";
            you = "Shindig twirls his hoopak around, reversing his "+
        ...
        if (enemy->query_hp() <= 0)
            write("Shindig sets his feet firmly in the ground, and yanks "+
