...
create_palan_room()
{
    ...
    set_long(BS("This is the waiting room. There are several chairs here "
          +  "and a little table. Otherwise, the room is rather empty "
          +  "except for the attendant standing by the door.", SL));
...
only_without_pen()
{
    ...
    if (the_pen)
    {
        write(BS("Hey! We don't want our inventory to wander off all the "
