 ...
 * Ship rerouted to Kalaman-Sparkle - Tapakah, 06/2009
 *
 * Removed check on wielding weapons when entering city. See notes below.
 * Mortis 09.2014
 */
...
create_palan_room()
{
...
#ifdef CITYCLOSED
    add_exit("/d/Krynn/solamn/palanthas/shipping/s15","south", 0, 0);
#else
    add_exit("/d/Krynn/solamn/palanthas/shipping/s15","south", 0, 0);
#endif
    ...
    add_item("lighthouse", "@@tower_desc");
    add_item(({"gate","guard"}),
...
/* Removed this, because it's ridiculous to make people unwield to enter
 * and tell them it's illegal to wield weapons in the city limits while
 * it's not and there is no mechanism to enforce it nor do I have any
 * intention to make it so. Mortis 09.2014
...
read(string str)
{
    ...
    {  return 0; }
    //write(SIGN_MESS);
    write("The shipline to Grey Havens has been discontinued.\n");
    return 1;
}
