...
reset_palan_room()
{
    if (proprietor)
        return;
    proprietor = clone_object(LIV + "proprietor");
...
create_palan_room()
{
    ...
    set_long(BS("This is a cozy room with a desk as the only accessory inside."
         +   " It's the office of the inn, where the business parts of owning "
         +   "an inn are handled.",SL));
