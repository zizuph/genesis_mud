...
create_palan_room()
{
    ...
    add_exit(ROOM + "pipeud4","down",0,1);
    add_exit(NERAKA_EXIT,"up",0,1);
    
    LIGHT;
    add_item(({"gems","small gems"}),
	     BS("These are two small gems set into the wall here. " +
		"Maybe you shouldn't touch them, but then again, maybe you should.",SL));
    add_item(({"shaft"}),
...
blow_me_away(object who)
{
    ...
    who->move_living("down, blown by the wind",ROOM + "pipe3",0,1);
}
