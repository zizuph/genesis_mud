...
create_palan_room()
{
    ...
    set_long(BS("You are in the corner of a horizontal and a vertical shaft. " +
	"A ladder leads up from here, and you can also walk along the " +
	"floor of the horizontal shaft. The walls of the shaft seem to glow a little.",SL));

    add_exit(ROOM + "pipeud3","up",0,5);
    add_exit(ROOM + "pipewe2","north",0,1);
...
blow_me_away(string where)
{
	...
	TP->move_living("north, blown by a gust of wind", ROOM + "pipe2",0,1);
    }
    else /*where = "south"*/
    {
	write("The wind has turned and you are now blown up by a strong force of wind along the shaft.\n");
	TP->move_living("up, blown by a gust of wind", ROOM + "pipe4",0,0);
    }
}
