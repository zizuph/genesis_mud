...
create_creature()
{
    ...
    set_long("A huge deadly dragon. It looks very powerful but you can feel its goodness."
      + " You can also 'mount dragon' and when mounted 'kick dragon' will tell"
      + " the dragon to take you from Solace to Vingaard Keep or back.\n");
    set_adj("bronze");
...
{
    seteuid(getuid(TO));
    write_file("/d/Krynn/log/dragon_remove","VKEEP: " + TP->query_name() +
      " " + ctime(time()) + ".\n");
    ::remove_object();
}
