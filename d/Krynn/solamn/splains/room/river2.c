...
reset_splains_room()
{
    if (monster || random(3) < 1)
      return;
    monster = clone_object(LIV + "aurak");
    monster->set_random_move(4);
...
create_splains_room()
{
    ...
    set_long(BS("There is a river blocking your path east. The river " +
		"flows lazily through the plains coming from the north of " +
		"you and going south. You are " +
    ...
    add_item(({"river"}),
	     ...
	     "plains. The river is east of you and flows from north to south.\n");
    ...
    add_exit(ROOM + "river3","south",0,3);
    add_exit(ROOM + "plain18","west",0,3);
    add_exit(ROOM + "river1","north",0,3);
