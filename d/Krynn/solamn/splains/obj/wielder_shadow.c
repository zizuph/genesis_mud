 ...
 * Changelog
 * 
 *   - Fixed division by zero problem, February 2009, Petros
 *
 * Shanoga, 2021 May 19:
...
#define PARRY_CHANCE (500 / max(1, shadow_who->query_skill(SS_PARRY)))
...
int query_not_attack_me(object who, int aid)
{
    ...
    if (!query_sword() || query_sword()->query_wielded() != shadow_who)
    {
    ...
    who->catch_msg("A green blur flashes in front of you, knocking aside "+
      "your "+weapon->short()+".\n");
    shadow_who->tell_watcher("A green blur flashes in front of "+QTNAME(shadow_who)+
