 ...
 *
 * Added coins (0-1 gc; 1-12 sc) and a coin purse.
 */
...
#define AXE WEP+"dewarwep"
#define SHIRT ARM+"dewararmour"
#define PANTS ARM+"dewararmour2"
#define PURSE "/d/Krynn/solamn/pal_gully/purse"
#define COINS MONEY_MAKE_GC(1 + random(2))
#define S_COINS MONEY_MAKE_SC(1 + random(12))

#define TYPE ({"forlorn", "depressed", "dour", "lithe", "grim", "ghastly"})
#define PHYS ({"tiny", "dimunative", "menacing", "gangly","disjointed", \
"venomous"})
#define EYES ({"bloodthirsty", "black", "grey", "piercing", "yellow", \
"bloodshot"})
#define FACE ({"menacing", "calculating", "bearded", "handsome"})
#define SKIN ({"dark", "pale", "smooth", "pocked", "blemished", "scarred"})
#define HAIR ({"grey", "black", "blood-red", "white", "brown", "blonde", \
"dirty blonde"})
#define HAIR2 ({"smooth", "greasy", "spiked", "messy", "mohawked", "clean", \
"slicked"})
...
nomask void create_krynn_monster()
{
        ...
        set_short(type + " " + phys + " " +gen_type+ " dewar");
        set_pshort(type + " " + phys + " " +gen_type+ " dewar");
        set_long("This "+type+" "+phys+" dewar dwarf is certainly angry. "
        +CAPhisher+" "+eyes+" eyes shoot about the room looking at "
        +"everything. "+CAPheshe+" is one of the Dewar who were cast out "
        +"due to their insanity. Now forced to live in places no other "
        +"dwarf would live. "+CAPhisher+" "+hair2+" hair is "+hair+" and "
        +hishers+" "+face+" face betrays "+hishers+" evil intent.\n");
...
attacked_by(object ob)
{
    ...
    query_team_others()->notify_ob_attacked_me(this_object(), ob);  
}
