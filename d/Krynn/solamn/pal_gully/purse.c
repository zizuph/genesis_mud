/*
 * Coin purse for Dewar - Arman 2017
 *
 * A smaller version of the /d/Faerun/obj/purse.c 
 * by Finwe, May 2006
 */
...
#define PACK_ID         "_dewar_money_purse_"
#define BAG_NAME        "purse"
...
create_container()
{
    ...
    add_name(PACK_ID);
    set_adj("leather");
    set_short("small leather " + BAG_NAME );
    set_long("@@long_func@@");
    set_slots(A_ANY_HIP);

    add_prop(OBJ_I_VALUE, 50+random(10));
...
long_func()
{
    string long_desc = "This is a " + short() + " of dwarven make. " +
         "The purse looks like it can hold a small number of coins.";

    if (query_prop(OBJ_M_NO_SELL))
        long_desc += " At present, the " + short() + " is protected " +
 ...
 * Description  : Checks if it is a coin
...
prevent_enter(object obj)
{
    ...
    if (!IS_COINS_OBJECT(obj) && !obj->id("_sybarun_token") && !obj->query_gem())
    {
        write("You can only put coins or gems in the " + query_name() + ".\n");
        return 1;
    }
...
print_item(object *ob)
{
    ...
    string ret = "";
    string name,scoin_qty, scoin_type, scoin, categ;
    object coinz;
    int qty, price;
...
{
    if (sizeof(all_inventory(TO)))
        return "There is still some stuff in the " + short() + "!\n";
    else
        return 0;
}
