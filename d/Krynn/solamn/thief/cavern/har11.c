...
create_thief_room()
{
    ...
    set_long("A deafening noise is right next to your ear. A stream " +
	     ...
	     "cavern north of the stream and light is shining in " +
	     "from somewhere above the water. The cavern extends " +
	     "into safety to the west into the mountain.\n");
    
    add_exit(CAVERN + "har10","west");
    ...
    add_item(({"light"}),
	     "It seems to be sunlight shining in from above.\n");
...
through_waterfall()
{
    ...
    TP->move_living("into the waterfall",THEIWAR + "r32",1,1);
    TP->command("$look");
