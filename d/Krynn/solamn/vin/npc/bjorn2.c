...
create_krynn_monster()
{
    ...
    add_supplied_commodity("s_sword", 7, 5, 1.2);
    ...
    add_ask(" [about] 'task' / 'quest'", "say Get me some supplies "+
      "and we will eward you handsomely!",1);
...
handle_given_object(object what, object who)
{
    ...
    {
        command("say Interesting... oh well... here you are");
        if (IS_ARMOUR_OBJECT(what))
        {
          command("say If you want to 'order', then 'order' first, " +
            "then give it to me when I ask for it");
        }

        return 0;

    }
    else if (!IS_ARMOUR_OBJECT(what))
    {
    ...
    {
        command("say I am currently very busy.");
        command("give " + what->query_name() + " to " + who->query_name());
        command("say Try again later");
        return 0;
    }
    else
    {
        command("say This " + what->query_name() + " looks nice");
        command("nod inter");
        command("drop " + what->query_name() );
...
public int
alter_armour(object person, object arm, int crestChoice)
{
    int mon;
    string *adjs;
    ...
    set_alarm(1.3, 0.0, &person->catch_tell("You request to have your " +
        arm->short() + " crested with a " + CRESTS[crestChoice] + ".\n"));
...
order(string str)
{
    ...
    if (wildmatch("/d/Krynn/solamn/vin/armour/*",
      file_name(armour[0])))
    {
...
forging_tells(object arm, object from)
{
    ...
    set_alarm(22.0, 0.0, &command("swear"));
