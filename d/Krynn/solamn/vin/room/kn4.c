...
#define SWORD "/d/Krynn/solamn/vin/knight/admin/admin"->query_conclave("sword")
#define WAIT \
   ({  \
      VROOM + "kn1", \
      VROOM + "void", \
...
string query_crest_str()
{
    string gm_name = C((ADMIN)->query_conclave("sword"));
...
string long_desc()
{
    return "You have entered a warmly-lit room.\nThis is the private "+
     "office of "+ ADMIN->query_conclave("sword")+
     ", the High Clerist of the Solamnian Knights."+
     "  A fire burns "+
     "in the fireplace along one wall, and hung above the mantle are "+
     "three swords.  A huge banner depicting "+query_crest_str()+" "+
...
create_vin_room()
{
    ...
    set_short("High Clerist Room");
    set_long("@@long_desc");

    add_prop(ROOM_I_LIGHT, 2);
    add_prop(ROOM_M_NO_SCRY, query_no_scry); 
   clone_object(VOBJ + "sword_b")->move(TO);
