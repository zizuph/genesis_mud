...
check_join(object player)
{
    ...
    if (strlen(lay_name))
    {
        /*Smiths, Blademasters, and Shieldbearers may join*/
        if (lay_name != SMITH_NAME && lay_name != BLADE_NAME &&
            lay_name != SHIELD_NAME
            && lay_name != ARCHER_NAME
            && lay_name !=GARDENER_NAME
            && lay_name != MILITIA_NAME
            )
        {
            write("You are presently allied to another organization. The "+
              "Knighthood demands the utmost commitment and dedication, "+
              "all other loyalties must be broken to join them.\n");
            return 0;
        }
    }
