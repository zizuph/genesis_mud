...
string *adjectives = ({ "feisty", "spirited", "steady", "powerful",
  ...
  "proud", "skittish", "grand", "noble", "mighty" });
...
create_vin_room()
{
    ...
    set_long("This is a stable, used by the Knights of Solamnia " +
      ...
      "area where undoubtedly food is kept. There is a sign on "+
...
string sign_desc()
{
    ...
    "Cavalry may choose there mounts here or retire here. To "+
...
int retrieve_horse(string str)
{
    ...
    ob = clone_object("/d/Krynn/morrigan/open/horse");
    ...
    me->catch_tell("You motion to a nearby squire to retrieve your mount. "+
      "The squire returns briefly, leading your mount, and hands "+
      "you the reigns.\n");
    tell_room(E(me), QCTNAME(me)+" motions to a squire, who then leaves into "+
      "one of the stalls. The squire returns briefly, leading a "+
...
int list_stuff(string str)
{
    ...
    if (str == "adjectives")
    {
	TP->catch_tell("Available adjectives:\n");

	while(i <= sizeof(adjectives))
	{
	    TP->catch_tell(" - "+adjectives[i - 1]+"\n");
	    i++;
	}
    }
    ...
    if (str == "colours")
    {
	TP->catch_tell("Available colours:\n");

	while(i <= sizeof(colours))
	{
	    TP->catch_tell(" - "+colours[i - 1]+"\n");
	    i++;
	}
    }
...
int choose_horse(string str)
{
    ...
    if (!(ADMIN)->query_cavalry(TP->query_name()))
	return 0;
    find_living("morrigan")->catch_tell("adj1-"+adj1+"  adj2-"+
      adj2+"   gen-"+gen+"\n");
    ...
    NF("One of your selections is invalid, please try again.\n");
    if (member_array(adj1, adjectives) == -1 ||
      member_array(adj2, colours) == -1 ||
      (gen != "male" && gen != "female"))
	return 0;
...
int enlist_cavalry(string str)
{
    ...
    write("You enlist in the Solamnian Cavalry, and are now allowed to "+
      "train here in the arts of mounted combat.\n");
