...
int return_horse(string str)
{
    ...
    {
		write("You must dismount before you can return your steed.\n");
/*    steed->do_dismount(TP, 1);
    write("As you dismount your "+steed->steed_short()+", you motion "+
      "to a nearby squire, who takes the reins obediently, "+
      "and leads your steed to the stables.\n");
    say("As "+QTNAME(TP)+" dismounts "+POSSESSIVE(TP)+" "+
      steed->steed_short()+", "+PRONOUN(TP)+" motions to a nearby "+
      "squire, who takes the reigns obediently, and leads "+
      "the steed away.\n");
    steed->remove_object();*/
    return 1;
    }
