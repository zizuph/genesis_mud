...
reset_vin_room()
{
    if (!gTrainer)
    {
	gTrainer = clone_object(VNPC + "marlin.c");
	gTrainer->move(TO);
    }
    if (!kn1)
    {
...
set_up_skills()
{
    ...
    sk_add_train(SS_WEP_POLEARM,"fight with a polearm",  0, 0, 75 );
    sk_add_train(SS_WEP_CLUB,   "fight with a club",     0, 0, 70 );
    sk_add_train(SS_2H_COMBAT,  "fight with two weapons",0, 0, 60 );
