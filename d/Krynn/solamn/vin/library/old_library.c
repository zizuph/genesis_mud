...
static object knight; /* The knight in the guild. */
...
create_vin_room()
{
    ...
    set_long("You are standing in the famous library of Vingaard Keep. " +
        ...
        "east is a small alcove used for composing books.\n");
...
read_sign()
{
    write("LIBRARY - QUIET PLEASE!\n\n" +
        ...
        "the librarian.\n");
    if (TP->query_real_name() == librarian)
...
hook_find_book(object who)
{
    ...
    if (!present(who,TO))
        {
        tell_room(TO,"The Knight arrives, looks around and says: Wasn't " +
...
{
    tell_object(who, "The Knight arrives and gives you the book.\n");
    tell_room(TO, "The Knight arrives with the ordered book.\n", who);
}

public void
hook_book_not_found(object who,string bname)
{
    tell_object(who, "The Knight arrives and tells you that the book " +
        "you were looking for doesn't exist.\n");
    tell_room(TO, "The Knight arrives without the ordered book.\n", who);
}

public void
hook_book_currently_borrowed(object who, string bname)
{
    tell_object(who, "The Knight arrives and tells you that the book has " +
