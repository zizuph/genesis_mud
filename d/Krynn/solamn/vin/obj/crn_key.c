...
create_key() 
{
    ...
    set_name("crown_conclave_key");
    add_name("key");
    set_adj(({"ornate", "bronze"}));
    set_pshort("ornate bronze keys");
    set_key(CROWN_C_KEY);

    set_short("ornate bronze key");
    set_long("A key wrought from bronze in the shape of a crown. "+
      "It is the property of the High Knight of the Crown, and opens "+
      "the door to that office.\n");
    ...
    add_prop(OBJ_S_WIZINFO, "A key which magically appears in the "+
        ...
        "east.  It opens the Crown Conclave Door.\n");
    add_prop(MAGIC_AM_ID_INFO, ({
	"A key which belongs to the High Knight of the Crown and "+
