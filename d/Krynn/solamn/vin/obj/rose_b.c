...
create_door()
{
   ...
   set_fail_pass("Walking through doors is a skill you haven't " +
    ...
    set_door_name(({"Rose Conclave door", "rose door", "rose order",
        "rose conclave", "door", "golden door", "rose"}));
   set_door_desc("This large door has the image of a rose carved into "+
      ...
      "handle glows with an inner light and opens the door into this room. "+
      "Rather heavy, it looks like it would take the combined weights of "+
      "several men thrown against it to force the door open.\n");
   set_other_room(VROOM + "greathall_balcony.c");
   set_open(0);
   set_locked(1);
   set_open_desc("The golden door stands open.\n");
   set_closed_desc("A golden door.\n");
   ...
   set_close_mess(({"pushes the golden door closed.\n", "You push the golden " +
      "door closed.\n"}));
   ...
   set_lock_desc("It is difficult to discern anything about the lock. "+
      "It is doubtful that anyone could open it without the key.\n");
