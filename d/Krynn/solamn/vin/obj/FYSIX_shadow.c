inherit "/d/Gondor/fysix/open/steed_sh";
...
hit_me(int wcpen, int dt, object attacker, int attack_id, int target_hitloc = -1)
{
	...
	if (!random(10) && armour_check())
	{
		...
		Steed->do_dismount(shadow_who, 1);
	    }
	    else
	    {
		i = random(2) + 1;
	...
	else
	{
	    shadow_who->catch_tell("As you are struck, you lose your balance "+
	      "and fall from your "+Steed->short_old()+".\n");
	    tell_room(E(shadow_who), "As "+PRONOUN(shadow_who)+" is struck, "+
...
maneuver(string str)
{
    ...
    NF("Your "+st->short_old()+" is not under attack, there is no need.\n");
    if (!enemies)
	return 0;
    ...
    st->add_prop(OBJ_M_NO_ATTACK, "As you move to attack the "+
      st->short_old()+", "+QTNAME(me)+" whirls the steed around to "+
      "face you.\n");
    me->catch_tell("Recalling your training of mounted combat, you "+
      "spur your "+st->short_old()+" forward and whirl "+POSSESSIVE(st)+
      " around to face "+POSSESSIVE(st)+" assailents.\n");
    tmp = enemies;
    tmp += ({ me });
    tell_room(E(me), QCTNAME(me)+" spurs "+POSSESSIVE(me)+" "+
      st->short_old()+" forward and whirls around, facing "+
	...
	enemies[i]->catch_tell(me->query_The_name(enemies[i])+
	  " spurs "+POSSESSIVE(me)+" "+st->short_old()+" forward and "+
...
int
charge_special(string str)
{
    object ob, *obj, weapon, *weapons, horse = Steed;
    ...
    liv->catch_tell("You salute "+ob->query_the_name(liv)+" in the tradi"+
      ...
      ", then spur your "+horse->short_old()+" forward, snapping the reigns.\n");
    return 1;
}
...
void do_charge_prespecial1(object them, object me, object horse)
{
    ...
    /* not really needed, Fysix
	if (member_array(me, horse->query_riders()) < 0)
	{
    ...
    me->catch_tell("Your adrenaline begins to surge as you slap the reigns "+
      "back and forth as your "+horse->short_old()+" gains momentum.\n");
    them->catch_tell(me->query_The_name(them)+" slaps the reigns back and forth as "+
      POSSESSIVE(me)+" "+horse->short_old()+" gains momentum.\n");
...
void do_charge_prespecial2(object ob, object liv, object horse)
{
    ...
    /* Not really needed, Fysix
	if (member_array(liv, horse->query_riders()) < 0)
	{
    ...
    liv->catch_tell("You lean forward in the saddle as your "+horse->short_old()+
    ...
    ob->catch_tell("The ground beneath your feet rumbles as "+
      ...
      " "+horse->short_old()+"! You feel fear creep over you as "+PRONOUN(liv)+
    ...
    liv->tell_watcher("The ground rumbles beneath your feet as "+QTNAME(liv)+
      ...
      horse->short_old()+". Leaning forward in the saddle, "+QTNAME(liv)+" shouts "+
...
void do_charge_special(object ob, object liv)
{
    ...
    pen = (statmod + weaponmod) / 2 + levmod * 3 + 20;
	...
	weapon->short()+" misses. You tug on the reigns of your "+horse->short_old()+
	...
	"of "+POSSESSIVE(liv)+" "+horse->short_old()+", turning to face you.";
	aud = QCTNAME(liv)+"'s attempt to strike "+QTNAME(ob)+" with "+POSSESSIVE(liv)+
	" "+weapon->short()+" misses. "+C(PRONOUN(liv))+" tugs on the reigns "+
	"of "+POSSESSIVE(liv)+" "+horse->short_old()+", turning to face "+QTNAME(ob)+".";
	break;
    case 1..5:
	att = "As your "+horse->short_old()+" bears down upon "+ob->query_the_name(liv)+
	", you strike "+POSSESSIVE(ob)+" "+hdesc+" with a glancing blow of your "+
	weapon->short()+".";
	tar = "As "+POSSESSIVE(liv)+" "+horse->short_old()+" bears down upon you, "+
	liv->query_the_name(ob)+" strikes your "+hdesc+" with a glancing blow of "+
	POSSESSIVE(liv)+" "+weapon->short()+".";
	aud = "As "+POSSESSIVE(liv)+" "+horse->short_old()+" bears down upon "+
	QTNAME(ob)+", "+QTNAME(liv)+" strikes "+POSSESSIVE(ob)+" "+hdesc+
	" with a "+"glancing blow of "+POSSESSIVE(liv)+" "+weapon->short()+".";
	break;
    case 6..20:
	att = "The rumbling of the ground beneath your "+horse->short_old()+"'s feet "+
	...
	horse->short_old()+" lending its strength to the blow.";
	...
	{   
	    liv->catch_tell("The thundering crescendo of your "+horse->short_old()+" reaches "+
	      ...
	      " "+str+" "+ob->query_the_name(liv)+"'s "+ (hdesc == "body" ? "head" : hdesc) +
	      " from "+POSSESSIVE(ob)+" body.\n");
	    ob->catch_tell("The thundering crescendo of "+liv->query_the_name(ob)+"'s "+
	      horse->short_old()+" reaches its peak as "+PRONOUN(liv)+" collides with you "+
	      "in a deafening explosion of sound. Aided by "+POSSESSIVE(liv)+" tremendous "+
	      "momentum, "+POSSESSIVE(liv)+" "+weapon->short()+ " "+str+" your "+
	      (hdesc == "body" ? "head" : hdesc) + " from your body.\n");
	    liv->tell_watcher("The thundering crescendo of "+QTNAME(liv)+"'s "+horse->short_old()+
	      ...
	      (hdesc = "body" ? "head" : hdesc) + " from "+POSSESSIVE(ob)+" body.\n", ob);
	    ob->do_die(liv);
