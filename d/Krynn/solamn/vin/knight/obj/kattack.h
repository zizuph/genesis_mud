...
// 9.jan.2004 Milan: fixed runtimes caused by bad use of TP in alarm called 
//                   functions.
//                   Modified set_alarm to use function calls (was string)


/********************* kattack *******************************************/

/* Function to return the shield, if worn, of Knight */
object
...
/*
 * Function to return weapon of Knight, now only used for
 *    when the Knight is using _only_ one weapon
 */
object
...
/*
 * Function to return the weapon that will be used for the kattack, used
 *    when the Knight is using two weapons.
 */
object *
...
reset_special(object liv)
{
    ...
    if (liv->query_attack() && medal->query_auto_kattack())
    {
        liv->command("$kattack");
        return;
    }
...
do_special(object ob, object liv)
{
    ...
    object *weapons;
    object medal;
    int hitres, weaponmod, statmod, pen, skill, weaponstat, levmod, to_hit;
    ...
    if (liv->query_steed() && liv->query_skill(SS_MOUNTED_COMBAT) > 15)
    	horsemod = (liv->query_skill(SS_MOUNTED_COMBAT) - 15) / 4;
    else
    	horsemod = 0;

    /* Final formula */
    skill = liv->query_skill(SS_KATTACK);
    pen = (statmod + weaponmod) / 2 + levmod + horsemod;
    hitres = F_PENMOD(pen, skill);
    wt = weapon->query_wt();
    ht = weapon->query_dt();
    to_hit = min(100, weaponmod + levmod + skill / 5+
        (liv->query_prop(LIVE_O_STEED) ?
        max(0, (liv->query_skill(SS_MOUNTED_COMBAT) - 15) /6) : 0));
    to_hit = max(65, to_hit);
    if (liv->query_combat_object()->cb_tohit(-1, 100, ob) != 1)
    	hitresult = ob->hit_me(0, ht, liv, -1);
    else
        hitresult = ob->hit_me(hitres, ht, liv, -1);
    hdesc = hitresult[1];

    /* New checks added - unseen opponent and opponents without bodies */
    if (!CAN_SEE(liv, ob) && hitresult[0] == 0)
    {
    	...
    	set_alarm(liv->query_speed(12), 0.0, &reset_special(liv));
    	return;
    }
    ...
    if (wt == W_SWORD)
    {
	    ...
	    default:
    	    att = "You confidently lunge forward, driving the blade of your "+
        		  weapon->short()+" deep into "+QTNAME(ob)+"'s "+hdesc+".";
    ...
    switch(hitresult[0])
    {
        ...
        case W_KNIFE:
            ...
            break;
        default:
            dam = ({ "scratch "+QTNAME(ob)+"'s "+hdesc+" with "+
              "your "+weapon->short()+".",
              "scratches your "+hdesc+" with "+POSSESSIVE(liv)+
              " "+weapon->short()+".",
              "scratches "+QTNAME(ob)+"'s "+hdesc+" with "+
              POSSESSIVE(liv)+" "+weapon->short()+"." });
              }
         ...
         aud = QCTNAME(liv)+" brings up "+POSSESSIVE(liv)+" "+
             shield->short()+" to block a blow from "+QTNAME(ob)+
         " and "+dam[2];
             break;
            }
    ...
    case 6..12:
        ...
        case W_KNIFE:
            ...
            break;
        default:
            dam = ({ "hit "+POSSESSIVE(ob)+" "+hdesc+" with your "+
                 weapon->short()+".",
                 "hits your "+hdesc+" with "+POSSESSIVE(liv)+" "+
                 weapon->short()+".",
                 "hits "+QTNAME(ob)+"'s "+hdesc+" with "+
                 POSSESSIVE(liv)+" "+weapon->short()+"." });
              }
    ...
    case 40..50:
        ...
        default:
         ...
         " and "+dam[2];
           ...
           {
           att = "You daze "+QTNAME(ob)+" with a fierce shield "+
              "bash and "+dam[0];
         tar = QCTNAME(liv)+" dazes you with a fierce shield "+
              "bash and "+dam[1];
         aud = QCTNAME(liv)+" dazes "+QTNAME(ob)+" with a fierce "+
    ...
    if (weapons && liv->query_weapon(W_RIGHT)->query_wt() != W_POLEARM &&
      liv->query_weapon(W_LEFT)->query_wt() != W_POLEARM && random(3) == 0)
    {
    switch(hitresult[0])
    {
    case 1..5:
    ...
    case 43..50:
        ...
        case W_CLUB:
            ...
            break;
        case W_AXE:
            att = "Nearly slashing through "+QTNAME(ob)+"'s body with your "+
            weapons[1]->short()+", you also "+dam[0];
            tar = "As "+PRONOUN(liv)+" nearly slashes through your body with "+
    ...
    if (liv->query_steed() && !random(5))
    {
    horse = liv->query_steed();
    ...
    case 5..10:
        ...
        tar = QCTNAME(liv)+" whirls "+POSSESSIVE(liv)+" "+horse->steed_short()+
        " around and slams its flank into you.";
        aud = QCTNAME(liv)+" whirls "+POSSESSIVE(liv)+" "+horse->steed_short()+
    ...
    case 11..18:
        ...
        tar = QCTNAME(liv)+" spurs "+POSSESSIVE(liv)+" "+horse->steed_short()+
        " forward and it jerks its head upwards, nearly knocking you "+
        "to the ground.";
        aud = QCTNAME(liv)+" spurs "+POSSESSIVE(liv)+" "+horse->steed_short()+
    ...
    case 30..40:
        ...
        tar = QCTNAME(liv)+"'s "+horse->steed_short()+" rears back on its "+
        "hindlegs, smacking its front hooves into you heavily, and "+
        "knocking you backwards.";
        aud = QCTNAME(liv)+"'s "+horse->steed_short()+" rears back on its "+
        "hindlegs, smacking "+QTNAME(ob)+" heavily with its front "+
    ...
    case 41..50:
        ...
        tar = QCTNAME(liv)+" brings "+POSSESSIVE(liv)+" "+horse->steed_short()+
        " around, and the steed kicks you tremendously hard with its "+
        "hindlegs. You see stars.";
        aud = QCTNAME(liv)+" brings "+POSSESSIVE(liv)+" "+horse->steed_short()+
    ...
    if (!liv->query_prop("_i_quiet_knight"))
    ...
    liv->remove_prop("_kspecial_preparing");
    if (liv->query_wiz_level() || liv->query_name() == "Sylvanjr")
    liv->catch_msg("Hitresult = "+hitresult[0]+"\n"+
               "pen = "+hitres+"\n"+
               "Damage = "+hitresult[3]+"\n"+
               "Hit = "+to_hit+"    Weaponmod = "+weaponmod+"     Levmod = "+levmod+"\n");
    liv->add_prop("_kspecial_resetting", time());
    set_alarm(liv->query_speed(12), 0.0, &reset_special(liv));
    if (ob->query_hp() <= 0)
    {
    ...
    if (liv->query_attack() && medal->query_auto_charge())
    {
        liv->command("$charge");
    }
...
/* Pre-attack part of kattack, initial parsing, etc */
int
special(string str)
{
    ...
    if (stringp(how))
    {
        // Block added to allow the knights to use their kattack special if they are attacked
        // By a huge foe they don't dare to attack. Maybe need to add check to see if they are already in combat.
	enemy_ob = TP->query_attack();
        enemy_list = TP->query_enemy(-1);
        if (F_DARE_ATTACK(TP, ob) || member_array(ob, enemy_list) == -1 || !present(ob, environment(TP))) 
        {
            NF(how);
            return 0;
        }
    ...
    if (liv->query_prop("_kcharge_preparing"))
    {
       NF("You cannot kattack while charging.\n");
        return 0;
    }
    ...
    else if (sizeof(liv->query_weapon(-1)) == 2)
    {
         weapons = query_two_weapons(liv);
    }
    else
    ...
    set_alarm(liv->query_speed(3), 0.0, &do_special(ob, liv));
