...
#define KNIGHT_UPGRADE "/d/Krynn/guilds/knights/lib/knight_upgrade"
...
{
    int did_join, stat_extra, present_stat_extra;
    string leader_name;

    did_join = shadow_who->team_join(new_team_member);

    if (query_knight_level() == L_ROSE && did_join)
    {
        leader_name = shadow_who->query_name();
        present_stat_extra = new_team_member->query_stat_extra(SS_DIS);

        if (present_stat_extra)
        {
        // Player has increased SS_DIS and will not get more here!
            new_team_member->catch_msg("The leadership of " + leader_name +
                " is very inspiring, but does not affect you.\n");

            return did_join;
        }

        stat_extra = shadow_who->query_base_stat(SS_DIS) / 20 +
        ...
        case 8:
        case 9:
             new_team_member->catch_msg("The confident leadership of " +
                 leader_name + " gives you courage.\n");
             break;
        ...
        case 12:
        case 13:
             new_team_member->catch_msg("The bold leadership of " +
                 leader_name + " steadies your resolve.\nYou grow more " +
                 "determined.\n");
             say(QCTNAME (TP) + " seems more determined to face evil.\n");
             break;
        ...
        case 16:
        case 17:
             new_team_member->catch_msg("The inspired leadership of " +
                 leader_name + " kindles fire in your heart.\nYou grit " +
                 "your teeth, ready to withstand any evil on the face of " +
                 "Krynn.\n");
             say(QCTNAME(TP) + " grits " + HIS(TP) + " teeth, a daring " +
                 "look in " + HIS(TP) + " eyes.\n");
             break;
        ...
        default:
             if (stat_extra > 17)
             {
                 new_team_member->catch_msg("The epic leadership of " +
                     leader_name + " heartens your spirit.\nYou smile " +
                     "proudly, confident and relaxed.\n");
                 say(QCTNAME(TP) + " smiles proudly and looks confident " +
                     "and relaxed.\n");
             }
             break;
        }

        new_team_member->set_stat_extra(SS_DIS, stat_extra);
    }
...
team_leave(object leaving_member)
{
    ...
    {
        the_team = shadow_who->query_team();
        team_member = member_array(leaving_member, the_team);
        if (team_member >= 0)
        {
        // Others may have set a extra_stat, but at the moment I do not
        // care. Just remove SS_DIS when the player leaves the team.
            leaving_member->set_stat_extra(SS_DIS, 0);
        }
    }
  ...
  string name)
{
    ...
    NF("The Knights of Solamnia do not wield magic.\n");
    if (style == "magic")
    return 1;

    NF("The Knights of Solamnia do not tolerate thievery.\n");
    if (style == "thief")
    return 1;
...
toggle_auto_kattack()
{
    return auto_kattack = !auto_kattack;
}
...
upgrade_knight(object player)
{
    ...
    KNIGHT_UPGRADE->upgrade_knight(player);
}
...
{ 
    ...
    set_alarm(1.0, 0.0, &upgrade_knight(shadow_who));
    return;
}
