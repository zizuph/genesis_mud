...
int auto_kattack; /* will kattack auto restart? */
int auto_charge; /* Well charge auto restart? */
int auto_aid; /* Will the Knight auto assist in combat? */
...
toggle_auto_charge()
{
    return auto_charge = !auto_charge;
}
...
query_auto_load()
{
    ...
    auto_kattack + " " + auto_charge + " " + auto_aid;
}
...
init_arg(string str)
{
    int i = sscanf(str, "%d %d %d %d  %d %d %d", knight_level, knight_sublevel,
      generous, primary, auto_kattack, auto_charge, auto_aid);
    if (i < 2)
    {
	knight_level = 0;
	knight_sublevel = 0;
	...
	set_auto_kattack(auto_kattack);
	set_auto_charge(auto_charge);
	set_auto_aid(auto_aid);
...
init()
{
    if (!MEMBER(TP) && environment(TO) == TP)
    {
	write("You are not a member of the Solamnian Knights and may not "+
	  "possess this object.\n");
	remove_object();
    }
    if (!MEMBER(TP))
...
setup_positions(object me)
{
    ...
    {
	clone_object(KOBJ + "warden_shadow")->shadow_me(me);
    ...
    {
	clone_object(KOBJ + "swordmaster_shadow")->shadow_me(me);
    }

    if (positions[POSITIONS[2]] == me->query_name())
	clone_object(KOBJ + "general_shadow")->shadow_me(me);

    if (positions[POSITIONS[3]] == me->query_name())
	clone_object(KOBJ + "east_lt_shadow")->shadow_me(me);

    if (positions[POSITIONS[4]] == me->query_name())
	clone_object(KOBJ + "south_lt_shadow")->shadow_me(me);
