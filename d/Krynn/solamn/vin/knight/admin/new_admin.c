...
#define BACKUP_SAVE_FILE    "/d/Krynn/solamn/vin/knight/log/backup_new_admin"
...
request_extended_leave(string member, string excuse)
{
    ...
    vacation += ([ vacation[member] : ({ excuse, "unapproved" }) ]);
...
query_extended_leave(string member)
{
    ...
    return C(member)+" can take extended leave because "+vacation[member][0]+
	". This request was approved by "+vacation[member][1]+".";
}

public int
remove_extended_leave(string member)
{
    if (member_array(member, m_indexes(vacation)) == -1)
	return 0;
...
cleanup_extended_list()
{
    ...
    for (i=0; i<sizeof(mylist); i++)
    {
	    ...
	    remove_extended_leave(mylist[i]);
    }
...
*/
    ...
    find_player("morrigan")->catch_tell(str);
    notify_knight("eagledraco", str);
}
...
remove_invalid_conclave()
{
    ...
    for (i=0; i<sizeof(conclave); i++)
    {
        ...
        if ((how_long > 14 && query_extended_leave(L(conclave[i])) == 0) ||
	    how_long > 28)
        {
            if (is_grandmaster(conclave[i]))
            {
...
remove_expired_knight_actions()
{
    ...
    while (++i < size)
      if ((SECURITY)->exist_player(L(mylist[i][0])) ||
	   member_array(mylist[i][0], m_indexes(allknights)) != -1)
          tmplist += ({ mylist[1] });
...
remove_expired_knight_awards()
{
    ...
    while (++i < size)
      if (member_array(mylist[i][0], m_indexes(allknights)) != -1)
          tmplist += ({ mylist[1] });
