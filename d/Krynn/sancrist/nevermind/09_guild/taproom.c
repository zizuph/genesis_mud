...
string stuck = ""; /* Who's stuck in the tap? */
...
create_room()
{
    ...
    add_exit(GGUILD + "workshop", "south", "@@stuck_check");
    INSIDE
    LIGHT
...
drink(string str)
{
   ...
   write("and drink some water. Its taste strongly reminds you of fish.\n");
	say("And drinks some fresh water.\n");
...
clear2(object player)
{
	...
	stuck = (player->query_real_name());
	remove_item("tap");
	add_item("tap", "An ordinary tap. The " + player->query_nonmet_name()
		+ " holds " + POSSESSIVE(player)
		+ " finger into it.\n");
}
...
pull2(object player)
{
	...
	fish = 0;
	clone_object(GGUILD + "obj/fish.c")->move(TO);
