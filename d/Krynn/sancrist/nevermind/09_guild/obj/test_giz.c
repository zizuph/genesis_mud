 ...
 * guild/module_desc.o.
 *
 * This is Dajala's personal Gizmo slightly modified 
...
#include "/d/Krynn/gnome/guild/modules_neu.h"
...
create_object()
{
    ...
    set_long(   "This is the symbol of your proud heritage as a "+
      ...
      "This seems to be the Guildmasters edition.\n");
    ...
    seteuid(getuid());
    restore_object("/d/Krynn/gnome/guild/module_desc_new");
}
...
modules()
{
    ...
    write("\nTry 'examine <module>' for a closer look at the module.\n");
    return 1;
}
...
soul(object ob)
{
    ob->add_cmdsoul("/d/Krynn/gnome/guild/obj/kgnome_s.c");
    ob->update_hooks();
}
...
reduce_water()
{
    ...
    if((water == 0) && fire)
    {
	E(TO)->catch_msg("OhNo!ThereIsNoWaterLeftToCreateSteam!\n");
	E(TO)->catch_msg("An alarming bell rings and the Steamengine"
	  + "EmergencyShutDownSequence is initiatied.\n");
...
burn_wood()             /* module 4 and 12 */
{
    ...
    seteuid(getuid());
    (GOBJ + "kgnome_s.c")->set_alarm(50.0, -1.0, "distill_contents", E(TO));
}
...
ext_fire()
{
    ...
    E(TO)->catch_msg("Your steamengine exhales a final cloud of steam and "
      + "ceases working.\n");
    seteuid(getuid());
    /* (GOBJ + "kgnome_s.c")->ext_everything(E(TO)); */
}
