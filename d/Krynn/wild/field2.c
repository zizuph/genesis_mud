...
reset_wild_room()
{
    if(!present("farmer"))
    {
        clone_object(MON + "farmer")->move(TO);
        tell_room(TO, "A farmer is here harvesting the wheat.\n");
    }
}
...
create_wild_room()
{
    ...
    set_long(
        ...
        "Further west is a forest of tall " +
        "trees. To the south a road runs west into the forest " +
        "and east to a distant mountain range. Far to the " +
    ...
    add_exit(TDIR + "field3", "east");
    add_exit(TDIR + "field1", "west");
    add_exit(TDIR + "east3", "south");

    add_item("forest", BS(
        "West of you, beyond the field, stands a forest of tall trees. " +
