...
create_wild_room()
{
    set_short("East road");
    set_long(
        "You are on the east road. The road leads west to a forest " +
        "and the village of Solace. " +
        "About a mile to the south, thick woodlands follow the road east towards " +
        "some low mountains. Farther south, a lone peak is visible " +
    ...
    add_exit(TDIR + "east2", "west");
    add_exit(TDIR + "grass2", "south");
    add_exit(TDIR + "east4", "east");
    add_exit(TDIR + "field2", "north");
