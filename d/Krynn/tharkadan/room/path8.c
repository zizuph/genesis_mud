...
create_tharkadan_room()
{
    ...
    add_exit(ROOM + "path7","north",0);
    add_exit(ROOM + "path9","south",0);
    add_invis_exit(ROOM + "fforest1", "southeast", "@@enter_forest");

    add_item(({"woods","mountain woods","forest","vale","trees",
        "fallen forest"}), "To your southeast the pass opens up to a " +
...
long_descr()
{
    return long_mountain_desc()+"To your southeast the trail " +
         "opens up to a vale where aspen, fir and lodgepole " +
         "pine form a secluded mountain woods.\n";
}
