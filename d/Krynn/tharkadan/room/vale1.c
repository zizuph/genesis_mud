...
create_tharkadan_room()
{
    ...
    add_exit(ROOM + "vale2","northwest",0);
    add_exit(ROOM + "vale3","west",0);
    add_exit(ROOM + "vale4","southwest",0);     
    add_exit(ROOM + "path16","southeast",0);

    add_item(({"narrow canyon","canyon","dark narrow canyon"}),
        "To your southeast opens a dark narrow canyon between two " +
        "ridges of the Tharkadan Mountain Range.\n");
    add_item(({"ridges","two ridges","ridge"}),
        "The walls of two ridges make a narrow canyon that leads out of " +
        "the valley to your southeast.\n");
...
long_descr()
{
    return "You stand in a wide vale rimmed by " +mountain_desc()+
        "Aspens and firs grow in abundance here and the vale is " +
        "somewhat protected from the wind and cold. Rising from the " +
        "the floor of this isolated valley like a lone tree on a " +
        "prairie is a towering peak, possibly an ancient volcano. " +
        "An opening to a dark, narrow canyon is to your southeast.\n";
}
