...
reset_tharkadan_room()
{
    ...
    if(!objectp(dwarf2))
    {
        dwarf2 = clone_object(LIVING + "theiwar_miner");
        dwarf2->move(TO, 1);
    }
...
create_tharkadan_room()
{
    ...
    add_exit(CAVERNS + "upper_mines5","west","@@passage_blocked_west");
    add_exit(CAVERNS + "upper_mines3","east", 0);
    add_exit(CAVERNS + "upper_mines6","south","@@passage_blocked_south");
    add_exit(CAVERNS + "upper_mines8","north","@@passage_blocked_north");
...
short_descr()
{
    return "an intersection of excavated tunnels, deep within a " +
        "dormant volcano";
}
...
long_descr()
{
    return "You stand at an intersection of excavated tunnels deep " +
        "within the depths of a dormant volcano. Expertly carved out " +
        "you have little doubt this was the work of the master " +
        "subterranean miners, the dwarves. Tunnels lead off here to " +
        "your north, east, west and south.\n";
}
