...
reset_tharkadan_room()
{
    ...
    if(!objectp(dwarf3))
    {
        dwarf3 = clone_object(LIVING + "theiwar_miner");
        dwarf3->move(TO, 1);
    }
...
create_tharkadan_room()
{
    ...
    add_exit(CAVERNS + "upper_mines16","northwest", 0);
...
long_descr()
{
    return "You stand at the end of an excavated tunnel deep within the " +
        "depths of a dormant volcano. Expertly carved out you have " +
        "little doubt this was the work of the master subterranean " +
        "miners, the dwarves. The tunnel leads off to your northwest.\n";
}
