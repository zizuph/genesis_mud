...
reset_tharkadan_room()
{
    if(!objectp(dwarf1))
    {
        dwarf1 = clone_object(LIVING + "theiwar_axeman");
        dwarf1->move(TO, 1);
    }
...
create_tharkadan_room()
{
    ...
    add_exit(CAVERNS + "upper_mines12","west", 0);
    add_exit(CAVERNS + "city_entrance","east","@@passage_blocked_east");
...
long_descr()
{
    return "You stand within an excavated tunnel deep within the depths " +
        ...
        "the dwarves. The tunnel continues here to your east and west.\n";
}
