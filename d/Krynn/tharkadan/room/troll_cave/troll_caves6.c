...
reset_tharkadan_room()
{
    ...
    if(!objectp(troll3))
    {
        troll3 = clone_object(LIVING + "cave_troll");
        troll3->move(TO, 1);
    }
...
create_tharkadan_room()
{
    ...
    add_exit(TROLL_CAVES + "troll_caves7","south","@@troll_block");
    add_exit(TROLL_CAVES + "troll_caves5","northwest",0);
...
long_descr()
{
    return "You stand within a narrow cave network that runs through " +
        ...
        "continues into darkness to your south and northwest.\n";
}
...
troll_block()
{
     if(objectp(troll) && CAN_SEE(troll, TP))
     {
        write("The " +troll->short()+ " blocks you from travelling " +
            "in that direction.\n");
        return 1;
     }

     if(objectp(troll2) && CAN_SEE(troll2, TP))
     {
        write("The " +troll2->short()+ " blocks you from travelling " +
            "in that direction.\n");
        return 1;
     }

     if(objectp(troll3) && CAN_SEE(troll3, TP))
     {
        write("The " +troll3->short()+ " blocks you from travelling " +
            "in that direction.\n");
        return 1;
     }
