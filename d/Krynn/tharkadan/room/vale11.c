...
reset_tharkadan_room()
{
    ...
    if(!objectp(giant))
    {
      giant = clone_object(LIVING + "stonegiant");
      giant->sit_down();
      giant->move(TO);
    }

   if(objectp(giant) && (!giant->query_attack()))
      giant->sit_down();
...
create_tharkadan_room()
{
    ...
    add_exit(ROOM + "vale12","southwest",0);
    add_exit(ROOM + "vale10","south",0);     
    add_exit(ROOM + "vale6","east",0);
    add_exit(ROOM + "vale5","northeast",0);
