...
create_tharkadan_room()
{
    ...
    add_exit(ROOM + "ledge7","northeast",0);
    ...
    add_item(({"towering peak","peak","ancient volcano","volcano",
         ...
         "ten meters to your west another ledge continues up the peak. " +
    ...
    add_cmd_item(({"across the wall","across wall","across the footholds",
         ...
         "run","@@run_across_wall");
    add_cmd_item(({"to ledge","ledge"}),"jump","Ten meters is too far " +
         "to jump. You'll have to figure out another way across.\n");
...
long_descr()
{
    return "You stand high on a ledge running along the side of a " +
        ...
        mountain_desc()+ "Approximately ten meters to your west you " +
        "see another ledge continue up the mountain.\n";
}
...
fourth_jump(object player, int jump)
{
    ...
    write("You grab the edge of the western ledge and skillfully pull " +
        "yourself up, successfully making it across!\n");
    say(QCTNAME(player) + " grabs the edge of the western ledge and " +
        "skillfully pulls " +HIM(player)+ "self up and over!\n");
    player->move_living("M", ROOM + "ledge9", 1, 0);
    tell_room(E(player), QCTNAME(player)+ " pulls " +HIM(player)+ 
        "self up from the edge and on to the ledge.\n", ({ player }) ); 
    return;       
}
...
*/
    ...
    write("You land on the second narrow shelf with your left foot and " +
        "begin nimbly sprinting along the wall face towards the next " +
        "shelf!\n");
    say(QCTNAME(player) + " lands on the second narrow shelf with " +
        HIS(player)+ " left foot and begins sprinting along the wall " +
...
*/
    ...
    write("You land on the first narrow shelf with your right foot and " +
        "launch yourself towards the next foothold!\n");
    say(QCTNAME(player) + " lands on the first narrow shelf with " +
        HIS(player)+ " right foot and then launches " +HIM(player)+ 
...
run_across_wall()
{
    ...
    write("You take a handful of steps back and then run towards the " +
        "ledge, jumping towards the first foothold jagging out from the " +
        "side of the steep peak wall.\n");
