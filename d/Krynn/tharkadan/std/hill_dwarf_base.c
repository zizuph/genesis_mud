...
/* This is a base file for hill dwarf npcs.
 *
 *
 ...
 *
 * Based on code from 6.01.2004, by Blizzard.
 * Arman April 2017.  
 */
...
setup_hill_dwarf(int level)
{
    ...
    set_base_stat(SS_STR, 75 + (level * 25) + random(level * 10));
    set_base_stat(SS_DEX, 65 + (level * 18) + random(level * 10));
    set_base_stat(SS_CON, 75 + (level * 25) + random(level * 10));
    set_base_stat(SS_INT, 65 + (level * 15) + random(level * 5));
    set_base_stat(SS_WIS, 65 + (level * 15) + random(level * 5));
    set_base_stat(SS_DIS, 75 + (level * 25) + random(level * 8));
    ...
    set_all_hitloc_unarmed(level * 10);

    // resistance to magic and poison
    add_prop(OBJ_I_RES_MAGIC, level * 5);
    add_prop(OBJ_I_RES_POISON, level * 5);
    ...
    // set_skill(SS_GUILD_SPECIAL_SKILL, 84 + (level * 4));
    // set_skill(BATTLERAGING_SKILL, 84 + (level * 4));
...
set_dwarf_description(int level)
{
    ...
    {
    case 0..1:
        str = "Before you stands a lightly-armoured hill dwarf from the " +
              "Tharkadan mountain range. At his side swings a well-used " +
              "weapon - common apparel amongst all dwarven clansmen.\n"; 
    break;
    case 2:
        str = "Before you stands a tough-looking hill dwarf, who looks " +
              "at you from beneath his helmet watchfully. " +
              "Being a skilled warrior of the hill dwarves he takes " +
              "defending his village seriously, and everyone who dares " +
              "to invade this land should be prepared for a hard fight.\n"; 
    break;
    case 3:
        str = "When you look at this well-built dwarf you understand " +
              ...
              "the Tharkadan mountain range are not to be made enemies " +
              "of lightly.\n";
    break;
    default:
        str = "This powerfully-built dwarf looks like a tough and " +
              "dangerous opponent. He must be an elite warrior of " +
...
hook_attacked(object enemy)
{
    ...
    break;
    case 2:
        command("emote roars 'Ye stupid maggot! Yer remains will rot " +
            "here!'");
    break;
    }
