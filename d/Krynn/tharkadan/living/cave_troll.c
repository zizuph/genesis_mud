...
string *gAdj1 = ({ "stooped", "grey-skinned", "snarling", "cunning",
    "red-eyed", "hunched", "beastly", "drooling", "gaunt" });
...
create_krynn_monster()
{
    ...
    set_short(adj1+ " cave troll");
    add_prop(LIVE_I_NEVERKNOWN, 1);

    set_long("Before you stands a cave troll, a smaller yet no less savage " +
        "cousin to the larger forest trolls of Krynn. Long, yellow fangs " +
        "reach out of his cruel mouth, and a foul smell surrounds him. " +
        "His furless grey skin makes him making him like a shadow in the " +
        "subterranean caverns he makes his lair. His eyes glow red with " +
        "a cunning animal form of intelligence.\n");
    ...
    set_stats(({180,125,160,120,120,130}));
    set_hp(query_max_hp());

    set_all_hitloc_unarmed(20);
    set_all_attack_unarmed(37, 40);
...
limb_fall_off(object attacker, string limb)
{
    ...
    attacker->catch_tell("You severed " + TO->query_the_name(attacker) + "'s " + 
        limb + ", which falls to the ground.\n");
...
set_hunting(object ob)
{
    ...
    if (member_array(ob, gHunted) == -1)
    {
        if(E(ob) == E(TO))
        {
            ob->catch_tell(TO->query_The_name(ob) + " sniffs at you and a " +
                "light glints in his eyes.\n");
