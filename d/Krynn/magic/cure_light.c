...
public mixed cure_success(object caster, object *targets);

public void
create_spell_cure()
{
    set_element(SS_ELEMENT_LIFE, 20);
    set_form(SS_FORM_TRANSMUTATION, 25);
    set_spell_time(3);
    set_mana_spell(100);
    set_find_target(cure_target);
    set_spell_message(cure_message);
    set_spell_success(cure_success);
}
...
public mixed
cure_success(object caster, object target)
{
    int result;

    if ((result = caster->resolve_task(TASK_SIMPLE,
          ({ SS_WIS, SKILL_AVG, SS_SPELLCRAFT, SS_ELEMENT_LIFE,
             SS_FORM_TRANSMUTATION, SKILL_END }) )) <= 0)
        return 0;
