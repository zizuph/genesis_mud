...
#define SPAWN       LIVING + "triton_wizard"
...
create_sea_room()
{
    ...
    add_exit(ROOM + "uw9_2", "up", "@@swim_up", 1, 1);
    add_exit(ROOM + "xxx", "down", "@@swim_down", 1, 1);
...
swim_down()
{
    write("You try and dive down further in to the dark waters, " +
        "however the current hinders your progress.\n");
    say(QCTNAME(TP) + " tries to dive deeper into the dark sea, however " +
        "the current is too strong to swim down any further.\n");
    return 1;
}
