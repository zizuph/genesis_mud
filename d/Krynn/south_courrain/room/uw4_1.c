...
create_sea_room()
{
    ...
    add_item(({"courrain ocean","ocean","sea"}),
        "You swim near the surface of the Courrain Ocean that stretches " + 
        "along the coastline of Ansalon.\n");
    ...
    add_exit(ROOM + "s_courrain-26-20-s", "up", "@@swim_up", 1, 1);
    add_exit(ROOM + "uw4_2", "down", "@@swim_down", 1, 1);

    set_sink_path(ROOM + "uw4_2");
...
swim_down()
{
    ...
    write("You try and dive down further in to the murky waters, " +
        "however the current hinders your progress. You'll need to " +
        "improve your swimming if you wish to swim deeper.\n");
    say(QCTNAME(TP) + " tries to dive deeper into the murky sea, however " +
...
short_descr()
{
    return "near the surface of the Courrain Ocean";
}
 
string
long_descr()
{
    return "You are near the surface of Courrain Ocean within dark and " +
