/*
 * Southern Courrain Ocean base room using Mapserver created by Arman
 *
 * When adding rooms to magic map, you need to go to each sailing room
 * and:
 *    execr "/secure/map_central"->add_maplink("/d/Krynn/south_courrain/room/s_courrain-4-8-B",
 *        "/d/Krynn/open/MAP_FILES/south_courrain_map.txt");
...
#define DEBUG(x)  find_player("arman")->catch_msg("[Cour] " + x + "\n")
...
string *herbs = 0;
static string *sailing_rooms = ({ "B", "s", "p", "c", "d", "e",
                                  "f", "g", "h", "i", "x", "y", "z" });
...
query_file_with_map()
{
    return S_COURRAIN + "courrain_map";
}
...
find_room_file(int off)
{
    ...
    // DEBUG("find_room_file called, off = " +off);
...
load_map_room_file(string file, string p)
{
    ...
    // DEBUG("load_map_room_file code = " +code);
...
load_map_room(int off)
{
    ...
    // DEBUG("load_map_room loaded, off: " +off);
...
add_row_exits(string place, string cmd, mixed efunc, mixed tired)
{
    ...
    // DEBUG("Add row exit added in add_row_exits for " +place);
    add_row_exit(place, cmd, efunc, tired);

    // DEBUG("add_row_exits completed and returns");
    return;
}
...
add_map_exits()
{
	...
	{
            // DEBUG("add_row_exit called in add_map_exits (1)");
	    add_row_exits(find_room_file(i), Std_dir[i], &block_map_room(i), 1);
	    Map_exit_mask[i] = 1;
	}
	else if (Relative == 1)
        {
            // DEBUG("add_row_exit called in add_map_exits (2)");
...
add_std_exits()
{
    // DEBUG("add_std_exits function called");
    if (!Check || Relative)
	return;
    Relative = -1; /*** room has standard exits ***/

    // DEBUG("add_map_exits function called from add_std_exits");
    add_map_exits();
}
...
exits_description()
{
    ...
    m_exits = ({ });
    exits = ::query_exit_cmds() - Std_dir;
    for (i = 0 ; i < 8 ; i++)
	if (Map_exit_mask[i] && !exit_not_obvious(Zz, Map[i..i]))
	    m_exits += ({ exit_map_cmd(i) });
...
do_update_map_room(object tp, int y, int x, string nnn)
{
    ...
    // DEBUG("do_update_map_room called for file: " +file+ ".");
...
update_map_rooms()
{
    ...
    // DEBUG(file+ " called in update_map_rooms");
...
create_map_room()
{
    ...
    set_area_handler(S_COURRAIN + "courrain_placer");
...
reset_sea_room()
{
    ...
    // DEBUG("Completed reset_sea_room");
}
...
create_sea_room()
{
    ...
    // DEBUG("Create_sea_room called, Check = " +Check);
    ...
    // DEBUG("create_sea_room Name = " +Name);
    // DEBUG("sscanf = " +sscanf(master, "%s"+Name+"-%d-%d-%s", Path, Xx, Yy, Zz));
    if (sscanf(master, "%s"+Name+"-%d-%d-%s", Path, Xx, Yy, Zz) == 4)
    {
        // DEBUG("Check set to 1");
	Check = 1;
	get_local_map(Xx, Yy);
    ...
    // DEBUG("Got to reset_sea_room() in create_sea_room");
    reset_sea_room();
}
...
try_row(string str)
{
	...
	{
            // DEBUG("Rel_dir off called in try row");

	    off = (off + tp->query_prop(OFFSET) + 7) % 8;
	}
	else 
        {
            DEBUG("other exit called in try_row");
	    return ::try_row(str);
        }
	...
	set_alarm(0.0, 0.0, &remove_prop(ALLOW_MOVE_OFF));
        ...
        //DEBUG("Final off value = " +off);
    }

    off--;
    if (!(d = load_map_room(off)))
    {
        // DEBUG("!(d = load_map_room(off)");
	return 0;
    }
    if (block_map_room(off))
    {
        //DEBUG("block_map_room(off)");
	return 0;    
    }
...
long_desc(string long)
{
    ...
    desc = (gSpecial_short ? gSpecial_short : short()) +
	...
	case EARLY_MORNING:
	    ...
	    break;
	case MORNING:
	    str = "The rising sun's morning rays shine down over the " +
	    "high sea. While not " +
