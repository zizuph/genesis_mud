/*
 *   Increased size and damage to be a bigger challenge.
 *   Arman, May 2021
 */
...
#define SHIELD          (SOBJ + "great_dr_turtle_shell")
#define SHELL           ("/d/Ansalon/goodlund/bloodsea/obj/broken_shell")
#define TOOTH           ("/d/Krynn/common/weapons/dragontooth")
...
create_creature()
{
    ...
    set_short("gigantic dragon turtle");
    ...
    set_long("Before you is a mythical creature feared by all mariners, a " +
        "dragon turtle! And not just any dragon turtle, but one of " +
        "gigantic proportions, its scales a dark black, and rivalling " +
        "the size of a galleon!\n");

    set_stats(({400, 400, 450, 350, 350, 300}));
    ...
    // Heavily armoured, high damage, special attack, quickness, heals
    set_exp_factor(200);

    set_alignment(-800);

    set_attack_unarmed(0, 55, 70, W_IMPALE, 50," bite");
    set_attack_unarmed(1, 60, 50, W_BLUDGEON, 20, "tail");
    set_attack_unarmed(2, 40, 60, W_SLASH, 60, "right claw");
    set_attack_unarmed(3, 40, 60, W_SLASH, 60, "left claw");

    set_hitloc_unarmed(0, 90, 20, "head");
    set_hitloc_unarmed(1, 120, 50, "shell");
    set_hitloc_unarmed(2, 70, 10, "tail");
    ...
    add_prop(CONT_I_VOLUME,     950000);
    add_prop(CONT_I_WEIGHT,     950000);

    set_act_time(7);
    add_act("emote slaps its tail in the water furiously.");

    add_act("emote dives under the water, then surfaces " +
    ...
    if (CLONE_MASTER->may_clone(SHIELD, 15))
        add_leftover(SHIELD, "shell", 1, "", 0, 1);
    else
        add_leftover(SHELL, "shell", 1, "", 0, 1);

    add_leftover(TOOTH, "tooth", 2, "", 0, 1);
...
leave_message()
{
    tell_room(E(TO),"The gigantic dragon turtle dives into the water " +
        "and vanishes.\n", TO);
    TO->remove_object();
}
