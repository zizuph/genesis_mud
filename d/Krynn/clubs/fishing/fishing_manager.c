...
public void
add_top_fish_sale(mixed player, string fish_desc, int value)
{
    mapping list = query_fishing_property("top_fish_sale");

    if (!m_sizeof(list))
    {
        // FISHING_DEBUG("List size: " +m_sizeof(list));
        list = ([ "one" : ({ player, fish_desc, value }) ]);
    }
    else if (member_array("one", m_indexes(list)) == -1)
    {
        // FISHING_DEBUG("No fish_desc detected");
    ...
    {
        // FISHING_DEBUG("List size: " +m_sizeof(list));
        list += ([ top_three[m_sizeof(list)] : ({ player, fish_desc, value }) ]);
    }
    else
    {
        string new_rank = "one";
        mixed * lowest_value = list[new_rank];
        mixed new_player = lowest_value[0];
        string new_fish = lowest_value[1];
        int new_value = lowest_value[2];

        foreach(string rank, mixed * fish_stats: list)
        {
            new_player = lowest_value[0];
            new_fish = lowest_value[1];
            new_value = lowest_value[2];
            // FISHING_DEBUG("Lowest value: " +lowest_value[2]);

            if (fish_stats[2] < new_value)
            {
                lowest_value = list[rank];
                new_rank = rank;
            }
        }

        // FISHING_DEBUG("Lowest value: " +lowest_value[2]+ ", new value: "+value);
            
        if(lowest_value[2] < value)
            list += ([ new_rank : ({ player, fish_desc, value }) ]);
    }

    set_fishing_property("top_fish_sale", list);
}
