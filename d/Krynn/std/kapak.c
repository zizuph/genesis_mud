/*
 * A stronger draconian soldier 
 *
 * Revisions by Stralle @ Genesis: 000921
 *   Added coating their bladed weapon with their tongues to make it
 *   poisoned.
 */

#pragma save_binary
#pragma strict_types

#include "/d/Krynn/common/defs.h"
#include "draconian.h"
#include <ss_types.h>
#include <wa_types.h>
#include <macros.h>

inherit DRACONIAN;

/*
 * Function name: set_kapak_skills
 * Description:   Calc and set the stats and skills for the draconian.
 */
nomask static void
set_kapak_skills()
{
    int i, level;

    for (i = 0; i < 6; i++) /* 30 - 57 */
        set_base_stat(i, (random(4) + 3) * 6 + 12 + random(10));

    level = query_draconian_level();

    set_skill(SS_DEFENCE,     level * 6 + random(20) + 10);
    set_skill(SS_PARRY,       level * 6 + random(8) + 10);
    set_skill(SS_WEP_SWORD,   level * 6 + random(8) + 10);
    set_skill(SS_WEP_KNIFE,   level * 6 + random(8) + 10);
    set_skill(SS_WEP_CLUB,    level * 6 + random(8) + 10);
    set_skill(SS_WEP_POLEARM, level * 6 + random(8) + 10);
    set_skill(SS_WEP_JAVELIN, level * 6 + random(8) + 10);
    set_skill(SS_WEP_AXE,     level * 6 + random(8) + 10);
    set_skill(SS_AWARENESS,   level * 5);

    set_draconian_alignment();
}

/*
 * Function name: create_kapak
 * Description:
 */
public void
create_kapak()
{
}

/*
 * Function name: create_draconian
 * Description:
 */
nomask public void
create_draconian()
{
    set_kapak_skills();
    set_draconian_type(KAPAK);

    create_kapak();

    add_name("kapak");

    set_exp_factor(120);
    add_prop(OBJ_I_RES_FIRE, 80);
    add_prop(OBJ_I_RES_ACID, 100);    

/*
    add_act("emote flaps its wings.");
*/
}

int
do_lick_weapon(object wep)
{
    object sh;

    if (wep->query_dt() & W_SLASH || wep->query_dt() & W_IMPALE)
    {
        if (wep->query_num_poisons())
        {
            /* Already coated with poison, continue */
            return 0;
        }
                
        sh = clone_object(KAPAK_POISON_SH);
        if (!sh->shadow_me(wep))
        {
            /* Bug!! Write a log somewhere */
            sh->remove_object();
            return 0;
        }

        this_object()->catch_tell("You pause mometarily " +
            "to sadistically lick your " + wep->short() + ".\n");
        tell_room(E(TO), QCTNAME(TO) + " pauses momentarily " +
            "to sadistically lick " + TO->query_possessive() +
            " " + wep->short() + ".\n", this_object());

        return 1;
    }

    /* A bludgeon weapon */
    return 0;
}

/*
 * Function name: special_draconian_attack
 * Description:   Called from the draconian special_attack
 *                routine. By redefining this, draconians can easily attack
 *                with spells or take special actions when engaged
 *                in combat.
 * Arguments:     victim (the one we are fighting right now)
 * Returns:       0 - if we want the round to continue
 *                1 - if we are done with this round
 */
public int
special_draconian_attack(object victim)
{
    object *wep = this_object()->query_weapon(-1);
    int x = -1;

    setuid();
    seteuid(getuid());

    switch (sizeof(wep))
    {
        case 0:
            /* No weapons wielded. Do nothing */
            break;
        case 1:
            /* One weapon wielded, check if it is a bladed one */
            return do_lick_weapon(wep[0]);
        case 2:
            /* Wielding two weapons, check both */
            if (!do_lick_weapon(wep[0]))
                return do_lick_weapon(wep[1]);
            else
                return 0;
    }
    
    return 0;
}

/*
 * Function name: second_life
 * Description:
 * Arguments:     killer - 
 * Returns:
 */
public int
second_life(object killer)
{
    object corpse, *team;

    team = killer->query_team_others();
    tell_room(E(TO),"The draconian melts down into a pool of " +
        "liquid!\n");

    corpse = clone_object(ACID);
    corpse->set_my_victims(({ killer }) + ({ team }));
    corpse->move(E(TO));
}

/*
 * Function name:
 * Description:
 * Arguments:
 * Returns:
 */
