 ...
 * Added flame-bladed glaive for 11-14 level draconians.
 * Added dragon-hide boots and scale-plated chainmails for 11-15 level
 * draconians. And set limits for those items.
 */
 ...
 *
 * Removed the Dragonarmy salute, as members outside of one army
 * wouldn't salute another.
 */
#define DRAC_MEET ([ "Priests of Takhisis" : ({ "dnod %s" }), \
...
#define WEAPONS ({ \
    ...
    14, \
      ({ AWEP + "pole/aurak_halberd", 25, \
         KRWEAPON + "flame_glaive", 25, \
	 SPLAINS + "bclaymore", 25, \
         ({ SPLAINS + "bsword", SPLAINS + "bknife" }), 25 \
      }), \
   })
...
#define ARMOURS ({ \
    ...
    11, \
	 ...
	 ({ SPLAINS + "blargeshield", 25, SPLAINS + "bleggings", 25, \
            SPLAINS + "bshinguard", 20, SPLAINS + "bheavyboots", 20, \
            KRARMOUR + "hide_boots", 10 }) \
      }), \
    13, \
      ({ ({ SPLAINS + "barmour1", 100 }), \
         ({ SPLAINS + "blargeshield", 50, "", 50 }), \
         ({ ({ SPLAINS + "bleggings", 50, SPLAINS + "bshinguard", 50 }), 50, \
            ({ KRARMOUR + "hide_boots", 50, \
               SPLAINS + "bheavyboots", 50 }), 50, \
         }), \
         ({ SPLAINS + "bdragonhelm", 50, "", 50 }) \
      }), \
    14, \
      ({ ({ SPLAINS + "bdragonarm", 50, KRARMOUR + "elite_chain", 50 }), \
         ({ ({ SPLAINS + "bshinguard", SPLAINS + "bheavyboots" }), 50, \
            KRARMOUR + "hide_boots", 50 }), \
         SPLAINS + "bdragonhelm" \
      }), \
    15, \
      ({ \
	 ({ SPLAINS + "bdragonarm", 30, KRARMOUR + "elite_chain", 70 }), \
         ({ ({ SPLAINS + "bshinguard", SPLAINS + "bheavyboots" }), 50, \
            KRARMOUR + "hide_boots", 50 }), \
          SPLAINS + "bdragonhelm" \
      }) \
...
#define LIMITS ([ \
       ...
       KRARMOUR + "hide_boots" : ({ 3, SPLAINS + "bheavyboots" }), \
        SPLAINS + "bdragonarm" : ({ 3, SPLAINS + "barmour1" }), \
      KRARMOUR + "elite_chain" : ({ 2, SPLAINS + "bdragonarm" }), \
     KRWEAPON + "flame_glaive" : ({ 3, AWEP + "pole/aurak_halberd" }), \
...
/* Death objects */
#define LIGHTNING   "/d/Krynn/common/objects/lightning"
