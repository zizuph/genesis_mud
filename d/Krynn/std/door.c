/* This file is copyrighted by me Chad Musick a.k.a. Quetzl@Genesis as well
 * as by Chalmers Dataforening since much of the code is theirs.
 * (In other words, please don't take it to other muds until it is put in
 * the mudlib. Thanks.
 */
/*
 * /std/door.c
...
/**
 * Navarre June 27th 2007, changed door_name[0] to
 * query_door_name()[0], as door_name is a function
 * and gave runtime errors.
 */
...
create_object()
{
    ...
    open_commands = ({});
    close_commands = ({});
    pick = 40 + random(20); /*100;*/
    add_prop(OBJ_I_WEIGHT, 60000);
    add_prop(OBJ_I_VOLUME, 80000);
    add_prop(DOOR_I_HEIGHT, 300); /* Default height 3 meters. */
...
this_door(string arg)
{
    ...
    ob = FIND_STR_IN_OBJECT(arg, environment());
...
do_open_door(int other)
{
    ...
    if (other)
        tell_room(env, check_call(open_mess[1]));
    if (strlen(pass_commands[0]) <= 2 && sizeof(pass_commands) > 1)
        environment()->add_exit(other_room, pass_commands[1], "@@pass_door");
    else
        environment()->add_exit(other_room, pass_commands[0], "@@pass_door");
    open_status = 1;
}
...
do_close_door(int other)
{
    ...
    if (other)
        tell_room(env, check_call(close_mess[1]));
    if (strlen(pass_commands[0]) <= 2 && sizeof(pass_commands) > 1)
        environment()->remove_exit(pass_commands[1]);
    else
        environment()->remove_exit(pass_commands[0]);
    open_status = 0;
}
...
void
do_lock_door(int other)
{
    if (other)
        tell_room(environment(), check_call(lock_mess[1]));
    lock_status = 1;
}
...
void
do_unlock_door(int other)
{
    if (other)
        tell_room(environment(this_object()), check_call(unlock_mess[1]));
    lock_status = 0;
}
 ...
 */
string
query_door_id(){ return "_to_" + other_room; }
...
load_other_door()
{
    ...
    if (!(ob = present("_to_" + MASTER_OB(environment()), ob)))
    {
...
public int
compare_doors(object ob)
{
    if ((ob->query_door_id() == query_door_id()) && ob != this_object())
        return 1;
}
...
{
    add_name("_to_" + query_other_room());
    if (sizeof(filter(all_inventory(dest), "compare_doors", this_object())))
    {
...
standard_open_mess2()
{
    open_mess = ({ standard_open_mess1(), "The " + short() + 
        " opens.\n" });
    return open_mess[1];
}
...
standard_fail_open2()
{
    fail_open = ({ standard_fail_open1(), "The " + short() + " is " +
        "locked.\n" });
    return fail_open[1];
}
...
{
    close_mess = ({ standard_close_mess1(), "The " + short() +
        " closes.\n" });
    return close_mess[1];
}
...
standard_lock_mess2()
{
    lock_mess = ({ standard_lock_mess1(), "You hear a soft 'click' from " +
        "the " + short() + ".\n" });
    return lock_mess[1];
}
...
standard_fail_lock2()
{
    fail_lock = ({ standard_fail_lock1(), "You must close the " + short() +
        " first.\n" });
    return fail_lock[1];
}
...
{
    unlock_mess = ({ standard_unlock_mess1(), "You hear a soft 'click' " +
       "from the " + short() + ".\n" });
    return unlock_mess[1];
}
...
standard_door_desc()
{
    set_door_desc("A sturdy looking " + short() + ".\n");
    return door_desc;
}
...
standard_fail_pass()
{
    set_fail_pass("The " + short() + " is closed.\n");
    return fail_pass;
}
...
standard_fail_close()
{
     fail_close = "The " + short() + " is already closed.\n";
     return fail_close;
}
...
{
    fail_unlock = "The " + short() + " is already unlocked.\n";
    return fail_unlock;
}
