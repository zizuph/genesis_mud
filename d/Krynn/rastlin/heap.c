...
split_heap(int num)
{
    find_player("rastlin")->catch_msg("SPLIT\n");
    mark_state();
    if (item_count <= num)
    {
	leave_behind = 0;
...
leave_env(object env, object dest)
{
    ...
    find_player("rastlin")->catch_msg("LEAVE\n");
    if (!leave_behind)
	return;
...
enter_env(mixed env, object old)
{
    object *ob;
    int i;
    find_player("rastlin")->catch_msg(sprintf("ENTER %O\n", this_object()));
	...
	if (ob[i] != this_object())
	    ...
	    {
	    find_player("rastlin")->catch_msg(sprintf("MERGE %O\n", this_object()));
...
config_split(int new_num, object orig)
{
    int             cp,
                    i;
    find_player("rastlin")->catch_msg("CONFIG\n");
    item_count = new_num;
