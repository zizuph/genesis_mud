...
{
    return "You sacrifaced " + COMPOSITE_DEAD(ingrs) + ".\n";
}
...
resolve_spell()
{
    ...
    gSpell_task += (gElement_needed > elem ? (gElement_needed - elem) * 5 : 0);
    gSpell_task += (gForm_needed > form ? (gForm_needed - form) * 5 : 0);
...
//        write_file("/d/Krynn/wayreth/std/debug", sprintf("%s        6\n", ctime(time())));
    gCaster->remove_prop(LIVE_O_CONCENTRATE);
    ...
    {
        mixed msg = gSpell_fail();
//        write_file("/d/Krynn/wayreth/std/debug", sprintf("%s        7\n", ctime(time())));
        if (stringp(msg))
            gCaster->catch_msg(gSpell_fail());
        return;
    }
 
//        write_file("/d/Krynn/wayreth/std/debug", sprintf("%s        8\n", ctime(time())));
    gResults = (intp(result) ? ({ result }) : result);
 
    while (i--)
        if (present(gTargets[i], environment(attacker)))
        {
//        write_file("/d/Krynn/wayreth/std/debug", sprintf("%s        9\n", ctime(time())));
...
//        write_file("/d/Krynn/wayreth/std/debug", sprintf("%s        10\n", ctime(time())));

    if (size)
    {
//        write_file("/d/Krynn/wayreth/std/debug", sprintf("%s        11\n", ctime(time())));

        gCaster->add_mana(-gMana_left);

        if (sizeof(gFound_ingrs))
	{
//        write_file("/d/Krynn/wayreth/std/debug", sprintf("%s        12\n", ctime(time())));
            gFound_ingrs = filter(gFound_ingrs, prepare_ingrs_heap);
            gCaster->catch_msg(hook_use_ingredients(gFound_ingrs));
            filter(gFound_ingrs, &->remove_object());
	}
//        write_file("/d/Krynn/wayreth/std/debug", sprintf("%s        13\n", ctime(time())));
