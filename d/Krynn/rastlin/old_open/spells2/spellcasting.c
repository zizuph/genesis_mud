...
int  gSpell_time,	       	/* The time it takes to prepare the spell */
     ...
     gMana_fail,                /* The % of mana to reduce when the spell
				   fails */
     gResolve_type, 
...
function gFind_target,		/* The function used to find the target(s) */
         ...
         gSpell_death,          /* Call this function when the target dies */
 ...
 */
public void
set_spell_mana_fail(int proc) { gMana_fail = proc / 100; }
...
resolve_spell()
{
    int i, caster, rand, target, *result = ({}), t;
        ...
        for (i = 0; i < sizeof(gTargets); i++)
	{
	    ...
	    result += ({ ((t = (100 * (rand + 2 * caster - target - gSpell_task) /
			  gSpell_task)) > 0 ? t : 0) });
	}
    return result;
}
...
start_spell_fail(string verb, string arg)
{
    ...
    reduce = gMana_needed * gMana_fail;
