...
search_now(mixed *arr)
{
    ...
    {
        found = all_inventory(this_object());
        found = filter(found, "search_hidden", this_object(), arr[0]);
        if (sizeof(found))
        {
        ...
        else if (!sizeof(dead) && sizeof(live))
            hidden = COMPOSITE_LIVE(live);
        else if (sizeof(dead) && sizeof(live))
            hidden = COMPOSITE_LIVE(live) + " and " + COMPOSITE_DEAD(dead);
	if (strlen(hidden))
	{
            tell_object(arr[0], "You find " + hidden + ".\n");
	    say(QCTNAME(arr[0]) + " finds " + hidden + ".\n");
	}
    }
