...
hit_target()
{
   ...
   /* calc firer's encumberance modifier */
   f_enc = (this_player()->query_encumberance_volume() +
      this_player()->query_encumberance_weight()) / 2;
   
   /* calc target's encumberance modifier */
   t_enc = (target->query_encumberance_volume() +
      target->query_encumberance_weight()) / 2;
   
   /* subtract from difficulty based on firer's equipment & condition */
   difficulty = TASK_HARD - aim_mod - bow_acc -
      this_object()->fire_difficulty_consider() - f_enc;

   /* add to difficulty based on target's & firer's relative positions */
   difficulty = difficulty + target_room_mods + firer_room_mods;
...
hurt_target()
{
   int damage, dam_type, mskill;
   mixed result;
   string hit_msg = "";
   
   /* get the correct damage type */
   if(!(dam_type = (this_object()->query_loaded())->query_ammo_dt()))
      dam_type = W_IMPALE;
   ...
   result = target->hit_me(damage,dam_type,this_player(),-1);
...
possible_targets(object for_ob)
{
   ...
   environment(for_ob)->query_valid_targets(for_ob);
}
