...
#define SANCTION_READ_N    1
#define SANCTION_READ_ALL_N 2
#define SANCTION_WRITE_N    4
#define SANCTION_WRITE_ALL_N 8
...
#define DOMAIN_DIR(x)      ("/d/" + euid + "/domain/sanctions")
...
private static mapping gSanctions = ([]);
...
static mixed
read_sanction_file(string euid)
{
    string error = "", wiz, line = read_file(DOMAIN_DIR(euid));
    string *linea, *sanction, *privs;
    int size, i, rights, tmp, ssize;
    ...
    while(size--)
    {
        sanction = explode(linea[size], " ") - ({ "" });
	ssize = sizeof(sanction);
	...
	if (!ssize)
        ...
        if ((ssize < 2) || (ssize > 3))
	{
            error = sprintf("Line %d: Wrong number of arguments.", 
			    size + 1) + error; 
	    continue;
	}
	...
	wiz = lower_case(sanction[0]);

	if ((wiz != "all") && (SECURITY->query_wiz_rank(wiz) < WIZ_NORMAL))
	{
            error = sprintf("Line %d: " + capitalize(wiz) + " is not " +
			    "a normal wizard.\n", size + 1) + error;
	    continue;
	}
	...
	if ((wiz == "all") && (ssize != 2))
	{
	    error = sprintf("Line %d: Too many arguments to the type " +
			    "'all'.\n", size + 1) + error;
	    continue;
	}

	privs = explode(sanction[1], "");
	i = sizeof(privs);
	
	if (sizeof(sanction) > 2)
	{
	    while(i--)
	        if (member_array(privs[i], PATH_SANCTIONS) != -1)
                    rights |= SANCTION_TOKEN_TO_FILE[privs[i]];
                else
		    error = sprintf("Line %d: Invalid sanction '" + 
				    privs[i] + "' for path sanction.\n",
				    size + 1) + error;
	}
	else
	    while(i--)
	        if (member_array(privs[i], DOMAIN_SANCTIONS) != -1)
		    rights |= SANCTION_TOKEN_TO_FILE[privs[i]];
	        else
		    error = sprintf("Line %d: Invalid sanction '" +
				    privs[i] + "' for domain sanction.\n",
				    size + 1) + error;
	...
	if (wiz == "all")
	{
	    gSanctions[euid][wiz] = rights;
	    continue;
	}
	
	if (gSanctions[euid][wiz])
	    gSanctions[euid][wiz] += ([sanction[2] : rights]);
	else
	    gSanctions[euid] += ([wiz : ([sanction[2] : rights])]);
    }
}
...
static int
valid_path_sanction(string user, string dname, int type, string *parts)
{
    mapping tmp = gSanctions[dname];
    string path = implode(parts, "/");

    if (!tmp)
    {
        read_sanction_file(dname);
	tmp = gSanctions[dname];
    }
    ...
    return (sizeof(filter(filter(m_indexes(tmp), &wildmatch(,path) @ 
				 &operator(+)(,"/*")), 
			  &operator(&)(type) @ &operator([])(tmp))) > 0);
}
...
{
    return valid_path_sanction(reader, dname, 
			       (SANCTION_READ_N|SANCTION_READ_ALL_N), parts);
}
...
{
    return valid_path_sanction(writer, dname, 
			       (SANCTION_WRITE_N|SANCTION_WRITE_ALL_N), parts);
}
 ...
 */
static varargs int
query_sanction(string giver, string receiver, int type, string path)
{
    mapping tmp = gSanctions[giver];

    if (!tmp)
    {
        read_sanction_file(giver);
	tmp = gSanctions[giver][receiver];
    }
    else
        tmp = tmp[receiver];
 ...
 */
static varargs int
create_sanction(string giver, string receiver, int type, string path)
{
    mapping tmp = gSanctions[giver][receiver];

    set_auth(this_object(), "root:root");

    switch(file_size(SANCTION_DIR + giver))
    {
        case -2:
            break; 
        case -1:
            if (!mkdir(SANCTION_DIR + giver))
	        return 0;
            break;
	default:
            return 0;
    ...
    if (!tmp)
        gSanctions[giver] += ([receiver : ([path : type])]);
    else
        if (tmp[path])
            gSanctions[giver][receiver][path] |= type;
        else
    ...
    save_map(gSanctions[giver][receiver], SANCTION_DIR + giver + 
	     "/" + receiver);
}
...
sanction(string str)
{
    ...
    if (sscanf(str, "update %s", str) == 1)
    {
	if (query_wiz_rank(name) < WIZ_ARCH)
	    return notify_fail("Only the administration may do this.\n");
	
	if (query_domain_number(str) == -1)
	    return notify_fail("No such domain.\n");

	str = read_sanction_file(capitalize(str));
	if (strlen(str))
	    write(str + "\n");
	else
	    write("Ok.\n");
