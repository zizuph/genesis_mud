...
create_door()
{
    ...
    set_door_name(({"trail", "bushes"}));
    set_open(0);
    set_open_desc("");
    set_closed_desc("");
    set_pass_command(({"follow", "under", "the", "bushes"}));
    set_fail_pass("You must kneel down in order to crawl under the bushes.\n");
    set_open_command("kneel");
    set_open_mess(({"@@open_mess1", "@@open_mess2"})); /* here, other rm */
    set_fail_open("You are already kneeling down.");
}
