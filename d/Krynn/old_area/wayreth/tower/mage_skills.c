...
//find_player("karath")->catch_msg(sprintf("%O %O %O %O %O\n", RNAME(TP), result, gLocal, snum, 0));

    if (result && gLocal)
    {
	  int t = TP->query_wohs_level() + 1;

	  /* This was changed to make it easier for full mages and 
	   * apprentices to actually train some skills. This way you
	   * don't have to be one level from conclave just to max some
	   * skills
	   */
        level = (ADMIN->query_number_levels() - 7);
        result = ftoi(((itof(result) / itof(level)) * itof((t >= level ? level : t))));
	if(snum == SS_HERBALISM || snum == SS_ALCHEMY)
	  {
	    result = ::sk_query_max(snum, silent);
	  }
    }
