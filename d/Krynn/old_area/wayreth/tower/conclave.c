...
create_tower_room()
{
    ...
    ACI(({"staff","staff of testing"}),"summon", "@@do_getstaff");

    add_exit(WTOWER + "stairwell08", "east", check_test);
...
do_getstaff()
{
    ...
    if(!staff_count)
      {
	write("You whisper a word of summoning, but it seems that the "+
	    "staff is gone!\n");
	say( QCTNAME(TP) + " whispers a word of summoning and looks "+
	    "annoyed when nothing happens.\n");
	return 1;
      }
    setuid; seteuid(getuid());
    staff = clone_object(WOHS_STAFF_OF_TESTING);
    staff->move(TP);
    write("You whisper a word of summoning and a magnificant staff "+
	"materalizes into your hands.\n");
    say( QCTNAME(TP) + " whispers a word of summoning and suddenly "+
	"a magnificant staff materalizes into " +HIS(TP)+ " hands.\n");
    staff_count = 0;
    return 1;
}

int
do_returnstaff(string str)
{
    object staff = present("_wohs_staff_of_testing", TP);
    if(objectp(staff) && staff->id(str))
       {
	write("You whisper a word of power and the " +staff->short()+ " "+
	    "disappears.\n");
	say( QCTNAME(TP) + " whispers a word of power and " +HIS(TP)+
	    " " + staff->short()+ " disappears mysteriously.\n");
	staff_count = 1;
	staff->remove_object();
	return 1;
       }
...
check_test()
{
	if(TP->query_prop("_live_i_wohs_testee") == 1)
	  {
		TP->move("/d/Krynn/wayreth/tower/test/room1",1);
		write("You shake your head in confusion.\n");
		return 1;
	  }
	return 0;
}
