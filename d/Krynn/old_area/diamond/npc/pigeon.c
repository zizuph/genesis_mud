...
#define AIR_ROOM    DIROOM + "air_room"

/* The home of the pigeon. */
#define PIGEON_HOME DIROOM + "mail"

/* The message. */
#define MESSAGE_OBJ DIOBJ + "message"
...
long_func()
{
	...
	"try again with the 'resend' command. " + 
	  /* "If you want the pigeon back, " +
	   * "you can 'whistle for pigeon', and it will return, if it's
	   * still alive. " + 
	   */
	"If you don't want the pigeon anymore, you can do 'return pigeon'.\n";
    if (owner && owner != this_player())
      return "This is "+owner->query_cap_name()+"'s carrier pigeon. It is " +
	"however a friendly bird, and you might be able to send messages to " +
	"others by 'send <player>' or 'send reply'. If the pigeon fails to " +
	"deliver the message you can tell it to try again with the 'resend' " +
	"command. You may also 'return pigeon'. "+
	"That will make the pigeon fly back to "+owner->query_cap_name()+".\n";
    return "This is a white carrier pigeon, which you can use to send " +
...
do_send(string who)
{
    ...
    if (E(tp)->query_prop(ROOM_I_INSIDE))
      {
	  write("You can't send out the pigeon when you're not out in " +
		"open air.\n");
	  return 1;
      }
    if (who == "reply")
...
send_mail()
{
    ...
    fly_alrm = 0;
    tell_room(E(TO), "The white carrier pigeon flaps its wings and " +
	      "flies up to the sky.\n");
...
deliver()
{
    ...
    if (target == find_player(gSender))
      tell_object(target, "Your white carrier pigeon returns, unable to find "
		  + C(gWho) + ". It lands on your shoulder and " +
		  "you retrieve your message.\n");
    else
      tell_object(target, "A white carrier pigeon descends from the " +
		  "sky, lands on your shoulder and gives you a message " +
		  "from " + C(gSender) + ".\n");
    /*return to owner if i'm not needed.*/
    if (owner && target != owner)
      {
	  tell_object(target, "If do not use this pigeon it will return to " +
		      "it's owner again.\n");
...
do_return(string str)
{
    ...
    if (str == "pigeon" || str == "white pigeon" || str == "carrier pigeon")
      {
	  write("You send the carrier pigeon up in the air again, to fly " +
		"back home.\n");
...
fly_to_someone(object dest)
{
    ...
    if (living(E(TO)))
	  tell_room(E(E(TO)), "The white pigeon flaps its wings and flies " +
		    "up in the sky!\n");
    else 
      tell_room(E(TO),
		"The white pigeon flaps its wings and flies up in the sky!\n");
    if(!dest)
      {
	  remove_object();
	  return 1;
    ...
    tell_object(dest,"A white pigeon comes flying, and lands on your " +
		"shoulder.\n");
    tell_room(E(dest), "A white pigeon comes flying, and lands on the " +
...
start_place()
{
    ...
    tell_room(E(TO),
	      "A white carrier pigeon comes flying, landing on the floor.\n");
...
test_pigeon_killer(object pl)
{
    if (pl->query_prop("_player_is_pigeon_killer")) 
      {
	  tell_object(pl,"The pigeon recognizes you as a killer of pigeons,\n"+
		      "and hurriedly flies away again!\n");
	  tell_room(E(pl), "The pigeon is scared by " + QTNAME(pl) + 
