...
create_door() {
  ...
  set_door_name(({"green door","not-sure","not-sure door"}));   /* name(s) for the door, first
                                                   string is std desc */

  /* commands */

  set_pass_command("not-sure");                 /* command(s) to pass the door */
  set_open_mess(({"@@open_mess_here",           /* msg when opening in this room */
                "@@open_mess_there"}));         /* msg in the other room */

  set_other_room(DIROOM + "lavatory_neutral"); /* room on the other side */
  ...
  set_door_desc("The door was recently painted with a strong green. On "
...
open_mess_here()
{
    ...
    if((DIROOM + "lavatory_neutral")->query_occupied())
    {
