...
create_dim_room()
{
    ...
    set_short("The Crystal Clairvoyance Pub");
    set_long("You find yourself in the Crystal Clairvoyance Pub, " +
	    "a pleasant place where you can buy refreshing concoctions " +
	    "and very mildly intoxicating herbal mixtures, but nothing " +
	    "so strong as to befuddle the mind. From somewhere in the " +
	    "room you hear the delicate music of a lyre, though you " +
	    "can't detect its source. A counter adorns the back of the " +
	    "room, with a small plaque floating behind it, and a menu " +
	    "resting upon it.\n");

     AI(({"lyre","music"}),
            "You can't seem to locate any visible source of the music " +
	    "and assume that it must be magical in nature.\n");
     AI(({"plaque","floating plaque"}),
	    "Though obviously magical, you should still be able to read it.\n");
     AI(({"menu","bar menu","list"}),
	    "Orderly and legible, this shows the wares of the " +
	    "Crystal Clairvoyance Pub. You are able to read the menu.\n");
   ...
   add_exit(DIROOM + "nw1", "southeast", 0);
}
...
do_read(string str)
{
        if ((str=="plaque")||(str=="magical plaque"))
         {
          write("Etched into the plaque are these words: Musical " +
		"entertainment provided by the Wizards of High " +
		"Sorcery in thanks for 'An establishment condusive " +
		"to clear thought and relaxation.'\n");	
	  say(QCTNAME(TP)+ " reads the plaque.\n");
           return 1;
	 } else
        if ((str=="menu")||(str=="list"))
         {
	  write("This is what we currently have to offer at the Crystal " +
		"Clairvoyance Pub:\n\n" +
