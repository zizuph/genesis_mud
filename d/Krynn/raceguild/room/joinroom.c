...
affiliate(string str, int silent = 0)
{
    ...
    // Minotaurs can't join in this room - Elmore.
    if (region == "draconian" && TP->query_name() != "Elmorejr")
    {
        NF("Sorry, the race-type you selected is not available at the moment.\n")
        return 0;
    }
...
do_leave(object who, int force)
{
    ...
    present(RACE_HIDER_OBJ, who)->remove_object();
    write_file(LEAVEPATH, extract(ctime(time()), 4, 15) + " " +
