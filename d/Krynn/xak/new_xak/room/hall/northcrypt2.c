...
{    
    ...
    add_exit(HALL + "northcrypt3", "east");
    add_exit(HALL + "northcrypt1", "west");
...
exa_coffin()
{
    ...
    if (gCount == 2)
    {
        tell_room(TO, QCTNAME(TP) + " is examining the large coffin when " +
		  "suddenly two ghostly figures emerges from the " +
		  "shadows!\n", TP);

        clone_object(NPC + "shadow")->move(TO);
        clone_object(NPC + "shadow")->move(TO);

	gCount--;

        return "You examine the large coffin when suddenly two ghostly " +
               "forms emerges from the shadows!\n";

    } 
    else 
    {
        tell_room(TO, QCTNAME(TP) + " is examining the large coffin when " +
		  "suddenly a ghostly figure emerges from the " +
		  "shadows!\n", TP);
