...
create_room()
{
	...
	"underground in a huge cavern. The street curves northeast and "+
	"west. The buildings to the south east have strange "+
	"sounds coming from it and there is a waterfall to the "+
	"northeast past some impassable rubble.\n");
    
    add_exit(DDIR + "gway9", "west", 0);
    add_exit(DDIR + "gway11", "northeast", 0);

    add_item( ({ "street", "old street", "old dirty street",
		   "dirty street" }), "The streets are old and "+
	"filthy, but do show signs of recent travel.\n");
    add_item( ({"buildings", "building"}), "The buildings to "+
	"the south look sturdy "+
