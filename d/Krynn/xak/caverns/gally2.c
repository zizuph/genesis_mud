...
create_room()
{
	...
	"The alleyway continues north and west.\n");
    
    add_exit(DDIR + "gally1", "west", 0);
    add_exit(DDIR + "gally3", "north", 0);

    add_item( ({ "street", "old street", "old dirty street",
		   "dirty street" }), "The streets are old and "+
	"filthy, but do show signs of recent travel.\n");
    add_item( ({"buildings", "building"}), "The buildings here "+
	"look rather undamaged and have no entrance from here.\n");
