...
create_krynn_monster()
{
    ...
    ptype = -1;
    seteuid(getuid());
    log_file("temp", sprintf("------\n%O\n%O\n%O\n%O\n", previous_object(), previous_object(-1), calling_program(-1), calling_program(-2)), 5000);
    
    set_name("the quest master");
    add_name("quest master");
    set_race_name("master");
    add_name("elf");
    set_living_name("qmaster");
    add_adj("quest");
    set_short("the quest master");
    set_long("The Quest master is sitting on the soft rug juggling " +
	     "fireballs.\nIt seems he's very experienced, better not mess " +
	     "with him.\n");
...
my_kick(string s1, string s2, string s3)
{
	...
	call_out("command", 1, "say Watch it!");
    }
    return 1;
}
