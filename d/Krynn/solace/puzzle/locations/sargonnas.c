/* The Sargonnas room. ~solace/puzzle/locations/sargonnas */
...
void reset_puzzle_room()
{
    ...
    add_exit(LOCATIONS + "gilean", "south", 0, 1);
    set_password();
}

void create_puzzle_room()
{
    set_short("The Sargonnas room");
    set_long("@@long_description");

    add_item("sign", "The sign is rectangular in shape and has sharp angular "+
        "words etched into its obsidian surface.\n");
...
int revolve_room()
{
    ...
    tutt = find_object(LOCATIONS + "gilean");
    shai = find_object(LOCATIONS + "paladine");

    if(!objectp(shai))
    {
        (LOCATIONS + "paladine")->teleledningsanka();
        shai = find_object(LOCATIONS + "paladine");
    }

    if(!objectp(tutt))
    {
        (LOCATIONS + "gilean")->teleledningsanka();
        tutt = find_object(LOCATIONS + "gilean");
    }
    ...
    if(!facing)
    {
        ...
        remove_exit("south");
        add_exit(LOCATIONS + "paladine", "north", 0, 1);

        tutt->remove_exit("north");
        shai->add_exit(LOCATIONS + "sargonnas", "south", 0, 1);
    }
    else
    {
        ...
        tutt->add_exit(LOCATIONS + "sargonnas", "north", 0, 1);
    }
