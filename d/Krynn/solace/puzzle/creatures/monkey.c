...
#define ACTS ({ "emote clambers onto the control panel and jumps up into "+ \
...
void setup_acts()
{
    int i;
    object obj;
    string tmp, dir, *arr = E(TO)->query_exit_cmds();

    alt_acts  = ({ });
    alt_acts += ({ arr[0] });

    obj = find_object(LOCATIONS + "hiddukel");
    dir = obj->query_room_dir_from_name("takhisis");
...
int feed_monkey(string str)
{
    ...
    if(parse_command(str, E(TP), "%o 'with' %s", obj, food))
    {
        ...
        if(objectp(banana))
        {
            ...
            set_alarm(2.0, 0.0, "get_pendulum", 0);
...
void get_pendulum(int timer)
{
    ...
    {
        command("emote climbs back through the hole and disappears!");
