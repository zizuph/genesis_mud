...
create_creature()
{
	...
	for (i = 0; i < 6; i++)
		set_base_stat(i, 8 + random(8));
	set_attack_unarmed(1,  2,  2, W_IMPALE, 60, "bite");
...
do_die(object killer)
{
    ...
    ob->leftover_init("bone","badger");
    ob->move(E(TO));
    ob = clone_object("/std/leftover");
    ob->leftover_init("skull","badger");
    ob->move(E(TO));
    ob = clone_object("/std/leftover");
    ob->leftover_init("bone","badger");
    ob->move(E(TO));
    ob = clone_object("/std/leftover");
    ob->leftover_init("rib","badger");
    ob->move(E(TO));
    ob = clone_object("/std/leftover");
    ob->leftover_init("rib","badger");
    ob->move(E(TO));
