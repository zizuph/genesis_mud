...
reset_solace_room()
{
    if (npc && sizeof(npc) > 0 && !objectp(npc[0]))
      set_alarm(1.0,0.0,"load_drac",0);
    if (npc && sizeof(npc) > 1 && !objectp(npc[1]))
      set_alarm(2.0,0.0,"load_drac",1);
    if (npc && sizeof(npc) > 2 && !objectp(npc[2]))
      set_alarm(3.0,0.0,"load_drac",2);
    if (npc && sizeof(npc) > 3 && !objectp(npc[3]))
      set_alarm(9.0,0.0,"load_drac",3);
    if (npc && sizeof(npc) > 4 && !objectp(npc[4]))
