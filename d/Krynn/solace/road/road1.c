...
/* Updated Louie 2005 to limit number of dracs*/
...
reset_solace_room()
{
    ...
    if (!drac2)
    {
	drac2 = RANDOM_MEET(DRAC_TABLE);
	drac2->set_color("red");
	drac2->set_random_move(10);
	drac2->move(TO);
    }

    if (!drac3)
    {
	drac3 = RANDOM_MEET(DRAC_TABLE);
	drac3->set_color("red");
	drac3->set_random_move(10);
	drac3->move(TO);
    }
...
create_solace_room()
{
    ...
    add_item("path", "An overgrown path seems to lead to your southeast.\n");
    add_item(({"water"}),
