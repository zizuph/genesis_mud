...
void clone_pin(object player);
int query_restore_pin(object player);
...
create_solace_room()
{
    ...
    KRLINK->register_restorable("wise_pin", 
        "The Wise People Membership Pin", 
        query_restore_pin, clone_pin);
}
...
int
query_restore_pin(object player)
{
    if ((REALAGE(player) < 8640000) || P(WISE_ID, player))
    {
        return 0;
    }
    
    if (member_array(C(player->query_real_name()), members) >= 0)
    {
        return 1;
    }
...
clone_pin(object player)
{
    ...
    player->catch_tell("You get your " + obj->short() + ".\n");
    return 1;
}
