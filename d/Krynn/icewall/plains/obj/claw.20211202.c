...
// Percent modifiers to Hit and Pen of unarmed attacks
#define HIT_MODIFIER 10
#define PEN_MODIFIER 20
...
wear(object arm)
{
    ...
    pen = W_HAND_PEN + F_UNARMED_PEN(uskill, player->query_stat(SS_STR));
    hit = W_HAND_HIT + F_UNARMED_HIT(uskill, player->query_stat(SS_DEX));

    pen += pen * PEN_MODIFIER / 100;
    hit += hit * HIT_MODIFIER / 100;
...
remove(object arm)
{
    ...
    co->cb_add_attack(hit,pen, W_BLUDGEON, 25, (hand == 1 ? W_RIGHT : W_LEFT),
      uskill + 6 );
    co->cb_modify_procuse();
