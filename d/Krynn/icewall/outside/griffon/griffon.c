...
land(object ob)
{
    ...
    tell_room(E(TO), "The mighty griffon suddenly takes off into the sky. The " +
      "force of the wind created by his wings nearly knocks you off the cliff, but you " +
    ...
    tell_room(E(TO), "A large fearless looking griffon descends from " +
      ...
      "knocks you off the cliff, but when " +
...
mount(string str)
{
    ...
    if ( TP->query_prop( "_in_icewall" ) != 1 ) {
        TP->add_prop( "_in_icewall", 1 );
        find_living("stevenson")->catch_msg(TP->query_name()+" is now entering Icewall!\n");
    }
    else {
        TP->remove_prop( "_in_icewall" );
        find_living("stevenson")->catch_msg(TP->query_name() +
          " is now leaving Icewall.\n" );
    }
...
go_void()
{
    tell_room(E(TO), "The mighty griffon suddenly takes off into the sky. The force of the wind created by his wings nearly knocks you off the cliff, but you manage to steady yourself.\n");
    (ON_DRAGON)->drop_all();
