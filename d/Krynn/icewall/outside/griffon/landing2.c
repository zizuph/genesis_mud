...
{
   object torch1, torch2;
   string room;
   
   if ( !torch1 && !torch2 ) {
      torch1 = clone_object("/std/torch");
      torch2 = clone_object("/std/torch");
      torch1->set_long("This torch is a pretty good light source.\n");
      torch2->set_long( torch1->query_long() );
      room = GET_RANDOM_ROOM(ICE_PLAINS);
      torch1->move(room);
      room = GET_RANDOM_ROOM(ICE_PLAINS);
      torch2->move(room);
   }
...
void create_icewall_room()
{
   ...
   set_long("This is a flat spot in the Icewall Mountains. On the ground you see " +
      "claw marks and strange feathers. To the north you can see an icy " +
      "path going down the mountains towards a house in the distance.\n"
	+ "");
   
   add_exit(ICE_OUT + "grif_2.c", "down", "@@slip");
   
   add_item("mountains","They encompass Icewall.\n");
   add_item("icewall","Stevenson and Macker worked hard on it so you "
      + " better like it!\n");
