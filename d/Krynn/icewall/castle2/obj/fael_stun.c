...
no_command()
{
    write("You are held by a magic bond, and unable to do that.\n");
    return 1;
}
...
remove_dragon_fear(object victim)
{
    ...
    vict->catch_msg("You are no longer held, and can move again.\n");

    tell_room(E(vict), QCTNAME(vict) + " appears able to move " +         "again.\n", vict, TO);
...
do_dragon_fear()
{
    ...
    victim->catch_msg("You feel tightly held!\n");
    victim->add_stun();

    if(victim->query_wiz_level())
	victim->catch_msg("As a wizard, you are immune to the " +
             "hold person spell.\n");


    tell_room(E(victim), QCTNAME(victim) + " looks tightly held!\n", victim, TO);

    if (victim->query_wiz_level())
	tell_room(E(victim), C(PRONOUN(victim)) + " is a wizard and " +           "thus immune to the hold person spell.\n", victim, TO);
