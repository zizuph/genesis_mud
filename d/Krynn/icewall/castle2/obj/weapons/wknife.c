 ...
 *
 * Updated random pen factor so that it couldn't exceed the maximum for a 
 * knife (40).  Arman 21 October 2017.
 *
 * 2021-09-27 - Cotillion
 * - Fixed an issue with thousands of add_item calls.
 */
...
create_weapon()
{
    ...
    set_long("@@my_long");
    set_default_weapon(30 + random(8), 35 + random(6), W_KNIFE, W_IMPALE, W_LEFT);
    set_wf(TO);
    ...
    add_prop(OBJ_S_WIZINFO,
         ...
         "balance allows a very high penetration.\n");
...
set_color(string new_color)
{
    if (color) {
        remove_item("gem");
        remove_item(color + " gem");
        remove_item("gemstone");
        remove_item(color + " gemstone");
    }
...
wield(object what)
{
   ...
   */
  if (wielder->resolve_task(1800,({ SKILL_WEIGHT, 900, TS_WIS })) > 0)
    {
...
unwield(object what)
{
  if(wielder)
  {
    wielder->catch_tell("You easily let the dagger slip out of your hands.\n");    
  }
  return 0;
}
