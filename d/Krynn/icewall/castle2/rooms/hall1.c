...
create_icewall_room()
{
    ...
    set_short("East end of the grand hall");
    set_long("@@long_descr");

    add_exit("battlement5","up","@@up");
    add_exit("hall2","west");
   ...
   add_item(({"wall","walls"}),"Which wall do you wish to examine " +
    "closer? South, or north?\n");
   add_item("south wall","A wall as many others. You see nothing " +
    "special about it... Except the doorway, of course.\n");
   add_item("east wall","A wall as many others. You see nothing " +
    "special about it... Except the doorway, of course.\n");
   ...
   add_tell("You hear some footsteps above you, on the second floor " +
    "of the lower building.\n");
     ...
     door = clone_object(OBJK + "ornatedoor2");
     door->move(this_object());
     gdoor = clone_object(OBJK + "oakendoor");
...
{ return ("" +
   ...
   "the smooth columns managed to survive the test of time. The hall " +
   "continues to the west, but can also be exited through a large " +
   "ironwood door to the south, and a smaller oaken door to the east.\n");
...
knock(string str)
{
    if(str == "on south wall")
      {
        write("That is pointless. There's a doorway there.\n");        
        say("For some reason, " + QTNAME(TP) + " appears baffled.\n");
        return 1;
      }
    ...
    if(str == "on north wall")
      {
        ...
        tell_room(ROOMSK + "bare_tunnel", "A faint sound " +
