...
create_icewall_room()
{
    pole = clone_object(OBJK + "pole_broken.c");
    pole->move(TO);       
    ...
    add_exit("east_path2","southwest");
    add_exit("east_path4","north");
    add_exit("ice_clearing1","southeast",0,1,1);
    ...
    add_item(({"forest","ice forest"}), "To the southeast, a frozen " +
     ...
     "dwells in it. An opening in the forest would allow you to " +
     "enter it by going southeast from here.\n");
