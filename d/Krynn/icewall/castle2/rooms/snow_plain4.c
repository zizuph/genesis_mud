...
create_icewall_room()
{
    ...
    add_exit("conquer20","west","@@west",1,1);
    add_exit("conquer25","southwest","@@southwest",1,1);
    add_exit("conquer26","south","@@south",1,1);
    add_exit("conquer27","southeast","@@southeast",1,1);
    add_exit("conquer21","east","@@east",1,1);
    add_exit("snow_plain3","northwest","@@northwest",1,1);
    add_exit("conquer16","north","@@north",1,1);
    add_exit("snow_plain5","northeast","@@northeast",1,1);
    ...
    add_item(({"sled marks","marks"}),"The sharp, frozen " +
      ...
      "in the frozen land. The sled marks visible here lead northeast " +
      "and northwest, which worries you a bit. Does the sudden turn " +
      "mean that the sled owners lost their way? Are you to stumble " +
      "shortly upon their frozen remains?.\n");
...
northwest()
{ 
  write("You smartly follows the sledmarks to the northwest.\n");
  say(QCTNAME(TP) + " follows the sledmarks to the northwest.\n");
  return 0;
}
