...
create_icewall_room()
{
    ...
    add_exit("conquer28","west","@@west",1,1);
    add_exit("conquer33","southwest","@@southwest",1,1);
    add_exit("conquer34","south","@@south",1,1);
    add_exit("conquer35","southeast","@@southeast",1,1);
    add_exit("conquer22","northwest","@@northwest",1,1);
    add_exit("conquer23","north","@@north",1,1);

   set_area_name(WAR_AREA_ICEWALL);

   add_item(({"block","blocks","block of ice","blocks of ice"}),"" +
     "Amassed by the ocean, they line the coast, forming a treacherous " +
...
{ return ("" +
   ...
   "To the east, huge blocks of ice rise from coast, blocking " +
   "your way to the ocean.\n");
}
