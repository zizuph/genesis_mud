...
reset_icewall_room()
{
    if (!minotaur1)
    {
    minotaur1 = clone_object(NPCK + "mino_blank_member");
    minotaur1->move_living("xx", TO);
    }
    if (!minotaur2)
    {
    minotaur2 = clone_object(NPCK + "mino_blank_member");
    minotaur2->move_living("xx", TO);
    }
    if (!minotaur3)
    {
    minotaur3 = clone_object(NPCK + "mino_blank_member");
    ...
    if (!leader)
    {
    leader = clone_object(NPCK + "mino_blank_leader");
...
create_icewall_room()
{
    set_short("South end of the west battlement");
    set_long("@@long_descr");

    add_exit("battlement2","north");
    add_exit("south_stable","down","@@down");
...
{ return ("" +
     "This is the south end of the western battlement. Here, the " +
     ...
     "small missile fire, seems oppressive and gloomy. A staircase " +
    ...
    if(str == "through outward slit" || str == "out through outward slit")
      {
         write("Looking far out, you can make a range of snow " +
           ...
           "straight down. An arctic canyon begins to form there, " +
           "making any attack from this side suicidal, if not " +
           "impossible.\n");
