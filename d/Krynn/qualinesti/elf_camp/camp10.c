 ...
 * first time.
 *
 * Reduced the chance of spawn rate of the super sword from 25% to 2%, which
 * is more aligned to clone_unique chance. It seems there are way too many
 * azure swords in the game at the moment.
 * Arman, December 2020.
 *
 * 2% ended up being too extreme. Ended up changing the spawn rate to 10%
 * (seeing it is 10% for a _chance_ of getting it through clone_unique)
 */
...
{   
    ...
    if ( leader = present("qualthas", TO) )
    {
      ...
      // Reset the sequence if it was stopped for some reason
        ...
        break;
        case "seq2":
            leader->command("say I would like to believe this is true, but " +
        ...
        // We give the sword to the player always the first time.
            ...
            {
         // Otherwise we give the crap sword most times.
         // Changed from a 1 in 4 chance to 1 in 10 chance
         // Arman, Dec 2020
                if(!random(10))
                {
                    sword = clone_unique(WEP + "elven_blade", 5, WEP+"crap_sword");
                }
                else
                {
                ...
                if(sword->id("_sarist_sword_reward_"))
                {
                  if(leader)
                  {
                      who->catch_msg("The tall long-haired male elf says to you: " +
                                     "Ohh, I see you have already gotten a reward, so I am " +
                                     "sure you won't mind Qualthas receiving this gift to " +
                                     "help defend the camp.\n");

                      tell_room(TO, "The tall long-haired male elf says to "+QTNAME(who) + 
                                    ": Ohh, I see you have already gotten a reward, so I am " +
                                    "sure you won't mind Qualthas receiving this gift to " +
                                    "help defend the camp.\n", who);

                      who->catch_msg("The tall long-haired male elf gives an " +
                                     "azure rune-etched longsword to "+QTNAME(leader)+".\n");
                      tell_room(TO, "The tall long-haired male elf gives an " +
                                    "azure rune-etched longsword to " + QTNAME(leader) + 
                                    ".\n", who);
                      sword->move(leader, 1);
                      leader->command("unwield all");
                      leader->command("wield sword");
                      leader->command("wield all");   
                  }
                  else
                  {
                      who->catch_msg("The tall long-haired male elf says to you: " +
                                     "Ohh, I see you have already gotten a reward, I will " +
                                     "just keep this one then.\n");
