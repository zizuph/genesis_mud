...
#define CAMP_ROOM CDIR + "camp11.c"
...
create_room()
{
    ...
    set_long("You are standing on a solid oaken branch high above the " +
	     ...
	     "You have also a perfect view on the camp below.\n");
	     
    add_item(({"ground", "camp", "forest", "encampment"}), &ground_desc());
...
ground_desc()
{
	...
	return "The Qualinesti forest stretches out in all directions. " +
	    "The small encampment is to the south.\n" +
    ...
    {
	return "The Qualinesti forest stretches out in all directions " +
	    "far below you. The small encampment is to the south.\n";
    }
}
...
reset_room()
{
    if (!objectp(archer))
    {
	archer = clone_object(NPC + "guard1");
	archer->move(TO, 1);
        archer->command("hide");
    }
    ::reset_room();
