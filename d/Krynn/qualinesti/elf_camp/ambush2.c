...
create_forest()
{
    add_my_desc("You see a clearing to your west. There is also " +
    ...
    add_item(({ "opening", "clearing" }),
        "There is a small clearing to the west. You feel a scent of " +
        "burning wood coming from that direction.\n");
    add_item(({ "trail", "trails" }),
        "You see a barely noticable trail leading to the east.\n");

    OUTSIDE;

    add_exit(CDIR + "camp12.c", "west", "@@exit_check", 0);
    add_exit(FDIR + "path9.c", "east", 0, 2);
...
climb_tree(string args)
{
    ...
    if ( args == "up" || args == "oak" || args == "tree" || args == "big oak" )
    {
        if ( guard = present("_guard_", TO) /* && CAN_SEE(guard, TP) */ &&
            !TP->query_wiz_level() )
        {
            notify_fail("The elf is standing just next to the tree. You " +
                "can not climb it.\n");
            return 0;
        }
...
exit_check()
{
        ...
        (TP->query_race_name() == "kender") )
    {
        TP->catch_msg("You stop and look around slowly. Intrigued, you pick " +
            ...
            "skillfully sneak west, wondering if anyone caught that!\n");
        ...
        if ( friend_or_foe(who) == 1 )
        {
            ...
            case 0:
                guard->command("say Stay there, " + who->query_race_name() +
                    "!");
                guard->command("say Ahhh, it is you again. You may pass.");
            break;
            case 1:
...
enter_inv(object dest, object old)
{
    ...
    if ( interactive(dest) && !dest->query_invis() )
        set_alarm(1.0, 0.0, &stop_intruder(dest));
