 ...
 * - Fixed ::did_hit
 * Carnak, 2018-02-11
 * - Fixed a "graphical" issue in did hit, the altering of hdescs.
 * - Added a bleed bonus to the dagger feeding
 */
...
look_into_eyes(string str)
{
    ...
    /*   if ( !str )
         {
         notify_fail("Gaze into what?\n");
         return 0;
         }
    
         if ( str != "into the black onyx eyes" )
         {
...
master_one(object ob)
{
    ...
    {  
        ob->catch_msg("\nAs you stare deeply into eyes of the electrum eagle " +
            ...
            "of flight. You feel caress of wind upon your skin and primal " +
            "predator like urges of hunt, you feel your heart is starting to " +
...
feed_me(object enemy, object wielder)
{
    ...
    /*   
         enemy->catch_msg(QCTNAME(wielder) + " blinks as " + POSSESSIVE(wielder) + 
         " blade tears into you.\n");
         say(QCTNAME(wielder) + " blinks as " + POSSESSIVE(wielder) + 
         " blade tears into " + QTNAME(enemy) + ".\n", enemy);
    */      
    ...
    {    
        ...
        tell_room(E(wielder), QCTNAME(wielder) + " seems to be exhausted " +
            ...
            wielder);
        wielder->command("$unwield " + OB_NAME(this_object()));
    }    
    else 
        ...
        int phit, int dam, int hid)
{
    ...
    if ( enemy->query_humanoid() && !enemy->query_prop(LIVE_I_NO_BODY) )
    {
        switch(hdesc)
        {
            case "legs":
            tmp = one_of_list(({
                "left leg", "left shin", "left knee", "left thigh",
                "right leg", "right shin", "right knee", "right thigh",
            }));
                break;
            case "body":
            tmp = one_of_list(({
                "chest", "left hip", "right hip", "abdomen",
            }));
                break;
            case "left arm":
            tmp = one_of_list(({
                "left arm", "left wrist", "left forearm", "left shoulder",
            }));
                break;
            case "right arm":
            tmp = one_of_list(({
                "right arm", "right wrist", "right forearm", "right shoulder",
            }));
                break;
            case "head":
            tmp = one_of_list(({
                "head", "neck", "forehead", "cheek", "left cheek", "right cheek"
            }));
                break;
            default:
            tmp = hdesc;
    ...
    if ( phurt == -1 || phurt == -2 )
    {
        ...
        {
            wielder->catch_msg("You thrust at " + enemy->query_the_name(wielder)
            + "'s " + tmp + " with your " + short() + ", but your attack has "
            + "no result.\n");
        }
        
        if (interactive(enemy) && !enemy->query_option(OPT_GAG_MISSES))
        {
            enemy->catch_msg(wielder->query_The_name(enemy) + " thrusts at "
            + "your " + tmp + " with " + POSSESSIVE(wielder) + " " + short()
            + ", but causes no damage.\n");
        }
        
        wielder->tell_watcher_miss(QCTNAME(wielder) + " thrusts at "
        + QTNAME(enemy) + "'s " + tmp + " with " + POSSESSIVE(wielder) + " "
    ...
    case 0:
        ...
        + enemy->query_the_name(wielder) + "'s " + tmp + ".\n");
        
        enemy->catch_msg(wielder->query_the_name(enemy) + " barely tickles "
        + "your " + tmp + " with the tip of " + POSSESSIVE(wielder) + " "
        + short() + ".\n");
    ...
    case 1..4:
        wielder->catch_msg("You graze the " + tmp + " of "
        + enemy->query_the_name(wielder) + " with the tip of your " + short()
        + ".\n");
        
        enemy->catch_msg(wielder->query_The_name(enemy) + " grazes your " + tmp
        + " with the tip of " + POSSESSIVE(wielder) + " " + short() + ".\n");

        wielder->tell_watcher(QCTNAME(wielder) + " grazes the " + tmp + " of "
        + QTNAME(enemy) + " with the tip of " + POSSESSIVE(wielder) + " "
        + short() + ".\n", enemy);
        break;
    
    case 5..9:
        wound_adj = one_of_list(({
            "shallow", "light", "minor", "mediocre"
        }));
        
        wielder->catch_msg("You open a " + wound_adj + " wound in the " + tmp
        + " of " + enemy->query_the_name(wielder) + " with the wide blade of "
        + "your eagle-shaped dagger.\n");
        
        enemy->catch_msg(wielder->query_The_name(enemy) + " opens a "
        + wound_adj  + " wound in your " + tmp + " with the wide "
        + "blade of " + POSSESSIVE(wielder) + " eagle-shaped dagger.\n");
        
        wielder->tell_watcher(QCTNAME(wielder) + " opens a " + wound_adj
        + " wound in the " + tmp + " of " + QTNAME(enemy) + " with the wide "
        + "blade of " + POSSESSIVE(wielder) + " eagle-shaped dagger.\n", enemy);          
        break;
    
    case 10..16:
        wielder->catch_msg("You skillfully pass the enemy's defenses and cut "
        + POSSESSIVE(enemy) + " " + tmp + " with your " + short()
        + " severely.\n");
        
        enemy->catch_msg(wielder->query_The_name(enemy) + " skillfully passes "
        + "your defenses and cuts your " + tmp + " with " + POSSESSIVE(enemy)
        + " " + short() + " severely.\n");
        
        wielder->tell_watcher(QCTNAME(wielder) + " skillfully passes the "
        + "enemy's defenses and cuts " + POSSESSIVE(enemy) + " " + tmp
        + " with the" + short() + " severely.\n", enemy);
        break;
    
    case 17..25:
        wielder->catch_msg("You pierce the " + tmp + " of "
        + enemy->query_the_name(wielder) + " with your " + short()
        + " wickedly.\n");
        
        enemy->catch_msg(wielder->query_The_name(enemy) + " pierces your "
        + tmp + " with " + POSSESSIVE(wielder) + " " + short()
        + " wickedly.\n");
    ...
    case 26..39:
        wound_adj = one_of_list(({
            "bleeding", "serious", "deep", "badly looking"
        }));
        
        wielder->catch_msg("You slash at " + enemy->query_the_name(wielder)
        + " with your " + short() + " opening a " + wound_adj + " wound in "
        + POSSESSIVE(enemy) + " " + tmp + ".\n");
        
        enemy->catch_msg(wielder->query_The_name(enemy) + " slashes at you "
        + "with " + POSSESSIVE(wielder) + " " + short() + " opening a "
        + wound_adj + " wound in your " + tmp + ".\n");
    ...
    case 40..54:
        wielder->catch_msg("You cunningly feint and stab "
        + enemy->query_the_name(wielder) + " in the " + tmp + " with your "
        + short() + " savagely.\n");
        
        enemy->catch_msg(wielder->query_The_name(enemy) + " cunningly feints "
        + "and stabs you with " + POSSESSIVE(wielder) + " " + short() + " in "
        + "the " + tmp + " savagely.\n");
        
        wielder->tell_watcher(QCTNAME(wielder) + " cunningly feints and " 
        + "stabs " + QTNAME(enemy) + " with " + POSSESSIVE(wielder) + " "
        + short() + " in the " + tmp + " savagely.\n", enemy);
        break;
    
    case 55..86:
        wielder->catch_msg("You bury your " + short() + " into the " + tmp
        + " of " + enemy->query_the_name(wielder) + " to the hilt.\n");
        
        enemy->catch_msg(wielder->query_The_name(enemy) + " buries "
        + POSSESSIVE(wielder) + " " + short() + " into your " + tmp
        + " to the hilt\n");
        
        wielder->tell_watcher(QCTNAME(wielder) + " buries "
        + POSSESSIVE(wielder) + " " + short() + " into the " + tmp + " of "
        + QTNAME(enemy) + " to the hilt.\n", enemy);
        break;
    default:
        wielder->catch_msg("You massacre the " + tmp + " of " 
        + enemy->query_the_name(wielder) + " with a horrifying swing of your "
        + short() + ".\n");
        
        enemy->catch_msg(wielder->query_The_name(enemy) + " massacres your "
        + tmp + " with a horrifying swing of " + POSSESSIVE(wielder) + " "
        + short() + ".\n");
        
        wielder->tell_watcher(QCTNAME(wielder) + " massacres the " + tmp
        + " of " + QTNAME(enemy) + " with a horrifying swing of "
    ...
    if ( dam > 60 && !random(3) )
    {
        if (gMastered && gBites)
        {
            wound_obj = clone_object(WOUNDING_OBJ);
            wound_obj->set_wounding_damage((dam * gBites) / 25);
        ...
        wielder->catch_msg("The pommel of the dagger struggles slightly in "
        + "your hand.\n");
