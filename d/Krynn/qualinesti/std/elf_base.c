/* elf_base.c
 *
 * Qualinesti elf base file for Krynn.
 *
 * Blizzard, 01/2003
 *
 * Fixed typos.
 ...
 * Mortis 06.2006
 *
 * Navarre June 25th 2006, as discussed with Louie
 * the elves' toughness have been downgraded some.
...
#define OAK1 CDIR + "oak1"

void    setup_elf();

string *warrior_descr = ({ " polishing " + POSSESSIVE(TO) + " weapon",
    " sitting at the campfire", " standing silently", " watching the area",
...
string *archer_descr = ({ " checking " + POSSESSIVE(TO) + " arrows",
    " sitting at the campfire", " pacing nervously around, watching the forest",
...
setup_elf()
{
    ...
    set_base_stat(0, 135 + random(35));
    set_base_stat(1, 165 + random(35));
    set_base_stat(2, 150 + random(35));
    set_base_stat(3, 140 + random(25));
    set_base_stat(4, 140 + random(25));
    set_base_stat(5, 140 + random(25));
...
create_krynn_monster()
{
    ...
    add_act("say Where are all those dracs coming from?!");
    add_act("say The rumour is that Daergar are trading with the dragonarmy!" +
        " Curse those dwarves!");
    ...
    add_act("say Pax Tharkas used to be a monument of peace... now it's " +
        "a nest of evil!");
    add_act("emote checks his weapon.");
...
aggressive_attack(object ob)
{
  ...
  // remove the permission for entering the camp.
    ...
    {
    case 0:
        command("say You will never leave this camp alive!");
        break;
    case 1:
...
notify_death(object killer)
{
    ...
    if ( killer->query_prop(ELF_CAMP_QUEST) )
    {
        if ( !(killer->query_prop(CAMP_VISITED)) )
        {
            ...
            killer->catch_msg("As your opponent falls dead to the ground, " +
                "you realize this is the place you were looking for.\n");
        }
    }
...
special_attack(object enemy)
{
    ...
    if ( !enemy )
      ...
      // Execute the special attack 1 in 6 rounds on average
    if (random(6))
        return 0;
