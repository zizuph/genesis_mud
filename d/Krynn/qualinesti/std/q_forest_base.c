...
create_qualinesti_room()
{
    ...
    add_item(({"forest", "woods"}),
        "The forest seems to be healthy, thriving of different types of " +
        "trees. You have a " +
    ...
    set_up_herbs( ({ ONE_OF(herbs), ONE_OF(herbs), ONE_OF(herbs),
        ONE_OF(herbs), ONE_OF(herbs) }), ({ "growth", "herb growth", }), 3);
...
breathe(string arg)
{
    if ( !stringp(arg) || arg == "air" || arg == "the air" || arg == "in" )
    {
        ...
        {
            if ( TP->query_intoxicated() < 15 )
                TP->set_intoxicated(0);
            else
                TP->set_intoxicated(TP->query_intoxicated() - 15);
        }
