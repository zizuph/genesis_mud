...
{
    int i = ADMIN->restore_player(player);
...
{
    int stat = shadow_who->query_stat(SS_OCCUP),
    nlevels = ADMIN->query_number_levels(); 

    if (stat > 100)
        stat = 100;

    return ((nlevels - 1) * stat) / 100;
}
...
{
    int asize, level, nlevels = ADMIN->query_number_levels();
...
query_wohs_level()
{
    int nlevels = ADMIN->query_number_levels(), 
    stat, level, asize;
...
query_wohs_title()
{
    int sex = shadow_who->query_gender();
    string *titles, name = RNAME(shadow_who);
    string order = ADMIN->query_member_order(name);
