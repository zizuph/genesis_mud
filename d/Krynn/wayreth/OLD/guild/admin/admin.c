...
#define RECORD_FILE "/d/Krynn/log/guild/wohs/record"
#define ERROR_FILE  "/d/Krynn/log/guild/wohs/admin_error"
...
public string query_member_type(object player);
...
/* We want to work with an object just now, so this is commented out
public void
create()
{
    setuid();
...
{
    if (WIZ_CHECK < WIZ_NORMAL) return;
 ...
 */
public int
query_vote_conclave(object player, string order)
{
    if( query_member_type(player) == order )
    {
        return 1;
    }
    return 0;
...
restore_player(object player)
{
    ...
    if (!query_member_type(player))
    {
        ...
        restored = C(player->query_real_name()) + " player lost or had the wrong bits";
    }
    ...
    if (occ_name != GUILD_NAME)
    {
        ...
        restored = C(player->query_real_name()) + " player was missing the shadow";
    }
    ...
    robe->force_init(player);
        restored = C(player->query_real_name()) + " player was missing the robe";
    }

    mentor = pers_arr[ASM_REWARDER];
    reward = pers_arr[ASM_REWARD];
    if (reward != 0 && mentor != 0)
      {
    if(reward > 0)
      {
        ...
        player->change_level_mod(reward);
            ...
            gMembers[pl_name][ASM_REWARD] = 0;
            gMembers[pl_name][ASM_REWARDER] = "";
            write_file(RECORD_FILE, sprintf("%s was "+
            "rewarded %i levels.\n", pl_name, reward));
          }
      }
    else
    ...
    if (strlen(restored))
    {
        write_file(ERROR_FILE, sprintf("%s.\n", restored));
    ...
    // TO->remove_wohs_member(player, 3);
    return 1;
}
/*
...
make_appr_to_real(object player)
{
    ...
    if (data[ASM_APPRENTICE])
    {
        ...
        set_wohs_bits(player);
    /* The apprentice might not have enough xp to grow to a full title */
    if ( (player->query_wohs_level() + 1) == query_size_apprentice_titles() )
        player->change_level_mod(1);
    if ( book = present(SPELL_BOOK, player))
...
remove_wohs_member(object who, int reason)
{
    ...
    write_file(RECORD_FILE, ctime(time()) + " " +
...
public varargs void
endtest_seq(object appr, object master = 0)
{
    object env;
    if(!objectp(master))
    master = TP;
    ...
    && !CALL_BY(WOHS_STAFF_OF_TESTING) )
    return;

    gFullwiz_seq = 0;

    gFullwiz_alarm = set_alarm(4.0, 5.0, &do_endtest_seq(master, appr, env));
}
...
save_con()
{
   gConclave_members = ({"rastlin", "aridor", "baldacin", "boron"});
    save_object(ADMINFILE);
}
