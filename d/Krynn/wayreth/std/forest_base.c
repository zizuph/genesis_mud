/*
   ...
   extra_desc()   Wasn't called correctly in the base file from this file
                  it is now ;)
                                                   ------- Boron

   Allowed aspirants of High Sorcery to get to the gates of the tower
...
welcome(object who)
{
   if(SOHM_MANAGER->query_renegade_wohs(who))
      return 0;

   if(TP->query_guild_name_occ() == "Wizards of High Sorcery")
      return 1;

   // If they are SoHM and are at the level of Aspirant to
   // take the Test they can find the gates of the Tower
   if((TP->query_guild_name_occ() == "School of High Magic") &&
      (TP->query_guild_level_occ() == 8) &&
      (!SOHM_MANAGER->query_aspirant_title(TP)))
      return 1;
...
query_descr()
{
    ...
    return "You are walking around in a dark gloomy forest. The trees " +
           ...
           "an opaque canopy above you, " + time + TO->extra_descr() + "This " +
