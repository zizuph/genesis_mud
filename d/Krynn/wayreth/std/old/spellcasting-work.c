...
{
    int i, j;
    object *items, *casterInv, *invToCheck; 
    ...
    if ((i = sizeof(gIngredients)))
	    ...
	    while (j--)
	    {
		find_player("karath")->catch_msg("item i'm checking is "+
			casterInv[j]->short() + "\n");
	    ...
	    find_player("karath")->catch_msg("going to parse_command\n"+
		"invToCheck is " + sizeof(invToCheck) + "\n");
		...
		find_player("karath")->catch_msg("items are "+sizeof(items)+"\n");
