 ...
 * : Karath 970829
 *
 *		Fixed my modifications. I changed the method of handling
 *		herbs by checking the containers for the herb of the
 *		component name, then i check that the player can id
 *		the herb in question, finally i do an add_name to each
 *		of the herbs (like all the suranies if "suranie" or
 *		"five suranie" is the ingredient it's checking). After
 *		that, if it fails a check for a component or if it
 *		completes the check, it then removes the names of 
 *		the herbs that it has modified.
 *
 * : Karath 970826
...
public void cleanup_herb_names(object *herblist);
public void my_spell_attack(object attacker, object *attackTargs);
...
{
    int i, j, k;
    object *items, *casterInv, *invToCheck, *subInvToCheck, *herbsWeNamed; 
    ...
    if(i = sizeof(casterInv))
	while (i--)
	{
	    if (function_exists("create_container", casterInv[i]))
	      {
		if(casterInv[i]->query_prop(CONT_I_HOLDS_COMPONENTS) &&
		    !(casterInv[i]->query_prop(CONT_I_CLOSED)))
		{
		     subInvToCheck += all_inventory(casterInv[i]);
		}
	      }
	}

    invToCheck = subInvToCheck + casterInv;

    /* Use filter here. */
    if ((i = sizeof(gIngredients)))
        while (i--)
        {
	  /* Here, we isolate all herbs in our lower inventory and */
	  /* add the true name of the herb if the player can identify it */
	    if (j = sizeof(items = filter(subInvToCheck,
		 &operator(==)(gIngredients[i]) @ &->query_herb_name())))
	      {
		if ( items[0]->do_id_check(gCaster))
		  {
		    items->add_name(items[0]->query_herb_name());
		    herbsWeNamed += items;
		  }
	      }
		...
		if (gTmp > 0)
		  {
    		    cleanup_herb_names(herbsWeNamed);
		    return 0;
		  }
	    }
	    else
	      {
		/* We need to return zero and clean up the herbs we've named */
    		cleanup_herb_names(herbsWeNamed);
		return 0;
	      }
	}
    /* We succeeded, but still need to un-name herbs */
    cleanup_herb_names(herbsWeNamed);
    return 1;
}

/* Function Name : cleanup_herb_names
 * Description   : Removes the add_name of the herb_name in the herbs in
 *		   our containers.
 * Arguments	 : *herblist - the array of herb objects we've messed with.
 * Returns       : Void
 */

public void
cleanup_herb_names(object *herblist)
{
    int i = sizeof(herblist);

    while (i--)
      {
	herblist[i]->remove_name(herblist[i]->query_herb_name());
      }
    return;
}
...
cast_spell()
{
    ...
    if (gSpell_attack || gSpell_offensive)
    {
        ...
        /* This must be called */
        if (gSpell_attack)
	    set_alarm(0.1, 0.0, &my_spell_attack(gCaster, gTargets));
...
/*
 * Function name: my_spell_attack
 * Description:	  This function is a replacement for the combat system call
 * Arguments:	  attacker    - The one who cast the spell
 *		  attackTargs - The one(s) who got hit by the spell
 */	
public void
my_spell_attack(object attacker, object *attackTargs)
{
    int hurt, i, size;
    ...
    while (i--)
        ...
        {
            hurt = gSpell_damage(attacker, attackTargs[i], gResults[i]);
            hurt -= hurt * gSpell_resist(attackTargs[i]) / 100;

            hitresult = attackTargs[i]->hit_me(hurt, gDamage_type, attacker,  -1);

            gSpell_message(attacker, attackTargs[i], hitresult);
            if (attackTargs[i]->query_hp() == 0)
                attackTargs[i]->do_die(attacker);
	    else
	        /* This will start an attack too */
	        attackTargs[i]->notify_got_hit_by_spell(attacker);
        }
