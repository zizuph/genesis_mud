...
create_tower_room()
{
   set_short("Southern waiting room");
   ...
   add_item(({"placard","golden placard","small placard"}),
   ...
   add_cmd_item(({"placard","golden placard","small placard"}),"read",
        "For a mug of coffee just 'pour coffee' for yourself and enjoy!\n");

   add_exit(WTOWER + "guestroom", "northeast", 0);

   clone_object(WOBJ + "guest_board")->move(TO);
   clone_object("/d/Krynn/guilds/wohs/objs/rules_book")->move(TO);
}
