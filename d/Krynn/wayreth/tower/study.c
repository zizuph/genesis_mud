...
create_tower_room()
{
   ...
   add_cmd_item(({"plaque","large plaque"}),"read","A large " +
            "plaque on the wall reads : 'Please fare quiet in this " +
            "area, people are studying here.'\n");

   add_exit(WTOWER + "studyroomn", "north", 0);
   add_exit(WTOWER + "stairwell07", "east", 0);
   add_exit(WTOWER + "post", "south", 0);
   add_exit(WTOWER + "mage_library", "west", 0);

   clone_object(WOBJ + "policy_board")->move(this_object());
}
