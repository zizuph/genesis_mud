...
start_spell_fail(string verb, string arg)
{
    ...
    if (!check_skills())
    {
	write("Sorry, but you are not skilled enough for this spell!\n");
	return 1;
    }
    ...
    {
        write("Sorry, but you don't have the right ingredients for the spell!\n");
        return 1;
    }
...
cast_spell()
{
    ...
    gCaster->catch_msg(sprintf("Results: %O\n", gResults));
...
spell_attack(object attacker, object target)
{
    ...
    gCaster->catch_msg(sprintf("SP-Results: %O\n", gResults));
