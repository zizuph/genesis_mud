...
{
    horse = TP->query_mount();

    if (!horse)
	remove_object();

    write("You gather the reigns in your hand, calming your "+horse->short()+".\n");
    say(QCTNAME(TP)+" gathers the reigns in "+POSSESSIVE(TP)+" hand, calming "+
      POSSESSIVE(TP)+" "+horse->short()+".\n");
    return 1;
}
...
int do_pat(string str)
{
    ...
    write("You lean forward, rubbing your "+horse->short()+"'s neck soothingly.\n");
    say(QCTNAME(TP)+" leands forward, rubbing "+POSSESSIVE(TP)+" "+horse->short()+
      "'s neck soothingly.\n");
    return 1;
}
...
int do_rear(string str)
{
    ...
    write("You pull sharply on the reigns and dig your heels into the "+
      horse->short()+"'s flanks, which rears back, kicking "+POSSESSIVE(horse)+
      " front legs high in the air.\n");
    say(QCTNAME(TP)+" pulls sharply on the reigns and digs "+POSSESSIVE(TP)+
      " heels into the "+horse->short()+"'s flanks, which rears back, kicking "+
...
int do_halt(string str)
{
    ...
    write("You tug on the reigns slightly, stilling your "+horse->short()+".\n");
...
int do_snap(string str)
{
    ...
    write("You snap the reigns, prodding your "+horse->short()+" forward.\n");
...
show_subloc(string subloc, object carrier, object for_obj)
{
    ...
    if(for_obj == carrier)
	str = "You are riding your "+horse->short()+tmp+".\n";
    else
...
laugh(string str)
{
    object *oblist, me = E(TO), horse = me->query_mount();
    string *how;

    if (!stringp(str))
    {
	write("You nearly fall off your "+horse->short()+
	  " laughing.\n");
	all(" nearly falls off "+POSSESSIVE(me)+" "+horse->short()+
	  " laughing.\n");
	SOULDESC("laughing");
	return 1;
    }
...
int pounce(string str)
{
    ...
    actor("You pounce from your "+horse->short()+" onto", 
      oblist, ", knocking " + obj + " flat on " + pos + 
      " back!");
    all2act(" pounces from "+POSSESSIVE(me)+" "+horse->short()+" onto", 
      oblist, ", knocking " + obj + " flat on " + pos + " back!");
    target(" pounces on you from "+POSSESSIVE(me)+" "+horse->short()+
...
int back(string str)
{
    ...
    if (!stringp(how[0]))
    {
	write("You back your "+horse->short()+" away" + how[1] + ".\n");
	all(" begins to back "+POSSESSIVE(me)+" "+horse->short()+
	  " away " + how[1] + ".");
	return 1;
    }
    ...
    actor("You back your "+horse->short()+" away" + how[1] + 
      " from", oblist);
    all2actbb(" backs "+POSSESSIVE(me)+" "+horse->short()+" away" + 
      how[1] + " from", oblist, 0, how[1]);
    targetbb(" begins to back "+POSSESSIVE(me)+" "+short() + 
...
int nudge(string str) 
{
    ...
    actor("You maneuver your "+horse->short()+" to nudge", oblist);
    all2act(" maneuvers "+POSSESSIVE(me)+" "+horse->short()+" to "+
      "nudge", oblist);
    target(" maneuvers "+POSSESSIVE(me)+" "+horse->short()+" to "+
      "nudge you.", oblist);
    return 1;
}
