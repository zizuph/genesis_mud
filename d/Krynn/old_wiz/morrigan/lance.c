...
do_rejoice(string s)
{
    ...
    gWielder->catch_msg("You hold the Dragonlance high and cry out in joy!\n");
    tell_room(E(gWielder), QCTNAME(gWielder)+" holds the Dragonlance high and "+
      "cries out in joy!\n", gWielder);
    return 1;
}
...
*                the dragonlance.
* Arguments    : object new = new environment
*/
void
no_take(object new)
{
...
*/
int wield_message1(object ob)
{
    ob->catch_msg("\nAs you grip the sleek silver lance, you see a "+
      "vision...\n\n\n");
    tell_room(E(ob), QCTNAME(ob)+"'s eyes glaze over, as "+
      PRONOUN(ob)+" grips the silver sleek lance in both hands, "+
      "and "+PRONOUN(ob)+" stands motionless, staring into the "+
      "distance with a blank look...\n", ob);

/*
    ob->add_prop(MAGIC_I_RES_FIRE,
      ob->query_prop(MAGIC_I_RES_FIRE) + 35);

    ob->add_prop(MAGIC_I_RES_COLD,
      ob->query_prop(MAGIC_I_RES_COLD) + 25);

    ob->add_prop(MAGIC_I_RES_ELECTRICITY,
      ob->query_prop(MAGIC_I_RES_ELECTRICITY) + 25);

    ob->add_prop(MAGIC_I_RES_ACID,
      ob->query_prop(MAGIC_I_RES_ACID) + 25);
*/

    if (E(TO) == ob)
	set_alarm(5.0, 0.0, "wield_message2", ob);
    return 1;
}

void wield_message2(object ob)
{
    ob->catch_msg("You see a charging stallion, armoured in purest "+
    ...
    if (query_wielded() == ob)
	set_alarm(5.0, 0.0, "wield_message3", ob);
}

void wield_message3(object ob)
{
    ob->catch_msg("Within the visor that covers his face is light, "+
      "brilliant and life-giving.\n\n");

    if (query_wielded() == ob)
	set_alarm(5.0, 0.0, "wield_message4", ob);
}

void wield_message4(object ob)
{
    ob->catch_msg("The great charger leaps into the air, and massive "+
    ...
    if (query_wielded() == ob)
	set_alarm(5.0, 0.0, "wield_message5", ob);
}

void wield_message5(object ob)
{
    ob->catch_msg("The lance shines with a life, a purpose of it's own,"+
      " and the darkness falls before it.  Born of the world and the "+
      "heavens, it has the true power, the true good.\n\n");

    if (query_wielded() == ob)
	set_alarm(5.0, 0.0, "wield_message6", ob);
}

void wield_message6(object ob)
{
    ob->catch_msg("The darkness destroyed, the dragon lands before you, "+
      "and you fall to your knees in awe and reverance.  The Knight "+
      "releases the lance from his grasp, and holds it towards you...\n\n");

    if (query_wielded() == ob)
	set_alarm(5.0, 0.0, "wield_message7", ob);
}

void wield_message7(object ob)
{
    ob->catch_msg("With some hesitation you reach out and take the lance.\n"+
...
unwield(object what)
{
    gWielder->catch_msg("You unwield the Dragonlance, and feel "+
      "its warmth retreat from your body.\n");
    tell_room(E(gWielder), QCTNAME(gWielder)+" unwields the "+
      "Dragonlance.\n", gWielder);

/*
    gWielder->change_prop(MAGIC_I_RES_FIRE,
      gWielder->query_prop(MAGIC_I_RES_FIRE) - 35);
    gWielder->change_prop(MAGIC_I_RES_COLD,
      gWielder->query_prop(MAGIC_I_RES_COLD) - 25);
    gWielder->change_prop(MAGIC_I_RES_ELECTRICITY,
      gWielder->query_prop(MAGIC_I_RES_ELECTRICITY) - 25);
    gWielder->change_prop(MAGIC_I_RES_ACID,
      gWielder->query_prop(MAGIC_I_RES_ACID) - 25);
*/
...
did_hit(int aid,string hdesc,int phurt,object enemy,int dt,int phit,int dam)
{
    ...
    {
	gWielder->catch_msg("Fear wrenches your body as you feel an intense "+
	  "pain tearing up your arm...\n");
	gWielder->catch_msg("The "+short()+" erupts in a violent explosion of "+
	  "silver flames, burning you badly!\n");
	tell_room(E(gWielder), QCTNAME(gWielder)+"'s "+short()+" erupts in a "+
	  "violent explosion of silver flames, burning "+OBJECTIVE(gWielder)+
	  " badly!\n", gWielder);
	...
	if(!random(3) && dam)
	{
	    ...
	    wielder->catch_msg("Your "+short()+" feels warm in your grasp "+
	      "as it"+dam_descr);
	    gWielder->tell_watcher("The "+short()+" wielded by "+
