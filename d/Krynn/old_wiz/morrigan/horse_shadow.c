...
void set_mount(object mount) { horse = mount; }
object query_mount() { return horse; }

string query_m_in()
{
    return "arrives astride a "+query_mount()->short()+".";
}

string query_m_out()
{
    return "rides "+POSSESSIVE(shadow_who)+" "+query_mount()->short();
}

void remove_object()
{
    query_mount()->remove_prop(OBJ_M_NO_ATTACK);
    query_mount()->unset_no_show_composite();
    query_mount()->set_rider(0);
    ::remove_object();
}
...
varargs int move_living(string how, mixed to_dest, int dont_follow, int no_glance)
{
	...
	tell_object(shadow_who,"You lose your balance and fall off your "+
	  query_mount()->short()+", landing with a thud.\n");
	tell_room(E(shadow_who), QCTNAME(shadow_who)+" falls off "+
	  POSSESSIVE(shadow_who)+" "+query_mount()->short()+", landing "+
	  "with a thud.\n",shadow_who);
	horse->command(query_verb());
	query_mount()->do_dismount();
	return 7;
    }
...
hit_me(int wcpen, int dt, object attacker, int attack_id, int target_hitloc = -1)
{
	...
	else
	{
	    shadow_who->catch_tell("As you are struck, you lose your balance "+
	      "and fall from your "+query_mount()->short()+".\n");
	    tell_room(E(shadow_who), "As "+PRONOUN(shadow_who)+" is struck, "+
...
{
    object me = shadow_who, st = query_mount();
    object *enemies, *tmp;
    int duration, i;
...
{
    object ob, *obj, weapon, *weapons, horse = query_mount();
