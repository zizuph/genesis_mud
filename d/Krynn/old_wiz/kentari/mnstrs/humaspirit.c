/* Material Spirit of Huma Dragonbane, Knight of the Crown
		 by Kentari
Made to defend the innocent baby dragon that is being
killed by undesirables in Kentari's workroom. He
comes upon Fizzy being attacked, and leaves when
the dragon hatchling is safe once again. *grin*
11/25/96
...
create_krynn_monster()
{
   ...
   set_alarm(3.0, 0.0, "defend_fizzy");
   trig_new("%w 'fail' %s", "retry_rescue");
   trig_new("%w 'attacks' %s", "rescue_fizzy");
}
...
{
   if (!attacker)
      return "say I am here to protect the dragon hatchling from any "+
   "that may attempt to cause him harm.";
   else
      return "say It's a good thing I was here to defend Fizzy from that "+
...
rescue_fizzy()
{
   set_alarm(2.0, 0.0, "command","rescue fizzy");
...
defend_fizzy()
{
   command("emote materializes before you to defend those in need!");
   command("kassist fizzy");
   set_alarm(1.0, 0.0, "command", "rescue fizzy");
   set_alarm(2.0, 0.0, "define_attacker");
   set_alarm(3.0, 0.0, "command", "say Attacking this divine hatchling"+
      "-child of Paladine is inexcusable! You have thrown away the "+
