...
#define KNIGHT_LOG      "/d/Krynn/solamn/guild/log/"
...
{
    add_knightlist(squire, knight);
    add_squirelist(squire, knight);
    if (!find_living(knight)->query_knight_level() &&
	!find_living(squire)->query_knight_level())
      SPONSOR_TREE->add_new_relation(knight,squire,0);
}
...
public nomask int
perform_non_knight_award(object who, string initiator, string reason,
			 mixed withwhat)
{
    object award_obj;
...
do_banish(string str)
{
    ...
    ret_val = perform_action(TP, who, reason, "banishment");
    if (ret_val == 1)
...
leave(object ob, int force)
{
    ...
    ob->add_exp(-(ob->query_exp_combat()) / 6, 1);   /* Combat exp */

    for (i = 0; i < sizeof(SKILLS); i++)
      {
	  num = ob->query_skill(SKILLS[i]);
	  if (force && SKILLS[i] <= 60000)
	    {
    ...
    /*ob->setup_skill_decay();*/
    /* Start the skill decay immediately */
