...
set_alignment(int a)
{
    ...
    if (a < -20 && find_call_out("evil_check") == -1)
      call_out("evil_check", 10);
}
		...
		if (new_level >= 3)
		  {
		      ...
		      if (shadow_who->test_bit("Krynn", 1, 4) == 0)
			{
			    if (shadow_who->test_bit("Krynn", 1, 5) == 0)
			      {
				  shadow_who->catch_msg("You are now once again"
							+ " a Squire of Solamnia!\n");
				  truelevel = 2;
			      }
			    if (shadow_who->test_bit("Krynn", 1, 5) == 1)
			      {
				  shadow_who->catch_msg("You are now once again"
		      ...
		      if (shadow_who->test_bit("Krynn", 1, 4) == 1)
			{
			    if (shadow_who->test_bit("Krynn", 1, 5) == 0)
			      {
				  shadow_who->catch_msg("You are now once again"
