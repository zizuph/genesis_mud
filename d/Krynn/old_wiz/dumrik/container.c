...
remove_subloc(string sloc)
{
    ...
    map(all_inventory(this_object()), "fix_ob_subloc_when_remove", 
	      this_object(), sloc);
    cont_subloc_ids = map(cont_subloc_ids, "map_subloc_id",
			     this_object(), sloc);
...
{
    return filter(all_inventory(), "subloc_filter", this_object(), sloc);
}
