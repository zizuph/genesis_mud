...
hide(string str)
{
	...
	{
	  temp->catch_msg(desc_many(({ this_player() })) +
...
search(string str)
{
    ...
    if (this_player()->query_prop(CONT_I_HIDDEN)) this_player()->reveal_me(1);
...
sneak(str)
{
	...
	environment(this_player())->query_prop(OBJ_I_LIGHT) -
	 this_player()->query_prop(OBJ_I_LIGHT)))
    {
	notify_fail("You cant sneak shining like that!\n");
	return 0;
    }
