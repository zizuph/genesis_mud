...
string
ask_destinations(object asker)
{
    mixed places;

    if (!gShip)
      command("say Sorry, I must have misplaced my ship, therefore I am unable " +
	      "to take you anywhere.");
    else
    {
	places = map(gShip->query_destinations(),capitalize);
	command("say Well, we have " + sizeof(places) + " different destinations " +
		...
		"sail to additional places soon.");
    }
}
...
set_destination(object asker, string where)
{
    ...
    if (member_array(L(where),gShip->query_destinations()) < 0)
    {
	if (asker)
	...
	gShip->start_ship(0);
	seteuid(getuid());
	log_file("secretship",sprintf("%s %s asked: %s\n",
				      ctime(time())[4..15],
				      asker->query_name(),
				      gDestination),20000);
    }
