/*
   Pax Tharkas, wall level, Southeast part of barracks
...
reset_pax_room()
{
   if((!P("sivak", TO)) && (!P("kapak",TO)) && (!P("bozak",TO)))
   {
      ...
      guard2->add_descr("sitting at the table");
      guard3->add_descr("sitting at the table");
...
create_pax_room()
{
   ...
   set_short("Southeast corner of barrack");
   set_long("@@room_descr");
   ...
   add_exit(WALL_LEVEL + "ne_barrack", "north", 0);
   add_exit(WALL_LEVEL + "sw_barrack", "west", 0);
   add_exit(WALL_LEVEL + "nw_barrack", "northwest", 0);

   clone_object(POBJ + "contraption")->move(TO);
}
...
barrack_descr()
{
   ...
   if (inv && sizeof(inv) > 0)
   {
     ...
     if (sizeof(live) > 0)
        return ("Looking across the rest of the room you see " +
                 COMPOSITE_LIVE(live) + ". ");
   }
   return ("Looking across the room you don't see anyone else. ");
}
...
room_descr()
{
   return "You are standing in the southeast corner of a very huge room. " +
          "This room is quite big, it probably covers this entire level " +
