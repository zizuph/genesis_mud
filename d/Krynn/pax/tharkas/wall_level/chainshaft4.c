/*
   ...
   in Sla Mori. Entrance to the chainchamber.
...
object up_room = WALL_LEVEL + "chainchamber1";
object down_room = WALL_LEVEL + "chainshaft3";
...
climb_chain(string dir)
{
   if (dir == "down" || dir == "down chain" || dir == "down the chain")
   {
      ...
      down_room->someone_arrives(TP,0);
      TP->move_living("M", WALL_LEVEL + "chainshaft3", 0 ,0);
      return 1;
   }
   ...
   if (dir == "up" || dir == "up chain" || dir == "up the chain")
   {
      ...
      TP->move_living("M", WALL_LEVEL + "chainchamber1", 0, 0);
      return 1;
   }
