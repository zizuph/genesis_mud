/*
   Pax Tharkas, wall level, Northwest part of barracks
...
reset_pax_room()
{
   if((!P("sivak", TO)) && (!P("kapak",TO)) && (!P("bozak",TO)))
   {
      ...
      guard3->add_descr("mending his armour");
...
create_pax_room()
{
   ...
   set_short("Northwest corner of barrack");
   set_long("@@room_descr");
   ...
   add_exit(WALL_LEVEL + "ne_barrack", "east", 0);
   add_exit(WALL_LEVEL + "sw_barrack", "south", 0);
   add_exit(WALL_LEVEL + "se_barrack", "southeast", 0);
}
...
barrack_descr()
{
   ...
   return ("Taking a look across the rest of the room, you don't see " +
           "anyone else. ");
}
...
room_descr()
{
   return "You are standing in the northwest corner of a very huge room. " +
          "This room is very large, it probably covers this entire level " +
