/*
   Pax Tharkas, Mountain road towards the open iron ore mine.

   mountain_road2.c
   ----------------

   Coded ........: 95/03/31
   By ...........: Jeremiah

   Latest update : 95/04/12
   By ...........: Jeremiah

*/


#include <stdproperties.h>
#include <ss_types.h>
#include <macros.h>
#include "/d/Krynn/common/defs.h"
#include "/d/Krynn/pax/local.h"
#include <composite.h>

inherit PAXOUTROOM;

void
create_pax_room()
{
   set_short("mountain road");
   set_long("You are walking on a narrow mountain road, wide enough " +
            "though, to drive a horsedrawn wagon along it. At one side " +
            "you have the mountain side rising, like an inpenetrable " +
            "wall, high above you. To the other side the mountain " +
            "falls steep downwards towards to courtyard of Pax Tharkas. " +
            "Off to the northeast you see the large scarred mountain " +
            "surface, where an open iron ore mine is.\n");

   OUTSIDE;

   add_exit(COURT_YARD + "mountain_road1", "northwest", 0);
   add_exit(COURT_YARD + "mountain_road3", "east", "@@upwards", 3);
}


int
upwards()
{
   write("You trudge your way up the mountain road.\n");
   return 0;
}
