/* Altered by Navarre on June 10th 2006
 * Added support for "say to".
 * 
 * Navarre July 2006, added oblist[1] to be excluded in tell_room
 */
...
do_say(string s)
{
  ...
  if( parse_command(lower_case(s), environment(this_player()), "'to' [the] %l %s", oblist, s))
  {
    write_vbfc("Your voice sounds cold and harsh as you speak to "+QTNAME(oblist[1])+" through the mask, '"+s+"'\n");
    oblist[1]->catch_msg("A cold voice thunders to you from beneath "+QTNAME(TP)+"'s mask, '"+s+"'\n");
    tell_room(environment(TP), "A cold voice thunders to "+QTNAME(oblist[1])+" from beneath "+QTNAME(TP)+"'s mask, '"+s+"'\n", ({this_player(), oblist[1]}));
    return 1;
  }        
