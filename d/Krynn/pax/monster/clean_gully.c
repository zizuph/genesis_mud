...
monster_do_pick()
{
    ...
    inv = filter(all_inventory(ob), "can_pick", this_object());

    if (sizeof(inv))
    {
        TO->command("emote points at the stuff on the ground.");
        TO->command("say Magic.");
        tell_room(E(TO),"Suddenly the stuff on the ground shimmers and then disappears.\n");
        TO->command("gasp");
