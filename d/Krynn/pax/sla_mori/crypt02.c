...
create_pax_room()
{
    ...
    gDoor->set_other_room(PSLA + "tomb01");
...
start_zombies()
{
    ...
    tell_room(TO, BS("Slowly the body begins to move. As you look into " +
		...
		"The guard rises from the bier and slowly approach " +
                "you.", 70));

    gZombie = clone_object(PMONSTER + "zombie_dwarf");
    gZombie->move(TO);

    if (PAXMASTER->query_zombie() == 2)
    {
...
my_desc()
{
    ...
    if (!gZombieactive)
        desc += " On the bier is a body of a short figure " +
	"dressed in resplendent armour lying. Skeletal hands grasping the " +
	"hilt of an axe lying across the body.";
