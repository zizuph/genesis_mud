...
update_treasuremap()
{
    string objname, iname, foo, indstr;
    int count;
    object *inv;
    ...
    string *pnames = map( FILTER_PLAYERS(all_inventory(shadow_ship)),
                            &->query_real_name() );
    ...
    foreach(object item: inv)
    {
        ...
        if ( !wildmatch("*#*",iname) )
            ...
            sscanf(iname,"%s#%s",objname,foo);

        indstr = val2str(time()*10+count);

        sunken_treasure_map[indstr] = ({objname}) + pnames;
