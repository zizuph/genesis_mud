...
create_sea_room()
{
    ...
    add_item(({"reef","great reef","coral","maze"}),
        "To your south is a great reef, a maze of coral that would take a " +
    ...
    add_item(({"waters", "water"}),
        ...
        "below the surface! If you dare, you could probably swim down " +
        "and investigate.\n");

    add_row_exit(ROOM + "7o", "north", "@@becalmed", 1);
    add_row_exit(ROOM + "5n", "southwest", "@@becalmed", 1);
    add_row_exit(ROOM + "6p", "east", "@@becalmed", 1);
    add_row_exit(ROOM + "6n", "west", "@@becalmed", 1);

    add_exit(ROOM + "6ouw1", "down", "@@swim_down", 1, 1);
    set_sink_path(ROOM + "6ouw1");
...
short_descr()
{
    return "churning waters on the edge of a great reef in the Turbidus Ocean";
}
 
string
long_descr()
{
    return turbidus_sea_desc() + "The waters here churn malevolently, as if " +
        "stirred up by something - or somethings - beneath the surface. " +
        "To your south is the edge of a great reef, a " +
