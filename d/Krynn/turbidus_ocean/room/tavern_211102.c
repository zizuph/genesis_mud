...
reset_sea_room()
{
    if (!tavernkeeper)
    {
        ...
        tell_room(TO,"The damp greenish tavern-keeper surveys the dismal state of " +
...
getput_treasuremap()
{
   ...
   for ( i=0;i<nind;i++ )
   {
       fname = sprintf("%s.c",sunken_treasure_map[ind[i]]);
       if ( file_size(fname) == -1 )
           sunken_treasure_map = m_delete(sunken_treasure_map,ind[i]);
   }
...
find_in_dross()
{
    ...
    if (!tavernkeeper)
    {
        ...
        {
            str = sunken_treasure_map[ind[random(nind)]];
        ...
        return "Free to search about the timbers and canvas dross of the ramshackle walls, "+
    ...
    else
    {
        ...
        tell_room(this_object(),QCTNAME(TP) + " breaks off a search of the dross that "+
                    "makes up the structure of the tavern.\n",TP);
    }
}
...
{
    if ( !tavernkeeper )
        return "A knot of cutthroats and brigands slink away in fear of the curse.\n";
    else
        return "A noisy patron goes suddenly silent; his body tumbles outside to the water.\n";
}
...
{
    if ( !tavernkeeper )
        return "Sea urchins, marooned cabin boys and girls, fossick about the place.\n";
    else
        return "A bottle of rum is smashed over the head of someone who looked thirsty.\n";
}
...
{
    if ( !tavernkeeper )
        return "A bulge of canvas and cordage makes a melancholy sound, caught in the chill wind.\n";
    else
        return "A group of Ergothian mutineers sing of better days, deep in their cups.\n";
}
...
create_sea_room()
{
    ...
    add_search(({ "timbers", "dross", "lost", "things lost" }), 5, "find_in_dross",1);
    ...
    add_tell("Gulls peer intently through the holes and gaps in the walls of the tavern.\n");
    add_tell("A crab picks particles of rotted patrons' flesh from the coral of the bar.\n");
...
{
    if ( !tavernkeeper )
        write("The chill sweeping in through the walls makes it hard to smell anything.\n");
    else
        write("Warm smoke fills the nostrils, keeping the cold of the sea at bay.\n");
}
	...
	else if (!test) 
	{
		...
		else 
		{
                    if ( !tavernkeeper ) 
                    {
                        ...
                        say(QCTNAME(ob) + " steals " + drink->short() + ".\n", ob);
                    }
                    else
                    {
