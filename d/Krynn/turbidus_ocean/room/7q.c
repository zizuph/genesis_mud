...
create_sea_room()
{
    ...
    add_item(({"sea stack", "stack", "column", "column of rock",
        "volcanic sea stack"}),
        "A steep column of volcanic rock rises out of the ocean here, " +
        "a sea stack isolated in the middle of the Turbidus Ocean.\n");

    add_row_exit(ROOM + "6q", "south", "@@becalmed", 1);
    add_row_exit(ROOM + "7r", "east", "@@becalmed", 1);
    add_row_exit(ROOM + "8q", "north", "@@becalmed", 1);
    add_row_exit(ROOM + "7p", "west", "@@becalmed", 1);

    add_exit(ROOM + "7quw1", "down", "@@swim_down", 1, 1);
    set_sink_path(ROOM + "7quw1");
...
short_descr()
{
    return "near a steep sea stack in the Turbidus Ocean";
}
 
string
long_descr()
{
    return turbidus_sea_desc() + "A volcanic sea stack rises out of " +
        "the ocean here, around which the waters churn malevolently, " +
