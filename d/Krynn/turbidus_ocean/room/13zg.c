...
create_sea_room()
{
    ...
    add_row_exit(ROOM + "12zg", "south", 0, 1);
    add_row_exit(ROOM + "13zh", "east", 0, 1);
    add_row_exit(ROOM + "14zg", "north", "@@no_go", 1);
    add_row_exit(ROOM + "13zf", "west", 0, 1);
...
{ 
    write("You decided not to sail in that direction, as the waters are " +
      ...
      "Turbidus Ocean are never seen of again!\n");
    return 1;
}
