...
special_attack(object enemy)
{
    ...
    int count, gullcount = sizeof(gulls);

    if (!random(10))
    {
        hitresult = enemy->hit_me((100 + random(100) + (gullcount * 25)),
                                  W_IMPALE, TO, -1);
        ...
        if (gullcount > 0)
        {
            ...
            // Gulls participate in the special attack even without
            // actually fighting the player. Make sure to attack the
            // player!
            for (count = 0; count < gullcount; count++)
            {
                gulls[count]->command("kill " + enemy->query_name());
            }            
        }
