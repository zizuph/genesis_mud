...
public int enter_cave();
public int query_cave_found();
...
create_kurinost_room()
{
    ...
    add_exit(RSROOMS + "rs_cave", "southwest", 0, enter_cave,
        query_cave_found);
 ...
 *  FUNCTION : enter_cave
 *  Arguments: none
 *  Returns  : int - fatigue = 5
 * 
 *  Shows a description of entering the cave, returns how tired the player becomes from taking this exit.
 *
 */
public int
enter_cave()
...
/*
 *  FUNCTION : query_cave_found
 *  Arguments: none
 *  Returns  : 1/0 - has player found the exit?
 * 
 *  Returns 0 if player has found the exit, 1 if player has not.
 *
 */
public int
query_cave_found()
{
    return !(TP->query_prop(RSCAVEFOUND));
}
