...
// G L O B A L   V A R S
int gItems = 0, gMaxItems = 4 + random(5);
status knife_found = 0; // only one knife per reset.
string gBeach_desc;
...
generate_beach(string beach_desc)
{
    ...
    add_item(({ "cliff", "cliffs", "steep cliff", "steep cliffs",
        "dark wall", "wall", "cliffside" }),
        "Rising like a dark gray wall that reaches almost a " +
        "hundred meters towards the sky in some places, the " +
        "steep granite cliffs of the coast are almost barren " +
        "with only a few small bushes and cactus plants " +
        "growing in sheltered spots on the cliffside.\n");
    add_item(({ "granite", "granite wall" }), "Glistening with " +
        "moisture from the ocean, the steep sides of the granite " +
        "cliffs seems to cover the coast in shades of gray, " +
        "ranging from light gray to black. Rising in tall " +
        "structures of varying heights, the cliffs are mostly " +
        "barren and reach as high as a hundred meters in some " +
        "places. Though appearing dark and foreboding, the dark " +
        "colors of the granite are occasionally broken by small " +
        "patches of moss, scattered few bushes or nests with " +
        "birdlife.\n");
    ...
    add_item(({ "shore", "coast", "coastline" }),
        "Stretching like a dark wall, the cliffs of the shore " +
        "disappear far away to the northwest and a short way to " +
        "the east. The coastline is rugged with large rocks " +
        "sticking out of the water in many places. Right here, " +
        "however, the shore has widened into a small beach, " +
        "covered in dark gray sand.\n");
    add_item(({ "water", "bay", "bay of balifor" }),
        "Strong ocean winds whip the wavetops to foam here and " +
        "there across the turbulent dark blue waters of the Bay " +
        "of Balifor.\n");
    add_item(({ "sea", "ocean" }), "Strong ocean winds tug at you " +
        ...
        "is the rugged shoreline with its tall cliffsides. " +
        "Wavetops are whipped to foam here and there, as the " +
        "ocean winds blow across the turbulent water.\n");
    ...
    add_item(({ "view", "spectacular view", "nice view" }),
        "Strong ocean winds tug at you as you gaze out over the " +
        "sea. From down here on the beach you have a spectacular " +
        "view of the towering dark granite cliffs of the coast. " +
        "Reaching as high as one hundred meters in some places, " +
        "the cliffs rise like a dark wall before you.\n");
    add_item(({ "seagull", "seagulls", "white seagull",
        "white seagulls", "life", "bird", "birds", "birdlife",
        "colonies", "small colonies", "nests" }),
    ...
    if (GET_TIMEOFDAY == TOD_NIGHT)
    {
            ...
            GET_MOON_PHASE(LUNITARI) == "full")
        {
            night_desc = "Dominating the night sky, the two full " +
                "moons Solinari, and Lunitari, spread a pale " +
                "reddish light out across the Bay of Balifor, " +
                "bathing the beach in a bright ghostly light.";
        }
...
show_items()
{
    ...
    {
        return "Searching through the sand of the beach with your " +
            "eyes, you quickly notice something sticking out of " +
            "the sand near your feet. ";
    }
}
...
search_beach(object who, string what)
{
    ...
    if (who->query_skill(SS_AWARENESS) < 20)
    {
        who->catch_msg("Low skill, have a driftwood.\n");

        // allow the player to find some driftwood,
        // even with very low awareness.
    ...
    {
        // Items to be found, and player has enough awareness!
        ...
        switch (random(10))
        {
            case 0..6:
                // Give player a piece of driftwood.
                say(QCTNAME(who) + " searches around in the " +
                    "coarse grey sand and finds a piece of " +
                    "driftwood!\n");

                clone_object(RSOBJS + "driftwood")->move(who, 1);

                return "You search around in the coarse grey " +
                    "sand and manage to find a piece of " +
                    "driftwood!\n";

            case 7..9:
            // Fallthrough: A rare item has been found. See below

        }
                                                               
        if (!knife_found && !random(10))
        {
            ...
            return "You search around in the coarse grey sand " +
                    "and manage to find an old rusty fishing " +
                    "knife!\n";
        }
        ...
        switch (random(10))
        {
            case 0..5:
                ...
                say(QCTNAME(who) + " searches around in the " +
                    "coarse grey sand and finds a small seashell!\n");

                clone_object(RSOBJS + "seashell")->move(who, 1);

                return "You search around in the coarse grey sand " +
                    "and manage to find a nice seashell!\n";


            case 6..7:
                // Give player a small amber gemstone.
                say(QCTNAME(who) + " searches around in the " +
                    "coarse grey sand and finds a small stone!\n");

                clone_object(AMBER)->move(who, 1);

                return "You search around in the coarse grey sand " +
                    "and manage to find a small yellow amber!\n";


            case 8..9:
                // Give player a small coral gemstone.
                say(QCTNAME(who) + " searches around in the " +
                    "coarse grey sand and finds a piece of coral!\n");

                clone_object(CORAL)->move(who, 1);

                return "You search around in the coarse grey sand " +
                    "and manage to find a piece of pink coral!\n";
