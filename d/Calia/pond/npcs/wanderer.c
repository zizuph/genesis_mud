...
{
    int i, x;
    object *weapons, *adj_rooms, *enemies;
    string *adj_rooms_desc;
    ...
    else
    {
        ...
        enemy->catch_msg(QCTNAME(TO) + " makes a quick motion with" +
            ...
            weapons[0]->short() + " flies out of your grasp!" +
            " The weapon flies " + adj_rooms_desc[i] + ".\n");
        tell_room(environment(TO), QCTNAME(TO) + " makes a quick" +
            " motion with his hand and barks an alien word, and" +
            " suddenly the " + weapons[0]->short() + " flies" +
...
reply()
{
    if (TP->query_prop(CALIA_I_TALK_WANDERER) ||
...
react_give(object ob, object from)
{
    ...
    if (ob->query_herb_name() == "angeltear")
    {
        ...
        if (!from->test_bit(GQ_DOMAIN, GQ_GROUP, GQ_BIT)
            && !from->query_prop(CALIA_I_TALK_WANDERER))
        {
            ...
            command("smile wryly");
            from->add_prop(CALIA_I_TALK_WANDERER, 1);
            command("put herbs in pouch");
...
hook_process_accept_items(mapping items)
{
    ...
    foreach (object player, mapping objs : items)
    {
        foreach (object obj : m_indexes(objs))
        {
            if (!objectp(obj))
            {
                send_debug_message("calia_hylar", "Given object no longer exists.");
                continue;
            }

            if (member_array(player, query_enemy(-1)) > -1)
            {
                command("say You can keep it, bastard!");
                give_back_helper(obj, player, objs[obj]);
                continue;
            }

            if (obj->query_herb_name() != "angeltear")
            {
                command("say Thank you. Not exactly what I need, though.");
                give_back(obj, present("pouch", this_object()), "put", "in", objs[obj]);
            }
            else
            {
                command("emote studies the pale white mushroom carefully.");
                if (!player->test_bit(GQ_DOMAIN, GQ_GROUP, GQ_BIT)
                    && !player->query_prop(CALIA_I_TALK_WANDERER))
                {
                    command("say Yes, this is exactly what I'm looking for!"+
                            " You've done good, so I'll let you in on a" +
                            " secret.");
                    command("whisper " + OB_NAME(player) + " The incantation" +
                            " you have to pronounce is known as klifsims. I'll" +
                            " finally be able to enter the grotto, and so can" +
                            " you, if you have another Angeltear! Don't expect" +
                            " me to tell you where it is though.");
                    command("smile wryly");
                    player->add_prop(CALIA_I_TALK_WANDERER, 1);
                    give_back(obj, present("pouch", this_object()), "put", "in", objs[obj]);
                    set_alarm(2.0, 0.0, &leave());
                }
                else
                {
                    command("say Yes, I could always use another one of" +
                            " these. You've helped me before though.");
                    command("put " + OB_NAME(obj) + " in pouch");
                }
            }
        }
    }
