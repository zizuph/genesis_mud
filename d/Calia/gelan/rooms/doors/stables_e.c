...
create_door() {
  ...
  set_door_name(({"wooden door","door"}));      /* name(s) for the door, first
                                                   string is std desc */
  set_door_desc(BS("It is a simple wooden door.\n"));

  /* commands */

  set_other_room(GELAN_ROOMS + "palace_road_ne");       /* room on the other side */
  set_pass_command(({"west","leave","out"}));   /* command(s) to pass the door */
