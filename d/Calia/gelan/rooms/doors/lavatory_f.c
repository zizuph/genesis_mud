...
create_door() {
  ...
  set_door_name(({"pink door marked <women>", "pink door", "women door"}));   
          /* name(s) for the door, first string is std desc */

  /* commands */

  set_pass_command(({ "pink", "women" }));                    
                     /* command(s) to pass the door */
  set_open_mess(({"@@open_mess_here",           /* msg when opening in this room */
                "@@open_mess_there"}));         /* msg in the other room */

  set_other_room(GELAN_ROOMS + "lavatory_female"); /* room on the other side */
  ...
  set_door_desc(BS("The door was recently painted with a strong pink. On "
...
open_mess_here()
{
    ...
    if((GELAN_ROOMS + "lavatory_female")->query_occupied())
    {
