...
create_room() {
    ...
    set_long(break_string("Just your basic non-descript hospital right now " +
        ...
        "as soon as I come up with one that I'm happy with.  Oh, and by the way, there's a sign on the wall.\n", 75));
    ...
    add_exit(GELAN_ROOMS + "road_w", "east", 0);
...
do_remove(string arg) {
    ...
    write(break_string("You hand over you " + text(payarr) + " and the scar on your " + arg + " is removed by the expert team of surgeons here.  You receive " + text(split_values(money_merge(payarr) - 1440)) + " in change.\n", 70));
