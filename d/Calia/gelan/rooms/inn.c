/* this is a room of the town Gelan
   ...
   history:    DD.MM.YY    what done                           who did
               01/09/2010  typos                               Lavellan
               24/08/2009  ?? previous modified date           ?
               16/12/2001  added code for gingerbread quest    Uhclem
               16/4/2000   auto drop glasses when 
                           trying to leave while fighting 
                           someone other than bartender        Maniac
 * $Log: inn.c,v $
 * Revision 1.3  1995/01/07  17:54:42  brambi
 * Removed bug when refilling bottle
 *
 * Revision 1.2  1994/10/21  18:57:41  brambi
 * Totaly renewed with the code of Milan's pub
 *   
               21.09.94    typos removed                   Deinonychus
	       19.12.93    created                         Deinonychus
...
#define BOTTLE    GELAN_OBJECTS + "bottle"
#define GLASS         BOTTLE
#define NUM           sizeof(MONEY_TYPES)
#define CALIA_LIVE_I_SITS "_calia_live_i_sits_"
...
create_gelan_room()
{
  ...
  set_short("Calian Inn");
  set_long(
    "You're in the famous Calian Inn of Gelan. On the oaken floor "
    + "are placed several round tables made of wheels. Each table "
    + "is decorated with a silver candlestick. Along a wall is a "
    + "bar with hundreds of colorful bottles filled with precious "
    + "liquids, and a wooden sign. On another wall you see some "
    + "barrels of beer. There is a menu lying on the counter. The "
    + "smell of tobacco from the Shire lingers in the air and you "
    + "feel like sitting on one of the comfortable wooden chairs at "
    + "the tables and ordering a good drink.\n"
  );

  add_item("floor", "The floor is made of the finest oaken wood.\n");
  add_item(({"table", "tables"}), "");
  add_item(({"bottles","bottle"}), "There are many kinds of bottles "
    + "ready for use.\n");
  add_item(({"barrels","barrel"}), "The huge barrels contain supply "
    + "for the never ending thirst of the citizens and adventurers.\n");
  add_item("candlestick", "");
  add_item(({"chair", "chairs"}), "");
  add_item("menu","An ordinary menu, what did you expect? It is "
  ...
  add_item("counter", "It's made out of wood. You see a menu on it.\n");

  add_exit(GELAN_ROOMS + "square_northend_w", "south", "@@block_exit@@");
...
read_menu()
{
  ...
  return "\nYou can buy:\n\n"+
	...
	"You can also buy whole bottles of drink to take away.\n"+
...
"   medium ............ 176 cc\n"+
"   big ............... 410 cc\n\n"+
"Here are prices for bottles of (excluding price of the bottle):\n"+
"   ale ................   21 /   38 /    86 cc\n"+
...
reset_room()
{
    ...
    if(!bartender)
    {
        bartender = clone_object(GELAN_MONSTERS + "orgulas");
        /*  bartender->arm_me(); */
    ...
    if(!present(bartender,TO))
    {
        ...
        tell_room(environment(bartender), QCTNAME(bartender) +
            " runs back to " + LANG_POSS(bartender) + " pub.\n");
        bartender->move_living("M", TO);
     ...
     *  into this room from December 16 to January 16 each year.
     *  The quest really begins on Dec 17, but we add this object
     *  a day early so that the dialog is certain to be ready when
     *  the first mortal comes fresh from the bakery with questions
     *  about gingerbread.
     */
    ...
    if (((datetext[1] == "Dec" && datetext[2] >= "16") ||
        (datetext[1] == "Jan" && datetext[2] <= "16")) &&
        !present("_gingerbread_inn_object", TO))
    {
        ob = clone_object(GINGERBREAD_INN_OBJECT);
        ob->move(TO);
    }
...
do_return(string arg)
{
  ...
  for(i=0; i<sizeof(tmp); i++)
    if(member_array("_c_i_g_", tmp[i]->query_names()) != -1)
      list += ({ tmp[i] });
...
{
  ::enter_inv(ob, from);
  if(!alarm && (member_array("_c_i_g_",ob->query_names()) != -1))
  {
...
{
  alarm = 0;
  if(bartender && present(bartender,TO) && present("_c_i_g_", TO))
  {
    bartender->command("say Why can't some customers return glasses?");
...
do_buy(string arg)
{
  ...
  if(glass)
  {
    glass->set_name(({"glass", "_c_i_g_"}));  /* calian_inn_glass */
...
/*********************************************************************/
  ...
  {
    bartender->command("oops");
    bartender->command("say I see that you can't carry it... "+
			"I will leave it here for you.");
...
block_exit()
{
  ...
  if (present("_c_i_g_",TP) && bartender && present(bartender,TO))
  {
    if (objectp(victim = TP->query_attack()) && 
        victim != bartender) {
        TP->command("$drop glasses"); 
        return 0; 
    } 
...
do_sit()
{
  if(TP->query_prop(CALIA_LIVE_I_SITS))
  {
    NF("You already sit on one of the comfortable chairs.\n");
    return 0;
  }
  else
  {
    write("You sit down on one of the comfortable chairs.\n");
    say(QCTNAME(TP) + " sits down on one of the comfortable chairs.\n");
    TP->add_prop(CALIA_LIVE_I_SITS, 1);
    TP->add_prop(LIVE_S_EXTRA_SHORT, " sitting on a chair");
    return 1;
  }
}    
...
do_rise()
{
  if(TP->query_prop(CALIA_LIVE_I_SITS))
  {
    write("You rise from your comfortable chair.\n");
//    say(QCTNAME(TP) + " rises from his comfortable chair.\n");
    say(QCTNAME(TP) + " rises from " + TP->query_possessive() +
        " comfortable chair.\n");
    TP->remove_prop(CALIA_LIVE_I_SITS);
    TP->remove_prop(LIVE_S_EXTRA_SHORT);
    return 1;
  }
  else
  {
    NF("You don't sit on one of the comfortable chairs.\n");
    return 0;
  }
}    
 ...
 * Description:   Called when objects leave the room.
 * Arguments:     objects and where it goes to
 * Returns:       ::leave_inv
 */
leave_inv(object ob, object to)
{
  if(ob->query_prop(CALIA_LIVE_I_SITS))
  {
    write("You rise from your comfortable chair.\n");
    ob->remove_prop(CALIA_LIVE_I_SITS);
    ob->remove_prop(LIVE_S_EXTRA_SHORT);
  }
  return ::leave_inv(ob, to);
} 
