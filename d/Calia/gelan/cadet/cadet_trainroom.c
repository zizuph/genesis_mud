...
/* 
   ...
   Copyright (c) Miguel Leith 1997, 1998, 1999, 2000, 2001 

   History: 
   
 12/05/16            fixed a typo in the room description         Jaacar
 27/12/01            added "graduate" command                     Maniac
 28/12/01            fix for unregistering cadet bug              Maniac 
   7/7/01            check for suspended graduate status on join  Maniac 
   3/7/01            graduates can join                           Maniac 
   3/7/01            excellent reputation allows re-joining       Maniac 
   1997/98/99/2001   blah blah blah many changes not documented   Maniac 
*/ 
...
#define GC_KEEP_FILE_TIME 3456000 /* 40 days */ 
#define GELAN_CADET_I_READ_RULES "_gelan_cadet_i_read_rules" 
...
set_up_skills()
{
    ...
    sk_add_train(SS_2H_COMBAT,  ({ me, ot }), 0, 0, 40 );

    me = "fight in the dark"; ot = me;
    sk_add_train(SS_BLIND_COMBAT,  ({ me, ot }), 0, 0, 50 );

    me = "parry"; ot = me;
    sk_add_train(SS_PARRY,      ({ me, ot }), 0, 0, 40 );
...
reset_room()
{
     ...
     {
         if (!SECURITY->exist_player(file_list[i]))
...
enter_inv(object ob, object from)
{
    ...
    { 
        ...
        { 
            ...
            tell_object(ob, "Your graduation ring is restored.\n"); 
 ...
 * Arguments:   n, the name of a graduate 
...
unregister_cadet(mixed cadet)
{
    ...
    if (file_size(GC_CADET_INFO_SAVE(n) + ".o") != -1) 
    { 
        player_info = restore_map(GC_CADET_INFO_SAVE(n)); 
...
{ 
    ...
    set_long("This is the training room of the " + GC_GUILD_NAME + ". " +
      ...
      "low amount of the experience that you gain while you're " +
      "a guild member. To find out more about the guild, read the " +
...
{ 
    ...
    object tmp_obj, board; 
    int i; 

    if (!IS_GRADUATE(player) && 
        (player->gc_level_title() == GC_LEVEL_GRADUATE) && !misconduct) 
        graduate = " (graduated)"; 
    else 
        graduate = ""; 
    ...
    if (objectp(tmp_obj = present(GC_SHOES_ID, player))) 
    ...
    else if (!IS_GRADUATE(player) || 
             is_unworthy(player->query_real_name())) 
    { 
    ...
    log_file("cadet", ctime(time()) + " " + 
             n + " left" + graduate + ".\n"); 
    if (objectp(tmp_obj = present(GC_RING_ID, player)))
        tmp_obj->ensure_graduate_shadow_active(player); 
    return 1; 
}


/* 
 * Function:     convert_to_postgraduate 
 * Description:  Action function where a cadet graduates and 
 *               becomes a postgraduate without having to 
 *               leave. 
 */ 
int
convert_to_postgraduate(string str)
{ 
    object player, ring, board; 
    string n; 

    player = this_player(); 

    if (!IS_GELAN_CADET(player)) 
    { 
        write("You are not a cadet!\n"); 
        return 1; 
    } 

    if (IS_GRADUATE(player) ||
        objectp(present(GC_RING_ID, player))) 
    { 
        write("You have already graduated.\n"); 
        return 1; 
    } 

    if (player->gc_level_title() != GC_LEVEL_GRADUATE) 
    {  
        write("To graduate you must have the title \"" + 
              GC_LEVEL_GRADUATE + "\".\n"); 
        return 1; 
    } 
    ...
    { 
        write("The guildmaster is not here.\n"); 
        return 1; 
    } 

    write("You ask " + guildmaster->query_the_name(player) + 
          " to graduate.\n"); 
    say(QCTNAME(player) + " asks " + QTNAME(guildmaster) + 
        " to graduate.\n"); 

    if (objectp(guildmaster->query_attack())) 
    { 
        guildmaster->command("$say Unfortunately, I'm busy right now!"); 
        return 1; 
    } 

    if (!CAN_SEE_IN_ROOM(guildmaster) || 
        !CAN_SEE(guildmaster, player)) 
    { 
        guildmaster->command("$say I can't see you!"); 
        return 1; 
    } 

    ring = clone_object(GC_DIR + "graduation_ring"); 
    ring->remove_prop(OBJ_M_NO_DROP); 
    ring->move(guildmaster); 

    guildmaster->command("$emote fetches " + LANG_ADDART(ring->short()) + 
                         " from a cabinet."); 
    guildmaster->command("$give " + OB_NAME(ring) + " to " + 
                         OB_NAME(player)); 

    if (!objectp(present(ring, player))) 
    { 
        guildmaster->command("$sigh"); 
        guildmaster->command("$say I can't give the " + ring->short() + 
                             " to you!"); 

        if (objectp(present(ring, guildmaster))) 
        { 
            guildmaster->command("$emote places the " + ring->short() + 
                                 " back in the cabinet for the time " + 
                                 "being."); 
        } 
    ...
    n = player->query_real_name(); 

    ring->configure_properties(); 
    ...
    log_file("cadet", ctime(time()) + " " + 
             n + " graduated and became a postgraduate.\n"); 

    guildmaster->command("$say Congratulations, " + 
                         capitalize(n) + "!"); 
    write("Ok, you've graduated and become a postgraduate!\n"); 
...
update_exemplary_and_unworthy(string name = 0)
{
    ...
    exemplary_vote_target = max(2, sizeof(opiners) / 10); 
    unworthy_vote_target = max(3, sizeof(opiners) / 10); 
...
register_opinion(string str)
{
    ...
    if (!worth[name] && !worthiness[name] && 
        !is_registered_cadet(name) && 
        !is_registered_graduate(name)) 
    { 
        notify_fail("There is no cadet or graduate of that name " +
                    "in the records at the moment.\n"); 
        return 0; 
    } 
...
list_reputations(string str) {
    write("The following people are on balance considered " + 
       "to have excellent reputations in the Cadets of Gelan guild: " + 
       (sizeof(exemplary) ? COMPOSITE_WORDS(map(exemplary, capitalize)) : 
       "none") + ".\nThe following people are on balance considered " + 
       "unworthy to be associated with the Cadets of Gelan guild: " + 
...
csign(string str)
{
    ...
    {
        notify_fail("You can't join while in the middle of " +
                    "a fight!\n"); 
        return 0; 
    } 
    ...
    { 
        ...
        { 
            notify_fail("You can't join while your graduate status is " + 
                        "suspended.\n"); 
            return 0; 
        } 
    } 
 ...
 */ 
    ...
    { 
        if (tp->test_bit(GC_DOM, GC_GROUP, GC_BIT2) && 
            !is_exemplary(tp->query_real_name())) 
        { 
            notify_fail("You may not re-join more than once unless " + 
        ...
        { 
            notify_fail("You will have to <csign again>, but remember " + 
                       "to check <help cadet joining> and " + 
                       "<help cadet rules> first.\n"); 
            return 0; 
        } 
        else 
        { 
            ...
            if (!is_exemplary(tp->query_real_name())) 
            { 
                ...
                { 
                    ...
                    if (m == 0) 
                    { 
                        notify_fail("You didn't have " + num + 
                                    " levels of " + 
                ...
                write("Ok, you lost " + num + 
                      " levels of combat skills.\n"); 
            } 
        } 
    } 

    if (str == "on" && !tp->query_prop(GELAN_CADET_I_READ_RULES)) 
    { 
         write("You may join the Cadets of Gelan by repeating " + 
               "\"csign on\" - HOWEVER you should only " + 
               "join if you've read and accepted the rules described " + 
               "in \"help cadet rules\". By joining you agree to do " + 
               "your best to abide by the rules and not seek to " + 
               "exploit them to antagonise fellow cadets or " + 
               "graduates.\n"); 
         tp->add_prop(GELAN_CADET_I_READ_RULES, 1); 
         return 1; 
    } 

    tp->remove_prop(GELAN_CADET_I_READ_RULES); 
    shadow = clone_object(GC_SHADOW); 
    if (shadow->shadow_me(tp, GC_GUILD_TYPE, 
                              GC_GUILD_STYLE, 
                              GC_GUILD_NAME) != 1) 
    { 
        notify_fail("For some unknown reason you cannot join " + 
                    "the cadets.\n"); 
    ...
    write("You sign on.\n"); 
    write("You just joined the Cadets of Gelan guild!\n"); 
    ...
    guildmaster->command("say Welcome!"); 
    tell_object(tp, "You're advised to read the guild information " + 
        "available via \"help cadet\". There are explanations of " + 
        "various topics about the theme of the guild and the features " + 
        "available to you.\n"); 
...
init()
{
    ...
    add_action(convert_to_postgraduate, "graduate"); 
