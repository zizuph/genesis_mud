...
/* 
    ...
    History: 
    1997       Created                                       Maniac
    1998/1999/2000/2001  Blah blah added new commands but 
                         didn't keep records.                Maniac 
    7/12/01    Added task-related call to notebook for 
               study emotes.                                 Maniac 
 */
...
cborn(string str)
{
    ...
    if (!objectp(check_notebook(this_player(), 1))) 
        return 0;
...
/* 
 * Function:     study_emote 
 * Description:  A common function for the study-related emotes 
 *               that may use a cadet notebook. This writes all 
 *               the relevant messages, and also calls a hook 
 *               in the notebook if used. 
 * Arguments:    string str - the argument of the emote 
 *               string verba1 - first person form of the first verb. 
 *               string verba3 - third person form of the first verb. 
 *               string adv - adverb 
 *               string verbb1 - first person form of the second verb. 
 *               string verbb3 - third person form of the second verb. 
 *               object notebook - notebook if one is used. 
 * Returns:      1 for success, 0 for failure. 
 */ 
int
study_emote(string str, string verba1, string verba3, string adv,  
                        string verbb1, string verbb3, object notebook)
{
    object *oblist; 
    ...
    if (objectp(notebook)) 
        notebook->cadet_just_studied(this_player(), oblist); 
...
cobserve(string str)
{
    ...
    if (objectp(nb = check_notebook(this_player(), 1))) 
    { 
    ...
    return study_emote(how[0], "observe", "observes", 
                       how[1], wr1, wr3, nb); 
}
...
cstudy(string str)
{
    ...
    return study_emote(how[0], "study", "studies", 
                       how[1], wr1, wr3, nb); 
}
...
cscrut(string str)
{
    ...
    return study_emote(how[0], "scrutinize", "scrutinizes", 
                       how[1], wr1, wr3, nb); 
}
...
cchange(string str)
{
    return this_player()->gc_change_hands(str); 
}
