/* this is a room of the town Gelan
   ...
   history:    DD.MM.YY    what done                       who did
               ...
               10/2/2001   chair descs update              Maniac 
 * $Log: inn.c,v $
...
reset_room()
{
  if(bartender)
  {
    ...
    tell_room(environment(bartender), QCTNAME(bartender)+" runs back to "+LANG_POSS(bartender)+
                           " pub.\n");
...
leave_inv(object ob, object to)
{
  if(ob->query_prop(CALIA_LIVE_I_SITS))
  {
    ...
    /* next message would not get through before the player 
       had already left.. maybe solve it at some point */ 
    /* 
    tell_room(this_object(), QCTNAME(TP) + " rises from " + 
        TP->query_possessive() + " comfortable chair.\n", TP); 
    */ 
