/*
   ...
   Stous (air elemental summon) spell for the Worshippers.
...
{
    object *gems = filter(deep_inventory(caster), &operator(==)("aquamarine") @ 
        &->query_gem_name());
    
    if (!sizeof(gems))
    {
        caster->catch_tell("You need an aquamarine in order to successfully cast" +
            " this spell!\n");
        return ({ });
    }
...
{
    set_spell_name("stous");
    set_spell_desc("Summon an air elemental");
    ...
    set_spell_object(WOR_OBJECTS + "air_summ.c");
...
{
    caster->catch_tell("You begin to murmur the Stous incantation," +
        " calling out to Lady Aeria to send you one of her servants.\n");
...
resolve_spell(object caster, object *targets, int *resist, int result)
{
    ...
    else
    {
        ...
        if (random(100) >= 90)
        {
            caster->catch_tell("You sacrificed an aquamarine.\n");
        ...
        tell_room(environment(caster), 
            ...
            +"a swirling zephyr.\n"
            + QCNAME(elemental) + " forms from the winds.\n");
