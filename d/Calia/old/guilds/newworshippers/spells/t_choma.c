/*
   ...
   Choma (earth elemental summon) spell for the Worshippers.
...
{
    object *gems = filter(deep_inventory(caster), &operator(==)("beryl") @ 
        &->query_gem_name());
    
    if (!sizeof(gems))
    {
        caster->catch_tell("You need a yellow beryl in order to successfully" +
            " cast this spell!\n");
        return ({ });
    }
...
config_spell(object caster, object *targets, string argument)
{
    ...
    set_spell_desc("Summon an earth elemental");
    ...
    set_spell_object(WOR_OBJECTS + "earth_summ.c");
    set_spell_ingredients(); //Handling component elsewhere.
...
{
    caster->catch_tell("You begin to murmur the Choma incantation, calling" +
        " out to Lady Gu to send you one of her servants.\n");
...
resolve_spell(object caster, object *targets, int *resist, int result)
{
    ...
    else if (environment(caster)->query_prop(ROOM_I_TYPE) == ROOM_IN_AIR)
    {
    ...
    else
    {
        ...
        if (random(100) >= 90)
        {
            caster->catch_tell("You sacrificed a yellow beryl.\n");
            comp->set_heap_size(comp->num_heap() - 1);
        }
            
        tell_room(environment(caster), "The earth shakes and groans, and" +
            " suddenly it parts as "+ QNAME(elemental) + 
            " forms from the clay.\n");
