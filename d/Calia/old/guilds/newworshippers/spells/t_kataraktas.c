/*
   ...
   Kataraktas (water elemental summon) spell for the Worshippers.
...
special_target(object caster, string str)
{
    ...
    if (!sizeof(gems))
    {
        caster->catch_tell("You need an amethyst in order to successfully cast" +
            " this spell!\n");
        return ({ });
    }
...
{
    set_spell_name("kataraktas");
    set_spell_desc("Summon a water elemental");
    set_spell_target(special_target);
    set_spell_time(8);
    set_spell_mana(100);
    set_spell_task(TASK_ROUTINE);
    set_spell_element(SS_ELEMENT_WATER, 35);
    set_spell_form(SS_FORM_CONJURATION, 30);
    set_spell_object(WOR_OBJECTS + "water_summ.c");
...
{
    caster->catch_tell("You begin to murmur the Kataraktas incantation," +
        " calling out to Lord Diabrecho to send you one of his servants.\n");
...
resolve_spell(object caster, object *targets, int *resist, int result)
{
    ...
    else
    {
        ...
        if (random(100) >= 90)
        {
            caster->catch_tell("You sacrificed an amethyst.\n");
            comp->set_heap_size(comp->num_heap() - 1);
        }
            
        tell_room(environment(caster), 
            "A small puddle begins to grow on the ground, gaining volume"  +
            " quickly.\n" + QCNAME(elemental) + " forms from the water.\n");
