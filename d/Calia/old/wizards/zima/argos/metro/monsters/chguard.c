/* Argosian guard in the village of Chorio Hagios
**
**   Coder         Date                Action
** -------------- -------- ----------------------------------------------
...
void create_monster() {
    if (!IS_CLONE) return;
    set_name(({"guard","vigilant","argosian","soldier","human"}));
    set_adj(({"vigilant","Argosian"}));
    set_race_name("human");
    set_short("vigilant Argosian guard");
    set_long(
        "He is a member of the Argosian Guard which was recently dispatched "+
        "to the village for its protection. He keeps a watchful eye on the "+
        "streets and strangers in the village.\n");
    ...
    add_cact("say you'll never make it out of the village alive!");
