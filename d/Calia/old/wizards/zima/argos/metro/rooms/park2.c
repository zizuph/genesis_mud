...
void create_room() {
    ...
    set_long(
        "The sound of birds singing drifts from the branches of the "+
        "dogwoods which inhabit the park, their songs carried on the "+
        "gentle breeze. You admire the craftsmanship of the statue "+
        "rising from the middle of the pool, a monument to the founding "+
        "of the city. The path continues around the long pool to the "+
        "north and south.\n");
   ...
   add_item(({"path","paths"}),
        "It leads north and south between the pool on the east "+
        "and the grove of dogwoods to the west.\n");
   add_item("hedge",
        "It is a well-trimmed hedge of evergreen bushes, walling "+
        "the park on all sides.\n");
 
   add_exit(ROOM_DIR+"park1","north",0);
   add_exit(ROOM_DIR+"park3","south",0);
   reset_room();
}
