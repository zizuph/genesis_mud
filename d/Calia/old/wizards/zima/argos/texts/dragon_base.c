...
int just_cloned = 1; /* Used to avoid double call of do_dragon_fear in both */
...
dragon_fear_player(object who)
{
    ...
    {
        if (!P("Dragon_fear", who) && !(test_friend(who)))
            clone_object(DRAGON_FEAR)->move(who);
    }
}
...
dragon_fear_room()
{
    ...
    for (i = 0; i < sizeof(ob); i++)
    {
        ...
        {
            if(!P("Dragon_fear", ob[i]) && !(test_friend(ob[i])))
                clone_object(DRAGON_FEAR)->move(ob[i]);
        }
    }
}
