...
create_room() {
   ...
   set_long(
      "You are amazed by the smoothness of the circular walls "+
      "of this particular corridor through the temple, a bit of light "+
      "from a window on the inner wall to the north reflecting off the " +
      "floor. The corridor circles around to the northwest and northeast "+
      "while a narrow corridor leads south.\n");
   ...
   add_item("window","It is a small arched window on the northern "+
   ...
   add_exit(ROOM_DIR+"corr25", "northwest", 0);
   add_exit(ROOM_DIR+"corr23", "northeast", 0);
   add_exit(ROOM_DIR+"corr21", "south", 0);
}
