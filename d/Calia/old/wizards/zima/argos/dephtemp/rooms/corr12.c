...
create_room() {
   ...
   set_long(break_string(
      ...
      "chamber to the west, while the corridor continues to the " +
      "northeast and southwest through the dark shrine.\n",70));
 
   add_prop(ROOM_I_INSIDE,1);
 
   add_item("banner", break_string(
      "Made of red silk, the banner is threaded with a circular " +
      "seal bearing a raven holding aloft a grecian lamp between " +
   ...
   add_exit(ROOM_DIR+"corr11",  "northeast", 0);
   add_exit(ROOM_DIR+"corr13",  "southwest", 0);
   add_exit(ROOM_DIR+"mastrnov","west", 0);
   reset_room();
}
