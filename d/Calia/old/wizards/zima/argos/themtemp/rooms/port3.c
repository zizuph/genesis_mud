...
void create_room() {
   ...
   set_long(
      "You are standing on the east end of the portico of the " +
      ...
      "temple to the west where you see an imposing statue.\n");
   ...
   add_item(({"roof","ceiling"}),
      ...
      "ceiling.\n");
   add_item("fresco",
      "Painted with somber colors, the fresco portrays a priestess "+
      "in a long white robe with her hair in one long braid, sitting "+
      "with a group of children with angelic faces intent upon their "+
      "instruction.\n");
   add_item(({"view","city"}),
   ...
   add_item("statue",
      "It is to the west on the portico. You could get a better look "+
      "if you went closer.\n");
 
   /* exits */
   add_exit(ROOM_DIR+"port2","west",0);
   add_exit(METRO_DIR+"rooms/lroad13","steps","@@steps");
}
