...
create_room() 
{
  ...
  add_item(({"notebooks", "books", "shelf", "old books", "ancient books",
             "new books", "old notebooks", "new notebooks", 
             "ancient notebooks"}), 
  ...
  add_exit(ROOM("alchemist"),"east",0);
...
names_long()
{
    ...
    if (tp->query_skill(SS_LANGUAGE) < AQ_LANGUAGES_MIN)
        ...
        "follows: " + 
        (MANIAC_OBJECTS + "glass_flask")->agent_names() +
        ".\n";
}
...
do_search(object searcher, string str)
{
    ...
    if (str == "books" || str == "notebooks") 
        return "But there are so many books on the shelf" +
               ", which type of book to you want to search?\n";
    ...
    if (str == "old books" || str == "old notebooks")  
        if (searcher->query_skill(SS_AWARENESS) < 
            ...
            if (!book)
               ...
               b = clone_object(MANIAC_OBJECTS + "notebook");
               if (b->move(searcher) == 0) {
                   ...
                   return "You spot a relevant book but have " +
                          "so much stuff on you that you can't " +
...
do_get(string str)
{
    ...
    if ((lower_case(str) == "liquids") || (lower_case(str) == "liquid") || 
        (member_array(lower_case(str), 
          (MANIAC_OBJECTS+"glass_flask")->query_agents()) > -1))  {
    ...
    if ((s1 == "flask" || s1 == "glass flask") &&
        ...
        if (!flask) {
            ...
            f = clone_object(MANIAC_OBJECTS + "glass_flask");
            if (f->move(tp) != 0) {
