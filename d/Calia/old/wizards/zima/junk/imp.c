...
// note: monster with stats 50+random(10) with lots of special attacks
//       and other features.
//       gives no corpse after killing turning into statue which
//       becomes alive again after some time.
// bug(s):
// to-do: make it carry away a player and lock it in chains.
//        requires a lot of work. right now basic structure is
//        commented out in function carry_away.
...
object poison;           // Object pointer to cool little object that
                         // we will give to an offender.
object chains;           // Object pointer to the mighty paralyzer
                         // to hold in the air.
int poisons_in_tail;     // We are not producers of infinite poisons.
...
string combat_hook()
{
    ...
    if(poisons_in_tail<3&&!random(8))
    {
        poisons_in_tail++;
        return "emote wraps its tail around its left leg.";
    }
    if(random(3))
       return "";
...
public int special_attack(object victim)
{
	...
	// Poisoning.
        ...
        if(poisons_in_tail&&random(3))
        {
            poisons_in_tail--;
            poison=clone_object(OBJ+"imp_poison");
            poison->move(this_object()->query_attack());
            poison->start_poison();
        }
        return 1;
        break;
...
{
    if(!attacker->query_weapon(aid)||
       attacker->query_weapon(aid)->query_wt()!=W_POLEARM)
    return ({ -1,({"body","tail","left wing","right wing"})[random(4)],
                -1,-1 });
    else
        return ::hit_me(wcpen,dt,attacker,aid);
}
...
do_die(object killer)
{
    ...
    tell_room(environment(TO),capitalize(short())+" steps back to the "+
	      "tunnel wall and turns into stone.\n");
    environment(TO)->add_prop(STATUES_IN_ROOM,environment(TO)->
			      query_prop(STATUES_IN_ROOM)+1);
