...
reset_monster()
{
    ...
    if (sells_kustennebel == 0)
    {
       ...
       if (barkeeper)
	 {
	   ...
	   barkeeper->add_cmd_item(({"kustennebel","schnapps"}),"buy",VBFC_ME("sorry_out"));
	 }
    }
    else
       sells_kustennebel--;
...
{
    command("say Please just ask me for a reward if you have done the job I gave you.");
    command("give " + ob->query_name() + " to " + to->query_real_name());
}
...
create_monster()
{
    ...
    add_ask(({"job","task","work"}), VBFC_ME("offer_job"));
    add_ask(({"reward","for reward"}), VBFC_ME("reward_job"));
    add_ask("help","I'm extremely busy, I have to finish my balancing.\n");
    ...
    if (number_of_job)
    {
        ...
        else if (number_of_job == 3)
        {
            ...
            if (has_malt == "Trelloq (Supervisor)" && permit)
            {
                ...
                {
                    barkeeper->command("shout Kustennebel on sale NOW! Special offer! For a limited time only!");
		    barkeeper->remove_cmd_item("kustennebel");
                    barkeeper->add_drink(({"schnapps","kustennebel"}),
                                     "Kustennebel","",100,40,80,
                                     "Kustennebel","Kustennebel",
                                     "This is another of Palanthas' "
                                     + "specialities: homemade "
                                     + "Kustennebel! And it is on sale...\n", 0);
/* Kustennebel is available (to everyone) only after someone has solved quest 2 and 3!!
 * Therefore it is sold cheaper than the standard price: (80cc, Standard: 170cc)
 */
                }
                sells_kustennebel = 5;
...
reward(object pers, int exp)
{
    ...
    if (DO_LOG_QUESTS == 1)
        write_file(QUEST_LOG + number_of_job, extract(ctime(time()), 4, 15)
              + " " + capitalize(pers->query_real_name()) + "\n");
...
offer_job()
{
  if (TP->query_prop(PLAYER_I_ON_INN_QUEST))
	...
	command("say I need some hop and malt from my garden in Palanthas.");
      else if (has_done_quests(TP) == 1)
	command("say You are supposed to bring me some peat from the peat "
		+  "cutter.");
      else 
	command("say I need an authorization form with the permission to sell highly alcoholic drinks.");
      return "";
    }
  else if (has_done_all_quests(TP))
    {
      command("say I am terribly sorry, but you already helped me a lot.\n"
	      +"Maybe in the future I will need your help again, who knows?");
  ...
  else if (has_done_quests(TP) == 0)
    {
      ...
      command("say Unfortunately, I will be running out of our home made "
	      +  "speciality very soon, so it is your job to bring me some "
	      +  "hop and malt from my garden in Palanthas"
	      +  ", which are the essential ingredients for our Pfuschbeer.");
  ...
  else if (has_done_quests(TP) == 2)
    {
      ...
      command("say I need an authorization form which gives permission to sell highly "
	      +   "alcoholic drinks.");
