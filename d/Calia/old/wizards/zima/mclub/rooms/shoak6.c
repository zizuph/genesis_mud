...
create_room() {
   ...
   set_long(
      "You are standing on the southern side of a steep hill hidden within "+
      "the woods. A large oak grows "+
      "here in solitude, separated from the surrounding forest. A set of "+
      "steps lead north up the hill into an entrance of the shrine, while "+
   ...
   add_item("shrine",
      ...
      "the hill. A set of steps lead up the side of the hill to the north "+
   ...
   add_item("steps",
      ...
      "steps leading up the hill to the north into an entrance of the "+
   ...
   add_exit(ROOM_DIR+"shoak7","west",0);
   add_exit(ROOM_DIR+"shoak5","east",0);
   add_exit(ROOM_DIR+"shvests","steps","@@steps");
}
 
int steps() {
   write("You climb the steps up the hill to the north.\n");
   return 0;
}
