...
reset_room()
{
    ...
    herm = present("hermit", TO);
    if (!objectp(herm))
        {
            herm = clone_object(HERMIT);
            herm->move(TO);
        }
}
...
create_room()
{
   ...
   add_item((({"view", "western view", "west", "sea"})),
      ...
      "extends all the way to the horizon. You see a ship "+
      "way out to sea, with its sails extended. Below "+
      "at the foot of the sand dune, a white sandy beach "+
   ...
   reset_room();
   add_exit(ROAD+"sea_path3", "out", 0,0);
}