...
private static int found_first = 0;
private static int worn = 0;
private static int lang_inc = 0;
...
create_object()
{
    ...
    add_prop(MAGIC_AM_ID_INFO,
		 ...
		 "Your understanding of spoken languages will improve as well.\n", 28 }));
...
do_wear(string str)
{
    ...
    if (worn)
      return 0;
    if (str && (sscanf(str,"%sglasses%s",dummy,dummy) == 2 ||
	sscanf(str,"%sspectacles%s",dummy,dummy) == 2))
      {
	  NF("You are already wearing something on your nose.\n");
	  if (TP->subloc_id("glasses"))
	    return 0;
	  ...
	  lang_inc = random(5) + 3;
	  TP->set_skill_extra(SS_LANGUAGE, TP->query_skill_extra(SS_LANGUAGE) + lang_inc);
	  worn = 1;
	  return 1;
      }
    return 0;
...
show_subloc(string subloc, object me, object for_obj)
{
    if (me->query_prop(TEMP_SUBLOC_SHOW_ONLY_THINGS))
      return "You are wearing a pair of crooked tiny spectacles on your nose.\n";
    
    if (for_obj == me)
      return "You are wearing a pair of crooked tiny spectacles on your nose.\n";
    else
      return C(PRONOUN(me)) + " is wearing a pair of crooked tiny spectacles on " +
	POSSESSIVE(me) + " nose.\n";
...
{
    set_alarm(360.0,0.0,"reset_found_first");
    ::leave_env(from, to);
}
