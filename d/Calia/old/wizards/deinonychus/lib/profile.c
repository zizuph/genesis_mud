 ...
 */
#ifdef  DO_PROFILE
#    define PROFILE(thefunction) DO_PROFILE->profile(thefunction)
#else
#    define PROFILE(thefunction)
#endif
...
#ifdef DO_PROFILE
void
profile(string thefunction)
{
    profile_data[thefunction]++;
}
...
print_profile(int max_values)
{
    ...
    functions = m_indexes(profile_data);
