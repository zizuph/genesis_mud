...
create_room()
{
  ...
  object sign;
  set_short("Trex hall");
  set_long(BS("You are in the trex hall. The hall is very " +
    ...
    "interesting facts.\n"));
  add_exit(ROOM("bat_hall"), "south", 0);
  add_prop(ROOM_I_LIGHT, 0);
  skeleton = clone_object(OBJECT("skeleton"));
  skeleton->write("t-rex");	/* set long for skeleton */
  skeleton->set_no_show_composite(1);
  skeleton->move(TO);
  sign = clone_object(OBJECT("sign"));
  sign->write("t-rex");		/* set writing for sign */
