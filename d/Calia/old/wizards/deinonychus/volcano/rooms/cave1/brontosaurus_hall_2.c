...
create_room()
{
  object skeleton;
  object sign;
  set_short("Brontosaurus hall");
  set_long(BS("You are in the brontosaurus hall. The hall is very " +
    ...
    "interesting facts.\n"));
  add_exit(ROOM("brontosaurus_hall_1"), "east", 0);
  add_exit(ROOM("antechamber"), "northwest", 0);
  add_prop(ROOM_I_LIGHT, 0);
  skeleton = clone_object(OBJECT("skeleton"));
  skeleton->write("brontosaurus_2");	/* set long for skeleton */
  skeleton->set_no_show_composite(1);
  skeleton->move(TO);
  sign = clone_object(OBJECT("sign"));
  sign->write("brontosaurus");		/* set writing for sign */
