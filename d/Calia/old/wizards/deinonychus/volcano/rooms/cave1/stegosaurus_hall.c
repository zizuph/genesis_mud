...
create_room()
{
  object skeleton;
  object sign;
  set_short("Stegosaurus hall");
  set_long(BS("You are in the stegosaurus hall. The hall is very " +
    ...
    "interesting facts.\n"));
  add_exit(ROOM("antechamber"), "west", 0);
  add_prop(ROOM_I_LIGHT, 0);
  skeleton = clone_object(OBJECT("skeleton"));
  skeleton->write("stegosaurus");	/* set long for skeleton */
  skeleton->set_no_show_composite(1);
  skeleton->move(TO);
  sign = clone_object(OBJECT("sign"));
  sign->write("stegosaurus");		/* set writing for sign */
