...
int *ps = ({0,0,0});	/* Point where to start/end the maze */
int *pe = ({0,0,0});	/* Point where to end/start the maze */
int *ep = ({0,0,0});	/* enterpoint of the maze */
mixed *vertex;	/* array holding the connections */
mixed *line;	/* used for drawing. Represents line of vertex */
mixed *reachable;	/* used for determining an enter-point */
...
create_room()
{
  ...
  line = allocate(L);
  reachable = allocate(L);
  for(x = 0; x < L; x++)
  {
    line[x] = allocate(MAX_EXITS + 1);
    reachable[x] = allocate(L);
  }
...
give_reward()
{
  if(player->test_bit(DOMAIN, BITFIELD, BITNUM))
  {
    tell_object(player, BS("A misterious voice whispers to you: " +
      "\"remember the word '" + PASSWORD  + "'.\n"));
    player->add_exp(SMALL_REWARD);
    tell_object(player, "You feel a little more experienced.\n");
    write_file(LOGFILE, player->query_name() + " solved cube-quest again.\n");
  }
  else
  {
    tell_object(player, BS("A misterious voice whispers to you: " +
    ...
    write_file(LOGFILE, player->query_name() + " solved cube-quest.\n");
  }
}
...
enter_inv(object obj, object from)
{
  if(ready && (from != TO))	/* No move inside maze */
    ...
    {
      tell_object(from, capitalize(LANG_THESHORT(obj)) + 
        " falls down to the bottom of the cave.\n");
      tell_room(CAVE2 + "cave2", capitalize(LANG_ASHORT(obj)) + 
        " falls down from the construction above you.\n");
      obj->move(CAVE2 + "cave2");
    }
  else
  {
...
make_path()
{
  ...
  int reachcount;	/* number of connected ground-level rooms */

  init_vertex();	/* init vertex */
  init_reachable();	/* init array of reachable ground level rooms */
  reachcount = 0;	/* init reachcount */
  /* fix start-point */
  x = ps[0] = random(L);
  y = ps[1] = random(L);
  ...
  {
    if((z == 0) && (reachable[x][y] == 0))	/*  new possible enterpoint */
    {
      reachcount++;
      reachable[x][y] = 1;
    }
...
/*
 * Function name: init_vertex
 * Description:   initialize the array of reachble ground level rooms
 * Arguments:	  none
 * Returns:	  none
 */
void init_reachable()
{
  reachable = ({ ({0, 0, 0}), ({0, 0, 0}), ({0, 0, 0}) });
}

/*
 * Function name: set_enter_point
 * Description:	  set an reachable enter point
 * Arguments:	  Number of reachable enter points
 * Returns:	  none
 */
void set_enter_point(int reachcount)
{
  ...
  index = random(reachcount);
  for(x = 0; (x < L) && (index >= 0); x++)	/* loop x-coord */
    for(y = 0; (y < L) && (index >= 0); y++)	/* loop y-coord */
      if(reachable[x][y] == 1)	/* room is reachable, decrement index */
        index--;

  ep[0] = --x;	/* x and y have been incremented once too much */
  ep[1] = --y;
  ep[2] = 0;
}
...
do_pull(string what)
{
  ...
  if(what != "cord")
    ...
    {
      write(BS("The entire construction seems to come alive. " +
