 ...
 *                the rope if it's worn, default settings.
...
long(object for_obj)
{
    ...
    str = "";
    if (tied_to)
        str = " It is tied to " + tied_to_text + ".\n";
...
#ifdef DEINONYCHUS_ROPE
		"/d/Calia/deinonychus/inherits/rope"))
#else
		"/std/rope"))
#endif
	return 1;
...
tie_it(string str)
{
    ...
    if (!sizeof(ob))
    {
	notify_fail("Can't find what object to tie " +
		LANG_THESHORT(this_object()) + " to.\n");
	return 0;
    }
	...
	if (ob[0]->query_prop(ROOM_I_IS) || living(ob[0]))
        ...
        if (item)
            ...
            set_tied_to_text(LANG_THESHORT(ob[0]));

        say(QCTNAME(this_player()) + " ties " + LANG_ASHORT(this_object()) +
	    " to " + tied_to_text + ".\n");
        write("Ok.\n");
...
untie_it(string str)
{
    ...
    if (tied_to->untie_object(this_object()))
    {
   	...
   	say(QCTNAME(this_player()) + " unties " +
