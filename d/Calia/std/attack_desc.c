                         ...
                         string *new_adverbs, int second_ing) 
{
    ...
    string attacker_msg, defender_msg, watcher_msg;
    string qenemy, qattacker;
    int de; 
    ...
    attacker_msg = ("You" + (((sizeof(inner_clause) > 0) && 
    ...
    if (!objectp(attacker))
    {
        send_debug_message("calia_attack", "Attacker in missing.");
        qattacker = "Someone";
    }
    else
    {
        qattacker = QCTNAME(attacker);
    }
    
    if (!objectp(enemy))
    {
        send_debug_message("calia_attack", "Enemy is missing.");
        qenemy = "someone";
    }
    else
    {
    ...
    watcher_msg = 
              ...
              (((sizeof(inner_clause) > 2) && stringp(inner_clause[2])) ? 
               ...
               move_vb[ONL_FORM] + " " + 
               attacker->query_possessive() + " " + weapon_desc + 
               " " + move_vb[VB_PREP] + " " + qenemy + conj + 
               (second_ing ? dam_vb[ING_FORM] : dam_vb[ONL_FORM]) + 
               " " + enemy->query_possessive() + " " + 
               hit_part + da + ".\n");

    return ({ attacker_msg, defender_msg, watcher_msg });
}
