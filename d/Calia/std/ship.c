...
/* Standard code inherited by ships */  
...
create_object()
{
    ...
    location = 0; 
    event_enable = 0;
    add_prop(OBJ_M_NO_GET, "You got to be kidding!\n");
    create_ship();
}
 ...
 */
public varargs int
start_ship(int where = -1)
{
    int time;
    object pier, captain, to_arrive;

    /* This line is needed as ships may go idle now */ 
    if (find_call_out("sail_away_out_on_the_open_sea") != -1)
        return 0;
    ...
    seteuid(getuid()); 
    to_arrive = places_to_go[where]->get_this_object(); 

    if (to_arrive != environment()) { 
        move(to_arrive);
        call_arrive(to_arrive);
    } 
	...
	if (captain)
   	{
            if (!present(captain, environment())) 
...
sail_away_out_on_the_open_sea()
{
	...
	if (places_to_go[location]->query_prop(ROOM_I_TYPE) != ROOM_IN_WATER)
	{
		...
		captain->move_living("the ship", places_to_go[location]);
                ...
                /* 
                 * Make the ship go idle if no-one's waiting at the
                 * start pier. Remains idle at the far pier until someone 
                 * boards it or someone arrives at the start pier.  
                 */ 
                if (idleness_enabled && 
                    stringp(start_pier) && 
                    (places_to_go[location] != start_pier) && 
                    !start_pier->query_players_present()) {  
                    stop_ship(""); 
                    set_board_enabled(1); 
                    captain->command("say I will wait here until " +
                                     "I am called away or get customers."); 
                    return;
                } 
		captain->command("say All aboard!  The " + short() +
...
{
    if (query_captain())
	query_captain()->command("say All board the ship, we are leaving in 20 seconds!");
}
...
board(mixed what)
{
    ...
    this_player()->move_living("boarding the " + short(), query_deck()[0]);

    /* Start the ship in case it was idle and someone has boarded */ 
    start_ship(); 
...
/* 
 * Function name: set_start_pier
 * Description	: Set the path for the pier that the ship starts at
 * Arguments	: pier - the pier path
 */
public void
set_start_pier(string pier)
{
    start_pier = pier; 
}

/* 
 * Function name: disable_idleness
 * Description	: Stop the ship from going idle. 
 */
public void
disable_idleness()
{
...
/* 
 * Function name: enable_idleness
 * Description	: Allow the ship to go idle when appropriate.  
 * Comments: You should make sure you call set_start_pier() and make your
 *           start pier inherit from the standard pier if you want ship 
 *           idleness to work. 
 */
public void
enable_idleness()
