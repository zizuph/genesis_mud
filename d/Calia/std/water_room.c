...
row_exit_desc()
{
    ...
    if (sizeof(row_exits_arg))
        desc += ("You may row: " + COMPOSITE_WORDS(row_exits_arg)+".\n");
...
create_room()
{
    ...
    create_water_room();  /* This function sets up the long, short, and
...
try_row(string str)
{
    ...
    if (i == -1)
    {
	notify_fail("You can't row in that direction.\n");
	return 0;
    }
    ...
    if (sizeof(row_exits_path[i]->query_boats()) >= MAX_BOATS_IN_ROOM) {   
...
reset_room()
{
    // Calling set_searched(0) on every reset guarantees that herbs will
    // regenerate.
    set_searched(0);
}
