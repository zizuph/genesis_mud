...
create_room()
{
    ...
    add_exit(ROOM("light_boardroom"), "north", can_leave);
    add_exit(ROOM("light_rack"), "east", can_leave);
    add_exit(ROOM("light_keyroom"), "southeast", can_leave);
    add_exit(GLYKRON_LIGHT_ROOMS + "private_library", "south", can_leave); 
...
enter_inv(object ob, object from)
{
    ...
    if (interactive(ob)) 
        set_alarmv(0.0, 0.0, "down_check", ({ob})); 
}
...
down_check(object ob) 
{
    ...
    ob->move_living("M", "/d/Calia/palace/rooms/light/guild"); 
    if (ob->query_default_start_location() == 
        "/d/Calia/glykron/palace/rooms/light/guild") { 
        ob->set_default_start_location("/d/Calia/palace/rooms/light/guild"); 
    } 
    tell_object(ob, "This is a recorded message: you just got two " + 
         "room descriptions because you were relocated to " +
         "a parallel universe (identical btw).\n"); 
    seteuid(getuid()); 
