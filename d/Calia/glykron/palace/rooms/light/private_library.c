...
/* 
 * The private library for Calians. 
 * Calians may write books here. The Council must approve books before
 ...
 * Exit to Scribes' Office added by Uhclem 31/12/96.
 * Exit to Scribes' Office deleted by Uhclem when office was moved to
 * Tower of Realms 9/22/98.
 */ 
...
#define PRIVATE_BOOK_DIR (GLYKRON_TEXTS + "private_library/") 

#define BOOK_EXISTS(id) (file_size(PRIVATE_BOOK_DIR+id) >= 0) 

#define EDIT "/obj/edit"
#define CALIAN_S_BOOK_TITLE "_calian_s_book_title"
#define CALIAN_AS_BOOK_INFO "_calian_as_book_info" 
...
create_room()
{
    set_short("Calian private library"); 
    set_long("You stand in the private library of Calia. " +
             "Here you may read books that fellow Calians have " +
    ...
    add_item(({"books", "shelves", "shelf", "bookshelves", "oak shelves"}), 
           ...
           "The Council " +
           "may delete or edit any books, while other Calians may " +
    ...
    add_exit(GLYKRON_LIGHT_ROOMS+"guild", "north");
}
...
done_writing(string message)
{
    ...
    if (message == "")  { 
        tp->remove_prop(CALIAN_S_BOOK_TITLE); 
        return;
    } 
    ...
    if (!stringp(book[0] = tp->query_prop(CALIAN_S_BOOK_TITLE))) { 
...
done_editing(string message)
{
    ...
    if (message == "")  { 
        tp->remove_prop(CALIAN_AS_BOOK_INFO); 
        return;
    } 
    ...
    book = tp->query_prop(CALIAN_AS_BOOK_INFO); 
    tp->remove_prop(CALIAN_AS_BOOK_INFO); 
...
write_book(string str)
{
    ...
    if (!is_calian(tp)) { 
         notify_fail("You are not a Calian, you may not write books here.\n"); 
         return 0;
    } 

    tp->add_prop(CALIAN_S_BOOK_TITLE, str); 
...
edit_book(string str)
{
    ...
    tp->add_prop(CALIAN_AS_BOOK_INFO, ({str, book[0], book[1], AWAITING})); 
...
delete_book(string str)
{
    ...
    if (!is_calian(tp)) { 
         notify_fail("You are not a Calian, you may not delete books here.\n"); 
         return 0;
    } 
