...
#define MAX_SPECIAL_RACK_ITEMS 25
...
static object this, weapon_rack, armour_rack, misc_rack;
...
read_sign()
{
    return
     ...
     "        appraise <item>  - use appraise skill on <items>\n" +
    ...
    if (stringp(str)) {
        if (str == "list") {
            ...
            tp->catch_msg("The following types of item are listed " +
                          ...
                          COMPOSITE_WORDS(sort_array(tmp)) + ".\n"); 
            return 1; 
        } 
        tmp = explode(str, " "); 
        if (sizeof(tmp) > 1) { 
