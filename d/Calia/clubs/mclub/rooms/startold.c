...
#define  TEMP_EXIT "/d/Calia/zima/argos/nwterr/rooms/sfor3"
...
void load_board() {
    ...
    bb=clone_object("/d/Calia/zima/argos/nwterr/rooms/mboard/board");
    bb->move(this_object());
}
...
int start (string where) {
   ...
   rc = this_player()->
      set_default_start_location("/d/Calia/zima/mclub/rooms/start");
   if (rc)
   write("Thou shalt enter this world in this forgotten place now.\n");
   else write("Sorry...there is still a problem.\n");
   return 1;
}
