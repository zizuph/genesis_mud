...
create_room() {
   ...
   set_long(
      "You are standing on the western side of a steep hill hidden within the "+
      "woods, on top of which sits a stone shrine. A large oak grows "+
      "here in solitude, separated from the surrounding forest. A set of "+
      "steps lead east up the hill into an entrance of the shrine, while "+
      "a path leads west toward the gate. Another path has been cleared "+
      "around the base of the hill to the north and south.\n");
   ...
   add_item("shrine",
      ...
      "the hill. A set of steps lead up the side of the hill to the east "+
   ...
   add_item("oak",
      ...
      "high above you providing a cooling shade. Its height and girth tell "+
      "you it has been here since ancient times.\n");
   add_item("steps",
      "Logs have been set into the side of the hill to make a set of "+
      "steps leading up the hill to the east to an entrance of the "+
   ...
   add_item(({"path","paths"}),
      "One leads west toward the gates while another has leads around "+
      "the base to the hill to the north and south.\n");
   add_item(({"gate","gates"}), "The gates are to the west.\n");
 
   add_exit(ROOM_DIR+"shoak10","north",0);
   add_exit(ROOM_DIR+"shoak8", "south",0);
   add_exit(ROOM_DIR+"shvestw","steps","@@steps");
   add_exit(ROOM_DIR+"path4","west",0);
}
 
int steps() {
   write("You climb the steps up the hill to the east.\n");
   return 0;
}
