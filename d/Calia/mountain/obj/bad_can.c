// OBJECT:  watering_can
    ...
    [96-02-21] Created by Uhclem from [C:\CALIA\MOUNTAIN\OBJ\WATERING.DOC].
...
water_something(string str)
    ...
    if(wildmatch("*ground", str))
        ...
        if(((thisroom->query_prop(ROOM_I_TYPE)) == ROOM_IN_WATER) ||
            ...
            write("You water the ground with your watering can.\n");
            say(QCTNAME(tp) + " waters the ground with a watering can.");

            tell_room("Fortunately, this small amount of water " +
            "dissipates quickly, so after only a few seconds there's " +
            "no puddle left.\n");
    ...
    if(living(ob))
        ...
        write("You hold the watering can high overhead and give " +
            QTNAME(ob) + " a nice cold shower!\n");

        write(QCTNAME(tp) + " holds a watering can high " +
