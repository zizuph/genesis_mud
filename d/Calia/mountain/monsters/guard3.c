/*
*	The guard for the tunnels
*	by Amelia
*       History:
*               23/10/95    some minor modifications     Maniac
*                2/10/95    bug corrected                Maniac
*                18/9/95    "says" in default answers    Maniac
*                16/7/95    inherit standard calian guard (Amelia)
*                30/5/95    modification                 Maniac
*                27/5/95    intro function added         Amelia
*                15/5/95    team pushed out as well      Maniac
*                12/5/95    Typo taken out               Maniac
*/
#pragma save_binary
...
move_out(object player)
{
    ...
    player->move_living("M", EAST_EXIT,1);
...
peek_out()
{
	...
	if((next_room = find_object(EAST_EXIT)) != 0)
...
/* else */
	...
	say("The guard whispers something to "+QTNAME(TP)+".\n");
	return "";
}
