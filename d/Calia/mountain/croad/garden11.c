// ROOM:  palace garden 11

    /* Calia Domain

    HISTORY

    [96-02-24] Created by Uhclem from [C:\CALIA\MOUNTAIN\CROAD\GARDEN11.DOC].
...
create_room()
    ...
    string *darks =
        ({"indigo","vermilion","violet","crimson","azure"});

    string *brights =
        ({"ivory","pale yellow","snow white","pink","gold"});

    string *flowers =
        ({"moonblooms","fairy-slippers","orchids","roses"});

    string *bunches =
        ({"bunches","rows","displays","clusters"});

    string *adjects =
        ({"vivid","brilliant","dazzling","spectacular"});
    ...
    set_short("Calia Palace Gardens, east side");
    ...
    add_exit(CROAD + "garden12", "southwest", 0);
    add_exit(CROAD + "garden10", "north", 0);
    ...
    add_item(({"flowers","shrubs","flower","shrub"}),
    ...
    add_item(("willows"),
        "The path through the willows leads back to the palace entrance.\n");
