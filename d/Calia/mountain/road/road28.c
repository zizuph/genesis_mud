/* 
        road leading to pier for Calia-Emerald ship.
...
reset_room()
{
	...
	if (!objectp(sign))
	{
        sign = clone_object(SHIP_DIR+"emerald/sign.c");
		sign->move(TO);
	}
}
...
create_room()
{
	...
	set_long("The docks are crowded now, as usual.  The noise and "+
		...
		"an arrow on it pointing southwest to a pier.\n");
	...
	add_exit(ROAD+"road27", "southeast", 0,0);
        add_exit(SHIP_DIR+"emerald/pier","southwest",0,0);
	reset_room();
}
