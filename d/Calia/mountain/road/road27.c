/* 
        road leading to pier for Calia-Gont ship.
...
reset_room()
{
	...
	if (!objectp(sign))
	{
        sign = clone_object(SHIP_DIR+"gont/sign.c");
		sign->move(TO);
	}
}
...
create_room()
{
	...
	add_exit(ROAD+"road26", "east", 0,0);
        add_exit(SHIP_DIR+"gont/pier","south",0,0);
    // add_exit(ROAD+"road28","northwest",0,1);
	reset_room();
}
