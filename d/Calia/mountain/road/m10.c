...
/* 
   ...
   History:
           ...
           16/1/2000  removed Diabrecho description, it 
                      doesn't fit with obelisk history   Maniac
           18/3/98   added Diabrecho description         Jaacar
...
{
    make_the_room("\n", 1);
    add_exit(ROAD+"m04", "northwest");
    add_exit(ROAD+"m09", "west"); 
    add_exit(ROAD+"m14", "southwest");
    add_exit(ROAD+"m15", "south"); 
    add_exit(ROAD+"m16", "southeast");
...
scop_quest()
{
    if (present("_diabrecho_staff_",TP))
    {
    ...
    else if (TP->query_prop(LIVE_I_INSERTED_DIABRECHO_STAFF))
    {
        scop_quest = "There is a blue staff sticking out of a small hole ";
...
do_insert(string str)
{
    ...
    staff = present("_diabrecho_staff_",TP);
    if (!staff)
        return 0;

    if (parse_command(str, ({}), 
        "[the] [staff] 'blue' [in] [the] [small] [hole]"))
    {
        NF("Insert what coloured staff in the small hole?\n");
        return 0;
    }

    write("You place the blue staff into the hole.\n");
    say(QCTNAME(TP)+" does something to the obelisk.\n");
    staff->remove_object();
    TP->add_prop(LIVE_I_INSERTED_DIABRECHO_STAFF,1);
    return 1;
}
