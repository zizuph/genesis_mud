/* Moorland in the mid-west of Calia.
   ...
   History:
          ...
          16/1/2000  removed Gu description, it 
                     doesn't fit with obelisk history   Maniac
          18/3/98   added Gu description                Jaacar
...
{
    make_the_room("\n", 1);
    add_exit(ROAD+"m19", "northeast");
    add_exit(ROAD+"m18", "north");
    add_exit(ROAD+"m17", "northwest");
    add_exit(ROAD+"m24", "east"); 
    add_exit(ROAD+"m29", "southeast");
...
scop_quest()
{
    ...
    else if (TP->query_prop(LIVE_I_INSERTED_GU_STAFF))
    {
        scop_quest = "There is a brown staff sticking out of a small hole ";
...
do_insert(string str)
{
    ...
    staff = present("_gu_staff_",TP);
    if (!staff)
        return 0;

    if (parse_command(str, ({}), 
        "[the] [staff] 'brown' [in] [the] [small] [hole]"))
    {
        NF("Insert what coloured staff in the small hole?\n");
        return 0;
    }

    write("You place the brown staff into the hole.\n");
    say(QCTNAME(TP)+" does something to the obelisk.\n");
    staff->remove_object();
    TP->add_prop(LIVE_I_INSERTED_GU_STAFF,1);
    return 1;
}
