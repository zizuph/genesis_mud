/*
 * Syndyasmos Water + Earth combination spell the 
...
config_stun_spell()
{
    set_spell_name("enerogis");
    set_spell_desc("Syndyasmos Water + Earth Combination");
    ...
    set_spell_object(ELEMENTALIST_SPELL_OBJS + "syndyasmos_stun_obj_earth");
...
hook_target_stun_succeeded(object caster, object * targets)
{
    ...
    target_message = QCTNAME(caster) + " extends " + 
			  ...
			  partner_ob->query_pronoun()+" does so, a wave of earthly "+
			  "energy shoots "+
 ...
 *              target - the person being stunned
 */
public void
