/*
 * Syndyasmos Spirit + Fire combination spell the 
...
config_blind_spell()
{
    set_spell_name("epyrkaousias");
    set_spell_desc("Syndyasmos Spirit + Fire Combination");
    ...
    set_spell_object(ELEMENTALIST_SPELL_OBJS + "syndyasmos_blind_obj");
...
hook_describe_blind(object caster, mixed * targets, int result)
{
    ...
    target_message = QCTNAME(caster) + " extends " + 
			  ...
			  " and as "+
			  caster->query_pronoun()+" does so, a wave of shadowy energy rushes "+
			  ...
			  partner_ob->query_pronoun()+" does so, a wave of fiery "+
			  "energy shoots "+
