...
//    	spell_target->dec_prop(LIVE_I_SEE_DARK, darkness_sight);

    // Remove see darkness (if racial) 
    if (spell_target && spell_target->is_elemental_racial_cleric())
    	spell_target->dec_prop(LIVE_I_SEE_DARK, darkness_sight);   
//    	spell_target->dec_prop(LIVE_I_SEE_INVIS, g_invis_amount);
