/*
 * Fire Wall Spell for the Elemental Cleric Guild of Calia
 *
...
spell_target_direction(object caster, string str)
{
    ...
    for(int i = 0; i < sizeof(existing_walls); i++)
    {
      ...
      {
            caster->catch_tell("There is already a fire wall blocking "+
            	"that path!\n");
            return ({ });
      }
    }
...
config_light_spell()
{
    set_spell_name("teichos");
    set_spell_desc("Fire wall");

    set_ability_hasted(0); // No movement preventative ability may be hasted.
    set_spell_vocal(1);
    set_spell_visual(1);
    ...
    set_spell_object(ELEMENTALIST_SPELL_OBJS + "teichos_obj");
...
query_spell_can_be_learned(object player)
{
    ...
    if (player->query_stat(SS_LAYMAN) < SPELL_MINIMUM_GSTAT_CLERIC_TEICHOS)
    {
        return 0;
    }
    ...
    if (cooldown) 
    {
    	if (cooldown > time())
    	{
	    	caster->catch_msg("You are not yet ready to use the gift of "+
	    	    "Teichos again for another " + (cooldown - time()) + 
	    	    " seconds.\n");
	    	return 0;
	    }
    }
    
    if (present("_cast_fire_wall_object_", caster))
    {
        caster->catch_msg("You have already blocked an exit using "+
            "teichos. You must dispel the one you have if you wish "+
            "to have another.\n");
        return 0;
    }               	 

    if (present("_EC_fire_wall", environment(caster)) ||
        present("_EC_shadow_wall", environment(caster)))
    {
        caster->catch_msg("There is already an elemental wall in this region, "
        + "preventing you from materializing another.\n");
        return 0;
    }
    return 1;
}
...
concentrate_msg(object caster, mixed * targets, string arg)
{
    ...
    caster_message = "You close your eyes and concentrate on "+
        "the powers granted to you by Lord Pyros.\n";
    ...
    setuid();
    seteuid(getuid());
    caster->add_prop("_fire_wall_cast",direction);
