...
/*
 * Function:    create_steed
 * Description: Create function of the steed
 */
public void
create_steed()
{
    ...
    set_no_trans(0);
    set_fatigue(this_object()->query_prop(LIVE_I_MAX_EAT));
}
...
{    
    ...
    set_allowed_food(({"leftover", "organ", "steak", "fish", 
        "sausage", "meat", "chicken", "fruit"}));
}
...
can_mount(object rider)
{
    ...
    if (rider != summoner)
    {
        say(QCTNAME(TP) + " attempts to mount the " +
            QTNAME(TO) + " but the elemental moves away from " +
            TP->query_objective() + ".\n");
    ...
    if(rider->query_skill(SS_RIDING) < 1)
    {
        rider->catch_msg("You have no idea how to ride an elemental.\n");            
