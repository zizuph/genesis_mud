/*
 * Syndyasmos Spirit + Water combination spell the 
...
config_stun_spell()
{
    set_spell_name("eousiaydato");
    set_spell_desc("Syndyasmos Spirit + Water Combination");
    ...
    set_spell_object(ELEMENTALIST_SPELL_OBJS + "syndyasmos_stun_obj_water");
...
hook_already_has_spell_object(object caster, object target)
{
    if (objectp(caster))
    {
        caster->catch_msg(QCTNAME(target) + " has already been struck by a "+
            " Syndyasmos stun!\n");
    }
}
...
check_valid_action(object caster, mixed * targets, string arg, int execute = 0)
{
    ...
    // Only spirit clerics get this spell
    if (GUILD_MANAGER->query_primary_element(caster) != "life")
    {
        return 0;
    }
    ...
    {
        if (present("_spell_stun_object",target))
        {
...
hook_target_stun_succeeded(object caster, object * targets)
{
    ...
    target_message = QCTNAME(caster) + " extends " + 
			  ...
			  " and as "+
			  caster->query_pronoun()+" does so, a wave of shadowy "+
			  "energy rushes "+
    ...
    watcher_message = QCTNAME(caster) + " extends " + 
			  ...
			  caster->query_pronoun()+" does so, a wave of shadowy energy "+
			  "rushes "+
			  "towards "+partner_ob->query_objective()+".\n"+
