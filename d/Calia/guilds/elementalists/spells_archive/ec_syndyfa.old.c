/*
 * Syndyasmos Fire + Air combination spell the 
...
{
    set_spell_name("aerafotias");
    set_spell_desc("Syndyasmos Fire + Air Combination");
...
concentrate_msg(object caster, mixed * targets, string arg)
{
    ...
    partner_message = caster_name+" holds out "+caster->query_possessive()+
        ...
        "on gathering the power of Lady Aeria.\n";
...
{
    string caster_message, target_message, watcher_message, partner_name;
    string partner_message;
    object partner_ob;    
    int phurt = result[0];
    ...
    {        
        ...
        case 1..10:
            target_message = "A streak of fire impacts directly into your " + 
            	hitloc_desc
                + ".\n";
            watcher_message = "A streak of fire impacts into " + 
            	QTPNAME(target)
                + " " + hitloc_desc + ".\n";
            break;
            
        case 11..25:
            target_message = "You turn your body to try and dodge the streak "+
                "of fire "
        ...
        case 41..65:
            target_message = "A large streak of fire strikes your " + 
            	hitloc_desc + " and "
                + "flames dig into your skin before exploding inside of you, "+
                "causing "
                + "scorching pain.\n";
        ...
        case 66..99:
            ...
            watcher_message = "The impact of the large streak of fire "+
            	"striking "
...
desc_harm_cast(object caster, object * targets)
{
    ...
    else
    {
        ...
        caster_message = "A fireball forms in your hand. A swirl of air "+
        	"forms in "+
            QTPNAME(partner_ob)+" hand. You and "+partner_name+" push "+
            "your hands out "+
            "towards each other causing the fireball and swirl to fly "+
            "at one another, "+
            "colliding in a violent explosion. Streaks of fire from the "+
            "explosion fly "+
            "off in the direction of " + COMPOSITE_LIVE(targets) + 
            ", leaving wisps of "+
            "cloud in "+go_plural2+" wake.\n";
        partner_message = "A swirl of air forms in your hand. A fireball "+
        	"forms in "+
            QTPNAME(caster)+" hand. You and "+caster_name+" push your "+
            "hands out "+
            "towards each other causing the fireball and swirl to fly at "+
            "one another, "+
            "colliding in a violent explosion. Streaks of fire from the "+
            "explosion fly "+
            "off in the direction of " + COMPOSITE_LIVE(targets) + 
        ...
        target_message = "A large fireball forms in "+QTPNAME(caster)+ 
        	" hand. A swirl "+
            "of air forms in "+QTPNAME(partner_ob)+" hand. "+QCTNAME(caster)+
            " and "+
            QTNAME(partner_ob)+" push their hands out towards each other "+
            "causing the "+
            "fireball and swirl to fly at one another, colliding in a "+
            "violent explosion. "+
            "Streaks of fire from the explosion fly right towards you, "+
            "leaving wisps of "+
            "cloud in its wake!\n";

        watcher_message = "A fireball forms in "+QTPNAME(caster)+ " hand. "+
        	"A swirl of air "+
            "forms in "+QTPNAME(partner_ob)+" hand. "+QCTNAME(caster)+
            " and "+QTNAME(partner_ob)+
            " push their hands out towards each other causing the fireball "+
            "and swirl to fly at "+
            "one another, colliding in a violent explosion. Streaks of fire "+
            "from the explosion "+
            "fly right towards " + COMPOSITE_LIVE(targets) + ", leaving "+
            "wisps of cloud in "+
            go_plural2+" wake.\n";
