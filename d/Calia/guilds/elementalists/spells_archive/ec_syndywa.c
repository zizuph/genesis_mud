/*
 * Syndyasmos Water + Air combination spell the 
...
config_stun_spell()
{
    set_spell_name("eneroaeras");
    set_spell_desc("Syndyasmos Water + Air Combination");
...
hook_target_stun_succeeded(object caster, object * targets)
{
    ...
    target_message = QCTNAME(caster) + " extends " + 
			  ...
			  partner_ob->query_pronoun()+" does so, a wave of airy "+
			  "energy shoots "+
