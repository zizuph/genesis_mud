...
config_harm_spell()
{
    set_spell_name("eatmoskoaera");
    set_spell_desc("Syndyasmos Earth + Air Combination");
        ...
        case 0..5:
            ...
            if (armour)
            {
                ...
                for(int i=0; i < sizeof(armour); i++)
                {
		        	...
		        	{
		        		armour[i]->set_condition(armour[i]->query_condition()+
		        			2);
...
desc_harm_cast(object caster, object * targets)
{
    ...
    target_message = QCTNAME(caster) + " extends " + caster->query_possessive()+
			  ...
			  partner_ob->query_pronoun()+" does so, a wave of airy energy "+
			  "shoots "+
