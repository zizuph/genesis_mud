/*
 * Evade Spell for the Elemental Cleric Guild of Calia
 *
...
config_evade_spell()
{
    set_spell_name("apofevgo");
    set_spell_desc("Evade damage");
    ...
    set_spell_combat_aid(APOFEVGO_AID);
    ...
    // Can only cast evade on themself
    set_spell_target(spell_target_caster);

    // Spell effect object
    set_spell_object(ELEMENTALIST_SPELL_OBJS + "apofevgo_obj");
}
...
query_spell_combat_aid_modifier()
{
    ...
    if (caster_is_maintaining_evimero_effects(this_player())) 
    {
        if (objectp(buff = present("_evimero_object_", this_player())))
        {
            ...
            {
                // If a boosted evimero is active, CAID for apofevgo is
                // reduced from 31 to 5
                result = 16.13; 
            }
        }
    }
...
query_spell_can_be_learned(object player)
{
    ...
    if (player->query_stat(SS_LAYMAN) < SPELL_MINIMUM_GSTAT_CLERIC_APOFEVGO)
    {
        return 0;
    }
...
concentrate_msg(object caster, mixed * targets, string arg)
{
    ...
    caster_message = "You close your eyes and concentrate on "+
        "the powers granted to you by Lady Aeria.\n";
...
{
    write("You are already protected by the apofevgo prayer!\n");
}
...
{
    write("You need the proper ingredients to make yourself harder to hit.\n");
}
