...
config_blind_spell()
{
    set_spell_name("perika");
    set_spell_desc("Blind");

    set_spell_combat_aid(PERIKA_AID);
    ...
    set_spell_object(ELEMENTALIST_SPELL_OBJS + "perika_blind_obj");
...
query_spell_can_be_learned(object player)
{
    ...
    if (player->query_stat(SS_LAYMAN) < SPELL_MINIMUM_GSTAT_CLERIC_PERIKA)
    {
        return 0;
    }
    ...
    if (cooldown) 
    {
    	if (cooldown > time())
    	{
	    	caster->catch_msg("You are not yet ready to use the gift of "+
	    	    "Perika again for another " + (cooldown - time()) + 
	    	    " seconds.\n");
	    	return 0;
	    }
    }    
...
{
    write("You do not feel ready to use the prayer of Perika again.\n");
}

public void
hook_already_active(object ability)
{
    write("You are currently using the prayer of Perika!\n");
}
...
concentrate_msg(object caster, mixed * targets, string arg)
{
    ...
    default:
        caster_message = "You clasp your hands and fervently pray to Lord "
            + "Pyros to grant you the gift of "
            + query_spell_name() + ".\n";
