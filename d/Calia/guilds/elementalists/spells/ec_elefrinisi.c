...
#define DEBUG(x)  find_player("arman")->catch_msg("[ec_elefrinisi] " + x + "\n")

#define REDUCED_CAID      (REDUCED_BASE_AID * 5.0)
#define MAX_EVIMERO_CAID  (APOFEVGO_AID + ELEFRINISI_AID + \
 SCHIRMO_AID + UNTERISTI_AID + DYNAMI_AID + EVIMERO_AID - REDUCED_CAID)
#define MAX_ASPIDA_CAID   (BASE_ASPIDA_AID + ELEFRINISI_AID + \
 SCHIRMO_AID + UNTERISTI_AID + DYNAMI_AID + EVIMERO_AID - REDUCED_CAID)
...
config_featherweight_spell()
{
    ...
    set_spell_combat_aid(ELEFRINISI_AID);
...
check_valid_action(object caster, mixed * targets, string arg, int execute = 0)
{
    ...
    if (caster_is_maintaining_aspida_effects(caster)) 
    {
        ...
        {
            if(ftoi(buff->query_spell_combat_aid()) > 
                ftoi(MAX_ASPIDA_CAID - ELEFRINISI_AID))
            {
                write("While you are focusing so heavily on your " +
                    "aspida prayer, you cannot call upon elefrinisi.\n");
                return 0; 
            }
        }
    }
    ...
    if (caster_is_maintaining_evimero_effects(caster)) 
    {
        ...
        {
            if(ftoi(buff->query_spell_combat_aid()) > 
                ftoi(MAX_EVIMERO_CAID - ELEFRINISI_AID))
            {
                write("While you are focusing so heavily on your " +
                    "evimero prayer, you cannot call upon elefrinisi.\n");
                return 0; 
            }
        }
    }

    if (caster_is_maintaining_kato_effects(caster)) 
    {
        write("While you are focusing so heavily on your " +
            "kato prayer, you cannot call upon elefrinisi.\n");
        return 0; 
    }
