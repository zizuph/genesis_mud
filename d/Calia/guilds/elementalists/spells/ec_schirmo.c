 ...
 * Spirit - grants a mana regenerative sheidl
 *
...
#define DEBUG(x)  find_player("arman")->catch_msg("[ec_schirmo] " + x + "\n")
...
query_spell_combat_aid_modifier()
{
    ...
    if (caster_is_maintaining_aspida_effects(this_player())) 
    {
        if (objectp(buff = present("_aspida_object_", this_player())))
        {
            ...
            {
                // If a boosted aspida is active, CAID for schirmo is 5
                return 20.0; 
            }
        }
    }
    ...
    if (caster_is_maintaining_evimero_effects(this_player())) 
    {
        if (objectp(buff = present("_evimero_object_", this_player())))
        {
            ...
            {
                // If a boosted evimero is active, CAID for schirmo is 5
                return 20.0; 
            }
        }
    }
    ...
    {
        // If kato is active, CAID for schirmo is 5
        return 20.0; 
...
//        shield_spell_object = "/d/Genesis/specials/std/spells/obj/shadow_obj";
    ...
    case SS_ELEMENT_LIFE:
        shield_spell_object = ELEMENTALIST_SPELL_OBJS + "spirit_shield_obj";
        break;
...
spell_target_caster_schirmo(object caster, string str)
{
    ...
    if (!element_skill)
    {
        caster->catch_tell("To which Elemental do you wish to pray? "+
        	"Pyros, Aeria, Gu, Diabrecho, or Psuchae?\n");
        return 0;
    }
...
config_spell()
{
    ...
    set_spell_combat_aid(SCHIRMO_AID);
...
query_spell_can_be_learned(object player)
{
    ...
    if (player->query_stat(SS_LAYMAN) < SPELL_MINIMUM_GSTAT_CLERIC_SCHIRMO)
    {
        return 0;
    }
    return 1;
        ...
        if (objectp(buff = present("_aspida_object_", caster)))
        {
            if(ftoi(buff->query_spell_combat_aid()) > ftoi(BASE_ASPIDA_AID))
            {
                write("While maintaining a boosted aspida, your " +
                    "schirmo prayer is diminished in power.\n");
            }
        }
    }
