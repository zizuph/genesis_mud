 ...
 *
 * Revised caid to take into account global changes to how
 * heals/harms work.
 * Arman (AoB) April 2022
 */
...
#define MAX_ALEIPSO_CAID   20.0
#define SECOND_BUFF_CAID   15.0
#define MAIN_BUFF_CAID     30.0
...
config_heal_spell()
{
    ...
    set_spell_combat_aid(MAX_ALEIPSO_CAID);
...
*/
    ...
    float total_combat_aid = MAX_ALEIPSO_CAID;
    float available_combat_aid = MAX_ALEIPSO_CAID;
    float caid_mod;
    ...
    {
        available_combat_aid -= SECOND_BUFF_CAID;
    }
    ...
    if(sizeof(GUILD_MANAGER->get_existing_dynami_objects(caster)) ||
        sizeof(GUILD_MANAGER->get_existing_evimero_objects(caster)))
    {
        if (sizeof(GUILD_MANAGER->query_defensive_spells_active(this_player())) > 1)
        {
            available_combat_aid -= (MAIN_BUFF_CAID - SECOND_BUFF_CAID);
        }
        else
        {
            available_combat_aid -= MAIN_BUFF_CAID;
        }
    }
...
check_valid_action(object caster, mixed * targets, string arg, int execute = 0)
{
    ...
    // spell
    // Restriction removed, Arman April 2022
/*
