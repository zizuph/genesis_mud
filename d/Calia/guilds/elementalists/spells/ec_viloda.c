/*
 * Fireball Attack Spell for the Elemental Cleric Guild of Calia
 *
...
#define MAX_CAID          (BASE_VILODA_AID + ELEFRINISI_AID + \
 SCHIRMO_AID + UNTERISTI_AID + DYNAMI_AID + EVIMERO_AID + PERIKA_AID)

#define DEBUG(x)  find_player("arman")->catch_msg("[ec_viloda] " + x + "\n")
...
config_harm_spell()
{
    set_spell_name("viloda");
    set_spell_desc("Burn your enemies with fireballs");
...
query_spell_combat_aid_modifier()
{
    ...
    float available_combat_aid = BASE_VILODA_AID;
    ...
    {
        // DEBUG("Elefrinisi aid added to viloda.\n");
        available_combat_aid += ELEFRINISI_AID;
    }

    if (!caster_is_maintaining_unteristi_effects(TP)) 
    {
        // DEBUG("unteristi aid added to viloda.\n");
        available_combat_aid += UNTERISTI_AID;
    }

    if (!caster_is_maintaining_dynami_effects(TP)) 
    {
        // DEBUG("dynami aid added to viloda.\n");
        available_combat_aid += DYNAMI_AID;
    }

    if (!caster_is_maintaining_evimero_effects(TP)) 
    {
        // DEBUG("evimero aid added to viloda.\n");
        available_combat_aid += EVIMERO_AID;
    }

    if (!caster_is_maintaining_schirmo_effects(TP)) 
    {
        // DEBUG("schirmo aid added to viloda.\n");
        available_combat_aid += SCHIRMO_AID;
    }

    if (!caster_is_maintaining_perika_effects(TP)) 
    {
        // DEBUG("perika aid added to viloda.\n");
        available_combat_aid += PERIKA_AID;
    }

    // DEBUG("Viloda available_combat_aid: " +ftoi(available_combat_aid));
    // DEBUG("Viloda max potential combat aid: " +ftoi(total_combat_aid));
...
query_spell_can_be_learned(object player)
{
    ...
    if (player->query_stat(SS_LAYMAN) < SPELL_MINIMUM_GSTAT_CLERIC_VILODA)
    {
        return 0;
    }
...
desc_harm_damage(object caster, object target, mixed *result)
{
    ...
    if (phurt < 0)
    {
        ...
        target_message = "A large ball of fire flies towards you with "+
        	"great speed, but at "
            + "the last moment you are able to move aside and dodge it, "
            + "escaping unscathed.\n";
        watcher_message = "A large ball of fire flies towards " + QTNAME(target) + " with "
    ...
    {        
        ...
        {
        case 0:
            target_message = "A small ball of fire strikes you on the "
                + hitloc_desc + " and barely hurts you.\n";
            watcher_message = "A small ball of fire strikes " + QTNAME(target)
                + " on the " + hitloc_desc + " and barely hurts "
                + target->query_objective() + ".\n";
            break;
            
        case 1..10:
            target_message = "A small ball of fire impacts directly "+
            	"into your " + hitloc_desc
                + ".\n";
            watcher_message = "A small ball of fire impacts into " + 
            	QTPNAME(target)
                + " " + hitloc_desc + ".\n";
            break;
        ...
        case 11..25:
            ...
            watcher_message = QCTNAME(target) + " turns " 
                ...
                "the small "
                + "ball of fire hurtling towards " + target->query_objective()
        ...
        case 26..40:
            target_message = "A ball of fire strikes you and breaks upon "
                + "contact into burning sparks, searing you on your "
                + hitloc_desc + " and causing many wounds.\n";
            watcher_message = "A ball of fire strikes " + QTNAME(target) + 
            	" and "
        ...
        case 41..65:
            target_message = "A large ball of fire strikes your " + 
            	hitloc_desc + " and "
                + "flames dig into your skin before exploding inside "+
                "of you, causing "
                + "scorching pain.\n";
            watcher_message = "A large ball of fire strikes " + 
        ...
        case 66..99:
            target_message = "The large ball of fire striking your "
            ...
            watcher_message = "The impact of the large ball of fire striking "
        ...
        default:
            target_message = "A massive fire ball explodes into sight "
                + "and pummels you on your " + hitloc_desc + 
                ", flames digging into "
                + "your body and exploding from within, causing your blood "
                + "to boil out of your body with excruciating pain.\n";
            watcher_message = "A massive fire ball explodes into sight "
...
desc_harm_cast(object caster, object * targets)
{
    ...
    case 0:
        caster_message = "Fire begins to form at your fingertips and "
            + "gradually grows into a fire ball. You direct it towards "
            + QTNAME(target) + ".\n";
        target_message = "Fire forms at " + QPNAME(caster) + " fingertips "
            + "as " + caster->query_pronoun() + " pushes out "
            + caster->query_possessive() + " palm towards you.\n";
        watcher_message = "Fire form at " + QTPNAME(caster) + " fingertips "
            + "as " + caster->query_pronoun() + " pushes out "
    ...
    case 1:
    default:
        caster_message = "Yellow and orange flames form a fire ball in "
            + "the palm of your hand. You push "
            + "it towards " + QTNAME(target) + ".\n";
        target_message = "Yellow and orange flames form a fire ball in "
            + "the palm of " + QTPNAME(caster) + " hand as "
            + caster->query_pronoun() + " pushes it "
            + "towards you.\n";
        watcher_message = "Yellow and orange flames form a fire ball in "
            + "the palm of " + QTPNAME(caster) + " hand as "
            + caster->query_pronoun() + " pushes it "
