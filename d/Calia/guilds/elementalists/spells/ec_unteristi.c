...
public int      query_spell_resistance_amount = 10;
...
config_resistance_spell()
{
    ...
    set_spell_combat_aid(UNTERISTI_AID);
...
query_spell_element_skill()
{
    ...
    object buff;
    int resist_amount = 10;

    if (caster_is_maintaining_aspida_effects(player)) 
    {
        if (objectp(buff = present("_aspida_object_", player)))
        {
            if(ftoi(buff->query_spell_combat_aid()) > ftoi(BASE_ASPIDA_AID))
            {
                // If a boosted aspida is active, CAID for unteristi is 5
                resist_amount = 5; 
                set_spell_combat_aid(REDUCED_BASE_AID);
            }
        }
    }

    if (caster_is_maintaining_evimero_effects(player)) 
    {
        if (objectp(buff = present("_evimero_object_", player)))
        {
            if(ftoi(buff->query_spell_combat_aid()) > ftoi(EVIMERO_AID))
            {
                // If a boosted evimero is active, CAID for unteristi is 5
                resist_amount = 5; 
                set_spell_combat_aid(REDUCED_BASE_AID);
            }
        }
    ...
    if (caster_is_maintaining_kato_effects(player)) 
    {
        // If a boosted kato is active, CAID for unteristi is 5
        resist_amount = 5; 
        set_spell_combat_aid(REDUCED_BASE_AID);
    }
    ...
    case "fire":
        ...
        query_spell_resistance_amount = resist_amount;
...
hook_describe_cast_resistance(object caster, object * targets)
{
        ...
        if (objectp(buff = present("_aspida_object_", caster)))
        {
            if(ftoi(buff->query_spell_combat_aid()) > ftoi(BASE_ASPIDA_AID))
            {
                write("While maintaining a boosted aspida, your " +
                    "unteristi prayer is diminished in power.\n");
            }
        }
    }
    ...
    {
        if (objectp(buff = present("_evimero_object_", caster)))
        {
            if(ftoi(buff->query_spell_combat_aid()) > ftoi(EVIMERO_AID))
            {
