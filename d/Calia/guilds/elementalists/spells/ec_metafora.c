...
hook_describe_teleport_origin_room(object caster, mixed* targets, object origin_room)
{
    ...
    {
        // All team members are targets, but only present ones are moved.
        if (target != caster && environment(target) == origin_room)
        {
        	target->catch_tell("A whirlwind of air suddenly appears from "+
    	    ...
    	    target->catch_tell("As you open your eyes, you find yourself "+
        	    "elsewhere.\n");
        }
    }
...
hook_cannot_teleport_target_not_relaxed(object caster, object target, string arg, int execute)
{
    ...
    {
        caster->catch_msg(QCTNAME(target) +" has too recently been in combat. They have to "
            + "relax first before you can invoke the gift of metafora.\n");
    }
}
...
hook_cannot_teleport_target_too_soon(object caster, object target, string arg, int execute)
{
    if (caster == target)
    {
        caster->catch_tell("You have teleported too recently. You have to "
            + "wait before you can invoke the gift of metafora.\n");
    }
    else 
    {
        caster->catch_msg(QCTNAME(target) + " has teleported too recently. They have to "
            + "wait before you can invoke the gift of metafora.\n");
    }
}
