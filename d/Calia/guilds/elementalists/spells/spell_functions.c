...
#define ELEFRINISI_SPELL_EFFECTS \
    ({ ELEMENTALIST_SPELL_OBJS + "elefrinisi_obj" })

#define UNTERISTI_SPELL_EFFECTS \
    ({ ELEMENTALIST_SPELL_OBJS + "unteristi_obj" })

#define DYNAMI_SPELL_EFFECTS \
    ({ ELEMENTALIST_SPELL_OBJS + "dynami_obj" })

#define EVIMERO_SPELL_EFFECTS \
    ({ ELEMENTALIST_SPELL_OBJS + "evimero_obj" })

#define ASPIDA_SPELL_EFFECTS \
    ({ ELEMENTALIST_SPELL_OBJS + "aspida_obj" })

#define APOFEVGO_SPELL_EFFECTS \
    ({ ELEMENTALIST_SPELL_OBJS + "apofevgo_obj" })

#define THALAMOS_SPELL_EFFECTS \
    ({ ELEMENTALIST_SPELL_OBJS + "thalamos_obj" })

#define KATO_SPELL_EFFECTS \
    ({ ELEMENTALIST_SPELL_OBJS + "kato_sh" })

#define PERIKA_SPELL_EFFECTS \
    ({ ELEMENTALIST_SPELL_OBJS + "perika_blind_obj" })

#define SKIAMORFI_SPELL_EFFECTS \
    ({ ELEMENTALIST_SPELL_OBJS + "skiamorfi_obj" })

#define AFOPLIZO_SPELL_EFFECTS \
    ({ ELEMENTALIST_SPELL_OBJS + "ec_disarm_obj" })
...
public int
caster_is_maintaining_elefrinisi_effects(object caster)
{
    return query_has_maintained_spell_effect(caster, ELEFRINISI_SPELL_EFFECTS);
}

public int
caster_is_maintaining_unteristi_effects(object caster)
{
    return query_has_maintained_spell_effect(caster, UNTERISTI_SPELL_EFFECTS);
}

public int
caster_is_maintaining_dynami_effects(object caster)
{
    return query_has_maintained_spell_effect(caster, DYNAMI_SPELL_EFFECTS);
}

public int
caster_is_maintaining_evimero_effects(object caster)
{
    return query_has_maintained_spell_effect(caster, EVIMERO_SPELL_EFFECTS);
}

public int
caster_is_maintaining_aspida_effects(object caster)
{
    return query_has_maintained_spell_effect(caster, ASPIDA_SPELL_EFFECTS);
}

public int
caster_is_maintaining_apofevgo_effects(object caster)
{
    return query_has_maintained_spell_effect(caster, APOFEVGO_SPELL_EFFECTS);
}

public int
caster_is_maintaining_thalamos_effects(object caster)
{
    return query_has_maintained_spell_effect(caster, THALAMOS_SPELL_EFFECTS);
}

public int
caster_is_maintaining_kato_effects(object caster)
{
    return query_has_maintained_spell_effect(caster, KATO_SPELL_EFFECTS);
}

public int
caster_is_maintaining_perika_effects(object caster)
{
    return query_has_maintained_spell_effect(caster, PERIKA_SPELL_EFFECTS);
}

public int
caster_is_maintaining_skiamorfi_effects(object caster)
{
    return query_has_maintained_spell_effect(caster, SKIAMORFI_SPELL_EFFECTS);
}

public int
caster_is_maintaining_afoplizo_effects(object caster)
{
    return query_has_maintained_spell_effect(caster, AFOPLIZO_SPELL_EFFECTS);
}

public int
caster_is_maintaining_schirmo_effects(object caster)
{
    setuid(); seteuid(getuid());
    object * effects = GUILD_MANAGER->get_existing_schirmo_objects(caster);
    if (sizeof(effects))    
    {   
        return 1;
    }
