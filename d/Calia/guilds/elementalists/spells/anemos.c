/*
 * Tell Spell for Elemental Worshippers of Calia
 * 
...
config_fartell_spell()
{
    ...
    set_spell_desc("Communicate with fellow Elemental servants");
...
is_valid_fartell_target(object caster, object target)
{
    ...
    if (IS_CLERIC(caster))
    {
        // Clerics can send fartells to anyone.
        return 1;
    }
    
    if (IS_WORSHIPPER(caster) || IS_SEEKER(caster))
    {
        // For worshippers and seekers, Guild masters, Calian wizards, and 
        // all members of the Elementals guild are valid targets.
        if (IS_MEMBER(target) || (IS_GUILDMASTER(target)) ||
        	(IS_CALIA_WIZ(target)) || (target->query_guild_name_occ() == "Spirit Circle of Psuchae"))
        {
            return 1;
        }
    }
...
hook_fartell_target_not_valid(object caster, string target)
{
    ...
    if (objectp(target_obj) && !IS_MEMBER(target_obj)
        && !IS_CALIA_WIZ(target_obj) && !IS_GUILDMASTER(target_obj))
    {
        caster->catch_tell("You only know how to use the gift of Anemos to "
            + "send a message to a fellow servant of the Elementals.\n");
    }
    else
    {
        caster->catch_tell("You do not sense " + capitalize(LANG_POSS(target)) 
...
query_spell_can_be_learned(object player)
{
    ...
    if (player->query_stat(SS_LAYMAN) < SPELL_MINIMUM_GSTAT_ANEMOS)
    {
        return 0;
    }
    return 1;
