/*
 * Blink Teammate Spell for the Elemental Cleric Guild of Calia
 *
...
config_blink_spell()
{
    set_spell_name("diasosi");
    set_spell_desc("Blink team member out of harms way");
    ...
    set_spell_object(ELEMENTALIST_SPELL_OBJS + "diasosi_obj");

}

/*
 * Function:    query_blink_interval
 * Description: This is the value that will be passed to the blink spell
object
...
{
    return 10.0; // set default blink interval to 10 seconds.
}
...
query_spell_can_be_learned(object player)
{
    ...
    if (player->query_stat(SS_LAYMAN) < SPELL_MINIMUM_GSTAT_CLERIC_DIASOSI)
    {
        return 0;
    }
...
concentrate_msg(object caster, mixed * targets, string arg)
{
    string caster_message, watcher_message;
        ...
        caster_message = "You concentrate and pray to Lady Gu.\n";
        watcher_message = QCTNAME(caster) + " begins to concentrate.\n";
...
hook_already_has_spell_object(object caster, object target)
{
    if (caster == target)
    {
        write("You are already blinking behind someone.\n");
    }
    else
    {
        caster->catch_msg("You are already maintaining the ability for " 
            + QTNAME(target) + "to blink behind someone.\n");
    }
}
...
{
    return "diasosi component (eye, peridot shard)";
}
