...
config_disarm_spell()
{
    set_spell_name("eaeraaeras");
    set_spell_desc("Syndyasmos Air + Air Combination");
...
{
    string caster_message, caster_name, watcher_message, partner_name, partner_message;
    object partner_ob;
...
hook_describe_disarm(object caster, mixed * targets, int result)
{
    ...
    target_message = QCTNAME(caster) + " extends " + 
		...
		partner_ob->query_pronoun()+" does so, a wave of airy energy shoots "+
