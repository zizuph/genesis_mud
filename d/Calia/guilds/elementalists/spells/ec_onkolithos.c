/*
 * Boulder Attack Spell for the Elemental Cleric Guild of Calia
 *
...
config_harm_spell()
{
    set_spell_name("onkolithos");
    set_spell_desc("Attack your enemies with boulders");
...
concentrate_msg(object caster, mixed * targets, string arg)
{
    ...
    default:
        caster_message = "You summon your inner reserves of power and "
            + "focus your mind on utilizing the element of earth.\n";
...
desc_harm_damage(object caster, object target, mixed *result)
{
    ...
    if (phurt < 0)
    {
        ...
        target_message = "A large ball of earth flies towards you with "+
        	"great speed, but at "
            + "the last moment you are able to move aside and dodge it, "
            + "escaping unscathed.\n";
        watcher_message = "A large ball of earth flies towards " +
            QTNAME(target) + " with "
    ...
    {        
        ...
        {
        case 0:
            target_message = "A small ball of earth strikes you on the "
                + hitloc_desc + " and barely hurts you.\n";
            watcher_message = "A small ball of earth strikes " + QTNAME(target)
                + " on the " + hitloc_desc + " and barely hurts "
                + target->query_objective() + ".\n";
            break;
            
        case 1..10:
            target_message = "A small ball of earth impacts directly into "+
            	"your " + hitloc_desc
                + ".\n";
            watcher_message = "A small ball of earth impacts into " +
                QTPNAME(target)
                + " " + hitloc_desc + ".\n";
            break;
            
        case 11..25:
            target_message = "A small ball of earth impacts your " +
            	hitloc_desc + " heavily.\n";
            watcher_message = "A small ball of earth impacts "+
            	QTPNAME(target) + " "+
                hitloc_desc + " heavily.\n";
            break;
            
        case 26..40:
            target_message = "A solid ball of earth pounds into your "
                + hitloc_desc + " with a resounding crack, causing a "+
                "severe bruising "+
                "pain.\n";
            watcher_message = "A solid ball of earth pounds into " + 
            	QTPNAME(target) + " "+
                hitloc_desc + " with a resounding crack, causing severe "+
                "bruising pain.\n";
            break;
            
        case 41..65:
            target_message = "A massive ball of earth and grit crashes "+
            	"loudly into your " +
                hitloc_desc + " and you feel something crunching to pieces "+
                "within you.\n";
            watcher_message = "A massive ball of earth and grit crashes "+
            	"loudly into " +
        ...
        case 66..99:
            target_message = "A large earthen boulder hurls towards your "+
                hitloc_desc + " and strikes it so hard that you almost lose "+
                "your footing. Covered in wounds and wracked with pain, you "+
                "struggle to regain your bearings.\n";
            watcher_message = "A large earthen boulder hurls towards "
                + QTPNAME(target) + " "+hitloc_desc+" and strikes "+
                target->query_objective()+
                " so hard that "+target->query_pronoun()+" almost loses "+
                target->query_possessive()+" footing. Covered in wounds "+
                "and wracked with "+
                "pain, "+QTNAME(target)+" struggles to regain "+
        ...
        default:
            target_message = "A roar alerts you to a massive boulder rushing "+
                "towards you with impossible speed. Before you are able to "+
                "react, "+
                "the boulder impacts your "+hitloc_desc+" and hammers you "+
                "down onto "+
                "the ground with all the force of a crashing mountain. Your "+
                "entire "+
                "body is wracked with pain as the boulder hammers you to "+
                "death in an "+
                "instant crushing moment of pain.\n";
            watcher_message = "A roar alerts you to a massive boulder "+
            	"rushing "+
                "towards "+QTNAME(target)+" with impossible speed. Before "+
                target->query_pronoun()+" is able to react, the boulder "+
                "impacts "+
                QTPNAME(target)+" "+hitloc_desc+" and hammers "+
                target->query_objective()+
                " down onto the ground with all the force of a crashing "+
                "mountain. "+
                capitalize(target->query_possessive())+" entire body is "+
                "wracked with "+
                "pain as the boulder hammers "+target->query_objective()+
                " to death in an "+
                "instant crushing moment of pain.\n";
            break;
        }
...
desc_harm_cast(object caster, object * targets)
{
    ...
    {
    case 0:
        caster_message = "Earth begins to form at your fingertips and "
            + "gradually grows into a ball. You direct it towards "
            + QTNAME(target) + ".\n";
        target_message = "Earth forms at " + QPNAME(caster) + " fingertips "
            + "as " + caster->query_pronoun() + " pushes out "
            + caster->query_possessive() + " palm towards you.\n";
        watcher_message = "Earth form at " + QTPNAME(caster) + " fingertips "
            + "as " + caster->query_pronoun() + " pushes out "
    ...
    case 1:
    default:
        caster_message = "Swirls of sand and grit become a solid form in "+
            "the palm of your hand. You push "
            + "it towards " + QTNAME(target) + ".\n";
        target_message = "Swirls of sand and grit become a solid form in "
            + "the palm of " + QTPNAME(caster) + " hand as "
            + caster->query_pronoun() + " pushes it "
            + "towards you.\n";
        watcher_message = "Swirls of sand and grit become a solid form in "
            + "the palm of " + QTPNAME(caster) + " hand as "
            + caster->query_pronoun() + " pushes it "
...
{
    return "earth component (cladina, peridot shard)";
}
