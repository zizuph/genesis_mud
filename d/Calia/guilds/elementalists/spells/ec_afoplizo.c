/*
 * Disarm Spell for the Elemental Cleric Guild of Calia
 *
...
config_disarm_spell()
{
    set_spell_name("afoplizo");
    set_spell_desc("Disarm");
    ...
    set_spell_combat_aid(AFOPLIZO_AID);
...
query_spell_combat_aid_modifier()
{
    ...
    if (caster_is_maintaining_evimero_effects(this_player())) 
    {
        if (objectp(buff = present("_evimero_object_", this_player())))
        {
            ...
            {
                // If a boosted evimero is active, CAID for afoplizo is
...
query_spell_can_be_learned(object player)
{
    ...
    if (player->query_stat(SS_LAYMAN) < SPELL_MINIMUM_GSTAT_CLERIC_AFOPLIZO)
    {
        return 0;
    }
...
concentrate_msg(object caster, mixed * targets, string arg)
{
    string caster_message, watcher_message;
        ...
        caster_message = "You pray to Lady Aeria and concentrate on "+
            "your target.\n";
        watcher_message = QCTNAME(caster)+" closes "+caster->query_possessive()+ 
           " eyes and whispers an ancient prayer.\n";
