/*
 * Neutralize Poison (Cleanse) Spell for the Elemental Clerics of Calia
 *
...
config_cleanse_spell()
{
    ...
    set_spell_desc("Neutralize poisons");	
...
query_spell_can_be_learned(object player)
{
    ...
    if (player->query_stat(SS_LAYMAN) < SPELL_MINIMUM_GSTAT_CLERIC_GIATREOU)
    {
        return 0;
    }
...
hook_all_poisons_cleansed(object caster, object target)
{
	if (caster)
	{
	    ...
	    if (caster != target)
	    	target->catch_msg(QCTNAME(caster) + " cures all of your "+
	    	    "poisons.\n");
	}
}
...
hook_some_poisons_cleansed(object caster, object target)
{
	if (caster)
	{
	    caster->catch_tell("You manage to cleanse some poisons, but still "
	        + "some remain.\n");
	    if (caster != target)
	    	target->catch_msg(QCTNAME(caster) + " cures some of your "+
	    	    "poisons, but "
	        	+ "some still remain.\n");
	}
}
...
{
    return "giatreou component (chicory, sapphire shard)";
}
