...
// Max possible boosted aid for Water branch is 135
#define MAX_CAID          (WATER_ALEIPSO_AID + ELEFRINISI_AID + \
 SCHIRMO_AID + UNTERISTI_AID + DYNAMI_AID + EVIMERO_AID)

#define DEBUG(x)  find_player("arman")->catch_msg("[ec_aleipso] " + x + "\n")
...
query_spell_combat_aid_modifier()
{
    ...
    float available_combat_aid = 0.0;
    ...
    // Are they a child cleric? If so, it's 20 CAID.
    if (GUILD_MANAGER->query_is_child_cleric(caster))
    {
        available_combat_aid += CHILD_ALEIPSO_AID;
    	if (TP->query_prop(CONDUIT_PENALTY) == 3)
    	{
    		result = (available_combat_aid / total_combat_aid) * 50.0;
    		actual_caid = 10.0;
    	}
    	else
    	{
    ...
    switch(primary_element)
    {
        ...
        case "fire":
        available_combat_aid += FIRE_ALEIPSO_AID;
        // Has guru quest been completed? If so, set at 20, otherwise 15
        ...
        if (TP->query_prop(CONDUIT_PENALTY) == 3)
        {
        	result = (available_combat_aid / total_combat_aid) * 37.5;
        	actual_caid = 7.5;
        }
        else
        {
        	result = (available_combat_aid / total_combat_aid) * 75.0;
        	actual_caid = 15.0;
        }
        branch = "fire";
        return result;
    ...
    case "earth":
        available_combat_aid += EARTH_ALEIPSO_AID;
        ...
        if (TP->query_prop(CONDUIT_PENALTY) == 3)
        {
        	result = (available_combat_aid / total_combat_aid) * 33.4;
        	actual_caid = 10.0;
        }
        else
        {
        	result = (available_combat_aid / total_combat_aid) * 66.7;
        	actual_caid = 20.0;
        }
        branch = "earth";
        return result;
    ...
    case "water":
        available_combat_aid += WATER_ALEIPSO_AID;

        // Water Branch aleipso has dynamic caid
        ...
        DEBUG("Water aleipso available_combat_aid: " +ftoi(available_combat_aid));
        DEBUG("Water aleipso max potential combat aid: " +ftoi(total_combat_aid));
    ...
    case "air":
        available_combat_aid += AIR_ALEIPSO_AID;
        // Has guru quest been completed? If so, set at 30, otherwise 20
        if (TP->test_bit("Calia",EC_GURU_GROUP,EC_GURU_BIT))
        {
            if (TP->query_prop(CONDUIT_PENALTY) == 3)
    ...
    case "life":
        available_combat_aid += SPIRIT_ALEIPSO_AID;
        // Has guru quest been completed? If so, set at 60, otherwise 40
        if (TP->test_bit("Calia",EC_GURU_GROUP,EC_GURU_BIT))
        {
            if (TP->query_prop(CONDUIT_PENALTY) == 3)
