/*
 * Syndyasmos Water + Spirit combination spell the 
...
public object * spell_target_team_and_enemies(object actor, string str);
...
config_heal_spell()
{
    set_spell_name("eneroousia");
    set_spell_desc("Syndyasmos Water + Spirit Combination");
    ...
    set_spell_target(spell_target_team_and_enemies);
...
public object *
spell_target_team_and_enemies(object actor, string str)
{
	object * team = target_present_team(actor, str);
	object * enemies = target_present_enemies(actor, str);
	object * combined = team + enemies + ({ actor });
...
concentrate_msg(object caster, mixed * targets, string arg)
{
    ...
    partner_message = caster_name+" holds out "+caster->query_possessive()+
        ...
        "on gathering the power of Psuchae.\n";
...
public void
hook_describe_heal(object caster, mixed * targets, int result)
{
    ...
    caster_message= "Your prayers being heard, you extend " +
		      ...
		      "do so, a wave of watery energy rushes towards "+
		      ...
		      " does so, a wave of shadowy energy rushes towards you.\n"+
    ...
    target_message = QCTNAME(caster) + " extends " + 
			  ...
			  " hands towards "+partner_ob->query_the_name(targets)+
			  " and as "+
			  caster->query_pronoun()+" does so, a wave of watery "+
			  "energy rushes "+
			  ...
			  partner_ob->query_pronoun()+" does so, a wave of shadowy "+
			  "energy shoots "+
...
hook_describe_heal_result(object caster, object *targets, int * heal_amounts)
{
    for (int index = 0; index < sizeof(targets); ++index)
    {
        ...
        case 0:
            ...
            target->catch_tell("The healing energies of Lord Diabrecho "
...
/*
 * Function:    hook_no_heal_message
 * Description: When it is not possible to heal, for whatever reason,
 *              this message is given.
 * Arguments:   caster - person casting the spell
 *              target - the person being healed
 */
public void
hook_no_heal_message(object caster, object target)
{
