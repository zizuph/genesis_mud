/*
 * Syndyasmos Water + Water combination spell the 
...
public object * spell_target_entire_present_team(object actor, string str);
...
config_heal_spell()
{
    set_spell_name("eneronerous");
    set_spell_desc("Syndyasmos Water + Water Combination");
    ...
    set_spell_target(spell_target_entire_present_team);
...
public object *
spell_target_entire_present_team(object actor, string str)
{
	object * combined = target_present_team(actor, str) + ({ actor });	
...
hook_describe_heal_cast(object caster, object *targets)
{
    ...
    target_message = QCTNAME(caster) + " extends " + 
			  ...
			  " and as "+
			  caster->query_pronoun()+" does so, a wave of watery energy "+
			  "rushes "+
			  ...
			  partner_ob->query_pronoun()+" does so, a wave of watery "+
			  "energy shoots "+
