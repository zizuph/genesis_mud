...
is_follower_of_elemental(object player, string elemental)
{
    ...
    {
    case "racial":
    	return (player->query_guild_name_race() == "Elemental Clerics of Calia");
    	break;
    	
    case "cleric":
    	return (player->query_guild_name_occ() == "Elemental Clerics of Calia"
    	    && EW_GUILD_MANAGER->query_is_child_cleric(player));
        break;
    
    case "mcleric":
    	return (player->query_guild_name_occ() == "Elemental Clerics of Calia"
    	    && EW_GUILD_MANAGER->query_is_master_cleric(player)
    	    && EW_GUILD_MANAGER->query_primary_element(player) == 0);
    	break;
    ...
    case "cpyros":
    	if (player->query_guild_name_occ() == "Elemental Clerics of Calia"
    ...
    case "worshipper":
    	if (player->query_guild_name_lay() == "Elemental Worshippers of Calia"
            && (player->query_skill(SS_PATRON_ELEMENTAL) == 0
            && EW_GUILD_MANAGER->query_primary_element(player) == 0))
        {
        	return 1;
        }
...
format_members_presentation(object * members, string elemental)
{
    ...
    foreach (object member : members)
    {
        ...
        if (elemental == "psuchae" && is_follower_of_elemental(this_player(),
        	"psuchae") && (this_player()->query_guild_name_occ() == 
        ...
        {                              
            if (COUNCIL_CODE->is_liberator(name))
                ...
                title += " (Intercessor of the Elementalists)";
...
list_members()
{
    ...
    object * clerics = filter(awake_players, &is_follower_of_elemental(,"cleric"));
    object * mclerics = filter(awake_players, &is_follower_of_elemental(,"mcleric"));
    object * psuchae = filter(awake_players, &is_follower_of_elemental(,"psuchae"));
    object * fclerics = filter(awake_players, &is_follower_of_elemental(,"cpyros"));
    object * aclerics = filter(awake_players, &is_follower_of_elemental(,"caeria"));
    object * wclerics = filter(awake_players, &is_follower_of_elemental(,"cdiabrecho"));
    object * eclerics = filter(awake_players, &is_follower_of_elemental(,"cgu"));
    object * sclerics = filter(awake_players, &is_follower_of_elemental(,"cpsuchae"));
    ...
    object * worshippers = filter(awake_players, &is_follower_of_elemental(,"worshipper"));
    object * seekers = filter(awake_players, &is_follower_of_elemental(,"seeker"));
    
    string cleric_members = format_members_presentation(clerics, "cleric");
    if (strlen(cleric_members))
    {
        write("Child Clerics:\n");
        write(cleric_members + "\n");
    }
    
    string mcleric_members = format_members_presentation(mclerics, "mcleric");
    if (strlen(mcleric_members))
    {
    	write("Master Clerics:\n");
    	write(mcleric_members + "\n");
    }
    ...
    string cpyros_members = format_members_presentation(fclerics, "cpyros");
    if (strlen(cpyros_members))
    {
        write("Clerics of Pyros:\n");
        write(cpyros_members + "\n");
    }
    
    string cdiabrecho_members = format_members_presentation(wclerics, "cdiabrecho");
    if (strlen(cdiabrecho_members))
    {
        write("Clerics of Diabrecho:\n");
        write(cdiabrecho_members + "\n");
    }
    
    string cgu_members = format_members_presentation(eclerics, "cgu");
    if (strlen(cgu_members))
    {
        write("Clerics of Gu:\n");
        write(cgu_members + "\n");
    }
    
    string caeria_members = format_members_presentation(aclerics, "caeria");
    if (strlen(caeria_members))
    {
        write("Clerics of Aeria:\n");
        write(caeria_members + "\n");
    }
    
    string cpsuchae_members = format_members_presentation(sclerics, "cpsuchae");
    if (strlen(cpsuchae_members))
    {
        write("Clerics of Psuchae:\n");
        write(cpsuchae_members + "\n");
    }
    ...
    string worshipper_members = format_members_presentation(worshippers, "worshipper");
    if (strlen(worshipper_members))
    {
        write("Elemental Worshippers:\n");
        write(worshipper_members + "\n");
    }
    ...
    object * ld_members = filter(ld_players, &operator(==)("Elemental Clerics of Calia") @ &->query_guild_name_occ()) 
                        + filter(ld_players, &operator(==)("Spirit Circle of Psuchae") @ &->query_guild_name_occ())
...
/*
 * Function:    list_racials
 * Description: The actual function called by the soul of the racial guild
 *              to execute the who command.
 */
public int
list_racials()
{
    object * awake_players = users();
    write("These Kyrullian servants of the Elementals currently walk the "+
        "Realms:\n\n");
    object * racials = filter(awake_players, &is_follower_of_elemental(,"racial"));
    
    
    string racial_members = format_members_presentation(racials, "racial");
    if (strlen(racial_members))
    {
        write(racial_members + "\n");
    }
    
    
    object * ld_players = filter(all_inventory(find_object("/d/Genesis/obj/statue")), living);
    object * ld_members = filter(ld_players, &operator(==)("Elemental Clerics of Calia") @ &->query_guild_name_race());
    ld_members = filter(ld_members, not @ &->query_wiz_level());
    if (!sizeof(ld_members))
    {
        return 1;
    ...
    write("\nThese Kyrullian servants of the Five are currently within the "+
        "Void:\n");
    write(format_members_presentation(ld_members, "") + "\n");
    return 1;
}
