...
#define WEDDING_LOG "/d/Calia/guilds/elementalists/logs/wedding_log"

#define DID_EBRIDE "_did_elemental_wedding_specify_bride"
#define DID_EGROOM "_did_elemental_wedding_specify_groom"
#define DID_PREP   "_did_elemental_wedding_preparation"
#define DID_BEGIN  "_did_elemental_wedding_begin"
#define DID_GWISH  "_did_elemental_wedding_gwish"
#define DID_BWISH  "_did_elemental_wedding_bwish"
#define DID_MARRY  "_did_elemental_wedding_marry"

#define PARALYSIS_OBJ "/d/Calia/guilds/elementalists/objs/paralyze"
...
void create_scroll()
{
    ...
    set_file(ELEMENTALIST_OBJS+"wedtome.txt");
}
...
do_prepare()
{
    ...
    if (!TP->query_prop(DID_EBRIDE))
    {
        NF("You have not specified the Bride's name.\n");
        return 0;
    }

    if (!TP->query_prop(DID_EGROOM))
    {
    ...
    setuid();
    seteuid(getuid(TO));
    TP->add_prop(DID_PREP,1);
    log_file(WEDDING_LOG, ctime(time())+" Preparation for wedding by "+ 
        TP->query_name() +".\n",-1);
    return 1;
}
...
do_begin()
{
    if (!TP->query_prop(DID_PREP))
    {
        NF("You have not prepared yet.\n");
        return 0;
    }
...
do_marry(string str)
{
    ...
    setuid();
    seteuid(getuid(MARRIAGE_OBJ));
    log_file(WEDDING_LOG, capitalize(TP->query_real_name()) +
        " did the wedding in the Elemental garden pavilion in Calia.\n");

    write("The wedding is completed. You should 'complete' now.\n");
...
do_complete()
{
    setuid();
    seteuid(getuid(TO));
    TP->remove_prop(DID_EBRIDE);
    TP->remove_prop(DID_EGROOM);
    TP->remove_prop(DID_PREP);
    TP->remove_prop(DID_BEGIN);
    TP->remove_prop(DID_GWISH);
    TP->remove_prop(DID_BWISH);
    TP->remove_prop(DID_MARRY);
    log_file(WEDDING_LOG, ctime(time())+" Completion done for wedding by "+ 
        TP->query_name() +".\n",-1);
    return 1;
}
