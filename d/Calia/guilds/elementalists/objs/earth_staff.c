/*
   This is the staff of earth, a magical quarterstaff that's found in Calia.
   It is mediocre for someone with no skill in earth magic, but for someone
...
adj()
{
    ...
    if (!objectp(ob) || ob->query_skill(SS_ELEMENT_EARTH) < 30)
    {
        return "painted";
    }
    else
    if (ob->query_skill(SS_ELEMENT_EARTH) < 60)
    {
        return "chiseled";
    }
    else
...
extra_long()
{
    ...
    {
        return " The staff radiates earthly energy, but you are" +
            " unaffected by it.";
    }
}
...
create_weapon()
{
    ...
    set_adj("@@adj@@");
    set_adj("brown");
    set_short("@@adj@@ brown quarterstaff");
    ...
    set_long("This solid quarterstaff must have been cut from a" +
        ...
        " has been painted with shades of brown and dark green. Thick"+
        " and thin lines snake their way around the weapon in"+
        " intricate patterns. It reminds you of a mountain."+
        "@@extra_long@@\n");
    ...
    add_prop(MAGIC_AM_ID_INFO, ({"This staff has been enchanted with" +
        ...
        " additional benefits to those who know earth magic.\n", 25, 
        "This staff performs better for masters of earth magic, in" +
        " addition to sometimes stunning opponents.\n", 40}));
    
    add_prop(OBJ_S_WIZINFO, "This staff gets higher hit/pen for a" +
        " wielder who has skill in earth magic, and has a 1 in 8 chance" +
...
wield(object weapon)
{
    ...
    skill = wielder->query_skill(SS_ELEMENT_EARTH);
