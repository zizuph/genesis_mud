/*
   This is the staff of water, a magical quarterstaff that's found in Calia.
   It is mediocre for someone with no skill in water magic, but for someone
...
adj()
{
    ...
    if (!objectp(ob) || ob->query_skill(SS_ELEMENT_WATER) < 30)
    {
        return "painted";
    }
    else
    if (ob->query_skill(SS_ELEMENT_WATER) < 60)
    {
        return "cold";
    }
    else
...
extra_long()
{
    ...
    {
        return " The staff exudes cold, but you feel no discomfort" +
            " holding it in your hands.";
    }
    else
    {
        return " The staff radiates freezing cold, but you are" +
            " unaffected by it.";
    }
}
...
create_weapon()
{
    ...
    set_adj("@@adj@@");
    set_adj("blue");
    set_short("@@adj@@ blue quarterstaff");
    ...
    set_long("This solid quarterstaff must have been cut from a" +
        ...
        " has been painted with shades of blue. Thick and thin lines" +
        " snake their way around the weapon in intricate patterns. It" +
        " reminds you of the sea.@@extra_long@@\n");
    ...
    add_prop(MAGIC_AM_ID_INFO, ({"This staff has been enchanted with" +
        ...
        " additional benefits to those who know water magic.\n", 25, 
        "This staff performs better for masters of water magic, in" +
        " addition to sometimes freezing opponents.\n", 40}));
    
    add_prop(OBJ_S_WIZINFO, "This staff gets higher hit/pen for a" +
        " wielder who has skill in water magic, and has a 1 in 8 chance" +
    ...
    int dam)
{
    ...
    {
        TP->catch_msg(enemy->query_The_name(TP) + " was frozen by" +
            " the " + TO->short() + ".\n");
        enemy->catch_msg("You are frozen by the " 
            + TO->short() + "!\n");
        TP->tell_watcher(QCTNAME(enemy) + " was frozen by the " +
            TO->short() + "!\n", enemy);
...
wield(object weapon)
{
    ...
    skill = wielder->query_skill(SS_ELEMENT_WATER);

    if (skill >= 30)
    {
        wielder->catch_msg("The quarterstaff feels colder as you" +
            " wield it.\n");
