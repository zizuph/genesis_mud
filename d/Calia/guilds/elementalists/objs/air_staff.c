/*
   This is the staff of air, a magical quarterstaff that's found in Calia.
   It is mediocre for someone with no skill in air magic, but for someone
...
adj()
{
    ...
    if (!objectp(ob) || ob->query_skill(SS_ELEMENT_AIR) < 30)
    {
        return "painted";
    }
    else
    if (ob->query_skill(SS_ELEMENT_AIR) < 60)
    {
        return "airy";
    }
    else
...
extra_long()
{
    ...
    {
        return " The staff radiates electrical energy, but you are" +
            " unaffected by it.";
    }
}
...
create_weapon()
{
    ...
    set_adj("@@adj@@");
    set_adj("white");
    set_short("@@adj@@ white quarterstaff");
    ...
    set_long("This solid quarterstaff must have been cut from a" +
        ...
        " has been painted with shades of white and light grey. Thick"+
        " and thin lines snake their way around the weapon in"+
        " intricate patterns. It reminds you of a cloudy sky."+
        "@@extra_long@@\n");
    ...
    add_prop(MAGIC_AM_ID_INFO, ({"This staff has been enchanted with" +
        ...
        " additional benefits to those who know air magic.\n", 25, 
        "This staff performs better for masters of air magic, in" +
        " addition to sometimes stunning opponents.\n", 40}));
    
    add_prop(OBJ_S_WIZINFO, "This staff gets higher hit/pen for a" +
        " wielder who has skill in air magic, and has a 1 in 8 chance" +
    ...
    int dam)
{
    ...
    {
        TP->catch_msg(enemy->query_The_name(TP) + " was shocked by" +
            " the " + TO->short() + ".\n");
        enemy->catch_msg("You are shocked by the " 
            + TO->short() + "!\n");
        TP->tell_watcher(QCTNAME(enemy) + " was shocked by the " +
            TO->short() + "!\n", enemy);
...
wield(object weapon)
{
    ...
    skill = wielder->query_skill(SS_ELEMENT_AIR);
