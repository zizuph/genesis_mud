...
int edit_cmd(string args)
{
    ...
    if(query_verb() == "title" || query_verb() == "btitle")
    {
        if(strlen(title))
...
string long(string str, object for_obj)
{
    ...
    // Add extra command for wizards who can't use conflicting <title> command
    if(this_player()->query_wiz_level())
    	result += "As a wizard, you can also use the command <btitle "+
    	    book_type+"> as you can't use the <title "+book_type+
    	    "> command as it conflicts with your other <title> command.\n";
