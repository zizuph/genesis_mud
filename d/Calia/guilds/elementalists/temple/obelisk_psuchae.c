...
create_room() 
{
    ...
    add_exit(ELEMENTALIST_TEMPLE+"obelisk_ante","up","@@check_spiritform",1,1);
...
depression_desc()
{
    ...
    depr_desc = "The depression is roughly circular shaped, just slightly "+
        "larger than your Mandala.";
    return depr_desc;
}
...
do_insert(string str)
{
    ...
    if (!parse_command(str, TP, " [the] [sacred] 'mandala' [in] [into] [the] [depression] "))
    {
    	if (!parse_command(str, TP, " [the] [holy] 'symbol' [in] [into] [the] [depression] "))
    	{
    ...
    symbol = present("_elemental_guild_mandala",TP);
    if (!symbol)
    {
        write("Insert what in the depression?\n");
        return 1;
    }
    ...
    TP->add_prop(LIVE_I_PLACED_CLERIC_SYMBOL,1);
    return 1;
}
