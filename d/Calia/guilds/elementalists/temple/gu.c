/*
 * Gu Statue Room in the Elemental Temple of Calia
 * 
...
create_temple_room()
{
    ...
    try_item( ({ "statue", "life-sized statue" }),
        "This statue is very large. It is carved out of a golden sandstone "
      + "that vibrates with energy. It depicts a woman standing atop a "
      + "newly risen mountain. It measures about ten feet across at the base "
      + "of the mountain, and stands almost fifteen feet high at its highest "
      + "point. The eyes of the statue seem to sparkle "
      + "an emerald green.\n");
                       
    add_exit(ELEMENTALIST_TEMPLE + "gu_start", "southwest", "@@cannot_enter_start_locations", 1, "@@cannot_see_start_locations");
    add_exit(ELEMENTALIST_TEMPLE + "outer13a", "east"); 
    add_exit(ELEMENTALIST_TEMPLE + "outer11", "northwest"); 
}
...
desc_long_description()
{
	string long_desc = 
        ...
        + "of the Elementals. The outer circle of the Temple "
        + "has hallways that lead into this room from the east and the "
        + "northwest. Featured in the center of the room is a white marble "
...
do_worship(string arg)
{
    ...
    arg = lower_case(arg);
    if (arg != "gu" && arg != "lady gu")
    {
        return 0;
    }

    string statue_element = "earth";
    ...
    if (element == 0)
    {
        ...
        write("The ground beneath your feet rumbles as you hear the "
              + "voice of Lady Gu: Welcome, "+capitalize(player->query_real_name())
        ...
        COUNCIL_LOG_BOOK->update_log_book(my_time() + capitalize(player->query_real_name()) +
        	" has set their primary Element as Earth.\n");
        if (GUILD_MANAGER->query_is_worshipper(player))
        {
    ...
    else if (player->query_prop(CONFIRM_CHANGE_PATRON) == statue_element)
    {
        ...
        write("The ground beneath your feet rumbles as you hear the "
              ...
              + "! May you compel the elemental earth in any way you need be.\n");
        COUNCIL_LOG_BOOK->update_log_book(my_time() + capitalize(player->query_real_name()) +
        	" has reset their primary Element to Earth.\n");
        if (GUILD_MANAGER->query_is_worshipper(player))
        {
        ...
        else
        {
            ...
            setuid();
            seteuid(getuid());
            clone_object(ELEMENTALIST_OBJS +"earth_mandala")->move(TP,1);
