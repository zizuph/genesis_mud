/*
 * Diabrecho Statue Room in the Elemental Temple of Calia
 * 
...
create_temple_room()
{
    ...
    try_item( ({ "statue", "life-sized statue" }),
        "This statue is very large. It is carved out of a beautiful block "
      + "of ice-like stone that seems to radiate power from within yet "
      + "doesn't melt. It depicts a man sitting on a throne, sceptre in "
      + "hand, surrounded by a few of his guards. It measures about five "
      + "feet across the base of the throne, and stands about fifteen feet "
      + "high at its highest point. The eyes of the statue glow a bright "
      + "blue.\n");
                   
    add_exit(ELEMENTALIST_TEMPLE + "hall2", "west", "@@cannot_enter_middle_circle", 1, "@@cannot_see_middle_circle");
    add_exit(ELEMENTALIST_TEMPLE + "outer8", "north"); 
    add_exit(ELEMENTALIST_TEMPLE + "outer10", "south"); 
    add_exit(ELEMENTALIST_TEMPLE + "diabrecho_start", "east", "@@cannot_enter_start_locations", 1, "@@cannot_see_start_locations"); 
}
...
do_worship(string arg)
{
    ...
    arg = lower_case(arg);
    if (arg != "diabrecho" && arg != "lord diabrecho")
    {
        return 0;
    }

    string statue_element = "water";
    ...
    if (element == 0)
    {
        ...
        write("Your body fluids race as the voice of Lord Diabrecho ring "
              + "in your ears: Welcome! May you conform the elemental "
              + "waters to any way you deem necessary on your journies.\n");
        COUNCIL_LOG_BOOK->update_log_book(my_time() + capitalize(player->query_real_name()) +
        	" has set their primary Element as Water.\n");
        if (GUILD_MANAGER->query_is_worshipper(player))
        {
    ...
    else if (player->query_prop(CONFIRM_CHANGE_PATRON) == statue_element)
    {
        ...
        write("Your body fluids race as the voice of Lord Diabrecho ring "
              ...
              + "waters to any way you deem necessary on your journies.\n");
        COUNCIL_LOG_BOOK->update_log_book(my_time() + capitalize(player->query_real_name()) +
        	" has reset their primary Element to Water.\n");
        if (GUILD_MANAGER->query_is_worshipper(player))
        {
        ...
        else
        {
            ...
            setuid();
            seteuid(getuid());
            clone_object(ELEMENTALIST_OBJS +"water_mandala")->move(TP,1);
