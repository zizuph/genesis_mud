/*
 * Psuchae Statue Room in the Elemental Temple of Calia
 * 
...
create_temple_room()
{
    ...
    try_item( ({ "statue", "life-sized statue" }),
        "This statues is fairly large, but you cannot really tell how big it "
      + "is because the edges seem to expand and contract on their own at "
      + "random intervals. The statue itself is made from some sort of dark "
      + "stone-like material that absorbs almost all of the light in the "
      + "area. It depicts a figure shrouded in a black cloak of shadow. As "
      + "best as you can estimate, the statue is about eight feet tall and "
      + "three feet wide at the base. The eyes of the statue are a deep "
      + "piercing black and seem to be searching your soul.\n");
                   
    add_exit(ELEMENTALIST_TEMPLE + "outer2", "west");
    add_exit(ELEMENTALIST_TEMPLE + "outer3", "east"); 
    add_exit(ELEMENTALIST_TEMPLE + "psuchae_start", "north", "@@cannot_enter_start_locations", 1, "@@cannot_see_start_locations"); 
    add_exit(ELEMENTALIST_TEMPLE + "middle2", "south", "@@cannot_enter_middle_circle", 1, "@@cannot_see_middle_circle"); 
}
...
desc_long_description()
{
    string long_desc = 
        ...
        + "has hallways that lead into this room from the east and the "
...
do_worship(string arg)
{
    ...
    if (element == 0)
    {
        ...
        write("A quiet voice echoes in your ears in which you recognize "
              + "the voice of Psuchae: Welcome, "+capitalize(player->query_real_name())
              + "! May you learn to control the elemental spirits to aid you in "
              + "all of your adventures.\n");
        COUNCIL_LOG_BOOK->update_log_book(my_time() + capitalize(player->query_real_name()) +
        	" has set their primary Element as Spirit.\n");
        if (GUILD_MANAGER->query_is_worshipper(player))
        {
    ...
    else if (player->query_prop(CONFIRM_CHANGE_PATRON) == statue_element)
    {
        ...
        GUILD_MANAGER->set_primary_element(player, statue_element);
        write("A quiet voice echoes in your your ears in which you recognize "
              + "the voice of Psuchae: Welcome, "+capitalize(player->query_real_name())
              + "! May you learn to control the elemental spirits to aid you in "
              + "all of your adventures.\n");
        COUNCIL_LOG_BOOK->update_log_book(my_time() + capitalize(player->query_real_name()) +
        	" has reset their primary Element to Spirit.\n"); 
        if (GUILD_MANAGER->query_is_worshipper(player))
        {
        ...
        else
        {
            ...
            setuid();
            seteuid(getuid());
            clone_object(ELEMENTALIST_OBJS +"spirit_mandala")->move(TP,1);
