...
create_temple_room()
{
    ...
    set_long("Here you find the post office of the Elemental Temple of "
        + "Calia. Residents and visitors of the temple can correspond "
        + "with the rest of the world via mail. Upon entering, you were "
        + "directed to your own booth, where you have access to read and "
        + "write mail. The northeast exit allows you to find your way back "
        + "to the outer circle of the temple.\n\n"
...
//        + "   mail worshippers  Mail the Elemental Worshippers Council.\n"
        ...
        + "   mail guildmaster  Mail the Elemental Guilds Guildmaster.\n"
    ...
    try_item( ({ "hall", "northeast", "northeast exit" }),
                "The outer circle of the Elemental Temple is just outside "
              + "the post office to your northeast.\n");
                   
    add_exit(ELEMENTALIST_TEMPLE + "outer11", "northeast");
}

private void
add_mail_aliases(object reader)
{
    // Add the aliases for the Guildmaster and Councils
...
//    }
    ...
    string * eccouncilmembers = GUILD_MANAGER->query_ec_council();
    if (sizeof(eccouncilmembers))
    {
        reader->set_alias("council", eccouncilmembers);
    }
}
...
{
    write("\nYou have" + new + " mail for you in the Elemental Temple post office.\n\n");
}
