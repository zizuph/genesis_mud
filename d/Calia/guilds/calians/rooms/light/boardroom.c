 ...
 * Exit to maul room removed, Maniac, 19/2/02
 */
...
create_room()
{
	...
	"A heavy oak writing table sits against the south wall, over which " +
        "hangs a large gold seal. Sitting on top of the table is a voting "
      + "box.\n");
...
//    add_exit(CRPALACE_LIGHT_ROOMS + "maul_room", "northwest", can_leave); 
    ...
    add_item( ({ "table", "writing table" }),
        ...
        "sit atop the table to be used for writing. Beside these items "
      + "is a voting box." +
        "\n");
    ...
    clone_object(CRPALACE_OBJECTS + "plaque")->move(this);
    clone_object(CRPALACE_OBJECTS + "voting_box")->move(this);
    ::create_room();
}
