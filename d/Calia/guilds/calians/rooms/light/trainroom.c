...
* Cherek - 2020-12-18
* - Changed < > to ' ' in room desc to prevent players from accidentally 
* leaving the guild by clicking <break trust>.
*/
...
can_enter_tunnel(object player)
{
    ...
    if (COUNCIL_CODE->is_tunnel_access_banned(player) ||
...
set_up_skills()
{
    ...
    sk_add_train(SS_SWARM_KNOWLEDGE, ({ me, ot }), "swarm",
        100, 100);
...
sk_query_max(int snum, int silent)
{
    ...
    else if (this_player()->is_calian_trainee())
    {
        return sk_query_max_trainee(snum, silent);
    }
    else if (this_player()->is_calian_warrior())
    {
...
up()
{
    ...
    tp = TP;
    if (!tp->query_wiz_level() && !is_calian(tp))
    {
    ...
    if (tp->is_calian_follower(tp))
    {
        // Followers are not allowed upstairs
        write("You step into the white light but nothing happens.\n");
        return 1;
    }
...
create_room()
{
    ...
    {
        clone_object(CRPALACE_OBJECTS+"drop_box")->move(this_object());
    }
...
list(string str)
{
    ...
    string guilds = TP->list_major_guilds();
    if (guilds)
