...
help(string str)
{
    ...
    { 
        tp->more("_Commands available to council members_\n" + 
          ...
          "glass              - list Calians who had rack points low enough for\n" + 
          "                     a glass medallion when they last left the realms.\n" + 
          "pending            - list commands awaiting confirmation.\n" +
          "rackban <player>   - ban <player> from using the racks.\n" +                  
          ...
          "swarmban <player>  - ban <player> from swarming.\n" + 
          "trainban <player>  - ban <player> from training.\n" +
          "tunnelban <player> - ban <player> from using the tunnels.\n" +
          "voteban <player>   - ban <player> from using the voting box.\n");        
        return 1; 
    } 
}
...
{
    return COUNCIL_CODE->record_ban_tunnel_access(str);
}

int
record_ban_vote(string str)
{
    return COUNCIL_CODE->record_ban_vote(str);
}

int
record_ban_rack(string str)
{
    return COUNCIL_CODE->record_ban_rack(str);
}
...
{
    return COUNCIL_CODE->list_glass_medallion_owners(str); 
}
...
init()
{
     ...
     add_action(list_glass_medallion_owners, "glass"); 
     add_action(see_council, "council");
     add_action(see_pending_commands, "pending");
     add_action(cancel_pending_commands, "rempending");
     add_action(record_ban_tunnel_access, "tunnelban");
     add_action(record_ban_vote, "voteban");
     add_action(record_ban_rack, "rackban");     
