...
query_accepted_string()
{
    ...
    if (sizeof(accepted) == 0)
        ...
        rstr = "The following players have been approved by the ROs and " +
...
{
    /* Updated on July 23, 2017 to fix an issue with using title names */
    if (!objectp(player))
    {
        return 1;
    }

    int title_index = member_array(minimum_level, SD_AV_TITLES);
    if (title_index < 0)
    {
        return 1;
    }
    
    int minimum_level_avg = SD_AV_LEVELS[title_index];
    if (player->query_average_stat() >= minimum_level_avg)
    {
        return 0;
    }
...
check_last_login(string app)
{
    ...
    result = "";
    if (applicant = find_player(app))
    {
        result += "\n" + capitalize(app) + " is awake right now.\n";
    }
    else
    {
        applicant = SECURITY->finger_player(app);
        if (objectp(applicant))
        {
            chtime = time() - applicant->query_logout_time();
            chtime = chtime / 86400; // number of seconds in a day
            result += "\n" + capitalize(app) + " was last awake ";
            switch (chtime)
            {
            case 0:
                result += "in the past day.\n";
                break;
            
            case 1..6:
                result += "in the past " + (chtime + 1) + " days.\n";
                break;
        ...
        {
            result += "\nNo login information could be found.\n";
        }
    }
...
{
    CALIAN_EVENTS_HANDLER->add_event_item(player, text);
}
