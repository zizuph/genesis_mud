...
list_active_calians()
{
    ...
    object *list;
    int warriors;
    string *name_list, s_desc, p_desc, title;
    ...
    for (i = 0; i < sizeof(list); i++)
    {
              ...
              list[i]->query_prop(LIVE_I_ALWAYSKNOWN))))
        {
            ...
            if (IN_ARRAY(list[i]->query_real_name(),
...
calian_info(string str)
{
    ...
    seteuid(getuid());
    if (!str) 
        CALIAN_MEMBER_HANDLER->list_calians_with_info(this_player());
    else
        CALIAN_MEMBER_HANDLER->see_info(this_player(), str); 
    return 1;
}
...
{
    return ([
    "swarm" : CRPALACE_SPECIALS + "swarm",
    "move" : CRPALACE_SPECIALS + "move_behind",
    "mbsw" : CRPALACE_SPECIALS + "mbsw",
     ]);
}
