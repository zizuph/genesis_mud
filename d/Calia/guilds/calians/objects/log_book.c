...
/* This code maintains the log book by updating the file
   ...
   entry.  By Maniac */
/* For wizards, make a separate file. Mercade */
/* Used in many places so varargs until those are fixed, Cotillion */
varargs int
update_log_book(string new_line, int wizard = 0)
{
    string text_file,
	whole_file_string, first_line_of_file, rest_of_file, new_file_string;
    int size;

    text_file = query_file() + (wizard ? ".wiz" : "");
