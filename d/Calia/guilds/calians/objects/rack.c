...
/* 
   ...
   History:
          20-oct-2010 Auto-Save functionality              Petros
          ...
          13/11/96    rack points system smartened up      Maniac
           ...
           20/6/96    made more efficient                  Maniac
            ...
            02.04.20  Allowed the removal of broken items  Arman
*/
...
{
    (CRPALACE_OBJECTS + "rack_log_book")->update_log_book(
        ctime(time()) + " " + capitalize(who->query_real_name()) + " borrowed " + 
...
prevent_leave(object ob)
{
    ...
    if (base_value == 1)
    {
        // If already not allowed to leave, we don't do anything else.
        return 1;
    }

    if (COUNCIL_CODE->is_rack_banned(this_player()))
    {
        write("You have been banned by the Calian Council from using the "
              + "racks.\n");
        return 1;
    }
...
{
    int base_value = ::prevent_enter(ob);
    if (base_value == 1)
    {
        // If not already allowed to enter, we 
        return 1;
    }
