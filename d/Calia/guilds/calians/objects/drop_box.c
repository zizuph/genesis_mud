 ...
 *
 *  Petros  Change Drop Box to add restriction against Followers. 3/2010
...
prevent_leave(object ob)
{
    ...
    if (IS_CALIAN_TRAINEE(TP))
    {
        write("As a Calian trainee, you are not yet able to retrieve "
            + "items donated to the Calians.\n");
        return 1;
    }
    else if (IS_CALIAN_FOLLOWER(TP))
    {
        write("As a Follower of Caliana, you are not yet able to retrieve "
            + "items donated to the Calians.\n");
        return 1;
    }
    else if (!IS_CALIAN_WARRIOR(TP))
    {
        write("You can only remove items from the receptable if you" +
            " put them there yourself or you are a Calian Warrior.\n");
        return 1;
