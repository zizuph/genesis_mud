 ...
 *
 * Bishop, May 2004.
 * December 2004: Removed resistance check from hammer throws. It's a physical
 * weapon once it's been conjured.
 */
    ...
    switch (hammer_level)
    {
        case 1:
            descrip = "This hammer is slightly warm to the touch. It does" +
                " not appear to be made from any earthly material, but has" +
                " been forged from a shadowy substance with a uniformly" +
                " flat grey finish.";
            break;
        case 2:
            descrip = "This grey hammer has a faint silvery sheen to it." +
                " Studying it closely, you can see the glow waxing and waning"+
                " ever so slightly. It does not appear to be made from any" +
                " earthly material, but has been forged from a shadowy" +
                " substance.";           
            break;
        case 3:
            descrip = "This hammer, forged from a shadowy substance, is a" +
                " strange sight. Tiny sparks appear on it at irregular times" +
                ", travelling along the surface of the hammer a while, before"+
                " disappearing again. They do not harm you at all, though" +
                " the hammer is quite warm to the touch.";
            break;
        case 4:
            descrip = "This hammer, forged uniformly from a shadowy substance"+
                ", is a marvel to behold. Shadows of varying darkness travel" +
                " back and forth beneath its surface, producing complex and" +
                " eerie patterns. It is hard to shake the feeling that this" +
                " weapon is alive.";
            break;
        case 5:
            descrip = "This hammer emits a ghostly whispering sound that is" +
                " hauntingly beautiful. Listening closely, you feel you are" +
                " very close to understanding the whisper, but it is always" +
                " just at the edge of coherence. The hammer itself is forged" +
                " uniformly from a shadowy material and is alive with" +
                " flickering shadows.";
            break;
        default:
            descrip = "This hammer fills your mind with song, the power and" +
                " beauty of which stirs you and tugs at your soul. At times" +
                " you feel you can recognize a single voice, other times it" +
                " is a chorus. The hammer itself appears forged from a" +
                " shadowy substance, and eldritch shadows dance across its" +
                " surface.";
    }
...
create_object() 
{
    ...
    add_prop(OBJ_S_WIZINFO, "This hammer is created from the Sfyri spell of" +
        " the Spirit Circle of Psuchae. It's a holdable item, and works as" +
        " a thrown weapon that returns to the holder's hand. Can't be given" +
        " away. Extra damage vs the undead.\n");
    add_prop(MAGIC_AM_ID_INFO,({"This hammer is a magical creation.\n", 20, 
        "This throwing hammer performs better against undead foes.\n", 40}));  
...
{
    int task = TASK_ROUTINE - thrower->query_casting_bonus();
...
improve_hammer_pen()
{
    ...
    i = spell_caster->query_stat(SS_OCCUP) + spell_caster->query_stat(SS_WIS)
...
check_hammer_upgrade()
{
    ...
    {
        hammer_level = 2;
        hammer_breakage = hammer_breakage / 2;
        remove_adj("dull");
        add_adj("silvery");
        spell_caster->catch_tell("Your dull grey hammer begins to glow" +
            " with a silvery colour!\n");
        tell_room(environment(spell_caster), LANG_POSS(QCTNAME(spell_caster)) + 
            " dull grey hammer begins to glow with a silvery colour!\n", 
            ({spell_caster}));
    }
    else
    ...
    if (hammer_level == 4 && hammer_pen >= 400)
    {
        ...
        tell_room(environment(spell_caster), LANG_POSS(QCTNAME(spell_caster)) + 
            " pulsating shadowy hammer begins to emit a whispering sound!\n", 
            ({spell_caster}));        
        hammer_level = 5;
        hammer_breakage = hammer_breakage / 2;        
    }
...
throw_hammer(string str)
{
    ...
    spell_caster->add_mana(-5);
    spell_caster->add_fatigue(-2);
    ...
    switch (random(4))
    {
        ...
        case 2:
            ...
            msg3 = "Twiling " + spell_caster->query_possessive() + " " +
                TO->short() + " in " + spell_caster->query_possessive() + 
                " hand, " + spell_caster->query_the_name(enemy) + " flings" +
                " it abruptly at you!";
            break;
        default:
            msg1 = "You distract " + enemy->query_the_name(spell_caster) + 
                " with a feint, before casting your " + TO->short() + " at " +
                enemy->query_objective() + " with a sudden underarm throw!";
            msg2 = QCTNAME(spell_caster) + " distracts " + QTNAME(enemy) +
                " with a feint, before casting " + 
                spell_caster->query_possessive() + " " + TO->short() + " at " +
                enemy->query_objective() + " with a sudden underarm throw!";
            msg3 = spell_caster->query_The_name(enemy) + " distracts you" +
                " with a feint, before casting " + 
                spell_caster->query_possessive() + " " + TO->short() + " at" +
                " you with a sudden underarm throw!";
    }
    ...
    else
    {
        ...
        if (random(crit_chance) == 0)
        {
            //Critical hit. 
            real_pen += real_pen;
            setuid();
            seteuid(getuid());
            write_file(SPIRIT_LOGS + "sfyri_crit_log", ctime(time()) + ": " + 
                spell_caster->query_name() + " got a critical hit, resulting in " +
                real_pen + " pen.\n");             
        }
    ...
    if ((random(hammer_breakage) == 0) && sizeof(hitresult))
    {
        spell_caster->catch_msg(" The " + TO->short() + " breaks!\n");
        spell_caster->tell_watcher(" The " + TO->short()+ " breaks!\n", 
            ({spell_caster}));
    ...
    spell_caster->tell_watcher("The " + TO->short() + " returns to " + 
        LANG_POSS(QTNAME(spell_caster)) + " hand.\n", ({spell_caster}));

    check_hammer_upgrade();
        
    spell_caster->add_prop(I_CANT_THROW_SPIRIT_HAMMER, 1);
    
    if (objectp(present("_skiamorfi_object_", spell_caster)))
    {
        //Somewhat faster recycle if caster is in shadow form.
        recast_alarm = set_alarm(itof(10 + random(10)), 0.0, &throw_again());
    }
    else
    {
        recast_alarm = set_alarm(itof(12 + random(11)), 0.0, &throw_again());
    }
