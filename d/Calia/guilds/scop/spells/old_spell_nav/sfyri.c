...
{   
    if (caster == 0)
    {
        ...
        set_spell_object(SPIRIT_SPELLS + "sfyri_ob");
        set_spell_mana(100);
        set_spell_time(6);
        set_spell_vocal(0);
...
resolve_spell(object caster, object *targets, int *resist, int result)
{
    ...
    new_val = MIN(CASTED_LIMIT,(old_val + 3));
    caster->set_skill(SS_SPELLS_CASTED, new_val);
    
    caster->catch_tell("Completing the Sfyri conjuration, you call upon" +
        " Psuchae to bequeath a hammer of spirit upon you. The air in" +
        " front of you quivers, and a shadowy nexus springs into being." +
        " Your hand guided by your deity, you reach within the nexus," +
        " pulling forth a dull grey hammer! The nexus collapses as soon as" +
        " you withdraw your hand.\n");
    caster->catch_tell("If you want to return the hammer to Psuchae, do" +
        " <hammerdispel>.\n");

    tell_room(environment(caster), QCTNAME(caster) + " completes the spell, " +
        caster->query_possessive() + " eyes expectant. The air in front of " +
        caster->query_objective() + " quivers, a shadowy nexus springing" +    
        " into being! Exultant, " + QTNAME(caster) + " reaches within the" +
        " nexus, pulling forth a dull grey hammer. The nexus collapses as" +
        " soon as " + caster->query_pronoun() + " withdraws " +
