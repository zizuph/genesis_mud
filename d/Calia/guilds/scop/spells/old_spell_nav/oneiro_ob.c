...
int state = 0;
object scried_room, scried; //The room we're scrying, and the person we're scrying.
string *allowed_actions = (({"wake", "visit", "sense", "dreamhelp", "watch",
    "enter", "withdraw"}));
...
create_object()
{
    ...
    add_prop(OBJ_S_WIZINFO, "This is a spell object for a SCoP spell. It" +
        " paralyzes the player, only allowing him/her a few actions.\n");
}
...
enter_dream_state()
{
    ...
    {
        state = SLEEP_STAGE_1;
        spell_caster->catch_tell("Your limbs grow leaden as you move further" +
            " and further away from the waking world.\n");
        set_alarm(itof(10 + random(20)), 0.0, enter_dream_state);            
    }
    else
    if (state == SLEEP_STAGE_1)
    {
        state = SLEEP_STAGE_2;
        spell_caster->catch_tell("Your physical body begins to feel like a" +
            " distant memory as you venture further and further into the" +
            " dream world.\n");
        set_alarm(itof(10 + random(20)), 0.0, enter_dream_state);  
    }
    else
    if (state == SLEEP_STAGE_2)
    {
        ...
        spell_caster->catch_tell("Softly, as if breaking through a thin" +
            " membrane, you fully enter a dreaming state. With the aid of" +
            " Psuchae, you might be able to travel in the shadowlands between"+
            " reality and the dream world. <dreamhelp> gives you" +
            " information on what actions are possible here.\n");
        tell_room(environment(spell_caster), QCTNAME(spell_caster) + " grows" +
            " faint and disperses into shadow.\n", (({spell_caster})));
        spell_caster->add_prop(SCOP_O_SLEEP_LOCATION, 
            environment(spell_caster));
...
dream_check()
{
    ...
    {
        spell_caster->catch_tell("You no longer have the mental energy" +
            " to maintain your presence in the dream world, and slowly" +
        ...
        state = WAKING_UP;
        set_alarm(itof(10 + random(20)), 0.0, 
            dispel_spell_effect);
    }
    else
    {
...
{
    int diff = TASK_ROUTINE - spell_caster->query_casting_bonus();
    ...
    if (!check_user(target) ||
        ...
        (({SKILL_WEIGHT, 200, SS_FORM_DIVINATION, SKILL_END, 
            TS_INT}))) <= 0 ||
        target->query_magic_res(MAGIC_I_RES_MAGIC) > random(100) ||
    ...
    else
    {
        spell_caster->catch_tell("After journeying for a time through the" +
            " dream world, and witnessing many strange and outlandish" +
            " sights, you begin to draw closer to your target. You" +
            " arrive at a place where the border between the dream world" +
            " and the real world is particularly thin. Here you can <watch>" +
            " or attempt to <enter> the waking world.\n");
...
do_dreamhelp(string str)
{
    ...
    desc = "You can perform the following actions in the dream world:\n" +
        ...
        " world at the person you are currently visiting and observe the" +
        " happenings there. While projected, you will be invisible, but may" +
        " still be vulnerable to the dangers of the waking world." +
        " It is fully possible to die in the dream. Bewarned!\n" +
...
dispel_spell_effect(object dispeller, int mode)
{
    ...
    {    
        ...
        spell_caster->move_living("M", 
            spell_caster->query_prop(SCOP_O_SLEEP_LOCATION), 1, 0);            
        tell_room(environment(spell_caster), QCTNAME(spell_caster) + 
            " coalesces from the shadows, lying prone on the ground with " +
            spell_caster->query_possessive() + " eyes closed. " +
            capitalize(spell_caster->query_pronoun()) + " opens them and" +
...
{   
    ...
    if (!check_user(scried) ||
        scried->query_magic_res(MAGIC_I_RES_MAGIC) > random(100) ||
    ...
    {
        spell_caster->catch_tell("Straining with the effort of projecting" +
            " your presence into the waking world, you arrive, somewhat" +
            " disoriented.\n");
...
do_visit(string str)
{
    ...
    if (spell_caster->query_mana() < 30)
    {
        notify_fail("You don't have the mental strength to visit anyone.\n");
        return 0;
    }
    ...
    if (stringp(str))
    {
        ...
        if (!objectp(target))
        {
            notify_fail("You are unable to find anyone with that name.\n");
            return 0;
        }
    }
    else
    ...
    if (state == FULL_SLEEP_PRESENT)
    {
        notify_fail("You are projected into the waking world! You need to" +
            " withdraw before you can visit anyone.\n");
        return 0;
    }
    ...
    spell_caster->catch_msg("Holding " + LANG_POSS(target->query_name()) + 
        " image in your mind, you begin searching the dream world for a" +
        " place where you can watch " + target->query_objective() + ".\n"); 
    spell_caster->add_mana(-30);
...
do_watch(string str)
{
    ...
    if (state == FULL_SLEEP_PRESENT)
    {
        notify_fail("You are projected into the waking world, and have" +
            " to withdraw before you can <watch>.\n");
        return 0;
    }
    ...
    if (!check_user(scried) ||
        sizeof(filter(all_inventory(env), 
            &->query_wiz_level())))
    {
    ...
    tell_object(spell_caster,"Surrounded by the haze of the dream world," +
        " through your window into the waking world, you see:\n " +
        desc + "\n");
