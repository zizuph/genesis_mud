...
config_spell(object caster, object *targets, string argument)
{
    if (caster == 0)
    {
        ...
        set_spell_ingredients(({"_metaballo_component_"}));
        set_spell_peaceful(1);
        set_spell_resist(spell_resist_identify);
        set_spell_object("/d/Calia/guilds/scop/spells/metaballo_obj");
    ...
    else
    {
        ...
        {
            caster->catch_msg("You can only use this gift on a weapon.\n");
            return 1;
        }

        if (!present(targets[0],caster))
        {
            caster->catch_msg("You can only use this gift on a weapon you "+
                "are carrying.\n");
            return 1;
        }
        ...
        {
            caster->catch_msg("Metaballo only works on magical weapons!\n");
            return 1;
        }

        if (targets[0]->query_broken())
        {
            caster->catch_msg("You cannot use this gift on a broken weapon.\n");
            return 1;
        }    
        return 0;
    }
...
{
    int old_val, new_val, stat, weapon_goodness;
    object weapon;
    
    old_val = caster->query_skill(SS_SPELLS_CASTED);
    new_val = MIN(CASTED_LIMIT, (old_val + 8));
    ...
    weapon_goodness = targets[0]->query_hit() + targets[0]->query_pen() +
        MIN(40, targets[0]->query_value()/100);
    ...
    if (targets[0]->query_hands() == W_BOTH)
    {
        weapon->set_twoh();
    }
    ...
    if (stat > 250 && weapon_goodness > 90)
    {
    ...
    caster->catch_msg("As you complete the spell, you hold your " + 
         targets[0]->query_short() +" up high, praying that Psuchae will see" +
         " fit to transform it into a weapon you can use to combat the" +
         " soulless. Shadows blossom from your outstretched hand, and as they"+
         " withdraw again, you are holding a " + weapon->short() + "!\n" + 
         " Use <wepdispel> if you wish to be rid of it.\n");
    tell_room(environment(caster), "As " + QTNAME(caster) + " completes " + 
         caster->query_possessive() + " spell, " + caster->query_pronoun() +
         " holds a " + targets[0]->query_short() + " up in the air. Shadows" +
         " blossom from " + caster->query_possessive() + " outstretched" +
         " hand, engulfing the weapon. The shadows withdraw, leaving a " +
         weapon->short() + ".\n", ({caster}));
     targets[0]->remove_object();
} 
