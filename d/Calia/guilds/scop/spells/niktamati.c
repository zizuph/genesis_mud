 ...
 * Petros, November 16, 2008: Fixed Niktamati so that it does not stack
 *                            when recast.
 */
...
config_spell(object caster, object *targets, string argument)
{
    if (caster == 0)
    {
        ...
        set_spell_ingredients(({"black_onyx_shard", "_leftover_eye"}));
    ...
    else
    {
        ...
        if (present("_niktamati_object_", targets[0]))
        {
            if(targets[0] == caster)
            {
                caster->catch_tell("You have already been blessed with niktamati, " +
                                   "but you pray to Psuchae again to prolong the effect.\n");
            }
            else
            {
                caster->catch_tell("That target has already been blessed with niktamati, " +
                                   "but you pray to Psuchae again to prolong the effect.\n");
            }
            return 0;
        }
...
resolve_spell(object caster, object *targets, int *resist, int result)
{
    ...
    niktamati = present("_niktamati_object_", tar);
    ...
    if(niktamati)
    {
        tar->add_prop(NIKTAMATI_IS_PRESENT, 1);
        niktamati->dispel_spell_effect();
    }
