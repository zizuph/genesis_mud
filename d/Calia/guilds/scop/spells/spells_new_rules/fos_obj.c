...
create_object()
{
    ...
    add_prop(OBJ_S_WIZINFO, "This is a spell object " +
                            "created by the fos spell " +
                            "of the Spirit Circle of " +
                            "Psuchae.\n");
...
start()
{
    int gstat, spirit, spellcraft, divination, time, wis;

    gstat = spell_target->query_stat(SS_OCCUP);
    wis = spell_target->query_stat(SS_WIS);
    ...
    add_prop(OBJ_I_LIGHT,MIN(10, 5+random(2+gstat/50)));
    time = MIN(900,(2*gstat + wis + spirit +
                    spellcraft + divination + random(200)));
...
show_subloc(string subloc, object me, object observer)
{
  ...
  return "A bright halo sits above " + 
         ( observer == me ? "your" :
