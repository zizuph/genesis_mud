...
config_spell(object caster, object *targets, string argument)
{
    if(caster == 0)
    {
        ...
        set_spell_vocal(0);
        set_spell_visual(0);
        set_spell_ingredients(({"pearl", "intestine"}));
    ...
    else
    {
        ...
        if((time() - caster->query_prop(SCOP_CAST_TIME)) < caster->query_prop(SCOP_SPELL_DELAY))
        {
            caster->catch_tell("You need to regain your mental focus before "+
                              "concentrating on another prayer.\n");
            return 1;
        }
...
resolve_spell(object caster, object *targets, int *resist, int result)
{
    ...
    res = MIN(100, result);                     
    base = guild_stat + wisdom + res + align/6; 
    base = (base * 3) /2; 
    ...
    // Heal cant give max combat aid if we are getting other kind of combat aid at the same time.
    // Sfysi takes the delay into account, so no need to make that deduct here.
    // Since the shadowform is a layman spell, it doesn't deduct anything.
    if (present("_metaballo_weapon_", caster))
    {
        // Metaballo gives less than 32% combat aid, heal can therefore at most give 68%.
        set_heal_amount(MIN(539, base));
    }
    ...
    if(tar->query_prop(LIVE_I_UNDEAD))
    {
      ...
      write_file(SPIRIT_LOGS + "therapeaou_heal_log", ctime(time()) + ": " + 
                 caster->query_name() + " cast therapeaou, damaging " +
                 "the undead with " + heal_result[3] + " hp.\n");    
    }
    else
    {
	...
	seteuid(getuid());
	write_file(SPIRIT_LOGS + "therapeaou_heal_log", ctime(time()) + ": " + 
		   caster->query_name() + " cast therapeaou, healing " +
		   heal + " hp.\n");    
    }
}
