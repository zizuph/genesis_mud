...
void hammer_effect(object spell_caster, object enemy, object hammer)
{
    ...
    else
    {
        ...
        // Sfyri cap of 60% combat aid
        real_pen = MIN(real_pen, 462);
        hitresult = enemy->hit_me(real_pen, MAGIC_DT, spell_caster, -1);


        setuid();
        seteuid(getuid());
        write_file(SPIRIT_LOGS + "sfyri_damage", ctime(time()) + ": " + 
                   spell_caster->query_name() + " sfyri hit with Pen: "+real_pen+ 
    ...
    spell_caster->add_prop(SCOP_SPELL_DELAY, reset_time);
}
...
throw_hammer(string str)
{
    ...
    if((time() - spell_caster->query_prop(SCOP_CAST_TIME)) <
                 spell_caster->query_prop(SCOP_SPELL_DELAY))
    {
        notify_fail("You need to regain your mental focus before "+
                    "you can concentrate on throwing your hammer.\n");
        return 0;
    }
