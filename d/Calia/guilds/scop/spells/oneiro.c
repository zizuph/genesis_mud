...
{   
    if (caster == 0)
    {
        ...
        set_spell_form(SS_FORM_DIVINATION,45);
        set_spell_task(TASK_ROUTINE - (caster->query_stat(SS_OCCUP)/2));
        set_spell_ingredients(({"diamond_shard", "_leftover_eye"}));
        set_spell_target(spell_target_caster);
        set_spell_target_verify(spell_verify_present);
        set_spell_object(SPIRIT_SPELLS + "oneiro_obj");
        set_spell_mana(200);
        set_spell_time(6);
        set_spell_vocal(0);
	...
	if (objectp(caster->query_attack()))
        {
            caster->catch_tell("You cannot sleep while fighting.\n");
            return 1;
        }
        ...
        {
            caster->catch_tell("You cannot use this gift while " +
                               "in shadow form.\n");
            return 1;
        }
    }
