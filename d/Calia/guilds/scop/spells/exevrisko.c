 ...
 *
 * Spell Recoded in the fall of 2006 by Navarre
 *
 */
...
config_spell(object caster, object* targets, string argument)
{
    ...
    if(caster == 0)
    {
        ...
        set_spell_ingredients(({"_leftover_skull"}));
    ...
    else
    {
        ...
        if(targets[0] == caster)
        {
            caster->catch_tell("If you have to ask Psuchae what colour your heart is, " +
                               "perhaps you are not following the path you have " +
                               "dedicated your life to follow.\n");
            return 1;        
        }
    }
    return 0;
...
resolve_spell(object caster, object *targets, int *resist, int result)
{
    ...
    caster->catch_tell("You close your eyes, concentrating on " +
                       tar->query_the_name(caster)+". You open " +
                       "your eyes and inspect " +
                       tar->query_the_name(caster) + "'s inner " +
                       "being carefully using the powers of " +
                       "vision granted to you by Psuchae. You " +
                       "determine that the inner aura is " +
                       desc_str + ".\n");
    ...
    tar->catch_tell(caster->query_The_name(tar)+" closes "+
                    ...
                    "inspects you closely, looking you up and " +
    ...
    say(QCTNAME(caster) + " closes "+caster->query_possessive() +
        ...
        QNAME(tar) + " closely, looking " + tar->query_objective() +
