...
public string
find_herbfile_using_name(string name, mapping list)
{
    ...
    singular_name = LANG_SWORD(name);    
    
    setuid();
    seteuid(getuid());
    foreach (string file : m_indices(list))
    {
        object herbobj;
        if (!objectp(herbobj = find_object(file)))
        {
            // File must not be loaded yet.
            if (LOAD_ERR(file))
            {
                continue;
            }
            if (!objectp(herbobj = find_object(file)))
            {
                // Really can't find it. Just move onto the next one
                continue;
            }
        }
        if (herbobj->query_herb_name() == singular_name
            || herbobj->query_herb_name() == name)
        {
            return file;
        }
    }
...
do_list(string arg)
{
    ...
    mapping matching_herbs = herb_list;
    ...
    {
        herbfile = find_herbfile_using_name(arg, herb_list);
        if (strlen(herbfile))
        {
            lt = 2;
            matching_herbs = ([ herbfile : herb_list[herbfile] ]);
        }
    }
            
    if (!m_sizeof(matching_herbs))
    {
    ...
    setuid();
    seteuid(getuid());
    TP->more(print_shop_list(matching_herbs, lt));
    return 1;
}
