...
create_room() 
{
    ...
    add_exit(SPIRIT_TEMPLE+"obelisk_lower2","north","@@check_spiritform",1,1);
    add_exit(SPIRIT_TEMPLE+"obelisk_aeria","northwest","@@check_spiritform",1,1);
    add_exit(SPIRIT_TEMPLE+"obelisk_gu","southwest","@@check_spiritform",1,1);
    add_exit(SPIRIT_TEMPLE+"obelisk_diabrecho","northeast","@@check_spiritform",1,1);
    add_exit(SPIRIT_TEMPLE+"obelisk_pyros","southeast","@@check_spiritform",1,1);
    add_exit(SPIRIT_TEMPLE+"obelisk_psuchae","down","@@check_step_two@@",1,1);
    add_exit(SPIRIT_TEMPLE+"obelisk_prison","up","@@check_step_three@@",1,1);
}
...
depression_desc()
{
    ...
    if ((present("_skiamati_object_",TP)) && 
        (!TP->query_skill(SS_PATRON_ELEMENTAL)))
    {
...
passages_desc()
{
    ...
    if (present("_skiamati_object_",TP))
    {
        ...
        pass_desc += "northwest, northeast, southwest, and southeast ";
...
walls_desc()
{
    ...
    if (present("_skiamati_object_",TP))
    {
        ...
        if (depression)
        {
            if (!TP->query_skill(SS_PATRON_ELEMENTAL))
...
can_see()
{
    ...
    if (present("_skiamati_object_",TP))
    {
        ...
        desc += "southwest, and southeast into the darkness.";
        if (notch)
...
do_insert(string str)
{
    ...
    if (str != "cylinder" && str != "glowing cylinder")
    {
    ...
    TP->add_prop(LIVE_I_PLACED_SPIRIT_CYLINDER,1);
...
do_place(string str)
{
    ...
    if (str != "triangle" && str != "black triangle")
    {
        write("The "+str+" does not fit in the notch.\n");
        return 1;
    }
    ...
    TP->add_prop(LIVE_I_PLACED_SPIRIT_TRIANGLE,1);
    write("You feel as though you are one step closer to freeing "+
        "the trapped spirit.\n");
    return 1;
}
