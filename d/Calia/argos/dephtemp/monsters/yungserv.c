/* A young servant in the Temple of Dephonia
**
**   Coder         Date                Action
** -------------- -------- ----------------------------------------------
** Zima           1/10/95    Created
*/
inherit "/std/monster";
#include <ss_types.h>
#include <stdproperties.h>
#include <macros.h>
#include <cmdparse.h>
#include "defs.h"
 
void equip_me() {
    object this=this_object();
    clone_object(ARMOUR_DIR + "slvtunic")->move(this);
    clone_object(WEAPON_DIR + "woodspn")->move(this);
    command("wield all");
    command("wear all");
}
 
void create_monster() {
    if (!IS_CLONE) return;
    set_name(({"young servant","servant","girl","human","young girl"}));
    set_adj("servant");
    set_adj("young");
    set_race_name("human");
    set_short("young servant girl");
    set_long(
        "She is a young girl with long brown hair and charming blue "+
        "eyes. She is dressed in the tunic of a servant of the temple.\n");
 
    add_prop(ATTACK_TEAM,DEPH_ATTACK_TEAM);
    set_gender(1); /* female */
 
    set_act_time(7);
    add_act("say I must check soon that there is enough food at the meeting.");
    add_act("say I don't remember Argos, I was a child when I was taken.");
    add_act("emote stirs a pot of food with her wooden spoon.");
    add_act("emote beats her wooden spoon on the side of a pot.");
    add_act("emote glances worriedly at the stacks of dirty dishes.");
    add_act("emote smiles charmingly.");
    add_act("emote sighs tiredly.");
 
    set_cact_time(3);
    add_cact("say What have I done to you?");
    add_cact("say May Arxes protect me!");
    add_cact("emote holds a hand over her face to deflect the blows.");
    add_cact("emote cries in terror.");
    add_cact("emote sobs I'll never see my mother again!");
 
    set_stats(({ 10, 10, 10, 10, 10, 10})); /* STR,DEX,CON,INT,WIS,DIS */
    set_skill(SS_WEP_CLUB, 10);
    set_skill(SS_PARRY, 10);
    set_skill(SS_DEFENCE, 10);
    set_hp(10000);
    set_alignment(150);
    add_prop(CONT_I_WEIGHT, 75000);
    add_prop(CONT_I_VOLUME, 75000);
    set_all_hitloc_unarmed(query_average_stat() / 4);
}
