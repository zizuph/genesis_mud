/*
 *   Vision of Delphia - Epsilon tome
 *
 *   DESCRIPTION:
 ...
 *   hints.
 *
 *   LOCATION FOUND: In the lounge (rooms/lounge)
 *
 *   HISTORY:
 *   CODER         DATE             ACTION
 *  ------------ -------- -----------------------------------------------
...
#include <config.h>
#include <cmdparse.h>
#define LANG_LEVEL EPSILON_LANG_LEVEL
#include "tomedefs.h"
#include "defs.h"
 
#define HOME_ROOM ROOM_DIR+"lounge.c"
#define OBJ_NUM 0
#include "../retrnobj.h"
...
create_scroll()
{
    set_name(({"ancient tome","tome","epsilon tome"}));
    set_short("ancient tome");
    set_long("An ancient tome entitled: The Vision of Delphia - Epsilon\n");
 
    add_prop(OBJ_M_NO_SELL,1);
    add_prop(OBJ_I_WEIGHT, 20);
    add_prop(OBJ_I_VOLUME, 150);
 
    set_file(OBJ_DIR+"tomee.txt");
}
