...
**  ---------- --------  -----------------------------------------------------
**  Zima       1/1/95    Created
**  Maniac     14/6/2000 number of ravens per reset limited.
...
#define MAX_RAVENS_PER_RESET 10
#define MAX_RAVENS 5

int ravens_summoned_this_reset = 0; 
object hiero=0;
...
create_room() {
   ...
   add_item("bed",
      "Its frame is made of antique mahogany and it is covered "+
      "neatly with a red blanket.\n");
 
   add_item("blanket",
      "It is a completely smooth spread without wrinkle covering "+
   ...
   add_item("altar", break_string(
      "It is a small prayer altar made of the same mahogany "+
...
int my_attack() {
   ...
   if (ravens_summoned_this_reset >= MAX_RAVENS_PER_RESET)
       return 0; 
...
exa_wardrobe() {
   string desc;
   desc="It is a tall ornate cabinet made of dark mahogany used by "+
   ...
   if (robe_here) {
      ...
      if (hiero) {
           this_player()->catch_msg(
                 "First you ask "+QTNAME(hiero)+" to borrow her robe, "+
