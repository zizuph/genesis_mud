...
** 1997/07/10  ?                Last update?
** 2010/09/02  Lavellan         Fixed a whatever -> a/an whatever
*/
inherit "std/object";
...
#define ADJ2 ({                                                        \
        ...
        "red-bearded","black-bearded","brown-bearded","bald",           \
...
#define LOOT ({"armours/ccloak","armours/cmail","armours/cboots",      \
               "armours/crobe","armours/chelmet","weapons/csword",     \
...
void search_done(object Tr, object To, object Tp) {
   ...
   for (i=0; i<numloot; i++) {
      ...
      ldesc=ldesc+({LANG_ADDART(loot[i]->query_short())});
      (loot[i])->move(Tp);
   }
