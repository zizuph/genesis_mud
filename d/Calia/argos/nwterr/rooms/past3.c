...
create_room()
{
   ...
   set_long(break_string(
       ...
       "southern edge of the field, but you may walk to other parts " +
       "of the pasture in the other directions.\n",70));
    ...
    add_item(({"pasture","field"}), break_string(
             ...
             "the fence, and stretches to the west, north and east.\n",70));
    add_item("fence", break_string(
             "A simple wooden fence, enclosing the field you are standing " +
             "in.  It borders the pasture to the south here.\n",70));
 
    reset_room();
 
    /* exits */
    add_exit(ROOM_DIR+"past2","west",0);
    add_exit(ROOM_DIR+"past6","north",0);
    add_exit(ROOM_DIR+"past4","east",0);
}
