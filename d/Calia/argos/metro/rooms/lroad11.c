...
void create_room() {
   ...
   set_long(
       "A set of steep steps lead up to the portico of the grand "+
       "temple to the north as the road passes east-west along the "+
       "edge of the city's commercial district. The smell of hearty "+
       "food drifts from the building to the south, making you feel "+
       "a bit hungry. There is a sign hanging above the entrance.\n");
 
   add_item("building",
       "It is one of the buildings of the commercial district, a "+
       "simple cube of marble. A sign hangs over the entrance.\n");
   add_item("sign",
       "Hanging from a pole over the entrance, the wood sign is "+
       "carved with the letters: THEROS INN.\n");
   add_cmd_item("sign","read","It reads: THEROS INN.\n");
   ...
   add_item("pillars",
       ...
       "sculpted with a ring of perched owls.\n");
   add_item("steps",
       "They lead up to the temple's portico.\n");
 
   /* exits */
   add_exit(THEMTEMP_DIR+"rooms/port1","steps","@@steps");
   add_exit(ROOM_DIR+"inn",    "south",0);
   add_exit(ROOM_DIR+"lroad10","west",0);
   add_exit(ROOM_DIR+"lroad12","east",0);
}
