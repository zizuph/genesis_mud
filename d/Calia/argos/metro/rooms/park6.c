...
void create_room() {
    ...
    set_long(
        "You are standing in the southeastern corner of the park of "+
        ...
        "around the pool to the west and north, while another cuts "+
        "through the hedge around the park to the southeast.\n");
   ...
   add_item(({"path","paths"}),
        "One path leads around the pool to the west and north, "+
   ...
   add_exit(ROOM_DIR+"aroad13","southeast",0);
   add_exit(ROOM_DIR+"park3","west",0);
   add_exit(ROOM_DIR+"park5","north",0);
   reset_room();
}
