...
**  Zima       11/18/95   Modified to press olive oil into new flasks, based
**                        on Gondoran design
**
*/
inherit "/std/room";
...
#define  OLIVES 1
#define  GRAPES 2
#define  AMT_PER_HANDFUL 2000  /* # ml of olive oil from one HF of olives */
...
void make_press() {
   ...
   press->move(THIS);
   tell_room(THIS,
      "There is a large wooden press in the center of the room.\n");
}
...
create_room() {
   ...
   add_cmd_item(({"plaque","instructions"}), "read",
      ...
      "| - You must be holding an oil flask    |\n"+
...
/*   if olives in bin and has flask, fills flask with oil             */
/*   if olives in bin and does not have flask, oil runs on floor      */
int pull(string Parms) {
   object Tp=this_player();
   object Tr=this_object();
   object bottle=present(ARGOS_EMPTY_BOTTLE,Tp);
   object flask =present(OIL_FLASK,Tp);
   int    orig_amt, max_amt, new_amt;
   ...
   if (bin_contents==CONTAINS_GRAPES)
      ...
      if (flask) {
         /* presses olives and has an empty flask, so fill flask */
         write("You hold the flask under the spigot and olive oil "+
               "runs into it.\n");
         tell_room(Tr,QCTNAME(Tp)+" holds a flask under the "+
                      "spigot of the press and olive oil runs into it.\n",Tp);
         orig_amt= (flask->query_amount());
         max_amt = (flask->query_max_amount());
         if (orig_amt >= max_amt) {
            /* flask already completely full */
            tell_room(Tr,
               "The flask is full and the oil runs out onto the floor!\n");
            }
         else {
            new_amt = orig_amt + AMT_PER_HANDFUL;
            if (new_amt > max_amt) {
               new_amt=max_amt;
               tell_room(Tr,"The flask fills up and the remaining oil "+
                            "runs out on the floor!\n");
            }
            flask->set_amount(new_amt);
         }
      }
      else {
         /* presses olives but has no flask, so oil runs on floor */
