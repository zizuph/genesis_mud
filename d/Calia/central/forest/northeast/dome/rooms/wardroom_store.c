 ...
 *
 * It has a special feature in it. Every time the reset_room is
 * called, the stock is checked and supplied if necessary.
 *
 * /Mercade, 7 January 1994
 *
 * If you are going to use this code for your objects (what examples
 * are meant for :-) ), please add a line in your comment header
...
create_room() 
{
    ...
    set_long("This cabinet is used as to store all " +
        "items that are for sale in the shop of the Dwarven race " +
        "guild.\n");
 ...
 * should be in stock.
 *
 * In this particular shop, two bags and four torches will be in the
 ...
 * there are more than four torches when reset_room is called, the
