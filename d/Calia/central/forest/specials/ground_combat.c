// OBJECT:  ground_combat
    ...
    [98-11-15] created by Uhclem from
    [D:\CALIA\CENTRAL\FOREST\SPECIALS\GROUND_C.DOC].

    PURPOSE

    When someone starts a fight in most interior Great Central Forest rooms,
    this object is cloned into the room to add additional combat features.
    There are various local items, such as logs, branches, and puddles, that
    may either trip up the unwary player or be used by an opponent to gain an
    advantage. In general, while combat continues, this object will
    periodically activate one of the special combat features in the room; the
    feature only threatens one player at a time. The period during which the
    feature is active is called the ground combat effect. Depending upon their
    stats and skills, players may become aware of the effect and make use of
    it.
    ...
    as panic or health (except the pit and slope which involce separate rooms
    ...
    indentifying special combat effects, he may be poor in exploiting them.
    However, a team composed of one player who is adept at identifying these
    effects and another who is better at exploiting them may work together to
...
create_object()
    ...
    set_no_show();
    set_name("_ground_combat_control");
    set_short("_ground_combat_control");
    set_long("_ground_combat_control");
