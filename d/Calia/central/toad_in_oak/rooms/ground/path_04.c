...
/* 
    Name        pond_drink
    Purpose     Respond if a player tries to drink from the pond
    Arguments   The rest of the command typed
...
create_tio_ground_room()
{
    ...
    set_long("You stand next to a stream that forms a small pond here, " + 
        "briefly pausing as it makes it's way through the forest of " + 
        "gigantic oak trees. A wooden bridge has been built across the " + 
        "stream here, leading off to the southwest. Thick green grass " +
        "covers the ground between the trees, leading up to a dense " + 
        "thicket of vegetation that blocks passage to the north and west. " +
        "Far above you can see the underside of a village built in the " + 
    ...
    add_item(Grass_Words, grass_describe(({"east", "southeast", "to the bridge"}))); 
    add_item(Pond_Words, "@@pond_describe@@"); 
    add_item(Stream_Words, stream_describe()); 
    add_item(Thicket_Words, thicket_describe("northwest")); 
    ...
    add_exit( TOAD_IN_OAK_ROOMS_GROUND + "path_05", "east"); 
    add_exit( TOAD_IN_OAK_ROOMS_GROUND + "pond_shore", "southeast"); 
    add_exit( TOAD_IN_OAK_ROOMS_GROUND + "path_03", "southwest"); 
}
