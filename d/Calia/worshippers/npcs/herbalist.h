...
/*
 *
 * functions copied and modified from /lib/skill_raise.c for Roger.
...
#define ROGER_S_HERB "_roger_s_herb"
#define ROGER_S_HERB_SOLD "_roger_s_herb_sold"
...
tell_herb(object player)
{
  ...
  TP->add_prop(ROGER_S_HERB,sherb);
}
...
sk_improve(string str)
{
  ...
  {
    if(!TP->query_prop(ROGER_S_HERB))
    {
      tell_herb(TP);
      return 1;
    }
    if(TP->query_prop(ROGER_S_HERB) != ROGER_S_HERB_SOLD)
    {
      TO->command("say Go and bring me some " +TP->query_prop(ROGER_S_HERB) + ".");
...
/* Roger teaches only one level at a time */
  if(str)
  {
    tmp = explode(str, " ");
