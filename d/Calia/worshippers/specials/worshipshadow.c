...
public void
ensure_guild_objects()
{
    // Make sure the guild object and all objects are present
    filter(all_inventory(shadow_who), &->id("worshipper_sceptre"))->remove_object();
    filter(all_inventory(shadow_who), &->id("worshipper_pentacle"))->remove_object();
    filter(all_inventory(shadow_who), &->id("worshipper_wand"))->remove_object();
    filter(all_inventory(shadow_who), &->id("worshipper_dagger"))->remove_object();
    filter(all_inventory(shadow_who), &->id("worshipper_chalice"))->remove_object();

    clone_object("/d/Calia/worshippers/objects/sceptre")->move(shadow_who);
    clone_object("/d/Calia/guilds/elementalists/objs/power_tool_pentacle")->move(shadow_who);
    clone_object("/d/Calia/guilds/elementalists/objs/power_tool_wand")->move(shadow_who);
    clone_object("/d/Calia/guilds/elementalists/objs/power_tool_dagger")->move(shadow_who);
    clone_object("/d/Calia/guilds/elementalists/objs/power_tool_chalice")->move(shadow_who);
}
...
init_lay_shadow(string str)
{
    ...
    set_alarm(5.0, 0.0, ensure_guild_objects);    
}
