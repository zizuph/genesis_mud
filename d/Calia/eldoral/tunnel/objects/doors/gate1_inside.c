...
// Defines
#define OTHER_ROOM     (UNDERGROUND + "undergrnd03_11")
...
create_door()
{
    ...
    set_open_desc("The stone gate to the west is open.\n");
    set_open_mess( ({ "opens the stone gate to the west.\n",
                      "The stone gate to the east swings open.\n" }) );
    set_closed_desc("The stone gate to the west is closed.\n");
    set_close_mess( ({ "closes the stone gate to the west.\n",
                       "The stone gate to the east swings shut.\n" }) );
...
do_open_door(string mess)
{
    ...
    if (query_open())
    {
        ...
        environment(this_object())->add_exit(OTHER_ROOM, "west", 0);
    }
}
