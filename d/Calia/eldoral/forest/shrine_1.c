...
create_room()
{
    set_long("You are inside a small building of some sort. " +
        ...
        "isle, which is mostly some sort of grey granite. The " +
        "building continues a short distance to the south.\n\n");

    create_shrine();

    add_exit(FOREST + "s_forest_15", "north");
    add_exit(FOREST + "shrine_2", "south");
}
