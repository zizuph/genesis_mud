...
create()
{
     int *poss_weak_rooms = ({95, 96, 97, 98, 86, 77, 70, 64, 58}); 
     int *poss_med_rooms = ({105, 106, 107, 108, 109, 110, 99, 100, 
                              87,  88,  78,  79,  71, 65});
     int *poss_el1_rooms = ({ 113, 114, 115, 116 }); 
     int *poss_com_rooms = ({ 117, 118, 119, 120 }); 
     int *poss_el2_rooms = ({ 111, 112, 101, 102 }); 
     int *poss_el3_rooms = ({ 89, 90, 80, 72 });
     ...
     for (i = 0; i < 6; i++) {
         ...
         kroug_rooms += ({ (FOOTHILL_DIR + "fh" + poss_weak_rooms[n]) });
