...
ask_help()
{
    ...
    if (TP->test_bit(THAL_E_ADMIN_DOMAIN, THAL_E_ADMIN_GROUP, THAL_E_ADMIN_BIT))
    {
    ...
    command("smile");
    TP->add_prop(THAL_E_ADMIN_PROP,1);
    command("whisper " + name + " If you can find the zoo "+
        "in the city, come back and ask me for a "+
        "reward.");
    return "";
}
...
ask_reward()
{
    ...
    if (!TP->query_prop(THAL_E_ADMIN_PROP))
    {
        command("whisper "+name+" You want a reward for what?");
        return "";
    }
    if (TP->query_prop(THAL_E_ADMIN_PROP) != 2)
    {
    ...
    TP->set_bit(THAL_E_ADMIN_GROUP, THAL_E_ADMIN_BIT);
    TP->add_exp(THAL_E_ADMIN_EXP);
    TP->catch_msg("You feel more experienced!\n");
    TP->command("save");
    log_file("thal_admin_east", TP->query_real_name() +
            " " + ctime(time())+".\n");
    command("whisper to "+name+" Congratulations. I am glad "+
          "I could help you out.");
    TP->remove_prop(THAL_E_ADMIN_PROP); 
    return "";
}
