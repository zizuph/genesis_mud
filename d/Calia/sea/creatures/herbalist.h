...
/*
 *
 * functions copied and modified from /lib/skill_raise.c for Lobar.
...
#define LOBAR_S_HERB "_lobar_s_herb"
#define LOBAR_S_HERB_SOLD "_lobar_s_herb_sold"
...
tell_herb(object player)
{
  ...
  TP->add_prop(LOBAR_S_HERB,sherb);
}
...
sk_improve(string str)
{
  ...
  {
    if(!TP->query_prop(LOBAR_S_HERB))
    {
      tell_herb(TP);
      return 1;
    }
    if(TP->query_prop(LOBAR_S_HERB) != LOBAR_S_HERB_SOLD)
    {
      TO->command("say Go and bring me some " +TP->query_prop(LOBAR_S_HERB) + ".");
...
/* Lobar teaches only one level at a time */
  if(str)
  {
    tmp = explode(str, " ");
