...
string *battle_emotes = 
   ({ ("emote growls: Curse the Calian Warriors, what a bunch "+
      ...
      "emote flings some slime at you.", 
      "emote claims: Thou distended frog's bladder!", 
...
arm_me()
{
    ...
    this = this_object(); 
    seteuid(getuid(this));
    clone_object(SEA_WEAPONS+"mkroug_knife")->move(this);
    command("wield stilleto");
...
create_monster()
{
    ...
    set_long(capitalize(LANG_ADDART(a2)) + " " + a1 + " water-kroug. " + 
        ...
        "kroug were not drastic changes enough!).  It's scales are quite "+
    ...
    add_act("emote flings slime at you.");
    add_act("emote adjusts its armour.");
    add_act("emote taunts you to fight it.");
    add_act("emote traces a line in front of you with slime dripping from "+
        "it's finger and dares you to cross it.");
