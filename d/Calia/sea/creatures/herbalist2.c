...
my_sell(string str)
{
    ...
    else 
    {
        ...
        command("say 66 years of gathering herbs, but yet another unknown " +
            herb->query_name()+"!");
        command("think");
        command("say I'll pay you "+herb->query_herb_value()/10+" coppers "+
            "for this, "+
            ((TP->query_gender() == G_MALE) ? "sir" : "mylady") +"!");
        price = herb->query_herb_value()/4;
    }
...
not_herb(mixed hf)
{
    ...
    command("give "+OB_NAME(hf[0])+" to "+OB_NAME(hf[1]));
}
...
test_identify(mixed h_arr)
{
    ...
    command("give "+OB_NAME(given_herb)+" to "+OB_NAME(player));
