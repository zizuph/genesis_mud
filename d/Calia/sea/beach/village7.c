...
create_room()
{
    ...
    add_item(({"rocks", "coastline"}),
          "South of here are large grey craggy rocks, a daunting " +
          "barrier to travellers. The rocks have well spaced crevices " +
          "that may allow a skilled climber to <scale> the rocks. To the " +
          "southeast, a cleft appears in the rocks, possibly the " + 
          "result of some recent erosion, revealing a narrow rocky " +
          "path to the southeast.\n"); 

    add_item(({"cleft", "narrow cleft", "path", "narrow path", 
               "rocky path", "narrow rocky path"}),
          "A narrow rocky path leads to the southeast. It's clear that "
        + "the cleft in the rocks is the result of some recent erosion.\n"); 

    add_item(({"craggy rocks", "grey craggy rocks", "grey rocks", "crevices",
               "crevice", "barrier", "daunting barrier", "large rocks",
               "large grey rocks", "large grey craggy rocks",
               "large craggy rocks"}),
          "The large grey craggy rocks seemingly prevent travel to the south. "
        + "The rocks are formed in a way that they do seem to provide "
        + "well spaced crevices that allow a skilled climber to <scale> "
        + "the rocks.\n"); 
...
do_scale(string str)
{
    ...
    if (!parse_command(str, tp, " [the] [large] [grey] [craggy] 'rock' / 'rocks' "))
    {
        return 0;
    }

    // Check to make sure that the person has at least skill level of superior student
    ...
    set_dircmd(str);    
    write("You skillfully scale the rocks and make it to the top.\n");
    tell_room(environment(tp), QCTNAME(tp)+" scales "+
        "the rocks and makes it to the top.\n",tp);
    tp->move_living("M", BEACH+"rocks");
