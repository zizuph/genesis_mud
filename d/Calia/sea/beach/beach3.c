...
reset_room()
{
	...
	if (!present("stick",TO))
	{
    	stick=clone_object("/d/Calia/sea/objects/stick");
    	stick->move(TO);
    }
    reset_herb_room();
}
...
create_room()
{
    ...
    add_exit(BEACH+"beach2","north");
    add_exit(SECTOR_PATH(0, 6, 0)+"wr_0_6_0","west");
    add_exit(BEACH+"beach4","south");
