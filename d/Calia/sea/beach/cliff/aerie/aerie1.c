...
create_room()
{
    ...
    set_long("Situated on a lofty, narrow crag of rock high"+
      ...
      " Before you to the west lies a very large nest,"+
      " supported on a platform of granite boulders"+
      " that make up the surface of the clifftop."+
	...
	"platform"}), "The granite"+
      " boulders have deep cracks between them. They are heaped into"+
      " a raised platform that contains a large bird nest."+
      " The boulders form a stairway that you might climb.\n");
    ...
    add_exit(AERIE + "aerie2", "east", 0, 0);
    add_exit(CLIFF_ROOM + "cliff4_4", "down", 0, 1);
...
{
    ::init();
    add_action(&climb_nest(), "climb");
}

int
climb_nest(string str)
{
    ...
    if (!parse_command(str, this_object(), " [up] / [boulders] "))
    {
	notify_fail("Climb boulders?\n");
	return 0;
    }


    tp->move_living("up", AERIE + "nest", 1,0);
    return 1;
