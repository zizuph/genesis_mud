...
create_water_room()
{
    ...
    set_long("You have entered into the sacred Grove of "+
        ...
        "himself. The grove continues to the northeast and "+
        "the northwest along a small pathway. The road "+
        "continues north to another bridge. Southeast you "+
        "notice the Thalassian Grandstand, where the races "+
        "are held. The Lesser Zone lies around you in all "+
        "other directions.\n");
    add_item(({"bridge","tower","guard tower"}),"The bridge leads off to "+
        "the north into the Citadel of the city. On the top "+
    ...
    add_item(({"grove","sacred grove","Grove of Diabrecho",
        ...
        "It continues off to the northwest and northeast along a "+
        "small pathway.\n");
    add_item(({"path","pathway","small path","small pathway"}),
        ...
        "northeast. Above you, you notice the underside of the "+
    ...
    add_swim_exit(THALC+"brguard_s_1_1","north",0,1);
    add_swim_exit(THALL+"lesser2","west",0,1);
    add_swim_exit(THALL+"lesser5","east",0,1);
    add_swim_exit(THALL+"lesser3","south",0,1);
    add_swim_exit(GROVE+"grove2","northwest",0,1);
    add_swim_exit(GROVE+"grove16","northeast",0,1);
    add_swim_exit(THALL+"lesser1","southwest",0,1);
    add_swim_exit(THALL+"grandstand1","southeast",0,1);
}
