...
create_water_room()
{
    ...
    set_long("You are in the entrance to the guard barracks. Here "+
        ...
        "leads off to the west and there branches to the north and "+
        "south. To the east lies the Lesser Zone of the city.\n");

    add_item("hallway", "The hallway leads to the west and then "+
        "becomes a T-intersection with branches leading to the "+
        "north and south.\n");

    add_swim_exit(THALL+"barrack_w_2","west",0,1);
    add_swim_exit(THALL+"lesser13","east",0,1);
}
