...
create_water_room()
{
    ...
    set_long("You are swimming around in the Lesser Zone of "+
        ...
        "the north, east and southeast. To the south you see the "+
    ...
    add_item(({"road","roads"}),"The roads are constructed of "+
        ...
        "road leads off to the north, east and southeast.\n");

    add_swim_exit(THALL+"lesser25","north",0,1);
    add_swim_exit(THALL+"lesser26","east",0,1);
    add_swim_exit(GROVE+"grove13","south",0,1);
    add_swim_exit(THALL+"lesser28","southeast",0,1);
}
