...
create_water_room()
{
    ...
    set_long("You are swimming around in the Lesser Zone of "+
        ...
        "the northwest, north, west and southwest. Surrounding "+
    ...
    add_item(({"road","roads"}),"The roads are constructed of "+
        ...
        "road leads off to the northwest, north, west and southwest.\n");


    add_swim_exit(THALL+"lesser28","north",0,1);
    add_swim_exit(GROVE+"grove13","northwest",0,1);
    add_swim_exit(THALL+"lesser30","west",0,1);
    add_swim_exit(THALL+"bank","east",0,1);
    add_swim_exit(THALL+"lesser31","southwest",0,1);
}
