...
create_water_room()
{
    ...
    set_long("You are swimming around in the Lesser Zone of "+
        ...
        "the northeast, east and south. To the north you see the "+
    ...
    add_item(({"road","roads"}),"The roads are constructed of "+
        ...
        "road leads off to the northeast, east and south.\n");

    add_swim_exit(GROVE+"grove13","north",0,1);
    add_swim_exit(THALL+"lesser28","northeast",0,1);
    add_swim_exit(THALL+"lesser29","east",0,1);
    add_swim_exit(THALL+"lesser31","south",0,1);
}
