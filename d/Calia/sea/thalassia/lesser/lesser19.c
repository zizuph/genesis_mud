...
create_water_room()
{
    ...
    set_long("You are swimming around in the Lesser Zone of "+
        ...
        "the north, west and northeast. To the east you see the "+
    ...
    add_item(({"road","roads"}),"The roads are constructed of "+
        ...
        "road leads off to the north, west and northeast.\n");

    add_swim_exit(THALL+"lesser18","north",0,1);
    add_swim_exit(THALL+"lesser17","west",0,1);
    add_swim_exit(THALL+"lesser20","northeast",0,1);
    add_swim_exit(GROVE+"grove9","east",0,1);
}
