...
create_water_room()
{
    ...
    add_item(({"road","roads"}),"The roads are constructed of "+
        ...
        "road leads off to the west, southwest, south and southeast.\n");

    add_swim_exit(THALL+"lesser20","west",0,1);
    add_swim_exit(GROVE+"grove9","southwest",0,1);
    add_swim_exit(THALL+"lesser22","south",0,1);
    add_swim_exit(THALL+"lesser23","southeast",0,1);
}
