...
create_water_room()
{
    ...
    set_long("You are swimming around in the Lesser Zone of "+
        ...
        "the east, southwest, south and southeast. Surrounding "+
    ...
    add_item(({"wall","silver wall","north","northwest",
        "north wall","northwest wall"}),"This shiny metal surrounds the "+
    ...
    add_item(({"road","roads"}),"The roads are constructed of "+
        ...
        "road leads off to the east, southwest, south and southeast.\n");

    add_swim_exit(THALL+"lesser20","east",0,1);
    add_swim_exit(THALL+"lesser17","southwest",0,1);
    add_swim_exit(THALL+"lesser19","south",0,1);
    add_swim_exit(GROVE+"grove9","southeast",0,1);
}
