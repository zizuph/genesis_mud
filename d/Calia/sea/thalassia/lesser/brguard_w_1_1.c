...
create_water_room()
{
    ...
    add_item("road","The road travels west into the Major "+
        "Zone of the city and east into the Lesser Zone.\n");
    add_item(({"major zone","Major Zone"}),"West from "+
        "here is the Major Zone of the city. This section "+
        "of the city looks to be the largest of the three sections "+
        "of the city.\n");
    add_item(({"lesser zone","Lesser Zone"}),"East from "+
    ...
    add_swim_exit(THALM+"major17","west",0,1);
    add_swim_exit(THALL+"lesser12","east",0,1);
    add_swim_exit(THALL+"brguard_w_2_1","up",0,1);
}
