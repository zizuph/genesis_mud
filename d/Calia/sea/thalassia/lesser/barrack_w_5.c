...
create_water_room()
{
    ...
    set_long("You are in the hallway in the guard barracks. Here "+
        ...
        "the north. To the east lies the mess hall, while to the west lies "+
        "one of the sleeping chambers.\n");

    add_item("hallway", "The hallway leads off to the south.\n");
    add_item("mess hall", "The mess hall lies off to the east.\n");
    ...
    add_swim_exit(THALL+"barrack_w_4","west",0,1);
    add_swim_exit(THALL+"barrack_w_6","east",0,1);
    add_swim_exit(THALL+"barrack_w_2","south",0,1);
}
