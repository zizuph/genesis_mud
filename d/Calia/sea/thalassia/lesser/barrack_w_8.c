...
create_water_room()
{
    ...
    set_long("You are in the hallway in the guard barracks. Here "+
        ...
        "the north. To the east lies the pub, while to the west lies "+
        "one of the sleeping chambers.\n");

    add_item("hallway", "The hallway leads off to the north.\n");
    add_item("pub", "The pub lies off to the east. You could probably "+
        "find something there to eat and drink.\n");
    add_item(({"sleeping chamber", "sleeping chambers","chamber","chambers"}),
        "To the west lies one of the sleeping chambers here in the guard "+
        "barracks. The guards go there to sleep when they are not on duty.\n");

    add_swim_exit(THALL+"barrack_w_2","north",0,1);
    add_swim_exit(THALL+"barrack_w_7","west",0,1);
    add_swim_exit(THALL+"barrack_w_9","east",0,1);
}
