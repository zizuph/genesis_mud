...
create_water_room()
{
    ...
    set_long("You are swimming around in the Lesser Zone of "+
        ...
        "you on the southwestern side you see the silver wall that "+
    ...
    add_item("sign","The sign reads 'Medical Facilities'.\n");
    ...
    add_item(({"wall","silver wall","southwest",
        "southwest wall"}),"This shiny metal surrounds the "+
    ...
    add_item(({"road","roads"}),"The roads are constructed of "+
        ...
        "road leads off to the north, northeast, east and southeast.\n");


    add_swim_exit(THALL+"lesser12","north",0,1);
    add_swim_exit(THALL+"hospital","west",0,1);
    add_swim_exit(THALL+"lesser11","east",0,1);
    add_swim_exit(GROVE+"grove5","northeast",0,1);
    add_swim_exit(THALL+"lesser9","southeast",0,1);
}
...
do_read(string str)
{
    ...
    write("The sign reads: 'Medical Facilities'\n");
    return 1;
}
