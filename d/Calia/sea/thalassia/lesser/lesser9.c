...
create_water_room()
{
    ...
    add_item(({"road","roads"}),"The roads are constructed of "+
        ...
        "road leads off to the northwest, north and southeast.\n");

    add_swim_exit(THALL+"lesser11","north",0,1);
    add_swim_exit(THALL+"lesser10","northwest",0,1);
    add_swim_exit(THALL+"lesser8","southeast",0,1);
}
