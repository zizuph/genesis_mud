...
create_water_room()
{
    ...
    set_long("You are swimming around in the Major Zone of "+
        ...
        "the north, west, northwest, south and southwest. To the "+
        "east you see the bridge leading out of the Major Zone "+
      ...
      add_item(({"bridge","tower","guard tower"}),"The bridge leads off to "+
        "the east into the entrance area of the city. On the top "+
        "of the bridge sits the three storey guard tower.\n");

    add_item(({"wall","bronze wall","northeast","southeast",
    ...
    add_swim_exit(THALM+"major19","north",0,1);
    add_swim_exit(THALM+"major18","northwest",0,1);
    add_swim_exit(THALM+"major16","west",0,1);
    add_swim_exit(THALM+"major14","southwest",0,1);
    add_swim_exit(THALM+"major15","south",0,1);
    add_swim_exit(THALL+"brguard_w_1_1","east",0,1);
}
