...
create_water_room()
{
    ...
    set_long("You are swimming around in the Major Zone of "+
        ...
        "the northwest, northeast and southwest. Surrounding "+
    ...
    add_item(({"road","roads"}),"The roads are constructed of "+
        ...
        "road leads off to the northwest, northeast "+
        "and southwest.\n");

    add_item("statue","This statue is very large. It is "+
        "carved out of some sort of glowing rock that seems to "+
        "pulse with power. There is incredible heat emanating from "+
        "it. It depicts a man rising out of the flaming ground, "+
        "hands rising above his head. It measures about eight feet "+
        "across at the base of the flaming area, and stands about "+
        "six feet high at its highest point. The eyes of the statue "+
        "glow bright yellow.\n");

    add_swim_exit(THALM+"major47","northeast",0,1);
    add_swim_exit(THALM+"mural19","northwest",0,1);
    add_swim_exit(THALM+"major7","southwest",0,1);
...
do_pray(string str)
{
    ...
    if ((!str) || (lower_case(str) != "to lord pyros"))
    {
        NF("Pray to whom? Lord Pyros perhaps?\n");
        return 0;
    }
    ...
    if (is_worship(TP))
    {
        ...
        if (diety == FIRE_FLAG && 
            !(TP->query_prop(LIVE_I_PRAYED_AT_PYROS_STATUE)))
        {
            write("You begin to pray to Lord Pyros with all of your "+
                ...
                "again. You look up at the statue of Lord Pyros and "+
                "notice it has changed, Lord Pyros is alive! He glances "+
            ...
            say(QCTNAME(TP)+" prays before the statue. "+capitalize(HIS(TP))+
                ...
                "a few moments.\n");
            TP->add_prop(LIVE_I_PRAYED_AT_PYROS_STATUE,1);
            return 1;
        }

        write("You begin to pray to Lord Pyros. You feel a presence "+
            ...
            "statue of Lord Pyros blink. A smile comes across the face "+
    ...
    if (is_spirit(TP))
    {
        ...
        if (TP->query_prop(LIVE_I_ON_SPIRIT_GURU_QUEST) && diety == FIRE_FLAG)
        {
            if (!present("_pyros_triangle_",TP))
            {
                write("You begin to pray to Lord Pyros with all of your "+
                    "heart and soul. You quickly become dizzy as the room "+
                    ...
                    "of Lord Pyros and notice it has changed, Lord Pyros "+
                    ...
                    "spirit must be freed from its prison inside of the "+
                ...
                triangle = clone_object(SPIRIT_OBJECTS+"pyros_triangle");
                triangle->move(TP,1);
                return 1; 
            }
            else
            {
                write("You begin to pray to Lord Pyros. You feel a "+
    ...
    if (is_cleric(TP))
    {
        ...
        if (TP->query_prop(LIVE_I_ON_CLERIC_GURU_QUEST) && cdiety == "fire")
        {
            if (!present("_pyros_triangle_",TP))
            {
                write("You begin to pray to Lord Pyros with all of your "+
                ...
                say(QCTNAME(TP)+" prays before the statue. "+
                    capitalize(HIS(TP))+" eyes glaze over but quickly "+
                    "return to normal after just a few moments.\n");
                triangle = clone_object(ELEMENTALIST_OBJS+"pyros_triangle");
                triangle->move(TP,1);
                return 1; 
            }
            else
