...
create_water_room()
{
    ...
    add_item(({"mural","colourful mural"}),"The mural is a depiction "+
        ...
        "horrible blackened creatures being transformed into "+
        "slimier versions of themselves, with dark eyes, scales on "+
        "their bodies, and a large fishlike tail. There appear to be "+
        "other murals to the northwest and southeast of here.\n");

    add_item(({"garden","gardens"}),"The gardens have many varieties of "+
        "plants that you do not recognize. They are very colourful "+
        "and plentiful.\n");

    add_swim_exit(THALM+"major34","northeast",0,1);
    add_swim_exit(THALM+"mural12","northwest",0,1);
    add_swim_exit(THALM+"mural14","southeast",0,1);
