...
create_water_room()
{
    ...
    set_long("You are swimming around in the Major Zone of "+
        ...
        "the northwest, north, west and east. A few statues "+
    ...
    add_item(({"road","roads"}),"The roads are constructed of "+
        ...
        "road leads off to the northwest, north, west "+
        "and east.\n");

    add_swim_exit(THALM+"major30","north",0,1);
    add_swim_exit(THALM+"major28","northwest",0,1);
    add_swim_exit(THALM+"major29","west",0,1);
    add_swim_exit(THALM+"major32","east",0,1);
}
