...
create_water_room()
{
    ...
    add_item(({"mural","colourful mural"}),"The mural is a depiction "+
        ...
        "two beautiful cities, each on the peak of a large mountain. The "+
        "gates are massive, made of gold, and face each other. South of "+
        "the peaks there is a large volcano. The sky above the cities is "+
        "bright blue with fluffy white clouds scattered throughout it. It "+
        "looks like an amazing place to live. There appears to be "+
        "another mural to the northwest of here.\n");

    add_swim_exit(THALM+"major8","southwest",0,1);
    add_swim_exit(THALM+"mural2","northwest",0,1);
}
