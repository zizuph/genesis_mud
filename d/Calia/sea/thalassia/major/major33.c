...
create_water_room()
{
    ...
    set_long("You are swimming around in the Major Zone of "+
        ...
        "the northwest, southwest and southeast. Surrounding "+
    ...
    add_item(({"wall","bronze wall","northeast","north",
        "northeast wall","north wall"}),"This shiny metal surrounds the "+
    ...
    add_item(({"road","roads"}),"The roads are constructed of "+
        ...
        "road leads off to the northwest, southwest "+
        "and southeast.\n");

    add_swim_exit(THALM+"major32","northwest",0,1);
    add_swim_exit(THALM+"mural12","southwest",0,1);
    add_swim_exit(THALM+"major34","southeast",0,1);
}
