...
create_water_room()
{
    ...
    set_long("You are swimming around in the Major Zone of "+
        ...
        "the northwest, northeast and southeast. Surrounding "+
    ...
    add_item("statue","This statue is very large. It is carved "+
        "out of a golden sandstone that vibrates with energy. It "+
        "depicts a woman standing atop of a newly risen mountain. "+
        "It measures about ten feet across at the base of the "+
        "mountain, and stands almost fifteen feet high at it's highest "+
        "point. The eyes of the statue shine like diamonds.\n");

    add_swim_exit(THALM+"major10","northwest",0,1);
    add_swim_exit(THALM+"mural2","northeast",0,1);
    add_swim_exit(THALM+"major8","southeast",0,1);
...
do_pray(string str)
{
    ...
    if ((!str) || (lower_case(str) != "to lady gu"))
    {
        NF("Pray to whom? Lady Gu perhaps?\n");
        return 0;
    }
    ...
    if (is_worship(TP))
    {
        ...
        if (diety == EARTH_FLAG && 
            !(TP->query_prop(LIVE_I_PRAYED_AT_GU_STATUE)))
        {
            write("You begin to pray to Lady Gu with all of your "+
                ...
                "again. You look up at the statue of Lady Gu and "+
                "notice it has changed, Lady Gu is alive! She glances "+
            ...
            say(QCTNAME(TP)+" prays before the statue. "+capitalize(HIS(TP))+
                ...
                "a few moments.\n");
            TP->add_prop(LIVE_I_PRAYED_AT_GU_STATUE,1);
            return 1;
        }

        write("You begin to pray to Lady Gu. You feel a presence "+
            ...
            "statue of Lady Gu blink. A smile comes across the face "+
    ...
    if (is_spirit(TP))
    {
        ...
        if (TP->query_prop(LIVE_I_ON_SPIRIT_GURU_QUEST) && diety == EARTH_FLAG)
        {
            if (!present("_gu_triangle_",TP))
            {
                write("You begin to pray to Lady Gu with all of your "+
                    "heart and soul. You quickly become dizzy as the room "+
                ...
                triangle = clone_object(SPIRIT_OBJECTS+"gu_triangle");
                triangle->move(TP,1);
                return 1; 
            }
            else
            {
                write("You begin to pray to Lady Gu. You feel a "+
    ...
    if (is_cleric(TP))
    {
        ...
        if (TP->query_prop(LIVE_I_ON_CLERIC_GURU_QUEST) && cdiety == "earth")
        {
            if (!present("_gu_triangle_",TP))
            {
                write("You begin to pray to Lady Gu with all of your "+
                ...
                say(QCTNAME(TP)+" prays before the statue. "+
                    capitalize(HIS(TP))+" eyes glaze over but quickly "+
                    "return to normal after just a few moments.\n");
                triangle = clone_object(ELEMENTALIST_OBJS+"gu_triangle");
                triangle->move(TP,1);
                return 1; 
            }
            else
