...
create_water_room()
{
    ...
    set_long("You are swimming around in the Major Zone of "+
        ...
        "the north, northeast, east and southwest. Surrounding "+
        "you on the southeasternern side you see the bronze wall that "+
    ...
    add_item(({"road","roads"}),"The roads are constructed of "+
        ...
        "road leads off to the north, northeast "+
        "and southwest.\n");

    add_swim_exit(THALM+"major42","north",0,1);
    add_swim_exit(THALM+"major43","northeast",0,1);
    add_swim_exit(THALM+"major45","southwest",0,1);
}
