...
create_water_room()
{
    ...
    add_item("road","The road travels north into the Major "+
        "Zone of the city and south back out towards the "+
        "main gates.\n");
    add_item(({"major zone","Major Zone"}),"North from "+
        "here is the Major Zone of the city. This section "+
        "of the city looks to be the largest of the three sections "+
        "of the city.\n");
    add_item(({"gates","main gates"}),"South from here are the "+
        "city guard towers and further beyond them is the tunnel "+
        "leading to the main city gates.\n");
    ...
    add_swim_exit(THALM+"major3","north",0,1);
    add_swim_exit(THAL+"path4","south",0,1);
    add_swim_exit(THALM+"brguard_s_2_1","up",0,1);
}
