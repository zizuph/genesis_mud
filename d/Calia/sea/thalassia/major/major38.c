...
create_water_room()
{
    ...
    set_long("You are swimming around in the Major Zone of "+
        ...
        "the north, east, southeast and south. A few statues "+
    ...
    add_item(({"road","roads"}),"The roads are constructed of "+
        ...
        "road leads off to the north, east, southeast and south.\n");

    add_swim_exit(THALM+"major37","north",0,1);
    add_swim_exit(THALM+"major39","east",0,1);
    add_swim_exit(THALM+"major40","south",0,1);
    add_swim_exit(THALM+"major41","southeast",0,1);
}
