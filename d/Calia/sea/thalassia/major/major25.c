...
create_water_room()
{
    ...
    set_long("You are swimming around in the Major Zone of "+
        ...
        "the northeast, east, southwest and southeast. Surrounding "+
    ...
    add_item(({"road","roads"}),"The roads are constructed of "+
        ...
        "road leads off to the northeast, east, southwest "+
        "and southeast.\n");

    add_swim_exit(THALM+"major26","northeast",0,1);
    add_swim_exit(THALM+"major27","east",0,1);
    add_swim_exit(THALM+"mural10","southeast",0,1);
    add_swim_exit(THALM+"major24","southwest",0,1);
}
