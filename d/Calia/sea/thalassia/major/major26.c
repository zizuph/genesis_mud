...
create_water_room()
{
    ...
    add_item(({"road","roads"}),"The roads are constructed of "+
        ...
        "road leads off to the east, southwest, south "+
        "and southeast.\n");

    add_swim_exit(THALM+"major28","east",0,1);
    add_swim_exit(THALM+"major25","southwest",0,1);
    add_swim_exit(THALM+"major27","south",0,1);
    add_swim_exit(THALM+"major29","southeast",0,1);
}
