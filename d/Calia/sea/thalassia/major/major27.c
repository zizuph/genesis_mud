...
create_water_room()
{
    ...
    set_long("You are swimming around in the Major Zone of "+
        ...
        "the north, northeast, west and east. A few statues "+
    ...
    add_item(({"road","roads"}),"The roads are constructed of "+
        ...
        "road leads off to the north, northeast, west "+
        "and east.\n");

    add_swim_exit(THALM+"major26","north",0,1);
    add_swim_exit(THALM+"major28","northeast",0,1);
    add_swim_exit(THALM+"major25","west",0,1);
    add_swim_exit(THALM+"major29","east",0,1);
}
