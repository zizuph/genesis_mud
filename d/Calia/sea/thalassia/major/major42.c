...
create_water_room()
{
    ...
    set_long("You are swimming around in the Major Zone of "+
        ...
        "the north, northeast, east and south. A few statues "+
    ...
    add_item(({"road","roads"}),"The roads are constructed of "+
        ...
        "road leads off to the north, northeast, east and south.\n");

    add_swim_exit(THALM+"major40","north",0,1);
    add_swim_exit(THALM+"major41","northeast",0,1);
    add_swim_exit(THALM+"major43","east",0,1);
    add_swim_exit(THALM+"major44","south",0,1);
}
