...
create_water_room()
{
    ...
    set_long("You are swimming around in the Major Zone of "+
        ...
        "the north, west, southwest and south. A few statues "+
    ...
    add_item(({"road","roads"}),"The roads are constructed of "+
        ...
        "road leads off to the north, west, southwest and south.\n");

    add_swim_exit(THALM+"major20","north",0,1);
    add_swim_exit(THALM+"major18","west",0,1);
    add_swim_exit(THALM+"major16","southwest",0,1);
    add_swim_exit(THALM+"major17","south",0,1);
}
