...
create_water_room()
{
    ...
    add_item(({"mural","colourful mural"}),"The mural is a depiction "+
        ...
        "two mountains setting down into the sea forming two half-moon "+
        "shaped islands facing each other, each with a beautiful city "+
        "on them. There appear to be other murals to the northeast and "+
        "southwest of here.\n");
    ...
    add_swim_exit(THALM+"major22","northwest",0,1);
    add_swim_exit(THALM+"mural8","northeast",0,1);
    add_swim_exit(THALM+"mural6","southwest",0,1);
