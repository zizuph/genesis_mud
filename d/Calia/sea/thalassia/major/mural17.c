...
create_water_room()
{
    ...
    add_item(({"mural","colourful mural"}),"The mural is a depiction "+
        ...
        "people praying before a large statue. The statue appears "+
        "to be in front of a bronze wall. There appears to be other "+
        "murals to the northeast and southwest of here.\n");

    add_item(({"garden","gardens"}),"The gardens have many varieties of "+
        "plants that you do not recognize. They are very colourful "+
        "and plentiful.\n");

    add_swim_exit(THALM+"major46","southeast",0,1);
    add_swim_exit(THALM+"mural16","northeast",0,1);
    add_swim_exit(THALM+"mural18","southwest",0,1);
