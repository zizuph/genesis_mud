...
create_water_room()
{
    ...
    add_item(({"mural","colourful mural"}),"The mural is a depiction "+
        ...
        "a large grandstand overlooking a race track. On the track "+
        "you see animals racing, though they do not look like normal "+
        "animals. Instead they appear to be some type of elephants with "+
        "a large tails where their back legs should be. There appear to "+
        "be other murals to the northeast and southwest of here.\n");

    add_swim_exit(THALM+"major48","southeast",0,1);
    add_swim_exit(THALM+"mural18","northeast",0,1);
    add_swim_exit(THALM+"mural20","southwest",0,1);
