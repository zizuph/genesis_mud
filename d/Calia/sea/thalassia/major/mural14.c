...
create_water_room()
{
    ...
    add_item(({"mural","colourful mural"}),"The mural is a depiction "+
        ...
        "the men and women of this city rebuilding it to more suit the "+
        "new habitat is lies in, namely underwater. It shows three circles, "+
        "the largest on the outside, a medium sized one inside of that, and "+
        "the smallest inside of that one. There are many buildings in each "+
        "circle except the middle one, which seems to only have four very "+
        "large buildings surrounding a central fountain. There appear to "+
    ...
    add_swim_exit(THALM+"major35","northeast",0,1);
    add_swim_exit(THALM+"mural13","northwest",0,1);
    add_swim_exit(THALM+"mural15","southeast",0,1);
