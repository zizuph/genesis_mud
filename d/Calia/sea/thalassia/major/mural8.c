...
create_water_room()
{
    ...
    add_item(({"mural","colourful mural"}),"The mural is a depiction "+
        ...
        "a beautiful woman and a large ugly creature battling on a "+
        "land that looks strangely familiar. Below the picture in a "+
        "language you can understand are the words `Lady Caliana and "+
        "Lord Kroug'. There appear to be other murals to the northeast "+
    ...
    add_swim_exit(THALM+"major23","northwest",0,1);
    add_swim_exit(THALM+"mural9","northeast",0,1);
    add_swim_exit(THALM+"mural7","southwest",0,1);
}
