...
make_monster(int i)
{
    ...
    tell_room(TO, QCTNAME(guard[i]) + " arrives and guards the door "+
        "to the Royal Guard sleeping quarters.\n");
}
...
create_water_room()
{
    ...
    set_long("You are inside of the Royal Palace of "+
        ...
        "palace to the east. West from here is the Royal "+
        "Baths while north from here you see the entrance to "+
        "the Royal Guard sleeping quarters. The ceiling itself "+
    ...
    add_item(({"baths", "royal baths"}), "To the west you see the "+
        "Royal Baths.\n");
    add_item(({"quarters","sleeping quarters"}), "To the north lies "+
        "the Royal Guard sleeping quarters, where the Royal Guard "+
        "rest when not on duty protecting the palace.\n");
    ...
    add_swim_exit(THALC+"palace_2_8","west",0,1);
    add_swim_exit(THALC+"palace_2_12","east",0,1);
