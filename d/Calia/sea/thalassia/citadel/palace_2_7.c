...
create_water_room()
{
    ...
    set_long("You are inside of the Royal Palace of "+
        ...
        "palace to the west. East from here is the Royal "+
        "Baths while north from here you see the entrance to "+
        "the Royal Bedroom. The ceiling itself "+
    ...
    add_item(({"baths", "royal baths"}), "To the east you see the "+
        "Royal Baths.\n");
    add_item(({"bedroom","royal bedroom"}), "To the north lies "+
        "the Royal Bedroom, where the King rests when not on duty "+
    ...
    add_swim_exit(THALC+"palace_2_4","west",0,1);
    add_swim_exit(THALC+"palace_2_8","east",0,1);
