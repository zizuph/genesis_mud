...
create_water_room()
{
    ...
    set_long("You are inside of the Royal Palace of "+
        ...
        "palace to the north and west. The ceiling itself "+
        "glows brightly, giving off a bright light, "+
        "illuminating the entire area. The walls seem like "+
        "they are made of pure gold and they shine brightly "+
        "in the light. There is a stairway here leading down "+
        "to the first floor of the palace.\n");

    add_item(({"stairway", "stairs"}), "There is a stairway "+
        "leading down to the first floor of the palace here. "+
    ...
    add_swim_exit(THALC+"palace_2_12","north",0,1);
    add_swim_exit(THALC+"palace_2_14","west",0,1);
    add_swim_exit(THALC+"palace_1_13","down",0,1);
}
