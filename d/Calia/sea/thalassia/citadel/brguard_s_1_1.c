...
create_water_room()
{
    ...
    set_long("You are on a bridge of sorts between two "+
        "sections of the city separated by a circular "+
    ...
    add_item("road","The road travels north into the Citadel "+
        "of the city and south into the Lesser Zone.\n");
    add_item(({"citadel","Citadel"}),"North from "+
        "here is the Citadel of the city. This section "+
        "of the city looks to be the smallest of the three sections "+
        "of the city.\n");
    add_item(({"lesser zone","Lesser Zone"}),"South from "+
    ...
    add_swim_exit(THALC+"citadel2","north",0,1);
    add_swim_exit(GROVE+"grove1","south",0,1);
    add_swim_exit(THALC+"brguard_s_2_1","up",0,1);
}
