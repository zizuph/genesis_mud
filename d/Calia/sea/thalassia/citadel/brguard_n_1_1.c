...
create_water_room()
{
    ...
    add_item("road","The road travels north into the Lesser "+
        "Zone of the city and south into the Citadel.\n");
    add_item(({"citadel","Citadel"}),"South from "+
    ...
    add_swim_exit(GROVE+"grove9","north",0,1);
    add_swim_exit(THALC+"citadel10","south",0,1);
    add_swim_exit(THALC+"brguard_n_2_1","up",0,1);
}
