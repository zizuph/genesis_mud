...
create_water_room()
{
    ...
    add_item(({"passage","passageway"}),"The passageway appears to "+
        ...
        "to the north while far off to the southeast you can see "+
        "the gates leading back out to the sea.\n");
    ...
    add_item(({"ring","rings","three rings","seperate rings",
        ...
        "last.\n");
    add_item("gates","You can't make out any details about the "+
        "gates from here.  You will have to swim back to the "+
        "southeast to see them properly.\n");

    add_swim_exit(THAL+"path2","northwest",0,1);
    add_swim_exit(THAL+"gates","southeast",0,1);
}
