...
#define HELP_FILE_DIR "/d/Calia/palace/texts/"
...
trainroom_long()
{
     if (is_calian(this_player()))
        return ("Being a Calian, you can go up or down from here, unless " +
...
{ 
    ...
    set_long("This is the Calian warrior's guild.  " +
        "If you wish to join, you have to type <promise trust>.  " +
        "In return for the benefits granted, you will have to invest " +
        "a quite high amount of the experience that you obtain " +
        "while you are a guild member.  " + 
        "You can leave whenever you like, but you will lose a significant " +
        "portion of your accumulated combat experience; just type " +
        "<break trust>.  " +
...
/* 
 * Function:     calian_trainee_sk_hook_limit 
 * Arguments:    int sknum - the skill number
 *               string skname - the skill name
 *               int maxlevel - maximum skill level trainable
 * Description:  Writes a message indicating that a Calian trainee 
 *               may not train a skill as high as has been attempted. 
 * Returns:      int - 0
 */ 
private int 
calian_trainee_sk_hook_limit(int sknum, string skname, int maxlevel) 
{ 
    if (maxlevel) 
    { 
        notify_fail("The maximum level that a trainee may advance to " + 
              "in " + skname + " is " + sk_rank(maxlevel) + ".\n"); 
        return 0; 
    } 

    notify_fail("Trainees may not learn " + skname + ".\n"); 
    return 0; 
} 
...
/*
 *  if (!level && (verb == "learn") && 
 *      ((snum == SS_MAUL) || (snum == SS_MAUL_KNOWLEDGE))) 
 *  { 
 *      if (this_player()->query_skill(REWARD) < 1) 
 *      { 
 *          write("You have to have been rewarded by the Council " +
 *                "before you can learn maul.\n"); 
 *          return 1; 
 *      } 
 *  } 
 */ 
    ...
    if (IS_CALIAN_TRAINEE(this_player())) 
    { 
        if (snum == SS_SWARM || snum == SS_SWARM_KNOWLEDGE)
        { 
            if ((level + steps) > CALIAN_TRAINEE_MAX_SWARM) 
            { 
                return calian_trainee_sk_hook_limit(SS_SWARM, "swarm", 
                           CALIAN_TRAINEE_MAX_SWARM); 
            } 
        } 
        else if (snum == SS_MAUL || snum == SS_MAUL_KNOWLEDGE) 
        { 
            if ((level + steps) > CALIAN_TRAINEE_MAX_MAUL) 
            { 
                return calian_trainee_sk_hook_limit(SS_MAUL, "maul", 
                           CALIAN_TRAINEE_MAX_MAUL); 
            } 
        } 
        else if (snum == SS_MOVE_BEHIND) 
        { 
            if ((level + steps) > CALIAN_TRAINEE_MAX_MB)
            { 
                return calian_trainee_sk_hook_limit(SS_MOVE_BEHIND, 
                           "move behind", CALIAN_TRAINEE_MAX_MB); 
            } 
        } 
        else if (snum == SS_WEP_SWORD) 
        { 
            if ((level + steps) > CALIAN_TRAINEE_MAX_SWORD)
            { 
                return calian_trainee_sk_hook_limit(SS_WEP_SWORD, 
                           "sword", CALIAN_TRAINEE_MAX_SWORD); 
            } 
        } 
        else if (snum == SS_WEP_POLEARM) 
        { 
            if ((level + steps) > CALIAN_TRAINEE_MAX_POLEARM)
            { 
                return calian_trainee_sk_hook_limit(SS_WEP_POLEARM, 
                           "polearm", CALIAN_TRAINEE_MAX_POLEARM); 
            } 
        } 
        else if (snum == SS_2H_COMBAT) 
        { 
            if ((level + steps) > CALIAN_TRAINEE_MAX_THC)
            { 
                return calian_trainee_sk_hook_limit(SS_2H_COMBAT, 
                           "two handed combat", CALIAN_TRAINEE_MAX_THC); 
            } 
        } 
        else if (snum == SS_DEFENCE) 
        { 
            if ((level + steps) > CALIAN_TRAINEE_MAX_DEFENCE)
            { 
                return calian_trainee_sk_hook_limit(SS_DEFENCE, 
                           "defence", CALIAN_TRAINEE_MAX_DEFENCE); 
            } 
        } 
        else if (snum == SS_PARRY) 
        { 
            if ((level + steps) > CALIAN_TRAINEE_MAX_PARRY)
            { 
                return calian_trainee_sk_hook_limit(SS_PARRY, 
                           "parry", CALIAN_TRAINEE_MAX_PARRY); 
            } 
        } 
    } 
...
/* Action function to handle an application for the Calian guild */
int
apply()
{
    ...
    if (RECRUITER_CODE->query_applications_blocked()) {
        write("Sorry, the guild is not currently accepting applications, " +
              "try again at a later date.\n"); 
        return 1;
    } 

    if (RECRUITER_CODE->query_level_block(tp)) {
        write("Sorry, to make an application here your level " +
              "must be at least \"" + 
    ...
    tell_object(tp, "Your application has been officially registered.\n");
    return 1;
}


/* Action function to withdraw an application */ 
int
withdraw(string str)
{
    string n; 
    ...
    if (str != "application") {
        notify_fail("Withdraw application?\n"); 
        return 0; 
    }  

    n = this_player()->query_real_name(); 
    if (!(RECRUITER_CODE->is_applicant(n))) {
        notify_fail("You have not applied!\n");
        return 0;
    }

    RECRUITER_CODE->rem_app(n); 
    write("Ok.\n"); 
    return 1;
}
