...
cteam(string str)
{
    ...
    { 
        notify_fail("Trainees can't do that.\n"); 
        return 0; 
    } 
...
calian_title(string str)
{
    ...
    if (IS_CALIAN_TRAINEE(tp)) 
    { 
        notify_fail("That feature is not available to trainees.\n"); 
        return 0; 
    } 
...
*/ 
     ...
     if (!sizeof(l)) { 
         tell_object(tp, 
             "There are currently no applicants in the realms.\n");
         return 1;
     } 

     tell_object(tp, "The following applicants are in the realms: " + 
                     COMPOSITE_WORDS(l) + ".\n");
     return 1;
}
