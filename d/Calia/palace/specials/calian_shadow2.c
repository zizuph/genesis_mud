...
#include "defs.h"
inherit (CRPALACE_SPECIALS + "calian_specials");
...
int is_calian(object tp) { return IS_CALIAN_WARRIOR(tp); } 
...
/* 
 * Function:     set_calian_trainee 
 * Description:  Sets whether the shadowed player is a Calian trainee. 
 *               Trainees are not yet "full Calians" e.g. in the 
 *               skills they can train and the rooms they can enter. 
 * Arguments:    int trainee - if non-zero, set to trainee, otherwise 
 *                   set to non-trainee. 
 * Returns:      int - 1 if successful, 0 if not. 
 */ 
int
set_calian_trainee(int trainee)
{
    ...
    { 
        ...
        if (!(shadow_who->remove_autoshadow(CRPALACE_SPECIALS + 
                                            "calian_shadow:"))) 
            return 0; 

        if (!(shadow_who->add_autoshadow(CRPALACE_SPECIALS + 
                                         "calian_shadow:1"))) 
        { 
            shadow_who->add_autoshadow(CRPALACE_SPECIALS + 
                                       "calian_shadow:"); 
            return 0; 
        } 

        shadow_who->set_guild_pref(SS_OCCUP, CALIAN_TRAINEE_TAX); 
        is_trainee = 1; 
        return 1; 
    } 
    else 
    { 
        ...
        if (!(shadow_who->add_autoshadow(CRPALACE_SPECIALS + 
                                         "calian_shadow:0"))) 
        { 
            shadow_who->add_autoshadow(CRPALACE_SPECIALS + "calian_shadow:"); 
            return 0; 
        } 

        shadow_who->set_guild_pref(SS_OCCUP, GUILD_TAX); 
        is_trainee = 0; 
        return 1; 
    } 
} 
