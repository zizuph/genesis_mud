...
init()
{
    ...
    // This is the old medallion. Replace it with the new one.
    set_alarm(0.0, 0.0, replace_old_medallion);
}

#define NEW_MEDALLION "/d/Calia/guilds/calians/objects/calian_medallion"

/*
 * Function     : replace_old_medallion
 * Description  : Removes the current medallion and moves the medallion
 *                of the new guild over to the inventory.
 * Arguments    : none
 * Returns      : nothing
 */
public void
replace_old_medallion()
{
    object medallion;

    seteuid(getuid()); 

    medallion = clone_object(NEW_MEDALLION);
    medallion->move(environment(this_object()), 1);
    
    // No need to remove_object because calian medallions
    // automatically delete other medallions during enter_env    
}
