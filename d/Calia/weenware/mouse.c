 ...
 *          [99-10-01] include and "home" changed by Uhclem
 *          [99-10-25] eat_the_fuzzie and enter_env added by Uhclem
 *
...
#include "/d/Calia/central/toad_in_oak/animals/defs.h"
...
/*  Name:       eat_the_fuzzie()
    Purpose:    get and eat a cheese fuzzie, if available
    Arguments:  none
    Returns:    nothing
*/
void
eat_the_fuzzie()
{
    ...
    if (fuzzie = present("cheese fuzzie", environment()))
    {
        if (fuzzie->num_heap() > 1)
        {
            fuzzie->set_heap_size(fuzzie->num_heap() - 1); 
        }
        else
        {        
            fuzzie->remove_object();
        }
        command("emote enjoys a cheese fuzzie snack.");
    }
    else
    {
        command("emote prods you with " + query_possessive() + " tusks in " +
            query_possessive() + " search for food."); 
    }

    if (living(environment()))
    {
        set_alarm(55.0, 0.0, eat_the_fuzzie);
    }
}

/*  Name:       enter_env(env, from)
    Purpose:    used here to trigger the mouse's irritating habit
                of eating the inventory of any player who is
                holding it; called when this object enters an
                environment.
    Arguments:  env - new environment; from - previous environment
    Returns:    nothing
*/
void
...
create_tio_animal()
{
    ...
    set_home("/d/Calia/domain_office");
    set_restlessness(20); 
