...
/*
 * /d/Calia/walkers/potions/aertrophia.c
 *
...
create_potion()
{
    set_potion_name("aertrophia");
    set_short("watery greenish-brown potion");
    set_pshort("watery greenish-brown potions");
    set_adj("watery");
    add_adj("greenish-brown");

    /* Set the long description if identified. */
    set_id_long("A watery greenish-brown potion, this is a mixture of water "+
        "and elkhorn kelp.\n");

    /* Set the long description if unidentified. */
    set_unid_long("This watery greenish-brown potion appears to have long "+
...
special_effect()
{
    object aertrophiaeffect;
    if (present("_aertrophia_potion_object_",this_player()))
    {
        write("You feel no effect.\n");
        return;
    }
    seteuid(getuid());
    aertrophiaeffect = clone_object(AERTROPHIA_POTION_OBJECT);
    aertrophiaeffect->set_remove_time(AERTROPHIA_TIME);
    aertrophiaeffect->move(this_player());
}
