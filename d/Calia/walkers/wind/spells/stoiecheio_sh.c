...
int Block_prop = 0;
string Adj1 = "lucid", Adj2 = "spirited";
...
long(mixed for_obj)
{
    ...
    return (SW == for_obj ? "You are a swirling mass of wind and debris.  "+
        "Your winds make objects around you gently flutter and move.  "+
        "Small dust clouds trail up from where "+
        "your feet touch the ground." : capitalize(SW->query_pronoun())+
        " is a swirling mass of wind and debris.  "+
        capitalize(SW->query_possessive()) + " winds make objects around "+
        SW->query_objective() + " gently flutter and move.  Small dust "+
        "clouds trail up from where " + SW->query_possessive() + 
