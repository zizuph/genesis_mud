 ...
 /*
   * Antidroaera (Resist Lightning) Spell for the Windwalkers
   * by Jaacar
   * April 25th, 1998
   */
...
public void antidroaera(object caster, object *targets, int *resist,
    int result);
public int create_spell_antidroaera(object caster, object *targets,
    string argument);

public object *
antidroaera_target_one_present_living(object caster, string str)
{
    object *oblist;

    if (!present("_antidroaera_potion_object_",caster))
    {
        write("You are not under the effects of the antidroaera "+
            "potion!\n");
        return ({});
    }
...
public int
create_spell_antidroaera(object caster, object *targets, string argument)
{
    set_spell_time(2);
    set_spell_mana(130);
    ...
    set_spell_effect(antidroaera);
    set_spell_object(ANTIDROAERA_OB);
    return 0;
}

public void
antidroaera(object caster, object *targets, int *resist, int result)
{
    object resob;

    if (targets[0] == caster)
    {
        if (present("_antidroaera_object_", caster))
        {
    ...
    if (present("_antidroaera_object_", targets[0]))
    {
    ...
    targets[0]->catch_tell("Your skin glows briefly.  You feel you "+
        "are somehow more resistance to lightning.\n");
    ...
    caster->catch_tell("You concentrate on the image of "+targets[0]->query_the_name(caster)+
        " and call upon the powers of Antidroaera.  "+targets[0]->query_The_name(caster)+
