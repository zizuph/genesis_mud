...
{
    seteuid(getuid());
    return "Windwalkers of Thalassia";
}
...
wchoose(string str)
{
    ...
    if (!(COUNCIL_CODE->is_wind_elder(TP->query_real_name())))
    {
    ...
    if (is_wind(who[0]))
    {
        NF("That person is already a Windwalker!\n");
        return 0;
    }
    ...
    if (present("_wind_chosen_", who[0]))
    {
    ...
    wind_apprentices += ({ apprentice });
    (COUNCIL_OBJECTS + "log_book")->update_log_book(my_time() +
        capitalize(my_name) + " has selected "+capitalize(apprentice) +
        " as "+TP->query_possessive()+" apprentice of Aeria.\n");
    save_object(COUNCIL);

    setuid();
    seteuid(getuid());
    chosen=clone_object(WIND_OBJECTS+"chosen");
    chosen->move(who[0],1);
    return 1;
}
...
wstretch()
{
    write("You stretch your body out and assume a resting position, "+
        "floating a few centimetres up in the air.\n");
...
list_winds()
{
    ...
    write("The following Windwalkers are in the Realms:\n");
    list = users();
    for (i=0; i < sizeof(list); i++)
    {
        if (!is_wind(list[i]) || list[i]->query_invis())
          continue;
       ...
       {
          g_short = list[i]->query_wind_walker_title();
...
w_help(string str)
{
    ...
    if (str == "windwalker agios" || str == "wind agios")
    {
        if (TP->query_skill(SS_WALKER_CHOSEN) & SPELLS_TEN_FLAG)
        {
            write("Agios - Fly :  Upon the successful "+
                "casting of this spell, you shall be able to fly to one of "+
                "various locations throughout the realms.  If you do not ask to be "+
                "taken to any specific location (cast agios), you shall fly through "+
                "the heavens and soar up to your cloud home above the lands of Calia.  "+
                "If you ask to be taken to the temple (cast agios temple), "+
                "you shall soar through the skies until you reach the silver "+
                "altar in the Worshippers temple.  If you ask to be flown to the "+
                "shrine (cast agios shrine), "+
                "you shall be propelled by the winds to a magical gateway that takes "+
                "you to the Elemental Shrine entrance.  As a special "+
                "gift granted only with the Elemental Shrine, once you are "+
                "finished your business within the Shrine, you only need enter the "+
                "archway to be returned to "+
                "the place where you began your journey.\nThere are some areas in "+
