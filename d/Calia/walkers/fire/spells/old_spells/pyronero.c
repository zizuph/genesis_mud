 ...
 /*
  * Pyronero (Fire Water) Spell for the Firewalkers Guild
  * by Jaacar
  * April 28th, 1998
  */
...
public void pyronero(object caster, object *targets, int *resist, int result);
public int create_spell_pyronero(object caster, string argument, object *targets);

public object *
pyronero_target_one_present_living(object caster, string str)
{
    object *oblist;
...
public void
create_spell_pyronero(object caster, string argument, object *targets)
{
    set_spell_time(1);
    set_spell_mana(28);
    ...
    set_spell_object(PYRONERO_OB);
    return 0;
}

public void
pyronero(object caster, object *targets, int *resist, int result)
{
    object decanter;

    if (random(100) < resist[0])
    {
        caster->catch_tell("You successfully cast the Pyronero spell, but "+
    ...
    if (caster == targets[0])
    {
        write("You pray upon the powers of Lord Pyros to grant you "+
            "the magical decanter of Pyronero.  A fiery coloured crystal "+
            "decanter appears in your hands.\n");
        tell_room(environment(caster),QCTNAME(caster) + " concentrates "+
            "intensely for a moment.\n",({caster}));
        seteuid(getuid());
        decanter = clone_object(PYRONERO_OB);
        decanter->move(TP,1);
    }
    else
    {
        caster->catch_msg("Your eyes flash as you incant the arcane words of "+
            "Pyronero and concentrate on the image of "+QTNAME(targets[0])+".\n");
        ...
        targets[0]->catch_msg(caster->query_The_name(targets[0])+ " mumbles "+
            "some strange words and stares intensely at you.  Suddenly a fiery "+
