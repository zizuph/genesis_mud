...
{
    spell_target->catch_msg("The light surrounding your hand fades away "+
        "into nothingness.\n");
    tell_room(environment(spell_target),"The light surrounding "+
        QCTNAME(spell_target)+"'s hand fades into nothingness.\n",
        ({spell_target}));
...
show_subloc(string subloc, object me, object observer)
{
    ...
    return "A blazing light "+
        "surrounds " + ( observer == me ? "your" :
