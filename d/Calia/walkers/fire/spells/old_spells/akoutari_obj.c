 ...
 /* Akoutari Object to block an exit
  * by Jaacar
  * November, 1997
  */
...
start()
{
    set_spell_effect_desc("akoutari");
...
create_object()
{
    ...
    set_short("wall of flames");
    set_long("@@flames_long");
    add_prop(OBJ_M_NO_GET,1);
    add_prop(MAGIC_AM_MAGIC, ({ 40, "prayer"}));
    add_prop(MAGIC_AM_ID_INFO, ({"This wall of flames does not seem to "+
        "be able to be penetrated unless you are protected by fire.\n", 40}));
...
try_to_exit()
{
    if (this_player()->query_wiz_level())
    {
        ...
        say(QCTNAME(this_player())+" walks through the wall of flames unharmed.\n");
        return 0;
    }
    if (is_walker(this_player()))
    {
        write("You quickly step through the flames, unharmed by their effects.\n");
        say(QCTNAME(this_player())+" steps through the flames, uneffected by them.\n");
        return 0;
    }
    if (TP->query_magic_res(MAGIC_I_RES_FIRE) > 40)
    {
        write("You boldy step through the flames unharmed.\n");
        say(QCTNAME(this_player())+" boldy steps through the flames unharmed.\n");
        return 0;
    }
    write("You attempt to leave throught the "+blocked_exit+" exit, but "+
        "as you approach the flames you sense you would be badly burned "+
        "and decide it would not be wise to do so.\n");
    say(QCTNAME(this_player())+" appears as though "+this_player()->query_pronoun()+
        " is going to leave through the "+blocked_exit+" but decides against "+
...
remove_object()
{
    tell_room(environment(this_object()),"The wall of flames slowly "+
