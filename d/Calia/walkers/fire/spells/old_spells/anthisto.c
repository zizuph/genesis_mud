 ...
 /*
   * Anthisto (Resist Fire) Spell for the Firewalkers
   * by Jaacar
   * April 28th, 1998
   */
...
public void anthisto(object caster, object *targets, int *resist,
    int result);
public int create_spell_anthisto(object caster, object *targets,
    string argument);

public int
create_spell_anthisto(object caster, object *targets, string argument)
{
    set_spell_time(2);
    set_spell_mana(130);
    ...
    set_spell_effect(anthisto);
    set_spell_object(ANTHISTO_OB);
    return 0;
}

public object *
anthisto_target_one_present_living(object caster, string str)
{
    object *oblist;

    if (!present("_anthisto_potion_object_",caster))
    {
        write("You are not under the effects of the anthisto "+
            "potion!\n");
        return ({});
    }
...
public void
anthisto(object caster, object *targets, int *resist, int result)
{
    object resob;

    if (targets[0] == caster)
    {
        if (present("_anthisto_object_", caster))
        {
    ...
    if (present("_anthisto_object_", targets[0]))
    {
    ...
    targets[0]->catch_tell("Your skin glows briefly.  You feel you "+
        "are somehow more resistance to fire.\n");
    ...
    caster->catch_tell("You concentrate on the image of "+targets[0]->query_the_name(caster)+
        " and call upon the powers of Anthisto.  "+targets[0]->query_The_name(caster)+
