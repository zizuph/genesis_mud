 ...
 /*
   * Stoiecheio (Elemental Form) Spell for the Firewalkers
   * by Jaacar
   * April 28th, 1998
   */
...
stoiecheio(object caster, object *targets, int *resist, int result)
{
    ...
    write("Concentrating all of your efforts on the task, you "+
        ...
        "bursts into flames enveloping everything you own, yet not "+
        "harming anything.\n");
    tell_room(environment(caster),QCTNAME(caster)+" is suddenly and violently "+
        "consumed in a scorching explosion of heat and smoke.\n",({caster}));
    seteuid(getuid());
