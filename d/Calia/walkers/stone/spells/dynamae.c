 ...
 /* Dynamae (Tell Line) spell for the Stonewalkers of Thalassia
  * by Jaacar 
  */
...
tell(object caster, object *targets, int *resist, int result)
{
    ...
    {
        caster->catch_tell("You use the powers of Dynamae to "+
            "speak to " + targets[0]->query_met_name() + ": " +
            query_tell_message() + "\n");
    }
    targets[0]->catch_tell("A loud booming sound quickly forms "+
        "the voice of " + 
