...
droppable(string str)
{
	...
	nDrop = query_prop(OBJ_I_NO_DROP);
	
	if(nDrop) write("You can now drop the brazier.\n");
	else write("You will now hang on to the brazier.\n");
	nDrop = 1 - nDrop;
	
	if(nDrop) add_prop(OBJ_I_NO_DROP, 1);
	else remove_prop(OBJ_I_NO_DROP);
	return 1;
}
...
{  
	...
	if(nRoomType == ROOM_I_IN_WATER || nRoomType == ROOM_I_UNDER_WATER)
		return 0;
