...
get_stuff() {
   ...
   seteuid(getuid());
   for (i = 0; i < 3; i++)
   ob1 = clone_object("/d/Rhovanion/mute/dungeon1/jet.c")->move(TO);
   ob2 = MONEY_MAKE(500,"copper")->move(TO);
   ob3 = MONEY_MAKE(115,"silver")->move(TO);
   ob4 = MONEY_MAKE(  3,"gold")->move(TO);
   ob5 = clone_object("/d/Rhovanion/mute/dungeon1/stat.c")->move(TO);
   ob6 = clone_object("/d/Rhovanion/mute/dungeon1/ingot.c")->move(TO);
}
