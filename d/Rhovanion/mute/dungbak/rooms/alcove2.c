...
create_room() {
   set_short("int\n");
   set_long("This is a small alcove set into the east wall of " +
   ...
   clone_object("/d/Rhovanion/mute/dungeon1/wguard1.c")->move(TO);
   add_exit("/d/Rhovanion/mute/dungeon1/int12","northwest","@@check_guard");
   add_exit("/d/Rhovanion/mute/dungeon1/int11","west");
}
int check_guard() {
   if(TP->query_npc()) { return 0; }
   if (present("guard")) {
      write("The guard is blocking that exit.\n");
      say(QCTNAME(TP)+" tries to go northwest, but the guard blocks him.\n");
      return 1;
   }
}
