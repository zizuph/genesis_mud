...
create_room()
{
  ...
  add_prop(ROOM_I_INSIDE, 1);
    ...
    set_long("In this room,goblin guards usually take their duty. "+
             "When they are here, they stop everyone from entering "+
             "the prison unless they are going to stay there. "+
...
block_exit(string room)
{
  if(present("_misty_goblin_", TO) &&
     ...
     (TP->query_prop(LIVE_O_LAST_ROOM) != find_object(room)))
   {
     ...
     say(QCTNAME(TP)+" was stopped from passing by the goblin guard.\n");
     return 1;
   }
  return 0;
}
