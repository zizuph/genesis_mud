...
create_room()
{
  ...
  add_prop(OBJ_S_SEARCH_FUN, "search_fun");
    ...
    set_long("Here goblin guards rest between their duties. They play their "+
             "rude games here, eat, sleep and sometimes even fight here. "+
             "There are rumours that they cook their victims and prisoners. "+
             "It seems even more likely to you when you see the big pot "+
             "hanging on a chain over the "+"@@fire_logs@@"+" that there "+
...
fire_logs() {
  ...
  return "A small heap of half burned logs";
}

string
pot_desc() {
  if(present("_misty_goblin_", TO))
    return "It is a big iron pot. The goblins will not let you come any closer "+
           "to it; so you can't see what is in it. Maybe it is better "+
           "that you don't know.\n";
  if(!pot_is_empty)
    return "It is a big iron pot with some thick soup in it. What is "+
           "the soup made from you can't tell but surely it isn't beef.\n";
  if(key_is_in_pot)
    return "It is a big iron pot. Surprisingly enough you see a small "+
           "rusty key in it.\n";
  return "It is a big empty iron pot.\n";
} 

string
fire_desc() {
  if(fire_is_on)
    return "It is quite a big fire and it is heating the pot "+
           "hanging above it.\n";
...
logs_desc() {
  if(!fire_is_on)
    return "It is only a small heap of half burned logs.\n";
...
extinguish_fire(string arg) 
{
  ...
  if(arg == "fire")
  {
    NF("The goblins don't let you do that.\n");
  ...
  if(arg == "fire with soup")
  {
    ...
    TP->catch_msg("You swing the pot on the chain. Some soup is spilled on "+
                  "the fire and extinguishes it.\n");
    say(QCTNAME(TP)+" swings the pot on the chain. Some soup is spilled on "+
        "the fire and extinguishes it.\n");
    fire_is_on = 0;
    TO->remove_item("fire");
    TO->add_item("logs", "@@logs_desc");
    call_out("cool_soup_slowly",40);
    return 1;
  }
  return 0;
}
...
cool_soup_slowly()
{
  ...
  {
    tell_room(TO, "The soup in the pot cools down a bit.\n");
    soup_is_hot -= 1;
    call_out("cool_soup_slowly",180);
  }
  return;
}
...
cool_soup(string arg)
{
  ...
  if(soup_is_hot)
  {
    if(fire_is_on)
    {
      TP->catch_msg("You try to cool down the soup by blowing at it.\n");
      say(QCTNAME(TP)+" tries to cool down the soup by blowing at it.\n");
      return 1;
    }
    TP->catch_msg("You cool down the soup a bit by blowing at it.\n");
    say(QCTNAME(TP)+" cools down the soup a bit by blowing at it.\n");
    soup_is_hot -= 1;
    return 1;
  }
  TP->catch_msg("You try to cool down the already cool soup.\n");
  say(QCTNAME(TP)+" tries to cool down the already cool soup.\n");
  return 1;
}
...
eat_soup(string arg)
{
  ...
  if(soup_is_hot)
  {
    TP->catch_msg("You try to eat the soup but it burns your tongue.\n");
    say(QCTNAME(TP)+" tries to eat the soup but it burns "+
  ...
  if(TP->query_race_name() != "goblin")
  {
    TP->catch_msg("You eat a spoonful of the soup.\n"+
                  "It has a horrible taste and makes you sick.\n"+
                  "You are very sick!\n"+
                  "You vomit even yesterdays lunch!\n");
    say(QCTNAME(TP)+" eats a spoonful of the soup.\n"+
        CAP(TP->query_pronoun())+" makes an unpleasant face. Maybe "+
        TP->query_pronoun()+" doesn't like soup?\n"+
        CAP(TP->query_pronoun())+" looks very sick!\n"+
        CAP(TP->query_pronoun())+" vomits even yesterdays lunch!\n");
...
empty_pot(string arg)
{
  ...
  if(present("_misty_goblin_", TO))
  {
    NF("The goblins don't allow you to do that.\n");
    return 0;
  }
  if(soup_is_hot)
  {
  ...
  if(TP->query_race_name() == "goblin")
  {
    ...
    say(QCTNAME(TP)+" takes the pot with the intent to empty it. Then "+
...
get_key(string arg)
{
  ...
  {
    TP->catch_msg("You get a small rusty key from the pot.\n");
    say(QCTNAME(TP)+" gets a small rusty key from the pot.\n");
    key_is_in_pot = 0;
...
search_fun(object searcher, string str)
{
  ...
  if(fire_is_on && ((str == "fire") || (str != "logs")))
  {
    ...
    return "Ouch!!! Your hands are severely burnt from the fire.\n";
  }

  if(pot_is_empty && (str == "soup"))
    return "But there is no soup in the pot!\n";

  if(soup_is_hot && ((str == "pot") || (str == "soup")))
  {
    searcher->set_hp(MAX(1,(searcher->query_hp())*6/7));
    return "Ouch!!! Your hands are severely burnt from the steaming pot.\n";
  }
