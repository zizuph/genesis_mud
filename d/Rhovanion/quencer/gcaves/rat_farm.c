...
void create_cave()
{
  ...
  set_short("Rat farm");
  set_long("You are at the end of a tunnel that burrows under the Misty "
    ...
    + "full of rats. No wonder because one greedy goblin made a rat "
    + "farm here. He trains rats to deliver messages and he hopes that "
    + "he can make a living on it. He has put a poster which you "
    + "may be able read.\n");

  add_item("poster","Maybe you are literate enough to read it.\n");
  add_cmd_item("poster","read","@@read_poster");
  add_item("cage","It is a massive iron cage full of dirty rats.\n");
...
int get_rat_from_cage(string arg)
{
  ...
  if((arg == "rat from cage") || (arg == "rats from cage") ||
     (arg == "all from cage"))
    NF("The cages are closed so you can't get to the rats.\n");
  return 0;
}
...
int open_cage(string arg)
{
  ...
  say(QCTNAME(TP)+" tries desperately to open the locked cage but it "+
      "remains closed.\n");
  TP->catch_msg("You try desperately to open the locked cage but it "+
                "remains closed.\n");
...
int unlock_cage(string arg)
{
  ...
  say(QCTNAME(TP)+" tries unlock the "+arg+" but it doesn't seem to work.\n");
  TP->catch_msg("You try to unlock the "+arg+" but it doesn't seem to work.\n");
  if(ratkeeper && present(ratkeeper,TO))
  {
...
void reset_room()
{
  if(ratkeeper && present(ratkeeper,TO)) return;
...
int check_intro()
{
  ...
  ratkeeper->command("say Yes you bastard! I mean you "+TP->query_nonmet_name()+"!");
  return 0;
}
...
int hire_rat(string arg)
{
  ...
  say(QCTNAME(ratkeeper)+" gets a rat from the cage and gives it to "+
      QTNAME(TP)+".\n");

  post_rat = clone_object(NPC_DIR+"post_rat");
  if (err, post_rat->move(TP))
