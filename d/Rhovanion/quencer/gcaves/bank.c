/* Bank by Milan */
/*
 * This is /d/Gondor/morgul/city/rooms/bank
 * - the money changer in Minas Morgul
 *
 * Using the example made by Nick: /d/Genesis/doc/examples/trade/bank.c
 *
 * Olorin, July 1993
 */
...
void create_cave()
{
  ...
  set_long("You are at the end of a tunnel that burrows under the Misty "
    ...
    + "the wall. It looks as though one greedy goblin made a private "
  ...
  add_exit(CAVE_DIR+"t60", "southeast");
...
void reset_room()
{
  if(banker && present(banker,TO)) return;
  if(sizeof(FILTER_LIVE(all_inventory(TO))))
  {
    set_alarm(200.0, 0.0, "reset_room");
    return;
  }
  banker = clone_object(NPC_DIR + "goblins/banker.c");
  banker->arm_me();
...
int open_safe(string arg)
{
  NF("It is too dark to see.\n");
  if(!CAN_SEE_IN_ROOM(TP)) return 0;
  NF("Open what?\n");
  if(!arg || (arg != "safe")) return 0;
  say(QCTNAME(TP)+" tries desperately to open the safe but it "+
      "remains closed.\n");
  TP->catch_msg("You try desperately to open the safe but it remains closed.\n");
...
int check_intro(string arg)
{
  ...
  banker->command("say If you want "+arg+" something then you should at "+
                  "least introduce yourself!");
  banker->command("kick "+TP->query_name());
  return 0;
}
