...
create_room()
{
  ...
  add_prop(ROOM_I_INSIDE, 1);
    ...
    set_long("You are in the cell of the goblin prison. It is dark with wet "+
    ...
    add_item(({"wall","south wall","east wall","west wall",
              ...
              "northern and southern walls that were built from big blocks "+
              "of rock. "+"@@hole_desc");
    add_item("blocks","Big blocks of dark cold rock. The walls are usually "+
...
string
hole_desc(){
  if(hole_i_is) return "One block is removed from the southern wall "+
                       "making a hole.\n";
  return "\n";
}
...
int knock_on(string arg){
  ...
  {
    TP->catch_msg("You start again systematically knocking on the blocks...\n"+
            "All other blocks sound similarly.\n");
    say(QCTNAME(TP)+" starts again systematically knocking on the blocks..\n"+
            "All other blocks sound similarly.\n");
    return 1;
  }
  block_i_is = 1;
  add_item("block","@@block_desc");
  TP->catch_msg("You start systematically to knock on the blocks...\n"+
          "Oops!!! This block in south wall sounds somehow different!\n");
  say(QCTNAME(TP)+" starts systematically to knock on the blocks..\n"+
...
int push_block(string arg){
  ...
  {
    TP->catch_msg("You push the block with all your strength but it "+
                  "doesn't move at all.\n");
    say(QCTNAME(TP)+" pushes the block with all "+TP->query_possessive()+
            " strength but it doesn't move at all.\n");
    return 1;
  }
  TP->catch_msg("You push the block with all your strength...\n"+
...
int pull_block(string arg){
  NF("Pull what?\n");
  if((!block_i_is) || (arg != "block")) return 0;
  TP->catch_msg("You try to pull the block but you can't get a firm grip.\n");
  say(QCTNAME(TP)+" tries to pull the block but "+TP->query_pronoun()+
            " can't get a firm grip.\n");
  return 1;
}
...
int enter_hole(string arg){
  ...
  tell_room(CAVE_DIR+"d5",QCTNAME(TP)+" enters the room through "+
                                      "the hole in wall.\n");
