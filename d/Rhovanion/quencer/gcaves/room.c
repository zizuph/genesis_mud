...
void create_room()
{
  ...
  add_item(({"wall","walls"}),"When you examine the wall more closely "+
           "you have a strange feeling. It reminds you of all the stories "+
...
string my_long()
{
  ...
  switch(i)
  {
    ...
    default: str = "You are at a branch of tunnels";
  }
  return str+" "+
...
string my_short()
{
  ...
  switch(i)
  {
    ...
    default: return "The branch of tunnels";
  }
}
...
void reset_room()
{
  ...
  if((!present("_misty_goblin_")) && random(2))
  {
    ...
    {
        goblin = clone_object(NPC_DIR+"goblins/goblin");
        goblin->arm_me();
...
int pull_something(string str)
{
      ...
      (str==pull_dir+" wall") || (str==pull_dir+" walls")))
  {
    if(CAN_SEE_IN_ROOM(TP))
    {
      ...
      {
        TP->catch_msg("You pull the wall revealing a small opening.\n");
        say(QCTNAME(TP)+" pulls the wall revealing a small opening.\n");
        if(sizeof(team = TP->query_team()))
          for(i=0; i<sizeof(team); i++)
            if(present(team[i], ENV(TP)))
            {
              team[i]->move_living("through the opening", pull_room);
              team[i]->catch_msg("You leave through the opening.\n");
            }
        TP->catch_msg("You pass and close the opening in wall.\n");
        TP->move_living("through the opening and it closes.",pull_room,1);
      }
      else
      {
        TP->catch_msg("You pull the wall with all your strength. It " +
            "starts to move but suddenly stops. Maybe you should eat more " +
            "spinach.\n");
...
int push_something(string str)
{
      ...
      (str==push_dir+" wall") || (str==push_dir+" walls")))
  {
    if(CAN_SEE_IN_ROOM(TP))
    {
      ...
      {
        TP->catch_msg("You push the wall revealing a small opening.\n");
        say(QCTNAME(TP)+" pushes the wall revealing a small opening.\n");
        if(sizeof(team = TP->query_team()))
          for(i=0; i<sizeof(team); i++)
            if(present(team[i], ENV(TP)))
            {
              team[i]->move_living("through the opening", push_room);
              team[i]->catch_msg("You leave through the opening.\n");
            }
        TP->catch_msg("You pass and close the opening in wall.\n");
        TP->move_living("through opening and it closes.",push_room,1);
      }
      else
      {
...
string search_wall(object who, string str)
{
     ...
     (who->query_skill(SS_AWARENESS)+who->query_stat(SS_WIS) > 100))
  {
    ...
    return "After carefully searching you decided that "+sss+
           " walls are very strange. Maybe you have find some "+
           "secret door. Now... How do they open?\n";
  }
}
