...
init_killers()
{
    ...
    seteuid(getuid());
    text = read_file("/d/Rhovanion/log/farhwini_killers");
    if (text)
...
add_killer(mixed killer)
{
    ...
    text = impode(list_of_killers, "\n");
    seteuid(getuid());
    rm("/d/Rhovanion/log/farhwini_killers");
    write_file("/d/Rhovanion/log/farhwini_killers", text);
}

void create_monster()
{
    set_living_name("_farhwini_mounted_combat_trainer_");
    set_title("trainer");
    set_name("farhwini");
    set_pname("humans");
    set_race_name("human");
    set_adj(({"dark","swarthy","old","wise","brown haired","bow legged"}));
    set_short("@@short_func");
    set_long("This is a weary old man with an evil but knowledgeable air "+
	     "riding a horse.  The rider looks as if he could help you with "+
	     "one of the skills of horsemanship.\n");
    ...
    add_chat("Dwarves make very poor riders!");
    add_chat("I can help all but dwarves in one of the skills of horsemanship.");
    set_cchat_time(2);
    add_cchat("WHAT DO YOU THINK YOU ARE DOING?");
    add_cchat("I will teach you to attack me!");
...
train_text(int skill)
{
    ...
    switch (skill) {
	case SS_MOUNTED_COMBAT:
	    what="fight mounted on a steed";
	    does=what;
	    break;
	default:
...
init_skills()
{
    ...
    sk_add_train(SS_MOUNTED_COMBAT,
		 train_text(SS_MOUNTED_COMBAT),
		 0,
		 0,
		 30);
}
...
sk_cost(int skill, int from, int to)
{
    ...
    if (player_is_killer(this_player()))
...
sk_improve(string str)
{
	...
	notify_fail("I don't train dwarves!\n");
	return 0;
    }
...
arm_me()
{
    ...
    ob = clone_object("/d/Rhovanion/common/weapons/rsword");
    ob->move(TO);
    command("wield all");

    ob = clone_object("/d/Rhovanion/common/armours/rchain");
    ob->move(TO);

    ob = clone_object("/d/Rhovanion/common/armours/rhelm");
    ob->move(TO);

    ob = clone_object("/d/Rhovanion/common/armours/rshield");
    ob->move(TO);
    command("wear all");
}
...
{
    if (riding) 
	return "The horse appears to be battle weary and slightly scarred "+
	       "despite this it still looks fearless and almost unflapable "+
	       "as if he was trained for battle.\n");
    return "";
}
