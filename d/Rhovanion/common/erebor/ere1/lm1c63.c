...
create_cave()
{
    ...
    set_cave("Erebor");
    set_cave_type(8);
    set_long("The floor is covered with hay to prevent the spilled beer from "
	+ "destroying the rocks underneath. "
    ...
    add_exit(CAVE_DIR + "lm1c64","east",0,1);

    south_door = clone_object(CAVE_DIR + "door_lm1c63");
    south_door->move(TO);
