...
add_magic_properties()
{
  ...
  add_prop(MAGIC_AM_ID_INFO,({"This club seems to be enchanted.\n",1,
	   ...
	   int phit, int dam)
{
	...
	enemy->catch_msg(QCTNAME(me) + " aims for your " + hdesc + 
			 ...
			 ", but misses.\n");
	me->tell_watcher(QCTNAME(me)+" aims for "+where+" with "+
        ...
        int dam)
{
  ...
  if((dam_mana > 0) && (enemy->query_mana() < 1))  /* enemy is knocked out */
  {
    ...
    call_out("notify_end_paralyze", dam_mana, ({enemy,attack_delay-(dam_mana/2)}));
