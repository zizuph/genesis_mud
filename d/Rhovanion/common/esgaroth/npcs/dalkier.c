...
create_monster()
{
    ...
    add_name("adventurer"); 
    add_name("drunk");     
    add_name("esg_quest_npc_dalkier");
    set_long("This gentleman is obviusly drunk as a skunk,and" +
	" you notice he has forgot all about eriquette and social skills. " +
	"In fact, you doubt you ever met anyone so repulsive.\n");
    ...
    set_chat_time(7);
    add_chat("I sshall never..... ever..... sshay anysching!");
    add_chat("My lipssh are schealed....");
    add_chat("I wissh I had anosher beer!");
...
{  
	...
	"interest.\n");
    from->catch_msg(QCTNAME(TO) + " leans towards you and mumbles in your " +
	"ear:\nWhy don't you buy me a drink?\n");
    say(QCTNAME(TO) + " leans towards " + QCTNAME(from) + " and mumbles " +
	"something in " + HIS(from) + " ear.\n");
    TO->command("grin hap");
...
arm_me()
{
    ...
    item = clone_object("/d/Rhovanion/common/weapons/esword");
...
call_for_help(object ob)
{
  ...
  for (i = 0; i < sizeof(arr); i++)
    if((arr[i]->query_real_name() == ("malcolm")) || (arr[i]->query_real_name() == ("lexor")) || (arr[i]->query_real_name() == ("taurek")))
...
{
  ::attacked_by(ob);
  command("say You fool, now you will die!");
