...
#define SUP_DWARF_LEVEL      120           /* Ibun   */
#define HIGH_DWARF_LEVEL      80           /* Ibun   */           
#define MEDIUM_DWARF_LEVEL    50           /* Ibun   */

string *ran_wep = ({ "axe", "pole" });
string *ran_arm = ({ "helm", "shield" });
string *def_arm = ({ "mail" });

string *looks = ({"angry","wise","hard","nice",
                  "friendly","cool","smart","stubborn","strong" });
string *c_chats = ({ "I see. Strangers in our mountain!",
                     "Someone go and inform the guard!" });
string *chats = ({ "What is Dain going to do with that messenger?",
                 "I have news about strangers coming by."
                });
int base_dwa = 30;               /* Ibun */
int ran_dwa = 90;                /* Ibun */
int knight_prestige, base;
string type = "_low";

/* Function create_dwarf() to be redefined when creating a dwarf in order
 * to set your own values to:
 *  ran_wep, ran_arm, def_arm, looks, c_chats, chats, base_dwa, ran_dwa
...
nomask void set_base_dwa(int i)    { base_dwa = i; }
nomask void set_ran_dwa (int i)    { ran_dwa  = i; }

nomask void create_monster()
{
  int i;
  string ll_2, ll_1 = "short";
  ...
  set_name("_ere_dwarf_");
  set_long("It is a short but strong looking dwarf.\n");

  create_dwarf();

  base = base_dwa + random(ran_dwa);
  if (base >= MEDIUM_DWARF_LEVEL){ type = "_med"; ll_1 = "thin"; }
  if (base >= HIGH_DWARF_LEVEL){ type = "_high"; ll_1 = "tough"; }
  if (base >= SUP_DWARF_LEVEL){ type = "_sup"; ll_1 = "powerful"; }
  ...
  set_base_stat(SS_DEX, (base * 4 / 5) + random(base / 2));
  ...
  set_alignment(2 * base);
  knight_prestige = (-(2 * base));
...
/*
 * Give the dwarf armour and a weapon.
 */
void arm_me()
{
  int i;
...
void slow_init_attack(object tp)
{
  ...
  /* Dwarves aren't attacked either */
  if (tp->query_race_name() == "dwarf") return;
...
void attacked_by(object ob)
{
  ...
  for (i = 0; i < sizeof(arr); i++)
    if((arr[i]->query_real_name() == "_ere_dwarf_") && (arr[i] != TO))
...
{
  command("shout Another dead enemy!");
  command("laugh");
