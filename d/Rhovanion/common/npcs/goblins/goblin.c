/*
 *  Updated by Ged 5/4/2002
 *  Changed random weapon for huge goblins to sword or club
 *  Huge goblin will have chance of 40% to have club and
 *  60% to have sword
 *  Changed huge goblins default armours to platemail,helm and shield
 */
...
nomask void create_monster()
{
  ...
  if (base >= LORD_GOBLIN_LEVEL){ type="_lord"; ll_1 = "enormous";}
...
int arm_me()
{
  ...
  if(type=="_sup")
   {
    if(random(10)<4)
      clone_object(WEP_DIR+"club_sup")->move(TO,1);
    else
      clone_object(WEP_DIR+"sword_sup")->move(TO,1);
    clone_object(ARM_DIR+"helm_sup")->move(TO,1);
    clone_object(ARM_DIR+"shield_sup")->move(TO,1);
    clone_object(ARM_DIR+"mail_sup")->move(TO,1);
   }
  else if(type=="_lord")
   {
    clone_object(WEP_DIR+"halberd_sup")->move(TO,1);
    clone_object(ARM_DIR+"helm_sup")->move(TO,1);    
    clone_object(ARM_DIR+"mail_sup")->move(TO,1);     
    clone_object(ARM_DIR+"tao_bracers")->move(TO,1);     
    clone_object(ARM_DIR+"tao_greaves")->move(TO,1);  
    command("wear all");
    if(present("halberd")) command("wield halberd in both hands");
  ...
  if(type=="_lord") command("wield halberd in both hands");
    else command("wield all");
  command("wear all");
