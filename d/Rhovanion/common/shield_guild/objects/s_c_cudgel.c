...
create_object()
{
  set_name(({"cudgel"}));
  set_adj(({"sergeant"}));
  set_short("sergeant cudgel");
  set_long("@@my_long");

  add_prop(OBJ_I_WEIGHT,100);
  add_prop(OBJ_I_VOLUME,100);
  add_prop(OBJ_M_NO_SELL,"You would never sell the sergeant cudgel "+
	                 "from Angmar's Army.\n");
  add_prop(OBJ_M_NO_DROP,"You decide to keep the sergeant cudgel "+
	                 "from Angmar's Army.\n");
...
{
  if(TP == ENV(TO))
    return "This is the cudgel used by Sergeants in Angmar's Army to "+
           "discipline foolish soldiers.  It has your personal seal "+
	   "carved into the hand grip. You also nitice small "+
	   "<help AA cudgel> written on it.\n";
  return "This is the cudgel used by Sergeants in Angmar's Army to "+
         "discipline foolish soldiers.  It has some seal carved into the "+
	 "hand grip.\n";
}
...
hide(string str)
{
  ...
  {
    NF("");
    write("Your cudgel is hidden now!\n");
    return 0;
  }

  write("You put aside your cudgel your standard of Angmar's Army neatly around the pole "+
