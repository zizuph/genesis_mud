...
public mixed query_guild_trainer_lay(){ return THIS_DIR+"train"; }

int
query_guild_tax_lay() { return GUILD_TAX; }
...
init_lay_shadow(string arg)
{
  ...
  shield_red = 50; /* this reduces efficiency of using shield to 60% */
...
make_him_pay()
{
  ...
  tell_room(ENV(tp), "\nSuddenly Dark Lord appears and points at "+
	...
	"!!!\n"+"\nMany angry warriors jump at "+
      ...
      "After a long time "+QTNAME(tp)+" regains his sense and touches "+
      HIS(tp)+" sore head. "+CAP(HE(tp))+" sure regrets what "+HE(tp)+
      " has done!\n\n\n", tp);
