
inherit "/d/Rhovanion/lib/room";
#include <macros.h>
#include "/sys/stdproperties.h"
#include "/d/Rhovanion/farlong.h"

create_room()
{
set_short("On the road, on the plains");
set_long(break_string(
	"You travel on a dirt road over the plains."
	+"  The Sea of Rhun, the largest known inland body of water,"
	+" stands nearby."
     +"\n",75));

AE(RHUN+"roads/section10/r","northeast",0);
AE(RHUN+"roads/section10/t","southwest",0);

add_item("road",break_string("The road is made of dirt, but it"
	+" is quite well-marked."
	+"\n",75));
add_item(({"sea","Sea","sea of rhun","Sea of Rhun"}),
	break_string("The Sea of Rhun stretches vastly on to the north."
	+"  It extends for quite a distance, and you wonder if it is"
	+" navigable.\n",75));
add_item("plains",break_string("The plains of Rhun seem to stretch on"
	+" forever.\n",75));

add_prop(ROOM_I_LIGHT,1);

}
