...
void create_room()
{
  ...
  set_long("This is the gate for this large town.  It"
    ...
    +"  You may enter town to the south, leave by taking the dirt"
    +" road north,  or follow along the inside edge of the wall to"
    +" the southeast or southwest.\n");

  add_exit(RHUN_DIR+"ltown_ne/streets/bywall2","southeast",0);
  add_exit(RHUN_DIR+"ltown_ne/streets/bywall1", "southwest", 0);
  add_exit(RHUN_DIR+"ltown_ne/streets/st1", "south", 0);
  add_exit(RHUN_DIR+"roads/section7/d", "north", 0);
