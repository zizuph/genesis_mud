...
set_up_skills()
{
    sk_add_train(SS_SPELLCRAFT,      "use general magic",           0, 0, 90);
    sk_add_train(SS_FORM_CONJURATION,"cast spells of conjuration",  0, 0, 90,SS_WIS );
    sk_add_train(SS_FORM_DIVINATION, "cast spells of divination",   0, 0, 40,SS_WIS );
    sk_add_train(SS_ELEMENT_DEATH,   "control the element of death",0, 0, 90,SS_INT );
    sk_add_train(SS_ELEMENT_EARTH,   "control the element of earth",0, 0, 50,SS_INT );
    sk_add_train(SS_ELEMENT_AIR,     "control the element of air",  0, 0, 50,SS_INT );
    sk_add_train(SS_AWARENESS,	     "be aware",                    0, 0, 40);
}
