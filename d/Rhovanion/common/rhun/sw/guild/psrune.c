...
create_room()
{
	...
	set_long("This is a small, round chamber."
		...
		+" presence of a dark power.  On the north"
	...
	add_exit(SWG + "strune","east",0);
	add_exit(SWG + "hallnw","west",0);
	...
	+" this place.\n");
	add_item("rune","This is a large and intricately carved rune."
	+"  It is formed of a large rectangle with a single"
	+" vertical line drawn inside."
	+" You find yourself"
...
transfer(string str)
{
	...
	else
	{
		...
		{
			TP->catch_msg("You trace your fingers"
				+" along the surface of the rune,"
				+" but nothing seems to happen.\n");
			tell_room(environment(), QCTNAME(TP) + " traces " + TPOS + " finger"
				+" along the surface of the rune.\n", TP);
		}
		else
		{   
