...
void create_room()
{
  ...
  set_long("You stand at the northern edge of the large"
	...
	+" forest's edge by heading east or west from here, or"
	+" head into the trees to the south."
     +"\n");

  add_exit(RHUN_DIR+"forest/g3","east",0);
  add_exit(RHUN_DIR+"forest/e3","west",0);
  add_exit(RHUN_DIR+"forest/f4","south",0);
  add_stuff();
}
