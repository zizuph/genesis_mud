...
create_object()
{
    set_name(({"tree","_Rhovanion_tree_"}));
...
int climb(string what)
{
    ...
    if (!room)
    {
      ...
      else
      {
          ...
          rm(file + ".c");
          room = find_object(file);
          if(!room->add_exit(file_name(ENV(TO)), "down"))
          {
            write_file("/d/Rhovanion/log/TREE", "Failed add_exit() in "+
                                 MASTER_OB(room)+" "+ctime(time())+"/n");
            room->remove_object();
            write("You slip and fall to the ground.\n");
            say(QCTNAME(TP)+" tries to climb the tree, but slips down.\n");
            return 1;
          }
      }
    }
    ...
    tell_room(room, QCTNAME(TP) + " climbs up the tree.\n");
