...
create_object() {
   set_name("potion");
   set_adj(ONE_OF_LIST(({"purple","red","blue","green","white",
               "brown","orange","yellow","black"})));
   set_adj(ONE_OF_LIST(({"light","dark","heavy","viscous",
               "syrupy","watery","thick","bubbling"})));
   set_long("@@my_desc");
...
part_drunk() {
   seteuid(getuid());
   write("You cry out in horror as you watch your skin change colours " +
      "right before your eyes!\n");
   say(QCTNAME(TP) + " cries out in horror as " + TP + "'s skin " +
      "begins to change colours.\n");
   clone_object("/d/Rhovanion/common/dungeon1/gear/dye.c")
   ->move(TP,1);
   drank=1;
   return 1;
}
