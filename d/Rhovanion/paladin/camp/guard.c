...
create_monster()
{
  ...
  set_gender(G_MALE);
  set_long("This barbarian seems to be easterling.\n"+
           "They came from far east to fill Sauron's army.\n"+
           "They have made a LONG way and now they are "+
           "enjoying some rest before they'll head to Minas Morgul.\n");
...
{
  command("curse"+lower_case(killer->query_nonmet_name()));
...
int special_attack(object enemy)
{
  ...
  if (wielded&&(wielded->query_wt()==W_AXE))
  {
    ...
    dam=random(550)-(random(900)*dex_st)/100;
    if (dam<0) dam=0;
