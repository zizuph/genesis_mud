/*
 * /d/Rhovanion/common/mirkwood/mirk_serv.c
 *
 */

/*
 * Update:
 * 28.3.94 Milan: added query_map_fatigue() to fatigue players
 *
 */
...
#define SPIDER_ROOMS   ({ "x","y","z","X","Y" })
#define	MAX_SPIDERS    ({  5,  4,  2, 30,  6  })
...
{
  if((arg == "m") || (arg == "r") || (arg == "R") || (arg == "f") || 
     (arg == "l") || (arg == "c") || (arg == "L") || (arg == "e"))
    return 0;
...
{
  if((here == "x") || (here == "X") || (here == "y") || (here == "Y") || 
     (here == "z"))
    return 8;
...
string *query_show()
{
  return ({ "m","T","P","p","r","R","f","b","c","l","M","s","e","C" });
}
...
{
  if(to == "m")
    return "You see the peaks of some mountains to ";
  if(to == "T")
    return "There is a bridge over Forest river to ";
  if(to == "P")
    return "The unused path is ";
  if(to == "p")
    return "You spot a small path ";
  if(to == "r")
    return "Forest river is blocking your way ";
  if(to == "R")
    return "Way ";
  if(to == "f")
    return "Some river is blocking your way ";
  if(to == "b")
    return "In ";
  if(to == "c")
    return "Way ";
  if(to == "l")
    return "The Long Lake lies ";
  if(to == "M")
    return "An old wide road is ";
  if(to == "s")
    return "To the ";
  if(to == "e")
    return "Lake-town Esgaroth is to the ";
  if(to == "C")
    return "There is light comming from ";
...
neighbour_desc_end(string from, string to)
{
  ...
  if(to == "p")
    return " from here";
  if(to == "R")
    return " is blocked by Enchanted river";
  if(to == "b")
    return " direction you spot a bridge over Enchanted river";
  if(to == "c")
    return " is blocked by Celduin river";
  if(to == "s")
    return " is grassy path that leads out of this accursed forest";
...
player_see_surrounding(object who, string from, string to)
{
  ...
  if(to == "M")
    if((who->query_skill(SS_AWARENESS) < 20) && (from != "M"))
      return 0;
  if(to == "C")
    if((who->query_skill(SS_AWARENESS) < 20) && (from != "C"))
      return 0;
...
query_no_spiders()
{
  return ({"p","P","T","E","W","S","N","C"});
}

int
clean_up()
{
  if(present("_Rhovanion_tree_", TO)) return 0;
  return ::clean_up();
}
...
reset_map_room(string where)
{
  ...
  i = member_array(where, SPIDER_ROOMS);
  if((i != -1) && !random(10))
  {
    name = "_spider_"+where+"_"+random(MAX_SPIDERS[i])+"_";
    if(find_living(name))
      return;
...
set_up_room(string str)
{
  ...
  switch (str)
  {
	...
	break;
    case "T":
	set_short("Bridge to Thraduil's Caverns");
	set_long("Someone forgot to put bridge to Thranduil's Cavern "+
			     "here.\n");
	add_exits();
	break;
    case "M":
	...
	break;
    case "b":
        set_short("Bridge through Enchanted river");
        set_long("@@my_long:"+file_name(TO)+"|"+str+"@@");

        add_item(({ "river", "water" }),
        "River has dark and gloomy water and even you can sense some "+
        "evil emmiting from it.\n ");
...
{
  if(where == "x")
    if(who->query_skill(SS_LOC_SENSE) < random(50))
      return 1;
  return 0;
}
            ...
            switch(TIME) {
                ...
                case "afternoon":
                    text = "You are somewhere in the Mirkwood. "+TREES+
                      ...
                      "see the sky "+
                      "because of the the trees. "+SOUNDS+" "+COBWEBS+
                      " The air here is quite hot and stuffy." + text;
                    break;
            }
            break;
        case "P":
            ...
            break;
        case "b":
            text = "You are standing on the bridge through Enchanted "+
                   "river. As you look down at gloomy water you remember "+
                   "the tale about poor dwarf Balin that once fell into this "+
            ...
            switch(TIME) {
                ...
                case "evening":
                  text = "You are on a wide ancient road that goes straight "+
                      ...
                      "maintained and since then it has been called Men-i-Naugrim. "+
                      "But now only fools use is on the way to their death. "+
