...
#include "manager/overseers.h"

void check_activity();

private mapping punish = ([]), brutus = ([]), charge = ([]), guruquest = ([]);
private string *revolters = ({ });
...
public int
query_revolter(string name)
{
    return IN_ARRAY(name, revolters);
}
...
overthrow_overseers()
{
	/* Add code here to overthrow the overseers. */
	string who;
	if (who = query_overseer(POS_HIGH))
	{
		remove_overseer(who);
		write_file(LOG_DEMOTE + "demote",
		ctime(time()) + ": " + who + " is overthrown.\n");
		return 0;
	}
	revolters = ({ });
	SAVE; return 1;
