...
do_charge(object target, object attacker)
{
    ...
    set_alarm(itof(MIN(92, (115 - (attacker->query_skill(SS_CHARGE))))),
...
int help(string str)
{
    ...
    if (str == "gladiator" || str == "glad")
    {
    string *arr = filter(get_dir(GLAD_HELP), &operator(==)(".gh") @
      &extract(, -3));

    if (!sizeof(arr))
        return NF("There doesn't appear to be any help!\n");
    else
        arr = map(map(arr, &extract(, 0, -4)), capitalize);

    write("Help is available to Gladiators on these subjects:\n");
    write(sprintf("\n%-#78s\n", implode(arr, "\n")));

    if (QUERY_OVERSEER(TP) >= 0)
    {
        arr = filter(get_dir(GLAD_HELP), &operator(==)(".oh") @
          &extract(, -3));

        if (sizeof(arr))
        {
        arr = map(map(arr, &extract(, 0, -4)), capitalize);
        write("\nHelp is available to Overseers on these "+
          "subjects:\n");
        write(sprintf("\n%-#78s\n", implode(arr, "\n")));
        }
    }

    write("\nUse 'help glad[iator] <subject>' to access help "+
      "files.\n\n");
    return 1;
    }

    if (sscanf(str, "glad %s", str) || sscanf(str, "gladiator %s", str))
    {
    string file;

    if (file_size(file = sprintf("%s%s.gh", GLAD_HELP, str)) > 0)
    {
        TP->more(read_file(file));
        return 1;
    }
    else if (QUERY_OVERSEER(TP) >= 0)
    {
        if (file_size(file = sprintf("%s%s.oh", GLAD_HELP, str)) > 0)
        {
        TP->more(read_file(file));
        return 1;
        }
