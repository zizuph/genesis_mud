...
int block_reader() { return (QUERY_OVERSEER(TP) == POS_NONE); }

int block_writer() { return (QUERY_OVERSEER(TP) == POS_NONE); }

int
allow_remove(int note)
{
    int position = MANAGER->query_overseer_position(TP->query_real_name());

    return (position == POS_WIZ);
}
