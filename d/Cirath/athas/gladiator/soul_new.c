...
void expire_trance(object ob, int con);
void do_trance(object ob);
void allow_trance(object ob);
...
trance()
{
    ...
    {
     NF("Entering a martial trance is a skill, not just something "
	+"you do.\n");
        return 0;
    }

    if (glad->query_prop(GLAD_I_IS_BERSERK) == 1)
        write("You are already in the middle of a martial trance.\n");
    else if (((glad->query_base_stat(SS_CON))/5) > (glad->query_fatigue()))
        write("You are too tired to focus properly.\n");
    else
    {
        glad->add_prop(GLAD_I_IS_BERSERK, 1);
        set_alarm(5.0, 0.0, &do_trance(glad));
        glad->catch_tell("You take a few deep breaths and prepare to "
	+"enter a martial trance.\n");
        say(QCTNAME(glad)+" takes a few deep breaths and settles into "
	+"a fighting stance.\n", glad);
    }
    return 1;
}
...
do_trance(object ob)
{
    ...
    set_alarm(itof(time), 0.0, &expire_trance(ob, con));
    ob->catch_tell("You have fully entered a martial trance.\n");
say(QCTNAME(ob) + " focuses "ob->query_possessive()+" mind into a "
	+"combat trance.\n", ob);
}


void
expire_trance(object ob, int con)
{
    ...
    set_alarm(itof(MAX(10, 25 - ob->query_skill(SS_BERSERK) / 5)), 0.0,
      &allow_trance(ob));
    tell_object(ob, "You begin to lose your focus.\n");
    say(QCTNAME(ob) + " seems to have returned to a normal frame of "+
      "mind.\n");
}

void
allow_trance(object ob)
{
    if (!objectp(ob)) return;
    tell_object(ob, "\nYou feel like you can enter a martial trance again.\n\n");
...
int help(string str)
{
    ...
    if (str == "gladiator")
    {
	if (!sizeof(subjects = filter(get_dir(GLAD_HELP),
    ...
    if (sscanf(str, "gladiator %s", help_file) &&
      file_size(help_file = GLAD_HELP + help_file + ".help") >= 0)
    {
