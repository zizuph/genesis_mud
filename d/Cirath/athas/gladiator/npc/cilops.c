/* cilops.c: The pet of the Overseers: Serpine, 5/6/95. */
...
create_creature()
{
    ...
    set_gender(2);
    set_stats(({ 300, 300, 300, 10, 10, 300 }));

    set_attack_unarmed(A_BITE,  100, 300, W_IMPALE, 60, "mandibles");
    set_attack_unarmed(A_LCLAW, 100, 150, W_SLASH,  20, "left antenna");
    set_attack_unarmed(A_RCLAW, 100, 150, W_SLASH,  20, "right antenna");

    set_hitloc_unarmed(H_HEAD, 80, 30, "head");
    set_hitloc_unarmed(H_BODY, 70, 70, "body");

    set_act_time(30);
    add_act("emote waves its antennae around.");
    ...
    add_act("emote brushes you with its antennae.");
    add_act("emote pokes around in its nest.");
}

void
do_die(object killer)
{
   WRITE_LOG(LOG_CILOPS, killer, "just killed the cilops!");
   ::do_die(killer);
}
...
emote_hook(string emote, object actor, string adverb, object *oblist, int cmd_attr, int target)
{
    ...
    command("emote clicks its mandibles together happily.");
}
