...
#define DEBUG(x)        find_player("samoth")->catch_tell(x);
...
{
  seteuid(getuid());
  restore_object(GLAD_LOG+"brutus_attacker");
  ...
  add_ask(({"arena","stadium","about arena","about stadium"}),
   ...
   add_cmdsoul(GLAD_DIR+"brut_soul");
   update_hooks();
}
...
arm_me()
{
    ...
    clone_object(GLAD_OBJ  +"brut_greaves")->move(TO);
    clone_object(GLAD_OBJ  +"brut_shield")->move(TO);
    clone_object(GLAD_OBJ  +"brut_cap")->move(TO);
    clone_object(TYR+"arm/body_chn.c")->move(TO);
    clone_object(TYR+"arm/arms_hid.c")->move(TO);
    clone_object(GLAD_OBJ +"test.c")->move(TO);
    command("wield all");
    command("wear all");
}
...
special_attack(object enemy)
{
    ...
    hitresult = enemy->hit_me(650+ random(250), W_BLUDGEON, me, -1);
...
no_train(string who)
{
  ...
  seteuid(getuid());
  save_object(GLAD_LOG+"brutus_attacker");
}
...
attacked_by(object ob)
{
   ...
   command("gladtell Attention Gladiators!  The guild is under "
	+"attack.  Please send reinforcements!\n"
	+"I repeat:  The guild is under attack, send reinforcements!");
...
{
	object enemy;
	environment(this_object())->jalla_func2(attacker);
	return ::hit_me();
}
*/
...
asktask()
{
    ...
    if((member_array(who->query_real_name(), attackers)) > -1)
    {
        ...
        command("kill "+lower_case(who->query_real_name()));
          environment(this_object()) -> jalla_func2(who);
        return "";
    }
