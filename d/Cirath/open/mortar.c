...
{
    return (function_exists("create_object", x) == HERB_OBJECT);
}
...
{
    return (function_exists("create_object", x) == HEAP_OBJECT);
}
...
{
    return (function_exists("create_heap",x) == DRINK_OBJECT);
}
...
set_up_mortar()
{
    ...
    while (i--)
    {
        d_potions = links[i]->query_potion_map();
        if (m_sizeof(d_potions))
            Ingredients += d_potions;
    }
}
