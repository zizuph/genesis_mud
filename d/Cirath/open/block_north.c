...
reset_room()
{
    object npc = present ("bartok", TO);
    if(!objectp(npc))
    {
	bring_room_mob("bartok", TYR_MESSAGE+"scorpion_merch.c", 1, 1);
    }
}
...
/*
   add_exit(TYR_STADIUM+"s_stadium.c","north","@@tunnel_message",1);
*/
   add_exit(TYR_STADIUM+"s_stadium.c","north","@@block_exit@@",1);
    add_exit(TYR_MARKET+"rd_sw_02.c","south",0,1);
    add_exit(GLAD+"room/entrance.c","west","@@stair_message",1);
    reset_room();
}
