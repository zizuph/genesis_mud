...
create_room()
{
	...
	add_exit(TYR_PITS+"pen9.c","east","@@block");
	add_exit(TYR_PITS +"pen3.c", "north");
	add_exit(TYR_PITS +"pen12.c", "south");
	add_exit(TYR_PITS +"pen13.c", "down", "@@pits");
	seteuid(getuid());
	reset_room();
}
block()
...
reset_room()
{
	...
	mon = clone_object(TYR_NPC+"goblinguard");
	mon->move(TO);
}
