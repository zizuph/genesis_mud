...
block_exit()
{
    TP->catch_msg("The Golden City is closed to travel "
                 +"while the plague remains.\n");
    return 1;
}
...
create_room()
{
    ...
    add_exit(TYR_MARKET+"rd_sw_03.c","north",0,1);
    add_exit(TYR_GATES+"gate_ts.c","east","@@block_exit@@",1);
    add_exit(TYR_MARKET+"rd_sw_01.c","south",0,1);
    reset_room();
}
