...
create_monster()
{
   ...
   set_long("This creature, burly even for his species, is covered head " +
            "to toe by flakes of bone, stone, and steel. He is hard at work " +
            "fashioning and repairing weapons, but seems quite content " +
            "and determined, as if he has found some important purpose for " +
            "his life.\n");
...
{
    reset_smith();
    return ::craftsman_purchase_syntax_failure_hook(who,str);
}
...
{
    reset_smith();
    return ::craftsman_purchase_exceeded_max_total_orders_hook(who,str);
}
...
{
    reset_smith();
    return ::craftsman_purchase_exceeded_max_user_orders_hook(who,str);
}
...
if(attr_map["material"] == "obsidian" && craftsman_query_item_file(id) !=TENT_OBS_WEP_FILE)
	   ...
	   if(attr_map["material"] == "black steel")
	   {
			if(!check_donation(TP->query_name()))
			{
				remove_prop(NPC_M_NO_ACCEPT_GIVE);
				command("grumble");
				DLY_EMOTE(1.0, "says: This is Athas my friend, if you want me to " +
						  "make you a metal weapon you will have to bring me " +
						  "something of value first.");
				DLY_EMOTE(1.5, "says: And I do not mean just any old weapon " +
...
craftsman_config_completed_hook(int id)
{
    ...
    mapping attr_map = order[ORDER_SELECTIONS];
   Status = 2;
   say(QCTNAME(TP) + " finishes " + HIS_HER(TP) + " conversation with " + QTNAME(TO) + ".\n");
    if (attr_map["_id"] == "weapon" && attr_map["material"] == "obsidian")
...
string coin_pay_text,
   ...
   else
   {
      ...
      cust->catch_msg("You pay "+coin_pay_text+" to " + QTNAME(this_object()) + ".\n");
      if (coin_get_text)
...
report_tentavvar_status()
{
   switch(Status)
   {
      case 0:
         ...
         break;
      case 1:
         notify_fail("Tentavvar is taking an order now, please wait.\n");
         return 0;
         break;
      case 2:
         notify_fail("Tentavvar is making an item for a customer now, " +
                     "please wait.\n");
         return 0;
         break;
      case 3:
         ...
         break;
      default:
         notify_fail("Tentavvar does not seem to know what he is doing. " +
   ...
   if(report_tentavvar_status())
   {
      switch(question)
      {
         ...
         case "about black steel":
            ...
            command("say And for this great kindness he demands not just simple " +
                    "coins but something of much greater value.");
...
string item_name,
   ...
   if (take_payment(tp, value))
   {
      ...
      tp->catch_msg("You give the "+item->short()+" to "+QTNAME(this_object())+".\n");
      say(QCTNAME(tp) + " gives " + tp->query_possessive() + " " + item->short() +
...
enter_inv(object ob, object from)
{
   ...
   if (ob->query_prop(OBJ_I_IS_MAGIC_WEAPON) != 0 && ob->query_prop(OBJ_I_BROKEN) == 0)
   {
      ...
      DLY_EMOTE(1.5, "says: I shall remember that you gave me the materials I seek.");
      DLY_EMOTE(1.7, "says: In the mean time I will make sure this goes where it " +
              "can be of use.");
      set_alarm(2.0, 0.0, &destroy_magic_item(ob));
      return;
   }

   DLY_EMOTE(1.0, "mumbles: I hope you don't think this " + ob->query_name() +
          " will be good enough to get me to make you a metal weapon.");
...
do_help(string topic)
{
   ...
   switch(topic)
   {
      ...
      case "blacksmith":
         ...
         "     'help weapons'     -  Information about the weapons you can order.\n" +
         "     'help obsidian'    -  Information about ordering obsidian weapons.\n" +
         "     'help armours'     -  Information about what armours you can order.\n" +
         "     'help recycle'     -  Information about unwanted and broken items.\n" +
         "     'help pay'         -  Information about how to pay for orders.\n\n";
         break;
      case "sharpen":
      case "sharpening":
         help_str = "\n" +
      ...
      case "ordering":
         ...
         "  To begin the ordering process type 'order weapon' or 'order armour'\n"+
         ...
         "  After you place an order it will be repeated back to you with a cost \n" +
         "  and the smith will start work on the item. \n"                           +
         "  You can type 'cancel order' at anytime before you pick up the item.\n"   +
         "  You can type 'help weapons', 'help armours', and 'help obsidian' for\n"  +
      ...
      case "dagger":
         ...
         "  You can order weapons or armours.\n"                                  +
         "  To begin the ordering process type 'order weapon' or 'order armour'\n"+
         "  You will then be asked to enter your choices for how the weapon or\n" +
         "  armour is to be made.\n"                                              +
         "  Your choices for weapon types are:\n"                                +
         "     " + format_str_list(m_indices(WEP_NAME), "and") + "\n\n"          +
         "  Your choices for weapon materials are:\n"                            +
         "     " + format_str_list(WEP_MATERIAL, "and") + "\n\n"                 +
         "  Obsidian stone types can be seen in the display case on the wall!\n\n"+
         "  Obsidian weapons can be finished is these ways:\n"                   +
         "     " + format_str_list(FINISH_TYPES, "or") + "\n\n"                  +
         "  If you want an obsidian weapon use 'help obsidian' to find out\n"    +
      ...
      case "reuse":
         help_str = "\n" +
         "  All materials are generally scarce in Athas. As such, this smith will\n" +
         "  collect any broken or unwanted items and make sure they are properly\n"  +
         "  destroyed and reused. Just give him an item you no longer want.\n"       +
         "  Be warned: Any item you give me will be destroyed and you will not\n"   +
         "  be able to get it back!\n\n";
         break;
      case "pay":
      case "payment":
         help_str = "\n" +
         "  After ordering an item, you will be given a short description of what\n"+
         "  you have ordered along with the price. When you order is complete \n"   +
         "  just type 'check order' and you will pay for the item and have it \n"   +
         "  given to you.\n\n"                    +
         "  You do not have the option of paying with certain coin types, you\n"    +
         "  always pay with your smallest coins first!\n\n"                         +
         "  Paying for sharpening, waxing or mending is done immediately if you\n"  +
...
{
   string log_txt = "";
   log_txt = sprintf("%-10s", TP->query_name()) + " waxed a " + item->query_name() + " " +
             ctime(time()) + "\n";
...
{
   string log_txt = "";
   log_txt = sprintf("%-10s", TP->query_name()) + " sharpened a " + item->query_name() + " " +
             ctime(time()) + "\n";
   log_file("tentavvar_repairs", log_txt, -1);
...
{
   string log_txt = "";
   log_txt = sprintf("%-10s", TP->query_name()) + " mended a " + item->query_name() + " " +
             ctime(time()) + "\n";
   log_file("tentavvar_repairs", log_txt, -1);
...
timeout_smith()
{
   ...
   {
      craftsman_abort_hook();
      command("say You have wasted enough of my time, if you want something you'll have " +
           "to order it again.");
   }
   reset_smith();
}
   ...
   switch(type)
   {
      ...
      case "bracers":
         say(QCTNAME(TO) + " stares at " + QTNAME(TP) + " carefully judging " +
                           HIS_HER(TO) + " size.\n");
         DLY_EMOTE(25.0, "turns around and selects some pieces of " + material +
                         " from the racks in the rear of the shop, laying " +
                         "them on the bench.");
         DLY_EMOTE(30.0, "reaches down and grabs some think, wide jozhal hide strips " +
                         "from under the bench and tosses them on the table.");
         DLY_EMOTE(35.0, "gives you one more quick glance and arranges " +
                         "the pieces of " + material + " face down on the " +
                         "table with the first sides of each part to be joined " +
         ...
         DLY_EMOTE(45.0, "drills some pilot holes he will use to " +
                         "attach the strapping.");
         DLY_EMOTE(50.0, "grabs a hammer and some steel pins and starts to " +
                         "fasten the parts together. Using no less than five " +
                         "pins for each attachment, this is going to be " +
                         "a well made piece of armour.");
         DLY_TELLR(55.0, "Carefully mushrooming the head of each pin, he " +
                         "starts to finish up the " + type + ".");
         DLY_EMOTE(60.0, "lays down his tools, looks over the " + 
                          type + ", and checks the movement and trim.");
      break;
      case "gauntlets":
      case "boots":
         ...
         DLY_EMOTE(20.0, "takes a pair of leather " + type + " from the " +
                         "racks in the rear of the shop and sets them " +
                         "on the bench.");
         DLY_EMOTE(25.0, "also grabs a couple of handfuls of " + material +
                         " pieces from the racks.");
         DLY_EMOTE(30.0, "sits down and starts the process of attaching " +
                         "the pieces of " + material + " to the leather " +
                         type + ", stopping to shape each one so they " +
         ...
         DLY_TELLR(45.0, "After trimming a few of the pieces in place, he is " +
                         "satisfied, sets it aside and begins work on the " +
                         "second of the pair.");
         DLY_EMOTE(55.0, "gives the last piece a final inspection and finishes " +
                         "up the " + type + ".");
         
      break;
      default:
         DLY_EMOTE(15.0, "starts to work on the item, it looks like it will "    +
                         "take a while.");
         DLY_EMOTE(25.0, "continues to work on the item, it looks like it will " +
                         "still take a while.");
         DLY_EMOTE(35.0, "continues to work on the item, it looks like he will " +
                         "be done shortly.");
         DLY_EMOTE(45.0, "continues to work on the item, it looks like he will " +
                         "be done soon.");
         DLY_EMOTE(55.0, "continues to work on the item, it looks like he is  "  +
                         "very close to being done.");
         DLY_EMOTE(60.0, "continues to work on the item, it looks like he is "   +
                         "finished.");
   }
}
...
{
   DLY_EMOTE(5.0, "turns around and takes a block of obsidian "  +
                  "from a stack in the corner.");
   DLY_EMOTE(10.0, "sets the block on the edge of his work bench and " +
                   "picks up a large hammer.");
   switch(weapon)
   {
   case "axe":
      ...
      DLY_EMOTE(35.0, "takes a short, wide piece of mekillot gut from a "   +
                      "pail on floor and lays it in the channel chipped "  +
                      "into the back of the axe head.");   
      ...
      DLY_EMOTE(55.0, "begins wrapping the axe head and the handle with the "    +
                      "gut strip, pulling it tighter and tighter as he "         +
                      "works his way up and down the back of the axe head.");
      DLY_EMOTE(60.0, "holds the head of the axe over the coals of his "         +
                      "forge fire, drying the strips of mekillot gut.");
      DLY_EMOTE(65.0, "spins the axe with blinding speed in one hand, checking " +
                      "it for balance, a smile spreads across his face.");
      DLY_EMOTE(70.0, "clamps the axe in a wooden vise, pulls a carving tool "   +
                      "from his apron and starts to engrave the handle.");
      DLY_EMOTE(75.0, "takes the axe from the vise and brushes off the handle.");
      break;
   case "sword":
      ...
      DLY_EMOTE(30.0, "gets to the hilt of the weapon and you can see he has "   +
                      "formed a savage edge on the front of the blade.");
      DLY_EMOTE(35.0, "turns the block around and starts to chip away at "       +
                      "back side of the blade.");
      DLY_EMOTE(40.0, "quickly gets to the tip of the blade, shards of glass "   +
                      "regularly hitting a growing pile of debris on the floor.");
      DLY_EMOTE(45.0, "flips the blade and starts working back down to the "     +
                      "hilt of the sword.");
      DLY_EMOTE(50.0, "finishes the blade of the sword and starts to form the "  +
                      "hilt, taking expert swings and rounding the end of "      +
                      "the block.");
      DLY_EMOTE(55.0, "puts down the hammer and gets a long strip of mekillot "  +
                      "hide from under the bench.");
      DLY_EMOTE(60.0, "starts wrapping the rough handle of the sword with the "  +
                      "leather, working up and down the rough stone he covers "  +
                      "the handle giving it a strong and sure grip.");
      DLY_EMOTE(65.0, "clamps the sword in a wooden vise and pulls an engraving "+
                      "tool from his apron and starts to engrave the handle "    +
                      "of the sword.");
      DLY_EMOTE(70.0, "finishes the engraving work and while sliding the tool "  +
                      "back into his apron he takes the sword from the vise and "+
   ...
   case "halberd":
      ...
      DLY_EMOTE(25.0, "finally starts chipping away at the obsidian, working  " +
                      "his way up the blade, knapping shards of obsidian off  " +
                      "as he goes.");
      DLY_EMOTE(30.0, "flips the block over and begins to chip away at the  "   +
                      "other side of the blade.");
      DLY_EMOTE(35.0, "finishes forming the halberd's blade and begins to  "    +
                      "narrow the obsidian at the base so it can be attached  " +
                      "to the handle.");
      DLY_EMOTE(40.0, "strikes the stone with sure and skilled strokes,  "      +
                      "creating a long, flat tongue of obsidian that will  "    +
                      "be fitted into the haft of the weapon.");
      DLY_EMOTE(45.0, "sets down his hammer and turns to get long, thick  "     +
                      "bone handle from the pile leaning against the  "         +
                      "obsidian rack.");
      DLY_EMOTE(50.0, "glances at the weapon head he just created and picks "   +
                      "up a bone cutting saw.");
      DLY_EMOTE(55.0, "saws a deep notch into the top of the weapons haft  "    +
                      "and picks up the obsidian blade from the table.");
      DLY_EMOTE(60.0, "forces the blade into the notch at the top of the  "     +
                      "handle with some effort.");
      DLY_EMOTE(65.0, "takes a long, wide strip of mekillot gut from a  "       +
                      "pail and the floor and starts wrapping the top of  "     +
                      "handle with it, binding the head to the haft.");
      DLY_EMOTE(70.0, "takes a strip of hide from under the bench and  "        +
                      "starts to wrap it around the main part of the  "         +
                      "weapon's handle, pulling it tight as he works his  "     +
                      "way along the haft.");
      DLY_EMOTE(75.0, "finally picks up a carving tool from the bench and "     +
                      "engraves the handle of the weapon.");
      break;
   case "club":
      DLY_EMOTE(15.0, "reaches down and grabs a tool from under the bench "   +
                      "that you do not recognize, it has the look of a "      +
                      "chisel, but with a curved, U shaped blade.");
      DLY_EMOTE(20.0, "grabs the long block of obsidian and looks it "        +
                      "over carefully, finally putting the larger, somewhat " +
                      "rounded end on the table and resting the other on "    +
                      "his shoulder.");
      DLY_EMOTE(25.0, "lays his chisel-like tool on the obsidian and using "  +
                      "the hammer starts to chip a channel down toward "      +
                      "the end of the block.");
      DLY_EMOTE(30.0, "works down the block, starting back up in the "        +
                      "middle after a full pass down the stone, each "        +
                      "time widening and deepening a channel in the block.");
      DLY_EMOTE(35.0, "rotates the stone a quarter turn and starts "          +
                      "to dig another channel in the obsidian.");
      DLY_EMOTE(40.0, "finishes making a wide channel after a few "           +
                      "passes, gives the block another quarter turn, "        +
                      "and starts in on the next side.");
      DLY_EMOTE(45.0, "is by now working quickly and gives the block "        +
                      "another turn to carve the last channel.");
      DLY_EMOTE(50.0, "completes the last channel and you realize that "      +
                      "he has just given this club four very sharp and "      +
                      "jagged edges.");
      DLY_EMOTE(55.0, "lays down his chisel, takes the stone off his "        +
                      "shoulder and starts to strike the very end of the "    +
                      "block, knocking off the rough corners and rounding "   +
      ...
      DLY_EMOTE(65.0, "starts furiously chipping away at the obsidian and "   +
                      "in no time gives the club a surprisingly well "        +
                      "tapered and not too jagged handle.");
      DLY_EMOTE(70.0, "grabs a long strip of mekillot hide and starts to "    +
                      "wrap the handle, pulling the leather tighter as "      +
                      "he works his way back and forth on the handle "        +
                      "to cover the sharp edges of the obsidian.");
      DLY_EMOTE(75.0, "finally picks up a carving tool from the bench and "   +
                      "engraves the handle of the weapon.");
      break;
   case "knife":
      DLY_EMOTE(15.0, "picks up the piece of obsidian from the bench and "    +
                      "holding it in one hand, braces it against his "        +
                      "waist.");
      ...
      DLY_EMOTE(29.0, "starts knapping the obsidian, chipping off small "     +
                      "shards and begins to form the front edge of the "      +
                      "knife.");
      ...
      DLY_EMOTE(64.0, "grabs a long strip of mekillot hide and starts to "    +
                      "wrap the handle, pulling the leather tighter as "      +
                      "he works his way up and down to cover the sharp "      +
                      "edges of the obsidian.");
      DLY_EMOTE(70.0, "finally picks up a carving tool from the bench and "   +
                      "engraves the handle of the weapon.");
      break;
   default:
      DLY_EMOTE(15.0, "starts to work on the weapon, it looks like it will "    +
                      "take a while.");
      DLY_EMOTE(25.0, "continues to work on the weapon, it looks like it will " +
                      "take a while.");
      DLY_EMOTE(35.0, "continues to work on the weapon, it looks like it will " +
                      "take a while.");
      DLY_EMOTE(45.0, "continues to work on the weapon, it looks like he will " +
                      "be done in a while.");
      DLY_EMOTE(55.0, "continues to work on the weapon, it looks like he will " +
                      "be done soon.");
      DLY_EMOTE(65.0, "continues to work on the weapon, it looks like he is  "  +
                      "very close to being done.");
      DLY_EMOTE(70.0, "continues to work on the weapon, it looks like he is "   +
                      "finished.");
   }
}
...
manufacture_std_wep(string weapon, string material)
{
   ...
   DLY_TELLR(10.0, "The half-giant grabs two iron handles sticking out " +
                   "of the floor in the rear of the shop and with a groan, " +
                   "lifts a huge stone block to reveal the door to a vault.");

   DLY_EMOTE(15.0, "disappears down a set of carved stone steps and returns " +
                   "with something.");
   ...
   switch(weapon)
   {
      ...
      DLY_TELLR(16.5, "It is a thin and moderately size axe head, with a " +
                      "blade face about the size of a half-giant's hand. " +
                      "At the back of the blade the metal has been formed " +
                      "into a channel for a handle to be attached.");
      break;
      case "halberd":
      DLY_TELLR(16.5, "It looks like an axe head, but with a longer, " +
                      "narrower blade, the ends of which curve backward. " +
                      "At the back of the blade the metal has been formed " +
                      "into a channel for a handle to be attached.");
      break;
      case "club":
      DLY_TELLR(16.5, "It is a short and narrow club, not much bigger at the " +
                      "head than the handle. The handle is formed from " +
                      "the same piece of metal, pounded a little less round " +
                      "than the shaft and head.");
      break;
      case "knife":
      DLY_TELLR(16.5, "It is a rather long knife, with the tip curved " +
                      "up slightly at the end. The base of the knife " +
                      "ends with a squared off tang that will form " +
                      "the handle.");
      break;
      case "sword":
      DLY_TELLR(16.5, "It is a moderate length sword blade, thin with " +
                      "a cutting edge on each side and very little taper " +
                      "to the point. The base of the blade " +
                      "ends with a squared off tang that will form " +
                      "the handle.");
      break;
   }
   ...
   DLY_EMOTE(25.0, "places his feet on the pedals that drive the wheel and " +
                   "starts to spin the wheel.");
   DLY_EMOTE(30.0, "runs the head of the club back and fourth on the wheel, " +
                   "turning it constantly to perfectly round the head.");
   DLY_EMOTE(35.0, "changes his grip on the club and moves down to work on " +
                   "the short area of taper between the head and grip.");
   DLY_EMOTE(40.0, "finishes his work at the wheel and stands up.");
   
   DLY_EMOTE(45.0, "picks up a large finishing stone, well worn in the middle " +
                   "from years of use.");
   ...
   {
   DLY_EMOTE(20.0, "places the " + weapon + " in the hot coals of the forge and " +
                   "pumps the bellows to heat the fire.");
   ...
   DLY_TELLR(35.0, "Glowing deep red, Tentavvar takes the " + weapon + " from " +
                  "the coals and lays it on the anvil.");

   DLY_TELLR(40.0, "Picking up a large hammer, Tentavvar starts to pound and " +
                   "thin the blade.");

   DLY_TELLR(45.0, "With careful and precise strokes Tentavvar starts " +
                   "to give the final edge to the " + weapon + ".");

   DLY_TELLR(50.0, "Finishing up the blade, Tentavvar lays down his " +
   ...
   DLY_TELLR(60.0, "Working carefully and deliberately, Tentavvar hones the " +
   ...
   switch(weapon)
   {
      ...
      DLY_TELLR(72.0, "Finally, he wraps the head and the handle with " +
                      "mekillot gut, which will shrink as it dries and " +
                      "permanently bond the head to the handle.");
      ...
      case "sword":
      DLY_EMOTE(64.0, "takes two small pieces of wood from under the bench, " +
                      "places them on either side of the tang and secures " +
                      "them with two steel pins.");
   ...
   DLY_EMOTE(75.0, "locks the " + weapon + " into a vise " +
                   "mounted on the bench, pulling an engraving tool " +
                   "from his apron.");
                   
   DLY_EMOTE(80.0, "starts to carve something into the handle.");
   DLY_EMOTE(85.0, "finishes the engraving, takes the " +
                    weapon + " from the vise and brushes off the handle.");
         break;
      default:
...
destroy_magic_item(object item)
{
   ...
   {
      DLY_EMOTE(3.0, "reaches into his apron and pulls out a small " +
               "stone orb.");
      ...
      DLY_EMOTE(12.0, "opens his eyes, turns and tosses the " + thing +
              " on a pile of scrap material and slides the orb back in " +
               "his apron.");
      set_alarm(15.0, 0.0,
