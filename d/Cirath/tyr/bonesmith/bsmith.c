...
create_room()
{
   ...
   set_long("You have entered the new blacksmith of Tyr's shop. "
	+"There is a large granite anvil on the floor near the "
	+"forge.  The forge is billowing dark smoke out through "
	+"a vent in the roof while the blacksmith is hammering "
	+"away on something with his back to you.  As you approach "
	+"the counter though, he turns and walks over to you with "
	+"a smile on his face.  You notice a sign propped up on "
	+"the counter that appears to be readable.\n");
   ...
   add_exit(TYR_CARAVAN+"weapmrch","west");
   add_exit(TYR_CARAVAN+"rd_cw_04","north");
...
{
    NF("Scrap what from where?\n");
    if(str != "flakes from anvil")
	return 0;
    write("You scrape some of the black flakes from the anvil into "+
      "your hand, and soon you have a small pile of black powder.\n");
    say(QCTNAME(TP)+" scrapes some of the black flakes from "+
      "the anvil into "+HIS(TP)+" hand.\n");
    clone_object("/d/Kalad/common/guilds/shadow_walkers/black_powder")->move(TP);
    return 1;
}
