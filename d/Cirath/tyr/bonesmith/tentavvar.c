/*
 * /some path to be determined/tentavvar.c
 *
 * The weapon and armour smith of Tyr
 *
 * Taken in part from /d/Gondor/morgul/npc/ghashdurub.c
 * and some from /d/Gondor/minas/npc/calmacil.c
 * 
 * This blacksmith can sharpen and wax weapons, repair armours
 * and manufacture weapons.
 *
 * Fylo, March 2000
 *
 * Modification log: Added shields Damaris 9/2002
 *                   Removed some typos Damaris 4/2004
 *                   Fixed the destry_magic_item and the
 *                   destroy_std_item routines. - Mirandus 12/17
 */
#pragma strict_types
...
/* Global variables */
int Status = 0;   /* Status of Tentavvar */
...
create_monster()
{
	...
	add_name(({"smith", "blacksmith", "weapon smith", "weapon maker",
        ...
        set_race_name("dwarf");
        set_adj(({"strong", "melancholy"}));
        set_title("the Master Smith of Tyr");
        set_long("This creature, burly even for his species, is "+
        "covered head to toe by flakes of bone, stone, and steel. He "+
        "is hard at work fashioning and repairing weapons, but seems "+
        "quite content and determined, as if he has found some "+
        "important purpose for his life.\n");
...
arm_me()
{
	clone_object(TYR_BONESMITH + "tentavvar_apron")->move(TO);
	clone_object(TYR_BONESMITH + "tentavvar_pants")->move(TO);
	command("wear all");
}
...
{
	if(report_tentavvar_status())
	{
		say(QCTNAME(TP) + " starts to exchange words "+
		"with " + QTNAME(TO) + ".\n");
		add_prop(NPC_M_NO_ACCEPT_GIVE, " is busy with "+
		"a customer.\n");
		Status = 1;
...
craftsman_config_attributes(int id, int pos)
{
	...
	say(QCTNAME(TP) + " exchanges some words with "+
	"" + QTNAME(TO) + ".\n");
	if (pos < sizeof(order[ORDER_ATTRS]))
	{
	   ...
	   if(attr_map["material"] == "black steel")
	   {
		   if(!check_donation(TP->query_name()))
		   {
			   remove_prop(NPC_M_NO_ACCEPT_GIVE);
			   command("grumble");
			   DLY_EMOTE(1.0, "says: This is Athas my "+
			   "friend, if you want me to make you a "+
			   "metal weapon you will have to bring me "+
			   "something of value first.");
			   DLY_EMOTE(1.5, "says: And I do not mean "+
			   "just any old weapon you can find on the "+
			   "ground.");
...
craftsman_completed_order_hook(int id, string owner)
{
	...
	if ((who = find_player(owner)) && craftsman_query_object_present(who))
	{
		...
		who->catch_tell("Use 'check order' to collect your "+
		"items.\n");
	}
	owner = CAP(owner);
...
craftsman_cancel_order_hook(int id)
{
	...
	DLY_EMOTE(1.0, "stops working, lays down his tools and returns "+
	"what he was working on to the racks.");
...
craftsman_check_receive_order_hook(object ob, int id)
{
	...
	if (order[ORDER_SELECTIONS]["_id"] == "armour")
	{
		...
		log_file("tentavvar_creations", log_txt, -1);
	}
	else
	{
		log_txt = sprintf("%-10s", TP->query_name()) + ob->query_short() + " ";
...
take_payment(object cust, int cost)
{
	...
	{
		coin_pay_text = text(exclude_array(money_arr, sizeof(MONEY_TYPES), sizeof(MONEY_TYPES)*2-1));
...
report_tentavvar_status()
{
	switch(Status)
	{
		...
		break;
		case 1:
		notify_fail("Tentavvar is taking an order now,"+
		"please wait.\n");
		return 0;
		break;
		case 2:
		notify_fail("Tentavvar is making an item for a "+
		"customer now, please wait.\n");
		return 0;
		break;
		case 3:
		notify_fail("As soon as finishes what he is working "+
		"on, he will deal with you.\n");
		return 0;
		break;
		default:
		notify_fail("Tentavvar does not seem to know what "+
		"he is doing. Please make a bug report.\n");
		return 0;
	}
}
...
int report_cost(string str)
{
	...
	if(report_tentavvar_status())
	{
		if (!strlen(str))
		{
			command("say You want to know the repair cost "+
			"for which item?\n");
			return 1;
		}
		...
		{
			command("say You want to know the cost to "+
			"mend what?");
			return 1;
		}
		...
		if (sizeof(item) > 1)
		{
			command("say Please be more specific! Which item "+
			"do you mean?");
			return 1;
		}
		if (sizeof(item) == 0)
...
catch_question(string question)
{
	if(report_tentavvar_status())
	{
		switch(question)
		{
			...
			command("say The great King Kalak has granted "+
			"me special favor to manufacture weapons made "+
			"of black steel from his mines.");
			command("say And for this great kindness he "+
			"demands not just simple coins but something "+
			"of much greater value.");
			command("say Kalak demands the powers of "+
			"enchantment themselves and has given me the "+
			"task to collect them for him in exchange for "+
			"my services in this shop.");
			command("say So, if it is an steel weapon you "+
			"want, you must first return to me a vessel of "+
			"this enchantment.");
			break;
			default:
			do_help("tentavvar");
...
do_repair_cost(object item, object tp)
{
	...
	/* if the item is a weapon we calc the cost of sharpening and 
      waxing here. */
      {
	      ...
	      {
		      command("say But that " + item->short() + " "+
		      "is broken!");
	       ...
	       if (v1 > -1)
	       {
		       command("say Sharpening "+it+" would cost "+
		       "about " + distort(v1) + " coppers.");
		       what = "sharpen";
	       }
	       if (v2 > -1)
	       {
		       command("say Waxing "+it+" would cost "+
		       "about " + distort(v2) + " coppers.");
		       if (strlen(what))
		       what += " or ";
		       what += "wax";
		}
		...
		command("say If you want me to "+ what +" the item, "+
		"use '"+ what + " <item>'.");
		return 1;
	}
	...
	/*the cost to mend*/
	{
		...
		if (v3 > -1)
		{
			...
			""+ distort(v3) + " coppers.");
			command("say If you want me to mend the item, "+
			"use 'mend <item>'.");
		}
		else
		{
			command("say "+capitalize(it)+" does not "+
			"need mending!");
		}
		return 1;
	}
...
enter_inv(object ob, object from)
{
   ...
   if(ob->id("_aod_forged_item"))
   {
	   DLY_EMOTE(1.0, "says: This item appears to have been touched by darkling " +
	   "magic. I refuse to touch this.");
	   set_alarm(1.5, 0.0, &return_object(ob, from));
       return;
   }   
...
fix_wax(object item)
{
   ...
   log_txt = sprintf("%-10s", TP->query_name()) + " waxed a "+
     "" + item->query_name() + " " +
             ctime(time()) + "\n";
   log_file("tentavvar_repairs", log_txt, -1);
 ...
 * Description:   Resets Tentavvar status so he can do more work
 * Arguments:     none
 * Returns:       nothing
 *
 */
...
{
	DLY_EMOTE(5.0, "turns around and takes a block of obsidian from "+
	"a stack in the corner.");
	DLY_EMOTE(10.0, "sets the block on the edge of his work bench "+
	"and picks up a large hammer.");
	switch(weapon)
	{
		case "axe":
		DLY_EMOTE(15.0, "hits the block with a perfect stroke "+
		"and fractures the face of the block.");
		DLY_EMOTE(20.0, "flips the block of obsidian over and "+
		"raises his hammer to strike the block again.");
		DLY_EMOTE(25.0, "hits the block perfectly a second time.");
		DLY_EMOTE(30.0, "grins as he admires his handy work.");
		DLY_EMOTE(35.0, "takes a short, wide piece of mekillot "+
		"gut from a pail on floor and lays it in the channel "+
		"chipped into the back of the axe head.");
		DLY_EMOTE(40.0, "takes a polished bone handle from a "+
		"rack.");
		DLY_EMOTE(45.0, "lays the axe handle into the groove "+
		"in the back of axe head.");
		DLY_EMOTE(50.0, "pulls a long thin strip of mekillot "+
		"gut from the pail.");
		DLY_EMOTE(55.0, "begins wrapping the axe head and the "+
		"handle with the gut strip, pulling it tighter and "+
		"tighter as he works his way up and down the back of "+
		"the axe head.");
		DLY_EMOTE(60.0, "holds the head of the axe over the "+
		"coals of his forge fire, drying the strips of mekillot "+
		"gut.");
		DLY_EMOTE(65.0, "spins the axe with blinding speed in "+
		"one hand, checking it for balance, a smile spreads "+
		"across his face.");
		DLY_EMOTE(70.0, "clamps the axe in a wooden vise, pulls "+
		"a carving tool from his apron and starts to engrave "+
		"the polished bone handle.");
		DLY_EMOTE(75.0, "takes the axe from the vise and brushes "+
		"off the polished bone handle.");
		break;
		case "sword":
		DLY_EMOTE(15.0, "lays the long, narrow block of obsidian "+
		"on the bench.");
		DLY_EMOTE(18.0, "begins knapping flakes of obsidian from "+
		"the block, working his way up to the end of the block.");
		DLY_EMOTE(25.0, "flips the block over and begins "+
		"chipping stone away working back down the blade toward "+
		"the hilt.");
		DLY_EMOTE(30.0, "gets to the hilt of the weapon and you "+
		"can see he has formed a savage edge on the front of the "+
		"blade.");
		DLY_EMOTE(35.0, "turns the block around and starts to "+
		"chip away at back side of the blade.");
		DLY_EMOTE(40.0, "quickly gets to the tip of the blade, "+
		"shards of glass regularly hitting a growing pile of "+
		"debris on the floor.");
		DLY_EMOTE(45.0, "flips the blade and starts working "+
		"back down to the hilt of the sword.");
		DLY_EMOTE(50.0, "finishes the blade of the sword and "+
		"starts to form the hilt, taking expert swings and "+
		"rounding the end of the block.");
		DLY_EMOTE(55.0, "puts down the hammer and gets a long "+
		"strip of mekillot hide from under the bench.");
		DLY_EMOTE(60.0, "starts wrapping the rough handle of "+
		"the sword with the leather, working up and down the "+
		"rough stone he covers the handle giving it a strong "+
		"and sure grip.");
		DLY_EMOTE(65.0, "clamps the sword in a wooden vise "+
		"and pulls an engraving tool from his apron and starts "+
		"to engrave the handle of the sword.");
		DLY_EMOTE(70.0, "finishes the engraving work and while "+
		"sliding the tool back into his apron he takes the "+
		"sword from the vise and blows the obsidian dust from "+
		"the handle.");
		break;
		case "halberd":
		DLY_EMOTE(17.0, "studies the block carefully, "+
		"determining the best side to make the front of the "+
		"blade.");
		DLY_EMOTE(25.0, "finally starts chipping away at the "+
		"obsidian, working  his way up the blade, knapping "+
		"shards of obsidian off as he goes.");
		DLY_EMOTE(30.0, "flips the block over and begins to "+
		"chip away at the other side of the blade.");
		DLY_EMOTE(35.0, "finishes forming the halberd's blade "+
		"and begins to narrow the obsidian at the base so it "+
		"can be attached to the handle.");
		DLY_EMOTE(40.0, "strikes the stone with sure and "+
		"skilled strokes, creating a long, flat tongue of "+
		"obsidian that will be fitted into the haft of the "+
		"weapon.");
		DLY_EMOTE(45.0, "sets down his hammer and turns to get "+
		"long, thick polished bone handle from the pile leaning "+
		"against the obsidian rack.");
		DLY_EMOTE(50.0, "glances at the weapon head he just "+
		"created and picks up a polished bone cutting saw.");
		DLY_EMOTE(55.0, "saws a deep notch into the top of "+
		"the weapons shaft and picks up the obsidian blade from "+
		"the table.");
		DLY_EMOTE(60.0, "forces the blade into the notch at the "+
		"top of the handle with some effort.");
		DLY_EMOTE(65.0, "takes a long, wide strip of mekillot "+
		"gut from a pail and the floor and starts wrapping the "+
		"top of handle with it, binding the head to the haft.");
		DLY_EMOTE(70.0, "takes a strip of hide from under the "+
		"bench and starts to wrap it around the main part of "+
		"the weapon's handle, pulling it tight as he works his "+
		"way along the haft.");
		DLY_EMOTE(75.0, "finally picks up a carving tool from "+
		"the bench and engraves the handle of the weapon.");
		break;
		case "club":
		DLY_EMOTE(15.0, "reaches down and grabs a tool from "+
		"under the bench that you do not recognize, it has the "+
		"look of a chisel, but with a curved, U shaped blade.");
		DLY_EMOTE(20.0, "grabs the long block of obsidian and "+
		"looks it over carefully, finally putting the larger, "+
		"somewhat rounded end on the table and resting the other "+
		"on his shoulder.");
		DLY_EMOTE(25.0, "lays his chisel-like tool on the "+
		"obsidian and using the hammer starts to chip a channel "+
		"down toward the end of the block.");
		DLY_EMOTE(30.0, "works down the block, starting back up "+
		"in the middle after a full pass down the stone, each "+
		"time widening and deepening a channel in the block.");
		DLY_EMOTE(35.0, "rotates the stone a quarter turn and "+
		"starts to dig another channel in the obsidian.");
		DLY_EMOTE(40.0, "finishes making a wide channel after a "+
		"few passes, gives the block another quarter turn, and "+
		"starts in on the next side.");
		DLY_EMOTE(45.0, "is by now working quickly and gives "+
		"the block another turn to carve the last channel.");
		DLY_EMOTE(50.0, "completes the last channel and you "+
		"realize that he has just given this club four very sharp and jagged edges.");
		DLY_EMOTE(55.0, "lays down his chisel, takes the stone "+
		"off his shoulder and starts to strike the very end of "+
		"the block, knocking off the rough corners and rounding "+
		"out the head of the club a little.");
		DLY_EMOTE(60.0, "spins the block around on his bench and "+
		"looks over the untouched end of the block.");
		DLY_EMOTE(65.0, "starts furiously chipping away at the "+
		"obsidian and in no time gives the club a surprisingly "+
		"well tapered and not too jagged handle.");
		DLY_EMOTE(70.0, "grabs a long strip of mekillot hide and "+
		"starts to wrap the handle, pulling the leather tighter "+
		"as he works his way back and forth on the handle to "+
		"cover the sharp edges of the obsidian.");
		DLY_EMOTE(75.0, "finally picks up a carving tool from "+
		"the bench and engraves the handle of the weapon.");
		break;
		case "knife":
		DLY_EMOTE(15.0, "picks up the piece of obsidian from "+
		"the bench and holding it in one hand, braces it "+
		"against his waist.");
		DLY_EMOTE(22.0, "smirks a little as he puts down the "+
		"large hammer and grabs a smaller one from the bench.");
		DLY_EMOTE(29.0, "starts knapping the obsidian, chipping "+
		"off small shards and begins to form the front edge of "+
		"the knife.");
		DLY_EMOTE(36.0, "works carefully, flipping the stone "+
		"and twisting his hand around to work both sides of "+
		"the knife.");
		DLY_EMOTE(43.0, "finishes the front edge of the blade "+
		"and begins to work on the back edge of knife.");
		DLY_EMOTE(50.0, "keeps knapping more and more stone, "+
		"making it smaller and forming the final blade of the "+
		"knife.");
		DLY_EMOTE(57.0, "finishes up the blade and begins to "+
		"tapper the handle.");
		DLY_EMOTE(64.0, "grabs a long strip of mekillot hide "+
		"and starts to wrap the handle, pulling the leather "+
		"tighter as he works his way up and down to cover the "+
		"sharp edges of the obsidian.");
		DLY_EMOTE(70.0, "finally picks up a carving tool from "+
		"the bench and engraves the handle of the weapon.");
		break;
		default:
		DLY_EMOTE(15.0, "starts to work on the weapon, it "+
		"looks like it will take a while.");
		DLY_EMOTE(25.0, "continues to work on the weapon, it "+
		"looks like it will take a while.");
		DLY_EMOTE(35.0, "continues to work on the weapon, it "+
		"looks like it will take a while.");
		DLY_EMOTE(45.0, "continues to work on the weapon, it "+
		"looks like he will be done in a while.");
		DLY_EMOTE(55.0, "continues to work on the weapon, it "+
		"looks like he will be done soon.");
		DLY_EMOTE(65.0, "continues to work on the weapon, it "+
		"looks like he is very close to being done.");
		DLY_EMOTE(70.0, "continues to work on the weapon, it "+
		"looks like he is finished.");
	}
}
...
manufacture_std_wep(string weapon, string material)
{
	switch(material)
	{
		...
		case "black steel":
		DLY_TELLR(5.0, "A massive male half-giant steps out of "+
		"the shadows.");
		DLY_TELLR(10.0, "The half-giant grabs two iron handles "+
		"sticking out of the floor in the rear of the shop and "+
		"with a groan, lifts a huge stone block to reveal the "+
		"door to a vault.");
		DLY_EMOTE(15.0, "disappears down a set of carved stone "+
		"steps and returns with something.");
		switch(weapon)
		{
			case "axe":
			DLY_TELLR(16.5, "It is a thin and moderately "+
			"size axe head, with a blade face about the "+
			"size of a half-giant's hand. At the back of "+
			"the blade the metal has been formed into a "+
			"channel for a handle to be attached.");
			break;
			case "halberd":
			DLY_TELLR(16.5, "It looks like an axe head, "+
			"but with a longer, narrower blade, the ends of which curve backward. At the back of the blade the metal has been formed into a channel for a handle to be attached.");
      break;
      case "club":
      DLY_TELLR(16.5, "It is a short and narrow club, not much bigger at the head than the handle. The handle is formed from the same piece of metal, pounded a little less round than the shaft and head.");
      break;
      case "knife":
      DLY_TELLR(16.5, "It is a rather long knife, with the tip curved up slightly at the end. The base of the knife ends with a squared off tang that will form the handle.");
      break;
      case "sword":
      DLY_TELLR(16.5, "It is a moderate length sword blade, thin with a cutting edge on each side and very little taper to the point. The base of the blade ends with a squared off tang that will form the handle.");
      break;
      }
      ...
      if(weapon == "club")
      {
	      DLY_EMOTE(20.0, "walks over to a grinding wheel and sits "+
	      "down.");
	      DLY_EMOTE(25.0, "places his feet on the pedals that drive "+
	      "the wheel and starts to spin the wheel.");
	      DLY_EMOTE(30.0, "runs the head of the club back and fourth "+
	      "on the wheel, turning it constantly to perfectly round "+
	      "the head.");
	      DLY_EMOTE(35.0, "changes his grip on the club and moves "+
	      "down to work on the short area of taper between the head "+
	      "and grip.");
	      DLY_EMOTE(40.0, "finishes his work at the wheel and stands "+
	      "up.");
	      DLY_EMOTE(45.0, "picks up a large finishing stone, well "+
	      "worn in the middle from years of use.");
	      DLY_TELLR(50.0, "Laying the club on the bench he starts "+
	      "to run the stone up and down the length of the weapon.");
	      DLY_TELLR(55.0, "After a short time the metal is smoothed "+
	      "to a sinister looking mat finish.");
	      DLY_EMOTE(60.0, "grabs a thick cloth and wipes down the "+
	      "weapon.");
	}
	else
	{
		DLY_EMOTE(20.0, "places the " + weapon + " in the hot "+
		"coals of the forge and pumps the bellows to heat the "+
		"fire.");
		DLY_EMOTE(25.0, "picks up a pair of tongs.");
		DLY_EMOTE(30.0, "turns the " + weapon + " in the fire "+
		"with the tongs, heating it evenly and thoroughly.");
		DLY_TELLR(35.0, "Glowing deep red, Tentavvar takes the "+
		"" + weapon + " from the coals and lays it on the anvil.");
		DLY_TELLR(40.0, "Picking up a large hammer, Tentavvar "+
		"starts to pound and thin the blade.");
		DLY_TELLR(45.0, "With careful and precise strokes "+
		"Tentavvar starts to give the final edge to the "+
		"" + weapon + ".");
		DLY_TELLR(50.0, "Finishing up the blade, Tentavvar "+
		"lays down his hammer and picks up a fine grain "+
		"polishing stone.");
		DLY_EMOTE(55.0, "sits down at the work bench, braces "+
		"the blade against the table and starts to run the "+
		"stone over the blade.");
		DLY_TELLR(60.0, "Working carefully and deliberately, "+
		"Tentavvar hones the edge to a razor's sharpness.");
	}
	switch(weapon)
	{
		case "axe":
		DLY_EMOTE(64.0, "takes a solid wood handle from the "+
		"rack in the back of the shop and fits it into the "+
		"rear of the axe head.");
		DLY_TELLR(68.0, "And then with a hammer, he drives "+
		"two steel pins through both the metal and the handle.");
		DLY_TELLR(72.0, "Finally, he wraps the head and the "+
		"handle with mekillot gut, which will shrink as it "+
		"dries and permanently bond the head to the handle.");
		break;
		case "halberd":
		DLY_EMOTE(64.0, "takes a solid wood handle from the "+
		"rack in the back of the shop and fits it into the "+
		...
		break;
		case "club":
		DLY_EMOTE(64.0, "pulls a long, wide strip of mekillot "+
		"hide from under the bench and wets it in a pail.");
		DLY_EMOTE(68.0, "tightly wraps the handle with the "+
		"leathery hide.");
		DLY_TELLR(72.0, "holds the handle over the coals for "+
		"a moment, to dry and harden the leather grip.");
		break;
		case "knife":
		DLY_EMOTE(64.0, "takes two small pieces of wood from "+
		"under the bench.");
		DLY_EMOTE(68.0, "fits the small blocks on either side "+
		"of the tang and secures them with two steel pins.");
		DLY_EMOTE(72.0, "steps over to the wheel and quickly "+
		"grinds the pins flush with the wooden handle.");
		break;
		case "sword":
		DLY_EMOTE(64.0, "takes two small pieces of wood from "+
		"under the bench, places them on either side of the "+
		"tang and secures them with two steel pins.");
		DLY_EMOTE(68.0, "takes a long, thin strip of mekillot "+
		"hide from under the bench and dampens it in a pail.");
		DLY_EMOTE(70.0, "wraps the handle, tapering the leather "+
		"at the top and bottom, forming a sure, comfortable "+
		"grip.");
		break;
		}
		DLY_EMOTE(75.0, "locks the " + weapon + " into a "+
		"vise mounted on the bench, pulling an engraving tool "+
		"from his apron.");
		DLY_EMOTE(80.0, "starts to carve something into the "+
		"handle.");
		DLY_EMOTE(85.0, "finishes the engraving, takes the "+
		"" + weapon + " from the vise and brushes off the "+
		"handle.");
		break;
		default:
		DLY_EMOTE(5.0, "turns around and grabs a roughly "+
		"finished " + material + " " + weapon + " from a rack.");
		DLY_EMOTE(15.0, "sits down at a grinding wheel and "+
		"begins giving the " + weapon + " a finished shape.");
		if(weapon != "club")
		{
			DLY_EMOTE(25.0, "bends close to the wheel "+
			"and starts to work on the blade's final edge.");
			DLY_EMOTE(35.0, "slowly works the blade back "+
			"and forth along the wheel honing the edge.");
		}
		else
		{
			DLY_EMOTE(25.0, "turns the club constantly on "+
			"the wheel, rounding and shaping the head.");
			DLY_EMOTE(35.0, "leans hard into the grinding "+
			"wheel as he tapers to form a handle for the "+
			"club.");
		}
		DLY_EMOTE(45.0, "stands up and locks the " + weapon + " "+
		"into a vise mounted on the bench, pulling an engraving "+
		"tool from his apron.");
		DLY_EMOTE(50.0, "starts to carve something into the "+
		"handle.");
		DLY_EMOTE(55.0, "finishes the engraving and takes the "+
		"" + weapon + " from the vise and brushes off the "+
		"handle.");
	}
}
...
destroy_magic_item(object item)
{
	...
	log_file("tentavvar_donations", log_txt, -1);
	DLY_EMOTE(3.0, "reaches into his apron and pulls out "+
	"a small stone orb.");
	DLY_EMOTE(6.0, "grasps the " + thing + " in one hand "+
	"and the small orb in the other.");
	DLY_EMOTE(9.0, "closes his eyes and tightens his grip "+
	"on both for a moment, the orb pulses in his hand.");
	DLY_EMOTE(12.0, "opens his eyes, turns and tosses the "+
	"" + thing + " on a pile of scrap material and slides "+
	"the orb back in his apron.");
	set_alarm(15.0, 0.0, &command("say Don't worry that "+
	"" + thing + " will be properly recycled, if not here, "+
	"then somewhere else in the realms."));
	item->remove_object();
...
destroy_std_item(object item)
{
	...
	if(broken)
	{
		set_alarm(9.0, 0.0, &command("say This thing is "+
		"broken!"));
	}
	else
	{
		set_alarm(6.0, 0.0, &command("hmm"));
		set_alarm(9.0, 0.0, &command("say Nothing very "+
		"special about this " + thing + "."));
	}
	set_alarm(12.0, 0.0, &command("say Either way, anything "+
	"can be reused here in Athas."));
	DLY_EMOTE(14.0, "turns and tosses the " + thing + " on "+
	"a pile of old weapons and armours.");
	set_alarm(15.0, 0.0, &command("say Don't worry that "+
	"" + thing + " will be properly recycled, if not here, "+
	"then somewhere else in the realms."));
	item->remove_object();
