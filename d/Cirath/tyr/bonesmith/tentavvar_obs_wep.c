 ...
 * Coded by Fylo 2000
 *
 * An obsidian weapon manufactured by Tentavvar, the Master Smith of Tyr. 
 *
 * 2021-04-03 - Cotillion
 * - Fixed recovery
 */
...
config_weapon(string type, string finish, string stone, string hands)
{
   ...
   add_name(Finish + " " + WEP_NAME[Type][0]);
   add_name(Stone + " " + WEP_NAME[Type][0]);
...
init_recover(string arg)
{
   ...
   if (!sscanf(arg, "%s#tow_t#%s#tow_f#%s#tow_s#%s#tow_h#%s#tow_e#%s#%s", foobar, Type, Finish, Stone, Hands, Engraving, foobar))
    {
      write("ERROR in init_recover: " + Type + Finish + Stone + Hands);
      TO->remove_object();
    }
   //sscanf(arg, "%s#tow_t#%s#%s", foobar, Type, foobar);
   //sscanf(arg, "%s#tow_f#%s#%s", foobar, Finish, foobar);
   //sscanf(arg, "%s#tow_s#%s#%s", foobar, Stone, foobar);
   //sscanf(arg, "%s#tow_h#%s#%s", foobar, Hands, foobar);
   //sscanf(arg, "%s#tow_e#%s#%s", foobar, Engraving, foobar);
   init_wep_recover(arg);
   if(!config_weapon(Type, Finish, Stone, Hands))
   {
      write("ERROR in init_recover: " + Type + Finish + Stone + Hands);
      TO->remove_object();
   }
