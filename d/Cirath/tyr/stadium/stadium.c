 ...
 * - Modified to not clone shadows into the void, also fixed so
 *   it can be updated.
 */
inherit "/std/room";
#include "defs.h"
#include <cmdparse.h>
#define SHADOW TYR_STADIUM+"stadiumshadow"
...
leave_inv(object ob,object to)
{
    ...
    if (living(ob))
    {
        elist = ob->query_enemy(-1);
        if (!pointerp(elist))
            elist = ({ });
        
	elist = elist & all_inventory(environment(ob));
