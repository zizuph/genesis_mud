...
create_room()
 {
  set_short("Path");
  set_long("The path that leads westward extends into a drier area "
	+"of the mansions courtyard. A strange smell utterly un-Athasian "
	+"is still present though. The ground here is covered in a "
	+"coarse grass that is dry and crunches underfoot. There are "
	+"ruts worn into the ground here also, as a result from some "
	+"wheeled vehicle making regular passes down this path.\n");
  ...
  ADD_SUN_ITEM
  ADD_ZIG_ITEM
  add_exit(TZ_MINTHOR + "mh3","east", 0);
  add_exit(TZ_MINTHOR + "mh9","west", 0);
}
