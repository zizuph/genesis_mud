...
void create_room()
{
  set_short("the storeroom of the weapon merchant");
    set_long("Various weapons are stored on shelves to be sold later in "+
             "Grik's shop.\n");
    INSIDE
    add_exit(TYR_CARAVAN+"weapmrch.c", "north", 0, 1);
...
#define STNEDGGR (TYR_WEP + "knife_sd.c")
#define QUABONE  (TYR_WEP + "quabone.c")
#define STONEAXE (TYR_WEP + "axe_ston.c")
 ...
 */
mixed to_check = ({ ({"create_weapon", STNEDGGR, 1}),
                    ({"create_weapon", QUABONE, 1}),
                    ({"create_weapon", STONEAXE, 1}),
                 });
