...
*   2022-10-28 Cotillion Fixed runtime issue
*/
...
mixed armed_hit(string with, object enemy, string burn);
mixed flame_shield_hit(object enemy, string burn);
mixed flame_items(object ob);
mixed start_mana_drain();

int flames = 0;
object gloves_shadow = 0;
...
create_unarmed_enhancer()
{
    ...
    set_default_enhancer(45, 47, W_BLUDGEON, 5, A_HANDS, ({ -2, 0, 2 }), this_object());
...
// Can only reach if have either, so only checking if defiler.
    if (TP->query_prop(C_I_AM_DEFILER))
    {
        ...
        TP->add_prop(OBJ_I_LIGHT, query_prop(OBJ_I_LIGHT) + flames);
...
wear()
{
    ...
    if (objectp(gloves_shadow))
    {
        gloves_shadow->remove_shadow();
        gloves_shadow = 0;
    }
...
remove()
{
    if (objectp(gloves_shadow))
    {
        // Shadow removed from shadowed version of remove()
         gloves_shadow = 0;
    }
...
*/
void
armed_hit(string with, object enemy, string burn)
{
    object wearer = query_worn();
...
*/
void
flame_shield_hit(object enemy, string burn)
{
    object wearer = query_worn();
