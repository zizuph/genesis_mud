/*
    ...
    Function slayattack(string str) should be executed when player issues command

    Following functions could be masked in division specific souls in order
    to customize behaviour of slayattack(). Default behaviour is that of blue
    army special slash. Only slayattack_message() needs to be masked always.
    For explanation see below.

int query_slayattack_damage_type()
float query_slayattack_prepare_time(object tp)
float query_slayattack_relax_time(object tp)
int query_slayattack_weapon_aid(object tp, object weapon)
int query_slayattack_weapon_hit(object tp, object weapon)
int query_slayattack_weapon_pen(object tp, object weapon)
int query_slayattack_skill(object tp)

void slayattack_do_die(object tp, object target)
void slayattack_message(object tp, object target,
		string weapon, int weapon_type,
		int phurt, string hitloc, int phit, int dam)

object choose_slayattack_weapon(object tp)
*/
#pragma strict_types
#pragma save_binary
#pragma no_clone
...
#define SLAYATTACK_PROP "_barbarian_slayattack_prop"
/*** Prototypes ****************************************************/
...
/*
 * Function name: query_slayattack_damage_type
 * Returns:       int - damage type of our special attack
 */
int
query_slayattack_damage_type()
{
    return W_SLASH;
}

/*
 * Function name: query_slayattack_prepare_time
 * Arguments:     object tp	- who does special attack
 * Returns:       float - time between command and actual attack
 */
float
query_slayattack_prepare_time(object tp)
{
    return itof(10 + random(5));
}

/*
 * Function name: query_slayattack_relax_time
 * Arguments:     object tp	- who does special attack
 * Returns:       float - time to wait after attack before new can be started
 */
float
query_slayattack_relax_time(object tp)
{
    return itof(10 + random(5));
}

/*
 * Function name: query_slayattack_weapon_aid
 * Arguments:     object tp	- who does special attack
 ...
 */
int
query_slayattack_weapon_aid(object tp, object weapon)
{
...
/*
 * Function name: query_slayattack_weapon_hit
 * Arguments:     object tp	- who does special attack
 *		  object weapon - weapon used for special attack
 * Returns:       int - hit value special attack is performed with
 */
int
query_slayattack_weapon_hit(object tp, object weapon)
{
    return weapon->query_hit()*3;
}

/*
 * Function name: query_slayattack_weapon_pen
 * Arguments:     object tp	- who does special attack
 *		  object weapon - weapon used for special attack
 * Returns:       int - pen value special attack is performed with
 */
int
query_slayattack_weapon_pen(object tp, object weapon)
{
...
/*
 * Function name: query_slayattack_skill
 * Arguments:     object tp	- who does special attack
 * Returns:       int - how skilled is for special attack
 */
int
query_slayattack_skill(object tp)
{
    return tp->query_skill(SS_SLAYING);
}

/*
 * Function name: slayattack_do_die
 ...
 */
void
slayattack_do_die(object tp, object target)
{
    target->do_die(tp);
}

/*
 * Function name: slayattack_message
...
void
slayattack_message(object tp, object target,
		string weapon, int weapon_type,
		int phurt, string hitloc, int phit, int dam)

{
    	if(phurt > 59)
    	{
   	tp->catch_tell("You drive forward with your "+weapon+", severely hurting " + target->query_the_name(tp)+"'s " + hitloc + ".\n");
	target->catch_tell(tp->query_The_name(target)+" drives forward with " + Q_PS(tp) + " " + weapon + ", severely hurting your " + hitloc + ".\n");
	tp->tell_watcher(QCTNAME(tp) + " drives forward with " + Q_PS(tp) + " " + weapon + ", severely hurting " + QTNAME(target)+"'s " + hitloc + ".\n", target);
    	}
	else if ((phurt < 60) && (phurt > 29))
	{
	tp->catch_tell("You drive forward with your "+weapon+", hurting " + target->query_the_name(tp)+"'s " + hitloc + ".\n");
	target->catch_tell(tp->query_The_name(target)+" drives forward with " + Q_PS(tp) + " " + weapon + ", hurting your " + hitloc + ".\n");
	tp->tell_watcher(QCTNAME(tp) + " drives forward with " + Q_PS(tp) + " " + weapon + ", hurting " + QTNAME(target)+"'s " + hitloc + ".\n", target);
    	}			
	else if ((phurt > 0) && (phurt < 30))
	{
	tp->catch_tell("You drive forward with your "+weapon+", cutting " + target->query_the_name(tp)+"'s " + hitloc + ".\n");
	target->catch_tell(tp->query_The_name(target)+" drives forward with " + Q_PS(tp) + " " + weapon + ", cutting your " + hitloc + ".\n");
	tp->tell_watcher(QCTNAME(tp) + " drives forward with " + Q_PS(tp) + " " + weapon + ", cutting " + QTNAME(target)+"'s " + hitloc + ".\n", target);
    	}		
	else
    	{
	tp->catch_tell("You drive forward with your "+weapon+", just missing " + target->query_the_name(tp)+"'s " + hitloc + ".\n");
	target->catch_tell(tp->query_The_name(target)+" drives forward with " + Q_PS(tp) + " " + weapon + ", just missing your " + hitloc + ".\n");
	tp->tell_watcher_miss(QCTNAME(tp)+" drives forward with " + Q_PS(tp) + " " + weapon + ", just missing " + QTNAME(target)+"'s " + hitloc + ".\n", target);
    	}
}


/*
 * Function name: choose_slayattack_weapon
 * Description:   Choose weapon to be used with special attack
 * Arguments:     object tp	- attacker that used special attack
 * Returns:       Object pointer to weapon or 0;
 */
object
choose_slayattack_weapon(object tp)
{
    object *weapons = tp->query_weapon(-1);
    int dt = query_slayattack_damage_type();
...
private void
reset_slayattack_special(object tp)
{
    if(!objectp(tp))
	return;
    tp->remove_prop(SLAYATTACK_PROP);
    tp->catch_tell("You feel ready to slay someone again.\n");
    tp->special_command_enabled("slay");
}

private void
do_slayattack(object tp, object target, string attack)
{
    object combat_ob, weapon;
    int aid, hit, pen, dt, phurt, hitsuc, alarm;
    mixed why, slayattack_resistance, *hit_result;

    if(!objectp(tp))
	return;

    alarm = set_alarm(query_slayattack_relax_time(tp), 0.0,
			&reset_slayattack_special(tp));
    tp->add_prop(SLAYATTACK_PROP, alarm);

    if (!target)
	target = tp->query_attack();

    /* make sure the target is still in the same room as the attacker */
    if (!objectp(target) || (ENV(target) != ENV(tp)))
    {
    ...
    if (!(weapon = choose_slayattack_weapon(tp)))
    {
    ...
    aid = query_slayattack_weapon_aid(tp, weapon);
    hit = query_slayattack_weapon_hit(tp, weapon);
    ...
    {
	pen = query_slayattack_weapon_pen(tp, weapon);
	pen = F_PENMOD(pen, query_slayattack_skill(tp));
	if (intp(slayattack_resistance = target->query_prop("_live_m_no_"+attack)))
	{
	    pen = pen * (100 - slayattack_resistance) / 75; // 75 was 100
	}

	dt = query_slayattack_damage_type();
	hit_result = target->hit_me(pen, dt, tp, aid);

	DBGMSG("*** slayattack pen: "+pen+", result: "+hit_result[0]+","+
	    hit_result[1]+","+hit_result[2]+","+hit_result[3]+" *" + phurt + "**\n");
    }
    else
    {
	hit_result = target->hit_me(hitsuc, 0, tp, aid);
	DBGMSG("*** slayattack miss, result: "+hit_result[0]+","+
...
*/
    ...
    applyv(&slayattack_message(tp, target, weapon->short(),
...
slayattack(string str)
{
    ...
    alarm = TP->query_prop(SLAYATTACK_PROP);
    if(alarm)
    {
	...
	{
	    DBGMSG("*** WARNING!!!  slayattack_PROP set and alarm not found! ***\n");
    ...
    if (!choose_slayattack_weapon(TP))
    {
	NF("You are not wielding an appropriate weapon.\n");
	return 0;
    }

    write("You prepare to "+verb+".\n");
/*
...
*/
    ...
    alarm = set_alarm(query_slayattack_prepare_time(TP), 0.0,
			&do_slayattack(TP, ob, verb));
    TP->add_prop(SLAYATTACK_PROP, alarm);
