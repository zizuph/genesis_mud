/*
    ...
    Function stabattack(string str) should be executed when player issues command

    Following functions could be masked in division specific souls in order
    to customize behaviour of stabattack(). Default behaviour is that of blue
    army special slash. Only stabattack_message() needs to be masked always.
    For explanation see below.

int query_stabattack_damage_type()
float query_stabattack_prepare_time(object tp)
float query_stabattack_relax_time(object tp)
int query_stabattack_weapon_aid(object tp, object weapon)
int query_stabattack_weapon_hit(object tp, object weapon)
int query_stabattack_weapon_pen(object tp, object weapon)
int query_stabattack_skill(object tp)

void stabattack_do_die(object tp, object target)
void stabattack_message(object tp, object target,
		string weapon, int weapon_type,
		int phurt, string hitloc, int phit, int dam)

object choose_stabattack_weapon(object tp)
*/
#pragma strict_types
#pragma save_binary
#pragma no_clone
...
#define STABATTACK_PROP "_barbarian_stabattack_prop"
...
/*
 * Function name: query_stabattack_damage_type
 * Returns:       int - damage type of our special attack
 */
int
query_stabattack_damage_type()
{
    return W_IMPALE;
}

/*
 * Function name: query_stabattack_prepare_time
 * Arguments:     object tp	- who does special attack
 * Returns:       float - time between command and actual attack
 */
float
query_stabattack_prepare_time(object tp)
{
    return itof(3 + random(3));
}

/*
 * Function name: query_stabattack_relax_time
 * Arguments:     object tp	- who does special attack
 * Returns:       float - time to wait after attack before new can be started
 */
float
query_stabattack_relax_time(object tp)
{
    return itof(3 + random(3));
}

/*
 * Function name: query_stabattack_weapon_aid
 * Arguments:     object tp	- who does special attack
 ...
 */
int
query_stabattack_weapon_aid(object tp, object weapon)
{
...
/*
 * Function name: query_stabattack_weapon_hit
 * Arguments:     object tp	- who does special attack
 *		  object weapon - weapon used for special attack
 * Returns:       int - hit value special attack is performed with
 */
int
query_stabattack_weapon_hit(object tp, object weapon)
{
    return weapon->query_hit()*2;
}

/*
 * Function name: query_stabattack_weapon_pen
 * Arguments:     object tp	- who does special attack
 *		  object weapon - weapon used for special attack
 * Returns:       int - pen value special attack is performed with
 */
int
query_stabattack_weapon_pen(object tp, object weapon)
{
...
/*
 * Function name: query_stabattack_skill
 * Arguments:     object tp	- who does special attack
 * Returns:       int - how skilled is for special attack
 */
int
query_stabattack_skill(object tp)
{
    return tp->query_skill(SS_STABBING);
}

/*
 * Function name: stabattack_do_die
 ...
 */
void
stabattack_do_die(object tp, object target)
{
    target->do_die(tp);
}

/*
 * Function name: stabattack_message
 ...
 */
void
stabattack_message(object tp, object target,
		string weapon, int weapon_type,
		int phurt, string hitloc, int phit, int dam)

{
    	if(phurt > 74)
    	{
   	tp->catch_tell("You stab at " + target->query_the_name(tp) + "'s " 
   		+ hitloc + " with your " + weapon + ", severely piercing " 
   		+ Q_OB(target) + ".\n");
	target->catch_tell(tp->query_The_name(target)+" stabs at your " 
		+ hitloc + " with "+ Q_PS(tp) + " " + weapon + 
		", severely piercing you.\n");
	tp->tell_watcher(QCTNAME(tp) + " stabs at " + QTNAME(target) +
		 "'s " + hitloc + " with " + Q_PS(tp) + " " + weapon + 
		 ", severely piercing " + Q_OB(target) + ".\n", target);
    	}
	else if ((phurt < 75) && (phurt > 49))
	{
	tp->catch_tell("You stab at " + target->query_the_name(tp) + "'s " 
   		+ hitloc + " with your " + weapon + ", piercing " 
   		+ Q_OB(target) + ".\n");
	target->catch_tell(tp->query_The_name(target)+" stabs at your " 
		+ hitloc + " with "+ Q_PS(tp) + " " + weapon + 
		", piercing you.\n");
	tp->tell_watcher(QCTNAME(tp) + " stabs at " + QTNAME(target) +
		 "'s " + hitloc + " with " + Q_PS(tp) + " " + weapon + 
		 ", piercing " + Q_OB(target) + ".\n", target);
	}
	else if ((phurt < 50) && (phurt > 29))
	{
	tp->catch_tell("You stab at " + target->query_the_name(tp) + "'s " 
   		+ hitloc + " with your " + weapon + ", severely hurting " 
   		+ Q_OB(target) + ".\n");
	target->catch_tell(tp->query_The_name(target)+" stabs at your " 
		+ hitloc + " with "+ Q_PS(tp) + " " + weapon + 
		", severely hurting you.\n");
	tp->tell_watcher(QCTNAME(tp) + " stabs at " + QTNAME(target) +
		 "'s " + hitloc + " with " + Q_PS(tp) + " " + weapon + 
		 ", severely hurting " + Q_OB(target) + ".\n", target);
	}			
	else if ((phurt < 30) && (phurt > 19))
	{
	tp->catch_tell("You stab at " + target->query_the_name(tp) + "'s " 
   		+ hitloc + " with your " + weapon + ", hurting " 
   		+ Q_OB(target) + ".\n");
	target->catch_tell(tp->query_The_name(target)+" stabs at your " 
		+ hitloc + " with "+ Q_PS(tp) + " " + weapon + 
		", hurting you.\n");
	...
	else if ((phurt < 20) && (phurt > 9))
	{
	tp->catch_tell("You stab at " + target->query_the_name(tp) + "'s " 
   		+ hitloc + " with your " + weapon + ", gashing " 
   		+ Q_OB(target) + ".\n");
	target->catch_tell(tp->query_The_name(target)+" stabs at your " 
		+ hitloc + " with "+ Q_PS(tp) + " " + weapon + 
		", gashing you.\n");
	...
	else if ((phurt < 10) && (phurt > 4))
	{
	tp->catch_tell("You stab at " + target->query_the_name(tp) + "'s " 
   		+ hitloc + " with your " + weapon + ", cutting " 
   		+ Q_OB(target) + ".\n");
	target->catch_tell(tp->query_The_name(target)+" stabs at your " 
		+ hitloc + " with "+ Q_PS(tp) + " " + weapon + 
		", cutting you.\n");
	...
	else if ((phurt < 5) && (phurt > -1))
	{
	tp->catch_tell("You stab at " + target->query_the_name(tp) + "'s " 
   		+ hitloc + " with your " + weapon + ", scratching " 
   		+ Q_OB(target) + ".\n");
	target->catch_tell(tp->query_The_name(target)+" stabs at your " 
		+ hitloc + " with "+ Q_PS(tp) + " " + weapon + 
		", scratching you.\n");
	tp->tell_watcher(QCTNAME(tp) + " stabs at " + QTNAME(target) +
		 "'s " + hitloc + " with " + Q_PS(tp) + " " + weapon + 
		 ", scratching " + Q_OB(target) + ".\n", target);
	}		
	else
    	{
	tp->catch_tell("You stab at " + target->query_the_name(tp) + "'s " 
   		+ hitloc + " with your " + weapon + ", missing " 
   		+ Q_OB(target) + ".\n");
	target->catch_tell(tp->query_The_name(target)+" stabs at your " 
		+ hitloc + " with "+ Q_PS(tp) + " " + weapon + 
		", missing you.\n");
...
private void
reset_stabattack_special(object tp)
{
    if(!objectp(tp))
	return;
    tp->remove_prop(STABATTACK_PROP);
    tp->catch_tell("You feel ready to go on the offensive again.\n");
    tp->special_command_enabled("stabattack");
}

private void
do_stabattack(object tp, object target, string attack)
{
    object combat_ob, weapon;
    int aid, hit, pen, dt, phurt, hitsuc, alarm;
    mixed why, stabattack_resistance, *hit_result;

    if(!objectp(tp))
	return;

    alarm = set_alarm(query_stabattack_relax_time(tp), 0.0,
			&reset_stabattack_special(tp));
    tp->add_prop(STABATTACK_PROP, alarm);
    ...
    if (!(weapon = choose_stabattack_weapon(tp)))
    {
    ...
    aid = query_stabattack_weapon_aid(tp, weapon);
    hit = query_stabattack_weapon_hit(tp, weapon);
    ...
    {
	pen = query_stabattack_weapon_pen(tp, weapon);
	pen = F_PENMOD(pen, query_stabattack_skill(tp));
	if (intp(stabattack_resistance = target->query_prop("_live_m_no_"+attack)))
	{
	    pen = pen * (100 - stabattack_resistance) / 100;
	}

	dt = query_stabattack_damage_type();
	hit_result = target->hit_me(pen, dt, tp, aid);
	DBGMSG("*** stabattack pen: "+pen+", result: "+hit_result[0]+","+
	    hit_result[1]+","+hit_result[2]+","+hit_result[3]+" ***\n");
    }
    else
    {
	hit_result = target->hit_me(hitsuc, 0, tp, aid);
	DBGMSG("*** stabattack miss, result: "+hit_result[0]+","+
	    hit_result[1]+","+hit_result[2]+","+hit_result[3]+" ***\n");
    }
...
*/
    ...
    applyv(&stabattack_message(tp, target, weapon->short(),
...
stabattack(string str)
{
    ...
    alarm = TP->query_prop(STABATTACK_PROP);
    if(alarm)
    {
	...
	{
	    DBGMSG("*** WARNING!!!  stabattack_PROP set and alarm not found! ***\n");
    ...
    if (!choose_stabattack_weapon(TP))
    {
...
*/
    ...
    alarm = set_alarm(query_stabattack_prepare_time(TP), 0.0,
			&do_stabattack(TP, ob, verb));
    TP->add_prop(STABATTACK_PROP, alarm);
