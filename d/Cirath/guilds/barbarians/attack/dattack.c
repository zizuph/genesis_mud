/*
    ...
    Function sattack(string str) should be executed when player issues command

    Following functions could be masked in division specific souls in order
    to customize behaviour of sattack(). Default behaviour is that of blue
    army special slash. Only sattack_message() needs to be masked always.
    For explanation see below.

int query_sattack_damage_type()
float query_sattack_prepare_time(object tp)
float query_sattack_relax_time(object tp)
int query_sattack_weapon_aid(object tp, object weapon)
int query_sattack_weapon_hit(object tp, object weapon)
int query_sattack_weapon_pen(object tp, object weapon)
int query_sattack_skill(object tp)

void sattack_do_die(object tp, object target)
void sattack_message(object tp, object target,
		string weapon, int weapon_type,
		int phurt, string hitloc, int phit, int dam)

object choose_sattack_weapon(object tp)
*/
#pragma strict_types
#pragma save_binary
#pragma no_clone
...
#define SATTACK_PROP "_barbarian_sattack_prop"
...
/*
 * Function name: query_sattack_damage_type
 * Returns:       int - damage type of our special attack
 */
int
query_sattack_damage_type()
{
    return W_SLASH;
}

/*
 * Function name: query_sattack_prepare_time
 * Arguments:     object tp	- who does special attack
 * Returns:       float - time between command and actual attack
 */
float
query_sattack_prepare_time(object tp)
{
    return itof(3 + random(10));
}

/*
 * Function name: query_sattack_relax_time
 * Arguments:     object tp	- who does special attack
 * Returns:       float - time to wait after attack before new can be started
 */
float
query_sattack_relax_time(object tp)
{
    return itof(3 + random(10));
}

/*
 * Function name: query_sattack_weapon_aid
 * Arguments:     object tp	- who does special attack
 ...
 */
int
query_sattack_weapon_aid(object tp, object weapon)
{
...
/*
 * Function name: query_sattack_weapon_hit
 * Arguments:     object tp	- who does special attack
 *		  object weapon - weapon used for special attack
 * Returns:       int - hit value special attack is performed with
 */
int
query_sattack_weapon_hit(object tp, object weapon)
{
    return weapon->query_hit()*2;
}

/*
 * Function name: query_sattack_weapon_pen
 * Arguments:     object tp	- who does special attack
 *		  object weapon - weapon used for special attack
 * Returns:       int - pen value special attack is performed with
 */
int
query_sattack_weapon_pen(object tp, object weapon)
{
...
/*
 * Function name: query_sattack_skill
 * Arguments:     object tp	- who does special attack
 * Returns:       int - how skilled is for special attack
 */
int
query_sattack_skill(object tp)
{
    return tp->query_skill(SS_SLAYING);
}

/*
 * Function name: sattack_do_die
 ...
 */
void
sattack_do_die(object tp, object target)
{
    target->do_die(tp);
}

/*
 * Function name: sattack_message
 ...
 */
void
sattack_message(object tp, object target,
		string weapon, int weapon_type,
		int phurt, string hitloc, int phit, int dam)
...
private void
reset_sattack_special(object tp)
{
    if(!objectp(tp))
	return;
    tp->remove_prop(SATTACK_PROP);
    tp->catch_tell("You feel ready to go on the offensive again.\n");
    tp->special_command_enabled("sattack");
}

private void
do_sattack(object tp, object target, string attack)
{
    object combat_ob, weapon;
    int aid, hit, pen, dt, phurt, hitsuc, alarm;
    mixed why, sattack_resistance, *hit_result;

    if(!objectp(tp))
	return;

    alarm = set_alarm(query_sattack_relax_time(tp), 0.0,
			&reset_sattack_special(tp));
    tp->add_prop(SATTACK_PROP, alarm);
    ...
    if (!(weapon = choose_sattack_weapon(tp)))
    {
    ...
    aid = query_sattack_weapon_aid(tp, weapon);
    hit = query_sattack_weapon_hit(tp, weapon);
    ...
    {
	pen = query_sattack_weapon_pen(tp, weapon);
	pen = F_PENMOD(pen, query_sattack_skill(tp));
	if (intp(sattack_resistance = target->query_prop("_live_m_no_"+attack)))
	{
	    pen = pen * (100 - sattack_resistance) / 100;
	}

	dt = query_sattack_damage_type();
	hit_result = target->hit_me(pen, dt, tp, aid);
	DBGMSG("*** sattack pen: "+pen+", result: "+hit_result[0]+","+
	    hit_result[1]+","+hit_result[2]+","+hit_result[3]+" ***\n");
    }
    else
    {
	hit_result = target->hit_me(hitsuc, 0, tp, aid);
	DBGMSG("*** sattack miss, result: "+hit_result[0]+","+
	    hit_result[1]+","+hit_result[2]+","+hit_result[3]+" ***\n");
    }
...
*/
    ...
    applyv(&sattack_message(tp, target, weapon->short(),
...
sattack(string str)
{
    ...
    alarm = TP->query_prop(SATTACK_PROP);
    if(alarm)
    {
	...
	{
	    DBGMSG("*** WARNING!!!  sattack_PROP set and alarm not found! ***\n");
...
*/
    ...
    alarm = set_alarm(query_sattack_prepare_time(TP), 0.0,
			&do_sattack(TP, ob, verb));
    TP->add_prop(SATTACK_PROP, alarm);
