/*
    ...
    Function chopattack(string str) should be executed when player issues command

    Following functions could be masked in division specific souls in order
    to customize behaviour of chopattack(). Default behaviour is that of blue
    army special slash. Only chopattack_message() needs to be masked always.
    For explanation see below.

int query_chopattack_damage_type()
float query_chopattack_prepare_time(object tp)
float query_chopattack_relax_time(object tp)
int query_chopattack_weapon_aid(object tp, object weapon)
int query_chopattack_weapon_hit(object tp, object weapon)
int query_chopattack_weapon_pen(object tp, object weapon)
int query_chopattack_skill(object tp)

void chopattack_do_die(object tp, object target)
void chopattack_message(object tp, object target,
		string weapon, int weapon_type,
		int phurt, string hitloc, int phit, int dam)

object choose_chopattack_weapon(object tp)
*/
#pragma strict_types
#pragma save_binary
#pragma no_clone
...
#define CHOPATTACK_PROP "_barbarian_chopattack_prop"
...
/*
 * Function name: query_chopattack_damage_type
 * Returns:       int - damage type of our special attack
 */
int
query_chopattack_damage_type()
{
    return W_SLASH;
}

/*
 * Function name: query_chopattack_prepare_time
 * Arguments:     object tp	- who does special attack
 * Returns:       float - time between command and actual attack
 */
float
query_chopattack_prepare_time(object tp)
{
    return itof(3 + random(10));
}

/*
 * Function name: query_chopattack_relax_time
 * Arguments:     object tp	- who does special attack
 * Returns:       float - time to wait after attack before new can be started
 */
float
query_chopattack_relax_time(object tp)
{
    return itof(3 + random(10));
}

/*
 * Function name: query_chopattack_weapon_aid
 * Arguments:     object tp	- who does special attack
 ...
 */
int
query_chopattack_weapon_aid(object tp, object weapon)
{
...
/*
 * Function name: query_chopattack_weapon_hit
 * Arguments:     object tp	- who does special attack
 *		  object weapon - weapon used for special attack
 * Returns:       int - hit value special attack is performed with
 */
int
query_chopattack_weapon_hit(object tp, object weapon)
{
    return weapon->query_hit()*2;
}

/*
 * Function name: query_chopattack_weapon_pen
 * Arguments:     object tp	- who does special attack
 *		  object weapon - weapon used for special attack
 * Returns:       int - pen value special attack is performed with
 */
int
query_chopattack_weapon_pen(object tp, object weapon)
{
...
/*
 * Function name: query_chopattack_skill
 * Arguments:     object tp	- who does special attack
 * Returns:       int - how skilled is for special attack
 */
int
query_chopattack_skill(object tp)
{
    return tp->query_skill(SS_SLAYING);
}

/*
 * Function name: chopattack_do_die
 ...
 */
void
chopattack_do_die(object tp, object target)
{
    target->do_die(tp);
}

/*
 * Function name: chopattack_message
 ...
 */
void
chopattack_message(object tp, object target,
		string weapon, int weapon_type,
		int phurt, string hitloc, int phit, int dam)

{
	...
	target->catch_tell(tp->query_The_name(target)+" hits your "+hitloc+" with "+Q_PS(tp)+" "+weapon+".\n");
	tp->tell_watcher(QCTNAME(tp)+" hits "+QTNAME(target)+"'s "+hitloc+" with "+Q_PS(tp)+" "+weapon+".\n");
    }
    else
    {
...
private void
reset_chopattack_special(object tp)
{
    if(!objectp(tp))
	return;
    tp->remove_prop(CHOPATTACK_PROP);
    tp->catch_tell("You feel ready to go on the offensive again.\n");
    tp->special_command_enabled("chopattack");
}

private void
do_chopattack(object tp, object target, string attack)
{
    object combat_ob, weapon;
    int aid, hit, pen, dt, phurt, hitsuc, alarm;
    mixed why, chopattack_resistance, *hit_result;

    if(!objectp(tp))
	return;

    alarm = set_alarm(query_chopattack_relax_time(tp), 0.0,
			&reset_chopattack_special(tp));
    tp->add_prop(CHOPATTACK_PROP, alarm);
    ...
    if (!(weapon = choose_chopattack_weapon(tp)))
    {
    ...
    aid = query_chopattack_weapon_aid(tp, weapon);
    hit = query_chopattack_weapon_hit(tp, weapon);
    ...
    {
	pen = query_chopattack_weapon_pen(tp, weapon);
	pen = F_PENMOD(pen, query_chopattack_skill(tp));
	if (intp(chopattack_resistance = target->query_prop("_live_m_no_"+attack)))
	{
	    pen = pen * (100 - chopattack_resistance) / 100;
	}

	dt = query_chopattack_damage_type();
	hit_result = target->hit_me(pen, dt, tp, aid);
	DBGMSG("*** chopattack pen: "+pen+", result: "+hit_result[0]+","+
	    hit_result[1]+","+hit_result[2]+","+hit_result[3]+" ***\n");
    }
    else
    {
	hit_result = target->hit_me(hitsuc, 0, tp, aid);
	DBGMSG("*** chopattack miss, result: "+hit_result[0]+","+
	    hit_result[1]+","+hit_result[2]+","+hit_result[3]+" ***\n");
    }
...
*/
    ...
    applyv(&chopattack_message(tp, target, weapon->short(),
...
chopattack(string str)
{
    ...
    alarm = TP->query_prop(CHOPATTACK_PROP);
    if(alarm)
    {
	...
	{
	    DBGMSG("*** WARNING!!!  CHOPATTACK_PROP set and alarm not found! ***\n");
    ...
    if (!choose_chopattack_weapon(TP))
    {
...
*/
    ...
    alarm = set_alarm(query_chopattack_prepare_time(TP), 0.0,
			&do_chopattack(TP, ob, verb));
    TP->add_prop(CHOPATTACK_PROP, alarm);
