...
sfish()
{
    ...
    {
        notify_fail("You don't have a fish to sacrify!\n");
        return 0;
    }

    fish->remove_object();

    write("Suddenly the statues comes to life " +
        "and bends down towards you. With one voice, they whisper: " +
        "You shall be rewarded for your kindness! We have a mission " +
        ...
        "seek out and find it so it can be returned to the wizard" +
        "-school of Roke to serve as guidance for the future mages." + //FIXA
    ...
    say("The statues whisper something to " + QCTNAME(TP) + ".\n");
...
{
    TP->catch_msg(break_string("The plaque has the inscription: "+
        "Raised in honour of the Great Wizards Lord Ged, Sir "+
        "Glindor and It Gresolle who built Gont over a night.\n",70));
     return 1;
}
*/
...
plaque()
{
    TP->catch_msg("The inscription on the "+
...
trans()
{
    if (!QDONE(ROKEGROUP, STATUEQUEST)) return 0;
    ...
    set_dircmd("me");
    TP->move_living("M", NYREESEDIR + "cas_tmp_room");
...
tell_him(object who)
{
    ...
    who->catch_msg("...you find yourself in a damp room with stone" +
                   " walls.\n");
    who->command("puke");
}
