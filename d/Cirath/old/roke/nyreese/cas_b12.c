...
create_room()
{
    ...
    set_long("You are standing in one of the cornerrooms of the second floor.\n" +
             "This room are dimly lighted by one small " +
             "blue-shimmering globe at the wall. " +
             "The exit to the southeast seems to end up in one of the towers.\n");
    ...
    add_exit("cas_t24", "southeast");
    add_exit("cas_b11", "west");
    add_exit("cas_b8", "north");
    INSIDE;
}
...
polish(string s)
{
    ...
    */
    ...
    if (!tissue->query_prop("_roke_i_polished_globe_4"))
        TP->catch_msg("The tissue glows for a short moment.\n");

    tissue->add_prop("_roke_i_polished_globe_4", 1);
    return 1;
}
