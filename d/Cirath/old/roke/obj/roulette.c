...
help(string str)
{
    ...
    write("To bet on the roulette type : bet <amount> [marker type] on <spot> \n"+
          ...
          "       dus1, dus2, dus3                 ; 1-12, 13-24, 25-36\n"+
          "       row1, row2, ... , row12          ;\n"+
...
init_table()
{
    ...
    betstr = ({ "red", "black", "odd", "even", "low", "high", "dus1", "dus2",
                "dus3", "col1", "col2", "col3" ,"012", "023", "0+1", "0+2",
                "0+3", "0:4" });
    for (t = 1; t <= 12; t++)
        betstr += ({ "row" + t });
    for (t = 1; t <= 11; t++)
...
calcodds(string bet)
{
  ...
  if (member_array(bet, ({"red", "black", "odd", "even", "low", "high" }))
      != -1 )
      return 2;
  if (member_array(bet, ({"dus1", "dus2", "dus3", "col1", "col2", "col3"}))
      != -1)
      return 3;
  if (bet == "012" || bet == "023")
      return 11;
...
do_bet(string str)
{
    int amount, value, prevb, payi, t, i;
    ...
    {
        if (bets[where][name])
            bets[where][name] = betcoins;
        else
            bets[where] = bets[where] + ([name: betcoins]);
    }
    else
        bets = bets + ([ where : ([name: betcoins])]);
 ...
 */
    ...
    write("You buy " + amount + " " + type + " marker" +
        (amount == 1 ? "" : "s") + ".\n");
    write("You pay " + text(coinarr[0 .. 3]));
    if (stringp(change_text))
        write(", and get " + change_text + " back.\n");
    else
        write(".\n");
