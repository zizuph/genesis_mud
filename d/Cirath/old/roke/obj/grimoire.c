...
enter_env(object dest, object old)
{
    ...
    if (interactive(dest))
    {
        ...
        dest->add_prop(OBJ_I_NO_TELEPORT, VBFC_ME("snor"));
    }
    ork = dest;
}
...
explodeall()
{
    ...
    room = TO;
    while (!(room = E(room))->query_prop(ROOM_I_IS)) ;
    ...
    // FILTER? FIXA
    objs = filter(objs, "check_living", TO);
