...
create_room()
{
   ...
   add_exit(AK_WELA_DIR + "szun_moon7", "north", 0);
   add_exit(AK_WELA_DIR + "qua_phing3", "east", 0);
...
{
   seteuid(geteuid(to));
   if (this_player()->query_wiz_rank() >= WIZ_APPRENTICE)
      return 0;
...
do_buy(str)
{
   ...
   if(!str || str =="")
   {
      tell_room(to, "The owner says: Tough to make up your mind eh, " +
   ...
   if (pay_type != "")
   {
      ...
      {
         tell_room(to, "The owner says: I don't accept those " + pay_type +
   ...
   if (get_type != "")
   {
      ...
      {
         tell_room(to, "The owner says: I don't have any " + get_type +
                   " coins, " + tp->query_nonmet_name() + ".\n");
         return 1;
      }
   }
   ...
   if (sscanf(item_name, "%d", int_str))
   {
      ...
      if (!sizeof(item_arr))
      {
         tell_room(to, "The owner says: I am afraid that is all sold out, " +
                   tp->query_nonmet_name() + ".\n");
         return 1;
      }
   ...
   else  /* The player gave an item-string, try to locate it. */
   {
      ...
      {
         tell_room(to, "The owner says: I don't have such " + item_name +
   ...
   if (sizeof(money_arr = pay(value, tp, pay_type, 0, 0, get_type)) == 1)
   {
      if (money_arr[0] == 1)
      {
         tell_room(to, "The owner says: I think that " + item->short() +
                   " is a bit more expensive, "+tp->query_nonmet_name()+".\n");
         return 1;
      }
      if (money_arr[0] == 2)
      {
         tell_room(to, "The owner says: You do not have any " + pay_type +
                   " coins, " + tp->query_nonmet_name() + ".\n");
         return 1;
      }
      else
   ...
   if(item->move(tp))
   {
      tell_room(to, "The owner says: You cannot carry that " + item->short() +
                ", " + tp->query_nonmet_name() + ".\n");
      tell_room(to, "The owner says: I will put it on the ground for you.\n" +
