...
* Date: 
*
* File: /d/Cirath/chukel/ciubori/arm/mring.c
...
create_object()
{
 	set_short("moonstone nose-ring");
  	set_pshort("moonstone nose-rings");
  	set_name("ring");
  	set_pname("rings");
...
{
  	if ((str == "nosering") && (TP->query_wiz_level())) /* only wizards will be able to connect/disconnect the ring */
    	{
      		connect == 0;
