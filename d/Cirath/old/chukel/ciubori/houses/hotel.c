...
* Date: 
*
* File: /d/Cirath/chukel/ciubori/houses/hotel.c
...
reset_room()
{ 	
    object npc = present ("joshua", TO);
    if(!objectp(npc))
	{
    		bring_room_mob("joshua", CIU_NPC + "joshua", 1, 1);
	}
}
...
create_room()
{
    	...
    	set_long("You are standing inside 'The Ciubori City Hotel'. The hotel "+
        	...
        	"well as a ticket for staying here overnight.");
	...
	INSIDE;
    	...
    	add_exit(CIU_STR + "cstreet3", "east", 0, 1); 
    	add_exit(CIU_HOU + "hotel2", "up", "@@block", 1);
    	reset_room();
}
...
read(string str)
{
    write("" +
        ...
        "pay with. You can also do  'buy 10 beer' would get you\n" +
...
order(string str)
{
	object guard = present ("joshua", TO);
