inherit "/d/Cirath/std/monster.c";
inherit "/lib/trade";

#include <money.h>
#include "defs.h"
#include "/sys/filter_funs.h"

#define NUM     sizeof(MONEY_TYPES)

object ship;
int has_introduced=0;

string *killers;
int *killingtimes;

void
set_ship(object s)
{
  ship=s;
}

void
create_monster()
{
  set_name("krull");
  add_name("captain");
  set_living_name("krull");
  set_race_name("human");
  add_adj("greedy");

  add_subloc("snok",this_object());

  default_config_npc(60);
  set_skill(SS_SWIM,100);
  set_skill(SS_UNARM_COMBAT,45);
  set_skill(SS_DEFENCE,34);

  heal_hp(10000);
  set_hp(50000);
    
  set_alignment(200);

  config_default_trade();

  seteuid(getuid(TO));

  trig_new("%w 'introduces' %s", "react_introduce");

  has_introduced = 0;
    
  set_mm_in("falls out of the sky.");
  set_mm_out("stumbles out of the room.");
  set_m_in("stumbles into the room.");

  set_title("Riverboat Captain");
}

public string 
show_subloc(string subloc, object on, object for_obj)
{
  return "He has a huge money bag in his belt.\n";
}

void
init_living()
{
  add_action("buy", "buy");
}

int
buy(string str)
{
  object buy_ob;
  string what, for_c, get_c, change;
  int    snok,*result, m_res, price;

  price = 24;

  if (!str)
  {
    notify_fail("Buy what?\n");
    return 0;
  }

  if(TP->query_average_stat()<20 && !present("DurniRiver",TP))
  {
    TP->catch_msg("You're so small and tiny that I'll let you ride for free.\n");
    clone_object(BOAT+"ticket")->move(TP); 
    return 1;
  }

  if (sscanf(str, "%s with %s and get %s", what, for_c, get_c) != 3)

  if (sscanf(str, "%s with %s", what, for_c) != 2)
    what = str;

  if (what != "ticket")
  {
    notify_fail("I have no such thing for sale.\n");
    return 0;
  }

  buy_ob=clone_object(BOAT+"ticket");
  if (!buy_ob)
  {
    notify_fail("I seem to be out of tickets.\n");
    return 0;
  }

  price=buy_ob->query_prop(OBJ_I_VALUE);

  result=pay(price,this_player(),for_c,0,0,get_c);
  if (sizeof(result)==1)
  {
    buy_ob->remove_object();
    return 0;
  }
  if (buy_ob->move(this_player()))
  {
    write("You drop the " + buy_ob->short() + " on the ground.\n");
    say(QCTNAME(this_player()) + " drops one " + buy_ob->short() +
      " on the ground.\n");
    buy_ob->move(environment(this_player()));
  }
  else
  {
    write("You get your " + buy_ob->short() + ".\n");
    say(QCTNAME(this_player()) + " buys a " + buy_ob->short() + ".\n");
  }
  write("You pay " + text(result[0 .. NUM - 1]) + ".\n");
  if (change)
    write("You get " + change + " as change.\n");

  return 1;
}

void
react_introduce(string person,string who)
{
  if (!has_introduced)
  {
    call_out("introduce_me", 4);
    has_introduced=1;
    call_out("remove_int",15);
  }
}

void
introduce_me()
{
  command("introduce myself");
}

void
remove_int()
{
  has_introduced=0;
}
