 ...
 * /Shiva--Sept 6, 1995: added lock/unlock commands. set_extra_short.
 */
 ...
 * set_num_notes_static(n) 0 = Increase the max number of notes when
 *                             a note is locked.
 *                         1 = Do not increase the max number of
 *                             notes. (default 0)
 ...
 * set_lock_lvl(n)         Minimum level required to lock or unlock a note
 *                         (default arches++)
 * set_extra_short(str)    String to be appended to a writer's short
 *                         description via the property LIVE_S_EXTRA_SHORT
 * set_num_locked_notes(n) Set the max number of notes that can be
                           locked at once. (default 10)
 ...
 *                         (default 0)
 *
 * There are five functions you can use to restrict usage of the board.
 * These functions are:
 *
 * varargs int check_reader()
 * varargs int check_writer()
 * varargs int check_remove()
 * varargs int check_lock()
 * varargs int check_unlock()
 *
 * These functions respectively restrict reading, writing, deleting,
 * locking, and unlocking notes (including your own). They must
 * return 1 to restrict and return 0 if manipulating the board. If
 * you print a message to the player if manipulating the board is
 * not possible, please check whether the player is present in the
 * room or else tools may give very ugly lists. The functions operate
 * on this_player(). There are set_ functions for all five access
 * levels that allow you to set VBFC to a function to check for access.
 *
 ...
 * lock    ( 2) 70..71 (spaces or symbol designating a locked note)
 */
...
#define LOCKED_FILE_SUFFIX    "~"
#define LOCKED_HEADER_SUFFIX  " *"
...
static int     fuse = 0;
static int     loaded = 0;
static int     max_locked_notes = 10;
static int     max_notes_static = 0;
static int     locked_notes;
static mapping writing = ([ ]);
static mixed   read_access = 0;
static mixed   write_access = 0;
static mixed   remove_access = 0;
static mixed   lock_access = 0;
static mixed   unlock_access = 0;

public int query_locked(string note);
...
/* Function name: set_lock_lvl
 * Description  : Set the minimum level people must have to
 *                be able to lock or unlock notes
...
/*
 * Function name: set_extra_short
 * Description  : Set the string to be added to the player's
 *                short (via the prop LIVE_S_EXTRA_SHORT) when
 *                he/she begins writing a note.
 * Arguments    : string str - the string to be added (prepended
 *                by a space)
 */
public void
...
/*
 * Function name: set_num_locked_notes
 * Description  : Set the maximum number of locked notes
 * Arguments    : n - the number of locked notes
 */
public void
set_num_locked_notes(int n)
{
    if (!fuse)
    {
        max_locked_notes = n;
...
/*
 * Function name: set_num_notes_static
 * Description  : Do/Don't increase the maximum number of notes when
 *                a note is locked.
 * Arguments      n -- 1 : Do not change max number of notes
 *                     0 : Increase max number of notes as notes are
 *                         locked.
 */
public void
set_num_notes_static(int n)
{
    if (!fuse)
    {
        max_notes_static = n;
...
/*
 * Function name: set_lock_access
 * Description  : Use this function to set a VBFC string to the function
 *                that tests whether this_player() is allowed to lock notes
 *                onto the board. Note that the VBFC should return true if
 *                the player is NOT allowed to lock notes.
 * Arguments    : string str - the intended VBFC.
 */
public void
set_lock_access(mixed l)
{
    if (!fuse)
    {
        lock_access = l;
 ...
 * Description  : This function checks whether this_player() is allowed
 *                to lock notes onto this board. If you print an error
 *                message on failure, please check whether this_player()
 *                is in the environment of the board.
 * Returns      : int - true if the player is NOT allowed to lock.
 */
varargs public int
check_lock()
{
...
/*
 * Function name: set_unlock_access
 * Description  : Use this function to set a VBFC string to the function
 *                that tests whether this_player() is allowed to unlock notes
 *                on the board. Note that the VBFC should return true if
 *                the player is NOT allowed to unlock notes.
 * Arguments    : string str - the intended VBFC.
 */
public void
set_unlock_access(mixed l)
{
    if (!fuse)
    {
        unlock_access = l;
...
/*
 * Function name: check_unlock
 * Description  : This function checks whether this_player() is allowed
 *                to unlock notes on this board. If you print an error
 *                message on failure, please check whether this_player()
 *                is in the environment of the board.
 * Returns      : int - true if the player is NOT allowed to unlock.
 */
varargs public int
check_unlock()
{
 ...
 * Description  : Determines whether a note is locked or not
 * Arguments    : The filename of the note
 * Returns      : 1 - note is locked; 0 - note is not locked
 */
public int
query_locked(string note)
{
    return (extract(note,
        -(strlen(LOCKED_FILE_SUFFIX))) == LOCKED_FILE_SUFFIX);
}
...
load_headers()
{
    ...
    for (i = 0, locked_notes = 0; i < msg_num; i++)
    {
        if (query_locked(headers[i][1]))
        {
            headers[i][0] += LOCKED_HEADER_SUFFIX;
            locked_notes++;
        }
    }
...
int lock_msg(string str);
int unlock_msg(string str);
...
init()
{
    ...
    add_action(lock_msg,   "lock");
    add_action(unlock_msg, "unlock");
}
...
done_editing(string message)
{
    ...
    total_notes = (max_notes_static ? notes + locked_notes : notes);
    i = -1;
    while (msg_num >= total_notes)
    {
        while (++i < msg_num && query_locked(headers[i][1]));

        /* All notes are locked, and no more may be added.
         */
        if (i >= msg_num)
        {
            break;
...
remove_msg(string what_msg)
{
    ...
    if (query_locked(headers[note][1]))
    {
      locked_notes--;
    }
...
/* Function name: lock_msg
 * Description  : Lock a note on the board.  Locked messages will
 *                not scroll off of the board.
 * Arguments    : string str - which note to lock
 * Returns      : int 1/0 - success/failure
 */
public int
lock_msg(string str)
{
    ...
    if (this_object()->check_lock() ||
        (no_special_fellow() && level < lock_lvl))
    {
        return 0;
    }

    notify_fail("Lock which note?\n");
    if (!sscanf(str, "%d", note_num) && !sscanf(str, "note %d",
        note_num))
    {
        return 0;
    }

    notify_fail("Not that many notes.\n");
    if (note_num < 1 || note_num > msg_num)
    {
        return 0;
    }
    ...
    notify_fail("That note has been locked already.\n");
    if (query_locked(headers[note_num][1]))
    {
        return 0;
    }

    notify_fail("Too many locked notes.\n");
    if (locked_notes >= max_locked_notes)
    {
        return 0;
    }
    ...
    rename(board_name + "/" + headers[note_num][1],
           board_name + "/" + (headers[note_num][1] +=
           LOCKED_FILE_SUFFIX));

    headers[note_num][0] += LOCKED_HEADER_SUFFIX;

    write("Note " + (note_num + 1) + " is locked on the board.\n");

    return 1;
}

/* Function name: unlock_msg
 * Description  : Unlocks a message that has been previously
 *                locked onto the board
 * Arguments    : string str - the note to unlock
 * Returns      : int 1/0 - success/failure
 */
public int
unlock_msg(string str)
{
    int note_num, level;
    string new_name, new_header;
    ...
    notify_fail("That note is not locked.\n");
    if (!sscanf(headers[note_num][1], "%s" + LOCKED_FILE_SUFFIX,
        new_name))
    {
        return 0;
    }
    ...
    rename(board_name + "/" + headers[note_num][1],
           board_name + "/" + (headers[note_num][1] = new_name));

    if (sscanf(headers[note_num][0], "%s" + LOCKED_HEADER_SUFFIX,
        new_header))
    {
        headers[note_num][0] = new_header;
    }

    write("Note " + (note_num + 1) + " is no longer locked on " +
        "the board.\n");
