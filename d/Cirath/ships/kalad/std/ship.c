...
start_ship(int where)
{
   ...
   captain->move_living("boarding the wagon", query_deck()[0]);
...
sail_away_out_on_the_open_sea()
{
	...
	if (places_to_go[location]->query_prop(ROOM_I_TYPE) != ROOM_IN_WATER)
	{
		...
		captain->command("say All aboard!  The " + short() +
		    ...
		    " seconds!");
		if (query_ticket_id())
              captain->command("say Buy your tickets now or stay here!");
		call_out("give_leave_warning", time - 20);
	    }
	    set_board_enabled(1);
	}
...
{
    if (query_captain())
   query_captain()->command("say All board the wagon, we are leaving in 20 seconds!");
}
...
board(mixed what)
{
	...
	{
     write("A guard prevents you from boarding the "+
		  short() + ".\n");
