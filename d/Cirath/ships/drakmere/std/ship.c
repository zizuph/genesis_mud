...
{
    set_name("wagon");
    set_long("An elegant passenger wagon.\n");
}
...
create_object()
{
    callout_fq = 3;
    event_enable = 0;
    add_prop(OBJ_M_NO_GET, "Would you like to try to pick up the "+
	     "mekillot and the wagon, or just the wagon, O mighty "+ 
             "one.\n");
    create_ship();
}
...
start_ship(int where)
{
	...
	if (captain)
   	{
	    captain->move_living("the wagon", environment());
	...
	captain->move_living("entering the wagon", query_deck()[0]);
	captain->command("say And we're off!");
	tell_room(pier, "The wagon leaves at once.\n");
    }
...
sail_away_out_on_the_open_sea()
{
	...
	if (captain = query_captain())
	    ...
	    {
		captain->move_living("entering the wagon", 
                query_deck()[0]);
		tell_room(pier,
		capitalize(LANG_ADDART(short())) + " starts on its"+
                " journey.\n");
	    }
	...
	if (places_to_go[location]->query_prop(ROOM_I_TYPE) != ROOM_IN_WATER)
	{
		...
		capitalize(LANG_ADDART(short())) + " just arrived.\n");
	    if (captain)
	    {
		captain->move_living("the wagon", places_to_go[location]);
		captain->command("say All aboard!  The " + short() +
		    " leaves in " + (time = query_callout_fq() * call_no) +
		    " seconds!");
		if (query_ticket_id())
		    captain->command("say Buy your tickets now for our"+ 
                    " next trip!");
		call_out("give_leave_warning", time - 20);
	    }
	    set_board_enabled(1);
...
{
    if (query_captain())
	query_captain()->command("say All board who are travelling on"+
        " this trip, we are leaving in 20 seconds!");
}
...
board(mixed what)
{
	...
	{
	    write("A menacing guard prevents you from boarding the "+
		  short() + ".\n");
    ...
    if (!query_ticket_id())
    {
	tell_room(environment(), QCTNAME(this_player()) + " enters the "+
		  short() + ".\n", this_player());
	...
	if (query_captain())
	{
	    tell_room(environment(), QCNAME(this_player()) + " gives "+
		      ...
		      QTNAME(query_captain()) + " and enters the " +
                      short()+
		      ".\n", this_player());
    ...
    this_player()->move_living("entering the " + short(),
    query_deck()[0]);
    return 1;
}
