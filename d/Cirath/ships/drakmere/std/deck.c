...
create_deck()
{
    set_short("An elegant wagon");
    set_long("You are inside an elegant wagon.\n");
}
...
create_room()
{
    ...
    set_deck_sound("Silk curtains sway gently in the windows as"+
       " the wagon rolls along a desolate road.\n");
    set_bump_sound("A liveried servant quietly announces: "+
...
disembark(mixed what)
{
    ...
    if (!exit_enabled) {
	notify_fail("A servant ushers you away from the door,"+ 
          " saying: It is too dangerous to leave right now. "+
          "Please wait until we arrive at our destination.\n");
	return 0;
    }
    write("You duck your head and step through the wagon door.\n");
