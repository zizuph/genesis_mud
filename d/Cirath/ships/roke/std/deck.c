...
surroundings()
{
    ...
    return "It seems like you are flying around in a void!\n";
...
ship_arrived(mixed where)
{
	...
	if (ship->query_captain())
      ...
      " says: You can open your eyes now, we're on the ground. "+
      "Disembark!\n");
...
disembark(mixed what)
{
    ...
    if (!exit_enabled) {
    notify_fail("You unstrap yourself from your belt, preparing to "+
     "disembark. Then you realize that you have no wings! "+
     "Hopes dashed, you return to your seat and strap back in.\n");
	return 0;
    }
    write("You clamber down from the back of the roc.\n");
    this_player()->move_living("the roc",environment(ship));
    return 1;
}
