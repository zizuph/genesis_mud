...
create_monster()
{
    ...
    add_act("say This is no place for ruffians.");
    add_act("say If you make trouble I will call the Templars.");
    add_act("say Feel free to sample the cuisine, if you can afford it.");
    add_cact("say Perhaps its better this way.");
    add_cact("shout GUARDS! GUARDS! COME TO THE HALL!");
    add_cact("say You are making some very wealthy enemies.");
    add_cact("say Perhaps you would look better as a newt?");
...
arm_me()
{
    ...
    clone_object(ATH_ARMOUR+"legs_lth.c")->move(TO);
    command("wield all");
    command("wear all");
}
