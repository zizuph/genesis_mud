...
void set_up_skills()
{
    ...
    sk_add_train(SS_AWARENESS, "notice things", "awareness", 0, 40);
...
arm_me()
{
    clone_object(ATH_WEAPON+"swrd_nrw.c")->move(TO);
    clone_object(ATH_ARMOUR+"shld_chn.c")->move(TO);
    clone_object(ATH_ARMOUR+"body_std.c")->move(TO);
    clone_object(ATH_ARMOUR+"head_lth.c")->move(TO);
    clone_object(ATH_ARMOUR+"legs_lth.c")->move(TO);
    clone_object(ATH_ARMOUR+"arms_lth.c")->move(TO);
    command("wield all");
    command("wear all");
}
