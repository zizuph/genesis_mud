...
{
    if(bigglad) return;
    bigglad=clone_object(TYR_NPCS+"lyden.c");
    bigglad->arm_me();
...
void create_room()
{
  ...
  set_long("Harsh sunlight reflects off the azure walls of the second "+
     ...
     "unpredictable baazrags working the pulleys above.\n"+
  ...
  add_item(({"baazrag","baazrags","huge baazrag","huge baazrags"}),
     ...
     "These creatues are huge, standing more than eight feet tall, "+
     "and are covered by bony shells. A series of sharp spikes, or "+
     "horns, runs down the center of their backs. The four legged "+
     "mammoths also have mouths full of sharp teeth, and very bad "+
     "tempers.\n");
  ...
  add_exit(TZ_ZIG+"out_ter1.c","down","@@godown",3);
