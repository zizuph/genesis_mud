...
#define IS_SPECIAL      ({ "tintin", "plugh", "quis", "tricky", "glindor", "mylos", "nick", "mecien" })
...
<= WIZLEV_APPRENTICE) && ("/secure/master"->query_wiz_dom(ob->query_real_name())=="Roke")) {
   ...
   if (who && !used) {
      if (is_founder(who->query_real_name())
         ...
         if (is_a_glad(environment()) && !no_noise)
            who->catch_msg("Hail, oh Gladiator!\n");
         else if (!no_noise)
