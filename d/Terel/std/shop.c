/*
 *  The standard shop of the domain Terel
 *
 *  Adapted from /d/Gondor/common/lib/shop.c
 *
...
create_shop()
{
    set_long("The standard shop of the domain Terel.\n");
    set_short("A shop");
    set_store_room("/d/Terel/std/store");
    set_shop_name("Terel's Standard Shop");
    set_keeper_file("/d/Terel/common/town/smitt");
    set_no_sell_prop("_Terel_no_sell");
}
...
create_room()
{
    ...
    add_item(({"item","items"}),
      ...
      + "should take a look at the pricelist, to get an idea of what "
    ...
    add_item("sign", exa_poster());
    add_item("poster", exa_poster());
    add_item("list", exa_poster());
    add_item("prices", exa_poster());
...
exa_poster()
{
    return "\n"
      ...
      + "    correct syntax is: 'list swords' also works.\n"
      + "\n";
}
...
check_keeper()
{
    ...
    if (keeper && present(keeper, TO)) {
        if(enemy = keeper->query_attack()) {
            if(enemy == TP)
                NF("The shopkeeper " +
		   " is trying to kill you, so you cannot do that!\n");
            else
...
/*    if (weapon_filter(x) || armour_filter(x))
        return "The shopkeeper says: I do not want your " +
	    x->short() + "!\n" +
