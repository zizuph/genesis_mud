...
{
    ::create_creature();
    set_short("giant white mastiff");
    add_name("dog");
    add_name("bitch");
    set_pshort("giant white mastiffs");
    set_long("The giant white mastiff stands upon its haunches " +
       "idly licking its massive paw.\n");
...
special_attack(object enemy)
{
    ...
    if(random(10) < 3) {
        ...
        tell_watcher(QCTNAME(me) + " tears at " + QTNAME(enemy) + 
           " with its massive jaws.\n", enemy); 
        enemy->catch_msg(QCTNAME(me) + " tears at your flesh with " +
           "its massive jaws!\n");
...
benice()
{
    ...
    if(TP->query_average_stat() <= 90)     /* Too small, throw em back!  */
    { 
       return 0;
    }
  return 1; 
