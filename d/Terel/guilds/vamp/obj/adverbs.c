...
#define V_ADVERB_SAVE_FILE         VAMP_DIR+"VAMP_ADVERBS"
#define V_ADVERB_REPL_SAVE_FILE  VAMP_DIR+"VAMP_ADVERB_REPLACEMENTS"
...
create()
{
    ...
    if (file_size(V_ADVERB_SAVE_FILE) > 0)
    {
	adverbs = sort_array(explode(read_file(V_ADVERB_SAVE_FILE), "\n"));
    ...
    if (file_size(V_ADVERB_REPL_SAVE_FILE) > 0)
    {
        lines = explode(read_file(V_ADVERB_REPL_SAVE_FILE), "\n");
        size = sizeof(lines);
        while(++index < size)
        {
...
member_adverb(string pattern)
{
     ...
     * into a configuration file just like the V_ADVERB_REPLACEMENTS. Mercade
     */
    if (pattern == "sad")
    {
