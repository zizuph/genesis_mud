...
create_room()
{
    ...
    add_prop(ROOM_I_SUNLIGHT,"@@check_sunlight@@");
...
public int valid_location(mixed destination,object player,string filename)
{
    if (!same_domain(destination[4],filename))
    {
        return 0;
    }
    if (member_array(destination[4],
        player->query_known_flight_locations())==-1)
    {
        return 0;
    }
    return 1;
...
{
    object start=ENV(player);
    
    string filename=MASTER_OB(start);
    
    Destinations=filter(TRAVEL_LOCATIONS,
        &valid_location(,player,filename));
...
public int check_sunlight()
{
    ...
    return Closest_destination[4]->query_prop(ROOM_I_SUNLIGHT);
}
...
public int descend(string str)
{
    ...
    TP->move_living("M",Current_destination[4],1,0);
    object * watchers=WATCHERS(TP);
    watchers->catch_msg(QCTNAME(TP) + " descends from above, like a "+
...
public void do_fly()
{
    ...
    {
        tell_room(TO,"Looking down, you see that you are approaching "+
            best[3]+".\n");
    ...
    else
    {
        ...
        {
            case 0:
            tell_room(TO,"The wind blows across your skin as you "+
                "travel.\n\n");
            break;
            case 1:
            tell_room(TO,"You see wisps of cloud pass by below "+
                "you.\n\n");
            break;
            case 2:
            tell_room(TO,"Various landmarks pass by far beneath you.\n\n");
            break;
            case 3:
            tell_room(TO,"You travel quickly across great distance.\n\n");
            break;
            case 4:
            tell_room(TO,"The ground moves below at great speed.\n\n");
            break;
            case 5:
            tell_room(TO,"Life below appears like ants from such "+
            "heights.\n\n");
            break;
        }
    }
    ...
    {
        tell_room(TO,"You arrive at your destination.\n");
        Closest_destination=Current_destination;
        setuid();
        seteuid(getuid());
