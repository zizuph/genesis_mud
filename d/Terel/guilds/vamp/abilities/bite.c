 ...
 *   -- added hook_targets_gone function.
 */
...
hook_targets_gone(object * targets)
{
    ...
    if (!sizeof(alive_targets))
    {
        if (sizeof(targets) > 1)
        {
            write("You cannot savage your prey as they have all been "
                + "slain!\n");
        }
        else
        {
            write("You cannot savage your prey it has already been slain!\n");
        }
    }
    else
    {
        write("You cannot savage " + COMPOSITE_LIVE(alive_targets)
