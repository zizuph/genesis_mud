...
move_living(string how, mixed to_dest, int dont_follow, int no_glance)
{
    ...
    for (i = 0, who_move = ({ }); i < sizeof(followers); i++)
    {
        ...
        {
            remove_cling_follower(followers[i]);
            continue;
        }
     ...
     * ENV by a regular exit: followers should not cling
...
init_living()
{
    ...
    if (cling_map[TP])
    {
        if (TP->query_prop(VAMP_O_CLING) != SW)
        {
            remove_cling_follower(TP);
            return;
        }
        ...
        if (sight_test(TP, SW))
        {
             ...
             * him from moving now.
             */
            if (alarm = cling_map[TP][ALARM_ID])
            {
                remove_alarm(cling_map[TP][ALARM_ID]);  
            }
      
            cling_map[TP][ALARM_ID] = 0;
        }
