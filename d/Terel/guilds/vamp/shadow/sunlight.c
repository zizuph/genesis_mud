...
query_sunlight_level(object room)
{
    setuid();
    seteuid(getuid());    
    int sunlight = GENESIS_TIMEKEEPER->query_sunlight_for_room(room);
    
    // Before querying sunlight, we make a call to update_light in the
    // room. This is necessary because there are items out there that
    // don't properly call update_light, leading to extremely wrong
    // values for the room.
    room->update_light(1);
...
check_sunlight()
{
    ...
    {
        // Players who are linkdead do not heal from the burn, but
        // they should not continue to burn.
        return;
    }
