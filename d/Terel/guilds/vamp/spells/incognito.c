 ...
 *  Incognito
 *
 *  For spell effect description, see doc/incognito.txt
 *
...
config_spell()
{
    ...
    set_spell_desc("Focus your mind to mask your description");
    
    set_spell_object(SPELLOBJ_DIR+"incognito_obj");
}
...
static int check_valid_action(object actor,mixed * targets,string arg,int 
                                execute = 0)
{
    if (actor->query_incognito())
    {
        actor->catch_tell("You are already disguised.\n");
        return 0;
    }
