...
elder_check()
{
    object elder = find_object(WHITE_LOG);
...
leader_check()
{
    object leader = find_object(WHITE_LOG);
...
do_accept(string str)
{
    ...
    oblist->add_prop(WHITE_ACCEPTED, 1);
    return 1;
}
...
do_warm()
{
        write("You hug yourself tightly and stomp your feet to try "+
                  "and keep warm.\n");
        say("Hugging "+OBJECTIVE(TP)+"self tightly, "+QCTNAME(TP)+" stomps "+
               POSSESSIVE(TP)+" feet to try to keep warm.\n");
        return 1;
}
