...
create_room()
{
    ...
    set_long("You are in the Company join room.\n");
}
...
join(string str)
{
    ...
    if(str == "order"){
      say(QCTNAME(TP) +" joins the Company.\n");
      TP->catch_msg("You join the Order\n");
      ...
      notify_fail("For unknown reasons we couldn't help you join " +
        ...
        shadow =  clone_object("/d/Terel/guilds/white/shadow");
      if ((result = shadow->shadow_me(TP, GUILD_TYPE, GUILD_STYLE_F,
                          GUILD_NAME))!=1) {
