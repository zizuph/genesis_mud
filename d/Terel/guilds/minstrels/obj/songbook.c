...
remove_power(string str)
{
    ...
    // DEBUG("songbook remove_power_songs called in songbook");
...
list_power()
{
    ...
    {
        title = (POWER_SONGS_DIR + power[i])->query_spell_name();
        if (title == 0)
        {
            title = "UNKNOWN";
        }
...
do_erase(string str)
{
    ...
    power_songs = SONGBOOK_MANAGER->query_power_songs(this_player());
    ...
    if (stringp(songs))
    {
        if(!IN_ARRAY(power_song, power_songs))
        {
            notify_fail(songs + "\n");
            return 0;
        }
