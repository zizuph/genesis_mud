/*
    ...
    Updated by Arman 11 August 2019
*/
...
do_approach(string str)
{
	...
	+ "and the hall around you fades to black.\n\nSuddenly a Vision appears: "
...
do_dedicate(string str)
{
	    ...
	    the_muse != lower_case(this_player()->query_muse_name()))
	{
        ...
        {
	    book = clone_object(NEW_GUILDOBJ + "songbook");
	    ...
	    book_given_flag = 1;
            ...
            if(present("performer_instrument", this_player()))
            {
                present("performer_song_book", this_player())->remove_object();
            }
	...
	if (!instrument)
	{
	    write("You take hold of the "+THIS_PED+" and "
			       ...
			       + "with music...\n");
	    instrument = clone_object(NEW_GUILDOBJ + "instrument");
		...
		+ " is left holding a true Minstrel's instrument!\n");
            ...
            if(present("performer_instrument", this_player()))
            {
                object p_inst = present("performer_instrument", this_player()); 
                write("You discard your " +p_inst->query_short()+ 
                    " now that you have a true Minstrel's instrument!\n");
                p_inst->remove_object();
            }
