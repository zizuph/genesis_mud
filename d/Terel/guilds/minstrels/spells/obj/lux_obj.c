...
#define LIGHT_SUBLOC        "_minstrel_light_subloc"
...
create_spell_object()
{
    set_name("_minstrel_light_object_");
    set_short("light spell object");    
    set_long("This is the standard light spell's object. It emits "
        + "light.\n");

    add_prop(OBJ_S_WIZINFO, "This is a spell object " +
                            "created by the Minstrel lux spell.\n");
    set_spell_effect_desc("song of light");
    set_spell_effect_notes("The light of the Muses will follow " +
        "the Minstrel until the harmonics of its existence fades " +
        "away or until the Minstrel dispels it.\n");
}
...
hook_spell_effect_started()
{
    ...
    if (objectp(target))
    {
        target->catch_tell("The very air around you begins to softly glow. " +
            "With careful notes you coax the phosphorescence, teasing the " +
            "light from the air, gradually expanding your Song to fill this " +
            "place with the light of the Muses.\n");
        tell_room(environment(target), QCTPNAME(target)+ " enlightening tune " +
            "awakens a soft glow into existence. " +QCTNAME(target) + 
            " plays on, seeming to coax the phosphorescence out of the very " +
            "music. The Song of Power builds, and reaches a crescendo, and " +
...
hook_spell_effect_ended()
{
    ...
    if (objectp(target))
    {
        target->catch_tell("The light surrounding you fades away.\n");
        tell_room(environment(target), "The light surrounding " +
            QTNAME(target) + " dims and fades away.\n", ({ target }));
    }
}
...
{
    if (for_obj == on)
        return  "You are surrounded by the light of the Muses.\n";
    else
        return capitalize(on->query_pronoun()) + " is surrounded " +
            "by the light of the Muses.\n";
}
