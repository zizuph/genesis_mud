 ...
 *      - Fixed a lot of very wacky formatting (lines with >130
 *          characters and inconsistent indentation)
 *      - Fixed the shop so that nothing works without the smith
 *          present
 *      - The smith now actually says most of the failure messages.
 *
 *
 */
...
{
    ::create_room();
    set_short("Blacksmith of Last");
    set_long("You've entered the blacksmith's forge in Last. Around "+
                   "the room you see various tools. Nearly everything in "+
                   "the forge is covered with soot. The forge itself stands "+
                   "directly in the center. On the wall is a little sign with "+
                   "instructions written on it. You may leave north back out "+
                   "to the street.\n");
    ...
    add_exit(LAST_DIR+"lroad7","north",0);
 ...
 * Description:   clone the smith if necessary
 *
 */
...
reset_room()
{
    ...
    if (!present("brogan")) {
        brogan = clone_object(LAST_DIR+"npc/brogan");
        brogan->move_living("east", TO);
    }
}
...
do_mend(string str)
{
    ...
    brogan->command("say Sorry, it is illegal for me to fix " +
        "broken weapons.");
    return 1;
}
...
do_cost(string str)
{
    ...
    if (sizeof(weapon) > 1)
    {
        brogan->command("say Be more specific. Which one?");
        return 0;
    }

    if (function_exists("create_object",weapon[0]) != "/std/weapon")
    {
        brogan->command("say That is not really a weapon.");
        return 0;
    }
    ...
    brogan->command("say " + sharp_resp);
    brogan->command("say " + wax_resp);
    brogan->command("say " + mend_resp);
...
do_fix(string str)
{
    ...
    */
    ...
    if (pay_type != "")
    {
        ...
        {
            brogan->command("say I don't accept those " +
    ...
    if (get_type != "")
    {
        ...
        {
            brogan->command("say " + capitalize(get_type) +
    ...
    if (!(weapon = FIND_STR_IN_OBJECT(str,this_player())) ||
        !sizeof(weapon))
    {
        brogan->command("say " + capitalize(verb) + " what?");
        return 1;
    }
    ...
    if (value == -1)
    {
        brogan->command("say I cannot " + verb + " that any further.");
        return 1;
    }
    ...
    if (sizeof(money_arr = pay(value, TP, pay_type, 0, 0, get_type)) == 1)
    {
        if (money_arr[0] == 1)
        {
            ...
            brogan->command("say I think that " + weapon[0]->short() +
        ...
        if (money_arr[0] == 2)
        {
            brogan->command("say You do not have any " + pay_type +
