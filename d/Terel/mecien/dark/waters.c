...
enter(string str){
   ...
   tell_room("/d/Terel/mecien/dark/pool",
     QCTNAME(this_player()) + " steps out from the fetid water.\n");
   this_player()->move_living("M", "/d/Terel/mecien/dark/pool");
   return 1;
}
...
create_room(){
  ...
  set_long("Sounds echo throughout the caverns and collect\n" +
   "in this large grotto-cavern. A fetid pool of water reaches\n" +
    ...
    "in great adundance and variety here.\n"
  );
   add_exit("/d/Terel/mecien/dark/s1", "north", 0);
   add_item(({"pool", "water"}),
