...
do_light(str) {
  ...
  if (!is_wiz()) return 0;
  mylight = this_player()->query_prop(OBJ_I_LIGHT);
  roomlight = environment(this_player())->query_prop(OBJ_I_LIGHT);
  if (str == "off") {
    if (roomlight <= 0) {
      notify_fail("The light in this room is already off.\n");
      return 0;
    }
    this_player()->add_prop(OBJ_I_LIGHT, mylight-roomlight);
  }
  else if (str == "on") {
    if (roomlight > 0) {
    ...
    this_player()->add_prop(OBJ_I_LIGHT, mylight-roomlight+1);
  }
  else if (str) {
    ...
    this_player()->add_prop(OBJ_I_LIGHT, mylight-roomlight+newlight);
  }
  mylight = this_player()->query_prop(OBJ_I_LIGHT);
  roomlight = environment(this_player())->query_prop(OBJ_I_LIGHT);
...
do_exec(str) {
  ...
  if (obj = find_object(fname)) destruct(obj);
