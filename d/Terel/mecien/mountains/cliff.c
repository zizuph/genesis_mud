...
object mogre;
if(present("mogre")) return;
mogre=clone_object("/d/Terel/mecien/mountains/mogre");
mogre->move(this_object());
...
add_item("cliff", "It is icy and sheer, but it might be possible to climb.\n");
add_cmd_item("cliff", "climb", "@@climb");
add_exit("/d/Terel/mecien/mountains/ice6","west", 0);
reset_room();
}
...
if((TP->query_skill(SS_CLIMB) + TP->query_stat(SS_DEX) +
   random(100) > 100)){
TP->move_living("by climbing up the cliff", PATH + "crater");
return 1;
}
