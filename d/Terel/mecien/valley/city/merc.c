...
#define MERC present("master_merchant")
#define STORE_ROOM "/d/Terel/mecien/valley/city/merc_st"
...
create_room()
{
   ...
   set_short("The Merry Mercantile");
   set_long(
      "Bright and cheerful, this building is a mercantile exchange " +
      "for goods and supplies. The walls are covered with bright " +
      "paintings and tapestries, carvings and decorations." +
      " A large table is used for examining merchandise.\n");
   
   add_item("paintings", "They are lovely, of nature, people and places far away.\n");
   add_item("tapestries", "Colourful hangings, they adorn the walls, creating a cheerful atmosphere.\n");
   add_item("carvings", "Probably from some local woodworker, they are of animals and creatures of myth.\n");
   add_item("decorations", "Mirrors, baskets, lamps, chains and pieces of jewelry.\n");
   add_item("floor", "It is a bright cherry-wood.\n");
   add_item("table", "It is a large cherr-wood table.\n");
   
   
   add_exit(PATH + "street13", "southeast", 0);
...
stuff(){
  ...
  r=clone_object("/std/rope");
   clone_object(OBJ + "candle")->move(STORE_ROOM);
  r->add_prop(OBJ_I_VALUE, 20);
   clone_object("/std/torch")->move(STORE_ROOM);
  r->move(STORE_ROOM);
}
