...
create_monster() {
   if (IS_CLONE) {
      set_name("tailor");
      set_short("smiling tailor");
      set_long(break_string("This man stands here day in and day out " +
            "selling clothes.  He knows that aspiring adventurers " +
            "sometimes need protection, and has decided to " +
            "make his fortune.  You can buy a cloak by typing 'buy cloak'.  " +
...
do_give() {
   ...
   if (environment()) {
      tell_room(environment(), "The tailor rises from his work.\n");
   }
   seteuid(getuid(TO));
   give(250, TO);
   command("get all");
   ...
   for (i=0;i<4;i++){
      cloak=clone_object(PATH + "plain_cloak");
