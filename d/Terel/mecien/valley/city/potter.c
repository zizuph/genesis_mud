...
create_monster() {
   ...
   set_chat_time(12);
   add_chat("Clay is a joy to work with!");
   add_chat("Pottery is an ancient craft...");
   add_chat("I consider myself an artist");
   add_chat("I have lived in Elnoven for ever so long");
...
do_give() {
    ...
    if (environment()) {
	tell_room(environment(), "Suddenly, from the back room comes " +
		  TO->short() + ".\n");
    }
    seteuid(getuid(TO));
/*
