...
#define STORE_ROOM "/d/Terel/mecien/valley/city/arm_st"
...
create_room()
{
  ...
  set_short("Armory");
    set_long(
  "Filled with smoke and steam, this rough stone hall is an\n"
 +"armory. The floor of deep gray stone, littered with small bits\n"
  +"of metal and links of chain. A large anvil and forge fill the\n"
  +"back portion of the hall, the rest is filled with samples of\n"
  +"different types armours.\n"
  );

  add_item("smoke", "It comes from the forge.\n");
   add_item("anvil", "It is a large steel anvil.\n");
  add_item(({"scraps", "chain", "bits of metal", "links",
     "links of chain"}), "Merely scraps from armour crafting.\n");
   add_item("forge", "A small forge for treating armour.\n");
  add_item("samples", "They are various pieces of armour on display.\n");

add_exit(PATH + "street4", "west", 0);
...
sell_it(object *ob, string str, int check) 
{
  ...
  if(ob[i]->check_armour()<1){
   NF("We only purchase armours.\n");
   continue;
   }
