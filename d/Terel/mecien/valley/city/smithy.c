...
#define STORE_ROOM "/d/Terel/mecien/valley/city/wep_st"
...
create_room()
{
   ...
   set_short("Smithy");
   set_long(
    "This is a dark and smokey smithy. All about the floor are " +
"bits and scraps of metal. A huge forge is open on the side of " +
"the wall, out of which pours smoke. A massive anvil rests here " +
"in the midst of all the debris. All about the walls are samples " +
"of weapons not yet completed.\n"
   );
   
   add_item("smoke", "It comes from the forge.\n");
   add_item("anvil", "It is a large steel anvil.\n");
   add_item(({"scraps", "chain", "bits of metal", "links",
            "links of chain"}), "Merely scraps from weapon crafting.\n");
   add_item("forge", "A small forge for treating armour.\n");
   add_item("samples", "They are parts of weapons that are not yet finished.\n");
   
   
   add_exit(PATH + "street11", "south", 0);
...
sell_it(object *ob, string str, int check) 
{
   ...
   for (i = 0; i < sizeof(ob); i++)
   {
      ...
      if(ob[i]->check_weapon()<1){
         NF("We only purchase weapons.\n");
         continue;
        }
