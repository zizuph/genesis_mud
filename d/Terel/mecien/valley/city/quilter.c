...
set_gender(1);
  add_chat("Peace is given to us here");
   add_chat("I would be very happy is the thunder grew louder");
  add_chat("I once was in the Great Shrine, a guest of the Elder");
add_chat("Did you know that the evil dead walk with the forest?");
  add_chat("If there were knights once more, no more would brigands come to this city");
   add_chat("The Order shall always be in my heart, I was changed from walking with them");
   add_chat("Wisdom comes most often with age");
  add_chat("Twelve statues there are! Rumours say they are divine, that were once gods turned to stone!");
  add_chat("Though you may not know it, many strange things have uses in these parts");
  add_chat("I saw lightning up on the hill");
  add_chat("Believe me, the Mystic powers are dark indeed!");
  set_act_time(1);
  add_act("emote works on some quilting.");
  add_act("smile mysteriously");
   add_act("emote utters a short prayer in honour of the stars.");
   add_act("emote shakes off her quilt.");
   add_act("emote pricks herself with a needle.");
   add_chat("Long ago, great knights guarded us from evil");
	seq_new("foo");
...
do_give() {
    ...
    if (environment()) {
   tell_room(environment(), "Standing here doing a bit of work is " +
		  TO->short() + ".\n");
    }
    seteuid(getuid(TO));
/*
 ...
 */
    ...
    for (i=0;i<1;i++){
     quilt=clone_object(PATH + "quilt");
      quilt->move(this_object());
      }
}
...
sell(string what) {
    ...
    set_money_give_out(what_coins(TP));
	...
	if (money_merge(result[0 .. NUM - 1]) <= ( tot_price + 
		money_merge(result[NUM .. 2 * NUM - 1])))
	{
...
value(string str)
{
	...
	write("A sell would result in:\n");
        ...
        if (sizeof(arr) > 1)
	{
	    if (money_merge(arr[0 .. NUM - 1]) <= ( price +
		money_merge(arr[NUM .. 2 * NUM - 1])))
	    {
	        arr = pay(price, TO, "", 1, TP);
	        if (sizeof(arr) == 1)
		    return 0;
