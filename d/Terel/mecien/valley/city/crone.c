...
create_monster() {
   if (IS_CLONE) {
      set_race_name("human");
      set_living_name("yeressa");
      add_adj("weathered");
      set_name("yeressa");
      add_name("crone");
      set_short("weathered crone");
      ...
      set_random_move(52);
      set_act_time(30);
      add_act("emote fiddles with some twigs.");
      add_act("emote looks up into the sky.");
      add_act("emote takes out a small scroll and reads it.");
      add_act("emote eats a piece of bread.");
...
stuff(){
  clone_object(OBJ + "herb2")->move(STORE_ROOM);
   clone_object(OBJ + "herb4")->move(STORE_ROOM);
   clone_object(PATH + "garland")->move(STORE_ROOM);
    clone_object(PATH + "wreath")->move(STORE_ROOM);
}
...
do_give() {
   ...
   */
   ...
   for (i=0;i<6;i++){
      flower=clone_object(PATH + "flower");
   flower->move(STORE_ROOM);
   }
}
