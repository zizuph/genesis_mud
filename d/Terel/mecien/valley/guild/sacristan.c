...
#define THINGS   ({OBJ+"mantle", OBJ+"robe", OBJ+"mana_incense",\
                   OBJ+"alb", OBJ+"heal_incense"})
#define HOWMANY  ({1, 2, 5, 2, 5})
...
create_monster() {
   if (IS_CLONE) {
      ...
      set_name("elyinar");
      add_name("sacristan");
      set_short("Elyinar the Sacristan");
      set_long(break_string(
            "He is the sacristan of the Order. He provides the vestments and "
            +"other necessities used in ceremonies. His short silver hair glistens "
            +"under his gray cowl, his soft blue eyes bring peace to those who "
            +"know truth. Garbed in a gray robe and mantle, this noble elf is a "
            +"monument of wisdom and discipline.\n", 70));
      ...
      add_ask(({"for lamp", "for sacred lamp", "lamp"}), "@@do_lamp", 1);
...
{
   if (environment()) {
      tell_room(environment(), "Arising from a deep meditation " +
         TO->query_name() + " stands and stretches.\n");
   }
   give(500, TO);
   command("get all");
   init_trader();
...
ask(string str){
   ...
   if(str!="thurible" && str!="for thurible") return 0;
   if(this_player()->query_guild_name_occ()!="The Ancient Mystic Order"){
      this_object()->command("say You are not of the Order, you have no business here");
      return 1;
   }
   obj=present("thurible", this_player());
   if(obj){
      this_object()->command("say You possess the sacred vessel already");
      return 1;
   }
   obj=clone_object(OBJ + "thurible");
   obj->move(this_player());
   write("The sacristan takes a thurible from the cabinet and gives it to you.\n");
   say("The sacristan takes something from the cabinet and gives it to " +
      QCTNAME(this_player()) + ".\n");
   return 1;
...
do_lamp(){
   ...
   obj=present("sacred_lamp", this_player());
   if(obj){
      this_object()->command("say You possess the sacred lamp already");
      return 1;
   }
   obj=clone_object(OBJ + "lamp");
   obj->move(this_player());
   write("The sacristan takes a lamp from the cabinet and gives it to you.\n");
   say("The sacristan takes something from the cabinet and gives it to " +
      QCTNAME(this_player()) + ".\n");
   return 1;
