...
#define STORE_ROOM present(MYSTIC_SATCHEL)
#define NF(x) notify_fail(x)
#define NUM 4

#define THINGS   ({OBJ+"hpot", OBJ+"candle", OBJ+"grapes",\
      OBJ+"herb2", OBJ+"herb1"})
#define HOWMANY  ({1, 3, 3, 3, 1})
...
create_monster() {
   ...
   set_living_name("herenas");
   set_race_name("human");
  set_name("herenas");
    add_name("vine-dresser");
    set_adj(({"wise", "old"}));
   config_default_trade();
   set_money_give_reduce( ({ 0, 2, 3, 3 }) );
   
    set_long("A rustic looking man, dressed in a simple leather coat, "+
  "green leggings and shirt. He looks out over the vinyard, making sure " +
"all things are as they should be. A great wisdom appears in his face, as if " +
"knows many things.\n");
   
   set_all_hitloc_unarmed(35);
   
  set_act_time(1);
   add_act("emote prunes away some dead vines.");
   add_act("emote points south.");
   add_act("emote whistles a mysterious tune.");
   set_chat_time(5);
  add_chat("I love this place");
   add_chat("So many wonderful things grow here");
   add_chat("I pray that the dryad will visit once again");
    add_chat("The spirits of this place are wonderful, they grant us peace");
   add_chat("Fruit and honey, how sweet is life");

   set_skill(SS_DEFENCE, 60);
   set_skill(SS_UNARM_COMBAT, 80);
   
   set_alarm(2.0, -1.0, "give_me_stuff");
   seq_new("foo");
...
give_me_stuff(){
   object gar;
   gar = clone_object("/d/Gondor/common/herbs/garlic");
   gar->move(STORE_ROOM);
   gar->add_prop(OBJ_I_VALUE, 50);
   clone_object(OBJ + "mana_incense")->move(STORE_ROOM);
   clone_object(OBJ + "herb3")->move(STORE_ROOM);
   clone_object(OBJ + "herb5")->move(STORE_ROOM);
...
init_trader()
{
   ...
   for (i=0; i<sizeof(THINGS); i++) {
      for (j=0; j<HOWMANY[i]; j++) {
         clone_object(THINGS[i]) -> move(STORE_ROOM);
        }
   }
}
...
{
   if (environment()) {
      tell_room(environment(), TO->short() + " cuts some vines with his hook.\n");
   }
   give(150, TO);
   command("get all");
  clone_object(OBJ + "satchel")->move(TO);
   clone_object(PATH + "courtyard/pruning_hook")->move(TO);
    command("wield hook");
   init_trader();
}
...
reset_trader()
{
   ...
   ob = all_inventory(STORE_ROOM);
   for (i=0; i<sizeof(ob); i++)
   if (member_array(MASTER_OB(ob[i]), THINGS) >= 0)
      ob[i]->remove_object();
...
do_list(string str)
{
   ...
   if (sizeof(item_arr) < 1)
      {
      NF("I am sorry. I have nothing to sell.\n");
      return 0;
   }
   ...
   if (i < MAXLIST)
      ...
      str = sprintf("%-25s",
         capitalize(LANG_ASHORT(item_arr[i])));
...
buy(string str) {
   ...
   obs = FIND_STR_IN_OBJECT(what, STORE_ROOM);
   if (sizeof(obs) == 0)
      {
