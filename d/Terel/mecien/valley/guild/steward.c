...
#define THINGS     ({OBJ+"chalice", OBJ+"candle_f"})
#define HOWMANY    ({2, 3})
...
create_monster()
{
    ...
    add_name("steward");
    add_name("man");
    set_short("Cusuth the Steward");
    set_long("A man of great virtue, he carries with him the\n"
            +"strength of the ancient days. He serves the mystics and\n"
            +"cares for the shrine. His hair is white and silvered, his eyes\n"
            +"dark and forboding. He seems aged, but has taken the toll\n"
            +"of time very well. We wears a white robe, a silver sash,\n"
            +"a silver mantle and a silver star amulet.\n");
    set_stats(({ 78, 83, 92, 87, 91, 89}));
    set_alignment(212);
    set_skill(SS_DEFENCE, 82);
    ...
    set_chat_time(5);
    add_chat("Hail!");
    add_chat("Our nourishment is of both worlds");
    add_chat("Truth comes to those with Light, Light comes to those" +
             " with truth...");
    add_chat("Do you need food?");
    add_chat("Peace go with you always...");
...
{
    if (environment()) {
	tell_room(environment(), TO->short() + " greets you with joy.\n");
    }
    give(150, TO);
    command("get all");
    init_trader();
