...
#define THINGS   ({OBJ+"mantle", OBJ+"candle_h", OBJ+"candle_m",\
		   OBJ+"vial", OBJ+"satchel"})
...
create_monster() {
   ...
   set_long(break_string(
       "A jolly and saintly man, he stands tall and proud, robed in royal blue"
      + " and cloaked in purple. His hair is white, his eyes blue. He is the"
      + " master of the mystic emporium, making sure the mystics stay supplied"
   ...
   set_alarm(1.0, -1.0, "give_me_stuff");
	seq_new("foo");
...
give_me_stuff(){
  clone_object(OBJ + "alb")->move(TO);
   clone_object(OBJ + "mana_incense")->move(TO);
  clone_object(OBJ + "chalice")->move(TO);
   clone_object(OBJ + "mantle")->move(TO);
      clone_object(OBJ + "cowl")->move(TO);
  clone_object(OBJ + "holy_incense")->move(TO);
