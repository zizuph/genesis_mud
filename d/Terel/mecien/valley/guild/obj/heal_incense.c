...
do_it(object to_be_affected)
{
    ...
    {
        to_be_affected->heal_hp(HP_TO_HEAL);
        return 1;
    }
    return 0;
}
...
{
    num_uses = uses;
    add_prop(OBJ_I_VALUE, 2 * num_uses * HP_TO_HEAL);
}
...
{
    set_name("incense");
    set_short("small amount of white incense");
    set_pshort("small amounts of white incense");
    add_adj(({"small", "white"}));
