...
void set_stats_and_skills_upon_joining(object who);
void set_stats_and_skills_upon_leaving(object who);

enter_inv(object ob, object from){
   ::enter_inv(ob, from);
if(living(ob)){
  if(ob->query_prop(LIVE_I_UNDEAD)){
    set_alarm(2.0, -1.0, "evil", ob);
   }
}
}
...
evil(object ob){
  ...
  here = EN(ob);
  if(here == TO){
   tell_room(here, "A strange noise comes from the statues.\n");
   ob->add_mana(-20);
   ob->add_panic(20);
  ob->catch_msg("You feel weaker as some mysterious force seems to grasp you.\n");
   set_alarm(5.0, -1.0, "evil", ob);
  }
}
...
join(string str)
{
  ...
  if(!present(MYSTIC_BEADS, TP) && !TP->query_skill(BLESSED))
  {
  NF("You have not yet found guidance.\n");
   return 0;
   }
   if (!TP->query_skill(BLESSED)) {
      NF("You have not been chosen by an ancient disciple.\n");
      return 0;
   }
   ...
   if (TP->query_skill(ENTRANCE_QUEST)==0) {
       ...
       write("A mysterious voice says: Let first thy name be proven. Bring " +
             "us an offering that shall mark thy name in dignity.\n");
...
/* If the MOQ is solved let the player join */
   ...
   if (TP->test_bit("Terel", QUEST_MOQ_GROUP, QUEST_MOQ_BIT)) {
       ...
       /*
        * Add the lay shadow too...
        */
       shadow = clone_object(MYSTIC_LAY_SHA);
       if (shadow->shadow_me(TP, LAY_TYPE, LAY_STYLE, LAY_NAME) != 1) {
           NF("For some unknown reason you cannot join this guild.\n");
           "/secure/master"->do_debug("destroy", shadow);
           return 0;
       }
       TP->remove_guild_race("/d/Terel/mecien/valley/guild/seeker_sh");

       set_stats_and_skills_upon_joining(TP);
       ...
       if (!mring) {
          ...
          mring->add_prop("_mystic_s_owner", TP->query_real_name());
       }
       write("Thou art now of this Order.\n");
...
leave(string str)
{
   ...
   if (TP->remove_guild_occ()) {
       ...
       set_stats_and_skills_upon_leaving(TP);
       write("The void enters your soul and the Ancients depart from you.\n");
       TP->remove_cmdsoul(MYSTIC_SOUL);
       for (i=0; i<sizeof(MYSTIC_OBJS); i++) {
         if (ring = present(MYSTIC_OBJS[i], TP)) {
           write("A spirit appears and takes away your " + ring->short() +
                 ".\n");
           ring->remove_object();
         }
       }
       ...
       write("So be it. The Ancients do not punish you though.\n");
       say(QCTNAME(TP) + " breaks his vows before the Ancients.\n");
       TP->move_living("for the mundane life", TP->query_def_start(), 1);
   } else {
...
ask(string str)
{
   ...
   ring->move(TP);
   ring->add_prop("_mystic_s_owner", TP->query_real_name());
...
set_up_skills()
{
   ...
   sk_add_train(SS_DEFENCE,     ({ me, ot }), 0, 0, MYSTIC_MAX_DEFENCE);
   me = "sense presences"; ot = me;
   sk_add_train(SS_AWARENESS,   ({ me, ot }), 0, 0, MYSTIC_MAX_AWARENESS);
   me = "parry"; ot = me;
   sk_add_train(SS_PARRY,       ({ me, ot }), 0, 0, MYSTIC_MAX_PARRY);
   me = "climb"; ot = me;
   sk_add_train(SS_CLIMB,       ({ me, ot }), 0, 0, MYSTIC_MAX_CLIMB);
}
...
{
   if (!MEMBER(TP)) {
      write("You are not a member of this Order.\n");
      return 1;
   }
...
void
set_stats_and_skills_upon_joining(object who)
{
    if (who->query_wiz_level()) return;

    /* First we reset the stats SS_OCCUP and SS_LAYMAN to zero */
    who->set_guild_stat(SS_OCCUP, 0);
    who->set_guild_stat(SS_LAYMAN, 0);

    /* Next, we set certain (weapon) skills not to exceed maximum
       allowed values for mystics, if necessary. */
    if (who->query_skill(SS_WEP_SWORD) > MYSTIC_MAX_SWORD) {
        who->catch_msg("The Ancients reduce your ability to use swords.\n");
        who->set_skill(SS_WEP_SWORD, MYSTIC_MAX_SWORD);
    }

    if (who->query_skill(SS_WEP_KNIFE) > MYSTIC_MAX_KNIFE) {
        who->catch_msg("The Ancients reduce your ability to use knives.\n");
        who->set_skill(SS_WEP_KNIFE, MYSTIC_MAX_KNIFE);
    }
}

void
set_stats_and_skills_upon_leaving(object who)
{
    if (who->query_wiz_level()) return;
...
*/
    ...
    /* We also remove special skills. */
/*
    who->remove_skill(SS_CHANNELING);
    who->remove_skill(MYSTIC_RANK);
    who->remove_skill(BLESSED); 
    who->remove_skill(PIETY);
*/

    who->setup_skill_display();

    /* Next, we remove or reduce certain (magic) skills that has been
     * learned in the guild */

    /* who->catch_msg("The Ancients make you forget some of the skills you " +
     *              "have been taught by them.\n");
                   
     * if (who->query_skill(SS_SPELLCRAFT) > MYSTIC_MAX_SPELLCRAFT/2)
     *  who->set_skill(SS_SPELLCRAFT, MYSTIC_MAX_SPELLCRAFT/2);
                   
     * if (who->query_skill(SS_FORM_DIVINATION) > MYSTIC_MAX_DIVINATION/2)
     *   who->set_skill(SS_FORM_DIVINATION, MYSTIC_MAX_DIVINATION/2);

     * if (who->query_skill(SS_ELEMENT_FIRE) > MYSTIC_MAX_FIRE/2)
     *   who->set_skill(SS_ELEMENT_FIRE, MYSTIC_MAX_FIRE/2);

     * if (who->query_skill(SS_ELEMENT_AIR) > MYSTIC_MAX_AIR/2)
     *   who->set_skill(SS_ELEMENT_AIR, MYSTIC_MAX_AIR/2);

     * if (who->query_skill(SS_ELEMENT_DEATH) > MYSTIC_MAX_DEATH/2)
     *   who->set_skill(SS_ELEMENT_DEATH, MYSTIC_MAX_DEATH/2);

     * if (who->query_skill(SS_ELEMENT_LIFE) > MYSTIC_MAX_LIFE/2)
     *   who->set_skill(SS_ELEMENT_LIFE, MYSTIC_MAX_LIFE/2);

     * if (who->query_skill(SS_ELEMENT_EARTH) > MYSTIC_MAX_EARTH/2)
     *   who->set_skill(SS_ELEMENT_EARTH, MYSTIC_MAX_EARTH/2);
     */
}
