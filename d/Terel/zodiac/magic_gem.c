 ...
 * 8/14/2008: Removed dusted gem check as per Lilith
 * 3/23/2009: Added relaxed from combat check for beginning and end
 * 4/27/2009: Reduced mana and charge penalty for when using when not
 *            relaxed from combat.
 */
inherit "/std/object";
...
#define CHARGE_TIME 2000.0
#define TELE_DELAY  10.0
...
int charges = 50;  /* How many charges left in the gem */
...
init_arg(string arg)
{
    ...
    if (swallow)
    {
        recharge_id = set_alarm(CHARGE_TIME, -1.0, "recharge");
        set_no_show();
    }
...
do_rub(string str)
{
    ...
    {
        write("The gem is suddenly filled with a luminous glow!\n");
    ...
    set_alarm(200.0, -1.0, "decrease_light");
    return 1;
}
...
do_kiss(string str)
{
    ...
    {
        // It costs a little mana to use when you aren't relaxed from
        // combat. This is to prevent abuse of it to check relaxed.
        mana_cost = CHARGE_MANA + TP->query_max_mana() / 10;
    }
        
    manas = TP->query_mana();
    if (manas < mana_cost)
    {
        write("A shiver goes through your body - " +
              "but it seems you're too weak.\n");
        return 1;
    }
    ...
    if (!TP->query_relaxed_from_combat())
    {
        // If not relaxed, we don't decrease any charges, just give
        // a message and return.
        write("You try to concentrate, but with all the recent action, "
            + "you just end up hurting your head a bit. Nothing "
            + "happens.\n");
        return 1;
    }
    ...
    if (success > 0)
    {
        if (!decrease_charges(12))
            return 1;
        write("You concentrate yourself and get together " +
              "all your power... You can feel something is going " +
              "on...\n");
        to_summon = TP;
        set_alarm(TELE_DELAY, -1.0, "do_summon");
    }
    else
        write("You concentrate yourself and get together " +
...
do_swallow(string arg)
{
    ...
    write("You're having a hard time getting it down. It's large " +
          "and unexpectedly coarse, but it's going doooooooooooooown...\n");
    set_no_show();
...
do_summon()
{
    ...
    if (!to_summon->query_relaxed_from_combat())
    {
        to_summon->catch_tell("Somehow you cannot be summoned. You sense "
            + "the adrenaline clouding your concentration may have "
            + "disturbed the summoning effect.\n");
        to_summon = 0;
        return;
    }
    
    tell_object(to_summon, "Suddenly, you feel magically summoned...\n");
    if(to_summon->query_temp_start_location())
    {
        to_summon->move_living("X",
                  to_summon->query_temp_start_location());
        to_summon = 0;
        return;
    }
    to_summon->move_living("X",
...
decrease_charges(int ch)
{
    ...
    if (charges < 0)
    {
        ...
        write_file(LOST_GEM_LOG, capitalize(TP->query_real_name()) +
...
dispel_magic(int strength)
{
    ...
    if (query_prop(OBJ_I_LIGHT) > 0)
    {
        ...
        if (living(ETO))
            say("The light in the gem held by " + QCTNAME(ETO) +
                " suddenly goes out!\n");
        else
...
decrease_light()
{
    ...
    if (living(ETO))
    {
        if (query_prop(OBJ_I_LIGHT) > 0)
        {
            ETO->catch_msg("The gem suddenly turns a bit duller!\n");
            tell_room(environment(ETO),
                      "The bright gem held by " + QTNAME(ETO) +
                      " suddenly turns a bit duller!\n", ETO);
        }
        else
        {
            ETO->catch_msg("The luminous glow from the gem suddenly " +
                           "winks out!\n");
            tell_room(environment(ETO),
                      "The light emanating from the gem held by " +
                      QTNAME(ETO) + " suddenly winks out!\n", ETO);
        }
    }
    else if (ETO->query_prop(ROOM_I_IS))
    {
        if (query_prop(OBJ_I_LIGHT) > 0)
            tell_room(ETO, "The bright gem suddenly turns a bit duller!\n");
        else
            tell_room(ETO, "The light emanating from the gem suddenly winks out!\n");
    }
}
...
recharge()
{
    ...
    if (charges <= 45)
    {
        ETO->catch_msg("You feel a funny twitch in your stomach!\n");

        charges += 5;
        ETO->add_mana(-CHARGE_MANA);

        fix_long(charges);
    }
    else
    {
        ETO->catch_msg("Your tummy is a bit upset!\n");
...
enter_env(object dest, object old)
{
    ...
    if (change)
        dest->catch_msg("The gems disintegrate into fine dust! However... the " +
              "cloud of incandescent particles immediately condenses into a " +
              "single gem again!\n");
