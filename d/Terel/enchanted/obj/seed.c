...
#define HERB_FILE    ENCHANT_DIR + "obj/trefoil"
...
grow_herb(object who)
{
    ...
    if (who)
        tmp = who->query_stat(SS_WIS)/2 + who->query_skill(SS_HERBALISM);
    herb->set_herb_value(5 * tmp / 2);
    herb->set_effect(HERB_HEALING, "hp", 2 * tmp);
    herb->move(ETO);
