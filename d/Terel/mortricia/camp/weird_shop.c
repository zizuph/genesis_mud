/* 
   ...
   This is the Gypsy Camp version, Modified by Mortricia on 9/25/92
*/
...
#define STORE_ROOM       (CAMP_DIR + "store")
#define MAXLIST          30
...
object keeper;
/*  This function will prepare the room for the shopkeeper */
...
reset_room(arg)
{
   ...
   keeper = clone_object(CAMP_DIR + "shop_woman");
   keeper -> move(TO);
}
...
create_room()
{
    ...
    set_short("Weird gypsy shop");
    set_long(break_string(
       "You are in a weird tent. There are many shelves "
     + "here where various goods are kept. There is a counter here, "
     + "and behind it, on the canvas, some words are painted. They "
     + "say 'WELCOME TO WEIRD SHOP'. "
     + "There is a small opening in the canvas behind the counter to the "    
     + "north, and the exit is to the south. "
     + "There is also a small sign with more instructions. "
     + "\n", 70));

    add_exit(CAMP_DIR + "camp_05", "south", 0);
    add_exit(STORE_ROOM          , "north", "@@wiz_check");

/*
    "/d/Terel/common/town/store"->teleledningsanka();
 */
    "/d/Terel/mortricia/camp/store"->teleledningsanka();
...
wiz_check()
{
    if (this_player()->query_wiz_level())
      	return 0;
    write("You are prevented from going north by a strong magic force.\n");
...
do_read(string str)
{
	...
	"               more than one item at a time. So if you really need\n" +
...
do_sell(string str)
{
    ...
    if(!keeper){
       NF("You can't do that!  Tricia is dead!\n");
      return 0;
    }
...
sell_it(object *ob, string str, int check) 
{
        ...
        if (ob[i]->query_prop(OBJ_I_VALUE) == 0)
	{
        ...
        if (ob[i]->query_prop("q_solver"))
	{
            NF("The " + ob[i]->query_short() + " has no value.\n");
	    continue;
        }
        if (not_interesting(ob[i])) continue;
        if ( (tmp = ob[i]->query_prop(OBJ_M_NO_SELL)) )
        {
	    if (stringp(tmp)) {
		NF(capitalize(tmp));
	    } else {
		keeper->command("say I won't buy the "+ob[i]->short()+".");
                NF("The " + ob[i]->short() + " looks strange.\n");
	    }
	    continue;
        }
	...
	if (check && (ob[i]->query_worn() ||
		      ob[i]->query_wielded()))
	{
       NF("I don't buy wielded or worn items.\n");
	    continue;
        }
