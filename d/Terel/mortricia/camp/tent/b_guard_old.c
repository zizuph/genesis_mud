...
create_monster()
{
    ...
    set_long(BSN(
        "It's a dark guard. He keeps people from going southwest."
    ));
    set_adj("dark");
    set_race_name("gypsy");
    set_gender(0);
...
do_show(string what)
{
    ...
    if (ring && ring->query_worn() == TP) {
        write("You show your golden ring to the guard.\n");
        write("The guard pushes you further into the tent.\n");
        TP -> move_living("southwest", TENT_DIR + "behind1");
    } else {
	TO -> command("tackle " + name);
