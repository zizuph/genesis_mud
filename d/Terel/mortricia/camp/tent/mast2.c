...
reset_room()
{
    ...
    if (!walker) {
        walker = clone_object(TENT_DIR + "walker");
        walker -> move(TO);
    }
}
...
create_room()
{
    set_short("Top of southern mast");
    set_long(BSN(
        "You have reached a platform up in the southern mast. " +
        "Up here you have quite an excellent view of the tent " +
        "interiors. There is a wire going northwest from here."
    ));
    ...
    add_item(({"wire"}), BSN(
        ...
        "mast in northwest corner of the tent. And, most important " +
        "of all, there is no safety net below on the manege."
    ));
    add_item(({"manege"}), BSN(
    ...
    add_exit(TENT_DIR + "tent_04", "down", 0);
    add_exit(TENT_DIR + "wire"   , "northwest", "@@to_wire");
...
to_wire()
{
    ...
    if (tp_dis > 20) {
        write(BSN(
             ...
             "walker. Hence, slowly you proceed out on the wire."
        ));
        return 0;
    }
    write(BSN(
        "You realise that walking out on the wire is above " +
