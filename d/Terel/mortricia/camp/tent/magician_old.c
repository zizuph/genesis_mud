...
do_show(string what)
{
    ...
    NF("Show ring to whom?\n");
    if (s2 != "magician" && lower_case(s2) != "penthir") return 0;

    ring = check_sh_pr("iron ring", "q_solver", TP);
    who = lower_case(TP->query_name());
    if (ring && ring->query_worn()) {
        write("You show your iron ring to the magician.\n");
        command("say So, you're a quest solver.");
        command("ruffle " + who);
    } else {
        ring = check_sh_pr("gold ring", "q_solver", TP);
        if (ring && ring->query_worn()) {
            write("You show your gold ring to the magician.\n");
