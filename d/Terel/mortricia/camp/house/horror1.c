...
reset_room()
{
    ...
    for (i=0; i<NO_OF_SOULS; i++) {
	if (!souls[i]) {
            souls[i] = clone_object(HAUN_DIR + "soul");
            set_alarm(itof(2+2*i*i), -1.0, &move_here(souls[i]));
	}
    }
}
...
create_room()
{
    ...
    set_long(
        ...
        "The chamber is lit up by some glowing stones which " +
    ...
    add_exit(HAUN_DIR + "house_4", "southeast", "@@leave");

    souls = allocate(NO_OF_SOULS);
