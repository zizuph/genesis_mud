...
create_room()
{
    ...
    set_long("The backyard is well maintained. A row of small shrubs " +
       "outline the white graveled path leading north towards two " +
       "smaller buildings. Another path leads west around the house.\n");
    ...
    add_item(("buildings"),"The building towards the northwest " +
       "looks like the greenhouse. The building to the northeast " +
       "looks like the servants quarters or guest house.\n");

    add_item(({"greenhouse","guest house","servants quarters","quarters"}),
       "You'll have to get closer to get a better look.\n");
    ...
    add_exit(CALATHIN_DIR + "houses/house4_yard3","west",0,1);
    add_exit(CALATHIN_DIR + "houses/house4_yard2","north",0,1);


    door = clone_object(CALATHIN_DIR + "houses/obj/house4da_door");
       door->move(TO);
