...
create_room()
{
    ...
    set_short("backyard");
    set_long("The path ends in a circular pattern between two buildings. " +
       "In the middle of the path is a large fountain. The building to " +
       "the west is the greenhouse. The building to " +
       "east appears to be the servants quarters.\n");

    add_item(({"large fountain","fountain"}),
       "The large fountain is of several small cherubs flying about a " +
       "maiden wearing a thin dress staring into the sky with her " +
    ...
    add_item(({"greenhouse","glass"}),"The greenhouse is mostly made from " +
       "large window panes supported by a wooden frame. The windows " +
       "are fogged due to steam.\n");

    add_item(({"servants quarters","quarters"}),
       "The small building can be seen to the east of the gravel path.\n");
   ...
   add_item(({"yard","backyard"}),
    ...
    add_exit(CALATHIN_DIR + "houses/house4_yard","south",0,1);
    
    door = clone_object(CALATHIN_DIR + "houses/obj/house4_gh_door");
       door->move(TO);
    door = clone_object(CALATHIN_DIR + "houses/obj/house4_sv1_door");
       door->move(TO);
...
do_smell(string what)
{
    ...
    ob = all_inventory(TO);
    poss = POSSESSIVE(TP);
    smells = ({"mint", "sumac", "cedar trees", "parsley"});
