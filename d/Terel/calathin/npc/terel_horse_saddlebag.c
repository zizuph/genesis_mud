...
//    return 0;
  ...
  write ("You drape the "+saddlebag->short()+" over the "+
	 steed->short_old()+"'s back.\n");

  say (QCTNAME(TP)+" drapes a "+saddlebag->short()+ " over the back of the "+
        steed->short_old()+".\n");
...
additional_longdesc()
{
  if (!present("saddlebag"))
    {  
      ...
      return capitalize(TO->query_pronoun())+" has a saddlebag draped across "+
...
do_remove(string arg)
{
  ...
  parse_command(arg, environment(TP), "[the] %s 'from' %o",target , steed);
...
do_open(string arg)
{
  ...
  if (!saddlebag->query_prop(CONT_I_CLOSED))
    {
    write ("The "+saddlebag->short()+" is already open!\n");
    return 1;
    }

  saddlebag->add_prop(CONT_I_CLOSED,0);

  write ("You open the "+saddlebag->short()+" on the "+
	 steed->short_old()+"'s back.\n");

  say (QCTNAME(TP)+" opens the "+saddlebag->short()+" on the "+
       steed->short_old()+"'s back.\n");
...
do_close(string arg)
{
  ...
  say (QCTNAME(TP)+" closes the "+saddlebag->short()+" on the "+
       steed->short_old()+"'s back.\n");
