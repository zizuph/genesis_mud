...
create_monster()
{
    if (IS_CLONE) {
        set_name("merchant");
        set_short("fat old merchant");
        set_long("This man stands here day in and day out " +
            "selling torches.  He knows that aspiring adventurers " +
            "sometimes need portable light, and has decided to " +
            "make his fortune.  You can buy a torch by typing 'buy torch'. " +
            " To see a list with prices, type 'list'.\n");
...
do_give()
{
    ...
    for (i=0;i<10;i++){
     torch=clone_object(TOWNDIR + "torch");
...
do_list(string str)
{
    ...
    item_arr = FILTER_CAN_SEE(item_arr, TO);
    /* We only want the torches to show up */
    item_arr = filter(item_arr, "torch_filter", TO);
...
/* Function to identify a torch/candle */

public int
torch_filter(object ob)
{
    return (ob->query_name() == "torch" || ob->query_name() == "candle");
}
...
buy(string str) {
    ...
    /* This merchant _only_ sells torches (or perhaps candles) */
    if (!torch_filter(buy_ob))
    {
	    NF("I have no such thing to sell to you.\n");
	    return 0;
    }
    price = 2 * buy_ob->query_prop(OBJ_I_VALUE);
