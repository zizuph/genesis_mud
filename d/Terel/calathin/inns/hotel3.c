 ...
 * A hotel room.
 *
 * This is a starting place for the Explorer's Club, which is also a drop room.
 * I have removed the access to the back entrance/trapdoor. 
 * It looks like Shinto intended it for his Company guild
 * which was never completed, and the AoD and AoB do not like clubs 
 * having hidden access to areas others cannot go.
 *
 * Lilith, May 2008
 *
...
{
    ::create_room();
    set_short("Explorer's lodging room");
    ...
    add_item("bed","You see a crate at the end of the bed, and a shelf "+
  		  "running along the wall next to it.\n");

    /*add_item("crate","A large wooden crate sits at the end of the bed. " +
                     "It looks like it can hold a lot of larger items like weapons and armour. "+
                     "You may <sort crate> to sort non-saving from the saving items.\n");

    add_item("shelf","A common wooden shelf on the northern wall. " +
                     "It looks like you can put smaller things like jars, belts" +
		     ", etc in it. You may <clean shelf> " +
                     "to sort non-saving from the saving items.\n");*/
   ...
   add_cmd_item(({"peephole","peep","hole"}),(({"use"})),
    ...
    //shelf = clone_object(CALATHIN_DIR + "inns/shelf");
      // shelf->move(TO);
...
enter_inv(object ob, object from)
{
    ...
    if (interactive(ob))
    {
        ...
        ob->catch_tell(" >>> THE EXPLORERS CLUB IS CLOSED! <<<\n\n");
...
trapdoor()
{
    ...
    write("You attempt to push open the trapdoor below you, but it "+
        "is permanently barred from the other side.\n");

/* This is illegal per AoD and AoB -Lilith May 2008
