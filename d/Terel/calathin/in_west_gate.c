...
#define N_GATE CALATHIN_DIR + "west_gate"
...
create_room()
{
    set_short("the west gate of Calathin");
    set_long("This is the west gate of Calathin. Large wooden gates stand before "+
                   "you, leading out through the walls of the town. Guards are posted "+
                   "above to operate the gates and keep watch. The road leads east "+
                   "into the town.\n");
    ...
    add_item(({"town","calathin","Calathin"}), 
                      "To the east lies the town of Calathin. Trees can be seen "+
                      "throughout the town lightly covered with snow. A number "+
                      "of buildings form the town square. To the south another a"+
                      "gate leads out of the town. The view to the east is "+
                      "blocked by a large mansion to the east.\n");
    ...
    add_exit(CALATHIN_DIR + "roads/road_w07", "east", 0);
    add_exit(CALATHIN_DIR + "west_gate", "west", "@@check_west");
    reset_room();
}
...
check_west()
{
    ...
    {
        write("You pass through the western gates of Calathin.\n");
        return 0;
    }
