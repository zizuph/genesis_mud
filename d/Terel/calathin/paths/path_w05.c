/* path_w05.c   -    Vader (adapted from Metaprats forest files)  */
...
create_room() {
    ...
    set_long(
      "You are on a trail, in a large pine forest.  It leads to the " +
       "northeast and south.  South leads toward a temple, and east " +
       "leads back to the road.  You can clearly see the temple " +
       "in the trees to your south.\n");
...
+
    ...
    add_exit(CALATHIN_DIR + "paths/path_w04", "northeast", 0);
    add_exit(CALATHIN_DIR + "paths/path_w06", "south", 0);
    add_exit(CALATHIN_DIR + "houses/house5_yard", "north", 0);        
    add_exit(CALATHIN_DIR + "paths/path_w07", "west", 0);
}