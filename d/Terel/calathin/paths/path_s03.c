...
#define TREE   CALATHIN_DIR + "trees/tree_s03"

public void
create_terel_room()
{
    set_short("intersection of paths in the snow");
    set_long("Three paths come together here: one from the "+
      "south, between the townhouses and the weapon shop, "+
      "another from the northeast, near the towering "+
      "pine tree, and a third from the west, where a trail "+
      "follows along the treeline behind the townhouses. "+
      "The sky is a deep blue and the air is crisp and "+
      "clear, with just a hint of woodsmoke.\n");	

    add_exit(CALATHIN_DIR + "paths/path_s02", "northeast", 0);
    add_exit(CALATHIN_DIR + "paths/path_s04", "west", 0);
    add_exit(CALATHIN_DIR + "paths/path_s09", "south", 0);	
