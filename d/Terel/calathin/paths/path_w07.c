/* path_w07.c   -    Vader (adapted from Metaprats forest files)  */
...
create_room() {
    ...
    set_long(
      "You are on a trail, in a large pine forest.  It leads to the " +
       "east and west.  West leads to some houses and east " +
       "leads back to the road.  You can clearly see the temple " +
       "in the trees to your southeast.\n");
...
"path"}),
    ...
    add_exit(CALATHIN_DIR + "houses/house3", "west", 0);
    add_exit(CALATHIN_DIR + "paths/path_w05", "east", 0);
    add_exit(CALATHIN_DIR + "houses/house1", "north", 0);        
}
