...
#define IN_GATE CALATHIN_DIR + "in_north_gate"
...
create_room()
{
    ...
    set_long("This is the north gate of Calathin. Before you stands a tall wooden "+
                   "gate through the timber walls that surround the town. Above, "+
                   "you can see a few guards keeping watch over the entrance. The "+
                   "road leads back into the forest to the north. The town itself is "+
    ...
    add_exit(SILVER_DIR + "road/road02","north",0);
//    add_exit(SILVER_DIR + "forest2","north",0);
    add_exit(CALATHIN_DIR + "in_north_gate", "south", "@@check_south");
    add_exit(CALATHIN_DIR + "guard_hut", "in",0);
    reset_room();
