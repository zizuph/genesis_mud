...
do_copy(string str)
{
    ...
    if (song_file=parse_song("power",str,({}),({}), ({}),title_list))
    {
        if (song_file->query_read_skill() > TP()->query_skill(SG_READ_MUSIC))
        {
            notify_fail(BS("You pore over the music, but it is too complex; "
        ...
        {
            sscanf(song_file, POWERDIR+"%s", song_short);
            if (song_book->add_power(song_short))
            {
                notify_fail("Your song book is full; no more Songs of Power will fit!\n");
                return 0;
            }
...
{
    notify_fail(BS("Due to their great musical and magical power, only the "
    + "Guildmaster may compose a Song of Power.\n"));
    return 0;
}
