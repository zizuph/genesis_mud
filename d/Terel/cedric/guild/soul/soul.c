...
/*=========================================================================*/
    case "barely":
         ...
         if (sizeof(folks)==1)
         {
            ...
            tell_room(QCTNAME(TP()) + " makes a slight motion towards "
                     + QTNAME(folks[0])+ " which might be mistaken for "
                     + "a curtsey.\n",({TP(),folks[0]}));
...
{
   object  song_book, *obs;
   mixed   songfile, *argarray, affected, affected_obs;
   string  songstr, songtext, selftext;
...
/* Alignment check - I'm commenting this out for now, since we are
 * going to see if it works to have evil players be Minstrels too.
 *   - Gorboth (December 2007)

    if (this_player()->query_alignment() < -100)
    {
        write("The Muses do not answer the call from your evil heart.\n");
...
*/
...
#ifdef INDEX_PARSE
    if ((sscanf(songstr, "%s %s", type, which) != 2) ||
        ((type != "mundane") &&
         (type != "epic") &&
         (type != "ballad") &&
         (type != "power")))
    ...
    if (sscanf(which, "%d", num))
    {
        if (!strlen(type))
        {
            notify_fail("You must specify a song type with a song number.\n");
            return 0;
        }
    }
    else
    ...
    if (!type || (type == "power"))
    {
        if (num != -1)
        {
            string *power = songbook->query_power();
            if (power && (sizeof(power) > num))
            {
                songfile = power[num];
            }
            else if (type == "power")
            {
                write("You don't know that many songs of power.\n");
                return 1;
            }
        ...
        if (!strlen(songfile))
        {
            if (type == "power")
            {
                write("There is no such song of power, \"" + which + "\".\n");
                return 1;
            }
            // else it's an unspecified song type, and we keep going to check
            // for other song types which match.
        }
        else if (member_array(songfile + ".c", song_book->query_power() == -1))
        {
            if (type == "power")
            {
                write("You know no such song of power, \"" + which + "\".\n");
                return 1;
            }
            // else it's an unspecified song type, and we keep going to check
            // for other song types which match.
        ...
        {
            songfile = POWERDIR + songfile;
        }
    }

    if (!strlen(songfile) && (type != "power"))
    {
        mixed song_id;
        ...
        setuid();
        seteuid(getuid());
        songfile = SONG_INDEX->find_song(song_id, type);
        if (!songfile)
        {
            ...
            if (strlen(type))
            {
                songfile = SONG_INDEX->find_song(type + " " + song_id);
            }

            if (!songfile)
            {
                write("You know no such song, \"" + which + "\".\n");
                return 1;
            }
        }
#endif INDEX_PARSE
 ...
 */
   ...
   /* ---------------- Song of Power ----------------- */
   if (wildmatch(POWERDIR+"*", songfile))
   {
        int alarm_id;
       newtitle = "";
       size = sizeof(title_words = explode(explode(read_file(songfile, 1, 1),
...
#if 0
          write("affected_obs = ");
          write(sizeof(affected_obs));
          write("\n");
          write("affected = "+affected+"\n");
#endif

       if (TP()->query_mana() < songfile->query_mana_cost())
       {
           write("You are mentally drained; "+CAP_TITLE(songfile)
                 + "is beyond your powers right now.\n");
           return 1;
       }
       verb = "attempt";

       TP()->CM(BS(capitalize(selftext + "you "+adverb+" "+verb + " "
                              + newtitle + ".\n")));

       say(capitalize(songtext+QTNAME(TP()) + " "+adverb+" "+verb + "s a "
                      + "Song of Power, and the air stirs with the beauty and "
                      + "power of the music.\n"), TP());

       TP()->add_prop("_minstrel_i_singing",1);

        alarm_id = set_alarm(itof(songfile->query_casting_time()), 0.0,
        &start_power(songfile, evocal, einstrumental, affected_obs, this_player()));
        this_player()->add_prop("_minstrel_i_sop_alarm", alarm_id);
       return 1;
   }
...
void song_hurt(object player)
{
    if (random(10) == 1)
    {
        ...
        player->CM("Hmmmmm...something seems wrong with your "
