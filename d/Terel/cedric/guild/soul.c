...
#define MOURN_SONG MUNDANEDIR+"mourning.song"
	...
	tell_room(where_died, "From afar you hear a mournful dirge. The sound "
		  ...
		  + "a deafening cresendo, and suddenly in a blast of sound "
	...
	if (MASTER_OB(environment(this_interactive())) == where_died)
	    TI()->CM("Inspired by the muses, your voice swells to a power "
		     ...
		     + "music swells, and reaches a deafening cresendo, and "
...
do_perform(string str, string special_verb)
{
   ...
   NF("Usage: "+query_verb()+" <type> <song> OR "+query_verb()+" <song>\n"
      + BS("Where <type> is mundane, ballad, or power, and <song> is the "
	   + "title, the first word of the title, or the number of a song "
	   + "in your book.\n"));

/* Alignment check - added by HO
 * Actually ... lets comment this out, since we are now making it possible
 * for evil people to join. - Gorboth
 ...
 */
   ...
   if (parse_command(str,TP(),"%s 'for' / 'at' %s",songstr,affected))
   {
	 ...
	 songfile = parse_song(type,which,song_book->query_mundane(),
      ...
      if (!songfile)
          if (!(songfile=parse_song("power", songstr, ({}), ({}), ({}),
			       song_book->query_power())))
	 ...
	 NF("Usage: "+query_verb()+" <type> <song> OR "+query_verb()
	   + " <song>\n"+BS("Where <type> is "
	   + "mundane, ballad, or power, and <song> is the title, "
	 ...
	 return 0;
      ...
      if (!(affected_obs = parse_this(affected, "%i")))
      {
	 NF("Play "+songstr+" for what?\n");
	 return 0;
      }
   }
 ...
 */
   ...
   if (wildmatch(POWERDIR+"*", songfile))
   {
	  ...
	  write("affected = "+affected+"\n");
       ...
       set_alarm(itof(songfile->query_casting_time()), 0.0,
		 &start_power(songfile, evocal, einstrumental, affected_obs,
			      this_player()));
