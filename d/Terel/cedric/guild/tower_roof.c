...
do_arrive(object player)
{
	...
	if (folks[i])
		...
		if (folks[i]->query_hear_bells())
		{
		    message = "Somewhere in the distance you hear "
		    + "the Bells of the Tower heralding the arrival of a "
		    + "Minstrel";
		    if (folks[i]->query_met(player->query_real_name()) ||
			folks[i]->query_special_title() ||
			player->query_special_title())
			message += ", ringing out the Epic of "
			+ player->query_name() + ".\n";
		    else 
			message += ".\n";
		    folks[i]->catch_msg(BS(message));
		}
}
...
rm_died()
{
	...
	if (folks[i])
	    if (present(folks[i]))
		folks[i]->catch_msg(BS("Suddenly, the bells begin to ring out "
		+ "violently! You quickly realize that their cacophonious "
		+ "tones signal an alarm, a call to action! The bells then "
		+ "soften, and in the sad, sweet notes of mourning sound out "
		+ "Epic of the Rumour Monger.\n"));
	    else
		if (folks[i]->query_hear_bells())
		{
		    message = "Somewhere in the distance you hear "
		    + "the Bells of the Tower ring out the Song of "
		    + "Mourning, followed in melancholy tones by "
		    + "Epic of the Rumor Monger.\n";
...
mc_died()
{
    ...
    message = "Somewhere in the distance you hear "
    + "the Bells of the Tower ring out the Song of "
    + "Mourning, followed in melancholy tones by the "
    + "Epic of Kyrsis.\n";
...
void 
minstrel_banished(string schmuck)
{
    object *folks;
    string message;
    int i;
    ...
    message = "Somewhere in the distance you hear "
	+ "the Bells of the Tower ring out the Last Ballad, "
	+ "followed in angry tones by the "
	+ "Epic of " + capitalize(schmuck) + ".\n";
	...
	if (folks[i])
	    if (present(folks[i]))
                folks[i]->catch_msg(BS("Suddenly, the bells begin to ring out "
                + "violently! You quickly realize that their cacophonious "
                + "tones signal an alarm, a call to action! The bells sound "
                + "out the Last Ballad, followed in angry tones by "
                + "the Epic of " + capitalize(schmuck) + ".\n"));
            else
                if (folks[i]->query_hear_bells())
                {
                    folks[i]->catch_msg(BS(message));
...
create_room()
{
    ...
    + " gardens.  Across the river you can see the bustling town of Calithin, "
    ...
    add_item(({"courtyard", "yard", "dais"}), BS("You peer down into the oddly "
    + "shaped courtyard.  From up here the yard looks dish-shaped, with the "
    + "dais at the focal point.  It looks like the perfect place to perform, "
    + "as the acoustics would be fantastic.\n"));
    add_item(({"town", "calithin"}), BS("In the distance you see the town of "
    + "Calithin.  This small town lies just south of the Silver Forest.  Smoke "
    ...
    + "northwestern corner of the town of Calithin.  This lone sentinel "
    ...
    + "residents of Calithin in order to provide building materials and "
    ...
    add_exit(GUILD+"tower_power", "down");
    add_prop(ROOM_I_INSIDE, 1);
    set_alarm(0.0, 60.0*60.0,time_ring); /* the bells ring every hour */
}
