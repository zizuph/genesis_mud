...
create_object()
{
    ...
    set_long("This is a scroll that contains the submitted ballads for the "
	     ...
	     + "guild, and 'list [recent] members' will list members of our August "
	     + "Order.  'beg <muse> <name>' will restore <muse> "
             + "as the muse of <name>.  Use it sparingly.\n");
...
/* 
 * Function:    do_set_muse
 * Description: sets the muse of the player
 *
 * by Vader, to allow Cycle members to fix muses
 */
int do_set_muse(string str)
{
   object playerobj;
   string playername, musename;

   


   
   if (sscanf(str, "calliope %s", playername) == 1)
   { 
     
     playerobj = find_living(playername);
     if (!playerobj)
      {
       NF("There is no player by that name.  Did you type " 
          +"it in correctly?.\n");
       return 0;
      }
     playerobj->set_muse(2);
     TP()->CM("Done.\n");
     playerobj->CM("Calliope has agreed rejoin you.  The next time "
                   +"you log in, she will complete the transition.\n");
     return 1;
   }

   if (sscanf(str, "thalia %s", playername) == 1)
   {
     playerobj = find_living(playername);
     if (!playerobj)
      {
     ...
     playerobj->set_muse(1);
     TP()->CM("Done.\n");
     playerobj->CM("Thalia has agreed rejoin you.  The next time "
                   +"you log in, she will complete the transition.\n");     
     return 1;
   }

   if (sscanf(str, "melpomene %s", playername) == 1)
   {
     playerobj = find_living(playername);
     if (!playerobj)
      {
     ...
     playerobj->set_muse(3);
     TP()->CM("Done.\n");
     playerobj->CM("Melpomene has agreed rejoin you.  The next time "
                   +"you log in, she will complete the transition.\n");     
     return 1;
   }

   if (sscanf(str, "clio %s", playername) == 1)
   {
     playerobj = find_player(playername);
     if  (!playerobj)
      {
       NF("There is no player by that name.  Did you type " 
          +"it in correctly?.\n");
     ...
     playerobj->set_muse(4);
     TP()->CM("Done.\n");
     playerobj->CM("Clio has agreed rejoin you.  The next time "
                   +"you log in, she will complete the transition.\n");     
     return 1;
   }

   NF("There is no muse by that name.\n");
   return 0;  
}
...
do_demote(string str)
{
	...
	write("Three members of the Cycle have cast a vote to demote "
        ...
        members[which] = CYCLE_POSITION_EMPTY;
...
do_promote(string str)
{
    ...
    committee = read_file(COMMITTEE_FILE);
    members = explode(committee, "**");
    which = member_array(CYCLE_POSITION_EMPTY, members);   
    if (which == -1)
    {
