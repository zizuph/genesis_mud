...
do_read(string str)
{
    ...
    write("You read \"" + SONG_INDEX->query_title(song) +
...
public void compose3(string title)
{
    ...
    if (pointerp(mundanes_heard))
    {
        ...
        song = SONG_INDEX->find_song_in_list(mundanes_heard);
        if (pointerp(song))
        {
            int i;
            ...
            {
                if (SONG_INDEX->query_lc_title(song[i]) == lctitle)
                {
                    string song_short = song[i][strlen(MUNDANE_SONGS_DIR)..];
                    mundanes_heard -= ({ song_short });
                    this_player()->add_prop(MINSTREL_AS_MUNDANES_HEARD,
                                            mundanes_heard);
                    add_mundane(song_short);
...
do_compose(string str)
{
    ...
    if (pointerp(mundanes_heard))
    {
        ...
        if (sizeof(mundanes_heard) > 0)
        {
            ...
            write("You can scribe the following songs from memory:\n");
    ...
    if (reply == 1)
    {
        ...
        else
        {
            ...
            me->catch_tell(this_player()->query_The_name(me) +
                " accepts your offer, and you share your song with him.\n");
...
do_teach(string str)
{
    ...
    target[0]->catch_tell(this_player()->query_The_name(target[0]) + " asks if " +
     ...
     clone_object(GUILD + "obj/yes_no")->get_answer(target[0],
                                                    &reply_teach(this_player(),
                                                                 this_player()->query_the_name(target[0]),
                                                                 songfile),
                                                    20.0);
     return 1;
}
