/*
    ...
    This is the Dark One who menaced the ancient sidhe. Slaying him
        completes part 2 of the Duncan Thane quest. This creature
        is the object that sets the bit and delivers the reward.

    I'm not certain when Cedric coded this quest. Clearly some
        time before 1996, but I am not certain exactly when.

    Modified: 13 June 2003, by Bleys
        - Changed to use modern defs.
        - Now logs to the proper logfile.
...
create_monster()
{
    add_name(({"presence", "one", "thane_dark_one"}));
    set_adj("dark");
    ...
    if (interactive(killer))
    {
        if (killer->test_bit("Terel", QUEST_TH1_GROUP, QUEST_TH1_BIT))
            if (!killer->test_bit("Terel", QUEST_TH2_GROUP, QUEST_TH2_BIT))
            {
                ...
                write_file(QUEST_LOG, killer->query_name() + "solved part " +
                    "2 of the Duncan Thane quest (" + ctime(time()) + ")\n");
            }
        killer->catch_msg(BS("With a flash of holy brilliance, " +
            "LightBringer cleaves through the Dark One. A clap of thunder " +
            "throws you across the room, but even louder is the scream of " +
            "triumph that echos through your mind. Both sword and Darkness " +
            "vanish in a blast that seems to shake the very fabric of the " +
            "universe, and suddenly all is quiet. You feel a great burden " +
            "lifted off your shoulders, and emerge a wiser, more " +
            "experienced " + killer->query_race_name() + ".\n"));
