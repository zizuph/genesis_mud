...
{
    set_name("candle");
    add_name("mys_fatigue_candle");
    add_adj(({"healing", "deep", "green"}));
    set_short("deep green candle @@short_is_lit@@");
    set_long(BS("This short, squat candle is a deep, forest green. "
    + "A strip of metallic foil, bearing a strange seal resembling an intricate "
    + "silver star, is wrapped around the bottom of the taper. The wax has a "
    + "pungent, pleasant smell, unidentifiable but invigorating.\n"
    + "@@how_much@@\n@@is_lit@@\n"));
...
burn_0()
{
	...
	for (i=0;i<sizeof(room_list);i++)
		...
		if (room_list[i]->query_skill(MYS_SKILL))
		{
		    ...
		    tell_object(room_list[i], BS("The pleasant, invigorating smell of the candle fills your "
		    + "senses and refreshes your body.\n"));
		}
		else 
		{
		    tell_object(room_list[i], BS("The pleasant, invigorating smell of the candle fills your "
		    + "senses, but there is something unsatifying about it.\n"));
		}
	    }
	call_out("burn_1", 30);
    }
...
do_light(string str)
{
    ...
    lit_flag = 1;
    write(BS("You light the deep green candle, and the air soon fills with a "
    + "wonderful fragrance. Inhaling the sharp, invigorating aroma, you soon "
    + "forget your fatigue and your worries, and begin to feel better.\n"));
    say(BS(QCTNAME(TP())+" lights the deep green candle, and the air soon fills with a "
    + "wonderful fragrance. Inhaling the sharp, invigorating aroma, you soon "
    + "forget your fatigue and your worries, and begin to feel better.\n"));
    amount_burned++;
...
do_unlight(string str)
{
    ...
    lit_flag = 0;
    write(BS("Regretfully, you extinguish the candle, and the sharp odor soon "
    + "vanishes.\n"));
    say(BS(QCTNAME(TP())+" extinguishes the candle, and the sharp odor soon "
    + "vanishes.\n"));
