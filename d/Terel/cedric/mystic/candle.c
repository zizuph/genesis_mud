...
{
    set_name("candle");
    add_name("mys_heal_candle");
    add_adj(({"healing", "tall", "thin"}));
    set_short("tall thin candle@@short_is_lit@@");
    set_long(BS("This tall, thin candle is so white as to be almost translucent. "
    + "A strip of metallic foil, bearing a strange seal resembling an intricate "
    + "silver star, is wrapped around the bottom of the taper. The wax smells of "
    + "wild rosebuds, and deep green forests, and misty sea shores, and myriad "
    + "other pleasant scents whose exact nature eludes you.\n"
    + "@@how_much@@\n@@is_lit@@\n"));
...
burn_0()
{
	...
	for (i=0;i<sizeof(room_list);i++)
		...
		{
		    room_list[i]->heal_hp(10);
		    tell_object(room_list[i], BS("The pleasant smell of the candle fills your "
		    + "senses, and has, you suspect, a salutory effect on your health as well.\n"));
		}
		else 
		{
		    tell_object(room_list[i], BS("The pleasant smell of the candle fills your "
		    + "senses, but there is something unsatifying about it. It is as if "
		    + "a certain bliss is in reach, but not to be granted "
		    + " to you today.\n"));
		}
	    }
	call_out("burn_1", 30);
...
do_light(string str)
{
    ...
    lit_flag = 1;
    write(BS("You light the tall, thin candle, and the air soon fills with a "
    + "wonderful fragrance. Inhaling the heady aroma, you soon forget your wounds "
    + "and worries, and begin to feel better.\n"));
    say(BS(QCTNAME(TP())+" lights the tall, thin candle, and the air soon fills with a "
    + "wonderful fragrance. Inhaling the heady aroma, you soon forget your wounds "
    + "and worries, and begin to feel better.\n"));
    amount_burned++;
...
do_unlight(string str)
{
    ...
    lit_flag = 0;
    write(BS("Regretfully, you extinguish the candle, and the delightful odor soon "
    + "vanishes.\n"));
    say(BS(QCTNAME(TP())+" extinguishes the candle, and the delightful odor soon "
    + "vanishes.\n"));
