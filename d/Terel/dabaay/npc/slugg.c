/*
 *
 * Slugg the Master Thug of the Hand
 *
...
#define KILL_LOG "/d/Terel/dabaay/log/slugg_kills"
...
{
    set_name("slugg");
//    set_living_name("slugg");
    set_adj("slim");
    set_adj("olive-skinned");
    set_race_name("human");
    set_alignment((random(200) + 200) * -1);
//    set_title("the Master Thug of the Hand");
    set_gender(0);
    set_short("slim olive-skinned male human");
    set_scar(random(512));
    set_height_desc("tall");
    set_width_desc("lean");
    set_stats(({130 + random(30), 
                155 + random(30), 
                145 + random(30), 
                130 + random(30), 
                100 + random(30), 
    ...
    add_prop(OBJ_M_HAS_MONEY, random(500) + 1700);

    add_prop(LIVE_I_QUICKNESS, random(30) + 65);
...
{
    FIX_EUID;
//    clone_unique(DABAAY_DIR + "weapon/dshort",5,DABAAY_DIR + "weapon/short")->move(TO);
...
do_die(object killer)
{
    ...
    killer->catch_tell(
      TO->query_The_name(killer)+ " gasps to you: You are marked to die by the Hand.\n");
/*
    TO->command("say to "+ killer->query_name() + " You are "+
        "marked to die by the Hand.");
*/
