...
{
    ::create_room();
    set_short("iOn a dirt road in a dense forest");
    set_long(
        "You are on a tree-lined dirt road that continues to the north "+ 
        "and the south. A variety of trees and shrubs line the road on the "+
        "sides.\n");
    ...
    add_exit(DAB_ROAD + "s5", "north", 0);
    add_exit(DAB_ROAD + "s7", "south", 0);
}
