...
create_room()
{
    ...
    set_long(
        "You are on a tree-lined dirt road that curves from the north "+ 
        "to the southeast. A variety of trees and shrubs line the "+ 
        "road on the sides.\n");
    ...
    add_exit(DAB_ROAD + "s6", "north", 0);
    add_exit(DAB_ROAD + "s8", "southeast", 0);
}
