...
create_monster()
{
    ...
    set_long("" +
	"The Quest master is sitting on the soft rug juggling fireballs.\n" +
	"It seems he's very experienced, better not mess with him.\n");
    set_alignment(3000); 
    ...
    {
        set_act_time(4);
    	add_act(({"sigh", "say Not much action here nowdays"}));
    	add_act("stare");
    	add_act("giggle");
    	add_act("twiddle");
        trig_new("%s 'attacks' %s", "kill");
  	trig_new("%s 'smiles' %s", "my_smile");
  	trig_new("%s 'bows' %s", "my_bow");
  	trig_new("%s 'pokes' %w %s", "my_poke");
  	trig_new("%s 'kicks' %w %s", "my_kick");
    }
...
{
    set_alarmv(1.0, -1.0, "give_back", ({ob}));
    if (ob->move(from))
...
my_kick(string s1, string s2, string s3)
{
    ...
    {
    	set_alarmv(1.0, -1.0, "command" ({"glare " + s1}));
	set_alarmv(1.0, -1.0, "command", ({"say Whatch it!"}));
    }
    return 1;
}
...
{
    if (s2 != "you") return 1;
    set_alarmv(1.0, -1.0, "command", ({"say Ohh no! Another Tricky fan."}));
    return 1;
}
...
{
    set_alarmv(1.0, -1.0, "command", ({"nod"}));
    return 1;
}
...
{
  set_alarmv(1.0, -1.0, "command", ({"smile"}));
  return 1;
}
