...
object door;
  ...
  set_long(BS("You are inside a large hothouse. The air is warm " +
        ...
        "small bushes, and strange plants. "+
              ...
              "garden proper. \n"));
  add_exit(JANUSDIR + "garden/orangery", "northwest",0);
  add_exit(JANUSDIR + "garden/orangery", "north",0);
  add_exit(JANUSDIR + "garden/orangery", "south",0);
  add_exit(JANUSDIR + "garden/orangery", "southeast",0);
  ...
  door = clone_object(JANUSDIR  + "garden/orangery_door2");
...
do_pick(string tool)
{
  ...
  if (diploma ){
    ...
    if (tool == "hatchet" || tool == "small hatchet" || tool == "up hatchet"){
      ...
      clone_object(JANUSDIR + "garden/hatchet")->move(TP);
      TP-> add_prop("picked_tool",test + 1);
      say(QCNAME(TP)+" picks up something from the tool heap.\n");
      return 1;
    }
    ...
    if (tool == "gizmo" || tool == "up gizmo"){ 
      ...
      clone_object(JANUSDIR + "garden/gizmo")->move(TP);
      TP-> add_prop("picked_tool",test + 1);
      say(QCNAME(TP)+" picks up something from the tool heap.\n");
      return 1;
    }
...
do_search()
{
  ...
  if ( i2 > 0 ){
    seteuid(getuid(this_object()));
        if (!recipy ){
            recipy = clone_object(JANUSDIR + "garden/recipy");
             recipy -> move(TO);
