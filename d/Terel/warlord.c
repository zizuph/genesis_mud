...
#define BOOTS           ARM_DIR + "ew_boots"
#define BRIEFS          ARM_DIR + "ew_briefs"
#define CLOAK           ARM_DIR + "ew_cloak"
#define HELMET          ARM_DIR + "ew_helmet"
#define GLOVES          ARM_DIR + "ew_gloves"
#define MAIL            ARM_DIR + "ew_mail"
#define SWORD           WEP_DIR + "ew_sword"
...
equip_me()
{
    ...
    ob = clone_unique(MAIL, 5, ARM_DIR +"ew_mail_alt");
    ob->move(TO);
    ...
    sword = clone_unique("/d/Terel/LoD/weapon/ew_sword", 4,
                "/d/Terel/LoD/weapon/deaths_sword", 1, 90);
    sword->move(TO);
...
break_weapon(object ob)
{
    ...
    for (i=0; i<sizeof(all); i++) 
    {
        ...
        if (all[i]->query_wielded() == ob) 
        {
            ...
            {
                all[i]->set_corroded(all[i]->query_corroded() + 1);
                continue;
            }
            all[i]->remove_broken();
...
hurt_all_enemies()
{
    ...
    for (i=0; i<sizeof(all); i++) 
    {
        dam = (all[i]->query_alignment() - query_alignment())/20;
...
hurt_random_enemy()
{
    ...
    dam = (enemy->query_alignment() - query_alignment())/20;
    if (sword && environment(sword) != TO) dam *= 2;
