...
create_room()
{
   ...
   set_long(BS(
         ...
         "are draped with dust. However, the cobwebs have been burned away "
         + "and the dust cleared from the floor. Standing proudly in the " 
         + "clear is a suit of armour.\n"
));
   
   add_item(({"suit of armour", "armour", "suit"}), BS(
         "It is a full suit of platemail armour. It looks like a man " +
         "standing ready for battle. It is somewhat rusted.\n"
));
   add_item(({"floor"}), BS(
   ...
   add_exit(TOWNDIR + "mansion/wing_w3", "north", 0);
   add_exit(MANSION + "rooms/wrkroom.c", "west", 0);
   add_exit(TOWNDIR + "mansion/wing_w1", "south", 0);
}
