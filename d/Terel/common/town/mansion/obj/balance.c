 ...
 *
 * Modified: 08 May 2019, by Shanoga
 *      - check to see if weight arg 'str' is a string (prevent runtime error)
 *
 */
...
reset_object()
{
    say("The coins in front of the balance are shuffled.\n");
    start_balance();
}
...
do_solve(string str)
{
	...
	write(BSN("Congratulations, you didn't use more than 3 tries!! " +
	    ...
	    SIGPR[calc_hint(3)] + SIGEQ[calc_hint(3)] + ". " + quest_solved()));
    }
    else
    {
