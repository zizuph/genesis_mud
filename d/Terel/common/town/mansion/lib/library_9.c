...
create_room()
{
	...
	+ "bookcase to the east and a decaying set of bookshelves that line "
	+ "the southern wall of the room. To your west is a small alcove. "
	+ "The air here is musty and dry, thick "
	+ "with the smell of ancient parchment and rotting wood.\n"));
	
	add_exit(LIBRARY+"library_8","north");
	add_exit(LIBRARY+"library_10","west");
	...
	+ "on the spines in most cases, but most seem to begin with the letters 'M' and 'N'.\n"));

	add_item(({"bookcase"}),BS("This rickety old bookcase seems "
	+ "precariously overloaded with books, but somehow it still stands.\n"));
	add_item(({"bookshelves","shelf","shelves"}),BS("These rotting bookshelves line the southern wall from floor "
