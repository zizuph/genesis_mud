...
create_container()
{
  object sc;
   ...
   seteuid(getuid(this_object()));
   add_prop(CONT_I_WEIGHT, 20000);	/* weighs 200 kg */
   add_prop(CONT_I_MAX_WEIGHT, 100000);	/* holds a lot of weight */
   add_prop(CONT_I_VOLUME, 20000);	/* damn big desk! */
   add_prop(CONT_I_MAX_VOLUME, 100000);	/* holds plenty o' stuff */
   add_prop(OBJ_I_VALUE, 5000);		/* pretty valuable */
   add_prop(CONT_I_LOCK,1);		/* and locked... */
   add_prop(CONT_I_RIGID,1);		/* it's rigid */

   ob=present("index");   
  
   add_prop(CONT_I_CLOSED,1);		/* it's closed at first */
  
}

get_scroll(object sc){
  if(!scroll){
   seteuid(getuid());
   scroll=clone_object("/d/Terel/mecien/valley/guild/quest/scroll");
  scroll->move(environment(T));
  tell_room(environment(scroll), "A scroll falls from the desk.\n");
  }
}
