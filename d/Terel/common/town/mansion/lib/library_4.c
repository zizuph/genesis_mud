...
{
	set_short("Library");
	set_long(BS("You are in a small alcove formed by rickety bookcases to  "
	+ "the north and west, and by the southern wall of the library, "
	+ "which is lined with bookshelves. " 
	+ "The air here is dry and musty, thick "
	+ "with the smell of ancient parchment and rotting wood.\n"));
	
	add_exit(LIBRARY+"library_3","east");
...
+ "on the spines in most cases, but most seem to begin with the letters 'E'"
	+ "and 'F'.\n"));

	add_item(({"bookcase"}),BS("This rickety old bookcase seems "
	+ "precariously overloaded with books, but somehow it still stands.\n"));
	add_item(({"bookshelves","shelf","shelves"}),BS("These decaying bookshelves line the southern wall from floor "
