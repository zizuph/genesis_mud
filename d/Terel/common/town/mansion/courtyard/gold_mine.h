...
create_room()
{
    ...
    add_item("gold",BS("You try to grab some gold out of the vein, " +
...
{
    return(
	BS("Those veins in the wall look like gold veins. " +
	(resources ? 
	("You don't find many of them, but you imagine there's still some " +
	 "gold that could be gained here.\n")
...
check_boots()
{
    ...
    this_player()->reduce_hit_point(80);
    this_player()->do_die();
    return 0;
}
...
do_chisel(string str)
{
	...
	arr[0] != "vein" && arr[0] != "veins" && arr[0] != "gold") return 0;
    ...
    if (resources) {
	resources--;
	write("You chisel the wall, and succeed in gaining a gold nugget.\n");
	if (random(30) == 10) {
	...
	say(QCTNAME(this_player()) + " chisels the wall and gains a " +
	    "gold nugget.\n");
	nugget = clone_object(MANSION + "obj/gold_nugget");
	nugget->move(this_object());
	return 1;
    }
    write("You chisel the wall, but the gold resources seem to be " +
	"exhausted.\n");
    say(QCTNAME(this_player()) + " chisels the wall, but the gold " +
