...
set_short("graveyard outside the mansion");
   ...
   add_item(({"engraving"}), BS(
       "HERE LIES SIR WILFRED DOUGAN, MISSED BY HIS WONDERFUL WIFE AND " +
        "TWO DAUGHTERS FOR EVER. \n"
        ));

    add_prop(ROOM_I_INSIDE, 0);

 add_exit(MANSION + "graveyard/grv2.c", "north", 0);
 add_exit(MANSION + "graveyard/crypt.c", "south", 0);
 add_exit(MANSION + "graveyard/grvw1.c", "west", 0);
add_exit(MANSION + "graveyard/grve1.c", "east", 0);
}
