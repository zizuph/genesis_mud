...
*/
#define CLOCK   MANSION + "second/clock.c"
...
*/
   ...
   "This magnificent clock is made from pure oak. Despite a loud ticking\n" +
   "sound eminating from the clock, the hands appear to remain still.\n");

    add_item(({"hands", "face", "clock face", "clock hands"}),
   "The hands state the time as two minutes before twelve.\n");

    add_item("dust", "The dust is lingering in the air.\n");

    ob = clone_object(CLOCK);
    ob->move(this_object());
}
