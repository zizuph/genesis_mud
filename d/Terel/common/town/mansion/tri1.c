...
reset_room()
{
    ...
    {
	stone = clone_object(MANSION + "obj/moonstone");
...
create_room()
{
	...
	"Unlike all other rooms, this room has only three walls. " +
	"The strangest feature is that the three walls have different " +
	"colors... one is green, one is yellow and the third is orange. " +
	"The floor however is covered by a black carpet, " +
	"whereas the ceiling is " +
	"painted in white. The colors confuse you so much that you loose " +
	"your orientation..." +
   ...
   add_item(({"web", "webs", "cobwebs", "cobweb"}), BS(
    ...
    add_exit(MANSION + "tri6", "green", 0);
    add_exit(MANSION + "tri2", "yellow", 0);
    add_exit(MANSION + "tri4", "orange", 0);
    reset_room();
}
