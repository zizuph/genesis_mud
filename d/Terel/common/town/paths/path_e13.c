...
create_room()
{
    ...
    add_item(({"river", "water"}), BS("You don't see that much of the river " +
        "from your location.\n"));

    add_exit(TOWNDIR + "paths/path_e12", "northeast", 0);
    add_exit(TOWNDIR + "paths/path_e14", "southwest", 0);
    add_exit(TOWNDIR + "paths/track_s01", "east", 0);
}
