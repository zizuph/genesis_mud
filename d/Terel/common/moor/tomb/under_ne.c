...
{
	::create_room();	
    add_npc(MOBDIR +"dweller2", 1);
    add_npc(MOBDIR +"dweller1", 2);
	...
	"clay seeping in from the cracks in the bricks that remain. The air "+
    "is so thick you can feel it sliding against your skin.\n"
    );
	
    add_exit("under_tomb01", "southwest", test_exit);
    add_exit("under_ne_a", "north", test_exit);
    reset_room();
}
