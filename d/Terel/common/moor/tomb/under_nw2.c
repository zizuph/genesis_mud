...
create_room()
{
    ...
    add_npc(MOBDIR +"dweller3", 1);
    add_npc(MOBDIR +"dweller2", 2);
	...
	"clay seeping from the cracks in the bricks that remain. The conditions "+
    "are so bad you seriously worry about the walls collapsing in on you.\n"
    );
	
    add_exit("under_nw3", "north", test_exit);
    add_exit("under_nw_b", "south", test_exit);
    reset_room();
}
