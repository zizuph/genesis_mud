...
create_room()
{
	...
	"clay seeping from the cracks in the bricks that remain. The risk "+
    "of the walls collapsing on you seems to be quite high.\n"
    );

	
    add_exit("under_nw_a", "north", test_exit);    
    add_exit("under_tomb01", "southeast", test_exit);
    reset_room();
}
