 ...
 *   -tweaks to attacks, more fireballs and fewer drain_all_enemies
 *  updated by Zizuph Oct 2022
 *   -Use F_PENMOD instead of direct values for specials.
 *   -This will reduce his heals when the F_PENMOD is scaled down.
 */
...
equip_me()
{
    ...
    string choice = one_of_list(({ TANTO, WASP_SW, MEAT_HOOK,
        VAMBRACES, DRAGON_EYE }));
        
    if (choice == VAMBRACES || choice == DRAGON_EYE)
    {
  	    unarm = clone_unique(choice, 10, 0, 0, 100);
	    if (unarm)
	    {
    	    unarm->move(this_object());			
	        write_file(UNIQUE_LOG, "Benton cloned "+ unarm->short() +" " + 
	            ctime(time()) + "\n");
	        upath = MASTER_OB(unarm);
	    }
    }
    else
    {
        wep = clone_unique(choice, 12, 0, 0, 100);
        if (wep)
        {
...
drain_all_enemies()
{
    ...
    TO->heal_hp(F_PENMOD(80, 80) + (F_PENMOD(80, 80) * i));
...
heal_myself()
{
	...
	heal_hp(F_PENMOD(80, 80) + (random(4) * F_PENMOD(80, 80))); // can heal 1/10 to 1/3 of hp.
...
fireball(object enemy)
{
	...
	int damage = F_PENMOD(40,100) + random(F_PENMOD(40,100));
