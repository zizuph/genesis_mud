...
string *names = ({"Alphidius","Carbith","Enkadr","Himla","Kuris",
		  "Mazarul","Odifin","Quamli","Simje","Umedki","Wivaz",
		  "Yeffas","Jepqa","Gherghern","Wapporo"});
...
create_room()
{
    ...
    set_long(
	"You are standing on a path in the cottage graveyard.  A cold breeze " +
	"rustles the leaves of the trees above you. In the distance, great icy "+
	"mountains thrust toward the sky. You can see no signs of life. "+
	"Everything is bleak and blighted. There is a grave stone nearby.\n"
    );
    ...
    add_item(({"mountains", "mountain", "distance"}),
    "The Dragon Mountains are icy and inhospitable.\n");
...
public string grave_desc()
{
     if (TP->query_skill(SS_LANGUAGE) < 39) 
...
public int do_dig(string how)
{
     object sandal;
     object skeleton;
     
     if (!present("shovel", TP)) {
	  write("The ground is frozen hard. You will need a shovel if you want "+
      "to dig here!\n");
	  return 1;
     }
	 
     if (how != "here") {
	  write("Are you trying to 'dig here' ?\n");
	  return 1;
     }
     ...
     if (query_prop("_sandal_present")){
	  if (sandal_present) {
	       ...
	       sandal = clone_object(OBJDIR + "sandal");
	    ...
	    if (skeleton_present) {
		 ...
		 skeleton = clone_object(MOBDIR + "skeleton");
