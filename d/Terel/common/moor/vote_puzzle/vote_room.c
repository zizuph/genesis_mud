/*
	...
	Modified: Jan 2022 by Lilith
	 - updated exit path and changed tomb.h to local.h as part 
	   of moving the whole area out of Sorgum's dir.
*/
...
answer_func(string str)
{
	...
	if (ans[i] != sol[current][i])
	    ...
	    TP->reduce_hit_point(TP->query_hp() / 8);
	    TP->move_living("X", TOMBDIR + "dop_room");
	    riddler = clone_object(MOBDIR + "riddler");
