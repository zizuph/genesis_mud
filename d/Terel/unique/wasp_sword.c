// file name:    ~Terel/unique/wasp_sword.c
// creator(s):   Lilith, Jan 2022
// purpose:      A magical sword for Benton the Cannibal Corpse
//               It is basically a cleaver/zanbato + bonesaw  
...
//               reading its runes.
//             Lilith May 2022:  converted to a sword from a knife, updated
//                h/p, desc, magic id info, etc.
...
{
    set_name("wasp");
    add_name(({"sword", "cleaver", "blade", "bonesaw", "saw",
      "zenbato", "zanbato", "horse-killer", "horse-cleaver",
      "dragonbane", "_bentons_wasp"}));
    set_short("cleaver named Wasp");
    set_adj("benton's");
    add_adj(({"ancient", "mysterious", "serrated", "forked" }));
    set_long("This is Wasp, one of the weapons favored by Benton the "+ 
      "cannibal corpse. You're not sure why he calls it Wasp, "+
      "because it looks like someone made a sword-length "+
      "meat cleaver and merged it with a bonesaw.\n"+ 
      "   There are two small forked points on the recurved tip, "+ 
      "great for piercing skin and sinew. The thick spine of the "+
      "blade has been beveled and sharpened, while the opposite "+ 
      "edge has many large teeth, each of which is sharpened "+
      "in both directions. "+
      "The hilt looks like it was made from a long bone, with "+
      "grooves etched into it to improve the grip.\n"+
      "   This wide, thick sword is made of a mysterious blue "+
      "alloy inscribed with powerful runes. "+
      "It looks hefty enough to take down a horse with one "+
      "swing. "+     
	  "\n");   
    ...
    add_item(({"alloy", "blue alloy", "mysterious alloy"}),  
...
// Benton is an unpleasant bugger. He should have superb weapons.
    set_default_weapon(MY_HIT, MY_PEN, W_SWORD, 
      W_SLASH | W_IMPALE, W_RIGHT, this_object());
    set_wf(this_object());
    set_likely_dull(-10);
    set_likely_corr(-10);
    set_likely_break(-10);
    ...
    add_prop(OBJ_S_WIZINFO, "This is Benton's Wasp, cloned by "+
	...
	" If it is protection you want, wear the ancient ring.\n", 55
      }));
