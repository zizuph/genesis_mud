 ...
 * -Players were getting mixed-up messages from the did_hit.
 *  Changed TP to tp=query_worn()
 *  Lilith, Oct-Nov 2021
 * -Prevent remove_broken from breaking these.  This is the one 
 *  unarmed enhancer that cannot be broken.  Approved by Ckrik.
 *  Zizuph - Oct 2022
 */
                ...
                int phit, int dam, int hid)
{
    ...
    attacker_msg = "Drawing upon the power of the earth beneath your feet, "
        +"you lash out with one of your runed black boots ";
    target_msg = QCTNAME(tp) + " lashes out at you with one of " 
        + tp->query_possessive() + " runed black boots ";
    watcher_msg = QCTNAME(tp) + " lashes out at " + QTNAME(enemy)
        + " with one of " + tp->query_possessive() + " runed black "
        + "boots ";
