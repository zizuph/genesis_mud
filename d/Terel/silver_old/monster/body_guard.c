...
void dress_me()
{
    ...
    switch(random(2)) 
   {
        case 0 :   clone_unique(SILVER_DIR + "weapon/dwarf_axe",7,
                      SILVER_DIR + "weapon/battleaxe")->move(TO);
                   command("wield axe");  
                  break;
        case 1 :   clone_unique(SILVER_DIR + "weapon/mbattleaxe",7,
                      SILVER_DIR + "weapon/battleaxe")->move(TO);
                   command("wield axe"); 
                  break;
    }
    clone_object(SILVER_DIR+"obj/dwarven_whiskey")->move(TO);
    clone_object(SILVER_DIR+"obj/dwarven_whiskey")->move(TO);
    clone_object(SILVER_DIR+"obj/dwarven_whiskey")->move(TO);
    clone_object(SILVER_DIR+"armour/dwarf_plate")->move(TO);
    clone_object(SILVER_DIR+"armour/dwarf_shield")->move(TO);
    command("wear all");
}
