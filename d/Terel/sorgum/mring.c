...
update_spells(object ob)
{
   ...
   if (T_WIZ(ob)) {
      ...
      add_spell("illu",      "cast_spell", "mystic illusion");
      add_spell("salus",     "cast_spell", "heavenly sending");
      add_spell("detect",    "cast_spell", "mystic identifying");
...
/*
   if (member_array(MYSTIC_LAY_SHA+":", ob->query_autoshadow_list()) == -1)
   {
      shadow = clone_object(MYSTIC_LAY_SHA); 
      if (shadow->shadow_me(ob, LAY_TYPE, LAY_STYLE, LAY_NAME) != 1)
      {
