...
#define TORQUE_SUBLOC	"_sorque_subloc"
#define MANA_COST	80
...
show_subloc(string subloc, object carrier, object for_obj)
{
   if (carrier->query_prop(TEMP_SUBLOC_SHOW_ONLY_THINGS))
      return "";
   if (for_obj == carrier)
      return "You are wearing " + LANG_ADDART(short()) + " around your neck.\n";
   return capitalize(carrier->query_pronoun()) + " is wearing "+
   LANG_ADDART(short()) + " around " + carrier->query_possessive() + " neck.\n";
}
...
scry(string arg)
{
   ...
   if (environment(who)->query_no_snoop() ||
      environment(who)->query_prevent_snoop() ||
