...
{
    if (!dweller1) {
        dweller1=clone_object(TOMBDIR + "dweller1");
        dweller1->move(this_object());
    }

    if (!dweller2) {
        dweller2=clone_object(TOMBDIR + "dweller1");
...
create_room()
{
	...
	"clay seeping in from the cracks in the bricks that remain.\n"
    );

    add_prop(ROOM_I_INSIDE,1);
    add_exit(TOMBDIR + "under_tomb01", "southwest", -5);
    add_exit(TOMBDIR + "under_ne2", "north", -5);
    reset_room();
}
