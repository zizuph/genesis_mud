...
#define CORNERSTONE "/d/Terel/common/road/cornerstone"
#define GORE TOMBDIR + "gore_chamber"
...
{
return(BSN(
      wiz + " visited four different places in Terel to collect four " +
	  "different gem stones.  You must figure out the order in which " +
      wiz + "visited the four places, and which gemstone he found in each " +
      "place, given the following clues: " +
...
int answer_func(string str)
{
     ...
     if (++solved == 5) {
	  ...
	  if (TP->test_bit("Terel", QUEST_SS1_GROUP, QUEST_SS1_BIT)) {
	       tell_object("You have done this quest already!  You are no " +
			   "more experienced than you were before!\n");
	  }
	  else {
	          if (TP->query_exp() <= QUEST_SS1_EXP) {
		       TP->add_exp(TP->query_exp());
		  }
		  else {
		         add_exp(QUEST_SS1_EXP);
		    }
		  TP->set_bit(QUEST_SS1_GROUP, QUEST_SS1_BIT);
		  tell_object(TP, "You feel more experienced.\n");
		  tell_room(TO, QCTNAME(TP + "has become more experienced.\n"));
		  write_file(QUEST_LOG, capitolize(TP->query_name()) +
			     " solved the oracle quest (" ctime(time())+ ")\n");
	     }
	  move_living("X", CORNERSTONE);
	  return 1;
     }
     sol_done[current] = 1;
     current = get_next();
