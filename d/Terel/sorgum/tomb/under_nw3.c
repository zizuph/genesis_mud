...
{
    if (!dweller1) {
        dweller1=clone_object(TOMBDIR + "dweller3");
        dweller1->move(this_object());
    }

    if (!dweller2) {
        dweller2=clone_object(TOMBDIR + "dweller3");
...
create_room()
{
    ...
    add_exit(TOMBDIR + "under_tomb02", "northeast", -3);
    add_exit(TOMBDIR + "under_nw2", "south", -5);
    reset_room();
}
