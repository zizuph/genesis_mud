...
create_room()
{
    ...
    add_exit(KOB_LAIR_PATH + "kob_lair_a12", "west", "@@trigger_trap");
...
long_desc()
{
    string result = "The natural cave continues to the north and west " 
     + "with signs the stone has been tool cut down to a heavy wooden "
     + "door fitted in the middle of the east wall.\n";
    if (!trap_set)
    {
        result += "There are two large spiked logs hanging down in the "
               + "cave to the west.\n";
    }
    return result;
}
        ...
        if (present(player))
        {
            if (player->resolve_task(TASK_DIFFICULT, SS_ACROBAT) <= 0)
            {
                ...
                tell_room(environment(player), player->query_name() 
                  + " is crushed between two swinging spiked logs.\n",
                  player);
                player->catch_tell("You are crushed between two spiked "
                  + "logs.\n");
            }
            else
            {
                tell_room(environment(player), player->query_name() 
                  + " performs an acrobatic roll to avoid a swinging "
                  + "log, and dives over another log. The logs crush " 
                  + "together and embed their spikes in each other "
                  + "causing the logs to spike together.\n", player);
                player->catch_tell("You perform an acrobatic roll to "
                  + "avoid one swinging log and dive over the other. " 
                  + "The logs crush together and embed their spikes in "
                  + "each other causing the logs to stick together.\n");
            }
        }
        trap_set = 0;
        return 1;
        ...
        if (searcher->resolve_task(TASK_EASY, SS_FR_TRAP) > 0)
        {
            if (trap_set)
            {
                ...
                return "two spiked logs are suspended from the ceiling. " 
                  + "A pressure plate on the floor will trigger the logs "
                  + "to crash down crushing the person travelling west.\n";
            }
            else
            {
                return "There are two large spiked logs handing in front "
            ...
            if (this_player()->resolve_task(TASK_ROUTINE, SS_FR_TRAP) > 0)
            {
                if (trap_set)
                {
                    ...
                    this_player()->catch_tell("You touch the trap and "
                      + "cause logs to swing down. The logs crush "
                      + "together and embed their spikes in each other "
                      + "causing the logs to spike together.\n");
                    tell_room(environment(this_player()), 
                      this_player()->query_name() + "touches the trap " 
                      + "and cause logs to swing down. The logs crush "
                      + "together and embed their spikes in each other " 
                      + "causing the logs to spike together.\n", 
                      this_player());
                }
