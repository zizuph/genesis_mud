...
{
    bolt_desc = desc;
    set_spell_name("flurry");
    set_spell_desc("Forms a flurry of " + desc + " around a target.");
...
config_spell(object caster, object *targets, string argument)
{
    ...
    set_spell_target_verify(spell_target_present_enemies);
...
{
    string bolt = "The flurry of " + bolt_desc + " misses ";
...
desc_bolt_cast(object caster, object *targets)
{
    ...
    bolt = ", and " + ((sizeof(targets) > 1) ? "flurrys" : "a flurry") +
        " of " + bolt_desc + ((sizeof(targets) > 1) ? " shoot" : " appears") +
        " from %s palm.\n";

    caster->catch_tell("You extend your arm toward " +
        FO_COMPOSITE_ALL_LIVE(targets, caster) + sprintf(bolt, "your"));

    msg = " extends " + pos + " arm toward you, forming a flurry of " + 
        bolt_desc + " from " + pos + " palm.\n";
    targets->catch_msg(({"The " +
        caster->query_Met_name() +  msg,
        capitalize(caster->query_nonmet_name()) + msg,
        "" }));
                           
    caster->tell_watcher("The " + QCTNAME(caster) + " extends " + pos + 
