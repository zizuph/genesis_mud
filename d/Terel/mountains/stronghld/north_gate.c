/*
 * north_gate.c  Entrance to the dwarven stronghold which protects the pass 
...
#define IN_GATE MOUNTAIN_DIR + "stronghld/in_north_gate"
#define N_CHAINRM MOUNTAIN_DIR + "stronghld/walk19"
...
create_room()
{
    ...
    set_short("Northern gates of Stonehold");
    set_long("You stand before the great mithril gates " +   
       "of Stonehold. The gates stand nearly 30 meters tall, " +
       "blocking the northern pass from southern Terel. " +
       "On either side of the gates are giant statues embedded " +
       "into the solid rock walls.\n");
 
    add_item(({"gates","gate","mithril gate","mithril gates"}),
       "The massive gates are made of solid mithril bars reinforced " +
       "with steel and solid oak cross members. The gates stand " +
       "close to thirty feet high and about five feet thick.\n");
    ...
    add_cmd_item(({"gates","gate"}),({"scale","climb"}),
       "You dont seem to find a decent looking hand or foothold. " +
       "You cant climb the gates.\n");
 ...
 //   add_exit(MOUNTAIN_DIR + "path/path01","south",0);
      add_exit("/w/tomas/workroom","north",0);
    add_exit(MOUNTAIN_DIR + "stronghld/in_north_gate", "south", "@@check_south");
...
open_gate()
{
    ...
    tell_room(N_CHAINRM,"The huge windless begins to turn as the iron " +
...
check_south()
{
    ...
    write("You pass through the north gates of the dwarven stronghold.\n");
    return 0;
}
