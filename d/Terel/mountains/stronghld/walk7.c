...
public string exa_sgate();

object warrior1;
object warrior2;

#define DEFAULT  "/d/Terel/mountains/stronghld/"
#define GATE        ({ DEFAULT + "in_main_gate", \
                       DEFAULT + "main_gate", \
                                 })
...
create_room()
{
    ...
    set_long("The catwalk runs along atop the southern wall continuing " +
       ...
       "fields within the stronghold. To the south is a platform " +
    ...
    add_item(({"gates","gate"}),"@@exa_sgate");

    add_exit(MOUNTAIN_DIR + "stronghld/walk6","west",0,1);
    add_exit(MOUNTAIN_DIR + "stronghld/walk8","east",0,1);
