...
{
    ::create_room();
    set_short("Stronghold Blacksmith");
    set_long("You are in the stronghold smithy. The walls, ceiling " +
             "and floor are covered in soot. The forge is controlled " +
             "by giant bellows located in the middle of the room. The " +
             "temperature here is very hot. A large anvil rests before " +
             "the forge. You notice a little sign hanging " +
             "on the wall.\n");

    add_item(({"large anvil","anvil"}),
       "A large steel anvil rests on the floor next to the forge.\n");
    add_item(({"ceiling","floor","walls","wall"}),
       "The entire room is covered in a layer of soot.\n");
    ...
    add_item(("bellows"),"Giant leather bellows used to force air " +
        "into the forge.\n");
    ...
    add_exit(MOUNTAIN_DIR + "stronghld/fortb","north",0,1);
...
reset_room()
{
    if (!present("keeper")) 
    {
        keeper = clone_object(MOUNTAIN_DIR+"stronghld/npc/blacksmith");
                                keeper->move_living("south", TO);
    }
}
...
race_invis_enemy()
{
    ...
    if(TP->query_prop(OBJ_I_INVIS))
    {
        switch(chat)
        {
            ...
            case 1..2:
                keeper->command("scratch");
                keeper->command("say BAH! I have no time for ghosts!");
                break;
            case 3:
                keeper->command("say Someone say something?");
                keeper->command("peer");
