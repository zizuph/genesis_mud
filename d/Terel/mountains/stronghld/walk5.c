/* This comment tells emacs to use c++-mode -*- C++ -*- */

/*
 *
 * Dwarven Stronghold
 * Tomas  -- Jan. 2000
 */

#include "/d/Terel/include/Terel.h"
#include <ss_types.h>
#include <cmdparse.h>
#include <composite.h>
#include <stdproperties.h>
#include <filter_funs.h>

inherit STDROOM;
inherit MOUNTAIN_DIR + "stronghld/catwalk_s";
inherit MOUNTAIN_DIR + "stronghld/blowhorn";

object warrior1;
object warrior2;

reset_room()
{
    if (!warrior1) {
        warrior1=clone_object(MOUNTAIN_DIR + "stronghld/npc/std_dwf_warrior");
        warrior1->move(this_object());
    }

    if (!warrior2) {
        warrior2=clone_object(MOUNTAIN_DIR + "stronghld/npc/std_dwf_warrior");
        warrior2->move(this_object());
    }

    if ((warrior1) && (warrior2))
        warrior1->team_join(warrior2);
}




public void
create_room()
{
    ::create_room();
    INSIDE;
    set_short("Catwalk");
    set_long("The fortified watch tower was carved from the solid rock face. " +
       "From here you have a complete view of the valley below.\n");

    add_cmd_item(({"horn","warhorn"}),"blow","@@blowhorn");
    add_exit(MOUNTAIN_DIR + "stronghld/walk4","east",0,1);

    reset_room();

}


