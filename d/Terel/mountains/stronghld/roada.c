...
create_room()
{
    ...
    set_long("The pass appears to have been carved out of solid rock " +
       "by dwarven miners. The area is kept clear of snow " +
       "and debris providing an unobstructed view from the catwalks " +
       "which run the length of the pass atop of the sheer cliff walls. " +
       "A cobblestone road leads north to the northern gate. The southern " +
       "gate is behind you.\n");
    ...
    add_exit(MOUNTAIN_DIR + "stronghld/roadb","north",0,1);
    add_exit(MOUNTAIN_DIR + "stronghld/in_main_gate","south",0,1);
    add_exit(MOUNTAIN_DIR + "stronghld/fielda","west",0,1);
    add_exit(MOUNTAIN_DIR + "stronghld/fieldb","east",0,1);
    add_exit(MOUNTAIN_DIR + "stronghld/fieldc","northwest",0,1);
    add_exit(MOUNTAIN_DIR + "stronghld/fieldd","northeast",0,1);
