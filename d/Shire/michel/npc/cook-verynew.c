...
void thank_chat (object oWhat, object oFrom)
{
  ...
  if (has_name (oWhat, "_shire_pieplate_"))
  {
    ...
    set_alarm (5.0,0.0, &command ("say I need you to fetch me some items then..."));
    iGotPlate = 1;
  ...
  if (bitm[iIndex] == DONE)
  {
    set_alarm (10.0,0.0, &command("think"));
    set_alarm (12.0,0.0, &command("say Hmmm, seems you've already given me everything I need..."));
    set_alarm (14.0,0.0, &command("say I guess I will make that pie for you now."));
    set_alarm (20.0,0.0, &cookit (oFrom));
  }
  else
  {
    string sHave = "";
    ...
    if (bitm[iIndex] & BOWL)    sHave += "a mixing bowl, ";
                           else sNeed += "a mixing bowl, ";
    if (bitm[iIndex] & MUSTARD) sHave += "a pack of mustard seeds, ";
                           else sNeed += "a pack of mustard seeds, ";
    if (bitm[iIndex] & HONEY)   sHave += "a bottle of honey, ";
                           else sNeed += "a bottle of honey, ";
    if (bitm[iIndex] & SPOON)   sHave += "a spoon, ";
                           else sNeed += "a spoon, ";
    if (bitm[iIndex] & PAN)     sHave += "a cooking sheet, ";
                           else sNeed += "a cooking sheet, ";
    if (bitm[iIndex] & SAUSAGE) sHave += "a shire sausage, ";
                           else sNeed += "a shire sausage, ";
    if (bitm[iIndex] & CLOTH)   sHave += "a cloth, ";
                           else sNeed += "a cloth, ";
    if (bitm[iIndex] & CLOVE)   sHave += "a clove, ";
                           else sNeed += "a clove, ";
    if (bitm[iIndex] & SAVORY)  sHave += "a savory, ";
                           else sNeed += "a savory, ";
    if (bitm[iIndex] & THYME)   sHave += "a thyme, ";
                           else sNeed += "a thyme, ";
    if (bitm[iIndex] & MINT)    sHave += "a mint, ";
                           else sNeed += "a mint, ";
    if (bitm[iIndex] & CHERVIL) sHave += "a chervil, ";
                           else sNeed += "a chervil, ";

    set_alarm (10.0,0.0, &command("sigh"));
    if (sHave == "") set_alarm (5.0,0.0, &command ("say We have recently moved and all of my things are lost."));
                else set_alarm (5.0,0.0, &command ("say You have given me " + sHave [.. strlen(sHave) - 2] + " according to my list at least."));
    if (sNeed == "") set_alarm (7.0,0.0, &command ("say Strange, I do not need anything from you."));
                else set_alarm (7.0,0.0, &command ("say Hmm, You need to get me " + sNeed [.. strlen(sNeed) - 2] + " then I can bake the pie."));
  };
