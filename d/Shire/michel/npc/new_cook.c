...
thank_chat(object *foo)
{
	...
	set_alarm(15.0,0.0, &command("say a spoon, a cooking sheet, a shire sausage...a cloth..."));
	...
	*/
	...
	command("say You can't ask me to cook something for you without a cooking sheet!");
    ...
    if(h_nam("_shire_honey_"))
    {
        ...
        say("Thanks for the honey. I'll need that for the pie.\n");
    }

    if(h_nam("_shire_mustard_"))
    {
	    bitm[index] = bitm[index] | MUSTARD;
        say("This mustard kicks it up a notch. Thanks!\n");
    }

    if(h_nam("_shire_mixing_bowl_"))
    {
	    bitm[index] = bitm[index] | BOWL;
        say("Oh good, you got me the bowl. Now I don't have to wash one.\n");
    }

    if(h_nam("_shire_mixing_spoon_"))
    {
	    bitm[index] = bitm[index] | SPOON;
        say("Oh, this spoon is much better than mine. Thank you.");
    }

    if(h_nam("_shire_sausage_"))
    {
    	bitm[index] = bitm[index] | SAUSAGE;
        say("The sausage is my secret ingredient. Thanks for getting it.\n");
    }

    if(h_nam("_shire_baking_pan_"))
    {
    	bitm[index] = bitm[index] | PAN;
        say("Thanks! This is a much better pan than any I have.\n");
    }

    if(h_nam("_shire_cloth_"))
    {
    	bitm[index] = bitm[index] | CLOTH;
        say("Thanks for the nice cloth. This will come in handy cooling the pie\n");
    }

    if(what->query_herb_name() == "clove")
    {
	    bitm[index] = bitm[index] | CLOVE;
        say("I just love clove. Thanks.\n");
    }
    
    if(what->query_herb_name() =="savory")
	{
        bitm[index] = bitm[index] | SAVORY;
        say("Oh, sweet savory. Thanks!\n");
    }

    if(what->query_herb_name() == "thyme")
    {
	    bitm[index] = bitm[index] | THYME;
        command("emote sniffs the thyme.");
        say("This smells fresh. It's about thyme!\n");
        command("emote laughs hysterically.");
    }

    if(what->query_herb_name() == "mint")
    {
	    bitm[index] = bitm[index] | MINT;
        say("Mint is very fragrant. Thanks for getting it.\n");
    }

    if(what->query_herb_name() == "chervil")
    {
	    bitm[index] = bitm[index] | CHERVIL;
        say("I have been looking for this. Thank you for finding it.\n");
    }
...
{
    set_alarm(10.0,0.0, "emote puts all the ingredients into the mixing bowl.\n");
    set_alarm(10.0,0.0, "emote measures flour and water and pours it into the bowl.\n");
    set_alarm(6.0,0.0, "emote cracks some eggs into the batter.\n");
    set_alarm(10.0,0.0, "emote stirs the batter thouroughly.\n");
    set_alarm(7.0,0.0, "emote greased the pan and pours the batter into it.\n");
    set_alarm(15.0,0.0, "emote opens up the oven and puts the pan in it.\n");
