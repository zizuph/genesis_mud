...
{
    return HOBBIT_RACE_GUILD_TRAINER;
}
...
query_guild_keep_player(object player)
{
    ...
    if (player->query_alignment() < HOBBIT_RACE_GUILD_ALIGN)
    {
...
show_subloc(string subloc, object on_obj, object for_obj)
{
	...
	(capitalize(on_obj->query_pronoun()) + " has")) +
      " extremely furry feet.\n");
}


/*
 * Function name: get_hobbit_soul
 * Description  : Gets the hobbit soul and guild item for the player.
 * Arguments    : int news - a flag used to tell if there is new news 
 *                relating to the guild
 */
void
get_hobbit_soul(int news)
{
    shadow_who->catch_tell("Welcome, Adventuresome Hobbit!\n");
    if (news)
	shadow_who->catch_tell("There is some news about the "+
          HOBBIT_RACE_GUILD_NAME+" within the help files.  Please read "+
          "help hobbit news for more information.\n");
    setuid(); seteuid(getuid());
    shadow_who->add_cmdsoul(HOBBIT_RACE_GUILD_SOUL);
    shadow_who->update_hooks();
    clone_object(HOBBIT_RACE_GUILD_PIPE)->move(shadow_who, 1);
}
...
init_race_shadow(string foobar)
{
    ...
    shadow_who->add_subloc(HOBBIT_RACE_GUILD_NAME, this_object());
    family_name = call_other(JOINROOM, "query_family_name", 
       shadow_who->query_real_name());

    if (!strlen(family_name))
    {
	write("\nYou have no family name. Please report this to one of the " +
	  "guildmasters of the " + HOBBIT_RACE_GUILD_NAME + ".\n\n");
    }
    if (shadow_who->query_alignment() < 0)
    {
	write("\nHobbits are a merry race and they are light in their " +
	  "hearts. An evil alignment is not something we condone in the " +
	  HOBBIT_RACE_GUILD_NAME + ". You should change that attitude " +
    ...
    if (shadow_who->query_login_time() <= file_time(HOBBIT_NEWS))
    {
	news = 1;
    }
    set_alarm(1.0,0.0,&get_hobbit_soul(news));
}
...
set_family_name(string name)
{
    ...
    {
	write("Bad family name, please report this to the guildmaster!\n");
	return 0;
    }
    else
    { 
