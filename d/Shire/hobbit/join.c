...
create_room()
{
   ...
   add_item(({"first painting"}),
      "The first painting depicts a happy, but obviously tired hobbit "+
   ...
   add_item( ({ "hole" }),
      "It seems very small, about the size of a pipe.\n");
   add_item( ({ "sign" }),
   ...
   add_cmd_item( ({ "sign" }), ({ "read" }),
      ...
      "\"request pipe\" to have their pipes back after they lost them.\n" +
   ...
   add_exit(HOBGLD_DIR + "entrance","north");
   add_exit(HOBGLD_DIR + "hall", "south","@@check_member@@");
   ...
   restore_object(HOBBIT_RACE_GUILD_SAVE);
   if (!mappingp(members))
      {
      members = ([ ]);
   }
...
do_try(string str)
{
   ...
   if (!strlen(str))
      ...
      notify_fail(capitalize(query_verb()) + " what? Smoking? To join " +
            "the guild, you must \"start smoking with <family>\".\n");
      return 0;
   }
   ...
   if (tp->query_race() != "hobbit")
      {
      write("You are not a hobbit. Please leave the guild.\n");
      return 1;
   }
   if(tp->query_alignment() <= 0)
      {
      write("You do not follow the path of goodness which all "+
         "Adventuresome hobbits are required to do.\n"+
   ...
   if (IS_MEMBER(tp))
      ...
      write("You are already a member of the guild. If you have lost " +
         "your pipe, you may get a new one using <request pipe>. In " +
         "case you want to change families, you shall have to leave the " +
         "guild and re-join selecting another family.\n");
      return 1;
   }
   
   if (tp->query_guild_member(HOBBIT_RACE_GUILD_TYPE))
      {
   ...
   if (member_array(family, titles) == -1)
      {
      write("The family \"" + family + "\" does not exist in our guild. " +
         "Use the command \"families\" to list all possible family " +
         "names.\n");
      return 1;
   }
   
   shadow = clone_object(HOBBIT_RACE_GUILD_SHADOW);
   
   if ((result = shadow->shadow_me(tp, HOBBIT_RACE_GUILD_TYPE,
               HOBBIT_RACE_GUILD_STYLE, HOBBIT_RACE_GUILD_NAME)) != 1)
   {
   ...
   write("You have become a habitual pipe smoker.\n");
   ...
   clone_object(HOBBIT_RACE_GUILD_PIPE)->move(tp, 1);
   //    write("Getting pipe...\n");
   write("You hear a soft rattling sound coming from the hole in front " +
      "of you. Eventually, a pipe comes sliding from inside the hole, " +
      "into your hands. You pick it up, light it, and take a long drag.\n");
   say(QCTNAME(tp) + " takes a pipe from the hole in the wall and begins " +
      "to smoke, immediately.\n");
   
   tp->clear_guild_stat(SS_RACE);
   tp->set_guild_pref(SS_RACE, HOBBIT_RACE_GUILD_TAX);
   tp->add_cmdsoul(HOBBIT_RACE_GUILD_SOUL);
   tp->update_hooks();
   //    write("Adding cmdsoul...\n");
   
   members[tp->query_real_name()] = ({ family, tp->query_age(), time() });
   
   save_object(HOBBIT_RACE_GUILD_SAVE);
...
lower_skill(object tp)
{
   ...
   tp->clear_guild_stat(SS_RACE);
   tp->remove_cmdsoul(HOBBIT_RACE_GUILD_SOUL);
   tp->update_hooks();
...
remove_player_changed_race(object tp)
{
   tp->catch_msg("\nYou are no longer a hobbit. Therefore you do not " +
      "belong in the " + HOBBIT_RACE_GUILD_NAME + " any more. You " +
...
{
   tp->catch_msg("\nEvil is not something a hobbit should be. Therefore " +
      "you do not belong in the " + HOBBIT_RACE_GUILD_NAME + " any more. " +
      "You are being expelled from the guild. Your pipe is removed.\n");
...
families(string str)
{
   ...
   if (!strlen(str))
      {
      write("The command \"families\" can be used to find information " +
         "on the members of this guild. You can \"families list\" to " +
         "list all members of the guild. \"families size\" will list the " +
         "number of members per family. The command \"families " +
         "<family>\" prints a pedigree of that family.\n" +
         "In the guild, you will find the following families:\n" +
   ...
   if (!m_sizeof(members))
      {
      notify_fail("No members of the " + HOBBIT_RACE_GUILD_NAME +
            " registered.\n");
      return 0;
   }
   ...
   if (str == "size")
      ...
      write("The following table shows the number of members per family:");
      write(sprintf("\n\n%-*#s\n", scrw, implode(names, "\n")));
      return 1;
   }
   ...
   if (member_array(str, titles) == -1)
      {
      write("No such family, \"" + capitalize(str) + "\". Do " +
         "\"families\" without arguments for the syntax.\n");
      return 1;
   }
   ...
   if (!sizeof(names))
      {
      write("No members of the " + HOBBIT_RACE_GUILD_NAME +
         " in the family " + str + ".\n");
      return 1;
   }
   ...
   write("The following members are a member of the family " +
      str + ":\n");
   write(sprintf("\n%-*#s\n", scrw, implode(names, "\n")));
