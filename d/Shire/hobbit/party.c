...
buy_party(string str)
{
    ...
    gHobbit->command("say So you want to purchase a party then? Well a "+
      "simple party costs you "+PARTY_PRICE+" platinum.  Since you "+
      "are the one paying for this party you automatically become the "+
      "host of the party.");
    gHobbit->command("say Now do you wish to add another person "+
      "or persons as co-hosts?  Type 'host <name1> <name2> .. <nameN>' "+
      "to add these players as hosts of this party, or 'host none' "+
      "if you are to be the only host of this party."); 
    return 1;
}
...
add_hosts(string str)
{
	...
	{
	    gHobbit->command("say I'm sorry but I couldn't find a single "+
	      "player name within "+str+".");
    ...
    gStage += 1;
    gHobbit->command("say Okay now is this party for anyone in particular?");
    gHobbit->command("say Someone's birthday or marriage?");
    gHobbit->command("say Type 'for <name1> <name2> .. <nameN>' "+
      "to add these people as honoured guests to this party, or "+
      "type 'for none' if this party isn't for anyone.");
    return 1;
}
...
set_for_who(string str)
{
    ...
    if (!str || !strlen(str))
    {
        NF("Add whom as a guest of honour?\n");
        return 0;
    }
    if (str != "none")
    {
    ...
    gStage += 1;
    gHobbit->command("say Ok please state the reason for this party by "+
      "typing 'reason <text>', keeping in mind invitations will be "+
      "printed in this following format.");
    gHobbit->command("say You have been invited to "+
...
set_reason(string str)
{
    ...
    gStage += 1;
    gHobbit->command("say Okay who are you inviting to this grand party "+
      "of yours?");
    gHobbit->command("say Type 'guests <name1> <name2> .. <nameN>' to add "+
      "these people as guests to this party.");
    gHobbit->command("say You may invite 'all' or you may invite "+
      "whole guilds if you wish.");
    gHobbit->command("say For a listing of guilds type 'list guilds'.");
    return 1;
}
...
set_guests(string str)
{
	...
	{
	    gHobbit->command("say I'm sorry but I couldn't find a single "+
	      "player name or guild name within "+str+".");
	    gHobbit->command("say Please try again.");
	    return 1;
	}
    }
    ...
    gStage += 1;
    gHobbit->command("say Now do you wish to pick up the tab for all the "+
      "food and drink at the party for a lump sum of "+FREE_FOOD+" platinum?");
    gHobbit->command("say Type 'food yes' if you want to pick up the tab "+
      "or 'food no' if you don't want to.");
    return 1;
}
...
set_food(string str)
{
    ...
    gStage += 1;
    gHobbit->command("say For a small fee of "+COSTUMES+" platinum, you "+
      "may have costumes at the party, useful if you want to have a costume "+
      "ball or to celebrate Halloween or some other festive event.");
    gHobbit->command("say Type 'costumes yes' if you want to pay this fee "+
      "and have the costumes or 'costumes no' if you don't want them.");
    return 1;
}
...
set_costumes(string str)
{
    ...
    gStage += 1;
    gHobbit->command("say There, we have everything set up and ready to go.");
    gHobbit->command("say This party will set you back "+gPrice+
      " platinum coins.");
    gHobbit->command("say If you find this acceptable type 'accept party' "+
      "if you don't wish to order yourself one then type 'cancel party'.");
    return 1;
}
...
pay_for_party(string str)
{
    ...
    if (gStage != 7)
    {
	NF("But you having finished setting up the party yet!\n");
	return 0;
    }
    if (str != "party")
    {
    ...
    {
	gHobbit->command("say Hmmm for some reason the party isn't set up "+
    ...
    {
	gHobbit->command("say Since you are a wizard the party will not "+
	  "cost you a thing but you best have a good reason for throwing it.");
    }
    else
    {
	...
	{
	    gHobbit->command("say But you don't have the nessacery funds "+
	      "to pay for the party!\n");
	    return 1;
	}
	coin_pay_text = text(exclude_array(money_arr, NUM, NUM*2-1));
	coin_get_text = text(exclude_array(money_arr, 0, NUM-1));
	write("You pay "+coin_pay_text+" for your party, which starts "+
	  "imediately.\n");     
...
cancel_party(string str)
{
    ...
    gHobbit->command("say Okay cancelling your order for a party.");
    reset_buying(1);
    return 1;
}
...
ring_bell(string str)
{
	...
	gHobbit->command("say You interested in ordering a party?");
    }
    else
    {
