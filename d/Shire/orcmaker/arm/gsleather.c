...
create_armour()
{
  ...
  set_short(SSHORT);
  set_pshort(SPSHORT);
  set_long("This "+short()+" has many steel studs set closely "+
    "together on a backing of good, durable leather.  The short "+
    "sleeves provide no protection to the arms.\n");
  set_default_armour(SAC,SAT,SAM,0);
  add_prop(OBJ_I_WEIGHT,F_WEIGHT_DEFAULT_ARMOUR(SAC,SAT) - random(31));
