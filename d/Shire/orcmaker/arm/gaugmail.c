...
#define SADJ "steel"
#define SSHORT "augmented steel hauberk"
#define SPSHORT "augmented steel hauberks"
...
create_armour()
{
  ...
  add_name(({"mail","coat"}));
  set_adj(SADJ);
  add_adj(({"augmented","steel","orcish"}));
  set_short(SSHORT);
  set_pshort(SPSHORT);
  set_long("This "+short()+" is of obvious orcish craftmanship, however "+
    "it is extremely well crafted and strong. Small strips of "+
    "steel have been woven into the chain links for added strength.\n");
