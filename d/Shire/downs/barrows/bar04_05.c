...
create_barrow_room()
{
    ...
    set_long("You stand in a large, stone cavern. A feeling of " +
        "death permeates the room. Rotting corpses lay in a " +
        "corner of the cavern, unfortunate travellers lulled " +
        "here by a wight. There is a handle is attached to the wall.\n");
    ...
    add_item(({"corpses", "rotting corpses", "skeletons"}),
        ...
        "barrows who knows how long ago. Little remains of them " +
    ...
    add_item(({"handle", "lever"}),"The handle is smooth yet " +
        "shows signs of rust. It obviously has not been used " +
        "for a long time.\n");

    add_prop(ROOM_I_LIGHT, 0);    
    reset_shire_room();

    add_exit(BAR_DIR + "bar04_04", "north","@@check_exit@@");
    add_exit(BAR_DIR + "bar04_out", "down","@@check_handle@@");
