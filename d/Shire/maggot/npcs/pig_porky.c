 ...
 *                     - Removed quickness and see invis (it's a pig!)
*/
//inherit "/std/monster";
...
void create_creature()
{
	...
	set_long("This pig is extremely porky! If at all possible, it seems to be wider than it is long.\n");
	...
	//set_attack_unarmed(A_BITE, 50, 60, W_IMPALE, 60, "bite");
	set_attack_unarmed(A_BITE, 50, 60, W_IMPALE, 60, "large teeth");
	...
	combat_reload();
	set_hitloc_unarmed(H_BODY, 30, 20, "body");
	set_hitloc_unarmed(H_BACK, 30, 20, "back");
	set_hitloc_unarmed(H_HEAD, 20, 18, "head");
	set_hitloc_unarmed(H_LFLEG, 15, 13, "left front leg");
	set_hitloc_unarmed(H_RFLEG, 15, 13, "right front leg");
	set_hitloc_unarmed(H_LHLEG, 15, 7, "left hind leg");
	set_hitloc_unarmed(H_RHLEG, 15, 7, "right hind leg");
	set_hitloc_unarmed(H_TAIL, 5, 2, "tail");
	
	set_act_time(5);
	add_act("emote roots around in the hay with its nose, looking for food.");
	add_act("emote snorts rather loudly.");
	add_act("emote rolls around on the floor.");
	add_act("emote stutters for a moment, then gives a very loud snort.");
}
