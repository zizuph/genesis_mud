...
create_area_room()
{
	...
	"one-story house and a red barn for animal stalls and " +
	"equipment storage. It must be a farmstead. Sounds of activity " +
	"can be heard nearby.\n");
	add_item(({"house"}), "The house is only a single story and " +
	"very short, reminiscent of hobbit holes one might see in other " +
	"areas of the Shire. From here it looks very clean and orderly.\n");
	add_item(({"barn"}), "It is a faded-red, wooden structure, " +
	"probably meant for animal and equipment storage.\n");
	add_item(({"ground"}), "Here on the lane, the ground is hard " +
	"packed dirt and pebbles, with wild grass growing in spots.\n");
	add_item(({"wild grass", "grass"}), "It's just common grass growing " +
	"on various spots on the lane.\n");
	add_item(({"dike", "causeway", "the causeway"}), "It is a long " +
	"dike and can barely be seen in distance.\n");
	...
	"the sun.");
	add_room_tell("Sounds of activity can be heard coming from the " +
	"farmstead.");
	
	add_exit(ROOM_DIR + "maggotlane01", "east");
	add_exit(ROOM_DIR + "maggotlane02b", "west");
