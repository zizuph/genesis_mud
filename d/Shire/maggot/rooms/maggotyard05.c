...
create_area_room()
{
	...
	"the fringes of the yard. An old barn stands east of here. There is " +
	"a window set into the wall of the house here.";
	...
	add_exit(ROOM_DIR + "maggotyard06", "north");
	add_exit(ROOM_DIR + "maggotyard04", "south");
...
int pull_weeds(string str)
{
	if(str == "garden" || str == "flower garden")
	{
		...
		{
			if(!TP->query_prop(WEED_THREE))
			{
				TP->add_prop(WEED_THREE, 1);
