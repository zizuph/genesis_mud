/*
     ...
     also the room where bores (/d/Shire/guild/smiths/npc/bores.c) is placed
*/
...
create_room() {
    ...
    add_exit(SMITHSDIR_ROOMS+"jewelsmith","south",0);

    if (!objectp("guild master")) room_add_object
	("/d/Shire/guild/smiths/npc/bores", 1, QCTNAME(TO)+" arrives.\n");
    if (!objectp("brown book")) room_add_object
	("/d/Shire/guild/smiths/obj/join_book", 1);
...
{
    seteuid(getuid());
    if ("/secure/master"->query_wiz_dom(TP->query_real_name()) == "Gondor")
        return 0;
    if (!IS_MEMBER(TP)) 
    {
...
leave(string str) {
    ...
    craft_guild=TP->query_guild_name_craft();
    ...
    if (craft_guild==GUILD_NAME) {
        if ((TP->remove_guild_craft()) != 1)
            write("Something is really wrong here.... You cannot leave.\n");
        TP->remove_cmdsoul(SMITHSDIR+"smiths_craft_soul");


        remove_smith_guild_member(TP);

        // TP->set_guild_pref(SS_CRAFT, 0);
