...
create_room() {
   ...
   add_exit(SMITHSDIR_ROOMS+"entrance","northeast",0);
    ...
    if (!objectp( board) )
    {
        ...
        board->set_board_name( "/d/Shire/guild/smiths/board2/");
        board->set_silent(0);
...
block_room()
{
    ...
    write("The forges have been shut down. A terrible fire swept through " +
        "them causing the forges to be shut down and rebuilt. Hopefully, " +
