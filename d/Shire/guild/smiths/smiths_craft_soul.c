/*     CMDSOUL FOR SMITHS GUILD CRAFT BRANCH
       By Morinir / Lars Madsen
       (c) Shire 1996 ;)

     Palmer 13-July-2006
       Ported to Craft slot 

     Palmer 20-July-2006
       Added 'smglance' command.
...
int smtitle(string s);
object find_item(string str,string error);
string get_armour_condition(int brief, object armour);
string get_weapon_condition(int brief, object weapon);
void whet_found();
...
help(string str)
{
    ...
    switch (str)
    {
        ...
        case "smiths emotions":
            ...
            break;
        case "smiths misc equip":
            MORE (SMITHSDIR_HELP + "misc_equip");
            break;
        case "smiths news":
            MORE (SMITHSDIR_HELP + "news");
            break;
...
string
get_weapon_condition(int brief, object weapon)
{
    string cond;

    cond = weapon->wep_condition_desc();
  
    if (brief && cond == "It looks like it is in prime condition.\n")
        return "";

    return ("    " + weapon->short() + " " + cond[17..]);
}
string
get_armour_condition(int brief, object armour)
{
    string cond;

    cond = armour->arm_condition_desc();
    if (brief) {
        if (cond == "It looks like it is in prime condition.\n" || cond == "It is intact and without any rips.\n")
            return "";
     }
     return ("    " + armour->short() + " " + cond[17..]);
}


int
smglance(string player_name) {
    int i, brief = 1, bad_hit = 0;
    object target, *armours, *weapons;;
    string str="", *armour_desc, *weapon_desc;

    write("You check if any worn armour or wielded weapons are damaged.\n");
    say(QCTNAME(TP) + " studies " + HIS_HER(TP) + " armour and weapons carefully.\n");
    if (player_name == "all")
        brief = 0;
    // target = find_player(player_name);
    target = TP;

    armours = target->query_armour(-1);
    armour_desc = map(armours, &get_armour_condition(brief));
    if (!sizeof(armour_desc)){
        str += "  No worn armours.\n";
        }
    else
        {
         for (i = 0; i < sizeof(armour_desc); i++)
            {
                if (!(armour_desc[i] == ""))
                {
                    bad_hit = 1;
         ...
         if (!bad_hit)
            str += "  All worn armours are in prime condition.\n";
         }

     weapons = target->query_weapon(-1);
     if (sizeof(weapons))
        {
         // str += "  These weapons need sharpening:\n";
         weapon_desc = map(weapons, &get_weapon_condition(brief));
         for (i = 0; i < sizeof(weapon_desc); i++)
             str += weapon_desc[i];
        }
    else
        str += "  No weapons wielded.\n";
    write(str);
    return 1;
}
...
smcheck(string str)
{
    object obj = find_item(str,"check");
...
{
    object item = find_item(str,"complain about");
    if (!item) return 0;
...
sminspect(string str) {
   object obj;
   object *to_inspect;
   int i, sz, tohit,topen,inspect,penalty;
   string *slots;
   string *protected = ({});
   ...
   if (function_exists(CHECK_WEAPON, obj) == STD_WEAPON) {
      ...
      inspect=TP->query_skill(SS_APPR_OBJ)+TP->query_stat(SS_CRAFT);
      if (inspect<10) {
   ...
   if (IS_UNARMED_ENH_OBJECT(obj))
   {
        tohit=obj->query_hit()+penalty; /* figure out tohit for wielded weapon */
        topen=obj->query_pen()+penalty; /* figure out topen for wielded weapon */
        ...
        if (topen<10)
           write("This unarmed enhancer doesn't seem very effective.\n");
        if ((topen>9) && (topen<20))
           write("This unarmed enhancer might do some damage with a lucky hit.\n");
        if ((topen>19) && (topen<30))
           write("This unarmed enhancer seems quite effective.\n");
        if ((topen>29) && (topen<40))
           write("This unarmed enhancer could most likely kill with a single swipe.\n");
        if ((topen>39) && (topen<50))
           write("This unarmed enhancer is extremely powerful and more lethal than"+
           " many of the famous weapons.\n");
        if (topen>49)
           write("This unarmed enhancer is an extremely powerful artifact.\nWith this"+
           " armour even a single warrior will be able to match a large"+
           " force.\n");
        ...
        if (tohit<10)
           ...
           " unarmed enhancer.\n");
        if ((tohit>9) && (tohit<20))
           write("It is difficult to use this unarmed enhancer properly.\n");
        if ((tohit>19) && (tohit<30))
           write("It feels good when you wear this unarmed enhancer--you would most"+
           " likely have no trouble getting a good hit with it.\n");
        if ((tohit>29) && (tohit<40))
           write("You should have no trouble handling this unarmed enhancer--or making"+
           " a cruel hit with it for that matter.\n");
        if ((tohit>39) && (tohit<50))
           write("Even an apprentice warrior could kill easily with this"+
           " unarmed enhancer.\n");
        if (tohit>49)
           write("With this unarmed enhancer almost every strike could be a killing"+
           " blow.\n");
   }
   ...
   if (function_exists("query_shield_slots", obj) != STD_ARMOUR) 
   {
      slots = obj->query_shield_slots();
      ...
      if (obj->id("shield"))
          write("This shield will protect one arm and your " + COMPOSITE_WORDS_WITH(protected, "and") + ".\n");
   }
   return 1;
...
smlist()
{
    ...
    for (i = 0; i < sz; i++)
    {
        if (pl[i]->query_guild_name_craft() == GUILD_NAME)
        {
    ...
    write(" " + sz + " Blacksmiths roam the lands.\n");
...
smtitle(string str) {
    if (str == "on") {
        write("Your title will be shown.\n");
        TP->add_prop("_smith_i_title", 1);
        return 1;
    }
    if (str == "off") {
        write("Your title will not be shown.\n");
        TP->add_prop("_smith_i_title", 0);
        return 1;
    }
    NF("You may smtitle on OR off to show or hide your title.\n");
    return 0;
}
...
smmark(string str)
{
    ...
    if (!stringp(obj->query_long())) // this can be functionp... Milan
    {
        write("You decided not to mark this item.\n");
        return 1;
    }
...
smremove(string str)
{
    string tmp, marked = "obj_has_smmark";
    object obj = find_item(str,"mark");
...
find_item(string str,string error)
{
    ...
    // be consistent and return 0 on fail. Milan
    if ((!strlen(str)) ||
        (!parse_command(str, TP, "[the] %i", to_mark)) ||
    ...
    if (sizeof(to_mark) != 1)
    {
        notify_fail("Please select only one item to "+error+".\n");
        return 0;
    }
