...
#define SMITHS_CRAFT_SOUL     "smiths_craft_soul"

#define SMITHS_CRAFT_SHADOW   "smiths_craft_shadow"

#define SMITHSDIR           "/d/Shire/guild/smiths/"
...
#define SMITHS_NEWS   (SMITHSDIR_HELP + "news")
#define SMITHS_TRANSFER_LOG   (SMITHSDIR_LOG + "transfer")
#define SMITHS_TRANSFER_OBJ   (SMITHSDIR_LIB + "transfer")
...
#define CRAFTSMAN_GUILD_TAX     4
#define GUILD_NAME           "The Smiths Guild"
#define GUILD_STYLE          "craft"

#define KNIGHT_GUILD_NAME    "Solamnian Knights"
#define UNION_GUILD_NAME     "Union of the Warriors of Shadow"
#define MAGE_GUILD_NAME      "Morgul Mages"
#define UNION_MASTER         "/d/Avenir/union/list"
...
// Formulas used
#define F_FORGE_GENEXP(cost) (ftoi(pow(cost, 0.7)) + 1)
#define F_FORGE_FAIL_GENEXP(cost) (ftoi(pow(cost, 0.3)) + 1)
#define F_MEND_GENEXP(cost) (ftoi(pow(cost, 0.5)) + 1)
#define F_MEND_FAIL_GENEXP(cost) (ftoi(pow(cost, 0.2)) + 1)
...
// #define SMITH_ADJ_SHADOW ("/d/Shire/guild/smiths/obj/bl_final")
...
/* misc */
#define SMITHS_ITEMS_FORGED     "_live_i_smith_ttl_forged"
...
#define SMITH_EXPULSION_LOG (SMITHSDIR_LOG + "expulsion.log")
#define MAX_MEMBERSHIP_LOG_SIZE 50000
...
void
log_expulsion(string s)
{
    setuid(); seteuid(getuid());

    if (file_size(SMITH_EXPULSION_LOG) > MAX_MEMBERSHIP_LOG_SIZE)
        rename(SMITH_EXPULSION_LOG, (SMITH_EXPULSION_LOG + ".old"));

    s = ctime(time())+" "+s;
    write_file(SMITH_EXPULSION_LOG, s);
}
...
IS_MEMBER(object pl)
{
    if (pl->query_guild_name_craft() != GUILD_NAME)
        return 0;
...
IS_KNIGHT(object pl)
{
    if (pl->query_guild_name_occ() != KNIGHT_GUILD_NAME)
        return 0;
...
IS_UNION(object pl)
{
    if (pl->query_guild_name_occ() != UNION_GUILD_NAME)
        return 0;
...
IS_MAGE(object pl)
{
    if (pl->query_guild_name_occ() != MAGE_GUILD_NAME)
        return 0;
...
remove_smith_guild_member(object member)
{
    ...
    member->set_guild_pref(SS_CRAFT, 0);
    if (objectp(item = present("smiths_necklace", member)))
        item->remove_object();

    member->remove_guild_craft();
    member->clear_guild_stat(SS_CRAFT);
    member->remove_cmdsoul(SMITHS_CRAFT_SOUL);
    member->update_hooks();
...
check_player()
{
    if (TP->query_stat(SS_CRAFT)>=50 || this_player()->query_wiz_level())
