 ...
 *   /d/Shire/guild/smiths/rooms/forge_r
 ...
 * -- Finwe, July 2004
 *
 * Added support for event forging.
 * -- Toby, 2008-05-02
 */
...
/* Define this to allow forging of copper stakes for the plant event. */
#define PLANT_EVENT_RUNNING
...
forge(string str)
{
    ...
    /* Access requirement. Member or attempting entrance quest. */
    if (!IS_MEMBER(TP) && !TP->query_prop(LIVE_I_SMITH_ENTRANCE_QUEST))
#ifdef PLANT_EVENT_RUNNING
        /* During event, allow non-members to forge. */
        if (str != "stake")
#endif PLANT_EVENT_RUNNING
    {
        if (TP->query_wiz_level())
            write("Lucky wizard (you can forge anyway).\n");
        else
        {
           NF("You need to check with Bores before attempting this.\n");
           return 0;
        }
    }
...
#ifdef PLANT_EVENT_RUNNING
        /* During event, allow all sizes to forge stakes. */
        if (str != "stake")
#endif PLANT_EVENT_RUNNING
    {
    ...
    if (str == "megaphone")
    {
        ...
        if (!objectp(leather = present("smiths_leather", TP)))
        {
            remove_forge_alarms();
            NF("You don't have the materials to do that!\n");
            return 0;
    ...
    {
        TP->add_prop(FORGING, 1);
        if (!objectp(bar = present("copper_smith_bar", TP)))
        {
            NF("You don't have the materials to do that!\n");
            return 0;
        }
        bar->remove_object();
        forge_messages(1, str);
        if (TP->query_stat(SS_DEX) < (random(30)))
            set_alarm(itof(random(40)+20), 0.0, &abort_forging(str));
        return 1;
    }
#endif PLANT_EVENT_RUNNING
    ...
    success = (TP->query_skill(SS_FORGING) / 2) + (TP->query_stat(SS_CRAFT) / 10) +
        random(TP->query_stat(SS_CRAFT / 5)) + (bonus/2) + random(bonus/2);
...
remove_forging_material()
{
    foreach (object ob: obAr)
    {
        /* We currently only handle removing one item from the heap */
...
query_bellow()
{
    ...
    {
        NF("The Great Forge of the Blacksmiths seems to be missing.\n");
        return 0;
    }
    ...
    {
        TP->set_fatigue(TP->query_fatigue() - (TP->query_max_fatigue() / 30));
        return 10 + random(10);
    }
...
forge_messages(int stage, string itemname)
{
    ...
    if (!TP->query_prop(FORGING))
    {
        //write("Contact a Shire wizard, something is wrong with the forging tools.\n");
        return;
    }
    ...
    switch (stage)
    {
        ...
        case 5:
            ...
            {
                item = clone_object("/d/Shire/guild/smiths/obj/apr_neck");
            ...
            if (itemname == "megaphone")
            {
                item = clone_object("/d/Shire/guild/smiths/obj/megaphone");
                item->set_forger(TP->query_name());
                item->move(TP,1);
                TP->catch_tell("You have finished forging a megaphone.\n");
                tell_room(ENV(TP), QCTNAME(TP)+" finishes forging a "+
                    "megaphone.\n", TP);
                stage = 0;
                remove_forge_alarms();
            ...
            if (itemname == "horseshoes")
            {
                item = clone_object("/d/Shire/guild/smiths/obj/horseshoes");
                item->set_forger(TP->query_name());
                item->move(TP,1);
                TP->catch_tell("You have finished forging a set of horseshoes.\n");
                tell_room(ENV(TP), QCTNAME(TP)+" finishes forging a "+
                    "set of horseshoes.\n", TP);
                stage = 0;
                remove_forge_alarms();
            ...
            if (itemname == "stake")
            {
                item = clone_object("/d/Shire/guild/smiths/obj/copper_stake");
                if(IS_MEMBER(TP))
                    item->set_smith_made_stake();
                item->move(TP,1);
                TP->catch_tell("You have finished forging a copper stake!\n");
                tell_room(ENV(TP), QCTNAME(TP)+" finishes forging a "+
                    "copper stake.\n", TP);
                stage = 0;
                remove_forge_alarms();
			    this_player()->add_exp_general(20+random(10));
                return;
            }
#endif PLANT_EVENT_RUNNING
            ...
            mod = MIN((amt/30) + TP->query_stat(SS_CRAFT)/30, 20);
            switch (itemtype)
            {
                case 1:
                    item = MANAGER->create_smith_weapon(itemname, mod);
