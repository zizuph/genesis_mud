/*
 * /d/Shire/guild/smiths/arm_final.c
 *
 * The standard armour used by the Smiths' guild
 *
 * Glaurung 8/97
 */
...
query_shield_slots()
{
    if (query_short() == "heater shield")
        return (::query_shield_slots() + ({ A_BODY }) );

    if (query_short() == "grand tower shield")
        return (::query_shield_slots() + ({ A_HEAD, A_BODY, A_LEGS }) );
