/*
 * /d/Shire/noldor/noldor_shadow.c
 *
 * This is the shadow of the noldor race guild. It is modified from the
...
#define HERALD_KILLS "/d/Shire/guild/noldor/log/noldorkill"
#define NPC_KILLS "/d/Shire/guild/noldor/log/noldornpckill"
//#define QSW      query_shadow_who()
#define JOINROOM (NOLGLD_DIR + "foyer")
#define VALLEY_DIR     "/d/Shire/rivendell/room/valley/"

/*
 * Global variable. It contains the family name of the player.
 */
private string family_name = "";
private string race_title = NOLDOR_RACE_GUILD_TITLE;
...
query_guild_keep_player(object player)
{
    ...
    // blocks Shadow Union from joining the guild.
    if(player->query_guild_name_occ() == "Union of the Warriors of Shadow")
    {
        write("Your chaotic behaviour displeases the Noldor; they refuse " +
            "to acknowledge you.\n");
        call_other(JOINROOM, "lower_skill", player);
        log_guild(ctime(time()) + " " + WRITE_NAME(player) + " was booted as a SU.\n");
...
{
//    shadow_who->catch_tell("Welcome, Noldor Elf!\n");
    if (news)
    shadow_who->catch_tell("\n\nThere is some news about the "+
          NOLDOR_RACE_GUILD_NAME+". See 'help noldor news' " +
          "for more information.\n\n");
    setuid(); seteuid(getuid());
    shadow_who->add_cmdsoul(NOLDOR_RACE_GUILD_SOUL);
    shadow_who->update_hooks();
//    clone_object(NOLDOR_RACE_GUILD_CIRCLET)->move(shadow_who, 1);
}
...
notify_you_killed_me(object killed)
{
    ...
    if (killed->id("_hobbitmaker_hobbit"))
    {
	tell_object(query_shadow_who(),
    ...
    if (killed->id("_shire_elf") && (killed->query_alignment() > 0))
    {
	tell_object(query_shadow_who(),
