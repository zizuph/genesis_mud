...
/*
 * ngraze - send your horse off to graze
 */
...
ngraze()
{
    ...
    player_name = QCTNAME(TP);
    // Need to switch euid to be able to access mapping saved on disk, then switch back to current euid.
    setuid();
    seteuid(getuid());
    steed_owners = restore_map("/d/Shire/rivendell/steeds/steed_owners");
    ...
    if (find_living(lower_case(horse_name)))
    {
        ...
        write("The " + horse->short() + " trots off.\n");
        say("The " + horse->short() + " trots off.\n");
        horse->remove_object();
        return 1;
    }

    write("Your horse isn't here.\n");
    return 1;
}
...
sing_song(object player, int stage)
{
    ...
    break;
    case 4:
    write("You sing: Na-chaered palan-d?riel\n");
    say(QCTNAME(TP)+" sings: Na-chaered palan-d?riel\n",TP);
    time = 1.5;
    break;
    case 5:
