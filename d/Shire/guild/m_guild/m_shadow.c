/*
	...
	*Shadow for the dwarven miners race guild in the Shire domain
	*Parts taken and modified from Noldor and Hobbit guilds
	*Altrus, August 2005
*/
// To test:
...
#define JOINROOM (MGLD_DIR + "rooms/join")
...
nomask private void log_guild(string text)
{
	...
	if (file_size(M_RACE_GUILD_LOG) > LOG_SIZE)
	{
		rename(M_RACE_GUILD_LOG, (M_RACE_GUILD_LOG + ".old"));
	}
	
	write_file(M_RACE_GUILD_LOG, text);
}


public mixed query_guild_trainer_race()
{
	return M_RACE_GUILD_TRAINER;
}
...
public int query_guild_keep_player(object player)
{
	...
	if (player->query_alignment() < M_RACE_GUILD_ALIGN)
	{
...
/*
	...
	* Description  : This function will return the title for the dwarf.
...
/*
	* Function name: get_miner_soul
	* Description  : Gets the soul and guild item for the player.
	* Arguments    : int news - a flag used to tell if there is new news
	*                relating to the guild
 */
void get_miner_soul(int news)
{
	if(news)
		shadow_who->catch_tell("\n\nThere is some news about the " + M_RACE_GUILD_NAME + ". See 'help miner news' for more information.\n\n");
	setuid();
	seteuid(getuid());
	shadow_who->add_cmdsoul(M_RACE_GUILD_SOUL);
...
nomask public void init_race_shadow(string foobar)
{
	...
	shadow_who->add_subloc(M_RACE_GUILD_NAME, this_object());
	...
	set_alarm(1.0,0.0,&get_miner_soul(news));
}
