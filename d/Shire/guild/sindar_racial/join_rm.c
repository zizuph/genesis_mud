...
//   Prototypes
void dest_guild_obj(object player);
...
void create_room()
{
    set_short("A plain initiation room");
    set_long("This is the initiation room for the Sindar race.\n");
   ...
   seteuid(getuid());
    ...
    add_exit(SINDAR_RACE_DIR + "board_rm", "north", "@@check_member@@");
}
...
check_member()
{
   ...
   if((!tp->query_wiz_level()) && (!IS_MEMBER(tp)))
      {
      write("Only Sindar are allowed past this point.\n");
      return 1;
   }
   return 0;
}
...
// creates the guild item   
    clone_object(SINDAR_RACE_GUILD_OBJ)->move(tp, 1);

// joining message
    write("The room begins to fill with bright " +
       "light. It surrounds you, enveloping you in " +
       "pure white light. A sextant appears in your hand. A " +
...
//    tp->reset_race_name();
   ...
   */
    ...
    set_alarm(2.0,0.0,&dest_guild_obj(tp));
    {
...
void dest_guild_obj(object player)
{
    ...
    guild_obj = present(GUILD_OBJ_ID, player);
    if (objectp(guild_obj))
    {
        guild_obj->remove_object();
    }
}
...
// remove the guild_obj  
    guild_obj = present(GUILD_OBJ_ID, tp);
    ...
    if (objectp(guild_obj))
    {
        ...
        say("A dark cloud passes overhead, blocking the light. " +
            ...
            " sextant desintegrates, leaving nothing " +
            "but a wisp of smoke.\n");
...
* Description  : Members who have lost their guild_obj, may get it back here.
* Arguments    : string str - the command line argument.
* Returns      : int 1/0    - success/failure.
*/
nomask static int request(string str)
{
    ...
    if (str != "sextant")
    {
        notify_fail("Request what? A new sextant?\n");
    return 0;
    }
    ...
    if (objectp(present(GUILD_OBJ_ID, tp)))
    {
        write("You already have a sextant. You don't need two.\n");
    return 1;
    }
    ...
    write("A new sextant appears before you. You reach out and " +
       "take it.\n");
    say(QCTNAME(tp)  + " stretches out " + HIS_HER(tp) + " hand " +
       "and takes a sextant floating in the air.\n");
...
remove_player_low_alignment(object tp)
{
    tp->catch_msg("\nIt seems the shadow of the Dark Lord has " +
        ...
        SINDAR_RACE_GUILD_NAME + ".\n\n" +
        "Your sextant bursts into flames, leaving behind a wisp " +
        "of smoke. \n");
