...
#define JOINROOM "/d/Shire/guild/heren_istimor/join"
...
create_room()
{
    ...
    set_long("Standing inside the large maple tree, you realize "+
      "the vastness of the trunk, here at ground level.  "+
      "For nearly twenty feet in any direction, there is "+
      "open space, amazing, considering that this is indeed "+
      "a living tree.  Sap dripping down the barky walls can "+
      "attest to the life of this tree, and an aura of goodness "+
      "pervades this chamber.  Around all the walls are drying "+
      "racks and hanging from them are rather large leather "+
      "pouches.  A rope ladder leads up, out onto the tree, and "+
      "you can slide back the bark door that allowed entrance "+
      "in order to exit.\n");
    ...
    add_exit(HEREN_ISTIMOR_DIR + "board","up");
...
do_slide(string str)
{
	...
	TP->move_living("M",HEREN_ISTIMOR_DIR + "join",1);
...
add_stuff()
{
    ...
    {
	herbalist = clone_object(HEREN_ISTIMOR_DIR + "herbalist");
	herbalist->move(TO);
    }
