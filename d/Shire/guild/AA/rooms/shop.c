 ...
 */
#define STORE_ROOM_SC AA_DIR + "rooms/store_sc"
#define STORE_ROOM_H  AA_DIR + "rooms/store_h"
#define STORE_ROOM_R  AA_DIR + "rooms/store_r"

#define KEEPER     AA_DIR + "npc/shop_keeper"
...
create_room()
{
  ...
  add_exit(AA_DIR+"rooms/c1", "west");
  add_exit(STORE_ROOM_SC, "east", "@@wiz_check", 0, 1);
...
do_show(string str)
{
    ...
    TP->catch_msg("You ask "+QTNAME(keeper)+" to show you "+str+".\n");
    say(QCTNAME(TP)+" asks "+QTNAME(keeper)+" to show "+HIM(TP)+" "+str+".\n");

    call_other(STORE_ROOM_SC, "load_me");
    call_other(STORE_ROOM_H,  "load_me");
    call_other(STORE_ROOM_R,  "load_me");

    item_arr = all_inventory(find_object(STORE_ROOM_SC));
    item_arr += all_inventory(find_object(STORE_ROOM_H));
    item_arr += all_inventory(find_object(STORE_ROOM_R));
    ...
    if (sizeof(item_arr) > 1)
    {
        keeper->command("say I will show you only one thing!");
    }
    say(QCTNAME(keeper)+" shows "+HIM(TP)+" "+LANG_ASHORT(item_arr[0])+".\n");
    TP->catch_msg(QCTNAME(keeper)+" shows you "+LANG_ASHORT(item_arr[0])+".\n");
...
{
  NF("");
  keeper->command("say I have nothing in stock at the moment.\n");
}
