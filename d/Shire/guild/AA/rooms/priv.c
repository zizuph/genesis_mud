...
create_shire_room()
{
    ...
    add_exit(AA_DIR+"rooms/scribe", "southeast", "@@officers_only");
...
/*
* Function name: officers_only
* Description:   Only permits officers to enter.
* Returns:       1 - Not allowed to enter
*                0 - Allowed to enter
*/
int
officers_only()
{
    ...
    {
        write("You are entering the scribe's room, but as a "+
              "wizard you are allowed inside.\n");
        return 0;
    }
    if (AA_RANKS_OBJ->lieut_cap(TP->query_name())) return 0;  
    write("A massive troll looms in from of you and shouts: " +
        "You don't belong in there, high ranking officers only!\n");
    return 1;
}
