 ...
 *     Feb 2019 - Finwe: Fix grammer errors
 */
#pragma save_binary
#pragma strict_types
...
train_text(int skill)
{
    switch (skill)
    {
        ...
        case SS_DEFENCE:
            return "survive in combat";
        case SS_BLIND_COMBAT:
            return "fight in the dark";
        case SS_SMASH:
            return "smash enemy with a club";
        case SS_SHIELD:
            return "to use a shield for dodging blows";
        case SS_BASH:
            return "to use a shield as a weapon";
        case SS_RETREAT:
            return "to retreat behind someone";
        case SS_AWARENESS:
            return "be aware of surroundings";
        case SS_APPR_MON:
            return "be able estimate a possible opponent";
        case SS_HIDE:
...
set_up_skills()
{
    ...
    sk_add_train(SS_RETREAT, train_text(SS_RETREAT), "retreat",75,100,SS_DEX,100);
...
string read_sign()
{
    mapping sign, members;
    string  lt1, lt2;
    
    if (!mappingp(sign = restore_map((AA_DIR + "save_files/AA_RANKS")) ||
        member_array("Name_rank", m_indexes(sign)) < 0))
        return "It's a blank sign!\n";

    foreach(string name, string rank: sign["Name_rank"])
    {
        if (!mappingp(members))
            members = ([]);
        
        if (member_array(rank, m_indexes(members)) < 0)
            members[rank] = ({});
        ...
        {
            if (member_array("Lieutenant", m_indexes(members)) < 0)
                members["Lieutenant"] = ({});
            
            members["Lieutenant"] += ({ name });
            continue;
        }
    ...
    + "Captain\n"
    + "-------\n\n"
    + make_string_from_list(members["Captain"])
    + "\n\n"
    + "Lieutenants\n"
    + "-----------\n\n"
    + make_string_from_list(members["Lieutenant"])
    + "\n\n"
    + "Master Sergeants\n"
    + "----------------\n\n"
    + make_string_from_list(members["Master Sergeant"] ||
                            members["Warrant Officer"])
    + "\n\n"
    + "Sergeants\n"
    + "---------\n\n"
    + make_string_from_list(members["Sergeant"])
    + "\n\n"
    + "Corporals\n"
    + "---------\n\n"
    + make_string_from_list(members["Corporal"])
    + "\n\n"
    + "Lance Corporals\n"
    + "---------------\n\n"
    + make_string_from_list(members["Lance Corporal"])
    + "\n\n"
    + "Standard Bearers\n"
    + "----------------\n\n"
    + make_string_from_list(members["Standard Bearer"])
    + "\n\n"
    + "Footmen\n"
    + "-------\n\n"
    + make_string_from_list(members["Footman"])
    + "\n";
