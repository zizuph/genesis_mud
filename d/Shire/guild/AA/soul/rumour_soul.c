/*
* Rumour soul - used to plant rumours
*
*
*
*  Modifications: 6/58/2008 Novo fixed uid/euid issues
*
*/
...
#define PLANT_LOG (AA_DIR+"log/plant_log")

#define SHADOW              (AA_DIR+"shadow/rumour_sh")
...
it_is_unacceptable_message(string mess)
{
    ...
    for (i = 0 ; i < sizeof(offensive) ; i++)
        if (sizeof(regexp(words, offensive[i])))
            return 1;
    for (i=0; i<sizeof(words); i++)
        if ("/secure/master"->exist_player(LOW(words[i])))
            return 1;
    return 0;
}
...
filter_bad_out(mixed list)
{
    ...
    for (i=0; i<sizeof(list); i++)
        if (list[i]->query_npc() && !list[i]->query_prop(I_NO_TELL_RUMOUR))
            tmp += ({ list[i] });
...
do_rumour(string str)
{
    ...
    {
        SHADOW->set_up_rumour(npc[j], mess, from, time());
    }
    ...
    write_file(PLANT_LOG,ctime(time())+" "+CAP(TP->query_real_name())+
        " starts rumour: "+ mess+"\n");
    return 1;
}
