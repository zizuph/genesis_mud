/*
 * WARNING!!!
 * This file is only included into aa_soul.c
 * Here are functions for noncombat commands:
 *      signal
 *      incognito
 *      aasay
 */
...
#define ME_DOMAINS  ({ "Shire","Gondor" })
             ...
             string str, string name, int dif)
{
    ...
    if(random(who->query_skill(SS_LOC_SENSE)) > random((20*dif)+10))
    ...
    if(!read_mess)
    {
        who->catch_msg("You are only able to recognize the signature '"+name+"'.\n");
        return;
    }
    if(!read_sign)
    {
        who->catch_msg("You are only able to recognize the symbol '"+str+"'.\n");
        return;
    }
...
incognito(string arg)
{
    if ((arg != "on") && (arg != "off"))
    {
        NF(capitalize(query_verb()) + " on or off?\n");
        return 0;
    }
    if (arg == "on")
    {
        if (TP->query_aa_incognito())
        {
            write("You are already trying to be incognito!\n");
            return 1;
        }
        TP->set_aa_incognito(1);
