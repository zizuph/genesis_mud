/*
 * Boosted NPCs base stats so they become much harder to kill much faster.
 * This is a known continuous spawning npc, so if they get bigger faster,
 * that should cut down the number of times they're killed. 
 * Finwe, August 2017
 */
...
#define GUARD_FILE AA_DIR+"npc/shield_guard"
#define AA_ENEMY   "_AA_enemy"
#define ARM_DIR   MISTY_DIR+"arm/"
#define WEP_DIR   MISTY_DIR+"wep/"
...
int base_stat = 155 + random(110);  /* set_guard_base_stat(int i) */
int was_armed;
...
update_guard()
{
  ...
  int STR, DEX, CON, INT, WIS, DIS, GUILD;
  ...
  switch(race)
  {
    case "goblin": prace = "goblins";
     ...
     GUILD = (110+random(40)) * base_stat / 100;
            break;
    case "dwarf": prace = "dwarves";
     ...
     GUILD = (90+random(40)) * base_stat / 100;
            break;
    case "orc": prace = "orcs";
     ...
     GUILD = (120+random(40)) * base_stat / 100;
            break;
    case "troll": prace = "trolls";
     ...
     GUILD = (130+random(40)) * base_stat / 100;
            break;
    default: prace = race+"s";
     ...
     DIS = (80+random(40)) * base_stat / 100;
     GUILD = (80+random(40)) * base_stat / 100;
            break;
  }
  set_stats(({ STR, DEX, CON, INT, WIS, DIS}));

  set_guild_stat(SS_OCCUP, GUILD);
  
  set_skill(SS_WEP_CLUB,     MIN(90, STR));
  set_skill(SS_SHIELD,       100);
  set_skill(SS_SMASH,        100);
  set_skill(SS_DEFENCE,      MIN(80,  DEX));
  set_skill(SS_PARRY,        MIN(80,  WIS));
  set_skill(SS_BLIND_COMBAT, MIN(80,  INT));
  set_skill(SS_AWARENESS,    MIN(85,  WIS));
...
/*
  if (rrr == "elf")
  {
        command("emote growls at " + tp->query_real_name() + "aggressively.");
//    switch(random(6))
//    {
//      case 0: command("say I hate elves! Let's kill this one!");
...
//    command("smash "+tp->query_real_name());
//    tp->add_prop(AA_ENEMY, 1);
    return;
  }
  if((rrr == "hobbit") || (rrr == "gnome"))
  {
    command("emote laughs at "+tp->query_real_name() + ".");
//    switch(random(6))
//    {
...
//              break;
//      case 5: command("say Let's scare this little "+rrr+"!");
//              command("scream");
...
init_attack()
{
  ...
  if(!interactive(TP)) return;     //don't attack NPCs
...
/*********  team stuff - they should help each other  ****************/
void
...
second_life(object killer)
{
    ...
    if (!interactive(killer)) return 0;
	...
	if (arr[i]->id("_shield_guild_guard_"))
        ...
        if (n < 5)
        {
            ...
            ob1->set_guard_base_stat(base_stat+50+random(20));
            ob1->was_cloned_from_do_die();
            ob1->arm_me();
            ob1->move(ENV(TO));
            tell_room(ENV(TO), (ob1->query_guard_race() == "orc" ? "\nAn " : "\nA ")+
        ...
        if (ob1 && !random(4))
        {
            ...
            ob2->set_guard_base_stat(base_stat+50+random(20));
            ob2->was_cloned_from_do_die();
            ob2->arm_me();
            ob2->move(ENV(TO));
            tell_room(ENV(TO), (ob2->query_guard_race() == "orc" ? "\nAn " : "\nA ")+
