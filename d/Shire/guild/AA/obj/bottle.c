/*
 * /d/Shire/guild/AA/obj/bottle.c
 *
 * Bottle that can be filled with booze.
 * Old and bad code, not to mention buggy.
 */

/* Fixed the volume problems with this object -Igneous- */
...
empty_bottle(string arg)
{
    ...
    update_state();
    filled_short = ({ });
    write("You pour the "+mix+" from the "+short()+" on the floor.\n");
    say(QCTNAME(TP)+" pours the "+mix+" from the "+short()+" on the floor.\n");
    return 1;
}
...
my_long()
{
    ...
    text = "This is a "+short()+". It can be emptied, filled by a drink and "+
