...
#define SUBLOC_STANDARD "_aa_standard_subloc"
...
call_them_here()
{
    object *rooms, *pcs, troom, *lower;
    int i,j,k,present_high;
    string sroom, cmd;
    mixed ex_fun, ranks;
            ...
            if(intp(ex_fun) && !ex_fun) /* player can move */
            {
                ...
                for(k=0; k<sizeof(pcs); k++)
                    if(CAN_SEE_IN_ROOM(pcs[k]))
                    {
                        if(IS_MEMBER(pcs[k]))
                        {
                            pcs[k]->catch_msg("You notice signal with standard of "+
                                         "Angmar calling troops to the "+cmd+".\n");
                            if(!present_high && !IS_LOWER(pcs[k]))
                                present_high = 1;
                            else
                                lower += ({ pcs[k] });
                        }
                        else
                            pcs[k]->catch_msg("You notice a red and black flag to the "+cmd+".\n");
                    }
                if(!present_high)
                    for(k=0; k<sizeof(lower); k++)
                        set_alarm(1.0, 0.0, &call_other(lower[k],"command",cmd));
            }
        }
}
...
make_them_happy(object *list)
{
    ...
    for(i=0; i<sizeof(list); i++)
        if(CAN_SEE_IN_ROOM(list[i]) && IS_LOWER(list[i]))
...
hide(string str)
{
    ...
    {
        NF("");
        write("Your Angmar standard is hidden now!\n");
        return 0;
    }
