...
create_scroll()
{
    ...
    set_adj("used");
    set_short("used book");
    set_long("This book looks well used, as the scribe uses it to record " +
        "the activity in the racks seen through the west window. If you " +
        "want to see a transcript of who has borrowed and donated " +
        "recently from those racks, just <read used book>.\n");
    set_file("/d/Shire/guild/AA/log/new_rack_log2");
    add_prop(OBJ_I_WEIGHT, 1000);
    add_prop(OBJ_I_VOLUME, 1000);
    add_prop(OBJ_I_VALUE, 0);
    add_prop(OBJ_M_NO_GET, "The used book is chained to the desk.\n");
}
...
turn_page(string str)
{
    ...
    if (!parse_command(str, ({ }), " [the] [page] [in] [used] 'book' "))
        return 0;  

    if(page)
    {
        set_file("/d/Shire/guild/AA/log/new_rack_log2.backup");
        page = 0;
    }
    else
    {
...
trim_book()
{
    ...
    if (f_size > max_size)
    {
        ...
        find_living("znagsnuf")->catch_tell("Renaming file.\n");
        write_file(file_name, top_file);
        return 1;
    }

    find_living("znagsnuf")->catch_tell("Does nothing.\n");
    return 1;
}
...
read_scroll(string str)
{
    ...
    {
        a[0]->read_it(vb);
        if((f_size > max_size) && (file_name == "/d/Shire/guild/AA/log/new_rack_log2"))
        {
