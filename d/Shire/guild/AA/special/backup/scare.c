 ...
 *                before invoking.
 * Returns:       0 - scare does not pretarget.
 */
public int
...
check_valid_action(object player, mixed * targets, string arg, int execute)
{
    ...
    //We do not pretarget (which would start the fight) - so we check
    //the target manually.
    if (!execute)
    {
        mixed * targets=target_can_scare(player,arg);
        if (!sizeof(filter(targets,&filter_kill_access(player, ))))
        {
            return 0;
        }
    }
