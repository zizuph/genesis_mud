...
inherit "/d/Genesis/specials/std/damage_new";
//inherit DAMAGE_LIB;
...
#define SMASH_ABILITY   (AA_DIR+"special/smash_new")
#define SMACK_ABILITY   (AA_DIR+"special/smack_new")
#define SCARE_ABILITY   (AA_DIR+"special/scare_new")
...
check_valid_action(object player, mixed * targets, string arg, int execute)
{
    ...
    if (SMASH_ABILITY->query_ability_in_cooldown(player)
        || SMASH_ABILITY->query_ability_in_cooldown(player)
        || SMASH_ABILITY->query_ability_in_cooldown(player))
    {
        player->catch_msg("You do not feel relaxed enough.\n");
        return 0;
    }
