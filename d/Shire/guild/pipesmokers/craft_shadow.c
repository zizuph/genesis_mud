...
// You can either have titles in the intro of a person or in their
// subloc... if the latter add to the guild object.
...
{
    return MIN(MAX_GUILD_LEVEL, query_shadow_who()->query_stat(SS_CRAFT) / 12);
}
...
{
    return titles[query_guild_level_craft()];
} 
...
init_guild_member()
{
    ...
    if (!present("pipesmokers_guild_object", who))
    {    
	clone_object(GUILD_EMBLEM)->move(who, 1);
    }
