...
config_haste_spell()
{
    set_spell_name("hortale");
    set_spell_desc("Channel the swiftness of the Mearas");
    ...
    // Spell effect object
    set_spell_object(HERALD_SPELLS_OBJ + "hortale_obj");
}
...
{
    // HERALD_DEBUG("Speed fail message activated");
...
query_spell_combat_aid_modifier()
{
    ...
    {
        // dual focus: max 2 caid, str divided by 1.6
        float_str = float_str / 1.6;
    }

    // HERALD_DEBUG("Speed aid mod: " +ftoi(float_str)+ ", lvl: " +level+ 
    //    ", corruption: " +corruption+ ".");

    combat_aid = 35 * ftoi(float_str) / 100;
    // HERALD_DEBUG("Speed combat aid: " +combat_aid+ ".");
...
check_valid_action(object caster, mixed * targets, string arg, int execute = 0)
{
    ...
    // Need to be mounted on a steed for this blessing to work.
    if(!caster->is_rider_shadow())
    {
        caster->catch_tell("To channel the swiftness of the Mearas you " +
            "must be mounted on a steed.\n");
        return 0;
    }
...
concentrate_msg(object caster, mixed * targets, string arg)
{
    ...
    object mount = caster->query_steed();
    string mount_str;

    if(!mount)
        return;
    else if(mount->steed_short())
        mount_str = mount->steed_short();
    else
        mount_str = mount->short();

    caster_message = "You call upon Orome the Rider and channel the " +
        "swiftness of his stallion Nahar, the sire of the Mearas and " +
        "greatest steed in Ea!\n";
    watcher_message = QCTNAME(caster) + " calls upon Orome the Rider, " +
        "channelling the speed of the Mearas into " +HIS_HER(caster)+
        " " + mount_str + ".\n";
...
{
    write("You are missing an ingredient to channel hortale!\n");
}
