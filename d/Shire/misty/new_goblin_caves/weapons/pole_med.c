...
make_pole_med(int arg)
{
	...
	set_long("This polearm has a broad spear-head with the \"ears\" like "+
	  "partisan - only larger and curved to point backwards.\n");
	set_dt(W_IMPALE);
	set_hands(W_BOTH);
	break;
    case 2:
	...
	set_long("This polearm has a broad spear-head with the \"ears\" "
	+ "like partisan - only larger, straight and pointy.\n");
	set_dt(W_IMPALE);
	set_hands(W_BOTH);
	break;
    case 3:
