...
reset_shire_room()
{
    ...
    {
    set_alarm(500.0, 0.0, reset_shire_room);
    return;
    }
    if((room = find_object(ROOMS_DIR+"c95")) && 
    ...
    for(i=0; i<sizeof(arr); i++)
        arr[i]->move(ROOMS_DIR+"c103");

    locker = clone_object(NPC_DIR + "locker");
    locker->move(TO);
...
create_shire_room()
{
    ...
    door = clone_object(OBJ_DIR + "grating_2_d");
    door->move(TO);
...
int enter_hole(string arg){
    ...
    tell_room(ROOMS_DIR+"c103",QCTNAME(TP)+" enters room through "+
      "hole in wall.\n");
    TP->move_living("M",ROOMS_DIR+"c103");
    return 1;
}
...
make_trap()
{
    ...
    if((ob = find_object(ROOMS_DIR+"c95")) && 
    ...
    alarm = 0;
    locked = 1;
    ob = clone_object(NPC_DIR + "locker");
    ob->move(ROOMS_DIR+"c95");
