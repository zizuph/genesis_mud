...
void create_cave()
{
    ...
    add_exit(CAVES_DIR + "c91", "southeast", "@@cleare_search");
}

string door_desc()
{
    if(TP->query_prop(PLAYER_HAS_FOUND))
	return "You can just barely see a well hidden door in southwestern wall.\n";
...
{
    if(TP->query_prop(PLAYER_HAS_FOUND))
	return "You can also see hidden door in southwestern wall.\n";
    return "";
}
...
int show_door(string arg)
{
	...
	if(who == "all")
	{
	    ...
	    TP->catch_msg("You show a hidden door to everybody in the room.\n");
	...
	if(present(who, TO))
	{
	    ...
	    TP->catch_msg("You show a  hidden door to "+QCTNAME(present(who, TO))+".\n");
...
int pull_something(string str)
{
	...
	{
	    TP->catch_msg("You wait while nobody looks at you, then you quickly "+
	      "pull the door, enter it and close it.\n");
	    say("Ooops!!! Suddenly you realize that "+QCTNAME(TP)+" isn't here!\n");
	    tell_room(ROOMS_DIR+"c93",QCTNAME(TP)+" enters room through "+
	      "a secret door.\n");
	    TP->move_living("M",ROOMS_DIR+"c93");
	    return 1;
	}
	TP->catch_msg("You can't find it in this dark...\n");
    }
...
string search_wall(object who, string str)
{
	  ...
	  (random(sum_skills) > 25))
	{
	    ...
	    return "Yes!!! Finally you discovered a secret door in southwestern "+
