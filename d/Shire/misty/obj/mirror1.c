 ...
 * Rohim
 *
 * Modified by Eowul to exclude objects in some rooms
 */
...
// Added by Eowull
static string  *see_no_objects_rooms = ({ 
	"/d/Shire/misty/goblin_caves/d10", // Lord
	"/d/Shire/misty/goblin_caves/d12", // Treasure 
	});
...
long_mirror()
{
    ...
    *dead;
	...
	if (CAN_SEE(TP, eye_object))
	{
		...
		// Check added by Eowul
		if(member_array(MASTER_OB(room), see_no_objects_rooms) == -1)
		{
...
remove_object()
{
	...
	tell_room(room, "The glass eye suddenly crumbles and turns to dust.\n");
    }
    ::remove_object();
} 
