...
my_no_sell()
{
    ...
    return "Better first empty the "+short()+" before you sell it.\n";
}
...
/*
    ...
    filled_short = ({ });
    update_state();
    write("You pour the "+mix+" from the "+short()+" on the ground.\n");
    say(QCTNAME(TP)+" pours the "+mix+" from the "+short()+" on the ground.\n");
*/
    write("You look inside the " + query_short() + " and see it has some " + 
        mix + " in it. That would make a mess if it was emptied " +
        "out here. So, you decide not to.\n");
    return 1;
}
...
my_long()
{
    ...
    text = "This is a "+short()+". It can be filled with beverages, and "+
	...
	return text+" it is about one quarter full of the "+mix+".\n";
    if(5*max_amount > 8*soft_amount)
	return text+" it is about half full of the "+mix+".\n";
    if(7*max_amount > 8*soft_amount)
	return text+" it is about three quarters full of the "+mix+".\n";
