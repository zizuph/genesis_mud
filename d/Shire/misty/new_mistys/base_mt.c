/*
 * Base file for tracks and caves in the Misty Mountains
...
create_area_room()
{
    ...
    set_areadesc("snow covered area");
    set_area("in");
    set_areaname("the Misty Mountains");
    set_land("Eriador");
    set_areatype(0);
    add_item(({"path", "ground", "area"}),
        "Deep drifts of snow cover the area. The snow hides everything, " +
        "and makes movement difficult in the mountains. An occasional track " +
        "can be seen across the surface of the snow.\n");
    add_item(({"snow drifts"}),
        "The snow drifts are large mountains of snow. Some are rounded " +
        "from the bitter cold winds that blow. Others are jagged and " +
        "appear to have fallen from higher up in the mountains, and " +
        "landed here. They make movement difficult, even though there " +
        "are tracks across some of the snow drifts.\n");   
    ...
    add_item(({"tracks", "animal tracks"}),
        ...
        "or perhaps by trolls or orcs that have crossed this way.\n");
    add_item(({"troll tracks", "orc tracks"}),
        "They are large and run across the snow. The tracks look twisted " +
        "and almost human like.\n");

    set_add_sky();
    set_room_tell_time(ROOM_TELL_TIME);
    add_room_tell("Bitter cold winds blow down from the mountain.");
    ...
    add_room_tell("The ground begins to shake as a rumble starts up in " +
        "the mountain and ends with an avalanche of snow falling in the distance.");
