/* 
 * /d/Shire/misty/npc/gcave_lord.c
 *
 ...
 *  halberds in game
 *  Finwe, October 2001
 * Added notifications cloned and when attacked, and name to find
 *  NPC in game. Is named Gashnak.
 *  Finwe, January 2004
 * Added new weapon, notable spear
 *  Finwe, November 2004
 */
...
void create_goblin()
{
    ...
    add_name("lord goblin");
    add_name("gashnak");
    set_living_name("gashnak");
    set_long("This large goblin is Gashnak, the Goblin Lord. He's taller " +
        "than most and looks " +
        "ferocious. His shaggy hair is matted with blood, and his dark, " +
        "scaly skin makes him look strong. Blood and guts drip from his " +
        "fangs from recent victims.\n");
...
#else
    add_equipment(({MISTY_ARM_DIR + "helm_sup", MISTY_ARM_DIR +
    "mail_sup" }));
#endif
    set_looks(({ "lord" }));
    ...
    // Prevent vamps from using vdrain. This special allows vamps to
    // latch onto victim and kill it without actually fighting.
    ...
    TELL_ADMIN("Gashnak, the goblin lord was cloned at " + ctime(time()));
...
arm_shire_npc()
{
    ...
    if (!Armed)
    {
        switch(random(6)) // randomize which weapon to clone
        {
            case 0: case 1:
            {
                wep = clone_unique(MISTY_WEP_DIR + "spear_sup", 
                    MAX_UNIQUE_ITEMS, MISTY_WEP_DIR + "pole_high");
                break;
            }
            case 2: case 3:
            {
                wep = clone_unique(MISTY_WEP_DIR + "halberd_sup",
                    MAX_UNIQUE_ITEMS, MISTY_WEP_DIR + "pole_high");
            }
            case 4: case 5:
            {
                wep = clone_unique(MISTY_WEP_DIR + "an_axem",
                    MAX_UNIQUE_ITEMS, MISTY_WEP_DIR + "pole_high");
            }
...
attacked_by(object ob)
{
    ...
    ::attacked_by(ob);
    TELL_ADMIN("Gashnak, the goblin lord attacked by " + ob->query_cap_name() + "("+ob->query_average_stat()+") at " + ctime(time()));
}
