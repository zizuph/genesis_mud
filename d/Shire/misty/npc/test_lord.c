...
void
trip_player(object player)
{
    if (player != query_attack() || !check_seen(player))
	return;

    if (query_prop(LIVE_I_STUNNED) || query_prop(LIVE_I_ATTACK_DELAY))
	return;

    if (!sizeof(weapons = query_weapon(-1)))
	return;

    // We try to trip the player and prevent them from leaving *grin*
