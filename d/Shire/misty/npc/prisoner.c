 ...
 * Updated by Igneous
 *
 * 2008/05/06 Last update
 * 2011/07/25 Lavellan - Nodding without introducing no longer runtimes.
 *                       Perhaps he should only responds to nods at him and
 *                       not just nods in general?
 *                     - Eats/drinks all given, not just one, and drops spare
 *                     - Returns heap objects properly.
 */
...
emote_hook(string emote, object actor, string adverb)
{
	...
	if (CHECK_QUEST(TP, "prisoner1"))
	{
	    ...
	    set_alarm(13.0,0.0, &command("whisper "+who+" Oh, one last "+
		"thing, mind you... I would like for you to avoid as much "+
        "battle as possible, I am not certain I can handle much "+
        "more struggle in my weakened state."));
...
//	    "to find out where is my friend first."));
	    "to find out where my friend is first."));
...
react_gift(object *foo)
{
      ...
      (what->query_key() == "_rusty_gcaves_quest_key_"))
    {
        ...
        {
            TELL_ADMIN(from->query_cap_name() +
            " is doing Prisoner quest and gave the key to the prisoner.");
...
//        command("eat "+what->query_names()[0]);
          ...
          (ate_food + drank_drink == 2))
        {
            ...
            TELL_ADMIN(from->query_cap_name() +
            " is doing Prisoner quest and just fed the prisoner.");
      ...
      (what->query_prisoner_sword()))
    {
          ...
          (filip_has_sword + filip_has_armour == 2))
        {
            ...
            TELL_ADMIN(from->query_cap_name() +
            " is doing Prisoner quest and just armed the prisoner.");
...
give_back(object *foo)
{
    ...
    if (IS_HEAP_OBJECT(foo[0]))
    {
        if (!command("give "+foo[0]->query_pnames()[0]+" to "+OB_NAME(foo[1])))
            command("drop "+foo[0]->query_pnames()[0]);
    }
    else
    {
...
enter_env(object dest, object old)
{
	...
	if (present(ob_leader, environment()))
	{
	    ...
	    {
            TELL_ADMIN(ob_leader->query_cap_name() +
            " completed the Prisoner quest and received " + reward + " in reward.");
