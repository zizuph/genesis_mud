...
//    add_name ("goblin");
    ...
    set_base_stat(SS_WIS, (base * 3 / 5) + random(base / 2));
    set_base_stat(SS_INT, (base * 3 / 5) + random(base / 2));
...
void arm_me()
{
    ...
    wep = clone_object(MISTY_WEP_DIR + ONE_OF(ran_wep) + type);
    wep->move(TO,1);
    if (wep->query_prop(OBJ_I_IS_MAGIC_WEAPON))
    {
	// Ugly but we'll live with it.
	wep->remove_object();
	wep = clone_unique(MISTY_WEP_DIR + "club" + type, MAX_UNIQUE_ITEMS, 
	  MISTY_WEP_DIR + "club_med");
	wep->move(TO, 1);
    }
    if (wep->query_prop(OBJ_I_IS_MAGIC_WEAPON))
	set_kill_log_name("club_shield");
    arm = clone_object(MISTY_ARM_DIR + ONE_OF(ran_arm) + type);
    arm->move(TO, 1);
    for (i = 0; i < sizeof(def_arm); i ++)
	clone_object(MISTY_ARM_DIR + def_arm[i] + type)->move(TO,1);
