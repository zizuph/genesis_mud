...
int pull_something(string str)
{
	...
	if(CAN_SEE_IN_ROOM(TP))
	{
		...
		TP->catch_msg("You pull the wall with all your strength. It " +
		  "starts to move but suddenly stops. It appears you're not " +
          "strong enough.\n");
...
int push_something(string str)
{
	...
	if(CAN_SEE_IN_ROOM(TP))
	{
		...
		if(sizeof(team = TP->query_team()))
			...
			if(present(team[i], ENV(TP)))
			{
			    team[i]->move_living("through an opening", push_room);
			    team[i]->catch_msg("You leave through an opening.\n");
			}
		TP->catch_msg("You pass through an opening in the wall and close " +
            "it behind you.\n");
		TP->move_living("through an opening in the wall and it behind",push_room,1);
	    }
	    else
	    {
		TP->catch_msg("You push the wall with all your strength. It " +
		  "starts move but sudenly stops. It seems you're not strong enough.\n");
