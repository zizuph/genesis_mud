...
void create_cave()
{
    ...
    add_exit(CAVE_DIR + "t16", "north", "@@cleare_search");
    add_exit(CAVE_DIR + "t18", "west",  "@@cleare_search");
}

string door_desc()
{
    if(TP->query_prop(PLAYER_HAS_FOUND))
	return "You can just barely see well hidden door in eastern wall.\n";
...
{
    if(TP->query_prop(PLAYER_HAS_FOUND))
	return "You can also see hidden door in eastern wall.\n";
    return "";
}
...
int show_door(string arg)
{
	...
	if(who == "all")
	{
	    ...
	    TP->catch_msg("You show hidden door to everybody in the room.\n");
	    say(QCTNAME(TP)+" shows you hidden door.\n");
	    return 1;
	}
	if(present(who, TO))
	{
	    ...
	    TP->catch_msg("You show hidden door to "+QCTNAME(present(who, TO))+".\n");
	    present(who, TO)->catch_msg(QCTNAME(TP)+" shows you hidden door.\n");
	    return 1;
	}
    }
    NF("Show what?\n");
...
int pull_something(string str)
{
	...
	{
	    TP->catch_msg("You wait while nobody looks at you, then you quickly "+
	      "pull door, enter it and close it behind.\n");
	    say("Ooops!!! Suddenly you realize that "+QCTNAME(TP)+" isn't here!\n");
	    tell_room(CAVE_DIR+"d0",QCTNAME(TP)+" enters room through "+
	      "secret door.\n");
	    TP->move_living("M",CAVE_DIR+"d0");
	    return 1;
	}
	TP->catch_msg("You can't find it in this dark...\n");
    }
...
string search_wall(object who, string str)
{
    ...
    if(((str == "wall") || (str == "walls")) && CAN_SEE_IN_ROOM(who))
    {
	if(who->query_prop(PLAYER_HAS_SEARCHED) && (sum_skills > 49) &&
	  (random(sum_skills) > 25))
	{
	    who->add_prop(PLAYER_HAS_FOUND, 1);
	    return "Yes!!! Finally you discovered secret door in eastern "+
