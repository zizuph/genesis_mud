...
void create_cave()
{
    ...
    set_short("Rat farm");
    set_long("You are at the end of the tunnel that burrows under the Misty "
      ...
      + "farm here. He trains rats to deliver messages and he hopes "
...
reset_shire_room()
{
    ...
    ratkeeper = clone_object(GOBLIN_NPC_DIR + "ratkeeper.c");
    ratkeeper->arm_me();
    ratkeeper->move(TO,1);
    tell_room(TO, QCTNAME(ratkeeper)+" rushes in.\n");
...
int check_intro()
{
    ...
    ratkeeper->command("say Yes, I mean you, you ugly "+
...
int hire_rat(string arg)
{
    ...
    post_rat = clone_object(MISTY_OBJ_DIR + "post_rat");
    if (post_rat->move(TP))
    {
	post_rat->move(TO);
