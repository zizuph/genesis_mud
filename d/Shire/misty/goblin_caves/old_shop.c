/*
 * /d/Shire/misty/goblin_caves/shop.c
 ...
 *                               Tricky, dec 1991
 *
 * Cleaned up the room's description.
 * -- Finwe, July 2004
 */
...
void create_cave()
{
    ...
    set_long("You are at the end of a collapsed tunnel under the Misty " +
        ...
        "or sell things offered to him. A poster is on the wall for " +
        "you to read.\n");
/*
...
//    if(keeper && present(keeper,TO)) return;
    if (keeper || present(keeper)) return;

    keeper = clone_object(GOBLIN_NPC_DIR + "keeper");
    keeper->arm_me();
...
int check_keeper(string arg)
{
	...
	if(enemy = keeper->query_attack())
	{
	    ...
	    {
		keeper->command("say Please "+who+"! You must wait until I kill "+
...
int check_intro(string arg)
{
    ...
    keeper->command("say Hey you dirty cur! I mean you "+TP->query_nonmet_name()+"!");
    return 0;
}
...
int do_buy(string args)
{
	...
	clone_object(MISTY_OBJ_DIR + "bed_cover")->move(STORE_ROOM);
