...
reset_shire_room()
{
    ...
    locker = clone_object(GOBLIN_NPC_DIR + "locker");
    locker->move(TO);
...
create_shire_room()
{
	...
	"north wall","walls"}),
      ...
      "of stone. "+"@@hole_desc");
    add_item("blocks","They are big blocks of dark cold rock, which make " +
		"up the cold walls.\n");

    door = clone_object(MISTY_OBJ_DIR + "grating_2_d");
    door->move(TO);
...
string
hole_desc(){
    if(hole_i_is) return "One block is removed from southern wall, leaving behind a large hole.\n";
    return "\n";
}
...
make_trap()
{
    ...
    alarm = 0;
    locked = 1;
    ob = clone_object(GOBLIN_NPC_DIR + "locker");
    ob->move(CAVE_DIR+"d4");
