...
create_monster() {
   if (!IS_CLONE) return;
   set_name("barliman");
   add_name(({"butterbur","innkeeper", "barliman"}));
   set_living_name("butterbur");
   set_title("Butterbur, Innkeeper of the Prancing Pony");
   set_race_name("human"); 
   set_adj(({"short","fat"}));
   set_long("@@my_long");
   ...
   add_act("emote wonders aloud if he'll need four kegs or five for " +
        "that dwarven party.");
   add_act("emote sighs wistfully, remembering when more hobbits came " +
        "from the Shire.");
   add_act("smile slee");
   ...
   add_chat("Even though I may not look it, I'm busy, busy, busy.");
   add_chat("I have not seen Gandalf the Grey in a long time.");
   add_chat("I hear rumors of orcs very near Bree.");
   add_chat("Travellers have reported orcs are growing stronger in the " +
        "Misty Mountains.");
   add_chat("Did you know we have a new game? Connect Four!");
   add_chat("What is it I am forgetting?");
   add_chat("We have a notary in town. It's above the Post Office.");
   add_chat("Some dwarves passing through town mentioned they were waylaid " +
        "by orcs when passing over the Misty Mountains.");
   add_chat("Word has it that orcs in the mountains are more active " +
        "than normal.");
}
...
my_long() {
   ...
   return "This is Barliman Butterbur the innkeeper. Barliman "+
