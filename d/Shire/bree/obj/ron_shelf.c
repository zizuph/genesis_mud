/*
* RoN shelf in ranger treetop to hold items
* Finwe, November 2004
*
...
create_container()
{   
    set_short("large gray shelf");
    set_name("shelf");
    set_adj(({"large", "gray"}));
    set_long("It is a large, gray shelf hidden on a large branch in the " +
        "treetop. The shelf is designed to hold supplies for rangers who " +
        "visit here on patrol or to donate to the Rangers of the North. You " +
        "can <sort gray shelf> to remove saving items.\n");
...
{ 
    if (arg != "gray shelf") 
        return notify_fail("Sort what, the gray shelf?\n"); 

    if (sizeof(all_inventory(TO)) < MAX_SORT) 
    { 
        do_sort();        
        write("You sort the gray shelf. Everything that saves falls out onto the floor.\n"); 
        say(QCTNAME(this_player()) +" sorts the gray shelf.\n"); 
        return 1; 
    } 
