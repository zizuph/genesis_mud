...
enter_env(object to, object from)
{
  ...
  if (!living(to)) 
  {
    ...
    set_alarm(1.0, 60.0, "stench_message", to);
...
{
  player->catch_msg("You almost drive yourself wild with such an "
    + "outrageous perfume.\n");

    tell_room(environment(environment(TO)), "A totally compelling scent drives you wild- it's outrageous!\n");
}
