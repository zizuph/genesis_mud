/*
 * A bag with sugar cubes in it, sold in Bree tack shop.
...
create_receptacle()
{
    ...
    set_name("bag of sugar cubes");
    add_name("bag");
    set_pname("bags of sugar cubes");
    add_pname("bags");
    set_adj( ({"thin","leather" }) );
    set_short("bag of sugar cubes");
    set_pshort("bags of sugar cubes");
    set_long("This is a brown leather bag, made for holding sugar cubes.\n");
    clone_object(BREE_STABLES_DIR + "obj/sugar")->move(this_object(), 1);
    clone_object(BREE_STABLES_DIR + "obj/sugar")->move(this_object(), 1);
    clone_object(BREE_STABLES_DIR + "obj/sugar")->move(this_object(), 1);
    clone_object(BREE_STABLES_DIR + "obj/sugar")->move(this_object(), 1);
    clone_object(BREE_STABLES_DIR + "obj/sugar")->move(this_object(), 1);
