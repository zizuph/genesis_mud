/*
 * /d/Shire/rooms/town/stable/npc/stable_master.c
 *
...
#define RENTAL_PRICE 3456    /*  2 platinum rental fee*/
...
rent_horse(string str)
{
    ...
    if(race == "kender" || race == "dwarf" || race == "hobbit")
    {
        if(horse_type[2] == "horse")
        {
    ...
    horse_name = str;
    command("say to " + player + " I'll be back with your " + horse_name +
    ...
    set_alarm(5.7, 0.0, &command("lead " + horse_name));
    set_alarm(6.5, 0.0, &command("west"));
    set_alarm(7.7, 0.0, &command("unlead " + horse_name));
    ...
    set_alarm(8.5, 0.0, &lead_horse(TP));
