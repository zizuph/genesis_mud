 ...
 *
 * Notes:
 *   To get a list of people who have rented type "renters"
 *   To get a list of horse owners, type "owners"
 *
 ...
 * Arandir - 06 January 2005 - Repeated rent allowed after one day
 * Palmer - 22 July 2006 - Only give pamphlet if you ask for one.
 */
...
create_shire_monster()
{
    ...
    add_ask(({"pamphlet", "for a pamphlet", "for pamphlet"}) ,VBFC_ME("ask_for_pamphlet"));
...
rent_horse(string str)
{
    ...
    set_alarm(8.1, 0.0, &command("say to " + player + " If you need instructions for your steed ask me for a pamphlet."));
...
/*
  Name: ask_for_pamphlet
  Desc: Give player scroll with riding and care instructions.
...
ask_for_pamphlet()
{
    ...
    if (TP->query_prop("_live_i_keleren_pamphlet"))
        {
            command("say I already gave you a pamphlet.");
            return "";
        }
    scroll = clone_object(BREE_STABLES_OBJ_DIR + "pamphlet");
    scroll->move(TO);
    command("say Read this pamphlet. " +
        "It contains important information about your steed.");
    command("give pamphlet to " + TP->query_real_name());
    TP->add_prop("_live_i_keleren_pamphlet", 1);
    return "";
...
give_task()
{
    ...
    {
        command("chuckle annoy" + TP->query_real_name());
