// Palmer, J''uary 2003
// Evolved from:
// Old Fogey's club, borrowing from Vladimir ''d Dunstable's
// Smart Goblin club, Dunstable 12/94.

// 2006/03/04 Last update
// 2011/07/20 Lavell'' - Fixed "join" response
...
create_room()
{
    ...
    // add_exit("/d/Shire/greyhavens/city/connect4","east", 0, 1, 1);
...
do_join(string str)
{
    ...
    if (!crop)
    {
        ...
        say(QCTNAME(TP) +
            " becomes a member of the Equestrian Society of Bree.\n");
    }
    return 1;
}
...
do_crop(string str)
{
    ...
    crop = clone_object(STABLE_DIR + "/club/crop");
    crop -> move(TP,1);
