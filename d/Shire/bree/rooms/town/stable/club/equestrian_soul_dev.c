...
edismount()
{
    ...
    if (sizeof(member))
    {    
    write("You signal to your party, indicating they should dismount their steeds.\n");
    all(" gestures to " + HIS_HER(TP) + " party, letting them know they should dismount.");    
    }
    TP->command("dismount");
...
ecrack()
{
    write("You crack your riding crop loudly in the palm of your hand, trying to get everyone's attention.\n");
    all(" cracks " + HIS_HER(TP) + " riding crop loudly in the palm of " +HIS_HER(TP)+ " hand, trying to get your attention.");
    return 1;
}
...
efinger()
{
    ...
    all(" fingers " + HIS_HER(TP) + " riding crop, thinking about " + HIS_HER(TP) + " next ride.");
    return 1;
}
...
eobserve(string str)
{
    ...
    if(!str)
    {
        write("You smack the palm of your hand with your riding crop, deep in thought.\n");
        all(" smacks the palm of " + HIS_HER(TP) + " hand with " + HIS_HER(TP) + " riding crop, deep in thought.");
        return 1;
    }
    ...
    actor("You observe",oblist, " while smacking the palm of your hand with your riding crop.");
    target(" observes you as " + HE_SHE(TP) + " smacks " + HIS_HER(TP) + " palm with " + HIS_HER(TP) + " riding crop.",oblist);
    all2act(" observes",oblist," as " + HE_SHE(TP) + " smacks "+ HIS_HER(TP) + " palm with " + HIS_HER(TP) + " riding crop.",oblist);
    return 1;
}
...
esearch()
{
    ...
    all(" grips " + HIS_HER(TP) + " riding crop nervously, looking for "  + HIS_HER(TP) + " steed.");
    return 1;
}
...
emap()
{
    ...
    all(" maps out the next trail in the dust with " + HIS_HER(TP) + " riding crop.");
    return 1;
}
...
eaccuse(string str)
{
    ...
    target(" smells you, looks around, then checks the soles of " + HIS_HER(TP) + " boots.",oblist);
    all2act(" smells", oblist, ", looks around, then checks the soles of " + HIS_HER(TP) + " boots.",oblist);
    return 1;
}
...
epoint(string str)
{
    ...
    *items,
    ...
    if (!strlen(how[0]))
    {
        write("You"+how[1]+" point in a general direction with your riding crop.\n");
        all(how[1]+" points in a general direction with " + HIS_HER(TP) + " riding crop.");
        return 1;
    }
