...
static string
    ...
    //reglin = "| %-40s | %-8s | %4d | %2d %2d %2d %2d |\n",

    reglin = "|%4s %-40s | %4d %4d %4d %4d |\n",
...
print_item(object *ob)
{
    ...
    if (categ != prev_categ)
        ret += blklin;
    prev_categ = categ;
    ret += sprintf(reglin, extract(ob[0]->short(), 0, 3), extract(ob[0]->short(), 0, 40), price / 1728, (price % 1728) / 144, (price % 144) / 12, price % 12);
    return ret;
}
