...
create_er_room()
{
   ...
   add_item(({"water","the water","stream"}),
        "Blocking passage to the north, the small stream runs off " +
        "the east, where it will eventually merge together with " +
        "the great river Baraduin.\n");
   ...
   add_exit(EAST_R_DIR + "er17n2","south",0,2);
   add_exit(EAST_R_DIR + "er16n2","west",0,2);
   add_exit(EAST_R_DIR + "er18n2","east",0,2);
