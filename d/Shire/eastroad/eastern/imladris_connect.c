 ...
 * Finwe, August 1998
 *
 * Added bushes along the west side of the road where rangers can spy
 * on who passes by.
 -- Finwe, Jul 2004
 */
#pragma save_binary
...
static string  patrol_dir = "south";
...
create_er_room()
{
    ...
    add_item(({"mountains", "misty mountains", "foothills"}),
      ...
      "the distance.\n");
    add_item(({"path", "paths"}),
        "One path looks well used as it heads into the mountains, skirting " +
        "the river to the east. Another less travelled path runs east to " +
        "the river.@@ranger_path@@\n");
    ...
    add_exit(TROLLSHAW_DIR + "mpath6",    "north", "@@tree_north@@", 1);
    add_exit(TROLLSHAW_DIR + "mpath4",    "south", "@@tree_south@@", 1);
    add_exit(RIVER_DIR    + "riverbank", "east", "@@tree_east@@", 2,1);
...
string ranger_path(string str)
{
        if (R_ALLOWED)
            return " A faint path leads into the bushes.\n";
        else
            return "";
}
...
enter_inv(object ob, object from)
{
   ...
   names = ob->query_names();
	...
	return;
       ...
       if (interactive(ob))
    {
        dir = ob->query_prop("_live_s_last_move");
        switch(dir)
        {
            ...
            case "southeast": dir = "nortwest"; break;
            case "southwest": dir = "north"; break;
            case "southwest": dir = "north"; break;
            case "southwest": dir = "north"; break;
            case "southwest": dir = "north"; break;
        ...
        tell_room("/d/Shire/eastroad/eastern/riv_bushes.c", QTNAME(TP) + 
