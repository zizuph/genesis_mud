...
reset_shire_room()
{
    ...
    if (!objectp(dwarf1 = present("dwarf1", TO)))
    {
	dwarf1 = clone_object(ERNPC_DIR + "dwarf_warrior");
	dwarf1->add_name("dwarf1");
	dwarf1->move_living("void",TO);
    }
    if (!objectp(dwarf2 = present("dwarf2", TO)))
    {
	dwarf2 = clone_object(ERNPC_DIR + "dwarf_warrior");
	dwarf2->add_name("dwarf2");
...
int climbout(string out)
{
    ...
    if (TP->resolve_task(TASK_DIFFICULT, ({TS_DEX, SS_CLIMB})) <= 0)
	set_alarm(6.0, 0.0, &do_fail_climb(TP));
    else
	set_alarm(6.0, 0.0, &do_successful_climb(TP));
...
{
    player->remove_prop(CLIMB_PROP);
    // Don't print messages of move the player if they aren't in the room.
    if (!present(player,TO))
	return 0;
