...
create_er_room()
{
    set_area("just south of");
    set_areaname("the Forgotten Inn by the Great East Road");
    set_land("Eriador");
    set_areatype(AREATYPE_GRASS);
    set_areadesc("open field");
    set_grass("lush, long");
    set_treetype("oak");

    set_extraline("The Chetwood is now further to your northwest "+
    "where it swallows up the road, while green fields provide plenty "+
    "of open space in all directions. From around the Inn you can "+
    "make out the Great East Road to the north.");
    ...
    add_exit(ER_DIR + "erb7s", "east", 0, "@@move_easy@@");
    add_exit(ER_DIR + "erb5s", "northwest", 0, "@@move_easy@@");
    add_exit(ER_DIR + "erb6", "north", 0, "@@move_easy@@");
...
int enterinn(string inn)
{
    ...
    TP->move_living("into the Inn", ER_DIR + "inn1");
