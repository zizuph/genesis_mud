...
create_er_room()
{
    ...
    add_item("fields",
      "The fields that were surrounding the road to the west are now "+
      "giving way as the Midgewater Marshes encroach on the road "+
      "from the north, and the South Down close in from the south. "+
      "You notice fields north of the road are much more damp than "+
      "those south, not doubt due to run-off from the Marshes.\n");
    ...
    add_exit(ER_DIR + "erb9",  "west", 0, 1);
    add_exit(ER_DIR + "erb11",  "east", 0, 1);
/*  add_exit(ER_DIR + "erb10n", "north", 0, 2, 1);
    add_exit(ER_DIR + "erb10s", "south", 0, 2, 1); */
