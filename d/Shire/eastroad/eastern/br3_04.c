...
//    set_add_weather_hills();
    ...
    set_extraline("The rocky land is covered by sparse grasses, wild " +
        "flowers, and shrugs. The land is dry and looks bare except " +
        "for an occasional grove of trees that dot the land.");

    add_item(({"path"}),
        "The path is little more than a thin line that winds through " +
        "the land.\n");

    add_exit(ER_DIR + "br3_05", "east");
    add_exit(ER_DIR + "br3_03", "west");
