...
int climbout(string out)
{
    ...
    task = (objectp(TP->query_attack()) ? TASK_DIFFICULT : TASK_ROUTINE);
    result = TP->resolve_task(task, ({TS_DEX, SS_CLIMB}));
    if (result <= 0)
    ...
    log_file("debug/climbing", ctime(time()) + " PlAYER: "+
      TP->query_real_name() +" TASK: "+task+"  SKILLS "+
      (TP->query_stat(SS_DEX) + TP->query_skill(SS_CLIMB)) + "  RESULT: "+
      result +".\n");
    return 1;
}
...
int do_fail_climb(object player)
{
	...
	return 0;
    if (present("_Shire_chair_stun", player))
	return 0;
