...
create_er_room()
{
    set_area("close to Weathertop on");
    ...
    set_extraline("Weathertop fills your view to the west where "+
    "the road has to head south to navigate its circumference. "+
    "Its base is smattered with sparse vegetation, trees growing "+
    "in clumps around the ancient watchpost, but its top remains "+
    "bare. The road continues to the southwest and east from here.");

    add_item("weathertop",
      "Looming over you to the west is Amon Sul, "+
    ...
    add_exit(ER_DIR + "erb21",  "southwest", 0, 1);
    add_exit(ER_DIR + "erb23",  "east", 0, 1);
/*  add_exit(ER_DIR + "erb21n", "west", 0, 2, 1);
    add_exit(ER_DIR + "erb22n", "north", 0, 2, 1);
    add_exit(ER_DIR + "erb22s", "south", 0, 2, 1);
*/
}
