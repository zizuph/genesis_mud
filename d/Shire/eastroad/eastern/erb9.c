...
create_er_room()
{
    ...
    set_extraline("The road is in the open now, with grassy fields "+
    "on all sides. Creeping closer from the north the pall "+
    "of the Midgewater Marshes swallows up any vision in that "+
    "direction. The road continues to the west and east from here.");
    ...
    add_item(({"hills","downs","south downs"}),
      "Filling the south horizon are the south downs, slowly "+
      "creeping up on the Great East road until they meet not too "+
      "far to the southeast.\n");
    add_item(({"pall","marsh","marshes","midgewater marshes"}),
      "The faint whiff of rotten eggs reaches you here as you "+
      "approach the pall of the Midgewater Marshes. The "+
    ...
    add_exit(ER_DIR + "erb8",  "west", 0, 1);
    add_exit(ER_DIR + "erb10",  "east", 0, 1);
    add_exit(ER_DIR + "erb9n1", "north", 0, 2);
//    add_exit(ER_DIR + "erb9s", "south", 0, 2, 1);
