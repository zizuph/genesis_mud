...
init_group_cluster()
{
    ...
    /* Added gNoGroup and query_team_others as I opted out A.I. clusters */
    if (interactive(this_player()) || !environment(this_object()) ||
        this_object()->query_leader() || !sizeof(gProspectiveGroup) ||
        gNoGroup || sizeof(this_player()->query_team_others()))
        return;
    ...
    while(Prospects--)
    {
        ...
        {
            set_alarm(0.0, 0.0, &add_to_group_cluster(this_player()));
            
            /* Obsolete now?..
             * Add subjects to the master cluster
             *if (objectp(cluster = this_object()->query_active_cluster()))
             *   cluster->add_cluster_subject(this_player()->query_active_cluster());
             */
            break;
        }
    }
