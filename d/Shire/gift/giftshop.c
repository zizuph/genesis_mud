 ...
 * but brought over by Finwe.
 * --Finwe, November 2001
 *
 * 2005/05/18 Last update
 * 2011/08/01 Lavellan - Changed to Shire base room so reset actually works!
 */
...
create_room()
{
    ...
    set_long("This is the headquarters for the Shire Gift " +
        ...
        "from all over to drop off gifts to be wrapped. A large " +
        "table sits in the center of the room " +
        "behind a counter. It is so popular that it is busy all " +
        "the time. A sign is posted on a wall.\n");
    add_item("counter",
    ...
    add_item("rack",
        "The rack is made of wood and set against a wall. It has many " +
        "slots  where rolls of wrapping paper and bows are stored.\n");
    add_item(({"materials"}),
        "The materials are stiff wrapping paper and formed in the " +
        "shapes of bows for decorating gifts.\n");
    ...
    add_exit(FROG_DIR + "road03", "south");    
...
reset_shire_room()
{
    if (!fuschia)
    {
        fuschia = clone_object(GIFT_DIR + "fuschia");
        fuschia->move(this_object());
    }
}
...
do_wrap(string str)
{
    ...
    if (!fuschia || !present(fuschia))
    {
        notify_fail("The giftwrapper has left the shop to get " +
            "more supplies. When she returns, she will wrap your " +
            "gifts for you.\n");
        return 0;
    }


    NF("I don't understand. Please read the sign for more information.\n");
