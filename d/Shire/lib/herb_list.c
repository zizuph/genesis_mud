...
sort_objects_to_list(object *a, object *b)
{
    ...
    if (a[0]->query_herb_name() < b[0]->query_herb_name()) return -1;
    return 1;
}
...
print_item(object *ob)
{
    ...
    if (!sizeof(ob))
        ...
        qty = sizeof(ob);
    categ = (ob[0])->query_herb_name();
    categ = CAP(categ);

//    if (categ != prev_categ)    // if first line
//        ret += devlin;
//    prev_categ = categ;
//    ret += sprintf(reglin, extract(ob[0]->short(), 0, 30), categ, qty);
    ret += sprintf(reglin, categ, qty);
...
print_shop_list(mixed *list)
{
    string ret;
    int i, j;
    total_qty = 0; // resets total number of herbs in sack
    i = sizeof(list);
    if (!i)
    {
        return "";
...
// counts recoverable herbs and prints totals
    count_herbs();
    ret += devlin + sprintf( reglin, "Total Herbs", total_qty);
    ret += endlin;
    return ret;
}
