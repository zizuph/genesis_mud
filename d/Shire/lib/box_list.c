 ...
 *
 * 2007/07/22 ?
 * 2011/07/12 Lavellan - Fixed qty and name display for heaps.
 */
...
print_item(object *ob)
{
    ...
    if (!sizeof(ob))
        ...
        qty = sizeof(ob);
    desc = extract(ob[0]->short(), 0, 50);
    categ = figure_category(ob[0]);
    price = query_buy_price(ob[0]);
    if (!old && IS_HEAP_OBJECT(ob[0]))
    {
        qty += ob[0]->num_heap() - 1;
        desc = extract(ob[0]->singular_short(), 0, 50);
    }
    if (categ != prev_categ)
        ret += blklin;
    prev_categ = categ;
...
//    ret += sprintf(reglin, extract(ob[0]->short(), 0, 50), categ, qty);
    //ret += sprintf(reglin, extract(ob[0]->short(), 0, 50), categ, qty);
    ret += sprintf(reglin, desc, categ, qty);
    return ret;
}
