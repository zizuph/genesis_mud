/* Ruthlessly stolen from GOndor because I was too lazy to code it myself.
 * Igneous Dec 1997
 */
/* 
 * Functions:
...
query_in_dir(object ob)
{
    ...
    while (!ob->query_prop(ROOM_I_IS))
        ob = environment(ob);

    // Normal room:
    if (wildmatch("/d/*", path = file_name(ob)))
        subdir = explode(path, "/");
    // Link-room:
    else if (stringp(path = ob->query_link_master()) &&
        wildmatch("/d/*", path)) 
        subdir = explode(path, "/");
...
filter_shire(object pl)
{
    ...
    return (room->query_domain() == "Shire");
}
...
tell_players_in_dir(string dir, string text)
{
    ...
    player = filter(users(), filter_shire);
    player = filter(player,  &operator(==)(dir) @ query_in_dir);
