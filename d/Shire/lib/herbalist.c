 ...
 * -- Finwe, Sept 2005
 * Added comma precision to herb quantity (1,234)
 * -- Finwe, March 2019
 */
...
do_sell(string str)
{
    ...
    skill_avg = min(max(50, skill_avg), 100);

    /* We're somewhat greedy */
    for (i = 0;i < sizeof(herbs);i++)
	value += herbs[i]->query_herb_value() * herbs[i]->num_heap() * skill_avg / 175;
    do_pay(TP, value);
    map(herbs, add_herb);
    save_herbs();
    return 1;
...
//left column
        ...
        {
            list = sprintf("%,5d  %-=17s  %2d %2d %2d %2d |",
                herb_amt, item_name,
...
//right column
        ...
        list = sprintf("%,5d  %-=17s  %2d %2d %2d %2d | %,5d  %-=17s  %2d %2d %2d %2d ",
            herb_amt, item_name,
            price / 1728, (price % 1728) / 144, (price % 144) / 12, price % 12,
...
{
    command("hmm");
    command("say These are the herbs I have in stock at the moment.\n");
}
