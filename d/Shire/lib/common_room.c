 ...
 *
 * To see how to implement this, look at /d/Shire/common/bree/inndining.c
 * It is really easy.
 */
...
// Change this to reflect where the food files are at
...
// configures the shire inn files.
    configure_shire_inn();
...
inn_hook_player_buys(object ob, string price, object *players)
{
  if (TP->query_prop("_is_sitting_down_AsMo_"))
    ...
    {
      write("You drop "+price+" onto the table and pick up "+
            ob->short()+".\n");
      tell_room(TO,QCTNAME(TP)+" drops "+price+" onto the bar "+
