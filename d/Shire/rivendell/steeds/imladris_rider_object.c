 ...
 *
 *
 * This object is carried by Imladris riders. It allows them
...
automount()
{
    object steed = this_player()->query_steed();
    
    if (sizeof(this_player()->query_team_others()) !=0 )
    {
        notify_fail("Automount can not be enabled while you lead a team.\n");
        return 0;
    }
    steed->do_auto_mount();
    return 1;
...
enter_env(object to, object from)
{
    ...
    add_action(automount, "automount");
   // add_action(autodismount, "autodismount");
    ...
    /* Saddlebag commands */
    add_action(hpack, "hpack"); // Show contents of pack to rider
}
...
Name: hboard()
Desc:  Buys two tickets and boards ship.
Args: None
Returns: int 1
*/
...
Desc: We over-rider Ranger's ability to stealth on/off
...
hhome()
{
    write("You need to dismount and lead your horse in order to send it home.\n");
    return 1;
}
...
Name: hpack()
Desc:  Shows content of saddlebag to driver of steed
...
hpack()
{
    ...
    if (saddlebag == 0)
    {
        write("You'll need a saddlebag strapped to your steed first.\n");
        return 1;
    }
...
Name: hhalt()
Desc:  Emote showing horse coming to a halt
Returns: int 1 if successful, 0 if fail
*/
...
Name: hpack()
Desc:  Shows content of saddlebag to either rider or leader
Returns: int 1 if successful, 0 if fail
*/
...
Name: hhelp()
Desc:  Shows help file to rider of horse
Returns: int 1 if successful, 0 if fail
*/
...
Name: hdrink()
Desc: Forces horse to "drink from <water source>"
Returns: int 1 if successful, 0 if fail
*/
