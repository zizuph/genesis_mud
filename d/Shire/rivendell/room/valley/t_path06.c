...
create_valley_room()
{
    ...
    set_extraline("Fruit tree branches hang over the path here " +
        "that follows the river bank. Water splashes noisily " +
        "into some pools in the river. The water boils up from " +
        "the pools before rushing on downstream. The path " +
        "meanders here past the house and gardens. The ground " +
    ...
    add_item(({"pools", "pool"}),
        "The pools in the river are deep and dark. The river " +
        "boils and churns as it pours into them and then flows " +
        "west.\n");
    ...
    add_exit(VALLEY_DIR + "t_path07",  "east");
    add_exit(VALLEY_DIR + "t_path05",  "west");
    reset_room();
}
