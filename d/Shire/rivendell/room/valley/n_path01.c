...
create_valley_room()
{
    ...
    set_extraline("You are between the stable and the herb shop.");
    ...
    add_item("stable",
        "The stable is a wooden structure with a gate facing " +
        "the house. You hear the sounds of horses within it.\n");

    add_exit(VALLEY_DIR + "n_path02", "north");
    add_exit(VALLEY_DIR + "v_path05", "south");
    add_exit(VALLEY_DIR + "herbshop", "east");
    add_exit(VALLEY_DIR + "stable",   "west");
...
block_room()
{
    if (this_player()->query_wiz_level()) return 0;
        ...
        write("The stables are closed temporarily while the elves make " +
        "some changes. They will be open soon.\n");
    return 1;
}
