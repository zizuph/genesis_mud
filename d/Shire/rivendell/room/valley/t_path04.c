...
create_valley_room()
{
    set_extraline("You stand at a bend in the river. It flows " +
        "northeast through the valley. The path wanders " +
        "beside the river. The air is filled with the sounds of " +
        "the water and the smells of the fruit trees. The " +
        "ground slopes towards the river.");
   ...
   set_fence("They are secured to posts that have " +
    ...
    add_exit(VALLEY_DIR + "t_path05",  "east");
    add_exit(VALLEY_DIR + "t_path03",  "northwest");
    reset_room();
}
