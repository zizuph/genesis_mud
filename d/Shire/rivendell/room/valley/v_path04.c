...
create_valley_room()
{
    ...
    set_extraline("You stand outside the stables. The " +
        "pungent smell of horses is strong yet clean. The " +
        "path heads southwest towards the front of the house and " +
        "east towards the forge.");

    add_item(({"stables", "stable"}),
        "The stables are north of you. The building is long " +
        "and rectangular, in the same style as the house.\n");
    add_item("path",
    ...
    add_exit(VALLEY_DIR + "v_path05", "east");
    add_exit(VALLEY_DIR + "v_path03", "southwest");
 
}
 
string short_desc()
{
    return "A path outside the stable";
}
