/*
 * Alternate path into Rivendell. Only Rangers and Noldor of good align
 * can use this way.
 *
 * Swim routine borrowed from Gondor, /d/Gondor/tharbad/ford/f04.c
 * By Finwe, July 2004
 */
...
create_shore_room()
{
    ...
    set_extraline("The shore drops abruptly into the river on the eastern " +
        "side of the river. Its banks are rocky and steep. The river flows " +
        "by swiftly.");

    add_exit(RIVER_DIR + "shore01", "west",0,4);
//    add_exit(RIVER_DIR + "path02", "northeast",0,4);
    add_item(({"riverbank", "ground", "shore", "down", "bank"}),
        "The shore is hard and rocky, and covered with trampled grass. " +
        "The sides drop suddenly into the river. Around the edge are " +
        "tall grasses, reeds, and rushes.\n");
...
cross_river(string arg)
{
	...
	TP->move_living("M", (RIVER_DIR + "path02"));
	if (TP->query_encumberance_weight() > 80)
	{
	    tired *= 2;
	}
