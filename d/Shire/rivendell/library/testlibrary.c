...
int add_librarian(string librarian);
int remove_librarian(string librarian);
...
"   +---------------------------------------------------------------+\n";
    ...
    if(this_player()->query_wiz_level())
    {
        result += "\nImmortals can also 'lock' or 'unlock' specific "  +
            "books for editing, and add and remove librarians. using " +
            "the commands ladd + name or lremove + name.            \n";
    }
...
//    init_library();
    ...
    //Allow immortals to add or remove librarians
    add_action(add_librarian,            "ladd");

    add_action(remove_librarian,           "lremove");
    add_action(remove_librarian,           "lrem");
...
int add_librarian(string librarian)
{
    int result=1;
    int negative=result;
    if (!stringp(librarian))
    {
       write("Syntax: ladd <name>\n");
        return negative;
    }
    write("You used the add_librararian routine syntaxically correct.\n");
    return result;
...
{  int result=1;
   ...
   if (!stringp(librarian))
    {
       write("Syntax: lrem(ove) <name>\n");
        return negative;
    }
    write("You used the remove_librararian routine syntaxically correct.\n");
