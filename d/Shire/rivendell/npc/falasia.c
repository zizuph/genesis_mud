/*
 * The squirrelkeeper of Imladris
 * Modified by Finwe
 * February 1998
 */
...
create_monster()
{
   if (!IS_CLONE) return;
   set_name("falasia");
   add_name(({"falasia","squirrelkeeper"}));
   set_living_name("");
   set_title("falasia, squirrelkeeper of Imladris");
   set_race_name("elf");
   ...
   add_act("say Would you like to hire a squirrel?");
    add_act("say Our squirrels are very efficent messengers.");
   add_act("emote peers at you.");
   add_act("smile happ");
   add_act("emote cleans a cage.");
   add_act("emote feeds some squirrels.");
...
my_long()
{
//    if (notmet_me(this_player())) this_player()->add_introduced("Falasia");
    return ("This is Falasia, the squirrelkeeper. She is " +
        "responsible for taking care of the squirrels that are " +
        "caged here. She is a gentle elf who carefully tends the " +
        "animals here.\n");
}
...
return_sigh(string who)
{
   ...
   command("say Would you like to send a message?\n");
   return 1;
}
