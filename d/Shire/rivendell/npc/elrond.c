 ...
 * -- Finwe, February 2002
 *
 * Added "heirloom" to ask_quest strings to trigger gem quest
 *   so it now matches the hint in the Tower of Realms.
 * Added "else" within give_back() so code works
 * -- 
 */
#include "/d/Shire/sys/defs.h"
#include "/d/Shire/rivendell/npc/local.h"
#include <macros.h>
#include <ss_types.h>
...
create_elf()
{
    ...
    set_long("He is tall and majestic with slight elvish " +
        ...
        "His face is ageless showing neither youth nor age but " +
...
//    MAKE_MONEY;
    ...
    add_ask(({"quest", "quests", "task", "tasks", "help"}), "@@ask_quest");
    add_ask(({"elladan", "elladan's patrol", "patrol"}), "@@ask_elladan");
    add_ask(({"gem", "missing gem", "jbs", "heirloom"}), "@@ask_gem");
...
string ask_quest()
{
    command("say which quest are you asking about, Elladan's patrol or the missing gem?");
    return "";
}
...
string ask_gem()
{
        if (TP->query_prop("_gem_quest_"))
        {
            command("say Have you found the missing gem yet? It is an ancient heirloom from th kings of Gondolin. Please find it and return it to me.");
            command("emote sighs deeply.");
            return "";
        }

        if (CHECK_QUEST(TP, GEM_QUEST))
        {
            command("say Thank you for your offer of assistance. You have already helped us locate the missing gem. Perhaps you can go on patrol and liberate the free people for us.");
            return "";
        }

        command("say In ages past, when elves and dwarves were friendly, we lived in Eregion. There, we fashioned great jewels from gems we found and gave them to our allies. When we learned of the deceit of Annatar and the War of the Elves and Sauron, many of our most priceless jewels were lost, including the Heart of Hollin. We have reports it became twisted and is now part of a weapon used by the Mages. Please find it and return it,");

        TP->add_prop("_gem_quest_", 1);
        command("say May the Valar protect and guide you.");
...
void test_found_gem_quest(object player, object obj)
{

    if (!REWARD_QUEST(player, GEM_QUEST))
    {
        command("say Thank you, but you have already found the gem for me.");
        return;
    }
    say(QCTNAME(player) + " gives something to Elrond.\n");
    command("emote looks inspects the jewel carefully.");
    command("emote smiles gladly.");
    command("emote rejoices!");
    command("say Thank you! This heirloom is priceless to us!");
    say(QCTNAME(player) + " gives something to Elrond.\n");
    tell_object(player, "Elrond rewards you. You feel more " +
        "experienced and enlightened.\n");
    say(QCTNAME(player) + " was rewarded.\n");
    player->remove_prop("_gem_quest_");
}
...
enter_inv(object obj, object from)
{
    ...
    else if(obj->id("_leech_jewel_"))
    {
        set_alarmv(3.0,0.0,"test_found_gem_quest",({from, obj}));
        return;
    }
    else
    {
