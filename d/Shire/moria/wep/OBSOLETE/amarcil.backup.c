...
CONSTRUCT_WEAPON
{
    ...
    add_prop(OBJ_I_LIGHT, "@@check_light_status@@");
    set_hands(W_RIGHT);
    FIXEUID;
    last_light_status = 0;
//    set_alarm(0.0, 10.0, "check_light_status");
}
...
{
    int delta = new_light - last_light_status;
    string str;
...
void
update_light_status(object ob = ETO)
{
    while (ENV(ob))
	ob = ENV(ob);
    ob -> update_light(1);
...
varargs int
check_light_status(int notify = 1)
{
    object *rooms;
    int i, evil = 0, sz, light_status;

    if (!query_wielded()) {
	last_light_status = 0;
	call_out("update_light_status", 0);
	return 0;
    }
	...
	if (evil < -2500) 
	    evil = -2500;
	light_status = -evil/500 + 1;
    }

    if (light_status != last_light_status) {
	call_out("update_light_status", 0);
	if (notify)
	    notify_status(light_status);
    }
...
my_long()
{
	...
	+ shining_text(check_light_status(0)) + "\n";
}
...
start_spell_msg(string arg)
{
    switch (spell_no) {
    case 0:
	tellem(TP, target, "$You $point Amarcil at #name, and $start releasing " +
	                   "${\"Sunfire\";a spell}!\n");
	return 1;
    default:
	return 0;
