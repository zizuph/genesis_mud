...
#define MIN_STR_2H      70
#define MIN_STR_1H     120
#define MIN_WIELD_SKILL 50

#define BAD_RACES ({ "troll", "orc", "uruk", "goblin", "half-troll", \
                     "peredhil", "yrch" })

#define MIN_INT_DWARF   60 /* Minimum inteligence required for dwarves. */
#define MIN_INT         90 /* Minimum inteligence required for others. */
#define MIN_WIS_DWARF   90 /* Minimum wisdom required for dwarves. */
#define MIN_WIS        120 /* Minimum wisdom required for others. */
...
{
    set_name(({"axe","waraxe"}));
    add_name("_axe_of_dain");
    set_short("@@my_short@@");
    set_pshort("waraxes");
    set_adj(({"dain's","dwarven","mithril","magical", "enchanted", "large"}));
    set_long("@@my_long");
    set_hit(30);
    set_pen(52);
	set_likely_dull(0);
...
do_wield(string str)
{
	...
	parse_command(str, all_inventory(TP),
		      "%i", items))
    {
	items = VIS_ACCESS(items, "wield_access", 1, 0);
	if (!sizeof(items))
	    return 0;
	what = items[0];
...
{
    return
	"You realize that this is Dain Ironfoot's Axe.\n    "+
	"Fashioned in Nogrod in the Blue Mountains during the late First Age, "+
	"Dain's Axe was used to slay the Orc-king Azog in the Battle of "+
	"Azanulbizar in Third Age 2799, when Dain was but a stripling. The "+
	"Dwarf-lord also used it in the Battle of the Five Armies and at his "+
	"death as Dain defended the body of his slain Mannish friend, King Brand "+
	"of Dale, from desecration.\n";
}
...
outlook_desc()
{
	...
	return ("You are looking at a marvelous dwarven waraxe with a twinkling " +
		...
		"edge. The sharp edge is semi-arced 18 cm (6\") with a thick cleaving " +
		"section. The blunt edge however consists of a large head with a terrible " +
		"mythical monster inscribed facing the opponent.\n");
    else
	return ("The waraxe is crafted in a typical dwarven fashion with a head "+
		...
		"mysteriously around you.  The head has one slicing and one crushing "+
		"edge. The sharp edge is semi-arced 18 cm (6\") with a thick cleaving "+
		"section typically invented for armour penetration. The blunt edge however "+
		"consists of a large head with a horrible mythical monster inscribed "+
		"facing the opponent, obviously having a scaring effect.\n");
}
...
my_short()
{
    int broken = query_prop(OBJ_I_BROKEN);
    
    if (HAS_IDED(TP))
	if (broken)
	    return "broken waraxe";
        else
	    return "Dain Ironfoot's waraxe";
    else
	if (broken)
	    return "broken dwarven waraxe";
	else
...
did_hit(int aid,string hdesc,int phurt,object enemy,int dt,int phit,int dam)
{
    ...
    if (member_array(enemy->query_race_name(), BAD_RACES) >= 0)
    {
	if (dam <= 0 && !random(10))
    ...
    if (enemy->query_race_name() == "orc" ||
         enemy->query_race_name() == "troll" ||
         enemy->query_race_name() == "goblin")
	{
	  if (TO->query_attack_id() == W_BOTH)
	  {
		...
		enemy->catch_msg(QCTNAME(tp)+"'s dwarven waraxe glows pale "+
		  "red as "+HE_SHE(tp)+" firmly hold it in both "+
                  HIS_HER(tp)+" hands!\n");
		tell_watcher(QCTNAME(tp)+"'s dwarven waraxe glows pale "+
	  ...
	  else
	  {
		...
		tell_watcher(QCTNAME(tp)+"'s dwarven waraxe glows palely "+
		  "red as "+HE_SHE(tp)+" firmly holds it in "+HIS_HER(tp)+
                  " right hand!\n");
	  }
    }
