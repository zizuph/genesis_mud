 ...
 *
 * Changed:
 * 2007-01-07 - Cotillion
 * - Fixed an issue with the mana removal which allowed players to 
 *   use the pike to regain mana.
 *
 * Navarre: December 23th 2007.
 * As requested by Gorboth after a correspondance with him, I have agreed to change this weapon.
 * The following Changes were made:
 * - Removal of 25 speed bonus. Speed weapons combined with a special attack is not well liked.
 * - Limited to increase wielders defence and parry, instead of at the same time lowering the enemy's defence and parry.
 * - Special attack moved from being executed when an attempt to hit the enemy is performed to when the enemy is hit.
 *
...
create_weapon()
{
    ...
    set_long("This simple, yet elegantly made pike " +
             "appears to be roughly half-again longer " +
             "than a man is tall, which actually seems " +
             "a bit short for a normal pike. Its haft " +
             "is made of polished oak, strong and a bit " +
             "heavy. The tip is a work of wonder, being " +
             "mithril worked into a simple spike with " +
             "rather fierce-looking barbs at the base " +
             "pointing back down the haft. At first glance " +
             "its surface appears smooth, but as you gaze " +
             "intently, shapes and patterns seemingly form " +
             "and reform along the metal. The longer you gaze, " +
             "the more pronounced the shapes become, and an " +
             "intense pressure starts to form in your head.\n");
	
	add_item(({"shapes", "patterns", "shapes and patterns"}),
                 "You have no hope of fully comprehending the shapes " +
                 "and patterns forming on the mithril tip; they form " +
                 "and reform too fast for you to keep them in your mind " +
                 "long enough to recognize anything.\n");
	add_item("haft", "The haft seems to be made of oak. Roughly half " +
                 "again as long as a man is tall, and little more than two " +
                 "fingers in width. It is flexible, but amazingly strong.\n");
	add_item(({"tip", "mithril tip"}), "The mithril tip is beautiful in " +
                 "its modesty. All of a foot long, the spike comes to a sharp " +
                 "point, with equally sharp barbs pointing back down the haft.\n");
	add_item("spike", "The spike is roughly two handspans long from where " +
                 "the tip is fastened to the haft of the pike. It tapers to a point " +
                 "so sharp, it seems only the strongest armour of the self-same " +
                 "metal could possibly turn the spike aside.\n");
	add_item("barbs", "Roughly a handspan in length, the barbs point in the " +
                 "opposite direction of the spike. It appears that if the pike " +
                 "were driven far enough into the victim, pulling it back out " +
...
/* Kender prop, allow alter ability to change the descriptions of the weapon */
    ...
    add_prop(MAGIC_AM_ID_INFO,({"This pike appears to have some sort of magical intelligence.\n",1,
                                 "Its effectiveness seems to be dependant on the skill and " +
                                 "intelligence of the user, and seems to be a rather speedy weapon.\n",25,
                                 "It seems this pike has a rudimentary intelligence, deciding in " +
                                 "battle whether it might lunge suddenly and hurt its opponent greatly, " +
                                 "or perhaps trip the opponent up, thus making him less effective in a fight.\n",65 }));
...
fatiguedrain()
{
    ...
    if (p->query_fatigue() < FATIGUE)
    {
        ...
        say("About to faint from exhaustion, " + QCTNAME(p) + " shifts " + HIS_HER(p) + " grip on " +
            HIS_HER(p) + " " + short() + ", feeling a little relief.\n",p);
...
int focus_tactics(string str)
{
	...
	else if(str == "on defence" || str == "defence")
	{
		...
		if(TP->query_prop(PIKE_DEF))
		{
			write("A faint voice sounds in your mind: 'But you are already " +
                              "focusing on defence. Perhaps you wish to focus on offence, or " +
                              "ease off this defensive stance.'\n");
			return 1;
		}
		...
		write("A faint voice sounds in your mind: 'Have no fear, I shall protect you against " +
                      "your foes!'\nYou shift your grip on the " +
                      this_object()->query_name() + " slightly, preparing to take defensive " +
                      "measures. You can 'ease off' this stance if you need to rest.\n");
	...
	else if(str == "on offence" || str == "offence")
	{
		...
		if(TP->query_prop(PIKE_OFF))
		{
			write("A faint voice sounds in your mind: 'But you are already focusing on " +
                              "offence. Perhaps you wish to focus on defence, or ease off this offensive stance.'\n");
			return 1;
		}
		...
		write("A faint voice sounds in your mind: 'Yes! YES!! ATTACK!! I shall guide you, and you " +
                      "shall not fail!'\nYou shift your grip on the " +
                      this_object()->query_name() + " slightly, preparing for an all out " +
                      "offensive. You can 'ease off' this stance if you need to rest.\n");
		say(QCTNAME(TP) + " shifts " + HIS_HER(TP) + " grip on " + HIS_HER(TP) + " " + short() + ".\n");
		return 1;
	}
	...
	{
		write("A faint voice sounds in your mind: 'How is it you want me to help you? " +
                      "Focus on either offence or defence.'\n");
		return 1;
	}
}
...
int return_normal(string str)
{
	...
	if(str != "off")
	{
		write("A faint voice sounds in your mind: 'What? Do you wish to 'ease off' " +
                      "your current stance?'\n");
		return 1;
	}
	...
	write("A faint voice sounds in your mind: 'Yes, of course. One can not push to the limit " +
              "at all times... Rest now... Rest.'\nYou ease off your grip on the " +
              this_object()->query_name() + ", taking a " +
              "moment to rest.\n");
	say(QCTNAME(TP) + " eases " + HIS_HER(TP) + " grip on " + HIS_HER(TP) + " " + short() +
            ", looking a little less strained.\n");
...
wield(object obj)
{
    ...
    if((TP->query_skill(SS_WEP_POLEARM) >= 90) && (TP->query_alignment() >= 460) && (TP->query_stat(SS_INT) >= 125))
    {
	write("As you take a firm grip on the " + this_object()->query_name() + ", a faint voice sounds " +
              "in your mind: 'At " +
              "last, one who is worthy of the power I hold!'\nA faint voice sounds in your mind: " +
              "'I can grant you great success in battle. Should you wish to fight more effectively, you " +
              "might try to 'focus' on defence or offence.'\n");
	say(QCTNAME(TP) + " looks as if " + HE_SHE(TP) + " is listening intently to something... " +
            "To what you can not be certain.\n" + capitalize(HE_SHE(TP)) + " takes a firm grip on " +
            HIS_HER(TP) + " " + short() + ".\n");
	
	TP->add_mana(-(TP->query_max_mana() / 4));
	TP->add_prop(PIKE_NRM, 1);
	return 1;
    }
    else
...
unwield(object obj)
{
	...
	write("A faint voice sounds in your mind: 'Our work may be done for now, but there is much " +
              "evil left in the world, remember me when the time comes to take up the fight again!'\n" +
              "You regretfully release your hold on the " +
            this_object()->query_name() + ".\n");
	say(QCTNAME(TP) + " gets a distant, almost regretful, look in " + HIS_HER(TP) + " eyes as " +
            HE_SHE(TP) + " releases " + HIS_HER(TP) + " " +
            this_object()->query_name() + ".\n");
    ...
    if ((random(100) < (skill/2)) && (random(skill+stat) > random(skill_e+stat_e)))
    {
        if(random(2))
        {
            ...
            if (enemy->query_hp() <= 0)
            {
                ...
                me->catch_msg("With lightning fast speed, you plunge your " + short() + " deep into " +
                              QTNAME(enemy) + "!\n" + QCTNAME(enemy) + " dies in an overflowing pool of blood!\n");
                enemy->catch_msg("With unbelievable speed, " + QCTNAME(me) + " plunges " + HIS_HER(me) +
                                 " " + short() + " deep into your body!\nAs blood flows freely from the " +
                                 "open wound, you feel your life passing.\n");
                me->tell_watcher("With unbelievable speed, " + QCTNAME(me) + " plunges " + HIS_HER(me) +
                                 " " + short() + " deep into " + QTNAME(enemy) + "!\n" + QTNAME(enemy) +
                                 " dies in an overflowing pool of blood!\n");
                enemy->do_die(me);
                return 0;
            }
            
            me->catch_msg("With suprising speed and accuracy, you pierce " + QTNAME(enemy) + " with your " +
                          short() + "!!\nRipping it back out, the barbs wound " + HIM_HER(enemy) + " " +
                          ptext + "!\n");
            enemy->catch_msg("With suprising speed and accuracy, " + QCTNAME(me) + " pierces you with " +
                             HIS_HER(me) + " " + short() + "!\nAs " + HE_SHE(me) + " rips it back out, " +
                             "the barbs wound you " + ptext + "!\n");
            me->tell_watcher("With suprising speed and accuracy, " + QCTNAME(me) + " pierces " + QTNAME(enemy) +
                             " with " + HIS_HER(me) + " " + short() + "!\nAs " + QCTNAME(me) +
                             " rips it back out, the barbs wound " + QTNAME(enemy) + " " + ptext + "!\n");
