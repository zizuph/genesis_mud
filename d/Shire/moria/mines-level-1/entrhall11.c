...
CONSTRUCT_ROOM {
    ...
    set_long(
	"You are standing in the north western corner of the huge Western Entry Hall.\n" +
	"A stairway winds upwards in the west here, and the northern part of the Hall " +
	"continues eastwards towards another exit. The hall itself is in utter darkness" +
	"@@desc_columns@@ ");
    add_item(({ "wall", "walls"}),
	     "Which wall, the north, south or west wall?");
    add_item(({ "west wall", "western wall", "stairway", "stairs" }),
	     "The wall seems to be of solid granite, and you can see the winding stairway "+
	     "wind upwards in that direction.");
...
int do_s(string arg)
{
    if (strlen(arg))
	return 0;
    TP->move_living("south", THIS_DIR + "entrhall21");
    return 1;
}
