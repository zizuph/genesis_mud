...
produce_msg(object enemy, string hdesc, int phurt, int dt, string short)
{
	...
	case W_IMPALE:
	    ...
	    tp->tell_watcher(QCTNAME(tp)+"'s "+short+" impales "+QTNAME(enemy)+
	...
	case W_SLASH:
	    ...
	    tp->tell_watcher(QCTNAME(tp)+" swings "+HIS_HER(tp)+" "+short+
	...
	case W_BLUDGEON:
	    ...
	    enemy->catch_msg(QCTNAME(tp)+"'s "+short+"  brutally crushes your "+
	      ...
	      "nothing.\n");
	    tp->tell_watcher(QCTNAME(tp)+"'s "+short+"  brutally crushes "+
    ...
    case -2:
// parries hit
    if (!tp->query_option(OPT_GAG_MISSES))
		tp->catch_msg("You block "+QTNAME(enemy)+" attack with your "+short+".\n");
	    if (!enemy->query_option(OPT_GAG_MISSES))
		enemy->catch_msg(QCTNAME(tp)+" blocks your attack with "+HIS_HER(tp)+" "+short+".\n");
	    tp->tell_watcher_miss(QCTNAME(tp)+" blocks "+QTNAME(enemy)+
	      "'s attack with "+HIS_HER(tp)+" "+short+".\n",enemy);
	    break;

	case -1:
// dodges hit
	    if (!tp->query_option(OPT_GAG_MISSES))
		tp->catch_msg("You stab the air next to "+QTNAME(enemy)+
		  " with your "+short+".\n");
	    if (!enemy->query_option(OPT_GAG_MISSES))
		enemy->catch_msg(QCTNAME(tp)+" stabs the air next to you "+
		  "with "+HIS_HER(tp)+" "+short+".\n");
	    tp->tell_watcher_miss(QCTNAME(tp)+" stabs the air next to "+QTNAME(enemy)+
	      " with "+HIS_HER(tp)+" "+short+".\n",enemy);
	    break;
	case 0:
	    ...
	    tp->tell_watcher(QCTNAME(enemy)+" giggles as "+QTNAME(tp)+" harmlessly "+
	...
	case 1..5:
	    ...
	    enemy->catch_msg(QCTNAME(tp)+" punctures your "+hdesc+" with "+
	      HIS_HER(tp)+" "+short+".\n");
	    tp->tell_watcher(QCTNAME(tp)+" punctures "+QTNAME(enemy)+"'s "+hdesc+
	      " with "+HIS_HER(tp)+" "+short+".\n",enemy);
	    break;
	case 6..10:
	    ...
	    enemy->catch_msg(QCTNAME(tp)+" impales your "+hdesc+" with "+
	      HIS_HER(tp)+" "+short+".\n");
	    tp->tell_watcher(QCTNAME(tp)+" impales "+QTNAME(enemy)+"'s "+hdesc+
	      " with "+HIS_HER(tp)+" "+short+".\n",enemy);
	    break;
	case 11..20:
	...
	case 21..40:
	    ...
	    tp->tell_watcher(QCTNAME(tp)+" gives "+QTNAME(enemy)+" a shallow "+
	...
	case 41..60:
	    ...
	    tp->tell_watcher(QCTNAME(tp)+" gives "+QTNAME(enemy)+" a painful "+
	...
	case 61..80:
	    ...
	    enemy->catch_msg("With a staggering performance, "+QTNAME(tp)+
	      ...
	      "through your "+hdesc+"!\n");
	    tp->tell_watcher("With a staggering performance, "+QTNAME(tp)+
	      " spins "+HIS_HER(tp)+" "+short+" and yields a puncture "+
	...
	case 81..90:
	    ...
	    tp->tell_watcher(QCTNAME(tp)+" deals a deep, piercing blow into "+
	...
	case -2:
	    if (!tp->query_option(OPT_GAG_MISSES))
		tp->catch_msg("You swing your "+short+", blocking "+
		  QTNAME(enemy)+"'s attack.\n");
	    if (!enemy->query_option(OPT_GAG_MISSES))
		enemy->catch_msg(QCTNAME(tp)+" swings "+HIS_HER(tp)+" "+short+
		  " around you, blocking your hits.\n");
	    tp->tell_watcher_miss(QCTNAME(tp)+" swings "+HIS_HER(tp)+" "+short+
	      " around "+QTNAME(enemy)+" while blocking the hits.\n",enemy);
	    break;
	...
	case 0:
	    ...
	    tp->tell_watcher("The air whistles as "+QTNAME(enemy)+" narrowly avoids "+
	      QTNAME(tp)+"'s "+short+"'s dangerous slash.\n",enemy);
	    break;
	case 1..5:
	    ...
	    tp->tell_watcher(QCTNAME(tp)+"'s "+short+" touches "+QTNAME(enemy)+
	...
	case 6..10:
	    ...
	    enemy->catch_msg(QCTNAME(tp)+" cuts your "+hdesc+" neatly with "+
	      HIS_HER(tp)+" "+short+".\n");
	    tp->tell_watcher(QCTNAME(tp)+" cuts "+QTNAME(enemy)+"'s "+hdesc+ 
	      " neatly with "+HIS_HER(tp)+" "+short+".\n",enemy);
	    break; 
	case 11..20:
	    ...
	    tp->tell_watcher(QCTNAME(tp)+" performs a nice swing with "+HIS_HER(tp)+
	...
	case 21..40:
	    ...
	    tp->tell_watcher(QCTNAME(tp)+" slashes "+QTNAME(enemy)+"'s "+hdesc+ 
	...
	case 41..60:
	    ...
	    tp->tell_watcher(QCTNAME(enemy)+" is badly hurt as "+QTNAME(tp)+
	...
	default:
	    ...
	    tp->tell_watcher("With a mighty slash of "+QTNAME(tp)+"'s "+short+
	...
	case -2:
	    if (!tp->query_option(OPT_GAG_MISSES))
		tp->catch_msg("You swing your "+short+", catching and parrying the attacks,\n");
	    if (!enemy->query_option(OPT_GAG_MISSES))
		enemy->catch_msg(QCTNAME(tp)+" swings "+HIS_HER(tp)+" "+short+
		  ", catching and parrying the hits.\n");
	    tp->tell_watcher_miss(QCTNAME(tp)+" swings "+HIS_HER(tp)+" "+short+
	      ", catching and parrying the hits.\n",enemy);
	    break;
    ...
    case -1:
	    if (!tp->query_option(OPT_GAG_MISSES))
		tp->catch_msg("You swing around your "+short+", hitting nothing but "+
		  "thin air.\n");
	    if (!enemy->query_option(OPT_GAG_MISSES))
		enemy->catch_msg(QCTNAME(tp)+" swings "+HIS_HER(tp)+" "+short+
		  ", hitting nothing but thin air.\n");
	    tp->tell_watcher_miss(QCTNAME(tp)+" swings "+HIS_HER(tp)+" "+short+
	      ", hitting but thin air.\n",enemy);
	    break;
	case 0:
	    ...
	    tp->tell_watcher(QCTNAME(enemy)+" laughs mockingly as "+QTNAME(tp)+
	...
	case 1..5:
	    ...
	    enemy->catch_msg(QCTNAME(tp)+" lightly taps your "+hdesc+" with "+
	      HIS_HER(tp)+" "+short+".\n");
	    tp->tell_watcher(QCTNAME(tp)+" lightly taps "+QTNAME(enemy)+"'s "+
	      hdesc+" with "+HIS_HER(tp)+" "+short+".\n",enemy);
	    break;
	case 6..10:
	    ...
	    tp->tell_watcher(QCTNAME(tp)+" almost unbalances "+QTNAME(enemy)+
	...
	case 11..20:
	    ...
	    tp->tell_watcher(QCTNAME(tp)+" performs a heavy blow with "+ HIS_HER(tp)+
	...
	case 21..40:
	    ...
	    tp->tell_watcher(QCTNAME(enemy)+" staggers as "+QTNAME(tp)+" crushingly "+
	      "hammers "+HIS_HER(tp)+" "+short+" upon "+QTNAME(enemy)+
	      "'s "+hdesc+".\n",enemy);
	    break;
	case 41..60:
	    tp->catch_msg("You crush "+QTNAME(enemy)+"'s "+hdesc+
	      " with your "+short+".\n");
	    enemy->catch_msg(QCTNAME(tp)+" crushes your "+hdesc+
	      " with "+HIS_HER(tp)+" "+short+".\n");
	    tp->tell_watcher(QCTNAME(tp)+" crushes "+QTNAME(enemy)+
	      "'s "+hdesc+" with "+HIS_HER(tp)+" "+short+".\n",enemy);
	    break;
	case 61..80:
	    tp->catch_msg("You smash your "+short+" on "+QTNAME(enemy)+"'s "+
	      hdesc+", with a bone crushing sound.\n"); 
	    enemy->catch_msg(QCTNAME(tp)+" smashes "+HIS_HER(tp)+" "+short+
	      " on your "+hdesc+", with a bone crushing sound.\n");
	    tp->tell_watcher(QCTNAME(tp)+" smashes "+HIS_HER(tp)+" "+short+
	      " on "+QTNAME(enemy)+"'s "+hdesc+", with a bone "+
	      "crushing sound.\n",enemy);
	    break;
	case 81..90:
	    ...
	    enemy->catch_msg(QCTNAME(tp)+" breaks several of your bones as "+
	      ...
	      hdesc+".\n");
	    tp->tell_watcher(QCTNAME(tp)+" breaks several of "+QTNAME(enemy)+"'s "+
	      "bones as "+HE_SHE(tp)+" slashes "+HIS_HER(tp)+" "+short+
	...
	default:
	    ...
	    tp->tell_watcher(QCTNAME(tp)+"'s "+short+" almost crushes "+
