...
{
    if (web_here) return
	"The eastern exit is blocked by a thick web! It seems to "+
	...
	"shiver of the thought of entering that way.";
}
...
exa_east()
{
	...
	"from there. You shiver in the thought of what might be in there.";
}
...
int do_burn(string what)
{
    ...
    web_here = 0;
    tell_room(TO,
	"Suddenly, the web flames up and in seconds the web disappears!!\n");
    return 1;
}
*/
int
do_burn(string what)
{
	...
	notify_fail("You need a torch or something with fire in it!\n");
	if(!(torch = present("torch", TP)) || !(torch->query_prop(OBJ_I_HAS_FIRE)) ||
		!present(torch->short("torch"), TP))
		return 0;
	
	notify_fail("You need to put flames into it first!\n");
	if((torch = present("torch", TP) && torch->query_prop(OBJ_I_LIGHT) == 0) ||
		(torch->short("torch") && torch->query_prop(OBJ_I_LIGHT) == 0))
		return 0;

	if((torch = present("torch", TP) && 
	    torch->query_prop(OBJ_I_LIGHT)>0 && torch->query_prop(OBJ_I_HAS_FIRE)) ||
		(torch->query_prop(OBJ_I_LIGHT)>0 && torch->query_prop(OBJ_I_HAS_FIRE)))
	{
		web_here = 0;
		tell_room(TO, "Suddenly, the web flames up and in seconds the web "+
		"disappears!!\n");
		return 1;
	}
