...
#define RNAMES ({\
    "umbathar", "lifstak", "snaga", "bindaram", "blunt", \
    "grom", "grim", "losh", "twam", "yarm",\
    "stycala", "rashk", "twuk", "unjim", "karuk",\
    "tuk-tuk", "gorr", "untuk", "tum-takarash", "snorr",\
    "stukuktur", "rumpatuk", "brutashg", "falas", "garhuk"\
})

#define RADJ ({ \
"devil-eyed", "skinny",	       "pathetic",	"ordinary",	 "blunt",\
"misformed",  "grim pale",     "evil-looking",	"unmerciful",	 "rotten-smelling",\
"puking",     "trash-smelling","odd-looking",	"deadly-looking","spitting",\
"squint-eyed","sniffing",      "crazy-looking","master",	 "red-squintingly eyed",\
"nude-loving","big-butted",    "brutal",	"thinking",	 "stubborn"})
...
#define RLONG ({ \
    "looks like he has got the devils eyes.",\
    "looks skinny, but the more deadly.",\
    "looks pathetic, but evil the more.",\
    "looks quite ordinary by the looks.",\
    "looks kind of blunt in appearance.",\
    "has got teeth pointing everywhere.",\
    "has a grim look on his pale face.",\
    "looks evil, very evil..",\
    "looks evil and unmerciful.",\
    "smells awful having all kinds of rotten stuff around him.",\
    "has a smell of puke on him!",\
    "smells like trash.",\
    "seems quite odd, but evil.",\
    "seems like a deadly fellow.",\
    "spits around all the time.",\
    "is very squint-eyed.",\
    "sniffs all the time.",\
    "looks like a real lunatic. Watch out!",\
    "seems like a boss! Don't mess with him.",\
    "looks awful with red squinted eyes to match his long arms.",\
    "constantly drops his jaw out of its place.",\
    "has a remarkable large butt.",\
    "looks really brutal and large.",\
    "looks rather displaced and walks thinking around.",\
    "looks like a really stubborn orc."\
})
...
create_troll_soldier(int r)
{
    ...
    set_race_name("orc");
    set_long(
	"You are looking at a "+RADJ[r]+" orc soldier!\n"+
	"This is one orc of a tribe of Moria-orcs living in the western parts "+
	"of Moria, called the Mines of Moria. You can see that by his sign on "+
	"the shield, a red claw. He "+RLONG[r]+" He looks very evil.\n");
...
arm_me()
{
    ...
    switch(m) {
	case 0:  wep = "orcaxe"; break;
	case 1:  wep = random(2) ? "orcfalch" : "orcscim"; break;
	case 2:  wep = "orcmace"; break;
	default: wep = ""; break;
    }

    if(strlen(wep))
    {
	clone_object(MORIA_WEP+wep)->move(TO);
