 ...
 * Description   : Sets up the new hoopak
 * Arguments     : object staff - the staff to use
 */
void
make_hoopak(object staff)
{
    int mod_hit, mod_pen;
    object hoopak;
    ...
    hoopak = clone_object(KGOBJ + "std_hoopak");

    mod_hit = (staff->query_hit() - 5 +
      (this_player()->query_skill(SS_HOOPAK_ALTER) / 10));
    mod_pen = (staff->query_pen() - 5 +
      (this_player()->query_skill(SS_HOOPAK_ALTER) / 10));
   
    hoopak->add_prop(OBJ_I_WEIGHT, staff->query_prop(OBJ_I_WEIGHT));
    hoopak->add_prop(OBJ_I_VOLUME, staff->query_prop(OBJ_I_VOLUME));
    hoopak->add_prop(OBJ_I_VALUE, staff->query_prop(OBJ_I_VALUE));
    hoopak->add_prop(OBJ_S_WIZINFO, "This hoopak was altered from " +
        "its original form from " + file_name(staff) + ".\n");
    hoopak->set_likely_break(staff->query_likely_break());
    hoopak->set_hands(staff->query_hands());

    hoopak->change_hoopak(mod_hit, mod_pen, staff->short());

    if (staff->may_not_recover())
        hoopak->set_may_not_recover();
    else
        hoopak->set_may_recover();

    if(staff->query_prop(OBJ_I_IS_MAGIC_WEAPON))
    {
        hoopak->add_prop(OBJ_I_IS_MAGIC_WEAPON, 1);
        hoopak->add_prop(MAGIC_AM_ID_INFO,
            ({ "This hoopak was altered from a magical staff", 20 }));
        hoopak->add_prop(MAGIC_AM_MAGIC,
            staff->query_prop(MAGIC_AM_MAGIC));
    }

    write("You sit down to work on the " + staff->short() + ".\n");
    write("You successfully alter the " + staff->short() +
        " into an unadorned plain hoopak.\n");
    say(QCTNAME(this_player()) + " sits down to work on the " + 
        staff->short() + ".\n");

    hoopak->move(this_player());
    staff->remove_object();
}
