 ...
 *
 * Put check in so player is at least a beginner before joining. 
 * Smaller players are creating errors when forging necklace for
 * the first time.
 * -- Finwe, March 2004
 *
 */
...
#define MAKE_NECKLACE   "_smith_task_join_"
...
create_monster()
{
   ...
   add_ask(({"join","membership"}), QCTNAME(TO)+" tells you: If you " +
        "want to join then you must be at least a beginner. We don't " +
        "have time for small fry running around, knocking over gear " +
        "and making a mess. Next, you must buy an iron bar from me " +
        "and prove you can forge a necklace. Once you have done that, " +
        "return  here so I may inspect your work. If you have done " +
        "well, I might accept you into the guild.\n");
   add_ask(({"buy","iron bar","iron bars"}),QCTNAME(TO)+" tells you: I sell iron bars for "+IRON_COST+" copper coins each.\n");
   add_ask(({"necklace"}),QCTNAME(TO)+" tells you: To make a necklace you"+
      " will need to purchase an iron bar.\n");
...
pay(string str) {
   ...
   {
        if (TP->query_average_stat() < 24)
        {
            TO->command("say You must become bigger before you can " +
            "become a Blacksmith. Come back to me when you have grown.");
         return 1;
        }
            
      if (!take_money(TP,IRON_COST)) {
         TO->command("say You don't have enough money to pay me. Come " +
         "back when you have earned some more.");
         return 1;
      }
      else 
      {
        if (!IS_MEMBER(TP))
        {
            // player making necklace for first time.
            TP->add_prop(MAKE_NECKLACE,1);
        }
...
/* ** emergency fix until I can make something a little more elegant ** */
    ...
    if ((item->short()=="unworn iron necklace") || (!check_on))
    {
        if (check_on)
        {
            if (TP->query_prop(!MAKE_NECKLACE)) 
            {
                TO->command("say Hmm...I'm not sure how you did this, but " +
                "you did not have my permission. I cannot accept you at " +
                "this time.");
                TP->remove_prop(MAKE_NECKLACE);
                return 1;
            }
...
// logs the join
    log_guild(extract(ctime(time()), 4, 15) + " " +
        extract(ctime(time()), 21, 4)+"     " + WRITE_NAME(TP) + 
        " joined the Blacksmiths.\n");
