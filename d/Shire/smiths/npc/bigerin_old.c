...
pay(string str) {
   ...
   if (str=="bigerin") {
      if (!take_money(giver,LEATHER_COST)) {
...
give(object newitem, object towho) {
   ...
   if (check_leather(newitem)) {
      ...
      LEATHER_COST+" copper coins to get something useful from this.");
      command("say to "+towho->query_name()+" Just <pay bigerin> to pay me "+
      "or <refuse bigerin> to get your thing back.");
      item=newitem;
      busy=1;
   }
   else {
...
check_leather(object newitem)
{
  ...
  //        return 0;
   ...
   {
        if (strlen(function_exists("query_race", newitem)))
...
give_back(object towho, object obj)
{
    ...
    {
        towho->catch_msg("Bigerin gives it to you.\n");
        say("Bigerin gives something to "+QNAME(towho)+" .\n");
    }
    else
        return success;
}
