 ...
 *
 * This is Bree blacksmith. He repairs armours.
 * Palmer 17 June 2006
 * Snarfed from Calmacil's code by Lord Elessar Telcontar.
 *
 */
...
/*
 * Function name:	create_shire_monster
...
create_shire_monster()
{
    ...
    add_name("smith");
    set_race_name("human");
    set_living_name("anvilin");
    set_title("Master Armourer");
    set_adj(({"sweaty","strong"}));
    set_long(BSN("This blacksmith can repair your armours if you give them to him..")); 
    ...
    default_config_npc(45 + random(6));

    set_base_stat(SS_CON, (55 + random(6)));
    set_base_stat(SS_STR, (85 + random(6)));

    set_skill(SS_WEP_CLUB, (65 + random(6)));

    set_tell_active(1);

    set_chat_time(8);
    add_chat("If you'd like me to repair your armour, give it to me and say 'Fix it for me'.");
...
catch_say(string what)
{
    ...
    if (sscanf(what, "%sive it back%s", dummy, dummy) == 2)
    {
        if (!pointerp(Fixing) ||
	    !sizeof(Fixing))
        {
...
hammer_noise(string what)
{
	...
	(strlen(what) ? what : "a glowing piece of iron")+".\n");
} /* hammer_noise */
...
{
  tell_room(environment(TO),
	QCTNAME(TO)+" wipes the sweat off his forehead.\n");
} /* sweat_action */
...
cool_noise(string what)
{
  tell_room(environment(TO),
	QCTNAME(TO)+" dips "+
	(strlen(what) ? what : "a glowing piece of iron")+" in a bucket of water.\n"+
...
heat_noise(string what)
{
  tell_room(environment(TO),
	QCTNAME(TO)+" puts "+
	(strlen(what) ? what : "a piece of iron")+" into the hot forge,\n"+
...
add_introduced(string name)
{
     ...
     */
    if (objectp(pl = present(name, environment())) &&
	interactive(pl) &&
...
do_intro(object pl)
{
    ...
    if (objectp(pl) &&
	environment(pl) == environment(TO) &&
        CAN_SEE_IN_ROOM(TO) &&
	CAN_SEE(TO, pl))
    {
	name = pl->query_name();
	command("say Welcome to my workshop, "+name+"!");
