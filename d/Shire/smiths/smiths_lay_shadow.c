...
query_guild_keep_player(object shadow_who)
{
    ...
    scroll = clone_object("/d/Shire/guild/smiths/obj/transfer_scroll");
    if (scroll->move(TP)!=0) {
        write("Please mail Palmer, the Smith's Guild Master. You are bugged and could not transfer to craft slot.\n");
     }


    write("*** IMPORTANT Blacksmith Announcement ****************************************\n");
    write("*** \n");
    write("*** \n");
    write("The Blacksmiths are now a Craft style guild. Please read your scroll.\n");
    write("*** \n");
    write("*** \n");
    guild_exp = shadow_who->query_acc_exp(SS_LAYMAN);
    SMITHS_TRANSFER_OBJ->add_player(shadow_who->query_name(), guild_exp);

    if (objectp(item = present("smiths_necklace", shadow_who)))
        item->remove_object();

    shadow_who->remove_guild_lay();
    shadow_who->set_guild_pref(SS_LAYMAN, 0);
    shadow_who->clear_guild_stat(SS_LAYMAN);
    shadow_who->remove_cmdsoul(SMITHS_LAY_SOUL);
    shadow_who->update_hooks();
    shadow_who->setup_skill_decay();
    log_membership(shadow_who->query_name() +" added to the transfer mapping with "+ guild_exp +" lay exp.\n");
    return 0;
}
...
add_soul()
{
    shadow_who->add_cmdsoul("/d/Shire/smiths/smiths_lay_soul");
    shadow_who->update_hooks();
}
