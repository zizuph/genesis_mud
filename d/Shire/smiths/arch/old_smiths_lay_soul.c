...
sminspect(string str) {
   ...
   if (function_exists(CHECK_WEAPON, obj) == STD_WEAPON) {
      ...
      if ((tohit>39) && (tohit<50))
         ...
         " weapon.\n");
      if (tohit>49)
         write("With this weapon almost every strike would be a killing"+
         " blow.\n");
...
smlaugh(string str) {
    ...
    notify_fail("Laugh at whos weapon?\n");
    if (!strlen(str)) {
	return 0;
    }
...
smlist()
{
    ...
    if (!sz)
    {
        NF("There are no Smiths present right now.\n");
        return 0;
    }
 
    if (sz == 1)
    {
        write("You are the only Smith present.\n");
        return 1;
    }

    write("---------------------------------------------------"+
        "---------------------\n"+
        "                        "+sz+" Smiths roam the lands:\n"+
        "---------------------------------------------------"+
        "---------------------\n");
    for (i = 0; i < sz; i++)
    {
...
show_subloc(string subloc, object on_obj = TP, object for_obj = TP)
{
    if ((on_obj->query_prop(TEMP_SUBLOC_SHOW_ONLY_THINGS)) ||
      (subloc != SMITHS_SUBLOC))
    {
	return on_obj->show_subloc(subloc, on_obj, for_obj);
    }

    switch(on_obj->query_skill(SS_FORGE_XP)) {
    case 0:
	xp = " New";
	break;
    case 1:
    ...
    if (on_obj->query_skill(SM_MASTER)==1) {
	xp += " Master";
    }
    else {
	xp += " Member";
