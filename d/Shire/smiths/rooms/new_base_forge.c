...
void do_breeze();
public mixed thieves();
void remove_forge_alarms();

public void create_forge_room(){}

void set_add_water(mixed water_short, string water_descr);
...
// adds drinking
    create_well_room();
    set_add_water(({"barrels", "water barrels", "barrel"}),
        "The barrels are made of wood and used for forging. They are all " +
        "different sizes and all look well used. Most of the barrels have " +
        "dirty water in them from when smiths plunge their items in to " +
        "cool the metal. One large barrel is full of clean water is used " +
        "to replenish the other barrels.");
...
leave_inv(object obj, object to)
{
    ...
    if (tools = present("smiths_forging_tools", obj))
    {
        ...
        tools->move("/d/Shire/smiths/rooms/forge_room");
        TP->add_fatigue(-100);
...
public mixed thieves()
{
    say("Two rough, burly looking smiths enter the Forge.\n" +
        "Berim glares at " + QTNAME(TP) +".\n" +
        "Berim says: We do not allow thievery in the forge. Begone!\n");
    write("The rough, burly looking smiths throw you out.\n");
    say("The rough, burly looking smiths throw " + QTNAME(TP) + 
        "out of the Forge.\n");
    TP->move_living("M", "/d/Shire/smiths/rooms/discus");
    return "";
}
...
string read_sign()
{
return "On the brass poster, you see enscribed:\n\n" +
...
"      |                                                              |\n"+
"      |   You may plate your newly forged items with gold, silver,   |\n"+
"      |   or platinum.                                               |\n"+
"      |                                                              |\n"+
...
/*
 * Adds water descriptions to room
 */
void set_add_water(mixed water_short, string water_descr)
{
    add_item(water_short,
        water_descr + "\n");
    add_prop(OBJ_I_CONTAIN_WATER, 1000000);
    set_drink_from( ({ water_short, "water"}) );
