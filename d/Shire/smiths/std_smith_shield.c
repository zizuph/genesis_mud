/*
 * /d/Shire/smiths/std_smith_shield.c
 *
 * The standard shield used by the Smiths' guild
 *
 * Glaurung 1/98
 */
...
set_state(string s)
{
    set_recover_str("#SMA#"+TO->query_ac()+"#"+TO->query_short()+
        "#"+s+"#"+query_forger()+"#");    
}
...
init_recover(string s)
{
    ...
    sscanf(s, "%s#SMS#%d#%d#%s#%s#%s#%s", junk, ac, slots, armour_name,
        state, forger, init_arm_rec_str);
    set_recover_str("#SMS#"+ac+"#"+slots+"#"+armour_name+"#"+
        state+"#"+forger+"#");
    if (!objectp(find_object(MANAGER)))
        MANAGER->load_object();

    MANAGER->init_smith_shield(ac, slots, armour_name, TO, state, forger);
...
public string
query_recover() { return MASTER+":"+Recover_str+query_arm_recover(); }
