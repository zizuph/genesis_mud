/* Special ring given to Smiths who won the 
 * "Guess the necklace level" contest.
 * This is the team winner's ring. 
...
create_object()
{
    set_name("ring");
    add_adj(({"pink" ,"gold"}));
    add_name("_smith_ring_reward_team");
    add_name("band");
    set_short("pink gold ring");
    set_long("The ring is a band of pink gold. It is polished has a brushed " +
        "finish. On the top of the ring is a blazing forge " +
        "with a border of a twining ivy leaves. It was won as a " +
        "team member in a special Smiths guild contest for " +
...
show_subloc(string subloc, object me, object for_obj)
{
    ...
    return str + "a " + short()+" on "+HIS_HER(me)+" finger, " +
        "declaring "+HIM_HER(me)+" a winner in the Smiths Guild contest.\n";
}
...
*/
    ...
    obj->set_long(obj->query_long()+"It is marked with an " +
        "impression of a blazing forge. The name "+
        TP->query_name()+" is stamped beneath the blazing forge. " +
...
patience()
{
    write("You advise everyone to be patient, like you are when you " +
        "forge as a Smith.\n");
