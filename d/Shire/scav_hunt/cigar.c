 ...
 * -- Finwe, October 2003
 * Ideas:
 *  smell the cigar --> done! Raymundo, March 2020
 *
 * Updated commands so they worked propertly. You can now
 * smoke, light, extinguish, tap, etc. the cigar.
 * Decided to include this in /d/Shire/waymeet/rooms/shops/novelty_shop.c
 *
 * Raymundo, March 2020
 */
...
init()
{
	...
	add_action("smell_cigar", "smell");
	add_action("smell_cigar", "sniff");

}

int
smell_cigar(string str)
{
	if(!strlen(str))
	{
		notify_fail("Smell what? The cigar?\n");
		return 0;
	}
	...
	if(puffs >= max_puffs)
	{
		write("The cigar smells like stale, leftover ashes.\n");
		say(QCTNAME(TP) + " smells the " + query_short() + " and looks "
			+ "disappointed.\n");
		return 1;
	}
	if(cigar_lit == 1)
	{
		write("The cigar smells like deep, rich tobacco. It's nice!\n");
		say(QCTNAME(TP) + " smells the cigar and looks happy.\n");
		return 1;
	}
	else
	{
		write("The cigar smells like tobacco and leather, maybe with a hint "
			+ "of brandy. Honestly, after smelling it, you'd like to light "
			+ "the " + query_short() + " and take a few puffs.\n");
		say(QCTNAME(TP) + " smells the " + query_short() + " and closes " 
			+ HIS_HER(TP) + " eyes, looking happy.\n");
		return 1;
	}
}
int
...
int extinguish_cigar(string str)
{
	...
	if(puffs >= max_puffs)
	{
		write("The " + query_short() + " has been smoked completely. You can't "
			+ "extinguish it anymore because it's already been extinguished.\n");
		return 1;
	}
...
int tap_cigar(string str)
{
    ...
    if (str == "cigar")
    {
		if(puffs > max_puffs)
		{
		write("You tap the " + query_short() + ", but since the cigar "
			+ "is used up, nothing really happens.\n");
		say(QCTNAME(TP) + " taps " + HIS_HER(TP) + " " + query_short() 
			+ ". Since it's all used up, nothing really happens.\n");
		return 1;
		}
        if (cigar_lit == 0)
        {
...
int bite_cigar(string str)
{
	if(!strlen(str))
	{
		notify_fail("Bite what? The end of the cigar off?\n");
		return 0;
	}
   if (parse_command(str, TP, " [the] [end] [off] [the] [end] [of] [the] [long] [dark] [brown] 'cigar' [off] "))
   {
/*write(str);
...
*/
   ...
   {
	   write("Bite what? The end of the cigar?\n");
	   return 1;
   }
...
int puff_cigar(string str)
{
	...
	if(puffs > max_puffs)
	{
		write("You puff on the " + query_short() + ", but since the cigar "
			+ "is used up, no smoke comes out.\n");
		say(QCTNAME(TP) + " tries to smoke a dead cigar. How funny!\n");
		return 1;
	}
...
int light_cigar(string str)
{
	...
	if(puffs >= max_puffs)
	{
		write("The " + query_short() + " has been smoked completely. You "
			+ "are unable to light it anymore.\n");
		return 1;
	}
    if(cigar_lit == 1)
    {
