...
get_order(string str)
{
  ...
  switch (str) 
  {
    ...
    case "tart": case "apple": case "apple tart": case "11":
      ...
      order->set_long("This apple tart is triangular shaped. " +
...
//   if(awareness < MIN_AWARE + random (MIN_AWARE)) return 0;
   if(!str || (str!= "table" && str != "tables")) return 0;
   if(pan) return 0;
   seteuid(getuid(TO));
   pan = clone_object("/d/Shire/scav_hunt/pan.c");
   pan->move(searcher,1);
   say(QCTNAME(searcher)+ " finds something on a empty table.\n");
    return "You find a "+pan->short()+" sitting on an empty table.\n";
}