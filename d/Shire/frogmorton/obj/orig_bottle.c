/*
 * A cleaned up version of the bottle code from the Misty Mountains.
 * Originally coded by Milan as a wizard in the Rhovanion domain
 * before it was absorbed into the Shire. Typos, poor English, 
 * etc. have been cleaned up.
 * -- Finwe, February 2003
 */
inherit "/std/object";
inherit "/lib/keep"; 
...
create_object()
{
    set_name("bottle");
    set_pname("bottles");
    set_adj(({"square","glass"}));
    set_short("square glass bottle");
    set_pshort("square glass bottles");
    set_long("@@my_long@@");
    max_amount = 1500;
    set_keep(1);
...
my_no_sell()
{
    ...
    return "You need to empty the "+short()+" before you can sell it.\n";
}
...
fill_bottle(string arg)
{
    ...
    NF("Fill what, the square bottle?\n");
    if(!arg) return 0;
...
fill_one_thing(object ob)
{
	...
	else 
	{
		...
		if (!arr) adjs = ({ });
		    ...
		    adjs = (arr - filled_adjs) - MINUS;
		filled_adjs += adjs;
		if(member_array(short, filled_short) == -1)
		    filled_short += ({ ob->query_short() });
		filled_names += names;
	    }
	    soft_amount += am1;
	    alco_amount += am2;
...
empty_bottle(string arg)
{
    ...
    NF("Empty what, the square bottle?\n");
...
drink_from_bottle(string arg)
{
    ...
    NF("Drink from what, the square bottle?\n");
	...
	if(soft_amount)
	{
	    write("You drink some "+mix+" from the "+short()+".\n");
	    say(QCTNAME(TP)+" drinks some "+mix+" from the "+short()+".\n");
	}
	else
	{
	    filled_short = ({ });
	    write("You drink "+mix+" from the "+short()+" until it is empty.\n");
	    say(QCTNAME(TP)+" drinks "+mix+" from the "+short()+" until it is empty.\n");
	}
	return 1;
    }
    if(sizeof(filled_short) == 1)
	...
	mix = "mixture of "+COMPOSITE_WORDS(filled_short);
    write("You can't drink any more "+mix+" from the "+short()+".\n");
    return 1;
}
...
my_long()
{
    ...
    text = "This is a "+short()+". It is light green colored an looks " +
        "like it fits neatly in a pocket. The bottle can be filled or " +
        "emptied as needed. You may also drink from it when it is full. " +
        "On the bottom of the bottle is a stamp that reads 'Blown by " +
        "Mungo Gamgee of Longbottom'. Right now";
