...
#include "/d/Shire/hobbitmaker/tobacco.h"
#include <ss_types.h>
#include <wa_types.h>
...
#define HOBBIT_WEP  "/d/Shire/hobbitmaker/wep/"
#define HOBBIT_ARM  "/d/Shire/hobbitmaker/arm/"
#define HOBBIT_OBJ  "/d/Shire/hobbitmaker/obj/"
...
#define EQUIPPED "hobbit_is_armed"
...
do_die(object killer)
{
	...
	{
	object dog1;
	dog1 = clone_object(WAYM_DIR + "npcs/guard_dog");
	dog1->move(environment(TO));
	dog1->command("kill " + killer->query_real_name());	
	
	for(i = 0; i<(random(2) + 3); i ++)
	{
	dog = clone_object(WAYM_DIR + "npcs/guard_dog");
	dog->move(environment(TO));
	dog->command("kill " + killer->query_real_name());
	
	dog->command("emote howls.");
	}
	tell_room(environment(TO), "\nAttracted by the noise of combat, a " 
		+ "pack of dogs arrives to defend their master.\n\n");
	}
