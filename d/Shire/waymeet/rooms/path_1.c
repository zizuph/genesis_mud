/*
    /d/Shire/waymeet/rooms/path_1.c
...
void create_er_room()
{
    ...
    add_item(({"surroundings","expanse","plain","shire"}),
        ...
        +"Shire.\n");
    add_item(({"path"}),
        "This is a simple dirt path leading west and southwest to the fields.\n");
    add_item(({"field", "fields"}),
        "These farm fields provide food for the village of Waymeet.\n");

    reset_shire_room();

    add_exit(WAYM_ROOM_DIR + "path_2","east");
    add_exit(WAYM_FIELD_DIR + "field20","west");
	add_exit(WAYM_FIELD_DIR + "s_field01", "southwest", 0,0,1);
}
