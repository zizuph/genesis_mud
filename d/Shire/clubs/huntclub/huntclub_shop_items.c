/*
 * 2006/01/24 Last update
 * 2011/08/02 Lavellan - Made buy price determined from master object rather
 *                       than cloned, so what you see is what you pay. (equip)
 */
...
#include "huntclub.h"
#include "/d/Shire/clubs/huntclub/huntclub_shop_items.h"
...
shop_hook_list_object(object ob, int price)
{
    ...
    {
        mess = sprintf(" %-8s | %2d %2d %2d %2d", categ, price / 1728, (price % 1728) / 144, (price % 144) / 12, price % 12);
        write("| " +str + "|" + mess + " |\n");
    }
    else
    {
...
//    price = query_buy_price(ob);
    price = query_buy_price(find_object(MASTER_OB(ob)));
...
do_list(string args)
{
    ...
    {
        ::do_list(args);
        write("I have these items fer sale: 'list equipment'\n");
        return 1;
    }

    write("This is the standard equipment in stock at this shop:\n\n" +
    "+-------------------------------------------------------------------+\n"+
