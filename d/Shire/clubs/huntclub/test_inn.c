...
configure_shire_inn()
{
	...
	TO->add_item("Rune","You notice an old rune for M in the room. You wonder who put it there?\n");// Haha! M.
}
...
{
    string comp_names;
    string plural_name=ob->query_plural_short();
    
    
    number_from_heap=ob->query_prop(HEAP_I_IS);
    
    dump_array(number_from_heap);

    plural_name=LANG_WNUM(number_from_heap) + " " + plural_name;
    dump_array(plural_name);
    string this_room="inn_hook_player_buys";
    dump_array(this_room);

    if (sizeof(players) == 1)
    {
	if (TP == players[0])
	{
        if (number_from_heap>1){
            write("You pay "+price+" and order yourself " + plural_name +".\n");
...
{   
    ...
    if (order->query_prop(HEAP_I_IS)){
        number_from_heap=order->query_prop(HEAP_I_IS);
        dump_array(amount);
        order->set_heap_size(amount*number_from_heap);
    } 
...
{  string this_room="get_order_test_inn";
   dump_array(this_room);
