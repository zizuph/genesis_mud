...
inherit "/d/Shire/clubs/huntclub/test_inn.c";
inherit "/lib/trade"; // for do_fix()

#include "/d/Shire/sys/defs.h"
#include "huntclub.h"
#include "huntclub_shop_items.h"
...
create_room()
{
    ...
    set_long("You're standing in the foyer of a large hall within the hunting " +
        "lodge. Supple leather armchairs and dark stained tables are " +
        "arranged around a massive dual-sided fireplace located in " +
        "the center of the hall. Warm looking fur rugs can be seen " +
        "resting over tightly fitted oak floorboards. A well lit antler " +
        "chandeliers hang from an ornately carved beamed cathedral " +
        "ceiling. Large shuttered windows set within the walls by the " +
        "door. Many heads of various animals are mounted on a trophy " +
        "wall on the west wall. There's a bulletin board posted on " +
        "the wall amongst the heads. A price list, a sign, and a plaque " +
        "hang on another wall. A large wicker basket containing long " +
        "white objects sitting on one of the tables.\n");

    add_item(({"lodge", "hunters lodge"}),
        "You are standing in a large hunters lodge.\n");

    add_item(({"table", "tables"}),
    	"Several dark stained tables can be seen near the chairs. They " +
        "appear to be strong enough to stand on. There's a large wicker " +
        "basket on one of the tables.\n");

    add_item(({"heads", "animals","trophies"}),
        "Taxidermy at finest. Various animal heads mounted on backboards, " +
        "and mounted on the western wall.\n");

    add_item(({"windows", "shuttered windows"}),
        "Large windows set deep within the walls by the doors. Heavy " +
        "shutters with arrow slits, currently cover the windows.\n");

    add_item(({"basket", "wicker basket"}),
        "Just a plain wicker basket. There's a brass plaque on the side.\n");

    add_item(({"white objects", "objects"}),
        "They look like sticks, or perhaps bones.. There's a plaque on " +
        "the basket that might help explain what they are.\n");

    add_item(({"chairs", "chairs", "arm chair", "armchairs"}),
        "Plush leather high backed arm chairs arranged around the fireplace, " +
        "for weary hunters to sit on.\n");

    add_item(({"chandeliers","chandelier"}),
        "Rustic multi-tiered antlers, holding at least 20 lit candles " +
        "extend from the ceiling. Two giant racks are mounted at either " +
        "ends above the fireplace hearth.\n");

    add_item(({"candles"}),
        "Large white candles are arranged within the chandeliers, providing " +
        "warm light throughout the hall.\n");

	add_item(({"carvings","ornately carved","beams","beamed","cathedral", "ceiling"}),
		"The ornately carved cathedral ceiling extends high above you. " +
        "The carvings are of various animals, bears, deer, all in various " +
        "exciting poses.\n");

    add_item(({"wall","walls"}),
        "They are made from horizontally laid pine trees with white " +
        "mortar lines.\n");

	add_item(({"mortar", "mortar lines", "lines"}),"Bleached white mortar " +
        "can be seen between the pine tree logs, which make up the walls.\n");

    add_item(({"floor", "ground","floorboards","boards"}),
    	"It is a cedar planked floor with the occasional fur rug here " +
        "and there.\n");

    add_item(({"fireplace", "dual-sided", "stone fireplace"}),
        "This massive natural stone fireplace looks large enough to " +
        "stand in. It rests at the center of the lodge. A roasting rack " +
        "sits alongside the fireplace. The fireplace has two sides " +
        "which share the same flue. There's a roaring fire inside.\n");

    add_item(({"fire","roaring fire"}),
        "A cracking fire can be seen within the massive fireplace, " +
        "providing warmth to the entire lodge. Cozy isn't it?\n");
    add_item(({"flue", "chimney"}),
        "The large natural rock chimney narrows as it extends through " +
        "and beyond the arched ceiling.\n");

	add_item(({"fur","fur rugs", "rugs","rug"}),
		"Several very comfortabe looking fur throw rugs can be seen " +
        "on the floor. They " +
		"look comfy enough to lay on.\n");

    add_item("sign", 
        "It is attached to the wall. Perhaps you could read it?\n");

    add_item(({"iron chains","chains"}),
      "Strong looking iron chains hold the heavy looking antler chandeliers " +
        "overhead.\n");
...
// signs and posters to read
    ...
    add_cmd_item("sign", ({ "read", "examine", "exa"}), "@@read_sign@@");
    add_cmd_item( ({"pricelist", "price list"}), ({ "read", "examine", "exa"}), 
        "@@read_pricelist@@");
...
do_fix(string str)
{
    ...
    if (pay_type != "")
    {
        ...
        {
            tell_room(this_object(), "The hunter says: I don' t ackcept thems"
    ...
    if (get_type != "")
    {
        ...
        {
            tell_room(this_object(), "The hunter says: " + capitalize(get_type) 
                + " coins? Never heerd of thems.\n");
            return 1;
        }
    }
    ...
    if (sizeof(weapon) > 1)
        return 0;
    notify_fail("Dat is nairy rilly a weppon.\n");
    if (function_exists("create_object",weapon[0]) != "/std/weapon")
        return 0;
    notify_fail("I cannot repair broken weppons.\n");
    if(weapon[0]->query_prop(OBJ_I_BROKEN))
        return 0;
        /* Talk about tricky code! ;-) */
        value = call_other(this_object(), "calc_"+verb+"_value", weapon[0]);
    notify_fail("The hunter says: I cannot " + verb + " dat inny further.\n");
    if (value == -1)
        return 0;
    ...
    if (sizeof(money_arr = pay(value, tp, pay_type, 0, 0, get_type)) == 1)
    {
        if (money_arr[0] == 1)
        {
            tell_room(this_object(), "The hunter says: I think dat " 
        ...
        if (money_arr[0] == 2)
        {
            tell_room(this_object(), "The hunter says: You do nairy have inny " 
                + pay_type + " coins, " + tp->query_nonmet_name() + ".\n");
            return 1;
        }
        else
        {
            tell_room(this_object(), "The hunter says: I think dat " 
                + weapon[0]->short() + " is a bit too expensive fer you, " 
                + tp->query_nonmet_name() + ".\n");
            return 1;
        }
    }
    ...
    write("You give the hunter the " + weapon[0]->short() + ".\n");
    say(QCTNAME(tp) + " gives the hunter " + tp->query_possessive() +
        " " + weapon[0]->short() + ".\n");
    tell_room(this_object(),
        "The hunter " + verb + ((verb == "sharpen") ? "s" : "es") + " the "
            + weapon[0]->short() + ".\n"
            + "The hunter gives the weapon back.\n");
    ...
    write("You pay the hunter " +coin_pay_text+ ".\n");
    if (coin_get_text)
    {
...
do_order(string str)
{
    ...
    if (!rumbald || !present(rumbald))
    {
        notify_fail("There is no one here to buy anything from.\n"+
          "The lodge owner is off travelling somewhere.\n");
        return 0;
    }
...
get_order(string item_name)
{
    ...
    case "small torch":
    case "small torchs":
        order = clone_object(TORCH);
        price = 144;
        break;
    case "bow string":
    case "bow strings":
        order = clone_object(BOWSTRING);
        price = 72;
        break;
    case "arrows":
    case "dozen wooden arrows":
    case "12 wooden arrows":
        order = clone_object("/d/Shire/clubs/huntclub/wep/arrwd");
        price = 1728;
        break;
    case "water":
    case "waters":
    ...
    case "glass of waters":
        order = clone_object(WATER);
        price = 24;
        break;
    case "oats":
    case "handful of oats":
    case "handfuls of oats":
        order = clone_object(OATS);
        price = 60;
        break;
    case "likker":
    case "likkers":
...
{
    return "On the price list you read:\n\n"+ 
    "\t+----------------------------------------------+\n" +
    "\t|                                              |\n" +
    "\t|  These are thay ... uhh, things ay done      |\n" +
    "\t|  collected ayn' sell:                        |\n" +
    "\t|                                              |\n" +
    "\t|  A small torch. . . . . . . . . 1 gold       |\n" +
    "\t|  A bow string . . . . . . . . . 6 silver     |\n" +
    "\t|  A dozen wooden arrows. . . . . 1 platinum   |\n" +
    "\t|  A glass of water . . . . . . . 2 silver     |\n" +
    "\t|  A handful of oats. . . . . . . 5 silver     |\n" +
    "\t|  A corn likker. . . . . . . . . 5 gold       |\n" +
    "\t|  A jerky strip. . . . . . . . . 3 silver     |\n" +
    "\t|  A roasted wild boar. . . . . . 5 gold       |\n" +
    "\t|  ------------------------------------------  |\n" +
    "\t|          Weapons sharpened here!             |\n" +
    "\t|             sharpen <weapon>                 |\n" +
    "\t+----------------------------------------------+\n\n";
}
...
string read_sign()
{
    return "On the sign you read:\n\n" +
        "\t+----------------------------------------------+\n" +
        "\t|                                              |\n" +
        "\t|  These're the skills y'all can learn here:   |\n" +
        "\t|                                              |\n" +
        "\t|  Animal Handling     Awareness               |\n" +
        "\t|  Hunting             Location Sense          |\n" +
        "\t|  Tracking                                    |\n" +
        "\t|                                              |\n" +
        "\t|  All these skills can be done learned up ta  |\n" +
        "\t|  Superior Layman levels                      |\n" +
        "\t|                                              |\n" +
        "\t+----------------------------------------------+\n\n";
}
