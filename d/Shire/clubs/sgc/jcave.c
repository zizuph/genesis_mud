...
create_room()
{
    ...
    add_exit(SGC_DIR + "boardrm","east",0,1);
...
do_begin(string str)
{
    ...
    write_file(GOB_LOG, CAP(TP->query_real_name())+" joined and became "+
      "a smart goblin: "+ extract(ctime(time()),4,15)+".\n");
    ...
    if (!tome)
    {
	tome = clone_object(SGC_DIR + "tome");
	tome->move(TP,1);
...
do_stupid(string str)
{
	...
	if (tome)
	{
        ...
        write_file(GOB_LOG, CAP(TP->query_real_name())+" left and became "+
           "a dumb goblin once again: "+ extract(ctime(time()),4,15)+".\n");
	}
