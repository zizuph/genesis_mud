...
create_room()
{
    ...
    add_exit(FANG_DIR(rooms/train),"west","@@mem",-1);
    add_exit(FANG_DIR(rooms/plan_room),"east","@@mem",-1);
    add_exit(FANG_DIR(rooms/hall),"south","@@mem",-1);
...
mem()
{
    if(IS_MEMBER(TP) || IS_GUILD_MASTER(TP))
        return 0;
