...
start_spell_fail(string verb, string arg)
{
    ...
    /* Sort out all the possible targets */
    ...
    /* If the spell somehow fails we should lose some mana */
    caster->add_mana(-spell_mana_needed/2); /* Olorin thinks this is too much. Fix! */

    /* Is it an attack spell? We should do a magic attack then */
    /* if(spell_attack)
	caster->add_prop(LIVE_O_SPELL_ATTACK, spell_origin); */

    /* It is now possible to start chanting */
    return 0;
}
