 ...
 * Arguments:	    arg: The arguments. (Is this needed? /Rastlin)
 * Description:     Checks weather the caster can cast a spell.
 * Returns:	    1 - If the spell is not to be cast.
 *		    0 - If not.
 *
 * The other players in the room should be able to see that we 
 * are loosing control of the power of magic. /Rastlin
 *
int
start_spell_fail(string arg)
{
    string *fumble = caster->query_prop(LIVE_AS_ATTACK_FUMBLE);

    reset_spell();

    if (sizeof(fumble))
    {
	write("You are fumbling with your spell!\n");
	write(fumble[0]);
	return 1;
    }
    else
