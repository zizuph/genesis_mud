/*
 *  /d/Shire/guild/estehilde/join.c
 *
 *  This is the join room of the Estehilde Occ guild.
...
#include "/d/Shire/guild/estehilde/estehilde.h"
#include "/d/Shire/bits.h"

#define START          (ESTE_DIR + "start")
...
create_shire_room()
{
    set_short("The Estehilde Join room");
    set_long("The Estehilde Join room");
...
log_guild(string text)
{
    ...
    if (file_size(ESTE_GUILD_LOG) > LOG_SIZE)
    {
	rename(ESTE_GUILD_LOG, (ESTE_GUILD_LOG + ".old"));
    }

    write_file(ESTE_GUILD_LOG, text);
}

/*
 * Function name: do_serve
 * Description  : When an Estehilde wants to join the guild, this function
...
do_serve(string str)
{
    ...
    if (tp->query_guild_member(ESTE_GUILD_TYPE))
    {
...
/*
    if (!(tp->query_prop("_i_can_join_the_estehilde")))
    {
	write("You need to find an Estehilde willing to vouch for you, for you "+
	  "to join the guild.\n");
	return 1;
    }
*/

    shadow = clone_object(ESTE_GUILD_SHADOW);

    if ((result = shadow->shadow_me(tp, ESTE_GUILD_TYPE,
	  ESTE_GUILD_STYLE, ESTE_GUILD_NAME)) != 1)
    {
    ...
    write("You kneel, and join the Estehilde.\n");
    ...
    tp->set_guild_pref(SS_OCCUP, ESTE_GUILD_TAX);
//  tp->set_bit(FOURTH_GROUP,ESTE_BIT);
    return 1;
}
...
lower_skill(object tp)
{
    ...
    tp->clear_guild_stat(SS_OCCUP);
    tp->remove_cmdlist(ESTE_GUILD_SOUL);
//    tp->clear_bit(FOURTH_GROUP,ESTE_BIT);
    tp->update_hooks();
    ...
    while (objectp(ob = present("_este_robe_", tp)))
    {
	ob->remove_object();
    }
    while (objectp(ob = present("_este_book_", tp)))
    {
	ob->remove_object();
    }
...
do_betray(string str)
{
    ...
    write("You leave the Estehilde.\n");
    say(QCTNAME(tp)+ " leaves the Estehilde.\n");
...
/*
	...
	if (objectp(robe = present("_este_robe_",tp)) && 
	...
	robe->remove_object();
	clone_object(ESTE_GUILD_OBJ)->move(tp, 1);
    ...
    if (objectp(present("_este_book_", tp)))
    {
    ...
    clone_object(ESTE_GUILD_SPELLS)->move(tp, 1);
