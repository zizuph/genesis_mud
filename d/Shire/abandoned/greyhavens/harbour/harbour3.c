...
public object check_blind(object o);
...
update_internal(int l, int w, int v)
{
    ...
    if (new_light_level < old_light_level)
    	...
    	p = FILTER_PLAYERS(all_inventory(this_object()));
        psee = filter(p, check_blind);
	tell_room(this_object(), "The light dims and you cannot see.\n", psee);
    }
...
public object
check_blind(object o)
{
    if (CAN_SEE_IN_ROOM(o))
        return o;
