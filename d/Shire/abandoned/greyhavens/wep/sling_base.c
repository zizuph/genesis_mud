...
// inherit "/d/Shire/abandoned/greyhavens/wep/launch_weapon_sling";
...
create_launch_weapon()
{
    ...
    set_keep(0);
    create_bow();
    set_valid_projectile_function("is_rock");
}
...
{
    tell_object(query_wielded(), "You are out of rocks!\n");
}
...
{
    // You nock a white-feathered rock in the pouch of your elven longbow,
    // aiming carefully at the black orc.

    if (ENV(archer) == ENV(target))
    {
        tell_object(archer, "You slip " +
		    LANG_ADDART(projectile->singular_short()) +
		    " in the pouch of your " + short() + ", and start swinging it over your head, aiming carefully at " +
    ...
    else
    {
        tell_object(archer, "You slip " +
		    LANG_ADDART(projectile->singular_short()) +
		    " in the pouch of your " + short() + ", and start swinging it over our head, aiming carefully at " +
		 ...
		 object projectile, string adj_desc)
{
    if (ENV(archer) == ENV(target))
    {
        tell_bystanders_miss(QCTNAME(archer) + " slips " +
			     LANG_ADDART(projectile->singular_short()) +
			     " into the pouch of " + archer->query_possessive() +
			     " " + short() + ", aiming carefully at " +
			     QTNAME(target) + ".\n",

			     QCTNAME(archer) + " slips " +
			     LANG_ADDART(projectile->singular_short()) +
			     " into the pouch of " + archer->query_possessive() +
			     " " + short() +
			     ", aiming at something.\n",
    ...
    else
    {
        tell_bystanders_miss(QCTNAME(archer) + " slips " +
			     ...
			     " into the pouch of " + archer->query_possessive() +
			     " " + short() +
			     ", and starts swinging it over his head, aiming carefully at " + QTNAME(target) +
			     " " + adj_desc + ".\n",
			     ...
			     " into the pouch of " + archer->query_possessive() +
			     " " + short() +
			     ", and starts swinging it over his head, aiming carefully at something " +
			     adj_desc + ".\n",
...
tell_target_load(object archer, object target, object projectile)
{
    ...
    {
        tell_object(target, archer->query_The_name(target) + " slips " +
		    LANG_ADDART(projectile->singular_short()) +
		    " in " + archer->query_possessive() + " " +
		    short() + ", and starts swinging it over " + archer->query_possessive() + " head.\n");
    }
}
		 ...
		 string adj_room_desc)
{
	...
	{
	  /*
	   * You shoot an rock at the black orc, but miss.
	   */

	  tell_object(archer, "You sling a rock at " +
	...
	else
	{
	    ...
	    {
		tell_object(archer, "You sling blindly at " +
			    target->query_met_name() + ".\n");
	    }
	    else
	    {
		tell_object(archer, "You sling blindly at the " +
	...
	{
	    // You shoot an rock at the black orc on the courtyard, but miss.
		 ...
		 string adj_room_desc, string org_room_desc)
{
	...
	{
	    // The tall green-clad elf slings a rock at you, but misses.

	    tell_object(target, archer->query_The_name(target) +
			" slings a rock at you, but misses.\n");
	}
	else
	{
	    tell_object(target, "You see  a rock " +
	...
	{
	    /*
	     * The tall green-clad elf slings a rock at you from
	     * the battlement, but misses.
	     */

            tell_object(target, archer->query_The_name(target) +
			" slings a rock at you from " +
			org_room_desc + ", but misses.\n");
	}
	else if (CAN_SEE_IN_ROOM(target))
	{
	    tell_object(target, "Someone slings a rock at you," +
			" but misses.\n");
	}
	else
	{
	    tell_object(target, "You see a rock go " +
			"flying past your head!\n");
	}
    }
}
		 ...
		 string adj_room_desc, string org_room_desc)
{
	...
	/*
	 * The tall green-clad elf slings a rock at the black orc,
	 * but misses.
	 */

	tell_bystanders_miss(QCTNAME(archer) + " slings a rock at " +
			     QTNAME(target) + ", but misses.\n",

			     QCTNAME(archer) +
			     " slings a rock at something.\n",

			     "A rock flies past " + QTNAME(target) + ".\n",

			     "You a rock go flying through " +
			     "the air.\n",
	 ...
	 *
	 * The tall green-clad elf slings a rock at the black orc on
	 * the courtyard, but misses.
	 */

	tell_bystanders_miss(QCTNAME(archer) +
			     " slings a rock at " + QTNAME(target) +
			     ...
			     "Someone slings a rock at " + QTNAME(target) +
			     " " + adj_room_desc + ", but misses.\n",
	...
	tell_bystanders_miss(QCTNAME(archer) + " slings a rock at " +
			     ...
			     QCTNAME(archer) + " slings a rock " +
			     " at something.\n",

			     "Someone slings a rock at " +
			     QTNAME(target) + ", but misses.\n",

			     "You see a rock go flying through " +
			     "the air.\n",
 ...
 * Description  : Produces a message to the archer when his rock hits the
 *                target without causing any harm. This is described as
 *                the rock bouncing off his armour.
 *
 * Arguments    : archer:        The player loading his weapon.
 *                target:        The target player is aiming at.
 *                projectile:    The projectile we are loading.
 ...
 *                org_room_desc: Description of originating room. 0 if
 *                               target stands in the same room as the archer.
 *                armour:        The armour that deflects the rock. armour
			  ...
			  string adj_room_desc, object armour)
{
    ...
    if (armour)
    {
	armour_desc = ", but the rock glances off " +
    ...
    if (ENV(archer) == ENV(target))
    {
        if (CAN_SEE(archer, target) && CAN_SEE_IN_ROOM(archer))
	{
	    // You shoot an rock at the black orc, but the rock glances off
	    // his helm.

	  tell_object(archer, "You shoot an rock at " +
	...
	{
	    /*
	     * You shoot an rock at the black orc on the courtyard,
	     * but the rock glances off his helm.
	     */
	    tell_object(archer, "You shoot an rock at " +
			target->query_the_name(archer) + " " +
 ...
 *                an rock that do no dammage. This is described as
 *                the rock bouncing off his armour.
 *                
 * Arguments    : archer:        The player loading his weapon.
 *                target:        The target player is aiming at.
			  ...
			  object armour)
{
    ...
    if (armour)
    {
	armour_desc = ", but the rock glances off your "
	    + armour->short() + ".\n";
    }
    else
    {
	armour_desc = ", but the rock glances off harmlessly.\n";
    }
	...
	{
	    /*
	     * The tall green-clad elf slings a rock
	     * at you, but the rock glances off your helm.
	     */

	    tell_object(target, archer->query_The_name(target) + 
			" slings a rock at you" + armour_desc);
	}

	else
	{
	    tell_object(target, "A rock from out of nowhere hits you, " +
	...
	{
	    /*
	     * The tall green-clad elf slings a rock at you
	     * from the battlements, but the rock glances off your helm.
	     */
	...
	{
	    tell_object(target, "Someone slings a rock at you from " +
 ...
 * Description  : Produces messages to bystanders when the archer's rock
 *                harmlessly hits the target. This is described as
 *                the rock bouncing off the target's armour.
 *                
 * Arguments    : archer:        The player loading his weapon.
 *                target:        The target player is aiming at.
 *                projectile:    The projectile we are loading.
 ...
 *                org_room_desc: Description of originating room. 0 if
 *                               target stands in the same room as the archer.
 *                armour:        The armour that deflects the rock. May
			  ...
			  object armour)
{
	...
	/*
	 * The tall green-clad elf slings a rock at the
	 * black orc, but the rock glances off his chainmail.
	 *
	 */
	...
	tell_bystanders_miss(QCTNAME(archer) + " slings a rock at " +
			     ...
			     QCTNAME(archer) + " slings a rock at " +
			     "something. You hear a thud as the rock " +
			     "hits something.\n",

			     "A rock hits " + QTNAME(target) + armour_desc,
	 ...
	 *
	 * The tall green-clad elf slings a rock at the black orc on
	 * the courtyard, but the rock glances off his chainmail.
	 */
	 ...
	 *
	 * The tall green-clad elf slings a rock at the black orc
	 * from the battlements, but the rock glances off his chainmail.
	 */
	...
	tell_bystanders_miss(QCTNAME(archer) + " slings a rock at " +
			     ...
			     QCTNAME(archer) + " slings a rock at " +
			     "something. " +
			     "You hear a thud as the rock hits something.\n",
		...
		string hdesc, int dt, int phurt, int dam, int hid)
{
	...
	{
	    /*
	     * You shoot an rock at the black orc.
	     * The rocks strikes into his legs.
	     */

	    tell_object(archer, "You shoot an rock at " +
			target->query_the_name(archer) +
			". The rock" + damage_desc +
	...
	else
	{
		...
		tell_object(archer, "You shoot blindly at " +
			    target->query_met_name() +
			    ". You hear a thud as the rock hits.\n");
	    }
	    else
	    {
		tell_object(archer, "You shoot blindly at the " +
        ...
        if (check_remote_seen(archer, target))
	{
	    /*
	     * You shoot an rock at the black orc on the courtyard.
	     * The rock hits him in the legs.
	     */
	    	
	    tell_object(archer, "You shoot an rock at " +
			target->query_the_name(archer) + " " +
			adj_room_desc + ". The rock" +	damage_desc +
			target->query_the_possessive_name(archer) +
			" " + hdesc + ".\n");
	} 
	else
		...
		int dt, int phurt, int dam, int hid)
{
	...
	{
	    tell_object(target, archer->query_The_name(target) +
			" slings a rock at you. The rock" +	damage_desc +
			"your " + hdesc + ".\n");
	}
	else
	{
	    tell_object(target, "A rock from out of nowhere" + damage_desc +
			"your " + hdesc + ".\n");
	}
	...
	{
	    tell_object(target, archer->query_The_name(target) +
			" slings a rock at you from " + org_room_desc +
			". The rock" + damage_desc + "your " + hdesc + ".\n");
	}
	else if (CAN_SEE_IN_ROOM(target))
	{
	    tell_object(target, "Someone slings a rock at you from " +
			org_room_desc + ". The rock" + damage_desc + "your " +
			hdesc + ".\n");
	}
	else
	{
		...
		int dt, int phurt, int dam, int hid)
{
	 ...
	 *
	 * The tall green-clad elf slings a rock at the black orc.
	 * The rocks strikes deeply into the orc's legs.
	 */

	tell_bystanders_hit(QCTNAME(archer) + " slings a rock at " +
			    QTNAME(target) + ". The rock" +
			    ...
			    "A rock hits " + QTNAME(target) + ". The rock" +
			    damage_desc + QTPNAME(target) +
			    " " + hdesc + ".\n",
	 ...
	 *
	 * The tall green-clad elf slings a rock at the black orc
	 * on the courtyard. The rock strikes soundly into the orc's legs.
	 *
	 */

	tell_bystanders_hit(QCTNAME(archer) + " slings a rock at " +
			    QTNAME(target) + " " + adj_room_desc +
			    ". The rock" + damage_desc + QTPNAME(target) +
			    " " + hdesc + ".\n",
	 ...
	 * Archer shooting to adjecent room. Target room:
	 *
	 * The tall green-clad elf slings a rock at the black orc from the
	 * battlements. The rock strikes soundly into the orc's legs.
	 *
	 */
 ...
 *                projectile:    The projectile we are loading.
 *                phurt:         The %hurt made on the enemy.
 * Returns      : (string) A string describing how much the rock hurt.
 */
public string
