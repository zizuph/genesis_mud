...
query_auto_load() 
{
  mixed arg = shadow->query_bleeding_variables();

  if (pointerp(arg) && (sizeof(arg)==2) && intp(arg[0]) && intp(arg[1]))
    return MASTER+":"+where+"#"+arg[0]+"#"+arg[1]+"#";
  return "";
}
...
init_arg(string arg)
{
  ...
  sscanf(arg,"%s#%d#%d#%s", where, a, b, foo);
