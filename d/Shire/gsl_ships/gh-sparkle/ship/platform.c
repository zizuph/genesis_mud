 ...
 * Ported to the new ship system, Tapakah, 06/2009
 * 2009/06/09 Last updated
 * 2011/07/18 Lavellan - Fixed "turn" error. Changed back to reset_room. May
 *      go back to reset_deck if that is added to GSL code.
 */
#pragma save_binary
...
create_room()
{
  ...
  set_long("The platform is situated above the rest of the deck of "+
    ...
    "operations upon, and above, the deck. The platform itself sits "+
    "centered between the swan wings of the ship, shielded from view "+
    "(and attack) by the up-thrust sections of the wings. Mounted to "+
  ...
  add_item("ship","The ship, built of strong and supple timber, has "+
    ...
    "the forecastle, where the crews quarters are located.\n");
  ...
  set_is_deck(1);
  add_exit(DECK, "down", "@@move_down",1);
...
int turn_wheel(string str)
{
  ...
  if(present("helmsman"))
  {
    ...
    //  "and pushes you away.");
    write("As you reach for the wheel the helmsman grabs your hand "+
      "and pushes you away.\n");
  ...
  else
  {
    ...
    write("You firmly grasp the wheel, but can not budge it, no "+
      "matter how hard you try!\n");
...
void add_stuff()
{
  ...
  if(!present("helmsman"))
  {
    helmsman = clone_object(HELMSMAN);
    helmsman->move(TO);
  }
}
