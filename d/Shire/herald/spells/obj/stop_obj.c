...
static int no_m_att, no_att, no_o_att;
...
start()
{
    ...
    no_o_att = spell_target->query_prop(OBJ_M_NO_ATTACK);
    spell_target->add_prop(ROOM_I_NO_ATTACK, 1);
    spell_target->add_prop(ROOM_I_NO_MAGIC_ATTACK, 1);
    spell_target->add_prop(OBJ_M_NO_ATTACK, 1);
...
dispell_spell_effect(object dispeller)
{
    ...
    spell_target->add_prop(OBJ_M_NO_ATTACK, no_o_att);

    tell_room(spell_target, "The solemn mood in the room has "+
      "disappeared.  The air of peace and tranquility is gone.\n");
    
    // Restart aggressive npcs if there are any
    players = FILTER_PLAYER_OBJECTS(all_inventory(spell_target));
    npcs = filter(all_inventory(spell_target), &->query_npc());
