 ...
 *
 * Revision history:
 * Added orcs to the list of unaccepted races. Nerull 29/10-15
 * Added so members of illegal guilds (AA) can't join, and is 
 * removed. Nerull 29/10-15
 */
...
query_guild_keep_player(object player)
{
    ...
    if (buffoon == "goblin" || buffoon == "orc" || buffoon == "dwarf" || buffoon == "gnome")
    {
...
get_herald_soul(int news)
{
	...
	// DISABLE THIS WHEN ENABLING THE OTHER!!! - NERULL
    clone_object(HERALD_GUILD_SPELLS)->move(shadow_who, 1);



	// Recode injection stuff - Nerull
	...
	shadow_who->add_cmdsoul(HERALD_DIR + "spells_recode/lay_guild_magic");
...
public string *query_spells()
{
	...
	/*if (!objectp(spell_obj = present(LAY_GUILD1_SPELL_OBJECT, shadow_who)))
	{
		return 0;
	}*/
...
public void remove_shadow()
{
	...
	shadow_who->remove_cmdsoul(HERALD_GUILD_SOUL);
