...
#define HERB_SAVE  HERALD_DIR + "herb_save"
#define BAG_PROP  "_herald_bought_herb_bag"
...
create_shire_monster()
{
    ...
    set_title("the Herbalist of the Heralds of the Valar");
    set_adj("curious");
    add_adj("smiling");
    set_long("@@long_func");
...
//    add_ask(({"herb wrap", "herb wraps", "wrap","wraps"}), VBFC_ME("ask_wraps"));


    set_introduce(1);
    add_equipment(HERALD_GUILD_OBJ);
...
ask_wraps()
{
/*    command("say My herb wraps let you bundle up to 25 herbs in a piece "+ 
        "of cloth. The wraps make saving and recovering herbs much easier, " +
        "keeps them fresh, and can be stored in an herb pouch or jar.");
    return "";
*/    
}
...
{
    string long_str;
    long_str = "This is Gilestel, the Herbalist for the Heralds. He " +
        "keeps a great store of herbs and can identify almost any herb " +
        "for you.  He will both buy and sell herbs in addition to " +
        "identifying them for you.";
    if (TP->query_wiz_level())
	return long_str+" As a wizard, type 'stat herbalist' for more info.\n";
    return long_str+"\n";
}
