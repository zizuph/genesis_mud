...
accept(string str)
{
	...
	log_fail(ctime(time()) + " " +WRITE_NAME(TP)+ " tried to accept "+str+", but "+
    ...
    log_accept(ctime(time()) + " " + WRITE_NAME(TP) + " ("+TP->query_average_stat()+") accepted "+ capitalize( (member->query_real_name()) )+" ("+member->query_average_stat()+").\n");
...
create_room()
{
    ...
    add_exit(HERALD_DIR + "train","southwest",0,1);
    add_exit(HERALD_DIR + "libraryroom","northeast",0,1);
