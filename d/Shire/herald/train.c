...
/*     Training room for the Heralds of the Valar guild.   */
...
create_room() {
    ...
    add_item(({"branch","long strong branch","trunk"}),
        "This branch here is thin but incredibly strong, and leads "+
        "back to the central trunk which must be thirty feet across.\n");

    add_item(({"rope","hill","hill below"}),
        "The rope will let you drop down to the grassy hill below, " +
        "where there is a large stone obelisk.\n");
    ...
    add_exit("/d/Shire/herald/join","down",0,1);
    add_exit("/d/Shire/herald/board","northeast",0,1);
}
...
do_improve(string str)
{
    ...
    {
	write("Hey, you're not a Herald of the Valar!\n"+
...
set_up_skills() 
{
    sk_add_train(SS_HERBALISM,train_text(SS_HERBALISM),0,0,60 );
...
/*
    sk_add_train(SS_ELEMENT_AIR,train_text(SS_ELEMENT_AIR),0,0,50 );
    sk_add_train(SS_ELEMENT_WATER,train_text(SS_ELEMENT_WATER),0,0,50 );
    sk_add_train(SS_ELEMENT_EARTH,train_text(SS_ELEMENT_EARTH),0,0,50 );
*/
    sk_add_train(SS_LANGUAGE,train_text(SS_LANGUAGE),0,0,50 );
    sk_add_train(SS_SPELLCRAFT,train_text(SS_SPELLCRAFT),0,0,60 );
...
train_text(skill) 
{
	...
	break;
    case SS_ELEMENT_AIR:
	what="channel the air element";
	break;
    case SS_ELEMENT_WATER:
	what="channel the water element";
	break;
    case SS_ELEMENT_EARTH:
	what="channel the earth element";
	break;
    case SS_LANGUAGE:
