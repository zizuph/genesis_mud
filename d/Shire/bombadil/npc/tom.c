...
add_stuff()
{
    ...
    sword = clone_unique(WEP_DIR+"shortsword", MAX_UNIQUE_ITEMS, 
...
check_health()
{
    ...
    {
        command("emote says something in a sing-song language. He dances " +
            "around with a quicker step in his stride.\n");
        heal_hp(2000 + random(1500));
...
// asks for 'quest'
    command("think");
    if ((!CHECK_QUEST(TP, LILIES_QUEST_NAME)) && (!CHECK_QUEST(TP, FEATHER_QUEST_NAME)))
    {
        command("say Thank you for asking. I need help getting a new lily for Goldberry, ");
