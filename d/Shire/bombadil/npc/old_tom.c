...
/*
    set_long("This is Tom Bombadill. He is a merry fellow with a " +
        ...
        "the world, he can often be found wandering around the forest, " +
        "dancing and singing. Nonetheless, do not underestimate him, " +
        "for he is strong in his own right.\n");
*/
...
// gives out quest
    ...
    command("say Please help me and then find some.");
...
{
// player done quest before?
    if (REWARD_QUEST(player, QUEST_NAME))
    {
        command("say Thank you for your offer, but I don't " +
            "think you can help me anymore.");
        return;
    }

// prop is set and player doing quest?
    if (obj->id(LILY_PROP)) 
    {
        if (player->query_prop(QUEST_LILY_PROP)) // doing lily quest
        {
    ...
    {
        command("say This is a nice item, but it's not what I'm looking for.");
...
string ask_feather()
{
    ...
    command("say My feather in my hat is looking a little old.");
...
enter_inv(object obj, object from)
{
    ...
    if (!living(from))
        return;
// lily quest
//      Is player working on lily quest? 
//      This is probably a redundent check, but just in case....
    if (TP->query_prop(QUEST_LILY_PROP))
    {
    // Is this an authorized lily?
    ...
    if (TP->query_prop(QUEST_FEATHER_PROP))
    {
    // Is this an authorized feather?
            set_alarm(2.0, 0.0, &test_feather(from, obj));  
            return;
    }

//    else
    // thanks, but give item back to player. we don't want it.
    {
