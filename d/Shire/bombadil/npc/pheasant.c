 /*
  * a pheasant in the fields of the shire
  ...
  * Oct 1994
  *
  * feathers used for Tom Bombadil's feather quest
  * -- Finwe, January 2002
  */
...
// inherit "/d/Gondor/lib/assist_friend";
...
create_creature()
{
    ...
    add_name("bird");
    add_name("creature");
    set_short("colorful ring-necked pheasant");
    set_long("This is a "+short()+". It is large with a dark green and " +
        "bluish head and a pale bill. There are large featherless red " +
        "facial areas around the eyes and a white ring around the neck. " +
        "The plumage is golden with black spots on the ends, and a " +
        "bluish and green iridescence. The golden tail feathers are " +
        "long and pointed.\n");
    set_adj(({"colorful","ring-necked"}));

    set_race_name("pheasant");
    set_alignment(75-random(25));
    set_stats(({7+random(8),10+random(6),5+random(5),5+random(5),
    ...
    seq_new("do_first");
    seq_addfirst("do_first",({"@@arm_me","blink","emote pecks at something in the dirt."}));
    add_leftover(OBJ_DIR+"feather", "feather", 1, 0, 1, 1);
    add_leftover(OBJ_DIR+"feather", "feather", 1, 0, 1, 1);
    add_leftover(OBJ_DIR+"feather", "feather", 1, 0, 1, 1);
