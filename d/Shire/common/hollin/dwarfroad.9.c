/* File: /d/Shire/common/hollin/dwarfroad.9.c */ 
...
create_dwarfroad()
{
    ...
    set_long(
        ...
        + "A small path continues into the Forest here to the north, but you can see "
	+ "that it is closed by some big trees laying on the ground. The Road continues "
    ...
    add_item(({"path"}),
        "A small path winds between the trees to the north here.\n"
    );
    add_exit(TEMP_DIR + "dwarfroad.10", "northeast", 0);
    add_exit(TEMP_DIR + "dwarfroad.8", "southwest", 0);
/* Removed from now, Sir Rogon.
..
    add_exit(STAND_DIR + "fpath.1", "north", 0);
*/
}
