...
create_monster()
{
     ...
     set_long(
     	...
     	+"made by Morgoth in his days. They usually lead large bands of orcs, "
        +"and are not common here inside tunnels or caves. It is a very special "
        +"race, a crossing between the feared Olog-hais and Variags, and they "
        +"represent the uttermost evil of Sauron's army. They are powerful and "
        +"horrible to fight.\n");
     ...
     set_stats(({250, 120, 280, 155, 150, 120}));
     set_hp(20000);
     MAKE_MONEY;
     set_act_time(3);
...
{
	object wep;
	wep = clone_unique(TROLLSH_DIR + "weapon/oakstaff", MAX_UNIQUE_ITEMS, TROLLSH_DIR + "weapon/tr_scimitar");
	wep->move(TO);
...
do_die(object killer)
{
   ...
   killer->add_prop("_i_killed_trollshaw_trolls",5);
    ::do_die(killer);
...
/* I have commented this out since it lacks proper checks to
 * prevent the troll leaving the domain Shire
 * Olorin, June 16th, 1995
