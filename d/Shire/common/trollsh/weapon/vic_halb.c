 ...
 *      Igneous Sept, 12/96
 *
 *    Added keepability functionally, -Igneous Dec 13th 1996
 */
...
did_hit(int aid, string hdesc, int phurt, object enemy, int dt, int phit, int dam)
{
   ...
   if (phurt == -1 || phurt == -2)
      ...
      tell_watcher(QCTNAME(tp)+" swings "+HIS_HER(tp)+" "+short()+
         " dangerously close to "+QTNAME(enemy)+"'s "+hdesc+".\n",enemy);
      return 1;
   }    
   else if(phurt < 5)
      {
   ...
   if (enemy->query_hp() <= 0)
      ...
      tell_watcher(QCTNAME(tp)+" swings "+HIS_HER(tp)+
      " "+short()+" powerfully into "+QCTNAME(enemy)+" "+
      hdesc+"!\n"+CAP(matter)+" splashes right onto "+
      QCTNAME(tp)+"'s face.\n"+
      QCTNAME(enemy)+" is massacred and thrown several feet " +
      "into the air by "+QTNAME(tp)+"'s deadly strike!\n"+
