...
// defined in gem_wep.c
    ...
    add_item(({"cryptic runes","runes","rune", "base runes"}),
        ...
        "dark power.\n");
    add_item(({"blade"}),
        "The blade is made of mithril and very light. A ridge runs down " +
        "the middle of the blade, strengthening it. Cryptic runes run " +
        "up the blade on either side of the ridge.\n");
    add_item(({"ridge"}),
        "The ridge runs up the blade, strengthening it without making " +
        "the blade any heavier. Runes run along both sides of it.\n");
    add_item(({"handle"}),
        "The handle connects to the blade and is wrapped with dark " +
        "leather. It is large enough so that the blade can be wielded " +
        "by one hand.@@query_jewel_desc@@\n");
    add_item(({"leather", "dark leather"}),
         "The dark leather is made from some unknown animal. It is " +
         "polished smooth from countless hands which have wielded the " +
         "blade yet appears as new as the day it was wrapped around " +
         "the grip.\n");
...
// starting gem
   set_wep_jewel("jade", GEM_SLIGHTLY_RARE);
    ...
    add_prop(MAGIC_AM_ID_INFO,({"The wep is magically enchanted.\n", 5,
       "Dark runes cover the " + gem_place + " of the wep of Golughosh. " +
       ...
       "wep has greater hit and penetration than normal weapons " +
       "and has several unique  abilities. Failure to wield it " +
       "may result in bodily harm.\n",
       55}));

    add_prop(OBJ_S_WIZINFO, "The wep is enchanted by use of gems that"   +
