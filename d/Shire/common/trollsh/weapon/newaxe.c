...
action_gem_pry(string str)
{
    ...
    if(str =="gem from axe" || str =="jewel from axe" || str == gem + " from axe")
    {
        if (query_wielded() == this_player())
        {
...
// let's rip the gem from the handle...
...
create_weapon()
{
    ...
    set_long("This is a corrupted mithril axe, once forged by " +
        "the elves of Lindon. It was used in countless wars " +
        "against the Dark One, but was stolen during one of their " +
        "battles. The Dark One took the axe " +
