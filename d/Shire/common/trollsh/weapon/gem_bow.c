...
//            " the " + LANG_THESHORT(this_object())+
            "the " + gem_place + ". It looks like it can be pried out");
...
action_wep_insert(string text)
{
    ...
    if (!text || !parse_command(text, ({}), "%s 'in' / 'into' [the] %s",
        ...
        (temp[0] != this_object()))
    {
       notify_fail("Insert {gem} into longbow?\n");
       return 0;
    }
...
create_bow()
{
    ...
    set_long("This is a " + short() + " was made by elves ages ago. The " +
        "longbow is almost as tall as an elf and the limbs are decorated " +
        "with runes. This once fair weapon was stolen by the Dark Lord's " +
        "servants and their black hands have twisted it to their own " +
        "purposes. @@check_string@@ " + "@@query_jewel_desc@@\n");

    add_item(({"cryptic runes","runes","rune", "base runes"}),
        "The runes decorate the " + gem_place + ". They run along the " +
        "limbs and are evil looking and seem to shine with a dark power.\n");
    add_item(({"mithril","true silver", "silver-steel", 
            "moria-silver","grey-gleam"}),
        "Mithril is the rarest metal in all the world. It is " +
        "lightweight yet durable, retaining its strength against the " +
        "hardest objects.\n");
    ...
    add_item(({"handle"}),
        ...
        "does not bend while shooting the arrow and is quite thick." +
        "@@query_jewel_desc@@\n");
...
//    set_dt(W_SLASH|W_IMPALE);
   ...
   set_wep_jewel("jade", GEM_SLIGHTLY_RARE);
    ...
    add_prop(MAGIC_AM_ID_INFO,({"The bow is magically enchanted.\n", 5,
       "Dark runes cover the " + gem_place + ". " +
       "They pierce your mind, saying that it may be commanded " +
       "or be fortified by it.", 25, 
