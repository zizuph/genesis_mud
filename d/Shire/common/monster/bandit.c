/* 
 * This bandit reads and blocks the exits of any room in which he calls
 * init_living().
 * He includes several triggers, chats, and actions, and will not hold
 * extra items in his inventory.  Only money and worn/wielded items are
 * kept.
 */
...
int *gold_wanted, old_gold;  /* how much to still take from each player, and 
                              * the gold he had before he was given something.
                              */
string *victims, *exit_cmds; /* the players who are trapped, and the commands
                              * that the bandit won't let them use.
                              */
...
void create_monster()
{
  ...
  string *poss_races, *first_adj, *secnd_adj; /* variable short desc w/o VBFC */
  object dagger, cloak;
  ...
  poss_races = (({"orc", "goblin", "human", "gnome", "hobbit"}));
  first_adj = (({"sneaky", "wicked", "mischievous", "clever", "mean", "sly", "scrappy", "scruffy"}));
  secnd_adj = (({"snickering", "grinning", "smirking", "sneering", "chuckling"}));
  victims = (({}));
  gold_wanted = (({}));
  ...
  add_name("mugger");
  add_name("highwayman");
  set_race_name(poss_races[random(sizeof(poss_races))]);
  if (query_race_name() == "hobbit")
    clone_tobacco();

  set_alignment(-100);
  set_short(first_adj[random(sizeof(first_adj))] + " " +
            secnd_adj[random(sizeof(secnd_adj))] + " " +
            query_race_name() + " bandit");

  set_long(break_string("This young " + query_race_name() + " has a " +
           "mischievous grin on his face, probably because he really enjoys " +
           "the idea of taking your hard-earned money.  He must be a member " +
           "of that gang of thieves you'd heard were hanging around the " +
           "area.\n", 70));
  ...
  set_stats(({ 30, 75, 28, 40, 29, 70}));
  set_skill(SS_UNARM_COMBAT, 50);
  set_skill(SS_WEP_SWORD, 32);
  set_skill(SS_WEP_CLUB, 40);
  set_skill(SS_WEP_KNIFE, 88);
...
// int query_knight_prestige() { return 75; }  Did not work because of gamedriver changes  Dain.
