...
#define PHRASES ({ "How dare you invade my caves.", \
                   ...
                   "I take no prisoners.", \
                   "Over my dead body.", \
                   "Only I hold the key." }) /* Quest hint */
...
create_monster()
{
    ...
    seteuid(getuid());
    set_name( ({ "sludge", "lord" }) );
    set_pname( ({ "lords", "sludges" }) );
    set_short("lord sludge");
    set_pshort("lord sludges");
    set_long("It seems he has been through many battles.");
    set_race_name("sludge lord");
...
attacker_out_of_mana(object enemy)  /* Called from his sword during attack */
{
	...
	say("The sludge lord sighs helplessly.\n");
