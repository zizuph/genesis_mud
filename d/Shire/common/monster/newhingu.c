...
create_monster()
{
    ...
    set_name("puddifoot");
   set_title("the Guard of the Hin Warriors");
    set_race_name("hobbit");
    set_long("This is the guard of the Hin Warriors. He looks as if he has "
    	    +"a lot on his mind- perhaps you could help?\n");
    ...
    set_chat_time(2);
   add_chat("The Hin Warriors have always lived here, seemingly.");
...
return_introduce(string who)
{
    ...
    if (obj = present(who,environment()))
    {
        ...
        if (ran == 0)
        {
            command("say Good day " + obj->query_cap_name() +", are you going to join the Hin Warriors?");
...
void mapping_quest()
{
    ...
    command("say We have seen fires on top of Hsqwin Mountain for weeks now.");
  ...
  { 
   ...
   if(obj->id("_mapping_quest_map_")) test_mapping_quest(from);
   else
      {
        ...
        write("The hobbit drops the "+obj->query_short()+".\n");
