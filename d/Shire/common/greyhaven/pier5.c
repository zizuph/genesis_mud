...
create_shire_room()
{
    ...
    add_item(({"sign"}),"The sign reads: Ship to Sybarus has been discontinued.\n");

    add_exit("/d/Shire/common/greyhaven/path5","east",0,1);
    add_prop(ROOM_I_NO_CLEANUP,1);

    /*set_alarm(3.0,0.0,"add_ship");*/
}
...
do_read(string str)
{
    ...
    write("The sign reads: Ship to Sybarus has been discontinued.\n");
    return 1;
}
...
add_ship()
{
  /*
    if (!ship)
    {
	ship = clone_object(SHIP_DIR + "ship3/ship");
