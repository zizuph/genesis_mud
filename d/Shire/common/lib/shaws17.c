...
create_shawrom()
{
	...
	+"unpredictably scatered and are covering the ground. Besides if "
	+"you should choose to jump down into that armada of bushes you "
	+"would surly hurt yourself very bad, mayby even die. To the north "
...
string
cliff_bend(string str)
{

	SAY (QCTNAME(TP) + " leans out over the cliff too look down but quickly "
			 + "stepp back, you wonder why....\n");

	WRITE ("The cliff is formed by the ridge your standing on. "
	     +"As you bend over the edge too look down you feel a little "
...
object ob;
	...
	if (ob->query_stat(SS_DIS) > 100)
	{
	
	WRITE("You step forward too jump off the cliff........\n"
