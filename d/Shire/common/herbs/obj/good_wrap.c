...
/* Changes:
 ...
 02/29/2004 - Palmer
 Wraps only create bundles from glowing herbs
...
public int
query_glowing(object o)
{
    return o->check_recoverable();
...
get_herbs(object o)
{
    object *things, *all_herbs, *glowing_herbs;
    
    things = all_inventory(this_player());
    all_herbs = filter(things, valid_herb);
    glowing_herbs = filter(all_herbs, query_glowing);
    return glowing_herbs;
}
...
{
    object herb, *all_herbs, bundle;
    int *decay_rates, weight, ttl_weight;
    int volume, ttl_volume, diff;
    string full_desc, unknown_desc;
   
    if (TP->query_prop("_unwrapping_bundle_"))    
    {
        write("You fumble as you try to wrap so quickly. Slow down.\n");
        return 1;    
    }
    if (!stringp(str))
    {
    ...
    if (str == "herb")
    {
        write("You must identify the herb by name.\n");
        return 1;
    }
   
    herb = get_herb_ref(str);
    Herb_name = herb->query_herb_name();
    
    if (str != Herb_name)
    {
        // First we check if they used singular name: pawnrose
        // If that doesn't work, see if they typed plural: pawnroses
        str = str[0..(strlen(str) - 2)];
        herb = get_herb_ref(str);
        Herb_name = herb->query_herb_name();
        if (str != Herb_name)
        {
            write ("You can't identify any herb in your possession " +
                "by that name.\n");
            return 1;
        }
    }
            
    // See if they have item in inventory
    if (!objectp(herb))
    {
        write ("The " + str + "is not an herb.\n");
        return 1;
    }
    
    // If they have the item, make sure its a valid herb name
    if (!valid_herb(herb))
    {
        write ("You cannot identify any herb in your " +
            "possesion by that name.\n");
        return 1;
    }

    
    all_herbs = get_herbs(herb);
    Nbr_herbs = sizeof(all_herbs);

    if (Nbr_herbs == 1)
    {
        write("You have to wrap more than one herb.\n");
        return 1;
    }

    if (Nbr_herbs > MAX_HERBS)
        Nbr_herbs = MAX_HERBS;

    decay_rates = make_decay_list(all_herbs);
    
    full_desc = herb->query_id_long();
    unknown_desc = herb->query_unid_long();
    diff = herb->query_id_diff();
    
    weight = herb->query_prop("_obj_i_weight");
    ttl_weight = weight * Nbr_herbs;
    
    volume = herb->query_prop("_obj_i_volume");
    ttl_volume = volume * Nbr_herbs;
    
    bundle = create_bundle(herb, decay_rates, ttl_weight, ttl_volume,
                         full_desc, unknown_desc, diff);
                         
    bundle->add_to_master();
    bundle->move(this_player());
    bundle->stop_decay();
    destroy_herbs(all_herbs);        
    find_player("palmer")->catch_tell(TP->query_name() + " wrapped.\n");
    write("You wrap your " + Nbr_herbs + " " + str + " into a bundle.\n");
    say(QCTNAME(TP) + " wraps some herbs into a bundle.\n");
