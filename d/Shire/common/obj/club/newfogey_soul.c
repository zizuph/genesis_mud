...
#define FOGEY_SUBLOC "_fogey_subloc_"
#define FOGEY_LIMP "_fogey_is_limping_"
...
show_subloc(string subloc, object who, object for_obj)
{
	...
	if(for_obj == who)
		return "You are an old Fogey.\n";
	else
		return CAP(HE_SHE(who))+" is an old Fogey.\n";
}
...
{
    player->set_m_in("arrives limping.");
    player->set_m_out("leaves limping");
}

void
stop_limp(object player)
{
    player->set_m_in("arrives.");
    player->set_m_out("leaves");
}
