 ...
 * Palmer,  01-Mar-04: Added 'hsort pouch' to remove non-glowing herbs from pouch.
 ...
 * Description  : Stops herb's decay and checks that it is an herb.
 ...
 * Description  : Restarts the decay in an herb.
 * Arguments    : object obj - the object that's leaving
 * Returns      : 0 always
 */
public int
prevent_leave(object obj)
{
    obj->restart_decay();
    return 0;
}
...
start_sort(string arg)
{
    ...
    {
        do_sort();       
	write("You remove non-glowing herbs from the pouch.\n");
        return 1;
    }
