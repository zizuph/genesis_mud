...
join_to_guild_from_statue()
{
    ...
    if (is_expelled(this_player()->query_real_name()))
    {
        write("You cannot join because you are expelled from the Guild.\n");
        return 0;
    }
    else
    {
...
do_expell(string str)
{
    ...
    if (is_founder(ob->query_real_name()))
    {
	notify_fail("Nob Nar is very fond of " + ob->query_name()
        + " and will not let you do it.\n");
	return 0;
    }
...
do_enlight(string str)
{
	...
	this_player()->query_wiz_level() <= WIZ_APPRENTICE)
    {
...
{
    if (this_player()->query_wiz_level() < WIZ_APPRENTICE)
	return 0;
