...
init_msg(object who)
{
	...
	{
#if !DEBUG
	    add_prop(OBJ_M_NO_GIVE,1);	/* Normally non giveable	*/
#endif
	    add_prop(OBJ_I_WEIGHT,100);	/* A very light pipe, 100 grams */
	    add_prop(OBJ_M_NO_DROP,"You like the pipe too much to drop it.\n");
	    add_prop(OBJ_I_VOLUME,20);	/* A small pipe, 0.02 litres	*/
	    add_prop(OBJ_M_NO_INS,1);	/* It is not a container	*/
	    add_prop(OBJ_M_NO_STEAL,1);	/* It is to be stolen		*/
	    add_prop(OBJ_M_NO_TELEPORT,1);	/* no teleports, please */

	    if (!no_noise)
		who->catch_msg(SHIRE_COUNCIL->query_hailing_message());

	    if (online == 0)
		who->catch_msg("Warning! Your pipe is offline so you cannot receive any messages!\n");

#if DEBUG
	    who->catch_msg("Your name: "
		 + capitalize(environment()->query_real_name())
		 + ", level: " + environment()->query_wiz_level() + "\n"
		 + "You are a member, ");

	    if (is_founder(environment()->query_real_name()))
		who->catch_msg("a founder");
	    else
		who->catch_msg("not a founder");

	    if (is_a_shire(who))
		who->catch_msg(" and a true Hin Warrior.\n");
	    else
		who->catch_msg(", but not a true Hin Warrior.\n");
#endif

#ifdef OWN_SOUL
            /* Check for the soul */
	    if (member_array(OWN_SOUL, who->query_cmdsoul_list()) == -1)
	    {
		if (!add_soul_at_top(OWN_SOUL, who))
		{
		    who->catch_msg("You lack the Hin Warrior feelings.\n");
		}
	    }
#endif
	}
	...
	{
	    who->catch_msg("You are not allowed to use the pipe. "
			 + "You're not a Hin Warrior!\n"
