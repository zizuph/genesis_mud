 ...
 * Cloned by ~Shire/common/hobbiton/cabin
 */
...
void dont_return_letter(mixed ob);
...
dont_return_letter(mixed ob) {
  ...
  command("say My what an odd letter. Not often one encounter a note in orc.");
...
enter_inv(object ob, object from) {
  ...
  if(!from->test_bit(DOMAIN, QUEST_GROUP, QUEST_BIT1)) {
    set_alarm(2.0,0.0,&dont_return_letter(({ ob , from })));
    return;
  }
