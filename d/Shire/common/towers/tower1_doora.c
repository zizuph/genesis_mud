...
create_door()
{
    set_other_room(STAND_DIR + "tower11");
    set_door_id("black door");
    set_door_name(({"black door","north door"}));
    set_door_desc(break_string(
	  "The black door is made of solid oak, and strengthened with "
	+ "iron bars. There is an iron lock on it.\n", 70));
    set_open_desc("There is an opened black door to the north.\n");
    set_closed_desc("There is a closed black door to the north.\n");
