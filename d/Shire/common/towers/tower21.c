...
create_room()
{
    set_short("Ground level of southern tower");
    set_long(break_string(
	  "You are on the ground level of the southern tower. "
	...
	+ "it will probably be a long way up.\n", 70));

    add_item("window", "It looks possible to climb out through the window.\n");

    /* Climbing the stairs is tiresome... */
    add_exit(STAND_DIR + "tower22", "up", 0, 4);
    add_exit(STAND_DIR + "path13", "north");

    add_prop(ROOM_I_INSIDE, 1);
    add_prop(ROOM_I_LIGHT, 0);

    clone_object(STAND_DIR + "tower2_doorb")->move(this_object());
}
