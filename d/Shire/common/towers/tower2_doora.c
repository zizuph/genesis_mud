...
create_door()
{
    set_other_room(STAND_DIR + "tower21");
    set_door_id("red door");
    set_door_name(({"red door","south door"}));
    set_door_desc(break_string(
	  "The red door is made of solid oak, and strengthened with "
	+ "iron bars. There is a brass lock on it.\n", 70));
    set_open_desc("There is an opened red door to the south.\n");
    set_closed_desc("There is a closed red door to the south.\n");
