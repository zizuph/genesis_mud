...
#define RIPP_OFF 110	/* % of the value that is added to get sell-value   */
...
create_room()
{
    ...
    set_long(break_string(
     ...
     + "This sure is a old, special place. The shopkeeper, an old "
...
do_sell(string str)
{
    ...
    {
	tell_room(TO,
	      "The owner says: You have something inside "
    ...
    {
	tell_room(TO,
	      "The owner sighs and says: I am sorry, my dear "
	    + (TP->query_gender()==0? "Sir" : "Lady") + ", but "
	    + COMPOSITE_DEAD(not_int_sell) + " is not interesting for me to buy.\n");
    }
    if (sizeof(no_value))
    {
	tell_room(TO,
	    "The owner laughs merrily.\n" +
	    "The owner says: Really, my dear " + (string)TP->query_race_name() + "!\n" +
	    "The owner says: Did you expect me to buy " + COMPOSITE_DEAD(no_value) + "?\n" +
