...
make_npcs()
{
   if(!hob1)
      {
      hob1 = clone_object(BREE_DIR + "npc/ponyhobbit");
      hob1->move(TO);
   ...
   if(!hob2)
      {
      hob2 = clone_object(BREE_DIR + "npc/ponyhobbit2");
      hob2->move(TO);
      tell_room(TO,capitalize(LANG_ADDART(hob1->query_nonmet_name()))+
   ...
   if(!ranger)
      {
      ranger = clone_object(BREE_DIR + "npc/ponyrang");
      ranger->move(TO);
   ...
   if(!man)
      {
      man = clone_object(BREE_DIR + "npc/ferny");
      man->move(TO);
   ...
   if(!dwarf)
      {
      dwarf = clone_object(BREE_DIR + "npc/dwarf");
      dwarf->move(TO);
    ...
    if(!spoon)
    {
      ...
      spoon->move(TO);
      spoon->add_prop(OBJ_I_HIDE, 15 + random(25));
    }
...
{
  if(!str) { NFN0("What do you want to stoke?"); }
/*
