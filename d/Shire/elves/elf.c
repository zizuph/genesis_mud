...
void
special_hit_msg(mixed hitresult, object enemy)
{
    string how;
    
   object me;
   mixed *hit_type, *emote_type;
   int x,y;

    hit_type=({"left leg", "right leg", "chest", "hip", "back","neck","left arm",
    "right arm","head"});
    
   emote_type=({"frowns", "sighs", "smirks", "smiles confidently", "howls joyously",
   "cheers triumphantly","cackles victoriously"});

    x = random(sizeof(hit_type));
    y = random(sizeof(emote_type));
   me = TO;
   if (random(5))
      return 0;
   
   // how = "deflects off of your";
   if (hitresult[0] > 0)
      how = "deflects off of your";
   if (hitresult[0] > 5)
      how = "slightly bruises your";
   if (hitresult[0] > 10)
      how = "cuts into your";
   if (hitresult[0] > 15)
      how = "slashes your";
   if (hitresult[0] > 20)
      how = "slices deeply into your";
   if (hitresult[0] > 25)
      how = "viciously slashes your";
   if (hitresult[0] > 30)
      how = "cleaves deeply into your";




	tell_room(ENV(TO), QCTNAME(TO) + " "+emote_type[y]+" as "+HIS_HER(TO)+
	 " " + TO->query_weapon(-1)[0]->query_name() + " "+how+" "+hit_type[x]+"!\n", enemy);
	 
	enemy->catch_vbfc(QCTNAME(TO) + " "+emote_type[y]+" as "+HIS_HER(TO)+" "
	+ TO->query_weapon(-1)[0]->query_name() +" "+how+" "+hit_type[x]+"!\n");
	
	// elf viciously slashes your left leg with his sword 
}
