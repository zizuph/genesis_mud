...
void create_area_room()
{
	...
	areatype = 0;
	extraline = "The hills here are dry, covered in a long grass. Bushes sprout among the hills here and there, and especially in and around the gullies. The Misty Mountains loom menacingly overhead, filling the view to the west. To the east, a wide expanse of field can be seen. The land grows marshy near the river, and further east the dark presence of Mirkwood Forest broods. A path here leads down one of the gullies, out onto the fields.\n";
	
	add_item("path", "You can probably follow the path down out of the foothills.\n");
	...
	add_exit(DOOR_DIR + "hill2", "west");
	add_exit(DOOR_DIR + "hill3", "north");
...
int follow_path(string str)
{
	if(str == "path")
	{
		TP->catch_msg("You follow a path out of the foothills.\n");
		say(QCTNAME(TP) + " follows a path out of the foothills.\n");
		tell_room(FOOT_DIR + "fd13", QCTNAME(TP) + " arrives, trudging down out of the foothills.\n");
		TP->move_living("M", FOOT_DIR +"fd13",1);
                return 1;
	}
