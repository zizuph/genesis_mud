...
void create_area_room()
{
	...
	add_exit(DOOR_DIR + "hill3", "east");
	add_exit(DOOR_DIR + "hill2", "south");
	add_exit(DOOR_DIR + "hill5", "northwest");
...
void make_npcs()
{
    ...
    {
        if (!objectp(guards[index]))
        {
		guards[index] = clone_object(NPC_DIR + "orc_guard");
		guards[index]->move(TO);
		tell_room(TO, capitalize(LANG_ADDART(guards[index]->query_nonmet_name()))+ " rushes in, ready for a fight.\n");
	}
    }
}
