...

{
    ...
    set_long("You are walking along a weary dirt road, which perhaps " +
  	    ...
  	    "which is known as the Greenway. To the west, you can see " +
  	    "the bright, blue ribbon of the mighty Brandywine river " +
  	    "as it winds through the fields. The road you are on appears " +
  	    "to run directly into the wide stone path which cuts across " +
  	    "through the water, otherwise known as Sarn Ford.\n");
    ...
    add_item(({"sarn ford", "sarn", "ford", "Sarn Ford", "stone", "stones"}),
        ...
        "Gondor. You can see the wide stones which cut a path " +
        "across the river in the shallows, granting easy access for " +
        "any who wish to cross.\n");
    add_item(({"river", "brandywine"}),
        "You can see the sparkling waters of the wide Brandywine river " +
        "as it twists and cuts its way through the grass fields. The " +
        "wide, stone path of Sarn Ford cuts across the river just to " +
        "the west.\n");
    ...
    add_exit(SARNFORD_DIR + "eroad3.c", "east");
    add_exit(SARNFORD_DIR + "fordeast.c", "west");

    add_std_herbs("field", 1);
}
