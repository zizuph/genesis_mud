...

{
    ...
    set_long("You are walking along a wide, well-travelled dirt road which " +
        "branches off to the hobbit village of Longbottom, home of the " +
        "Longbottom Leaf. The road also continues south towards Sarn " +
        "Ford, and out of the Shire. If you were to follow the road " +
        "north from here, you will find yourself quickly entering " +
    ...
    add_item(({"gondor", "shire", "rohan", "bree"}),
        ...
        "or Gondor can eventually be found by following the road east " +
        "from here.\n");
    add_item(({"bush", "bushes", "undergrowth"}),
        "There are a couple of  thick clumps of bushes and undergrowth which " +
        "have been scattered in the fields both east and west of the " +
        "road you are on.\n");
    add_item(({"tree", "trees"}),
    ...
    add_item(({"longbottom", "village"}),
        "You are currently just outside the village of Longbottom. If you "+
        "take the southeast fork in the road here, you should enter the " +
        "village proper.\n");
    ...
    add_item(({"sarn ford", "sarn", "ford", "Sarn Ford", "stone", "stones"}),
        ...
        "Gondor. You can find it following the road south from here.\n");
    ...
    add_exit(SARNFORD_DIR + "wroad12.c", "north");
    add_exit(SARNFORD_DIR + "wroad10.c", "south");
    add_exit(LB_DIR + "rooms/road05.c", "southeast", "@@valley@@");

    add_std_herbs("field", 13);
}

int
valley()
{
write("You follow the road as it winds its way down to the " +
