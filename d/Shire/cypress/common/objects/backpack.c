/*
 * A backpack for Larston
 * Based on the backpack of Bree
...
long_description()
{
    string long_desc = "The "+short()+" large and made of dark leather. " +
        "The backpack is designed for travellers who need to carry " +
        "supplies for trips. If you " +
        "type <fillpack>, you will fill your backpack with " +
        "all your unused supplies except for money and " +
        "unused containers. <emptypack> will make you " +
        "remove those items from your backpack. ";

//    if (query_prop_setting(OBJ_M_NO_SELL) == NO_SELL_PROP)
    if (query_keep())
    {
        long_desc += "\n  For now the "+short()+" is protected from " +
...
do_keep(string str)
{
    if (!parse_command(str, TP, " [the] [sturdy] [leather] 'backpack' / " +
        "'backpack' / 'pack' "))
    {
