 ...
 *
 *  Following in the steps of Igneous, this file was ruthlessly 
 *  borrowed indefinately from the Shire, and modified to fit 
 *  the needs of Mysteria.
 *  -- Finwe, May 2001
 */
#pragma save_binary
#pragma strict_types
...
#include <ss_types.h>
#include "/d/Shire/cypress/sys/terrain.h"

inherit CYPRESS_ROOM;
 ...
 *      Global variables:
 */
static int     areatype,    /*  0 = no type desc, 1 = Path, 2 = Bushes,
                             *  3 = Trees,        4 = Farmland, 5 = Rocky,
                             *  6 = Snow,         7 = Forest,   8 = Road, 
                             *  9 = Barren,      10 = Village, 11 = Path, 
                             * 12 = Ruins,       13 = Marsh,   14 = Hills, 
                             * not implemented yet: 15 = Caves        16 = Passages
                             * Definitions found in ~Shire/CYPRESS/sys/terrain.h */
forest_density;
static string  area = "central",    // eg "northern"
areadesc = "field",                 // eg "green, grassy field", "dense thicket"
areaname,                           // eg "Anarion", "Lebennin", "Rohan"
land = "Cypress",                   // "Cypress"
grass = "green",                    // (1), (2), (3)
...
short_desc()
{
    ...
    if (!item_id("field"))
        add_item("field","The field is covered by "+grass
          + " grass.\n");
    if (!item_id("grass"))
        add_item("grass","The "+grass+" grass grows everywhere " +
        "on the field.\n");
    ...
    if (!item_id(areadesc))
        add_item(areadesc, "On the green "+areadesc+", "+grass+
          " grass grows along with various bushes.\n");
    if (!item_id("bushes"))
        add_item("bushes", "Some bushes growing here on the "
          + areadesc+". They grow in groups across the "+areadesc+".\n");
    ...
    if (!item_id("field"))
        add_item("field", 
        "The field is used for agriculture by the people of "
        + land+". It is covered by furrows and plough-marks "
        + "here and there.");
    if (!item_id("furrow"))
        add_item("furrow",
          "The furrows are fresh and deep in the ground.\n");
    ...
    case AREATYPE_SNOW:
    if (!item_id("ground"))
        add_item("ground","The ground is covered with snow.\n");
    if (!item_id("snow"))
    ...
    if (!item_id("trees"))
        add_item(({treetype, treetype+" trees", "trees"}),
          "The "+treetype+" trees growing here make up the most "
          + "of the forest.\n");
    ...
    if (!item_id("ground"))
        add_item("ground", "The ground is quite barren of " +
    ...
    if ( !item_id("ground") && !item_id("ruin") &&
      !item_id("stone") )
        ...
        "stones", }),
          "Small " + treetype + " trees grow " +
        "from cracks in the old ruins. Once works of " +
        "craft were built here, but they crumbled " +
