...
//    add_exit(RANGER_GON_DIR + "wcorr", "south", 0,0);
...
discriminate()
{
    ...
    else if (IS_DUNADAN(tp) || IS_SINDAR(tp))
        bargain = (bargain * 97) / 100;
...
/* -- finwe
    if (query_friend_or_foe(TP)<0)
        return not_welcome();
    else if (TP->query_skill(SS_PUNISHMENT)==NOTRADE
...
do_report()
{
    ...
    if((TP->query_wiz_level())
    {
        ...
        {
            TP->catch_tell("You look at the shopkeeper's report and " +
                "find that business is very slow. He hasn't purchased anything today.\n");
        ...
        {
            TP->catch_msg("You look at the shopkeeper's report and " +
    ...
    {
        TP->catch_tell("You find the customer lists unavailable to " +
