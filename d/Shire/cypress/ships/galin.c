...
create_monster()
{
    set_name("galin");
    add_name("captain");
    set_title("Captain of the Soaring Wind");
    set_living_name("_galin_");
    set_race_name("human");
    add_adj("thin");

    add_prop(OBJ_M_NO_ATTACK, "Strong magic protects this seafarer. Perhaps he's made a deal with the sorcerers of Shal'atain.\n");
...
buy(string str)
{
    ...
    buy_ob=clone_object(SHIP_DIR + "ticket");
    if (!buy_ob) {
    ...
    {
	if (present("the cypress line", TP))
	{
	    command("sigh");
	    command("say You already have a ticket!");
	    return 1;
	}
	command("smile");
	command("say Hop aboard. Newbies ride for free!\n");
	price = 0;
    }
    else
