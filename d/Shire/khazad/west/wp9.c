...
create_moria_room()
{
  ...
  set_long("You stand in a bend in the Western passage. Empty torch "+
     ...
     "into the univiting darkness.\n");

  add_item(({ "bend", "corridor", "passageway", "passage" }),
     "The corridor bends to the northeast and southwest here.\n");
  ...
  add_exit("/d/Shire/khazad/west/wp8","southwest",0,1);
  add_exit("/d/Shire/khazad/west/wp10","northeast",0,1);
}
