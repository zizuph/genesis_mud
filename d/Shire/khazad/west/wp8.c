...
create_moria_room()
{
  ...
  set_long("You stand in a bend in the Western passage. Empty torch "+
     ...
     "Khazad-dum. To the northeast the corridor winds its way deeper into "+
     "the univiting darkness.\n");

  add_item(({ "bend", "corridor", "passageway", "passage" }),
     "The corridor bends to the northeast and south here.\n");
  ...
  add_exit("/d/Shire/khazad/west/wp7","southwest",0,1);
  add_exit("/d/Shire/khazad/west/wp9","northeast",0,1);
}
