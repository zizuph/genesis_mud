...
string pin_type = "snowman";

public void 
create_object()
{
    set_name(({"holiday", "pin"}));
    add_name("_shire_snowman_pin");
    set_adj(({"small", "snowman"}));
    set_short("small snowman pin");
    set_long("This is a nice stick pin in the shape of a snowman. Three white balls are stacked on top of each other. The top ball is smaller than the others. It is decorated with black dots for eyes, the nose, and a smile. A black top hat sits on top of it. Something is etched on the back of the pin.\n");
...
wear(object obj)
{
    ...
    write("You pin the " + short() + " onto your left breast.\n");
    say(QCTNAME(this_player()) + " pins a " + short() + " onto " +
        this_player()->query_possessive() + " left breast.\n");
...
remove(object obj)
{
    ...
    write("You remove the " + short() + " from your left breast.\n");
