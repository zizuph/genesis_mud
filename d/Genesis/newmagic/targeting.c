...
/*
 * Function name: spell_target_one_distant_player
 * Description:   Target a spell on a player in the game
 * Arguments:     string str - arguments to spell command
 * Returns:       An array of targets
 */
public object *
spell_target_one_distant_player(object caster, string str)
{
    object living;
    ...
    living = find_player(str);

    if (!objectp(living) || 
        (living->query_wiz_level() && living->query_invis()))
    {
    	caster->catch_tell("You do not sense " + capitalize(LANG_POSS(str)) +
                " presence in the realms.\n");
    	return ({ });
    }
