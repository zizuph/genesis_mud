 ...
 *     craftsman_set_item_file(order_id, "/d/Emerald/shiva/newcloak");
 * 
...
public void craftsman_config_order(int id, string str);
...
public string craftsman_query_item_file(int id);
 ...
 *                2. (int)     the order id
 *                3. (int)     the position of the current attribute
 * Returns:       1/0 - attribute set/not set
 */
public int
craftsman_set_attribute(string value, int id, int pos)
{
    mixed *order = m_orders[id];
    mixed attrs;
    string attr_nm;
    ...
    attr_nm = order[ORDER_ATTRS][pos][0];
    attrs = order[ORDER_ATTRS][pos][1];
 ...
 * Arguments:     1. (int) the order id
 *                2. (int) the position of the current attribute
 */
public void
craftsman_config_attributes(int id, int pos)
{
    ...
    if (pos < sizeof(order[ORDER_ATTRS]))
    {
	    ...
	    seteuid(getuid());
            ...
            input_obj->get_input(&craftsman_set_attribute(, id, pos));
            
            order[ORDER_INPUT] = input_obj;
	}
#else
        input_to(&craftsman_set_attribute(, id, pos));
#endif
    }
}
...
craftsman_present_order(int id)
{
    ...
    ob = craftsman_clone_item(craftsman_query_item_file(id), id);
...
/*
 * Function name: craftsman_add_selection
 * Description:   Add an attribute selection to an order.  An attribute whose
 *                name begins with '_' is considered to be hidden; i.e. it is
 *                not displayed to the user.
 * Arguments:     1. (int)    the order id
 *                2. (string) the name of the attribute
 *                3. (mixed   the value for the attribute
 */
public void
