 ...
 * not recover their link within a certain time, they are destructed.
 */
...
long_description()
{
    ...
    string text = "The hall is part of the Keepers' art collection. This " +
        "is where they keep the statues of some unfortunate adventurers. " +
    ...
    /* Info about linkdeath duration is for wizards only. */
    ...
    /* No statues found. */
    if (!sizeof(statues + nonpresent))
	return text;

    foreach(object statue: statues + nonpresent)
    {
        lines += ({ sprintf("%-11s %s", capitalize(statue->query_real_name()),
            TIME2STR(time() - statue->query_linkdead(), 2) ) });
    }

    scrw = ((scrw >= 40) ? (scrw - 3) : 77);
    return text + sprintf("%-*#s\n", scrw, implode(sort_array(lines), "\n")) + "\n";
}
...
create_room() 
{
    ...
    add_exit("/d/Genesis/start/human/town/tower", "south", may_not_leave, 0);
...
query_linkdead_players()
{
    ...
    players = filter((statues + nonpresent), &not() @ interactive);
    /* Filter out novices with stat average up to SD_AV_NOVICE. */
    players = filter(players, &not() @ &operator(<=)(, SD_AV_NOVICE) @ &->query_average_stat());
...
remove_linkdead_player()
{
    ...
    players = filter(statues, &operator(<=)(, (time() - LD_PERIOD)) @ &->query_linkdead() );
    statues -= players;

    /* Find the moment the next statue should be destructed. */
    if (sizeof(statues))
    {
        delays = map(statues, &operator(+)(LD_PERIOD - time()) @ &->query_linkdead());
        delay = applyv(min, delays);

        alarm_id = set_alarm(itof(delay + 1), 0.0, remove_linkdead_player);
    }
...
linkdie(object ob) 
{
    ...
    {
	set_this_player(ob);
        ob->catch_tell("\nLink lost during Armageddon. You are logged out.\n");
	...
	tell_room(environment(ob), ({
    	    capitalize(ob->query_real_name()) + " loses touch with reality.\n",
    	    "The " + ob->query_nonmet_name() + " loses touch with reality.\n",
    	    "" }),
	    ({ ob }) );
    }
    else
    ...
    foreach(string prop: props_to_save)
    {
	ob->add_prop(STATUE_ROOM + prop, ob->query_prop_setting(prop));
    }
...
revive(object ob)
{
	...
	{
	    ob->remove_prop(prop);
	    ob->remove_prop(STATUE_ROOM + prop);
	}
    }
    else
    {
	...
	foreach(string prop: props_to_save)
	{
	    ob->add_prop(prop, ob->query_prop_setting(STATUE_ROOM + prop));
	    ob->remove_prop(STATUE_ROOM + prop);
	}
    }

    if (IN_ARRAY(ob->query_race(), RACES))
    {
	tell_room(environment(ob), ({
