...
/* Definitions */
#define            CENSUS      "/d/Genesis/census/census_master"
...
public string      Date3 = "";
public string      Date4 = "";
public string      Census1 = CENSUS_FILE;
public string      Census2 = CENSUS_FILE;
...
public int         test_float(string arg);
...
do_display(string arg)
{
    ...
    if (arg == "graph")
    {
        ...
        {
            census_arr = CENSUS->get_info(Census1, Date1);
        }
        ...
        if (Census1 != CENSUS_FILE)
        {
            write(sprintf("%|80s", 
                upper_case(SECURITY->query_guild_long_name(Census1))
              + "\n"));
        }
    ...
    if (arg == "yeargraph")
    {
        ...
        census_arr = CENSUS->census_year(Date1);
        ...
        CENSUS->census_graph_year(this_player(),
                                  census_arr,
                                  CENSUS->parse_date(Date1),
                                  Weekday,
                                  Which_Hour);
    ...
    if (arg == "chart")
    {
        if (!strlen(Date1))
        {
            notify_fail("The start date for Period A has not yet been set. Use"
              + " <dateset 1 yyyymmdd> to set it.\n");
            return 0;
        }
        ...
        if (!strlen(Date2))
        {
            notify_fail("The end date for Period A has not yet been set. Use"
        ...
        if (!strlen(Date3))
        {
            notify_fail("The start date for Period B has not yet been set. Use"
              + " <dateset 3 yyyymmdd> to set it.\n");
            return 0;
        }

        if (!strlen(Date4))
        {
            notify_fail("The end date for Period B has not yet been set. Use"
              + " <dateset 4 yyyymmdd> to set it.\n");
            return 0;
        }

        CENSUS->census_chart(this_player(), Date1, Date2, Date3, Date4);
        return 1;
    }
    ...
    if (arg == "graphs")
    {
        ...
        census_arr2 = CENSUS->get_info(Census2, Date2);
...
dateset(string arg)
{
    ...
    if (num == "census1")
    {
        if (strlen(date))
        {
            write("Census1 set to: " + date + ".\n");
            Census1 = date;
            return 1;
        }

        notify_fail("Guild not specified.\n");
        return 0;
    }
    ...
    dates = CENSUS->get_info(Census1, date);
    ...
    switch (lower_case(num))
    {
        ...
        case "2":
            Date2 = date;
            write("Secondary date set to: " + date + "\n");
            break;
        case "3":
            Date3 = date;
            write("Period B start date set to: " + date + "\n");
            break;
        case "4":
            Date4 = date;
            write("Period B end date set to: " + date + "\n");
            break;
        default:
...
public int
test_float(string arg)
{
    int     i;
    string  pre_f;
    float   post_f;
    mixed  *avg_arr = CENSUS->census_year(Date1);

    notify_fail("Useage example: <float 1.6 1>\n");
    ...
    if (pre_f == "test")
    {
        foreach(mixed avg: avg_arr)
        {
            CENSUS->place_float(avg[0], i);
        }

        return 1;
    } 

    sscanf(pre_f, "%f", post_f);

    switch(CENSUS->place_float(post_f, i))
    {
        case 0:
            write(ftoa(post_f) + " is below " + i + ".0\n");
            return 1;
            break;
        case 1:
            write(ftoa(post_f) + " is between " + i + ".0 and " + i
              + ".5\n");
            return 1;
            break;
        case 2:
            write(ftoa(post_f) + " is between " + i + ".5 and " + (i + 1)
              + ".0\n");
            return 1;
            break;
        case 3:
            write(ftoa(post_f) + " is greater than or equal to "
              + (i + 1) + ".0\n");
            return 1;
            break;
        default:
