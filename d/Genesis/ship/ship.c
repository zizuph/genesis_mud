// Adjusted board function for boarding of steeds with riders,
// Fysix, Sep 1997
// Replaced call_out by set_alarm etc.
...
static mixed	ticket_id;		/* Unique id for the ticket needed */
					/* to travel */
static int      callout_fq;             /* Alarm frequence - secs between */
                                        /* each alarm */
...
static int      call_no;                /* Counts how many alarms we stay */
					/*  somewhere */
static int      *time_between_stops;    /* Number of alarms with callout_fq */
					/* frequency */
static int      alarm_id;               // all purpose alarm id
	...
	if (captain)
   	{
	    captain->move_living("the ship", environment());
            captain->command("emote shouts: All aboard!  The " + short() +
		" leaves in " +
		(time = query_callout_fq() * time_between_stops[where]) +
		" seconds!");
            alarm_id = set_alarm(itof(time) - 20.0, 0.0, give_leave_warning);
	}
    }
...
sail_away_out_on_the_open_sea()
{
	...
	if (captain = query_captain())
		...
		captain->move_living("boarding the ship", query_deck()[0]);
		tell_room(pier,
		    capitalize(LANG_ASHORT(this_object())) + " starts moving away.\n");
	    }
	...
	if (places_to_go[location]->query_prop(ROOM_I_TYPE) != ROOM_IN_WATER)
	{
		...
		if (query_ticket_id())
		    captain->command("say Buy your tickets now or stay ashore!");
                set_alarm(itof(time) - 20.0, 0.0, give_leave_warning);
	    }
	    set_board_enabled(1);
	}
	else
...
{
    if (query_captain())
        query_captain()->command("emote shouts: All board the ship, we are leaving in 20 seconds!");
}

/*
 * Function name: random_event
 * Description  : Check if a random event should occur. If one is called,
 *		  alarms are disabled.
...
{
    /*
     * In case of event, the alarms are removed just in case
     * random_event was called from the outside - if not, no new
     * alarm will be issued in the first place. Remember that
...
board(mixed what)
{
    ...
    if (!stringp(ticket_id = query_ticket_id()) ||
    ...
    good_tickets = filter(all_inventory(this_player()), &->id(ticket_id));
    ...
    if (sizeof(good_tickets) < no_of_tickets_wanted)
    {
        if (query_captain())
        {
            ...
            write(capitalize(query_captain()->query_the_name(this_player())) +
             	" scowls at you.\n");
            str = (no_of_tickets_wanted > 1 ? "not enough tickets" :
                "no ticket");
            query_captain()->command("say You have " + str + ", "+
 ...
 * Description	: Set up alarm frequency. This is used to decide how
 *		  fast the ship moves
 * Arguments	: x - number of seconds between alarms
 */
public void
 ...
 * Description	: get the time interval to be used between alarms
