...
join_mercs(string str)
{
    ...
    else if (!who->query_prop(MALIGANT_JOIN_PERMISSION))
    {
	gRecruiter->command("say YouNeedMaligant'sPermissionToJoin,GoTalkToHim.");
	gRecruiter->command("smile hopefully");
        return 1;
    }
    else if (query_not_allowed(who->query_real_name()))
    {
	gRecruiter->command("say GetOut! GetOutImmediately! YouAreNotWelcomeHere!");
	gRecruiter->command("say MaybeMaligantDoesn'tRememberYou,ButMyBookDoesn't"+
	    "Lie.");
        return 1;
    }
...
leave_mercs(string str)
{
    ...
    {
	gRecruiter->command("say No! Don'tDoIt!");
    ...
    {
        write("The guards nearby regard you with mirth, but show no signs "+
	    "of agression.\n");
        say("Several guards nearby snicker as they look at "+QTNAME(who)+".\n");
    }
    else if (who->query_prop(MALIGANT_REDUCED_LEAVE_PENALTY))
    {
        if (level < LOW_PENALTY_LEVEL)
        {
            write("The guards nearby regard you with respect as you announce "+
	        "your intention to leave, and make no move to harass you.\n");
	    say("Several guards nearby regard "+QTNAME(who)+" with respect, and "+
		"make no move to harass "+OBJECTIVE(who)+" as "+PRONOUN(who)+
		"announces "+POSSESSIVE(who)+" intention to leave.\n");
        }
        else if (level < MID_PENALTY_LEVEL)
        {
            write("The guards nearby surround you, and shove you around a bit "+
		"before letting you go. You get the impression that they are "+
		"used to doing much worse.\n");
	    say("Several guards nearby surround "+QTNAME(who)+" and shove "+
		OBJECTIVE(who)+" around a bit before letting "+OBJECTIVE(who)+
		" go.\n");
	    who->set_hp(who->query_max_hp() * 3 / 4);
        }
        else if (level < FULL_PENALTY_LEVEL)
        {
            write("The guards nearby surround you, and begin to beat on you. "+
	        "You think back to Maligant's promise that they'd "+
	        "go easy on you. It sure doesn't feel very easy, though. "+
		"After a short while, they stop, and you are free to go.\n");
            say("Several guards nearby surround "+QTNAME(who)+" and begin to "+
	        "beat on "+OBJECTIVE(who)+". Momentarily, they stop, and "+
	        "allow "+OBJECTIVE(who)+" to go.\n");
            who->set_hp(who->query_max_hp() / 2);
        }
        else
        {
            write("The guards nearby surround you, and begin to beat you "+
	        "mercilessly. You think back to Maligant's promise that they'd "+
	        "go easy on you. It sure doesn't feel very easy, though. "+
		"After a short while, they stop, and you are free to go.\n");
            say("Several guards nearby surround "+QTNAME(who)+" and begin to "+
	        "beat "+OBJECTIVE(who)+" mercilessly. Momentarily, they stop, "+
		"and allow "+OBJECTIVE(who)+" to go.\n");
            who->set_hp(who->query_max_hp() / 4);
	}
    }
    else
    {
        ...
        {
            write("The guards nearby surround you, and shove you around a bit "+
		"before letting you go. Soon, they stop, and several even "+
		"give you a hearty slap on the back as you go.\n");
	    say("Several guards nearby surround "+QTNAME(who)+" and shove "+
		OBJECTIVE(who)+" around a bit before letting "+OBJECTIVE(who)+
		" go.\n");
	    who->set_hp(who->query_max_hp() * 3 / 4);
        ...
        {
            write("The guards nearby surround you, and begin to beat on you. "+
	        "You feel yourself getting roughed up quite a bit, and wonder "+
		"how long you can take this abuse. "+
		"After a short while, they stop, and you are free to go.\n");
            say("Several guards nearby surround "+QTNAME(who)+" and begin to "+
	        "beat on "+OBJECTIVE(who)+". Momentarily, they stop, and "+
	        "allow "+OBJECTIVE(who)+" to go.\n");
            who->set_hp(who->query_max_hp() / 3);
        }
        else
        {
            write("The guards nearby surround you, and begin to beat you "+
	        "mercilessly. You wonder if it will ever end. After what seems "+
		"like an eternity, they stop, and you are free to go.\n");
            say("Several guards nearby surround "+QTNAME(who)+" and begin to "+
	        "beat "+OBJECTIVE(who)+" mercilessly. Eventually, they stop, "+
		"and allow "+OBJECTIVE(who)+" to go.\n");
            who->set_hp(who->query_max_hp() / 10);
	}
    }
