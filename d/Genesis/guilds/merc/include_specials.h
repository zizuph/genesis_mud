 ...
 * Maximal at 120 OCC stat and a 40 weapon will be 110.
 *
 *
 * Changelog:                                                                  
 * 2006-01-18 Lunatari                                                        
 ...
 * 2006-01-19 Lunatari                                                         
 *  Added macros for the statmods and tohit calculations, this                 
 *  to make it easier to change and update them, also to make                 
 *  outer calls to these macros easier.                                     
 *                                          
 */
...
#define MERC_TOHIT(x,y) MIN(110, 70 + (MIN(20,(x->query_stat(SS_OCCUP)/6)) + MIN(20,(y->query_hit())/2)))
...
void do_special(object ob, object liv)
{
  ...
  set_alarm(itof(MATTACK_RESET), 0.0, &reset_special(liv));
    ...
    "Pen: "+pen+" | Skill: "+skill+" | Damage: "+hitresult[3]+" | Percent: "+
	hitresult[0]+"\n"+
	    "*************************************************************\n");
...
{
    if(liv->query_merc_shadow_version() >= MERC_SHADOWVERSION)
    { 
    ...
    { 
      /*
       * Old shadow, we have to reset it in the actual cmd code.
       */
      if (liv->query_skill(SS_MERC_AUTOMATTACK) && liv->query_attack())
      {
        liv->remove_prop(PLAYER_I_MATTACK_RESETTING);
	      liv->command("$mattack");
      	return;
      }

      if (liv->query_skill(SS_MERC_AUTOMATTACK))
      {
        liv->remove_prop(PLAYER_I_MATTACK_RESETTING);
      	return;
      }
...
int defend(string str)
{
    ...
    setuid();
    seteuid(getuid());
    (shadow = clone_object(MDIR+ "defended_shadow"))->shadow_me(ob);
