...
/*
 * Function name: m_count_elders
 * Description:   Returns the number of elders
 *                there currently are in the monks
 * Returns:       The number of elders
 */
static int
m_count_elders()
{
...
static int
m_count_council_and_elders ()
{
    return m_count_council_members () + m_count_elders ();
}
...
/*
 * Function name: m_query_council_or_elder
 * Description:   Check to see if the specified player
 *                is a council member or an elder.
 * Arguments:     who - string or object to check
 *                allow_wizards - if this flags is set, we
 *                    will also return true if the wizard
 *                    is a valid Genesis wizard
 * Returns:       1 if they are a council or elder, 0 if not
 */
static varargs int
m_query_council_or_elder(mixed who = 0, int allow_wizards = 0)
{
    if (m_query_council_member (who, allow_wizards))
        return 1;

    return MANAGER->query_elder(who);
}

/*
 * Function name: m_query_elder
 * Description:   Check to see if the specified player
 *                is an elder.
 * Arguments:     who - string or object to check
 * Returns:       1 if they are an elder, 0 if not
 */
static varargs int
m_query_elder(mixed who = 0)
{
    return MANAGER->query_elder(who);
}
