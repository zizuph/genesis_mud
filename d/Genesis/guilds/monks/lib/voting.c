...
static int
m_remove_elder(mixed who)
{
    m_log(m_get_name(who) + " was removed as an Elder.");
    MANAGER->set_elder_status (who, 0);
    return !m_query_elder (who);
}
 ...
 */
public int
m_start_elder_application(mixed who)
{
    string   name = m_get_name (who);
    string   desc;
    ...
    if (!strlen (who))
    {
	notify_fail ("Start a vote to elect who as an elder?\n");
	return 0;
    }

    if (!strlen (name) || !m_query_monk (name))
    {
	notify_fail (capitalize (who) + " is not a member of "
	    + "the Order\n");
	return 0;
    }

    if (m_query_elder (who))
    {
	notify_fail (capitalize (who) + " is already an Elder.\n");
	return 0;
    }

    if (sizeof (MANAGER->query_elders ()) > MONK_MAX_ELDERS)
    {
	notify_fail ("There are already the maximum number of "
	    + "allowed elders (" + LANG_WNUM (MONK_MAX_ELDERS) + ").\n");
	return 0;
    }

    desc = "Should " + capitalize(name) + " be an Elder of the monks?";

    desc += " Vote yes or no. A tie, or if no one votes, will "
        + "result in the elder election being approved. More no "
        + "votes than yes votes will result in the elder applicant "
        + "being rejected.";

    // add a new vote for the application, 30 day limit
    if (!m_add_vote(lower_case(name) + " elder application", desc,
        (M_VOTE_COUNCIL_ONLY | M_VOTE_YESNO),
        (60 * 60 * 24 * 30) + time(),
        MONKLIB, "m_tally_elder_application",
        "the Council",
        m_count_council_members()))
	{
	notify_fail ("That vote could not be started.\n");
	return 0;
	}

    write ("You begin a vote for " + capitalize (name)
	+ " to become an Elder of the Dragon Order.\n");
    return 1;
}

public int
m_start_elder_removal(mixed who)
{
    string   name = m_get_name (who);
    string   desc;
    ...
    if (!strlen (who))
    {
	notify_fail ("Start a vote to remove who as an elder?\n");
	return 0;
    }
    ...
    if (!m_query_elder (who))
    {
	notify_fail (capitalize (who) + " is not an Elder.\n");
	return 0;
    }

    desc = "Should " + capitalize(name) + " be removed as "
	+ "an Elder of the monks?";

    desc += " Vote yes or no. A tie, or if no one votes, will "
        + "result in the elder being removed. More no "
        + "votes than yes votes will result in the elder remaining.";

    // add a new vote for the application, 30 day limit
    if (!m_add_vote("remove elder " + lower_case(name), desc,
        (M_VOTE_COUNCIL_ONLY | M_VOTE_YESNO),
        (60 * 60 * 24 * 30) + time(),
        MONKLIB, "m_tally_elder_application",
        "the Council",
    ...
    write ("You begin a vote for " + capitalize (name)
	+ " to be removed as an Elder of the Dragon Order.\n");
    return 1;
}


public int
m_tally_elder_application(string *votes, string name, string by_who)
{
    int       num_winners;
    string    vote;
    int	      success = 0;
    ...
    // vote name should be "cirion elder application" for
    // election, or "remove elder cirion" for removal.
    if(sscanf(name, "remove elder %s", name))
    {
	removal = 1;
    }
    else if(sscanf(name, "%s elder application", name))
    {
	removal = 0;
    }
    else
	...
	if (success)
	{
	    m_post_board_msg("Vote: " + vname,
		"Ballot", capitalize(name) + " has been voted to "
		+ "be an Elder of the Dragon Order\n",
		MONKBOARDPRIV);
	    MANAGER->set_elder_status (name, success);
	    m_log (name + " was voted to be an Elder of the Order");
	    return 1;
	}
	else
	{
	    m_post_board_msg("Vote: " + vname,
		"Ballot", capitalize(name) + " has been voted to "
		+ "be denied status as an Elder of the Dragon Order\n",
		MONKBOARDPRIV);
	    MANAGER->set_elder_status (name, success);
	    m_log (name + " was voted to not be an Elder of the Order");
	    return 1;
	}
    }
    else // the vote was for removal
    {
	if (success)
	{
	    m_post_board_msg("Results of vote: " + vname,
		"Ballot", capitalize(name) + " has been voted to "
		+ "be removed as an Elder of the Dragon Order\n",
		MONKBOARDPRIV);
	    MANAGER->set_elder_status (name, !success);
	    m_log (name + " was voted to be removed as an Elder of the Order");
	    return 1;
	}
	else
	{
	    m_post_board_msg("Results of vote: " + vname,
		"Ballot", capitalize(name) + " has been voted to "
		+ "be not be removed as an Elder of the Dragon Order\n",
		MONKBOARDPRIV);
	    MANAGER->set_elder_status (name, !success);
	    m_log (name + " was voted to not be removed an Elder of the Order");
	    return 1;
	}
...
m_do_punish(string str)
{
    ...
    if (m_query_elder (this_interactive ()))
	write ("The Council will rule on this punishment and "
	    + "the appropriate resulting action will take place.\n");
 ...
 *                from the council or elder. Will first give a
...
#define MONK_RESIGN_WARNING_PROP    "monk_resign_warning_prop"
    ...
    if (m_query_elder (this_player ()) && m_query_council_member (this_player ()))
    {
	if (str != "elder" && str != "council")
	{
	    notify_fail ("Resign from which position? Council or Elder?\n");
	    return 0;
	}
	else
	    type = str;
    }
    else if (m_query_elder (this_player ()))
	type = "elder";
    else if (m_query_council_member (this_player ()))
	type = "council";
    ...
    if(!(this_interactive()->query_prop(MONK_RESIGN_WARNING_PROP)))
    {
        ...
        write("Are you sure you want to permanently step down "
            + "from the " + type + "? Enter the command again to "
            + "confirm.\n");
        return 1;
    }
    ...
    else // elder resigation
    {
	if(!m_remove_elder(this_interactive()))
	{
	    write("A problem has occured: you have not been removed.\n"
		+ "Please leave a bug report.\n");
	    m_log("ERROR: " + this_interactive()->query_real_name()
		+ " tried to resign from the Elders, but failed.");
	    return 1;
    
	}

	write("You resign from your position as an Elder.\n");
	m_log(this_interactive()->query_real_name() + " resigned from "
	    + "the Elders.");
    }
...
m_council_init()
{
	...
	add_action(m_start_elder_application, "melder");
	add_action(m_start_elder_removal, "munelder");
    }
}
...
public void vote_enter_inv (object who, object from)
{
    int num;

    num = m_do_vote_list ("", 1, who);

    if (num > 0)
    {
	// print the number of votes we have not yet cast
	set_alarm (1.0, 0.0, &m_tell_player (who, "There "
	    + (num == 1 ? "is " : "are ") + LANG_WNUM (num)
	    + " " + (num == 1 ? "vote" : "votes")
	    + " you have not yet particpated in.\n"));
    }
}
