...
m_query_title_base (object who = this_player())
{
    ...
    switch(rank)
    {
        ...
        case 21 .. 30:
            ...
            if(m_query_monk(who, SS_OCCUP))
                ret += ", Elder " + m_query_title_rank(who, "Acolyte")
                + " of the Dragon Order";
            break;
        case 31 .. 35:
            if (ret == "")
    ...
    if (ret == "")
        ret = "Undefined Monk of the Dragon Order";
...
m_do_application_stat(string str)
{
    ...
    if(stat == -1) // already been rejected
    {
        ...
        */
        ...
        notify_fail("Your application has been rejected. You may "
...
m_join_init()
{
    ...
    // support function for transferring from the old guild
    // to the new.
    if(m_is_old_member())
    {
        add_action(m_do_transfer, "transfer");
        set_alarm(1.0, 0.0, &m_tell_player(this_player(), "You sense that you can "
            + "'transfer' from Kaheda to the Order of the Dragon in "
            + "this room.\n"));
    }
...
/*
LAYMAN:
exec int occup = 0; object who = JR; string occ_shadow = "/d/Roke/common/gont/monks/obj/monk_occ_shadow"; string lay_shadow = "/d/Roke/common/gont/monks/obj/monk_lay_shadow"; int KAHEDA_GROUP = 0; int KAHEDA_MEMBER  = 12; int KAHEDA_OCCUP  = 13; object shadow; if(occup) { shadow = clone_object(occ_shadow); if (shadow->add_monk_shadow(who) != 1) throw("Error adding monk shadow");  who->set_bit(KAHEDA_GROUP, KAHEDA_OCCUP); } else { shadow = clone_object(lay_shadow); if (shadow->add_monk_shadow(who) != 1)  throw("Error adding monk shadow"); } who->set_bit(KAHEDA_GROUP, KAHEDA_MEMBER);

OCCUPATIONAL:
exec int occup = 1; object who = JR; string occ_shadow = "/d/Roke/common/gont/monks/obj/monk_occ_shadow"; string lay_shadow = "/d/Roke/common/gont/monks/obj/monk_lay_shadow"; int KAHEDA_GROUP = 0; int KAHEDA_MEMBER  = 12; int KAHEDA_OCCUP  = 13; object shadow; if(occup) { shadow = clone_object(occ_shadow); if (shadow->add_monk_shadow(who) != 1) throw("Error adding monk shadow");  who->set_bit(KAHEDA_GROUP, KAHEDA_OCCUP); } else { shadow = clone_object(lay_shadow); if (shadow->add_monk_shadow(who) != 1)  throw("Error adding monk shadow"); } who->set_bit(KAHEDA_GROUP, KAHEDA_MEMBER);
