    ...
    string by_who = this_player()->query_real_name())
{
    ...
    if(sizeof(member_data))
    {
        ...
        // ### this is so the restriction only kicks in after a month
        // past the guild's opening. Remove this check once this time
        // has elapsed (Thu Sep  9 08:16:02 1999)
        if (time () > 936857762)
        {
...
m_get_vote_list ()
{
    ...
    for (i=0;i<sizeof(vote_names);i++)
    {
        ...
        // if it is a council-only vote, and I am not on
        // the council, do not show the vote
        if((vote_props & M_VOTE_COUNCIL_ONLY) && !m_query_council_member(this_interactive(), 1))
            continue;
...
m_do_vote_list (string str)
{
    ...
    saw_votes = 0;
    write ("The following votes are available:\n");
    ...
    if (saw_votes)
    {
        write ("For more information on a particular vote, "
