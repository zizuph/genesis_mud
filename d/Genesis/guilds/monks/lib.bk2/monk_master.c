...
remove_member(mixed who)
{
    ...
    if (member_array (name, m_indices (members)) == -1 &&
	member_array (name, m_indices (council_info)) == -1)
        return 0;

    // remove them from the member list
    members = m_delete(members, name);
    council_info = m_delete(council_info, name);
 ...
 * Description:   Changes the guild type for the specified player,
 *                for transferring branches.
 * Arguments:     who - string or object of the player
 *                type - the new branch (SS_LAYMAN or SS_OCCUP)
 * Returns:       1 if successful
 */
public int
set_guild_type (mixed who, int type)
{
    string name = get_name(who);
...
clean_members()
{
    clean_member_list (members);
    clean_member_list (council_info);
}
...
/*
 * Function name: query_elder
 * Description:   Check to see if a player is an elder
 * Arguments:     who - player name or object to check.
 * Returns:       1 if they are an elder, 0 otherwise
 */
public int
query_elder (mixed who)
{
    string name;
    ...
    // sanity check
    if (sizeof (members [name]) < 6)
        return 0;

    return members [name][6];
}

/*
 * Function name: set_elder_status
 * Description:   Set whether a player is an elder or not
 * Arguments:     who - player name or object to set.
 *                stat - 1 to set them as a member, 0 to revoke.
 */
public void
set_elder_status (mixed who, int stat)
{
    string name;
    name = get_name (who);
