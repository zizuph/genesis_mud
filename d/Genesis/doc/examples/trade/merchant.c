...
#define STORE "/d/Genesis/doc/examples/trade/sack"
#define AXE   "/d/Genesis/doc/examples/weapons/axe"
#define MAIL  "/d/Genesis/doc/examples/armours/mail"
...
create_monster() 
{
    ...
    set_long("You are looking at a rough-bearded dwarf. He is " +
    ...
    set_store_room(file_name(store));
...
shop_hook_sold_items(object *item)
{
    ...
    tell_room(environment(), QCTNAME(this_object()) + " puts " +
        (sizeof(item) <= 1 ? "something" : "some things") + " in his sack.\n");
    return 1;
}
