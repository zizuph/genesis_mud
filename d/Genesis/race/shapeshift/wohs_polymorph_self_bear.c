/*
 *  Bear shapeshift creature for WoHS polymorph self spell
 *  Tanky with minimal damage output
 *
 *  Arman 2017
 */
...
string  *adj_list1 = ({"massive", "towering", "large", "hulking", "imposing",
                       "mammoth" }); 
string  *adj_list2 = ({"silver-tipped", "brown", "rugged", "dark-brown"});
string  adj1, adj2;
...
create_shapeshift_creature()
{
    ...
    set_race_name("bear");
    set_short(adj1 + " " + adj2 + " bear");
    set_title("the " + adj1 + " " + adj2 + " bear");
    set_adj(({ adj1, adj2 }));
    set_long("This is "+LANG_ADDART(adj1)+" muscular brown bear, the fur on its back tipped " +
       "silver. A pronounced hump is noticeable between its shoulder blades, although not as " +
       "noticeable as its long 4 inch claws. It stares at you with intelligent eyes that you " +
       "find a little unsettling.\n");

    if (!IS_CLONE)
        return;

    add_prop(CONT_I_WEIGHT, 270000); /* weight = 270 kilograms */
    add_prop(CONT_I_HEIGHT, 2000); /* height = 2 metres */
    add_prop(CONT_I_VOLUME, 220000); /* volume = 220 liters */

    add_prop(NPC_I_NO_LOOKS, 1);
    add_prop(LIVE_I_NEVERKNOWN, 1);
    add_prop(LIVE_M_MOUTH_BLOCKED, "While polymorphed in to this form you are unable " +
                  "to vocalise as you did in humanoid form!\n");
...
hook_pre_shapeshift_transform()
{
    ...
    tell_room(env, QCTNAME(player) + " throws " +HIS(player)+ " head back and " +
        "releases a primal roar as " +HIS(player)+ " body violently bulges," +
         " transforming into "+ LANG_ADDART(this_object()->query_nonmet_name()) + ".\n",
        ({ this_object(), player }));
    player->catch_tell("You throw your head back and release a primal roar, " +
       "your spell of transformation turning you into "
...
hook_pre_shapeshift_return()
{
    ...
    tell_room(env, QCTPNAME(this_object()) + " ursine form suddenly begins " +
        "to contract, and reform into " +QTNAME(player)+ ".\n", ({ this_object(), player }));
    this_object()->catch_tell("You feel your ursine form suddenly begin to " +
...
hook_prepare_shapeshift_target()
{
    ...
    // dex casters wis to a max of 200
    stat_dex = min(200, player_wis);

    // con +20% of casters int to a max of 300
    stat_con = min(300, (player_int * 12) / 10);
    ...
    set_attack_unarmed(A_BITE, poly_hit, poly_pen, W_IMPALE, 80, "sharp teeth");
    set_attack_unarmed(A_CLAW, poly_hit, poly_pen, W_SLASH, 20, "large claw");

    /* Hit_loc, *Ac against (impale/slash/bludgeon/magic), %hit, hit_desc */

    // base ac between 30 and 120 depending on transmutation skill and player int
    int poly_ac = (player->query_skill(SS_FORM_TRANSMUTATION) / 2) + min(70, (player_int / 3));
