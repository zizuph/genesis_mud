 ...
 * Apr 2013 Gorboth - Added notify_fail() message in mount command.
 * Nov 2019 Carnak  - rider_object could only handle 1 rider, fixed.
 */
#pragma save_binary
#pragma strict_types
...
/* The leader shadow file that is cloned when player leads. */
static string leader_shadow_file = LEADER_SHADOW;
...
/* Function name: set_leader_shadow_file
 * Description:   Set the leader shadow file.
 *                The leader shadow file that is cloned when player leads.
 * Arguments:     string leader_object_file
 */
public void set_leader_shadow_file(string s)
{
    leader_shadow_file = s;
}

/* Function name: query_leader_shadow_file
 * Description:   Query the leader shadow file.
 * Returns:       string leader_shadow_file
 */
public string query_leader_shadow_file()
{
    return leader_shadow_file;
}
...
add_rider(object rider)
{
    ...
    rider_object[rider] = clone_object(query_rider_object_file());
    if (!objectp(rider_object[rider]))
    {
        sh->remove_object();
        return STEED_STS_ILL_RIDER_OBJ;
    }
    rider_object[rider]->move(rider, 1);
    rider_object[rider]->set_steed(this_object());
    rider_object[rider]->set_rider(rider);
...
remove_rider(object rider)
{
    ...
    rider_object[rider]->remove_object();
    m_delkey(rider_object, rider);
    has_riders = sizeof(riders);
    ...
    else 
    {
        switch (sts) {
          ...
          case STEED_STS_ILL_RIDER_OBJ:
            ...
            break;
          case STEED_STS_CANNOT_BE_MOUNTED:
            living->catch_tell("The " + short() + " cannot be mounted.\n");
            break;
          default :
    ...
    else 
    {
        switch (sts) {
          case STEED_STS_ILL_LEADER_OBJ:
            ...
            break;
          case STEED_STS_CANNOT_BE_LEAD:
            living->catch_tell("The " + short() + " cannot be lead.\n");
            break;
          default :
...
do_lead(object new_leader)
{
    ...
    {
        shadow = clone_object(leader_shadow_file);
...
/* Function name: lead
 * Description:   start leading a steed (not necessarily this instance).
...
mount(string s)
{
    ...
    {
        if (sizeof(o_list) <= -index || !o_list[-index]->is_steed())
        {
            return 0;
        }
