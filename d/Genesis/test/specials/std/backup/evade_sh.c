...
// List of objects with evade percentages
object * evasion_effects = ({ });
...
query_evade_percentage()
{
    int percentage = 0;
    int x, y;
    foreach (object effect : evasion_effects)
    {
        if (!effect)
        {
        ...
        // Stacking Evade Formula (x, y)
        //    100 - (((100 - x) * (100 - y)) / 100)
        // This works out to be the same as:
        //    x + y - (x * y / 100)
        x = percentage;
        y = effect->query_evade_percentage();
...
{
    // Formula for calculating combat aid based on evade is:
    //   (e * 100) / (100 - e)
    int percentage, caid;
    
    percentage = query_evade_percentage();
