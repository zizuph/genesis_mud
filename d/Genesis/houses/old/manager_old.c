...
{
    string deName, dePath, dePrice;
    
    if (!strlen(str) || !parse_command(str, ({}), "%s ':' %s", deName, dePath, dePrice))
    {
        notify_fail("add <deed_name> : <deed_path> : <deed_price>?\n");    
        return 0;
    }
    
    switch(modify_deeds(deName, dePath, dePrice))
    {
        case -1:
