...
void autoenter_portal(object player)
{
    if (!player)
        return;
    if (environment(player) != this_object())
        return;
    player->catch_msg("You are suddenly drawn into the portal.  As " +
        "you enter it, you glimpse your old body melding with the " +
        "column of light, returning to the ether.\n");

    player->set_ghost(GP_BODY);
    player->remove_skill(RACIAL_ABILITY_SKILL);
    player->ghost_ready();
...
do_enter(string arg)
{
        ...
        "[the] [blue] [shimmering] 'portal'"))
    {
        if (this_player()->query_prop(TRIED_PORTAL_PROP) ||
            (this_player()->query_ghost() & GP_NOSKILL_LOSS))
        {
            write("As you step into the portal, you glimpse your old"
              + " body melding with the column of light, returning to"
                + " the ether.\n");
	    
            if (!(this_player()->query_ghost() & GP_NOSKILL_LOSS))
            {
                foreach (int skill : this_player()->query_all_skill_types())
                {
                    if (skill <= SS_MAX)
                    {
                        this_player()->set_skill(skill, 
                            this_player()->query_skill(skill) / 2);
                    }
                }
            }
            ...
            this_player()->remove_skill(RACIAL_ABILITY_SKILL);
        ...
        "[the] [column] [of] [white] 'light'"))
    {
        if ((this_player()->query_ghost() & GP_NOSKILL_LOSS))
        {
            write("You can change your body with no loss of skill.  You should " +
                 "<enter portal> instead.\n");
            return 1;
        }
...
public void enter_inv(object ob, object old)
{
    if (ob->query_ghost() & GP_NOSKILL_LOSS)
    {
        set_alarm(5.0, 0.0, &autoenter_portal(ob));
    }
}
