...
#define RACEMAP ([                                          \
		...
		"im_human"  : "/d/Immortal/domain/race/human_imm",   \
		"im_elf"    : "/d/Immortal/domain/race/elf_imm",     \
		"im_dwarf"  : "/d/Immortal/domain/race/dwarf_imm",   \
		"im_hobbit" : "/d/Immortal/domain/race/hobbit_imm",  \
		"im_gnome"  : "/d/Immortal/domain/race/gnome_imm",   \
		"im_goblin" : "/d/Immortal/domain/race/goblin_imm",   \
		])
/*
 * RACEATTR
 *
 ...
 *
 * Mapping is: race:  str, dex, con, int, wis, dis, race, occ, lay
 */
#define RACESTATMOD ([	 	  			       \
		"human"  : ({ 10, 10, 10, 10, 10, 10, 10, 10, 10 }), \
 ...
 * HEIGHTDESC
 */
#define HEIGHTDESC ({"extremely short", "very short", "short", "of normal length", "tall", "very tall", "extremely tall" })

/*
 * WIDTHDESC
 */
#define WIDTHDESC ({"very skinny", "skinny", "lean", "of normal width", "plump", "fat", "very fat" })
...
#define CONVERT_OLD "/d/Genesis/login/convert_old_players"
 ...
 * LOGIN_NAME_WARNING
 *
 * This text contains the warning that is displayed to people when they are
 * creating a new character. It is printed between the moment the player
 * entered a new name and the password query. Don't forget to add a trailing
 * space to the string.
 */
#define LOGIN_NAME_WARNING ("Genesis has a medieval fantasy theme setting. " + \
    "We promote that people use names fitting for this theme. The " + \
    "administration reserves the right to delete players whose names are " + \
    "too much out of character or that are generally offensive. ")
