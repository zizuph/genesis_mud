/*
 *  /d/Genesis/imbuement/imbuement_master.c
 *
 *  Database for imbuement data
 *  
 *  2010/09 Lavellan
 */
...
/* Prototypes */
public int              check_access(string prog);
public void             create();
...
public static int *     checkmissing(int armageddon);


/*
 * Function name: check_access
 * Description:   Check for correct caller
 */
public int
check_access(string prog)
{
if (prog == "d/Sparkle/area/orc_temple/npc/enchanter.c")
return 1;
    if (wildmatch(IMBUE_DIR + "*", "/" + prog))
        return 1;
    else
        return 0;
} /* check_access */
...
get_imbuements()
{
    if (!check_access(calling_program())) return 0;
    return imbuements;
} /* get_imbuements */
