...
//public void                 save();
//public void                 updated();
public string               get_file(string player);
...
store(object ply, object obj)
{
    ...
    IMBUE_LOGGER->log("stones", capitalize(pnam) + " stored " + obj->short() +
        ". Now has " + nums[0] + " " + nums[1] + " " + nums[2] + " " +
        nums[3] + " " + nums[4] + ".");

    IMBUE_LOGGER->log("lapidarium", capitalize(pnam) + " stored " + obj->short() +
        ". Now has " + nums[0] + " " + nums[1] + " " + nums[2] + " " +
        nums[3] + " " + nums[4] + ".");

    save_map(rec, get_file(pnam));
...
//    if (!rec)
    ...
    if (parse_command(arg, ply, "%w [from] [the] [my] 'lapidarium'", type))
    {
    }
    else
    if (parse_command(arg, ply, "%w %w [from] [the] [my] 'lapidarium'",
        qual, type))
    {
    }
    else
    if (!parse_command(arg, ply, "%w %w", qual, type))
    {
        type = arg;
        qual = 0;
    }
    ...
    for (i = (pl ? num : 1); i > 0; i--)
    {
        ...
        stone->move(ply, 1);
        IMBUE_LOGGER->log("stones", capitalize(pnam) + " extracted " +
            stone->short() + ". Now has " + nums[0] + " " + nums[1] + " " +
            nums[2] + " " + nums[3] + " " + nums[4] + ".");

        IMBUE_LOGGER->log("lapidarium", capitalize(pnam) + " extracted " +
            stone->short() + ". Now has " + nums[0] + " " + nums[1] + " " +
            nums[2] + " " + nums[3] + " " + nums[4] + ".");
...
check(string pnam)
{
    ...
    if (member_array(this_interactive()->query_real_name(),
        ALLOWED_WIZARDS) < 0)
        pnam = this_interactive()->query_real_name();

    string out = "";
    //mapping rec = data[pnam];
    mapping rec = restore_map(get_file(pnam));
 ...
 * Description  :        Get name of lapidarium data file for a player
 * Arguments    :        string player - name of player
 * Returns      :        path/filename of data file
 */
public string
get_file(string player)
{
    return LAPID_DATA_DIR + player[..0] + "/" + player;
} /* get_file */
