...
public int                  store(string arg);
public int                  extract(string arg);
public int                  getrandom(string arg);
...
check(string arg)
{
    ...
    object junk2;
    string player;
    int wiz = (SECURITY->query_wiz_rank(tp->query_real_name()) > WIZ_NORMAL);
    ...
    if (!parse_command(arg, this_object(), "[the] [my] %s %i / 'contents' %s",
        junk, junk2, player))
        return 0;
 ...
 * Description  :        adds a random stone to lapidarium. Just for testing ;)
 */
public int
getrandom(string arg)
{
    setuid();
    seteuid(getuid());
    object ob = clone_object(IMBUE_DIR + "obj/imbuement_stone");
    ob->randomise();
    ob->move(this_player());
    LAPID_MASTER->store(this_player(),ob);
    ob->remove_object();
    return 1;
} /* getrandom */