...
{   
    ...
    IMBUE_LOGGER->log(Shadow_ID, Shadow_Num, file_name(this_object())[29..] +
        " created on " + (shadow_who ? file_name(shadow_who) : "???"));
...
dispel_magic(int magic, object agent = 0)
{
    ...
    IMBUE_LOGGER->log(Shadow_ID, Shadow_Num, "Dispelled by " +
...
remove_object()
{
    IMBUE_LOGGER->log(Shadow_ID, Shadow_Num, "Removed by " +
        file_name(calling_object()) + " " +
        this_interactive()->query_real_name());
...
query_recover()
{
    if (is_toplevel_imbue_shadow())
    {
        ...
        {
            IMBUE_LOGGER->log(Shadow_ID, Shadow_Num, "Saved for recovery by " +
                file_name(calling_object()) + " " +
                this_interactive()->query_real_name());
...
enter_env(object to, object from)
{
    if (is_toplevel_imbue_shadow())
    {
        IMBUE_LOGGER->log(Shadow_ID, Shadow_Num, "Moved from " +
