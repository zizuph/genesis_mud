...
remove(object obj)
{
    ...
    int cur = wearer->query_skill_extra(Skill);

    if (res >= 0)
    {
        set_alarm(0.0, 0.0, &wearer->catch_tell("As you remove the " +
            shadow_who->short() + ", " + Unwieldmsg + "\n"));
        wearer->set_skill_extra(Skill, cur - BOOST_AMOUNT[Power_Level-1]);
        //  Special handling for sneak, give hide skill too
        if (Skill == SS_SNEAK)
        {
            cur = wearer->query_skill_extra(SS_HIDE);
            wearer->set_skill_extra(SS_HIDE, cur - 
                BOOST_AMOUNT[Power_Level-1]);
        }
    }
    return res;
...
do_wear()
{
    ...
    {
        cur = wearer->query_skill_extra(SS_HIDE);
        wearer->set_skill_extra(SS_HIDE, cur + BOOST_AMOUNT[Power_Level-1]);
    }
