...
public void              init_resistance(int lvl, string res, string restext,
                string desc,
                string wieldmsg = "a strange tingle goes through your body.",
                string unwieldmsg = you suffer an involuntary shudder.");
...
unwield(object obj)
{
    ...
    if (res >= 0)
    {
        set_alarm(0.0, 0.0, &wearer->catch_tell("As you release the " +
...
remove(object obj)
{
    function wf = shadow_who->query_wf;
    int res = shadow_who->wf();
    object wearer = shadow_who->query_wielded();

    if (res >= 0)
    {
        set_alarm(0.0, 0.0, &wearer->catch_tell
        wearer->catch_tell("As you remove the " + shadow_who->short() +
            ", " + Unwieldmsg + "\n"));
        wielder->remove_magic_effect(this_object());
