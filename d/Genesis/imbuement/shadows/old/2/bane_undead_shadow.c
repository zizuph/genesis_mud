/*
 *  /d/Genesis/imbuement/shadows/bane_undead_shadow.c
 *
 ...
 *
 *  This particular effect gives a special attack when used against undeads.
 *
...
stat_object()
{
    ...
    str += "This weapon has been given a random magic mod that does"
         ...
         + " amount) roughly 10% of the time to undeads.\n";
    return str;
} /* stat_object */
...
{   
    ...
    set_races( ({ "undead", "zombie", "wraith", "wight", "skeleton",
                  "vampire", "lich", "ghost", "goul", "ghoul" }) );
    // Also affects anything with LIVE_I_UNDEAD
    set_hit_descs( ({ "tingles the flesh of", "singes the flesh of",
                      "broils the flesh of", "immolates the flesh of",
                      "destroys the flesh of" }) );
    set_damage_msg("A flash of power erupts from the bone formations on");
    set_add_item_array(   ({ "glow",
                             ...
                             "unusual bone formations",
                             "unusual formations" }) );
    set_add_item_desc(
        "The " + shadow_who->short() + " is covered with unusual bone"
...
long(string str, object for_obj)
{
    ...
    return shadow_who->long(str, for_obj)
      + "Bone formations along its surface emit "
...
dispel_msg_hook()
{
    return "The bone formations along the surface of the "
