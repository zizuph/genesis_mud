...
query_recover()
{
    ...
    if (member_array(func, ({ "compute_recover_str", "add_rack_entry",
        "create_auction" })) >= 0)
    {
    ...
    if (!(
        ((cobj == shadow_who) && 
        (member_array(func, ({ "check_recoverable", "stat_object"})) >= 0)) ||
        ((cobj == environment(shadow_who) || 
            cobj == environment(environment(shadow_who))) && 
        (member_array(func, ({ "compute_values" })) >= 0))))
        IMBUE_LOGGER->log("recover", "query_recovery in " + Shadow_ID +
            " called by " + file_name(calling_object()) + ":" +
            func + " "  +
            this_interactive()->query_real_name());
