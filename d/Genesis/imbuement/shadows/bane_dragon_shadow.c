/*
 *  /d/Genesis/imbuement/shadows/bane_dragon_shadow.c
 *
 ...
 *
 *  This particular effect gives a special attack when used against dragons.
 *
...
stat_object()
{
    ...
    str += "This item has been given a random magic mod that does"
         ...
         + " amount) roughly 10% of the time to dragons.\n";
    return str;
} /* stat_object */
...
query_races()
{
    return ({ "dragon", "wurm", "wyvern", "basilisk", "draconian",
        "drake", "hydra", "lizard", "lizardman", "saurian" });
} /* query_races */
...
{   
    ...
    //set_races( ({ "dragon", "wurm", "wyvren", "basilisk" }) );
    set_hit_descs( ({ "tingles the scales of", "singes the scales of",
                      "broils the scales of", "immolates the scales of",
                      "destroys the scales of" }) );
    set_damage_msg("A flash of power erupts from the diamond formations on");
    set_add_item_array(   ({ "glow",
                             ...
                             "formations",
                             "unusual formation",
                             "unusual diamond formation",
                             "unusual diamond formations",
                             "unusual formations" }) );
    set_add_item_desc(
        "The " + shadow_who->short() + " is covered with unusual diamond"
...
dispel_msg_hook()
{
    return "The diamond formations along the surface of the "
