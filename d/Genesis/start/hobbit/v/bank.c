...
#define NOT_WANTED ({ "gnome", "goblin" })
...
create_room() {
   ...
   set_long("This is the bank of Hobbiton. The hobbits themselves never liked "
    + "the idea of a bank, since they love counting their own money. "
    + "However, since more and more people stop by this town, the need "
    + "for a trustworthy bank has grown. Behind the iron-barred counter "
    + "is a hobbit with greedy little eyes. On the counter is a sign.\n");

   add_exit(STAND_DIR + "h_lane3", "east", 0);

   add_item(({"hobbit","clerk"}),
      "The hobbit tries to estimate how much money you carry with you.\n");
   ...
   add_item(({"counter","iron-barred counter"}), break_string(
    ...
    + "enclosed by thick iron bars. You wonder how the hobbit got in.\n",70));
  ...
  /* Set up the trading system */
   ...
   set_money_give_out(   ({100000,100000,100000,2 })); /* Max_pay   */
...
init() {
   ::init();
   if (member_array(this_player()->query_race_name(), NOT_WANTED) >= 0)
      set_alarm(3.0, 0.0, &throw_out(this_player()));
...
do_minimize(str) {
   ...
   int value;
    ...
    notify_fail("Minimize coins perhaps?\n");
...
throw_out(tp) {
   ...
   if (present(tp))
   {
      ...
      tp->catch_msg(break_string(
        "The clerk gasps in astonishment and looks wide eyed at you. "
       + "He points at you and mumbles something. "
       + "Then he shouts: Guards! Help! The bank is being robbed! "
       + "Immediately two big hobbits come running towards you and grab "
    ...
    /* You cannot use break_string here... */
      ...
      tell_room(this_object(),
         "The clerk gasps in astonishment and looks wide eyed at "
       + QTNAME(tp) + ".\n"
       + "He points at " + QTNAME(tp) + " and mumbles something.\n"
       + "Then he shouts: Guards! Help! The bank is being robbed!\n"
       + "Immediately two big hobbits come running towards "
       + QTNAME(tp) + "\n"
      ...
      trashcan = present("gnome_trash",find_object(STAND_DIR + "h_lane3"));
      if (trashcan)
      {
          tp->move(trashcan);
          tell_room(find_object(STAND_DIR + "h_lane3"),
