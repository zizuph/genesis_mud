...
void give_back_immediately(mixed return_arr);
void name_price(object what);
void thank_chat(object *arr);
...
create_monster()
{
   ...
   /* Triggers, to make him as annoying as Harry. */
...
enter_inv(object what, object from)
{
   ...
   {
      set_alarm(1.0, 0.0, &give_back_immediately(({what, from})));
      return;
   }
   ...
   {
      set_alarm(itof(3 + random(4)), 0.0, &name_price(what));
   }
...
name_price(object what)
{
   ...
   if (who = present(from_arr[i], environment()))
   {
      ...
      {
         if ((MASTER_OB(what) != "/std/leftover")
           || ((j = member_array(what->short(), tr_name_arr)) < 0))
         give_back(what);
         return;
      }
      ran = random(4);
...
react_spit(string who, string str) {
    ...
    */
      str = "";
      set_alarm(3.0, 0.0, &return_spit(({who, str})));
      return 1;
   }
}
