...
create_room()
{
   ...
   set_short("Bakery");
   set_long("You have entered the local bakery. The smell of fresh bread "
   ...
   add_item(({"counter","glass counter"}), 
...
can_afford(price, food) {
 /* Try to take the money */
   ...
   write("You order " + LANG_ADDART(food) + " and pay " + coin_pay_text + ".\n");
   if (coin_get_text)
