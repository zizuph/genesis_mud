...
create_wild_room()
{
    ...
    add_exit(THIS_DIR + "orc_road1", "southwest", "@@hill@@", 1,1);
    add_exit(THIS_DIR + "s_road1a", "southeast", 0,1,1);
    add_exit(THIS_DIR + "orc_road1", "north", "@@spur@@", 1,1);
    add_exit(THIS_DIR + "orc_road1", "northwest", "@@spur@@", 1,1);
 ...
 * Description  :        Gives an explanation for why the mortal cannot travel
                         across the hills in this direction
 * Returns      :        int 1 - denies moving along the exit path
 */
int
hill()
{
    write("The hill looks a little bit too difficult to climb in that " +
