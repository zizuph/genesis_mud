 ...
 * Prototypes
 */
nomask int sk_improve(string str);
...
sk_fix_cost(int snum, int steps)
{
    ...
    if (next_level > (max = sk_query_max(snum)))
        ...
        cost = sk_cost(snum, this_level, next_level) + " iron";
    ...
    if( !sk_hook_allow_train_skill( this_player(), sk_trains[snum][0], next_level ) )
       return "";
    else
...
{
    notify_fail("You already know that skill, try improve it.\n");
    return 0;
}
 ...
 *                *arg   - The exploded string from sk_improve
 *                 verb  - improve or learn typically
 * Returns:       1 - yes (default) / 0 - no
 */
public varargs int
sk_hook_allow_train(object who, mixed arg, string verb)
{
    return 1;
}
...
{
    notify_fail( "You are not allowed to train "+ ( skill ?
...
#ifdef SURTAX
    ( "/secure/banking" )->store_bank_fees( (cost * SURTAX) / 100 );
#endif SURTAX
