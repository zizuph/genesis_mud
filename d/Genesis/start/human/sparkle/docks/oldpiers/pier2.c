...
create_room()
{
    set_short("Berth 2, west pier");

    set_long("Outside the pub.\n"+
      "Many sailors have drifted into oblivion here on the pier after\n"
      + "a hard time at the pub. And not few of them have awakened without\n"
      + "a copper left so it is not a recommended place to rest...\n"
      + "");

    add_prop(ROOM_I_INSIDE, 0);
    add_prop(ROOM_I_TYPE, ROOM_NORMAL);
    add_my_desc("There is a sign here marked 'Berth 2'\n");

    add_exit(THIS_DIR + "ship_info", "north", 0);
    add_exit(THIS_DIR + "pub",   "west",      0);
