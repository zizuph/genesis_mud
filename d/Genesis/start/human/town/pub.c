...
/*	
 *	Gayryn's pub - Sparkle	
 *	--Styles. 910618
 *	
 *	
 *	Minor fixes 080318 -Styles
 *
 *      Added new rules for the pub - no brawling and no thieving!
 *      - Arman, 04 November 2017	
 *	
 */
...
public void    create_sailors(int n);
public void    clone_sailor(int i, mixed sarr);
public object  check_monster(string who = "bartender");
public void    call_keeper();

/* Name,		Pretitle,	Title,		Nickname,	adj1,	adj2,	Starting loc,	Path to walk,	Delay  */
mixed   sail_arr = ({
  ({"tatilar",	"",			"the sailor",  "tatty",  "tall","skinny", "church", "s e e e n n w", 3}),
  ({"velomyn",	"",			"the sailor",  "furrface",  "black-bearded", "", "parrot", "w w w w n n n w", 4}),
  ({"galgaj",	"",			"the bold",    "",  "big","muscular", "jetty2", "n w w w w w w w", 9}),
  ({"moguin",	"",			"the experienced adventurer",  "mo", "grim","scarred", "church", "s e e e n n w", 6}),
  ({"kantram",	"",			"the utter novice",  "",  "fat","hairy", "jetty2", "n w w w w w w w", 2}),
  ({"gryej",	"",         "the pirate",  "nimblefoot", "one-legged","", "parrot", "w w w w n n n w", 5}),
  ({"lumpar",	"",			"of Liwitch",  "beacon",  "tall","sturdy", "vill_gate", "e s s e e e e n n w", 7}),
  ({"gidorin",	"Captain",  "of the Flunder",  "captain",  "red-bearded", "", "church", "s e e e n n w", 8}),
  ({"stutch",	"",			"Ironfist",  "pigpen",  "sloppy","unshaven", "shop", "n e n n w", 6}),
  ({"vondoj",	"",			"the pirate",  "hawkeye",  "one-eyed", "", "vill_gate", "e s s e e e e n n w", 7}),
  ({"aarion",	"",			"of Antraal",  "",  "tall","proud", "church", "s e e e n n w", 6}),
  ({"fistus",	"",			"the seasoned adventurer",  "Shorty", "short","encumbered", "church", "s e e e n n w", 5}),
  ({"carius",	"",			"the merchant",  "carie",  "well-dressed", "", "parrot", "w w w n n n w", 4}),
  ({"monbier",	"Captain",  "of HMS Exelcior",  "captain",  "decorated", "", "jetty2", "n w w w w w w w", 8}),
  ({"burun",	"",			"the sailor",  "crow",  "old","bearded", "jetty2", "n w w w w w w w", 5}),
  ({"munto",	"",			"the storyteller",  "humpey",  "old", "hunchbacked", "church", "s e e e n n w", 3}),
  ({"sillaric",	"",			"of Magonien",  "lilly",  "perfumed", "", "vill_gate", "e s s e e e e n n w", 5}),
  ({"dalid",	"",			"the fierce",  "boner",  "short", "weatherbeaten", "vill_gate", "e s s e e e e n n w", 7}),
  ({"schmerk",	"",			"the gourmet chef",  "parsley",  "dirty", "", "church", "s e e e n n w", 4}),
  ({"pintim",	"",			"the experienced novice",  "woggy", "nervous", "", "jetty2", "n w w w w w w w", 2})
});

static object *sailors = ({}), keeper;
static int     tmp_rang;    
int            keeper_called;
...
create_room()
{
    ...
    add_exit(THIS_DIR + "pier2", "east", 0);
    ...
    add_drink( ({"special brew", "special", "brew" }),
                ({"brew", "special", "_sparkle_tour_quest_drink", }),
		({ "special" }), 200, 24, 72,
		"special brew", "special brews",
    ...
    add_drink( ({"rum", "captain's rum", }),
		({"rum", }),
		({"captain's", }), 100, 36, 144,
		"Captain's rum", "Captain's rums",
    ...
    add_drink( ({"firebreather", "fire", }),
		({"firebreather", "fire", "breather", "rotgut", }),
		0, 100, 50, 288,
...
/*    if (!objectp(keeper))
    {
	keeper = clone_object(OBJ_DIR + "bartender");
	keeper->arm_me();
    }*/
    keeper_called = 0;
}
...
/*    if (!objectp(keeper))
    ...
    if (!present(keeper) && !tmp_rang && !keeper_called)
    {
//	set_alarm(5.0, 0.0, &keeper->move_living("east", TO));
      	set_alarm(5.0, 0.0, call_keeper);
      	tmp_rang = 1;
        keeper_called = 1;
...
clone_sailor(int i, mixed sarr)
{
    ...
    sailors[i] = clone_object(OBJ_DIR + "sailor");
    ...
    sailors[i]->move_living("down", TOWN_DIR + sarr[6]);
...
kick_for_brawling()
{
    ...
    tell_room(TO, QCTNAME(tp) + " decides to start a pub brawl!\n", 
        ({ tp, keeper }) );
    ...
    keeper->command("scowl "+tp->query_real_name());
    keeper->command("shout Hey, no brawling in my pub!");
    keeper->command("emote points at the sign.");
    keeper->command("shout Get out of here, you filthy dog!");

    tp->catch_msg(QCTNAME(keeper) + " throws you out of the pub.\n");
    tell_room(TO, QCTNAME(keeper) + " throws " + QTNAME(tp)
      + " out of the pub.\n", ({ tp, keeper }) );
    tp->move_living("east", THIS_DIR + "pier2");
...
kick_for_stealing()
{
    ...
    keeper->command("scowl "+tp->query_real_name());
    keeper->command("shout Hey, keep your hands off my patrons!");
    keeper->command("emote points at the sign.");
    keeper->command("shout Get out of here, you thieving dog!");
