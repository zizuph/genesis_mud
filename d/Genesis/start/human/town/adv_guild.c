 ...
 *          : 02-Februar-2006 by Lunatari
 *              - Updated the poster to clone the Sparkle one.
 *          : 09-June-2009 by Gorboth
 *             - Added ROOM_S_MAP_FILE functionality
 *             - Changed exit to integrate into New Sparkle
 *          : 13-March-2010 by Gorboth
 *             - Added block skill
 *             - Created filters to prevent block training by certain
 *               Guilds.
 */
...
// Global Variables
public string *     BLOCK_BANNED_GUILDS = 
    ({ 
        "Army of Darkness",
     });
...
void                set_up_skills();
void                reset_room();
public string *     player_guilds(object player);
public int          cannot_train_block(object player);
...
create_room()
{
    ...
    set_long("Welcome to the Sparkle Adventurer Guild.\n\n" +
      ...
      " where many adventurers often sit down and relax. A creaky" +
      " stairway leads up through a ceiling in one corner of the" +
      " room.\n\n" +
    ...
    add_item( ({ "ceiling", "up" }),
        "The ceiling is sturdy and attractive. You notice a stairway"
      + " leading up through it.\n");
    ...
    add_exit("/d/Sparkle/area/city/rooms/streets/center_e", "north");
    add_exit("/d/Sparkle/area/city/rooms/inside/exp_drain_room", "up",
        block_exit, 1, 1);
...
set_up_skills()
{
    ...
    me = "prevent people from leaving"; ot = me;
    sk_add_train(SS_BLOCK, ({ me, ot }), 0, 0, 30 );
}
...
climb_stairs(string arg)
{
    ...
    if (!parse_command(arg, ({}),
        "[up] [the] [creaky] 'stair' / 'stairs' / 'stairway'"
      + " / 'staircase' [case]"))
    {
...
/*
 * Function name:        player_guilds
 * Description  :        Populate and return an array of the guild
 *                       names that a given player is a member of.
 * Arguments    :        object player - the player
 * Returns      :        string array - 0: occ guild name
 *                                      1: lay guild name
 *                                      2: race guild name
 *                                      3: craft guild name
 */
public string *
player_guilds(object player)
{
    return ({ player->query_guild_name_occ(),
              player->query_guild_name_lay(),
              player->query_guild_name_race(),
              player->query_guild_name_craft() }) - ({ 0 });
} /* player_guilds */


/*
 * Function name:        cannot_train_block
 * Description  :        Is this player allowed to train the block
 *                       skill?
 * Arguments    :        object player - the player
 * Returns      :        1 - the player is not allowed to train
 *                       0 - the player is allowed to train
 */
public int
cannot_train_block(object player)
{
    string *guilds = player_guilds(player);

    foreach (string guildname : guilds)
    {
        if (IN_ARRAY(guildname, BLOCK_BANNED_GUILDS))
        {
            return 1;
        }
    }
...
{ 
    if (skill != "block")
    {
        return ::sk_hook_allow_train_skill(who, skill, level);
    }

    if (cannot_train_block(who))
    {
        return 0;
    }

    return ::sk_hook_allow_train_skill(who, skill, level);
} /* sk_hook_allow_train_skill */
...
sk_hook_not_allow_train_skill(string skill)
{
    if (skill == "block")
    {
        ...
        {
            write("You start to think about blocking opponents, but"
              + " then a not so quiet voice in your head interrupts,"
              + " saying: BLOCK?! SHUT UP! YOU NOT BLOCK, YOU SMASH!"
              + " BLOCK FOR TRICKY FIGHTERS! YOU SHUT UP AND SMASH"
              + " SMASH SMASH!!! MAYBE OGRESTOMP IF REALLY ANGRY!\n");
    ...
    return 1;
} /* sk_hook_not_allow_train_skill */


/*
 * Function name:        block_exit
 * Description  :        provide an explanation why the exit is
 *                       blocked
 * Returns      :        int 1 - blocked exit
 */
public int
block_exit()
{
/*
    write("As you try to climb the stairs, a strange haze clouds your"
      + " vision. Into your mind, the voice of Gorboth speaks:\n\n"
      + "Sorry, folks, I'm closing down the ability to remove your"
      + " exp right now. The issue is, I'm considering implementing"
      + " a system where the death penalty returns in a much smaller"
      + " form (5%) but that we remove death recovery entirely. If we"
      + " do that, I don't want to give people the chance to remove"
      + " their exp with the understanding that they will have recovery,"
      + " only to remove recovery globally later. A decision will be"
      + " made soon. In the meanwhile, you're stuck with your stats as"
      + " they are.\n\n"
      + "The voice fades, and you peer up at the staircase with"
      + " new understanding about why you cannot proceed there.\n");
*/

    write("The entrance to the attic appears to have been nailed shut.\n");
