...
#define FLOWER_TYPES  ({ "rose" , "dandelion" , "daisy" , "lilly"  })
#define FLOWER_PTYPES ({ "roses", "dandelions", "daisys", "lillys" })
#define FLOWER_VALUES ({ 5      ,  1          ,  1      ,  2       })
#define FLOWER_BOUKET ({ 1      ,  100        ,  50     ,  10      })
#define FLOWER_WEIGHT ({ 2      ,  1          ,  1      ,  1       })
#define FLOWER_VOLUME ({ 2      ,  1          ,  1      ,  1       })
...
create_flowers()
{
    set_name("rose"); //FLOWER_TYPES[0]);
    set_pname("roses"); //FLOWER_PTYPES[0]);
    add_name("flower");
    add_pname("flowers");
	add_pname("bouqet");
    set_heap_size(1);
    set_flower_type("rose"); //FLOWER_TYPES[0]);
}
