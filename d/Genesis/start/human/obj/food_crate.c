...
// last update: 
// purpose:     Crate with FOOD items; for the Underdark.
//              It is placed in rooms using the utt_treas.c file
// bug(s):
// to-do:
...
{
    // fill the box with some worn items
    if (sizeof(all_inventory(this_object())) < 2)
    {
	clone_object(BAZAAR + "Obj/misc/ptorch")->move(this_object(),1);
	clone_object(DARK +"obj/cheese")->move(this_object(),1);
	clone_object(DARK +"obj/jerky")->move(this_object(),1);
	clone_object(DARK +"obj/dfruit")->move(this_object(),1);  
	clone_object(DARK +"obj/dnuts")->move(this_object(),1);  
