...
arm_me()
{
    object wobj;
    string  wep,
           *wep_arr = ({"bbottle", "dagger", "knife", "knife", "axe",
                        "ssword", "lsword", "rapier", "knife", "knife",
                        "knife" });
...
//    clone_object(OBJ_DIR + wep)->move(TO);
    wobj = clone_object(OBJ_DIR + wep);
    set_skill(wobj->query_wt(),6);
    wobj->move(TO); 
    command("wield all");
}
