/*
 *  /d/Genesis/start/human/obj/sparkle_map.c
 *
 *  This is a map of Sparkle that players can take and keep with them.
 *
 *  Obtained at: /d/Genesis/start/human/town/road4.c
 *
...
{
    set_name("map");
    add_name( ({ "_sparkle_map" }) );

    add_adj( ({ "sparkle", "old", "crumpled" }) );
    set_short("old crumpled map");
    set_long(describe);

    add_cmd_item( ({ "map", "sparkle map", "map of sparkle",
                     "crumpled map", "old crumpled map" }),
                  ({ "read" }), describe);
...
describe()
{
    return read_file(OBJ_DIR + "map.txt", 0);
} /* describe */
