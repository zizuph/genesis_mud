...
create_room()
{
    set_short("West side of Camp");
    set_long(OUTSIDE_DESC+OUT_TENT_DESC+CENTER_DESC_W);
    add_item(({"tent", "tents"}), "Tents surround the center of the camp.\n");
    add_exit(MERC_CAMP_ROOM + "c_w.c", "east", 0, 1, 0);
    add_exit(MERC_CAMP_ROOM + "c_nw_w.c", "northeast", 0, 1, 0);
    add_exit(MERC_CAMP_ROOM + "c_sw_w.c", "southeast", 0, 1, 0);
    reset_room();
}
