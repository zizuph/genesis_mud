...
create_room()
{
    set_short("East of Camp Center");
    set_long(OUTSIDE_DESC+OUT_TENT_DESC+CENTER_DESC_E);
    add_item(({"tent", "tents"}), "Tents surround the center of the camp.\n");
    add_exit(MERC_CAMP_ROOM + "c_e_e.c", "east", 0, 1, 0);
    add_exit(MERC_CAMP_ROOM + "c_ne_e.c", "north", 0, 1, 0);
    add_exit(MERC_CAMP_ROOM + "c_se_e.c", "south", 0, 1, 0);
    reset_room();
}
