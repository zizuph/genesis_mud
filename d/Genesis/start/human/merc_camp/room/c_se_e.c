...
create_room()
{
    set_short("Southeast of Camp Center");
    set_long(OUTSIDE_DESC+OUT_TENT_DESC+CENTER_DESC_SE);
    add_item(({"tent", "tents"}), "Tents surround the center of the camp.\n");
    add_exit(MERC_CAMP_ROOM + "c_se.c", "west", 0, 1, 0);
    add_exit(MERC_CAMP_ROOM + "c_se_s.c", "southwest", 0, 1, 0);
    add_exit(MERC_CAMP_ROOM + "c_e.c", "north", 0, 1, 0);
    add_exit(MERC_CAMP_ROOM + "c_e_e.c", "northeast", 0, 1, 0);
    reset_room();
}
