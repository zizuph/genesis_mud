...
create_room()
{
    set_short("South of Camp Center");
    set_long(OUTSIDE_DESC+"You spot the entrance to the camp off to the south. "+
	OUT_TENT_DESC+CENTER_DESC_S);
    add_item(({"tent", "tents"}), "Tents surround the center of the camp.\n");
    add_exit(MERC_CAMP_ROOM + "c_s_s.c", "south", 0, 1, 0);
    add_exit(MERC_CAMP_ROOM + "c_sw_s.c", "west", 0, 1, 0);
    add_exit(MERC_CAMP_ROOM + "c_se_s.c", "east", 0, 1, 0);
    reset_room();
}
