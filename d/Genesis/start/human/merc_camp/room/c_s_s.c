...
create_room()
{
    set_short("Entrance of Camp");
    set_long("You are at the entrance to this small camp. A path leads out to "+
	"south. "+OUTSIDE_DESC+OUT_TENT_DESC+CENTER_DESC_S);
    add_item(({"tent", "tents"}), "Tents surround the center of the camp.\n");
    add_exit(MERC_CAMP_ROOM + "c_s.c", "north", 0, 1, 0);
    add_exit(MERC_CAMP_ROOM + "c_se_s.c", "northeast", 0, 1, 0);
    add_exit(MERC_CAMP_ROOM + "c_sw_s.c", "northwest", 0, 1, 0);
    reset_room();
}
