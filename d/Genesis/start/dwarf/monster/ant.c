/*
 * Changes
 * 2005-10-13 - Cotillion
 * - Fixed so they don't just start lots of alarms. 
 * 10/08/21 Lavellan Now just "move"s objects into hill instead of entering
 *              and dropping.  
 */
#pragma strict_types
#pragma save_binary
...
enter_inv(object from, object to)
{
    ...
    {
        move_alarm = set_alarm(1.0, 0.0, move_home);
    }
}
...
move_home()
{
    ...
    if (whereto)
    {
        ...
        if (!get_alarm(move_alarm))
            move_alarm = set_alarm(10.0, 0.0, move_home);
    }
    ...
    {
        tell_room(E(TO),
            "The tiny ant enters the ant hill, but returns almost " +
            "immediately.\n");
        
        all_inventory(TO)->move(whereto, 1);
        //move_living("M",whereto);
        //command("drop all");
