/*
 * File         : /d/Genesis/new/start/goblin/town/cavern_b5.c
...
create_room()
{
    ...
    "The walls here are dimly lit by a soft, green glow. Water " +
    "flows past you through a channel which has been cut into " +
    "the stone floor of the cavern here. The channel leads from " +
    "the Shop, which lies to the north, down to the Museum, " +
    "which is southwest of here. A small footbridge traverses " +
    "the channel, approaching the Central Plaza to the northwest.\n");
    ...
    "bridge offers you access to the town's Central Plaza.\n");
    ...
    add_item(({"shop", "kalerus shop"}),
    "The Shop is a small stone building which looks onto the " +
    "town's Central Plaza.\n");

    add_item(({"museum", "kalerus museum"}),
    "The Kalerus Museum looks onto the town's Central Plaza. It " +
    "seems to give rise to a sizeable river which mysteriously appears " +
    "from beneath its foundation.\n");

    add_exit("cavern_c3", "northwest");
    add_exit("cavern_a5", "south");
}
...
cross_bridge()
{
   ...
   tell_room("/d/Genesis/new/start/goblin/town/cavern_c3",
   QCNAME(this_player()) + " arrives crossing over the footbridge.\n",
   this_player());

   this_player()->move_living("M", "/d/Genesis/new/start/goblin/town/cavern_c3", 0, 0);
