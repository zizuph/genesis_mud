/*
 * File         : /d/Genesis/start/hobbit/npc/officer.c
 * Creator      : Seaweed
 * Copyright    : Copyright (C) 1998 Joakim Larsson
 * Date         : 2/24/99
 * Purpose      : 
 * Related Files: 
 * Comments     :
 *         * - Add chat * - Add triggers
 * 
 * TODO         :
 
 *
 * Modifications:
 *     N.B. This file is generated from templates hosted elsewhere and
 *     if someone corrects something in this file, please mail Seaweed
 * 
 */

/* directives */
#pragma strict_types

/* includes */
#include <stdproperties.h> 
#include <macros.h> 
#include <ss_types.h> 
#include <filter_funs.h>

#include "../hobbit.h"

/* inherits */
inherit "/std/monster";
inherit "/std/act/action";
inherit "/d/Genesis/lib/intro";
/* defines */
// NPC states (bit values)
#define SLEEPING   1

/* prototypes */
void create_monster();
void officer_actions();
void equip_me();

/* global variables */
int     Status;
object  Visitor;

/*
 * Function name: create_monster
 * Description  : set up the humanoid
 */
void
create_monster()
{
    if (!IS_CLONE)
        return;
    set_name("bon");
    set_race_name("hobbit");
    
    set_gender(G_MALE);
    set_long(
      "The shipping officer keeps track of all the ships and what their " +
      "destinations are.\n");
    add_prop(CONT_I_HEIGHT, 110);
    add_prop(CONT_I_WEIGHT, 37000);
    add_prop(NPC_I_NO_LOOKS, 1);
    add_prop(OBJ_S_WIZINFO, "Made by Seaweed 2/24/99.\n");
    
    set_stats(({40, 70, 70, 50, 70, 90}));
    set_hp(query_max_hp());
    add_name("officer");
    add_adj(({"young", "alert", "happy" }));
    set_act_time(5);
    add_act("@@officer_actions@@");
    set_default_answer(officer_default_answer);
    add_ask(({"seagull", "seagulls", "bird", "birds", "guano"}),
        "say The birds are a part of the special harbour atmosphere around here.", 1);
    equip_me();
    
    
}
/*
 * Function name: introduce
 * Description  : A player that introduces himself to the nPC for the 
 *                first time will get a respons defined by this function
 * Arguments    : object who - The introduced player object
 */
void
introduce(object who)
{
    command("emote smiles enthusiastically.");
    command("introduce me to " + OB_NAME(who));
    command("say How can I help you?");
}

/*
 * Function name: greet
 * Description  : A player that has already introduced himself to the NPC 
 *                will get a respons defined by this function.
 * Arguments    : object who - The introduced player object
 */
void
greet(object who)
{
    command("nod cheer at " + OB_NAME(who));
    command("say How can I help you?");
}

/*
 * Function name: set_visitor(who)
 * Description  : access method for the Visitor variable
 * Arguments    : object who - the player object visiting us.
 */
public void
set_visitor(object who)
{
    Visitor = who;
}

/*
 * Function name: officer_actions()
 * Description  :  Handles NPC actions
 */
string
officer_actions()
{
    int i;
    if (Visitor && present(Visitor, environment(this_object())))
    {
        i = random(3);
        switch(i)
        {
            case 0..1: // Smile enthus
            {
                command("smile enthus at " + OB_NAME(Visitor));
                break;
            }
            case 2: // nothing
            {
                break;
            }
            default:
            {
                command("sob deep");
                break;
            }
        }
    }
    Visitor = 0;
    i = random(5);
    switch(i)
    {
        case 0:
        {
            command("emote reads some papers.");
            break;
        }
        case 1:
        {
            command("laugh .");
            break;
        }
        case 2:
        {
            command("say May I help you with something?");
            break;
        }
        case 3:
        {
            command("say There are some really exciting trips to make from here.");
            break;
        }
        case 4:
        {
            command("smile enthus");
            break;
        }
        default:
        {
            command("emote does something totally unexpected.");
        }
    }
    return "";
}

/*
 * Function name: officer_default_answer()
 * Description  : Handles the default answer for unknown questions.
 */
string
officer_default_answer()
{
    command("say I am sure something can be arranged to let you find out.");
    return "";
}
/*
 * Function name: equip_me()
 * Description  : create cloths and equipments and wear it.
 */
void
equip_me()
{
    object clone;
    
    seteuid(getuid(TO));
    
    // Make some basic clothes
    clone = clone_object(EX_OBJ + "linnen_pants");
    clone->move(TO);
    clone = clone_object(EX_OBJ + "linnen_shirt");
    clone->move(TO);
    
    // Make some armour featured clothes
    clone = clone_object(EX_ARM + "leather_vest");
    clone->move(TO);
    clone = clone_object(EX_ARM + "leather_shoes");
    clone->move(TO);
    
    // Put it on!
    command("wear all");
}

