/*
 * File         : /d/Genesis/start/hobbit/town/woodb1.c
 * Creator      : Seaweed
...
create_room()
{
    ...
    set_long(
      ...
      "seen eastwards and to the west a small creek flows by.\n");
    
    add_prop(ROOM_S_EXIT_FROM_DESC, local_m_in_message);
    add_prop(OBJ_S_WIZINFO, "Made by Seaweed 2/27/99.\n");
    
    add_exit("woodpath4.c", "east");
    add_exit("wooda1.c", "north");
    add_exit("woodpath3.c", "northeast");
    add_exit("woodc1.c", "south");
    add_exit("woodpath5.c", "southeast");
...
local_m_in_message()
{
    return ((query_verb() == "east") ? "from the forest." : 0);
}
