 ...
 * 
 * Changes:
 * 2017-10-23 - Cotillion
 * - Rewrote this system to use a central object to track the available
 *   tags to cut down the cpu usage.
 */
...
#define LOG_PATH                ("/d/Genesis/log/")
#define LOG_FILE(player)        ("debugger/" + (player) + ".txt")
...
public int          setdebuglog(string arg);
public int          process_message(string message);
...
create_object(void)
{
    ...
    set_long("This debugger sends tell messages to those people who want "
           ...
           + "  list available     - Shows all the available tags you can "
                                  + "listen to\n"
           + "  logtofile          - Causes the messages to be written to "
           + "                       file. (eg. /d/Genesis/specials/debugger/log/petros.txt)\n"
...
init()
{
    ...
    add_action(setdebuglog, "logtofile");
}
...
{    
    ...
    case "available":
        string *tags = DEBUGGER_CENTRAL->query_tags();
        if (sizeof(tags))
        {
...
showfiles(string arg)
{
    ...
    string *files = DEBUGGER_CENTRAL->query_files(arg);
    output = "";
    if (sizeof(files))
    {
...
add_file(mixed arg)
{
    ...
    DEBUGGER_CENTRAL->start_listening(this_object(), filename);
    write("Ok.\n");
    return 1;
}
...
remove_file(mixed arg)
{
    ...
    listening_files -= ({ filename });
    DEBUGGER_CENTRAL->stop_listening(this_object(), filename);
    write("Ok.\n");
    return 1;
}
...
add_tag(string arg)
{
    ...
    listening_tags += ({ arg });
    DEBUGGER_CENTRAL->start_listening(this_object(), arg);
...
remove_tag(string arg)
{
    ...
    listening_tags -= ({ arg });
    DEBUGGER_CENTRAL->stop_listening(this_object(), arg);
    write("Ok.\n");
    return 1;
}

/*
 * Function name: setdebuglog
 * Description  : Sets the filename that all debug messages should be written
 *                to.
 * Arguments    : arg - tag to be added
 * Returns      : 0/1 - non-success/success
 */
public int          
setdebuglog(string arg)
{
    logging = 1;
    write("Ok. Now writing all debug messages to " + 
        LOG_PATH + LOG_FILE(this_player()->query_real_name()) + "\n");
    return 1;
}

public void
process_message(string message)
{
    ...
    if (logging) {
        string strLogfile = LOG_FILE(player->query_real_name());
        if (strlen(strLogfile) && file_size(strLogfile) >= -1)
        {
            if (log_file(strLogfile, ctime(time()) + ": " + message))
            {
                // If it doesn't succeed, we want to just do the normal.
                return;
            }
        }
    }

    // Default implementation is to just do a catch tell of the message
    // to the player
