 ...
 *   - Added objects with OBJ_I_BROKEN set to list of non-glowing items
 *       so that a kick will allow them to drop out. Ckrik, Jan 2022
 */
...
do_kick(string arg)
{
    ...
    nonglowing += filter(all_inventory(), &operator(!=)(0) @ &->query_prop(OBJ_I_BROKEN));
