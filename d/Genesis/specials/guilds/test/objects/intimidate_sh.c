...
move_living(string how, mixed to_dest, int dont_follow, int no_glance)
{
    ...
    if (oldIsInside ^ newIsInside)
    {
        ...
        {                                        
            ...
            shadow_who->catch_tell("You feel uneasy as you enter the "
                + "enclosed space, looking for signs of danger from "
                + "an unknown assailant.\n");
        }
        else
        {
            objs[0]->remove_intimidate_effects(shadow_who);
            shadow_who->catch_tell("You feel at ease as you leave "
                + "the enclosed space, glad that you no longer need "
                + "to glance over your shoulder for an unknown "
                + "assailant.\n");            
        }
    }
