...
#define ONGOING_DMG_LOG     "/d/Genesis/specials/guilds/test/log/ongoing_dps"
...
create_monster()
{
    ...
    add_prop("_imbuement_rolled", 1); // prevent imbuements from generating
    clone_object(DAMAGE_TRACKER_SH)->shadow_me(this_object());
...
load_guild(string guildname)
{
    ...
    GUILD_TESTER->set_guild_skills(guildname, this_object());
//    set_max_skills();    
    return 1;    
}
...
public void
report_damage(object player)
{
    player->report_damage_tracker_dps(ONGOING_DMG_LOG);
}
...
public void
load_from_mapping(mapping data, string file)
{
    ...
    this_object()->set_is_tracking(npc_data["track_damage"]);
    this_object()->set_prevent_damage(npc_data["prevent_damage"]);
    foreach (string guildname : npc_data["guilds"])
    {
        send_debug_message("test_npc", "Loading guild " + guildname + " for test npc.");
...
load_data(string file)
{
    ...
    load_from_mapping(loaded_data, file);
}
...
public void
call_notify_function(string notify_function, object spellobj, string arg)
{
    call_other(this_object(), notify_function, spellobj, arg);
}

public void
notify_spell_complete(object spellobj, string arg)
{
    send_debug_message("test_npc", "Notified for spell completion of "
        + spellobj->query_spell_name());
    if (strlen(npc_data["spell_notify_function"]))
    {
        set_alarm(0.1, 0.0, &call_notify_function(npc_data["spell_notify_function"], spellobj, arg));
        return;
    }
}

public void
notify_spell_cooldown_complete(object spellobj, string arg)
{
    send_debug_message("test_npc", "Notified for cooldown completion of "
        + spellobj->query_spell_name());
    if (strlen(npc_data["spell_notify_function"]))
    {
        set_alarm(0.1, 0.0, &call_notify_function(npc_data["spell_notify_function"], spellobj, arg));
...
public void
notify_spell_effect_complete(object spelleffect)
{
    send_debug_message("test_npc", "Notified for spell effect "
        + spelleffect->query_spell_effect_name());
    if (strlen(npc_data["spell_notify_function"]))
    {
        set_alarm(0.1, 0.0, &call_notify_function(npc_data["spell_notify_function"], spelleffect, 0));
        return;
    }
}
