...
config_light_spell()
{
    ...
    set_spell_object("/d/Genesis/specials/examples/grimoire_light_obj");
}
...
concentrate_msg(object caster, mixed * targets, string arg)
{
    string caster_message, watcher_message;
        ...
        caster_message = "You incant the words of magic from the leather-bound grimoire, " +
             "'Illuminatus!'\n";
        watcher_message = QCTNAME(caster) + " incants words of magic from a leather-bound " + 
              "grimoire.\n";
...
check_valid_action(object caster, mixed * targets, string arg, int execute = 0)
{
    ...
    if(caster->query_skill(SS_SPELLCRAFT) < 5)
    {
       caster->catch_tell("You need to be trained in spellcraft to at least " +
           "confident student to cast this spell.\n");
       return 0;
     }

    if (!environment(caster)->query_prop("_room_i_tutorial"))
    {
       caster->catch_tell("You attempt to call upon the 'Illuminatus' spell, but " +
           "it appears it only works in the tutorial area of Greenhollows!\n");
       return 0;
    }
