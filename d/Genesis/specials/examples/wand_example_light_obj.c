...
#define LIGHT_SUBLOC        "_example_wand_light_subloc"
...
create_spell_object()
{
    set_name("_example_wand_light_object_");
...
hook_spell_effect_started()
{
    ...
    if (objectp(target))
    {
        target->catch_tell("Your example wand begins to emit light "
            + "and the room gets brighter.\n");
        tell_room(environment(target), QCTPNAME(target) + " example wand " 
...
hook_spell_effect_ended()
{
    ...
    if (objectp(target))
    {
        target->catch_tell("The light surrounding your example wand fades.\n");
...
hook_spell_effect_warn_fading()
{
    ...
    if (objectp(target))
    {
        target->catch_tell("You sense the intensity of the light surrounding "
            + "your example wand begin to wane.\n");
    }
}
...
hook_spell_effect_not_maintained()
{
    ...
    if (objectp(caster))
    {
        caster->catch_tell("You are mentally unable to maintain "
            + "the light surrounding your example wand.\n");
    }    
}
