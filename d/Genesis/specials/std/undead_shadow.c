...
private void make_player_undead(object player) {
    ...
    player->inc_prop(OBJ_I_RES_POISON, 100);
    ...
    player->configure_racial_bonuses();
}
...
private void make_player_living(object player) {
    ...
    player->dec_prop(MAGIC_I_RES_POISON, 100);
...
{
    if (skill == RACIAL_ABILITY_SKILL && shadow_who->query_prop(LIVE_I_UNDEAD))
    {
        // This will override the ones selected by the player, for thematics.
        return GOOD_MEMORY | STALWART;
    }
