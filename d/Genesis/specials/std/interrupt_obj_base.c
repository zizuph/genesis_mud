...
start()
{
    object  player, target, ability, *a_filter;
    mapping abilities;
    int     timed, value;
    
    if (!objectp(player = query_effect_caster()) &&
        objectp(player = this_player()))
        set_effect_caster(player);
    
    if (!objectp(target = query_effect_target()) || !objectp(player))
        return !!remove_object();
    ...
    /* Make sure that caid is set, or take the value from calling ability */
    if (!query_effect_combat_aid(player))
    {
        if (timed < 0)
        {
            hook_unable_to_start();
        ...
        if (!(value = previous_object()->query_combat_aid()) &&
            !(value = ftoi(previous_object()->query_spell_combat_aid())))
        {
            hook_unable_to_start();
            return !!remove_object();
        }
        
        set_interrupt_combat_aid(value);
    }
    ...
    if (strlen(m_interruptTarget))
    {
        ...
        {
            abilities = query_active_abilities(target)["abilities"];
    ...
    return add_interrupt_effect(target);
} /* start */