...
static mapping valid_dam_mods = ([]);
static mapping applicable_dam_mod = ([]);
/*
...
resolve_applicable_damage_modifications(mapping mod, object effect)
{
    ...
    int     incr = effect->query_dam_mod_combat_aid(target);
    ...
    if (member_array(effect, m_indexes(valid_dam_mods)) > -1)
    {
        int perc = valid_dam_mods[effect];
        perc = perc < 0 ? 0 : perc;
...
query_applicable_damage_modifications(object * effects)
{
    ...
    foreach(object effect: effects)
    {
        ...
        if (effect->query_dam_mod_absorb())
        {
            ...
            caid = absorb[query_dam_mod_category(effect)]["total"];
        }
        else
        {
            amplify = resolve_applicable_damage_modifications(amplify, effect);
            caid = amplify[query_dam_mod_category(effect)]["total"];
        }
        send_debug_message("dam_mod_debug", "Dam mod caid from "
        + effect->query_dam_mod_combat_aid(shadow_who) + " to " + caid + ".\n");
    }
...
query_effect_combat_aid()
{
    ...
    mapping effect_tally = ([]);
    int total;
    
    if (m_sizeof(valid_dam_mods))
        effects = m_indexes(valid_dam_mods);
    
    effect_tally = query_applicable_damage_modifications(effects);
    
    foreach (string key, mapping value: effect_tally)
    {
        total += value["total"];
    }
...
{
    int percentage = query_effect_percentage();
    
    if (!percentage)
    {
        // We need to account for negative dam mod value
        percentage = min(query_effect_combat_aid(), 0);
    }
...
hit_me(int wcpen, int dt, object attacker, int aid, int target_hitloc = -1)
{
    ...
    if (wcpen > 0)
    {
        foreach(object effect : query_dam_mod_effects())
        {
            ...
            valid_dam_mods[effect] = effect->query_dam_mod_strength(shadow_who, 
                                                        attacker, aid);
            send_debug_message("dam_mod_debug", "Dam mod effect percentage " + valid_dam_mods[effect] + ".\n");
        
        }
        oldwcpen = wcpen;
        modifier = query_damage_modification_percentage();
        send_debug_message("dam_mod_debug", "Total damage modified by: " + modifier + ".\n");
    ...
    {
        if (!m_sizeof(applicable_dam_mod))
            query_applicable_damage_modifications(m_indexes(valid_dam_mods));
