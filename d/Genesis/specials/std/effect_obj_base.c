...
/*
 * Function name:   convert_effect_caid_to_percentage
 * Description:     Convert caid to percentage
 * Arguments:       (int) caid - the caid to convert
 * Returns:         (int) number between 1 and 100
 */
private int
convert_effect_caid_to_percentage(int caid)
{
...
} /* convert_effect_caid_to_percentage */
...
{
    int caid = query_effect_combat_aid(player);
    return convert_effect_caid_to_percentage(caid);
...
query_effect_strength(object player, object attacker, int attack_id)
{
    ...
    effect = effect * skill_modifier * F_STR_FACTOR(stat_modifier) / 10000;
    effect = convert_effect_caid_to_percentage(effect);
    send_debug_message("base_effect", "Effect Strength: " + effect);
    return effect;
} /* query_effect_strength */
