...
calculate_stun_times()
{
    ...
    {
        stun_percentage = itof(effect->query_stun_percentage(shadow_who)) / 100.0;
        total_percentage *= stun_percentage;
        stun_percentage = total_percentage;
        stun_caid = effect->query_stun_combat_aid(shadow_who);
        total_caid += stun_caid;
        cycle = effect->query_stun_cycle_time(shadow_who);
