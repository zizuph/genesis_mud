/*
 * Standard Charged Harm Spell Object
 *
 * This is the spell object for the standard Genesis Magic System Charged
 * Harm spell. When started, it will continually "charge" until the charge
 * is released into an attack on the currently targeted enemy.
 *
 * It is intentional to not allow this to initiate attacks. Rather, it
 * will only work against a target when already in battle with them.
 *
...
create_spell_object()
{
    set_name("_charged_harm_object_");
    set_short("charged harm spell object");    
    set_long("This is the standard charged harm spell's object. It temporarily "
        + "allows one to discharge harmful effects on enemies.\n");

    add_prop(OBJ_S_WIZINFO, "This is a spell object " +
                            "created by the charged harm spell.\n");
    set_spell_effect_desc("charged harm");    
}
...
setup_spell_effect()
{
    ...
    foreach (int stat_id, int caid : stat_mappings)
    {
        switch (stat_id)
        {
        case SS_STR:
            stat_boost_mapping[stat_id] = query_str_stat_bonus(player, caid);
            break;
        case SS_DEX:
        case SS_CON:
        case SS_WIS:
        case SS_INT:
        case SS_DIS:
            stat_boost_mapping[stat_id] = query_stat_bonus(player, stat_id, caid);
            break;
        }
        player->set_stat_extra(stat_id, player->query_stat_extra(stat_id) + stat_boost_mapping[stat_id]);
    }
