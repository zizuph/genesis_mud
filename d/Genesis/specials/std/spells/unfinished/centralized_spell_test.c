 ...
 *
 *
 * Changed how damage is determined.
 * - Arman Kharas, May 2020
 */
...
// Default spell pen value. This should probably be moved
// to <formulas.h> at some point.
// 
// The formula essentially involves a scaling pen formula based on skills:
//      Superior amateur (AG level)            :  7
//      Superior craftsman (layman max level)  :  21
//      Confident professional (occ SoHM)      :  26
//      Superior guru (max skill)              :  35
// plus a mental stat component scaling at 4 pen points per 100 stat.
 
#define F_SPELLPOWER_PEN(skill, mental) (ftoi(itof(skill) / 2.85) + (mental) / 25)


#define SPELL_DEBUG(x)  find_player("arman")->catch_msg("[TestSpell] " + x + "\n")
...
static mixed        Stats_used;
private int         Spell_success;
...
/*
 * Function:    set_spell_stats
 * Description: Sets the TS_* stat type or types as defined in 
 *              /sys/tasks.h that determine the damage output 
 *              of the spell.
 */
public void 
...
/*
 * Function:    query_spellcaster_spellpower
 * Description: Queries the spell power value of any items or
 *              weapons wielded or held. If none with spell power,
 *              return 0;
 * Arguments:   object caster - the object casting the spell.
 */

public int
query_spellcaster_spellpower(object caster)
{
    object * items = caster->query_tool(-1);
    int item_num = sizeof(items);
    int spellpower = 0;

    foreach(object spell_item: items)
    {
        int item_spellpower = spell_item->query_magic_spellpower();

        if(item_spellpower > spellpower)
            spellpower = item_spellpower;
    }

    send_debug_message("debug_spell_pen", "Spell power of EQ: " + spellpower + ".\n");
...
query_spell_weapon_class()
{
    ...
    if (query_spell_at_will())
    {
        ...
        SPELL_DEBUG("Atwill mana cost: " +at_will_mana+ ".");
    ...
    {
        // Default 'Spell power' is determined by skill and stat.
        // The skill influencer is the casters element spell skill.
        // The stat is what is set for query_spell_weapon_class_stat_influencer(),
        // defaulting to intelligence.

        object caster = this_player();
        int mental = query_spell_weapon_class_stat_influencer(); 
        int element_skill = caster->query_skill(Element_skill);
        int tool_bonus = query_spellcaster_spellpower(caster);

        spell_wc = max(1, F_SPELLPOWER_PEN(element_skill, mental));

        send_debug_message("debug_spell_pen", 
                               "Default spell weapon class: " + spell_wc 
            + " consists of " + mental + " (mental) " + element_skill
            + " (element spell skill).\n");

        // Magical items worn, held, or wielded may provide a better 'pen'
        // than the casters natural casting ability.
        if(tool_bonus > spell_wc)
            spell_wc = tool_bonus;

        send_debug_message("debug_spell_pen", 
                               "Total spell weapon class: " + spell_wc 
            + " consists of " + mental + " (mental) " + element_skill
            + " (element spell skill), "
            + tool_bonus + " item spellpower.\n");
    }

    // A flaming black broadsword with an intense sharpstone has a pen of 68.
    // We will cap maximum wc at that for harm spells.    
    return min(68, spell_wc);
}
...
get_success_chance()
{
    // Hit and pen calculations are adjusted according to the 
    // difficulty of the cast. This function returns the success
    // chance depending on the spell TASK setting.
    // TASK_EASY        - 100%
    // TASK_ROUTINE     - 97%
    // TASK_HARD        - 85%
    // TASK_DIFFICULT   - 65%
    // We cap at TASK_DIFFICULT.

    int task = get_spell_task();

    if (task >= TASK_DIFFICULT)
        return 65;                 // 65% success
    else if (task >= TASK_HARD)
        return 85;                 // 85% success 
    else if (task >= TASK_ROUTINE)
        return 97;                 // 97% success 
    else
 ...
 *                Copied from /std/combat/cbase.c
 */
...
query_success(object actor, object target, mixed *items)
{
    ...
    if(living(target))
    {
        if(query_ability_offensive())
        {
            ...
            SPELL_DEBUG("Target defence values: " +tmp+ ".");
            ...
            SPELL_DEBUG("Target defence values (npc check): " +tmp+ ".");
           ...
           result = 4 * fixnorm(random(result) + random(result) + 
            ...
            SPELL_DEBUG("Fixnorm success result: " +result+ ".");
           ...
           result += 2 * fixnorm(actor->query_stat(SS_DIS), 
            ...
            SPELL_DEBUG("Fixnorm success result (SS_DIS check): " +result+ ".");
        }
        else
        {
           //  For spells without a target, the caster's SS_DIS plays a role
           //  in spell success. If they are not disciplined enough they will
           //  struggle to control their own magical power.
           dis = actor->query_stat(SS_DIS); 
           mentals = (actor->query_stat(SS_INT) + actor->query_stat(SS_WIS)) / 2;
    ...
    SPELL_DEBUG("Final combined success result: " +result+ ".");

    // we emulate a simple d100 roll against the result value for success.
    Spell_success = max(0, result - random(100));

    SPELL_DEBUG("Spell success roll: " +Spell_success+ ".");

    if(Spell_success)
    {
        SPELL_DEBUG("Spell success!");
        return 1;
    }
    else
    {
        SPELL_DEBUG("Spell failure!");
        return 0;
    }
...
{
    object caster = this_player();
    int task_stat = TS_WIS;
    int result;

    // Instead of the stat modifier defaulting to TS_WIS, the damage
    // stat can be determined in the set-up of the spell. - Arman Kharas.

    if(query_spell_weapon_class_stat_influencer())
    {
        switch(query_spell_weapon_class_stat_influencer())
        {
            case SS_STR:
                task_stat = TS_STR;
                break;
    ...
    // For at will spells, we allow damage to scale 10% up/down
    // depending on stat modifier.
    // maximum (F_STR_FACTOR(127) == 110).
    // Normal harm spells are uncapped upwards, similar to melee
    // special attacks.

    if (query_spell_at_will())
    {
        result = min(127, max(90, 
            caster->find_drm( ({ task_stat }) ) / 2));
    }
    else
    {
        result = max(90, caster->find_drm( ({ task_stat }) ) / 2);
    }
...
calculate_pen(int resist, int bAddLostCombatAid = 1)
{
    ...
    SPELL_DEBUG("Pen result after stat modifier applied: " + result + ".");
    ...
    if (bAddLostCombatAid)
    {
        ...
        SPELL_DEBUG(sprintf("Adding %f lost CAID.", lost_caid));
        caid += lost_caid;
    }
    ...
    SPELL_DEBUG("Result after " + ftoi(caid) + " CAID applied: " + result + ".");
    ...
    SPELL_DEBUG("Result after time_factor " + time_factor + " applied: " + result + ".");
    ...
    SPELL_DEBUG("Result after difficulty applied: " + result + ".");
    ...
    if (query_spell_at_will())
    {
        // At will spells do not benefit from magic vulnerability as much.
        // It defeats the intent of the introduction of these spells.
        if(resist < 0)
        {
            resist = resist / 2;
        }
    ...
    SPELL_DEBUG("Result after resistance " + resist + " applied: " + result + ".");
