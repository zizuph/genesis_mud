 ...
 *
 * Zizuph Nov 2021 - Add a cooldown to teleported players.  Team teleport is
 *      providing a lot of combat aid.
 *
 */
...
#define LIVE_I_NEXT_TELEPORT_TIME "_live_i_next_teleport_time"
...
public void             hook_describe_ready_to_teleport(object target);
...
int get_next_teleport_delay(object destination)
{
...
config_spell()
{
    ...
    set_next_teleport_delay(3600 * 2);
    set_spell_mana(30);
...
check_valid_action(object caster, mixed * targets, string arg, int execute)
{
    ...
    for (int i = 1; i < sizeof(targets); i++)
    {
        ...
        if (target->query_prop(LIVE_I_NEXT_TELEPORT_TIME) > time())
        {
...
resolve_spell(object caster, mixed * targets, int * resist, int result)
{
    ...
    for (int i = 1; i < sizeof(targets); i++)
    {
        ...
        {
            int delay = get_next_teleport_delay(target_room);
            target->move_living("M", MASTER_OB(target_room), 1, 0);
            target->add_prop(LIVE_I_NEXT_TELEPORT_TIME,
                time() + delay);
            if (delay > 0)
            {
                set_alarm(itof(delay + 1), 0.0,
                   &hook_describe_ready_to_teleport(target));
            }
        }
    }
...
/*
 * Function:    hook_describe_ready_to_teleport
 * Description: This is the hook that notifies the target that they can be
 *              teleported again.
 */
public void
hook_describe_ready_to_teleport(object target)
{
    if (target)
    {
        target->catch_tell("You feel ready to teleport again.\n");
    }
}
