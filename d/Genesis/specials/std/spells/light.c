 ...
 * General Light Spell
 * 
 * This is a generic light spell that should be used as the basis for all
 * light spells under the Genesis Magic system created in 2009.
 *
...
public string * query_light_ingredients(object caster);
...
config_light_spell()
{
    ...
    set_spell_object(STD_SPELL_OBJ_DIR + "light_obj");
}
...
config_spell()
{
    ...
    // Call this after the config_light_spell to ensure that
...
{
    resolve_light_spell(caster, targets, resist, result);
    ...
    if (query_spell_can_be_renewed())
    {
        ...
        {
            make_spell_object(query_spell_object(), caster, remaining_targets, remaining_resists, result, result);    
        }
    }
    else
    {
...
check_valid_action(object caster, mixed * targets, string arg, int execute)
{
    ...
    if (!query_spell_can_be_renewed())
    {
        // The light spell cannot be cast if it is already in effect
...
{
    write("You are already maintaining the light that surrounds you.\n");
}
...
{
    write("You renew the light that you are maintaining!\n");
}
