 ...
 *
 * 2022-02 Zizuph - transitioned to new resistance formula that includes AC-based resistance.
 *
 */
...
resolve_spell(object caster, mixed * targets, int * resist, int result)
{
    ...
    for (int index = 0; index < sizeof(targets); ++index)
    {
        ...
        foreach (string element : all_resistance_types)
        {
            ...
            {
                // We are going to ignore natural resistance for the output.
                int natural_res = 0;
                res = combine_resistances(target, ({ element, MAGIC_I_RES_MAGIC }));
                res = combine_resistance_with_base(target,natural_res, res);
            }
            if (res == 0)
            {
                continue;
