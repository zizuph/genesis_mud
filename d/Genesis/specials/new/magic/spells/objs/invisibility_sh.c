...
{
    hook_spell_no_fighting(m_spell_object, target);
    m_spell_object->dispel_effect(target);
}
...
hook_i_caught_stealing(int success, object item, object from)
{
    ...
    {
        hook_spell_action_caught(m_spell_object, from);
        m_spell_object->dispel_effect(from);
    }
}
