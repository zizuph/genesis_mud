...
setup_spell_effect()
{
    ...
    add_prop(OBJ_I_LIGHT, max(1, power / 200));
...
hook_spell_effect_started()
{
    ...
    if (objectp(target))
    {
        target->catch_tell("You begin to emit light and the room gets "
            + "brighter.\n");
        tell_room(environment(target), QCTNAME(target) + " begins to emit "
            + "light and the room gets brighter.\n", ({ target }));
    }    
}
