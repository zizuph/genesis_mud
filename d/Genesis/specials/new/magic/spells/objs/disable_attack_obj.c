...
create_spell_object()
{
    set_name("_disable_attack_object_");
    set_short("disable attack object");    
    set_long("This is the standard disable attack spell object. " +
             "It disables attacks from a weapon of the target.\n");

    add_prop(OBJ_S_WIZINFO, "This is a spell object " +
                            "created by the disable attack spell.\n");
    set_spell_effect_desc("disable attack");
...
setup_spell_effect()
{
    ...
    effect_obj = clone_object(SPELL_OBJ_DIR + "disable_attack_effect_obj");
    effect_obj->set_effect_caster(query_spell_effect_caster());
    effect_obj->set_effect_target(query_spell_effect_target());
    effect_obj->set_disable_attack_stats(query_spell_stats());
    ...
    if (mappingp(input))
    {
        ...
        effect_obj->set_disable_attack_tohit(tohit);
        resistance = input["resistance"];
        attack_id = input["attack_id"];
        targets = max(1, input["targets"]);
        send_debug_message("debug_disable_attack",
            sprintf("Resistance : %f, tohit: %d, aid: %d", 
            resistance, tohit, attack_id));
    }
    else
    {
        send_debug_message("debug_disable_attack",
            "Mapping for disable attack is missing.");
        // We need an attack ID from the mapping
        return 0;
    }
    effect_obj->set_disable_attack_chance(100);
    // Apply increase in CAID due to innate resistance here.
    int combat_aid = ftoi(query_spell_combat_aid() * 100.0 / (100.0 - resistance));
    combat_aid = combat_aid / targets;
    
    send_debug_message("debug_disable_attack",
        sprintf("Final combat aid : %d", combat_aid));
    effect_obj->set_disable_attack_combat_aid(combat_aid);
    effect_obj->set_disable_attack_attack_ids(({ attack_id }));
...
dispel_spell_effect(object dispeller)
{
    ...
    query_effect_target()->remove_disable_attack_effect(effect_obj);
    return 1;
}
