/*
 * Turn Target Spell - Zizuph, Jan 2022
 *
...
// Prototypes
public void     resolve_turn_target_spell(object caster, mixed * targets, int * resist, int result);
public string*  query_turn_target_ingredients(object caster);
public void     resolve_spell_resisted(object caster, object target, int resist, int result);
public void     hook_describe_turn_target(object caster, mixed * targets, int result);
public void     hook_maintaining_turn_target(object caster, object target);
public void     hook_target_not_valid(object caster, object target);

/*
 * Function:    config_turn_target_spell
 * Description: Config function for turn target spells. Redefine this in your
 *              own turn target spells to override the defaults.
...
config_spell()
{
    ...
    // Call this after the config_turn_target_spell to ensure that
...
{
    resolve_turn_target_spell(caster, targets, resist, result);
    
    hook_describe_turn_target(caster, targets, result);
 ...
 */
public int target_valid(object target)
{
    return 0;    
}
...
check_valid_action(object caster, mixed * targets, string arg, int execute)
{
    ...
    if(caster->query_turn_target_caster_shadow())
    {
        hook_maintaining_turn_target(caster, targets[0]);
        return 0;
    }
    
    // The turn target spell cannot be cast if it is already in effect. At some
    // point, if this spell is going to be mass cast, we should allow the
    // spell to be cast even if some of the targets have the turn target spell on.
    ...
    foreach (object target : targets)
    {
        ...
        {
            hook_target_not_valid(caster, target);
            return 0;
        }  
    }
    return result;    
...
public void
hook_describe_turn_target(object caster, mixed * targets, int result)
{
}

/*
 * Function:    resolve_turn_target_spell
...
{
    caster->catch_msg("You are currently maintaining a spell of turn target.\n");
}
...
hook_target_not_valid(object caster, object target)
{
    if (objectp(target))
    {
        caster->catch_msg(QCTNAME(target) + " cannot be targeted!\n");
    }
}
