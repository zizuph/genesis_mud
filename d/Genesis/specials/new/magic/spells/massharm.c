 ...
 */
public void
set_spell_target_limit(mixed limit)
{
    if (intp(limit))
    {
        limit = itof(limit);
    }
    target_limit = min(2.0, limit);
...
config_harm_spell()
{
    ...
    set_spell_mana_factor(0.4 + (target_limit * 0.8));
}
...
do_harm_damage(object caster, object target, int pen)
{
    ...
    if (sizeof(enemies[caster]))
        pen = min(pen, ftoi(itof(pen) * target_limit) / sizeof(enemies[caster]));
