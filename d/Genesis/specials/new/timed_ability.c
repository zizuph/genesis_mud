 ...
 *
 * Modified 2021-03-21 by Carnak:
 * - Added an abort object to all timed abilities.
 */
...
static int      Ability_haste,          /* Is this ability affected by haste? */
                ...
                Ability_abort;          /* True if this ability can be aborted */
static mapping  prepare_alarms = ([ ]);
static mapping  cooldown_alarms = ([ ]);
static mapping  two_handed_combat = ([ ]);
static mapping  timed_master = ([]);    /* Holds the timed master objects */
...
public void set_ability_abort(int abort)
{
    Ability_abort=abort;
}

public int query_ability_abort()
{
    return Ability_abort;
}
...
void schedule_ability(object actor,mixed * targets,string arg)
{
    ...
    // There is a 1 rotation penalty on concentration abilities if they do
    // not exceed 5 seconds in activation time.
    if (query_ability_concentrate() && time < speed)
    {
        // If there is already a present attack delay which is higher,
        // we do not add to it.
        int value = actor->query_prop(LIVE_I_ATTACK_DELAY);
        value = max(ftoi(speed + time), value);
...
set_ability_pending(object actor, object room, int set)
{
    ...
    if (objectp(actor) && !objectp(timed_master[actor]))
    {
        timed_master[actor] = present("_timed_master_object", actor);
        if (!objectp(timed_master[actor]))
        {
            setuid();
            seteuid(getuid());
            timed_master[actor] = clone_object(TIMED_MASTER_OBJ);
            timed_master[actor]->move(actor, 1);
            timed_master[actor]->set_ability_actor(actor);
        }
    }
    ...
    if (set)
    {
        ...
        timed_master[actor]->add_ability_file(this_object());
    }
    else
    {
        actor->remove_prop(LIVE_O_ACTION+Ability_name);
        timed_master[actor]->remove_ability_file(this_object());
    }        
}
