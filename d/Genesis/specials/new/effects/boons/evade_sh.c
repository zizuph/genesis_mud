...
/*
 * Function     : query_max_evade_percentage
...
query_not_attack_me(object attacker, int att_id)
{
    ...
    if (!sizeof(valid_evades))
    {
        // Don't do any processing if there are no valid evade effects
        return shadow_who->query_not_attack_me(attacker, att_id);
    }

    report_total_evade(query_effect_percentage(valid_evades),
      query_effect_combat_aid(valid_evades));

    
    if (query_effect_percentage(valid_evades) > random(100))
    {
        object effect = one_of_list(valid_evades - ({ 0 }));
        send_debug_message("evade_sh", capitalize(effect->query_name()) 
            + " caused an evade.");
