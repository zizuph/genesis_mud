...
/*
 * Function     : query_max_reflect_percentage
 ...
 */
public int 
query_max_reflect_percentage()
{
    return query_effect_percentage(query_effects());    
}

/*
 * Function     : query_max_reflect_combat_aid
...
do_reflect_damage(int wcpen, int dt, object attacker, int att_id)
{
    ...
    foreach (object effect : query_reflect_effects())
    {
        ...
        /*
          Rather than simply call query_effect_percentage(valid_reflects), each reflect
          has a percentage
        */
        send_debug_message("reflect_sh", "Real effect caid: " + effect->query_effect_combat_aid() + ".");
        caid = effect->query_effect_modified_combat_aid(shadow_who);
        send_debug_message("reflect_sh", "Stat modified caid: " + caid + ".");
    }
    ...
    total_reflect_percentage = query_effect_percentage(valid_reflects);

    // Call the reporting function
    report_total_reflect(total_reflect_percentage, total_reflect_aid);
    
    if (total_reflect_percentage > 0 && !attacker->query_not_attack_me(shadow_who, -1))
    {
        object effect = one_of_list(valid_reflects);
        send_debug_message("reflect_sh", capitalize(effect->query_name()) 
        + " caused an reflect.");
        
        reflect_pen = wcpen * total_reflect_percentage / 100;
        send_debug_message("reflect_sh", "Unmodified pen: " + wcpen
        + " after " + total_reflect_percentage + " percentage mod: "
        + reflect_pen + ".");
