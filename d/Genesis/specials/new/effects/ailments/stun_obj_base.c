...
public void         hook_stun_failed(object stunner, object stunnee);
public void         hook_stun_blocked(object stunner, object stunnee);
...
query_stun_percentage(object player)
{
    ...
    effect_percentage = convert_caid_to_percentage(caid);    
...
query_stun_combat_aid(object player)
{
    ...
    percentage = ::query_effect_modified_combat_aid(player);
 ...
 */
public int query_stun_valid(object player, object attacker, 
                            int attack_id)
{
    return 1;
}
...
public int add_stun_effect(object target)
{
    ...
    if (target->query_prop(LIVE_I_NO_STUN))
    {
        ...
        hook_stun_blocked(query_effect_caster(), query_effect_target());
        remove_object();
        return 0;
    }
    ...
    if (target && living(target))
    {
        ...
        if (target->add_stun_effect(this_object()))
        {
        ...
        {
            hook_stun_failed(query_effect_caster(), query_effect_target());
        }
    }    
    return 0;
}
...
hook_stun_failed(object attacker, object target)
{
    ...
    if (objectp(attacker))
    {            
        attacker->catch_msg("You try to stun " + QTNAME(target) + ", but fail.\n");
    }
}

public void
hook_stun_blocked(object attacker, object target)
{
    if (!objectp(target))
    {
        return;
    ...
    if (objectp(attacker))
    {            
        attacker->catch_msg(QCTNAME(target) + " cannot be stunned.\n");
    }
}
