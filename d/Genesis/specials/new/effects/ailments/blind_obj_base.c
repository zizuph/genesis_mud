 ...
 *                  (object*) all other active effects.
 * Returns:         Nothing
 */
public void
...
query_blind_chance()
{
    return query_effect_likelyhood();
} /* query_blind_chance */
...
{
    return query_effect_modified_combat_aid(player);
...
/*
 * Function name:   end_blind_effect
 * Description:     Ends the blind effect on the affected target and prints
 *                  corresponding hook.
 ...
 */
public status
end_blind_effect(object target)
{
    object *effects = ({});
    ...
    effects = target->query_active_blind_effects();
        
    function func;
    if (functionp(func = gBlindEnded))
        applyv(func, ({ query_effect_caster(), target, effects}));
 ...
 * Description:     Ends the cooldown for the blind effect, and removes it.
 * Arguments:       (object) target - the one affected by blind
 * Returns:         (status) 1 - effect successfully removed
 *                           0 - effect failed to be removed
 */
...
{
    end_blind_effect(query_effect_target());
