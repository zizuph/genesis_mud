/* 
 * Badge for Getting Five Checkins
 *
...
create_badge()
{
    set_badge_name("Five Checkins");
    set_badge_description("For checking into five different locations, you earn "
                        + "this very cool Five Checkins Badge!\n");
    set_listening_events( ({ "event_location_checkin" }) ); // string array of events we're listening to
}
...
{
    int num_checkins = m_sizeof(badge_row["badge_history"]);
    if (num_checkins >= 5)
    {
        return 1;
    }
...
hook_notify_player_earned_badge(object player)
{
    ...
    player->catch_tell("You've checked in five times! For that, you have earned the "
