/* 
 * Christmas Event 2013 Badge - Found Some In Avenir
 *
...
create_badge()
{
    set_badge_name("Found Something In Avenir");
    set_badge_description("Avenir, land of opportunity! Opportunity to collect for the "
                          + "Christmas 2013 event! You get this badge for having "
                          + "located your target with the rune stone in Avenir or "
                          + "Terel. Collect from enough domains to become known "
                          + "as the Donut Trotter Hammie!\n");
...
does_event_apply(object player, string event_name, mixed data)
{
    ...
    if (wildmatch("/d/Avenir/*", room_file)
        || wildmatch("/d/Terel/*", room_file))
    {
        return 1;
    }
...
hook_notify_player_earned_badge(object player)
{
    ...
    player->catch_tell("You've managed to collect something in the Avenir or Terel domain! For that, you \n"
