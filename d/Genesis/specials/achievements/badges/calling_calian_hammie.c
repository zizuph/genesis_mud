/* 
 * Christmas Event 2013 Badge - Calling Calian Hammies
 *
...
create_badge()
{
    set_badge_name("Calling Calian Hammies");
    set_badge_description("Which of the Kyrullians know where the Hams are? Probably none "
                          + "of them, which is why when someone needs some ham and chips, "
                          + "they call upon you! Keep it up and "
...
hook_notify_player_earned_badge(object player)
{
    ...
    player->catch_tell("You've managed to collect something in the Calia domain! For that, you \n"
...
does_event_apply(object player, string event_name, mixed data)
{
    ...
    if (wildmatch("/d/Calia/*", room_file))
    {
        return 1;
    }
