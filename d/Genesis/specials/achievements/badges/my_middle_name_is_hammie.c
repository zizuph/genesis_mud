/* 
 * Christmas Event 2013 Badge - My Middle Name Is Hammie
 *
 * This is a badge for the Christmas Event 2013. You get this for
 * collecting and reaching 250 points
 *
...
create_badge()
{
    set_badge_name("My Middle Name Is Hammie");
    set_badge_description("Been spending too much time with Christmas Ham and Chips? You "
                          + "smell like smoked ham all the time. So much so that "
                          + "you're pretty sure your middle name is Hammie. You've "
                          + "collected 250 points so far. Keep it up and you just "
                          + "might be fit to be called a Professional Hammie!\n");
...
does_event_apply(object player, string event_name, mixed data)
{
    ...
    foreach (mixed badge_row : earned_badge_rows)
    {
        ...
        if (wildmatch("*i_collect_better_than_you", badge_file))
        {
            return 1;
        }
    }
