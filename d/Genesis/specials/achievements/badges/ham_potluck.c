/* 
 * Christmas Event 2013 Badge - Ham Potluck
 *
 * This is a badge for the Christmas Event 2013. You get this for
 * collecting at least one of each type of available hams
 *
...
create_badge()
{
    set_badge_name("Ham Potluck");
    set_badge_description("A potluck of Christmas Hams? That's what you've managed to "
                          + "achieve after collecting at least one set of Hampshire, Berkshire "
                          + "and Mangalitsa Hams!\n");
...
{
    if (data["object_id"] != ED_HAMPSHIRE_HAM
        && data["object_id"] != ED_BERKSHIRE_HAM
        && data["object_id"] != ED_MANGALITSA_HAM)
    {
        return 0;
    }
...
update_badge_history_and_data(object player, string event_name, mixed data, object bracelet, mixed badge_row)
{
    ...
    {
    case ED_HAMPSHIRE_HAM:
        badge_row["badge_history"]["hampshire"] = 1;
        break;

    case ED_BERKSHIRE_HAM:
        badge_row["badge_history"]["berkshire"] = 1;
        break;

    case ED_MANGALITSA_HAM:
        badge_row["badge_history"]["mangalitsa"] = 1;
        break;
    }
...
{
    if (badge_row["badge_history"]["hampshire"]
        && badge_row["badge_history"]["berkshire"]
        && badge_row["badge_history"]["mangalitsa"])
    {
        return 1;
    }
...
hook_notify_player_earned_badge(object player)
{
    ...
    player->catch_tell("You've managed to collect at least one Hampshire Ham, one Berkshire Ham, and one \n"
                       + "Mangalitsa Ham! For that, you have earned the "
