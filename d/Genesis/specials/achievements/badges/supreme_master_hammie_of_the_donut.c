/* 
 * Christmas Event 2013 Badge - Supreme Master Hammie of the Donut
 *
 * This is a badge for the Christmas Event 2013. You get this for
 * collecting and reaching 1000 points
 *
...
create_badge()
{
    set_badge_name("Supreme Master Hammie of the Donut");
    set_badge_description("Pigs and Wood Chips tremble before you no matter where you "
                          + "go. You have achieved that which this Donut has never "
                          + "seen and are fit to be known as the Supreme Master Hammie "
                          + "of the Donut. Now go enjoy some ham! You've already collected "
                          + "1000 points.\n");
...
does_event_apply(object player, string event_name, mixed data)
{
    ...
    foreach (mixed badge_row : earned_badge_rows)
    {
        ...
        if (wildmatch("*professional_hammie", badge_file))
        {
            return 1;
        }
    }
