/* 
 * Christmas Event 2013 Badge - Calling on Kalad
 *
...
create_badge()
{
    set_badge_name("Calling on Kalad");
    set_badge_description("Who you gonna call when you need some Christmas Ham and Chips? Kalad! "
                          + "Kalad! Kalad! Keep it up and "
...
hook_notify_player_earned_badge(object player)
{
    ...
    player->catch_tell("You've managed to collect something in the Kalad domain! For that, you \n"
...
does_event_apply(object player, string event_name, mixed data)
{
    ...
    if (wildmatch("/d/Kalad/*", room_file))
    {
        return 1;
    }
