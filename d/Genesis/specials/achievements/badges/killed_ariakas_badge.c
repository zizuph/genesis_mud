/* 
 * Badge for Killing Ariakas
 *
 * This is a sample badge demonstrating a badge that you can get for
 * checking into five unique locations.
 *
...
create_badge()
{
    set_badge_name("Killed Ariakas");
    set_badge_description("For killing Ariakas, you get this very cool Killed Ariakas Badge!\n");
    set_listening_events( ({ "event_enemy_kill" }) ); // string array of events we're listening to
}
...
does_event_apply(object player, string event_name, mixed data)
{
    ...
    if (enemy != "/d/Ansalon/taman_busuk/sanction/npc/ariakas")
    {
        // This badge is only for Ariakas
        return 0;
    }
...
update_badge_history_and_data(object player, string event_name, mixed data, object bracelet, mixed badge_row)
{
    ...
    badge_row["badge_history"][timestamp] = enemy;
...
hook_notify_player_earned_badge(object player)
{
    ...
    player->catch_tell("You've killed Ariakas! For that, you have earned the "
