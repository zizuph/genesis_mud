/* 
 * Christmas Event 2013 Badge - Fair Faerun Hammie
 *
...
create_badge()
{
    set_badge_name("Fair Faerun Hammie");
    set_badge_description("They say that rubbing smoked ham juice over your skin makes it "
                          + "fair. Regardless of whether that's true or not, you've done "
                          + "good work in recovering ham and chips from the thieves! Keep it up and "
...
hook_notify_player_earned_badge(object player)
{
    ...
    player->catch_tell("You've managed to collect something in the Faerun domain! For that, you \n"
...
does_event_apply(object player, string event_name, mixed data)
{
    ...
    if (wildmatch("/d/Faerun/*", room_file))
    {
        return 1;
    }
