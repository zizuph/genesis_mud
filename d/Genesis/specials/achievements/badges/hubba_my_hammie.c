/* 
 * Christmas Event 2013 Badge - Hubba My Hammie
 *
...
create_badge()
{
    set_badge_name("Hubba My Hammie");
    set_badge_description("Sparkle is the Donut's Hub. So what better place to get some Christmas "
                          + "ham and chips than to look here? Keep it up and "
...
hook_notify_player_earned_badge(object player)
{
    ...
    player->catch_tell("You've managed to collect something in the Hub of Genesis! For that, you \n"
...
does_event_apply(object player, string event_name, mixed data)
{
    ...
    if (wildmatch("/d/Sparkle/*", room_file)
        || wildmatch("/d/Genesis/*", room_file))
    {
        return 1;
    }
