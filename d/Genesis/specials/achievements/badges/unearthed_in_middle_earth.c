/* 
 * Christmas Event 2013 Badge - Unearthed in Middle Earth
 *
...
create_badge()
{
    set_badge_name("Unearthed In Middle Earth");
    set_badge_description("Buried in the depth of Middle Earth are Ham and Chips. And you've "
                          + "managed to unearth them! Keep it up and maybe you can become "
...
hook_notify_player_earned_badge(object player)
{
    ...
    player->catch_tell("You've managed to collect something in the Middle Earth domains! For that, you \n"
...
does_event_apply(object player, string event_name, mixed data)
{
    ...
    if (wildmatch("/d/Gondor/*", room_file)
        || wildmatch("/d/Shire/*", room_file))
    {
        return 1;
    }
