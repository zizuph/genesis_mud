/* 
 * Christmas Event 2013 Badge - Scoured Earthsea
 *
...
create_badge()
{
    set_badge_name("Scoured Earthsea");
    set_badge_description("Scouring the Earthsea domain has been productive! Forget the "
                          + "Kargs and find some Christmas ham and chips. Keep it up and "
...
hook_notify_player_earned_badge(object player)
{
    ...
    player->catch_tell("You've managed to collect something in the Earthsea domain! For that, you \n"
...
does_event_apply(object player, string event_name, mixed data)
{
    ...
    if (wildmatch("/d/Earthsea/*", room_file))
    {
        return 1;
    }
