...
{
    if (rolled)
        return "The cozy red and yellow flannel sleeping bag is rolled "
...
do_roll (string str)
{
    if (!strlen(str) ||
        !parse_command(str, ({ }),
             "[up] [the] [sleeping] 'bag' / 'sleeping-bag'"))
    {
...
do_unroll (string str)
{
    ...
    {
        notify_fail ("Unroll what? The sleeping bag?\n");
        return 0;
    }
    if  (rolled !=1) 
    {
        notify_fail ("The sleeping bag is not rolled up.\n");
        return 0;
    }
    rolled = 0; 
    {
