...
create_room()
{
    ...
    add_exit("/d/Genesis/clubs/rpg/ac_inner", "north", "@@enter_test", 1, "@@can_see_exit");
    add_exit("/d/Genesis/clubs/rpg/th_stage", "south");
...
enter_test()
{
	...
	{
	    tell_room(this_object(), QCTNAME(TP) + " hesitantly feels at the north wall, but " + TP->query_possessive() + " hand just passes through without resistance, and " + TP->query_pronoun() + " stumbles through head over heels.\n", TP);
	    return 0;
	}
    ...
    tell_room(this_object(), QCTNAME(TP) + " makes a gesture at the north wall, which then becomes transparent for a second or so, allowing " + TP->query_objective() + " to pass through.\n", TP);
    return 0;
}
