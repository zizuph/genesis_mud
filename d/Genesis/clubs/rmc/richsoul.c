...
#define FILTHILY if((RICHROOM)->query_purchased(TP)<2000) return "No such RMC command.\n"
#define BIGSHOT  if((RICHROOM)->query_purchased(TP)<4000) return "No such RMC command.\n"
...
{
  write("You find your bankbook.\nYou have "+summa+" platinum coins in the bank.\n");
...
radvice(string s)
{
  ...
  if (!s || s == "all")
  {
    write("You advice everyone that the best way of gaining a lot of money is "+
          "to not spend them.\n");
    say(QCTNAME(TP)+" advices you that the best way of gaining a lot of money "+
  ...
  if (s == "me")  
  {
    write("Advice yourself?\n");
    say(QCTNAME(TP)+" advices "+HIM_HER(TP)+"self something. What a strange "+
        "person!\n");
    return 0;
  }
...
rcompare(string s)
{
    ...
    acc1 = ((total_account(TP) + (RICHROOM)->query_purchased(TP)) / 100) * 100;
    ...
    TP->catch_msg("You have approx "+acc1+" plats and "+QTNAME(obs[0])+" has "+
...
rconsider(string s)
{
    ...
    if (!sizeof(obs) || sizeof(obs) > 1)  return "Consider who?\n";
    ...
    acc1 = total_account(TP) + (RICHROOM)->query_purchased(TP);
...
rget()
{
  ...
  {
      write("You look into your "+query_purse()+" and quickly pick up it's contents.\n");
  ...
  {
      write("You look into your "+query_purse()+" and find out that it's empty!\n");
      say(QCTNAME(TP)+" looks into "+HIS_HER(TP)+" "+query_purse()+
...
richlist()
{
    ...
    for (i = 0; i < sizeof(players); i++)
        if (present("rich_club_obj",players[i]))
            list += ({ ({ players[i], (RICHROOM)->query_level(players[i]),
                          (RICHROOM)->query_valuation(players[i]) }) });

    if (sizeof(list))
    {
        no = sizeof(list);
        write("| There are "+sizeof(list)+" active members right now.   \t        |\n");
        ...
        for (i = 150; i > 0; i--)
        {
			...
			str = "| (You) ";
                    ...
                    else if ((RICHROOM)->query_president() == p->query_real_name())
                        str = "| (Prs) ";
                    else if ((RICHROOM)->query_vice_president() == p->query_real_name())
                        str = "| (VPr) ";
		    else if (p == TP)
			str = "| (You) ";
                    else str = "| (---) "; 
                    ...
                    if (lvl && (RICHROOM)->query_humble(p))
                        str += "Humble Member\t\t\t|";
                    else
                    {
                        str += (RICHROOM)->query_num_title(lvl, p) + "|";
                    }
/*                    if (strlen(str) < 33)
                        str+="\t\t|";
...
rpos()
{
    ...
    write((RICHROOM)->query_pos(TP->query_real_name(), 1));
    return 0;
}
...
rsetlevel(string str)
{
    ...
    if (TP->query_real_name() == "emil")
	return "(RMC) är du riktigt säker, Håkan? :)\n";
...
rtitle()
{
    ...
    if (v > 2000)
        s = "You'll have to wait to a new title arrives. You have the highest already.\n";
    else if (v > 1900)
...
rwonder(string str)
{
    ...
    ma = (RICHROOM)->query_valuation(TP);
    oa = (RICHROOM)->query_valuation(find_player(str));

    if (oa <= 0)
    {
	if ((RICHROOM)->query_acc(str) < 0)
