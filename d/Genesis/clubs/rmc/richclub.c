...
change_account(object ob)
{
    ...
    plats = total_account(ob) + RMC_PURCHASE(member);
...
query_member_list()
{
    ...
    for (i = 0; i < sizeof(rdata); i++)
    {
        ...
        if (RMC_PAID(i) == 8)
            em++;
        if (RMC_PAID(i) > 15 && RMC_PAID(i) < 30)
            if(RMC_PURCHASE(i) > 4000) bs++;
            else if (RMC_PURCHASE(i) > 2000) fr++;
            else om++;
        if (RMC_PAID(i) > 29)
            wz++;
        if (RMC_NEWS(i))
    ...
    for (i = 0; i < sizeof(players); i++)
    {
        ...
        if (p >= 0)
        {
            ...
            t += " "+RMC_PAID(p)+"\t  "+RMC_ACCOUNT(p)+"\t "+RMC_HUMBLE(p)+"   "+RMC_VISIBLE(p)+
                "   "+RMC_NEWS(p)+"   "+
    ...
    s = "\n+---------------------------------------------------------------+\n"+
        ...
        capitalize(RMC_MEMBER(president))+"\t|\n"+
        "|  - Poor Members     : "+pm+"\t| Vice President:  "+
        capitalize(RMC_MEMBER(vicepres))+"\t|\n"+
        "|  - Average Members  : "+am+"\t+-------------------------------+\n"+
...
update_president()
{
    ...
    for (i = 0; i < sizeof(rdata); i++)
    {
        if (RMC_PAID(i) == 16)
        {
            w = (RMC_PURCHASE(i)/175) +
		((RMC_ACCOUNT(i) - RMC_PURCHASE(i))/250) + 1;
            if (w > c)
            {
                c = w;
                p = i;
...
query_mypos(object ob)
{
    ...
    if (no != -1)
    {
        ma = query_valuation(ob);
        for (i = 0; i < sizeof(rdata); i++)
        {
            p = RMC_PAID(i);
            if (no != i && p > 15 && p < 20)
            {
                mo = (RMC_PURCHASE(i)/175) +
		    ((RMC_ACCOUNT(i)-RMC_PURCHASE(i))/250) + 1;
                if (ma < mo)
                {
                    pos++;
...
query_pos(string s)
{
    ...
    v = (RMC_PURCHASE(no)/175) + ((RMC_ACCOUNT(no)-RMC_PURCHASE(no))/250) + 1;
...
pursecheck()
{
	...
	write("\n[RMC] : ");
	write(pursepay);
	write(" plats has been paid on broken purses.\n");

	data = read_file(RICHCLUB + "purse.log");
	write(data);
