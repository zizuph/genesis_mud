...
create_room()
{
  ...
  add_exit(RICHCLUB+"richprivate3","up","@@check");
  add_exit(RICHCLUB+"richprivate1","down",0);
...
check()
{
  ...
  if (lv > 8 || TP->query_wiz_level()>0) return 0;
...
int start(string s)
{
   TP->set_temp_start_location(RICHCLUB+"richprivate2");
   write("Ok, you will be starting here. But notify that it's only temporary. "+
     "You have to type 'start here' every time you log in. This inconvinience "+
     "has occured since the club hasn't gained the status as default starting "+
