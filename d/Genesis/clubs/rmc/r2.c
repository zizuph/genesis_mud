...
construct()
{
	...
	member_list[rdata[i][0]] = rdata[i];
    save_object(RICHCLUB + "test");
}
int
...
query_level(object ob)
{
    ...
    if (member_list[person] != 0)
	return RMC_PAID(member_list[person]);
    else
	return 0;
}
...
{
    return (200 * query_level(ob)) - 100;
}

int
inv_account(object ob)
{
    return TOTAL_MONEY(ob) / 1728;
}

int
total_account(object ob)
{
    return level_account(ob) + inv_account(ob) + bank_account(ob);
}

void
change_account(object ob)
{
    string person = ob->query_real_name();
    mixed *acc = member_list[person];

    if (acc)
    {
	plats = total_account(ob) + RMC_PURCHASE(acc);

	RMC_ACCOUNT(acc) = plats;
	member_list[person] = acc;
...
void
set_purse_hole(object ob)
{
    pursehole += ([ ob->query_real_name() : 1]);
    save_object(RICH_OBJ);
}
...
{
    object p = present("rich_club_obj", ob);
    string n = ob->query_real_name();
	...
	if (pursehole[n])
	{
	    if (can_pay( 17280 , ob) == 0) /*check if the player has enough money */
	    {
		ob->catch_msg("You don't have the money to repair it! It costs "+
			      "10 platinum coins.\n");
		return 1;
	    }

	    pay( 17280, ob, "", 0, 0, "", 1); /* let the player pay */
	    ...
	    write("Your "+p->short()+" is "+
		  "repaired, take more care of it now.\n");

	    save_object(RICH_OBJ);
	    return 1;
	}
	write("There's nothing wrong with your "+p->short()+"!\n");
	return 1;
    }
...
int
query_purse_hole(object ob)
{
    int a;

    a = pursehole[ob->query_real_name()];
...
{
    return RMC_SUSPEND(member_list[ob->query_real_name()]);
}
...
init()
{
    ...
    add_action("pursecheck", "pursecheck");
...
check()
{
    ...
    if (member != -1 || TP->query_wiz_level()>0)
	return 0;
...
{
    return RMC_HUMBLE(member_list[ob->query_real_name()]);
}
...
{
    RMC_NEWS(member_list[ob->query_real_name()]) = 0;
}

int
query_news_status(object ob)
{
    return RMC_NEWS(member_list[ob->query_real_name()]);
}
...
remove_member(object ob)
{
    ...
    m_delete(member_list, ob->query_real_name());
...
query_member_list()
{
    ...
    playing_members = filter(users(), &query_member());
    for (i = 0; i < sizeof(playing_members); i++)
    {
	t += sprintf("| %-20s %8i %8i %3i %3i %3i %3i %3i %3i", 
		     capitalize(RMC_MEMBER(playing_members[i])),
		     RMC_PAID(playing_members[i]),
		     RMC_ACCOUNT(playing_members[i]),
		     RMC_HUMBLE(playing_members[i]),
		     RMC_VISIBLE(playing_members[i]),
		     RMC_NEWS(playing_members[i]),
		     RMC_SUSPEND(playing_members[i]),
		     RMC_NOTE(playing_members[i]),
		     RMC_PURCHASE(playing_members[i])
	    );
    }
    t += "+---------------------------------------------------------------+\n\n";

    s = "\n+---------------------------------------------------------------+\n"+
          "| $                   RMC Member Statistics NEW!!!!!          $ |\n"+
        "+-------------------------------+-------------------------------+\n"+
        "| Total no of members : "+m_sizeof(member_list)+"\t| President     :  "+
...
{
            return RMC_ACCOUNT(member_list[str]) + 
		RMC_PURCHASE(member_list[str]);
}
...
query_valuation(object ob)
{
    ...
    {
        pr = RMC_PURCHASE(no);
        ac = RMC_ACCOUNT(no) - RMC_PURCHASE(no);
...
query_pos(string s, int own = 0)
{
    ...
    for (i = 0; i < sizeof(arr); i++)
    {
	if (RMC_PAID(arr[i])>15 && RMC_PAID(arr[i])<20)
	{
	    if (worth < ((RMC_PURCHASE(arr[i]) / 175) +
		     ((RMC_ACCOUNT(arr[i]) - RMC_PURCHASE(arr[i])) / 250) + 1))
		pos++;
	}
    }
...
pursecheck()
{
	...
	write(read_file(RICHCLUB + "purse.log"));
