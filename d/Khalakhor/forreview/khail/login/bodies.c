...
possess_body(string str)
{
    ...
    {
        write("You direct your conscience into your new body, and take " +
            "over it's awareness, and step down from the altar it " +
            "rested upon.\n");
        say(capitalize(bodies[0]->query_real_name()) + " stirs slightly, " +
            "and steps down from the altar it rested upon.\n");
        set_alarm(10.0, 0.0, &replace_body(bodies[0]->query_race_name(),
  ...
  /* Things to do if player's here because he died. */
    if (tp->query_ghost() & GP_DEAD)
    {
      ...
      /* is neutral somehow), switch bodies and send them on their way. */
        if (tp->query_race() == ob->query_race_name() &&
            (tp->query_gender() == ob->query_gender() ||
             tp->query_gender() == G_NEUTER))
        {
          /* A double-check to ensure old players have 2 adjectives. */
          /* If this player doesn't, send him/her to get some. */
            ...
            {
                tell_object(tp, "You seem less than fully featured, " +
                    "you must go to the Well of Souls and select " +
                    "those adjectives which describe you.\n");
      ...
      /* If the player did change race or gender */
        else
        {
         ...
         /* variable as requiring mangling (height/width) and features. */
            tp->set_race(ob->query_race_name());
