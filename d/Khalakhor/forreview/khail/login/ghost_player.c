 ...
 *           must be choosen.
 *
 * Revision: July 26/96 - Khail.
 *    Basically just removed the virtual coins as no longer necessary
 *    and documented existing code properly.
 * Revision: Feb 23/97 - Khail
 *    Converted the existing code to reverse the roles of query_race()
 *    and query_race_name() in the ghost players to make it more secure
 *    against shadows that used them.
 */

#pragma strict_types                    // Force proper type casting
#pragma save_binary                     // Save a binary copy
...
enter_new_player(string name, string pass)
{
   ...
   */
	...
	if (wildmatch("*jr", name))
	{
	    ...
	    time_out_alarm = set_alarm(TIMEOUT, 0.0, time_out);
	    return;
	}
...
check_identity1(string id)
{
  ...
  /* Check with /secure/master.c if the name to test has any wizard rank. */
...
{	
  ...
  /* Turn player's race name into 'ghost' if need be. */
    if (query_race_name() != "ghost")
    {
        set_race_name("ghost");
        save_me(0);
    }
  ...
  /* playerfile that caused it. We will, however, log it. */
    else if (query_ghost() == 0)
    {
        log_file("GHOST_START_LOG",
            ...
            file_name(this_object()) + " with a ghost_var of 0, " +
            "on " + ctime(time()) + ".\n");
...
ghost_ready()
{
  ...
  /* Move them to their racial start location, and save the new data. */
    set_default_start_location(RACESTART[this_object()->query_race()]);
    save_me(0);
...
reincarnate_me()
{
  ...
  /* Copy the player's query_race() name over to the ghost body to */
  /* serve as its query_race(). */
    gh->set_race(pl->query_race());
...
/*
 * RACE NAMING ROUTINES
 */

/*
 * Function name: set_race
 * Description  : Sets the 'true_race_name' variable so the ghost's
 *                true race will always be that of the player's usual
 *                player object as defined by query_race() in the player
 *                objects. If 'race' is not a valid race name, as
 *                contained in the RACEMAP mapping in login.h, use
 *                'human' as a default rae.
 * Arguments    : race - A string describing the race.
 * Returns      : n/a
 */
nomask void
set_race(string race)
{
  /* This function can only be called from ghost_player.c */
  /* or bodies.c in this directory. */
    if ((MASTER_OB(previous_object()) == MASTER) ||
        (MASTER_OB(previous_object()) == LOGIN_BODIES))
    {
      /* If the specified race isnt' valid for some reason, */
      /* default to human. */
        if (!strlen(RACEMAP[race]))
            true_race_name = "human";
        else
            true_race_name = race;
        save_me(0);
    }
 ...
 * Description  : Returns the true race name of this ghost, as obtained
 *                from query_race() in the normal body of this player.
 * Arguments    : n/a
 * Returns      : The true race name as a string.
 */
nomask string
query_race()
{
    return true_race_name;
}

/*
 * Function name: set_race_name (MASK)
 * Description  : Only masked here to make it static so it cannot
 *                be called externally.
 * Arguments    : race - New race name.
 * Returns      : n/a
 */
nomask static void
...
/*
 * Function name: query_race_name (NAME)
 * Description  : Returns the race_name of this player. Normally,
 *                this would be their base race (as defined by
 *                query_race()), or their sub-race (reset by a
 *                guild shadow). In ghosts, however, the race_name
 *                shall always be 'ghost'.
 *                During the changeover in race naming routines,
 *                however, we'll mask query_race_name() to always
 *                return the base race, in order to provide guild
 *                coders time to adjust their code.
 * Arguments    : n/a
 * Returns      : A race name as a string. Usually just 'ghost'.
 */
nomask string
query_race_name()
{
    object caller;
#ifdef NEW_NAME_IMPLEMENTING
    caller = previous_object();
    if (function_exists("query_guild_not_allow_join_guild",caller) ==
        "/std/guild/guild_base")
    {
        log_file("GUILD_QUERY_RACE_NAME", file_name(caller) + "\n");
    }
    return true_race_name;
#endif
...
/*
 * The following routines are masked to make them non-static so we
 * can call them externally during the creation sequence.
 */
