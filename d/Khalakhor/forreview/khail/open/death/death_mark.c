...
tell_the_others()
{
    ...
    others = FILTER_LIVE(all_inventory(environment(dead)) - ({dead}));
    if (sizeof(others))
        next = others[random(sizeof(others))];
    wizarr = filter(others, &->query_wiz_level());

    hour = random(12) + 1;
    minute = random(49) + 10;

    time = hour + ":" + minute + " TOMORROW " +
        ((hour == 12 || hour < 5) ? ({"NIGHT", "AFTERNOON"})[random(2)]:
        ({"MORNING", "EVENING"})[random(2)]);

    tell_room(environment(dead), "Death arrives and ushers poor " +
        QTNAME(dead) + " off to the afterlife.\n", ({dead}));
  
    if (next)
    {
        tell_object(next, "Death glances around, points at you, and " +
            "says: I'LL SEE YOU AT " + time + "!\n");
        tell_room(environment(next), "Death glances around, points " +
            "at " + QTNAME(next) + " and says: I'LL SEE YOU AT " + time + 
            "!\n", ({dead, next}));
    }
    if (sizeof(wizarr))
        tell_room(environment(dead), "Death winks at you and says: " +
