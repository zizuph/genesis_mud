...
manip_relocate_to(object item_o, object to)
{
    ...
    if (item_o->query_prop(HEAP_I_IS) && query_verb() == "hide")
...
manip_relocate_from(object item_o)
{
	...
	say(QCNAME(this_player()) + " tries" + tmp + "the " + QSHORT(env) +
	    ".\n", ({ env, this_player() }) );
	this_player()->catch_msg("You fail" + tmp + "the " + QSHORT(env) +
	    ".\n");
...
inventory(string str)
{
    ...
    {
	str = COMPOSITE_DEAD(obarr);
	write(break_string((tp == this_player() ? "You are" : QCNAME(tp) + " is") +
			   " carrying " + str + ".", 74, 2) + "\n");
    }
    else
    {
	write("  " + (tp == this_player() ? "You are" : QCNAME(tp) + " is") +
 ...
 *
   ...
   look at 'object' carried by 'living'
    ...
    look at 'add_item' on 'object'
             Gives a specific add_item on a single object.
 */
int 
look(string str)
{
...
/* BEGINNING OF OBJECT ADD_ITEM EXA HACK */
  /* Look at an add_item on a specific object. */
    if (parse_command(str, all_inventory(this_player()) +
        all_inventory(environment(this_player())),
        "'at' / 'prp_examine' %s 'on' %i", item, marr))
    {
        if (pointerp(marr))
        {
            if (sizeof(marr) < 2)
            {
                notify_fail("Examine " + str + " on what?\n");
                return 0;
            }
            if (marr[0] == 0)
            {
                notify_fail("Be specific, what item do you " +
                    "want to look at " + str + " on?\n");
                return 0;
            }
            if (marr[0] < 0)
            {
                obai = (object)marr[-marr[0]];
            }
            else if (marr[0] > 0)
            {
                obai = (object)marr[marr[0]];
            }
            if (!CAN_SEE(this_player(), obai))
            {
                notify_fail("You see no such object.\n");
                return 0;
            }
            if (!obai->item_id(item))
            {
                notify_fail("You see no " + item + " on the " +
                    obai->short() + "\n");
                return 0;
            }
            write(obai->long(item));
            return 1;
        }
    }

/* END OF EXAMINABLE ADD_ITEM ON OBJECT HACK */
...
peek(string str)
{
	...
	write("Ooops! " + p[0]->query_the_name(this_player()) +
