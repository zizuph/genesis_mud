...
query_recover()
{
    return MASTER + ":DON" + implode(known_by,"$$") +
        "##" + is_magic + "HAM" +
        query_wep_recover() + query_keep_recover();
}
...
sword_wield(string str)
{
    ...
    if (parse_command(str, TP, %s 'in' %w 'hand' / 'hands'),
        swield, hand))
    {
        obarr = FIND_STR_IN_OBJECT(swield,TP);
      /* Produce a normal fail if nothing matching 'swield' */
      /* is in the player's inventory. */
        if (!sizeof(obarr))
            return wield_me(str);

      /* Produce a normal fail if more than one item */
      /* matching 'swield' is in the player's inventory. */
        if (sizeof(obarr) > 1)
            return wield_me(str);

      /* If the player's trying to wield something other */
      /* than this sword, produce a normal fail. */
        if (obarr[0] != TO)
            return wield_me(str);

      /* Ok, it appears the player is trying to wield this */
      /* particular sword, now figure out how the player */
      /* is trying to wield. */
        if (hand == "left")
        {
            set_hands(W_LEFT);
        ...
        if (wielder)
        {
            TP->catch_msg("You change your grip on your " +
                short() + " and wield it " + msg + ".\n");
            say(QCTNAME(TP) + " changes " + HIS_HER(TP) +
                " grip on " + HIS_HER(TP) + " " + short() +
                " and wields it " + msg + ".\n");
            wielder->update_weapon(TO);
            return 1;
        }
