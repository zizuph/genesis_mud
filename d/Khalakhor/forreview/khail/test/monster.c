...
#define SCROLLING_LOG(file, entry)   ((file_size(file) > 20000) ? \
 ...
 */
public void reset_khalakhor_monster()
{
}
 ...
 */
public void create_khalakhor_monster() {}
...
/*
 * Function name: team_up_check
 * Description  : Checks to see if the npc should join a team
 *                leader.
 * Arguments    : Object who - object pointer to the npc we're
 *                interested in joining.
 */
public void
team_up_check(object who)
{
  /* Here, we'll add a check for team followers to join */
  /* a team leader that arrives in their environment, */
  /* if they can. */

  /* First make sure 'who' is still in the room */
    if (!present(environment(TO),who))
        return;
  ...
  /* Don't join if our team names are dissimilar */
    if ((TO->query_team_name()) != (who->query_team_name()))
        return;
        
  /* If we've got a team name, and the potential leader */
  /* doesn't, don't join. */
//    if (!who->query_team_name() ||
//        !strlen(who->query_real_name()))
//        
//        return;
 ...
 *                same environment as this object.
 */   
public void
init_living()
{
    ...
    set_alarm(0.5, 0.0, &team_up_check(TP));
}
...
catch_question(string question)
{
    ...
    if (TO->query_log_ask())
    {
    SCROLLING_LOG("/d/Khalakhor/log/ask_log",
