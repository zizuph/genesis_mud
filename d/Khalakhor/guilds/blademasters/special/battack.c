...
enter_env(object to, object from)
{
    ...
    if (get_alarm(prepare_alarm_id))
    {
        tell_object(shadow_who, "Your movement has prevented " +
            "completion of your attack form!\n");
...
{
    attack_state = 0;
    tell_object(shadow_who, "You feel ready to use a special " +
        "attack form again.\n");
}
...
can_still_attack(object *weapons, object target, int style)
{
    ...
    {
        tell_object(shadow_who, "You are too tired to use a " +
            "special attack form!\n");
        attack_state = 0;
        return 1;
    }
    ...
    {
        tell_object(shadow_who, "It appears your quarry has " +
    ...
    {
        tell_object(shadow_who, "No matter what your skill " +
            "with a blade, you cannot harm a ghost!\n");
        attack_state = 0;
        return 1;
    }
...
decap_no_corpse(object vic, mixed weps)
{
    ...
    shadow_who->catch_msg("As you slash your " +
    ...
    tell_room(environment(shadow_who), QCTNAME(shadow_who) +
        " raises " + shadow_who->query_possessive() + " " +
        weps[0]->short() + " to administer the final blow, " +
        "but " + QTNAME(vic) + " collapses away before " +
        shadow_who->query_pronoun() + " can swing!\n",
        ({shadow_who, vic}));
    vic->do_die(shadow_who);
}
...
decap_corpse(object vic, mixed weps)
{
    ...
    {
        shadow_who->catch_tell("Considering the absence of " +
            "a head upon " +  vic->query_the_name(shadow_who) +
    ...
    shadow_who->catch_msg("As " + QTNAME(vic) + " hovers " +
    ...
    tell_room(here,  "As " + QTNAME(vic) +
        " hovers on the verge of death, " + QTNAME(shadow_who) +
        " strikes forward with " + shadow_who->query_possessive() +
    ...
    corpse = present(corpsename,environment(shadow_who));
    ...
    int form, mixed aid)
{
    ...
    if (target->query_hp() < 1)
    {
        ...
        if (shadow_who->query_prop(BLADEMASTER_NO_DECAPITATE))
        {
        ...
        else
        {
            if (target->query_prop(LIVE_I_NO_CORPSE))
                decap_no_corpse(target, wep);
            else
                decap_corpse(target, wep);
        }
    }
