...
#define BATTACK_LOG            (SPECIAL + "battack_damage")
...
public void             hook_targets_gone(object * targets);
...
bm_get_style()
{
    ...
    {
        if (player->query_stat(SS_STR) >= FORM_STR_MIN)
            form = ({3, obarr[0]});
        else
            return 3;
    }
    ...
    {
        if (player->query_stat(SS_DEX) >= FORM_DEX_MIN)
            form = ({1, obarr[0]});
        else
            return 4;
    }
...
validate_battack_requirements(object player)
{
    ...
    if (intp(form = bm_get_style()))
    {
        if (form == 1)
        {
            player->catch_tell("You have no swords to attack with!\n");
        }
        else if (form == 2)
        {
            player->catch_tell("You have neither the skill to attack with "
              + "the two one-handed sword forms, nor the dexterity " 
              + "to attack with the single one-handed sword forms!\n");
        }
        else if (form == 3)
        {
            player->catch_tell("You do not have the strength to attack "
              + "with the two-handed weapon forms!\n");
        }
        else if (form == 4)
        {
            player->catch_tell("You do not have the dexterity to attack "
              + "with the single one-handed weapon forms!\n");
        }
        else if (form == 5)
        {
            player->catch_tell("You do not possess the skill to attack "
...
check_valid_action(object player, mixed * targets, string arg, int execute)
{
    ...
    if (!bIsAttackingEnemy)
    {
        player->catch_tell("You can only use battack on opponents you "
                     ...
                     string arg, int pretest)
{
    ...
    if (intp(form = bm_get_style()))
    {
        // Could not get a correct form, so do not have the
        // required items.
        return 0;
    }

    // weapons are listed in the form variable array
    return form[1..];    
}
