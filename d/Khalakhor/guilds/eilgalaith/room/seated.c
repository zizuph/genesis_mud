 ...
 * - Will allow more than one bathing or seating in a room         - *
 * -                                                               - *
...
#define SDESC \
"Several cushions are resting on the floor in front of "+ \
"the fireplace. The fabrics are dark in color with emerald, "+ \
"crimson, and gold threads woven in intricate patterns. There "+ \
"are also a few soft cushioned chairs scattered around as well.\n"

mixed sdef = ({
({
({"cushion","on cushion", "in cushion"}),
"You slip down onto a cushion and sit before the fire.\n",
" slips down onto a cushion and sits before the fire.\n",
"You arise from your soft cushion.\n",
" arises from a soft cushion.\n",
"You are sitting comfortably on a cushion before the fire.\n",
"You slides off a cushion and sits before the fire.\n",
" sitting comfortably on a cushion before the fire",
" sitting comfortably on a cushion before the fire.\n",
}),

({
({"chair","in chair", "on chair"}),
"You sit down on one of the soft cushioned chairs.\n",
" sits on one of the soft cushioned chairs.\n",
"You arise from your soft cushioned chair.\n",
" arises from one of the soft cushioned chair.\n",
"You are seated comfortably on one of the soft cushioned chairs.\n",
"You arise from your soft cushioned chair and leave the room.\n",
" sitting comfortably on one of the soft cushioned chairs",
" sitting comfortably on one of the soft cushioned chairs.\n",
})
...
leave_inv(object ob, object to)
{
	...
	if (ob && !present(ob) && seated[ob])
	{
		...
		if (seated[ob][0] == 2 && (to = seated[ob][1]))
		{
			...
			to->catch_tell("You are dumped unceremoniously "+
			"onto the floor.\n");
			tell_room(this_object(), QCTNAME(to) + " is "+
...
do_stand(string arg)
{
	...
	{
		this_player()->catch_msg("You climb off of "+
		"" + QTPNAME(ob) + " lap.\n");
		ob->catch_tell(this_player()->query_The_name(ob) + " "+
		"climbs off of your lap.\n");
		say(QCTNAME(this_player()) + " climbs off of "+
		"" + QTPNAME(ob) + " lap.\n", ({ this_player(), ob }));
	...
	else
	{
		...
		if (tmp[0] == 2 && objectp(ob = tmp[1]))
		{
			...
			"onto the floor.\n");
			tell_room(this_object(), QCTNAME(ob) + " is "+
			"dumped unceremoniously onto the floor.\n", ob);
			seated = m_delete(seated, ob);
		}
	}
		...
		while (++i < j)
		{
			...
			if (seated[obs[i]][0] == 2)
			{
				...
				else
				{
					...
					ob->query_the_name(this_player())) + " snuggled up on "+
