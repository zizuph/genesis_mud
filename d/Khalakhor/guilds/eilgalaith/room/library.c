...
library_exit_block()
{
	...
	{
		/* A scroll was found.  Give a warning and return 1 
		 *to block the exit */
...
create_guild_room()
{
	set_short("Hall of scrolls");
	set_long("   This is the hall of scrolls where you will find "+
	"many shelves that have scrolls placed neatly on them. "+
	"Eil-Galaith has placed them here for your information and "+
	"continued study of the Khalakhor elven nation. There are a "+
	"few wooden tables and chairs situated around them invitingly. "+
	"There are tapestries on the walls. A sign has been placed "+
	"here also that will assist you in your knowledge. ");
	add_item(({"shelf", "shelves"}),
	"The shelves are made from marble and are attached to the "+
	"walls.\n");
	add_item(({"table", "tables"}),
	"The tables are made from lightwood and they have a few "+
	"chairs around them.\n");
	add_item(({"chair", "chairs"}),
	"The chairs are made of lightwood and are scattered invitingly "+
	"around the tables.\n");
	...
	"crafted with elven hands depicting elves in various "+
	"vocations.\n");
	 ...
	 * leaves the library with a scroll */
	 ...
	 add_exit("entrance", "east", library_exit_block);
	 add_fail("auto", "You walked into a wall!\n"+
	 "Ouch! That's gotta hurt!\n");
	 
	 add_prop(ROOM_I_INSIDE, 1);
	 
	 /* Indicate that we want players to have to "borrow" a scroll 
	  * in order to read it. */
	  ...
	  * are saved */
	  ...
	  /* Add shelves to the library so that scrolls can be 
	   * classified under different categories.  Note that for each
	   * shelf you must add a subdirectory with the same name under 
	   * the base directory specified with set_scroll_directory(). */
	  add_scroll_shelf(({ "general", "history", "lineage" }));
	  
	  create_library();
	  
	  /* Add a sign that gives instructions on how to use the 
	   * library */
...
{
	/* We'll allow any member of the racial guild  to write 
	 * scrolls */
