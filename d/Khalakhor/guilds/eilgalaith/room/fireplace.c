...
stoke_fire(string str)
{
	...
	{
		write("You take out a poker and stoke up the fire in the "+
		...
		say(QCTNAME(TP)+" returns the fireplace poker to its "+
		"holder.\n");
		return 1;
	}
	else
...
extinguish_fire(string str)
{
	...
	{
		write("You try to extinguish the fire in the "+
		"fireplace but fail.\n");
		say(QCTNAME(TP)+" tries to extinguish the fire "+
		"in the fireplace but fails.\n");
		return 1;
	}
	else
	{
		notify_fail("Extinguish what? Extinguish the "+
		"fire perhaps?\n");
		return 0;
	}
}
