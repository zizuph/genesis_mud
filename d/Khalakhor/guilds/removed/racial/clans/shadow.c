...
{
    string STARTLOC;
    STARTLOC=SERVER->query_clan(shadow_who->query_real_name());

    switch(STARTLOC)
    {
    case "MacDunn":
        return (MACDUNN + "start");
        break;
    case  "Lohrayn":
        return (LOHRAYN + "start");
        break;
    case "MacTabor":
        return (MACTABOR + "start");
        break;
    case "MacFadden":
        return (MACFADDEN + "start");
        break;
    case "MacVay":
        return (MACVAY + "start");
        break;
    case "MacFaolain":
        return (MACFAOLAIN + "start");
        break;
    }
    return "";
}
...
leave_guild_race()
{
   ...
   if (IS_CLANCHIEF(shadow_who))
    ...
    while(ob = present(OBJECT_ID, shadow_who))
      ...
      if (((shadow_who->query_default_start_location()) == (shadow_who->query_clan_startloc())))
