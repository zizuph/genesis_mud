//--------------------------------------------------------------------------
// Tine Spell - cause the guild staff to glow with fire light
//				staff then provides lamp light, and has increased hit/pen
//				it is also prepared (after further anointing) to be used
//				to cast STUA
// prep:		must have guild staff; 
//				staff must be anointed with blessed beatha oil
//--------------------------------------------------------------------------
#include "defs.h"
...
{
   set_spell_name("tine");  
   set_spell_desc("Enflame thy staff");
   set_spell_time(5);
   set_spell_task(TASK_ROUTINE);			//default TASK_ROUTINE
   set_spell_element(SS_ELEMENT_FIRE,  30); //element, required level
   set_spell_form(SS_FORM_ENCHANTMENT, 30), //form,    required level
   set_spell_mana(300);						//amt of mana used
...
resolve_spell(object caster, object *targets, int *resist, int result)
{
	...
	//calculate minutes staff will remain lit, and light it, max 5mins=300sec
	seconds = ftoi(((300.0 * (ints+wis+fire+ench+spcraft))/500.0));
	staff->illuminate(2,seconds);

	//reduce harmony
	harmony -= SR_SPELL_HARMONY_TINE;
