...
mixed sdef = ({
({
({"blanket","on blanket", "grape leaves", "upon grape leaves", "upon blanket", "soft grape leaves"}),
"You sink down onto a soft blanket of grape leaves.\n",
" sinks onto a soft blanket of grape leaves.\n",
"You arise from a soft blanket of grape leaves.\n",
" arises from a soft blanket of grape leaves.\n",
"You are laying comfortably on a soft blanket of grape leaves.\n",
"You arise from a soft blanket of grape leaves and leave the area.\n",
" laying comfortably on a soft blanket of grape leaves",
" laying comfortably on a soft blanket of grape leaves.\n",
}),

({
({"grass", "down", "on grass", "green grass", "on green grass", "upon grass", "upon green grass"}),
"You lay down onto the vibrant green grass.\n",
" lays onto the vibrant green grass.\n",
"You arise from the vibrant green grass.\n",
" arises from the vibrant green grass.\n",
"You are laying comfortably stretched out on a blanket of vibrant green grass.\n",
"You arise once you are ready and leave the area.\n",
" laying comfortably stretched out on a blanket of vibrant green grass",
" laying comfortably stretched out on a blanket of vibrant green grass.\n",
})
...
leave_inv(object ob, object to)
{
	...
	if (lay[ob][0] == 2 && (to = lay[ob][1]))
	{
	    ...
	    to->catch_tell("You reluctantly stand up.\n");
	    tell_room(this_object(), QCTNAME(to) + " reluctantly stand up.\n", to);
	}
...
do_lay(string str)
{
	...
	if (sizeof(obs) > 1)
	{
	    return notify_fail("You may only lay snuggled closely to one "+
	    "body at a time.\n");
	}
	...
	this_player()->catch_msg(
	    "You snuggle closely to " + QTPNAME(obs[0]) + " body.\n");
	obs[0]->catch_tell(this_player()->query_The_name(obs[0]) +
	  " snuggles close to your body.\n");
	say(QCTNAME(this_player()) + " snuggles close to " + QTPNAME(obs[0]) +
	  " body.\n", ({ obs[0], this_player() }));
    }
    else
    {
...
do_stand(string arg)
{
    ...
    {
	this_player()->catch_msg(
	    "You stop snuggling " + QTPNAME(ob) + " body.\n");
	ob->catch_tell(this_player()->query_The_name(ob) + " stops "+
	  "snuggling you close.\n");
	say(QCTNAME(this_player()) + " stops snuggling " + QTPNAME(ob) +
	  " body.\n", ({ this_player(), ob }));
	...
	if (tmp[0] == 2 && objectp(ob = tmp[1]))
	{
	    ...
	    ob->catch_tell("You reluctantly stand up.\n");
	    tell_room(this_object(), QCTNAME(ob) + " reluctantly stands up.\n", ob);
...
lay_desc()
{
	...
	if (lay[obs[i]][0] == 2)
	{
	    ...
	    {
		who += ({ "yourself with " +
		    ob->query_the_name(this_player()) +	" snuggled "+
...
show_subloc(string subloc, object on, object for_ob)
{
	...
	if (objectp(tmp = tmp[1]))
	{
	    str += " with " + (tmp == for_ob ? "you" :
	      tmp->query_the_name(on) ) + " snuggled up closely to " +
    ...
    if (tmp[1] == for_ob)
	return str + " snuggled closely to your body.\n";

    return str + " snuggled closely to " + 
