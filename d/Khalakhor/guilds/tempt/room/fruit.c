 ...
 * -		    mixed adj - Adjectives we want to use (opt.)   - *
 * -       Returns: Void.                                          - *
...
set_fruit(string str, mixed adj)
{
	...
	if (!FRUITS[str])
	{
		...
		str = one_of_list(m_indices(FRUITS));
	}
	...
	if (FRUIT_SHORT[str])
	{
		/* To use special short and pshorts. */
	...
	{
		/* Include adjectives in the short. */
		set_short(implode(query_adjs(), " ") + " " + str);
		set_pshort(LANG_PWORD(query_short()));
	}
	else
