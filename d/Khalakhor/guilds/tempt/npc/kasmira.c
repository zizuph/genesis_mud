/**********************************************************************
 * - kasmira.c                                                      - *
 * - Kasmira is a npc that teaches a bit of history.                - *
...
create_khalakhor_human()
{
	...
	add_adj("beautiful");
	set_gender(G_FEMALE);
	set_long("She looks like temptress.\n"+
	"She has silky pale coloured skin.\n"+
	"She has mysterious sultry eyes.\n"+
	"Her hair is hip length, warm brown with delicate "+
	"curls.\n"+
...
hook_return_intro(string str)
{
	if (!remembered[str])
	{
		...
		command("bow " + str);
		remembered[str] = "nice";
		present(str, environment(TO))->add_introduced("kasmira");
	}
