/**********************************************************************
 * - fia.c                                                          - *
 * - Fia is a npc that teaches a bit of history.                    - *
...
create_khalakhor_human()
{
	...
	"She has silky white coloured skin.\n"+
	"She has mysterious sultry eyes.\n"+
	"Her hair is waist length, raven black with soft "+
	"curls.\n"+
...
hook_return_intro(string str)
{
	if (!remembered[str])
	{
		...
		command("bow " + str);
		remembered[str] = "nice";
		present(str, environment(TO))->add_introduced("fia");
	}
