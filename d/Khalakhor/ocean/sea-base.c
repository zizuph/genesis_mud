...
/*** Global variables ***/
static string *_rel_dir = ({"forward",  "rightforward",
...
static int Xx, Yy, _check, _relative, *_map_exit_mask, _bug;
 ...
 * Description:	  Test if TP lost directions when leaving
 *		  this room.
...
is_lost()
{
  return random(query_difficulty() > TP->query_skill(SS_LOC_SENSE)) ? 1 : 0;
}
...
find_room_file (int off)
{
  ...
  string room_file = _path+_name+"-"+x+"-"+y+"-"+_map[off..off];
  //  dump_array(({room_file, off}));
  return room_file;
}
...
block_map_room (int off)
{
  ...
  if (room && !room->block_enter()) {
    ...
    if (_relative == 1 && !TP->query_npc() &&
        (room->query_relative() == 1) && (msg = is_lost())) {
	    TP->add_prop(IS_LOST, 1);
	    // player may have turn left or right
	    TP->add_prop(OFFSET, ((off + 6 + (2 * random(3))) % 8) + 1);
      write(stringp(msg) ? msg : DEFAULT_BLOCK_MSG);
    }
    return 0;
  }
  return 1;
...
exit_map_cmd (int off)
{
  if (!TP->query_prop(IS_LOST))
    return _std_dir[off];
  return _rel_dir[(8 + off + 1 - TP->query_prop(OFFSET)) % 8];
}
...
add_row_exits (string place, string cmd, mixed efunc, mixed tired)
{
  ...
  else if (member_array(cmd, _rel_dir) != -1)
    return;
...
add_map_exits ()
{
  ...
  for (i = 0; i < 8; i++) {
    ...
    if (is_room(d) &&
	    ...
	    add_row_exits(0, _std_dir[i], &block_map_room(i), 0);
  }
}
...
add_std_exits ()
{
  //find_player("tapakah")->catch_msg("before add_std\n");
  if (!_check || _relative)
    return;
  _relative = -1; /*** room has standard exits ***/
  //find_player("tapakah")->catch_msg("in add_std\n");
  add_map_exits();
}
...
add_rel_exits ()
{
  if (!_check || _relative)
    return;
  _relative = 1; /*** room has relative exits ***/
  add_map_exits();
}
...
point (string str)
{
  ...
  else if ((off = member_array(str, _rel_dir)) < 0)
    return 0;
  else
...
init ()
{
  ...
  ::init();
  if (!_check || (_relative != 1))
    return;

  for (i=0; i<8; i++)
    add_action(unq_move, _rel_dir[i]);
...
unq_move(string str)
{
  ...
  if (_relative != 1) // we mask this function only for relative exits
    return ::unq_move(str);

  if (!(off = query_prop(ALLOW_MOVE_OFF))) {
    if ((off = member_array(query_verb(), _std_dir)) != -1) {
	    if (tp->query_prop(IS_LOST)) {
        str = COMPOSITE_WORDS(sort_array(_rel_dir + ({})));
    ...
    else if ((off = member_array(query_verb(), _rel_dir)) != -1) {
	    off = (off + tp->query_prop(OFFSET) + 7) % 8;
    }
    ...
    if (!_map_exit_mask[off]) // no exit in this direction
...
enter_inv(object ob, object from)
{
  ...
  if (_relative != 1 && ob->query_prop(IS_LOST)) {
...
create_sea_room ()
{
  ...
  set_sink_path(ROOM + "sink-path");

  reset_sea_room();
  add_item("coords", coords);
