 ...
 *
 * This particular version is not a base object, rather a separate
 * module which must be inherited into another npc. It requires
 * a little more hard coding than the knowledge_npc.c base object,
 * but contains identical functionality.
 *
 * To use it, the following hooks are required in the relevant
 * functions.
 *
...
mapping Gremembered;
int *global_coords, *local_coords;
string info_module;
 ...
 */
public void
kb_enter_env(object to, object from)
{
  //    int *temp;
    ...
    if (!(global_coords = to->query_global_coords()))
	global_coords = ({});
    /*
...
{
    object player;
    int align, gender, age, t, do_align, info, negative;
    string resp;
    ...
    info = NAME_TO_RANDOM(player->query_real_name(), str2val(OB_NUM(TO)), 5);
	...
	" is somewhere in " + ({"Ansalon", "Avenir" ,"Calia",
	  ...
	  "Emerald"})[NAME_TO_RANDOM(player->query_real_name(),
	    str2val(OB_NUM(TO)), 9)] + ", though I can't say for sure. ";
    else if (info == 2)
...
new_answer_question(string question)
{
    ...
    {
	my_communicate("Elves? Now there's a curious group. " +
	  "Don't see many of them, supposedly they once " +
	  "ruled, but something wiped most of their " +
	  "numbers out a long time ago, and their numbers " +
    ...
    else if (rval = TO->query_default_answer())
    {
	if (strlen(rval))
	    command("say " + rval);
    }
...
catch_question(string question)
{
    ...
    if (TO->query_dont_answer_unseen() && (!TP->check_seen(TO) ||
	!CAN_SEE_IN_ROOM(TO)))
    {
	set_alarm(rnd()*3.0+1.0, 0.0, TO->unseen_hook);
	return;
    }
    ...
    if (TO->query_ask())
    {
	for (i = 0; i < sizeof(TO->query_ask()); i++)
	{
	    if (member_array(question, (TO->query_ask())[i][0]) >= 0)
	    {
		set_alarm(rnd()*4.0, 0.0, &(TO)->answer_question((TO->query_ask())[i][1], (TO->query_ask())[i][2]));
		return;
	    }
	}
    }
