 ...
 *           are array values, the indices are the default names
 *           which are resolved as the indices of the aliases
 *           mapping.
 *
...
resolve_aliases(string str)
{
    ...
    alias_index = m_indices(aliases);
    
    if (member_array(str, alias_index) > -1)
        return str;

    for (i=0,sa_i=sizeof(alias_index);i<sa_i); i++)
    {
        temp = alias_index[i];
