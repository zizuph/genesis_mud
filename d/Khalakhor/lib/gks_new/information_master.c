 ...
 *
 * Reviewed and updated by Tapakah, 01/2006
 */
...
{
    int i, *tcoords, sm_i;
    string index, *module_index;
    
    setuid();
    reset_euid();

    module_index = m_indexes(modules);
    for (i=0,sm_i=sizeof(module_index);i<sm_i; i++)
    {
...
composite_options(string *str)
{
  ...
  case 2:
    ...
    break;
  default:
    return implode(str[0..str_size-2],", ") + " or " + str[str_size-1];
  }
}
 ...
 */
public string
get_response(string location, object caller, string info_string)
{
    int *oglobal, *myglobal,
      *olocal, *mylocal,
      dist,
      x_diff,
      y_diff,
    ...
    if (!sizeof(myglobal = caller->get_my_global_coords()))
