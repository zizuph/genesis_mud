...
#ifndef ACTION_BLIND
/* Just an arbitrarily high number to prevent conflicts. */
#define ACTION_BLIND 0x0400000000
#endif
...
actor(string str, object *oblist, string str1, int poss)
{
	...
	write(str + " " + (poss == 0 ?
	    ...
	    COMPOSITE_WORDS(map(map(oblist,
		&->query_the_name(this_player())),
		&LANG_FILE->name_possessive()))) +
	    (strlen(str1) ? (str1 + "\n") : ".\n"));
    }
    else
    {
		...
		int cmd_attr = 0, int poss = 0)
{
    ...
    if (!living(oblist[0]))
    {
        oblist->emote_hook(query_verb(), this_player(), adverb, cmd_attr, 1);
        return;
    }
    ...
    if (sizeof(players))
    {
        players->catch_msg((poss == 0 ? QCTNAME(this_player()) :
	    QCTPNAME(this_player())) + str + "\n");
    }
 ...
 */
varargs void
all(string str, string adverb = "", int cmd_attr = 0, int poss = 0)
{
    int index = -1;
    int size;
    object *oblist;
    ...
    if (sizeof(players))
    {
        oblist->catch_msg((poss == 0 ? QCTNAME(this_player()) :
	    QCTPNAME(this_player())) + str + "\n");
    }

    if (sizeof(npcs))
    {
        npcs->emote_hook(query_verb(), this_player(), adverb, 0, cmd_attr);
    }
}
...
all2act(string str, object *oblist, string str1, string adverb = "",
    int cmd_attr = 0, int poss_obj = 0, int poss_act = 0)
{
    ...
    if (size = sizeof(players))
    {
        ...
        if (living(oblist[0]))
        {
            while(--size >= 0)
            {
		players[size]->catch_tell((poss_act == 0 ?
		    this_player()->query_The_name(players[size]) :
		    this_player()->query_The_possessive_name(players[size])) +
		    str + " " + (poss_obj == 0 ? COMPOSITE_WORDS(map(oblist,
			&->query_the_name(players[size]))) :
		    COMPOSITE_WORDS(map(map(oblist,
			&->query_the_name(players[size])),
			&LANG_FILE->name_possessive()))) + str1);
            }
        }
        else
        {
