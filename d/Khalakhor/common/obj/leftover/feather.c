 ...
 *                it being eaten. Also note it cannot be made into a heap
 *                object because of variant values.
 * Nerfed these variant values. Too high.
 * Tapakah, 07/2021
 */
...
{ 
  ...
  case 0..5:
    add_prop(OBJ_I_VALUE,
             previous_object()->query_average_stat() + (random(5)));
    break;

  case 6..20:
    add_prop(OBJ_I_VALUE,
             previous_object()->query_average_stat() + (random(15)));
    break;

  case 21..55:
    add_prop(OBJ_I_VALUE,
             previous_object()->query_average_stat() + (random(25)));
    break;

  case 56..128:
    add_prop(OBJ_I_VALUE,
             previous_object()->query_average_stat() + (random(35)));
    break;

  case 129..680:
    add_prop(OBJ_I_VALUE,
             previous_object()->query_average_stat() + (random(55)));
    break;

  default:
    add_prop(OBJ_I_VALUE,
             previous_object()->query_average_stat() + (random(75)));
    break;
...
decay_fun()
{
  if (--decay_time)
    ...
    tell_room(environment(this_object()),
              ...
              " has slowly dried away to nothing.\n");
    remove_object();
  }
}
