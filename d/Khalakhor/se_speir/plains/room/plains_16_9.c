...
{
    extra = "To the west, the road leads through the " +
        "earthen embankment and the gates into Port " +
        "Macdunn. ";
    ...
    proxim = (["harbour": ({2, "south"})]);
    road = ({"east", "west"});

    create_plain();

    add_exit(PLAINS + "plains_16_8", "north");
    add_exit(PLAINS + "plains_17_8", "northeast");
    add_exit(PLAINS + "plains_17_9", "east");
    add_exit("/d/Khalakhor/se_speir/port/room/road_12_1", "west");
}
int
do_lay(string str)
{
