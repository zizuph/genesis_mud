...
drink_water(string arg)
{
  ...
  if (arg=="water")
    {
      notify_fail("Drink water from the pool?\n");
      return 0;
    }
  
   if (parse_command( arg, TO, 
                     "[the] 'water' 'from' / 'at' [the] 'pool'"))
    {
      ...
      if (! TP->drink_soft(amount)) 
        {
          write ("You cup your hands under the water, but you are "
            ...
            say ("Water overflows from "+QTNAME(TP)+"'s cupped hands.\n");
          //top it off anyway.
          TP->drink_soft(TP->drink_max() - TP->query_soaked());  
        }
        else
          {
        write ("You cup your hands in the pool and drink "
             + "of the ice cold water.\n");
        say(QTNAME(TP)+ " cups "+HIS+ " hands in the pool "
          + "and drinks the ice cold water.\n");
          }
        return 1;
    }
}
...
if (parse_command( arg, 
                   ...
                   animal))
    {
    say (QCTNAME(TP)+" leads "+HIS+" "+animal->short()+
