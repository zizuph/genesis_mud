...
void
 create_room()
 {
  ...
  add_exit(CAMP_DIR3+"training3","west",0,1);
  add_exit(CAMP_DIR3+"training0","north",0,1);
  add_exit(CAMP_DIR3+"training5","east",0,1);
  add_exit(CAMP_DIR3+"training11","south",0,1);
...
enter_tent(string str)
{
   ...
   TP->move_living("M",CAMP_DIR4+"trainingtent4",0,0);
   return 1;
}
void
init()
