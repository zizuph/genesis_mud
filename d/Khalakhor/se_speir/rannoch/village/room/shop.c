/* File         : /d/Khalakhor/se_speir/rannoch/village/room/shop.c
...
reset_room()
{
    ...
    if (!shopkeeper)
    {
        shopkeeper = clone_object(NPC + "bathar");
        shopkeeper->arm_me();
        shopkeeper->move(TO);
...
{
    set_short("shop of Abhainn Ruith");
    set_long("You've entered what appears to be the " +
        "local shop of Abhainn Ruith. The low-ceilinged " +
    ...
    add_exit(ROOM + "store_room.c", "west", "@@check_allowed", 0, 1);
