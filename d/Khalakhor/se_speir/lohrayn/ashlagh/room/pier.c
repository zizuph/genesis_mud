//
// pier - where the ship from Port MacDunn lands
// converted to the new ship system by Tapakah
//
#pragma save_binary
...
create_room()
{
  ::create_pier();
  create_proad();
  set_short("A wooden pier at the village of Baile Ashlagh");
  set_long("You are standing on a wooden pier which extends from a "+
           "rocky coastline over the waters of the sea. There is a rocky "+
           "winding road leading off the pier to the west which climbs "+
           "the nearby hills to the gates of a small costal village.\n");
  add_exit("proad7","west");
  add_pier_entry(MASTER_OB(TO), "Baile Ashlagh", "Cabotage to Port Macdunn");
  initialize_pier();
}
