/*
 * Door for the neuter toilet at Cadu pier
...
pass_door(string arg)
{
    ...
    ob = find_object(NYREESEDIR + "cadu_pier_toilet_m");
...
create_door()
{
  ...
  set_door_name(({"green door","not-sure","not-sure door","door"}));   /* name(s) for the door, first
                                                   string is std desc */

  /* commands */

  set_pass_command(({ "leave", "green", "not-sure"}));                    /* command(s) to pass the door */

  set_other_room(NYREESEDIR + "cadu_pier_toilet");             /* room on the other side */
  ...
  set_door_desc("It's a freshly green painted door.\n");
