...
#define ROKE_I_WAITER_TALE "_roke_i_waiter_tale"
...
buybeer(string str)
{
    ...
    if (!find_living("kalufs"))
    {
        ...
        command("shiver");
        command("say Ehh, I trust you haven't done anything " +
                "naughty with him...?\n");
        return 1;
    }
    if (!MONEY_MOVE(1, "silver", TP, TO))
    {
        ...
        say("The waiter gives an ale to " + QTNAME(TP) + ".");
      
        if (!TP->query_prop(ROKE_I_WAITER_TALE) && 
            !QDONE(ROKEGROUP, STATUEQUEST))
        {
            TP->add_prop(ROKE_I_WAITER_TALE, 1);
            write("Flint says: " + 
                    "I will tell you the tale of the statues: \n" +
            ...
            say("The waiter whispers something to " + QTNAME(TP) + ".\n");
        }
      
        i++;
        if (i == 5)
        {
            set_alarm(3.0, 0.0, buymyselfbeer);
            i = 0;
        }
    } 
    return 1;
...
create_monster()
{
    ...
    add_ask(({"help", "task", "job"}),
            "say Relax, have an ale, and let me handle the work.", 1);
