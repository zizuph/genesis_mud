...
/* no longer a separate magic attack property
    add_prop(OBJ_I_NO_MAGIC_ATTACK,
             "A forcefield protects the woodpecker.\n");
*/

    add_ask(({"blue dragon", "dragon"}),
            "say I've heard that the dragon was a wizard before.", 1);
    add_ask("wizard", "say Refnas zapped a wand at him.", 1);
    add_ask("wand", "say Wands are usually zapped.", 1);
    add_ask("ogion", "say Ogion lives in the forest.", 1);
    add_ask(({"jhonlee", "healer"}),
            "say Jhonlee meditates beyond the Stairs to the Sky.", 1);
    add_ask(({"element", "elements"}),
            "say The four elements are water, earth, fire and air.", 1);
    add_ask("water", "say Ah, water, lifegiving water. The vast sea, " +
            "the beach, letting the waves flow over your talons.", 1);
    add_ask("earth", "say Ah, earth, rich earth, its domains stretch " +
            "from the sand of the beach to the rocks of the mountains.", 1);
    add_ask("fire", "say Ah, fire, its blazing heat warming my " +
            "feathers in the chilly mornings.", 1);
    add_ask("air", "say Ah, air, my wonderful domain. I am " +
            "free to fly among the trees and mountain tops.", 1);
    set_default_answer(
        "The woodpecker stares at you with disgust and asks you " +
...
ask(string s)
{
    if (!wildmatch("* warrior*", s) || !(TP->query_prop(MQ4PROP)))
    {
...
tenk(object tp)
{
    tp->catch_msg("I think I have found the solution to your problems.\n\n" +
                  ...
                  "powers that hold the Stick you must go to places close " +
