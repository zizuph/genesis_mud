 ...
 * Maked all gruggs the same and more interesting - Tapakah, Jul 2011.
 */
...
default_config_npc(int i)
{
    int j;
    for (j = 0; j < 6; j++)
      set_base_stat(j - j / 10 + random(j/5), i);
}
...
grow(object ob)
{
  ...
  tell_room(ENV(TO), ("The grugg eats the " + ob->query_name() +
                      " and seems to grow a little.\n"));

  set_alarm(1.0, 0.0, &bomb(ob));
  average = query_average_stat();
  default_config_npc(average + 1);
  heal_hp(10000);
  set_skill(SS_DEFENCE, query_skill(SS_DEFENCE) + average % 2);
  set_skill(SS_UNARM_COMBAT, query_skill(SS_UNARM_COMBAT) + average % 2);
...
enter_inv(object ob, object from)
{
  ...
  if(from == environment())
    tell_room(ENV(TO),
	      "The grugg finds " + ob->short() + " in the room.\n");

  set_alarm(itof(5 + random(4)), 0.0, &grow(ob));
}

void
get_me(object ob) {
  command("get " + ob->query_name());
}

void
consume(object ob)
{
  if(ob == this_object() || !CAN_SEE(TO, ob))
	return;
  consume_timer = consume_timer + 1.0;
  set_alarm(consume_timer, 0.0, &get_me(ob));
}
...
void check_env(object myenv)
{
  ...
  map(all_inventory(ENV(TO)), consume);
  set_alarm(consume_timer + 4.0, 0.0, check_env);
}
