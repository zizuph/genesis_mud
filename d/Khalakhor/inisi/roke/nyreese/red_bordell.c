...
void dirty2(object tp);
void dirty3(object tp);
void dirty4(object tp);
void dirty5(object tp);
void dirty6(object tp);
void dirty7(object tp);
void dirty8(object tp);
void dirty9(object tp);
...
companion (string g)
{
  ...
  if (TP->query_gender() != 0) {
    if (TP->query_gender() == 1)
      ...
      write("I'm sorry, there is not much I can do for your kind.\n");
    return 1;
  }
  ...
  id = set_alarm(5.0, 0.0, &dirty2(TP));
  return 1;
}
...
dirty2(object tp)
{
  ...
  id = set_alarm(5.0, 0.0, &dirty3(tp));
}
...
dirty3(object tp)
{
  ...
  id = set_alarm(2.0, 0.0, &dirty4(tp));
}
...
{
  tp->catch_msg("YES!\n");
  id = set_alarm(1.0, 0.0, &dirty5(tp));
}

void
dirty5(object tp)
{
  tp->catch_msg("You think you are dreaming... You haven't experienced anything like it before.\n");
  id = set_alarm(6.0, 0.0, &dirty6(tp));
}
...
dirty6(object tp)
{
  ...
  id = set_alarm(5.0, 0.0, &dirty7(tp));
}
...
dirty7(object tp)
{
  ...
  id = set_alarm(7.0, 0.0, &dirty8(tp));
}
...
dirty8(object tp)
{
  tp->catch_msg("That felt really good! You kiss the lady good bye and " +
                "walk down the stairs to the entrance, feeling extremely " +
                "satisfied.\n");

  tp->move_living("M", NYREESEDIR + "red_bordell");
  tp->heal_hp(tp->query_max_hp() / 10);
  tp->add_fatigue(-tp->query_max_fatigue() / 3);

  id = set_alarm(14.0, 0.0, &dirty9(tp));
}
