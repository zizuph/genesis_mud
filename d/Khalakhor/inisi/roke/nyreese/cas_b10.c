 ...
 * Fixed the bug of key disappearing if the player is overtaxed.
 * Tapakah, 07/2021
 */
...
desk_search(object who, string s)
{
  ...
  if (parse_command(s, TO, "'drawer' / 'drawers' / 'desk' %s", gunk)) {
    if (key_is_here) {
      key = clone_object(ROKEDIR + "obj/key");
      if (key->move(who)) {
        extra = "The key drops to the floor.\n";
        key->move(environment(who));
      }
      say(QCTNAME(who) + " finds something in the desk.\n");
      key_is_here = 0;
