...
create_room()
{
    ...
    set_long("You are standing in one of the corner rooms of the second floor. " +
             "This room is dimly lit by a small blue shimmering globe at the wall. " +
             "The exit to the northwest seems to end up in one of the towers.\n");

    add_item(({"globe", "shimmering globe", "small globe", "blue globe"}),
    ...
    add_exit("cas_t21", "northwest");
    add_exit("cas_b2", "east");
    add_exit("cas_b5", "south");
    INSIDE;
}
...
polish(string s)
{
    ...
    */
    ...
    if (!tissue->query_prop("_roke_i_polished_globe_1"))
        TP->catch_msg("The tissue glows for a short moment.\n");

    tissue->add_prop("_roke_i_polished_globe_1", 1);
