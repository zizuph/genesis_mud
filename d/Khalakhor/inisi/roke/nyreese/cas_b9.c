...
create_room()
{
    ...
    set_long("You are standing in one of the cornerrooms of the second floor. " +
             "This room is dimly lit by a small blue shimmering globe on the wall. " +
             "The exit to the southwest seems to end up in one of the towers.\n");
    ...
    add_exit("cas_t23", "southwest");
    add_exit("cas_b5", "north");
    INSIDE;
}
...
polish(string s)
{
    ...
    */
    ...
    if (!tissue->query_prop("_roke_i_polished_globe_3"))
        TP->catch_msg("The tissue glows for a short moment.\n");

    tissue->add_prop("_roke_i_polished_globe_3", 1);
    return 1;
}
