...
object shopkeeper;
string storeroom = HH_ROOM + "storeroom";
int check_allowed();
...
create_khalakhor_room()
{
  ...
  set_short("Hammerhands shop");
  set_long("This shop is run by a renegade dwarf working for the "+
	   "Hammerhands. It is a spacy room with shelves along its walls.\n");

  add_item(({"walls", "shelves"}),"@@exa_shelves");
...
reset_room()
{
  ::reset_room();
  if (!shopkeeper) {
    shopkeeper = clone_object(HH_SHKPR);
    shopkeeper->equip_me();
    shopkeeper->move(TO);
...
exa_shelves()
{
  ...
  if (!sizeof(inv))                                                          
    return
      "The shelves and racks are all bare!\n";
  if (sizeof(inv) < 5)
    return
      "The shelves and racks are sparsely laden " +
      "with " + COMPOSITE_DEAD(inv) + ".\n";
  if (sizeof(inv) < 15)
    return
      "The shelves and racks are well-stocked with " +
      COMPOSITE_DEAD(inv) + ".\n";
  return
    "The shelves and racks are sagging under the load " +
...
do_sell(string str)
{
  if (shopkeeper && present(shopkeeper)) {
    if (query_this_faction(FACTION_DAENBRAIGH,TP) < FCT_VALUE_TOLERABLE) {
...
{
  if(ob->query_prop(HH_PROPERTY)) {
    shopkeeper->command("say this one belongs to the racks, not here!");
    NF0("");
  }
  return 1;
}
