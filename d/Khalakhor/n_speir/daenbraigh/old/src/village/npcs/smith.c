...
create_khalakhor_monster()
{
  ...
  set_name("Foryn");
  add_name(({"foryn","smith"}));
  set_race_name("dwarf");
  set_adj("grim");
  add_adj("sweaty");
  set_long("This is " + VILLAGE_NAME + " smith. His story is unclear "+
  ...
  add_ask(({"repair","polish","wax","sharpen"}),
	  "say I can fix your weapon for a reasonable price.",
	  1);
  add_ask(({"price","cost"}),
	  "say My prices reflect the quality of my work.",
	  1);
...
equip_me()
{
  ...
  clone_object(V_HAMMER)->move(TO,1);
  clone_object(V_MAIL)->move(TO,1);
  clone_object(V_BOOTS)->move(TO,1);
  command("wear all");
  command("wield all");
