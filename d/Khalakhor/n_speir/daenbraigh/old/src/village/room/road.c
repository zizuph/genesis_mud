...
create_gvillage_road()
{
  ...
  set_short("A path inside " + VILLAGE_NAME);
  set_long("You are walking on the path in the goblin village of "          +
	   VILLAGE_NAME + ". You hear snow and ice crust croaking under "   +
	   "your feet. It is quite cold and the wind is blowing strongly. " +
	   "The village has been established on a clearing between the "    +
	   "Currain ocean and the dark forest, although it is biting more " +
	   "and more into the trees as it expands. ");
  add_item(({"huts"}),
	   "Crudely built but sturdy looking huts look to be serving " +
	   "home for the village inhabitants.\n");
}

void complete_creation() {
...
collect_garbage(string str)
{
  ...
  if(str == "garbage" ||
     ...
     str == "dirt") {
    if(has_garbage) {
      garbage = clone_object(V_GARBAGE);
      garbage->move(TO);
...
{
  if(has_garbage)
    return "Typically for a goblin settlement, dirt and refuse lay "+
      "everywhere.\n";
  else
    if(present(V_GARBAGE,TO)) 
      return "Surprisingly there is no garbage around, as if someone cleaned "+
	"here, except for a large pile on the side.\n";
    else
      return "Surprisingly there is no garbage around, as if someone cleaned "+
	"here.\n";
