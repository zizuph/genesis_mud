...
create_gvillage_hut()
{
  ...
  set_short("A inside a hut in " + VILLAGENAME);
  set_long("You are inside an ordinary hut inside "+VILLAGENAME+". It has only one\n"+
	   "room but is relatively spacey. You can see a sleeping mat near wall and\n"+
	   "a cupboard opposite to it. A crudely built table and a stool are in the\n"+
	   "center of the room.\n");
...
{
  if(! villager1) {
    villager1 = clone_object(VILLAGER);
    villager1->move_living("X",TO);
  }
  if(! villager2) {
    villager2 = clone_object(VILLAGER);
