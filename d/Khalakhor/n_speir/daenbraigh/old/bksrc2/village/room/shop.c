...
object shopkeeper;
string storeroom = VROOM + "storeroom";
void
create_khalakhor_room()
{
  set_short(VILLAGENAME + " shop");
  set_long(BSN("The shop is a dirty, spacy building with numerous shelves."));
...
reset_room()
{
  ::reset_room();
  if (!shopkeeper) {
    shopkeeper = clone_object(SHPKPR);
    shopkeeper->equip_me();
    shopkeeper->move(TO);
...
exa_shelves()
{
  ...
  if (!find_object(storeroom))
    ...
    if (sizeof(inv) < 5)
      return BSN("The shelves and racks are sparsely laden " +
		 "with " + COMPOSITE_DEAD(inv) + ".\n");
    if (sizeof(inv) < 15)
      return BSN("The shelves and racks are well-stocked with " +
		 COMPOSITE_DEAD(inv) + ".\n");
    return BSN("The shelves and racks are sagging under the load " +
...
do_sell(string str)
{
  if (shopkeeper && present(shopkeeper)) {
    if (TP->query_align > 0) {
      shopkeeper->cmd("say you don't look like one I want to deal with.\n");
      return 1;
    }
    return ::do_sell(str);
  }
