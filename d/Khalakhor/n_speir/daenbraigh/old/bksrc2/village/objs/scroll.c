...
{
  set_name("scroll");
  add_name(VOBJID + "scroll");
  set_adj(({"wrinkled","parchment"}));
  set_short("wrinkled parchment scroll");
  set_long(BSN("This scroll has been dearly guarded by the village elder. "+
	       "You can probably read it."));
  set_scroll_file(SCROLL_TEXT);
}
void
init()
{
...
chant_me(string str)
{
  ...
  if(sscanf(str,"%s at %s",word,enemy) == 1)
	...
	confuse_spell(target,TP);
      else
	write("But your target is not present!\n");
      TP->set_mana(-CONFUSION_COST);
      return 1;
    }
  notify_fail("Chant what?\n");
  return 0;
...
command_read(int more_flag)
{
  ...
  while(cont = read_bytes(SCROLL_TEXT,charnum,1)) {
    if (cont == " " ||
	cont == "\n" ||
	cont == "\t" ||
