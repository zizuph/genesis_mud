...
#define SDESC \
"There are a few bar stools placed directly in front "+ \
"of the bar along with a few tables and soft cushioned "+ \
"chairs scattered around the room invitingly.\n"

mixed sdef = ({
({
({"stool","on stool", "barstool", "on barstool", "bar"}),
"You hop up on a wooden bar stool and sit down.\n",
" hops up on a wooden bar stool and sits down.\n",
"You slide off your wooden bar stool.\n",
" slides off a wooden bar stool.\n",
"You are sitting comfortably on a wooden bar stool.\n",
"You slides off a wooden bar stool and leave the room.\n",
" sitting comfortably on a wooden bar stool",
" sitting comfortably on a wooden bar stool.\n",
}),
...
leave_inv(object ob, object to)
{
	...
	if (ob && !present(ob) && seated[ob])
	{
		...
		if (seated[ob][0] == 2 && (to = seated[ob][1]))
		{
			...
			to->catch_tell("You are dumped onto the "+
			"floor.\n");
...
do_stand(string arg)
{
	...
	if (tmp[0] == 3 && objectp(ob = tmp[1]))
	{
		...
		say(QCTNAME(this_player()) + " climbs off "+
		"of " + QTPNAME(ob) + " lap.\n", ({ this_player(), ob }));
	...
	else
	{
		...
		{
			ob->unset_no_show_composite();
			ob->catch_tell("You are dumped onto the floor.\n");
