/**********************************************************************
 * - fbath.c                                                        - *
 * - Female bathing area. It allows bathing (removes dirty prop) &  - *
 * - allows free emotes.                                            - *
...
create_khalakhor_room()
{
	...
	"the northern wall with a small sign with flowing script "+
	"dangling from it. Directly diagonal from the fireplace is a "+
	"small crystal coffer with a silver plaque. A wonderfully "+
	"substantial tub dominates the center of this spacious room "+
	"yet allows space for a couple of padded benches.\n");
	...
	coffer=clone_object(IOBJ + "ccoffer");
	coffer->move(TO);
