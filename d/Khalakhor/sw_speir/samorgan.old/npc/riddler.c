  ...
  hook_return_intro(string str)
{
    ...
    {
	command("growl sof");
	command("say I remember thee cleraly, so be on thy way!");
    }
    else
    {
	command("giggle");
  ...
  get_riddle(object who)
{
    ...
    command("ask "+ whom +" Here is a riddle I need help with. "+
	    "Give me the answer within two minutes to proceed...");
    command("clear throat");
    command("ask "+ whom +" "+ riddles[current]);
	
    add_ask(answers[current], "@@riddle_answer@@", 0);
  ...
  receive_globe(object ob, object from)
{
    ...
    {
	is_busy = questor = success = tries = 0;
	command("scratch"); command("shrug");
	ob->remove_object(); return;
    }
...
#if 0
    if (from->query_guild_member("Eil-Galaith"))
    {
	command("ask "+ whom +" Thou hath no need to "+
		"answer my riddles.");
	return;
    }
#endif
    ...
    if (from->query_race() == "goblin" || (from->query_alignment() <= GUILD_MIN_ALIGN))
    {
