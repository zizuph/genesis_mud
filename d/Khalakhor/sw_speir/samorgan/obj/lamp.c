 ...
 * - Created by Damaris@Genesis 06/2006                            - *
 *********************************************************************/
#pragma strict_types
inherit "/std/torch";
inherit "/lib/keep"; 
...
create_torch()
{
	...
	set_pname("lamps");
	add_pname("oil-lamps");
	set_short("emerald oil-lamp");
	set_pshort("emerald oil-lamps");
...
long_description()
{
	string longstr;
	if (!strlen(lamp_desc))
	longstr = "This is a lovely emerald oil-lamp. The base is "+
	"a stout bowl made of emerald tinted glass that shimmers "+
	"whether filled or empty of oil. The emerald bowl of the "+
	"lamp is surrounded by highly polished silver fittings along "+
	"the edge. It has a wick on one end and a curved polished "+
	"silver handle on the other. ";
...
do_refill(string str)
{
	...
	if (!parse_command(str,TP," [the] [emerald] [oil] 'lamp' / "+
	"'oil-lamp' [with] [lamp] [oil] / [lamp-oil] "))
	{
		notify_fail(CAP(query_verb()) + " what with what?\n");
		return 0;
	}
	...
	{
		notify_fail("You have no extra oil to fill the "+
		""+ short() +" with!\n");
		return 0;
	}
	...
	{
		notify_fail("The "+ short() +" is already filled with "+
		"oil!\n");
		return 0;
	}
...
query_light_fail()
{
	if (!query_time(1))
	{
		return "You try to light your " + short() + ", but it "+
...
notify_low_on_oil()
{
	...
	if (living(env))
	{
		env->catch_msg("Your "+ short() +" flickers briefly. "+
		"It seems to be running out of oil.\n");
		LTELL_ROOM(ENV(env), env, "The "+ short() +" that "+
		"", " holds, flickers briefly. It seems to be running "+
		"out of oil.\n", env);
	}
	else if (env->query_prop(ROOM_I_IS))
	{
		tell_room(env, "The " + short() + " flickers briefly. "+
		"It seems to be running out of oil.\n");
	}
}
