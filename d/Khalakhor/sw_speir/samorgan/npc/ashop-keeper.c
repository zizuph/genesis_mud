...
{
	set_name("martin");
	set_title("Keller, from the Clan Morgan, Proprietor of "+
	"SaMorgan Armoury");
	add_name(({"shopkeeper","keeper", "man"}));
	set_race_name("human");
	set_gender(G_MALE);
	add_adj(({"broad-chested", "muscular"}));
	set_long("He is an armours dealer and proprietor.\n"+
	"He has coal black eyes.\n"+
	"He has smooth leathered skin.\n"+
	"His hair is very short, black with grey streaks along "+
	"his temples.\n");
...
hook_return_intro(string str)
{
	...
	else if (SECURITY->query_wiz_rank(TIQRN, "Khalakhor"))
	{
		...
		"thee, a new cloak or perhaps you would prefer a cape?");
		present(str, environment(TO))->add_introduced("martin");
		command("kneel " + str);
		remembered[str] = "nice";
	}
