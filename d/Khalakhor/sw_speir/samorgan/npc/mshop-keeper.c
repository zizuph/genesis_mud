/**********************************************************************
 * - brenetta.c                                                     - *
 * - Clothing shop owner in SaMorgan                                - *
 * - /d/Khalakhor/sw_speir/samorgan/room/mshop.c                    - *
...
#include <std.h>
#include "npc.h"
#define DRESS "/d/Khalakhor/sw_speir/samorgan/arm/dress2"
...
{
	set_name("brenetta");
	set_title("Chalahan, weaver of cloths and shop keeper.");
	add_name(({"shopkeeper","keeper", "woman"}));
	set_race_name("human");
	set_gender(G_FEMALE);
	add_adj(({"quirky", "well-dressed"}));
	set_long("She is a fine seamstress who considers herself "+
	"more a weaver of cloths than a shopkeeper.\n"+
	"She has startingly crystal blue eyes.\n"+
	"She has smooth dark skin.\n"+
	"Her hair is shoulder length, light red and styled into a "+
	"bun atop her head.\n");
        ...
        add_act("say Oh I do hope you will be pleased with my "+
          "kilts and tunics.");
        add_act("say All my clothing have been sewn with love and care.");
//        add_act("emote sighs: Oh I wish I had bought some new ribbons "+
//        "today, I am always running out.");
...
hook_return_intro(string str)
{
	if (SECURITY->query_wiz_rank(TIQRN) >= WIZ_ARCH)
	{
		...
		present(str, environment(TO))->add_introduced("brenetta");
		command("kneel " + str);
	...
	else if (SECURITY->query_wiz_rank(TIQRN, "Khalakhor"))
	{
		...
		"thee, a new doublet or perhaps you would prefer a "+
	...
	{
		command("growl sof");
		command("say I remember thou very well. So be on thy "+
		"way!");
	}
	else
	{
