...
go_climb()
{
    ...
    if((TP->query_skill(SS_CLIMB) <= 15))
    {
        ...
        say(QCTNAME(TP) + " attempts to climb down the " +
          "mastpole, but fails.\n");
        return "";
    }
    ...
    set_dircmd("down");
    TP->move_living("climbing down the mastpole",
                    SHIP_ROOM_DIR + "deck", 0, 0);
    return "";
}
...
go_fly()
{
    if(!(TP->query_prop(LIVE_I_FLYING)))
    {
        write("You are not able to fly down at the moment.\n");
        say(QCTNAME(TP) + " attempts to fly downwards, " +
    ...
    set_dircmd("down");
    TP->move_living("flying downwards",
                    SHIP_ROOM_DIR + "deck", 0, 0);
    return "";
}
...
go_jump()
{
    ...
    set_dircmd("down");
    TP->move_living("jumping over the edge",
                               SHIP_ROOM_DIR + "deck",
      0, 0);
    TP->heal_hp(-500);
    write("You crunch onto the deck!\n");
    say(QCTNAME(TP) + " crunches into the deck, falling " +
      "from above!\n");
    if (TP->query_hp() <= 0)
    {
        TP->do_die();
...
look_down()
{
    room = find_object(SHIP_ROOM_DIR + "deck");
