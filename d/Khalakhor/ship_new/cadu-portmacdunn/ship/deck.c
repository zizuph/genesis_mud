...
create_room()
{
    ...
    add_item(({"ship","vessel","Corran","corran","boat"}),"This is the ship " +
      "that ferries passengers and freight between the lands of " +
      "Khalakhor and Nyreese.\n");
    ...
    add_exit("mast", "asglkjhsag", 1,1,1);
...
go_climb()
{
  ...
  set_dircmd("up");
  TP->move_living("climbing the mastpole",
                  SHIP_ROOM_DIR + "mast", 0, 0);
  return "";
}
...
go_fly()
{
    if(!(TP->query_prop(LIVE_I_FLYING)))
    {
        ...
        say(QCTNAME(TP) + " attempts to fly upwards, " +
          "but fails miserably.\n");
        return "";
    }
