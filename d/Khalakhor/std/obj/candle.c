//
// Standard candle - std torch that disappears when burns out
// Zima - 5/23/98
//
inherit "/std/torch";
...
public void burned_out()
{
    ...
    if (living(env))
       {
       tell_object(env, "The " + tmp + " burns out and melts away.\n");
       ...
       if (rm->query_prop(ROOM_I_IS))
          tell_room(rm,  "The " + tmp + " on the " + (env->short()) +
                         " burns out and melts away.\n");
       }
   remove_object();
}
