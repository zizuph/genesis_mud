/*
 * help-me.c
 * Adapted from Port-Macdunn particular by Khail
 * Tapakah, 04/2021
 */
 ...
 *   [optional] : ID of the guards to respond to the call.
 *                defaults to _port_macdunn_guard for compatibility.
 * Returns      : n/a
 */
public varargs void
notify_others_of_attack(object who, string helper="_port_macdunn_guard")
{
  object *rooms,
    *others;
  int i;
  ...
  /* Do nothing if the npc's ENV has no exits. */
  if (!sizeof(ENV(TO)->query_exit_rooms()))
    return;

  rooms = ({ENV(TO)});
  rooms += map(ENV(TO)->query_exit_rooms(), find_object);
  ...
  for (i = 0; i < sizeof(rooms); i++)
    {
      others = filter(all_inventory(rooms[i]),
                      &->id(helper));
      others->come_assist_me(TO, who, file_name(ENV(TO)));
    }
}
...
much_stronger(object who)
{
  switch (random(3))
    ...
    case 0:
      ...
      break;
    case 1:
      TO->command("say Are ye daft or drunk, " +
...
stronger(object who)
{
  switch (random(3))
    ...
    case 0:
      TO->command("chuckle");
      TO->command("say Well, if it's a beating ye be lookin' " +
                  "for, I'll happily take care of your needs.");
      break;
    case 1:
      TO->command("ruffle " + OB_NAME(who));
      TO->command("say I almost feel bad about this, " +
                  "but ye've got it coming to ye!");
      break;
    default:
...
slightly_stronger(object who)
{
  switch (random(3))
    ...
    case 0:
      TO->command("growl");
      TO->command("say Alright, if it's a fight ye " +
                  "want, let's " + "go!");
      break;
    case 1:
      TO->command("rolleyes");
...
slightly_weaker(object who)
{
  ...
  switch (random(3))
    ...
    case 0:
      ...
      break;
    case 1:
      TO->command("emote sets " + HIS_HER(TO) +
                  " jaw and braces for the fight ahead.");
      break;
    default:
      g = who->query_gender();
      TO->command("say Bite me ye " +
                  ({"bastard","bitch","freak of nature"})[g] + ", I'll " +
