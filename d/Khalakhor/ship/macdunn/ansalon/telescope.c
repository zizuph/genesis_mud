...
create_object()
{
    set_name("telescope");
    set_adj("metal");
    add_adj(({"shiny","grey","iron","well-oiled"}));
    set_short("metal telescope");
    set_pshort("metal telescopes");
    set_long("This metal telescope is a shiny grey colour. It is made " +
      "from tempered iron, and hence is well-oiled, to prevent " +
      "corrosion. Considering that it is found in the crow's nest of " +
      "the boat, it can certainly see quite a distance. You can use " +
      "it to focus on the destinations that this boat travels to.\n");

    add_cmd_item(({"khalakhor","macdunn","port macdunn","on macdunn",
        "on khalakhor","on port macdunn"}),({"focus","view"}),
      "@@see_khalakhor");
    add_cmd_item(({"ansalon","on ansalon","kalaman",
        "on kalaman"}),({"focus","view"}),"@@see_ansalon");
    ...
    add_cmd_item(({"telescope","scope","metal telescope","grey telescope",
        "shiny telescope","iron telescope"}),({"break","smash","destroy"}),
...
see_khalakhor()
{
    room1 = find_object("/d/Khalakhor/ship/macdunn/pier2");
