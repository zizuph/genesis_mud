...
create_monster()
{
    ...
    set_long("This blonde-bearded ship captain is one of the calmest "+
      ...
      "affecting his serene and peacefuly face. He seems to be a quiet "+
...
public string query_destination()
{
    if((file_name(this_object()->query_prop(LIVE_O_LAST_ROOM)))
      == ("/d/Khalakhor/ship/macdunn/pier4"))
    {
        return ("say We're headed to Baile Ashlagh matey.");
    }
    if((file_name(this_object()->query_prop(LIVE_O_LAST_ROOM)))
      == ("/d/Khalakhor/se_speir/lohrayn/ashlagh/room/pier"))
    {
        return ("say We're headed to Port MacDunn matey.");
    }
    else return ("say I'm not sure matey, but I think we're headed "+
                 "to Baile Ashlagh or Port Macdunn.");
}
