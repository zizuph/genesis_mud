...
create_room()
{
    ...
    add_exit(S_LOCAL_ROOM_DIR + "board", "west");
...
pray(string str)
{
    ...
    if (TP->query_race() != "dwarf" && TP->query_race() != "ghost")
    {
        ...
        write(BS("A blob of stinqking goo drops on you from above and a bolt of lightning strikes your bottom!\n"));
        goo_blob = clone_object(S_LOCAL_OBJ_DIR + "goo_blob");
        goo_blob->move(TP, 1);
