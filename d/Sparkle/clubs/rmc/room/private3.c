...
/*  set_long(
    ...
    "An open\ndoor in the east wall leads to a small room.\n"+
    "There are two obvious exits: east and down.\n"+
    "Three magnificent portals.\n");*/

    set_long(
    "You are on the top floor of the tower-like building where "+
    "the Rich Men's Club has its quarters. The view from the "+
    "large window is incredible, and the atmosphere in the room "+
    "makes you relax. Three portals are placed in the north, "+
    "south and west walls. A golden plaque is placed over the window. "+
    "An open door in the east wall leads to a small room.\n"+
    "There are two obvious exits: east and down.\n"+
    "Three magnificent portals.\n");
  ...
  add_exit(RMCROOMS+"private2","down");
  add_exit(RMCROOMS+"private4","east");
  
  INSIDE;

  restore_object(RMC_PORT);
}
