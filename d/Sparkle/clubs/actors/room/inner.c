 ...
 *
 * This is where Mrpr's magic is performed!
 *
 * 2006 : Lunatari
 * + Genesis -> Sparkle move.
 * 2008-01-03 : Lunatari
 * + Updated to support the new functions within the 
 *   central.c to handle offline invites and lauding.
 * 2008-02-21 : Lunatari
 * + Removed laud/deride commands. (Only one rank now)
 */
#pragma strict_types
...
create_room()
{
    ...
    add_exit(AC_ROOMS+"script", "east", "@@enter_test", 1, "@@can_see_exit");
    add_exit(AC_CLUB, "south");
...
init()
{
    ...
    if (QUERY_LEVEL(TP) >= MEMBER_RANK || IS_MANAGER(TP))
    {
    ...
    if (IS_MANAGER(TP) || (QUERY_LEVEL(TP) == ADMIN_RANK))
    {
...
do_read(string arg)
{
	...
	{
	case "":
	    TP->more(read_file(AC_HELP + "index", 1));
	    break;
	...
	case "rules":
	case "symbol":
	    TP->more(read_file(AC_HELP + arg, 1));
	    break;
	...
	TP->more(read_file(AC_HELP + "general", 1));
 ...
 * Returns:	1 - success, 0 - not
 *
 * Version: 2008-01-03 Lunatari BETA
 */
static nomask int
do_invite(string str)
{
    ...
    {
		write(capitalize(name) + " does not seem to exist " +
		    "within the realms of Genesis.\n");
		return 1;
    }

    if (CHECK_MEMBER(pl))
    {
    	write(capitalize(name) + " is already a member.\n");
    	return 1;
    }

    if (SECURITY->query_wiz_rank(name) != WIZ_MORTAL)
    {
	    write("Unfortunately " + capitalize(name) + " is a wizard. " +
	        "Wizards are considered to be permanently OOC and thus " +
	        "not suitable as members of the Actors Club.\n");
    	return 1;
    }

    if(!AC_CENTRAL->query_suggest(name))
    {
		write("For the sake of community you should suggest " +
		    capitalize(name) + " to the club before you invite.\n");
		return 1;
	}

    TP->command("$snap");

    write("You have invited " + capitalize(name) + " to the Actors Club.\n");

    if(present(pl, this_object()))
      pl->catch_msg(QCTNAME(TP) + " invited you to become an Actor.\n");

    // INVITE_MOD[QUERY_LEVEL(people[i])
    if(TP->query_real_name() == WIZ_MANAGER)
        votes = 3;
    else
        votes = 3;
/* THIS WAS OLD CODE
        votes = INVITE_MOD[QUERY_LEVEL(TP)];
*/

    AC_CENTRAL->add_invite(name, votes, TP->query_real_name());
...
do_apply(string str)
{
    ...
    invnum = AC_CENTRAL->query_invites(name);

    req = AC_CENTRAL->query_expelled(name) ? INVITE_MIN * 2 : INVITE_MIN;

    if (!nwiz && invnum < req)
    {
    	write("Unfortunately you haven't been invited by a sufficient " +
    	    "number of members (" + req + ") to be accepted.\n");
	   return 1;
    }
    ...
    nlist = AC_CENTRAL->query_inviters(name);
    AC_CENTRAL->add_member(TP->query_real_name(), nlist);
    AC_CENTRAL->delete_invite(TP->query_real_name());
...
do_remove(string str)
{
    ...
    if (!str && QUERY_LEVEL(TP) != ADMIN_RANK)
      return 0;
    else if (!str)
      return notify_fail("To remove a Club Member, use the syntax: remove <name> <reason>\n");

    args = explode(str, " ");

    if (sizeof(args) < 2)
	return notify_fail("Syntax: remove <name> <reason>\n");
    ...
    AC_CENTRAL->remove_member(name, TP->query_real_name(), args[1]);
...
do_review(string str = "")
{
    ...
    default:
    /*
     * This has been added to let Thespians review "history" but not the
     * actual player logs.
     *
     * Why? Because admin and I agree the logs are too powerful.
     *
     * Lunatari 2006-09-12
     */
    if(!IS_WIZ(TP))
    {
        write("The review command only takes history as argument.\n");
        return 1;
        break;
    }
	logs = explode(str, " ");
	if (sizeof(logs) > 1)
	{
	    ...
	    if (wildmatch(name + "*", TP->query_real_name()))
	    {
...
do_honour(string str)
{
    ...
    if (!str) return notify_fail("Honour who?\n");

    if (!wiz && !CHECK_MEMBER(TP))
	return notify_fail("How did you get in here?\n");
...
do_list(string str)
{
    ...
    case "invited":
	AC_CENTRAL->list_invites();
	break;

    default:
	return notify_fail("List what? Members, removal, suggested or invited?\n");
    };
...
enter_inv(object to, object from)
{
	...
	AC_CENTRAL->update_last(TP->query_real_name());
    }
    else
	set_alarm(1.0, 0.0, &write("\nYou have an odd feeling of intruding on something, or someone.\n\n"));
    ::enter_inv(to, from);
}
...
/*
 * Name: do_suggest
 * Desc: So that I can debug the suggestion system, Lunatari
 * Args:
 * Returns:
 */
public nomask int
do_suggest(string arg)
{
    ...
    else if (sizeof(args) > 2 || sizeof(args) < 1)
    {
        return 0;
    }
    else
