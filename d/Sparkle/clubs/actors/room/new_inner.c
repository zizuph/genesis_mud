 ...
 *
 * Updated by Lunatari during 2006 for Genesis -> Sparkle move.
 * Updated by Lunatari 2008-01-03 to support the new functions
 *  within the central.c to handle offline invites and lauding.
 */
#pragma strict_types
...
create_room()
{
    ...
    set_short("Lunatari's Actor Testing Area");
...
//    add_exit(AC_ROOMS+"script", "east", "@@enter_test", 1, "@@can_see_exit");
    ...
    add_exit("/w/lunatari/workroom", "back");
    add_exit(AC_ROOMS+"script", "east", "@@enter_test", 1, 1);
...
do_invite(string str)
{
    ...
    if (CHECK_MEMBER(pl) || (AC_CENTRAL->query_invites(name)))
    {
    	write(capitalize(name) + " is already a member or has " +
    	    "already been invited by someone.\n");
    	return 1;
	}
    ...
    AC_CENTRAL->set_invite(name, votes, TP->query_real_name());
 ...
 * Returns:	1 - success, 0 - not
 *
 * Version: 2008-01-03 Lunatari ALPHA
 */
static nomask int
do_apply(string str)
{
...
do_list(string str)
{
    ...
    default:
	return notify_fail("List what? Members, removed, expelled or suggested?\n");
    };
