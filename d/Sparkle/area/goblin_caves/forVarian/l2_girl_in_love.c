...
enter_inv(object ob, object from)
{
    ...
    else if(ob->id(LEVEL2_QUEST_FLOWER))
    {
        if(interactive(from))
        {
            ...
            {
                command("thank " + TP->query_name());
                command("say Flowers are nice but Dima should try harder.");
                this_player()->add_prop(GCL2_QUEST_PROP, 4);
                set_alarm(1.0, 0.0, &drop_the_object(ob));
            }
            else
            {
                command("thank " + TP->query_name());
                command("say I love black flowers.");
...
reject_letter(object ob, object player)
{
    ...
    command("say These words are nice. But they are just words.");
    command("say He should bring me something I will like.");
    
    command("say You can return this letter to Dima.");
...
{
    command("thank " + TP->query_name());
    command("say Yes, this diamond really shows Dima's passion. Pass him this letter.");
