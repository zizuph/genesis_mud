 ...
 * Updated by Gronkas August 10, 2016 to fix some typos
 * and introduce syntax to search "under desk" and "under the desk"
 */
#include "defs.h"
#pragma strict_types
...
create_room()
{
    ...
    set_long("This is the recruiter private cave. " +
             "There is a desk with some papers on it and there are some "+
             "chairs. You can see a door in the western wall. \n\n");

    add_item(({"desk","under desk", "under the desk"}), "@@get_desk_desc");
...
do_search(object pl, string arg)
{
    if (!strlen(arg) || (member_array(arg,
        ({ "desk", "under desk", "under the desk"}) ) == -1))
    {
	...
	|| (this_player()->query_prop(GCL1_QUEST_PROP) != 3))
		return "There is nothing under the recruiter's desk.\n";
	...
	{
		return "You find a goblin armour under the desk.\n";
	}
	else
	{
		quest_armour->move(this_object());
		return "You find a goblin armour under the desk but are unable to "+
				"hold it. It falls to the floor.\n";
	}
...
get_desk_desc()
{
	...
	return "This is a small wooden desk with some papers on it. But " +
           "you can see something lying under the desk.\n";
