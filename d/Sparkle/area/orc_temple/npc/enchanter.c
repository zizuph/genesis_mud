 ...
 *
 *  Modification Log:
 *    - January 2019: Implemented "ironstone" rollback features (Gorboth)
 *    - November 2020: Removed Imbuements global (Cotillion)
 */
#pragma strict_types
...
#define            AUCTION_DATA_STORE  ("/d/Sparkle/area/city/auction/obj/auction_data_store")

#define            IMBUE_COST          1000
#define            STONE_COST          25
#define            LAPIDARIUM_COST     10
#define            IDENTIFY_COST       2
...
public string    *Available_Stones = m_values(Create_Stone);
...
public int         equipped_check(object obj);
public int         check_remaining_quests(object who);
public int         stone_trade(string arg);
...
create_sparkle_npc()
{
    ...
    add_ask( ({ "thief", "robbery", "thieves", "vault", "theft" }),
        "say Ah, so you heard about that, did you? Yes, I have been"
      + " the victim of a very serious theft. Be wary, for these are"
      + " dreadful times!", 1);
...
public mixed * get_imbuement_info(int imbue_id)
{
    mixed * result = ({});
    if (imbue_id > 0)
    {
        mapping imbuements = IMBUE_MASTER->get_imbuements();
        result = imbuements[imbue_id];
    }
    else
    {
        int auction_id = -imbue_id;
        result = AUCTION_DATA_STORE->get_imbue_info("" + auction_id);
    }
    return result;
}
...
id_imbuements(string arg)
{
    ...
    mapping  mod_secrets = ([
      ...
      "effect_slow"        : ({ "will place a curse of slowness upon any"
                              ...
                              + " who strike it", }),
      "effect_unbreakable" : ({ "has become unstable, and is no longer"
                              + " able to be used" }),
    ...
    if (equipped_check(obj[0]))
    {
        return 1;
    }

    imbue_info = get_imbuement_info(imbue_id);
...
complete_payment(object who, int needed, string reason)
{
    ...
    IMBUE_LOGGER->log("imbue_payments",
        capitalize(this_player()->query_name())
      + " pays " + needed + " platinum coins. (" + reason + ")");
...
do_buy(string arg)
{
    ...
    switch(sales[0])
    {
        ...
        case "diminution amulet":
            ...
            pl_size = this_player()->query_average_stat();

            /* Here, we introduce a special bit of code to ensure that
             * players beneath 99 stat ave do not get stuck paying
             * huge amounts of plats for the amulet. 99 and above will
             * have to work for it!
             */
            if (pl_size <= 52)
            {
        ...
        case "lapidarium":
            ...
            //    TIME2FORMAT(time(), "mm/dd/yyyy")
            //  + " (" + ctime(time())[11..18] + ") "
            IMBUE_LOGGER->log("buy_lapidarium",
                capitalize(this_player()->query_name())
...
enchanter_work(string arg)
{
    ...
    // Based on bug report on Feb 4, 2016, removing this debug line.
//    write("mod1 is " + mod1 + "\n");
    ...
    if (disenchant)
    {
        ...
        word = "takes the " + item_short + " and places it on"
          + " the table before him. He passes a strange prism of some"
          + " kind over the table, muttering quietly.";

        if (mod1_lvl)
            word += " " + obj[0]->unimbue(1)[..-2];
        if (mod2_lvl)
            word += " " + obj[0]->unimbue(2)[..-2];
        if (mod3_lvl)
            word += " " + obj[0]->unimbue(3)[..-2];

        word += " With a swift gesture"
        ...
        IMBUE_LOGGER->log("disenchantments", 
            capitalize(this_player()->query_name())
          + " disenchanted an object for 5 plats:\n\t"
          + obj[0]->short() + ": " + file_name(obj[0]) + "\n\t"
          + "ID:\t\t" + imbue_id + "\n\t"
        ...
        filename = file_name(obj[0]);
        obj[0]->remove_object();
        IMBUE_GENERATOR->unimbued(imbue_id, filename, 0);
        return 1;
    }
...
//      + " (" + ctime(time())[11..18] + ") "
//      + capitalize(this_player()->query_name())
    log_txt = capitalize(this_player()->query_name())
    ...
    filename = file_name(obj[0]);
    obj[0]->remove_object();
    IMBUE_GENERATOR->unimbued(imbue_id, filename, 1);

    //IMBUE_GENERATOR->imbue_log("stones_created", log_txt);

    IMBUE_LOGGER->log("stones_created", log_txt);
...
enchanter_imbue(string arg)
{
    ...
    int      imbue_id;
    string   item_short;
    object   free_token = present("_free_imbue_token", this_player());
    int      has_token = objectp(free_token);
        ...
        this_player()->query_real_name())
    {
        if (has_token)
        {
            command("say Yes, I would be willing to imbue this "
              + item_short + " for you. Oh! And you have one of the"
              + " tokens that only immortals give out on very rare"
              + " occasions! Well, that means you can do it for free"
              + " just this once. If you wish to proceed, confirm by"
              + " asking me to <imbue> the item a second time.");
        }
        else
        {
    ...
    {
        command("say Good! Let's have that token then.");
        write("You give the sparkling token to the enchanter.\n");
        command("say The fee is waived for you, just this once!");
    }

    IMBUE_GENERATOR->imbue_random(obj[0]);
    if (IMBUE_GENERATOR->get_error()) {
        command("sigh");
        command("say Unfortunately, I am unable to imbue this item at this"
            + " time.");

        if (!has_token)
        {
            money = MONEY_MAKE_PC(IMBUE_COST);
            money->move(this_object());
            command("give coins to " + OB_NAME(this_player()));
        }
        else
        {
            command("emote gives the token back.");
        }
        return 1;
    }
    ...
    if (has_token)
    {
        free_token->remove_object();
        IMBUE_LOGGER->log("free_token",
            capitalize(this_player()->query_real_name())
          + " used their free token!");
    }
...
return_imbued_item(object ob, object temp_location, object who)
{
    ...
    IMBUE_LOGGER->log("enchanter_imbue", 
        capitalize(this_player()->query_name())
      + " imbued an object:\n\t"
      + item_short + ": " + file_name(ob) + "\n\t"
      + "ID:\t\t" + imbue_id + "\n\t"
...
react_stones(object stone)
{
    ...
    if (stone->id("ironstone"))
    {
        command("say Ah! An ironstone. These have become quite rare"
          + " nowadays. I will <trade> it with you if you like. You"
          + " can request any other stone type and I will give you"
          + " that stone at the same quality level of the ironstone"
          + " you wish to trade me.");
        return;
    }
...
/*
 * Function name:        equipped_check
 * Description  :        check to make sure the item is not being worn or
 *                       wielded.
 * Arguements   :        object obj - the object to check
 * Returns      :        0 - not worn/wielded
 *                       1 - it is equipped
 */
public int
equipped_check(object obj)
{
    if (obj->query_worn() || obj->query_wielded())
    {
        command("say Please unequip the item, and lets try again.");
        return 1;
    }
...
/*
 * Function name:        stone_trade
 * Description  :        Allow players to trade the decomissioned
 *                       "ironstones" 1 for 1 with the enchanter.
 * Arguments    :        string arg - what was typed
 * Returns      :        int 1 - success, 0 - failure
 */
public int
stone_trade(string arg)
{
    object  *stones;
    object   ob;
    object   newstone;
    ...
    mixed    has_ironstone = present("ironstone", this_player());
    string   name = query_the_name(this_player());
    ...
    if (!has_ironstone)
    {
        notify_fail(cname + " says: I don't believe you have anything"
          + " I am interested in trading. Come back if you ever find"
          + " an ironstone, however.\n");
        return 0;
    }

    if (!strlen(arg))
    {
        notify_fail(cname + " looks interested and says: I see that"
          + " you have an ironstone. I will trade you one to one"
          + " for any other stone type of the same quality. Just"
          + " <trade ironstone for ...> listing"
          + " the stone type you wish to acquire in trade.\n");
        return 0;
    }

    if (!parse_command(arg, all_inventory(this_player()),
        "%i 'for' %w", stones, newtype) ||
        !sizeof(stones = NORMAL_ACCESS(stones, 0, 0)))
    {
        notify_fail(cname + " says: I'd be happy to trade. For"
          + " example you could <trade ironstone for mercurystone>"
          + " or any other type of stone you desire.\n");
        return 0;
    }
    ...
    if (!ob->id("ironstone"))
    {
        notify_fail(cname + " says: I would be happy to trade you"
          + " something for an ironstone, but not for "
          + ob->short() + ".\n");
        return 0;
    }
    
    if (!ob->id("_imbuement_container_stone"))
    {
        notify_fail(cname + " says: This is clearly not a true"
          + " ironstone, and I have no interest in it whatsoever.\n");
        return 0;
    }

    if (!IN_ARRAY(newtype, Available_Stones))
    {
        notify_fail(cname + " says: Trade your ironstone for "
          + LANG_ADDART(newtype) + "? I do not recognize that as"
          + " a valid imbuement stone. Please select a type that"
          + " actually exists.\n");
        return 0;
    }

    if (newtype == "ironstone")
    {
        notify_fail(cname + " says: Are you jesting? We will not"
          + " waste time by trading identical items, now, will"
          + " we?\n");
        return 0;
    }

    stone_quality = ob->query_stone_quality();

    write(cname + " happily trades for your ironstone, giving you "
      + LANG_ADDART(stone_quality) + " " + newtype + " in"
      + " return.\n\n");

    ob->set_stone_type(newtype);

    tell_room(environment(this_object()),
        QCTNAME(this_player()) + " trades something with the"
      + " enchanter.\n", this_player());

    command("say I would store that in a lapidarium immediately"
      + " if I were you!");

    IMBUE_LOGGER->log("ironstones",
        capitalize(this_player()->query_real_name()) + " trades"
      + " for " + LANG_ADDART(stone_quality) + " " + newtype);
