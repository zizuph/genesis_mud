/*
 *  /d/Sparkle/area/tutorial/npc/lion.c
 *
 *  This is the file for the mountain lions in the cliffs below
 *  Krolocks castle. From them can be taken useful claw leftovers
 *  used in creating a grappling hook.
 *
...
create_creature()
{
    ...
    set_long("This mountain lion is a well-built hunter. Muscles"
      + " move in steady motion beneath the sleek yellow coat of"
      + " fur which covers the beast. Its long claws and sharp"
      + " teeth are vicious weapons against prey.\n");

    set_skill(SS_AWARENESS, 15);
    set_alignment(0);

    add_prop(CONT_I_WEIGHT, 30000);      /* 60 lbs */
    /* 15" tall by 10" long by 8" wide */
    add_prop(CONT_I_HEIGHT, 65);        /* 35 inches */
    add_prop(CONT_I_VOLUME, 50330);     /* 3200 cubic inches */
    ...
    default_config_creature(11);

    set_skill(SS_DEFENCE, 15);
    set_skill(SS_UNARM_COMBAT, 5);

    set_attack_unarmed(0,  7, 9, W_IMPALE,  70, "front teeth");
    set_attack_unarmed(1,  9, 7, W_SLASH,   30, "claws");
...
arm_me()
{
    ...
    object  pelt = clone_object(ARM_DIR + "large_pelt");

    //pelt->config_pelt("large");
    pelt->move(this_object());

    if (!random(7))
    {
        catseye = clone_object(OBJ_DIR + "catseye")->move(this_object());
    }
} /* arm_me */
