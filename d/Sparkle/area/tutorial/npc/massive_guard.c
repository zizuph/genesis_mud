/*
 *  /d/Sparkle/area/tutorial/npc/massive_guard.c
 *
 *  This is the file for the elite guards in Krolocks Castle in the
 *  Tutorial Area.
 *
...
/* Definitions */
#define  DEMON_SKILL        20  /* combat skill levels          */
...
create_monster()
{
    ...
    add_adj( ({ "black", "demonic", "massive" }) );
    set_race_name("demon");

    set_short("massive demonic guard");
    set_long("This guard is much larger and looks stronger than the other"
      + " guards in the castle. He must be an elite type of soldier."
      + " Though shaped much like a man, this is clearly a"
    ...
    add_prop(CONT_I_WEIGHT, 89600);      /* 200 lbs */
    /* 15" tall by 10" long by 8" wide */
    add_prop(CONT_I_HEIGHT, 145);        /* 85 inches */
...
arm_me()
{
    object    mail   = clone_object(ARM_DIR + "demon_mail");
    object    armour = clone_object(ARM_DIR + "demon_"
                + one_of_list( ({ "helm", "greaves", "bracers",
                                  "shield" }) ) );
