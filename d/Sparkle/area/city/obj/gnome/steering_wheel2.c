...
/*
 * Function name:        angle_next
 * Description  :        tell the next machine to angle a certain
 *                       way
 * Arguments    :        string which - whether this is forward/backward
 *                       string angle - the way we want to angle
 */
public void
angle_next(string which, string angle)
{
    string  angle_txt;
    object *next_mach = query_next_mach();

    switch(angle)
    {
        case "middle":
            angle_txt = "fixed in place, its flat edge parallel to the"
                      + " ground";
            break;
        case "left":
            angle_txt = "fixed in place, its flat edge angled to the left";
            break;
        case "right":
            angle_txt = "fixed in place, its flat edge angled to the right";
            break;
    }

    if (sizeof(next_mach))
    {
        foreach (object next_m: next_mach)
        {
            switch(which)
            {
                case "forward":
                    next_m->set_f_forward(angle_txt);
                    break;
                case "backward":
                    next_m->set_f_backward(angle_txt);
                    break;
            }
        }
    }
