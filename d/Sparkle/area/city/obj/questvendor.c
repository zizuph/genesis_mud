 ...
 *
 *  Updated October 3, 2016 by Gronkas to remove mortar
 *  as quest object due to conflict with Ranger's mortar
 *  after talking it over with Cherek and Varian
 *
 ...
 *
 *  Thanks as always to Mercade for guidance on a few sticky points
 *  regarding heaps, and general coding structure advice.
 */
#pragma strict_types
...
exa_kiosk()
{
    ...
    if (VENDOR->get_info(QUEST_FILE(name), "day") != date)
    {
        ...
        VENDOR->set_info(CURRENT_QUESTS, "unfinished", old_array);
...
read_poster(string arg)
{
    ...
    write("\n"
        ...
        + "\t|  <inquire x>    - Ask the vendors about the history of |\n"
        + "\t|                   a given item.                        |\n"
...
do_deliver(string arg)
{
    ...
    foreach (object obj: objs)
    {
        ...
        if (!obj->id(v_item))
        {
            notify_fail("The vendors are not in the market for that"
              + " item.\n");
            continue;
        }
        ...
        else if (obj->query_keep() && !obj->query_unsellable())
        {
        ...
        if (IS_HEAP_OBJECT(obj))
        {
            ...
            if (heap_amount > remaining)
            {
                ...
                delivered += remaining;
                VENDOR->set_info(QUEST_FILE(name), "returned", num);
                break;
            }
            else
            {
            ...
            VENDOR->set_info(QUEST_FILE(name), "returned",
...
claim_reward(string arg)
{
    ...
    if (returned >= num)
    {
        ...
        else
        {
            ...
            if (MAX_REPEATS ==
                VENDOR->get_info(QUEST_FILE(name), "repeats"))
            {
        ...
        write("\nOne of the vendors says: \"Oh, and please remember to"
          + " come back next " + MARKETDAY + "! We are sure to need"
...
log_quest(object player, int exp, string name, int final = 0)
{
    ...
    if (final)
    {
        write_file(VENDOR_FINAL_LOG, sprintf("%s %-12s (%30s)\n",
            ctime(time()),
            (string)player->query_name(),
            "Finished Final Vendor Quest"));
    }
} /* log_quest */
