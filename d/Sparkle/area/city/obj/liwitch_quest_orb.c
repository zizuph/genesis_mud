/*
 * /d/Sparkle/area/city/obj/liwitch_quest_orb.c
 *
 * Orb that tells a player what quests he has done in
 * a particular domain
 *
 * Created April 2008, by Petros
 * Modifyed 080427,    by Styles 
 *
 *
 * THIS orb is specifically intended for the Liwitch domain.
 * If counting the nomber of cloned ORIGINAL orbs this orb should NOT
 * be included in that count since some people don't believe it to exist.
 *
 ...
 * Description  :       Lets us search the orb for non obvious information.
 * Arguments    :		player - The player object who does the search.
						args   - The string targeting what to search.
 * Returns      :		The search result string, if anything.
 * Styles 080427
 */
public string
search_cmd(object player, string args)
{
    ...
    {
		case "orb":
			return	long() + "\nThe color however is a bit uncommon for an office orb.\n";
	}
	return "";
}
...
/* Function name:       query_extra_line
 * Description  :       
 *                      
 * Styles 080427                     
 */
...
/* Function name:       set_orb_owner
 * Description  :       Sets a owner of the orb. When set we will alert
 *                      the owner when the quests are viewed.
 * Arguments    :       name - The player name of the owner. This is
 *                      likely the domain Liege.
 * Returns      :       Nothing.
 * Revised by   :		Styles 080427.
 */
public void
set_orb_owner(string name)
{
	Orb_owner = lower_case(name);
...
quests_cmd(string arg)
{
	...
	// If the orb has a designated owner we will tell him/her that the quests was viewed.  /Styles.
	if (Orb_owner)
	{
		object owner = find_player(Orb_owner);		
		string when = ctime(time());
		string name = this_player()->query_name();
		int level = this_player()->query_wiz_level(name);

		if (owner) tell_object(owner, when + strchar(7) + "  Quests viewed by "+ name +" - "+ level +"\n\n");
	}
...
// Altered text when presentating the Liwitch quests. /Styles
    write("Of the " + total +" "+ capitalize(domain) +" quests, eligible to you "+ 
 ...
 * Description  : This particular orb is dedicated to Liwitch quests.
 * Arguments    : none
...
hints_enabled()
{
    //Set to one when Liwitch hints eventually are enabeled.
    return 0;
}
