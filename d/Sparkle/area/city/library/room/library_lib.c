/*
 * Base Lib for Library
 *
 * Inherits from /lib/library.c and customizes the hooks for use
 * in creating a customizable library. Every room in the library
 * that supports retrieving books will use this inheritable lib.
 *
 * How to create your own library:
 * 1. Call init_genesis_library inside your own init function
 *     public void 
 *     init() 
 *     {
 *         ::init();  
 ...
 *     }  
 * 2. Call setters for book location
 *       set_library_dir("base/library/directory");
 *       set_approval_dir("/base/library/directory/approval");
 *       set_removed_dir("/base/library/directory/removed"); 
 * 3. Call create_genesis_library inside your room creation
 *       create_genesis_library( ({ 
 *           "fighter", "spellcaster", "stealth", "general" }), // shelves
 *           "occguilds" ); // subdirectory
 *
 * Created by Petros, March 2014
 * Adapted from the Genesis Library code created in 2008
 */
...
// Global Variables
private string library_book_dir = LIBRARY_BOOK_DIR;
private string library_approval_dir = LIBRARY_APPROVAL_DIR;
private string library_removed_dir = LIBRARY_REMOVED_DIR;

// Prototypes
public void             create_genesis_library(string *shelves, 
                                               string subdir);
public void             init_genesis_library();
public void             set_library_dir(string dir);
public void             set_approval_dir(string dir);
public void             set_removed_dir(string dir);
public string           get_library_dir();
public string           get_approval_dir();
public string           get_removed_dir();
...
public void             
set_library_dir(string dir)
{
    library_book_dir = dir;
}

public void             
set_approval_dir(string dir)
{
    library_approval_dir = dir;
}

public void             
set_removed_dir(string dir)
{
    library_removed_dir = dir;
}
...
get_approval_dir()
{
    return library_approval_dir;
}

public string           
get_removed_dir()
{
    return library_removed_dir;
}
    ...
    { 
        ...
        formatted_output = get_formatted_short_row("Book Title", 
                         ...
                         + get_formatted_short_row(
                               ...
                               "===============", 0)
                         + formatted_output;
        this_player()->more(formatted_output);
...
create_genesis_library(string *shelves, string subdir)
{
    ...
    string bookdir = get_library_dir();
    if (strlen(subdir))
    {
        bookdir += subdir + "/";
    }
    set_book_directory(bookdir); 
    set_book_approval_directory(get_approval_dir());
    set_book_removal_directory(get_removed_dir());
