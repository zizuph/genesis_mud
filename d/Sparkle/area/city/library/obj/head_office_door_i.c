/*
 * /w/petros/sparkle/library/obj/head_office_door_i.c
 *
 * Door to the Office of the Head Librarian (Inside)
 *
...
{
    ::create_door();
    add_name( ({ "_head_office_door_in" }) );    
    set_door_name( ({ "door", "office door" }) );
    set_door_desc(&office_door_description());
    set_door_id("_head_office_door");
    set_other_room(LIBRARY_ROOM_DIR + "waiting_room");
    set_open_desc("The office door to the north is open.\n");
    set_open_mess( ({ "opens the office door to the north.\n",
                      "The office door to the south swings open.\n" }) );
    set_closed_desc("The office door to the north is closed.\n");
    set_close_mess( ({ "closes the office door to the north.\n",
                       "The office door to the south swings shut.\n" }) );
...
office_door_description()
{
    ...
    description = "The door is really quite plain compared to the "
                + "decor in the rest of the Library. ";
...
{
    string headoffice_room = LIBRARY_ROOM_DIR + "headoffice";
    ::do_open_door(mess);
    
    if (query_open())
    {
        // door is currently open. we add the exit
        headoffice_room->add_exit(LIBRARY_ROOM_DIR + "waiting_room", 
                                  "north", 0);
    }
}
...
do_close_door(string mess)
{
    ...
    {
        // door is currently open. we add the exit
        headoffice_room->remove_exit("north");
    }
}
