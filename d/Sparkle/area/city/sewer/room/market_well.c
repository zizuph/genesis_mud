 ...
 *
 *              May 2009:
 *                      Moved the code from the /d/Genesis/ location to the
 *                      sewers directory here in the Sparkle domain. (Gorboth)
 */
...
create_room()
{
	...
	add_item("walls",
		...
		"contempt. They was however built buy humans and they are "+
	...
	add_item("hole",
		...
		"It is low, just about twenty centmeters high and about "+
		"two meters wide.\n");
...
climb_up(string str)
{
    ...
    if (TP->query_skill(SS_CLIMB) > needed_skill_level)
	{
		...
		TP->move_living("M", TOWN_DIR + "square");

		TELL_STY(TP->query_real_name() +" climbed out of the well.\n");
    }
	else
		{
			write("You don't seem to be able to get a good hold. You "+
				  "slide back as soon as you put your weight on.\n");
			say( QCTNAME(TP) +" tryes to climb out of the well.\n");
