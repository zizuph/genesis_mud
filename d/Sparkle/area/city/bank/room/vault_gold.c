/*
 * /d/Sparkle/area/city/bank/room/vault_gold.c
 *
 * This is the gold vault
...
create_bank_room()
{
    ...
    set_long("This vault is a square room with shelves and cabinets lining "
        ...
        + "walls are made of red bricks. Gold coins and ingots are in "
        + "piles on the shelves and on the floor. The exit, to the southeast, "
    ...
    add_item( ({ "gold coins", "ingots", "gold ingots", "coins", "pile",
                 "piles", "heap", "heaps", "gold" }),
          "Common legal tender and bars of gold fill the shelves and "
        + "litter the floor.\n");
    ...
    add_item( ({ "floor", "ground", "slabs", "granite", "granite slabs" }),
        ...
        + "gold coins and ingots. Two glass tubes stick out from the "
        + "ground.\n");
    
    add_item( ({ "shelves", "shelf", "cabinets", "cabinet", 
                 "cherry cabinet", "cherry cabinets" }),
          "The shelves and cabinets are filled to the brim with gold "
        + "coins.\n");
    ...
    lever1->set_adj( ({ "blue", "steering" }) );
    lever2->set_adj( ({ "rusty", "steering" }) );

    gear1->set_adj( ({ "large", "copper" }) );
    gear3->set_clockwise(1);
    gear3->set_adj( ({ "small", "copper" }) );
    gear5->set_adj( ({ "medium", "copper" }) );
    gear7->set_clockwise(1);
    gear7->set_adj( ({ "average", "copper" }) );
    screw1->set_adj( ({ "long", "copper" }) );
    
    gear2->set_adj( ({ "large", "steel" }) );
    gear4->set_clockwise(1);
    gear4->set_adj( ({ "small", "steel" }) );
    gear6->set_adj( ({ "medium", "steel" }) );
    gear8->set_clockwise(1);
    gear8->set_adj( ({ "average", "steel" }) );
    screw2->set_adj( ({ "long", "steel" }) );
    ...
    /* Exits */
    add_exit(BANK_ROOM + "guard_room", "southeast", 0);
