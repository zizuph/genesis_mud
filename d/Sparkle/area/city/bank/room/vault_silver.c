/*
 * /d/Sparkle/area/city/bank/room/vault_silver.c
 *
 * This is the silver vault
...
create_bank_room()
{
    ...
    set_long("This vault is a square room with shelves and cabinets lining "
        ...
        + "walls are made of red bricks. Silver coins and ingots are in "
        + "piles on the shelves and on the floor. The exit, to the east, "
    ...
    add_item( ({ "silver coins", "ingots", "silver ingots", "coins", "pile",
                 "piles", "heap", "heaps", "silver" }),
          "Common legal tender and bars of silver fill the shelves and "
        + "litter the floor.\n");
    ...
    add_item( ({ "floor", "ground", "slabs", "granite", "granite slabs" }),
        ...
        + "silver coins and ingots. Two glass tubes stick out from the "
        + "ground.\n");
    
    add_item( ({ "shelves", "shelf", "cabinets", "cabinet" }),
          "The shelves and cabinets are filled to the brim with silver "
        + "coins.\n");
    ...
    add_item( ({ "east wall", "eastern wall", "door", "vault door",
    ...
    gear1->set_adj( ({ "large", "brass" }) );
    gear3->set_clockwise(1);
    gear3->set_adj( ({ "small", "brass" }) );
    gear5->set_adj( ({ "medium", "brass" }) );
    gear7->set_clockwise(1);
    gear7->set_adj( ({ "average", "brass" }) );
    screw1->set_adj( ({ "long", "brass" }) );
    
    gear2->set_adj( ({ "large", "iron" }) );
    gear4->set_clockwise(1);
    gear4->set_adj( ({ "small", "iron" }) );
    gear6->set_adj( ({ "medium", "iron" }) );
    gear8->set_clockwise(1);
    gear8->set_adj( ({ "average", "iron" }) );
    screw2->set_adj( ({ "long", "iron" }) );
    ...
    /* Exits */
    add_exit(BANK_ROOM + "guard_room", "east", 0);
