/*
 * /d/Sparkle/area/city/bank/room/vault_copper.c
 *
 * This is the copper vault
...
create_bank_room()
{
    ...
    set_long("This vault is a square room with shelves and cabinets lining "
        ...
        + "walls are made of red bricks. Copper coins and ingots are in "
        + "piles on the shelves and on the floor. The exit, to the northeast, "
    ...
    add_item( ({ "copper coins", "ingots", "copper ingots", "coins", "pile",
                 "piles", "heap", "heaps" }),
          "Common legal tender and bars of copper fill the shelves and "
        + "litter the floor.\n");
    ...
    add_item( ({ "floor", "ground", "slabs", "granite", "granite slabs" }),
        ...
        + "copper coins and ingots. Two glass tubes stick out from the "
        + "ground.\n");
    
    add_item( ({ "shelves", "shelf", "cabinets", "cabinet" }),
          "The shelves and cabinets are filled to the brim with copper "
        + "coins.\n");
    ...
    add_item( ({ "walls", "north wall", "northwest wall", "west wall",
                 "southwest wall", "south wall", "southeast wall",
                 "east wall", "northern wall", "northwestern wall",
    ...
    add_item( ({ "northeast wall", "northeastern wall", "door", "vault door",
    ...
    lever1->set_adj( ({ "dented", "steering" }) );
    lever2->set_adj( ({ "polished", "steering" }) );

    gear1->set_adj( ({ "large", "bronze" }) );
    gear3->set_clockwise(1);
    gear3->set_adj( ({ "small", "bronze" }) );
    gear5->set_adj( ({ "medium", "bronze" }) );
    gear7->set_clockwise(1);
    gear7->set_adj( ({ "average", "bronze" }) );
    screw1->set_adj( ({ "long", "bronze" }) );
    
    gear2->set_adj( ({ "large", "nickel" }) );
    gear4->set_clockwise(1);
    gear4->set_adj( ({ "small", "nickel" }) );
    gear6->set_adj( ({ "medium", "nickel" }) );
    gear8->set_clockwise(1);
    gear8->set_adj( ({ "average", "nickel" }) );
    screw2->set_adj( ({ "long", "nickel" }) );
    ...
    /* Exits */
    add_exit(BANK_ROOM + "guard_room", "northeast", 0);
