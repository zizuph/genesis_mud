...
create_rooftop_room()
{
    ...
    set_long("The view from up here is simply magnificent. One can see "
           ...
           + "the streets. You realize you are at the northwest corner "
    ...
    add_item( ({ "east", "tower", "tower of realms" }),
               ...
               + "to get a better view of it. To your east on the rooftop "
               + "you also see a lot of plants that someone has placed on "
               + "the roof."
               + "\n");
                              
    add_item( ({ "west", "guild", "adventurer guild", "adventurers guild" }),
                 "You stand on the northwestern edge of the rooftop. To "
               + "your west, you can tell just how close you are to the "
               + "next building, which looks like the Adventurer Guild. "
               + "\n");
    
    add_item( ({ "ledge", "window ledge" }),
                 "A window ledge is right below you, over the edge of "
               + "the roof to the west. You think you might be able to "
    ...
    add_item( ({ "rooftop", "rooftops" }),
               ...
               + "east. You also see a strange array of plants "
               + "to the east of you."
               + "\n");

    add_item( ({ "plants" }),
                 "The rooftop seems to contain an array of plants to the "
               + "east of you. You might be able to get a closer look if "
               + "you walked a little east."
               + "\n");
    ...
    // Exits Section
    add_exit(ROOFTOP_ROOM_DIR + "rooftop_garden", "east", 0);
...
climb_cmd(string args)
{
    ...
    case "down to ledge":
        ...
        new_room = safely_load_master_file(LIBRARY_ROOM_DIR + "upper_nw_stairwell");
