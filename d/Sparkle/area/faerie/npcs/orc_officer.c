...
void create_orc()
{
    ...
    add_name(ORC_OFFICER_NAME);
    set_name("orc");
    set_race_name("orc");
    set_adj(mental);
    set_adj(phys);
    set_short(mental + " " + phys + " orc");
    set_long("This is an officer of the orcs. The dog-like face is " +
        "black and blue and has sharp fangs. Its hair is disheveled " +
        "and bristly looking. The long arms almost reach the ground. " +
        "It commands everything in the camp.\n");
    add_name(FAERIE + ORC + "_officer");
    add_name("orc");

    make_gold(1+random(5));

    set_base_stat(0, 100, 10);
    set_base_stat(1, 100, 10);
    set_base_stat(2, 100, 10);
    set_base_stat(3, 90, 10);
    set_base_stat(4, 90, 10);
