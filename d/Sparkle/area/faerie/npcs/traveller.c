...
void create_monster()
{
    ...
    mental = ({"tireless", "alert", "perceptive", "wandering", "aware", 
        "observant", "wise", "keen", "active"});
    phys  = ({"strong", "energetic", "weather-worn"});
    type = ({"traveller", "vagabond", "adventurer", "explorer"});

    x = random(sizeof(mental));
    y = random(sizeof(phys));
    z = random(sizeof(type));
    set_name("human");
    add_name(FAERIE+TRAVELLER+"_");
    add_name(type[z]);
    set_race_name("human");
    set_gender(random(2));
    set_adj(mental[x]);
    ...
    set_long("The " + query_short() + " looks like a well travelled " +
        "human, is tanned from " + HIS_HER(TO) + " many journeys, " +
        "and has long dark hair. " + CAP(HE_SHE(TO)) + " has sparkling " +
        "eyes that have probably seen many wonders.\n");

    default_config_npc(10+random(20));
    ...
    set_alignment(0);
    default_config_npc(50 + random(15));
