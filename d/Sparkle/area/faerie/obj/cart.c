/* 
 * Cart used for push cart quest
 * Finwe, January 2005
 * 
 * Removed DEX check. Just relies on STR.
 * - Finwe 1/1/2017
 */
#include "defs.h"
...
#define FIX_QUEST   "fix_cart"
#define CART_DIR    "/d/Sparkle/area/faerie/obj/"
...
create_object()
{
    ...
    add_item(({"pony"}),
        ...
        "of the cart.\n"); 
    add_item(({"wheel", "wagon wheel", "cart wheel", "back end"}),
                "@@wheel_descr@@\n");        
...
int push_cart(string str)
{
    ...
    if(TP->test_bit(DOMAIN, QUEST_GROUP, CART_QUEST_BIT))
    {
        write("You have done this before already.\n");
        return 1;
    }
...
// if player didn't repair cart, they won't get exp for just pushing it.
            ...
            if (stat >=min_str)
            {
                write("You pushed the cart successfully!\n");
                in_mud = 1;

/* Has done quest already */
                if(TP->test_bit(DOMAIN, QUEST_GROUP, CART_QUEST_BIT))
                ...
                TP->set_bit(QUEST_GROUP, CART_QUEST_BIT);
            ...
            {
                write("You grunt and groan, but are not strong enough to push " +
                    "the cart. Perhaps you should try lifting some " +
                    "weights, and try again when you are stronger.\n");
                return 1;
            }
        }
...
long_descr()
{
    string long_desc = "This is a wooden cart. It is rectangular shaped " +
        "and used to transport goods and supplies by a farmer. It looks " +
...
// check to see if player has all the items
    ...
    if (!objectp(nails = present("_faerie_nails", TP)))
        write("You don't have the supplies needed to fix the wheel.\n");
    else
        has_supplies++;

    if (!objectp(band = present("_faerie_iron_band", TP)))
...
//    if( (stat_str < min_str) && (stat_dex < min_dex) )
    if( (stat_str < min_str) )
    {
        write("The cart is too heavy for you to move. Perhaps you should " +
...
fix_wheel(object player, int stage)
{
    ...
    nails = present("_faerie_nails", TP);
    band = present("_faerie_iron_band", TP);
    ...
    switch (stage)
    {
        ...
        case 6:
            ...
            say(QCTNAME(TP)+ " steps back and admires " + HIS_HER(TP) + " completed work.\n",TP);
            time = 1.0;
            break;
        case 7:
            nails->remove_object();
