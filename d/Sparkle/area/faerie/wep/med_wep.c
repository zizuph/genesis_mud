/*
 * Medium quality weapons for graveyard
...
create_weapon()
{
    ...
    adjectives = ({"ancient", "bloody", "broad", "deadly", "rusty", "sharp", "shoddy"});
    wep_type = ({"sword", "axe", "knife", "dagger", "halberd"});
    ...
    switch(wep)
    {
        ...
        {
            set_name("halberd");
            set_short(adj + " " + adj1 + " halberd");

            set_wt(W_POLEARM);
            set_dt(W_IMPALE | W_SLASH);
            set_hands(W_BOTH);
            set_long("The " + short() + " looks old. It has a long " +
                "blade on a long handle, and looks able to hurt an " +
                "enemy. \n");
            break;
        }
