...
create_room()
{
    ...
    set_long("You are walking along a small dirt path, which winds its way " +
        ...
        "you here, it continues south before trailing away towards the " +
        ...
        "remote corners of the world. Just to the north, an old bridge " +
        "reaches across the river.\n");

    add_exit(THIS_DIR + "riv3","south");
    add_exit(THIS_DIR + "riv4","north","@@block@@",0,0);

    add_item(({"bridge", "old bridge", "wooden bridge"}),
             "Gracefully spanning the river to the north is wooden bridge " +
             "that is so old, the wood has turned grey. Despite its age, " +
             "the bridge still appears to be in very good condition.\n");
    ...
    add_item(({"water", "waters"}),
             "You can see the waters of the Diamond River as they rush along " +
             "past you on their journey to the shining blue water " +
...
if((str != "bridge") && (str != "old bridge") && (str != "wooden bridge") && (str != "slope"))
{ write("Climb where?\n");
return 1;
}
write("You climb up the slope to the top of the wooden bridge.\n");
SAYBB(" climbs up the slope to the top of the bridge.");
TP->move_living("M",WILD_DIR + "n_road2",1,0);
SAYBB(" arrives climbing up the slope beside the old wooden bridge.");
return 1;
}
...
block()
{
    write("You attempt to follow the river north under the bridge, but it " +
        "seems like the water levels are too high, you can only climb up " +
        "the slope or go back south.\n");
    return 1;
}