...
create_room()
{
    ...
    add_item(({"floor" }),"Part from being highly polished and very " +
                          "shiny, you see nothing special.\n");
    add_item(({"walls" }),"Very shiny indeed.\n");
    ...
    add_exit(S_AREA_ROOM_DIR + S_AREA_TOWN + "road4", "north");
    add_exit(S_LINK_AOE_OFFICE, "down");
    add_exit(S_COMMON_ROOM_DIR + "common", "common");
    add_exit("admin", "south");
    add_exit("wizapp", "east");
    add_exit("commtower", "west", "@@nonewbie");
    add_exit("tfloor1", "up", "@@up");
...
fix_board()
{
    ...
    if (hub) {
        hub->set_board_name(S_VAR_BOARD_DIR + "tower");
        ...
        hub->set_err_log(S_LOG_BOARD_DIR + "tower");
...
query_office_floor(string dom)
{
    ...
    return (S_LOCAL_ROOM_DIR + floor_names[e[0]]);
}
