...
create_room()
{
    ...
    set_long("This is one of the hot spots in the world of Genesis." +
      ...
      "able to read your words. You may also mail the Playerarch from any " +
    ...
    add_item("banner","The banner fills the space between the" +
      " lintel and the roof, and is centered between the" +
    ...
    add_item(({"bell","bells"}),"Bells of various sizes adorn" +
      " this strange contraption, tinkling occassionally with" +
      " the movement of air. Perhaps you could ring a bell.\n");

    add_cmd_item("bell","ring","Curiousity is a strange thing." +
    ...
    add_exit(S_DOMAIN_START_DIR + "human/post", "west");
...
reset_room(){
    object board;
    if(!present("board")){
        board = clone_object(S_LINK_RESTORE_BOARD);
...
{
    ::enter_inv(ob, from);
    if (!LOAD_ERR(S_LINK_RANGER_HALL))
        S_LINK_RANGER_HALL->test_restore_ranger(ob);
}
