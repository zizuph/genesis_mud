...
create_room()
{
    ...
    add_exit("road4","west");
    add_exit("shop","south");
    add_exit("circle","east");
    add_exit(S_LINK_DONATION_OFFICE, "north");
    ...
    clone_object(S_LINK_DONATION_COLUMN)->move(this_object(), 1);
}
...
climb_down(string str)
{
    ...
    if (TP->query_skill(SS_CLIMB) > random(30))
        TP->move_living("climbs down the well", S_LOCAL_ROOM_DIR + "well");
    else
    {
