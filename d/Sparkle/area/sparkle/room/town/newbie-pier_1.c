...
create_pier ()
{
  set_short("Berth A - Newbie line to Sarn Ford and Town of Last");
  set_long("\n"+
           ...
           "To the west you see the old pier and the town of Sparkle. "+
		   ...
		   "There is a sign here that reads: Berth A - Sarn Ford and Town of Last.\n");
  add_item(({"pier" }),
  ...
  add_exit(TOWN_DIR + "pier1", "west");
  add_exit("newbie-pier_2", "east");

  add_prop(ROOM_I_TYPE, ROOM_NORMAL);
  //add_prop(ROOM_S_MAP_FILE, "sparkle_city_current.txt");

  add_pier_entry(MASTER_OB(this_object()), "Sparkle", "Newbie Line A");
  initialize_pier();
