...
create_ship()
{
  ...
  set_long("This fishing boat is the Cuinnsear, a dagger-shaped " +
           "fishing vessel that transports small people around Sparkle, Sarn Ford " +
           "and Town of Last in Terel. Although small, this ship does " +
           "appear to be able to withstand an ocean storm, because of " +
           "its unique shape.\n");
  set_ship_line(MASTER_OB(this_object()), 
                CAPTAIN, TICKET, 
                ({ SPARKLE_PIER, SARN_FORD_PIER,
                   LAST_PIER }) );
