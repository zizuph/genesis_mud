...
create_monster()
{
    ...
    set_attack_unarmed(1, 30, 30, W_BLUDGEON, 10, "left elbow");
    set_attack_unarmed(1, 30, 30, W_BLUDGEON,  40, "left fist");
    set_attack_unarmed(1, 30, 30, W_BLUDGEON, 10, "right elbow");
...
arm_me()
{
    ...
    MONEY_MAKE(random(2) + 1, "gold")->move(TO);
    MONEY_MAKE(random(20) + 5, "silver")->move(TO);
    MONEY_MAKE(random(20) + 10, "copper")->move(TO);

    clone_object(S_LOCAL_ARM_DIR + "loin")->move(TO);
    clone_object(S_LOCAL_WEP_DIR + "boulder")->move(TO);
    command("wear all");
    command("wield all");
}
