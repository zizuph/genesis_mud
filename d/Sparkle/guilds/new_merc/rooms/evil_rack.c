 ...
 *
 *  This is the rack room exclusive to evil members of the Mercenary
 *  Guild.
 *
...
create_merc_room()
{
    set_short("the evil-faction armoury");
    set_long("This chamber appears to be an armoury. There are no"
      + " windows in this room, and the only exit is south, through"
      + " an archway that leads back to the sleeping quarters.\n\n");
    ...
    add_item( ({ "exit", "arch", "archway", "south",
                 "south wall", "southern wall",
                 "wall to the south" }),
        "The south wall opens through an archway onto the sleeping"
      + " quarters.\n");
    ...
    add_exit("evil_start", "south");

    set_no_exit_msg( ({ "northwest", "north", "northeast", "west", "east",
                        "southeast", "southwest", }),
    ...
    room_add_object(OBJ_DIR + "evil_weapon_rack");
    room_add_object(OBJ_DIR + "evil_armour_rack");
    room_add_object(OBJ_DIR + "evil_misc_rack");
} /* create_merc_room */
