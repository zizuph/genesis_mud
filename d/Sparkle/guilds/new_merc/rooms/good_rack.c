 ...
 *
 *  This is the rack room exclusive to good members of the Mercenary
 *  Guild.
 *
...
create_merc_room()
{
    set_short("the good-faction armoury");
    set_long("This chamber appears to be an armoury. There are no"
      + " windows in this room, and the only exit is north, through"
    ...
    add_item( ({ "exit", "arch", "archway", "north",
                 "north wall", "northern wall",
                 "wall to the north" }),
        "The north wall opens through an archway onto the sleeping"
      + " quarters.\n");
    ...
    add_exit("good_start", "north");

    set_no_exit_msg( ({ "northwest", "northeast", "west", "east",
    ...
    room_add_object(OBJ_DIR + "good_weapon_rack");
    room_add_object(OBJ_DIR + "good_armour_rack");
    room_add_object(OBJ_DIR + "good_misc_rack");
} /* create_merc_room */
