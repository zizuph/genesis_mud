 ...
 *  Updated by Gronkas August 8, 2016 to fix maffiliate when
 *  affiliated with Kender and then doing <maffiliate clean>
 *  Line 2073 changed from guild="inventor" to guild="kender"
 *
 *  Updated by Arman August 2019, added new affiliate options.
 */
#pragma  strict_types
#pragma  save_binary
#pragma  no_inherit
...
/* Prototypes */
public int skill_extra_guru(object who);
...
public void    adapt_script1(int skillnum, object who, object npc);
public void    adapt_cost(int skillnum, object who, object npc);
public void    adapt_train1(int skillnum, object who, object npc);
public int     trainer_busy(object who, object npc);
public void    merc_pay1(int skillnum, object who, object npc);
public void    check_new_adapt_msg(object who, int skillnum);
public void    adapt_increase(int skillnum, object who, object npc, int adapt_alarm);
public void    adapt_cleanup(object who, object npc, int skillnum, string ended = "");
public int     skill_extra_total(object who);
public int    *query_skillnum_index();
public void    display_skill_enhance(mixed who);
public int     set_skill_enhance(object who, int skillnum, int amount);
 ...
 *                       interaction.
 * Arguments    :        int skillnum - the number of the skill to adapt
 *                       object who - the player in question,
 *                       object npc - the npc who is the trainer
 */
public void
adapt_script1(int skillnum, object who, object npc)
{
    string  t_name = capitalize(Adapt_Map[skillnum][MERC_ADAPT_TRAINER]);
    int     remem;
    ...
    {
        set_alarm(2.0, 0.0, &adapt_train1(skillnum, who, npc));
        return;
    }

    set_alarm(2.0, 0.0, &adapt_cost(skillnum, who, npc));
} /* adapt_script1 */
 ...
 */
public void
adapt_cost(int skillnum, object who, object npc)
{
    int     attitude = Adapt_Map[skillnum][MERC_ADAPT_ATTITUDE];
    string  a_desc = Adapt_Map[skillnum][MERC_ADAPT_DESC];
    string  skill = SS_SKILL_DESC[skillnum][0];
 ...
 */
public void
adapt_train1(int skillnum, object who, object npc)
{
    int     attitude = Adapt_Map[skillnum][MERC_ADAPT_ATTITUDE];
    string  a_desc = Adapt_Map[skillnum][MERC_ADAPT_DESC];
    string  skill = SS_SKILL_DESC[skillnum][0];
 ...
 */
public void
merc_pay1(int skillnum, object who, object npc)
{
    ...
    int guru_skill = skill_extra_guru(who);
    if (guru_skill != -1 && guru_skill != skillnum &&
        who->query_adapt_enhanced_base_skill(skillnum) > ADAPT_NON_FOCUS_SKILL_LEVEL)
    {
        who->catch_tell("You are not able to focus in more than"
          + " one skill to train beyond " + sk_rank(ADAPT_NON_FOCUS_SKILL_LEVEL)
          + ". You need to clear your focused adaptation in"
          + Adapt_Map[guru_skill][MERC_ADAPT_DESC] + " in order"
          + " to proceed in your training further.\n");
        return;
    }
    ...
    which_cost = ( (member_array(skillnum,
    ...
    set_alarm(2.0, 0.0, &tell_room(environment(who), QCTNAME(who)
      + " begins training how to " + Adapt_Map[skillnum][MERC_ADAPT_DESC]
      + " properly.\n", who));
    who->add_prop(TRAINING_PROP, 1);
    who->remove_prop(MERC_COST_PROP);

    if (!IN_ARRAY(skillnum,
    ...
    set_alarm(3.0, 0.0, &adapt_increase(skillnum, who, npc));
 ...
 * Arguments    :        object who   - the player
 *                       int skillnum - the skill they adapted
 */
public void
check_new_adapt_msg(object who, int skillnum)
{
    int     a_lvl;
    string  a_txt;
 ...
 */
public void
adapt_increase(int skillnum, object who, object npc, int adapt_alarm)
{
    ...
    if (npc->query_attack())
    {
        ...
        adapt_cleanup(who, npc, skillnum, "(combat)");
        return;
    }
    ...
    if (environment(who) != environment(npc))
    {
        ...
        adapt_cleanup(who, npc, skillnum, "(left room)");
        return;
    }
        ...
        who->query_prop(TRAINING_PROP) == "stop")
    {
        ...
        adapt_cleanup(who, npc, skillnum, "(typed 'stop')");
        return;
    }
    ...
    if (member_array(skillnum, freebie_skills) > -1)
    {
        if (!who->query_base_skill(skillnum))
        {
            if (amount > 29)
            {
                ...
                adapt_cleanup(who, npc, skillnum, "(freebie skill capped)");
                return;
            }
        }
    }
    ...
    if (skillnum == SS_BLOCK)
    {
        if (amount > 19)
        {
            ...
            adapt_cleanup(who, npc, skillnum, "(reached block cap)");
            return;
        }
    }
    ...
    if (set_skill_enhance(who, skillnum, 1))
    {
        adapt_cleanup(who, npc, skillnum);
    }
    else
    {
        set_alarm(3.0, 0.0, &adapt_increase(skillnum, who, npc));
 ...
 *                       int skillnum - the number of the skill
 *                       string ended - the reason training ended
 */
public void
adapt_cleanup(object who, object npc, int skillnum, string ended = "")
{
    who->remove_prop(TRAINING_PROP);
    npc->remove_prop(ADAPTING_PROP);
    check_new_adapt_msg(who, skillnum);

    write_file(LOG_DIR + "adapt_log",
        TIME2FORMAT(time(), "mm/dd/yyyy") + " ("
      + ctime(time())[11..18] + ") "
      + capitalize(who->query_real_name()) 
      + " raised " + upper_case(SS_SKILL_DESC[skillnum][0]) + " by "
...
/*
 * Function name:        skill_extra_guru
 * Description  :        Checks if there is 
 * Arguments    :        object who - the player
 * Returns      :        int - the skill num, or -1
 */
public int
skill_extra_guru(object who)
{
    int     *v_skills = query_skillnum_index(),
             total = 0;
    mapping  enhance_map;
    ...
    foreach (int skillnum: v_skills)
    {
        if (enhance_map[skillnum] > 0 &&
          (who->query_base_skill(skillnum) + enhance_map[skillnum]) > ADAPT_NON_FOCUS_SKILL_LEVEL)
        {
            return skillnum;
        }
    }
...
static mapping ss_skills = SS_SKILL_DESC;

/*
 * Function name: can_adapt_sufficient_stat
 * Description  : Check if a player can train a skill to a certain level.
 * Returns      : -1 on success on failure the stat number which is 
 *                limiting the player.
 */
int
can_adapt_sufficient_stat(object who, int skill, int level)
{
    /* Always allow low levels to be taught. */
    if (level <= STAT_LIM_MIN_SKILL)
    {
        return -1;
    }
    ...
    int limit_stat = ss_skills[skill][2];
    int limit_weight = ss_skills[skill][3];

    if ((limit_stat >= 0) &&
        (limit_weight > 0) && 
        ((limit_weight * who->query_stat(limit_stat)) < (level * 100)))
    {
        return limit_stat;
    }
 ...
 */
public int
set_skill_enhance(object who, int skillnum, int amount)
{
    mapping en_mapping;
    int     old_amount;
    int     new_amount;
    ...
    guru_skill = skill_extra_guru(who);
    limit_to_sup_master = guru_skill != -1 && guru_skill != skillnum &&
        (who->query_adapt_enhanced_base_skill(skillnum) + amount) > ADAPT_NON_FOCUS_SKILL_LEVEL;
    if ( (who->query_adapt_enhanced_base_skill(skillnum) + amount) > 100 ||
        limit_to_sup_master)
    {
        if (limit_to_sup_master)
        {
            who->catch_tell("You have reached the limits of mastery"
              + " with your ability to " + Adapt_Map[skillnum][MERC_ADAPT_DESC]
              + ". You step back from your training and smile with"
              + " satisfaction, however to achieve complete mastery"
              + " of this particular lore you must clear your "
              + " adaptation to " + Adapt_Map[guru_skill][MERC_ADAPT_DESC]
              + ".\n");
        }
        else
        {
    ...
    /* =============================================================
     * We will make sure to limit adaptation to levels that a player
     * has the stats to actually train in any guild.
     */
    int limit_stat = can_adapt_sufficient_stat(who, skillnum, 
        who->query_adapt_enhanced_base_skill(skillnum) + amount);

    if (limit_stat >= 0)
    {
        who->catch_tell("You fail to adopt what the lesson would have"
          + " taught you due to insufficient " 
          + SD_LONG_STAT_DESC[limit_stat] + "."
          + " You step back from your training and consider the"
          + " growth you will need to achieve before learning more.\n");
        tell_room(environment(who), QCTNAME(who) + " steps back"
          + " from " + who->query_possessive() + " training and"
          + " frowns thoughtfully.\n", who);
        return 1;
    ...
    if (new_amount <= ADAPT_NON_FOCUS_SKILL_LEVEL)
    {
        who->catch_tell("Your ability to " + Adapt_Map[skillnum][MERC_ADAPT_DESC]
          + " rises to " + sk_rank(who->query_adapt_enhanced_base_skill(skillnum))
          + ".\n");
    }
    else
    {
 ...
 *                       int skillnum - the name of the skill to
 *                                      clear. If -1, all are cleared
 *                       int    leaving - true if the player is leaving
 *                                        the guild
 */
public void
clear_adapt(object who, int skillnum = -1, int leaving = 0)
{
    int     *v_skills;
    int      s_amount;
    mapping  enhance_map;
    ...
    {
        if (enhance_map[v_skill])
        {
            enhance_map[v_skill] = 0;
        }
    ...
    else
    {
        who->catch_tell("You clear your mind of "
          + ((skillnum != -1) ? "all adapted " + SS_SKILL_DESC[skillnum][0] 
          : "all previously adapted") + " skill allocations.\n");
    }
    ...
    write_file(LOG_DIR + "adapt_log",
      ...
      + capitalize(who->query_real_name()) 
      + " *** cleared " 
      + ((skillnum != -1) ? SS_SKILL_DESC[skillnum][0] : "all")
      + " allocations.\n");
} /* clear_adapt */

/*
 * Function name: fix_guild_member
 * Description:   Clean up guild member state such as over adapted skills
 */
void
fix_guild_member(object who)
{
    int guru_skill = skill_extra_guru(who);
    ...
    if (guru_skill == -1)
    {
        // Nothing over ADAPT_NON_FOCUS_SKILL_LEVEL level of skill so
        // we are done.
        return;
    }
    ...
    foreach (int skillnum: v_skills)
    {
        if (enhance_map[skillnum])
        {
            adapted_level = who->query_adapt_enhanced_base_skill(skillnum);
            int max_level = skillnum != guru_skill ? ADAPT_NON_FOCUS_SKILL_LEVEL : 100;
            if (adapted_level > max_level && skillnum != guru_skill)
            {
                int diff = adapted_level - max_level;
                
                enhance_map[skillnum] = enhance_map[skillnum] - diff;

                set_info(who, "skill_enhance", enhance_map);

                who->catch_msg("You sense that your ability to " +
                    Adapt_Map[skillnum][MERC_ADAPT_DESC] + " has degraded.\n");

                write_file(LOG_DIR + "adapt_fix_log",
                  TIME2FORMAT(time(), "mm/dd/yyyy") + " ("
                + ctime(time())[11..18] + ") "
                + capitalize(who->query_real_name()) 
                + " *** fixed " 
                + SS_SKILL_DESC[skillnum][0]
                + " from " + adapted_level + " to "
                + who->query_adapt_enhanced_base_skill(skillnum)
                + ".\n");
            }
        }
    }
...
print_columns(string *skill_names, object who, mapping enhance_map)
{
    ...
    if (n)
    {
        for (i = 0; i < half + odd_true; i++)
        {
            ...
            column2 = "";
            c1_val = enhance_map[SS_SKILL_LOOKUP[skill_names[i]]];
            ...
            if (half > i)
            {
                ...
                c2_val = enhance_map[SS_SKILL_LOOKUP[
                              skill_names[i + half + odd_true]]];
...
parse_affiliation(string arg, object who)
{
    ...
    case "school of high magic":
        txt += "the School of High Magic. May you help protect"
          + " fledgling Magi in their persuits of the arcane!";
        mtitle = "Guardian of Aspiring Magi and Protector of the Arcane";
        guild = "sohm";
        break;
    case "templar":
    case "templars":
    ...
    case "thanar guild":
        ...
        break;
    case "oots":
    case "order of the stars":
    case "holy order of the stars":
    case "mishakal":
    case "branchala":
    case "kiri-jolith":   
        if (who->query_race_name() == "goblin" ||
              who->query_race_name() == "hobgoblin" ||
              who->query_race_name() == "orc" ||
              who->query_race_name() == "ogre")
        {
            who->catch_tell("Your race is antithesis to the teachings " +
              "of the Holy Order of the Stars.\n");
            return 0;
            break;
        }

        txt += "the Holy Order of the Stars. May you help bring knowledge"
          + " of the return of the Pantheon of Light!";
        mtitle = "Devoted Advocate of the Pantheon of Light and Believer "
          + "in the Old Gods";
        guild = "oots";
        break;
    case "thief":
    ...
    case "wohs":
    case "high sorcery":
        txt += "the Wizards of High Sorcery. May your steel and their spells"
          + " conquer all in the advancement of mage craft!";
        mtitle = "Resolute Bulwark for the Orders of Magic and Protector of "
