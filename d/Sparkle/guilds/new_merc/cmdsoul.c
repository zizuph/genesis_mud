...
guildpunch(string str)
{
    ...
    if (!(this_player()->query_skill(SS_MERC_TACTICS_SKILL)))
    {
...
testo(string arg)
{
    ...
    if (!strlen(arg))
    {
        notify_fail("What npc do you want to test? Name it.\n");
        return 0;
    }
    ...
    if (npc)
    {
        write("We have an npc!\n");
        npc->command("say Ho ho ho! I'm the dude!");
    }
    else
    {
...
public int
skilltest(string arg)
{
    mapping a_map;

    if (!strlen(arg))
    {
        notify_fail("What skill do you want info on?\n");
        return 0;
    }
    ...
    write("To become adaptable in " + arg + " you must seek out "
      + capitalize(a_map[arg][MERC_ADAPT_TRAINER]) + " who you can find "
      + a_map[arg][MERC_ADAPT_LOCATION] + ".\n");
    return 1;
} /* skilltest */
