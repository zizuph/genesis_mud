...
adapt(string arg)
{
    ...
    // We use a member array here as the sword skill uses index 0
    // which would normally indicate failure
    if (member_array(arg, m_indices(SS_SKILL_LOOKUP)) == -1) 
    {
        if (arg == "list")
        {
        ...
        if (parse_command(arg, ({}),
            "'clear' %s", skill))
        {
            if (member_array(skill, m_indices(SS_SKILL_LOOKUP)) == -1) 
            {
            ...
            int clearskillnum = SS_SKILL_LOOKUP[skill];
            MERC_MASTER->clear_adapt(this_player(), clearskillnum);
            return 1;
        }
    ...
    int skillnum = SS_SKILL_LOOKUP[arg];
    a_map = MERC_MASTER->query_adapt_map();
    trainer_name = a_map[skillnum][MERC_ADAPT_TRAINER];
    ...
    MERC_MASTER->adapt_script1(skillnum, this_player(), npc);
    return 1;
} /* adapt */
...
mercpay(string arg)
{
    ...
    MERC_MASTER->merc_pay1(skillnum, this_player(), npc);
    return 1;
} /* mercpay */
...
set_adapt(string arg)
{
    ...
    int skillnum = SS_SKILL_LOOKUP[skill];
    if (query_verb() == "lessadapt")
    {
        MERC_MASTER->set_skill_enhance(this_player(), skillnum, -n);
        return 1;
    }
