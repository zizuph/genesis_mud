 ...
 *  Change log:
 *      04/20/2022 - Ckrik
 *          - Added query_adapt_enhanced_base_skill() to compute
 *            base skill + adapt enhanced skill in the safest way
 *          - Converted query_skill() masked function to
 *            return query_adapt_enhanced_base_skill() +
 *            query_skill_extra()
 */
#pragma strict_types
...
init_guild_member()
{
    ...
    /* clean up non-nerfed skill levels */
    MERC_MASTER->fix_guild_member(who);
} /* init_guild_member */


public int set_skill(int skill, int val)
{
    /* Fix adaptability if they trained after adapting. */
    int ret = shadow_who->set_skill(skill, val);
    MERC_MASTER->fix_guild_member(shadow_who);
    return ret;
}
...
/*
 * Function name:   query_adapt_enhanced_base_skill
 * Description:     Gives the value of a specific skill.
 * Arguments:       skill: The number of the skill to check
 * Returns:         The value of the base skill + adapt enhanced
 */
public int
query_adapt_enhanced_base_skill(int skill)
{
    mapping enhance_map;
    int base_skill = shadow_who->query_base_skill(skill);

    if (catch(enhance_map = MERC_MASTER->get_info(shadow_who, "skill_enhance")))
    {
        // If catch returns true, then an error occurred. Use mudlib base skill.
        return base_skill;
    }
    
    if (!mappingp(enhance_map) || base_skill >= 100 || !enhance_map[skill])
    {
        // Some weird condition or skill was not adapted. Use mudlib base skill.
        return base_skill;
    }
    
    // Cap enhanced base skill to 100 levels
...
{   
    // We mirror the mudlib and add base skill to extra skill
    return query_adapt_enhanced_base_skill(skill) +
        shadow_who->query_skill_extra(skill);
}
...
query_all_skill_types()
{
    ...
    skills = shadow_who->query_all_skill_types();

    // Ckrik - Mercs can adapt skills have base levels of 0 rather
    // than 20. For the mudlib display command to appear properly
    // we need to fix the skill types array.
    ...
    foreach (int skill : freebie_skills)
    {
        if (!shadow_who->query_base_skill(skill))
        {
            skills += ({ skill });
        }
    }
