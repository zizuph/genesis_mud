...
adjust_combat_on_move(int leave)
{
    ...
    shadow_who->catch_msg(enemy_ob->query_name()+ " is your enemy.\n" + 
        "Is taget present?: "+ enemy_pr + "\n");
...
int *query_mattack_stats()
{
    ...
    if (gMattack[0]) 
    {
         ...
         s += sprintf("Attacks: %5d (%3d miss, %3d no damage, %3d dodged and %4d blocked)\n", gMattack[0], gMattack[1], gMattack[2] , gMattack[3], gMattack[4]);
         s += sprintf("Accuracy: %3d%% (not counting no damage, block and dodge)\n", ((gMattack[0]-gMattack[1])*100)/gMattack[0]);
         s += sprintf("Accuracy: %3d%% (counting no damage, block and dodge)\n", ((gMattack[0]-(gMattack[1]+gMattack[2]+gMattack[3]+gMattack[4]))*100) / gMattack[0]);
...
int update_mattack_stats(int hit, int argF, int argS)
{
    ...
    if (hit)
    {
        ...
        else gMattack[5] = argF;

        if (argS) gMattack[6] = argS;
