/*
 * /d/Genesis/guilds/merc/merc_shadow.c - 2005
 * /d/Sparkle/guilds/mercs/merc_shadow.c - 2006
 *
 * Shadow for Mercenaries guild, by Morrigan 10/2001
 *
 * Adapted from code by Gwyneth and Auberon
 *
 * Changed and Moved to Sparkle by Lunatari 2006
 *
 * Modification log:
 * 2006-01-16 Lunatari 
 * - query_guild_title_occ: 
 *   Fixed loads of multiple queries of the same value.
 *   MCREST->query_status_lord was creating way too much load.
 *   because of being called 6 times and also doing 3 calls
 *   each time to MCREST->query_number_followers which had
 *   some for loop with sizeof(members), replaced with
 *   while loop and static size inside the loop. 
 * 2006-01-18 Lunatari
 * - adjust_combat_on_move: 
 *   Added the function and checked for Mattack props, since 
 *   it was possible to uber bounce with the special as it was
 *   coded before.
 * - reset_merc_special:
 *   Added this function to call it from the cmdsoul and from
 *   within adjust_combat_on_move to reset the special props.
 * - query_merc_shadow_version: 
 *   Added to be able to check from the CMDsoul what version
 *   the player actually had, useful when doing updates.
 */
...
#define LOG_MATTACK(x,y) write_file(LOG_DIR+(x->query_real_name()), ctime(time()) + "\n" + y)

int gLordStatus, gDefending, gDefending_alarm;
int *gMattack = ({ 0, 0, 0, 0, 0, 0, 0, });
string gLord;
object gDefended;
 ...
 *                int    transfer - true if we are transfering to new mercs
 *                                  default = 0
 * Returns       : 1 - success,  0 - fail
 */
public int
remove_occ_merc(object who, string reason, int transfer = 0)
{
    int ret, penalty;
    object emblem;
    ...
    /* This is suspended for now. Players can leave without penalty.
     *   - Gorboth (August 2009)
     *
    who->add_exp(-(penalty), 1);
     */

    /* We only want to clear the guildstat if the player is leaving the
     * guild. If, instead, they are transfering to the new mercenary
     * guild, we preserve the guildstat. (Gorboth, August 2009)
     */
    if (!transfer)
    {
...
init_merc_occ_member()
{
    ...
    shadow_who->catch_tell("\n\n\tThings are afoot in the Mercenary Guild!\n"
      + "\tIf you have not already done so, you should check the guild\n"
      + "\tboard for news and updates!\n\n");
    ...
    if (MJOIN->query_on_banish_list(shadow_who->query_name()))
    {
        ...
        if (file_name(environment(shadow_who)) == MSTART)
	{
...
{
    object enemy_ob = shadow_who->query_enemy();
    int    enemy_pr = 0;

    shadow_who->adjust_combat_on_move(leave);

    if (present(enemy_ob, ENV(shadow_who))) enemy_pr = 1;
...
/*
 * Functions: query_mattack_stats(), update_mattack_stats(int arg)
 *
 * Description: A simple little statistic function.
 */
int *query_mattack_stats()
{
    string s = "";
    ...
    if (gMattack[0]) 
    {
         ...
         s += sprintf("Attacks: %5d (%3d miss, %3d no damage, %3d dodged and %4d parried)\n", gMattack[0], gMattack[1], gMattack[2] , gMattack[3], gMattack[4]);
         s += sprintf("Accuracy: %3d%% (pure to_hit chance)\n", ((gMattack[0]-gMattack[1])*100)/gMattack[0]);
         s += sprintf("Accuracy: %3d%% (counting parry and dodge etc)\n", ((gMattack[0]-(gMattack[1]+gMattack[2]+gMattack[3]+gMattack[4]))*100) / gMattack[0]);

        LOG_MATTACK(shadow_who, s);
    }
...
/*
 * Function: update_mattack_stats(hit, argF, argS)
 */
int update_mattack_stats(int hit, int argF, int argS)
{
    gMattack[0]++;

    if (hit)
    {
        if(argF == 0) gMattack[2]++;
        else if(argF == -1) gMattack[3]++;
        else if(argF == -2) gMattack[4]++;
        else gMattack[5] = gMattack[5]+argF;

        if (argS) gMattack[6] = gMattack[6]+argS;
