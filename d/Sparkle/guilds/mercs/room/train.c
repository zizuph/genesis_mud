...
#define LEAVE_CAMP "/d/Genesis/start/human/wild2/n_road2"
...
/*
 * Lets take this out, since Maligant has left the camp.
 *   - Gorboth (August 2009)
 *
public void
reset_room()
...
leave_inv(object ob, object to)
{
    ...
    /* When leaving, make sure the 'to' exists before using it. */
    if (objectp(to) &&
        MJOIN->query_member(ob->query_real_name()))
    {
	if(MASTER_OB(to) == ENTER_CAMP)
	    ob->add_prop(MERC_CAMP_ENTRY, 1);
	
	if(MASTER_OB(to) == LEAVE_CAMP)
	    ob->add_prop(MERC_CAMP_ENTRY, 2);
    }
...
init()
{
    ...
    /* Lets block a few of these, since Maligant is no longer here.
     *   -Gorboth (August 2009)
     *
