...
/* Store support inheritance disabled by Eowul on 27-6-2003  */

#pragma save_binary

// inherit "/lib/store_support";  // Disabled by Eowul
inherit "/std/room";
...
create_room()
{
    set_short("Storeroom of the Mercenary Guild Lord Armoury");
    set_long("This is the storeroom of the Mercenary Lord Armoury "+
        ", used to store items that are sold there.\n");
    add_prop(ROOM_I_INSIDE, 1);

    add_exit(MROOM + "armoury", "east");
...
{
    ::enter_inv(ob, from);
//    store_update(ob); // Disabled by Eowul
}
