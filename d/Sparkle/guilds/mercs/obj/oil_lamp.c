 ...
 * 26/3/99  base value set to 70, query_torch_may_decay added  Uhclem
 * 10/5/08  changed parsing for fill/refill to be able to identify
 *          the correct lamp when you have multiple ones (Petros)
 */
...
do_refill(string str)
{
    ...
    if (parse_command(str , TP, "[the] %i 'with' [the] %i", lamps, oils))
    {
        index = lamps[0] < 0 ? (lamps[0] * -1) : lamps[0];
        if (sizeof(lamps) <= index)
        {
            NFN0("You can't " + query_verb() + " that.\n");
        }
        lamp = lamps[index];
        index = oils[0] < 0 ? (oils[0] * -1) : oils[0];
        if (sizeof(oils) <= index)
        {
            NFN0("You can't " + query_verb() + " using that.\n");
        }
        oil_flask = oils[index];
        if (lamp != this_object())
        {
            NFN0("You can't " + query_verb() + " that.\n");
        }
    ...
    else if (parse_command(str , TP, "[the] %i", lamps))
    {
        index = lamps[0] < 0 ? (lamps[0] * -1) : lamps[0];
        if (sizeof(lamps) <= index)
        {
