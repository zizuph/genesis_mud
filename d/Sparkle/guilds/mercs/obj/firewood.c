        ...
        switch (random(100)) 
        {
            ...
            case 9:
                for (i = 0; i < sizeof(rooms = room->query_exit_rooms()); i++)
                {
                    ...
                    {
                        tell_room(rooms[i], "You suddenly notice the "
                          + "smell of woodsmoke drifting here from "
                          + "somewhere close by.\n");
                    }
                }
                break;
...
extinguish(string str)
{
    ...
    if (environment(this_object()) == find_object("/d/Genesis/guilds/merc/room/center"))
        return 0;
