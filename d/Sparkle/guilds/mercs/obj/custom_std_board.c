...
check_reader(int note = 0)
{
    ...
    return (block_reader(note) ? no_special_fellow() : 0);
}
...
long(int start = 1, int end = msg_num)
{
    ...
    names = this_object()->query_allowed_names(name);

	while (++start < end)
	{
	    if (this_object()->query_custom_board() &&                              	                member_array(capitalize(query_author(start + 1)), 
        names) != -1)
	    {
		str += sprintf("%2d: %s\n", (start + 1), headers[start][0]);
	    }
            else if (name == query_author(start + 1))
	    {
		str += sprintf("%2d: %s\n", (start + 1), headers[start][0]);
	    }
