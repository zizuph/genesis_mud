...
{
    set_name("crate");
    add_name("_merc_guild_junk_box_");
    add_adj("junk");
    set_short("wooden crate");
    set_long("A wooden crate, adorned with the emblem of a red dragon. "+
	"Mercenaries can place their equipment for use by other mercenaries. "+
	"The word 'junk' has been painted into the side.\n");
...
leave_inv(object ob, object to)
{
    ...
    new_value = ob->query_prop(OBJ_I_VALUE);
...
enter_inv(object ob, object from)
{
    ...
    {
        // Only add the shadow to weapons and armours
        if (function_exists("create_weapon", ob) ||
