...
int
m_filter_armour_not_need_repair(object oArmour)
{
  return oArmour->query_condition() <= oArmour->query_repair();
}

int
m_filter_armour_need_repair(object oArmour)
{
  return oArmour->query_condition() > oArmour->query_repair();
}
...
int m_sort_container (object oContainer, string sDescription)
{
  ...
  aoGlowarm = filter (aoGlowarm, m_filter_armour_not_need_repair);
  ...
  if (sizeof (aoGlowarm) && (oContainer != oShelfGlowarm))
  {
    tell_room (oRoom,
               ...
               "shelf for prime condition armours. He then moves " +
  ...
  aoArmours = filter (aoArmours, m_filter_armour_need_repair);
  if (sizeof (aoArmours) && (oContainer != oShelfArmours))
  {
    tell_room (oRoom,
               ...
               sDescription + " that should be on the " +
               "shelf for armours needing repairs. He then moves " +
...
void m_sort ()
{
  ...
  if (!m_sort_container (oShelfGlowarm, "shelf for prime armours"))
  if (!m_sort_container (oShelfArmours, "shelf for armours needing repair"))
