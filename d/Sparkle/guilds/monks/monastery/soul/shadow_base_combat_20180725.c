...
static private int iAcquiredHit = 0;
static private int iAcquiredPen = 0;
...
mixed m_may_special_attack (object oWhom, string sAttack)
{
  ...
  if (stringp (xResult = oWhom->query_prop (OBJ_M_NO_ATTACK)) ||
      stringp (xResult = environment (oWhom)->query_prop (ROOM_M_NO_ATTACK)))
  {
    return (xResult);
  }
  else if (xResult)
