 ...
 * 20181014 Ckrik       Fix no logging when player quits in room
 *
 */
...
private static mapping mRoomTakeText = ([ ]);
private static mapping mRoomDrop = ([ ]);
private static mapping mRoomDropText = ([ ]);
...
void create_room ()
{
  // The order of cloning here is made so that the 
  // glow shelf is first, top shelf is also the second,
  // the middle shelf is third and the bottom shelf is fourth.
  ...
  oShelfArmours->m_configure_shelf (({ "top", "topmost", "armour", "armours",
        "repair", "armours needing repair" }),
                                    "shelf for armours needing repair", "Topmost is the shelf for armours needing repair");
  oShelfGlowarm = dg_clone_here (M_PATH_OBJ + "/armoury_shelf_glw");
  oShelfGlowarm->m_configure_shelf (({ "new", "east", "eastern", 
        "prime", "armour", "armours" }),
                                    "shelf for prime armours", "on the eastern wall is the shelf for prime armours");
...
void m_log_transfer (object oWho, string sText, string sDirection, int t)
{
  ...
  if (present (oGuardian))
  {
    ...
    sMessage = ctime(t) + " - " + capitalize (sName) + " " +
               sDirection + " " + sText + ".";
    //find_player("ckrik")->catch_msg(sMessage + "\n");
...
void m_log_player (object oWho)
{
  ...
  string sTake = mRoomTakeText [sWho];
  string sDrop = mRoomDropText [sWho];
  object* aoTake = mRoomTake [sWho];
  object* aoDrop = mRoomDrop [sWho];
  int t = time();

  if (sizeof(aoTake) > 0)
  {
    m_log_transfer (oWho, sTake, "took", t);
  }
  if (sizeof(aoDrop) > 0)
  {
    m_log_transfer (oWho, sDrop, "left", t);
  }

  m_delkey (mRoomTake, sWho);
  m_delkey (mRoomDrop, sWho);
}
...
void m_observe_take (object oWho, object oWhat)
{
  ...
  if (living (oWho))
  {
    ...
    mRoomTakeText [sWho] = COMPOSITE_DEAD (mRoomTake [sWho]);
    mRoomDropText [sWho] = COMPOSITE_DEAD (mRoomDrop [sWho]);
  }
}
