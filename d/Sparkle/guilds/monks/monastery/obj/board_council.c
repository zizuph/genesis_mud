 ...
 * 02/06/13 Manat       Moved to council directory
...
void create_board ()
{
    set_board_name (M_PATH_BOARD_COUNCIL);
    set_num_notes (60);
    set_silent (1);
    set_show_lvl (0);
    set_keep_discarded (1);
    set_remove_str ("Only guildmasters and archwizards " +
                    "may remove notes from this board.\n");

    set_name ("board");
    set_adj (({ "message" }));
    add_adj (({ "private" }));
    set_short ("message board");
    set_long ("It is the message board for the council. This " +
...
int allow_remove ()
{
  ...
  return (m_is_authorized_wizard (oHim) ||
