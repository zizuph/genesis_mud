...
public object * target_hitloc(object actor, string arg);
...
public void config_ability()
{
    ...
    set_ability_target(target_hitloc);
}
...
public mixed *
target_hitloc(object actor, string arg)
{
    object enemy;
    ...
    // No target let library pick a random hitloc
    if (!arg || !strlen(arg))
    {
        return ({ enemy });
    }
    
    int hitloc_id = get_hitloc_id(enemy, arg);
    if (hitloc_id == -1)
    {
        actor->catch_tell("Your enemy does not seem to have a " + arg +
            " to target your strike at!\n");
        return ({ });
    }
