...
#define LOG(x,y)       write_file("/d/Faerun/faerun_logs/" + x, extract(ctime(time()), \
                            4, 15) +" "+ y )            
...
query_add_oxygen_debt()
{
     ...
     s = player->query_skill(SS_SWIM);
     c = player->query_stat(SS_CON); 
...
do_drown()
{
     /*log_file("faerun_drowned", player->query_name() + " drowned in " + 
              file_name(environment(player)) + " at " + 
              ctime(time()) + ".\n");*/
              
     LOG("faerun_drowned", capitalize(this_player()->query_real_name()) +
                " drowned in " + 
              file_name(environment(player)) + " at " + 
              ctime(time()) + ".\n");
