 ...
 *
 *
 * 8/4/2004 -- Tilorop:  Made some minor define alterings in the code, but
 *                       the rest is intact. The default teamsize is set to 10
 *                       though it should most likely to be lowered. 
 *                       We may use this file if permitted as the domain's
 *                       standard team-code.
 *
 */
...
int gMax_team_size = 10, gNoTeamMember = 0;
...
/* set the max team size (default 10), don't join anymore if team
 * reaches this size.
 */
void
...
add_new_to_team(object who)
{
    ...
    if ((sizeof(this_object()->query_team_others()) + 1) >= gMax_team_size)
      return;

    if (sizeof(who->query_team_others()))
        return; /* this should never happen */

    if (this_object()->query_not_accept_member(who) || who->query_not_accept_member(this_object()))
        return;

    if (who->query_average_stat() > this_object()->query_average_stat() && !gNoTeamMember)
    {
        /* who is bigger than this_object() so he might become the leader instead */
        prospective = who->query_prospective_team();
        i = sizeof(prospective);
        while(i--)
            if (this_object()->id(prospective[i]))
            {
                others = this_object()->query_team_others();
                i = sizeof(others);
                while (i--)
                {
                    this_object()->team_leave(others[i]);
                    who->team_join(others[i]);
                }
                who->team_join(this_object());
                if(sizeof(others))
                {
                    tell_room(environment(this_object()), QCTNAME(who) + " became the leader of "+
                        QTNAME(this_object())+"'s team.\n");
                }
                else
                {
                    tell_room(environment(this_object()), QCTNAME(this_object()) + " joined the team of " +
                            QTNAME(who) + ".\n");
                }
                return;
            }
    }
    if (this_object()->team_join(who))
        tell_room(environment(this_object()), QCTNAME(who) + " joined the team of " +
                QTNAME(this_object()) + ".\n");
}
...
init_team_pickup()
{
    ...
    if (interactive(this_player()) || !sizeof(gProspective_team) ||
        this_object()->query_leader() || !environment(this_object()) || this_player()->query_no_team_member() ||
        sizeof(this_player()->query_team_others()))
      return;
    
    i = sizeof(gProspective_team);
    
    while(i--)
      if (this_player()->id(gProspective_team[i]))
      {
          set_alarm(0.0, 0.0, &add_new_to_team(this_player()));
          break;
      }
}
