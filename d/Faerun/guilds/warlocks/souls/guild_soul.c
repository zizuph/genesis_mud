...
s_wahelp(string str)
{
    ...
    {   
        ...
        +"| urhadoele| dried heart piece  virgin's blood        oregano    |\n"
    ...
    if (str == "emotes")
    {
        ...
        +"|                                                              |\n"
        +"| wareveal <target>   : Reveal the alien nature of the world   |\n"
        +"|______________________________________________________________|\n"); 
        }
    ...
    +"|                                         off for your spells.      |\n"
    +"|    > wascatter <skull/kneecap/heart>  : Break into pieces         |\n"
    +"|    > spelleffects                     : List active spells        |\n"
    +"|-------------------------------------------------------------------|\n"
    +"|    ** You can also address charms, chips and heart pieces         |\n"
    +"|       as 'ingredient' | 'ingredients' for various container       |\n"
    +"|       purposes.                                                   |\n"
...
create_charm(string str)
 {
    ...
    obj1[0]->reduce_heap_size();
    obj2[0]->reduce_heap_size();
    obj3[0]->reduce_heap_size();

    setuid();
    seteuid(getuid());

    charm = clone_object(WARLOCK_GUILDDIR + "obj/" + charm_ob_name);
    charm->move(this_player());
...
chip_components(string str)
{
    ...
    if (str == "skull" || str == "skulls")
    {
        ...
        if (comp->id("leftover") == 0)
        {
            write("You don't have any leftover skulls.\n");
    ...
    if (str == "kneecap" || str == "kneecaps")
    {
        ...
        if (comp->id("leftover") == 0)
        {
            write("You don't have any leftover kneecaps.\n");
    ...
    if (str == "heart" || str == "hearts")
    {
        ...
        if (comp->id("leftover") == 0)
        {
            write("You don't have any leftover hearts.\n");
...
int do_wareveal(string str)
{
    if (WARLOCK_MASTER->is_patr2_warlock(this_player()->query_name()))
    {
        write("With the power of Hastur, your body starts to bend the "
        +"time and space around you, reminding everyone that the world "
        +"is more alien and strange than they could ever imagine.\n");

        all("'s presence seem to bend and twist the very fabric of "
        +"space itself. For a moment, tiny black spots and transparent "
        +"threads suddenly becomes visible in every direction for a brief "
        +"moment, reminding you that the world may be stranger and more "
        +"alien than it seems.");
