/*
* /d/Faerun/guilds/warlocks/obj/war_heart_charm_7.c
*
...
query_spell_active(object player)
{
    ...
    if (player->query_guild_level_lay() < INFUSION_UNLOCK)
        return 0;
...
{
    if (str == "vibrating charm" 
        || str == "vibrating heart charm"
        || str == "urkenu"
        || str == "hardened vibrating heart charm")
    {
        write("You sacrifice a hardened vibrating heart charm!\n");
...
create_heap()
{
    ...
    add_name("_war_heart_charm_7");
    add_prop(HEAP_S_UNIQUE_ID, "_war_heart_charm_7");
    
    set_adj("hardened");
    add_adj("vibrating");
    add_adj("heart");
    
    set_short("hardened vibrating heart charm");
    set_pshort("hardened vibrating heart charms");
    ...
    setup_spell_effect("urkenu", "_infusion_charge", SPELL_STD_CHARGE);
}
