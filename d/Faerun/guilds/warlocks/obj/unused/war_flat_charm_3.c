/*
* /d/Faerun/guilds/warlocks/obj/war_flat_charm_3.c
*
...
query_spell_active(object player)
{
    ...
    if (player->query_guild_level_lay() < GATE_UNLOCK)
        return 0;
...
{
    if (str == "rune-carved charm" 
        || str == "rune-carved bone charm"
        || str == "flat rune-carved bone charm")
    {
        write("You sacrfice a flat rune-carved bone charm to "
        +"your patron!\n");
...
create_heap()
{
    ...
    set_name("charm");
    set_pname("charms");
    add_name("_war_flat_charm_3");
    add_prop(HEAP_S_UNIQUE_ID, "_war_flat_charm_3");
    
    set_adj("flat");
    add_adj("rune-carved");
    add_adj("bone");
    
    set_short("flat rune-carved bone charm");
    set_pshort("flat rune-carved bone charms");
    ...
    +"rune of power is carved at both sides of "
    +"the charm.\n");
    ...
    setup_spell_effect("urmhol", "_gate_charge", SPELL_STD_CHARGE);
}
