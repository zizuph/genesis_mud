/*
* /d/Faerun/guilds/warlocks/obj/war_flat_charm_1.c
*
...
query_spell_active(object player)
{
    ...
    if (player->query_guild_level_lay() < WARP_REALITY_UNLOCK)
        return 0;
...
{
    if (str == "snow-white charm" 
        || str == "snow-white bone charm"
        || str == "uralane"
        || str == "flat snow-white bone charm")
    {
        write("You sacrifice a flat show-white bone charm!\n");
...
create_heap()
{
    ...
    add_name("_war_flat_charm_1");
    add_prop(HEAP_S_UNIQUE_ID, "_war_flat_charm_1");
    
    set_adj("flat");
    add_adj("snow-white");
    add_adj("bone");
    
    set_short("flat snow-white bone charm");
    set_pshort("flat snow-white bone charms");
    set_long("This flat magical charm is roughly rectangular in "
    +"shape. It measures about two inches in lenght, one inch "
    +"in width, and a quarter of an inch in thickness. A vague "
    +"arcane symbol of power is carved at both sides of "
    +"the snow-white charm.\n");
    ...
    setup_spell_effect("uralane", "_warp-reality_charge", SPELL_STD_CHARGE);
}
