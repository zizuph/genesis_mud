/*
* /d/Faerun/guilds/warlocks/obj/war_flat_charm_4.c
*
...
query_spell_active(object player)
{
    ...
    if (player->query_guild_level_lay() < FALSELIFE_UNLOCK)
        return 0;
...
{
    if (str == "misty charm" 
        || str == "misty bone charm"
        || str == "urbienc"
        || str == "flat misty bone charm")
    {
        write("You sacrifice a flat misty bone charm!\n");
...
create_heap()
{
    ...
    add_name("_war_flat_charm_4");
    add_prop(HEAP_S_UNIQUE_ID, "_war_flat_charm_4");
    
    set_adj("flat");
    add_adj("misty");
    add_adj("bone");
    
    set_short("flat misty bone charm");
    set_pshort("flat misty bone charms");
    ...
    +"mist-encovered arcane symbol of power is carved at both sides of "
    +"the charm.\n");
    ...
    setup_spell_effect("urbienc", "_falselife_charge", SPELL_STD_CHARGE);
}
