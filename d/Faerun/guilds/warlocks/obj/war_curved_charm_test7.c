/*
* /d/Faerun/guilds/warlocks/obj/war_curved_charm_6.c
*
...
query_spell_active(object player)
{
    ...
    if (player->query_guild_level_lay() < NUKE_UNLOCK)
        return 0;
...
{
    if (str == "cracked charm" 
        || str == "cracked bone charm"
        || str == "urchem"
        || str == "curved cracked bone charm")
    {
        write("You sacrifice a curved cracked bone charm!\n");
...
create_heap(string charmname)
{
    ...
    add_name("_war_curved_charm_6");
    add_prop(HEAP_S_UNIQUE_ID, "_war_curved_charm_6");
    
    set_adj("curved");
    add_adj("cracked");
    add_adj("bone");
    
    set_short("curved cracked bone charm");
    set_pshort("curved cracked bone charms");
    ...
    setup_spell_effect("urchem", "_mask_charge", SPELL_STD_CHARGE);
