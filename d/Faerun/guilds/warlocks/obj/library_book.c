/*
*  /d/Faerun/guilds/warlocks/obj/library_book.c 
*
*  Book that's being used in my standard library implementation.
*
*  Created by Linor, 24-11-2003
*  Ported from Angalon to Genesis by Eowul, 30-8-2004
*  Borrowed for the Warlock guild - Nerull.
*/
...
void create_scroll()
{
    ...
    add_name("_linor_library_book");

    text = "";

    add_prop(OBJ_M_NO_DROP, "Return it to the library instead "
    +"of dropping it.\n");
...
* Description  : Set the object pointer to the library this book 
*                belongs to
...
int return_library_book()
{
    ...
    if (!strlen(book_id))
    {
        write("You must submit your " 
        + book_type + " to add it to the " 
        +"library.\n");
        return 0;
    }
...
int submit_library_book()
{
    ...
    if (!strlen(text))
    {
        write("The " + book_type + " titled '" 
        + title + "' has no contents.\n");
        return 0;
    }
...
string long(string str, object for_obj)
{
    ...
    // Add extra commands
    result += "The following commands are available: 'title " 
    + book_type +
