/*
* /d/Faerun/guilds/warlocks/obj/war_curved_charm_4.c
*
...
query_spell_active(object player)
{
    ...
    if (player->query_guild_level_lay() < FEAR_UNLOCK)
        return 0;
...
{
    if (str == "shadowy charm" 
        || str == "shadowy bone charm"
        || str == "urgashe"
        || str == "curved shadowy bone charm")
    {
        write("You sacrifice a curved shadowy bone charm!\n");
...
create_heap()
{
    ...
    set_pname("charms");
    add_name("urgashe");
    add_name("_war_curved_charm_4");
    add_name("_spell_ingredient");
    add_prop(HEAP_S_UNIQUE_ID, "_war_curved_charm_4");
    
    set_adj("curved");
    add_adj("shadowy");
    add_adj("bone");
    
    set_short("curved shadowy bone charm");
    set_pshort("curved shadowy bone charms");
    ...
    +"arcane symbol of power is floating as tiny shadows at both sides of "
    +"the charm.\n");
    ...
    setup_spell_effect("urgashe", "_dread_charge", SPELL_STD_CHARGE);
}
