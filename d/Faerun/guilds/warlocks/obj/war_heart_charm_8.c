/*
* /d/Faerun/guilds/warlocks/obj/war_heart_charm_8.c
*
...
query_spell_active(object player)
{
    ...
    if (player->query_guild_level_lay() < SCRY_UNLOCK)
        return 0;
...
{
    if (str == "smoky charm" 
        || str == "smoky heart charm"
        || str == "urlob"
        || str == "hardened smoky heart charm")
    {
        write("You sacrifice a hardened smoky heart charm!\n");
...
create_heap()
{
    ...
    add_name("_war_heart_charm_8");
    add_prop(HEAP_S_UNIQUE_ID, "_war_heart_charm_8");
    
    set_adj("hardened");
    add_adj("smoky");
    add_adj("heart");
    
    set_short("hardened smoky heart charm");
    set_pshort("hardened smoky heart charms");
    ...
    setup_spell_effect("urlob", "_scry_charge", SPELL_STD_CHARGE);
}
