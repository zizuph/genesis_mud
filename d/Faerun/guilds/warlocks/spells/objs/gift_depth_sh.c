/*
 * Support Shadow for the Gift of Depth Spell
 *
 ...
 * Borrowed by Nerull, 2018 for Warlocks.
...
query_prop(string prop)
{
    ...
    if (intp(result))
    {
        return max(1, result + gift_depth_boost);
    }
    else 
    {
        return 1;
...
hook_spell_effect_started(object spell_object)
{
    ...
    if (objectp(target))
    {
        target->catch_tell("Your lungs temporary change. You can "
        +"now breathe under water.\n");
...
hook_spell_effect_ended(object spell_object)
{
    ...
    if (objectp(target))
    {
        target->catch_tell("Your lungs turn back to normal. You can "
...
hook_spell_effect_warn_fading(object spell_object)
{
    ...
    if (objectp(target))
    {
        target->catch_tell("You feel Urfenn begins to "
            + "fade.\n");
    }
}
