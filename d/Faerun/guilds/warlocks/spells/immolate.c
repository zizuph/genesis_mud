/*
 * Immolate - Sacrificial spell for Warlocks
 * By Nerull, 2018
 * 
...
config_harm_spell()
{
    set_spell_name("nessusari");
    set_spell_desc(" - Immolate: Combust and collect souls "
    +"from prisoners/girls/boys/orphans");
...
int execute = 0)
{
    ...
    if ((target->id("girl")) || (target->id("boy")) || (target->id("orphan")) 
    || (target->id("prisoner")))
    {
    ...
    +"prisoners, girls, boys and orphans.\n");
...
concentrate_msg(object caster, mixed * targets, string arg)
{
    ...
    +"power from them as you prepare to Immolate and collect the "
    +"screaming soul of your weak victim!\n";
...
desc_harm_damage(object caster, object target, mixed *result)
{
    ...
    caster->catch_msg("With the power of your patron, you instantly "
    +"combust "
    +QTNAME(target)+" in the flames of Hell! "
    +QTNAME(target)+" screams loudly in terrible agony as the "
    +"intense flames sear wither flesh! In the exact moment "
    +QTNAME(target)+" expires, you quickly snap and entrap "
    +target->query_possessive()+" soul within yours with the "
    +"aid of magic!\n"); 
    
    target->tell_watcher(QCTNAME(caster)+" grins devilishly at "
    +QTNAME(target)+", which suddenly combust "
    +"into hellish flames! "+QCTNAME(target)+" screams loudly "
    +"in terrible agony as the intense flames sear and wither "
    +"flesh! In the exact moment when "+QTNAME(target)
    +" expires, "+QTNAME(caster)+" collects "
    +target->query_possessive()+" soul!\n\nFor performing "
    +"this ruthless act, your "
