/*
* /d/Faerun/guilds/black_flame/obj/reconstitute_vial.c
*
...
{
    if (str == "luminous vial" 
        || str == "tiny luminous vial"
		|| str == "tiny vial"
		|| str == "vial")
    {
        write("You consume a tiny luminous vial!\n");
...
create_heap()
{
	...
	set_name("_reconstitute_vial");
    add_name("convalesce");
    add_name("vial");
    set_pname("vials");
    add_name("_spell_ingredient");
    add_prop(HEAP_S_UNIQUE_ID, "_reconstitute_vial");
    
    set_adj("tiny");
    add_adj("luminous");
    
    set_short("tiny luminous vial");
    set_pshort("tiny luminous vials");
    ...
    setup_spell_effect("reconstitute", "_reconstitute_charge", SPELL_STD_CHARGE);
}
