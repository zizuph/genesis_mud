/*
* /d/Faerun/guilds/black_flame/obj/flashbang_vial.c
*
...
{
    if (str == "glowing vial" 
        || str == "tiny glowing vial"
		|| str == "tiny vial"
		|| str == "vial")
    {
        write("You consume a tiny glowing vial!\n");
...
create_heap()
{
	...
	set_name("_flashbang_vial");
    add_name("flare");
    add_name("vial");
    set_pname("vials");
    add_name("_spell_ingredient");
    add_prop(HEAP_S_UNIQUE_ID, "_flashbang_vial");
    
    set_adj("tiny");
    add_adj("glowing");
    
    set_short("tiny glowing vial");
    set_pshort("tiny glowing vials");
    ...
    setup_spell_effect("flashbang", "_flashbang_charge", SPELL_STD_CHARGE);
}
