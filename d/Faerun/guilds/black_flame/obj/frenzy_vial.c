/*
* /d/Faerun/guilds/black_flame/obj/frenzy_vial.c
*
...
{
    if (str == "metallic vial" 
        || str == "tiny metallic vial"
		|| str == "tiny vial"
		|| str == "vial")
    {
        write("You consume a tiny metallic vial!\n");
...
create_heap()
{
	...
	set_name("_frenzy_vial");
    add_name("fury");
    add_name("vial");
    set_pname("vials");
    add_name("_spell_ingredient");
    add_prop(HEAP_S_UNIQUE_ID, "_frenzy_vial");
    
    set_adj("tiny");
    add_adj("metallic");
    
    set_short("tiny metallic vial");
    set_pshort("tiny metallic vials");
    ...
    setup_spell_effect("frenzy", "_frenzy_charge", SPELL_STD_CHARGE);
}
