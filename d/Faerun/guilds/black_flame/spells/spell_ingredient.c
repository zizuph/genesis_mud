...
/*
 *  This heap is created as a way for black flame viasl to grant multiple castings
 *  of a spell.
 */
...
create_heap() 
{
    ...
    add_prop(HEAP_S_UNIQUE_ID, "_bf_spell_ingredient");
...
configure_spell_ingredient(string name, string *spells)
{
    ...
    set_name(name);
    add_prop(HEAP_S_UNIQUE_ID, "_bf_spell_ingredient" + name);
}
