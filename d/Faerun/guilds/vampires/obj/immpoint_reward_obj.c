/*
 *  /d/Faerun/guilds/vampires/obj/immpoint_reward_obj.c
 *
 *  Nerull, 2021
 *
 */
...
create_object()
{
    set_name("immpoint_reward_obj");
    add_adj( ({"moldy"}) );

    set_short("moldy immpoint_reward_obj");
    set_long("This is a vampire immpoint_reward_obj.\n");

    add_prop(OBJ_S_WIZINFO,"A vampire immpoint reward object.\n");
...
{
    if (player->query_guild_name_lay() != LAY_VAMPIRES_GUILD_NAME)
    {
        remove_object();
    ...
    int gagg = VAMPIRES_MAPPINGS->query_vampires_resilience(player->query_name());
    ...
    VAMPIRES_MAPPINGS->set_vampires_resilience(player->query_name(), newresi);
    
    SURVIVEBURN_LOG("survivedburn_log",
        "The Vampire (Spawn) " +player->query_name() 
        +" received 1 lost immpoint, plus other immpoints "+gagg+", of a total of "+newresi+".\n");
