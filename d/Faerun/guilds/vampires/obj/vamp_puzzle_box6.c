/* /d/Faerun/guilds/vampires/obj/vamp_puzzle_box6.c
 * 
 * Nerull 2021
 */
...
{  
    ...
    set_short("small bronze medallion");
       
    set_long("The medallion measures about two inches in diameter and "
    +"is made of solid bronze. The chain is missing, and you can see "
    +"no motives or carvings on the front of the medallion. It is quite "
    +"shiny though, except for the dull backside.\n");

    add_item(({"backside"}),
        "Partly hidden on the backside of the "
...
press_button(string str)
{
    ...
    {         
        ...
        +"button at the backside of the "+short()+", but nothing "
    ...
    && (dial_nr3 == correct_nr3))
    {
        ...
        +"button at the backside of the "+short()+".\n");
        
        if (VAMPIRES_MAPPINGS->query_vampire_seeker(this_player()->query_name()) == 6)
        {       
            //write("Suddenly, your mind disconnects from reality!\n");
            ...
            if (!present("box6_paralyze", this_player()))
            {
                clone_object(BOX6_PARALYZE)->move(this_player(), 1);
            }
            
                        
            VAMPIRES_OBJHANDLER->set_box6_inactive();
