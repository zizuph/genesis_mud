/* /d/Faerun/guilds/vampires/obj/vamp_puzzle_box3.c
 * 
 * Nerull 2021
 */
...
check_me()
{
    if (VAMPIRES_MAPPINGS->query_vampire_seeker(this_player()->query_name()) != 3)
    {
        return 0;
    }
...
{  
    ...
    set_short("empty green bottle");
    set_pshort("empty green bottles");
       
    set_long("This is an empty bottle made of rough "
    +"glass, and has a thick bottom. It has a fainted green colour, and the cap "
    +"is gone. A quick glance "
    +"reveals that it may contain "
    +"some debris or maybe some moisture inside, but nothing else of "
    +"interest.\n");

    add_item(({"bottom"}),
        "Partly hidden at the bottom of the "
...
press_button(string str)
{
    ...
    {         
        ...
        +"button at the bottom of the "+short()+", but nothing "
    ...
    && (dial_nr3 == correct_nr3))
    {
        ...
        +"button at the bottom of the "+short()+".\n");
        ...
        {       
            ...
            VAMPIRES_MAPPINGS->set_vampires_seeker(this_player()->query_name(), 4);
            
            if (!present("box3_paralyze", this_player()))
            {
                clone_object(BOX3_PARALYZE)->move(this_player(), 1);
            }
            
                        
            VAMPIRES_OBJHANDLER->set_box3_inactive();
