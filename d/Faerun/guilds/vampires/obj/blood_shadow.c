 ...
 *
 * Removed alarm to start bleeding from bites 
...
#define VAMPIRE_BITE "vampire_bite"
...
public object set_killed_by_bite(object killer)
{
...
public void check_remove()
{
    ...
    {
        QSW->remove_subloc(VAMPIRE_BITE);
        remove_shadow();
    }
}
...
public void bite_location(int hitloc_id)
{
    if (!m_sizeof(Start_alarms) && !m_sizeof(End_alarms))
    {
        QSW->add_subloc(VAMPIRE_BITE,this_object());
    }
...
public object make_corpse()
{
    ...
    object shadow = clone_object(VAMPIRES_OBJ_DIR+"corpse_shadow");
...
public varargs mixed hit_me(int wcpen, int dt, object attacker, int 
attack_id, int target_hitloc = -1)
{
    if (target_hitloc==-1 && m_sizeof(End_alarms) && random(100) < 25)
    {
        target_hitloc=one_of_list(m_indexes(End_alarms));
    }
    return QSW->hit_me(wcpen,dt,attacker,attack_id,target_hitloc);
}
