/* /d/Faerun/guilds/vampires/obj/vamp_puzzle_box5.c
 * 
 * Nerull 2021
 */
...
check_me()
{
    if (VAMPIRES_MAPPINGS->query_vampire_seeker(this_player()->query_name()) != 5)
    {
        return 0;
    }
...
{  
    ...
    set_short("pair of rusty iron shackles");
    set_pshort("pairs of rusty iron shackles");
       
    set_long("Once forged by iron, these shackles has seen "
    +"better days. Popular for slavetrading and keeping "
    +"criminals restricted, these would cost a penny when "
    +"in prime condition. The inside of the shackles looks "
    +"more preserved than the rest of the shackles.\n");

    add_item(({"inside"}),
        "Partly hidden inside one of the "
...
press_button(string str)
{
    ...
    && (dial_nr3 == correct_nr3))
    {
        ...
        {       
            ...
            if (!present("box5_paralyze", this_player()))
            {
                clone_object(BOX5_PARALYZE)->move(this_player(), 1);
            }
            
                        
            VAMPIRES_OBJHANDLER->set_box5_inactive();
