...
remove_link()
{
    ...
    if (!gExploded)
    {
        ...
        if (!query_ghost())
        {
            ...
            vampire_msg = " and the link to your thrall dissipates.";
        }
...
start_watch()
{
    ...
    gVampire->catch_msg("You etablish a link to your thrall.\n");
...
watch_target()
{
    ...
    {
        gVampire->catch_msg("The link to your thrall fails for some unknown reason.\n");
    ...
    {
        txt = "You are trying to connect to your thrall ...\n";

        if (!target_env->query_prop(ROOM_I_KEBAB) && interactive(gTarget))
        {
            txt += "You feel the connection to your thrall is forming.\n";
    ...
    {
        if (!gFail)
            txt = "Your connection to your thrall is slightly unstable.\n";
        else
            txt = "";
        ...
        if (target_env->query_prop(ROOM_I_KEBAB) || !interactive(gTarget))
        {
            if (gFail++ > MAX_FAIL)
            {
                txt += "The link to your thrall fails for some unknown reason.\n";
                
                set_alarm(0.0, 0.0, remove_link);
            }
            else
            {
                txt += "The connection to your thrall is temporary lost, but "
