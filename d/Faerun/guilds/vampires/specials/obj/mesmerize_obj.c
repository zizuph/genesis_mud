...
create_effect()
{
    set_name("vampires_stun");
    add_name( ({"vampires_stun_object" }) );
    set_short("hidden vampires stun object");    
    set_long("This is a hidden vampires stun object.\n");
...
{
    player->catch_msg("You shrug off the attempt to mesmerize you.\n");
...
{
    player->catch_msg("You are no longer mesmerized! You feel like you can move freely again.\n");
...
hook_stun_begin(object attacker, object target)
{
    ...
    if (objectp(attacker))
    {            
        target->catch_msg(QCTNAME(attacker) + " mesmerizes you!\n");
        target->tell_watcher(QCTNAME(attacker) + " mesmerizes " + QCTNAME(target));
    }
    else
    {
        target->catch_msg("You have been mesmerized!\n");
        target->tell_watcher(QCTNAME(target) + " has been mesmerized!\n");
    }
}