...
int query_unmask_presence()
{
    return unmask_presence;
}


int set_unmask_presence()
{
    if (unmask_presence == 0)
    {
        shadow_who->remove_prop(LIVE_I_NEVERKNOWN);
    
        shadow_who->catch_tell("You unmask your presence. You will "
        +"no longer be dead to the world.\n");
    ...
    if (unmask_presence == 1)
    {
        shadow_who->add_prop(LIVE_I_NEVERKNOWN, 1);
    
        shadow_who->catch_tell("You mask your presence. You are now "
        +"dead to the world.\n");
    ...
    // Should not occur, but we reset the states nonetheless.
    unmask_presence = 0;
    shadow_who->add_prop(LIVE_I_NEVERKNOWN, 1);
...
do_vamp_drink()
{
    ...
    int thirstvalueaddocc = thirstvalueocc + 1;
    ...
    {      
        ...
        VAMPIRES_MAPPINGS->clean_thrall_pool(drainee->query_name());
        
        drainer->catch_tell("Your Immortalitypoints increases to "+vamppoints+".\n");
...
add_undead_shadow()
{
    ...
    // Full vampires are always unmasked.
    unmask_presence = 1;
...
///////////////////////// NO_XP and DO_DIE MODULE ////////////////////////
// Comment out these functions for the guild to operate normally in
// regards to death and xp.
// Vampires don't gain combat xp
/*public void
...
init_lay_shadow(string str)
{
    ...
    int stat_adjustment = 0;
    
    int vampire_age = VAMPIRES_MAPPINGS->query_vampires_gauge(who->query_real_name());
    
    
        // If NO_XP and DO_DIE MODULE is activated, we use this //
        
    if (VAMPIRES_MASTER->is_nonxp_vampire(shadow_who->query_name()))
    {
        /*switch(vampire_age)
        {
            case 0..72:
            ...
            who_avg = who->query_average();       
            differene = 160 - who_avg;        
            recalculatedstat = who_avg + difference;
            
            // AVG is capped at 160
            stat_adjustment = recalculatedstat;
            ...
            who_avg = who->query_average();       
            differene = 170 - who_avg;        
            recalculatedstat = who_avg + difference;
            
            // AVG is capped at 170
            stat_adjustment = recalculatedstat;
            ...
            who_avg = who->query_average();       
            differene = 180 - who_avg;        
            recalculatedstat = who_avg + difference;
            
            // AVG is capped at 180
            stat_adjustment = recalculatedstat;
            ...
            who_avg = who->query_average();       
            differene = 190 - who_avg;        
            recalculatedstat = who_avg + difference;
            
            // AVG is capped at 190
            stat_adjustment = recalculatedstat;
            ...
            who_avg = who->query_average();       
            differene = 200 - who_avg;        
            recalculatedstat = who_avg + difference;
            
            // AVG is capped at 200
            stat_adjustment = recalculatedstat;
            ...
            who_avg = who->query_average();       
            differene = 210 - who_avg;        
            recalculatedstat = who_avg + difference;
            
            // AVG is capped at 210
            stat_adjustment = recalculatedstat;
            ...
            who_avg = who->query_average();       
            differene = 220 - who_avg;        
            recalculatedstat = who_avg + difference;
            
            // AVG is capped at 220
            stat_adjustment = recalculatedstat;
            ...
            who_avg = who->query_average();       
            differene = 240 - who_avg;        
            recalculatedstat = who_avg + difference;
            
            // AVG is capped at 240
            stat_adjustment = recalculatedstat;
            ...
            who_avg = who->query_average();       
            differene = 260 - who_avg;        
            recalculatedstat = who_avg + difference;
            
            // AVG is capped at 260
            stat_adjustment = recalculatedstat;
            ...
            who_avg = who->query_average();       
            differene = 280 - who_avg;        
            recalculatedstat = who_avg + difference;
            
            // AVG is capped at 280
            stat_adjustment = recalculatedstat;
            ...
            // Caps at 2 years gameactivity
            case 17530..65000:
                   
            who_avg = who->query_average();       
            differene = 300 - who_avg;        
            recalculatedstat = who_avg + difference;
            
            // AVG is capped at 300
            stat_adjustment = recalculatedstat;
    ...
    who->set_stat_extra(SS_STR, stat_adjustment);
    who->set_stat_extra(SS_CON, stat_adjustment);
    who->set_stat_extra(SS_DEX, stat_adjustment);
    who->set_stat_extra(SS_INT, stat_adjustment);
    who->set_stat_extra(SS_DIS, stat_adjustment);
    who->set_stat_extra(SS_WIS, stat_adjustment); 
