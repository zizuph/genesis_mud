...
do_vamp_drink()
{
    ...
    int thirstvaluepreserve;
    
    thirstvaluepreserve = thirstvalueocc;
    ...
    if (drainsum >= 20 || present("_vampire_drain_cooldown_ob", drainee))
    {
        newsum = 20;
    ...
    if (drainee->query_hp() <= 0)
    {
        ...
        if (drainer->query_prop(VAMP_TURN_ON) == 1)
        {
            if (drainee->query_race() == "human")
            {
                ...
                if (present("vampthrall_amp_obj", drainee))
                {
                    drainer->catch_tell("Your target was weakened by an "
                    +"active vampire curse.\nTurning failed.\n");
...
add_undead_shadow()
{
    ...
    shadow_who->add_prop(LIVE_M_NO_SCRY, 1);
