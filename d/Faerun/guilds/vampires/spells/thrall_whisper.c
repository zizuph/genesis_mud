...
{
    set_spell_name("thrallwhisper");
    set_spell_desc(" - send a telepathic whisper to your thrall");
    set_spell_vocal(0);
...
is_valid_fartell_target(object caster, object target)
{
    ...
    string thrall_check = (caster->query_thrall());
    
    if (target->query_real_name() != thrall_check)
    {
        caster->catch_tell("That person is not your thrall.\n");
...
hook_send_gmcp_comms(object caster, object target, string message_text)
{
    ...
    caster->gmcp_comms(spell_name, 0, "You telepathically whisper to your thrall "
...
hook_do_fartell_spell(object caster, object target, string message_text)
{
    ...
    caster->catch_msg("You telepathically whisper to your thrall " 
...
hook_fartell_no_message(object caster)
{
    ...
    +"whisper to your thrall? (" + query_verb() + " <recipient> <message>)\n");
}
...
{
    caster->catch_tell("Which thrall do you want to telephatically whisper "
    +"to? (" + query_verb() + " <recipient> <message>)\n");
}
