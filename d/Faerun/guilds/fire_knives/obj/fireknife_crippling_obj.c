...
create_spell_object()
{
    set_name("_fireknife_crippling_obj");
...
setup_spell_effect()
{
    ...
    object slow_shadow = clone_object("/d/Faerun/guilds/fire_knives/specials/cripple_slow_sh");
	...
	set_alarm(10.0, 0.0, &slow_shadow->remove_slow_shadow());
...
hook_spell_effect_started()
{
    ...
    if (objectp(target))
    {
        target->catch_tell("Your feel slow, weak and disoriented.\n");
        tell_room(environment(target), QCTNAME(target) + " seems to be slow, weak "
            + "and disorinented.\n", ({ target }));
    }    
}
...
hook_spell_effect_ended()
{
    ...
    if (objectp(target))
    {
        ...
        tell_room(environment(target), QCTPNAME(target) + " seems "
            + "to be normal again.\n", ({ target }));
    }
}
...
hook_slow_start_moving(object player, string direction)
{
    player->catch_tell("You start to move towards the " + direction
        + " exit, but are slowed by the disorientation and "
		+"the weakened state.\n");
    tell_room(environment(player), QCTNAME(player) + " starts to move "
        + "towards the " + direction + " exit, but is slowed "
        + "due to weakness and disorientation.\n", ({ player }) );
}
