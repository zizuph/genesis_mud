...
* Member List Utility
*
* This is a utility object that combines the who lists from the Fire Knives.   
*
* Originally created by Petros, August 2010
* Borrowed by Nerull 2018.
*/
...
*/
public int
sort_occ_fireknife_ranks(object user1, object user2)
{
...
{
    return sort_occ_fireknife_ranks(user1, user2);
}
...
non_spec(object player, string spec)
{
    ...
    {
        case "checkmember":
        if (player->query_guild_name_occ() == FIREKNIVES_GUILD_NAME)
        {
            return 1;
        }
    }
...
format_members_presentation(object * members, string spec)
{
    ...
    foreach(object member : members)
    {
        ...
        string title = member->query_guild_title_occ_internal();
        
        if (FIREKNIVES_MASTER->is_occ_fireknife(member->query_name()))
        {
...
list_members()
{
    ...
    write("Fire Knives sharpening their blades:\n");
    write("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
    ...
    object * ld_members = 
    filter(ld_players, &operator( == )(FIREKNIVES_GUILD_NAME) @ 
    ...
    write("\nFire Knives currently hiding in the shadows:\n");
