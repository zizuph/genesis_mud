    ...
    int resist, int result)
{
    ...
    if (result > 0)
    {
        ...
        object gar_cooldown_obj;
       
        gar_cooldown_obj = clone_object(FIREKNIVES_GUILD_DIR 
        + "specials/obj/silence_cooldwn_obj");
        
        gar_cooldown_obj->set_bestower(actor->query_name());
        gar_cooldown_obj->move(actor, 1);
    }
    else
    {
		...
		object gar_cooldown_obj2;
       
        gar_cooldown_obj2 = clone_object(FIREKNIVES_GUILD_DIR 
        + "specials/obj/silence_cooldwn_obj");
        
        gar_cooldown_obj2->set_bestower(actor->query_name());
        gar_cooldown_obj2->move(actor, 1);
    }
}
...
hook_special_complete()
{
	...
	if (token = present("_silence_cooldwn_obj", this_player()))
	{   		
		token->remove_object();
	}
