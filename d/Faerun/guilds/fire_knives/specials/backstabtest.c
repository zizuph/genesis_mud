...
#define BACKSTABTEST_DEBUG_LOG(x,y)   write_file(FIREKNIVES_LOG_DIR + x, extract(ctime(time()), \
                            4, 15) +" "+ y )
...
config_ability()
{
    ...
    //set_combat_aid(70);
    set_combat_aid(169);
    set_tohit(110); // 84% chance of hitting
    set_dt(W_IMPALE);
                ...
                int dam, mixed * items, int result)
{
    ...
    BACKSTABTEST_DEBUG_LOG("backstabtest_debug_log",
        "FK " +this_player()->query_name() +"'s BS data: Maxdmg: "+max_dmg+", prc_dmg: "+prc_dmg+", wep_pen: "+wep_pen+", wep_lev: "+wep_lev+", wep_dmg: "+wep_dmg+".\n");
...
validate_requirements(object player)
{
    ...
    weaponob = player->query_weapon(W_LEFT);
    
    if(!objectp(weaponob))
    {
        player->catch_tell("You need a blade wielded in the left hand!\n");
...
check_valid_action(object player, mixed * targets, string arg, int execute)
{
    ...
    if (FIREKNIVES_MAPPINGS->query_assassin_rank(player->query_name()) < AB6)
    {
        player->catch_tell("You are not qualified to use this ability.\n");
    ...
    if (!player->query_skill(SS_BACKSTAB))
    {
...
public void hook_special_complete()
{
    ...
    write("You feel ready to execute backstabtest or any other primary ability again.\n");    
}
