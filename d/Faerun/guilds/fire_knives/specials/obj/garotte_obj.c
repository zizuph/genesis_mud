...
create_effect()
{
    set_name("fireknives_garotte");
    add_name( ({"fireknives_garotte_object" }) );
    set_short("hidden fireknives garotte object");    
    set_long("This is a hidden fireknives garotte object.\n");
...
start()
{
    ...
    if (objectp(enemy = environment(this_object())) && living(enemy))
    {
        ...
        + "specials/obj/garotte_sh");
        
        shadow->shadow_me(enemy);
        
        enemy->init_garotte_shadow();
        
        // 90 caid is 50 seconds duration. 
        set_alarm(SILENCE_DURATION, 0.0, &remove_object());
    }
