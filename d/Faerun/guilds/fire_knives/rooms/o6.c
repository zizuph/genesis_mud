/* /d/Faerun/guilds/fire_knives/rooms/o6.c
 *
 * Northeastern point in the sanctuary
 *
 * Nerull 2019
 *
 */
...
create_fireknives_room()
{
    ...
    set_short("Northwestern point in the sanctuary");
    
    set_long("The spacious, rectangular area revolves "
    +"around a statue centered atop "
    +"a marble block in the middle of "
    +"the scope of this area. It seems unfinished "
    +"and raw, yet the features are already "
    +"distinctive enough to make out. Spanning "
    +"around it is a circular concourse with "
    +"six narrow passageways opening past stoney "
    +"archways that seemed to have sustained "
    +"the passage of time. Between each of them "
    +"is a massive wall with an iron sconce from "
    +"which a thick, wooden torch hangs high, shedding off "
    +"light to the area. To the west past an "
    +"archway you can hear the whistling of blades "
    +"and its dull sounds as it hits against leather, "
    +"while to the north is the main "
    +"hallway. \n");

    add_item(({"area"}),
    "The area is spacious and rectangular in its form. "
    +"A dominant statue centered atop "
    +"a marble block is found in the middle of "
    +"it, while the concourse in which you are "
    +"in revolves around it in a circular fashion.\n");

    add_item(({"statue"}),
    "The statue seems unfinished, raw even, yet the "
    +"features are already distinctive "
    +"enough to depict a hooded man crouching down "
    +"as if ready to spring off in the "
    ...
    +"standing a tad behind the other and escaping "
    +"the cover of his long robes. In "
    +"his hands is a loaded crossbow.\n");

    add_item(({"block"}),
    "The marble block serves as a pedestal for the statue.\n");

    add_item(({"concourse"}),
    "The concourse revolves around the statue in a circular "
    +"fashion, its walls cut six times "
    +"by arching frames that allow passage "
    +"into other areas.\n");

    add_item(({"passageways","passageway"}),
    "The passageways lead into six different directions "
    +"around the area, past arching frames. "
    +"From this point you can reach three of "
    +"them: south further along the circular concourse,"
    +"and west into a training chamber.\n");
    ...
    add_exit(FIREKNIVES_ROOMS_DIR + "o7.c", "north");
    add_exit(FIREKNIVES_ROOMS_DIR + "ti", "west");
    add_exit(FIREKNIVES_ROOMS_DIR + "o5.c", "south");
