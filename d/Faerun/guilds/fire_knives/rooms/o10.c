...
create_fireknives_room()
{
    ...
    +"is tied. A single torch is found within a sconce hanging "
    +"from the northern "
    ...
    add_item(({"bollard"}),
    "A sturdy vertical post from which a rope hangs.\n");

    add_item(({"ground","floor"}),
    "The ground is of a cold, rough stone, and devoid of "
    +"any decorative functions. "
    +"at its center is a hole wide enough for you to crawl "
    +"through, into which a "
    +"rope is lost.\n");

    add_item(({"walls"}),
    "The walls are bare, without any decorations. Only "
    +"a single sconce with a torch "
    +"in it serves to beautify it somewhat, its light "
    +"dispersing the darkness that "
    +"would otherwise consume the area.\n");

    add_item(({"cell","room","area"}),
    "The cell is small and compact, separating the "
    +"long hallway to your east "
    +"from the door to your west.\n");

    add_item(({"yard"}),
    "You cannot see the yard from here, but it "
    +"lies beyond the door to the west, "
    +"which has now been sealed shut.\n");

    add_item(({"sconce","torch"}),
    "There is a single sconce hanging from the wall "
    +"with a lit torch resting in it. "
    +"The light is enough to brighten the otherwise dark room.\n");

    add_item(({"door","wooden door"}),
    "The wooden door lies to your west, thick, firm "
    +"and sealed shut by a big heavy "
    +"log lodged between it and the rest of the room. "
    +"It would lead outside to the "
    +"yard of the keep. \n");

    add_item(({"wind","winds"}),
    "The winds cannot be felt here, but their presence "
    +"is still obvious due to the "
    +"strong sounds coming from outside.\n");

    add_exit(FIREKNIVES_ROOMS_DIR + "o9", "east");
    add_exit(FIREKNIVES_ROOMS_DIR + "rope6", "down");
