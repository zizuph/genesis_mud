/* /d/Faerun/guilds/fire_knives/rooms/o5.c
 *
 * Southeastern point in the sanctuary
 *
 * Nerull 2019
 *
 */
...
create_fireknives_room()
{
    ...
    set_short("Southwestern point in the sanctuary");
    
    set_long("The spacious, rectangular area r"
    +"evolves around a statue centered atop "
    +"a marble block in the middle of the scope "
    +"of this area. It seems unfinished and "
    +"raw, yet the features are already distinctive "
    +"enough to make out. Spanning around "
    +"it is a circular concourse with six narrow "
    +"passageways opening past stoney archways "
    +"that seemed to have sustained the passage "
    +"of time. Between each of them is a massive "
    +"wall with an iron sconce from which a thick, "
    +"wooden torch hangs high, shedding off "
    +"light to the area. To the west past an archway "
    +"you sense a silence that seems "
    +"reverential, and various flickers of tiny "
    +"lights hanging low, while to the east "
    +"the concourse continues around the statue. \n");

    add_item(({"lights","tiny lights","flickers","flicker"}),
    "Various flickers of tiny lights can be seen to "
    +"the west, hanging low as if not from "
    +"the walls but rather from tables.\n");

    add_item(({"area"}),
    "The area is spacious and rectangular in its "
    +"form. A dominant statue centered atop "
    ...
    +"enough to depict a hooded man crouching "
    +"down as if ready to spring off in the "
    ...
    add_item(({"block"}),
    "The marble block serves as a pedestal for "
    +"the statue. At its base is an inscription.\n");
    ...
    "The passageways lead into six different directions "
    +"around the area, past arching frames. "
    +"From this point you can reach three of them: "
    +"north and east further along the circular "
    +"concourse, west into a library and south to a postoffice.\n");

    add_item(({"archway","archways","frames"}),
    "The archways crown the exits into other areas "
    +"covered from here by the darkness brought "
    ...
    add_exit(FIREKNIVES_ROOMS_DIR + "o6.c", "north");
    add_exit(FIREKNIVES_ROOMS_DIR + "o4.c", "east");
    add_exit(FIREKNIVES_ROOMS_DIR + "po", "south");
    add_exit(FIREKNIVES_ROOMS_DIR + "lb.c", "west");
