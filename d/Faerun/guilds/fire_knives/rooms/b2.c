/* /d/Faerun/guilds/fire_knives/rooms/b2.c
 *
 * Hallway between reception and Fist's room
 *
 * Nerull 2019
 *
 */
...
block_check()
{
    if (present("fireknives_visit_object_fist", this_player()))
    {
    ...
    {
        write("The guard dogs raise their heads to "
        +"look at you, then look down "
...
create_fireknives_room()
{
    ...
    +"Its walls are bare save for the torches resting in sconces in its walls. "
    +"A pair of guard dogs is resting here. "
    +"To the west the halls come to a halt upon a door. \n");    

    add_item(({"floor"}),
    "The room is covered in a simple black carpet the muffles all sound "
    +" and provides protection against the "
    +"coldness of the stone underneath it.\n");
    ...
    add_item(({"door"}),
    "The door lies to the east. It bears the symbol of a "
    +"gauntleted fist in its center.\n");

    add_exit(FIREKNIVES_ROOMS_DIR + "fist.c", "west", block_check);
    add_exit(FIREKNIVES_ROOMS_DIR + "reception.c", "east");
    reset_faerun_room();
}
