/* /d/Faerun/guilds/fire_knives/rooms/h6.c
 *
 * Around the column, east of the forges
 *
 * Nerull 2019
 *
 */
...
create_fireknives_room()
{
    ...
    +"of the wide marble column which dominates the center of the floor. The "
    +"western wall starts to bend southeastwards to the counterpoint of the "
    +"column front in a sharp turn. To the west a passage opens into "
    +"a small cubic room whence the soft flowing of water can be "
    +"heard. Soft carpets cover "
    +"the otherwise cold, stone floor. "
    +"A pair of guard dogs is resting here.\n");
    
    add_item(({"column","thick column","marble column",
    "thick marble column"}),
    "Before you directly to the east rises "
    +"from the bottom of the floor a tall "
    +"and wide marble column that claims all "
    +"around it to itself. It seems to "
    +"protrude from the very stone as if it had "
    +"grown from it or the halls in "
    +"which you walk around in were carved out of "
    ...
    +"They extend from this point to the north and southeast, while "
    +"to the west opens a passage to a warm, "
    +"stuff-aired space: the forges.\n");

    add_item(({"sconces","sconce"}),
    "The sconces are placed at an equal distant to "
    +"each other along the wall to the "
    +"southwest. Inside each one of them are torches.\n");
    ...
    "Inside each one of the sconces along the wall is "
    +"a torch, currently lit.\n");
    ...
    +"southwest is a lean, muscular man "
    +"of short black hair, trimmed goatee and moustache "
    +"with a grey tinged skin.\n"
    ...
    add_exit(FIREKNIVES_ROOMS_DIR + "contracts", "west", block_check);
    add_exit(FIREKNIVES_ROOMS_DIR + "h7.c", "north");
    add_exit(FIREKNIVES_ROOMS_DIR + "h5.c", "southeast");
