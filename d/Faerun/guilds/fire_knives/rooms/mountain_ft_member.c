...
int
do_gowest(string str)
{
    write("\n\nYou silently dive into the snow and vanish "
    +"westwards.\n\n");

    this_player()->move_living("M", "/d/Faerun/highroad/rooms/htr11", 1);
        
    tell_room("/d/Faerun/highroad/rooms/htr11", 
    "The snow moves slightly.\n", this_player());
...
create_fireknives_outdoor()
{
    ...
    +"reaching up to the clouds that fog the sky. "
    +"Flakes and layers of snow blow "
    ...
    +"trail. Here and there from the base upwards "
    +"jut out jagged stones that seem "
    ...
    +"and maintaining a steady height between "
    +"the new that accumulates and the old "
    ...
    "The base of the mountain is wide and extends "
    +"towards the east and south of you, "
    +"leaving a trail open to the west back whence "
    +"you came from. From this perspective "
    +"it seems almost vertical even if there is "
    +"a slight incline towards the top. The "
    +"mountain extends up to high heavens, its "
    +"peaks lost between clouds and fog.\n");
    ...
    +"the area and making your stance all the "
    +"harder to maintain. The otherwise "
    +"tolerable cold becomes bitter and stern "
    +"as it is slammed into everything with "
    +"full force.\n");

    add_item(({"track","tracks","prints","footprints","footprint","print"}),
    "The tracks made by your feet are soon thereafter covered by the winds "
    +"which buffet the mountain, throwing fresh "
    +"layers of snow upon everything.\n");

    add_item(({"path"}),
    "The path whence you came from leads back to the "
    +"west to a trail in the snow.\n");
    ...
    add_exit(FIREKNIVES_ROOMS_DIR + "rope_bottom", "east");
}
