/* /d/Faerun/guilds/fire_knives/rooms/ft.c
 *
 * Training Halls for Full Members
 *
 * Nerull 2019
 *
 */
...
block_check()
{
    /* Fire Knives Guru quest completed */
...
create_fireknives_room()
{
    ...
    +"padded bunks sewn together and maintained in "
    +"place by criss-crossing wooden planks that "
    +"are supported by the stone columns rising "
    +"from the ground and meeting the "
    +"ceiling. From the various planks, hanging by "
    +"thick iron chains, are sacks "
    +"and dummies for practice. The floor's "
    +"rectangular stones are rough and stained "
    +"with old sweat and blood. On it, to the "
    +"northwestern side is a delimited area "
    +"covered in mats and marked by thick square "
    +"blocks on the floor, while the walls "
    +"are bare, devoid of any decorations save "
    +"the markings of the passage of time. A pair "
    +"of guard dogs is resting here.\n");
     
    add_item(({"room","hall"}),
    "The hall is ample enough for various persons to "
    +"be within it, all doing heavy "
    +"movement. The divisions created by "
    +"the columns don't detract from the purpose "
    +"of the space, but rather compartamentalize it.\n");

    add_item(({"bunks"}),
    "The bunks are thick mats filled with a "
    +"deafening material, keeping both the sound "
    +"in and the cold out.\n");
    ...
    add_item(({"ceiling"}),
    "The ceiling is low, and artificial. The real "
    +"one being beyond the padded bunks that "
    +"have been sewn together and maintained in "
    +"place by the wooden planks that are "
    +"supported by the stone columns dividing the room in sections.\n");

    add_item(({"stone", "floor","ground","stones"}),
    "The floor is stained with old sweat and what "
    +"appears to be blood. The only protection "
    +"from it is in the sparring area to the northwestern "
    +"side of the room, which is marked "
    +"by a mat on the floor.\n");

    add_item(({"planks"}),
    "From the various planks hang sacks and dummies "
    +"for practice from thick iron chains.\n");

    add_item(({"columns","stone columns"}),
    "The thick colums divide the room in various "
    +"sections, allowing for different training "
    +"sessions to take place at the same time. They "
    +"are of a thick, cold stone.\n");

    add_item(({"chains","iron chains"}),
    "The iron chains fall heavily from the sturdy "
    +"wooden planks. From them hang dummies and "
    +"sacks on which you can practice your skills "
    +"when you don't have a sparring partner.\n");

    add_item(({"sacks", "dummies","training dummies"}),
    "The sacks are full of sand, making them heavy and "
    +"strong, while the dummies are wooden, "
    +"both ideal for their own specific style of "
    +"fighting being practiced. By the chains from "
    +"which they hang you can gather that they could "
    +"also be swung by someone wanting you to "
    +"practice how to protect yourself.\n");

    add_item(({"sweat","blood","stain","stains"}),
    "Darkened blotches are around the stone floor, hinting "
    +"at past sparring sessions that required "
    +"more than just a sharp mind.\n");

    add_item(({"area","delimited area"}),
    "The area to the northwest is delimited by blocks "
    +"of stone with a thick mat in the middle "
    +"making it look like a sparring one. Here you "
    +"can test out your skills against a willing "
    +"partner.\n");

    add_item(({"mat","mats"}),
    "The mats are thick and hard, but still softer than "
    +"the stone floor they cover. Their purpose "
    +"seems clear as does their wear, stained and "
    +"crusty as they are.\n ");

    add_item(({"block","blocks"}),
    "Thick cubic blocks are placed around the mat "
    +"delimiting the sparring area. They seem to have "
    +"been moved by a considerable effort due to their apparent weight.\n");

    add_item(({"walls"}),
    "The walls are devoid of any decorative traits, their "
    +"only distinguishing markings are those "
    +"of the passage of time.\n");
    
    create_skill_raise();
    set_up_skills();
    
    add_exit(FIREKNIVES_ROOMS_DIR + "h7", "east");
    add_exit(FIREKNIVES_ROOMS_DIR + "guru", "west", block_check);
...
set_up_skills()
{
    ...
    sk_add_train(SS_ACROBAT,         ({ to, to }), 0, 0, 70);

    to = "sneak";
    sk_add_train(SS_SNEAK,         ({ to, to }), 0, 0, 100);
    
    to = "hide";
    sk_add_train(SS_HIDE,         ({ to, to }), 0, 0, 100);
    
    to = "backstab";
    sk_add_train(SS_BACKSTAB,         ({ to, to }), 0, 0, 100);
...
sk_hook_raise_rank(int snum, int to_lev, int cost)
{
    ...
    this_player()->catch_msg("The slim stern-eyed female human teaches you to " 
    + sk_tdesc[snum][0] +" better.\n");

    write("You achieve the rank of " + rank + ".\n");

    say("The slim stern-eyed female human teaches "
...
reset_faerun_room()
{
    ...
    if (!kyri)
    {
        kyri = clone_object(FIREKNIVES_GUILD_DIR + "npc/kyri");
        kyri->move(this_object(), 1);
    }
}
