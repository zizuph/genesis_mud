 ...
 *
 * This shadow was originally coded by Arman. Thanks Arman!!!
 * Finwe, February 2016
 *
...
display_flicker_message(object target)
{
    ...
    switch (random(2))
    {
     case 0:
       caster_message = "Your surroundings are dimly illuminated by the pale "+
            "violet enchantment surrounding your " + shadow_who->short() + ".\n";
       watcher_message = "Your surroundings are dimly illuminated by the " +
            "pale violet enchantment surrounding " +
            "the " + shadow_who->short() + " held by " + QTNAME(owner)+ ".\n";
     break;
     case 1:
     default:
       caster_message = "Your " + shadow_who->short() + " flickers with a pale " +
             "violet glow.\n";
       watcher_message = "The " + shadow_who->short() + " held by " + QTNAME(owner)+
    ...
    {
        tell_room(environment(owner), "The " + shadow_who->short() + " flickers " +
             "with a pale violet glow.\n", ({ owner }), owner);
    }

    set_alarm(65.0, 0.0, &display_flicker_message(target));
}
...
hook_spell_effect_started(object spell_object)
{
    ...
    light_bonus = min(3, max(1, (power - 220) / 25));
    ...
    if (objectp(target))
    {
      if(living(owner))
      {
        caster->catch_msg("Your faerie fire enchantment on the " + 
            shadow_who->short() + " expires.\n");
        tell_room(owner, "The " +shadow_who->short()+ " is no longer outlined in a "+
            "violet glow.\n", ({ target }));
        tell_room(environment(owner), "The " + shadow_who->short() + " held by " 
            + QTNAME(owner)+ 
            " is no longer outlined in a violet glow.\n", ({ owner }));
      }
      else
      { 
        tell_room(environment(owner), "The " + shadow_who->short() + " in " 
