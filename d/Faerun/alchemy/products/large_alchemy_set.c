...
remove_set()
{
    tell_room(environment(this_object()), "The large alchemy set "
    +"suddenly explodes due to alchemical instabilities!\n");
...
create_receptacle()
{
    ...
    add_name("set");
    add_prop(HEAP_S_UNIQUE_ID, "large_alchemy_set");
    ...
    add_prop(HEAP_I_UNIT_WEIGHT, 15000);
    /* Volume of a single heap item */
    add_prop(HEAP_I_UNIT_VOLUME, 15000);
    /* Value of a single heap item */
    add_prop(HEAP_I_UNIT_VALUE, 20000);
    ...
    // Destroys itself after 24 hours.
    set_alarm(86400.0, 0.0, &remove_set());
}
...
do_processing()
{
    ...
    if (process_timer == 0)
    {
        write("You place the ingredients into "
        +"a container with volatile chemicals.\n");
    
        tell_room(environment(this_object()), QCTNAME(this_player())
        +" places "
        +"some ingredients into a container with volatile "
        +"chemicals.\n", this_player());        
    }
...
do_craftcomponents(string str)
 {
    ...
    {
        // Physical component
        obj1_component = filter(deep_inventory(this_player()),
        &operator(==)(query_component1()) @&->query_name());

        if (!sizeof(obj1_component))
        {
            write("You are missing a component!\n");
            return 1;
        }
        ...
        if (large_flasks == 0)
        {
            write("There are no large flasks left to use.\n");
            return 1;
        }
    ...
    if (str == "rust powder")
    {
        ...
        {
            write("There are no more volatile chemicals left.\n");
            return 1;
        }
