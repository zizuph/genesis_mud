...
create_receptacle()
{
    ...
    +"the necessary components placed in the large alchemy set, you "
    +"may 'craft <product>'. You may also "
    ...
    add_prop(CONT_I_MAX_VOLUME,31000);
...
do_craftcomponents(string str)
 {
    ...
    switch(str)
    {
        ...
        case "dragonfire":
        set_component1("_volatile_agent");
        set_herb_component1("pepper");                        
        product_string = "dragonfire";       
        break;
        ...
        case "volatile agent":
        set_component1("_pure_sulphur");
        set_component2("_dragonfire_crystal");          
        product_string = "volatile_agent";       
        break;
    ...
    if (str == "volatile agent")
    {
        ...
        obj2_component = filter(deep_inventory(this_object()),
        &operator(==)(query_component2()) @&->query_name());

        if (!sizeof(obj2_component))
        {
            write("A component is missing in the large alchemy set!\n");
            return 1;
        }
        ...
        obj1_component[0]->reduce_heap_size();
        obj2_component[0]->reduce_heap_size();
        
        volatile_chemicals = volatile_chemicals - 3;
        beakers = beakers - 2;
    ...
    if (result > alchemy_skill)
    {
        ...
        LARGE_CRAFTING_LOG("crafting_log",
        "The player "+this_player()->query_name()
        +" failed crafting a product. (rolled "
        +result+"\nAlchemyskill: "+alchemy_skill+".\n");
