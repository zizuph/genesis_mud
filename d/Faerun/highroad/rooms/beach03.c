/*
 * Beach south of the High Road, Shire Pier
 * -- Finwe, July 2006
 */
#pragma no_clone
#pragma strict_types
...
create_road()
{
    ...
    set_extraline("The beach follows the shoreline, dipping into the cold water and disappearing. Birds run across the sand, and waves crash on the beach. There are ruts in the sand where ships once landed here. They look old and the pier abandoned.");
...
//    add_stuff();
    ...
    add_exit(HR_DIR + "str01", "east");
    add_exit(HR_DIR + "beach01", "south");
//    add_exit(HR_DIR + "beach04", "northwest");
}
...
add_stuff()
{
	...
	if (!ship)
	{
		ship = clone_object("/d/Faerun/ships/flotsam/ship");
		ship->start_ship();
	}
}
...
restart_ship(string reason)
{
	...
	write_file("/d/Faerun/log/shiprestart", extract(ctime(time()), 4, 15)
        + " Faerun to Flotsam " + capitalize(this_player()->query_real_name())
        + " " + reason + "\n");
