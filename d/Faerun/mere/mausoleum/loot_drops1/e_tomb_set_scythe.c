/*
 *  /d/Ravenloft/droptables/fos_drops/e_tomb_set_scythe.c
 *
...
create_weapon()
{
    set_name("_e_tomb_scythe");
    add_name( ({"scythe"}) );

    set_adj(wname);
    add_adj("bone");

    set_short(wname+"'s bone scythe"); 
    set_pshort(wname + "'s bone scythes");
    ...
    set_default_weapon(TOMB_E_SPEAR_HIT, TOMB_E_SPEAR_PEN, 
    ...
    add_prop(OBJ_I_WEIGHT, F_WEIGHT_DEFAULT_WEAPON(TOMB_E_SPEAR_PEN, W_POLEARM));
    add_prop(OBJ_I_VOLUME, TOMB_E_SPEAR_VOLUME);
    add_prop(OBJ_I_VALUE, TOMB_E_SPEAR_VALUE);
    ...
    dam)
{
    ...
    if (random(100) < TOMB_E_SPEAR_DMG_PROC)
    {
        add_prop(LAST_SPECIAL_HIT, time());

        // The damage formula gives an extra 15-25% damage
        extra_damage = TOMB_E_SPEAR_DMG_MULTIPLIER * ((dam * 15 / 100) + random(dam / 10));
