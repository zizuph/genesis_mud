/*
 *  /d/Ravenloft/droptables/fos_drops/l_fos_mace.c
 *
...
create_weapon()
{
    ...
    set_short("black-diamond spiked ebony mace"); 
    set_pshort("black-diamond spiked ebony maces");

    set_long("A black-diamond spiked ebony mace. The "
    +"hilt of this mace is a smooth ebony shaft about "
    +"three inches in diameter and four feet in length. At "
    +"the top is a perfectly smooth one foot diameter ebony "
    ...
    set_default_weapon(FOS_L_MACE_HIT, FOS_L_MACE_PEN, 
    ...
    add_prop(OBJ_I_WEIGHT, F_WEIGHT_DEFAULT_WEAPON(FOS_L_MACE_PEN, W_CLUB));
    add_prop(OBJ_I_VOLUME, FOS_L_MACE_VOLUME);
    add_prop(OBJ_I_VALUE, FOS_L_MACE_VALUE);
    ...
    dam)
{
    ...
    if (random(100) < FOS_L_MACE_DMG_PROC)
    {
        add_prop(LAST_SPECIAL_HIT, time());

        // The damage formula gives an extra 15-25% damage
        extra_damage = FOS_L_MACE_DMG_MULTIPLIER * ((dam * 15 / 100) + random(dam / 10));
