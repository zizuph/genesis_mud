/*
 *  /d/Ravenloft/droptables/fos_drops/e_fos_axe.c
 *
...
create_weapon()
{
    ...
    set_short("moon-bladed spiked battleaxe"); 
    set_pshort("moon-bladed spiked battleaxes");

    set_long("The axe is fully crafted from some unknown glowing "
    ...
    set_default_weapon(FOS_E_AXE_HIT, FOS_E_AXE_PEN, 
	...
	add_prop(OBJ_S_WIZINFO, "This is en epic weapon dropping from the "
		...
		+ "adds "+ FOS_E_AXE_HASTE_BONUS+" to _live_i_quickness at "
		+ "the wielder.\n");

    add_prop(OBJ_I_WEIGHT, F_WEIGHT_DEFAULT_WEAPON(FOS_E_AXE_PEN, W_AXE));
    add_prop(OBJ_I_VOLUME, FOS_E_AXE_VOLUME);
    add_prop(OBJ_I_VALUE, FOS_E_AXE_VALUE);

	set_hit(FOS_E_AXE_HIT);
	set_pen(FOS_E_AXE_PEN);
