/*
 *  /d/Ravenloft/droptables/fos_drops/e_tomb_set_longsword.c
 *
...
create_weapon()
{
    ...
    set_short(wname+"'s bone longsword"); 
    set_pshort(wname + "'s bone longswords");

    set_long("Pulsing with necromantic powers this sword fulfills "
    ...
    set_default_weapon(TOMB_E_SWORD_HIT, TOMB_E_SWORD_PEN, 
    ...
    add_prop(OBJ_I_WEIGHT, F_WEIGHT_DEFAULT_WEAPON(TOMB_E_SWORD_PEN, W_SWORD));
    add_prop(OBJ_I_VOLUME, TOMB_E_SWORD_VOLUME);
    add_prop(OBJ_I_VALUE, TOMB_E_SWORD_VALUE);
    ...
    dam)
{
    ...
    if (random(100) < TOMB_E_SWORD_DMG_PROC)
    {
        add_prop(LAST_SPECIAL_HIT, time());

        // The damage formula gives an extra 15-25% damage
        extra_damage = TOMB_E_SWORD_DMG_MULTIPLIER * ((dam * 15 / 100) + random(dam / 10));
