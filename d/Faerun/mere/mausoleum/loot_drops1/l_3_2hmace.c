/*
 *  /d/Ravenloft/droptables/fos_drops/l_fos_2hmace.c
 *
...
create_weapon()
{
    set_name("_l_fos_2hmace");
    add_name( ({"mace","club", "warmace"}) );

    set_adj("black-diamond");
    add_adj("spiked");
    add_adj("ebony");

    set_short("black-diamond spiked ebony warmace"); 
    set_pshort("black-diamond spiked ebony warmaces");

    set_long("A black-diamond spiked ebony mace. The "
    +"hilt of this mace is a smooth ebony shaft about "
    +"three inches in diameter and five feet in length. At "
    +"the top is a perfectly smooth two foot diameter ebony "
    +"wood sphere which has black - diamonds spikes protruding "
    +"from its face. The spikes have been infused into the "
    +"surface by magical means and glow in many colours at "
    +"their base.\n");

    add_item(({ "spike", "spikes", "diamonds"}), "Many different colours "
    +"magically shift at the base of the spikes. The diamonds that have "
    +"been attached to this weapon and protrude from the face glisten "
    +"and shift as the colours alter their appearance.\n");

    set_default_weapon(FOS_L_2HMACE_HIT, FOS_L_2HMACE_PEN,
    ...
    add_prop(OBJ_I_WEIGHT, F_WEIGHT_DEFAULT_WEAPON(FOS_L_2HMACE_PEN, W_CLUB));
    add_prop(OBJ_I_VOLUME, FOS_L_2HMACE_VOLUME);
    add_prop(OBJ_I_VALUE, FOS_L_2HMACE_VALUE);
...
wield(object what)
{
    ...
    if (this_player()->query_skill(SS_WEP_CLUB) < 80)
    {
    ...
    dam)
{
    ...
    if (random(100) < FOS_L_2HMACE_DMG_PROC)
    {
        add_prop(LAST_SPECIAL_HIT, time());

        // The damage formula gives an extra 15-25% damage
        extra_damage = FOS_L_2HMACE_DMG_MULTIPLIER * ((dam * 15 / 100) + random(dam / 10));
