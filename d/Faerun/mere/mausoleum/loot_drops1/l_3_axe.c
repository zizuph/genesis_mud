/*
 *  /d/Ravenloft/droptables/fos_drops/l_fos_axe.c
 *
...
create_weapon()
{
    set_name("_l_fos_cleaver");
    add_name( ({"cleaver","axe"}) );

    set_adj("sturdy");
    add_adj("long-handled");

    set_short("sturdy long-handled mithril cleaver"); 
    set_pshort("sturdy long-handled mithril cleavers");

    set_long("Seeing this axe leaves you "
    +"in awe. The shaft is long and darkened with what seems "
    +"like dry blood, small scratches and scars run along its "
    +"shaft up to its head. The axes head itself is made a "
    +"strange greyish metal, light, and cold to touch. Two "
    +"wolves howling at the moon, with their manes made to "
    +"act as the blade of this formiddable artifact. Center of "
    +"the axe head you notice some strange runes, faintly "
    +"glowing with a dark red color that makes "
    +"the hairs on your back rise.\n");

    add_item(({ "dry blood", "scratches", "scars" }), "This "
    +"weapon has obviously been used in many fights, and slain "
    +"many foes.\n");

    add_item(({ "strange runes"}), "The runes glows in many shifting "
    +"colours, emanating hints of the ancient powers in the "
    +"weapon. You do not understand what they say though.\n");

    set_default_weapon(FOS_L_AXE_HIT, FOS_L_AXE_PEN, 
    W_AXE, W_SLASH | W_IMPALE, W_ANYH, this_object());
    ...
    add_prop(OBJ_I_WEIGHT, F_WEIGHT_DEFAULT_WEAPON(FOS_L_AXE_PEN, W_AXE));
    add_prop(OBJ_I_VOLUME, FOS_L_AXE_VOLUME);
    add_prop(OBJ_I_VALUE, FOS_L_AXE_VALUE);
...
wield(object what)
{
    ...
    if (this_player()->query_skill(SS_WEP_AXE) < 80)
    {
    ...
    dam)
{
    ...
    if (random(100) < FOS_L_AXE_DMG_PROC)
    {
        add_prop(LAST_SPECIAL_HIT, time());

        // The damage formula gives an extra 15-25% damage
        extra_damage = FOS_L_AXE_DMG_MULTIPLIER * ((dam * 15 / 100) + random(dam / 10));
