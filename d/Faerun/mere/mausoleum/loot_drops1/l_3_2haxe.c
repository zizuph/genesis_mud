/*
 *  /d/Ravenloft/droptables/fos_drops/l_fos_2haxe.c
 *
...
create_weapon()
{
    set_name("_l_2hfos_cleaver");
    add_name( ({"cleaver","axe"}) );

    set_adj("huge");
    add_adj("long-handled");

    set_short("huge long-handled mithril cleaver"); 
    set_pshort("huge long-handled mithril cleavers");

    set_long("Seeing this huge axe leaves you "
    ...
    set_default_weapon(FOS_L_2HAXE_HIT, FOS_L_2HAXE_PEN,
    ...
    add_prop(OBJ_I_WEIGHT, F_WEIGHT_DEFAULT_WEAPON(FOS_L_2HAXE_PEN, W_AXE));
    add_prop(OBJ_I_VOLUME, FOS_L_2HAXE_VOLUME);
    add_prop(OBJ_I_VALUE, FOS_L_2HAXE_VALUE);
    ...
    dam)
{
    ...
    if (random(100) < FOS_L_2HAXE_DMG_PROC)
    {
        add_prop(LAST_SPECIAL_HIT, time());

        // The damage formula gives an extra 15-25% damage
        extra_damage = FOS_L_2HAXE_DMG_MULTIPLIER * ((dam * 15 / 100) + random(dam / 10));
