/*
 *  /d/Ravenloft/droptables/fos_drops/e_tomb_set_dagger.c
 *
...
create_weapon()
{
    set_name("_e_tomb_longsword");
    add_name( ({"finger", "dagger"}) );

    set_adj(wname);
    add_adj("bone");

    set_short(wname+"'s bone dagger"); 
    set_pshort(wname + "'s bone daggers");

    set_long("Pulsing with necromantic powers this dagger fulfills "
    +"" + wname + "'s prayer. " + wname + "'s middle finger has "
    +"been mummified "
    +"and cast in adamantine to create this dagger. Your hand "
    +"actually slides into " + wname + "'s hand to grasp the hilt of "
    +"this dagger, its skin somehow forming to yours. Rumour has "
    +"it this brings greater proficiency in back stabbing, guided "
    +"by " + wname + "'s spirit. Foul black specks can be seen upon the "
    +"fingers tip. This dagger is extremely cold.\n");
    
    add_item(({"finger", "middle finger", "dagger", "hilt",}), "This is "
    +"probably the most unique weapon you've ever seen. The middle finger "
    +"of "+wname+" has been mummified, cast in adamantium, and sharpened "
    +"to create a dagger.\n");

    set_default_weapon(TOMB_E_DAGGER_HIT, TOMB_E_DAGGER_PEN, 
    W_KNIFE, W_SLASH | W_IMPALE, W_ANYH, this_object());
    ...
    add_prop(OBJ_I_WEIGHT, F_WEIGHT_DEFAULT_WEAPON(TOMB_E_DAGGER_PEN, W_KNIFE));
    add_prop(OBJ_I_VOLUME, TOMB_E_DAGGER_VOLUME);
    add_prop(OBJ_I_VALUE, TOMB_E_DAGGER_VALUE);
    ...
    dam)
{
    ...
    if (random(100) < TOMB_E_DAGGER_DMG_PROC)
    {
        add_prop(LAST_SPECIAL_HIT, time());

        // The damage formula gives an extra 15-25% damage
        extra_damage = TOMB_E_DAGGER_DMG_MULTIPLIER * ((dam * 15 / 100) + random(dam / 10));
