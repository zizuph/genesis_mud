/*
 *  /d/Ravenloft/droptables/fos_drops/l_fos_2hsword.c
 *
 *  Legendary sword from the Forest of Shadows.
 *
...
create_weapon()
{
    set_name("_l_fos_2hsword");
    add_name( ({"blade","sword", "longsword"}) );
    set_adj("iridicent");
    set_adj("golden-hilted");

    set_short("iridicent golden-hilted longsword"); 
    set_pshort("iridicent golden-hilted longswords");

    set_long("An iridecent beam of light extends from the "
    +"sword to about 2.1 meters. The colours of the beam "
    ...
    set_default_weapon(FOS_L_2HSWORD_HIT, FOS_L_2HSWORD_PEN,
    ...
    add_prop(OBJ_I_WEIGHT, F_WEIGHT_DEFAULT_WEAPON(FOS_L_2HSWORD_PEN, W_SWORD));
    add_prop(OBJ_I_VOLUME, FOS_L_2HSWORD_VOLUME);
    add_prop(OBJ_I_VALUE, FOS_L_2HSWORD_VALUE);
    ...
    dam)
{
    ...
    if (random(100) < FOS_L_2HSWORD_DMG_PROC)
    {
        add_prop(LAST_SPECIAL_HIT, time());

        // The damage formula gives an extra 15-25% damage
        extra_damage = FOS_L_2HSWORD_DMG_MULTIPLIER * ((dam * 15 / 100) + random(dam / 10));
