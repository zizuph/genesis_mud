/*
 *  /d/Ravenloft/droptables/fos_drops/l_fos_staff.c
 *
...
create_weapon()
{
    set_name("_l_fos_staff");
    add_name( ({"staff","polearm"}) );

    set_adj("orange-veined");
    add_adj("ash-black");

    set_short("orange-veined ash-black staff"); 
    set_pshort("orange-veined ash-black staves");

    set_long("Six foot in length with two obsidian "
    ...
    set_default_weapon(FOS_L_STAFF_HIT, FOS_L_STAFF_PEN, 
    W_POLEARM, W_BLUDGEON | W_IMPALE, W_ANYH, this_object());
    ...
    add_prop(OBJ_I_WEIGHT, F_WEIGHT_DEFAULT_WEAPON(FOS_L_STAFF_PEN, W_POLEARM));
    add_prop(OBJ_I_VOLUME, FOS_L_STAFF_VOLUME);
    add_prop(OBJ_I_VALUE, FOS_L_STAFF_VALUE);
    ...
    dam)
{
    ...
    if (random(100) < FOS_L_STAFF_DMG_PROC)
    {
        add_prop(LAST_SPECIAL_HIT, time());

        // The damage formula gives an extra 15-25% damage
        extra_damage = FOS_L_STAFF_DMG_MULTIPLIER * ((dam * 15 / 100) + random(dam / 10));
