/*
 *  /d/Ravenloft/droptables/fos_drops/e_tomb_set_2hlongsword.c
 *
...
create_weapon()
{
    set_name("_e_tomb_2hlongsword");
    add_name( ({"sword", "greatsword"}) );

    set_adj(wname);
    add_adj("bone");

    set_short(wname+"'s bone greatsword"); 
    set_pshort(wname + "'s bone greatswords");

    set_long("Pulsing with necromantic powers this greatsword fulfills "
    +"Jakredaks prayer. Jakerdaks forearm ulna has been sharpened "
    +"and cast in adamantine creating this longsword. So fine is "
    +"the sword one can still see chips in the bone, damage inflicted "
    +"upon Jakredak ages ago. Incredibly light this sword whistles as "
    +"it slices air, but also incredibly sturdy as it hums when it "
    +"strikes and vibrates. This sword is extremely cold.\n");

    set_default_weapon(TOMB_E_2HSWORD_HIT, TOMB_E_2HSWORD_PEN,
    W_SWORD, W_SLASH | W_IMPALE, W_BOTH, this_object());
    ...
    add_prop(OBJ_I_WEIGHT, F_WEIGHT_DEFAULT_WEAPON(TOMB_E_2HSWORD_PEN, W_SWORD));
    add_prop(OBJ_I_VOLUME, TOMB_E_2HSWORD_VOLUME);
    add_prop(OBJ_I_VALUE, TOMB_E_2HSWORD_VALUE);
    ...
    dam)
{
    ...
    if (random(100) < TOMB_E_2HSWORD_DMG_PROC)
    {
        add_prop(LAST_SPECIAL_HIT, time());

        // The damage formula gives an extra 15-25% damage
        extra_damage = TOMB_E_2HSWORD_DMG_MULTIPLIER * ((dam * 15 / 100) + random(dam / 10));
