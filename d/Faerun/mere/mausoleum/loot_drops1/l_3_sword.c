/*
 *  /d/Ravenloft/droptables/fos_drops/l_fos_sword.c
 *
...
create_weapon()
{
    ...
    set_default_weapon(FOS_L_SWORD_HIT, FOS_L_SWORD_PEN, 
    ...
    add_prop(OBJ_I_WEIGHT, F_WEIGHT_DEFAULT_WEAPON(FOS_L_SWORD_PEN, W_SWORD));
    add_prop(OBJ_I_VOLUME, FOS_L_SWORD_VOLUME);
    add_prop(OBJ_I_VALUE, FOS_L_SWORD_VALUE);
    ...
    dam)
{
    ...
    if (random(100) < FOS_L_SWORD_DMG_PROC)
    {
        add_prop(LAST_SPECIAL_HIT, time());

        // The damage formula gives an extra 15-25% damage
        extra_damage = FOS_L_SWORD_DMG_MULTIPLIER * ((dam * 15 / 100) + random(dam / 10));
