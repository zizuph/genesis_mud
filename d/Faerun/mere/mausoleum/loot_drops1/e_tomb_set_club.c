/*
 *  /d/Ravenloft/droptables/fos_drops/e_tomb_set_club.c
 *
...
create_weapon()
{
    set_name("_e_tomb_club");
    add_name( ({"club"}) );

    set_adj(wname);
    add_adj("bone");

    set_short(wname+"'s bone club"); 
    set_pshort(wname + "'s bone clubs");
    ...
    set_default_weapon(TOMB_E_MACE_HIT, TOMB_E_MACE_PEN, 
    W_CLUB, W_BLUDGEON, W_ANYH, this_object());
    ...
    add_prop(OBJ_I_WEIGHT, F_WEIGHT_DEFAULT_WEAPON(TOMB_E_MACE_PEN, W_CLUB));
    add_prop(OBJ_I_VOLUME, TOMB_E_MACE_VOLUME);
    add_prop(OBJ_I_VALUE, TOMB_E_MACE_VALUE);
    ...
    dam)
{
    ...
    if (random(100) < TOMB_E_MACE_DMG_PROC)
    {
        add_prop(LAST_SPECIAL_HIT, time());

        // The damage formula gives an extra 15-25% damage
        extra_damage = TOMB_E_MACE_DMG_MULTIPLIER * ((dam * 15 / 100) + random(dam / 10));
