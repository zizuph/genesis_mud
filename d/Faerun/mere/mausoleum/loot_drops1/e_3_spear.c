/*
 *  /d/Ravenloft/droptables/fos_drops/e_fos_spear.c
 *
...
create_weapon()
{
    set_name("_e_fos_spear");
    add_name( ({"spear"}) );

    set_adj("scrimshaw");
    add_adj("tipped");

    set_short("scrimshaw tipped spear"); 
    set_pshort("scrimshaw tipped spears");

    set_long("Made light by the walnut shaft, this spear is easily "
    +"six feet in length and quite sturdy. The scrimshaw head shows a "
    ...
    set_default_weapon(FOS_E_SPEAR_HIT, FOS_E_SPEAR_PEN, 
    W_POLEARM, W_SLASH | W_IMPALE, W_ANYH, this_object());
    ...
    add_prop(OBJ_S_WIZINFO, "This is en epic weapon dropping from the "
        ...
        + "adds " + FOS_E_SPEAR_HASTE_BONUS + " to _live_i_quickness at "
        + "the wielder.\n");

    add_prop(OBJ_I_WEIGHT, F_WEIGHT_DEFAULT_WEAPON(FOS_E_SPEAR_PEN, W_POLEARM));
    add_prop(OBJ_I_VOLUME, FOS_E_SPEAR_VOLUME);
    add_prop(OBJ_I_VALUE, FOS_E_SPEAR_VALUE);

    set_hit(FOS_E_SPEAR_HIT);
    set_pen(FOS_E_SPEAR_PEN);
