/*
 *  /d/Ravenloft/droptables/fos_drops/e_tomb_set_axe.c
 *
...
create_weapon()
{
    set_name("_e_tomb_axe");
    add_name( ({"axe"}) );

    set_adj(wname);
    add_adj("bone");

    set_short(wname+"'s bone axe"); 
    set_pshort(wname + "'s bone axes");

    set_long("Pulsing with necromantic powers this axe fulfills "
    +wname+"'s prayer. "+wname+"'s femur and shoulder blade have "
    +"been fused together with an adamantine coating to create "
    +"this insane axe. Not only is the femur excessively long for "
    +"an axe handle, but the shoulder blade of " + wname + " is "
    +"probably bigger than any axeblade ever made. Sharp to "
    +"say the least, the magic within this axe throbs in "
    +"your hand with each swing, singing in your mind songs "
    +"of the frost giants. This axe is extremely cold.\n");
    
    add_item(({"femur", "handle", "shaft",}), "The femur of "+wname+" is "
    +"incredibly long and incredibly strong. It shimmers, coated with "
    +"adamtinium.\n");
    
    add_item(({"shoulder", "blade", "shoulder blade",}), "The shoulder "
    +"blade of "+wname+" is incredibly big and incredibly sharp. It "
    +"shimmers, coated with adamtinium.\n");

    set_default_weapon(TOMB_E_AXE_HIT, TOMB_E_AXE_PEN, 
    ...
    add_prop(OBJ_I_WEIGHT, F_WEIGHT_DEFAULT_WEAPON(TOMB_E_AXE_PEN, W_AXE));
    add_prop(OBJ_I_VOLUME, TOMB_E_AXE_VOLUME);
    add_prop(OBJ_I_VALUE, TOMB_E_AXE_VALUE);
    ...
    dam)
{
    ...
    if (random(100) < TOMB_E_AXE_DMG_PROC)
    {
        add_prop(LAST_SPECIAL_HIT, time());

        // The damage formula gives an extra 15-25% damage
        extra_damage = TOMB_E_AXE_DMG_MULTIPLIER * ((dam * 15 / 100) + random(dam / 10));
