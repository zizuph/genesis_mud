/*
 *  /d/Ravenloft/droptables/fos_drops/e_tomb_set_2hclub.c
 *
...
create_weapon()
{
    ...
    set_short(wname+"'s large bone club"); 
    set_pshort(wname + "'s large bone clubs");

    set_long("Pulsing with nercomantic powers this fist "
    +"fulfills "+wname+"'s prayer. "+wname+"'s fist and forearm have "
    +"been mummified and cast in adamantine to help protect his "
    +"people. So carefully crafted and preserved was this club "
    +"that you can still see chips in the fingernails and scars "
    +"on the forearm, forever left under the adamantine coating. The "
    +"hilt is a cavity inside the forearm where one graps the radius, "
    +"the ulna removed. This club is extremely cold.\n");
    
    add_item(({"forearm", "radius", "handle",}), "The radius of "+wname+" is "
    +"incredibly long and incredibly strong. It shimmers, coated with "
    +"adamtinium.\n");
    
    add_item(({"fist", "nails", "scars",}), "The fist "
    +"of "+wname+" is is massive. It has been incredibly well preserved "
    +"that you can make out the chips of the fingernails as well as "
    +"the scars which once were present on the hand. It "
    +"shimmers, coated with adamtinium.\n");

    set_default_weapon(TOMB_E_2HMACE_HIT, TOMB_E_2HMACE_PEN,
    W_CLUB, W_BLUDGEON, W_BOTH, this_object());
    ...
    add_prop(OBJ_I_WEIGHT, F_WEIGHT_DEFAULT_WEAPON(TOMB_E_2HMACE_PEN, W_CLUB));
    add_prop(OBJ_I_VOLUME, TOMB_E_2HMACE_VOLUME);
    add_prop(OBJ_I_VALUE, TOMB_E_2HMACE_VALUE);
    ...
    dam)
{
    ...
    if (random(100) < TOMB_E_2HMACE_DMG_PROC)
    {
        add_prop(LAST_SPECIAL_HIT, time());

        // The damage formula gives an extra 15-25% damage
        extra_damage = TOMB_E_2HMACE_DMG_MULTIPLIER * ((dam * 15 / 100) + random(dam / 10));
