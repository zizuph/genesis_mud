...
#define PATH "/d/Faerun/menzo/rooms/camp"
...
create_room()
{
    ...
    set_long("The forest thins out before you. You see three tents are " +
		"spread out around the area. There is a green tent to the northwest, " +
		"a brown tent to the east, and a large red tent set up right in " +
		"front of you. There is a cook fire set in the midst of the " +
		"trifecta.\n");
    ...
    add_item(({"third tent","red tent","large tent"}),
        ...
        "you think somone of importance resides within.\n");

    add_item(({"cookfire","fire"}),
        "This is a large smoldering cookfire, the fire looks like it was " +
        "recently extinguished, only faintly glowing embers remain. Three " +
		"tall sturdy sticks are tied together and spread over the pit. " +
		"A perfect place to hang a pot directly over the fire for cooking.\n");
		
	add_item(({"embers","coals"}),
	    "The embers are glowing slightly and a bit of smoke rises " +
		"lazily. You think you see something scratched into the " +
		"surface of one of the stones.\n");
		
	add_item(({"stones"}),
	    "The stones are pressed closely together to form a circle " +
		"surrounding the pit. One stone seems to have a peculiar symbol " +
		"etched into its surface. It seems to be the symbol of a spider.\n");
		
	

    add_exit(CAMP_DIR + "menzo_start", "west");
    add_exit(CAMP_DIR + "green_tent","northwest");
    add_exit(CAMP_DIR + "brown_tent","east");
    add_exit(CAMP_DIR + "red_tent","enter");
    add_exit(CAMP_DIR + "hidden_trail1", "north", 0, 1, 1);
}
