...
{
    extraline = "This broad avenue passes between the church and the shop."
                +" It extends north and south to other areas of the city."
                +" To the east a shop sits on a pier while the church"
                +" rises to the west. A large building is farther north"
                +" and to the south you see what looks like a hedge."
    ...
    add_item(({"shop"}),
        ...
        +" The entrance is immediately to the east.\n");
    ...
    add_exit(ROOM_DIR + "phlanshop", "east");
    add_exit(ROOM_DIR + "church1", "west");
    add_exit(ROOM_DIR + "road3", "north");  
    add_exit(ROOM_DIR + "road9", "south");     
}
