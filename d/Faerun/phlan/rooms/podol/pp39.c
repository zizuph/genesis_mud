...
create_phlan_outdoor()
{
    extraline = "Podol Plaza is a field of short clipped, manicured grass."
                ...
                +" the plaza, where the city wall turns to the west. Northwest"
                +" is Trader's Way, bisecting the market from east to west."
...
*/
    ...
    add_room_tell("You hear waves lapping against the stones on the other side"
        +" of the wall.");      


    reset_faerun_room();

    add_exit(PODOL_DIR + "tw1", "northwest");         
    add_exit(PODOL_DIR + "pp32", "north");             
    add_exit(PODOL_DIR + "pp46", "south");    
