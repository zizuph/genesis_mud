...
create_phlan_outdoor()
{
    extraline = "The road is of hard packed dirt. To the east it leads to the"
                ...
                +" northeast lies the wealthy district while to the north and"
                +" northwest lie the Stojanow Gate and beyond that Valjevo Castle."
                +" Kuto's Well is to the south and east and Podol Plaza is"
                +" to the south.";
...
*/    
    ...
    add_exit(PODOL_DIR + "pps8", "east");    
    add_exit(PODOL_DIR + "pps10", "west");  
    add_exit(PODOL_DIR + "pp5", "southeast");   
    add_exit(PODOL_DIR + "pp6", "south");
    add_exit(PODOL_DIR + "pp7", "southwest");    
