...
create_phlan_outdoor()
{
    extraline = "The road is of hard packed dirt. Far to the east is the"
                ...
                +" northern edge of Podol Plaza and eventually splits to the"
                +" bridge crossing the Stojanow River and Cadorna Street leading"
                +" to the Cadorna Textile Mill. Across the river to the"
                +" northeast lies the Stojanow Gate and beyond that Valjevo"
...
*/    
    ...
    add_item(({"bridge", "podol bridge"}),
        ...
        +" to the Valhingen Graveyard and farmlands beyond. From here it looks"
        +" like it is made of plain unadorned grey stone.\n");   
    ...
    add_item(({"door", "doors"}),
        "The doors stand twenty feet tall with no visible hinges. Straps"
        +" of blackened iron reinforce the wooden beams.\n");      

    add_item(({"beam", "beams"}),
        "The beams used in the gate are of thick grained ironwood.\n");   

    add_item(({"strap", "straps"}),
        "Blackened iron straps reinforce the beams of the doors.\n");           
    ...
    add_exit(PODOL_DIR + "pps13", "east");    
    add_exit(PODOL_DIR + "pps15", "west"); 
    add_exit(PODOL_DIR + "pp9", "southeast");   
    add_exit(PODOL_DIR + "pp10", "south");     
    add_exit(PODOL_DIR + "pp11", "southwest");   
    add_exit(PODOL_DIR + "sj1", "northeast");    
