...
create_phlan_outdoor()
{
    extraline = "Podol Plaza is a field of short clipped, manicured grass."
                ...
                +" Avenue crosses the market south to north."
                +" A bright green tent stands to the north and a plain tan one"
                +" farther north and east. To the southwest you can see the Mantor"
...
*/    
    ...
    add_item(({"wall", "city wall"}),
        ...
        +" is the southern edge, while to the east it turns north"
...
*/
    ...
    add_room_tell("A small cloud of fruit flies fly by, attracted by the smells.");    


    reset_faerun_room();

    add_exit(PODOL_DIR + "pa6", "northeast"); 
    add_exit(PODOL_DIR + "pp56", "east"); 
    add_exit(PODOL_DIR + "pp58", "west");
    add_exit(PODOL_DIR + "pp68", "southwest");    
