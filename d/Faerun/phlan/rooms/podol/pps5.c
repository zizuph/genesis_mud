...
create_phlan_outdoor()
{
    extraline = "The road is of hard packed dirt. To the east it leads to the"
                ...
                +" disctrict. Kuto's Well is to the south and Podol Plaza is farther west.";
...
*/        
    ...
    add_room_tell("A servant carrying buckets of water on a pole across their"
        +" neck makes their way from the well to the Kovel Bridge.");       

    reset_faerun_room();
        
    add_exit(PODOL_DIR + "pps4", "east");    
    add_exit(PODOL_DIR + "pps6", "west");  
    add_exit(PODOL_DIR + "pp1", "southeast");   
    add_exit(PODOL_DIR + "pp2", "south");
    add_exit(PODOL_DIR + "pp3", "southwest");    
