...
create_phlan_outdoor()
{
    extraline = "Podol Plaza is a field of short clipped, manicured grass."
                ...
                +" feet every year. Kuto's Well is directly to the northeast while"
                +" Podol Street lies along the northern edge of the plaza."
                +" The city wall to the south angles in a more southwesterly direction"
                +" as it follows the unseen shoreline beyond."
...
*/
    ...
    add_room_tell("A seagull momentarily alights on the wall then takes off for the sky.");                 

    reset_faerun_room();
 
    add_exit(PODOL_DIR + "kw", "northeast"); 
    add_exit(PODOL_DIR + "pp13", "north");    
    add_exit(PODOL_DIR + "pp14", "northwest");
    add_exit(PODOL_DIR + "pp20", "east"); 
    add_exit(PODOL_DIR + "pp22", "west");     
