...
create_phlan_outdoor()
{
    extraline = "The road is of hard packed dirt. To the east it leads to the"
                ...
                +" Stojanow River. The Kovel Bridge crosses the river to the"
                +" northeast, linking to the wealthy district. Kuto's Well is"
                +" to the southwest and Podol Plaza is farther west.";
...
*/    
    ...
    add_item(({"flame", "flames"}),
        "The flames are bright white but seem to give off neither heat"
        +" nor sound. The magical flames require no oil.\n");        
    ...
    add_exit(PODOL_DIR + "pps3", "east");    
    add_exit(PODOL_DIR + "pps5", "west");  
    add_exit(PODOL_DIR + "kb1", "northeast");   
    add_exit(PODOL_DIR + "pp1", "south");
    add_exit(PODOL_DIR + "pp2", "southwest");    
