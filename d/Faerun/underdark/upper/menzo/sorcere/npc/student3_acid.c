/*
 *  faerun/underdark/upper/menzo/sorcere/npc/student3_acid.c
 *
 *  A 3rd level wizard student in the Sorcere
 *
...
void create_sorcere_drow()
{
    ...
    set_stats(({ 130, 195, 150, 165, 150, 140 }));
    set_alignment(-800);

    set_skill(SS_ELEMENT_WATER, 65);
    set_skill(SS_ELEMENT_AIR, 65);
    set_skill(SS_SPELLCRAFT, 85);
    set_skill(SS_FORM_CONJURATION, 55);
    ...
    add_spell(P_FAERUN_SPELLS + "drow_acid_bolt");
    set_cast_chance(50);
...
void equip()
{
    ...
    clone_object(P_MENZO_SORCERE + "arm/ns_bracers")->move(this_object());
    clone_object(P_MENZO_SORCERE + "arm/ns_guards")->move(this_object());
    clone_object(P_MENZO_SORCERE + "arm/ns_armour")->move(this_object());
    ...
    clone_object(P_MENZO_SORCERE + "wep/qstaff")->move(this_object());
    command("wear all");
    command("wield all");
}
