/*
 *  faerun/underdark/upper/menzo/sorcere/second23.c
 *
 *   
 *  Study on the second floor in the south tower of sorcere.
 *  Created by Midnight, 29-2-2004
 */

#include "defs.h"

#define STUDENT P_MENZO_SORCERE + "npc/student2_fire"

inherit P_MENZO_SORCERE + "std/base_room";

private object student_fire;

public void
reset_room()
{
    if(!objectp(student_fire))
    {
        student_fire = clone_object(STUDENT);
        student_fire->move(this_object(), 1);
        student_fire->command("emote takes a step out of the shadows.");
    }
...
void create_sorcere_room()
{
	...
	set_short("A half-moon shaped study room in the south tower");
	set_long("You've entered a half-moon shaped study of a wizard " +
                 ...
                 "on a delicate wooden desk near the only window in the " +
	...
	door->configure_ground_door("east", "s2_23", 
		P_MENZO_SORCERE + "south2");
