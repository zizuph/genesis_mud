/*
 *  faerun/underdark/upper/menzo/sorcere/wep/m_dagger.c
 *
 *  This is dagger for wizard masters
 *
 *  Created by Midnight, 22-3-2004
 */
...
create_faerun_weapon()
{
    ...
    add_adj("frosted");
    add_adj("ice-bladed");
    set_long("This vicious-looking dagger is known as 'The Iceblade'. " +
              "It's history is a mystery, but it's said that a drow wizard " +
              "aided the forging of this weapon deep in the mountains " +
              "of Icewind Dale. Its sharp icy blade is apparently " +
              "forged of a metal, but has been covered with endurable ice " +
              "which seems extremely sharp.\n");

 
    set_short("frosted ice-bladed dagger");
    
    add_prop(OBJ_I_IS_MAGIC_WEAPON, 1);
    add_prop(MAGIC_AM_ID_INFO, ({
        "The blade has been macigally enchanted with " +
        "magical oils, so the blade will not become so damaged.\n", 70 }));
    add_prop(OBJ_S_WIZINFO, "A master of the Sorcere wields this dagger. " +
        "It does not dull.\n");

    set_default_weapon(HIT, PEN, W_KNIFE, W_SLASH | W_IMPALE,
        W_ANYH, this_object());
    ...
    int dt, int phit, int dam)
{
    ...
    if(random(20) > 15)
    {
        tell_room(environment(me), "You hear a low rhythmical throbbing,  " +
          "like the sound of a slow heartbeat.\n", ({}));
    }
    ...
    switch(phurt)
    {
        ...
        case 16..20:
            enemy->catch_msg(QCTNAME(me) + " impales you " + desc +
                ...
                desc + " in the wound, almost freezing the blood " +
                "pouring from your gaping wound to ice!\n");
            me->catch_msg("You impale " + QTNAME(enemy) + " " + desc +
                "with the sharp blade of the " + query_short() + "!\n " +
                "You twists the " + query_short() + " " +
                desc + " in the wound, almost freezing the blood " +
                "pouring from " + QTPNAME(enemy) + " gaping wound " +
                "to ice!\n");
            me->tell_watcher(QCTNAME(me) + " impales " + QTNAME(enemy) +
                 " " + desc + " with the sharp blade of the " + query_short() 
                 + "!\n " +
                "You twists the " + query_short() + " " +
                desc + " in the wound, almost freezing the blood " +
                "pouring from " + QTPNAME(enemy) + 
                " gaping wound to ice!\n", enemy, us);
        break;
