...
Snarfed by Palmer, Jan 2003
Modified to work with /d/Genesis/steeds/new/steed.c
Used by Bree horses and Imladris horses

Filename: /d/Terel/calathin/npc/terel_horse_saddlebag.c
          /d/Terel/calathin/npc/terel_horse includes this file.

Purpose:  Allows the steed to wear a pack, and provides fill and
...
do_empty(string arg)
{
  ...
  {
      notify_fail("You can't empty the saddlebag because you're not leading or riding the steed.\n");
      if (steed)
      {
          tell_room(ENV(TP), QCTNAME(TP) + " tried to empty the saddlebag on the " + steed->short_old() + ".\n", TP);
      }
      return 0;
  }
...
do_strap(string arg)
{
  ...
  notify_fail("That steed already carries a saddlebag!\n");
...
//    return 0;
  ...
  parse_command(arg, environment(TP), "[the] 'saddlebag' 'on' %o", steed);
  ...
  if (saddlebag->move(steed))
    {
      write ("The saddlebag seems to be too heavy for the "+
            steed->short_old()+" to carry.\n");
      saddlebag=0;
      return 0;
    }

  write ("You strap the saddlebag to the " + steed->short_old() + "'s back.\n");

  say (QCTNAME(TP) + " straps a saddlebag to the back of the " +
       steed->short_old() + ".\n");
  ...
  if (!present("saddlebag"))
    {
      ...
      return capitalize(TO->query_pronoun())+" has a saddlebag strapped to "+
...
do_unstrap(string arg)
{
  ...
  if (!query_valid_person())
  {
      notify_fail("You failed to unstrap the saddlebag because you're not leading or riding the steed.\n");
      if (steed)
      {
          tell_room(ENV(TP), QCTNAME(TP) + " tried to unstrap the saddlebag from the " + steed->short_old() + ".\n", TP);
      }
      return 0;
  }
  ...
  if (saddlebag->move(TP))
    {
      ...
      say (QCTNAME(TP)+" unties the straps of the " +steed->short_old()+
...
do_open(string arg)
{
  ...
  if (!saddlebag->query_prop(CONT_I_CLOSED))
    {
    write ("The saddlebag is already open!\n");
    return 1;
    }

  saddlebag->add_prop(CONT_I_CLOSED,0);

  write ("You open the saddlebag on the "+
         steed->short_old()+"'s back.\n");

  say (QCTNAME(TP)+" opens the saddlebag on the "+
       steed->short_old()+"'s back.\n");
...
pack_fill_hook(object *moved_items)
{
    ...
    else
    {
     ...
     /*
        write("You store " + COMPOSITE_ALL_DEAD(moved_items) + " " +
            (query_prop(CONT_I_ATTACH) ? "on" : "in") + " the "
          + saddlebag->short() + ".\n");
