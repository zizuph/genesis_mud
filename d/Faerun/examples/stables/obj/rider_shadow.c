...
public void remove_rider_shadow();

// --- Functions ------------------------------------------------------

/* Function name: dismount
 * Description:   dismount the rider plus tell him he dismounted.
 */
private void dismount()
{
    steed->sh_do_dismount(shadow_who, 1);
    steed->handle_dismount_status(STEED_STS_OK, shadow_who);
}

/* Function name: steed_is_updated
 * Description:   Since this shadow caches a few steed values for
 *                performance (the query_rider_m_in/query_rider_m_out
 *                values for example) this cache may need an update
 *                once in a while.
 */
public void
steed_is_updated()
...
// Give message unless where_to == "M".
private void
...
move_living(string how, mixed to_dest, int dont_follow, int no_glance)
{
    ...
    {
      case 10:
        give_msg(&steed->steed_is_tired_hook(shadow_who), "");
        break;
      case 9:
        give_msg(&steed->is_not_driver_hook(shadow_who), "");
        break;
      case 8:
        // Steed refuses to go there, but maybe steed
        // will go there when we dismount (when auto_dismount is on).
        if (
            steed->query_auto_dismount() &&
            STEED_STS_SUCCESS(steed->can_lead(shadow_who))
            )
        {
            int sts = steed->check_unmounted_destination(to_dest);
            if (STEED_STS_SUCCESS(sts)) {
                // Dismount and lead the steed and move.
                return steed->dismount_lead_move(
                    shadow_who, how, to_dest, dont_follow, no_glance);
                // No code will be executed after here because this
                // shadow will be removed by function above.
            }
        }
        give_msg(&steed->invalid_dest_hook(shadow_who), "go there");
        break;
    }
    ...
    if (how == "X")
    {
        ...
        if (
            steed->query_no_trans() ||
            sizeof(steed->query_riders()) > 1 || 
            r
            )
        {
            dismount();
    ...
    // Added to allow other steeds being led to follow too. - Trofast
    ...
    {
        dismount();
        remove_rider_shadow();  // No code will be executed after this statement
    }
...
query_skill(int skill_nr)
{
    ...
    if (
        (member_array(skill_nr, steed->query_redir_skills()) >= 0) && 
...
add_fatigue(int f)
{
    // The random 4 indicates the 25% mentioned above. Pass all healing
    // to the player.
    if (
        (f < 0) &&
        calling_object()->query_prop(ROOM_I_IS) &&
        random(4)
        )
    {
        steed->add_fatigue(f);
...
delayed_attacked_by(object o)
{
    ...
    attacked_by_alarm = 0;
    attacker = shadow_who->query_attack();
    if (objectp(attacker) && (environment(attacker) == environment(shadow_who)))
    {
        steed->dismount_and_lead(shadow_who);
        // No code is excecuted after here because shadow will be removed.
    }
}

// When rider is attacked, dismount him if steed->mounted_combat_allowed()
// returns false. Otherwise just start normal combat.
public void
attacked_by(object o)
{
    if (!steed->mounted_combat_allowed())
    {
...
// When rider attacks somebody, dismount him if steed->mounted_combat_allowed()
// returns false. Otherwise just start normal combat.
public void
...
/* Function name: remove_rider_shadow
 * Description:   removes this rider shadow.
 */
public void
remove_rider_shadow()
{
...
query_cannot_move_to(mixed destination, string where_to)
{
    ...
    if (
        r = steed->query_cannot_move_to(shadow_who,
