...
void create_object()
{
	...
	set_adj("green");
	set_adj("jointed");
	set_short("green jointed arm");
	set_pshort("green jointed arms");
	set_long("It is a part of a toy. It looks like a small " +
	"jointed arm. You might try to 'attach' it to some other " +
	"piece.\n");
...
void init()
{
	add_action("attach_green_arm", "attach");
}

int attach_green_arm(string str)
{
	if(str == "green arm" | str == "the green arm" | str == "green " +
		"jointed arm" | str == "the green jointed arm")
	{
		
		if(TP->query_prop(YELLOW_PROP))
		{
			TP->catch_msg("You attach the green arm " +
			"to the metal body.\n");
			
			say(QCTNAME(TP) + " attaches one of the pieces " +
			"in " + HIS_HER(TP) + " hand to one of the other " +
			"pieces " + HE_SHE(TP) + " is holding.\n"); 
			
			TP->add_prop(GREEN_PROP, 1);
			remove_object();
		}
		
		else
		{
			TP->catch_msg("Hmm... Attaching the green arm right " +
	...
	{
		TP->catch_msg("Attach what? The green arm?\n");
	}
