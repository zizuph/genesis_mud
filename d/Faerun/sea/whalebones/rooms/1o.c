...
create_whaleb_outdoor()
{
    ...
    + "ush scatter around the base of a steep craggy mountain to your we"
    +"st. Cool fresh"
    ...
    add_exit(ROOM_DIR + "clifftop.c", "northeast");
    add_exit(ROOM_DIR + "6o.c", "south");

    set_no_exit_msg( ({"east", "southeast", "north"}),
        "There's nothing in that directi"
        +"ction except thin air. You decide against it.\n");

    set_no_exit_msg( ({"southwest", "west", "north", "northwest"}),
