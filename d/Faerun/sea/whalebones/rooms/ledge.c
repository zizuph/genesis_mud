/*  /d/Faerun/sea/whalebones/rooms/ledge.c
*
*   By Nucifera, 2020
*
*/
...
//if function is triggered with phrase 'climb', but nothing else
    ...
    if(parse_command(str, ({}), "'down'")){
        ...
        this_player()->move_living("M", ROOM_DIR + "beach3", 1, 0);
            tell_room(ROOM_DIR + "ledge", QCTNAME(this_player()) + " clim"
    ...
    if (this_player()->resolve_task(TASK_CLIMBUP, ({SKILL_WEIGHT, XDEX,
        TS_DEX, SKILL_END, SKILL_1, SKILL_2})) > 0)
        {
            ...
            tell_room(this_object(), QCTNAME(this_player()) + " arrives climbing " +
                "up the side of the cliff.\n");
            tell_room(ROOM_DIR + "beach3", QCTNAME(this_player()) + " climbs up " +
                "the rugged stone cliff.\n");
            this_player()->move_living("M", ROOM_DIR + "clifftop", 1, 0);
            return 1;
        }
      else
      {
