...
create_whaleb_outdoor()
{
    extraline = "There is a gentle "
        ...
        +" have dispersed on the gray-tinged sand.";

    set_short("Somewhere on the narrow bone littered beach of Finback "
        +"island");

    //Examinable items within room
    add_item(({"column", "columns", "vertebrate","remains"}),
        "Large columns of ribcage pertrude out of the sand. These are cle"
        +"arly remains from a creature much larger than you.");
    ...
    add_item(({"sea","water","coast"}),"Flawless turquoise water forming "
        +"the "
...
//more immersive expirience.
    ...
    set_no_exit_msg( ({"north","northeast"}), "You consider moving furthe"
        +"r north, but"
        +" noticing the crumbling rockface just above to your north, deci"
        +"de against it.\n");
...
//Adding in NPC
    add_npc(NPC_DIR + "crab", random(6)-2);

//Adding in objects (debris)
    add_object(OBJ_DIR + "/debris/bottle", random(4)-2);
    add_object(OBJ_DIR + "/debris/driftwood", random(4)-2);
