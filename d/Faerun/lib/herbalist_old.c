...
#undef  DEBUGGING
#ifdef  DEBUGGING
#define DEBUG_WHO   find_player("palmer")
 ...
 * Description   : gives the herbalist 100 herb wraps
 */
public void
add_wraps()
{
//    Herbs += (["wrap": ({100, 33, WRAP})]);
}
...
do_buy(string str)
{
	...
	return do_list(str);
/*
    if (str == "wraps" || str == "herb wrap" || str == "herb wraps")
        str = "wrap";
*/
    words = explode(str, " ");
    if (sizeof(words) > 1)
...
do_list(string str)
{
    int i, price, price2, herb_amt, herb_amt2,wrap_count;
...
//left column
        ...
        /*
        if (herbname=="wraps" || herbname=="wrap")
        {
            // dont want wraps showing up in list. they aint herbs.
            wrap_count=i;
            i++;
            herbname = herb_list[i];
        }
...
//        /*
//        if (herbname2=="wraps" || herbname2=="wrap")
//        {
//            // dont want wraps showing up in list. they aint herbs.
//            wrap_count=i;
//            i++;
...
/*
    if (wrap_count>0)
    {
        list = sprintf("%4d  wraps remaining.",wrap_count);
//        write("\n " + wrap_count + " wraps remaining.\n");
        write("\n" + list + "\n");
    }
*/
//	TP->more(list);
