...
/* ACTION_BLIND - This action is not displayed towards those that cannot
 * see the actor.  Use this instead of the *bb() functions.
 */
#ifndef ACTION_BLIND
#define ACTION_BLIND 524288
#endif
 ...
 */
public static string parse_msg = "";
 ...
 *		  int poss	 - return oblist in possessive form.
 */
public varargs void
actor(string str, object *oblist, string str1, int poss)
{
    if (living(oblist[0]))
    {
	...
	write(str + " " + COMPOSITE_WORDS(map(oblist, oname)) +
	  (strlen(str1) ? (str1 + "\n") : ".\n"));
    }
    else
    {
	write(str + " " + COMPOSITE_WORDS(map(oblist,
 ...
 *		  int poss	 - return actors name in possessive form.
 */
public varargs void
target(string str, object *oblist, string adverb, int cmd_attr, int poss)
{
    int size;
    object *npcs, *players;
    ...
    if (!living(oblist[0]))
    {
	oblist->emote_hook(query_verb(), this_player(), adverb, 
	    oblist, cmd_attr, 1);
	return;
    }
	...
	while(--size >= 0)
	    players[size]->catch_msg(name(players[size]) + str + "\n");
    }
 ...
 *                This is an obsolescent wrapper.
 *                Please use ACTION_BLIND in your cmd_attr arg.
 * Example      : targetbb(" smiles happily.", oblist);
 * Arguments    : see target().
 */
public varargs void
targetbb(string str, object *oblist, string adverb, int cmd_attr, int poss)
{
    target(str, oblist, adverb, cmd_attr | ACTION_BLIND, poss);
}
 ...
 *                string adverb - the optional adverb if one was used.
 *                int cmd_attr	- the action attributes (from cmdparse.h)
 *		  int poss	- print actors name in possessive form.
 */
public varargs void
all(string str, string adverb, int cmd_attr, int poss)
{
    int size;
    object *oblist, *npcs, *players;

    oblist = all_inventory(environment(this_player()));
    oblist = FILTER_OTHER_LIVE(oblist);
 ...
 *                This is an obsolescent wrapper function.
 *                Please use ACTION_BLIND in your cmd_attr arg.
 * Example      : allbb(" smiles happily.");
 * Arguments    : see all().
 */
public varargs void
allbb(string str, string adverb, int cmd_attr, int poss)
{
    all(str, adverb, cmd_attr | ACTION_BLIND, poss);
}
 ...
 *                string adverb  - the optional adverb if one was used.
 *                int cmd_attr   - the action attributes (from cmdparse.h)
 *		  int poss_obj	 - returns oblist in possessive form.
 *		  int poss_act	 - returns actors name in possessive form.
 */
public varargs void
all2act(string str, object *oblist, string str1, string adverb,
        int cmd_attr, int poss_obj, int poss_act)
{
    int    size;
    object *livings, *npcs, *players;
    ...
    {
	livings -= oblist;
	livings = FILTER_OTHER_LIVE(livings);
    }
    ...
    npcs = livings - players;

    if (size = sizeof(players))
    {
	function name = (poss_act == 0 ?
	    this_player()->query_The_name :
	    this_player()->query_The_possessive_name);
	...
	else
	{
	    while(--size >= 0)
	    {
		players[size]->catch_tell(name(players[size]) +
		  str + " " + COMPOSITE_WORDS(map(oblist,
		      &desc_theshort(, players[size], poss_obj))) + str1);
	    }
	}
    }
    ...
    if (sizeof(npcs))
    {
	npcs->emote_hook_onlooker(query_verb(), this_player(), adverb, 
	    oblist, cmd_attr);
    }
}
...
parse_this(string str, string form, int cmd_attr, int allow_self)
{
    object *oblist;
    ...
    return oblist = check_block_action(oblist, cmd_attr);
}
...
parse_adverb(string str, string def_adv, int trail)
{
    ...
    {
#ifndef Genesis
	if (!this_player()->query_option(OPT_DEF_ADVERBS))
	    def_adv = BLANK_ADVERB;
#endif
	return ({ 0, def_adv });
    }
