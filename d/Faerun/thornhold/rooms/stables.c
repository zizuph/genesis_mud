...
#define D_STABLES_DIR		"/d/Faerun/stables/"
...
create_house()
{
    ...
    set_long("You are in the Thornhold stables. Dappled " +
    		 "light spills in from the courtyard, through " +
    		 "spaces in the wooden " +
             "planks that were used to construct the stable entrance. " +
             "In contrast, the ceiling and exterior walls are " +
             "constructed of " +
             "the same sturdy gray stone of the rest of the hold. " +
             "A thick layer of hay blankets the floor here, and " +
             "several box stalls " +
             "with feeding and watering troughs for horses, ponies, " +
             "and mules currently in the care of the dwarves of " +
             "Thornhold. You can enter a farrier's workshop to the " +
             "north, or return west to the courtyard.\n");

	add_item(({ "hay", "hay blankets" }),
    ...
    add_item(({ "workshop", "farriers workshop" }),
    	"You can see the Farrier's workshop to the north.\n");
        
    add_item(({ "light", "dappled light", "rays of dappled light" }),
    	"Rays of dappled light sprinkle down from the ceiling " +
    	"above.\n");
    
	add_item(({ "thornhold", "Thornhold", "hold", "fortress", "fort" }),
    	"You are standing in the stables of Thornhold.\n");
    
    add_item(({ "stalls", "box stalls"}),
    	"Slots or spaces barely wide enough to fit a large horse. " +
    	"Where horses are kept.\n");

    add_cmd_item("poster", ({"read", "exa", "examine"}), 
    "@@read_poster@@");
...
yard_security()
{
    // Block all but wizards, the stable master, 
    // and horses from entering the corral
    ...
    write("You can't seem to enter the corral. Please bug " +
            "report this.\n");
    return 1;
}
...
corral_desc()
{
    ...
    if (!LOAD_ERR(STABLES_ROOM_DIR + "corral"))
        ...
        return "ERROR: There was an error in the corral descr.";

    opeople = FILTER_LIVE(all_inventory(other_room));
    if (!sizeof(opeople))
        return "Behind the corral fence you see no horses.\n";
    else
        return "Behind the corral fence you see " + 
        COMPOSITE_LIVE(opeople) + ".\n";
}
...
read_sign()
{
    ...
    if (!LOAD_ERR(STABLES_ROOM_DIR + "corral"))
        ...
        return "ERROR: We have a problem with the corral sign. " +
