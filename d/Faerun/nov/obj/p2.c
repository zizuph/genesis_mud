...
create_object()
{
    ...
    set_adj("huge");
    add_adj("red");
    set_short("huge green portal");
    set_long("This is a huge green portal which has a peculiar " +
...
do_port(string str)
{
        ...
        tell_room(DEST2, this_player()->query_name() + " arrives in a puff of smoke!\n", this_player());
        
        tell_room(environment(this_object()), this_player()->query_name() + " enters " +
        "the "+short()+"!\n", this_player());
                
        this_player()->move_living("M", DEST2, 1);
