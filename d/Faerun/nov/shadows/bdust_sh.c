...
void
remove_bonedust_shadow()
{
	
    //invis_level2 = shadow_who->query_prop(OBJ_I_INVIS);
	
    shadow_who->tell_watcher("The bonedust on "+QCTNAME(shadow_who)+"'s body fades away.\n");
    shadow_who->add_prop(OBJ_I_INVIS, shadow_who->query_prop(OBJ_I_INVIS) + 50);
    //shadow_who->add_prop(OBJ_I_INVIS, invis_level2);
    shadow_who->remove_prop(IHAVEBONEDUST);
    tell_object(shadow_who, "The bonedust fades away.\n");
...
init_bonedust_shadow(int i)
{
      ...
      shadow_who->add_prop(IHAVEBONEDUST, 1);
    
      set_alarm(itof(i),0.0,&remove_bonedust_shadow());
}