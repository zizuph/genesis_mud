...
create_nov_room()
{
    ...
    add_exit(NOV_DIR + "temple/nmagic",   "west", block_studyers);
...
void
research_spell2(int level, object pl)
{
...
end_tome()
{
           ...
           if (this_player()->query_skill(RESEARCH2) >= 90)
		{
			...
			this_player()->remove_prop(LIVE_S_EXTRA_SHORT);
			this_object()->research_spell2(2, this_player());
	      ...
	      paralyze = clone_object("/std/paralyze");
           ...
           this_player()->set_skill(RESEARCH2, this_player()->query_skill(RESEARCH2) + random(RTIME));
    	   tell_room(environment(this_player()), QCTNAME(this_player())+" closes the tome " +
...
do_study(string str)
{
	...
	// The actual research.
	        ...
	        if (!this_player()->query_skill(RESEARCH2))
	        {
	        	this_player()->set_skill(RESEARCH2, 1);
	        }
