...
create_neverw_outdoor()
{
    ...
    set_short("Main Street of Protector's Enclave");
    
    //If we want herbs here.
    //add_std_herbs("mere");
        
    add_item(({"sign"}),
       "The sign displays directions for places of interest in town, "
       +"perhaps if you read the sign, you might find where you are.\n ");
    ...
    add_exit(ROOM_DIR + "n2.c", "south");
    add_exit(ROOM_DIR + "n4.c", "north");
	add_exit(ROOM_DIR + "nae1.c", "east");
