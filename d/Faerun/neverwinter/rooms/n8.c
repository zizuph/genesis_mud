...
block_check()
{      
    write("Knights in shining silver armor block the way, halting "
       +"all travel on to the bridge towards Castle Neverwinter.\n");
...
create_neverw_outdoor()
{
    extraline = "The street has narrowed immensely compared to the main "
	   ...
	   +"a bridge lies to the northwest, giving way to Castle "
	   +"Neverwinter.";
	...
	add_item(({"street"}),
       ...
       +"the main street of Protector's Enclave. The workers have yet "
	   +"to get to this part of the street, and so it remains in some "
	   +"would say, shambles.\n");

    reset_faerun_room();
    
	add_exit(ROOM_DIR + "n9.c", "southeast");
	add_exit(ROOM_DIR + "n7.c", "southwest");
	add_exit(ROOM_DIR + "w1.c", "northwest", block_check);
}
