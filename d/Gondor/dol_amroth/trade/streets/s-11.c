/*
 * trade/streets/s-11.c
 *
 * By Skippern 20(c)01
 *
 * A jatta jatta jatta
 */
#include "../../defs.h"

inherit DOL_STD + "city.c";

public void
create_dol_amroth()
{
    set_short("a dirty street");
    set_extralines("The street goes right by the waterfront.");
    set_lastline("To the west a large square is seen.");

    set_no_of_lamps(2);
    reset_room();

    set_sounds("Everywhere is a buzzing sound of people talking.");
    set_sound_detail("Some people are arguing over the price and the " +
                     "quality of the merchendice.");

    street_tells();

    add_exit(DOL_TRADE + "streets/t-12", "southeast", 0, 1);
    add_exit(DOL_TRADE + "streets/r-11", "west", 0, 1);
    clone_object(DOL_OBJ + "doors/s-11-door")->move(this_object());
}

