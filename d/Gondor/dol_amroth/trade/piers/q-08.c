...
create_dol_amroth()
{
    ...
    set_lastline("Above the warehouse there is a light in a window.");
    ...
    add_exit(DOL_TRADE + "piers/r-08", "east", 0, 1);
    add_exit(DOL_TRADE + "piers/p-08", "west", 0, 1);

    o = clone_object(DOL_OBJ+"doors/q-08-door");
    o->move(this_object());
}
...
reset_room()
{
	...
	o->add_name("ship");
	o->add_name("vessel");
	o->set_dest(DOL_OTHER+"ships/deck-2");
	o->move(this_object());
    }
}
