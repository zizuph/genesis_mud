...
create_gondor()
{
    ...
    add_item("pond", "In the middle of the garden there is a small pond. " +
	     "In the pond small waterlillys float, and when you look " +
    ...
    set_drink_from( ({"pool", "pond", "fountain"}) );

    add_exit(DOL_HILLS + "streets/h-11", "east", 0, 1);
    add_exit(DOL_HILLS + "g-11/garden2", "northwest", 0, 1);
    add_exit(DOL_HILLS + "g-11/garden4", "southwest", 0, 1);

    o = clone_object(DOL_NPC + "g11-imp");
    o->move(this_object());
...
reset_room()
{
    ...
    o = clone_unique(DOL_NPC + "g11-imp", 1);
    o->move(this_object());
}
