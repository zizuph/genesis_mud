...
create_inside()
{
    ...
    set_long("You are in an arched corridor going round the main hall. " +
	     ...
	     "on the south wall, but only one is open on each of the walls.\n");

    add_exit(DOL_HILLS + "i-12/uphall2", "north", 0, 1);
    add_exit(DOL_HILLS + "i-12/uphall3", "south", 0, 1);
    add_exit(DOL_HILLS + "i-12/hall1", "down", 0, 1);
}
