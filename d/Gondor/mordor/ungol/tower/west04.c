/*
 * /d/Gondor/mordor/ungol/tower/west04.c
 *
...
create_room()
{
    ...
    set_long(BSN("You are at ground level in the tower at Cirith Ungol. " +
        ...
        "are in a prison hall, which continues to the east. To the north " +
        "are several small cells where prisoners, captured here, are held " +
        "until they are send further for questioning or punishment. Several " +
        "torches shed a little light on the walls. To the south you see a " +
        "great arched gate."));
    ...
    make_door(CIRITH_DIR + "west03", "north", 2);

    add_exit(CIRITH_DIR + "west01", "east");
    add_exit(CIRITH_DIR + "west05", "south");
