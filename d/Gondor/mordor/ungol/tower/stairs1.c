/*
 * /d/Gondor/mordor/ungol/tower/stairs1.c
 *
...
create_room()
{
    make_torch();    /* don't get an add_item torch     */
    make_the_room(); /* including several add_items :-) */

    set_name("Ungol tower, first floor");
    add_name( ({ "room", "passage" }) );

    set_short("The tower at Cirith ungol at the first floor in the staircase");
    set_long(BSN("You are at the first floor in the tower at Cirtih Ungol. " +
        "This hallway continues to the south. There are some openings in " +
        "north wall, in it you vaguely recognize some stairs leading up and " +
        "down in this dreadful place."));
    ...
    make_door(CIRITH_DIR + "west13", "west", 1);

    add_exit(CIRITH_DIR + "winding0", "northeast", "@@walk_northeast", 2);
    add_exit(CIRITH_DIR + "winding1", "northwest", "@@walk_northwest", 2);
    add_exit(CIRITH_DIR + "west11",   "south");
