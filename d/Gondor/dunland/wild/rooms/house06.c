...
create_gondor()
{
	...
	+ "house. They are not adorned at all, except for the "
	+ "southern wall where some have carved in a figure."));
    add_item(({"figure", "south wall", "southern wall", 
	"theoden", "carving"}), "@@exa_carving");
    ...
    add_item("ceiling", "The ceiling is made of wood. It looks "
      ...
      + "rain in Rohan.\n");
    add_item(({"crumbs", "bread crumbs"}), BSN(
	"Bread crumbs are all over the small table."));
    ...
    add_exit(DUN_WILD_ROOMS + "field13", "out");

    add_npc(DUN_WILD_NPC + "dunl_soldier", 2 + (random(4)));
}

public string
exa_carving()
{
    return "The figure apparently depicts Theoden. "
    + "There looks to be some marks from thrown "
    + "knives around it:\n\n"
    + read_file(DUN_WILD_OBJ + "carving");
}
