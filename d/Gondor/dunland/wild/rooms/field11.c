...
create_area_room()
{
    ...
    set_land("Enedwaith");
    set_extraline("You find yourself in the south part of a "
      ...
      + "can spot a plow. To your south you can enter one of "
      + "the houses.");
	...
	"In the middle of the field, just to the north of here, "
	+ "you can see a plow, it does not look to be recently "
	+ "used, but you would have to get closer to be sure."));

    set_no_exit_msg("southwest", "The way to the southwest is " +
        "blocked by an old fence. You see nothing of interest " +
        "behind it.\n");
    set_no_exit_msg("southeast", "The way to the southeast is " +
        "blocked by an old fence. You see nothing of interest " +
        "behind it.\n");
    set_no_exit_msg("south", "To the south stands a house. To " +
        "enter it, you just need to walk inside.\n");

    add_exit(DUN_WILD_ROOMS + "house05", "in", 0, 2);
    add_exit(DUN_WILD_ROOMS + "field10", "west", 0, 2);
    add_exit(DUN_WILD_ROOMS + "field06", "north", 0, 2);
    add_exit(DUN_WILD_ROOMS + "field12", "east", 0, 2);
    add_exit(DUN_WILD_ROOMS + "field05", "northwest", 0, 2);
    add_exit(DUN_WILD_ROOMS + "field07", "northeast", 0, 2);
