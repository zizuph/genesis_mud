...
create_area_room()
{
    ...
    set_no_exit_msg("southeast", "To the southeast you only see " +
        "the western wall of a house. You decide not to go there.\n");
    set_no_exit_msg("south", "The way to the south is blocked by " +
        "an old fence. You see nothing of interest behind it.\n");

    add_exit(DUN_WILD_ROOMS + "field11", "west", 0, 2);
    add_exit(DUN_WILD_ROOMS + "field07", "north", 0, 2);
    add_exit(DUN_WILD_ROOMS + "field13", "east", 0, 2);
    add_exit(DUN_WILD_ROOMS + "field06", "northwest", 0, 2);
    add_exit(DUN_WILD_ROOMS + "field08", "northeast", 0, 2);
