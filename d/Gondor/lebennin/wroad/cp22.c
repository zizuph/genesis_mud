...
create_lebennin_room()
{
    ...
    add_exit(AREA_DIR + "wroad/cp23", "east", 0, 1);
    add_exit(AREA_DIR + "wroad/cp17", "southeast", 0, 1);
    add_exit(AREA_DIR + "wroad/cp16", "south", 0, 1);

    set_no_exit_msg(get_all_no_exits(({"e", "se", "s"})),
