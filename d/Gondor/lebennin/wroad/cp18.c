...
create_lebennin_room()
{
    ...
    add_exit(AREA_DIR + "wroad/cp24", "north", 0, 1);
    add_exit(AREA_DIR + "wroad/cp23", "northwest", 0, 1);
    add_exit(AREA_DIR + "wroad/cp17", "west", 0, 1);
    add_exit(AREA_DIR + "wroad/cp20", "southwest", 0, 1);
    add_exit(AREA_DIR + "wroad/cp21", "south", 0, 1);

    set_no_exit_msg(get_all_no_exits(({"nw", "n", "w", "sw", "s"})),
