...
create_lebennin_room()
{
    ...
    add_exit(AREA_DIR + "wroad/cp4", "north", 0, 1);

    set_no_exit_msg(({"west", "northwest", "southwest"}), "The current of " +
        "the river is too swift for swimming.\n");
    set_no_exit_msg(({"south", "east", "southeast", "northeast"}), "Dense " +
        "bushes block the way.\n");
}
