...
create_lebennin_room()
{
    ...
    add_exit(AREA_DIR + "wroad/cp18", "south", 0, 1);
    add_exit(AREA_DIR + "wroad/cp17", "southwest", 0, 1);
    add_exit(AREA_DIR + "wroad/cp23", "west", 0, 1);

    set_no_exit_msg(get_all_no_exits(({"s", "sw", "w"})),
