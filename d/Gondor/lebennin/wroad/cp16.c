...
create_lebennin_room()
{
    ...
    add_exit(AREA_DIR + "wroad/cp22", "north", 0, 1);
    add_exit(AREA_DIR + "wroad/cp23", "northeast", 0, 1);
    add_exit(AREA_DIR + "wroad/cp17", "east", 0, 1);
    add_exit(AREA_DIR + "wroad/cp20", "southeast", 0, 1);
    add_exit(AREA_DIR + "wroad/cp19", "south", 0, 1);
    add_exit(AREA_DIR + "wroad/cp15", "west", 0, 1);

    set_no_exit_msg(get_all_no_exits(({"n", "ne", "e", "se", "s", "w"})),
