...
create_lebennin_room()
{
    ...
    add_exit(AREA_DIR + "wroad/cp16", "northwest", 0, 1);
    add_exit(AREA_DIR + "wroad/cp17", "north", 0, 1);
    add_exit(AREA_DIR + "wroad/cp18", "northeast", 0, 1);
    add_exit(AREA_DIR + "wroad/cp21", "east", 0, 1);
    add_exit(AREA_DIR + "wroad/cp19", "west", 0, 1);

    set_no_exit_msg(get_all_no_exits(({"nw", "n", "ne", "e", "w"})),
