/*
 * /d/Gondor/lebennin/sroad/path9.c
 *
 * Copyright (C) May 27, 1998 by Ron Miller (Tyr)
 *
 * This is the eastern edge of a clearing where the lens taken from 
...
create_gondor()
{
    ...
    set_long(BSN("This is the eastern edge of a small clearing. "+
        "The remains of a small campfire to the west suggest "+
        "that someone recently camped here."));

    add_item(({"edge","eastern edge","clearing","small clearing",
    ...
    add_item(({"remains","campfire","small campfire","pile","ashes",
        ...
        "west lies a pile of wet ashes and partially burnt firewood."));
    ...
    add_item(({"west","center"}), BSN("The center of the camp lies to "+
        "the west."));
    ...
    add_exit("path8", "west", 0, 1);
    add_exit("path11","southwest", 0, 1);
    add_exit("path12", "south", 0, 1);
