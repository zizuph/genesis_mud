/*
 * Dol Guldur - /d/Gondor/rhovanion/dolguldur/ruins3.c
 *
...
string do_search(object searcher, string str)
{
    if (str == "ruins")
    {
        ...
        if (this_player()->query_prop(MM_GURUQUEST) == 3)
        {
...
create_dg_room()
{
    ...
    set_long("You are standing in the middle of a broken mass of rubble, " +
        ...
        "restored. Nearby to the northwest stands a tall tower, broken " +
    ...
    add_exit(DG_DIR + "ruins2", "north", "@@climb@@", 5, 1);
    add_exit(DG_DIR + "lane3", "southwest", "@@climb@@", 5, 1);
}
