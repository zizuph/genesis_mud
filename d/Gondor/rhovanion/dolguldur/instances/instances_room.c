 ...
 * Description  : If a ranger is present, it will block the desired exit.
 */
public int
block_player()
{
    if (present("_instance_rangernpc_"))
    {
...
add_instance_room()
{
    ...
    {
        instances_endroom = clone_object(RHOVANION_DIR
        +"dolguldur/instances/instances_endroom");

        query_instancetype_end_descr();

        this_object()->add_exit(file_name(instances_endroom),
        "opening", block_player);

        instances_endroom->add_exit(file_name(this_object()), "out");
...
create_dg_room()
{
	...
	add_npc(RHOVANION_DIR +"dolguldur/npc/scout_test", random(4));
