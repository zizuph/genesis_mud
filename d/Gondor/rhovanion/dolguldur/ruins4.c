/*
 * Dol Guldur - /d/Gondor/rhovanion/dolguldur/ruins4.c
 *
...
string do_search(object searcher, string str)
{
    if (str == "ruins")
    {
        ...
        if (this_player()->query_prop(MM_GURUQUEST) == 4)
        {
...
create_dg_room()
{
    ...
    set_long("You are standing in the middle of a broken mass of rubble, " +
        "clearly this is a part of Dol Guldur which has yet to be " +
        "restored. This looks like it was once a residential area, with " +
        "remains of broken furniture strewn throughout the rubble. The " +
        "tall tower in the middle of Dol Guldur is northeast of here, " +
        "broken and jagged as it stands out in the " + VBFC("daylight_desc") +
    ...
    add_item( ({"furniture", "broken furniture"}),
        "You can see the remains of beds, tables and chairs all mixed " +
        "in with the rubble here. You get the feeling that there were " +
        "most likely several homes here.\n");
    add_item( ({"homes", "broken homes", "residential area"}),
        "There are no homes here anymore, everything is broken.\n");
    ...
    add_exit(DG_DIR + "main3", "east", "@@climb@@", 5, 1);
    add_exit(DG_DIR + "ruins5", "west", "@@climb@@", 5, 1);
}
