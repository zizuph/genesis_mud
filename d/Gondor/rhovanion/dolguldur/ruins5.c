/*
 * Dol Guldur - /d/Gondor/rhovanion/dolguldur/ruins5.c
 *
...
string do_search(object searcher, string str)
{
    if (str == "ruins")
    {
        ...
        if (this_player()->query_prop(MM_GURUQUEST) == 5)
        {
...
create_dg_room()
{
    ...
    set_long("You near the center of Dol Guldur, with the tall tower at " +
        "the heart of the city rising up to the northeast, broken " +
        "and jagged as it stands out in the " + VBFC("daylight_desc") +
        ". You are close to what appears to be the ruins of some sort of " +
        "shrine, although it is so thoroughly destroyed, it is hard to " +
        "say for sure. The rubble continues in every direction, but you " +
        "do see some restored buildings off to the south. There is not " +
    ...
    add_item( ({"shrine", "destroyed shrine", "destroyed building",
            "building"}),
        "Your best guess would be that these remains were once a " +
        "shrine of some sort, but it really is impossible to know " +
        "for certain.\n");
    ...
    add_exit(DG_DIR + "ruins4", "east", "@@climb@@", 5, 1);
    add_exit(DG_DIR + "ruins6", "west", "@@climb@@", 5, 1);
}
