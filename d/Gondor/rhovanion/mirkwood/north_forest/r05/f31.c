...
void create_mirkwood_north_forest_forest_room() {
    ...
    add_exit(MIRKWOOD_NORTH_FOREST_DIR + "r06/f30", "northwest", "@@block@@",2,1);
    add_exit(MIRKWOOD_NORTH_FOREST_DIR + "r06/f31", "north", "@@block@@",2,1);
    add_exit(MIRKWOOD_NORTH_FOREST_DIR + "r06/f32", "northeast", "@@block@@",2,1);
    add_exit(MIRKWOOD_NORTH_FOREST_DIR + "r05/f32", "east", "@@block@@",2,1);
    ...
    add_item_light(({"river", "forest river"}),
        ...
        "from the west to the southeast before it empties out into " +
        "Long Lake.\n", LIGHT_ALL);

    set_river_name("forest river");
        add_follow_direction(MIRKWOOD_NORTH_FOREST_DIR + "r04/f32", "southeast", 
            "downstream towards Esgaroth");
        add_follow_direction(MIRKWOOD_NORTH_FOREST_DIR + "r05/f30", "west", 
