...
void create_mirkwood_north_forest_forest_room() {
    ...
    add_exit(MIRKWOOD_NORTH_FOREST_DIR + "r04/f33", "southeast", "@@block@@",2,1);
    add_exit(MIRKWOOD_NORTH_FOREST_DIR + "r04/f32", "south", "@@block@@",2,1);
    add_exit(MIRKWOOD_NORTH_FOREST_DIR + "r04/f31", "southwest", "@@block@@",2,1);
    add_exit(MIRKWOOD_NORTH_FOREST_DIR + "r05/f31", "west", "@@block@@",2,1);
    ...
    add_item_light(({"river", "forest river"}),
        ...
        "from the northwest to the east before it empties out into " +
        "Long Lake.\n", LIGHT_ALL);

    set_river_name("forest river");
    add_follow_direction(MIRKWOOD_NORTH_FOREST_DIR + "r06/f31", "northwest", 
        "upstream, deeper into Mirkwood");
    add_follow_direction(MIRKWOOD_NORTH_FOREST_DIR + "r05/f33", "east", 
