...
void create_mirkwood_north_forest_forest_room() {
    ...
    add_exit(MIRKWOOD_NORTH_FOREST_DIR + "r06/f27", "northwest", "@@block@@",2,1);
    add_exit(MIRKWOOD_NORTH_FOREST_DIR + "r06/f28", "north", "@@block@@",2,1);
    add_exit(MIRKWOOD_NORTH_FOREST_DIR + "r06/f29", "northeast", "@@block@@",2,1);
    add_exit(MIRKWOOD_NORTH_FOREST_DIR + "r05/f29", "east", "@@ench@@",2,1);
    add_exit(MIRKWOOD_NORTH_FOREST_DIR + "r04/f29", "southeast", "@@ench@@",2,1);
    add_exit(MIRKWOOD_NORTH_FOREST_DIR + "r04/f28", "south", "@@ench@@",2,1);

    add_long("You are standing on a sliver of land which narrows the further " +
        "it goes northeast, as two mighty rivers crash together and merge " +
        "here before continuing their journey east through the forest.",
    ...
    set_river_name("forest river");
        add_follow_direction(MIRKWOOD_NORTH_FOREST_DIR + "r05/f27", "west", 
            "upstream, deeper into Mirkwood");
    set_river_name("enchanted river");
        add_follow_direction(MIRKWOOD_NORTH_FOREST_DIR + "r04/f27", "southwest", 
            "upstream, deeper into Mirkwood");
}
void
init()
