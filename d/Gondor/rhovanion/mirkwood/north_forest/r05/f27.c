...
void create_mirkwood_north_forest_forest_room() {
    ...
    add_exit(MIRKWOOD_NORTH_FOREST_DIR + "r06/f26", "northwest", "@@block@@",2,1);
    add_exit(MIRKWOOD_NORTH_FOREST_DIR + "r06/f27", "north", "@@block@@",2,1);
    add_exit(MIRKWOOD_NORTH_FOREST_DIR + "r06/f28", "northeast", "@@block@@",2,1);
    add_exit(MIRKWOOD_NORTH_FOREST_DIR + "r04/f28", "southeast", "@@ench@@",2,1);

    add_long("To the north, you can see the wide, rushing waters of a " +
        "large river as it cuts through the forest while not far to the " +
        "southeast, a second, smaller river winds its way through the black " +
        "trees.",
    ...
    add_item_light(({"river", "rivers"}),
        ...
        "the forest, meeting up somewhere to the east.\n", LIGHT_ALL);
    add_item_light(({"enchanted river", "second river"}),
        "Rushing as it winds through the black trees far to the southeast, " +
        "is the Enchanted river. This river is rumoured to be full of an " +
        "ancient elven magic which protects the Woodland Realm.\n", LIGHT_ALL);

    set_river_name("forest river");
        add_follow_direction(MIRKWOOD_NORTH_FOREST_DIR + "r05/f28", "east", 
            "downstream towards Esgaroth");
        add_follow_direction(MIRKWOOD_NORTH_FOREST_DIR + "r05/f26", "west", 
