...
int horses,max_horses,livings,max_livings,speed,leavable,one_tour_only;
string *tour, *retour;
string goal;
...
retour_move(int turn)
{
  ...
  while (n<maxn)
	...
	this_object()->move(exit_arr[n*3]);
	tell_object(exit_arr[n*3],"A wooden carriage pulled by horses comes rambling in.\n");
      }
      n++;
    }
