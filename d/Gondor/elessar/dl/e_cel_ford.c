...
#define OTHER_ROOM  (LORIEN_DIR + "paths/w_cel_ford")
...
{
    loc = 9;
    set_short_desc("by the east bank of the Silverlode");
    set_show_time();
    set_show_feel();
    extraline = "The Celebrant is already a strong stream here, running " +
      "both swift and deep. It does not seem " +
      ...
      "It seems possible to walk east through the thickets. ";
    ...
    add_exit(LORIEN_DIR + "paths/c_path1","east",0);
    add_exit(OTHER_ROOM, "west", "@@cross_river");
...
leave_inv(object ob, object to)
{
    ...
    if (ob->query_prop(OBJ_I_HIDE))
        return;
    tell_room(OTHER_ROOM, "Across the river you see " +
        QNAME(ob) + " disappearing into the thickets.\n");
}
...
{
    if (OTHER_ROOM->query_tied_rope() == rope->query_prop(LORIEN_ROPE_END))
    {
        cur_alarm = set_alarm(30.0,0.0,"current_pull",rope);
...
do_throw(string str)
{
    ...
    if(sscanf(str,"%s acros%s", what, dir) != 2 && sscanf(str,"%s wes%s", what, dir) != 2)
        return 0;
