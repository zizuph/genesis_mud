/*
 * /d/Gondor/river/anduin/lorien2rauros/river20.c
 *
...
create_anduin_onwater()
{
	...
	BSN("just above sarn gebir, about 337.9 miles downstream from "+
	    "the silverlode."));
    set_short("on the River Anduin");

    set_tod_long(
	"Sharp rocks thrust far out into the river and only a few yards "+
	"ahead the Anduin crashes onto the teeth of Sarn Gebir. "+
	"Doom is at hand!");
    ...
    add_exit("river19", "north");
    add_exit("river21", "south");
    add_exit("riverbottom20", "down", no_dive);	/* never succeeds */
    /* no shore rooms */
    ...
    add_item( ({ "jagged rocks", "sharply pointed rocks", "pointed rocks",
    	"great serrated teeth", "serrated teeth", "stone teeth", "teeth",
	"rocky shoals", "shoals", "rock", "rocks" }),
	BSN("The rocks stick up from the river like great serrated teeth, "+
	    "ripping the waters as they race past."));
	...
	BSN("The stone teeth of Sarn Gebir loom only a few yards "+
	    "downstream."));
} /* create_anduin_onwater */
...
do_listen(string arg)
{
    ...
    if (!strlen(arg))
    {
	WRITE("The waters crash onto the rocks with the roar of thunder!");
	return 1;
    }
	...
	"[to] [the] [loud] 'crash' / 'roar' / 'thunder' "+
	"/ 'noise' / 'sound' / 'rapids' / 'sarn' [gebir]"))
    {
	WRITE("The waters crash onto the rocks with the roar of thunder!");
    }
    else
...
exa_noise_icky(string arg)
{
    ...
    {
	if (parse_command(arg, ({}),
	    "[at] [the] [loud] 'crash' / 'roar' / 'thunder' "+
	    "/ 'noise' / 'sound' "))
	{
