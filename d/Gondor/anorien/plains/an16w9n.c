/*
 * Anorien - /d/Gondor/anorien/plains/an16w9n.c
 *
 * Varian 2016
 */
...
create_plains_room()
{
    ...
    set_short("Anorien");
    set_long("You are wandering across the rolling plains of Anorien on the " +
        "banks of the Anduin, just north of the ruined city of Osgiliath. " +
        "The roar of the wide river churns loudly here, separating you " +
        "from the forest of Ithilien on the far bank. Green fields full of " +
        "wildflowers and tall grass spread out towards the west, broken " +
        "up only by the groves of trees which are scattered across the " +
        "plains. To the south, the snow-capped peaks of the White Mountains " +
        "run along the horizon, with a large stone citadel anchored on " +
        "their eastern edge.\n");
    ...
    add_exit(PLAINS_DIR + "an15w10n","southwest",0,2,1);
    add_exit(PLAINS_DIR + "an16w10n","south",0,2,1);
    add_exit(PLAINS_DIR + "an15w9n","west",0,2,1);
    add_exit(PLAINS_DIR + "an15w8n","northwest",0,2,1);
    add_exit(PLAINS_DIR + "an16w8n","north",0,2,1);
