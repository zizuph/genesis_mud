/*
 * Anorien - /d/Gondor/anorien/plains/an16w6n.c
 *
 * Varian 2016
 */
...
create_plains_room()
{
    ...
    set_short("Anorien");
    set_long("You are wandering across the rolling plains of Anorien, " +
        "as the " + VBFC("light_type") + " shines down upon the " +
        "sparkling Anduin River which flows past your feet to the east. " +
        "Groves of trees grow all over the fields of colourful wildflowers " +
        "and tall green grass, while farther in the distance, the rocky " +
        "peaks of the White Mountains stretch out across the southern " +
        "horizon. To the east, you can see the forests of Ithilien " +
        "rising up on the other side of the river.\n");
    ...
    add_exit(PLAINS_DIR + "an15w7n","southwest",0,2,1);
    add_exit(PLAINS_DIR + "an16w7n","south",0,2,1);
    add_exit(PLAINS_DIR + "an15w6n","west",0,2,1);
    add_exit(PLAINS_DIR + "an15w5n","northwest",0,2,1);
    add_exit(PLAINS_DIR + "an16w5n","north",0,2,1);
