/*
 * Anorien - /d/Gondor/anorien/plains/an11w2n.c
 *
 * Varian 2016
 */
...
create_plains_room()
{
    ...
    set_short("Anorien");
    set_long("You are standing near the mouths of the Entwash as they " +
        "wander across the rolling plains of Anorien. The river bleeds " +
        "into the rushing waters of the Anduin River just east of here, " +
        "as it cuts through the green fields of tall grass and brightly " +
        "coloured wildflowers. Several groves of trees dot the plains, " +
        "and far to the south, you notice the jagged peaks of the " +
        "White Mountains reaching towards the " + VBFC("day_desc")  +
        " sky.\n");
    ...
    add_exit(PLAINS_DIR + "an12w2n", "east",0,2,1);
    add_exit(PLAINS_DIR + "an10w2n", "west",0,2,1);
    add_exit(PLAINS_DIR + "an10w3n","southwest",0,2,1);
    add_exit(PLAINS_DIR + "an11w3n","south",0,2,1);
    add_exit(PLAINS_DIR + "an12w3n","southeast",0,2,1);
