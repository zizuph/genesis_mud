/*
 * Anorien - /d/Gondor/anorien/plains/an9w2n.c
 *
 * Varian 2016
 */
...
create_plains_room()
{
    ...
    set_short("Anorien");
    set_long("You are in the middle of tall fields of green grass, where " +
        "the " + VBFC("daylight_desc") + " shines down upon Anorien. " +
        "The fields are filled with hundreds of colourful wildflowers, " +
        "and slowly grows into marshy wetlands as they near the " +
        "grey waters of the Entwash which flows nearby to the north. The " +
        "plains stretch out in all directions, dotted with dozens of " +
        "tree groves and stretching all the way to the jagged " +
        "peaks of the White Mountains which claw at the " +
        VBFC("sun_moon") + " across the southern horizon in the " +
        VBFC("day_desc") + " sky.\n");  
    ...
    add_exit(PLAINS_DIR + "an10w2n", "east",0,2,1);
    add_exit(PLAINS_DIR + "an8w2n", "west",0,2,1);
    add_exit(PLAINS_DIR + "an8w3n","southwest",0,2,1);
    add_exit(PLAINS_DIR + "an9w3n","south",0,2,1);
    add_exit(PLAINS_DIR + "an10w3n","southeast",0,2,1);
