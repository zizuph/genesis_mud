/*
 * Anorien - /d/Gondor/anorien/plains/an16w8n.c
 *
 * Varian 2016
 */
...
create_plains_room()
{
    ...
    set_short("Anorien");
    set_long("You are moving through the colourful wildflowers and the " +
        "tall green grass that grows upon the fields of Anorien. Hundreds " +
        "of trees have gathered in groves, all the way up to the banks " +
        "of the Anduin River which rushes past nearby to the east, " +
        "separating you from the dark forests of Ithilien on the far side. " +
        "The river runs south towards the broken ruins of Osgiliath before " +
        "continuing its journey towards the sea. Far away to the south " +
        "and west, you can see Minas Tirith standing proudly beneath the " +
        VBFC("sun_moon") + " above, vigilant on its perch at the edge " +
        "of the White Mountains that stretch across the horizon.\n");
    ...
    add_exit(PLAINS_DIR + "an15w9n","southwest",0,2,1);
    add_exit(PLAINS_DIR + "an16w9n","south",0,2,1);
    add_exit(PLAINS_DIR + "an15w8n","west",0,2,1);
    add_exit(PLAINS_DIR + "an15w7n","northwest",0,2,1);
    add_exit(PLAINS_DIR + "an16w7n","north",0,2,1);
