/*
 * Anorien - /d/Gondor/anorien/plains/an10w2n.c
 *
 * Varian 2016
 */
...
create_plains_room()
{
    ...
    set_long("The " + VBFC("daylight_desc") + " shines down upon " +
        "the tall green grass and wildflowers that surround you. Lush green " +
        "fields dotted with dozens of tree groves stretch out in all " +
        "directions, while the bright " + VBFC("light_type") + " dances " +
        "merrily along the surface of the Entwash that flows just north of " +
        "here on its journey east to the Anduin. Far to the south, the " +
        "rocky slopes of the White Mountains reach up towards the " +
    ...
    add_exit(PLAINS_DIR + "an11w2n", "east",0,2,1);
    add_exit(PLAINS_DIR + "an9w2n", "west",0,2,1);
    add_exit(PLAINS_DIR + "an9w3n","southwest",0,2,1);
    add_exit(PLAINS_DIR + "an10w3n","south",0,2,1);
    add_exit(PLAINS_DIR + "an11w3n","southeast",0,2,1);
