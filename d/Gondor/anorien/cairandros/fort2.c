/*
 * Cair Andros - /d/Gondor/anorien/cairandros/fort2.c
 *
 * Varian 2016
 */
...
{    
    ...
    set_long("You are wandering down a long corridor which runs east and " +
        "west along the southern end of the fort at Cair Andros. The " +
        "hallway is made from heavy stone blocks, broken up by a series " +
        "of sturdy wooden doors that line the northern side of the " +
        "corridor while several torches have been mounted along the " +
        "southern wall, providing plenty of light. The fortress appears " +
        "to be in good condition, as it appears Gondor is currently " +
        "maintaining an active garrison here.\n"); 
    ...
    add_exit(CAIRANDROS_DIR + "fort3","east",0,0,0);
    add_exit(CAIRANDROS_DIR + "fort1","west",0,0,0);

    door = clone_object(AN_OBJ_DIR + "storage_door_in_sw");
    door->move(TO);
}