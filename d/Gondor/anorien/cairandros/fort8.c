/*
 * Cair Andros - /d/Gondor/anorien/cairandros/fort8.c
 *
 * Varian 2016
 */
...
{    
    ...
    set_long("You are currently wandering inside the sturdy confines of a " +
        "large fortress that guards the island of Cair Andros. A long stone " +
        "hallway stretches out to the east, into the middle of the " +
        "fortress, while wide stone steps climb up the northern wall " +
        "to the parapet above. In the middle of the southern wall, you " +
        "can see a strong wooden door. This is the western end of the " +
    ...
    add_item(({"hall","hallway","corridor"}),
        ...
        "the fort.\n");
    add_item(({"wall", "walls"}),
        "You are at the western end of a long hallway, with walls to both " +
        "the north and south. Which wall did you want to examine?\n");
    add_item(({"west","western wall","southwest","northwest"}),
    ...
    set_no_exit_msg(({"northwest","northeast","southwest","southeast","west"}),
    ...
    add_exit(CAIRANDROS_DIR + "fort9", "east",0,0,0);
    add_exit(CAIRANDROS_DIR + "caf9","up",0,0,0);

    door = clone_object(AN_OBJ_DIR + "fort_door_in_nw");
    door->move(TO);
}