/*
 * Cair Andros - /d/Gondor/anorien/cairandros/fort7.c
 *
 * Varian 2016
 */
...
{    
    ...
    set_long("You are currently inside the fortress that guards the island " +
        "of Cair Andros. The interior of the fort is rather plain, having " +
        "been made for functionality rather than beauty. The same stone " +
        "blocks that spread across the floor and ceiling have also been " +
        "into the strong walls that line the corridor you are presently " +
        "walking along. Standing guard at both the northern and southern " +
        "ends of the hallway are thick doors made from sturdy oak which " +
        "grant passage into the rest of the fortress.\n"); 
    ...
    add_item(({"wall", "walls"}),
        "You are at the northern end of a long hallway, with walls to both " +
    ...
    add_item(({"north", "northern wall","northeast","northwest"}),
        "The corridor ends at the northern wall, where there is nothing " +
    ...
    add_item(({"west", "western wall"}),
        ...
        "unadorned and made up of nothing but large blocks of grey stone " +
        "with the occasional torch mounted upon it.\n");
    add_item(({"south", "southern hallway","southwest","southeast",
            "southern corridor"}),
        "Looking to the south, you can see the long stone hallway ending " +
    ...
    add_exit(CAIRANDROS_DIR + "fort6","south",0,0,0);

    door = clone_object(AN_OBJ_DIR + "hall_door_out_n");
    door->move(TO);
}