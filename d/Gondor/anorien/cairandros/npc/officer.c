...
string *helmets = ({ AN_ARM_DIR + "helm"});
string *body = ({AN_ARM_DIR + "plate" });
string *shield = ({ AN_ARM_DIR + "shield"});
string *arms = ({ AN_ARM_DIR + "bracer"});
string *legs = ({ AN_ARM_DIR + "greaves"});
string *misc = ({ AN_ARM_DIR + "brooch", AN_ARM_DIR + "bracelet", AN_ARM_DIR + "necklace" });
string *key = ({ AN_OBJ_DIR + "stornw_key" });
...
create_monster()
{
    ...
    set_stats(({220, 280, 220, 85, 95, 170}), 20);
    set_alignment(500);
...
{
    string *guardname;
    guardname = ({"thamanil", "bargandur", "borgan", "therandor", "aramil",
        "galbadan"});
...
physlook()
{
    ...
    physlooks = ({"tall", "blue-eyed", "grey-eyed", "brown-eyed", "imposing",
        ...
        "lean", "large", "thick-armed", "sturdy", "proud"});
