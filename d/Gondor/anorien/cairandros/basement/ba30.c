 ...
 * /d/Gondor/anorien/cairandros/basement/ba30.c
 *
 * Varian - March, 2020
 */
...
{    
    ...
    add_item( ({"south", "south wall", "southern wall"}),
        "Several wooden boxes appear to have been stacked up against " +
    ...
    add_item( ({"boxes", "box", "wooden box", "wooden boxes"}),
        "Upon closer inspection, you can see that these boxes " +
        "seem to be filled with all sorts of tools.\n");
    add_item( ({"tool", "tools", "hammer", "hammers", "saw", "saws",
            "building tool", "building tool"}),
        "There are all sorts of different building tools stored " +
        "inside the boxes here, from tools to saws and everything " +
        "in between. It seems likely that the men stationed here " +
        "are expected to perform their own repairs.\n");
    ...
    add_exit(CABASE_DIR + "ecor1", "north", 0, 0, 0);
...
desc()
{
    write("You are in the sleeping quarters for some of the " +
        "soldiers who protect Gondor here in Cair Andros. Dozens " +
        "of simple bunks have been set against the eastern and " +
        "western walls here, while a large pile of boxes has " +
        "been neatly stacked up against the grey stone of the " +
        "southern wall here. Looking through a wide opening " +
        "that splits the northern wall, you can see the corridor " +
        "you came in from.\n");
}