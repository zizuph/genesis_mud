 ...
 * /d/Gondor/anorien/cairandros/basement/ecor1.c
 *
 * Varian - March, 2020
 */
...
{    
    ...
    add_item( ({"west", "western corridor", "west corridor"}),
        "The corridor opens up into an intersection to the west, " +
        "where it meets a wide hallway running north and south.\n");
    ...
    add_exit(CABASE_DIR + "ecor2", "east", 0, 0, 0);
    add_exit(CABASE_DIR + "hall5", "west", 0, 0, 0);
    add_exit(CABASE_DIR + "ba29", "north", 0, 0, 0);
    add_exit(CABASE_DIR + "ba30", "south", 0, 0, 0);
...
desc()
{
    write("You are currently in the lower levels of the fortress " +
        "on the island of Cair Andros in Gondor. A long corridor " +
        "runs eastward from an intersection to the west, while openings " +
        "in the north and south walls lead into rooms where the " +
