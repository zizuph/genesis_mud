/*
 * Cair Andros - /d/Gondor/anorien/cairandros/bar1.c
 *
 * Varian 2016
 */
...
{    
    ...
    set_long("You are currently standing inside some barracks that " +
        ...
        "north where some more bunks have been laid out. The rest of " +
    ...
    add_item(({"north", "northern wall"}),
        "The northern wall is at the far end of the barracks, but what " +
    ...
    add_exit(CAIRANDROS_DIR + "bar2", "north",0,0,0);

    door = clone_object(AN_OBJ_DIR + "fort_door_out_se");
    door->move(TO);
}