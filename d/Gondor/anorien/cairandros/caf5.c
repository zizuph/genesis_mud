/*
 * Cair Andros - /d/Gondor/anorien/cairandros/caf5.c
 *
 * Varian 2016
 */
...
create_ca_room()
{
    ...
    set_long("You are wandering along the top of a tall fort on the " +
        ...
        "A pathway follows the parapet to both the south and the " +
        "west, while a nearby set of sturdy stone stairs marches down into " +
    ...
    set_no_exit_msg(({"southeast","east", "northwest","north","northeast"}),
    ...
    add_exit(CAIRANDROS_DIR + "caf8", "west",0,0,0);
    add_exit(CAIRANDROS_DIR + "caf6", "south",0,0,0);
    add_exit(CAIRANDROS_DIR + "fort5", "down",0,0,0);
