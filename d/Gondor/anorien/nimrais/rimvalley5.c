/*
 *	/d/Gondor/anorien/nimrais/rimvalley5.c
 *
 *	Coded by Olorin
 *
 *	Modification log:
...
create_area_room()
{
    ...
    set_extraline("You are in a south-going valley on the north side "
      ...
      + "the north. A small stream flows northward in the middle of "
      + "the valley. The valley really narrows here, and turns to "
      + "the east a bit, but you can still go on.");
    ...
    add_item(({"slope","slopes"}), BSN(
        ...
        "to climb them."));
    add_exit(ANO_DIR + "nimrais/rimvalley4","north",0,2);
    add_exit(ANO_DIR + "nimrais/quest/base","southeast",0,3);
...
do_climb(string str)
{
    ...
    case "slope":
    case "slopes":
        write(BSN("The slopes are exceedingly steep and covered with loose "
          + "rubble so that you cannot climb them."));
        return 1;
        break;
    default:
        return 0;
