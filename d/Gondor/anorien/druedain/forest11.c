 ...
 *   01-April-2005, Finwe:     Added auto messages to rangers watching so 
 *                             they know who passes by. 
 */
#pragma strict_types
...
create_area_room()
{
    ...
    add_exit(COMMON_DIR + "quest/enchant_seed/garden", "north", "@@north_msg@@", 2, 1);
    add_exit(DRUEDAIN_DIR + "forest11", "south", "@@south_msg@@", 2, 1);
    add_exit(DRUEDAIN_DIR + "forest10", "west",  "@@east_msg@@", 2, 1);
#ifndef NEW_RANGERS_OPEN
    add_exit(DRUEDAIN_DIR + "forest13", "east",  "@@west_msg@@", 2, 1);
#endif
...
{
    if(test_loc == 0)
        tell_room(RANGERS_ITH_TUNNEL + "thicket_entrance", QCTNAME(TP) + " went north towards a hut.\n");
    else
        tell_room(RANGERS_ITH_TUNNEL + "thicket_entrance", QCTNAME(TP) + " tried to go north but failed.\n");
}

void south_msg()
{
    if(test_loc == 0)
        tell_room(RANGERS_ITH_TUNNEL + "thicket_entrance", QCTNAME(TP) + " went south in the forest.\n");
    else
        tell_room(RANGERS_ITH_TUNNEL + "thicket_entrance", QCTNAME(TP) + " tried to go south but failed.\n");
}

void east_msg()
{
    if(test_loc == 0)
        tell_room(RANGERS_ITH_TUNNEL + "thicket_entrance", QCTNAME(TP) + " went east into the forest.\n");
    else
        tell_room(RANGERS_ITH_TUNNEL + "thicket_entrance", QCTNAME(TP) + " tried to go east but failed.\n");
}

void west_msg()
{
    if(test_loc == 0)
        tell_room(RANGERS_ITH_TUNNEL + "thicket_entrance", QCTNAME(TP) + " went west into the forest.\n");
    else
        tell_room(RANGERS_ITH_TUNNEL + "thicket_entrance", QCTNAME(TP) + " tried to go west but failed.\n");
}
