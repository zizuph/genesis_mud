/*
 *	/d/Gondor/anorien/druedain/forest4.c
 *
 *	Coded by Elessar.
 *
 *	Modification log:
...
create_area_room()
{
    ...
    add_exit(DRUEDAIN_DIR + "forest3",  "north", test_loc, 2, 1);
    add_exit(DRUEDAIN_DIR + "forest6",  "east",  test_loc, 2, 1);
    add_exit(DRUEDAIN_DIR + "forest4",  "south", test_loc, 2, 1);
    add_exit(DRUEDAIN_DIR + "forest14", "west",  test_loc, 2, 1);

    set_up_herbs( ({ ONE_OF_LIST_WITH_SEED(herbs, 401),
		     ONE_OF_LIST_WITH_SEED(herbs, 402),
		     ONE_OF_LIST_WITH_SEED(herbs, 403), }),
		  ({ "forest", "ground", }), 8);
}
