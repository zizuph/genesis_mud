...
create_room()
{
    ...
    set_long(BSN("You are in a rather dark and damp corridor with stone "
      + "walls. There is a little light coming from the northwest, while "
      + "only darkness is waiting to the east. The walls of the corridor "
      + "are smooth, never turning around corners, but rather bending "
      + "slightly in one direction as if forming a round circle."));
    add_prop(ROOM_M_NO_TELEPORT, check_teleport);
    add_prop(ROOM_I_INSIDE,1);
    add_prop(ROOM_I_LIGHT,0);
    add_item("walls",BSN("The walls are made of smooth white stone, though "
      + "the passage of hundreds of years since repair have left solid "
      + "tracks."));
    add_exit(OSG_DIR+"lvl1corr2","east",0,1);
    add_exit(OSG_DIR+"w1archway","northwest",0,1);
