...
int
gondor_monster_filter(object monster)
{
   if (monster->id(npc_name))
       return 1;
   return 0;
}
...
attacked_by(object ob)
{
   ...
   while (!room->query_prop(ROOM_I_IS))
      room = ENV(room);
   arr = filter(all_inventory(room), gondor_monster_filter);
