...
create_gondor()
{
    ...
    add_exit(WALL_TUNNEL_THREE + "set3.c",       "northeast");
...
move_player()
{
    ...
    tell_room(WALL_TUNNEL_THREE + "set1", QCNAME(TP) + " arrives from the "
        + "other end of the narrow stone walkway.\n");

    TP->move_living("M", WALL_TUNNEL_THREE + "set1", 1, 0);
    return 1;
}
