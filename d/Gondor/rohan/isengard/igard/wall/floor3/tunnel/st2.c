...
long_extratext()
{
    ...
    + "the southern part of the ring wall on the third level. "
    + "A narrow stone walkway leads to the east.\n";
...
create_gondor()
{
    ...
    add_exit(WALL_TUNNEL_THREE + "st1.c",        "west");

    add_item(({"tapestry", "tapestries"}), "Hanging from one of the walls is a " 
        + "fresh tapestry. Its green cloth has a wide weave that is shot "
        + "through with gold thread. The tapestry looks rather shiny.\n");
...
move_player()
{
    ...
    tell_room(WALL_TUNNEL_THREE + "st3", QCNAME(TP) + " arrives from the "
        + "other end of the narrow stone walkway.\n");

    TP->move_living("M", WALL_TUNNEL_THREE + "st3", 1, 0);
    return 1;
}
...
test_quest()
{
    ...
    TP->remove_prop(SENT_IGARD_WALL7);
