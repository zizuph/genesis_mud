...
long_extratext()
{
    ...
    + "the northern part of the ring wall on the third level. A wide "
    + "stairwell leads down to the second level.\n";
}
...
create_gondor()
{
    ...
    add_item(({"stairwell", "stairs", "steps", "step", "stair"}), "The "
        + "wide stairwell leads down to the second level of the ring "
        + "wall.\n");
    ...
    add_exit(WALL_TUNNEL_THREE + "net9.c",      "southeast");
    add_exit(WALL_TUNNEL_THREE + "nt2.c",       "west");
    add_exit(WALL_TUNNEL_TWO + "nt1.c",          "down");
