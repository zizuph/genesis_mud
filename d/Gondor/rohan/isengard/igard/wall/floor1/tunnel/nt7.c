...
create_gondor()
{
    ...
    add_exit(WALL_TUNNEL_ONE + "nt6.c",      "east");
    add_exit(WALL_TUNNEL_ONE + "nt8.c",      "west");
...
leave_room()
{
    ...
    tell_room(WALL_FLOOR_ONE + "1_archroom_1c.c", QCTNAME(TP) + " slips silently "
        + "into the room from behind the tapestry.\n");

    TP->move_living("M", WALL_FLOOR_ONE + "1_archroom_1c.c", 1, 0);
