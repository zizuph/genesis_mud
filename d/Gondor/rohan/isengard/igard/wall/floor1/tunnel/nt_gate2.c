...
create_gondor()
{
    set_short("Behind the mighty northern gate");
    set_long(&long_desc());
    ...
    add_exit(WALL_TUNNEL_ONE + "nt_gate1.c",    "north");
    add_exit(WALL_FLOOR_ONE +  "nt_guardrm1",   "west");
    add_exit(WALL_FLOOR_ONE +  "nt_guardrm2",   "east");
...
permit_exit()
{
    ...
    tell_room(environment(actor), QCTNAME(actor) + " pushes on the massive "
        ...
        ({actor}));
    tell_room(IGARD_PLAIN + "north_gate", "The great iron doors open "
        + "for a moment and " + QTNAME(TP) + " slips quickly outside.\n"
        + "The doors return to their original position.\n");

    TP->move_living("M", IGARD_PLAIN + "north_gate", 1, 0);
