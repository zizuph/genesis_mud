...
long_extratext()
{
    ...
    + "the eastern part of the ring wall on the ground level. A threadbare "
...
create_gondor()
{
    ...
    add_item(({"west wall"}), "The west wall is mostly covered with a large "
        + "threadbare tapestry.\n");
    ...
    add_exit(WALL_TUNNEL_ONE + "et4.c",       "south");
    add_exit(WALL_TUNNEL_ONE + "et6.c",       "north");
...
leave_room()
{
    ...
    tell_room(WALL_FLOOR_ONE + "1_archroom_1b.c", QCTNAME(TP) + " slips silently "
        + "into the room from behind the tapestry.\n");

    TP->move_living("M", WALL_FLOOR_ONE + "1_archroom_1b.c", 1, 0);
