...
long_desc()
{
    ...
    + "doors of iron stand to the northwest, set beneath a great stone "
    + "arch, while the tunnel itself leads away to the southeast. Two "
...
create_gondor()
{
    ...
    add_exit(WALL_TUNNEL_ONE + "set5.c",         "southeast");
    add_exit(WALL_FLOOR_ONE +  "set_guardrm1",   "southwest");
    add_exit(WALL_FLOOR_ONE +  "set_guardrm2",   "northeast");
...
permit_exit()
{
    ...
    tell_room(environment(actor), QCTNAME(actor) + " pushes on the massive "
        ...
        ({actor}));
    tell_room(IGARD_PLAIN + "southeast_gate", "The great iron doors open "
        + "for a moment and " + QTNAME(TP) + " slips quickly outside.\n"
        + "The doors return to their original position.\n");

    TP->move_living("M", IGARD_PLAIN + "southeast_gate", 1, 0);
