...
long_extratext()
{
    ...
    + "the northwestern part of the ring wall on the ground level.\n";
}
...
create_gondor()
{
    ...
    add_exit(WALL_TUNNEL_ONE + "nwt2.c",      "northeast");
    add_exit(WALL_TUNNEL_ONE + "nwt4.c",      "southwest");

    add_item(({"tapestry", "tapestries"}), "Hanging from one of the walls is a " 
        + "dull tapestry. Its green cloth has a loose weave that is shot "
        + "through with silver thread. The tapestry looks rather clean.\n");
...
test_quest()
{
    ...
    TP->remove_prop(SENT_IGARD_WALL2);
