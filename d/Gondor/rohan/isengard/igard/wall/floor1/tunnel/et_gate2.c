...
long_desc()
{
    ...
    + "doors of iron stand to the west, set beneath a great stone "
    + "arch, while the tunnel itself leads away to the east. Two "
    + "small doorways open to the north and south.\n";
}
...
create_gondor()
{
    set_short("Behind the mighty eastern gate");
    set_long(&long_desc());
    ...
    add_exit(WALL_TUNNEL_ONE + "et_gate1.c",     "east");
    add_exit(WALL_FLOOR_ONE +  "et_guardrm1",   "north");
    add_exit(WALL_FLOOR_ONE +  "et_guardrm2",   "south");
...
permit_exit()
{
    ...
    tell_room(environment(actor), QCTNAME(actor) + " pushes on the massive "
        ...
        ({actor}));
    tell_room(IGARD_PLAIN + "east_gate", "The great iron doors open "
        + "for a moment and " + QTNAME(TP) + " slips quickly outside.\n"
        + "The doors return to their original position.\n");

    TP->move_living("M", IGARD_PLAIN + "east_gate", 1, 0);
