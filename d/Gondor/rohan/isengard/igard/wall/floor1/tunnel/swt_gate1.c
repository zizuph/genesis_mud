...
long_desc()
{
    ...
    + "doors of iron stand to the northeast, set beneath a great stone "
    + "arch, while the tunnel itself leads away to the southwest. Two "
...
create_gondor()
{
    set_short("Behind the mighty southwestern gate");
    set_long(&long_desc());
    ...
    add_exit(WALL_TUNNEL_ONE + "swt6.c",         "southwest");
    add_exit(WALL_FLOOR_ONE +  "swt_guardrm1",   "northwest");
    add_exit(WALL_FLOOR_ONE +  "swt_guardrm2",   "southeast");
...
permit_exit()
{
    ...
    tell_room(environment(actor), QCTNAME(actor) + " pushes on the massive "
        ...
        ({actor}));
    tell_room(IGARD_PLAIN + "southwest_gate", "The great iron doors open "
        + "for a moment and " + QTNAME(TP) + " slips quickly outside.\n"
        + "The doors return to their original position.\n");

    TP->move_living("M", IGARD_PLAIN + "southwest_gate", 1, 0);
