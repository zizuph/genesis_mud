...
long_desc()
{
    ...
    + "doors of iron stand to the southeast, set beneath a great stone "
    + "arch, while the tunnel itself leads away to the northwest. Two "
    + "small doorways open to the southwest and northeast.\n";
}
...
create_gondor()
{
    set_short("Behind the mighty northeastern gate");
    set_long(&long_desc());
    ...
    add_exit(WALL_TUNNEL_ONE + "nwt5.c",         "northwest");
    add_exit(WALL_FLOOR_ONE +  "nwt_guardrm1",   "southwest");
    add_exit(WALL_FLOOR_ONE +  "nwt_guardrm2",   "northeast");
...
permit_exit()
{
    ...
    tell_room(environment(actor), QCTNAME(actor) + " pushes on the massive "
        ...
        ({actor}));
    tell_room(IGARD_PLAIN + "northwest_gate", "The great iron doors open "
        + "for a moment and " + QTNAME(TP) + " slips quickly outside.\n"
        + "The doors return to their original position.\n");

    TP->move_living("M", IGARD_PLAIN + "northwest_gate", 1, 0);
