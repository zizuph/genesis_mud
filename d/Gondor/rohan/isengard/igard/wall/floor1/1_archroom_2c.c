/*
 *  WALL_FLOOR_ONE + "1_archroom_2c.c"
 *
...
long_extratext()
{
    return "There are several cross-shaped slots in the west wall and "
    + "a tapestry hangs on the north wall.\n";
}
...
create_gondor()
{
    ...
    add_item(({"west wall"}), "The west wall is made from rough-hewn "
        + "black stone and is perforated by numerous cross-shaped slots.\n");
    add_item(({"north wall"}), "The north wall is mostly covered with a large "
        + "threadbare tapestry.\n");
...
scan_slot()
{
    ...
    adjoining = WALL_TUNNEL_ONE + "nt_gate1.c";
...
leave_room()
{
    ...
    tell_room(WALL_TUNNEL_ONE + "nt5.c", QCTNAME(TP) + " slips silently "
        + "into the room from behind the tapestry.\n");

    TP->move_living("M", WALL_TUNNEL_ONE + "nt5.c", 1, 0);
