...
long_extratext()
{
    ...
    + "the southeastern part of the ring wall on the second level. "
    + "A narrow stone walkway leads to the southwest.\n";
...
create_gondor()
{
    ...
    add_exit(WALL_TUNNEL_TWO + "set3.c",       "northeast");
...
move_player()
{
    ...
    tell_room(WALL_TUNNEL_TWO + "set1", QCNAME(TP) + " arrives from the "
        + "other end of the narrow stone walkway.\n");

    TP->move_living("M", WALL_TUNNEL_TWO + "set1", 1, 0);
    return 1;
}
