...
create_gondor()
{
    ...
    add_exit(WALL_TUNNEL_TWO + "set7.c",       "southwest");
...
move_player()
{
    ...
    tell_room(WALL_TUNNEL_TWO + "set9", QCNAME(TP) + " arrives from the "
        + "other end of the narrow stone walkway.\n");

    TP->move_living("M", WALL_TUNNEL_TWO + "set9", 1, 0);
    return 1;
}
