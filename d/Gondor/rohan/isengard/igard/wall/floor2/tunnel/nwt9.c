...
long_extratext()
{
    ...
    + "the northwestern part of the ring wall on the second level.\n";
}
...
create_gondor()
{
    ...
    add_exit(WALL_TUNNEL_TWO + "nwt8.c",      "northeast");
    add_exit(WALL_TUNNEL_TWO + "nwt10.c",      "southwest");

    add_item(({"tapestry", "tapestries"}), "Hanging from one of the walls is a " 
        + "dull tapestry. Its violet cloth has a frayed weave that is shot "
        + "through with copper thread. The tapestry looks rather tangled.\n");
...
test_quest()
{
    ...
    TP->remove_prop(SENT_IGARD_WALL4);
