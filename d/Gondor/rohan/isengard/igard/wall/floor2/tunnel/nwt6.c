...
create_gondor()
{
    ...
    add_exit(WALL_TUNNEL_TWO + "nwt5.c",      "northeast");
    add_exit(WALL_TUNNEL_TWO + "nwt7.c",      "southwest");
...
reset_room()
{
    if (!objectp(Troop))
    {
        Troop = clone_object(IGARD_NPC + "igard_uruk_lead.c");
        Troop->set_restrain_path(WALL_TUNNEL_TWO);
        Troop->arm_me();
