...
long_desc()
{
    ...
    + "doors of iron stand to the south, set beneath a great stone "
    + "arch, while the tunnel itself leads away to the north. Two "
...
create_gondor()
{
    set_short("A tunnel behind the great southern gate");
    set_long(&long_desc());
    ...
    add_exit(IGARD_WALL + "gate_tun2",           "north");
    add_exit(IGARD_WALL + "tun_guardrm1",        "east");
    add_exit(IGARD_WALL + "tun_guardrm2",        "west");
...
+"||        exit. All others face the peril they are due.    ||\n"
...
permit_exit()
{
    ...
    tell_room(environment(actor), QCTNAME(actor) + " pushes on the massive "
        ...
        ({actor}));
    tell_room(IGARD_WALL + "gate", "The great iron doors open for a "
...
push_gate(string str)
{
    ...
    if (BADDIE)
    {
        ...
        {
        write("A voice from behind shouts at you: Farewell, friend "
            + "of Isengard! You may pass.\n");
        tell_room(environment(actor), "A voice from behind shouts at " 
            + QTNAME(actor) + ": Farewell, friend of Isengard! You may "
            + "pass.\n", ({actor}));
        permit_exit();
        return 1;
        }
