...
long_desc()
{
    return "Two mighty doors of iron rise above you here at the "
      + "southeastern end of the great plain of Isengard. The doors are set "
      ...
      + "of the arch. A street leads northwest onto the great encircled plain "
...
create_gondor()
{
    set_short("The great southeastern gate of Isengard");
    set_long(&long_desc());
    ...
    add_item(({"street", "wide street", "road"}), "A wide street leads northwest "
    ...
    add_exit(IGARD_PLAIN  + "southeast_road1.c",      "northwest");
...
permit_entry()
{
    ...
    tell_room(environment(actor), QCTNAME(actor) + " pushes on the massive "
        ...
        ({actor}));
    tell_room(WALL_TUNNEL_ONE + "set_gate1.c", QCPNAME(TP) + " footsteps "
        + "echo across the tunnel as " + PRONOUN(actor) + " enters "
        + "through the massive iron doors.\n"
        + "The doors return to their original position.\n");

    TP->move_living("M", WALL_TUNNEL_ONE + "set_gate1.c", 1, 0);
