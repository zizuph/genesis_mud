/*
 *  IGARD_PLAIN + "east_gate.c"
 *
 *  Great eastern gate of Isengard.
 *
...
long_desc()
{
    return "Two mighty doors of iron rise above you here at the "
      + "eastern end of the great plain of Isengard. The doors are set "
      ...
      + "of the arch. A street leads west onto the great encircled plain "
...
create_gondor()
{
    set_short("The great eastern gate of Isengard");
    set_long(&long_desc());
    ...
    add_exit(IGARD_PLAIN  + "east_road1.c",      "west");
...
permit_entry()
{
    ...
    tell_room(environment(actor), QCTNAME(actor) + " pushes on the massive "
        ...
        ({actor}));
    tell_room(WALL_TUNNEL_ONE + "et_gate2.c", QCPNAME(TP) + " footsteps "
        + "echo across the tunnel as " + PRONOUN(actor) + " enters "
        + "through the massive iron doors.\n"
        + "The doors return to their original position.\n");

    TP->move_living("M", WALL_TUNNEL_ONE + "et_gate2.c", 1, 0);
