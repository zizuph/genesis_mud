...
create_gondor()
{
    ...
    add_exit(IGARD_PLAIN_PATHS  + "nw_path2e.c",      "north");
...
move_player()
{
    say(QCTNAME(TP) + " reaches the top and falls into the copper "
        + "pipe!\n");
    tell_room(IGARD_DEEP_ARMOURY + "entry_stair1", QCNAME(TP) 
        + " tumbles down from above, landing on the floor with a loud "
        + "splat.\n");

    TP->move_living("M", IGARD_DEEP_ARMOURY + "entry_stair1", 1, 0);
