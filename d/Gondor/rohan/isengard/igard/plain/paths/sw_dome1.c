...
create_gondor()
{
    ...
    add_exit(IGARD_PLAIN_PATHS  + "sw_path1c.c", "north", check_dome, 2, 0);
...
move_player()
{
    say(QCTNAME(TP) + " reaches the top and falls into the copper "
        + "pipe!\n");
    tell_room(IGARD_DEEP_WARG + "entry_stair1", QCNAME(TP) 
        + " tumbles down from above, landing on the floor with a loud "
        + "splat.\n");

    TP->move_living("M", IGARD_DEEP_WARG + "entry_stair1", 1, 0);
