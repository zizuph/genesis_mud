...
create_gondor()
{
    ...
    add_item(({"room", "area", "surroundings", "area", "place",
        "landscape"}), long_desc);
    add_item(({"gravel", "pebble", "pebbles", "blue pebbles"}), 
        "The pebbles on the "
        + "gravel path are a strange blue colour.\n");

    add_exit(IGARD_PLAIN_PATHS  + "se_path1d.c", "south", check_dome, 2, 0);
...
move_player()
{
    ...
    tell_room(IGARD_QUEST + "entry1_stair", QCNAME(TP) 
            + " tumbles down from above, landing on the floor with a loud "
            + "splat.\n");
    TP->move_living("M", IGARD_QUEST + "entry1_stair", 1, 0);
    TP->remove_prop(CLIMBING_THE_DOME);
    return 1;
}
