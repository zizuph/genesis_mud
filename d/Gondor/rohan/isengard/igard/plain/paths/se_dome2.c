...
create_gondor()
{
    ...
    add_item(({"gravel", "pebble", "pebbles", "green pebbles"}), 
        "The pebbles on the "
        + "gravel path are a strange green colour.\n");

    add_exit(IGARD_PLAIN_PATHS  + "se_path2h.c", "southwest", check_dome, 2, 0);
...
move_player()
{
    ...
    tell_room(IGARD_QUEST + "entry2_stair", QCNAME(TP) 
            + " tumbles down from above, landing on the floor with a loud "
            + "splat.\n");
    TP->move_living("M", IGARD_QUEST + "entry2_stair", 1, 0);
    TP->remove_prop(CLIMBING_THE_DOME);
    return 1;
}
