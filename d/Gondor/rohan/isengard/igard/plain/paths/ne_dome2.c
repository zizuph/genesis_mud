...
create_gondor()
{
    ...
    add_exit(IGARD_PLAIN_PATHS  + "ne_path2f.c", "south", check_dome, 2, 0);
...
move_player()
{
    ...
    {
    case 0..50:
        tell_room(IGARD_DEEP_ORC + "yorc1", QCNAME(TP) 
            + " tumbles down from above, landing on the floor with a loud "
            + "splat.\n");
        TP->move_living("M", IGARD_DEEP_ORC + "yorc1", 1, 0);
        TP->remove_prop(CLIMBING_THE_DOME);
        break;
    case 51..100:
        tell_room(IGARD_DEEP_ORC + "yuruk1", QCNAME(TP) 
            + " tumbles down from above, landing on the floor with a loud "
            + "splat.\n");
        TP->move_living("M", IGARD_DEEP_ORC + "yuruk1", 1, 0);
        TP->remove_prop(CLIMBING_THE_DOME);
        break;
    case 101..150:
        tell_room(IGARD_DEEP_ORC + "torc1", QCNAME(TP) 
            + " tumbles down from above, landing on the floor with a loud "
            + "splat.\n");
        TP->move_living("M", IGARD_DEEP_ORC + "torc1", 1, 0);
        TP->remove_prop(CLIMBING_THE_DOME);
        break;
    default:
        tell_room(IGARD_DEEP_ORC + "turuk1", QCNAME(TP) 
            + " tumbles down from above, landing on the floor with a loud "
            + "splat.\n");
        TP->move_living("M", IGARD_DEEP_ORC + "turuk1", 1, 0);
        TP->remove_prop(CLIMBING_THE_DOME);
        break;
    }
