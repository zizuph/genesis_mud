...
create_gondor()
{
    set_short("A street on the southern plain of Isengard");
    set_long(&long_desc());
    ...
    add_item(({"street", "wide street", "road"}), "A wide street leads north "
        ...
        + "A small gravel path leads to the east.\n");
    add_item(({"path", "gravel path", "small path"}), "The path is made from "
        + "crushed gravel. It crunches and shifts as you walk across it.\n");
    ...
    add_exit(IGARD_PLAIN  + "south_road4.c",      "south");
    add_exit(IGARD_PLAIN  + "south_road6.c",      "north");
    add_exit(IGARD_PLAIN_PATHS + "se_path1a.c",   "east");
...
long_desc()
{
    return "You are standing on a street built with great skill on the "
      + "southern plain of Isengard." + sky_desc() + plain_tower_desc(distance)
      + "A small gravel path leads to the east.\n";
}
