/*
 *  IGARD_PLAIN + "north_gate.c"
 *
 *  Great northern gate of Isengard.
 *
...
long_desc()
{
    return "Two mighty doors of iron rise above you here at the "
      + "northern end of the great plain of Isengard. The doors are set "
      ...
      + "of the arch. A street leads south onto the great encircled plain "
...
create_gondor()
{
    set_short("The great northern gate of Isengard");
    set_long(&long_desc());
    ...
    add_exit(IGARD_PLAIN  + "north_road1.c",      "south");
...
permit_entry()
{
    ...
    tell_room(environment(actor), QCTNAME(actor) + " pushes on the massive "
        ...
        ({actor}));
    tell_room(WALL_TUNNEL_ONE + "nt_gate2.c", QCPNAME(TP) + " footsteps "
        + "echo across the tunnel as " + PRONOUN(actor) + " enters "
        + "through the massive iron doors.\n"
        + "The doors return to their original position.\n");

    TP->move_living("M", WALL_TUNNEL_ONE + "nt_gate2.c", 1, 0);
