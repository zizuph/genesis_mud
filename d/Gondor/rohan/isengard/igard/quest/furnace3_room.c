...
create_gondor()
{
    ...
    add_exit(IGARD_QUEST   + "entry3_stair", "up", &prevent_up());
...
back_to_dome()
{
    TP->move_living("M", IGARD_PLAIN_PATHS + "sw_dome2", 1, 0);
}
...
move_quester_alive()
{
    ...
    set_alarm(6.0, 0.0, &tell_room(environment(actor), QCTPNAME(actor)
            ...
            + "out.\n", ({actor})));
    TP->remove_prop(SENT_FURNACE_IGARD3);
