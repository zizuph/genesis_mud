/*
 * This file is IGARD_NPC + "igard_young_orc"
 *
...
long_text()
{
    ...
    case 1:
       long_looks = CAP(LANG_ADDART(implode(query_adjs()," ")))
       + " orc from the warrens beneath Isengard. He is very short "
       + "and squat, mewling in a sqeaky voice. There is something not "
       + "altogether 'orcish' about him, although you cannot quite lay "
       + "your finger on it.\n";
	      break;
    case 2:
       ...
       + "weaknesses. For some reason, this orc seems almost, but not quite "
...
create_igard_monster()
{
    ...
    set_skill(SS_WEP_SWORD,  15 + rnd / 3);
    set_skill(SS_WEP_CLUB,   15 + rnd / 3);
    set_skill(SS_WEP_KNIFE,  15 + rnd / 3);
    set_skill(SS_PARRY,      15 + rnd / 2);
    set_skill(SS_DEFENCE,    15 + rnd / 2);
    add_prop(CONT_I_HEIGHT,   75 + rnd / 2);
    add_prop(CONT_I_WEIGHT, 45000 + rnd * 400);
    add_prop(CONT_I_VOLUME, 40000 + rnd * 400);    
