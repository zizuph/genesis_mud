/*
 * This file is IGARD_NPC + "igard_orc4_rider.c"
 *
...
add_warg()
{
    ...
    warg = clone_object(IGARD_NPC + "igard_warg");
    warg->move_living("down", ENV(TO));
...
add_3_orcs(int i)
{
    ...
    while (++n < i)
    {
       	orc = clone_object(IGARD_NPC + "igard_orc2_rider");
       	orc->move_living("down", ENV(TO));
       	orc->arm_me();
        orc->command("mount warg");
       	TO->team_join(orc);
    }
}
