/*
 *  /d/Gondor/rohan/isengard/tunnel1.c
 *
 *  Opus, April 1997
 *
...
create_isengard()
{
    set_short("inside the ringwall just inside the outer gate");
    set_long(BSN(
        "This tunnel leads through the volcanic ring encircling the "+
        "fortress of Isengard. A torch hangs on the wall which provides "+
        "light for this area of the tunnel. The outer gates are just "+
        "south of here and northward the tunnel continues toward the "+
        "inner gates."));

    add_item( ({"gates", "gate", "outer gate", "outer gates"}), BSN(
        "The great gates of the fortress of Isengard are made of solid "+
        "black iron. Twenty feet tall, and equally wide, the gates "+
        "lead into the wilderness that surrounds Isengard."));
    add_item( ({"torch", "flame"}), BSN(
    ...
    add_cmd_item( ({"torch", "flame"}), "extinguish", BSN(
        "Some type of magical force prevents the flame from being "+
        "extinguished."));
    ...
    add_item( ({"rock", "black rock", "smooth rock", 
        "smooth black rock"}), BSN(
            "This strange black rock makes up the entire tunnel. "+
            "It has been carved and shaped to form this passageway through "+
            "the ringwall."));
    add_item( ({"inner gate", "inner gates"}), BSN(
        "Almost out of sight, the inner gates are barely visible to the "+
        "north.")); 
    ...
    add_item( ({"tunnel", "passage", "passageway"}), BSN(
        "Carved through the strange, black rock making up the volcanic "+
        "ring, this passage remains the only known way in and out of "+
        "Isengard."));
    ...
    add_exit(ISEN_DIR + "tunnel2",  "north", 0);
    add_exit(ISEN_DIR + "isengate", "south", 0);  
