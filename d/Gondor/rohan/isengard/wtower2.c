/*
 *     /d/Gondor/opus/rohan/isengard/watchtower2.c
 *
 *     Opus, June 1997
 *
...
create_isengard()
{
    set_short("second watchtower of Isengard");
    set_long( BSN(
    ...
    add_item( ({"path", "pathway", "gates", "opening", "outside"}), pathdesc);
    ...
    add_item( ({"floor", "ground"}), BSN(
       "There is a fine layer of dirt that is covering the floor. Wind "+ 
       "stirs up the dirt a bit as it whips through the tower.")); 
    ...
    add_cmd_item( ({"banner", "large banner"}), ({"tug", "pull", "grab",
        ...
        "towards the top of the banner."));
    add_item( ({"tower", "watch tower", "towers", "watch towers"}), BSN(
        "This is one of the two watch towers that loom high above the "+
        "gates and pathway below."));
    add_item( "dirt", BSN(
    ...
    add_item(  "doorway", BSN(
        ...
        "back out onto the lookout to the west.")); 
    ...
    add_item( ({"ringwall", "cliffs", "rigid cliffs"}), BSN(
    ...
    add_item( ({"cloth", "fine cloth"}), BSN(    
        "The the cloth is very smooth to the touch. It "+
    ...
    add_item( "lookout", BSN(
        "The lookout, seen to the west through the doorway, runs across "+
    ...
    add_exit(ISEN_DIR + "lookout1.c", "west", 0);
    ...
    switch (tod())
    {
        case "night":
            pdesc = "The darkness from night prevents a "+
                ...
                "movement and occasional mumbling can be heard and the faint "+
                "glow from a hand-held lantern can be seen from below.";
            break;
        default:
            ...
            if (sizeof(lnames)>0)
            {
               pdesc += "Currently " +COMPOSITE_LIVE(lnames)+ " are on "+
               "the path below.";
            }
            else
            {
