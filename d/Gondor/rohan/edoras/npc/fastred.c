/*
 * Fastred's key now removed so players cant get it and lock the inn
 * -- Finwe, Nov 2005
 * 
 * Changes: 
 * 2017-12-04 - Cotillion
 * - Redid the counter offensive guard cloning to prevent guards from being
 *   stuck in the void.
 * Commented out code where the NPC calls for guards endlessly. Finwe Feb 2020 
 */
...
#define INN     (EDORAS_DIR+"stallion")
...
//    set_base_stat(SS_STR,60);
    ...
    set_equipment( ({ EDORAS_DIR + "obj/broadsword", EDORAS_DIR + "obj/leathcoat", }) );
...
//    if (sizeof(filter(guards, &->query_attack()))) 
//    {
...
////    guards->start_attack(); 
//    reset_inn_alarm = set_alarm(120.00, 0.0, reset_inn);
//}
...
{
    int i;
    command("shout Help! The inn is under attack!!!");
/*
    for (i = 0; i < 4; i++)
    {
        object guard = clone_object(EDORAS_DIR + "npc/innguard");
        guard->move(environment(TO));
...
do_die(object killer)
{
    ...
    {
//        clone_object(EDORAS_DIR + "obj/inn_key")->move(TO);
//        command("Ha! I'll get you for this!");
//        command("close door");
//        command("unlock door with key");
