...
create_gondor()
{
	...
	price_long());
    add_exit(EDORAS_DIR + "DBGstairs2", "west", check_combat, 1);
    add_exit(EDORAS_DIR + "DBGdining", "south", check_combat, 1);
    add_exit(EDORAS_DIR + "DBGhotelr", "up", check_combat, 2);
    add_prop(ROOM_I_INSIDE,1);
    config_default_trade();
    set_money_give_max(2000);
    set_money_give_out(({ 2000, 700, 50, 4}));
    set_money_give_reduce(({1,1,1,1})); 
    clone_object(EDORAS_DIR + "rooms/DBGinndoorin")->move(TO);
    reset_room();
}
