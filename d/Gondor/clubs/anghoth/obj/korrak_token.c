/*
 *  The Anghoth token worn by Korraks.
...
{
  set_name("ring");
  add_name(({"korrak_token", "korrak_emblem"}));
...
string set_emblem_short(object pl)
{
  if (TP == environment(TO))
      {
      return emblem_type + " shield-shaped ring of the Korrak Anghoth";
      }
...
string set_emblem_long(object pl)
{
    ...
    if (ANGHOTH_MEMBER(TP))
        return ("It is a ring of " + emblem_type + " shaped like a shield, "
            + "the emblem of the Korrak Anghoth, which someone must "
