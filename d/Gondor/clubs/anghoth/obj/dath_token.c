/*
 *  The Anghoth token worn by Daths.
...
{
  set_name("ring");
  add_name(({"dath_token", "dath_emblem"}));
...
string set_emblem_short(object pl)
{
  if (TP == environment(TO))
      {
      return emblem_type + " shield-shaped ring of the Dath Anghoth";
      }
...
string set_emblem_long(object pl)
{
    ...
    if (ANGHOTH_MEMBER(TP))
        return ("It is a ring of " + emblem_type + " shaped like a shield, "
            + "the emblem of the Dath Anghoth, which someone must "
