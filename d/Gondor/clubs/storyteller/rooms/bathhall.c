/*
 * /d/Gondor/clubs/storyteller/rooms/bathhall.c
 *
 * This is the bath of the Storyteller's club
 * Modified from the Rangers of the North's bathroom.
 * Copied from:
...
#define	ATTENDANT_FILE	(STORYT_DIR + "npc/bath_attendant")
...
create_gondor()
{
    ...
    add_item("walls", "The walls are constructed from white stone, smooth " +
        "and damp with moisture from the tubs.\n");
    add_item(({"mats", "matting", "rushes", "rush matting", "floor"}),
	"The floor is covered with soft rush matting.\n");
    add_item(({"white stone", "stone"}),
	"The stone is smooth, white and slightly damp.\n");
    ...
    add_item(({"arch", "archway", "south wall"}),
	"An open archway leads south.\n");
    add_item("sink", 
    ...
    add_item(({"tub", "tubs", "wooden tub", "wooden tubs"}), 
	"The heavy oak tubs are large enough to hold quite a lot " +
        "of hot water. You may enter a tub, lather yourself with " +
        "soap, wash, rinse, relax and even splash. When you are done, " +
        "you may stand up and the attendant will give you a towel.\n");
    ...
    add_exit((STORYT_DIR + "rooms/shallway"), "east", check_exit);
    reset_room();  
} /* create_gondor */
...
{ 
    ...
    {
	clone_object(STORYT_DIR+"obj/hamper")->move(TO);
    }
    else
    {
    ...
    {
        clone_object(STORYT_DIR+"obj/clothes_stand")->move(TO);
    }
...
check_exit()
{
    ...
    if (objectp(Attendant))
    {
        ...
        {
            write(Attendant->query_The_name(tp) + " says: You must " +
                "leave the tub and dry off first.\n");
            return 1;
        }
        if (tp->query_prop(WET))
        {
            write(Attendant->query_The_name(tp) + " says: You must dry " +
        ...
        if (sizeof(soaps))
        {
            write(Attendant->query_The_name(tp) + " blocks the exit.\n" +
                Attendant->query_The_name(tp) +
                " says: Please give your soap to me.\n" +
                Attendant->query_The_name(tp) +
                " says: You should not take club property.\n");
            return 1;
        }
...
enter_inv(object ob, object from)
{
    ...
    if (objectp(Attendant))
    {
        if (!Soap_alarm && ob->id("soap"))
        {
            Soap_alarm = set_alarm(6.0, 0.0, att_get_soap);
        }
        if (!Towel_alarm && ob->id("towel"))
        {
...
exa_reflection()
{
    ...
    txt = "The lightly fogged reflection is that of yourself. " +
          "You are a " + (TP->query_gender() == G_MALE ? "handsome man" :
          "lovely woman") + ", ";
...
*/
    ...
    if (objectp(Attendant))
    {
        ...
        if (!objectp(present("soap", tp)))
        {
            ...
            FIX_EUID;
            ob = clone_object(STORYT_DIR+"obj/soap");
            if (ob->move(tp))
            {
...
do_rinse(string str)
{
      ...
      sizeof(who = NORMAL_ACCESS(who, 0, 0)))
    {
        ...
        write("You rinse " + who[0]->query_the_possessive_name(tp) + " back.\n");
...
{
    string t1, t2, poss= POSSESSIVE(pl);
    ...
    case "face":
        ...
        t2 = " scoops some water up with " + poss + " cupped hands, and " +
            "washes " + poss + " face.";
        break;
    case "neck":
    case "shoulders":
...
do_splash(string str)
{
    ...
    if (parse_command(str, all_inventory(TO), "[water] [at] / [on] %l", 
      target) && sizeof(target = NORMAL_ACCESS(target, 0, 0)))
    {
...
{
    Soap_alarm = 0;
    if (objectp(Attendant) && present("soap", TO))
    {
...
{
    Towel_alarm = 0;
    if (objectp(Attendant) && present("towel", TO))
    {
        Attendant->command("sigh");
...
get_towel(string str)
{
    ...
    FIX_EUID;
    ob = clone_object(STORYT_DIR+"obj/towel");
...
expel_aggressive()
{
    ...
    write(Attendant->query_The_name(tp)+" hustles you out of the room.\n");
    say(QCTNAME(Attendant)+" hustles "+QTNAME(tp)+" out of the room.\n");
    tp->move_living("M", (STORYT_DIR+"rooms/ehallway"));
...
fire_desc(string str)
{
    ...
    case "fireplace":
        ...
        {
        case FIRE_BURNING:
            return "A great stone fireplace fills the north wall. "+
        ...
        default:  /* FIRE_ASHES */
            return "A great fireplace fills the north wall.\n";
        }
    ...
    default:  /* room long description */
        if ( fire_state == FIRE_BURNING )
            return "Large wooden tubs are arrayed in rows side by side, " +
                "and from a great fireplace set in the far north wall, a " +
                "very large fire warms the room. " +
        ...
        return "Large wooden tubs are arrayed in rows side by side, and " +
              "a great fireplace is set in the far north wall. " +
              "Hooks along the east wall hold plush towels, and below a " +
              "long mirror mounted on the west wall are a sink and a rack " +
              "of soaps.\n";
    }
