...
*     him tougher and more dangers. Also, increased combat stats by 100.
...
*  ----------------------------------------------------------------------------
*  30/04-17              Cleaned up the code                           Znagsnuf
*  30/04-17              Made a work around for him attacking npcs     Znagsnuf
*                        He won't attack Gondor / Shire and Steed
*                        npcs.
*
*/
...
#define NAZGUL_SKILLS         ( {TS_DEX, SS_SPELLCRAFT, SS_FORM_CONJURATION, \
                                 SS_ELEMENT_DEATH} )
#define PLAYER_SKILLS         ( {TS_DEX, SS_SPELLCRAFT, SS_FORM_ABJURATION, \
                                 SS_ELEMENT_LIFE} )
 ...
 * Description:     check_attack, checking if the Nazgül may attack
 * Arguments:       The object to attack
 */

void
check_attack(object enemy)
{
    string find_name;
    find_name = file_name(enemy);

    // Will not attack objects from Shire /Gondor, and either
    // steeds.
    if(wildmatch("/d/Shire/*", file_name(enemy))
       || wildmatch("/d/Gondor/*", file_name(enemy))
       || enemy->is_steed()
       || function_exists("create_steed", enemy))
    {
        enemy->stop_fight(this_object());
        this_object()->stop_fight(enemy);

        tell_room(environment(this_object()), QCTNAME(enemy) + " is "
                + "so terrified at the prospect of facing such a fearsome "
                + "foe, that " + enemy->query_pronoun() + " quickly flee "
                + "into the distance.\n", this_object());
...
attack_object(object ob)
{
    ...
    // Check if it will continue the onslaught.
    set_alarm(1.0, 0.0, &check_attack(ob));
}
...
{
    if (CAN_SEE(TO,TP))
	     TP->catch_tell("You feel that the black shape is staring straight "
                    + "at you.\n");
}
...
init_attack()
{
    if ((!monster_aggressive) 
       || (TP->query_npc()) 
       || (query_friend_or_foe(TP) < 1)) 
        return;

    if ((monster_attack_chance == 0) 
...
query_not_attack_me(object att, int att_id)
{
   ...
   if (!weapon->query_prop(OBJ_I_IS_MAGIC_WEAPON)) 
   {
       if (!random(10))
          ...
          + weapon->query_name() + " seems to have no effect against the "
          + "wraith!\n", att);
       else 
   ...
   if (!pointerp(mag_arr))
   {
       tell_room(ENV(TO), "The "+weapon->query_name() + " is broken. "
   ...
   hiss("Your pitiful " + check_call(weapon->short()) + " does not worry "
      + "me, mortal!");
...
do_die(object killer)
{
    ...
    tell_object(killer,"You swing your weapon in between helm and mantle of "
                     + "the Nazgul.\n");

    tell_room(ENV(TO), BSN(QCTNAME(killer) + " swings " 
              + HIS(killer) + " weapon between helm and mantle of the dark "
              + "figure."), killer);

    tell_room(ENV(TO), BSN("The helm rolls to the ground with a clang, and "
                         + "behold! The mantle and hauberk are empty, and " 
                         + "fall shapeless to the ground, torn and tumbled!") 
                     + BSN("A cry goes up into the shuddering air, fading to "
                         + "a wailing passing with the wind, a voice, "
                         + "bodiless and thin, dies out.")); 
    ...
    write_file(NPC_LOG, "Black Rider: Killed by " + 
	                      killer->query_name() + " (" + 
	                      killer->query_average_stat() + ") on " 
                        + ctime(time()) + ".\n");  

    if (objectp(Last_Weapon))
	      write_file(NPC_LOG, " - possibly killed with " 
          + Last_Weapon->short()+" - " 
...
{ 
  ...
  {
      victim->catch_msg(QCTNAME(TO) + " hisses a cold, dark breath at "
...
static int filter_enemy(object x)        
{ 
...
special_attack(object enemy)
{
  ...
  else
  {
    ...
    while (i--)
    {
         ...
         {
		        victim[i]->catch_msg(QCTNAME(TO) + " hisses a cold, dark breath "
                                             + "at you!\n");
		        tell_room(ENV(TO),QCTNAME(TO) + " hisses at " + QTNAME(victim[i])
                                          + " coldly.\n", victim[i]);
		        fear(50, victim[i]);
	       }
	     }
...
attacked_by(object ob)
{
    ...
    ::attacked_by(ob);
    TELL_ADMIN("[Nazgul attacked by " + ob->query_cap_name() 
