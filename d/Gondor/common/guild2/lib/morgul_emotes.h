/*
 *    /d/Gondor/common/guild2/lib/morgul_emotes.h
 *
...
//Added for the slave_speak option. to communicate with orcs/uruks
...
#define MAGE_MEMBER(p) ((p)->query_guild_name_occ() == "Morgul Mages")
...
public int designate_ingredient( string str )
{
 ...
 mixed  *adjs;
    ...
    case "naro":
        ...
        break;
    case "gostangwedh":
/* The original Olorin's requirement is to stern. Nobody wants to carry
 * extra pieces of armour for any spell. Since I changed the spell itself
 * I decided that any valuable item suffice. The power of spell depends on
 * the item's weight and volume so it's a mage who decides whether to carry
 * an oversized object or not. Chmee.
        if (function_exists( "create_object", oblist[0] ) != "/std/armour")
        {
            notify_fail( "You can only designate a piece of armour as "+
                "an ingredient for the spell \"gostangwedh\"!\n" );
            return 0;
        }
        if (!sizeof( adjs = oblist[0]->query_adjs() ) ||
            !sizeof( adjs & ({"steel", "iron", "metal"})) )
        {
            notify_fail( "You can only designate a piece of armour that "+
                "is made from iron or steel as an ingredient for "+
                "the spell \"gostangwedh\"!\n" );
            return 0;
        }
        if ((oblist[0]->query_prop( OBJ_I_VALUE ) < 120) ||
            (oblist[0]->query_ac() <= 10) )
*/
        if (oblist[0]->query_prop( OBJ_I_VALUE ) < 120)
        {
            notify_fail( "That "+ oblist[0]->short()+ " is not worthy to "+
...
public int hail( string str )
{
 ...
 object *oblist, *non, *higher, *lower, *mem, *wiz;
    ...
    mem += filter( oblist, special_wizard_filter );
    non = oblist - mem;
...
public int mhiss( string str )
{
 ...
 string *how, text;
    ...
    if (!strlen( how[0] ))
    {
        write( "You hiss"+ how[1]+ ".\n" );
        all( " hisses"+ how[1]+ "." );
        IDLENESS->increase_activity( this_player(), TALK );
        return 1;
    }
    oblist = parse_this( how[0], "[at] [the] %i" );    
    oblist -= ({ this_player() });
        ...
        " 'at' / 'to' [the] %i %s ", oblist, text ))
    {
        ...
        if (sizeof( oblist ) && strlen( text ))
        {
            ...
            {
                write( "You hiss to "+ COMPOSITE_LIVE( oblist )+ ": "+
                    text+ "\n" );
            }
            else
            {
            ...
            oblist->catch_msg( QCTNAME( this_player() )+ " hisses to you: "+
                text + "\n" );
            all2act( " hisses at", oblist, ": "+ text );
            IDLENESS->increase_activity( this_player(), TALK );
            return 1;
        }
    }
...
public int minspect( string str )
{
 ...
 string  item_str;
    ...
    IDLENESS->increase_activity( tp, EMOTION );
    return 1;
}
...
public int msniff( string str )
{
    write( "You lower yourself to the ground and sniff for tracks.\n" );
    allbb( " lowers "+ TP_HIM+ "self to the ground and seems to sniff for "+
        "something, as if hunting." );
    IDLENESS->increase_activity( this_player(), EMOTION );
}
 ...
 string *how, text;
        ...
        " 'at' / 'to' [the] %i %s ", oblist, text) )
    {
        ...
        if (sizeof( oblist ) && strlen( text ))
        {
            if (this_player()->query_option( OPT_ECHO ))
            {
                write( "You rasp to "+ COMPOSITE_LIVE( oblist )+ ": "+ text+
                    "\n" );
            }
            else
            {
            ...
            members += filter( oblist- members, special_wizard_filter );
            others = oblist- members;
            if (sizeof( members ))
            {
                target( " rasps to you: "+ text, members );
            }
            if (sizeof( others ))
            {
            ...
            all2act_split( "rasps at", oblist, others, ": "+ text );
    ...
    if (sizeof( members ))
    {
        target( " rasps: "+ str, members );
    }
    if (sizeof( others ))
    {
        target( " rasps some low guttural sounds.", others );
    }
    IDLENESS->increase_activity( this_player(), TALK );
    return 1;
}
...
public int slave_speak(string str)
{
	...
	write ("You hiss in the dialect of the slaves: " + str + "\n");
	...
	for (iListener = 0 ; iListener < sizeof (aoListeners) ; iListener ++)
	{
		...
		{
			oListener->catch_tell (oActor->query_The_name (oListener) + " hisses in the dialect of the slaves: " + str + "\n");
		}
		
		else if(ORC_MEMBER (oListener))
		{
			oListener->catch_tell (oActor->query_The_name (oListener) + " hisses in the black tongue: " + str + "\n");
		}
		...
		{
			oListener->catch_tell (oActor->query_The_name (oListener) + " hisses something unintelligible.\n");
		}
