/*
 *  /d/Mineth/linor/std/library.c
 *
 *  A set of functions to create a mortal maintained library. 
 *
...
void create_library()
{
    ...
    {
        library_book_object = "/w/eowul/mages/library_book";
    }
 ...
 * Returns      : a string with the result
 */
string change_shelf_description(string shelf_name, string description)
{
    // Make sure everything exists
    if(!mappingp(library_data))
        library_data = ([]);
 ...
 * Arguments    : book_id - the name of the book
 *              : new_name - the new name of the shelf
 * Returns      : a string with the result
 */
string rename_book(int book_id, string new_name)
{
    string  book_name;
    ...
    library_add_log("New title '" + new_name + "' for '" + book_id + "'");
 ...
 * Returns      : a string with the result
 */
string rename_shelf(string shelf_name, string new_name)
{
    int     i;
    string  *book_names;
 ...
 */
varargs mixed add_new_book(string title, string contents, string author, 
...
int library_rename_cmd(string args)
{
    ...
    if(!args || !parse_command(args, ({}), 
        "[" + library_book_type + "] %d 'to' %s", book_id, new_name))
    {
        // Syntax check
        if(!args || !parse_command(args, ({}), 
            "[shelf] %s 'to' [shelf] %s", name, new_name))
        {
            notify_fail("Rename what shelf to what shelf?\n");
            return 0;
        }

        // Do the renaming
        result = rename_shelf(name, new_name);
        write(result);

        return 1;
    }

    result = rename_book(book_id, new_name);
    write(result);
