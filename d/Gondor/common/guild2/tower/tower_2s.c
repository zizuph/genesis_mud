...
create_morgul_tower_room()
{
    ...
    add_walls();
    add_floor();
    add_item(({"staircase", "dark staircase"}), BSN("The staircase "
      + "lies beyond the doorway to the north."));

    add_item(({"window", "cleft", "city wall", "wall", "face",
       "mountain face", "narrow cleft"}), BSN("Looking out of the "
       + "window, you see the "
      + "cliffs that rise south of the Morgul vale. Below the window "
      + "is the top of the city wall, and beyond the walls is only "
      + "a narrow cleft separating the walls from the steep mountain "
      + "face."));
}
...
check_time(string str)
{
    ...
    write(BSN("You look through the window to study the sky. "
