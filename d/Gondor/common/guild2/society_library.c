...
create_morgul_room()
{
    ...
    set_extraline( 
      ...
      + "public library upstairs, while a doorway leads east into the "
      + "section of the library restricted to mages.");

    add_prop(ROOM_I_LIGHT,1);
    add_prop(ROOM_I_INSIDE,1);
    add_exit(MORGUL_DIR + "city/rooms/mmlibrary", "up",   check_exit, 0);
    add_exit(MAGES_DIR + "mage_library",          "east", mage_check, 0);

    add_item(({"plaque", "instructions", }), exa_plaque());
    add_cmd_item(({"plaque", "instructions", }), "read", exa_plaque());

    set_save_path(MAGES_DIR + "books/open_library");
    set_write_dir(MAGES_DIR + "books/mages");
    set_librarian_room(MAGES_DIR + "library_office");
    set_library_name("the Library of the Society of Morgul Mages");
...
mage_check()
{
    if ((this_player()->query_morgul_level() <= APPRENTICE_LEVEL) &&
        (MORGUL_NAZGUL_MASTER->query_is_nazgul( this_player() ) <= 0))
    {
        write("That part of the library is not accessible to you.\n");
        return 1;
    }
    return 0;
}
...
move_book(string book, string shelf)
{
    ...
    {
    NFN("There is no " + booktype + " " + book + " in the Society library!");
    return 0;
    }

    if (member_array(CAP(shelf), shelves = query_shelf()) < 0)
    {
    NFN("There is no shelf " + shelf + " in the Society library!");
    return 0;
    }
    ...
    write("You move the " + booktype + " " + book +
      " to the shelf " + shelf + " in the Society library.\n");
