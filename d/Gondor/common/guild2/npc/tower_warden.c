/*
 *	/d/Gondor/common/guild2/npc/tower_warden.c
 * 
...
create_mage()
{
    ...
    set_name(({"warden of the Tower", "warden", "tower warden"}));
    set_pname("wardens");
    set_race_name("human"); 
    remove_adj(({"dark-skinned", "red-eyed"}));
    add_adj(({"dark", "tall", "terrible", "undead"}));
    set_short("tall warden");
    set_pshort("tall wardens");
    set_long(BSN("A dark towering shape with eyes glowing like coal. He " +
	"is one of the servants of the Dark Lord, ages ago a mighty warrior, " +
	"but now he is undead, not a living man anymore, but not one of the " +
        "mighty Nazgul. His duty is to protect the Tower of Morgul. From " + 
	"time to time, he will also help others in the service of the Dark " +
	"Lord or those who he thinks are helping the Dark Lord. If you dare, " +
	"then ask him!"));
	...
	"about quest", "about help", "about job"}), VBFC_ME("ask_help"));
...
default_answer()
{
    ...
    command("glare " + name);
    command("hiss What do you want, you creepy " + tp->query_race_name() + "?");
...
ask_standard()
{
    ...
    whisper_stage = 0;
    whisper_flag = "angmar";
    hook_alarm = set_alarm(60.0, 0.0, remove_hook);
