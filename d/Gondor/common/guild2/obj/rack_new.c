...
{
    // Since we have a list command, we don't show the equipment when
    // examining the container.
    for_obj->catch_tell("You can 'list' or 'list <type>' the various " +
        "equipment and 'sort' whatever equipment you are carrying.\n");
}
...
int list_cmd(string args)
{
    ...
    for(i=0;i<sizeof(sorted);i++)
    {
        ...
        result += sprintf("| %-50s | %-8s | %4d |\n",
            extract(sorted[i][0]->short(), 0, 49), category, sizeof(sorted[i]));
    }
    result += "|                                                    |          |      |\n";
    result += "+----------------------------------------------------+----------+------+\n";
...
int sort_cmd(string args)
{
    ...
    rack_obs = filter(useable_obs, is_weapon);
    rack_obs += filter(useable_obs, is_armour);
    switch(rack_type)
    {
        // Saving items
        case 0:
