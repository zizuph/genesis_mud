...
public nomask  int lt_new_msg(string msg_head);
public nomask  int nazgul_new_msg(string msg_head);
public nomask  int king_new_msg(string msg_head);
...
/*
 * Morgul specific defines.
 */
#include "/d/Gondor/defs.h"
#include "/d/Gondor/common/guild2/lib/morgul_defs.h"

#define ME_DOMAINS    ({ "Gondor", })
...
block_reader(int note = 0)
{
    ...
    IDLENESS->increase_activity( this_player(), READ_NOTE );
...
block_writer()
{
    IDLENESS->increase_activity( this_player(), BOARD_NOTE );
    return 0;
}
...
allow_remove(int note)
{
    if (this_player()->query_nazgul())
    {
        ...
        {
            IDLENESS->increase_activity( this_player(), REMOVE_NOTE );
            return 1;
        }
    }
...
{
    if (note && (this_player()->query_real_name() == query_author(note)))
	   return 0;

    if(this_player()->query_morgul_title() == "Lieutenant of Morgul" &&
        query_author(note) == "lieutenant")
       return 0;
...
create_object()
{
    ...
    add_prop(OBJ_M_NO_GET, "It is firmly bolted and secured to the wall.\n");
...
init()
{
    ...
    add_action(lt_new_msg,     "ltnote");
    add_action(nazgul_new_msg, "nnote");
    add_action(king_new_msg,   "knote");
...
/*
 * Function name: lt_new_msg
 * Description  : Enabled for the Lieutenant of Morgul to write a note.
 * Arguments    : string msg_head - the header of the header.
 * Returns      : int 1/0 - success/failure.
 */
public nomask int
lt_new_msg(string msg_head)
{
    string date;
    int    rank;
    object editor;
...
/*
 * Function name: nagul_new_msg
 * Description  : Enabled for the Nazgul to write a note.
 * Arguments    : string msg_head - the header of the header.
 * Returns      : int 1/0 - success/failure.
 */
public nomask int
nazgul_new_msg(string msg_head)
{
    string date;
    int    rank;
    object editor;
...
/*
 * Function name: king_new_msg
 * Description  : Enabled for the King of Morgul to write a note.
 * Arguments    : string msg_head - the header of the header.
 * Returns      : int 1/0 - success/failure.
 */
public nomask int
king_new_msg(string msg_head)
{
    string date;
    int    rank;
    object editor;
