...
inherit "/d/Gondor/common/guild2/lib/library";
...
void create_morgul_tower_room()
{
    ...
    add_exit( MAGES_DIR + "throne_room", "north", "@@prevent_leave_with_books@@", 0 );
    ...
    set_library_save_path("/d/Gondor/common/guild2/books/nazgul/");
    set_library_save_file("/d/Gondor/common/guild2/books/nazgul_library");
    set_library_allow_new_books(1);
    set_library_log_file("morgul_nazgul_lib");
...
hang_cmd( string str )
{
 ...
 string  vb = query_verb();
    ...
    IDLENESS->increase_activity( this_player(), 4*MINOR_ACTIVITY );
    return 1;
}
...
{
    // Reward the Nazgul
    IDLENESS->increase_activity( this_player(), 2*MINOR_ACTIVITY );
...
{
    // Reward the nazgul
    IDLENESS->increase_activity( this_player(), NAZ_LIB_MOVE);
...
{
    // Reward the nazgul
    IDLENESS->increase_activity( this_player(), NAZ_LIB_CREATE );
