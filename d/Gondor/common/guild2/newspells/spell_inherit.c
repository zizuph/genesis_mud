 ...
 *              : Chmee, May, 2002.
 *              : Added more descriptions to the casting of spells to 
 *              : give a grander experience to the players.
 *              : Toby, 21 Oct, 2006
 *              : restore_components(gReq_ingrs_found) added to the movement check
...
public int
is_open_component_container(object obj)
{
    if (obj->query_prop(CONT_I_HOLDS_COMPONENTS)
        && !obj->query_prop(CONT_I_CLOSED))
    {
        return 1;
    }
    return 0;
...
/*
 * Function:    get_item_search_list
 * Description: This is used by the function find_components in the base spell
 *              object to search through all the available components that the
 *              user has access to. This function is fundamentally the same
 *              as the base class's version, but using filters and foreach
 *              instead.
 *              The old version of this function is commented out. It used to
 *              allow for components to be fetched from closed containers, and
 *              also searched nested (1st level) containers. This is not an
 *              approved ability, and also was causing performance issues.
 */
public object *
...
{      
    ...
    object * component_containers = filter(inv, is_open_component_container);
    foreach (object component_container : component_containers)
    {
        inv += all_inventory(component_container);
    }
