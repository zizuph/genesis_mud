...
resolve_spell( object caster, object *targets, int *resist, int result )
{
 ...
 object anvil, *spect_caster, *spect_target;
    ...
    {
        write("check_mage() in resolve_spell().\n");
	return;
    }
...
int config_spell( object caster, object *targets, string args )
{
    ...
    {
	write("Spell failed here.\n");
        return 1;
    }
    set_spell_mana( 70 );
    set_spell_time(spell_resolve_time( caster->query_stat(SS_OCCUP),2,6 ));
    ...
    {
        set_spell_ingredients(({"gosta_ingr2"}));
    }
    write("Checkpoint1.\n");
    return 0;
}
