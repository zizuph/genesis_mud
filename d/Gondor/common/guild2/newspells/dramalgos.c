/*
 *  Changed if (targets[0]->query_hp() <= 0) to
 *          if (objectp( targets[0] ) && targets[0]->query_hp() <= 0)
 *  This should take care of the runtime error recieved.
 *          Toby, 2006-12-05 >
 *
 *  Toby, 2007-05-11
 *     Upped mana 30 to 40-45 and lowered casting time with 2
 *     in an attempt to make the occ casters not die out entirely
 */
...
desc_bolt_cast( object caster, object *targets )
{
    caster->catch_tell( one_of_list( ({ 
        "With the power of the Dark Lord has bestowed upon you, you call the Storm Wind.\n", 
        "Using the powers the Black Master has bestowed upon you, you call the Storm Wind.\n", 
        "With a touch of the Dark Lord's powers, you call the Storm Wind.\n", 
        "Feeling the power of the Dark Lord flowing through you, you call the Storm Wind.\n", 
        "The power of the Black Master surges through your undead body as you call the Storm Wind!\n", 
                                          })) );
}
    ...
    for (i=0;i<k;i++)
    {
        ...
        if (j >= ran)
        {
            ...
            break;
                                ...
                                /*Ocasionaly I get an index out of bounds
                                 *runtime error at this point. This should
                                 *patch the problem, however, the choice
                                 *of the i value in the case of error might
                                 *not be the best.                        */
    i = min( i, k- 1 );
...
config_spell( object caster, object *targets, string arg )
{
    ...
    if (caster->query_npc())
    {
        ...
        set_spell_time( max(spell_resolve_time( caster->query_stat( SS_OCCUP ),
            2, 4 ), COMBAT_SPELL_I_MIN_PREP_TIME - 2));
        set_spell_mana( 30 ); 
    ...
    set_spell_mana( 40 + random(5) ); // upped mana (30->40-45) to compensate for slightly faster cast time
    set_spell_task( TASK_SIMPLE );
    set_spell_element( SS_ELEMENT_AIR, 34 );
    set_spell_form( SS_FORM_CONJURATION, 40 );
    bolt_pen = calculate_dramalgos_pen( caster );
