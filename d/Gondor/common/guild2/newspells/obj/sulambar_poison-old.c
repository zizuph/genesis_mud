...
public int *poison_count = ({ 0, 0, 0 });
                                /*Note, that sizeof( _time_stamps ) is
                                 *is always equal to Dose- 1              */
public int Dose = 1;
float *_time_stamps = ({});
...
public string query_poison_recover()
{
 ...
 string output = "";
    ...
    {
        for (i=0;i<j;i++)
        output += ","+ ftoi( _time_stamps[i] );
    }
    output += ","+ poison_count[pc_STR]+ " "+ poison_count[pc_CON]+
        " "+poison_count[pc_DEX];
    return output;
}
...
public void init_poison_recover( string arg )
{
    ...
    arglist = explode( arg, "," );
    sscanf( arglist[0], "%d", Dose);
    j = Dose-1;
    for (i=0;i<j;i++)
    {
        sscanf( arglist[i+1], "%f", temp);
        _time_stamps += ({ temp });
    }

    if (sscanf( arglist[i+1], "%d %d %d", poison_count[pc_STR],
        poison_count[pc_CON], poison_count[pc_DEX] ) != 3)
    {
        return;
    }
}
...
int increase_dose( int vpd_time, int vpd_str )
{
 float delta;
    ...
    delta = itof( vpd_time )- get_alarm( a_time )[2];
    if ( delta >= 1.0 )
    {                           /*strength is a global variable declared in
                                 *poison_effect and set by set_strength() */
        if (vpd_str != strength)
        {
            set_strength( (strength* Dose + vpd_str)/ (Dose+ 1) );
        }
        _time_stamps += ({ delta });
        return Dose++;
    }
