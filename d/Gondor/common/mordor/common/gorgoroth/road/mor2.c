...
// "flies eastwards towards Barad-Dur.\n");
    ...
    set_short("At a broad crossroad"); 

    set_long("The broad paved road splits in four directions leading " +
     "southwards further into Mordor while northward towards the " +
     "gates of Carach Angren.  The eastroad continue following the " +
     "mounainrange Ered Lithui while westroad follows Ephel Duath.\n");

    add_exit(ROOM + "mor1", "north");
    add_exit(ROOM + "mor3", "south");
    add_exit(ROOM + "more1", "east");
    add_exit(ROOM + "morw1", "west");
