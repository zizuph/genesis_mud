 ...
 *                Chmee's version for Morgul library.
 * Arguments    : string file - the filename for the desired book
 * Returns      : The book's author
 * Caveats      : This function is far from perfect since it has problems
 *                if the author's name word is separated from one of its
 *                neighborow by EOL - '\n' character.
 */
public string
query_book_author( string file )
{
 ...
 string *words;
    ...
    str = read_file( file, 1, 3 );
    words = explode( str, " " );

    if ((i = member_array( "written", words )) == -1)
    {
        i = member_array( "written\n", words );
    }
    if (i != -1)
    {
        if ((words[i+1] == "by") || (words[i+1][-1..] == "\n"))
        {
            author = words[i+2];
        }
    }
