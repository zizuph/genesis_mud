/*
 *	/d/Gondor/common/lib/inn.c
 *
 *	Standard functions for inns and pubs.
 *
 *	Olorin, November 1995
 *	Using old code by Elessar.
 *
 *	Modification log:
 *
 *	Gnadnar, 15-Feb-98:	support an optional Innkeeper_npc npc;
 *				also allow customizing innkeeper name
 *				even if the inn doesn't use an npc.
 */
#pragma strict_types
...
buy_it(string item_name, int number, string pay_type, string get_type)
{
    ...
    switch (item_name)
    {
        ...
        case "single room":
        case "night":
            if (TO->query_friend_or_foe(TP) == -1)
            {
                tmpstr = CAP_INNKEEPER(TP);
                write(tmpstr+" glares at you.\n"+
                      tmpstr+" says: I'll let you drink "+
		          "my ale and eat my food.\n"+
                      tmpstr+" says: But I will not have "+
		          "filthy scum like you sleeping in my beds!\n");
                return 1;
            }
	    if (number > 1)
	    {
...
do_buy(string str)
{
    ...
    if (number > max_buy)
    {
        NFN("You cannot buy more than "+max_buy+" items at a time!");
        return 0;
    }
