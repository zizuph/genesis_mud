 ...
 *
 *  Modification log:
 * March 2019, Finwe: Fixed some grammar issues.
 */
#pragma save_binary
#pragma strict_types

inherit "/d/Gondor/std/room.c";
inherit "/d/Genesis/lib/shop.c";
...
create_shop()
{
    set_long("The standard shop of the domain Gondor.\n");
    set_short("A shop");
    set_store_room(COMMON_DIR + "lib/store");
    set_shop_name("Gondor's Standard Shop");
    set_keeper_file(EDORAS_DIR + "npc/alwine");
    set_no_sell_prop("_Gondor_no_sell");
}
...
create_gondor()
{
    ...
    add_item(({"item","items"}), BSN(
        "There are far too many items laying around to pick out just one. You "
      + "should take a betterlook at the pricelist to get an idea of what "
    ...
    add_item(({"owner","store owner"}), BSN(
      ...
      + "own to check out the stuff that is laying around."));
