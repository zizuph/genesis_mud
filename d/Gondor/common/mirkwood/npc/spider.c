...
#define WEB "../obj/web"
#define MIRK_OBJ_DIR "../obj/"
//#define ONE_OF_LIST(x) (x[random(sizeof(x))])
#define LEG	(MIRK_OBJ_DIR+"spider_leg")
...
sting_web(int arg)
{
	...
	if (sizeof(in = FILTER_LIVE(all_inventory(ob))))
	{
        ...
        tell_room(environment(), QCTNAME(TO)+" stings the web.\n");
	...
	tell_room(environment(), QCTNAME(TO)+" unweaves the web.\n");
	ob->remove_object();
	return;
    }
    if (sizeof(FILTER_PLAYERS(all_inventory(environment(TO)))))
    {
	set_alarm(itof(random(20)+10), 0.0, &sting_web(0));
	return;
    }
...
init_attack()
{
	...
	return;
    TP->catch_msg(QCTNAME(TO)+" is running to you on its "+
