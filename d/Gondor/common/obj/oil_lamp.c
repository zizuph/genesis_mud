 ...
 *		don't rot
 *  26-Jun-2004, Mercade: changed math so that it wouldn't do an
 *      alarm delay of 115 days (!).
 *  27-May-2006, Palmer: Use standard "_flask_of_oil" name 
 *      to ensure compatability from domain to domain
 */
...
create_torch()
{
    ...
    set_short("iron oil-lamp");
    set_pshort("iron oil-lamps");
...
long_description()
{
    ...
    if (!strlen(lamp_desc))
        longstr = "It is an iron oil-lamp. It is a " + cond +
	    " lamp with a wick behind a " + bulb_desc + " glass bulb. ";
    else longstr = lamp_desc;

    if (Max_Time <= 0 ||
        !query_time_left())
    {
        longstr += "The oil-lamp does not contain any oil. ";
    }
    else
    {
        longstr += "The oil-lamp contains some oil. ";
    }

    if (query_keep())
    {
        longstr += "At present, you do not want to sell the oil-lamp.";
    }
    else
    {
        longstr += "If you do not want to sell the lamp in the future, just " +
           "decide to <keep oil-lamp>.";
    }
...
do_refill(string str)
{
    ...
    oil_flasks += filter(all_inv, &->id("_flask_of_oil_"));
