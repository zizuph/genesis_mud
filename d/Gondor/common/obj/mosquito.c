 ...
 *      - Jan 1999, gnadnar: check for armour on target body part. it looks
 *			 odd to be told "you're stung on your right hand"
 *			 when you're wearing steel gauntlets.  not even
 *			 new jersey mosquitos are that tough :-)
 */
...
#define DOUBLE_PARTS ({ "biceps", "hand", "cheek", "ear", "leg", "thigh", \
			"fore-arm", "upper arm", "calf", "ankle" })
#define DOUBLE_LOCS ({	TS_LARM, TS_LHAND, 0, TS_LEAR, TS_LEGS, TS_LEGS, \
			TS_LARM, TS_LARM, TS_LEGS, TS_LANKLE, \
			TS_RARM, TS_RHAND, 0, TS_REAR, TS_LEGS, TS_LEGS, \
			TS_RARM, TS_RARM, TS_LEGS, TS_RANKLE, })
#define SINGLE_PARTS ({ "nose", "belly", "breast", "chin", "forehead" })
#define SINGLE_LOCS ({ 0, TS_TORSO, TS_CHEST, 0, 0 })

#define RANGER_I_NOT_BRAWL "_ranger_i_not_brawl"
#define BASE_DEX           (5)
#define DEX_LIMIT          (5)

#define MOSQUITO_POISON    (POISON_DIR + "mosquito_sting")
...
int *double_locs = DOUBLE_LOCS;
int *single_locs = SINGLE_LOCS;
...
try_sting(object victim)
{
    ...
    s = sizeof(single_parts);
    d = sizeof(double_parts);
    part = random((2 * d) + s);
    if (part < s)
    {
	loc = single_locs[part];
	body_part = single_parts[part];
    }
    else
    {
	part -= s;
	loc = double_locs[part];
	if (part < d)	/* left */
	{
	    body_part = LEFT_RIGHT[0] + " ";
	}
	else /* right */
	{
	    part /= 2;
	    body_part = LEFT_RIGHT[1] + " ";
	}
	body_part += double_parts[part];
    }

    if (loc &&
	objectp(victim->query_armour(loc)))
    {
	/* part is protected */
	return;
    }
