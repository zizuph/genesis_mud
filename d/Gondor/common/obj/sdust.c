 ...
 *
 * A bag of silver dust to be used as a paint additive and
 * a quest object. If tossed into the air, there is a chance
 * that some will get in your eyes. The dust is magic, so
 * will raise one's awareness. It is located in the treasure
 * chest in the corsair manor: 
 * /d/Gondor/ithilien/poros/obj/chest_treasure.c in
...
create_gems()
{
    config_gems("bag of silver dust", "bags of silver dust", "", 5000,
        GEM_VERY_RARE, 52, 64);
    set_long("These are bags of silver dust, packed full " +
        "of beautiful fine silver dust. You imagine that one might " +
        "be able to toss the dust " +
        "into the air and watch it sift down like magic faerie dust.\n");
    add_name(({"dust", "bag", "_gondor_silver_dust"}));
    add_adj(({"silver", "of", "bag"}));
    remove_name("gem");
    remove_adj("bag of silver dust");
    add_prop(MAGIC_AM_MAGIC, ({75, "enchantment"}));
    add_prop(MAGIC_AM_ID_INFO, ({"A glimmer of enchantment emanates " +
        "from the dust.\n", 20, "A small amount of silver dust in " + 
...
toss(string str)
{
    ...
    if (member_array(SHINE_SUBLOC, this_player()->query_sublocs()) == -1)
    {
        ...
        shine->set_colour("silver");
        shine->move(this_player(), 1);
        shine->fun_stuff();

        if (random(100) < 66)
        {
            extra = "Some flecks of silver dust get in your eyes, " +
                "and your surroundings take on a silver glow.";
            shine->add_extras();
        }

        write("You toss the bag of silver dust into the air, where " +
            "its tiny particles float down to the ground. A stray " +
            "current of air sends some silver dust straight into " +
            "your face. " + extra + "\n");
        tell_room(environment(this_player()), QCTNAME(this_player()) +
            " tosses a bag of silver dust into the air, where " +
            "its tiny particles float down to the ground. A stray " +
            "current of air sends some silver dust straight onto " +
            QTPNAME(this_player()) + " face.\n", this_player());
    }
    else
    {
        write("You toss the bag of silver dust into the air, where its " +
            "tiny particles float down to the ground.\n");
        tell_room(environment(this_player()), QCTNAME(this_player()) +
            " tosses a bag of silver dust into the air, where its " +
            "tiny particles float down to the ground.\n", this_player());
    }
