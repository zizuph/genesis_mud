...
inherit "/d/Gondor/common/obj/animal_corpse";
...
{
    NF(BSN("You have to skin and disembowel the rabbit before you can "+verb+" it!"));
    return skinned;
}
...
do_skin(string str)
{
    ...
    if (!sizeof(knives))
    {
        write("You need a knife to "+verb+" the rabbit.\n");
        return 1;
    }
    ...
    {
        write(BSN("Inexpertly, you try to "+verb+" the rabbit, but you completely "
          + "ruin the pelt in the process. All you are left with is the skinned "
          + "body of the rabbit."));
       say(QCTNAME(TP)+" tries to "+verb+" a rabbit, but completely ruins the "
          + "pelt in the process.\n");
       return 1;
  }

    write(BSN("You skillfully skin and disembowel the dead rabbit. "
      + "Now you have a skinned rabbit and a rabbit pelt."));
    say(QCTNAME(TP) + " skillfully skins and disembowels a dead rabbit.\n");
    FIX_EUID
    pelt = clone_object(OBJ_DIR + "rabbitpelt");
