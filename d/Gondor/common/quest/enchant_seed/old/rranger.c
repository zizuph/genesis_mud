...
//Prototypes
void    do_give_back(object obj, object to);
void    do_inform(string what, object pl);
void    do_reward(object player);
void    do_thank(object player);
void    no_sentence(string what);
void    pigeon_msg();
...
create_ranger()
{
  ...
  set_race_name("human");
  set_gender(G_MALE);
  set_short("old rugged man");
  set_long(BSN("This old man looks like he has experienced quite a lot. His "+
    "face is wrinkled and weather-beaten, and his hair gray of age. He looks "+
    "back at you with a gleam of interest in his eyes."));
  set_adj(({"old", "rugged", }));
...
enter_inv(object obj, object from)
{
    ...
    if (!objectp(from) || !living(from) || !objectp(obj))
       return;

    if (!(obj->id("enchanted_seed")))
    {
        set_alarm(3.0, 0.0, &do_give_back(obj, from));
        return;
    }

    if (from->test_bit("Gondor", GONDOR_GROUP, ENCHANT_SEED_BIT))
...
do_thank(object player)
{
    ...
    command("say Thanks again for your help!");
    command("say Here is an attanar-herb - hope it will be helpful!");
...
do_reward(object player)
{
    ...
    clone_object(HERB_DIR+"attanar")->move(pouch,1);
    clone_object(HERB_DIR+"attanar")->move(pouch,1);
    clone_object(HERB_DIR+"suranie")->move(pouch,1);
    clone_object(HERB_DIR+"tuo")->move(pouch,1);
    pouch->move(TO, 1);
    ...
    else
    {
        ...
        player->catch_msg("You feel more experienced.\n");
        command("give pouch to "+player->query_real_name());
        command("say There you are! You can probably buy more herbs from Lothmir.");
        command("say Both can tell you more about these herbs, if you like.");
        command("smile");
    }
...
do_inform(string what, object pl)
{
    ...
    case "magic":
        ...
        break;
    case "magic2":
        command("say I know of someone who still takes interest in magic though.");
    ...
    case "rangers":
        ...
        break;
    case "seeds":
        command("say I don't know how to enchant the lothore-seeds,");
    ...
    case "lebennin":
        ...
        command("say Ex-ranger, I should say. He's very interested in Numenorean magic.");
        command("ponder");
        break;
    case "leb_ranger":
...
pigeon_msg()
{
    tell_room(ENV(TO), BSN(
        "The old ranger writes a brief message on a piece of paper, "+
        "ties it to the leg of a carrier pigeon, which he sends off."));
}
