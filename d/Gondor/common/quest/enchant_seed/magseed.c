/*
 * /d/Gondor/common/quest/enchant_seed/magseed.c
 *
 * coded by ???
 *
...
create_object()
{
    set_name( ({ "seed", "_gondor_enchanted_seed" }) );
    set_adj( ({ "shimmering", "green" }) );
    set_long("This green seed shimmers with a faint red light.\n");
...
plant_me(string arg)
{
    ...
    SAY(" plants a shimmering green seed in the ground.");
    FIX_EUID;
    clone_object(HERB_DIR + "lothore")->move(env);
    tell_room(env,
	"After a short while, a beautiful red flower springs up!\n");
