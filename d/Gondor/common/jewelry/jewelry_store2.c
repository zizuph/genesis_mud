...
restore(string str)
{
    ...
    object room, jewelry;
    string *vars = ({});
    string str1, gem1, gem2, engrave, order, jtype, metal, desc,
           emblem, colour;

    if (!strlen(str))
    {
        notify_fail("Restore <'regalia', owner gem1/'n', number-gem1, " +
            "gem2/'n', number-gem2, order> or <'jewelry', type, metal, " +
            "desc/none, gem1/'n', number-gem1/0, gem2/'n', number-gem2/0, " +
            "engraving>\n");
        return 0;
    }
    ...
    if (sizeof(vars) == 7 && lower_case(vars[0]) == "regalia")
    {
        engrave = capitalize(engrave);
        if (!SECURITY->exist_player(engrave))
        {
            write(engrave + " is not a real player.\n");
            return 1;
        }

        (JEWELRY_DIR + "chivalry")->teleledningsanka();
        room = find_object(JEWELRY_DIR + "chivalry");
        order = room->case_check(order);
        rank = room->find_rank(order, engrave);

        if (rank == -1)
        {
            write(engrave + " is not a member of the Order of " +
                order + ".\n");
            return 1;
        }

        emblem = room->query_order(order)[1];
        colour = room->query_order(order)[2];
        ...
        if (rank != MEMBER_RANK)
        {
            if (gem1 != "n")
            {
                ...
                if (gem1 == "lapis" || gem1 == "lazuli")
                    gem1 = "lapis lazuli";
                else if (extract(gem1, -2) == "es" &&
                  gem1 != "rubies" && gem1 != "chalcedonies")
                    gem1 = extract(gem1, 0, -2);
                else
                    gem1 = LANG_SWORD(gem1);
            }

            if (gem2 != "n")
            {
                if (gem2 == "lapis" || gem2 == "lazuli")
                    gem2 = "lapis lazuli";
                else if (extract(gem2, -2) == "es" &&
                  gem2 != "rubies" && gem2 != "chalcedonies")
                    gem2 = extract(gem2, 0, -2);
                else
                    gem2 = LANG_SWORD(gem2);
            }

            if (rank == KGC_RANK && (num1 + num2) != 10 &&
              (num1 + num2) != 0)
            {
                write("The number of gems required for " +
                    LANG_POSS(engrave) + " regalia is 10, not " +
                    (num1 + num2) + ".\n");
                return 1;
            }
            else if (rank == KC_RANK && (num1 + num2) != 5 &&
              (num1 + num2) != 0)
            {
                write("The number of gems required for " +
                    LANG_POSS(engrave) + " regalia is 5, not " +
                    (num1 + num2) + ".\n");
                return 1;
            }
        }
        ...
        jewelry = clone_object(JEWELRY_DIR + "regalia");
        jewelry->set_owner(engrave);
        jewelry->set_order(order);
        jewelry->set_order_rank(rank);
        jewelry->set_emblem(emblem);
        jewelry->set_colour(colour);
        jewelry->set_gems(gem1, num1, gem2, num2);
        jewelry->init_regalia();
        jewelry->move(this_player(), 1);
        write("A " + jewelry->short() + " materializes in your " +
            "inventory.\n");
        log_file("jewelry", extract(ctime(time()), 4, 15) + " " +
            capitalize(this_player()->query_real_name()) +
            " restored " + jewelry->short() + " for " + engrave + ".\n");
        return 1;
    }
    else if (sizeof(vars) == 9 && lower_case(vars[0]) == "jewelry")
    {
        jewelry = clone_object(JEWELRY_DIR + "jewelry");
        jewelry->set_desc(desc);
        jewelry->set_metal(metal);
        jewelry->set_gems(gem1, num1, gem2, num2);
        jewelry->set_type(jtype);
        jewelry->set_engrave(engrave);
        jewelry->init_jewelry();
        jewelry->move(this_player(), 1);
        write("A " + jewelry->short() + " materializes in your " +
            "inventory.\n");
    ...
    {
        notify_fail("Restore <'regalia', owner gem1/'n', number-gem1, " +
            "gem2/'n', number-gem2, order> or <'jewelry', type, " +
            "metal, desc/none, gem1/'n', number-gem1/0, gem2/'n', " +
            "number-gem2/0, engraving>\n");
        return 0;
    }
}
