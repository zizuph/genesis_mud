...
private varargs int     add_dunadan(string name, string house, string lordname = 0);
private         int     do_join(string house, object tp, object newbie);
private         int     remove_from_tree(string name, string house, int nohouse = 1);
private         void    set_up_houses();
private         void    add_to_tree(string name, string house);
...
create_gondor()
{
    ...
    set_long(BSN(
        "The Chamber of the Houses in the House of the Dunedain in "
      + "Gondor. In this Chamber ceremonies important to the Families of "
      + "the Dunedain are performed. Here betrothals are announced, dead "
      + "lie in state, and the young are formally introduced into their "
      + "rights and duties as grown-ups when they have reached the age. "
      + "In this Chamber, the heads of the Houses of the Dunedain can "
      + "also adopt new members into their families. On the west wall there "
      + "are large tables listing the members of the Houses of the "
      + "Dunedain. On a stone pedestal in front of the tables is a large "
      + "book."));
...
purge_house(mixed house)
{
    ...
    // Purge the members of the house
...
do_release(string name)
{
    ...
    newbie->add_prop(DUNEDAIN_S_RELEASED_BY, tp_name);
...
do_join(string house, object tp, object newbie)
{
    ...
    if(!(result = add_dunadan(LOW(nname), house, lname)))
    {
        ...
        write("For unknown reasons you are not able to adopt "+nname+".\n");
        return 0;
    }
 ...
 * Description:   renounce your House - leave the House, but not the guild
 ...
 *                the dunadan will remain in the guild
 * Argument:      str - command line argument
 * Returns:       1/0 - success/failure
 */
public int
do_expel(string str)
{
    ...
    {
        write("That person is not within the realms of Genesis at the moment!\n");
        return 1;
    }
