/*
 *	/d/Gondor/common/dunedain/rooms/advance_room.c
 *
...
#include "/d/Gondor/common/dunedain/lib/riddles.h"
...
create_gondor()
{
    ...
    set_long(BSN("The Chamber of Knowledge in the Houses of the Dunedain. "
      + "Here the members of the Houses of the "
      + "Dunedain are tested regarding their knowledge of history and "
      + "lore. Only those who pass the tests gain status in the eyes "
      + "of their fellow dunedain. If you want to take a test, try "
      + "to 'advance' your status."));

    add_exit(DUNEDAIN_DIR + "join_room", "east", 0, 0);
...
do_advance(string str, string name = TP->query_name())
{
	...
	if ((member_array(house, DUNEDAIN_LIBRARY->query_names()) < 0) &&
	    (member_array(house, PUBLIC_LIBRARY->query_names()) < 0))
	{
	    write("You cannot advance further!\n"
		  + BSN("Before you can become "
			...
			+ "Library of the Dunedain!"));
	    return 1;
	}
	
	result = (DUNEDAIN_MASTER->promote_to_lord(TP->query_name()));
	if (stringp(result))
	{
    ...
    log_file("dunedain_rank", TP->query_name() + ": "
      + riddles[q*2] + "\n");
...
answer_riddle(string str, mixed sol)
{
    ...
    {
        write("You failed.\n");
        log_file("dunedain_rank", TP->query_name() + " failed with "
    ...
    if (!sizeof(sol))
    {
        write("You have answered correctly and passed the test!\n"
            + "You have gained status among the dunedain!\n");
        TP->set_dunadan_level(
            MIN(DL_SUB_LORD, (new_level = (TP->query_dunadan_level() + DL_RIDDLE_SCALE))));
        log_file("dunedain_rank", TP->query_name() + " advanced to level "
