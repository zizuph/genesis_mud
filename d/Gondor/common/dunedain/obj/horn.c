/*
 *	/d/Gondor/common/dunedain/obj/horn.c
 *
 *	Olorin, 31-Jan-1997
 *
...
check_give()
{
    if (strlen(House))
    {
	if (House == TP->query_dunadan_house())
	    return 0;
	else
	    return 0;
    }
...
blow_horn(string str)
{
    ...
    filter(users() - players - ({ tp }), &operator(==)(House) @ 
	   &->query_dunadan_house())->catch_tell("From afar, you hear " +
...
query_auto_load()
{
    if (strlen(House) && (ENV(TO)->query_dunadan_house() == House))
...
move(mixed dest, mixed subloc)
{
	...
	(dest->query_dunadan_house() != House))
    {
...
public int
set_house(string str, int force = 0)
{
