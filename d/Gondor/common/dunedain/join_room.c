/*
 *    Master file for Dunedain guild
 *
 *    Copyright (c) 1994, 1997 by Christian Markus
 *
 *    Coded by Olorin, 1994.
 *
 *    Modification log:
 *    10-Feb-1997, Olorin:    Added list and remove commands for
 *                Lords. 
 *    11-Feb-1997, Olorin:    Revision.
 *                Added "release" command and support.
 *      20-Feb-2000, Skippern:  Revisions in release.
 *      03-May-2002, Skippern:  Added "drelease" command since many players
 *
 *  07-May-2006 Palmer/Tigerlily:
 *     Removed remove_lord_through_death
 */
...
#define DUNEDAIN_TREE           (GUILD_DIR + "dunedain")
#define DUNEDAIN_S_APPLIED_TO   "_dunedain_s_applied_to"
#define DUNEDAIN_M_OATH         "_dunedain_m_oath"
#define DUNEDAIN_I_LINE         "_dunedain_i_line"
#define DUNEDAIN_S_VOUCHED_FOR  "_dunedain_s_vouched_for"
#define DUNEDAIN_S_RELEASED_BY  "_dunedain_s_released_by"
/* 2 months idle time */
...
public  varargs int     query_number_dunedain(string house, int silent = 1);
...
public          string  exa_tables();
public  varargs string  query_dunadan_house(string name);
public          string  query_is_lord(string lname);
public varargs  int     add_unhoused_dunadan(string name, string house, string lordname = 0);
public varargs  int     add_dunadan(string name, string house, string lordname = 0);
...
public          int     remove_dunadan_from_house(object pl, string house, string reason);
...
public          void    purge_dunedan_houses();
public          void    remove_lord_from_tree(string name, string house);
...
create_gondor()
{
    ...
    set_short("the Chamber of the Houses");
    set_long("This is the Chamber of the Houses of the Dunedain in Gondor. " +
        "Here, ceremonies important to the families of the Dunedain are " +
        "performed. Betrothals are announced, dead lie in state, and " +
        "the young are formally introduced into their rights and duties " +
        "as grownups when they have reached the age. The heads of the " +
        "Houses can also adopt new members into their families. Large " +
        "tables line the west wall. Each table is dedicated to members " +
        "of the Houses of the Dunedain. A plaque hangs on a wall, and " +
        "on a stone pedestal in front of the tables sits a large book.\n");

    add_exit(GUILD_DIR + "rooms/hallw_n", "south", 0, 0);
    add_exit(GUILD_DIR + "rooms/advance_room", "west", check_exit, 0);

    add_prop(ROOM_I_INSIDE, 1);

    add_item(({"tables", "table", "houses", "house", "dunedain", }),
        exa_tables);
    add_item("pedestal", BSN(
        "The stone pedestal is standing in front of the tables hanging "
      + "on the walls. On it lies a large book."));

    add_item("plaque", "It is made of brass and hangs on a wall.\n");
    add_cmd_item("plaque", "read", "@@read_plaque@@");
    
    seteuid(getuid());
    restore_object(DUNEDAIN_TREE);

    if (!m_sizeof(Houses))
        set_up_houses();

    clone_object(DUNEDAIN_DIR + "obj/guild_book")->move(TO);
    ...
    set_alarm(3666.0, 0.0, &purge_dunedan_houses());
}
...
{
    int     i,
            sh = sizeof(DUNEDAIN_HOUSES);

    for (i = 0; i < sh; i++)
    {
        Houses[DUNEDAIN_HOUSES[i]] = ({ 0, -1, ({}) });
    }

    save_object(DUNEDAIN_TREE);
}

/*
 * Function name: purge_house
 * Description:   help function to purge_dunedan_houses
 */
public mixed
purge_house(mixed house)
{
    ...
    if (check_idle_lord(lower_case(house[DT_S_LNAME])))
    {
        ...
        write_file(DUNEDAIN_LOG, "LORDS: " + capitalize(house[DT_S_LNAME]) +
...
/*
 * Function name: purge_dunedan_houses
 * Description:   purges non existing players from the
 *          Houses mapping.
 */
public void
purge_dunedan_houses()
{
...
/*
 * Function name: exa_tables
 * Description:      examine/read the tables
...
exa_tables()
{
    ...
    desc = "There are "+LANG_WNUM(sizeof(DUNEDAIN_HOUSES))+" tables, "
         + "each listing the members of one of the Houses of the "
         + "Dunedain. The names of the Houses are: "
         + COMPOSITE_WORDS(DUNEDAIN_HOUSES)
         + ".";
...
do_examine(string str)
{
    ...
    if (!parse_command(str, ({ }),
        "[the] 'house' / 'table' [of] [the] [house] [of] %w", house))
        return 0;

    house = CAP(house);

    if (member_array(house, DUNEDAIN_HOUSES) < 0)
        NFN0("There is no House of the Dunedain called "+house+"!");

    hlist = Houses[house];

    desc = BSN("You look at the table listing the members of the House of "
         + house+".") + "\n";
...
/*
 * Function name: tell_dunedain
 * Description:   send a message to all dunedain
 * Arguments:     msg  - the message to be sent
 *                name - either a string or an array containing the
 *                       name or names of dunedain that should not
 *                       receive the message.
 */
public varargs object *
tell_dunedain(string msg, mixed name)
{
    int     i;
    object *dun,
            soul = find_object(DUNEDAIN_SOUL);

    FIX_EUID;
    dun = filter(users() - ({ 0 }), soul->query_is_dunadan);
    if (stringp(name))
...
do_enter(string str)
{
    ...
    if (tp->query_race() != DUNEDAIN_RACE)
        NFN0("Only humans can belong to the Houses of the Dunedain!");

    if ((gname = tp->query_guild_name_race()) == GUILD_NAME)
    {
        house = tp->query_dunadan_house();
        if (strlen(house) && member_array(house, DUNEDAIN_HOUSES) > -1)
        {
            if (!strlen(lordname = tp->query_prop(DUNEDAIN_S_RELEASED_BY)) ||
    ...
    {
        NFN("Enter which House of the Dunedain?");
    return 0;
    }
    ...
    if (member_array(house, DUNEDAIN_HOUSES) < 0)
    {
        NFN("There is no House of the Dunedain called "+house+"!");
    return 0;
    }
    ...
    if (!(query_open_house(house)))
    {
        NFN("The House of "+house+" has no members yet, and it is "
          + "presently not possible to open a new House of the Dunedain.");
        return 0;
    }
    ...
    NFN("A member of the House of " + house + 
        ...
        if (!(strlen(voucher = tp->query_prop(DUNEDAIN_S_VOUCHED_FOR))))
            return 0;

        if (!(objectp(member = find_player(LOW(voucher)))) ||
            !objectp(present(member, TO)) ||
            ((member_array(voucher, Houses[house][DT_AS_MEMBERS]) < 0)
         && (member->query_dunadan_level() < DL_LORD)))
            return 0;
    }

    if (gname == GUILD_NAME)
    {
        if (result = add_unhoused_dunadan(name, house))
        {
    ...
    if (result = add_dunadan(name, house))
    {
        say(QCTNAME(tp)+" enters the House of "+house+"!\n", ({tp}));
        return 1;
    }
...
do_vouch(string str)
{
    ...
    if (newbie->query_guild_name_race() == GUILD_NAME &&
      strlen(newbie->query_dunadan_house()))
    {
        NFN(CAP(name)+" is already a member of the Houses of the Dunedain!");
        return 0;
    }

    if (newbie->query_race() != DUNEDAIN_RACE)
    {
        NFN("Only humans may join the Houses of the Dunedain!");
        return 0;
    }

    newbie->add_prop(DUNEDAIN_S_VOUCHED_FOR, tp_name);
...
do_release(string name)
{
    ...
    if ((tp->query_dunadan_level() < DL_LORD) && !tp->query_wiz_level())
    {
        NFN("Only the Lord or the Lady of a House of the Dunedain "
    ...
    if (newbie->query_guild_name_race() != GUILD_NAME)
    {
        NFN(CAP(name)+" is not a member of the Houses of the Dunedain!");
        return 0;
    }

    if (newbie->query_dunadan_house() != tp->query_dunadan_house())
    {
        NFN(CAP(name)+" is not a member of your House!");
        return 0;
    }

    remove_dunadan_from_house(newbie, newbie->query_dunadan_house(), "released by "+CAP(tp->query_name())+" from");
    write("You release "+CAP(name)+" from your service.\n");
    say(QCTNAME(tp)+" releases "+QTNAME(newbie)+" from " + 
    tp->query_possessive() + " service.\n", ({ tp, newbie }));
    newbie->catch_msg(QCTNAME(tp)+" releases you from " + 
              tp->query_possessive() + " service.\n" +
              "You are now free to ask another Lord or Lady " +
              "of the Houses of the Dunedain to adopt you.\n");
    return 1;
}
...
do_apply(string str)
{
    ...
    if (tp->query_race() != DUNEDAIN_RACE)
    {
        NFN("Only humans can belong to the Houses of the Dunedain!");
    return 0;
    }
     ...
     */
        if (strlen(house = tp->query_dunadan_house()) && 
        (member_array(house, DUNEDAIN_HOUSES) > -1))
    {
        if ((strlen(lordname = query_house_lord(house))) &&
            (lordname != tp->query_prop(DUNEDAIN_S_RELEASED_BY)))
        {
    ...
    {
        NFN("Apply for membership in which House of the Dunedain?");
    return 0;
    }
    ...
    tp->add_prop(DUNEDAIN_S_APPLIED_TO, house);
...
do_adopt(string str)
{
    ...
    object  newbie,
            tp = TP;
    string  house = tp->query_dunadan_house(),
            title;
    if (!strlen(house) || (member_array(house, DUNEDAIN_HOUSES) < 0))
    {
    NFN("You are not a member of a House of the Dunedain!");
        return 0;
    }
    ...
    if (newbie->query_prop(DUNEDAIN_S_APPLIED_TO) != house)
    {
    ...
    newbie->add_prop(DUNEDAIN_M_OATH, ({ tp, house, title, str, }) );
    newbie->add_prop(DUNEDAIN_I_LINE, 1);
...
{
    object  tp  = TP;
    int     i   = tp->query_prop(DUNEDAIN_I_LINE);
    mixed   arr = TP->query_prop(DUNEDAIN_M_OATH);
    string  lname,
            line,
            vb  = query_verb();
    ...
    {
        tp->remove_prop(DUNEDAIN_I_LINE);
        tp->remove_prop(DUNEDAIN_M_OATH);
 ...
 * Arguments:     arr  - the array in DUNEDAIN_M_OATH
 *                nl   - the line counter
 *                tp   - the applicant
 */
private void
continue_repeat(mixed arr, int nl, object tp)
{
    ...
    tp->add_prop(DUNEDAIN_I_LINE, nl);
    return;
}

/*
 * Function name: do_join
 * Description:   a dunadan is adopted into a House by the Lord/Lady
...
do_join(string house, object tp, object newbie)
{
    ...
    say(QCTNAME(tp)+" adopts "+QCTNAME(newbie)+" and receives "
      + newbie->query_objective() + " into the House of "+house+"!\n",
      ({ tp, newbie, }));
    newbie->remove_prop(DUNEDAIN_S_APPLIED_TO);

    if (newbie->query_guild_name_race() == GUILD_NAME)
    {
        if (!(result = add_unhoused_dunadan(LOW(nname), house, lname)))
        {
    ...
    if(!(result = add_dunadan(LOW(nname), house, lname)))
    {
 ...
 */
public varargs int
add_unhoused_dunadan(string name, string house, string lordname = 0)
{
    int result;
    object newbie;
    string gname,
    ...
    {
        newbie->remove_dunadan_from_house(newbie, DUNEDAIN_NO_HOUSE, "joined " +
            "the House of " + CAP(house));
        result = 1;
    }
    add_to_tree(name, house);
    newbie->set_dunadan_house(house);
    newbie->save_me(1);
 ...
 */
public varargs int
add_dunadan(string name, string house, string lordname = 0)
{
    int     result;
    object  newbie,
            shadow;
    ...
    else if (strlen(oldhouse = newbie->query_dunadan_house()))
    {
        /*
         *    If the player is a member of another House, 
         *  remove him/her now:
         */
        newbie->remove_dunadan_from_house(newbie, oldhouse, "applied " +
        "to the House of "+CAP(house)+" and was released by " +
        newbie->query_prop(DUNEDAIN_S_RELEASED_BY) + " of");
        newbie->remove_prop(DUNEDAIN_S_RELEASED_BY);
            result = 1;
    }
    add_to_tree(name, house);
    }
    ...
    shadow = clone_object(DUNEDAIN_SHADOW);
    if ((result = shadow->shadow_me(newbie, GUILD_TYPE,
                    GUILD_STYLE, GUILD_NAME)) != 1)
    {
        write_file(DUNEDAIN_LOG, "ERROR: " + CAP(name) +
        ...
        newbie->add_soul();
        newbie->add_subloc(DUNEDAIN_S_SUBLOC, shadow);
    newbie->clear_guild_stat(SS_RACE);
    newbie->set_skill(SS_DUNEDAIN_LEVEL, 0);
    }

    newbie->set_dunadan_house(house);
    newbie->catch_tell("From now on you will start your journeys in " +
               "the Houses of the Dunedain.\n"); 
    newbie->set_default_start_location(DUNEDAIN_START_LOC);
    newbie->save_me(1);

    if (strlen(lordname))
        write_file(DUNEDAIN_LOG, "DUNEDAIN: " + CAP(name)
          + " was adopted into the House of "+CAP(house)
          + " by "+CAP(lordname) + ". " + ctime(time()) + ".\n");
    else
        write_file(DUNEDAIN_LOG, "DUNEDAIN: " + CAP(name)
          + " entered the House of "+CAP(house)
      + " vouched for by " + newbie->query_prop(DUNEDAIN_S_VOUCHED_FOR)
      + ". " + ctime(time()) + ".\n");
 ...
 * Description:   remove a dunadan from his or her house, but keep
...
remove_dunadan_from_house(object pl, string house, string reason)
{
    ...
    // If a Lord/Lady releases themselves, then the accounting
    // for the Lord of a Dunedain House becomes very messy.
    // We need to make sure to resign first from being Lord,
    // and then continue with the release.
    if (strlen(house = query_is_lord(name)))
    {
        pl->set_dunadan_level(DL_SUB_LORD);
        remove_lord_from_tree(name, house);
    }
    
    if (!strlen(house))
        house = pl->query_dunadan_house();
    
    result = remove_from_tree(name, house, 1);
    write_file(DUNEDAIN_LOG, "DUNEDAIN: "+name+" "+reason +
           " the House of "+house+" on "+ctime(time()) +
           ". Result = "+result+".\n");
    pl->set_dunadan_house(DUNEDAIN_NO_HOUSE);
...
remove_shadow(object pl)
{
    ...
    {
        pl->clear_guild_stat(SS_RACE);
        pl->remove_skill(SS_DUNEDAIN_LEVEL);
        pl->setup_skill_decay();
        if (pl->query_default_start_location() == DUNEDAIN_START_LOC)
            pl->set_default_start_location(pl->query_def_start());
        pl->remove_cmdsoul(DUNEDAIN_SOUL);
        pl->update_hooks();
    }
 ...
 * Description:   remove a dunadan from the guild
 * Arguments:     pl     - object pointer to player
 *                house  - name of the house
 *                reason - why do we remove the player?
 * Returns:       1 / 0  - success/failure
...
remove_dunadan(object pl, string house, string reason)
{
    ...
    {
        write_file(DUNEDAIN_LOG, "ERROR: "+name+" failed to leave on "
 ...
 * Description:   add the name of a new dunadan to the Houses mapping
 *                or to the No_House array
 * Arguments:     name - name of the dunadan
...
add_to_tree(string name, string house)
{
     ...
     *    dunedain without a House:
     */
    if (!stringp(house) || (house == DUNEDAIN_NO_HOUSE))
    {
    ...
    {
        write_file(DUNEDAIN_LOG, "ERROR: "+name+" added to the " +
              "list of Dunedain without a House, but is " +
    ...
    if (member_array(name, members) >= 0)
        write_file(DUNEDAIN_LOG, "ERROR: " + name + 
    ...
    /*
     *    If the player was listed as dunadan without a House,
     *    remove that entry:
     */
    No_House -= ({ name, 0 });
...
add_lord_to_tree(string name, int gender, string house)
{
    ...
    {
        members += ({ lordname });
        write_file(DUNEDAIN_LOG, "LORDS: "+lordname
    ...
    write_file(DUNEDAIN_LOG, "LORDS: "+name+" became "
 ...
 *                          list of dunedain without a House?
...
remove_from_tree(string name, string house, int nohouse = 1)
{
    ...
    if (!strlen(house))
        house = query_dunadan_house(name);
    ...
    if (member_array(name, No_House) < 0)
    {
        write_file(DUNEDAIN_LOG, "ERROR: tried to remove " + 
     ...
     *    added to the list of dunedain without a House:
     */
    if (nohouse && result)
    {
    No_House += ({ name });
...
remove_lord_from_tree(string name, string house)
{
    ...
    write_file(DUNEDAIN_LOG, "LORDS: "+name+" resigned as "
 ...
 */
/*
Removed by Palmer per Tigerlily's orders May 14, 2006
public void
remove_lord_through_death(string lname)
{
    ...
    Houses[house][DT_S_LNAME]    =  0;
    Houses[house][DT_I_LGENDER]  = -1;
    write_file(DUNEDAIN_LOG, "LORDS: "+lname+" died and was removed "
...
/*
 * Function name: query_number_dunedain
 ...
 * Returns:       the number of dunedain in the guild/a House
 */
public varargs int
query_number_dunedain(string house, int silent = 1)
{
    int     ndu,
           *ndh,
            i;
    ...
    ndh = map(m_indexes(Houses), &query_number_dunedain( , silent));
 ...
 * Description:      how many Lords and Ladies of the Dunedain are there?
 * Returns:       the number
 */
public int
query_number_lords()
 ...
 *                - there are on average less than 3 dunedain per
 *                  open House.
...
query_open_house(string house)
{
    ...
    ndu = query_number_dunedain();
    // no new house if not enough dunedain
    if ((nho * 3) > ndu)
        return 0;
...
query_house_lord(string house)
{
    ...
    if (!strlen(house))
    {
        for (i = 0; i < sizeof(DUNEDAIN_HOUSES); i++)
            if (stringp(Houses[DUNEDAIN_HOUSES[i]][DT_S_LNAME]))
                lords += ({ Houses[DUNEDAIN_HOUSES[i]][DT_S_LNAME] });
        return lords;
    }
 ...
 * Description:   is <lname> a Lord or a Lady of the Dunedain?
...
query_is_lord(string lname)
{
    ...
    for (i = 0; i < sizeof(DUNEDAIN_HOUSES); i++)
        if (Houses[DUNEDAIN_HOUSES[i]][DT_S_LNAME] == lname)
            return DUNEDAIN_HOUSES[i];
...
query_dunadan_house(string name)
{
    int     i;
    ...
    for (i = 0; i < sizeof(DUNEDAIN_HOUSES); i++)
    {
        ...
        if (!sizeof(Houses[DUNEDAIN_HOUSES[i]]))
            return DUNEDAIN_HOUSES[i];
        if ((member_array(name, Houses[DUNEDAIN_HOUSES[i]][DT_AS_MEMBERS]) >= 0) ||
            (Houses[DUNEDAIN_HOUSES[i]][DT_S_LNAME] == name))
            return DUNEDAIN_HOUSES[i];
    }
    if (member_array(name, No_House) >= 0)
    return DUNEDAIN_NO_HOUSE;
    return 0;
}
...
promote_to_lord(string name)
{
    ...
    if (cand->query_guild_name_race() != GUILD_NAME)
        return "You are not a member of a House of the Dunedain, "
          + CAP(name)+"!\n";

    if (cand->query_wiz_level())
        return "No wizard may become "
          + (cand->query_gender() ? "Lady" : "Lord")
          + " of a House of the Dunedain, "+CAP(name)+"!\n";

    house = cand->query_dunadan_house();
    if (stringp(lordname = Houses[house][DT_S_LNAME]))
    {
    ...
    if (cand->query_dunadan_level() < DL_SUB_LORD)
        return "You are not yet worthy to become "
          + (cand->query_gender() ? "Lady" : "Lord")
          + " of a House of the Dunedain, "+CAP(name)+"!\n";

    cand->set_dunadan_lord_level();
...
do_renounce(string str)
{
    ...
    my_house = tp->query_dunadan_house();

    if (!strlen(my_house))
    {
        str = lower_case(str);

        if (!parse_command(str, ({ }), "[the] 'dunedain'"))
        {
            notify_fail("Renounce the Dunedain?\n");
            return 0;
        }
        if (result = remove_dunadan(tp, DUNEDAIN_NO_HOUSE, "renounced"))
        {
            write("You renounce your membership in the Dunedain!\n");
            say(QCTNAME(tp)+" renounces " + tp->query_possessive() +
                " membership in the Dunedain.\n");
        }
 
        notify_fail("For some unknown reason you are unable to " +
            "renounce the Dunedain.\nPlease make a bug report!\n");
        return result;
    }
    ...
    {
        NFN("Renounce which House of the Dunedain?");
    return 0;
    }
    ...
    if (tp->query_dunadan_level() == DL_LORD)
    {
    ...
    if (result = remove_dunadan(tp, house, "renounced"))
    {
 ...
 *Description:   resign the office of Lord or Lady of the Dunedain
 *Argument:      str - command line argument
 *Returns:       1/0 - success/failure
 */
public int
do_resign(string str)
{
    ...
    if ((title != my_title) || (tp->query_dunadan_level() < DL_LORD))
    {
        NFN("You are not the "+title+" of your House!");
        return 0;
    }

    if (member_array(my_house, DUNEDAIN_HOUSES) < 0)
    {
        NFN("There is no House of the Dunedain called "+house+"!");
    return 0;
    }

    if ((house = tp->query_dunadan_house()) != my_house)
    {
        NFN("You are not the "+title+" of the House of "+my_house+"!");
        return 0;
    }

    tp->set_dunadan_level(DL_SUB_LORD);
 ...
 *                the dunadan will remain in the guild. Any lord or lady
 *                may expel an unhoused dunedan.
 * Argument:      str - command line argument
 * Returns:       1/0 - success/failure
 */
public int
do_expel(string str)
{
    ...
    {
        NFN(CAP(str)+" is not a member of a House of the Dunedain!");
        return 0;
    }
    ...
    /* Only allow the lord/lady to expel members of their own house
       or unhoused members. */
    house = tp->query_dunadan_house();
    if (victim->query_dunadan_house() != house && 
      strlen(victim->query_dunadan_house()))
    {
        NFN(CAP(str)+" is not a member of your House!");
        return 0;
    }

    if (remove_dunadan(victim, house, "expelled by "+tp->query_name()+" from"))
    {
...
keep_player(object pl)
{
    ...
    {
        pl->catch_msg("Only humans can belong to the Houses of the Dunedain!\n");
        return 0;
    }
    return 1;
}
...
wiz_add(string str)
{
    ...
    if (!str
        || sscanf(LOW(str), "%s to %s", name, house) != 2)
    {
    NFN("Add whom to which house?");
    return 0;
    }

    if (result = add_dunadan(name, CAP(house)))
...
lord_list(string str)
{
    ...
    if (!(s = sizeof(lost)))
    {
    write("There are no members of the Houses of the Dunedain " +
          "that are lost.\n");
    return 1;
    }

    msg = "\n" +
    "\tThese members of the Houses of the Dunedain are lost:\n";

    for (i = 0; i < s; i++)
    {
    msg += "\t" + CAP(lost[i]) + " of the House of " +
        query_dunadan_house(lost[i]);
    if (i == (s-1))
        msg += ".\n";
    else
        msg += ",\n";
...
lord_remove(string name)
{
    ...
    write_file(DUNEDAIN_LOG, "LORDS: " + TP->query_name() +
           " removes " + name + 
...
init()
{
    ...
    add_action(wiz_add,     "dadd");
    add_action(lord_list,   "dlist");
    add_action(lord_remove, "dremove");
}

/*
 * Function name: oath_newbie
 * Description:   the oath sworn by a dunadan being adopted into a House
 * Arguments:     house - the name of the House
 *                title - "Lord"/"Lady"
 ...
 */
public varargs string
query_dunedain_house(string name)
{
    return query_dunadan_house(name);
}
...
fix_it(string name)
{
    ...
    if (!TP->query_wiz_level())
    return 0;
    pl->set_dunadan_house(query_dunadan_house(CAP(LOW(name))));
    return 1;
}
...
string read_plaque()
{
    ...
    desc = CAP(LANG_WNUM(sizeof(DUNEDAIN_HOUSES))) + " tables line the west wall, each dedicated to a House of the Dunedain. The names of the Houses are:\n\n";
    desc += break_string(COMPOSITE_WORDS(DUNEDAIN_HOUSES), 60, "\t") + "\n\n";
    desc += "To learn more about each House, do 'look at table of <house>'.\n";
    return desc;
}