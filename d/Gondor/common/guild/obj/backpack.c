...
static string	fill_verb = "fillpack",
...
public int	item_filter(object ob);


/*
 * Function name:	create_backpack
 * Description	:	configure your backpack here
 */
public void
create_backpack()
{
 ...
 * Description	:	configure basic pack
 */
nomask void
...
public void	set_backpack_long(string s)	{ backpack_long = s; }
public string	query_backpack_long()		{ return backpack_long; }
public void	set_fill_verb(string s)		{ fill_verb = s; }
public void	set_empty_verb(string s)	{ empty_verb = s; }
 ...
 * Returns	:	int -- 0 if may be sold
 *		     or string -- msg if may not be sold
 */
public mixed
no_sell_vbfc()
{
...
{
    /* Add the prop only if the backpack is still in 
 ...
 * Description	:	filter out objects that should not be store
 * Arguments	:	object ob -- the object
 * Returns	:	1 if should be stored, 0 if not
 *
 * TEMPORARY: check OBJ_I_HAS_FIRE here until is added to the genesis pack
 */
public int
