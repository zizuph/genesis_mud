...
create_door() 
{
    ...
    set_other_room(RANGER_ITH_DIR + "erockchamber");
 ...
 * Description	:	mask parent so 1) only officers can close
 *			off the room and 2) "tie curtain shut" works
...
lock_door(string arg)
{
    ...
    if (!R_COUNCIL->is_officer(TP->query_name()) &&
	!TP->query_wiz_level())
    {
	NFN0("Only officers may prevent others from entering " +
	    "the recess.");
    }
