...
create_gondor()
{
    ...
    set_long(BSN(
	"A few light wooden tables rest against one wall of this " +
	"rough-roofed rock chamber, while others are set up on " +
	"trestles and laden with platters, " +
	"bowls and dishes of clay. A long low bench runs along " +
	"the wall behind the tables, and crates and barrels are " +
	"stored against the inner wall, with a pricelist hanging " +
	"above them. To the northwest, a dark passage winds away."));
	...
	"sturdy trestles and laden with crockery. " +
	"All appear lightweight and easily moved."));
    add_item( ({ "trestle", "trestle" }),
	BSN("The trestles are simple folding frameworks, used "+
	    "to support the table tops."));
    add_item( ({ "crockery", "platter", "platters", "bowl", "bowls",
	"dish", "dishes" }),
	BSN("The plain, sturdy dishes all are of clay. Most are " +
	    "rather large, more suitable for serving groups of " +
	    "people than for individual use."));
    add_item( ({ "long bench", "low bench", "long low bench",
	"wooden bench", "bench", }),
	look_bench);
    add_item( ({ "stack", "crate", "barrel", "crates", "barrels" }),
	BSN("Various crates and barrels are stacked along the inner " +
	...
	BSN("The crates and barrels are closed, but the pricelist " +
	    "probably gives a fair notion of their contents."));
    add_item("floor", BSN(
	"The rock floor is very clean. There's not a crumb " +
	"nor a drop to be seen."));
    add_item("walls", BSN(
	"The walls are grey rock, curving gently up to the "+
	"low roof. Except for the pricelist on the inner wall, "+
	"they are bare."));
    add_item( ({ "west wall", }), 
	"Several table tops rest against the west wall of the chamber.\n");
    add_item( ({ "north wall", "wall", }), BSN(
	"The wall is bare grey rock, curving gently up to the "+
	"low roof."));
    add_item( ({ "roof", "ceiling" }), BSN(
	"The roof, formed of the same rock as the walls and floor, " +
	"is rough of surface and uneven in height."));
    add_item( ({ "stone", "rock" }),
	"The rock is rough surfaced, and dark grey in colour.\n");
    add_item( ({ "south wall", "inner wall", }), BSN(
	"The inner wall of the chamber seems to be used for "+
	"storage; several crates and barrels are stacked against it."));
    add_item( ({ "one wall", "east wall", }), BSN(
	"A long bench runs along the wall, with "+
	"several wooden tables drawn up before it."));

    add_item( ({ "pricelist", "list", "prices", "menu" }), PRICELIST_TEXT);

    add_cmd_item( ({ "pricelist", "list", "prices", "menu" }),
		 "read", PRICELIST_TEXT);
    add_cmd_item( ({ "light wooden tables", "wooden tables",
	"table", "tables" }),
	({ "move", "push", "pull" }),
	BSN("You give one of the tables a gentle push, and it slides " +
	    "a few inches across the floor."));
    add_cmd_item( ({ "long bench", "low bench", "long low bench",
	"wooden bench", "bench", }),
	({ "move", "push", "pull" }),
	BSN("You give the bench a gentle push, but it seems to be "+
    ...
    add_exit((RANGER_ITH_DIR + "erockchamber"), "northwest", check_sitting);
 ...
 * Description	:	replace ranger, jar
 */
public void
reset_room()
{
	...
	Jar->add_prop(OBJ_M_NO_GET,
	    "The jar seems to be stuck to the ground!\n");
	Jar->move(TO);
    }
    Cook = clone_npc(Cook, (RANGER_ITH_DIR + "npc/ceodar"));
 ...
 * Description	:	mask parent to handle local error cases (no access, foes)
 * Arguments	:	string str -- whatever the player typed
 * Returns	:	1 on success (or access blocked etc)
 *			0 on failure (we don't understand the arg)
 */
public int
do_buy(string str) 
{
    ...
    punish = TP->query_skill(SS_PUNISHMENT);
    if ((punish == NOTRADE ||
	 punish == NOACCESS) &&
        objectp(Cook) && 
	ENV(Cook) == TO)
    {
	Cook->command("say Sorry, but the Council of Rangers informed me "
    ...
    {
	/* "cannot happen" -- no enemy should ever be here */
	tell_room(TO,
	...
	tell_room(TO,
	    "The rangers escort " + QTNAME(TP) + " from the room.\n", TP);
	TP->move_living("M", (ITH_DIR + "nforest/fpath/bushes5"));
	tell_room(ENV(TP),
...
buy_it(string item_name, int number, string pay_type, string get_type)
{
    ...
    case "dry whiskeys":
        ...
        if (Whiskey - number < 0)
        {
            ...
            {
                Cook->command("say I'm sorry to say I only have about ...hmm ..");
                Cook->command("say " + LANG_WNUM(Whiskey) + " glasses of whiskey left.");
            }
            Cook->command("shrug");
            return 1;
        }
...
public int	hook_query_max_items()	{ return MAX_ITEMS; }
