...
spell_target_caster_environment(object caster, string str)
{
	...
	caster->catch_tell("Forces stronger than the Valar "+
	  "prevent you from repopulating this area with "+
	  "herbs.\n");
	return ({ });
    }
    if (target->query_prop(ROOM_I_RANGER_REGROWTH) && time() <=
      target->query_prop(ROOM_I_RANGER_REGROWTH))
    {
	caster->catch_tell("You sense the blessing " +
	  "of Yavanna is already present here and shall have to "+
	  "wait some time before you can call upon it for this area again.\n");
	return ({ });
    }
    return ({ target });
}
