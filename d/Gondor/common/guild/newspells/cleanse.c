/* File         : /d/Gondor/common/guild/newspells/cleanse.c
...
resolve_spell(object caster, object *targets, int *resist, int success)
{
    ...
    if (targets[0] != caster)
    {
	caster->catch_tell("You clench the athelas herb within your hand and "+
	  "press it against "+
	  targets[0]->query_the_possessive_name(caster) +
	  " forehead, attempting to cleanse " + POSSESSIVE(targets[0]) +
	  " blood.\n");
	targets[0]->catch_tell(caster->query_The_name(targets[0]) +
	  " clenches a green leaf within "+POSSESSIVE(caster)+" hand and "+
	  "presses it on your forehead"+
	...
	tell_room(ENV(caster), QCTNAME(caster) + " clenches a green leaf  "+
	  ...
	  " it on "+QTPNAME(targets[0])+" forehead, while "+
	  "chanting softly in an ancient tongue.\n", ({caster, targets[0]}));
    }
    else
    {
	caster->catch_tell("You clench the athelas herb within your hand and "+
	  "press it upon your forehead, attempting to "+
	  "cleanse your own blood.\n");
	tell_room(ENV(caster), QCTNAME(caster) + " clenches a green leaf "+
	  "within "+POSSESSIVE(caster)+" hand and presses "+POSSESSIVE(caster)+
    ...
    {
	caster->catch_tell("You fear the poison was too strong "
	  + "for your attempt.\n");

	if (caster != targets[0])
	    targets[0]->catch_tell("You still feel the poison "+
	      "within your viens.\n");
    }
