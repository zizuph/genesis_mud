...
create_book()
{
    set_max_pages(8);
    set_name("book");
    add_name("_new_ranger_spellbook");
...
setup_spells(object player, int spell)
{
	...
	add_spell(RANGER_SPELL_DIR + "whistle");

    if (known_spells & SPELL_HEAL)
	add_spell(RANGER_SPELL_DIR + "heal");

    if (known_spells & SPELL_CLEANSE)
	add_spell(RANGER_SPELL_DIR + "cleanse");

    if (known_spells & SPELL_REST)
	add_spell(RANGER_SPELL_DIR + "rest");

    if (known_spells & SPELL_NIGHTVISION)
	add_spell(RANGER_SPELL_DIR + "nightvision");

    if (known_spells & SPELL_ELBERETH)
	add_spell(RANGER_SPELL_DIR + "elbereth");

    if (known_spells & SPELL_SPEEDGROW)
	add_spell(RANGER_SPELL_DIR + "speedgrow");
}
