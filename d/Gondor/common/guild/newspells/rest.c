/* File         : /d/Gondor/common/guild/newspells/rest.c
...
#define SP_REST_TIRED_REDUCTION     35
#define SP_REST_EXTRA_REDUCTION     15
...
resolve_spell(object caster, object *targets, int *resist, int success)
{
	...
	caster->catch_tell("You " + msg + " in " +
	  "an attempt to envigorate "+targets[0]->query_the_name(caster)+".\n");
	if (Redweed)
	{
	...
	{
	    targets[0]->catch_tell(caster->query_The_name(targets[0]) +
	      " places " + POSSESSIVE(caster) + " hand upon your weary shoulder "+
    ...
    if (rest_amt <= 0 || (targets[0]->query_fatigue() 
	>= targets[0]->query_max_fatigue()))
    {
