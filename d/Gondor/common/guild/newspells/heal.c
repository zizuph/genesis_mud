/* File         : /d/Gondor/common/guild/newspells/heal.c
...
resolve_spell(object caster, object *targets, int *resist, int success)
{
	...
	targets[0]->catch_tell(caster->query_The_name(targets[0]) +
	  " presses "+POSSESSIVE(caster)+" hand to your forehead and "+
	  "murmurs a soft chant in an ancient tongue.\n");
    }
    else
    {
    ...
    tell_room(ENV(caster), QCTNAME(caster) + " presses "+ POSSESSIVE(caster) +
      " hand on "+ (targets[0] == caster ? POSSESSIVE(caster) + " own" :
	QTPNAME(targets[0])) + " forehead, mumbling some words in an ancient "+
      "tongue.\n", ({ caster, targets[0] }));
	...
	caster->catch_tell("You feel that someone answers your prayer.\n");
