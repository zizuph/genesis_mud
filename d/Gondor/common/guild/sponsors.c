...
#define APPLICATION_LOG "/d/Gondor/log/ranger_application"
#define DATE       0
#define APPROVALS  1
#define OFFICER    2
#define RECRUIT    3
...
public int remove_denied_applicant(string str);
...
string *Mentors  = ({ }); 
/*
 * List of pupils graduated and ready to join a company
 *    "Pupil1", "Pupil2", ....
 */
string *Graduated = ({ });
...
/*
 * Function name: query_graduated
 * Description:   Check if a person is a graduated pupil ready to join a company
 * Arguments:     name - name of the pupil
 * Returns:       1 if name is graduated, -1 if not a pupil at all, else 0.
 */
public int
query_graduated(string name)
{
    if (!strlen(name))
        return -1;
    name = CAP(LOW(name));
    if (member_array(name, Graduated) >= 0)
        return 1;
    return 0;
}
...
add_applicant(string name, int date = time(), int test = 0)
{
    ...
    if (test)
        return 0;
    FIX_EUID;
    write_file(APPLICATION_LOG, name + " applied to join the Rangers. "
      + ctime(time()) + ".\n");
...
remove_pupil(string pupil) 
{
    ...
    if (sizeof(Graduated))
    {
        n = member_array(pupil, Graduated);
        if (n > -1)
            Graduated = exclude_array(Graduated, n, n);
    }
    FIX_EUID;
    save_object(SPONSOR_SAVE);
    seteuid(0);
...
/*
 * Function name: graduate_pupil
 * Description:   Move a pupil to graduate status, releasing her from her mentor.
 * Arguments:     name - the name of the pupil to be graduated.
 * Returns:       1 on successful change of status, 0 otherwise.
 */
public int
graduate_pupil(string name)
{
    FIX_EUID
    name = CAP(LOW(name));
    if (!sizeof(Graduated))
...
remove_graduated(string name)
{
    ...
    else if (member_array(name, Graduated) == -1)
        return -1;
    else
        Graduated -= ({ name });
    save_object(SPONSOR_SAVE);
...
list_applicants(string arg)
{
    ...
    if (m < 1)
        return "No" + (strlen(arg) ? " " + arg : "") + " applicants currently.\n";
    else
        return ltxt;
}
...
public string
list_graduated_pupils()
{
    int     s, i;
    string  lstr = "";

    if (!sizeof(Graduated))
        return "No pupils are graduated currently.\n";
...
public int
remove_denied_applicant(string str)
{
    if (!strlen(str))
        return -1;
    str = CAP(LOW(str));
...
change_applicant_status(string name, string what, mixed value)
{
    ...
    if (what == "approve")
    {
        if (value != "recruiter" && !strlen(Applicants[name][RECRUIT]))
        {
            return -2;
        }
        ...
        write_file(APPLICANT_LOG, name + " approved by " + CAP(value)
