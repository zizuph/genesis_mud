 ...
 * Elessar, May 1994.
 * 
 * Made some minor adjustments to the shadow.  -Igneous-
 * - Added a mask on check_seen to give rangers teamsneak May 31, 2000
 *   Gwyneth
 * - Added some runtime preventions, 10/27/02
 *   Gwyneth
 */
#pragma save_binary
#pragma strict_types
...
init_occ_shadow(string arg)
{
    ...
    FIX_EUID
#ifdef NEW_RANGER_SHADOW
    clone_object(SPELLBOOK)->move(pl, 1);
    clone_object(TOKEN[query_bin_bits(1, 16, 2, pl)])->move(pl, 1);
#endif
    (R_SPONSORS)->load_me();
 ...
 * Description   : Masks check seen to allow rangers to
...
inherit "/d/Gondor/guilds/rangers/transfer/transfer_shadow.c";
#endif
