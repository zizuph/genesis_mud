 ...
 * Olorin, 9-nov-1994
 * Made "list" command possible for mortals (was wizard-only) 
 * Stern, 9-jan-2001
 */
#pragma save_binary
#pragma strict_types
...
create_room()
{
    ...
    set_library_name("the Library of the Rangers of Gondor");
    ...
    set_long("You are in a large hall with a domed ceiling. Along "+
      ...
      "an aging man, probably the librarian. A doorway leads out to the corridor "+
      "east. Your glance stops at a poster by the open door, which reads: "+
    "No talking! Further down on the poster you see some instructions. "+
    "Another doorway leads south into a dusty chamber.\n");

    add_exit(RANGER_GON_DIR + "upcorr1",    "east",  lib_check,    0);
    add_exit(RANGER_GON_DIR + "lib_office", "south", office_block, 0);
    add_exit(RANGER_GON_DIR + "story_room", "west", 0, 1);
    ...
    set_list_cmd("list");
    set_book_type("book");
    set_save_path(RANGER_DIR + "gondor/books/ranger_library");
    set_write_dir(RANGER_DIR + "gondor/books");
    set_log_file("guild");
    set_librarian_room(RANGER_DIR + "gondor/lib_office");
    set_up_library();
...
do_list(string str)
{
//   lets have list command for mortals too, they requested it.
...
query_author_list()
{
    ...
    author_list = BSN("Currently these mortals which are not rangers " +
        "have access to the library of the Gondorian Rangers:") +
    BSN(author_list);
...
office_block()
{
    ...
    if (MASTER_TITLES[mr] == "Scholar" || MASTER_TITLES[mr] == "Historian")
    {
    ...
    say("The librarian stops " + QTNAME(TP) + " from entering the south "
