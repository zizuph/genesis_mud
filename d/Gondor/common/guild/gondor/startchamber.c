/*
 * /d/Gondor/common/guild/gondor/startchamber.c
 *
 * The start location for the Gondor company.
 */
#pragma save_binary
#pragma strict_types
...
check_ranger()
{
   ...
   if (!IS_RANGER(TP))
   {
        write("Some of the people resting here see you heading for " +
            "the armoury, so you stop, knowing you should not go there.\n");
        return 1;
   }
...
test_ranger(object player)
{
   ...
   if (!IS_RANGER(player))
   {
      player->catch_msg("You are not allowed to start here!\n"
                      + "Only Rangers can use this room.\n"
