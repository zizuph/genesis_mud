...
introduce(mixed who)
{
    ...
    command("hail " + who);
    command("say My name is "+query_name()+". I am the Master Ranger here.");
...
new_token()
{
    RANGER_GUILD->ask_for_token("get token");
    command("smile");
    return "";
}
...
inform_ithilien()
{
    ...
    command("spit");
    command("say If you are no ranger, do not go there without leave by the Steward of Gondor!");
...
is_captain_of_this_company(object pl)
{
    ...
    switch(company_no) 
    {
        ...
        break;
        case 2:
            return (COUNCIL->query_north() == TP->query_name());
        break;
        case 3:
...
can_check_applicants(object pl)
{
    if (pl->query_ranger_mentor())
        return 1;
    if (COUNCIL->is_officer(pl))
        return 1;
    if (query_is_recruiter(pl))
        return 1;
    if (SECURITY->query_wiz_rank(pl->query_real_name()))
    return 0;
}
...
deny_application(string str)
{
    ...
    {
        command("say That is not for you to do.");
        return 1;
    }
    if (!strlen(str))
    {
...
list_pupils(string str)
{
    if (!strlen(str))
    {
        ...
        if (can_check_applicants(TP))
        {
            command("say I can disclose all applicants, new applicants, "
              + "approved applicants, or denied applicants. I can also "
              + "tell you the status of individual applications if you "
              + "do 'status <name>'. And ...");
        }
        if (IS_RANGER(TP))
        {
            command("say I can disclose all pupils, pupils with sponsors, "
              + "pupils without sponsors, all officers, all "
              + "master rangers, all sponsors, sponsors with "
              + "pupils, or sponsors without pupils."); 
        }
        else
        {
            command("say I can disclose all officers and possible sponsors.");
        }
    ...
    case "applicants":
      ...
      write(SPONSORS->list_applicants());
      break;
    case "new applicants":
	...
	break;
    case "pupils with sponsors":
    case "sponsored pupils":
	if (!IS_RANGER(TP))
	{
	    command("say What are you trying to do? Spy on us?");
	    break;
	}
	write("These pupils have sponsors:\n");
	write(SPONSORS->list_pupils()+".\n");
	break;
    case "sponsors with pupils":
	if (!IS_RANGER(TP))
	{
	    command("say I will not give secret information like that to you.");
	    break;
	}
	write("This is the current rota of sponsors with their pupils:\n");
	write(SPONSORS->query_sp_list()+".\n");
	break;
    case "pupils without sponsors":
    case "unsponsored pupils":
    case "pupils without sponsor":
	if (!IS_RANGER(TP))
	{
	    command("say You won't get secret information about our guild from me!");
	    break;
	}
	write("These pupils are without instructors:\n");
	...
	write("These are the current rota of people who can be sponsors:\n");
	write(SPONSORS->list_mentors()+",\n");
	write("and Ranger Officers:\n");
	write(COMPOSITE_WORDS(COUNCIL->query_officers())+".\n");
	break;
    case "possible sponsors":
    case "sponsors without pupils":
        command("say These Rangers do not have a pupil currently:");
	command("say "+SPONSORS->list_free_sponsors()+".");
	break;
    default:
	command("say Hmm. I seem to have lost the rota you ask for.");
	if (IS_RANGER(TP))
	    command("say The rota I have are of: all pupils, sponsored " +
		    "pupils, unsponsored pupils, officers, master rangers, " +
		    "all sponsors, possible sponsors, and sponsors with pupils.");
	else
	    command("say The rota I have are of: officers and possible sponsors.");
	break;
    }
    return 1;
}
...
create_ranger()
{
    ...
    add_ask(({"for token","token", "for ring", "ring", "for amulet",
		  "amulet", "for brooch", "brooch", }),   new_token);
    ...
    add_ask( ({ "info", "information", "rota", "pupils", "all pupils",
	"ranger pupils", "pupils with sponsors", "sponsored pupils",
	"sponsors with pupils", "pupils without sponsors",
	"unsponsored pupils", "pupils without sponsor", "master rangers",
	"all masters", "all master rangers", "officers", "all sponsors",
	"possible sponsors", "sponsors without pupils" }), inform_disclose);
}
