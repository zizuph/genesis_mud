...
#define COMPANY         ({"Ranger pupils","Gondor","the North","Ithilien"})
#define TOKEN           ({RANGER_DIR+"rring",RANGER_DIR+"rring",\
                          RANGER_DIR+"rbrooch",RANGER_DIR+"ramulet"})   
#define TOKEN_NAME      ({"ring", "ring", "brooch", "amulet"})
...
create_gondor()
{
    ...
    set_long(BS("You are in a white walled hall with burning torches lining "+
        ...
        "Upon the altar lies a large book.\n"));
    ...
    add_item(({"walls","wall"}),BS("The walls are made of white polished stone "+
        "slabs, neatly fitted among eachother. This is "+
...
give_token(object pl, int how)
{
    ...
    else 
    {
        ...
        say(QCTNAME(Ranger)+" hands "+QTNAME(pl)+" "+
            LANG_ADDART(TOKEN_NAME[company]) + ".\n");
        if (objectp(token = present("ranger_guild_object", pl)))
            token->remove_object();
    }
...
int 
ask_for_token(string str)
{
    string  s1,
            s2;
    object  company_token;
    int     company, 
            i_ranger, 
            i_token;
    ...
    {
        i_ranger = query_ranger(TP->query_name());
        if (i_ranger) 
            company = query_bin_bits(1,16,2,TP);
        switch (s2)
        {
        case "ring":
        case "brooch":
        case "amulet":
            if (i_ranger && (s2 != TOKEN_NAME[company]))
            {
                i_token = 1;
                while (s2 != TOKEN_NAME[i_token]) 
                    i_token++;
                write("The master ranger says: You are no Ranger of "+
                    COMPANY[i_token]+", why don't you ask for " +
                    LANG_ADDART(TOKEN_NAME[company]) + "?\n");
                say(QCTNAME(TP)+" asks for a new "+s2+
                    ", but doesn't get one.\n");
                return 1;
            }
            break;
        case "token":
            if (!i_ranger)
            {
                write("The master ranger" +
                    " says: You will have to join our guild first!\n");
                say(QCTNAME(TP)+" asks for a new "+s2+
                    ", but doesn't get one.\n");
                return 1;
            }
            if (!objectp(company_token = present("ranger_guild_object",TP)))
            {
                write("The master ranger says: Lost your "+s2+", have you?\n"+
                    "Be more careful with the new one!\n");
                say(QCTNAME(TP)+" asks for a new "+s2+", and is given \n"+
                    "one by the master ranger.\n",TP);
                give_token(TP, 0);
                write_file(RANGER_LOG,"RANGERS: "+TP->query_name()+
                    " given a new "+TOKEN_NAME[company]+" on  "+ctime(time())+".\n");
            }
            else
            {
                if (company_token->query_name() != TOKEN_NAME[company])
                {
                    write("The master ranger says: Hmm, you shouldn't have " +
                        "that "+company_token->query_name()+". Well, I'll " +
                        "exchange it for "+LANG_ADDART(TOKEN_NAME[company])+"!\n");
                    company_token->remove_object();
                    say(QCTNAME(TP)+" asks for a new "+s2+", and is given \n"+
                        "one by the master ranger.\n",TP);
                    write_file(RANGER_LOG, "RANGERS: "+TP->query_name()+
                        " given a new "+TOKEN_NAME[company]+" on  "+ctime(time())+".\n");
                ...
                {
                    write("The master ranger says: But you already have " +
                        LANG_ADDART(company_token->query_name()) + "!\n");
                    say(QCTNAME(TP)+" asks for a new "+s2+", but already has " +
                        LANG_ADDART(company_token->query_name()) + ".\n");
                }
            }
            return 1;
...
int
ask_for_ring(string str)
{
    return ask_for_token(str);
}
...
vouch_for_pupil(string str)
{
    ...
    NFN("Amaron says: You are not authorized to vouch for new members.\n");
    if (!IS_RANGER(TP) || (!COUNCIL->is_officer(tp_name)
                           && (!SPONSOR->query_can_sponsor(tp_name)))
	|| !query_bin_bits(RANGER_GROUP, RANGER_GUILD_BIT, 2, TP))
	return 0;
    NFN("Vouch for whom?");
    if (!strlen(str)) 
	return 0;
    ...
    write("You kneel before Amaron, taking a solemn oath to be the faithful\n"+
	  "instructor and sponsor of " + CAP(name) + ", until an Officer\n"+
...
check_join_guild(mixed player = this_player())
{
	...
	return CAP(player->query_align_text()) +
	    ...
	    "You must try to behave better, and stop being tempted " +
	    "by evil ways!\n" +
...
join_rangers(string str)
{
	...
	return 0;
    NFN("The master ranger says: But your guide must be present when you join!");
    if (!objectp(present(LOW(sponsor), TO)))
	return 0;
...
remove_ranger(string name, int how = 0, string reason = "")
{
    ...
    if (pl->query_skill(SS_MASTER_TASK) > 0)
    {
        ...
        SPONSOR->remove_master(pl->query_name());
    }
...
check_members()
{
    ...
    old_company_north    = company_rangers[2];
    old_company_ithilien = company_rangers[3];
    ...
    add_north    = new_north    - old_company_north;
    ...
    if (sizeof(add_north))
    {
        log_msg += "North rangers in ranger list, but not in company list:\n" +
            implode(add_north,", ") + "\n";
        to_be_removed += add_north;
    }
    ...
    sub_north    = old_company_north    - new_north;
    ...
    if (sizeof(sub_north))
    {
        log_msg += "North rangers in company list, but not in ranger list:\n" +
            implode(sub_north,", ") + "\n";
        to_be_removed += sub_north;
    }
