 ...
 * 
 * Modified by Tigerlily--2003-09-26 to update ranger
 *   retreat to /d/Shire/bree/rooms/town/innfront
 */
...
create_dunadan()
{
    ...
    set_long(BSN("This man seems to prefer to stay apart from all the "
      ...
      + "is partly hidden within the shadows of the hood of his dark-green "
      + "cloak, but he seems alert to everything that is happening."));
...
do_thank(object player)
{
    ...
    coin->move(TO);
    command("give five gold coins to "+player->query_real_name());
...
return_to_inn()
{
    ...
    TO->move("/d/Shire/bree/rooms/town/innfront", 1);
    tell_room(ENV(TO), QCNAME(TO) + " strides in, looking around.\n");
    TO->command("emote sits in the shadows");
}
