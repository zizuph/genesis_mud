 ...
 * Description	:	respond to unexpected queries
 * Returns	:	null str (response given via command())
 */
public string
default_answer()
{
    command("say I'm very sorry, but I don't understand your question.");
    return "";
} /* default_answer */
...
ask_soap()
{
	...
	if (!tp->query_prop(RON_I_HAVE_BATHED))
	{
		...
		command("blush");
		tp->catch_msg(QCTNAME(TO) + " turns "+POSSESSIVE(TO)
...
ask_water()
{
	...
	tp->catch_msg(QCTNAME(TO) + " pours the hot water over you, and "
...
give_towel(object tp)
{
	...
	tp);
    FIX_EUID;
    towel = clone_object(RANGER_NORTH_DIR + "obj/towel");
    if (towel->move(tp))
    {
...
give_soap(object tp)
{
	...
	tp);
    FIX_EUID;
    soap = clone_object(RANGER_NORTH_DIR + "obj/soap");
    if (soap->move(tp))
    {
	tell_object(tp, "Oops! You dropped it.\n");
	tell_room(env,
