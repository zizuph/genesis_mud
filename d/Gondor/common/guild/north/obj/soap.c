...
create_object()
{
    ...
    set_long(BSN("The oblong bar of soap is fairly soft, and "+
	"sweetly scented."));
...
do_lather(string arg)
{
	...
	tp->catch_msg("You lather " + QTNAME(who[0])
	      + "'s back.\n");
	who[0]->catch_msg(QCTNAME(tp) + " lathers your back.\n");
	say(QCTNAME(tp)+" lathers " + QTNAME(who[0]) + "'s back.\n",
	    ({ who[0], tp }));
	return 1;
    }
