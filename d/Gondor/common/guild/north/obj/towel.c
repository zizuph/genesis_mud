...
long_desc()
{
    if (Wet)
    {
	return BSN("The plush towel is soft and comfortable, quite "
		 + "absorbent, and right now rather wet. It has been dyed "
		 + "a deep shade of royal blue.");
    }
    return BSN("The plush towel is soft and comfortable, and quite "
...
do_dry(string str)
{
    ...
    SAY(" dries "+OBJECTIVE(TP)+"self off with the plush blue towel.");
    Wet = 1;
    if (!Dry_alarm_id)
    {
...
do_snap(string str)
{
        ...
        (random(who[0]->query_stat(SS_DEX) + who[0]->query_skill(SS_ACROBAT))))
    {
        ...
        tp->catch_msg("You "+vb+" your wet towel at " + QTNAME(who[0])
          + ". The end of the towel "+vb+"s at " + POSSESSIVE(who[0])
          + " " + where + "!\n");
        who[0]->catch_msg("You scream as " + QTNAME(tp) + " "+vb+"s "
          + POSSESSIVE(tp) + " wet towel at your " + where + "!\n");
        say(QCTNAME(tp) + " "+vb+"s " + POSSESSIVE(tp) + " wet towel "
          + "at " + QTNAME(who[0]) + "'s " + where + "!\n", ({ tp, who[0] }));
        who[0]->command("$scream");
        who[0]->reduce_hit_point(random(5)+5);
        return 1;
    }

    who[0]->catch_msg("You hear a snapping sound and realize that "
      + QTNAME(tp) + " tried to "+vb+" you with " + POSSESSIVE(tp)
      + " wet towel!\n");
    tp->catch_msg("You try to "+vb+" your wet towel at " + QTNAME(who[0])
      + ", but miss!\n");
