...
string  i_ranger,g_ranger;
string  g_deputy,i_deputy;
string  g_srgt, i_srgt;
string *council = ({0,0,0,0,0,0});

string *to_vote;
mixed  *votes_cast;
mixed  *election;                 /*  int      company number: 1-Gondor, 2-Ithilien
...
static varargs void log_punishment(string msg, string why);
...
pigeon_message(string txt, string from, object to)
{
	...
	fromob->catch_msg("You hand "+QTNAME(to) 
	  + " the judgement on a piece of paper.\n");
    }
    else
    {
...
remove_punishment(string who, object cm, string why)
{
    ...
    if (cm)
	notefrom = cm->query_name();
    else
	notefrom = "The Council of Rangers";
    pigeon_message(notetxt, notefrom, pl);
    log_punishment(CAP(who) + " cleared for punishment of "
      + PTYPE[ptype] + ", by "+notefrom+".\n", why);
    return 1;
}
...
execute_block(string who, int ptype, object cm, string why)
{
    ...
    log_punishment(CAP(who) + " punished with " + PTYPE[ptype]
      + ", by " + notefrom + ".\n", why);
...
do_degrade(string who, object cm, string why)
{
	...
	notefrom = cm->query_name();
    pigeon_message(notetxt, notefrom, crim);
    log_punishment(CAP(who) + " degraded in rank, by " + notefrom + ".\n", why);
    clear_pending_punishment(who);
    return 1;
}
...
int do_reward(string who, object cm, string why)
{
	...
	notefrom = cm->query_name();
    pigeon_message(notetxt, notefrom, crim);
    log_punishment(CAP(who) + " rewarded with a rank, by " + notefrom + ".\n", why);
    clear_pending_punishment(who);
    return 1;
}
...
do_commend(string who, object cm, string why)
{
	...
	notefrom = cm->query_name();
    pigeon_message(notetxt, notefrom, crim);
    log_punishment(CAP(who) + " given a commendation, by " + notefrom + ".\n", why);
    clear_pending_punishment(who);
    return 1;
}

int 
pupilize_ranger(string who, object cm, string why)
{
    object  crim;
    int     i,
    lowered;
    ...
    if (!(crim = find_player(who)))
	return 0;
    if (sizeof(Pupilized_once) 
      && (member_array(who, Pupilized_once) != -1))
    {
	notetxt = crim->query_name() +
	"!\nFor the second time you are being severely degraded for your\n" +
	"wrongful actions! Therefore you are now expelled from the guild!\n";
	if (!cm) 
	    notefrom = "The Council of Rangers";
	else 
	    notefrom = cm->query_name();
	pigeon_message(notetxt, notefrom, crim);
	log_punishment(CAP(who) + " expelled from the Rangers, by "
	  + notefrom + ".\n", why);
	RANGER_GUILD->remove_ranger(who, 1, "pupilized twice");
	crim->set_default_start_location(crim->query_def_start());
    ...
    if (lowered) 
	notetxt = notetxt + "In the process you have become less skilled.\n";
    notetxt = notetxt + "\nThe Council of Rangers.\n";
    if (!cm)
	notefrom = "The Council of Rangers";
    else 
	notefrom = cm->query_name();
    pigeon_message(notetxt,notefrom,crim);
    log_punishment(CAP(who)+
      " degraded to Ranger pupil, by "+notefrom+".\n", why);
    if (!sizeof(Pupilized_once))
	Pupilized_once = ({ who });
    else
	Pupilized_once += ({ who });
...
int expel_ranger(string who, object cm, string why)
{
    ...
    {
	log_punishment("Failed to expel " + CAP(who) + " from the Rangers. "
    ...
    pigeon_message(notetxt, notefrom, crim);
    log_punishment(CAP(who)
      + " was expelled from the Rangers, by " + notefrom + ".\n", why);
    clear_pending_punishment(who);
    crim->set_default_start_location(crim->query_def_start());
    check_ranger_loc_ok(crim, EXPULSION);
    return 1;
...
int ban_ranger(string who, object cm, string why)
{
	...
	+ "The Council of Rangers.\n";
	log_punishment( CAP(who) + " was expelled and banned "
	...
	+ "\nThe Council of Rangers.\n";
	log_punishment( CAP(who) + " was banned from the "
	  + "Rangers, by " + notefrom + ".\n", why);
    }
    pigeon_message(notetxt,notefrom,crim);
    clear_pending_punishment(who);
...
unban_player(string who, object cm, string why)
{
    ...
    + "The Council of Rangers.\n";
    pigeon_message(notetxt, notefrom, pl);
    log_punishment( CAP(who) + " is no longer banned from the Rangers, "
...
execute_punishment(string who)
{
    ...
    object  pl;
    if (find_player("elessar"))
	find_player("elessar")->catch_msg("DEBUG: execute_punishment: who = "
	  + who + ". ptype " + punishments[who] + ".\n");
    init_council_object();
    who = lower_case(who);
    ptype = punishments[who];
    if (!(pl = find_player(who))) 
    {
	log_punishment(CAP(who) + " not found when executing punishment.\n");
	return 0;
    }
    pold = pl->query_skill(SS_PUNISHMENT);
    if (ptype == pold)
    {
	log_punishment(CAP(who) + " already punished type " + ptype
	  + ". No change.\n");
	return 0;
    }
    switch(ptype)
    ...
    default:
	log_punishment(CAP(who)
	  + " failed to be punished, in execute_punishment func. ptype = "
	  + ptype+".\n");
	break;
    }
}
...
varargs string read_vote(int icfv, int ilog = 0)
{
	...
	if(ilog)
		...
		lstr += (ic <= NCAP? "Captain" : "Lieutenant") 
		+ " of " + COMPANY[ic <= NCAP? ic : ic - NCAP] + ": ";
    ...
    rstr = "'Ayes': " + LANG_WNUM(aye)+ (aye?"":"ne") + " - 'Nays': " 
    + LANG_WNUM(nay)+(nay?"":"ne") + ". ";
    if(aye == nay && (aye || nay))
    ...
    {
	dstr = ctime(time());
	lstr += "\nVoting terminated by " + TP->query_name() + " at " + 
	dstr[0..9] + dstr[19..23] + ": " + rstr;
...
varargs int call_election(int icompany, string candidate = 0)
{
    ...
    election += ({ icompany, ({candidate, 0, }), 
      GUILD->query_company_members(icompany), time(), });
...
string add_candidate(string name)
{
    ...
    if(vsum)
    {
	NF("You cannot nominate a new candidate because some "
	  +"rangers already voted in this election!\n");
	return 0;
    }
...
int call_for_impeachment(object caller, string victim)
{
    ...
    NF("You can only call for an impeachment if there are two Captains in office!\n");
    if (!stringp(council[0]) || !stringp(council[1]))
	return 0;
...
static varargs void
log_punishment(string msg, string why = "")
{
    string	dstr = ctime(time());

    if (strlen(why) > 5)
    {
	msg += "  Reason was: "+why+".\n";
    }
    write_file(PUNISH_LOG, dstr[0..9]+dstr[19..23]+": "+msg);
}
