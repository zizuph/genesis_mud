/* 
 *Bleeding object, to work with the haradrim shamshir. short bleed time
 *Altrus, July 2005
*/
...
{
	write("The shallow wound closes and the bleeding stops.\n");
	say(QCTNAME(me)+ "'s shallow wound stops bleeding.\n",me);
	me->remove_subloc("_short_bleeding");
...
bleed_me(object me, int stage)
{
   ...
   switch (stage)
    {
    case 1:
    write("The shallow wound bleeds, leaving you feeling weak.\n");
    say(QCTNAME(me)+ "'s shallow wound bleeds slightly.\n",me);
    time = 10.5;
    break;
    case 2:
    ...
    break;
    case 4:
    write("A small amount of blood seeps from your shallow wound.\n");
    say("Blood seeps from " +QCTNAME(me)+"'s shallow wound.\n",me);
    time = 15.5;
    break;
    default:
    return;
...
{
    ::enter_env(to,from);
    to->add_subloc("_short_bleeding", this_object());
...
show_subloc(string subloc, object me, object for_obj)
{
    ...
    if (for_obj == me)
        return "You are bleeding from a shallow wound!\n";
    
    //What others see when they examine the welt-haver
    else
        return CAP(HE_SHE(me))+" is bleeding from a shallow wound!\n";
}
