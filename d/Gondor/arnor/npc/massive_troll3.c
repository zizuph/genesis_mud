...
special_attack(object victim)
{
    ...
    if (hurt >= victim->query_hp() / 2)
        hurt = (victim->query_hp())/rate + 20 ;
        ...
        if (sizeof(others))
        {
            ...
            for (i = 0; i < sizeof(others); i++)
            {
               // no crits with special and hit enemies only
                ...
                {
                    if (hurt >= others[i]->query_hp() / 2)
                        hurt = (others[i]->query_hp())/rate + 20 ;
    ...
    if (random(10) > (RAN))
    {
        ...
        if (present(next_victim, environment(this_object())))
        {
             ...
             command("grin " + next_victim->query_real_name());
        }
    }
    return 1;
}
