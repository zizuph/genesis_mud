/*
 *      
 */
#pragma strict_types

#include "arnor_defs.h"
inherit NEW_ARNOR_ROOM;

#include <macros.h>
#include <stdproperties.h>
#include "/d/Gondor/defs.h"
static object *Bunnies = allocate(3);

public void
create_arnor()
{
    string extra = " A clump of sumac bushes makes a nest for " +
        "small creatures.";
    set_short("A sea of grass.");
    set_long(&set_grass_room_long(extra));
    add_road_downs_tells();
    set_up_herbs(({ ONE_OF_LIST(HERB_MASTER->query_herbs(HERB_LOC1)),
                    HERB_DIR + "attanar",
                    HERB_DIR + "sumac"}),
                    ({ "ridge", "hills", "hill", "grass", }), 5);
    add_exit(ND_OFFROAD + "offw1_3", "east", &off_road(), 0, 1);
    set_grass_wandering_msg();
    add_grass_room_items();
    reset_room();
}


void
enter_inv(object ob, object from)
{
    ::enter_inv(ob, from);
    if (interactive(ob))
    {
        sky_add_items();
        start_room_tells();
    }   
}
public void
reset_room()
{
    clone_npcs(Bunnies, NPC_DIR + "wrabbit", -1.0);
    set_searched(0);
}

