...
create_gondor()
{
    set_short("A gentle slope in the North Downs");
    set_long("@@set_arnor_long@@");

    add_item(({ "", "" }), "");
    add_item("", "");

    add_exit(NORTH_DOWNS_DIR + "stone_circle", "north", 0, 3);
    add_exit(NORTH_DOWNS_DIR + "small_valley", "south", 0, 2);
...
set_arnor_long()
{
   ...
   desc = "On a long slow-climbing slope in the North Downs, going from "+
   "the valley floor up to the hill-top to the north. Rocks are scattered "+
   "on the grassy slope. ";
