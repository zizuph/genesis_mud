 ...
 * Base room for the path to Lorien
 *
 * Mona 2020 
 *
...
// link to my defs in lorien dir above this one
// define in my defs
#include "/d/Gondor/lorien/lorien_defs.h"

inherit LORIEN_STD_DIR + "lorien_room";
 ...
 */
void create_dimrill_room() 
{
}

nomask void create_lorien_room() 
{
...
* Description:          uses add_tell to set dimrill area room tells
*/
...
set_items_dimrilldale()
{
    ...
    add_item( ({"caradhras", "azanulbizar", "redhorn", "barazinbar"}),
        "This immense mountain rises up, giving almost malevolent feeling of "+
        "domination, as it's hulking mass reaches up to a snow-capped peak. "+
        "The falls of Dimrill Stair, make their cascade down its front.\n");
    add_item( ({"celebdil", "zirakzigil", "silvertine'"}),
        "This is the southern and western most of the three mountains.  It "+
        "is a little smaller than the other two, and it's peak rises up like "+
        "a silver spike into the sky.\n");
}
 ...
 * Description  :        Items added to rooms when you include the line
 *                       set_items_mereafar(); 
 *                       Use for rooms where Mirrormere can be seen from afar  
 */
void
set_items_mereafar()
{
    add_item( ({"mirrormere", "mere", "lake"}),
    "The mere is long and oval shaped, running like the head of a spear, "+
    "thrust through the narrow and deep northern glen.  It runs "+
    "between Celebhil and Fanuidhol, terminating in a point, "+
    "at the foot of Carahdras.\n");
}
/*
 * Function name:        set_items_w_road
 * Description  :        Items added to rooms when you include the line
 *                       set_items_W_road(); 
 *                       Use for all road rooms to the west of Mallorn Forest
 */
void
set_items_w_road()
{
...
set_items_silverlode()
{
    add_item( ({"river", "stream", "water", "waters", "silverlode", 
        ...
        "The river flows to  north of the path. Deep and quiet the water " +
...
set_herbs_sunnymere()
{
    set_up_herbs( ({EARTHSEA_HERB_DIR + "clover3", 
                    ...
                    //add blung to sides and lothore 7 pawnrose to DS
...
do_harvest(string str)
{
 ...
 //maybe later add here allowance to kill the tree               
            ...
            setuid();
            seteuid(getuid() );
            bark = clone_object(LL_OBJ_DIR + "birchbark");
/* add birchbark.c-- undecided as to have this a quest item with only uses such
* as have it sewn into items or to add it as an herb but with those 
*additional uses.
*/

            bark->move(this_player(), 1);
            write("You harvest some bark from the birch tree.\n");
//fail for strip or pull bark-- also more than one tree from the x number tree
