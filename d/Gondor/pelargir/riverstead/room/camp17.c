...
create_gondor()
{
    set_short("in the northeastern part of a settlement");
    set_long(&long_desc());
    ...
    add_exit(NCAMP_ROOM + "camp15.c", "west");
    add_exit(NCAMP_ROOM + "dock2.c", "east");
...
long_desc()
{
    return "Where once a great number of huts and cabins stood, "
        + "now there is only smoke and ruin."
        + sky_desc() 
        + " What is left of the settlement lies to the west, "
        + "and a dock can be seen to the east.\n";
}
