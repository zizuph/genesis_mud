 ...
 * Olorin, April 93
 * 
 * Tigerlily, May 2004: Modified for ranger recode
 *         Removed call to faramir
 * 
 */
#pragma strict_types
...
create_monster()
{
    ...
    set_adj("grey-eyed");
    set_adj("strong");
    set_stats(({ 70+random(10), 70+random(10), 150+random(10),
    ...
    add_chat("Beautiful Ithilien, oh why had you to fall under the power "+
        "of the Enemy!");
    add_chat("I have a task to do.");
    add_chat("Could you do us a favour?");
    add_chat("The Southrons come marching up the ancient roads to swell "+
        "the hosts of the Dark Tower.");
    set_cact_time(1);
    add_cact("brawl");
    ...
    add_ask(({"about meal","about rabbit","about stewed", "about warm",
        "about coney"}),VBFC_ME("inform_meal"));
    ...
    add_ask(({"faramir","captain"}),
        "say Faramir is our Captain! He leads now in all perilous ventures.",1);
...
#endif
    clone_object("/d/Gondor/ithilien/obj/rlsword")->move(TO, 1);
    clone_object("/d/Gondor/ithilien/obj/rcloak")->move(TO, 1);
    clone_object("/d/Gondor/ithilien/obj/rhood")->move(TO, 1);
    clone_object("/d/Gondor/ithilien/obj/rgauntlets")->move(TO, 1);
    command("wear amulet");
...
long_func()
{
  return BS("He is a pale-skinned man, dark of hair, with grey eyes, "+
    "and a proud and sad face. You realize that he must be one of the "+
...
send_pigeon()
{
  ...
  tell_room(ENV(TO),
    QCTNAME(TO) + " gives a message to " + TO->query_possessive() +
    " pigeon.\n");
  tell_room(ENV(TO),
    "The white carrier pigeon flaps its wings and flies up to the sky.\n");
  tell_room(ITH_DIR + "nforest/ridge2",
    BS("Suddenly, you notice a pigeon flying from the top of one of the " +
    "two giant cedars standing to the west. \n"));
...
*/
  ...
  player->query_real_name())
  {
    ...
    player->catch_msg(QCTNAME(TO) + " takes away your badge.\n");
    badge->move(TO);
    say(QCTNAME(TO) + " takes away a badge from " + QTNAME(player) + ".\n");
...
*/
  ...
  START_QUEST(TP, "STEWED RABBIT (RANGERS)");
  return "";
}
...
inform_fire()
{
	...
	command("say Someone left a fire unattended at the foot "+
	    "of this tree!");
    ...
    {
	command("sigh");
	command("say Why does it always take a ranger to make a "+
	   "simple campfire?");
...
inform_ithilien()
{
    ...
    command("say Only we rangers are still fighting against the Enemy "+
	"this side of the Anduin.");
    command("say But we are too few, and the whole land is overrun by "+
	"the orcs of Mordor.");
    command("spit");
    return "";
}
...
inform_orcs()
{
    ...
    command("say Orcs?");
    command("spit");
    command("say They and those cursed Southrons fill up the armies "+
	"of the Dark Lord.");
    ...
    command("say But still, if we only had more men, we could chase "+
	"them over the hills!");
    command("sigh");
    return "";
}
...
inform_road()
{
    if (!check_player(TP)) 
        return "";
    command("say From here we are watching the road from "+
	"the crossroads to the");
    command("say Morannon, the Dark Gate leading into Mordor.");
    command("say The cursed Southrons come now marching up "+
	"the ancient roads");
    command("say to swell the hosts of the Dark Tower.");
    command("say Yea, up the very roads that craft of Gondor made.");
    command("say And they go ever more heedlessly, we learn");
    command("say thinking that the power of their new master "+
	"is great enough,");
    command("say so that the mere shadow of His hills will protect them.");
    command("say But soon we will teach them another lesson.");
...
return_item(object ob, object from)
{
	...
	command("say Thanks a lot. We will use it to buy weapons "+
	    "for our battle.");
	return;
    }
    ob->move(from);
...
tell_fire(object player)
{
  ...
  command("say Anyway, it was very foolish, and endangered "+
      "us and our mission!");
}
...
quest_done(object player)
{
  ...
  if(meneldil = present("meneldil",ENV(TO)))
	...
	process_string(QTNAME(meneldil),1) +
...
eating2(object meal)
{
    ...
    command("say I can taste " +
	meal->query_prop("_rabbit_herb_added_to_stew") +
...
eating3(object meal)
{
  object meneldil;
  if(meneldil = present("meneldil",ENV(TO)))
    command("emote and " + process_string(QTNAME(meneldil),1) +
...
eat_no_reward(object *arr)
{
  ...
  command("emote starts eating the rabbit stew.");
  if(meneldil)
    tell_room(ENV(meneldil),process_string(QCTNAME(meneldil),1) +
...
init_attack()
{
    ...
    if (IS_GONDOR_WIZARD(tp) || IS_RANGER(tp) || query_attack())
        return;
...
{
  object fire;
  if(!(fire = present("firewood",ENV(TO))) &&
  !(fire = present("campfire",ENV(TO))))
  {
    remove_alarm(lf_aid);
...
ranger_test(object player)
{
    ...
    if(iranger)
        set_alarm(1.0, 0.0, &command("unmask me"));
    return iranger;
}
...
inform_recipe()
{
    ...
    command("emote inhales deeply the air of Ithilien, "+
	"filled with the frangance of herbs.");
    return "";
}
...
inform_herbs()
{
    ...
    command("say You can use just about any herbs "+
	"that go with rabbit and stews.");
