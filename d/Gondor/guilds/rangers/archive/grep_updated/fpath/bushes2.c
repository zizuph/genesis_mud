...
create_area_room()
{
    ...
    set_extraline("You are walking through a bushy thicket under old " +
        "trees in ever-green woods. To the northeast, a small river is " +
    ...
    set_up_herbs( ({ HERB_DIR + "oregano", HERB_DIR + "redweed", 
         HERB_DIR + "tyelka",  }), 
    ...
    add_item(({"gorge","river","stream"}), BSN(
        ...
        "overhung with ilex and dark box-woods. Not far from here, the " +
        "gorge turns to the north, running along at the foot of a large " +
        "dark rock rising in the midst of the forest."));
    ...
    add_exit(ITH_DIR + "nforest/fpath/bushes1", "southeast", 0, 3);
    add_exit(ITH_DIR + "nforest/fpath/bushes3", "south",     0, 3);
    add_exit(ITH_DIR + "nforest/fpath/bushes4", "west",      &go_west(), 3, 1);
}
...
//old ranger Ithilien designation
    ...
    if (player->query_prop(RANGER_ITHILIEN_FOUND) >= 4) 
        return 2;
    if ((track+aware)/2 >= MIN_SKILL) 
    {
        player->add_prop(RANGER_ITHILIEN_FOUND, 4);
        return 1;
    }
    return 0;
}
...
go_west()
{
    ...
    case 2: 
        ...
        break;
    case 1: 
        write("You discover a path leading west and follow it.\n");
        break;
    default:
...
exa_bushes()
{
    string desc = "Bushes are growing all around you, except to the " +
         "northeast where the river is rushing westward in its deep-cloven " +
         "bed. The bushes are green and fair, with a delicate fragrance.";
  
    switch (check_track(TP))
    {
    case 4: 
        desc += " Hidden by the bushes to the west is the path " +
            "to the Henneth Annun, the secret hideout of the " +
            "rangers of Ithilien."; 
        break;
    case 3: 
    case 2: 
        desc += " Behind the bushes to the west, you see a path."; 
        break;
    case 1: 
        desc += " Behind the bushes to the west, you discover " +
            "a well hidden path.";
        break;
    default:
        break;
