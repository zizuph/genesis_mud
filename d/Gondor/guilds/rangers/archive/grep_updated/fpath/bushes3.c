/*
 *      /d/Gondor/ithilien/nforest/fpath/bushes2.c
 *
...
create_area_room()
{
    ...
    set_extraline("You are walking through a bushy thicket under old " +
        "trees in ever-green woods. The bushes continue in all " +
        "directions, with some rowan trees scattered among them. " +
        "The land falls steeply down towards the west. Looking over " +
        "the top of the bushes there, you can see lowlands and " +
        "broad meads, and glinting far off the wide waters of the Anduin.");

    set_up_herbs( ({ HERB_DIR + "marjoram", HERB_DIR + "suranie", 
             HERB_DIR + "tuo", }), 
    ...
    add_exit(ITH_DIR + "nforest/fpath/fpath4",  "southeast", 0, 3);
    add_exit(ITH_DIR + "nforest/fpath/fpath5",  "south",     0, 3);
    add_exit(ITH_DIR + "nforest/fpath/fpath6",  "southwest", 0, 3);
    add_exit(ITH_DIR + "nforest/fpath/bushes2", "north", &go_north(), 3, 1);
}
...
//old ranger Ithilien designation
    ...
    if (player->query_prop(RANGER_ITHILIEN_FOUND) >= 3) 
        return 2;
    if ((track+aware)/2 >= MIN_SKILL) 
    {
        player->add_prop(RANGER_ITHILIEN_FOUND, 3);
        return 1;
    }
    return 0;
}
...
exa_ground()
{
    ...
    if (ic = check_track(TP))
    {
        desc = "Looking carefully, you can make out several tracks " +
            "among the dry leaves and twigs on the floor. There " +
            "appears to be a path running through the bushes north " +
            "here."; 
        if (ic == 4)
        ...
        desc = "There are mostly dry leaves and moss on the forest " +
            "floor, and a few tracks leading southwest.";
...
exa_bushes()
{
    string desc = "Bushes are growing all around you. They are " +
        "green and fair, with a delicate fragrance.";
  
    switch (check_track(TP))
    {
    case 4: 
        desc += " Hidden by the bushes to the north is the path " +
            "to the Henneth Annun, the secret hideout of the " +
            "rangers of Ithilien."; 
        break;
    case 3: 
    case 2: 
        desc += " Behind the bushes to the north, you see a path."; 
        break;
    case 1: 
        desc += " Behind the bushes to the north, you discover " +
            "a well hidden path.";
        break;
    default:
        break;
