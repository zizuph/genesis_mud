...
catch_tell(string str)
{
    ...
    if (strlen(str) && sscanf(str, "%s says: %s\n", who, what) == 2) 
    {
	what = lower_case(what);
	if (TP->query_prop(WIGHT_ASKED_HELP) == QUESTER_RETURNED) 
	{
	    words = explode(what," ");
	    if (sizeof(words) > 2) 
	    {
		set_alarm(3.0,0.0, &want_one_word(TP));
		return;
	    }
	    else if (sizeof(words) < 3) 
	    {
		if (what == lower_case(Kingsname))
		    set_alarm(3.0, 0.0, &correct_answer(TP));
		else
		    set_alarm(3.0, 0.0, &wrong_answer(TP));
		return;
	    }
	    else 
		set_alarm(3.0, 0.0, &want_one_word(TP));
	}
	else if (TP->query_prop(WIGHT_ASKED_HELP) == QUESTER_SPARED) 
	{
	    words = explode(what," ");
	    if (sizeof(words) > 1) 
	    {
