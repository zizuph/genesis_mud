...
#define ADMIN_COMMANDS ({ "approve", "promote", "demote", "reward", \
                          "punish", "unpunish", "expel", "banish", \
                          "unbanish", "outlaw", "notorious", "pardon" })
/*
...
has_access()
{
    ...
    /* Captains may perform any admin command */
...
passage(string str)
{
    ...
    if (!strlen(str))
    {
        ...
        {
            entrance = (RANGERS_ITH_DIR + "entrance.c");
        ...
        {
            entrance = (RANGERS_NOR_DIR + "stairs.c");
            entrance->teleledningsanka();
            list = entrance->query_all_allowed();
            if (!sizeof(list))
                write("Nobody currently is allowed passage to the " +
...
//        MANAGER->send_message(target, "Your punishment has been lifted!\n");
        return 1;
    }
}
