/*
 * /d/Gondor/guilds/rangers/soul/mark.c
 *
 ...
 *
 * Revision history:
 * 20 April 2001 Gwyneth: Moved to /d/Gondor/guilds/rangers/soul/mark.c
 */
...
#define MARK             (RANGERS_OBJ + "mark_obj")
...
is_exit(string exit)
{
    return (member_array(exit, 
        environment(this_player())->query_exit_cmds()) >= 0);
}
...
mark(string str)
{
    ...
    if ((environment(tp)) && environment(tp)->query_prop(ROOM_I_INSIDE))
    {
        notify_fail("You cannot leave a mark indoors.\n");
        return 0;
    }

    if (skill < MINIMUM_TRACKING)
    {
        notify_fail("Your tracking skill is not high enough to leave a mark. " +
    ...
    mark->move(environment(tp), 1);
    skill = tp->query_skill(SS_HIDE);
    mark->set_hide( ((poorly == 1) ? min(skill, DEFAULT_HIDE) : skill) );

    say(QCTNAME(tp) + " bends to the ground and arranges some small " +
        "rocks and twigs.\n");
    write("You leave " +
