 ...
 *  Borrowed and adapted by Tigerlily for
 *    a special library for recruiters to add
 *    books on apprentices--16 October 2004
 *
 *    Modification Log:
...
create_room()
{
    set_short("In the Library of the Ranger Recruiters");
    set_long("You are in a large hall with a domed ceiling. Along "+
        ...
        "probably the librarian. He looks remarkably like the librarian " +
        "in the general library, possibly they are brothers. " +
        "Your glance stops at a poster " +
        "by the open door, which reads: No talking! Further down on the " +
        "poster you see some instructions.\n");

     add_exit(RANGERS_HQ_DIR + "recruiter_office","down", 0, 1);
    ...
    set_lib_long("the Library of the Ranger Recruiters");
    set_lib_id("rec_lib_id");
    set_lib_path("/d/Gondor/guilds/rangers/library_rec");
...
hook_librarian_requirements()
{
    ...
    if (MANAGER->query_master_specialty(name) == "Scholar" ||
        ...
        MANAGER->query_master_specialty(name) == "Recruiter" ||
