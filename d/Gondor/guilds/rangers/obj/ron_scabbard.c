/*
 *  /d/Gondor/guilds/rangers/obj/ron_scabbard.c
 *
...
#define SCABBARD           "/d/Gondor/guilds/rangers/obj/ron_scabbard.c"
...
set_scabbard_data()
{
    ...
    set_name("scabbard");
    add_name("ronscabbard");
    set_adj(adj);
    add_adj("leather");
    add_adj("grey");
    set_short(short_description);
    set_pshort(adj + " grey leather scabbards");
...
string short_description()
{
    if (sizeof(all_inventory(TO)))
        return adj + " grey leather scabbard with a sword sheathed " +
            "inside";

    return adj + " grey leather scabbard";
}

string long_description()
{
    string long_desc = "This " + adj + " grey leather scabbard is " +
        "is decorated with a ornate depictions of leaves and vines " +
        "curling gracefully around the words 'Arthor na Forlonnas'. " +
        "It has clearly been designed to be worn on your hip, " +
        "allowing you to sheathe a sword inside it.";

    if (query_prop(OBJ_M_NO_SELL))
        long_desc += "At present, you do not want to sell the " +
        adj + " grey leather scabbard.\n";
    else
