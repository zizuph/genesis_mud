...
create_object()
{
    ...
    set_long("The oblong bar of soap is fairly soft, and sweetly scented.\n");
...
do_lather(string arg)
{
    ...
    if ((vb == "wash" || vb == "bathe") &&
      file_name(ENV(tp)) == RANGERS_NOR_DIR+"bathhall")
        return 0;

    if (!strlen(arg) || arg == "me" || arg == "myself" ||
      LOW(arg) == tp->query_real_name())
    {
        if (!tp->query_prop(BATHING))
        {
            NF("But you aren't in a tub!\n");
            return 0;
        }

        write("You lather yourself with soap.\n");
        say(QCTNAME(TP) + " lathers " + OBJECTIVE(tp) + "self with soap.\n");
        return 1;
    }
      ...
      sizeof(who = NORMAL_ACCESS(who, 0, 0)))
    {
        ...
        {
            NF(CAP(PRONOUN(who[0])) + " isn't bathing.\n");
            return 0;
        }

        write("You lather " + who[0]->query_the_possessive_name(tp) +
            " back.\n");
        who[0]->catch_tell(tp->query_The_name(who[0]) +
            " lathers your back.\n");
...
do_smell(string arg)
{
    ...
    if (!strlen(arg) || !parse_command(arg, ({ TO }), "%o", ob))
    {
        NF(CAP(QVB)+" what?\n");
        return 0;
    }
    ...
    say(QCTNAME(TP) + " " + QVB + "s a bar of soap.");
    return 1;
}
