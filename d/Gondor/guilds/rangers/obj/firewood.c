 ...
 *  Shanoga, November 2018: Adjust test_weight to prevent negative values
 */
inherit "/std/object";
...
int     gAlarmId = 0,
        ...
        gBurntDown,
        gIsStack,
        gNotBurning,    // If this is set, the wood won't burn.
        gWasLit,
        gCoolTicks,
        gCoolAlarm;
...
{
    if ((gIsStack && stack) || (!gIsStack && !stack))
        return 0;
...
{
    gBurnTime = i;
    gOrigBurnTime = gBurnTime;

    if (gAlarmId && sizeof(get_alarm(gAlarmId)))
        update_fire();
}
...
{
    mark_state();
    gBurnTime = max(gBurnTime - amount, 0); 
    update_state();
}
...
test_weight()
{
    return max((gBurnTime * 50 / 3), 10 + (gOrigBurnTime / 10));
}
...
get_torch(string str)
{
    ...
    if (gIsStack || !parse_command(str, ({ TO }),
    ...
    if (res = query_prop(FIREWOOD_M_GET_NO_TORCH))
    {
        if (stringp(res))
            ...
            NF("You cannot get a torch from the " + short() + ".\n");
    ...
    if (gBurnTime > 375) 
    {
        /* We declare this campfire big enough to have a real branch to get. */
        FIX_EUID
        ...
        if (get_log)
        {
            ...
            torch->set_long("This is a " + torch->short() +
                " that was chopped into timber for burning.\n");
        }
        else
        {
            ...
            torch->set_long("This dry branch looks like it has fallen from " +
                "a tree.");
        }
        ...
        else 
        {
            ...
            if (test_lit())
            {
                write("You get a burning " + torch->query_name() +
                    " from the " + query_name() + ".\n");
    ...
    if ( id("ashes") )
        ...
        write("There are no branches large enough; the " + query_name() +
            " consists of only small dry sticks, not suitable for a torch.\n");
    else
        write("There are no logs of a suitable size.\n");
    return 1;
}
...
short_func()
{
    ...
    if (!(ENV(TO)->query_prop(ROOM_I_IS)) || (gIsStack && !query_lit()))
    {
        if (sizeof(query_adjs()))
            shortstr += " stack of " + implode(query_adjs(), ", ") +
                        " firewood";
        else
...
{
    int hrs;
    string longstr, size = determine_size();
    ...
    if (gBurntDown) 
    {
        ...
        if (hrs)
            longstr += " By feeling the temperature of the ashes you guess " +
                "that this " + query_name() + " burned down about " +
                LANG_WNUM(hrs) + " hour" + (hrs > 1 ? "s" : "") + " ago.";
        else if (gCoolTicks > 45)
            longstr += " By feeling the temperature of the ashes, you " +
                "estimate that this " + query_name() + " burned down less " +
                "than an hour ago.";
        else if (gCoolTicks > 30)
            longstr += " You also find a few live coals left, glowing in " +
                "the ashes. The " + query_name() + " probably burned down " +
                "about half an hour ago.";
        else
            longstr += " The glowing pieces of live coals and firewood " +
                "left amongst the ashes makes you understand that this " +
                "fire burned out quite recently.";
        return (longstr + "\n");
    }
    ...
    {
        hrs = gCoolTicks / 60;
        longstr = "This " + size + " " + query_name() +
            " had burned a while when it was extinguished again. By " +
            "feeling the temperature of the ashes, you guess that ";
        if (hrs)
            longstr += ("the " + query_name() + " was extinguished about " +
                (hrs > 1 ? (LANG_WNUM(hrs) + " hours") : "an hour") + " ago.");
        else if (gCoolTicks > 45)
            longstr += "the " + query_name() + " was extinguished less " +
                "than an hour ago.";
        else if (gCoolTicks > 30)
            longstr += "the " + query_name() + " was extinguished about " +
                "half an hour ago. The few live coals you find glowing " +
                "amongst the ashes confirm this.";
        else
            longstr += "the " + query_name() + " was extinguished quite " +
                "recently, since you see glowing pieces of wood and live " +
                "coals in the ashes.";
    ...
    {
        hrs = gBurnTime / 60;
        longstr = "This " + size + ", burning " + query_name() +
            " gives both light and warmth out here in the open. You guess " +
            "that it will still burn for ";
        if (hrs > 1)
            longstr += "about another " + LANG_WNUM(hrs) + " hours before " +
                "it dies out.";
        else if (hrs)
            longstr += "about one more hour before it dies out.";
        else if (gBurnTime > 45)
            longstr += "less than an hour before it dies out.";
        else if (gBurnTime > 30)
    ...
    if (sizeof(query_adjs()))
        longstr = "This " + size + " stack of " +
            implode(query_adjs(), ", ") + " firewood";
    else
        longstr = "This " + size + " stack of firewood";

    if (gIsStack)
        return (longstr + " is comprised of numerous logs which have " +
            "been chopped into timber for burning.\n");
    else
...
query_lit()
{
    return (gAlarmId && sizeof(get_alarm(gAlarmId)));
}

void
blow_away_ashes()
{
    tell_room(ENV(TO), "The remaining ashes of the " + query_name() +
        " blow away with the wind.\n");
    remove_alarm(gCoolAlarm);
    remove_object();
}
...
light_fire(string str)
{
    ...
    if (gNotBurning)
    {
        NF("The ash-grey firewood does not catch fire.\n");
        return 0;
    }
    ...
    if (!(room->query_prop(ROOM_I_IS)))
    {
        if (ENV(TO) == TP)
            ...
            NF("You cannot light the firewood there!\n");
        return 0;
    }
    ...
    {
        NF("You cannot light the firewood while it is in water!\n");
        return 0;
    }

    if (gAlarmId && sizeof(get_alarm(gAlarmId)))
    {
        NF("But the " + query_name() + " is already burning!\n");
        return 0;
    }
    ...
    {
        write("You light the campfire, and feel the warmth of " +
    ...
    gAlarmId = set_alarm(itof(INTERVAL), itof(INTERVAL), light_tick);
...
light_tick()
{
    ...
    if (!gIsStack)
    {
        ...
        case 0:
            ...
            break;
        case 1:
            tell_room(room, "Flames dance across the burning firewood.\n");
            break;
        ...
        case 9:
            ...
            {
                if (rooms[i] != file_name(environment(TO)))
                {
    ...
    ti = (gBurnTime/INTERVAL) * INTERVAL;

    if (ti == 60)
        tell_room(room, "The " + query_name() + " slumps down a little as " +
            "it begins to burn down.\n");
    else if (ti == 45)
        tell_room(room, "The " + query_name() + " flickers and the heat " +
            "grows less intense.\n");
    else if (ti == 30)
        tell_room(room, "The " + query_name() + " looks like it will burn " +
            "out soon.\n");
    else if (ti == 15)
        tell_room(room, "The " + query_name() + " grows smaller; it's " +
            "about to go out.\n");
...
extinguish(string str)
{
    ...
    if (gBurntDown || gWasLit)
    {
        NF("But the " + query_name() + " is already extinguished!\n");
        return 0;
    }

    if (!(gAlarmId && sizeof(get_alarm(gAlarmId))))
    {
        NF("But the " + query_name() + " has not even been lit yet!\n");
        return 0;
    }

    for (i = 0, size = sizeof(gGuard); i < size; i++)
    {
        /* Guard must be present, not the extinguisher and of
           higher ranger rank than the extinguisher. */
        guard = present(gGuard[i], ENV(TO));
        if (objectp(guard) && TP != guard &&
          TP->query_ranger_rank() <= guard->query_ranger_rank())
        {
            write("You cannot extinguish the " + query_name() + ", as " +
                guard->query_the_name(TP) + " is guarding it.\n");
            guard->catch_tell(TP->query_The_name(guard) + " attempts " +
                "to extinguish the " + query_name() + " you are guarding, " +
                "but you stop " + OBJECTIVE(TP) + ".\n");
            say(QCTNAME(TP) + " attempts to extinguish the " + query_name() +
                ", but is stopped by " + QTNAME(guard) + ".\n", TP);
            return 1;
        }
    }
...
kick_fire(string str)
{
    ...
    for (i = 0, size = sizeof(gGuard); i < size; i++)
    {
        ...
        if (objectp(guard) && TP != guard &&
          TP->query_ranger_rank() <= guard->query_ranger_rank())
        {
            write("You cannot kick the " + query_name() + ", as " +
                guard->query_the_name(TP) + " is guarding it.\n");
            guard->catch_tell(TP->query_The_name(guard) + " attempts " +
                "to kick out the " + query_name() + " you are guarding, " +
                "but you stop " + OBJECTIVE(TP) + ".\n");
            say(QCTNAME(TP) + " attempts to kick the " + query_name() +
                ", but is stopped by " + QTNAME(guard) + ".\n", ({ guard, TP}));
            return 1;
        }
    }
    ...
    if (gAlarmId && sizeof(get_alarm(gAlarmId)))
    {
        if (gOrigBurnTime > 500 && gBurnTime > 200)
        {
            write("The " + query_name() + " is too large to just kick " +
                "it out. You will have to extinguish it the normal way.\n");
            return 1;
        }

        write("You kick the burning " + query_name() + ", spreading " +
            "sticks and branches around, soon to go out by themselves.\n");
        say(QCTNAME(TP)+" kicks out the " + query_name() + ", " +
            "extinguishing it effectively.\n", TP);
        tell_room(room, "Only a sad pile of ashes are left.\n");

        burn_it(gBurnTime); 

        gOrigBurnTime = gOrigBurnTime / 10;
        gBurntDown = 1;
        remove_alarm(gAlarmId);
        gAlarmId = 0;
        gWasLit = 1;
        ...
        room->update_light();
        remove_name(({"stack", "stack of firewood", "firewood",
            "campfire", "wood", }));
        ...
        if (!(gCoolAlarm && sizeof(get_alarm(gCoolAlarm))) &&
          !(room->query_prop(ROOM_I_INSIDE)))
            gCoolAlarm = set_alarm(itof(COOL_INTERVAL), itof(COOL_INTERVAL),
                cool_down);
    }
    else if (gBurntDown)
    {
        write("You kick the ashes of the " + query_name() + ", and spread " +
            "them to remove the traces.\n");
        say(QCTNAME(TP) + " kicks the ashes of the " + query_name() +
            ", spreading them to remove the traces.\n", TP);
        blow_away_ashes();
    }
    else if (!(gAlarmId && sizeof(get_alarm(gAlarmId))))
    {
        write("You kick at the readied " + query_name() + ", and spread " +
            "the firewood all around.\n");
        say(QCTNAME(TP) + " kicks at the readied " + query_name() + 
            ", spreading the firewood all around.\n");
        remove_object();
    }
...
go_out(int i)
{
    ...
    if (i)
    {
        tell_room(room, "The flames of the " + query_name() + " flicker " +
            "briefly. Then it goes out, leaving only a smoking pile " +
            "of ashes.\n");
        gBurntDown = 1;
    }
    else
    {
        tell_room(room, "The flames of the " + query_name() + " silently " +
...
cool_down()
{
    gCoolTicks += COOL_INTERVAL;

    if (gCoolTicks > 10 * gOrigBurnTime)
    {
        blow_away_ashes();
        return;
    }
...
do_search(object tp, string arg)
{
    if ( test_lit() )
    {
        ...
        tell_object(tp,
            "Ouch! You burn yourself while searching the campfire.\n");
    }
 ...
 * Description  : if there's a fire in the env, allow putting more wood on it.
 *                since OBJ_I_HAS_FIRE can be set for torches, also check for
 *                a "fire" item in the env. Since we have no idea whether
 *                the fire is in a fireplace or pit or campfire or ..., we
 *                can only do very limited parsing:
...
put_on_fire(string arg)
{
    ...
    if ( !strlen(arg) ||
      ...
      all_inventory(TP), "%o 'on' / 'in' [the] 'fire'", ob) || ob != TO )
        return 0;
    ...
    say(QCTNAME(TP) + " puts some wood on the fire.\n");
    remove_object();
    return 1;
}
...
public int
query_guarding(string name)
{
    name = lower_case(name);
    return (member_array(name, gGuard) != -1);
}

public void
set_guarded_by(string name)
{
    name = lower_case(name);

    if (member_array(name, gGuard) == -1)
        gGuard += ({ name });
}

public int
remove_guarded_by(string name)
{
    name = lower_case(name);
