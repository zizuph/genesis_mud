/*
 *  /d/Gondor/guilds/rangers/obj/roi_holster.c
 *
...
#define ADJ                ({"dark", "light", "mottled", "forest", \
                               "speckled", "spotted"})
#define BACK_O_SUBLOC       "back_o_subloc"
#define LIVE_AO_BACK_SUBLOC "_live_ao_back_subloc"
#define SCABBARD            "/d/Gondor/guilds/rangers/obj/roi_holster.c"
...
string short_description()
{
    if (sizeof(all_inventory(TO)))
        return adj + " green leather holster with a polearm stored " +
            "inside";

    return adj + " green leather holster";
}

string long_description()
{
    string long_desc = "This " + adj + " green leather holster " +
        "is decorated with a silvery image of the White Tree of " +
        "Gondor and Seven Stars painted upon it. It has clearly " +
        "been designed to be worn on your back, allowing you to " +
        "store a polearm inside it.";
    ...
    long_desc += "You can <store> a polearm if the holster is " +
        "empty, or <withdraw> a polearm if there is one stored inside.";
    return (long_desc + "\n");
}
...
static int
filter_polearm(object obj)
{
    return (obj->query_wt() == W_POLEARM);
}
...
prevent_enter(object obj)
{
    ...
    {
        write("Only polearms may be stored inside your " + 
            query_name() + ".\n");
        return 1;
    }
...
{
 int     result;
 object *polearm, wearer;

    wearer = this_player();
    
    if (!str)
    {
        polearm = wearer->query_weapon(-1);
    }
    else
    {
        polearm = filter(FIND_STR_IN_OBJECT(str, wearer), "filter_weapon",
            this_object());
    }
    polearm = filter(polearm, "filter_polearm", this_object());

    if (sizeof(polearm) != 1)
    {
        notify_fail("Store which polearm?\n");
        return 0;
    }

    if (sizeof( all_inventory( this_object() ) ))
    {
        notify_fail("The " + polearm[0]->short() + " cannot be stored " +
        "as " + LANG_ASHORT(all_inventory(this_object())[0]) + " is " +
        "already stored in your " + short() + ".\n");
        return 0;
    }

    if (polearm[0]->query_wielded() == wearer)
    {
        polearm[0]->unwield_me();
        if (polearm[0]->query_wielded() == wearer)
        {
    ...
    if (result = polearm[0]->move(this_object()))
    {
        switch (result)
        {
        case 1:
            notify_fail("The " + polearm[0]->short() + " is too heavy " +
            "for the " + short() + ". That is unfortunate!\n");
            break;
        case 8:
            notify_fail("The " + polearm[0]->short() + " is too large " +
            "for the " + short() + ". That is unfortunate!\n");
            break;
        default:
            notify_fail("Oddly enough, your " + polearm[0]->short() + 
    ...
    wearer->catch_msg("You store your " + polearm[0]->short() + ".\n");
    tell_room( environment(wearer), QCTNAME(wearer) + " stores " +
        wearer->query_possessive() + " " + polearm[0]->short() + ".\n", wearer);
...
do_draw(string str)
{
    ...
    if (!str)
    {
        polearm = all_inventory(this_object());
    }
    else
    {
        polearm = FIND_STR_IN_OBJECT(str, this_object());
    }
    if (!sizeof(polearm))
    {
        if (!str)
    ...
    wearer->catch_msg("You draw your " + polearm[0]->short() + "!\n");
    tell_room(here, QCTNAME(wearer) + " draws "+ wearer->query_possessive() + 
        " " + polearm[0]->short() + ".\n", wearer);
    if (polearm[0]->move(wearer))
    {
        polearm[0]->move(wearer, 1);
    }
    
    if (stringp(res = polearm[0]->command_wield()))
    {
        write(res);
    }
    return 1;
