/*
 * /d/Gondor/guilds/rangers/npc/herbalist.c
 *
 ...
 *
 * 24-Aug-2004, Raven: Added heap support in: sell(), do_sell_all(),
 *              add_herb(), herb_filter()
 * 15-Sep_2005, Finwe: Disabled force_dry()
 * 25-Sep-2006, Toby: Fixed a typo, he now claims he buys and sells herbs
 */
...
list(string arg)
{
    ...
    if (arg == "herbs" || arg == "all herbs")
    {
        ...
        TP->more(hlist);
        tell_room(ENV(TO), QCTNAME(TO) + " checks his stores, " +
...
buy(string arg)
{
    ...
    tell_room(ENV(TO), QCTNAME(TP) + " asks " + QTNAME(TO) + " for " +
    ...
    if (evil_guild_test(TP) || !pointerp(hdata = All_herbs[arg]) || !hdata[H_COUNT])
    {
    ...
    {
        Num_herbs[TP->query_real_name()] += max_n;
    }
    ...
    command("give " + tname + " to " + OB_NAME(TP));
    if (ENV(herb_ob) == TO)
    {
         command("frown");
         command("say I'll just put " +
             ((herb_ob->num_heap() == 1) ? "it" : "them") + " here, then.");
         command("drop " + tname);
    }
...
ask_herbs()
{
    command("say Herbs?");
    command("smile .");
    set_alarm(3.0, 0.0, &command("say Well, I know most of them!"));
    set_alarm(6.0, 0.0, &command("say You can buy some from me, and I buy herbs, too!"));
    set_alarm(9.0, 0.0, &command("say If you have an unknown herb, I can try to identify."));
    set_alarm(12.0, 0.0, &command("say it for you, if you give it to me."));
    set_alarm(15.0, 0.0, &command("say If you want to learn about herblore, ask me about it."));
    return "";
}
...
sk_hook_write_header(int lessons)
{
    ...
    write("  Skill:                Cost:      "+
          ...
          "--------------------------------------"+
          "--------------------------------------\n");
    tell_room(ENV(TO), QCTNAME(TO) + " recites a list of skills for " +
        QTNAME(TP) + ".\n", TP);
}
