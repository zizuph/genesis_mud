/*
 *    Amaron, the Master Ranger of Ranger Central Headquarters.
 *
 ...
 *      - Alto, 24-Feb-2002
 *      Added toggle message for not accepting new rangers
 *      - Tigerlily, 02-Jan-2004
 */
#include <composite.h>
#include <macros.h>
#include <ss_types.h>
...
#define MASTER_RANGER_FILE RANGERS_NPC + "amaron_save"
#define CO_NO    0
...
#define REP_PLACES ({ "the Ithilien hideout", \
                      "the enemy encampment in Ithilien", \
                      "the Pelennor fields", "the plains of Rohan", \
                      "Isengard", "Pelargir", })
...
create_master_ranger()
{
    ...
    set_title("the Recruitment Officer and Master Ranger of the Westlands");
    ...
    add_ask(({"to be quiet", "be quiet",}), &asked_quiet());
    add_ask(({"to chat", "stop being quiet", "chat", "to talk again",
        "to chat again", "to talk"}), &asked_end_quiet());
    add_ask(({"reports", "report", "about reports", "about report",}),
        &ask_about_reports());
    add_ask(({"about ceremony", "ceremony", }), &ask_ceremony());
    add_ask(({"about joining", "joining", "join", "join rangers",
        "joining the rangers" }), &ask_joining());
    add_ask(({"about test", "test", }), &ask_test());
}
...
ask_joining()
{
    ...
    {
        command("say The Rangers are not accepting new members " +
            "at this time by orders from above!");
        return "";
    }

    command("say You are interested in joining?");
    command("say If you indeed wish to serve the Free Peoples with all "
        + "you have to give, then you must begin by reading this book!");
    command("say After reading the book and preparing yourself, and after " +
        "you have been vouched for by a company ranger, " +
...
ask_test()
{
    ...
    command("say If you are certain that you wish to serve the Free "
        + "Peoples, you must pass a test. The rangers will not accept "
        + "just any sword off the street, you know!");
    command("say You must have a knowledge of our ways and the land "
        + "we defend.");
    command("say Once you are vouched for and deemed a worthy applicant, " +
        "you may <take test>.");
    return "";
}
...
asked_quiet()
{
    ...
    {
        command("say I only grant such requests to full rangers.");
        ask_about_reports();
    }
    return "";
}
...
ask_about_reports()
{
    ...
    if (sizeof(reps))
    {
        ...
        command("say I don't need any reports right now.");
...
setup_chats()
{
    ...
    if (!(Reported & ROHAN_P_R))
        add_chat("The plains of Rohan are not safe for travel, rumours from "
          + "the Rohirrim have it that orcs have been seen there in great "
          + "numbers.");
    if (!(Reported & ISENGARD_R))
        add_chat("I do fear that Saruman the White might be up to something, "
          + "but I have not heard any reports about Isengard in a while.");
    if (!(Reported & PELARGIR_R))
        add_chat("There has been no news from Pelargir for some time. Some "
          + "people fear the Black Fleet may be on the move.");
}
...
enchant_seed()
{
    ...
    command("say a word with him, and he said he needed help.");
