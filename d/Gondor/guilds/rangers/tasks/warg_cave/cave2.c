...
enter_inv(object ob, object from)
{
    ...
    if (CAN_SEE_IN_ROOM(ob) && 
        (ob->query_prop(RANGER_I_TASK) == WARRIORS_TASK) && 
        (sizeof(ob->query_team_others()) == 1))
    {
...
ranger_entrance()
{
    ...
    if ((tp->query_prop(RANGER_I_TASK) == WARRIORS_TASK) && 
        (sizeof(tp->query_team_others()) == 1) &&
        (tp->query_team_others()[0]->query_prop(RANGER_I_TASK) == WARRIORS_TASK))
    {
        return 0;
    }
