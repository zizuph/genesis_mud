/*
 *  /d/Gondor/guilds/rangers/hq/gully_north.c
 *
...
create_gondor()
{
    ...
    set_long("You are standing in a shallow gully deep inside the Druedain forest. "
        ...
        + "from north to south. Although the way south is clear, the northern "
    ...
    add_item(({"log", "logs"}), "Logs have been stacked across the small stream, "
        ...
        + "loose. Perhaps you could angle around them to the northeast.\n");

    add_exit(TRACKTASK_DIR  + "gully",   "south", &allowed(), 1);
    add_exit(TRACKTASK_DIR  + "gully_north_a",   "northeast", &allowed(), 1, 1);
