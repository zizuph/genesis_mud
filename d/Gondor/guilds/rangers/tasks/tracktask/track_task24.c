/*
 *  /d/Gondor/guilds/rangers/hq/track_task24.c
 *
...
create_gondor()
{
    ...
    set_long("The trees close about you in all directions but to the "
        + "southwest. Due to the thick bushes growing beneath the trees, it seems "
    ...
    add_item(({"bush", "bushes"}), "Bushes grow in thick clusters beneath "
        ...
        + "the path leading to the southwest. On closer inspection, however, you "
        + "notice a small gap beneath some of the bushes to the northwest.\n");
    add_item(({"gap", "gap in the bushes"}), "Beneath some of the bushes "
        + "is a small gap, probably used by woodland creatures to scurry "
        + "through to the northwest. Although it looks rather tiny, you could "
        + "probably get through.\n");

    add_exit(TRACKTASK_DIR + "track_task27", "northwest", &allowed(), 1, 1);
    add_exit(TRACKTASK_DIR + "track_task22", "southwest", &allowed(), 1);
