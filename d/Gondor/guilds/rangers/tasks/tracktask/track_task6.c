/*
 *  /d/Gondor/guilds/rangers/hq/track_task6.c
 *
...
create_gondor()
{
    ...
    set_long("The trees close about you in all directions but to the "
        + "south, where the path appears to eventually reach a bend. Due "
    ...
    add_item(({"bend", "corner", "path"}), "The path runs to the south, "
        + "but in the distance appears to bend to the west.\n");
    add_item(({"bush", "bushes"}), "Bushes grow in thick clusters beneath "
        + "the trees. They block your passage in all directions but to "
        + "the south.\n");

    add_exit(TRACKTASK_DIR  + "track_task4",   "south", &allowed(), 1);
