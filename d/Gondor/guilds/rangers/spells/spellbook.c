...
setup_spells(object player, int spell)
{
    ...
    /*  These spells are added via the soul now.
    if (known_spells & SPELL_WHISTLE)
        add_spell(RANGERS_SPELL + "virtue_whistle");

    if (known_spells & SPELL_HEAL)
        add_spell(RANGERS_SPELL + "virtue_heal");

    if (known_spells & SPELL_CLEANSE)
        add_spell(RANGERS_SPELL + "virtue_cleanse");

    if (known_spells & SPELL_REST)
        add_spell(RANGERS_SPELL + "virtue_rest");
