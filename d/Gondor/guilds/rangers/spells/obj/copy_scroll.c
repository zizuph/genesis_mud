/*
 * /d/Gondor/guilds/rangers/spells/obj/copy_scroll.c
 *
...
copy_scroll(string str)
{
    ...
    {
        NF("Copy what into what?\n");
        return 0;
    }
    if (scroll != this_object())
    {
        NF("Copy which scroll?\n");
        return 0;
    }
    ...
    // Apprentices can only copy heal, whistle and rest spells.
    if ((TP->query_ranger_level() <= APPROVED_APP_LEVEL) &&
      (Spell_Code >= SPELL_CLEANSE) && !TP->query_wiz_level())
    {
        write("This scroll is too advanced for you to learn.\n");
        return 1;
    }
    ...
    write("You carefully copy the spell onto some blank pages of your " +
        "spellbook.\nJust as you are finished, the scroll crumbles to " +
        "dust between your fingers!\n");
    say(QCTNAME(TP)+" copies a spell into "+TP->query_possessive() +
        " little brown book.\nSuddenly, the scroll " + TP->query_pronoun() +
        " was copying from crumbles to dust!\n", TP);
    remove_object();
    return 1;
}
