 ...
 */
void
desc_use_ingredients(object *ingrs)
{
    object what;

    if (sizeof(ingrs))
    {
        foreach (object what : ingrs)
        {
...
int has_spellcasting_virtue(object player)
{
    if (APPRENTICE_MEMBER(player))
    {
        return 1;
    }
    if (!NORTH_MEMBER(player) && !ITHILIEN_MEMBER(player))
    {
        return 0;
    }
    
    if (player->query_ranger_major_virtue() 
        == RANGER_MAJOR_VIRTUE_SENTINEL_OF_IMLADRIS)
    {
        return 1;
    }

    if (player->query_ranger_minor_virtue() 
