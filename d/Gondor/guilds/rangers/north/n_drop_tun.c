/*
 *  RANGERS_NOR_DIR + "n_drop_tun.c"
 *
 *  Temporary room for travelling to RoN droproom.
 *
...
public int         exit_tunnel(object who);
...
{
    set_alarm(2.0, 0.0, &actor->catch_tell("Hands in the dark force you "
        + "with great urgency into a cart hitched to several stamping and "
        + "huffing horses.\n"));
    set_alarm(5.0, 0.0, &actor->catch_tell("The horses leap forward!\n"));
    set_alarm(7.0, 0.0, &actor->catch_tell("The cart strikes a large bump "
        + "and you are nearly thrown off!\n"));
    set_alarm(10.0, 0.0, &actor->catch_tell("Wind howls along the tunnel "
        + "and flattens you against the back of the cart.\n"));
    set_alarm(15.0, 0.0, &actor->catch_tell("Reaching a sudden bend in the "
        + "tunnel, the horses turn quickly, nearly upsetting the cart!\n"));
    set_alarm(20.0, 0.0, &actor->catch_tell("A light appears far ahead in the "
        + "distance.\n"));
    set_alarm(22.0, 0.0, &actor->catch_tell("The horses swerve and you are "
        + "thrown from the cart! After bouncing further along the tunnel, "
        + "you eventually roll to a stop.\n"));
...
exit_tunnel(object actor)
{
    ...
    tell_room(environment(actor), QCTNAME(actor) + " is thrown from "
        + "the cart!\n", actor);
    tell_room(RANGERS_NOR_DIR + "r_corridor", QCTNAME(actor) + 
        " suddenly rolls into the room.\n", actor);
    actor->move_living("M", RANGERS_NOR_DIR + "r_corridor", 1, 0);
    return 1;
}
