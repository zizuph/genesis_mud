/*
 *  /d/Gondor/guilds/rangers/north/tunnel_e.c
 *
...
long_desc()
{
    ...
    return "You are in a small tunnel somewhere beneath the ruins of Fornost. "
        + "Set into the west wall is an iron torch stand" + txt + "There is "
        + "a fork in the tunnel here, with exits leading down a slope to the "
        + "north, up a slope to the northeast, or directly to the east.\n";
}
...
create_gondor()
{
    ...
    add_exit(RANGERS_NOR_DIR + "tunnel_d","north");
    add_exit(RANGERS_NOR_DIR + "tunnel_f","northeast");
    add_exit(RANGERS_NOR_DIR + "vantage_stair","east");
