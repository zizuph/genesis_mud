...
<rider> - Grants access to mortal version of our horses.
        ...
        Missing relegation command added - Nerull
        30th April 2019.
*/
...
private string print_votes(string which)
{
    ...
    else if ((which == "lord") || (which == "king"))
    {
        ...
        mapping votes = MORGUL_MASTER->query_lord_nazgul_votes();
        if (m_sizeof(votes) == 0)
        {
            return str + "\n\tThere have been no votes cast yet!\n\n";
        }
...
public int check_cmd(string args)
{
    ...
    {
        // which = "nazgul";
        notify_fail("Check votes for Nazgul or for Lord?\n");
        return 0;
    }
...
public int relegate_cmd(string str)
{
    if (!this_player()->query_nazgul()) return 0;   
    
    if (!str)
    {
        notify_fail("Relegate whom?\nThe player will be removed from the position "
        +"of Nazgul, but you must offer a valid name.\n");
        
        return 0;
    }
    
    if (!MORGUL_MASTER->query_is_nazgul(str))
    {
        notify_fail("That person is not a Nazgul!\n");
        return 0;
    }
    
    if (MORGUL_MASTER->query_lord_nazgul() != this_player()->query_real_name())
    {
        notify_fail("Only the Lord of the Nazgul may relegate a Nazgul!\n");
        return 0;
    }
   
    write("You remove "+capitalize(str)+" from the ranks of the Nazguls.\n");
    MORGUL_MASTER->remove_player_as_nazgul(str, "relegated");
    MORGUL_MASTER->set_as_archmage(str);
...
void init()
{
    ...
    // Promoting / relegating / resigning
    add_action(promote_cmd,     "promote");
    add_action(resign_cmd,      "resign");
    add_action(relegate_cmd,     "relegate");
