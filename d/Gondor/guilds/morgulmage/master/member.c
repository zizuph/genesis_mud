 ...
 *
 *  Adjusted a default start location for mages demoted to Tuvale
 *  Varian - 2016
 */
 ...
 */
void initialize_member(string name)
{
...
/*
 * Function   : query_is_tuvale
 * Description: Check to see if the mage is only a tuvale
 * Arguments  : mixed mage - the mage to check for
 */
public int query_is_tuvale(mixed mage) {
    string magename = get_mage_real_name(mage);
    initialize_member(magename);

    if (member_array("tuvale", m_indices(member_information[magename])) == -1) {
        return 0;
    }

    return member_information[magename]["tuvale"];
}

/*
 * Function   : set_tuvale
 * Description: Sets the tuvale status of a mage
 * Arguments:   mixed mage - the mage to check for
 *              int tuvale - 0 when not a tuvale, 1 when a tuvale
 */
public void set_tuvale(mixed mage, int tuvale) {
    string magename = get_mage_real_name(mage);
    initialize_member(magename);

    // Sets the tuvale start location
    mage->set_default_start_location("/d/Gondor/morgul/cellar/tuvale2");

    // Tells the tuvale of his new start location
    mage->catch_tell("You will now start the game in the cellar with " +
        "the other tuvales.\n");

    member_information[magename]["tuvale"] = tuvale;
}
 ...
 * Arguments  : mixed mage - the name or player to check
 */
public int
query_is_mage(mixed mage)
{
    string magename = get_mage_real_name(mage);
    return (member_array(magename, m_indices(member_information)) != -1);
}
...
/*
 * Function name: get_mage_rank
 * Description  : Get the last known rank information
 * Arguments    : mixed mage
 * Returns      : the last known information
 */
public mixed get_mage_rank(mixed mage) {
    string magename = get_mage_real_name(mage);
    ...
    return member_information[magename]["rank"];
}
   ...
   foreach(string mage : mages) 
   {
       if(!SECURITY->exist_player(mage)) {
           write_file(MORGUL_REGULAR_RECORDS, "MORGUL MAGES: "
               + capitalize(mage) + " is no longer in the realms "
               + "and was purged on " + ctime(time()) + ".\n"); 
           write_file(MORGUL_NAZGUL_RECORDS, capitalize(mage) 
               + " left the Morgul Mages. Reason: " 
               + "Player does not exist." + ctime(time()) + ".\n");
           remove_mage_from_list(mage); 
       }
   }
}

/*
 * Function   : query_member_information
 * Description: Utility function to retrieve all information known about
 *            : a mage.
 * Arguments  : mage - the mage to check for
 */
public mapping query_member_information(mixed mage) 
{
    string magename = get_mage_real_name(mage);
    if (member_array(magename, m_indices(member_information)) == -1) return 0;
    return member_information[magename];
}