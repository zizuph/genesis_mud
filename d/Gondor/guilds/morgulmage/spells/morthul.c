...
#define BASE_COMBAT_AID     100.0
#define NO_NARO_CAID        20.0
#define NO_FEAR_CAID        40.0
...
{
    if (!query_ability_offensive(targets))
    {
        return verify_present(actor, targets, arg);
    }
    
    return ::verify_change_enemy(actor, targets, arg);
} /* verify_change_enemy */
...
query_spell_combat_aid_modifier()
{
    ...
    {
        available_combat_aid += NO_FEAR_CAID;
    }
    
    if (!caster_is_maintaining_naro_effects(this_player())) 
    {
        available_combat_aid += NO_NARO_CAID;
    }
    ...
    return (available_combat_aid / total_combat_aid) * 100.0;
}
