...
public object* naro_verify_target(object caster, object* targets, string str);
...
public nomask void config_spell()
{
    ...
    set_ability_target_verify(naro_verify_target);
...
public object* naro_verify_target(object caster, object* targets, string arg)
{
    object* modified_targets = spell_verify_present(caster, targets, arg);
    if (sizeof(targets) != sizeof(modified_targets))
    {
        // One of two naro maetho targets may have left the room.
        return ({ });
    }
...
public object *naro_target(object caster, string str)
{
    ...
    if (parse_command(str, ({ }), "%s 'libo' 'makil' / 'makil!'", target)) {
        ...
        object* result = target_one_other_present_living_or_unaffected_enemy(get_best_wielded_attack_id, caster, str);
        if (sizeof(result) && !get_best_wielded_attack_id(result[0]))
        {
            caster->catch_tell("The Dark Lord cannot disarm those that "
                + "are not armed!\n");
            return ({ });
        }
