...
void create_morgul_tower_room()
{
    ...
    add_item(({"stairs","staircase","winding staircase",
               "winding stairs"}),
        "The stairway to the north offers access from both above"
    ...
    add_item(({"shadow","shadows"}),
        "Shadows are strewn about this place as cobwebs in a"
      + " forgotten crypt. They offer comfort against the unsure light"
      + " of the outside world.");
    add_item(({"door","doorway"}),
        "The doorway to the north has been perfectly shaped by"
    ...
    add_item(({"sheet","metal","dull metal","black metal",
        ...
        "The sheet of metal is very thick, and has been firmly"
    ...
    add_item(({"clasp","clasps","huge clasps","huge metal clasps"}),
        "These claps have been cunningly formed from a dark metal"
      + " of great strength and weight. They secure the metal sheet"
      + " firmly to the wall.");
    add_item(({"steps","step"}),
        "There are countless steps on the stairway to the north,"
    ...
    add_exit(MORGUL_ROOM_DIR + "tower_3c", "north",  0);

    if (!present("board",TO))
    {
        board = clone_object(MORGUL_OBJ_DIR + "mage_board");
        board->move(TO);
    }
}
