...
create_morgul_tower_room()
{
    ...
    set_extra_long("The chamber is windowless and empty, save for "
      + "a equipment rack standing at the north wall. The "
      + "doorway leads east into the dark staircase.");

    add_prop(ROOM_I_LIGHT, 1);

    add_exit(MORGUL_ROOM_DIR + "tower_3c", "east", 0, 0);

    add_roof();
    add_walls();
    add_floor();

    rack = clone_object(MORGUL_OBJ_DIR + "rack_3w");
    rack->move(this_object());
    rack->set_rack_type(2);
