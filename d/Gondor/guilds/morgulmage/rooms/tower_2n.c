...
create_morgul_tower_room()
{
    ...
    add_item(({"hall","great hall"}), 
          "Except for the stone slab to the north, the hall is devoid of "
        + "decorations and furniture. The walls are also almost featureless, "
        + "the seams between the stone blocks are hardly visible. The roof "
        + "is hidden in the darkness above. A doorway is leading out of the "
        + "hall to the south.");
    add_item(({"door","entrance","doorway","exit"}),
          "The doorway is just a rectangular opening in the south wall of the "
        + "hall. It leads back out onto the staircase.");

    add_prop(ROOM_I_LIGHT,1);

    add_exit(MORGUL_DEATH_ROOM, "north", warning,  0);
...
answer_death(string str, object player)
{
    ...
    if (str == "death")
    {
        write("If you seek death, then go north, and enter " +
            "the service of the Lord of Morgul!");
        player->add_prop(MORGUL_I_DEATH_WARNING, 1);
    }
    else
        write("There is nothing but Death to find at the Stone of Blood!");
}
...
give_warning(object player)
{
    write("When you try to go north, you are held back by a powerful evil force.\n"
        + "You feel like a terrible power is turning you inside out, " 
...
warning()
{
    ...
    if (!MORGUL_MASTER->is_acceptable_as_member(TP))
    {
