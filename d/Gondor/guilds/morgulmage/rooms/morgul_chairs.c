 ...
 *
 * This is just a redefinition of:
 *
 *   /d/Gondor/open/room_chairs/room_chairs.c
 *
 * I needed to redo it to get the descriptions I wanted. (Gorboth)
 *
 * Created June 2011 by Gorboth (originally by Gnadnar)
 */
...
static string	Exit_desc;	    /* the standard exits description, to 
				     * which we append on the fly a list
				     * of seated players
				     */
static int	Did_exit_desc;	    /* have we already constructed it? */
static int	No_sitting_in_desc; /* don't append list of seated
				     * players to exits desc
				     */
static int	Mentioned;	    /* is chair mentioned in room desc? */
static int	Multiples;	    /* are there multiple identical
				     * chairs, that we treat all alike?
				     */
static int	Invis_ok;	    /* for chairs in wizard workroom.
				     * NOT recommended for chairs in
				     * mortal areas.
				     */
...
public varargs int	add_chair(string name, string parse_pat,
				  int capacity, string prep, 
				  int mentioned, int multiples);
public int		remove_chair(string name);
public mapping		query_room_chairs();
public varargs string	sitting_on_chair(string name, object for_obj,
...
public void		set_chair_invis_ok(int i);
public int		query_chair_invis_ok();
 ...
 *				       default is "on"
 *			int mentioned -- is chair mentioned in room desc?
 *					default is 1
 ...
 */
public varargs int
add_chair(string name, string parse_pat, int capacity = 1,
	  string prep = "on", int mentioned = 1, int multiples = 0)
{
    if (strlen(name) &&
	strlen(parse_pat))
    {
 ...
 *				formatted, + " is" or " are"
 *			int definite -- if true, use a definite
 *				article when referring to chair
...
sitting_on_chair(string name, object for_obj, int namesonly,
		 int definite = 1)
{
    mixed	*chairptr;
    object	*occupants,
	...
	if (Multiples)
	{
	    if (s == 1)
	    {
		desc += " sitting " + chairptr[I_PREP] + 
			" one of the " + LANG_PWORD(name) + ".\n";
	    }
	    else
	    {
		desc += " each sitting " + chairptr[I_PREP] + 
			" the " + LANG_PWORD(name) + ".\n";
	    }
	}
	else if (definite)
...
do_sit(string arg)
{
	    ...
	    parse_command(arg, ({}), chairptr[I_PATTERN]))
	{
		...
		do_sit_msgs(tp, chairptr[I_PREP], indices[i], Mentioned);
		return 1;
	    }
...
show_subloc(string subloc, object pl, object for_obj)
{
	...
	if (for_obj != pl)
	{
		...
		chairptr[I_PREP] + name + ".\n";
	}
	return "You are sitting " + chairptr[I_PREP] + name + ".\n";
    }
    return "";
} /* show_subloc */
...
do_sit_msgs(object pl, string prep, string name, int definite = 1)
{
    ...
    write("You sit down " + prep + tmpstr + ".\n");
    say(QCTNAME(this_player()) + " sits down " + prep + tmpstr + ".\n");
} /* do_sit_msgs */
...
exits_description()
{
	...
	while ( s-- > 0)
	{
	    desc += sitting_on_chair(chairnames[s], tp, 0, Mentioned);
	}
    }
    return desc;
} /* exits_description */
...
public void	set_chair_invis_ok(int i)	{ Invis_ok = 1; }
public int	query_chair_invis_ok()		{ return Invis_ok; }
