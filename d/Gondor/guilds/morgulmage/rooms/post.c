...
create_morgul_tower_room() 
{
   ...
   add_exit(MORGUL_ROOM_DIR + "tower_3c", "south", 0);
...
init() 
{
	...
	rdr->set_alias("guildmaster",
        GONDOR_MASTER->query_domain_aliases(GUILD_NAME));
	rdr->set_alias("nazgul", 
        MORGUL_MASTER->query_nazgul_names());
...
do_hire(string str) 
{
   ...
   raven = clone_object(MORGUL_OBJ_DIR + "raven");
   if(raven->move(TP))
   {
