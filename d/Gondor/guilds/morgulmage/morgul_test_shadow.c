...
{
    return (MORGUL_ROOM_DIR + "train_high");
}
...
public void prepare_mage() {
    ...
    object undead_shadow = clone_object(MORGUL_GUILD_DIR + "undead_shadow");
...
void add_old_morgulmage_robe() {
    if (!objectp(present(MORGUL_S_SPELL_OBJECT, shadow_who))) {
        object robe = clone_object(OLD_MORGUL_GUILD_DIR + "mage_robe");
        robe->move(shadow_who, 1);
        shadow_who->catch_msg("The "+ robe->query_short()
            + " materializes in your inventory.\n");
    }
...
int is_using_original_spells() {
    return using_original_spells;
}

void set_using_original_spells(int value) {
    object robe = present(MORGUL_S_SPELL_OBJECT, shadow_who);
    if (objectp(robe)) robe->remove_object();
    
    if (value == 1) {
        log_file("morgulmage_playertest", ctime(time()) + " "
            + capitalize(this_player()->query_real_name()) + " started using "
            + "the original spells.\n");
...
customize_guild_spell_effect_messages(object *effects, string message_text)
{
    ...
    return "\n                     Currently Active Potions \n"
        + "                     ======================== \n\n"
        + break_string("You can cast " + COMPOSITE_WORDS(possible_spells) 
        + ".", 71)
        + "\n\n";
}
