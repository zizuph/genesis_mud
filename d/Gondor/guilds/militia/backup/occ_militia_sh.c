/*
 * /d/Gondor/guilds/militia/occ_mil_sh.c
 *
 ...
 * 08/23/02 - Allowed militia to be joined as an occ. guild without
 *            changing any abilities -- i.e. two layman guilds.
 ...
 * Description  : Returns the militia title
 * Returns      : string - the title
 */
nomask public string
query_militia_title()
 ...
 *                 The title of this function is slightly deceiving as it
 *                 was constructed when the guild had no occ. side to it,
 *                 but it remains.
 * Arguments    : object who    - the member to expel
 *                string reason - the reason they are to be expelled
 *                int mswitch   - optional argument to indicate that the
 *                                member is just changing from occ to lay.
...
remove_lay_militia(object who, string reason = "unknown", int mswitch = 0)
{
    ...
    prev_exp = shadow_who->query_acc_exp(SS_OCCUP);

    /* Clear the player's occ guild stat */
    ...
    if (!mswitch)
    {
        ...
        log_file("militia", extract(ctime(time()), 4, 15) + " " +
            capitalize(shadow_who->query_real_name()) + " removed because of " +            reason + ".\n", 500000);
    }
    else
    {
        emblem = present(MITEM_ID, shadow_who);
        ...
        shadow_who->set_guild_stat(SS_LAYMAN, prev_exp);

        log_file("militia", extract(ctime(time()), 4, 15) + " " +
            capitalize(shadow_who->query_real_name()) + " switched from " +
            "occupational to layman.\n");
    }
 ...
 *                 was completed.
 *                 This function name is slightly deceiving as it was
 *                 constructed when the guild had no occ. branch, but
 *                 it remains.
 */
public nomask void
...
{
    int ret;
    ret = shadow_me(who, "occupational", MILITIA_STYLE, MILITIA_NAME);
...
impale_enemy(object who)
{
    ...
    {
/* Spams up the logs too much from Calian's move behinds.
...
*/
    ...
    gbonus = shadow_who->query_stat(SS_OCCUP);
    gbonus /= 10;
...
{
    object comb, debg = find_living("gwyneth");
