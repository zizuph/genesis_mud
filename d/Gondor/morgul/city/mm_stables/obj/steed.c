...
pet_horse(object who, string emote)
{
    ...
    write("The " + steed_short() + " swiftly moves its head as you carelessly " +
          "reach out with the intent to " + emote + " it, and bites you!\n");
    say("The " + steed_short() + " swiftly moves its head as " + QTNAME(who) +
...
short(object for_obj)
{
    ...
    if (query_driver()->notmet_me(for_obj) && query_driver()->check_seen(for_obj))
    {
        return "black rider";
    }
...
{
    /* Mounting is only for morgul mages (and domain wizards for testing) */
    if (!IS_MORGUL_MAGE(rider) &&
        (SECURITY->query_wiz_dom(rider->query_real_name()) != getuid()))
    {
...
public void set_horse_desc(string name)
{
    add_name(name);
