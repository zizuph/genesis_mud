...
create_morgul_room()
{
    ...
    "rooms apparently have been pulled down. The only exit is to the " +
  ...
  add_exit(MORGUL_DIR + "city/barrstr5","east","@@check_exit",1);
  door = clone_object(MORGUL_DIR + "city/doors/b6trapup");
...
reset_room()
{
    set_alarm(20.0 + rnd() * 20.0, 0.0, &clone_npcs(orcs, MORGUL_DIR + "npc/ithil_soldier", 60.0));
}

string
do_search(object searcher, string str)
{
  if(!str || (str != "floor" && str != "rags" && str != "furniture"))
    return 0;
  if(!door) return 0;
  if(door->query_open()) return 0;
  if(door->query_closed_desc() != "") return 0;
  door->set_closed_desc("There is a closed trap door leading down.\n");
  change_my_desc(door->query_closed_desc(),door);
  say(QCTNAME(TP) + " searches the floor and finds a trap door leading " +
    "down hidden below the rags and broken furniture. \n");
  return BSN("Searching through the rags and the broken furniture, you " +
    "find a large wooden board lying on the ground close to one wall. " +
    "It looks as if this might be a trap door leading down into the " +
    "basement. ");
}
