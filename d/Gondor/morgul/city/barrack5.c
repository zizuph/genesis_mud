...
create_morgul_room()
{
    ...
    "to the first floor of the house. The door leading out is " +
    "stuck open, and the windows are all closed with shutters.");
  ...
  add_item(({"street"}), BSN(
    ...
    "a dark and narrow street and the Red Eye Inn across the street. "));
  set_road(5);
  set_side("north");

  add_exit(MORGUL_DIR + "city/darkstr3","east","@@check_exit",1);
  add_exit(MORGUL_DIR + "city/rooms/b5up","up","@@stairs",1);
