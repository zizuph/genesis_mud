...
#define MY_PRICES	({  3, 10, 16, 46, 110, 18, 30, 58, 225, 550, 288})
...
price_long()
{
   return (
    ...
    + "   Fried Elfmeat         ...  " + money_text(MY_PRICES[9]) + "\n\n"
    + "ROOMS:\n"
...
buy_it(string item_name, int number, string pay_type, string get_type)
{
    ...
    case "blackroot":
    case "blackroots":
        if (can_afford(MY_PRICES[6], number, "blackroot stew", pay_type, get_type))
    ...
    case "poached spiders":
        if (can_afford(MY_PRICES[7], number, "poached spider", pay_type, get_type))
    ...
    case "fried elfmeat":
    case "fried elfmeats":
        if (can_afford(MY_PRICES[9], number, "fried elfmeat", pay_type, get_type))
