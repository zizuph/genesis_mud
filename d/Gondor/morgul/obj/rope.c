...
int do_cut(string str)
{
    ...
    object *weapons  = filter(TP->query_weapon(-1),"filter_slash",TO),
            new_rope,
           *ob;
    int     i,
            ol,
...
string long_desc()
{
    ...
    desc = "It is an old frayed rope that has probably seen a lot " +
           "of use. If one can help it one probably should not rely on " +
           "it that this rope will not snap at the most inconvenient " +
           "moment. It is ";

    if(feet == 0)
        desc += "less than one foot long.";
    else if(feet == 1)
        desc += "about one foot long.";
    else if(feet < 13)
        desc += "about "+LANG_WNUM(feet)+" feet long.";
    else if(feet < 50)
    {
        feet = (2*feet+5)/10 * 5;
        desc += "about "+LANG_WNUM(feet)+" feet long.";
