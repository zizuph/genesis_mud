...
#define OBSIDIANSHARD   "/d/Gondor/common/guild2/newspells/obj/obsidian_shard.c"
#define MORGUL_GLOVES   "/d/Gondor/common/guild2/obj/morgul_gloves"
#define MORGUL_BOOTS    "/d/Gondor/common/guild2/obj/morgul_boots"
#define TALISMAN        "/d/Gondor/guilds/morgulmage/obj/corpse_cleaner"
...
read_sign()
{
    ...
    "\t|        jars, pouches and cords,          |\n" +
    "\t|     but we do not buy these items!       |\n" +
    "\t|  You can list our herbs and other wares  |\n" +
    "\t|  as well but be reasonable and don't let |\n" +
    "\t| the shopkeeper to show all the available |\n" +
    "\t|     herbs each time you want to check    |\n" +
    "\t|    the number of those green mushrooms   |\n" +
    "\t|                in stock.                 |\n" +
...
do_list( string str )
{
    ...
    if (strlen( str ) == 0)
    {
        if(this_player()->query_nazgul() || this_player()->query_wiz_level())
            write( "The Black Master has bestowed upon you the following commands:\n" +
                   "   - List herbs/all herbs [category]\n" +
                   "   - List wares/other wares\n\n" +
                   "The Herbalist of the Society have the following commands as do you:\n" +
                   "   - List focus herbs\n" +
                   "   - fadd <herb>\n" +
                   "   - fremove <herb>\n");
        else if(this_player()->query_morgul_level())
        {
            write( "The Nine have bestowed upon you the following commands:\n" +
                   "   - List herbs/all herbs [category]\n" +
                   "   - List wares/other wares\n\n");
            if(MORGUL_HERBALIST_MASTER->query_herbalist() == this_player()->query_name())
            {
               write("Being the Herbalist of the Society you also have the following commands awailable:\n" +
                   "   - List focus herbs\n" +
                   "   - fadd <herb>\n" +
                   "   - fremove <herb>\n");
            }
            write("Use the commands wisely or suffer the wrath of the Nine!\n");
        }
        else
            write( "The Nine, in their benevolence, have granted you the following commands in His herb shop:\n" +
                   "   - List herbs/all herbs\n" +
                   "   - List wares/other wares\n\n" +
                   "Be adviced to use the herb shop wisely or suffer the wrath of the Black Master!\n");
        return 1;
    }
    else if (str == "wares" || str == "other wares")
    {
        write( "      ========================================"+
            "===========\n");
        write( "===\\\\ We also sell other fine wares related to "+
            "herbalism: //===\n");
        write( "      ========================================="+
            "==========\n\n");
        price = HERBJAR->query_prop( OBJ_I_VALUE );
        shop_hook_list_other( find_object( HERBJAR ), price );

        price = HERBPOUCH->query_prop( OBJ_I_VALUE );
        shop_hook_list_other( find_object( HERBPOUCH ), price );
/*
        price = WRAP->query_prop( OBJ_I_VALUE );
        shop_hook_list_other( find_object( WRAP ), price );
*/
        price = HERBGLOVES->query_prop( OBJ_I_VALUE );
        shop_hook_list_other( find_object( HERBGLOVES ), price );

        price = MORGUL_GLOVES->query_prop( OBJ_I_VALUE );
        shop_hook_list_other( find_object( MORGUL_GLOVES ), price );

        price = MORGUL_BOOTS->query_prop( OBJ_I_VALUE );
        shop_hook_list_other( find_object( MORGUL_BOOTS ), price );

        price = HERBCORD->query_prop( OBJ_I_VALUE );
        shop_hook_list_other( find_object( HERBCORD ), price );
        
        write( "\n      ========================================"+
            "===========\n");
        write( "===\\\\ We also offer these other wares:        "+
               "            //===\n");
        write( "      ========================================="+
            "==========\n\n");

        price = OBSIDIANSHARD->query_prop( OBJ_I_VALUE );
        shop_hook_list_other( find_object( OBSIDIANSHARD ), price );

        price = TALISMAN->query_prop(OBJ_I_VALUE);
        shop_hook_list_other(find_object(TALISMAN), price);

        write( "\n      ========================================="+
            "==========\n");
        return 1;
    }
    else if (str == "focus" || str == "focus herbs")
    {
        if(MORGUL_HERBALIST_MASTER->query_herbalist() != this_player()->query_name() &&
        ...
        focus_list = MORGUL_HERBALIST_MASTER->query_focus_herbs();

        if(!sizeof(focus_list))
        {
            write("Ahh, the Herbalist of the Society! You honour me with your visit. " +
                  "You want a report, but alas! We have not agreed on any herbs for " +
                  "me to keep track of.\n");
            return 1;
        }
        ...
        for (i=0; i < j;i++)
        {
            ...
            herbs_focus += ([ Ts1 : herbs[file_name(herb_objects[i])] ]);
        }

        write("The herbalist bows to you and say: ");
        write("Ahh, the Herbalist of the Society! You honour me with your visit. ");
        write("I have prepared this report for you as we agreed.\n");
        write("The herbalist hands you a piece of parchment.\n");
        write("He adds: You may naturally discard it, at your convenience, at any time.\n");

        note_str = "\t,____, ,___,____,___,, ,____._____, ,____,,\n";
        note_str += "\t|     \\|              \\|          |/      |\n";
        note_str += "\t|                                         |\n";
        note_str += "\t|                                         |\n";
        note_str += sprintf( "\t|     %-21s %-13s %s", "Herb name:", "In store:", "|") + "\n";
        note_str += sprintf( "\t|     %-35s %s", "+++++++++++++++++++++++++++++++", "|") + "\n";

        for (i = 0; i < sizeof( focus_list ); i++)
        {
            note_str += sprintf( "\t|      %-'.' 24s %4d %6s", capitalize(focus_list[i]) + " ",
                        herbs_focus[focus_list[i]], "|") + "\n";
        }
        note_str += sprintf( "\t|     %-35s %s", "+++++++++++++++++++++++++++++++", "|") + "\n";
        note_str += "\t|                                         |\n";
        note_str += "\t|                                         |\n";
        note_str += "\t,____,___|\\,_____,__,________,/|____,_____|\n";

        herb_note = clone_object("/d/Gondor/common/guild2/obj/herbnote.c");
        herb_note->add_name("report");
        herb_note->add_pname("reports");
        herb_note->set_short("herb report");
        herb_note->set_pshort("herb reports");
        herb_note->set_long(note_str);
        herb_note->move(this_player());        
    ...
    else if (parse_command(str, ({ }), " [all] 'herbs' %s", sCategory))
    {
        ...
        if(stringp(sCategory) && (strlen(sCategory) > 3) && 
           (this_player()->query_morgul_level() || this_player()->query_wiz_level()))
        {
        ...
        else
        {
           ...
           for (i=0;i < j;i++)
           {
               if (strlen( Ts1 = herb_files_krynn[i]->query_herb_name() ) != 0)
               {
                   ...
                   herb_names_krynn += ([ Ts1 : sprintf( "\t%4d %-'.'34s ",
    ...
    {
        write ("List what? List [herbs/all herbs/wares/other wares]\n");
        return 1;
    }
...
do_buy(string str)
{
    ...
    if (str1 == "glass jar" || str1 == "jar" ||
        ...
        // str1 == "herb wrap" || str1 == "wrap" ||
        str1 == "gloves" || str1 == "pair of gloves" ||
        str1 == "cord" || str1 == "leather cord" || 
        str1 == "herbcord" || str1 == "black gloves" ||
        str1 == "pair of black gloves" || str1 == "pair of black boots" ||
        str1 == "black boots" || str1 == "boots" || 
        str1 == "shard" || str1 == "small shard" || 
        str1 == "obsidian shard" || str1 == "small obsidian shard" ||
        str1 == "small palm-sized talisman" || str1 == "talisman" ||
        str1 == "small talisman" || str1 == "palm-sized talisman")
    {
...
/*
    else if (WRAP->id(str1))
        ob = clone_object(WRAP);
*/
    else if (HERBGLOVES->id(str1))
        ob = clone_object(HERBGLOVES);
    else if (MORGUL_GLOVES->id(str1))
        ob = clone_object(MORGUL_GLOVES);
    else if (MORGUL_BOOTS->id(str1))
        ob = clone_object(MORGUL_BOOTS);
    else if (HERBCORD->id(str1))
        ob = clone_object(HERBCORD);  
    else if (OBSIDIANSHARD->id(str1))
        ob = clone_object(OBSIDIANSHARD);
    else if (TALISMAN->id(str1))
        ob = clone_object(TALISMAN);
    else
        return 0;

    price = query_buy_price_special(ob);

    /* in case herbs or other herb gear ever become heaps:
...
sell_it(object *ob, string str, int check) 
{
    ...
    for (i = 0; i < sizeof(ob); i++)
    {
        ...
        if (price > 0)
        {
            ...
            string *herb_name = ob[i]->query_herb_name();
            if (member_array(herb_name, this_object()->query_focus_herbs()) != -1)
                MORGUL_MASTER->increase_activity(this_player(), ACTIVITY_SELL_FOCUS_HERB);
            else if (member_array(lower_case(ob[i]->query_herb_name()), MORGUL_HERBS) >= 0)
                MORGUL_MASTER->increase_activity(this_player(), ACTIVITY_SELL_MAGE_HERB);
            else
                MORGUL_MASTER->increase_activity(this_player(), ACTIVITY_SELL_HERB);
