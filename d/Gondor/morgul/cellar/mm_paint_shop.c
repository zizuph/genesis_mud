/*
 * /d/Gondor/morgul/cellar/paintq.c
 *
 * Painting room for the painter in Minas Morgul.
 *
 * Coded by Gwyneth, August 27, 2000
 * Completely revised on November 8, 2000 by Gwyneth
 */
...
read_sign()
{
    ...
    "  Available colours are red, gold and black.\n");
}
...
start_paint(string str)
{
    ...
    if (wildmatch("/d/Gondor/guilds/rangers/obj/*cloak*",
      file_name(armour[0])))
    {
        notify_fail("That item can't be painted over.\n");
        return 0;
    }
      ...
      (!gPainter->allow_paint(this_player()->query_real_name(), paint)))
    {
        gPainter->command("whisper to " + OB_NAME(this_player()) +
            ...
            paint + " paint yet. You can still paint it black, " +
