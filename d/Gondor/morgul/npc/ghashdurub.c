/*
 *      /d/Gondor/morgul/npc/ghashdurub.c
 *
 *      The weapon smith of Minas Morgul
 *
...
#define ITHIL_WEAPON	(MORGUL_DIR + "obj/ithil_weapon")
...
create_ithil_man()
{
    ...
    set_title("the Master Weapon Smith of the Morgul Army");

    set_long(BSN("This strong dark-skinned man is one of the " + 
        "many weapon smiths of the Morgul army, who are forging " +
        "weapons for the final assault on Minas Tirith. But for " +
        "good money, he will sell some of his weapons to servants " +
        "of the Dark Lord who do not belong to the Morgul army."));
...
//    set_default_answer(VBFC_ME("default_answer"));
    set_default_answer(
        QCTNAME(TO) + " says: To see what I sell, look at the pricelist!\n" +
...
arm_me()
{
    ...
    clone_object(WEP_DIR + "smithhammer")->move(TO);
    command("wield hammer");
    clone_object(ARM_DIR + "smithapron")->move(TO);
    command("wear apron");
}
...
answer_weapon()
{
    command("say I fabricate swords, polearms, axes, maces, and daggers.");
...
catch_question(string question)
{
    ...
    if (query_feeling(tp) > 0)
    {
        command("say We do not want you here in Minas Morgul!");
        command("say Leave before I call the soldiers!");
        return;
    }
...
{
    return (MASTER_OB(ob) == ITHIL_WEAPON);
}
...
{
    command("say The "+(ITHIL_WEAPON->query_weapon_short(wtype))
      + " would cost you " + text + ".");
}
...
query_weapon_price(string wt)
{
    ...
    value = ITHIL_WEAPON->query_price(wt);
...
buy_weapon(string wt, string str2, string str3) 
{
    ...
    if (!shop_hook_allow_buy(find_object(ITHIL_WEAPON)))
        return 0;

    price = query_weapon_price(wt);

    bought = clone_object(ITHIL_WEAPON);
...
do_weapon_list(string str, int silent = 0)
{
    ...
    write("\n"
     + "    *** Ghashdurub, Master Weapon Smith of the Morgul Army ***\n\n"
...
special_attack(object victim)
{
    ...
    if (objectp(wep) && (MASTER_OB(wep) == ITHIL_WEAPON))
        return 0;
