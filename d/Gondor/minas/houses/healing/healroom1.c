...
object person_in_bed, healer;

create_room()
{
  set_short("The Healing room for women");
  set_long("@@long_func");
...
add_stuff()
{
  setuid();
  seteuid(getuid());
  if (!healer) healer = clone_object(MINAS_DIR+"npc/healerfem");
  healer->move(TO);
}
...
{
  string longstr;
  write("This is the room where the female patients are treated.\n"+
