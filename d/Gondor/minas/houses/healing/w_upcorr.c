...
create_room()
{
  ...
  set_long(BSN("The corridor runs north and south here, in the west wing of "+
    "the House. In the east wall windows are letting fresh air "+
    "in, and there is a doorway to the west. The white-walled corridor "+
    "leads north back to the main building, while the end is visible further south."));
  add_item(({"window","garden","windows"}),BSN(
    "Outside you see a green garden behind the Houses of Healing."));
    add_prop(ROOM_I_INSIDE,1);
  add_exit(HOH_DIR + "sw_upcorr","south",0,0);
  add_exit(HOH_DIR + "nw_upstair","north",0,0);
  add_exit(HOH_DIR + "fheal_wait","west",0,0);
}
