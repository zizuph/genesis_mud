...
return_with_herbmaster(object oldroom)
{
    ...
    if (!hmaster) {
        command("say I am sorry, but I could not find the herb-master.");
        command("say He was not in his office.");
    }
}
...
return_without_kingsfoil(object oldroom)
{
    ...
    if (hmaster) {
        hmaster->move_living("with the old woman",oldroom);
        hmaster->kingsfoil_lecture();
    }
    if (!hmaster) command("say I am most sorry, but I could not find any kingsfoil!");
}
...
asked_herbs()
{
  command("say Yes, "+ ((TP->query_gender() == G_MALE) ? "lord" : "lady") +
    ", we do have herbs in the House.");
  command("say But not enough, I reckon, for all that will need them.");
  ...
  command("say as I am sure your "+
	...
	"ship will know.");
  command("say The herb-master would be able to tell you more about herbs, I am sure.");
  return "";
}
...
asked_kingsfoil()
{
  ...
  if (athelas_asked_for)
    command("say Well, if your "+
      ((TP->query_gender() == G_MALE) ? "lord" : "lady") +
      "ship had named it at first, I could have told you.");
  command("say No, we have none of it here, I am sure.");
...
asked_king()
{
    command("say 'The hands of the king are the hands of a healer,");
    command("say and so shall the rightful king be known.'");
    command("say This my great-grandmother told me, and this I believe to be true.");
    command("say Perchance we will some day see the king return to Gondor.");
    return "";
}

void
witness_king_healing()
{
    command("emote exclaims: King! Did you hear that? What did I say? The hands of a healer, I said.");
}

void
witness_athelas()
{
    command("say Well now! Who would have believed it?");
    command("say The weed is better than I thought.");
    command("say It reminds me of the roses of Imloth Melui when I was a lass,");
    command("say and no king could ask for better!");
}
