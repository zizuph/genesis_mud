...
create_gondor()
{
    set_short("in the tunnel of the Fourth Circle of Minas Tirith");
    set_long(long_desc);
    add_exit(MINAS_DIR + "e3crc4","north", 0, 1);
    add_exit(MINAS_DIR + "e2crc4","south", 0, 1);
    add_prop(ROOM_I_INSIDE, 1);
    add_prop(ROOM_I_LIGHT, daylight);

    add_item(({"walls","wall"}), BSN(
        "The walls are carved from the living rock of the great cliff.  "
      + "Set into the west wall is a metal gate; an iron torch "
      + "stand is attached to the wall beside the door."));

    Door = clone_object(MINAS_DIR + "doors/sewer-c4-out");
    Door->move(TO);
...
long_desc()
{
    /* N.B. We assume the torch stand and the door stay put. since they
     * set OBJ_M_NO_GET, this seems reasonable.
     */
    if ( Tstand->query_torch_is_lit() )
    {
	 return BSN(
	    "This tunnel of the Fourth Circle cuts beneath "
	  ...
	  + "onto the Main Street of the Fourth Circle of Minas "
	  + "Tirith. Set into the rock of the west wall is "
          + (Door->query_open() ? "an open" : "a closed")
          + " metal gate.");
    }
    return BSN(
	"This tunnel of the Fourth Circle cuts beneath "
