/*  The Minas Tirith bakery, a modification of
 *  The Hobbiton bakery
 */
inherit "/std/room";
inherit "/lib/trade";
...
create_room() {
   set_short("The Minas Tirith Bakery");
   set_long(break_string(
      "You are in the Minas Tirith bakery. The smell of fresh bread "
    + "lingers in the air. You see all kinds of delicious cakes, "
    + "bread and pies inside the glass counter. A smiling human baker "
    + "stands behind the counter, ready to serve you if you feel like "
    + "ordering something. On the wall is a pricelist.\n",70));
   ...
   add_item(({"pie","pies"}), break_string(
    ...
    + "of all. It is a fruit pie, just like your grandmother used to "
   ...
   add_exit("/d/Gondor/minas/e2crc3", "west", 0);
...
price_long() {
   return (
      "The pricelist reads:\n\n"
    + "   Fruit pie       ...   " + MY_PRICES[0] + " cc.\n"
    + "   Strawberry cake ...   " + MY_PRICES[1] + " cc.\n"
    + "   White bread     ...   " + MY_PRICES[2] + " cc.\n"
    + "   Brown bread     ...   " + MY_PRICES[3] + " cc.\n\n");
}
 ...
 */
   ...
   if (item_name == "pie" || item_name == "fruit pie")
   {
      if(can_afford(MY_PRICES[0], "fruit pie"))
      {
         food = clone_object("/d/Gondor/minas/obj/pie");
         if(food->move(this_player()))
         {
            write("The baker says: You can't carry that pie, so I'll just put it down.\n");
	    food->move(environment(this_player()));
            return 1;
         }
         say(QCTNAME(this_player()) + " buys a piece of fruit pie.\n");
      }
      return 1;
   }
   if (item_name == "cake" || item_name == "strawberry cake")
   {
      ...
      {
         food = clone_object("/d/Gondor/minas/obj/cake");
         if(food->move(this_player()))
         {
            write("The baker says: You cannot carry that cake, so I'll just put it down.\n");
	    food->move(environment(this_player()));
            return 1;
         }
         say(QCTNAME(this_player()) + " buys a strawberry cake.\n");
      }
      return 1;
   }
   if (item_name == "bread")
   ...
   if (item_name == "white" || item_name == "white bread")
   {
      ...
      {
         food = clone_object("/d/Gondor/minas/obj/whbread");
         if(food->move(this_player()))
         {
            write("The baker says: You cannot carry that bread, so I'll just put it down.\n");
	    food->move(environment(this_player()));
            return 1;
         }
   ...
   if (item_name == "brown" || item_name == "brown bread")
   {
      ...
      {
         food = clone_object("/d/Gondor/minas/obj/brbread");
         if(food->move(this_player()))
         {
            write("The baker says: You cannot carry that bread, so I'll just put it down.\n");
	    food->move(environment(this_player()));
   ...
   if (sizeof(money_arr = pay(price, this_player(), pay_type, 0, 0, get_type)) == 1)
   {
      if (money_arr[0] == 2)
      {
         tell_room(this_object(), break_string(
            "The baker says: You don't have any " + pay_type
      ...
      {
         tell_room(this_object(), break_string(
            "The baker says: I think that " + food + " is too "
      ...
      {
         tell_room(this_object(), break_string(
            "The baker says: I'm afraid that " + food + " is a bit "
          + "above your budget, honored " + QNAME(this_player())
          + ".\n",70));
         return 0;
      }
   }
