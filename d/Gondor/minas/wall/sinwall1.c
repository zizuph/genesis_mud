...
create_room()
{
  set_long(BSN("This is the guard room where soldiers of the City spend "+
    "their time when not on duty. A doorway leads south to an office, "+
  ...
  add_exit(MINAS_DIR + "e2crc1","west",0,1);
  add_exit(MINAS_DIR + "wall/sinwall2", "south", 0, 1);
  add_exit(MINAS_DIR + "wall/topswall1", "up", 0, 2);
  ...
  clone_object(MINAS_DIR+"obj/sinwdoor1")->move(TO, 1);
  nr = 6 + random(3);
  guards = allocate(nr);
  reset_room();
}
