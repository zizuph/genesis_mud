...
create_room() {
  ...
  set_long("@@describe");
  add_exit(MINAS_DIR+"wall/sinwall1","down",0,0);
  add_exit(MINAS_DIR+"wall/battlement1","north",0,0);
...
describe()
{
  ...
  if (time == "night") {
    longstr = "In the darkness of the night you don't see much, but the city "+
      ...
      "Further north of here the First Gate leads through the wall below. A "+
  ...
  if (time == "morning"|| time=="early morning") {
    longstr = "The rising sun in the east casts long shadows in the City "+
      ...
      "north the First Gate leads through the wall you are on. A stairway "+
  ...
  if (time == "noon")
	...
	"The First Gate leads through the wall further north of here, and "+
  ...
  if (time == "afternoon" || time == "evening")
	...
	"inside the wall here, while further along the wall to the north "+
