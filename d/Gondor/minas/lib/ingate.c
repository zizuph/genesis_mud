...
ranger_title(object pl)
{
    ...
    else if (R_COUNCIL->query_lieutenant(pl->query_name()))
	return "the Lieutenant of the Rangers of "+pl->query_company();
    else if (R_COUNCIL->query_sergeant(pl->query_name()))
	return "the Sergeant of the Rangers of "+pl->query_company();
...
do_knock(string str)
{
	...
	if( (query_friend_or_foe(TP) > - 1) && (!sizeof(enemy)) )
	{
		...
		if (present("ranger_guild_object",TP))
		    ...
		    else if (IS_DUNADAN(TP)) fname = "a Dunadan";
	    }
	    ...
	    tell_room(TO,BSN("From the room to the "+GDIR+", someone shouts: Open the gate, "
			     +fname+" wants to leave!"));
	    tell_room(gate->query_other_room(),BSN("You hear a muffled shout from the other side of the gate.\n"));
	    ...
	    other_gate->do_open_door("The gate opens as the iron doors of the gate swing inwards.\n");
	    gate->do_open_door("The gate opens as the iron doors of the gate swing inwards.\n");
	}
	else
	{
	    ...
	    tell_room(TO,BSN("You hear more loud shouts coming from the "+GROOMDIR+"!"));
	    if(!enemy)
		enemy = ({TP});
	    if (oc < 3)
