...
#define ADJ0       ({ "dark", "light", "striped" })
#define ADJ1       ({ "supple", "brown", "gray" })
#define FABRIKES   ({ "leather", "woolen", "cotton" })
#define STYLE      ({ "comfortable" })
#define EXTRA      ({ "" })
...
create_armour()
{
    string adj0, adj1, material, style, extra;
    adj0 = one_of_list(ADJ0);
    adj1 = one_of_list(ADJ1);
    material = one_of_list(FABRIKES);
    style = one_of_list(STYLE);
    extra = one_of_list(EXTRA);

    set_name("trousers");

    set_default_armour(ARM, A_LEGS, ({ 0, 0, 0 }), 0);
    set_may_not_recover();

    add_prop(OBJ_I_WEIGHT, 1200);
    add_prop(OBJ_I_VOLUME, 700);
    add_prop(OBJ_I_VALUE, (F_VALUE_ARMOUR(ARM) + random(10)));
