...
create_area_room()
{
    ...
    set_extraline("To the north the White Mountains towers up, and at " +
        ...
        "west a ridge with a strange hill at its end can be seen.");
                                   /* set extra longdesc */
    ...
    add_exit(E_FIELD + "E4.c", "south", &on_field(), 3, 1);
    add_exit(E_FIELD + "D3.c", "west", &blocked(), 3, 1);
    add_exit(E_VALE + "F3.c", "east", &blocked_path(), 3, 1);
    add_exit(E_VALE + "E2.c", "north", &blocked_path(), 3, 1);
    add_exit(E_FIELD + "F4.c", "southeast", &on_field(), 3, 1);
}
