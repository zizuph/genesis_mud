...
create_area_room()
{
    ...
    set_extraline("Some white mountain tops are seen over the landscape. " +
        "A bit to the west the sound of running water can be heard. The " +
	"field is dark green, and some bushes and small trees are scattered " +
	"over the field.");
                                   /* set extra longdesc */
    ...
    add_exit(E_VALE + "D9.c", "south", &blocked_path(), 3, 1);
    add_exit(E_VALE + "E8.c", "east", &blocked_path(), 3, 1);
    add_exit(E_FIELD + "C8.c", "west", &blocked_river(), 3, 1);
    add_exit(E_FIELD + "D7.c", "north", &on_field(), 3, 1);
    add_exit(E_VALE + "C9.c", "southwest", &to_road(), 3, 1);
    add_exit(E_FIELD + "E7.c", "northeast", &on_field(), 3, 1);
}
