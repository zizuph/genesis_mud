...
create_area_room()
{
    ...
    set_extraline("To the northwest a strange hill is seen with the " +
        "White Mountains towering up behind it. The field is dark green, " +
        "some bushes and trees are scatered over the field. The sound of " +
        "running water can be heard from the north and the east.");
                                   /* set extra longdesc */
    ...
    add_exit(E_FIELD + "E6.c", "south", &on_field(), 3, 1);
    add_exit(E_FIELD + "D5.c", "west", &blocked(), 3, 1);
    add_exit(E_FIELD + "E4.c", "north", &blocked_river(), 3, 1);
    add_exit(E_FIELD + "F5.c", "east", &blocked_river(), 3, 1);
}
