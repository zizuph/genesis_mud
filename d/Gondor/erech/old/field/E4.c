...
create_area_room()
{
    ...
    set_extraline("To the north the White Mountains towers up, and at " +
        "its foot a road can be seen. The field is dark green, some " +
        "bushes and small trees are scatered over the field. To the " +
        "west a strange hill rises at the end of a ridge. A bit to the " +
        "south the sound of running water can be heard.");
                                   /* set extra longdesc */
    ...
    add_exit(E_FIELD + "F4.c", "east", &on_field(), 3, 1);
    add_exit(E_FIELD + "D3.c", "northwest", &on_field(), 3, 1);
    add_exit(E_FIELD + "E3.c", "north", &on_field(), 3, 1);
    add_exit(E_FIELD + "E5.c", "south", &blocked_river(), 3, 1);
    add_exit(E_VALE + "D4.c", "west", &blocked_path(), 3, 1);
}
