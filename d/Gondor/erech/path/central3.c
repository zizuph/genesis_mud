...
create_gondor()
{
  ...
  clone_object(E_NPC+"morthec")->move(this_object());

  nr = 3;
  ghost = allocate(nr);
  reset_room();

  add_exit(E_PATH + "central2.c", "west", "@@pass", 2, 0);
  add_exit(E_PATH + "south1.c", "southeast", "@@dis_test", 2, 0);
  clone_object(E_OBJ + "pdoor1.c")->move(this_object());
}
...
reset_room()
{
    ...
    o = clone_unique(E_NPC + "morthec", 1);
    o->move(this_object());
