/*
 *	/d/Gondor/ithilien/emyn-arnen/vrd4.c
 *
 *	Coded by Olorin.
 *
 *	Modification log:
...
create_area_room()
{
    ...
    + "the market place of this hamlet. To the south is the entrance "
    + "to the the large stone building.");
    add_item("road",BSN(
    ...
    + "The entrance is to south."));
    add_item(({"entrance", "gate", "door"}), BSN(
      "A large archway is leading into the large building to the south. "
    + "The large hinges on which the wings of the gate once hung can "
    + "still be seen at the sides of the archway, but of the gate itself "
    ...
    + "products of their fair country. Most of the houses of the hamlet "
    + "are north of the market place."));
    add_exit(ITH_DIR + "emyn-arnen/mp1",           "north", 0, 1);
    add_exit(ITH_DIR + "emyn-arnen/vrd3",          "east",  0, 1);
    add_exit(ITH_DIR + "emyn-arnen/palace/hall1",  "south", 0, 1);
    add_exit(ITH_DIR + "emyn-arnen/vrd5",          "west",  0, 1);
}
