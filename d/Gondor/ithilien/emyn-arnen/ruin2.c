/*
 *	/d/Gondor/ithilien/emyn-arnen/ruin2.c
 *
 *	Coded by Olorin.
 *
 *	Modification log:
...
create_area_room()
{
    ...
    set_extraline("All that is left of this building are the foundations "
      + "and half of the southwest wall. The door into the house once "
    ...
    add_item(({"building", "ruin", "house", }), BSN(
      ...
      + "and the remnants of the southwest wall are black and charred. "
      + "Of the interior of the house nothing is left."));
    add_item(({"southwest wall", "remnants", "wall", "walls", "foundations", }), BSN(
        "The remnants of the southwest wall and the foundations are all "
    ...
    add_item(({"floor", "ground", }), BSN(
      ...
      + "Plants have reclaimed the space, and grass, flowers, and ever "
    ...
    add_item(({"square", "market place", "small square"}), BSN(
      ...
      + "north of it."));
    add_exit(ITH_DIR + "emyn-arnen/vrdn", "southeast", 0, 1);
  
    herbs = HERB_MASTER->query_herbs( ({ "garden", "meadow", "subtropical", }) );
    set_up_herbs( ({ HERB_DIR + "red_pepper",
		     ONE_OF_LIST_WITH_SEED(herbs, 393565),
		     ONE_OF_LIST_WITH_SEED(herbs, 192909),
		     ONE_OF_LIST_WITH_SEED(herbs, 490699), }),
