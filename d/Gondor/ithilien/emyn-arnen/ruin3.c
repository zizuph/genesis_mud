/*
 *	/d/Gondor/ithilien/emyn-arnen/ruin3.c
 *
 *	Coded by Olorin.
 *
 *	Modification log:
...
create_area_room()
{
    ...
    set_extraline("All that is left of this building are the foundations "
      + "and half of the east wall. The door into the house once "
      + "was to the west, where the doorstep is still visible.");
    ...
    add_item(({"building", "ruin", "house", }), BSN(
      ...
      + "and the remnants of the east wall are black and charred. "
      + "Of the interior of the house nothing is left."));
    add_item(({"east wall", "remnants", "wall", "walls", "foundations", }), BSN(
        "The remnants of the east wall and the foundations are all "
    ...
    add_item(({"square", "market place", "small square"}), BSN(
      ...
      + "north of it."));
    add_exit(ITH_DIR + "emyn-arnen/vrdn", "west", 0, 1);
    remove_item("road");
  
    herbs = HERB_MASTER->query_herbs( ({ "garden", "meadow", "subtropical", }) );
    set_up_herbs( ({ HERB_DIR + "red_pepper",
		     ONE_OF_LIST_WITH_SEED(herbs, 671234),
		     ONE_OF_LIST_WITH_SEED(herbs, 938088),
		     ONE_OF_LIST_WITH_SEED(herbs, 281081), }),
