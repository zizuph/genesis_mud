...
create_monster()
{
  ...
  set_adj("strong");
  set_stats(({ 70+random(10), 70+random(10), 75+random(10),
	70+random(10), 60+random(10), 80+random(10) }));
  set_hp(7000);
  set_mana(1000);
  ...
  add_ask(({"help"}),
    "say Yes, we could need help. Please ask for a task!", 1);
  add_ask(({"favour","favor","task","quest","job","tasks","quests","jobs"}),
    VBFC_ME("give_task"));
  add_ask(({"for favour","for favor","for task","for quest","for job",
    "for tasks","for quests","for jobs"}),
    VBFC_ME("give_task"));
  add_ask(({"about favour","about favor","about task","about quest",
    "about job","about tasks","about quests","about jobs"}),
    VBFC_ME("give_task"));
  add_ask(({"meal","rabbit","stewed","herbs","warm","coney"}),
    VBFC_ME("inform_meal"));
  add_ask(({"about meal","about rabbit","about stewed","about herbs",
    "about warm","about coney"}),VBFC_ME("inform_meal"));
...
long_func()
{
  return BSN("You are looking at a strong black-skinned Haradrim. " +
    ...
    "a soldier or mercenary, and his confident air tells you that he "+
    "must be an officer. He is probably in the service of the Dark Lord. ");
}
...
inform_meal()
{
  ...
  START_QUEST(TP, "STEWED RABBIT (HARADRIM)");
  return "";
}
...
make_fire(object player)
{
  ...
  command("say I'll climb down and will make a fire "+
...
inform_gondor()
{
  if(!check_player(TP)) return "";
  command("spit");
  command("say These damned Gondorians have been waging war "+
    "against us for ages!");
  command("say But now we will get even!");
  command("grin");
  command("say Not long any more and we will cross the Anduin!");
  command("say Then we will plunder Minas Tirith, and "+
    "Gondor will be history!");
  command("cackle");
  return "";
}
...
inform_ithilien()
{
  ...
  command("say Once Minas Tirith has been destroyed, we Haradrim "+
    "will come to Ithilien and settle here!");
  command("smile");
  return "";
}
...
inform_rangers()
{
  if(!check_player(TP)) return "";
  command("spit");
  command("say Rangers? You mean those dirty tramps from Gondor "+
    "that can't realize ");
...
inform_duty()
{
  if(!check_player(TP)) return "";
  command("sigh");
  command("say We are here on duty on lookout for reinforcements "+
    "that will come up on the road.");
  command("say We are to warn them that the cursed rangers are "+
    "planning some crazy ambush.");
  command("say We've been here for two days, because that fool "+
    "of a Captain probably has forgotten us!");
  return "";
}
...
{
  if(!check_player(TP)) return "";
  command("say We Haradrim have been living in Middle Earth "+
    "far longer than those cursed Gondorians!");
  command("say Out home is Harad, in the South!");
  command("smile");
  command("say Where the sun always smiles, and where the "+
    "mightiest men and beasts of the world dwell!");
  command("say We are true allies of the Dark Lord!");
  command("say We are not His servants, we are just combining "+
    "our forces to destroy Gondor!");
  return "";
}
...
{
  if(!check_player(TP)) return "";
  command("say Beautiful Harad, our home, lies south of the Harnen, "+
    "far south of Ithilien!");
  ...
  command("say And there has never been friendship between those "+
    "cursed dunedain and Harad!");
  return "";
}
...
inform_sauron()
{
  ...
  command("say He is helpful, because He has some resources of His own, "+
    "like the orc bands.");
  command("say But he does not control us, and we only follow him "+
    "where we want.");
...
{
  if(!check_player(TP)) return "";
  command("say Those orcs stink, and they are foul creatures, "+
    "but they are useful.");
  command("say After the war, we will destroy them, or chase them "+
    "away into the north.");
  command("say I have no love for them, but it's nice when they help "+
    "us kill a few Gondorians!");
...
string inform_enemies()
{
  if(!check_player(TP)) return "";
  command("spit");
  command("say We fight the Gondorians and whatever creatures ally "+
    "with those cursed dunedain!");
  return "";
}
...
string inform_road()
{
  ...
  command("say It leads further north to Minas Morgul and the Morannon, "+
    "the Black Gate of Mordor.");
  command("grin");
  command("say Yeah, the cursed dunedain built this little road, "+
    "and we use it to march up ");
  command("say from Harad to make war on Minas Tirith!");
  command("cackle");
  command("say Serves them right, they built the road to threaten us, "+
...
return_item(object *arr)
{
	...
	member_array(from, query_enemy(-1)) == -1)
    {
...
tell_fire(object player)
{
  ...
  command("say Anyway, it was very foolish, and endangered us "+
    "and our mission!");
}
...
quest_done(object player)
{
  ...
  if(haradrim2 = present("haradrim2",ENV(TO)))
    tell_room(ENV(TO), "Finally, " + QTNAME(TO) + " and " +
      QTNAME(haradrim2) + " seem to have finished eating.\n");
  else
    tell_room(ENV(TO), "Finally, " + QTNAME(TO) +
      " seems to have finished eating.\n");
  ...
  {
    pot->move(player);
    player->catch_msg(QCTNAME(TO) + " gives " +
      LANG_ADDART(pot->query_name()) + " back to you.\n");
    say(QCTNAME(TO) + " gives " + LANG_ADDART(pot->query_name()) +
...
eat_no_reward(object *arr)
{
  ...
  if(haradrim2 = present("haradrim2",ENV(TO)))
    command("emote shares the rabbit stew with " +
      process_string(QTNAME(haradrim2)) + ".");
  command("emote starts eating the rabbit stew.");
  if(haradrim2)
    tell_room(ENV(haradrim2),process_string(QTNAME(haradrim2)) +
      " starts eating the rabbit stew.\n");
  meal->remove_object();
  command("sigh");
  command("say Well, it was nourishing, but to be honest, "+
    "it wasn't the meal we've been waiting for!");
...
extra_herb(object pot, object meal, object player, string extra_herb_name)
{
  if(!present(player,ENV(TO))) return 0;
  command("emote sniffs at the stew in the pot.");
  command("say Well, almost, but why did you put in " +
    extra_herb_name + "?");
  ...
  command("say you are preparing rabbit meat, like basil, "+
    "savory, and tarragon.");
...
eating3(object meal)
{
  object haradrim2;
  if(haradrim2 = present("haradrim2",ENV(TO)))
    command("emote and " + process_string(QTNAME(haradrim2)) +
      " are silently eating the rabbit stew.");
  else
...
eating2(object meal)
{
  ...
  if(haradrim2 = present("haradrim2",ENV(TO)))
  {
    command("emote shares the rabbit stew with the " +
      haradrim2->query_nonmet_name() + ".");
...
stop_fleeing(object player)
{
  ...
  if(player->query_attack())
    return 0;
  player->catch_msg(QCTNAME(TO) + " stops you.\n");
  say(QCTNAME(player) + " tries to climb down, but is stopped by " +
    QTNAME(TO) + ".\n");
