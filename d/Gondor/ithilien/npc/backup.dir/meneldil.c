...
create_monster()
{
  ...
  set_adj("silent");
  set_adj("strong");
  set_stats(({ 65+random(10), 70+random(10), 75+random(10), 60+random(10), 60+random(10), 80+random(10) }));
  ...
  add_chat("Our forefathers built the road, and now our enemies are marching on it.");
  ...
  add_chat("The Southrons have gone over to the Enemy, as so many have done in the East.");
  ...
  add_ask("rangers",BS(QCTNAME(TO) + " says: We Rangers of Ithilien are fighting the armies of the Evil One.\n"));
  add_ask("duty",BS(QCTNAME(TO) + " says: We have to observe the road running towards the Morannon " +
...
arm_me()
{
  ...
  amulet = clone_object("/d/Gondor/common/guild/ramulet");
  amulet->move(TO);
  command("wear amulet");
...
long_func()
{
  return break_string("He is a pale-skinned man, dark of hair, with grey eyes, and a proud and sad " +
...
{
  if(query_attack() || ranger_test(victim)) return;
...
ranger_test(object player)
{
  ...
  iranger = (player->test_bit("Gondor",1,18) || player->test_bit("Gondor",1,19));
  if(iranger) command("unmask me");
  return iranger;
}
