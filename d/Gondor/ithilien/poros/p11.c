...
create_area_room()
{
    ...
    set_extraline("The path here is now inland from the river Poros, "
      ...
      + "A few small trees grow along the sides of the path in contrast "
      + "with the rest of the surrounding region. "
    ...
    add_herb_file(HERB_DIR + "laurel");
    add_herb_file(HERB_DIR + "saffron");
    add_herb_file(HERB_DIR + "athelas");

    add_exit(ITH_DIR + "poros/p14", "northwest", 0, 2);
    add_exit(ITH_DIR + "poros/p12", "northeast", 0, 2);
    add_exit(ITH_DIR + "poros/p10", "southeast", 0, 2);
    ...
    add_item(({"trees", "bushes", "brush", "scrub brush", "scrub",
      ...
      + "be seen along the path here, contrasting with the rest of the "
