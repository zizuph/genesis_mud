...
create_area_room()
{
    ...
    add_herb_file(HERB_DIR + "strawberry");
    add_herb_file(HERB_DIR + "clove");
    add_herb_file(HERB_DIR + "foxglove");

    add_exit(ITH_DIR + "poros/p11", "southwest", 0, 2);
    add_exit(ITH_DIR + "poros/p13", "north", 0, 2);
...
reset_room()
{
    ...
    if (!objectp(Ranger[i]))
    {
      Ranger[i] = clone_object(POROS_DIR + "npcs/ranger");
      Ranger[i]->arm_me();
      Ranger[i]->move_living("down", TO);
    }
    }
}
