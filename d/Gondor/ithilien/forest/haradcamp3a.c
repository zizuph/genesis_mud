...
create_camp()
{
    ...
    set_long("Orderly tents line the path, which runs east and west. The " +
        "forest surrounds the camp, and is full of tall oak trees.\n");
    add_item(({ "rows", "tents", "tent" }), 
	    "The tents are kept in good order. They are used by the common " +
        "soldiers.\n");
    add_item("forest", 
        "The forest surrounds the camp, and full of tall oak trees..\n");
    add_item("path",
	    "The path leads east and west, and seems to have been made by " +
        "humans. A side path leads north.\n");

    add_exit(ITH_DIR+"forest/olipath1", "north", 0);
    add_exit(ITH_DIR+"forest/haradcamp3", "west", 0);
    add_exit(ITH_DIR+"forest/haradcamp3b", "east");
