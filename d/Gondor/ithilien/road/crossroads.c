...
public int     warning(string dir);
public string  exa_figure();
public string  exa_head(int i);
...
create_area_room()
{
    ...
    set_extraline("You have entered a great roofless ring of trees, "
      ...
      + "figure here.\n");
    add_mountains(1, "only about half a league");
    add_item( ({ "valley", "cleft", }),
    ...
    add_item(({"road","roads","crossroads"}), 
    ...
    add_item(({"symbols","foul symbols","scrawls"}), 
    ...
    add_exit(MORGUL_DIR + "road/mrd1", "east",   test_exit,        3);
    add_exit(ITH_DIR + "road/sr1",     "south", &warning("south"), 3);
    add_exit(ITH_DIR + "road/er7",     "west",   test_exit,        3);
...
{
    if (!head_placed)
        return "It is a huge sitting stone figure, still and solemn. "
    ...
    return "It is a huge sitting stone figure, shaped as a still and "
      + "solemn king. Its head sits loosely upon its shoulders. The "
      + "years have gnawed it, and violent hands have maimed it, but it "
      + "still looks impressive. The eyes are hollow and the carven "
...
exa_head(int i)
{
    string desc;
    if (!head_placed && !i)
        return "The head of the figure is a pure mockery, a round "
    ...
    if (head_placed)
      ...
      desc = "The old king's head lays by the roadside.";
    ...
    desc += ". And in the crevices of his stony hair, yellow "
      + "stonecrop gleams.";
    if (head_placed)
        desc += " A round, rough-hewn stone lays by the roadside, its "
...
warning(string dir)
{
    ...
    write("The armies of the Dark Lord are using this road, too, " +
          "and they don't like spies!\n\n");
    return 0;
}
...
"The black raven on the window ledge caws softly.\n");
}                                                                              
