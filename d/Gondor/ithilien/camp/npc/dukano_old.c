...
remove_name(({"_ithil_monster","_morgul_monster"}));
    ...
    set_stats(({106+rnd, 106+rnd, 115+rnd, 120+rnd, 100-rnd, 86+rnd,0, 90+rnd,0}));
...
set_skill(SS_ANI_HANDL, 70);
set_skill(SS_RIDING, 70);
...
default_react(object tp)
{
	...
	if (tp == query_attack())
	    command("hiss " + ONE_OF_LIST(
		({ "ask Lars, you're going to meet him!",
		  "Like I'm going to help you _now_!",
		  "Why quest?  I can lower your brute right now!",
		})));
	else
	    command("hiss " + ONE_OF_LIST(
		({ "hey, I'm no npc, stupid", 
		  "Have you tried doing your own thinking for a change?",
		  "I hate questing.",
		  "Hey I can email you some docs, hold on a sec..",
		})));
	break;
    case 2:
