...
create_gondor()
{
    set_short("on the catwalk above the commander's tent");
    set_long(long_func);
    add_item(({"clearing","camp","down","ground"}),"@@exa_camp|" +
      "Looking down you can see the commander's tent.  |"+
      CAMP_ROOM_DIR+"fort4@@");
    add_item(({"wall","walls","wooden walls"}),wall_desc);

    add_exit(CAMP_ROOM_DIR + "wall2.c","northeast");
    add_exit(CAMP_ROOM_DIR + "wall4.c","southeast");
    add_exit(CAMP_ROOM_DIR + "fort4.c","down");
...
long_func()
{
    ...
    "the west end of this outpost.  " +
    "Below is the ladder which leads up to the catwalk, " +
    "for anyone to go up or down.  ";
...
do_leap(string str)
{
	...
	TP->move_living("M",CAMP_ROOM_DIR + "fort4");
...
leap_access(object obs)
{
    ...
    if (!CAN_SEE_IN_OTHER_ROOM(TP,CAMP_ROOM_DIR+"fort4"))
    {
	return 0;
    }
    if (!CAN_SEE(TP,obs))
...
reset_room()
{
	...
	if (!objectp(Orc1))
	{
	    ...
	    Orc1->set_monster_home(CAMP_ROOM_DIR + "wall3");
	}
	if (!objectp(Orc2))
	{
