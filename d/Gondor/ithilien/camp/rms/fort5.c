...
create_gondor()
{
    set_short("at the south side of the camp in Ithilien");
    set_long(long_func);
    ...
    add_item(({"southwest tent","west tent","second tent"}),nw_tent_desc);
    add_exit(CAMP_ROOM_DIR + "fort2.c","north");
    add_exit(CAMP_ROOM_DIR + "fort4.c","northwest");
...
long_func()
{
    ...
    if (rq_stage() < RQ_GANDALF_IN_BAGEND)
    {
	longtxt = "This is the southern end of this small outpost.  " +
	...
	"be placed.  There is nothing but trees to the south.  ";
    }

    if (rq_stage() >= RQ_OSGILIATH_ATTACK)
    {
	longtxt = "You are standing at the southern edge of a fort in " +
	...
	"of the Red Eye.  " +
	"A black tent rests to the south as well, and another one can be seen to " +
	"the northwest.  The center of this outpost is to the north.  ";
    ...
    {
	longtxt = "This is the southern end of this outpost.  " +
	"A massive wall rises before you, completely surrounding the " +
	"camp.  " +
	"A large tent is also to the south, and you can see another tent to " +
	"the northwest.  To the north is the center of this outpost.  ";
    }
...
tent_desc()
{
	...
	"There are metallic clanking noises coming from inside.\n";
    }
}
...
nw_tent_desc()
{
    if (rq_stage() < RQ_GANDALF_IN_BAGEND)
    {
	return "There is no tent to the northwest.\n";
    }
    if (rq_stage() >= RQ_GANDALF_IN_BAGEND)
    {
	return "It is too difficult to make out details about the " +
	"tent to the northwest, though a path leads there.\n";
    }
}
...
reset_room()
{
    ...
    if (rq_stage() >= RQ_GANDALF_IN_BAGEND)
    {
	if (member_array("south",query_exit_cmds()) < 0)
	    add_exit(CAMP_ROOM_DIR + "tent3","south");
    }
	...
	if (!objectp(Orc1))
	{
	    ...
	    Orc1->set_monster_home(CAMP_ROOM_DIR + "fort5");
	}
	if (!objectp(Orc2))
	{
	    Orc2 = ::clone_npc(Orc2, CAMP_NPC_DIR + "ithil_olog");
	    Orc2->set_monster_home(CAMP_ROOM_DIR + "fort5");
	}
    }
}
