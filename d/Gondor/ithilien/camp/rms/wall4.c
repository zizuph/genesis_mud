...
//
//  2006.11.17, Toby: Fixed a typo, fortt -> fort
//
/*
 * Modification Log:
...
create_gondor()
{
    set_short("on the catwalk above the south end of the camp");
    set_long(long_func);
    add_item(({"clearing","camp","down","ground","tent"}),"@@exa_camp|" +
      "Looking down you can see the south side of this hidden camp, and " +
      "a tent that has no obvious use.  |"+
      CAMP_ROOM_DIR+"fort5@@");
    add_item(({"wall","walls","wooden walls"}),wall_desc);

    add_exit(CAMP_ROOM_DIR + "wall1.c","northeast");
    add_exit(CAMP_ROOM_DIR + "wall3.c","northwest");
...
long_func()
{
    ...
    "the south side of this fort.  " +
    "Down below is the southernmost tent of the camp, " + 
    "while the middle of the camp is to the north.  ";
...
do_leap(string str)
{
	...
	TP->move_living("M",CAMP_ROOM_DIR + "fort5");
...
leap_access(object obs)
{
    ...
    if (!CAN_SEE_IN_OTHER_ROOM(TP,CAMP_ROOM_DIR+"fort5"))
    {
	return 0;
    }
    if (!CAN_SEE(TP,obs))
