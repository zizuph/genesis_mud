...
//  Modification log:
//  Gwyneth 4/1/01
//  Fixed a runtime bug when examining a room that wasn't loaded.
//
#pragma strict_types
...
create_gondor()
{
    set_short("on the catwalk above the camp entrance");
    set_long(long_func);
    add_item(({"clearing","camp","down","ground"}),"@@exa_camp|" +
      "Looking down you can see the entrance to this hidden camp.  |"+
      CAMP_ROOM_DIR+"fort1@@");
    add_item(({"wall","walls","wooden walls"}),wall_desc);

    add_exit(CAMP_ROOM_DIR + "wall2.c","northwest");
    add_exit(CAMP_ROOM_DIR + "wall4.c","southwest");
...
long_func()
{
    ...
    "the eastern end of this outpost.  " +
    "Directly below is the entrance to the camp, the only way " +
    "for anyone to enter or leave the fort.  ";
...
do_leap(string str)
{
	...
	TP->move_living("M",CAMP_ROOM_DIR + "fort1");
...
leap_access(object obs)
{
    ...
    if (!CAN_SEE_IN_OTHER_ROOM(TP,CAMP_ROOM_DIR+"fort1"))
    {
	return 0;
    }
    if (!CAN_SEE(TP,obs))
...
reset_room()
{
	...
	if (!objectp(Orc1))
	{
	    ...
	    Orc1->set_monster_home(CAMP_ROOM_DIR + "wall1");
	}
	if (!objectp(Orc2))
	{
