...
create_gondor()
{
    ...
    set_no_exit_msg(({"north", "northwest", "southeast", "south"}), 
    ...
    add_item(({"bank", "banks", "west bank", "east bank"}), "The west "
        + "bank of the river is just a few yards away, but the east bank "
    ...
    add_cmd_item(({"river", "anduin", "river anduin", "across", 
        ...
        "across the anduin", "across the river anduin", "Anduin", "east"}), 
        "swim", "@@swim_river");
    ...
    add_exit(LOR_DIR + "room/lp01", "west", 0, 3, 1);
}

string
long_desc()
{
    return "In the shallow water near the west bank of the river Anduin. "
        + sky_desc() + "the river as it flows swiftly south. The water "
        + "is fairly shallow here, and a short distance to the northwest a "
        + "dense forest of tall trees rises above the river" 
        + mallorn_desc() +".\n"; 
}
...
swim_river()
{
    if (TP->query_stat(SS_STR) > 40)
    {
        ...
        TP->move_living("M", AND_PATH + "ap48a", 100);
