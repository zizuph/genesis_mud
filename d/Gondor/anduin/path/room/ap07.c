...
create_gondor()
{
    set_short("A well-trodden path in a swamp");
    ...
    add_item(({"mouth", "mouths", "mouths of the entwash", "entwash",
        "Mouths", "Mouths of the Entwash", "Entwash"}), "The mouths of "
        + "the river Entwash empty from the west into the Anduin and "
        + "the surrounding swamplands.\n");
    add_exit(AND_PATH + "ap08", "northwest", 0, 5, 0);
    add_exit(AND_PATH + "ap06", "southeast", 0, 5, 0);
}
...
long_desc()
{
    return "On a well-trodden path across Nindalf near the river Anduin."
        ...
        + "as the eye can see, fed by the Mouths of the Entwash across "
