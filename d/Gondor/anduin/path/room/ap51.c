...
create_gondor()
{
    ...
    set_no_exit_msg(({"east", "southeast"}), 
        ...
        + "interest.\n");
    set_no_exit_msg(({"north", "southwest", "west", "northwest"}), 
        "The swift flow of the mighty Anduin blocks your progress in "
        + "that direction.\n");

    add_item(({"room", "area", "surroundings", "place"}), long_desc);

    add_exit(AND_PATH + "ap52", "northeast", 0, 3, 0);
    add_exit(AND_PATH + "ap50", "south", 0, 3, 0);
}
...
long_desc()
{
    return "On a well-trodden path across a field near the river "
        ...
        + "banks of the river, which widens out here. A few leagues "
        + "to the southwest, a forest of tall trees stands on "
        + "the far shore of the river" + mallorn_desc() +", the colour "
