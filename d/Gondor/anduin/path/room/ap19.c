...
create_gondor()
{
    ...
    set_no_exit_msg(({"northeast", "east", "southeast", "southwest",
        "west", "northwest"}), "The hill is too steep in that "
        + "direction.\n");

    add_item(({"room", "area", "surroundings", "place"}), long_desc);
    add_item(({"hill", "hills", "emyn muil", "Emyn Muil"}), "To the "
        + "northeast, the hills of the Emyn Muil rise above the land.\n");
    ...
    add_exit(AND_PATH + "ap20", "north", 0, 30, 0);
    add_exit(AND_PATH + "ap18", "south", 0, 15, 0);
}
