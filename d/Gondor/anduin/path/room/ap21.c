...
create_gondor()
{
    ...
    add_item(({"path", "narrow path", "steep path"}), "The path levels out "
        + "to the north and leads downhill to the south.\n");
    ...
    add_exit(AND_PATH + "ap22", "north", 0, 3, 0);
    add_exit(AND_PATH + "ap20", "south", 0, 15, 0);
...
long_desc()
{
    return "On a steep path in the southwest part of Emyn Muil." 
        ...
        + "which opens up a bit to the north.\n"; 
}
