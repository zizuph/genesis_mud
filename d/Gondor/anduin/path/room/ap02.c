...
create_gondor()
{
    ...
    add_item(({"forest", "forests", "ithilien", "Ithilien"}), "The "
        + "forests of Ithilien stand a few leagues to the southeast.\n");

    add_exit(AND_PATH + "ap03", "northwest", 0, 3, 0);
    add_exit(AND_PATH + "ap01", "southeast", 0, 3, 0);
}
...
long_desc()
{
    return "On a well-trodden path across a swamp near the river Anduin."
        ...
        + "banks of the river. Swamps extend to the north as far "
        + "as the eye can see. To the southwest, a long island rises from "
