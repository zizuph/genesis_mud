/* Name      : /d/Gondor/harondor/room/camp/room/s02.c
...
create_area_room()
{
    ...
    set_extraline("Across the dry and barren land dried grasses grow " +
        ...
        "mounds in different sizes. Looking north the " +
    ...
    add_exit(HAR_RM_CAMP_DIR + "trail10", "north",0,1,1);
    add_exit(HAR_RM_CAMP_DIR + "trail09", "northwest",0,4,1);
    add_exit(HAR_RM_CAMP_DIR + "s01", "northeast",0,2,1);
    add_exit(HAR_RM_CAMP_DIR + "s03", "east",0,2,1);
    add_exit(HAR_RM_CAMP_DIR + "s05", "southeast",0,2,1);
    add_exit("", "south","@@s_block",1,1);
    add_exit("", "west","@@s_block",1,1);
