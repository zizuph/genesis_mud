/* Name      : /d/Gondor/harondor/room/camp/room/campE01.c
 * Purpose   : The main campentrance to the camp of Haradrims
...
create_camp()
{
    ...
    set_extraline("Across the dry and barren land dried grasses grow " +
        ...
        "mounds in different sizes. East of here tents are raised in " +
        "neat lines, a few bigger then most. There is a small pavilion " +
        "errected just off the path offering some shadow from the " +
        "daytime blazing sun. The path leads west away from the pavilion " +
        "and east closer to the tents.");
    ...
    add_exit(HAR_RM_CAMP_DIR + "camp08", "southeast", check_allow, 2);
    add_exit(HAR_RM_CAMP_DIR + "camp01", "east", check_allow, 2);
    add_exit(HAR_RM_CAMP_DIR + "trail08", "west");
...
enter_pavilion()
{
    if (!check_allow())
    {
        ...
        TP->move_living("into the small pavilion", HAR_RM_CAMP_DIR + "spavilion01.c");
    }
    return "";
}
