/* Name      : /d/Gondor/harondor/room/camp/room/camp03.c
...
create_camp()
{
    ...
    set_extraline("Across the dry and barren land dried grasses grow " +
        ...
        "the area, a few bigger then most. South of you is one " +
        "particularly large with tent flaps open. Northwards there are " +
        "no tents but there is a small pavilion errected.");
    ...
    add_exit(HAR_RM_CAMP_DIR + "campE02", "north", check_allow, 2);
    add_exit(HAR_RM_CAMP_DIR + "camp04", "east", check_allow, 2);
    add_exit(HAR_RM_CAMP_DIR + "camp09", "south", check_allow, 2);
    add_exit(HAR_RM_CAMP_DIR + "camp02", "west", check_allow, 2);
    
    set_tent_path(HAR_RM_CAMP_DIR + "camptent03");
