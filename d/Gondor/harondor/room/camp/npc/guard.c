/* Name      : /d/Gondor/harondor/room/camp/npc/guard.c
 * Purpose   : The standard haradrim warrior for the camp.
...
create_monster()
{
   ...
   default_config_npc(160+random(10));    
   ...
   add_chat("I whish I had something to fight, I hate this waiting!");
   
   set_cchat_time(35+random(10));
   ...
   set_act_time(250+random(30));
   add_act("glare");
   add_act("hiss");
   add_act("grin");
   
   set_cact_time(15+random(10));
   add_cact("scream"); 
   add_cact("growl");
...
get_money()
{
   MONEY_MAKE_CC(20 + random(60))->move(TO);
   MONEY_MAKE_SC(     random(3))->move(TO);
...
arm_me()
{
   ...
   clone_object(HAR_RM_WEP_DIR + 
        one_of_list(({"seax", "seax", "framea", "hc_scimitar"})))->move(TO);
   ...
   if(random(100) > 40)
   {
      arm = clone_object(HAR_RM_ARM_DIR + "luna");
      arm->move(TO);
      command("wear shield");
   }
               ...
               // no crits with special and hit enemies only
                if (present(others[i], environment(this_object())))
                {
                    if (hurt >= others[i]->query_hp())
                        hurt = (others[i]->query_hp())/rate + 10 + random(25);
