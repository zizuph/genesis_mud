/* North Forest Room, Sarr */
/* 2017-06-28 Malus: Use new standard and adapt description to fit. */
#include "defs.h"
inherit NFOREST_ROOM;

void
create_room()
{
    string climb = "You scramble up the steep mountainside, " +
            "only to slide back down again.\n";
    forest_intro = "You are near the fringes of a dark forest.";
    forest_features = " Trees and darkness continue in all directions but " +
        "west, where the trees disperse into a barren plain."; 
    ::create_room();
    add_item(({"plain","flats"}),
            "The flats extend north into the mountains. They look lifeless.\n");
    forest_details = " To the north a steep mountainside rises high " +
        "above you.";
    ::create_room();
    add_item(({"mountain","mountains","mountainside"}),
            "Mountains rise from the forest, blocking passage " +
            "north.\n");
    add_cmd_item(({"mountain","mountains","mountainside"}), "climb", climb);
    set_no_exit_msg(({"north","northwest"}), climb);
    add_exit(FOREST_DIR + "nforest47","northeast",0,FOREST_FATIGUE);
    add_exit(FOREST_DIR + "nforest38","east",0,FOREST_FATIGUE);
    add_exit(FOREST_DIR + "nforest27","southeast",0,FOREST_FATIGUE);
    add_exit(FOREST_DIR + "nforest26","south",0,FOREST_FATIGUE);
    add_exit(FOREST_DIR + "nforest25","southwest",0,FOREST_FATIGUE);
    add_exit(NFLATS_DIR + "nflats23","west",0,FOREST_FATIGUE);
}

