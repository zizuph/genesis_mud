...
create_room()
{
    ...
    set_no_exit_msg(({"southwest","south","southeast","east","northeast"}), 
            climb);
    add_exit(FOREST_DIR + "eforest19","north",0,FOREST_FATIGUE);
    add_exit(FOREST_DIR + "eforest10","west",0,FOREST_FATIGUE);
    add_exit(FOREST_DIR + "eforest18","northwest",0,FOREST_FATIGUE);
}
