 ...
 *
 *  Polearm, deals additional magical cold damage, and undull.
 *
 *  Nerull 2021
 *
 */
...
create_weapon()
{
    set_name("_tomb_scythe");
    add_name( ({"scythe"}) );

    set_adj("cold");
    add_adj("bone");

    set_short("adamantine bone scythe"); 
    set_pshort("adamantine bone scythes");

    set_long("Pulsing with necromantic powers this scythe "
    +"fulfills " + wname + "'s vile prayers. " + wname + "'s femur "
    +"knee and "
    +"tibia have been mummified and cast in adamantine. So "
    +"as to be easy to "
    ...
    set_default_weapon(48, 48, 
    W_POLEARM, W_SLASH | W_IMPALE, W_LEFT, this_object());
    ...
    add_prop(MAGIC_AM_ID_INFO, ({ "This weapon has been enhanced by "
        ...
        + "by the weapon, and it may never dull.\n", 50 }));

    add_prop(OBJ_S_WIZINFO, "This is an epic weapon dropping from the chest "
        + "of Damon Skagg, captain of the Bountiful. It procs magical "
        + "cold damage.\n");

    add_prop(OBJ_I_WEIGHT, F_WEIGHT_DEFAULT_WEAPON(48, W_POLEARM));
...
wield(object what)
{
    ...
    if (this_player()->query_skill(SS_WEP_POLEARM) < 85)
    {
