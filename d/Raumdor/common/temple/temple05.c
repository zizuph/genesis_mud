...
create_room()
{
    ...
    set_long(
      "You stand to the west side of the great statue. The room smells " +
    ...
    add_item(({"torches"}), "They light the chamber.\n");
    add_item(({"stairs"}), "They are to the south.\n");
    add_exit(TEMPLE_DIR + "temple10", "northeast", 0);
    add_exit(TEMPLE_DIR + "temple06", "east", 0);
    add_exit(TEMPLE_DIR + "temple03", "southeast", 0);
    add_exit(TEMPLE_DIR + "temple02", "south", 0);
    clone_object(TEMPLE_OBJ_DIR + "western_door_outer")->move(TO, 1);
