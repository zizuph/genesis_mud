...
create_room()
{
    ...
    set_long("Looking in all directions you quickly come "
    +"to the realization that you "
    +"are closing in on the middle of the camp. Crusaders "
    +"in various states of "
    +"inebriation are clearly preparing for something, "
    +"or you reckon, attempt to "
    +"ignore their fears for whatever lurks behind the "
    +"drawbridge separating the "
    +"barely visible castle to the east from the path "
    +"leading to it. The ground "
    +"is littered with bottles, rags, food scraps and even a few dead rats. "
    +"The tents are set up close to one another, and as a "
    +"natural consequence the air is "
    +"both stale and full of bodily odours. From "
    +"here you can move north towards "
    +"the edge of the camp, south towards the very centre or continue in the "
    +"east-west direction towards what appears to be more of the same. \n");
    ...
    add_exit(MTRAIL_DIR + "siegecamp_20", "west");
    add_exit(MTRAIL_DIR + "siegecamp_24", "north");
    add_exit(MTRAIL_DIR + "siegecamp_18", "east");
    add_exit(MTRAIL_DIR + "siegecamp_13", "south");
