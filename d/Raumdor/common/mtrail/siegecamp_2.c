...
create_room()
{
    ...
    set_long("You find yourself at the very edge of the "
    +"camp. Deep grooves in the mud make "
    +"out something resembling a road going in the "
    +"east-west direction. There is not much "
    +"to see here, but your nostrils tell you that further "
    +"progress westwards will lead "
    +"to a more populated area. You could always find your "
    +"way back to the mountain path "
    +"to the east, but looking south you notice a path leading south "
    +"towards the very edge of the camp.\n");
    ...
    add_exit(MTRAIL_DIR + "siegecamp_10", "west");
    add_exit(MTRAIL_DIR + "siegecamp_1", "east");
    add_exit(MTRAIL_DIR + "siegecamp_3", "south");
