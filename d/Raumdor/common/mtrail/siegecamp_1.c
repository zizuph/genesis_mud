...
create_room()
{
    ...
    set_long("Deviating from the mountain path and moving "
    +"away from the eerie castle, "
    +"you now find yourself outside a camp. Large grooves "
    +"make out a road "
    +"of sorts, formed by what can only be a siege "
    +"weapon of considerable size. "
    +"There are no banners or otherwise demoralizing "
    +"artifacts here designed "
    +"to scare off potential intruders. Perhaps they've "
    +"concluded that such a gesture "
    +"might serve to soothe the enemy rather than inducing "
    +"dread. You can always "
    +"return to the mountain path to the southeast, or follow "
    +"the grooves into "
    +"the camp due west.\n");
    ...
    add_exit(MTRAIL_DIR + "siegecamp_2", "west");
    add_exit(MTRAIL_DIR + "mtrail_path4", "southeast");
