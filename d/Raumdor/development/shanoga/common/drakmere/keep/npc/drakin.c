...
void
convert_amulet_steps(object ob, object from)
{
    object new_amulet;
...
convert_amulet(object ob, object from)
{
    ...
    {
        conversion_alarm = set_alarm(0.0, 1.0, &convert_amulet_steps(ob, from));
    }
    else
    {
        ...
        set_alarm(2.5, 0.0, &command("give phylactery to " +
          from->query_name()));
        set_alarm(2.5, 0.0, remove_prop(LIVE_M_NO_ACCEPT_GIVE));
    }
}
    ...
    if (from)
    {
        if (MASTER_OB(ob) == COMMON_ARM_DIR + "lich_phylactery")
        {
            add_prop(LIVE_M_NO_ACCEPT_GIVE,
              " is busy speaking with someone else. Please wait " +
