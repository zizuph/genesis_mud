...
// We'll have to update these and probably put them in defs.h, but for now...
#undef  DOMAIN_DIR
#undef  COMMON_DIR
#define DOMAIN_DIR      "/d/Raumdor/development/shanoga/"
#define COMMON_DIR      DOMAIN_DIR + "common/"

inherit "/d/Raumdor/std/undead.c";
inherit "/lib/unique";
inherit "/d/Raumdor/lib/paralyze.c";
inherit "/d/Raumdor/lib/reanimate.c";
inherit "/d/Raumdor/lib/follow.c";

#define SPELLS          COMMON_DIR + "npc/spells/"
#define SPELL_SOUL      COMMON_DIR + "npc/soul/lich_spell_soul"
...
{
    ::create_monster();
    set_living_name("chogaros");
    set_race_name("lich");
    set_gender(G_NEUTER);
    set_adj(({"looming", adj}));
    set_short("looming " + adj + " lich");
    update_long();
    set_title("the Decimated, Lord of the Forest");
...
*              : do no damage and provide no experience.
* Returns      : int value - 0 if real, 1 if llusion
*/
int
query_is_illusion()
{
...
* Returns      : int value - number of illusions still "alive"
*/
int
    ...
    if (query_attack() != new_target)
    {
        ...
        else
        {
            new_target->catch_msg(QCTNAME(TO) + " senses weakness in you and " +
              "shifts the focus of its deadly energy to you.\n");
            TO->tell_watcher(QCTNAME(TO) + " senses weakness in " +
              QTNAME(new_target) + " and shifts the focus of its deadly " +
              "energy to " + new_target->query_objective() + ".\n", new_target,
              ({ TO, new_target }));
...
create_copies()
{
    ...
    {
        copies[copy] = clone_object(COMMON_DIR + "npc/lich.c");
...
command_reanimation(object reanimation)
{
    ...
    reanimation->set_hp(reanimation->query_max_hp());
 ...
 *                follow too quickly. The following int is checked to delay
 *                the lich's movement for each follow after the first.
...
notify_enemy_leaves(object enemy)
{
    if (following)
    {
        set_alarm(1.5, 0.0, &follow_target(query_weakest_link(),1.5));
    }
    else
    {
        following = set_alarm(0.0, 0.0,
                              &follow_target(query_weakest_link(),1.5));
    }
}
...
do_die(object killer)
{
    ...
    if (removed)
    {
        TELL_CAN_SEE(TO, "The " + LANG_NUM2WORD(removed) +
          " remaining " + (removed == 1 ? "lich" : "liches") + " suddenly " +
