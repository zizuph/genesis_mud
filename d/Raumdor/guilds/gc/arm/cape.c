/*********************************************************************
 * - cape.c                                                        - *
 * - A random coloured cape for the shops in the Gentleman's club  - *
...
set_up_cape()
{
    set_short("satin lined " + colour + " cape");
    add_name(({""+ colour + " cape", "satin cape"}));
    set_adj(colour);
    set_long("This is a " + short() + ". This cape is long "+
...
create_armour()
{
    ...
    add_item(({"lining"}),
      "This cape has satin lining that smoothly caresses the body and "+
