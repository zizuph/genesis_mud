/* /d/Raumdor/guilds/the_order/shadows/theorder_sh_lay_cleric.c
 *
 * 
 *
 */
...
static string *o_titleslay_c = ({ LAYCLER1,
                                  LAYCLER2,
                                  LAYCLER3,           
                                  LAYCLER4,
                                  LAYCLER5,
...
add_soul()
{
    ...
    + "souls/theorder_guild_soul_lay_cleric");
...
{
    return MIN(THEORDER_MAX_RANK_LAY_MEMBER_CLERIC, 
...
query_guild_title_lay()
{
	...
	return o_titleslay_c[query_guild_level_lay()];
}
...
{
    return THEORDER_GUILD_NAME_LAY_CLERIC;
}
...
init_lay_shadow(string str)
{
    ...
    if (THEORDER_MASTER->is_lay_theorder_cleric(who->query_name()))
    {
        if (!present("theorder_guild_object_lay_cleric", who))
        {
            clone_object(THEORDER_LAY_ITEM_CLERIC)->move(who, 1);
        }         
    }
}
...
public void remove_shadow()
{
    ...
    while (emblem = present("theorder_guild_object_lay_cleric", who))
    {
