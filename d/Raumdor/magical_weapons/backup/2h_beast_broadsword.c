...
create_weapon()
{
    ...
    set_long("This two-handed sword has a long grip clad in green ropy "
        ...
        +"made of a rust-red metal. On either of it's side you see an " 
        +"engraving of a skull and a scythe, stretching all along the "
        +"lenght of the blade.\n");
        
    set_hit(TWOH_BEAST_BROADSWORD_HITPEN_DEFAULT);
    set_pen(TWOH_BEAST_BROADSWORD_HITPEN_DEFAULT);

    set_hands(W_BOTH);
    set_wf(this_object());
    
    add_item(({"engraving"}), "Engraved into the surface of the blade on "
        +"both sides, you see a long-stretched decoration of an decayed "
    ...
    "the sword and help dealing additional melee damage to the " +
    "enemy.\n", 50}));
    ...
    add_prop(OBJ_S_WIZINFO, "This two-handed weapon has a hit/pen of " 
        ...
        +"(TWOH_BEAST_BROADSWORD_PROC) and "
        +"(TWOH_BEAST_BROADSWORD_SPECIAL_MULTIPLIER).\n");
...
int dam)
{
    ...
    if(random(100) < TWOH_BEAST_BROADSWORD_PROC)
    {
        ...
        wielder->tell_watcher("As "+QCTNAME(wielder)+" strikes "
            ...
            +"edge of it's blade, forcing it to penetrate deeply into "
            +enemy->query_possessive()+" "+hdesc+", sending sinewy flesh "
            +"and blood flying everywhere.\n",enemy);
 
        // The damage formula.
        mindam = TWOH_BEAST_BROADSWORD_SPECIAL_MULTIPLIER * 
