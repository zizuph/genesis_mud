 ...
 *
 *  Recoded cosmetically for Thwil herb shop on Roke
 *  Ckrik 9/2003
 *  Fixed by Ckrik June 2014 to reflect herbs becoming heaps.
 */
#pragma strict_types
...
#define HERBALIST THWIL+"monster/herbalist"
#define HERBJAR HERB_OBJ+"herb_jar.c"
#define HERBPOUCH HERB_OBJ+"herb_pouch.c"
#define HERB_SAVE_FILE "/d/Earthsea/roke/thwil/herb_save"
...
create_earthsea_room()
{
	...
	set_max_herb_quantity(2000);
	set_id_level(85);
	init_herb_store();
	set_short("Herb shop");
	set_long("As you enter the husky herb shop a veil "+
			"of spicy, exotic fragrances hits you. " +
			"You inhale deeply, feeling refreshed in "+
			"mind and body. Most of the herbs in this shop "+
			"have been collected by the apprentices of the Wizard's " +
			"School, under the strict surveillance of "+
			"Master Herbal. There is a sign here leaning "+
			"against the wall. \n");

	add_item("wall", "Nothing spectacular about it.\n");
	add_exit(THWIL + "t6", "west");
	reset_room();
	INSIDE;
}
...
list_items(string str)
{
	...
	if (!objectp(herbalist))
	{
		write("The herbalist is not here to do business with you.\n");
		return 1;
	}
	if (str == "equipment" || str == "equip" ||
			str == "herbs" || !str)
	{
		result += "                           Wizard's Herb Shop\n\n";
		result += "                               @}-}--\n";
		result += "---------------------------------------------------------------------------\n";
		if (!str || str == "herbs")
		{
...
buy_equip(string str)
{
	...
	{
		stuff->move(TO);
		tell_room(TO, QCTNAME(herbalist) + " sets "+
...
shop_hook_list_empty_store(string str)
{
	string result = "";
	result += "          ";
	result += "     ***** The store is currently out of herbs ***** \n";
	return result;
}
...
do_buy(string str)
{
	...
	if (!objectp(herbalist))
	{
		notify_fail("The herbalist is not here to do business with you.\n");
		return 1;
	}
	...
	for (i = 0; i < num; i++)
	{
		if (herbs[i]->query_herb_name() == "ginseng")
		{
			herbs[i]->add_prop(GINSENG_BOUGHT_HERE, 1);
		}
		if (herbs[i]->move(tp))
		{
			flag = 1;
...
show_return(string str)
{
	...
	if (str == "herbs" || str == "equipment" || str == "equip")
		return 0;
...
do_show(string str)
{
	...
	if (!objectp(herbalist))
	{
		notify_fail("The herbalist is not here to examine your herb.\n");
		return 1;
	}
	...
	if (sizeof(item) > 1)
	{
		herbalist->command("say Please show me only one thing at a time.");
		return 1;
	}
	tell_room(tp, QCTNAME(tp) + " shows a " + item[0]->query_name() +
			" to " + QCTNAME(herbalist) + ".\n");
	if (!filter_herb(item[0]))
	{
		herbalist->command("chuckle");
		herbalist->command("say That is not an herb.");
		return 1;
	}
	if (item[0]->query_id_diff() > id_level)
	{
		herbalist->command("hmm");
		herbalist->command("say It is called a " + item[0]->query_herb_name() + ", but I am afraid I do not know anything about this herb.");
		return 1;
	}
	herbalist->command("brighten");
	set_alarm(2.0, 0.0, &herbalist->command("say Oh! That is a " +
			item[0]->query_herb_name() + ". " +
