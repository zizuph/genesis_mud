...
/* Code for repairing weapons and armour for the blacksmith Bubba in Gelan.
 * Code included into bubba.c
 *
...
calc_wax_value(object obj)
{
   ...
   if (!F_LEGAL_WEAPON_REPAIR_CORR(obj->query_repair_corr()+1,
                                  obj->query_corroded()))
      return -1;
   return ((obj->query_prop(OBJ_I_VALUE)+1) * WAX_COST) / 100;
}
...
calc_sharpen_value(object obj)
{
   ...
   if (!F_LEGAL_WEAPON_REPAIR_DULL(obj->query_repair_dull()+1,
                                  obj->query_dull()))
      return -1;
   return ((obj->query_prop(OBJ_I_VALUE)+1) * SHARPEN_COST) / 100;
}
...
do_a_cost(string str)
{
  ...
  command("say This "+armour[0]->short()+" is damaged. I can fix it " +
          "for...hmm..");
  command("say ...about "+price+" silver I guess.");
  return 1;
}
...
{
   object *armour, tp;
   int int_str, price, *money_arr, cond, rep;
   string item_name, pay_type, get_type, coin_pay_text, coin_get_text,
          verb;
  ...
  if (rep && !(F_LEGAL_ARMOUR_REPAIR(rep+1,cond))) {
   ...
   /* Try to take the money */
   if (sizeof(money_arr = pay(price*12, tp, pay_type, 0, 0, get_type)) == 1)
   {
      if (money_arr[0] == 1)
      {
