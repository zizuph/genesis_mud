inherit "/std/object";

#include <stdproperties.h>

query_schludderlist()
{
	return ([
	"s":"sch",
	"t":"th",
	"g":"gh",
	"'":"..'",
	"c":"ch",
	"f":"phf",
	"z":"sz",
	"n":"ng",
	"y":"i",
	"I":"Ah",
	"1":"3",
	"2":"1",
	"3":"8",
	"4":"5",
	"5":"9",
	"6":"2",
	"7":"6",
	"8":"4",
	"9":"0",
	"0":"7",
	]);
}

create_object()
{
	set_name("fylla");
	set_adj("billig");
	set_long("Detta ar en billig fylla.\n");

	add_prop(OBJ_I_INVIS,100);
	add_prop(OBJ_M_NO_DROP,1);
}
init()
{
	::init();

	add_action("say","say");
}
enter_env(object a,object b)
{
  ::enter_env(a,b);

    tell_room(environment(this_object()),"You feel a little drunk.\n");
    this_player()->add_subloc("sketfull", this_object());
}
public void
leave_env(object from, object to)
{
    this_player()->remove_subloc("sketfull");
    this_player()->catch_msg("You feel less drunk.\n");
}
public string 
show_subloc(string subloc, object on, object for_obj)
{
    string data;

    if (for_obj == on)
        data = "You look ";
    else
        data = capitalize(environment(this_object())->query_pronoun()) +
	  " looks ";

    data += "a little drunk.\n";

    return data;
}

say(string s)
{	
	string *str,ss;
	mapping mapp;
	int i,a,b;

	if(!s)	
	return 0;

	if((a = this_player()->query_intoxicated())>(this_player()->query_stat(2)/3))
	  {
	    mapp = query_schludderlist();
	    b  = this_player()->query_stat(2);

	    str = explode(s,"");

	    for(i=0;i<sizeof(str);i++)
	      {
		if(random(a)>random(b))
		  {
		    ss = mapp[str[i]];
		    if(ss)
		      str[i] = ss;
		  }
	      }
	    
	    this_player()->communicate(implode(str,""));
	    
	    return 1;
	  }
	else
	  {
	    call_out("remove_object",1);
	    return 0;
	  }
      }

/*apa*/




