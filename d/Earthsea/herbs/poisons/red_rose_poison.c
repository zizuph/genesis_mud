...
tell_damage_player(object ob)
{
    ...
    {
    case 0:
	ob->command("$smile stupidly");
	ob->catch_msg("The rose's perfume uplifts your soul.\n");
	break;
	...
	ob->command("$giggle uncontrollably");
	break;

    case 3:
	ob->command("$whistle drunkenly");
	ob->command("hug all");
	break;
...
enter_env(object env, object from)
{
	...
	env->catch_msg("You feel gay now!\n");
	return 1;
    }
    return 1;
}
