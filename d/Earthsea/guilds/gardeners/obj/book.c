...
my_book_adj()
{
  return GARDENER_BOOKS[environment()->query_gardener_subrank()];
}

public string
my_page_adj()
{
  return GARDENER_PAGES[environment()->query_gardener_rank()];
}
...
create_book()
{
   ...
   add_name("journal");
   add_name("herb journal");
   add_name("book");
   add_adj(PAGE_ADJ);
   add_adj(BOOK_ADJ);
   set_short(PAGE_ADJ + " " + BOOK_ADJ + " leather book");
