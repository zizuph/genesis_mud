...
inherit "/d/Earthsea/lib/guild_room2";
...
create_earthsea_room()
{
    ...
    add_config_guild_room(GARDENER_NAME, GARDENER_SOUL, GARDENER_SHADOW_LAY,
        GARDENER_STYLE, "layman", "init_new_gardener",
        "remove_gardeners_guild", ({ "join" }), ({ "leave" }),
        "layman", "layman");
    add_config_guild_room(GARDENER_NAME, GARDENER_SOUL, GARDENER_SHADOW_CFT,
        GARDENER_STYLE, "craft", "init_new_gardener",
        "remove_gardeners_guild", ({ "join" }), ({ "leave" }),
        "craft", "craft");
    sk_add_train(SS_HERBALISM, "find herbs", "herbalism", 0, 60);
    sk_add_train(SS_AWARENESS, "detect hidden details", "awareness", 0, 50);
    sk_add_train(SS_LOC_SENSE, ({ "sense details about the environment", "sense details about the environment" }),
        "location sense", 0, 50);
