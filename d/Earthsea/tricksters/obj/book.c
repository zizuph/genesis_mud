...
void create_object()
{
 ...
 add_adj("dusty");
 set_long("@@book_long");
 add_prop(OBJ_M_NO_GET,"It's chained to the altar.\n");
}
...
string book_long()
{
 ...
 res="This is ancient and very dusty book. If you want to read it "+
   "you may open it and turn page backward or forward or to certain "+
   "page. It is chained to altar.\n";
 if(open) res+="The book is open at page "+NUM_TO_STR(page)+".\n";
 else res+="The book is closed.\n";
...
int read(string what)
{
 ...
 if(!open)
  {
   write("You try to read a closed book.\n");
   say(QCTNAME(TP)+" looks kinda silly as "+PRO(TP)+" tries to read "+
     "a closed book.\n");
   return 1;
  }

 write("\n"+read_file(BOOK+sprintf("page%d.txt",page))+"\n");
