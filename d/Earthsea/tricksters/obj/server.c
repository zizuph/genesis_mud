...
void post_news()
{
 ...
 news=({ });
 for(i=0;i<sizeof(members);i++) news+=({ members[i] });
 save_object(SERVERO);
}
...
void notify_news(object pl)
{
 if(member_array(pl->query_real_name(),news)==-1) return;

 pl->catch_msg("\n\nWHEEE! NEWS! \nPlease check the 'news' topic "+
   "of tricksters help to see the important updates!\n\n");
}

void remove_notify(object pl)
{
 news-=({ pl->query_real_name() });
 save_object(SERVERO);
}
