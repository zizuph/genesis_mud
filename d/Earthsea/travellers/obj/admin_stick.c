...
#define CHECK_ACCESS if(this_interactive() != this_player()) return 0; \
   if(member_array(this_interactive()->query_real_name(), MASTERS) == -1) \
      return 0
#define MORTAL_MEMBERS (filter(filter(users(), &not() @ &->query_wiz_rank()), \
...
private static void
reshadow(object player)
{
    object shadow;

    player->remove_race_shadow();

    shadow = clone_object(TRAVELLERS_SHADOW);
    shadow->add_race_shadow(player);
}
...
trshadow(string str)
{
   ...
   if(str != "reload") {
      return notify_fail("Trshadow reload?\n");
   }
