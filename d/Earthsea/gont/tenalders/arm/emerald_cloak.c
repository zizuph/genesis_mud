...
create_armour()
{
    set_name("cloak");
    set_pname("cloaks");
    set_adj(({"emerald-green", "green", "velvet" }));
    set_short("emerald-green velvet cloak");
    set_pshort("emerald-green velvet cloaks");
    set_long("A long flowing emerald-green velvet cloak. "+
    ...
    set_default_armour(20, A_ROBE, ({-1, -1, -1}));
...
swirl_this(string str)
{
    ...
    if(to->query_worn() == tp)
    {
        write("You swirl the emerald-green velvet cloak around "+
            "yourself with a dramatic flourish.\n");
        tell_room(environment(tp), QCTNAME(tp) + " swirls the "+
            "ankle-length emerald-green velvet cloak around "+
