...
{
    set_name("cloak");
    set_adj(({"midnight-black", "black", "velvet"}));
    set_short("midnight-black velvet cloak");
    set_long("A long flowing black velvet cloak. "+
        "It is dark as midnight and seems to emanate shadows. "+
...
swirl_this(string str)
{
    ...
    if(to->query_worn() == tp)
    {
        write("You swirl the midnight-black velvet cloak "+
            "around yourself with a dramatic flourish.\n");
        tell_room(environment(tp), QCTNAME(tp) + " swirls the "+
            "long midnight-black velvet cloak around "+
