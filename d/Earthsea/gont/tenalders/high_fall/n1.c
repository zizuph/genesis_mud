...
create_forest_room()
{
    ...
    set_long("Tall trees of the ancient evergreen forest " +
        "stand north of you ascending the slope and scenting " +
        "the crisp mountain air with fresh pine. The " +
        "forest floor is covered with dense " +
        "tangled briars and bushes. A rugged path, carpeted with " +
        "pine needles, ascends the mountain, winding among the " +
        "vegetation and rocks. To the southeast there " +
        "is a clearing where you see a garden.\n");
    ...
    set_up_herbs(select_herb_files(FOREST_HERBS),
        FOREST_LOCATIONS, 3);
    reset_room();
    add_exit("n2", "northeast", 0, 1, 0);
    add_exit("../village/witch_garden_7", "southeast");
}
