...
create_herb_room()
{
    ...
    set_long("The meadow ends here at the forest edge. " +
        "The tall limestone cliff towers above, and directly to the " +
        "east past some bushes you see where there is a steep " +
        "dropoff, blocking progress in that direction. To the " +
        "southeast the forest covers the steep mountainside. There " +
        "is a very large redwood tree that stands above the rest.\n");
    ...
    add_item((({"top", "mountain top"})),
        ...
        "from here.\n");
    add_item((({"flowers", "grass" })),
        "Among the flowers and green grasses you see purple " +
        "clover flowers and bees buzzing around them.\n");
    ...
    add_item((({"tree", "redwood tree"})),
        "A huge redwood tree stands in the forest just " +
        "below.\n");
    reset_room();
    set_tell_time(110);
...
reset_room()
{
    ...
    while (i--)
    {
        if(!objectp(goats[i]))
        {
            ...
            goats[i]->command("emote leaps in.");
        }
    }
}
