...
create_forest_room()
{
    ...
    set_long("A dark trail. A hedge of dark hemlock " +
        "bushes looms just to the south. Tree branches arch overhead, " +
        "blocking the light. A tangle of briars and thick undergrowth " +
        "make walking here difficult. " + "@@see_path");
    add_item((({"hemlock bushes", "hedge", "bushes"})),
        "You notice an exit out to the forest path through the hedge.\n");
    ...
    set_up_herbs(select_herb_files(FOREST_HERBS),
        FOREST_LOCATIONS, 3);
    reset_room();
    add_exit("trail2", "north", "@@test_loc", 0, 1);
    add_exit("ne2", "out");
}
