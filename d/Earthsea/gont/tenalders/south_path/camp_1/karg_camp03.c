...
create_camp()
{
    ...
    add_item(({"path"}), 
        "It leads back to the path to Ten Alders.\n");
    add_exit("karg_camp02", "north");
    add_exit("karg_camp01", "east");
    add_exit("karg_camp04", "south");
    add_exit("karg_camp06", "west");
    reset_room();
}
...
tent(string str)
{
    if(str == "tent")
    {
        this_player()->move_living("entering the tent",
            "/d/Earthsea/gont/tenalders/south_path/camp_1/tent03");
        return 1;
    }
