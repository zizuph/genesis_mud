...
/* For random pipe exits */
#include "/d/Earthsea/gont/sewers/lib/pipe_room_set.h"
...
create_sewer()
{
    ...
    try_item( ({ "large pipes", "large pipe", "big pipes", "big pipe", 
                 "larger pipe", "larger pipes" }),
             "Large pipes leave this area of tunnel in sloped, but general "
           + "horizontal angles. They are large enough for you to crawl "
           + "into, but you assess that you wouldn't be able to turn around "
           + "inside the pipe. You would have to travel to the end of the "
           + "pipe... Wherever that may be.\n");
    ...
    add_cmd_item(({"pipe", "through the pipe", "through pipe", "thru pipe",  
        "thru the pipe", "in the pipe", "in pipe", "into pipe" }), 
        "crawl", "@@crawl");
...
/*
 * This allows for players to crawl through tributary pipes and reappear
 * in random places.
 */
public int
crawl()
{
    write("You crawl into the dark pipe and emerge elsewhere in the "
           + "sewers.\n");
    this_player()->move_living("crawling through a dark pipe",
        "/d/Earthsea/gont/sewers/room/sewer_spoke/high_e_1" );
    return 1;
}
